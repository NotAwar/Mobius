...
DEFAULT_PKGS_TO_TEST := ./cmd/... ./tools/... ./orbit/pkg/... ./orbit/cmd/orbit ./pkg/... ./server/... ./tools/...
...
mobiuscli-docker: xp-mobiuscli
	docker build -t ghcr.io/notawar/mobiuscli --platform=linux/amd64 -f tools/mobiuscli-docker/Dockerfile .

bomutils-docker:
	cd tools/bomutils-docker && docker build -t ghcr.io/notawar/bomutils --platform=linux/amd64 -f Dockerfile .

wix-docker:
	cd tools/wix-docker && docker build -t ghcr.io/notawar/wix --platform=linux/amd64 -f Dockerfile .
...
vex-report:
	...
	sh -c 'echo "## \`ghcr.io/notawar/mobius\` container image\n" >> security/status.md'
	sh -c 'go run ./tools/vex-parser ./security/vex/mobius >> security/status.md'
	sh -c 'echo "## \`ghcr.io/notawar/mobiuscli\` container image\n" >> security/status.md'
