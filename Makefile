...
.pre-mobius:
	$(eval APP_NAME = mobius)

.pre-mobiuscli:
	$(eval APP_NAME = mobiuscli)

# For the build target, decide which binaries to build.
# Default to building both
BINS_TO_BUILD = mobius mobiuscli
ifeq (build,$(filter build,$(MAKECMDGOALS)))
	BINS_TO_BUILD = mobius mobiuscli
	ifeq ($(ARG1), mobius)
		BINS_TO_BUILD = mobius
	else ifeq ($(ARG1), mobiuscli)
		BINS_TO_BUILD = mobiuscli
	endif
endif
.help-short--build:
	@echo "Build binaries"
.help-long--build:
	@echo "Builds the specified binaries (defaults to building mobius and mobiuscli)"
.help-usage--build:
	@echo "$(TOOL_CMD) build [binaries] [options]"
.help-options--build:
	@echo "GO_BUILD_RACE_ENABLED"
	@echo "Turn on data race detection when building"
	@echo "EXTRA_MOBIUSCTL_LDFLAGS=\"--flag1 --flag2...\""
	@echo "Flags to provide to the Go linker when building mobiuscli"
.help-extra--build:
	@echo "AVAILABLE BINARIES:"
	@echo "  mobius      Build the mobius binary"
	@echo "  mobiuscli   Build the mobiuscli binary"
build: $(BINS_TO_BUILD)
...
.help-short--serve:
	@echo "Start the mobius server"
.help-short--up:
	@echo "Start the mobius server (alias for \`serve\`)"
.help-long--serve: SERVE_CMD:=serve
.help-long--up: SERVE_CMD:=up
.help-long--serve .help-long--up:
	@echo "Starts an instance of the Mobius web and API server."
	@echo
...
.help-options--serve .help-options--up:
	@echo "HELP"
	@echo "Show all options for the mobius serve command"
	@echo "USE_IP"
	@echo "Start the server on the IP address of the host machine"
	@echo "NO_BUILD"
	@echo "Don't build the mobius binary before starting the server"
	@echo "NO_SAVE"
...
ifdef SHOW
serve:
	@SAVED_ARGS=$$(cat ~/.mobius/last-serve-invocation); \
	if [[ $$? -eq 0 ]]; then \
		echo "$$SAVED_ARGS"; \
	fi
else ifdef HELP
serve:
	@./build/mobius serve --help
else ifdef RESET
serve:
	@touch ~/.mobius/last-serve-invocation && rm ~/.mobius/last-serve-invocation
else
serve:
	@if [[ "$(NO_BUILD)" != "true" ]]; then make mobius; fi
	$(call filter_args)
...
# IF FORWARDED_ARGS is empty, attempt to repeat the last invocation.
	@if [[ "$(FORWARDED_ARGS)" != "" ]]; then \
		if [[ "$(NO_SAVE)" != "true" ]]; then \
			echo "./build/mobius serve $(FORWARDED_ARGS)" > ~/.mobius/last-serve-invocation; \
		fi; \
		./build/mobius serve $(FORWARDED_ARGS); \
	else \
		if ! [[ -f ~/.mobius/last-serve-invocation ]]; then \
			echo "./build/mobius serve --server_address=localhost:8080 --dev --dev_license" > ~/.mobius/last-serve-invocation; \
		fi; \
		cat ~/.mobius/last-serve-invocation; \
		$$(cat ~/.mobius/last-serve-invocation); \
	fi
endif
...
mobiuscli: .prefix .pre-build .pre-mobiuscli
	# Race requires cgo
	$(eval CGO_ENABLED := $(shell [[ "${GO_BUILD_RACE_ENABLED_VAR}" = "true" ]] && echo 1 || echo 0))
	$(eval MOBIUSCTL_LDFLAGS := $(shell echo "${LDFLAGS_VERSION} ${EXTRA_MOBIUSCTL_LDFLAGS}"))
	CGO_ENABLED=${CGO_ENABLED} go build -race=${GO_BUILD_RACE_ENABLED_VAR} -o build/mobiuscli -ldflags="${MOBIUSCTL_LDFLAGS}" ./cmd/mobiuscli

mobiuscli-dev: GO_BUILD_RACE_ENABLED_VAR=true
mobiuscli-dev: mobiuscli
...
FAST_PKGS_TO_TEST := \
	...
	./server/policies
MOBIUSCTL_PKGS_TO_TEST := ./cmd/mobiuscli/...
...
mobiuscli-docker: xp-mobiuscli
	docker build -t mobiusmdm/mobiuscli --platform=linux/amd64 -f tools/mobiuscli-docker/Dockerfile .

bomutils-docker:
	cd tools/bomutils-docker && docker build -t mobiusmdm/bomutils --platform=linux/amd64 -f Dockerfile .

wix-docker:
	cd tools/wix-docker && docker build -t mobiusmdm/wix --platform=linux/amd64 -f Dockerfile .
...
binary-bundle: xp-mobius xp-mobiuscli
	...
	cd build/binary-bundle && mkdir mobiuscli-macos && cp darwin/mobiuscli mobiuscli-macos && tar -czf mobiuscli-macos.tar.gz mobiuscli-macos
	cd build/binary-bundle && mkdir mobiuscli-linux && cp linux/mobiuscli mobiuscli-linux && tar -czf mobiuscli-linux.tar.gz mobiuscli-linux
	cd build/binary-bundle && mkdir mobiuscli-windows && cp windows/mobiuscli.exe mobiuscli-windows && tar -czf mobiuscli-windows.tar.gz mobiuscli-windows
	cd build/binary-bundle && cp windows/mobiuscli.exe . && zip mobiuscli.exe.zip mobiuscli.exe
	cd build/binary-bundle && shasum -a 256 mobius.zip mobiuscli.exe.zip mobiuscli-macos.tar.gz mobiuscli-windows.tar.gz mobiuscli-linux.tar.gz

# Build orbit/mobiusdaemon mobiusdaemon_tables extension
mobiusdaemon-tables-windows:
	GOOS=windows GOARCH=amd64 go build -o mobiusdaemon_tables_windows.exe ./orbit/cmd/mobiusdaemon_tables
mobiusdaemon-tables-windows-arm64:
	GOOS=windows GOARCH=arm64 go build -o mobiusdaemon_tables_windows_arm64.exe ./orbit/cmd/mobiusdaemon_tables
mobiusdaemon-tables-linux:
	GOOS=linux GOARCH=amd64 go build -o mobiusdaemon_tables_linux.ext ./orbit/cmd/mobiusdaemon_tables
mobiusdaemon-tables-linux-arm64:
	GOOS=linux GOARCH=arm64 go build -o mobiusdaemon_tables_linux_arm64.ext ./orbit/cmd/mobiusdaemon_tables
mobiusdaemon-tables-darwin:
	GOOS=darwin GOARCH=amd64 go build -o mobiusdaemon_tables_darwin.ext ./orbit/cmd/mobiusdaemon_tables
mobiusdaemon-tables-darwin_arm64:
	GOOS=darwin GOARCH=arm64 CGO_ENABLED=1 go build -o mobiusdaemon_tables_darwin_arm64.ext ./orbit/cmd/mobiusdaemon_tables
mobiusdaemon-tables-darwin-universal: mobiusdaemon-tables-darwin mobiusdaemon-tables-darwin_arm64
	lipo -create mobiusdaemon_tables_darwin.ext mobiusdaemon_tables_darwin_arm64.ext -output mobiusdaemon_tables_darwin_universal.ext
mobiusdaemon-tables-all: mobiusdaemon-tables-windows mobiusdaemon-tables-linux mobiusdaemon-tables-darwin-universal mobiusdaemon-tables-linux-arm64 mobiusdaemon-tables-windows-arm64
mobiusdaemon-tables-clean:
	rm -f mobiusdaemon_tables_windows.exe mobiusdaemon_tables_linux.ext mobiusdaemon_tables_linux_arm64.ext mobiusdaemon_tables_darwin.ext mobiusdaemon_tables_darwin_arm64.ext mobiusdaemon_tables_darwin_universal.ext
...
e2e-reset-db:
	...
	./build/mobius prepare db --mysql_address=localhost:3307  --mysql_username=root --mysql_password=toor --mysql_database=e2e

e2e-setup:
	./build/mobiuscli config set --context e2e --address https://localhost:8642 --tls-skip-verify true
	./build/mobiuscli setup --context e2e --email=admin@example.com --password=password123# --org-name='Mobius Test' --name Admin
	./build/mobiuscli user create --context e2e --email=maintainer@example.com --name maintainer --password=password123# --global-role=maintainer
	./build/mobiuscli user create --context e2e --email=observer@example.com --name observer --password=password123# --global-role=observer
	./build/mobiuscli user create --context e2e --email=sso_user@example.com --name "SSO user" --sso=true
...
e2e-setup-with-software:
	curl 'https://localhost:8642/api/v1/setup' \
		--data-raw '{"server_url":"https://localhost:8642","org_info":{"org_name":"Mobius Test"},"admin":{"admin":true,"email":"admin@example.com","name":"Admin","password":"password123#","password_confirmation":"password123#"}}' \
		--compressed \
		--insecure
	./tools/backup_db/restore_e2e_software_test.sh

e2e-serve-free: e2e-reset-db
	./build/mobius serve --mysql_address=localhost:3307 --mysql_username=root --mysql_password=toor --mysql_database=e2e --server_address=0.0.0.0:8642

e2e-serve-premium: e2e-reset-db
	./build/mobius serve  --dev_license --mysql_address=localhost:3307 --mysql_username=root --mysql_password=toor --mysql_database=e2e --server_address=0.0.0.0:8642

# Associate a host with a Mobius Desktop token.
#
# Usage:
...
changelog-chrome:
	$(eval TODAY_DATE := $(shell date "+%b %d, %Y"))
	@echo -e "## mobiusdaemon-chrome $(version) ($(TODAY_DATE))\n" > new-CHANGELOG.md
	sh -c "find ee/mobiusdaemon-chrome/changes -type file | grep -v .keep | xargs -I {} sh -c 'grep \"\S\" {}; echo' >> new-CHANGELOG.md"
	sh -c "cat new-CHANGELOG.md ee/mobiusdaemon-chrome/CHANGELOG.md > tmp-CHANGELOG.md && rm new-CHANGELOG.md && mv tmp-CHANGELOG.md ee/mobiusdaemon-chrome/CHANGELOG.md"
	sh -c "git rm ee/mobiusdaemon-chrome/changes/*"

# Updates the documentation for the currently released versions of mobiusdaemon components in old Mobius's TUF (tuf.mobiuscli.com).
mobiusdaemon-old-tuf:
	sh -c 'echo "<!-- DO NOT EDIT. This document is automatically generated by running \`make mobiusdaemon-old-tuf\`. -->\n# tuf.mobiuscli.com\n\nFollowing are the currently deployed versions of mobiusdaemon components on the \`stable\` and \`edge\` channel.\n" > orbit/old-TUF.md'
	sh -c 'echo "## \`stable\`\n" >> orbit/old-TUF.md'
	sh -c 'go run tools/tuf/status/tuf-status.go channel-version -url https://tuf.mobiuscli.com -channel stable -format markdown >> orbit/old-TUF.md'
	sh -c 'echo "\n## \`edge\`\n" >> orbit/old-TUF.md'
	sh -c 'go run tools/tuf/status/tuf-status.go channel-version -url https://tuf.mobiuscli.com -channel edge -format markdown >> orbit/old-TUF.md'

# Updates the documentation for the currently released versions of mobiusdaemon components in Mobius's TUF (updates.mobiusmdm.com).
mobiusdaemon-tuf:
	sh -c 'echo "<!-- DO NOT EDIT. This document is automatically generated by running \`make mobiusdaemon-tuf\`. -->\n# updates.mobiusmdm.com\n\nFollowing are the currently deployed versions of mobiusdaemon components on the \`stable\` and \`edge\` channel.\n" > orbit/TUF.md'
	sh -c 'echo "## \`stable\`\n" >> orbit/TUF.md'
...
db-reset:
	docker compose exec -T mysql bash -c 'echo "drop database if exists mobius; create database mobius;" | MYSQL_PWD=toor mysql -uroot'
...
#
# Usage:
# MOBIUS_DESKTOP_APPLE_AUTHORITY=foo MOBIUS_DESKTOP_VERSION=0.0.1 make desktop-app-tar-gz
#
...
MOBIUS_DESKTOP_VERSION ?= unknown
...
#
# Usage:
# MOBIUS_DESKTOP_VERSION=0.0.1 make desktop-windows
#
# Output: mobius-desktop.exe
desktop-windows:
	go run ./orbit/tools/build/build-windows.go -version $(MOBIUS_DESKTOP_VERSION) -input ./orbit/cmd/desktop -output mobius-desktop.exe
...
#
# Usage:
# MOBIUS_DESKTOP_VERSION=0.0.1 make desktop-windows-arm64
#
# Output: mobius-desktop.exe
desktop-windows-arm64:
	go run ./orbit/tools/build/build-windows.go -version $(MOBIUS_DESKTOP_VERSION) -input ./orbit/cmd/desktop -output mobius-desktop.exe -arch arm64

# Build desktop executable for Linux.
#
# Usage:
# MOBIUS_DESKTOP_VERSION=0.0.1 make desktop-linux
#
...
desktop-linux:
	...
	docker run --rm -v $(shell pwd):/output desktop-linux-builder /bin/bash -c "\
		mkdir -p /output/mobius-desktop && \
		go build -o /output/mobius-desktop/mobius-desktop -ldflags "-X=main.version=$(MOBIUS_DESKTOP_VERSION)" /usr/src/mobius/orbit/cmd/desktop && \
		cd /output && \
		tar czf desktop.tar.gz mobius-desktop && \
		rm -r mobius-desktop"

# Build desktop executable for Linux ARM.
#
# Usage:
# MOBIUS_DESKTOP_VERSION=0.0.1 make desktop-linux-arm64
#
# Output: desktop.tar.gz
desktop-linux-arm64:
	docker build -f Dockerfile-desktop-linux -t desktop-linux-builder .
	docker run --rm -v $(shell pwd):/output desktop-linux-builder /bin/bash -c "\
		mkdir -p /output/mobius-desktop && \
		GOARCH=arm64 go build -o /output/mobius-desktop/mobius-desktop -ldflags "-X=main.version=$(MOBIUS_DESKTOP_VERSION)" /usr/src/mobius/orbit/cmd/desktop && \
		cd /output && \
		tar czf desktop.tar.gz mobius-desktop && \
		rm -r mobius-desktop"
...
db-replica-setup:
	...
	if [ "${MOBIUS_MYSQL_IMAGE}" == "mysql:8.0" ]; then MYSQL_PWD=toor mysql --host 127.0.0.1 --port 3309 -uroot -AN -e "change master to get_master_public_key=1;"; fi
...
db-replica-reset: mobius
	MYSQL_PWD=toor mysql --host 127.0.0.1 --port 3308 -uroot -e "drop database if exists mobius; create database mobius;"
	MOBIUS_MYSQL_ADDRESS=127.0.0.1:3308 ./build/mobius prepare db --dev

# db-replica-run runs mobius serve with one main and one read MySQL instance.
db-replica-run: mobius
	MOBIUS_MYSQL_ADDRESS=127.0.0.1:3308 MOBIUS_MYSQL_READ_REPLICA_ADDRESS=127.0.0.1:3309 MOBIUS_MYSQL_READ_REPLICA_USERNAME=mobius MOBIUS_MYSQL_READ_REPLICA_DATABASE=mobius MOBIUS_MYSQL_READ_REPLICA_PASSWORD=insecure ./build/mobius serve --dev --dev_license

vex-report:
	sh -c 'echo "<!-- DO NOT EDIT. This document is automatically generated by running \`make vex-report\`. -->\n# Vulnerability Report\n\nFollowing is the vulnerability report of Mobius and its dependencies.\n" > security/status.md'
	sh -c 'echo "## \`mobiusmdm/mobius\` docker image\n" >> security/status.md'
	sh -c 'go run ./tools/vex-parser ./security/vex/mobius >> security/status.md'
	sh -c 'echo "## \`mobiusmdm/mobiuscli\` docker image\n" >> security/status.md'
	sh -c 'go run ./tools/vex-parser ./security/vex/mobiuscli >> security/status.md'
