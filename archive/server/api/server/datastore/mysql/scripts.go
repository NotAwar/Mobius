...
import (
	...
	constants "github.com/MobiusDM/mobius/server/api/pkg/scripts"
)
...
func (ds *Datastore) NewHostScriptExecutionRequest(ctx context.Context, request *mobius.HostScriptRequestPayload) (*mobius.HostScriptResult, error) {
	var res *mobius.HostScriptResult
...
func (ds *Datastore) newHostScriptExecutionRequest(ctx context.Context, tx sqlx.ExtContext, request *mobius.HostScriptRequestPayload, isInternal bool) (*mobius.HostScriptResult, error) {
	const (
		getStmt = `
SELECT
...
`
	...
	var script mobius.HostScriptResult
...
func (ds *Datastore) insertNewHostScriptExecution(ctx context.Context, tx sqlx.ExtContext, request *mobius.HostScriptRequestPayload, isInternal bool) (string, int64, error) {
	const (
		insUAStmt = `
...
`
	...
	result, err := tx.ExecContext(ctx, insUAStmt,
		...
		request.Priority(),
		request.UserID,
		request.PolicyID != nil, // mobius-initiated if request is via a policy failure
		execID,
		request.SyncRequest,
		isInternal,
		request.UserID,
...
func (ds *Datastore) SetHostScriptExecutionResult(ctx context.Context, result *mobius.HostScriptResultPayload) (*mobius.HostScriptResult,
	string, error,
) {
...
`
	...
	var hsr *mobius.HostScriptResult
	var action string
...
func (ds *Datastore) ListPendingHostScriptExecutions(ctx context.Context, hostID uint, onlyShowInternal bool) ([]*mobius.HostScriptResult, error) {
	return ds.listUpcomingHostScriptExecutions(ctx, hostID, onlyShowInternal, false)
}

func (ds *Datastore) ListReadyToExecuteScriptsForHost(ctx context.Context, hostID uint, onlyShowInternal bool) ([]*mobius.HostScriptResult, error) {
	return ds.listUpcomingHostScriptExecutions(ctx, hostID, onlyShowInternal, true)
}

func (ds *Datastore) listUpcomingHostScriptExecutions(ctx context.Context, hostID uint, onlyShowInternal, onlyReadyToExecute bool) ([]*mobius.HostScriptResult, error) {
	extraWhere := ""
	if onlyShowInternal {
  ...
  SELECT
	...
	var results []*mobius.HostScriptResult
...
func (ds *Datastore) GetHostScriptExecutionResult(ctx context.Context, execID string) (*mobius.HostScriptResult, error) {
	return ds.getHostScriptExecutionResultDB(ctx, ds.reader(ctx), execID, scriptExecutionSearchOpts{})
}

func (ds *Datastore) GetSelfServiceUninstallScriptExecutionResult(ctx context.Context, execID string, hostID uint) (*mobius.HostScriptResult, error) {
	return ds.getHostScriptExecutionResultDB(ctx, ds.reader(ctx), execID, scriptExecutionSearchOpts{UninstallHostID: hostID})
}

func (ds *Datastore) getHostScriptExecutionResultDB(ctx context.Context, q sqlx.QueryerContext, execID string, opts scriptExecutionSearchOpts) (*mobius.HostScriptResult, error) {
...
`
	...
	var result mobius.HostScriptResult
...
func (ds *Datastore) NewScript(ctx context.Context, script *mobius.Script) (*mobius.Script, error) {
	var res sql.Result
	err := ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		var err error
...
func (ds *Datastore) UpdateScriptContents(ctx context.Context, scriptID uint, scriptContents string) (*mobius.Script, error) {
	const stmt = `
...
func insertScript(ctx context.Context, tx sqlx.ExtContext, script *mobius.Script, scriptContentsID uint) (sql.Result, error) {
	const insertStmt = `
INSERT INTO
  scripts (
...
func (ds *Datastore) Script(ctx context.Context, id uint) (*mobius.Script, error) {
	return ds.getScriptDB(ctx, ds.reader(ctx), id)
}

func (ds *Datastore) getScriptDB(ctx context.Context, q sqlx.QueryerContext, id uint) (*mobius.Script, error) {
	const getStmt = `
SELECT
  id,
  team_id,
...
var errDeleteScriptWithAssociatedPolicy = &mobius.ConflictError{Message: "Couldn't delete. Policy automation uses this script. Please remove this script from associated policy automations and try again."}
...
func (ds *Datastore) ListScripts(ctx context.Context, teamID *uint, opt mobius.ListOptions) ([]*mobius.Script, *mobius.PaginationMetadata, error) {
	var scripts []*mobius.Script
...
func (ds *Datastore) GetHostScriptDetails(ctx context.Context, hostID uint, teamID *uint, opt mobius.ListOptions, hostPlatform string) ([]*mobius.HostScriptDetail, *mobius.PaginationMetadata, error) {
	var globalOrTeamID uint
	if teamID != nil {
		globalOrTeamID = *teamID
	}
	...
	case hostPlatform == "windows":
		...
		extension = `%.ps1`
	case mobius.IsUnixLike(hostPlatform):
...
`
	...
	results := make([]*mobius.HostScriptDetail, 0, len(rows))
	for _, r := range rows {
		results = append(results, mobius.NewHostScriptDetail(hostID, r.ScriptID, r.Name, r.ExecutionID, r.ExecutedAt, r.ExitCode, r.HSRID))
	}

	return results, metaData, nil
}

func (ds *Datastore) BatchSetScripts(ctx context.Context, tmID *uint, scripts []*mobius.Script) ([]mobius.ScriptResponse, error) {
...
`
	...
	// of processing
	incomingScripts := make(map[string]*mobius.Script, len(scripts))
	for i, p := range scripts {
		incomingNames[i] = p.Name
		incomingScripts[p.Name] = p
	}

	var insertedScripts []mobius.ScriptResponse
	var activateAffectedHosts []uint

	if err := ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		var existingScripts []*mobius.Script
...
func (ds *Datastore) GetHostLockWipeStatus(ctx context.Context, host *mobius.Host) (*mobius.HostLockWipeStatus, error) {
	const stmt = `
		SELECT
			lock_ref,
			wipe_ref,
...
`
	...
	status := &mobius.HostLockWipeStatus{
		HostMobiusPlatform: mobiusPlatform,
	}
	...
	// if we have a mobius platform stored in host_mdm_actions, use it instead of
	// the host.MobiusPlatform() because the platform can be overwritten with an
	// unknown OS name when a Wipe gets executed.
	if mdmActions.MobiusPlatform != "" {
		mobiusPlatform = mdmActions.MobiusPlatform
		status.HostMobiusPlatform = mobiusPlatform
	}

	switch mobiusPlatform {
...
func (ds *Datastore) getHostMDMWindowsCommand(ctx context.Context, cmdUUID, hostUUID string) (*mobius.MDMCommand, *mobius.MDMCommandResult, error) {
	...
	// that specific host.
	var cmdRes *mobius.MDMCommandResult
...
func (ds *Datastore) getHostMDMAppleCommand(ctx context.Context, cmdUUID, hostUUID string) (*mobius.MDMCommand, *mobius.MDMCommandResult, error) {
	cmd, err := ds.getMDMCommand(ctx, ds.reader(ctx), cmdUUID)
	if err != nil {
	...
	for _, r := range cmdResults {
		...
		if r.Status == mobius.MDMAppleStatusAcknowledged || r.Status == mobius.MDMAppleStatusError || r.Status == mobius.MDMAppleStatusCommandFormatError {
			cmdRes = r
			break
		}
	}
...
func (ds *Datastore) LockHostViaScript(ctx context.Context, request *mobius.HostScriptRequestPayload, hostMobiusPlatform string) error {
	var res *mobius.HostScriptResult
...
// host_mdm_actions in a single transaction.
func (ds *Datastore) UnlockHostViaScript(ctx context.Context, request *mobius.HostScriptRequestPayload, hostMobiusPlatform string) error {
	var res *mobius.HostScriptResult
	return ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		var err error
...
func (ds *Datastore) WipeHostViaScript(ctx context.Context, request *mobius.HostScriptRequestPayload, hostMobiusPlatform string) error {
	var res *mobius.HostScriptResult
	return ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		var err error
...
func (ds *Datastore) UnlockHostManually(ctx context.Context, hostID uint, hostMobiusPlatform string, ts time.Time) error {
	const stmt = `
	INSERT INTO host_mdm_actions
	(
		host_id,
	...
	_, err := ds.writer(ctx).ExecContext(ctx, stmt, hostID, unlockRef, hostMobiusPlatform)
...
func (ds *Datastore) BatchExecuteScript(ctx context.Context, userID *uint, scriptID uint, hostIDs []uint) (string, error) {
	...
	if err != nil {
		return "", mobius.NewInvalidArgumentError("script_id", err.Error())
	}

	// We need full host info to check if hosts are able to run scripts, see svc.RunHostScript
	fullHosts := make([]*mobius.Host, 0, len(hostIDs))
	...
	executions := make([]mobius.BatchExecutionHost, 0, len(fullHosts))
	...
	if err := ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		for _, host := range fullHosts {
			...
			if noNodeKey || scriptsDisabled {
				executions = append(executions, mobius.BatchExecutionHost{
					HostID: host.ID,
					Error:  &mobius.BatchExecuteIncompatibleMobiusd,
				})
				continue
			}

			if !mobius.ValidateScriptPlatform(script.Name, host.Platform) {
				executions = append(executions, mobius.BatchExecutionHost{
					HostID: host.ID,
					Error:  &mobius.BatchExecuteIncompatiblePlatform,
				})
				continue
			}

			executionID, _, err := ds.insertNewHostScriptExecution(ctx, tx, &mobius.HostScriptRequestPayload{
				HostID:          host.ID,
				UserID:          userID,
				ScriptID:        &script.ID,
...
func (ds *Datastore) BatchExecuteSummary(ctx context.Context, executionID string) (*mobius.BatchExecutionSummary, error) {
	stmtExecutions := `
SELECT
...
WHERE
	...
	var summary mobius.BatchExecutionSummary
