...
import (
	...
	ctxabm "github.com/MobiusDM/mobius/server/api/server/contexts/apple_bm"
	"github.com/MobiusDM/mobius/server/api/server/contexts/ctxerr"
	"github.com/MobiusDM/mobius/server/api/server/logging"
	"github.com/MobiusDM/mobius/server/api/server/mdm/apple/mobileconfig"
	"github.com/MobiusDM/mobius/server/api/server/mdm/internal/commonmdm"
	...
	depsync "github.com/MobiusDM/mobius/server/api/server/mdm/nanodep/sync"
)

const (
	// SCEPPath is Mobius's HTTP path for the SCEP service.
	SCEPPath = "/mdm/apple/scep"
	// MDMPath is Mobius's HTTP path for the core MDM service.
	...
	// MobiusUISSOCallbackPath is the front-end route used to
	// redirect after the SSO flow is completed.
	MobiusUISSOCallbackPath = "/mdm/sso/callback"

	// MobiusPayloadIdentifier is the value for the "<key>PayloadIdentifier</key>"
	// used by Mobius MDM on the enrollment profile.
	MobiusPayloadIdentifier = "com.mobiusmdm.mobius.mdm.apple"
...
// internal usage within Mobius. If you need to expose any of the functionality
...
func (d *DEPService) getDefaultProfile() *godep.Profile {
	return &godep.Profile{
		ProfileName:      "Mobius default enrollment profile",
		AllowPairing:     true,
...
func (d *DEPService) createDefaultAutomaticProfile(ctx context.Context) error {
	...
	payload := mobius.MDMAppleEnrollmentProfilePayload{
		Token:      token,
		Type:       mobius.MDMAppleEnrollmentTypeAutomatic,
...
func (d *DEPService) buildJSONProfile(ctx context.Context, setupAsstJSON json.RawMessage, appCfg *mobius.AppConfig, team *mobius.Team, enrollURL string) (*godep.Profile, error) {
	...
	// automatically by Mobius or manually by the user if
...
func (d *DEPService) RegisterProfileWithAppleDEPServer(ctx context.Context, team *mobius.Team, setupAsst *mobius.MDMAppleSetupAssistant, abmTokenOrgName string) (string, time.Time, error) {
	...
	defaultProf, err := d.ds.GetMDMAppleEnrollmentProfileByType(ctx, mobius.MDMAppleEnrollmentTypeAutomatic)
	if err != nil {
...
// Apple API.
func (d *DEPService) ValidateSetupAssistant(ctx context.Context, team *mobius.Team, setupAsst *mobius.MDMAppleSetupAssistant, abmTokenOrgName string) error {
	appCfg, err := d.ds.AppConfig(ctx)
	if err != nil {
...
// token.
func (d *DEPService) EnsureDefaultSetupAssistant(ctx context.Context, team *mobius.Team, abmTokenOrgName string) (string, time.Time, error) {
	...
	defProf, err := d.ds.GetMDMAppleEnrollmentProfileByType(ctx, mobius.MDMAppleEnrollmentTypeAutomatic)
...
// no error.
func (d *DEPService) EnsureCustomSetupAssistantIfExists(ctx context.Context, team *mobius.Team, abmTokenOrgName string) (string, time.Time, error) {
	var tmID *uint
	if team != nil {
		tmID = &team.ID
	}
...
func (d *DEPService) RunAssigner(ctx context.Context) error {
	...
	teamsByID := make(map[uint]*mobius.Team, len(teams))
	...
	for _, token := range tokens {
		var macOSTeam, iosTeam, ipadTeam *mobius.Team
		...
		teams := []*mobius.Team{macOSTeam, iosTeam, ipadTeam}
...
func (d *DEPService) processDeviceResponse(
	...
	macOSTeam *mobius.Team,
	iosTeam *mobius.Team,
	ipadTeam *mobius.Team,
) error {
	...
	// Mobius in ABM. A host in this state will have a row in `host_dep_assignments` where the
...
// changes.
func NewDEPClient(storage godep.ClientStorage, updater mobius.ABMTermsUpdater, logger kitlog.Logger) *godep.Client {
	return godep.NewClient(storage, mobiushttp.NewClient(), godep.WithAfterHook(func(ctx context.Context, reqErr error) error {
        ...
        <dict>
          ...
          <array>
            ...
            <array>
              <array>
                <string>CN</string>
                <string>Mobius Identity</string>
              </array>
            </array>
          </array>
        </dict>
        <key>PayloadIdentifier</key>
        <string>com.mobiusmdm.mobius.mdm.apple.scep</string>
        <key>PayloadType</key>
...
// enrollmentProfileMobileconfigTemplate is the template Mobius uses to assemble a .mobileconfig enrollment profile to serve to devices.
//
...
<dict>
	...
	<array>
		<dict>
			...
			<dict>
				...
				<key>Subject</key>
				<array>
					<array><array><string>O</string><string>Mobius</string></array></array>
					<array><array><string>CN</string><string>Mobius Identity</string></array></array>
				</array>
			</dict>
			<key>PayloadIdentifier</key>
			<string>com.mobiusmdm.mobius.mdm.apple.scep</string>
		...
		<dict>
			...
			<string>BCA53F9D-5DD2-494D-98D3-0D0F20FF6BA1</string>
			<key>PayloadIdentifier</key>
			<string>com.mobiusmdm.mobius.mdm.apple.mdm</string>
			<key>PayloadType</key>
	...
	<key>PayloadIdentifier</key>
	<string>` + MobiusPayloadIdentifier + `</string>
...
func GenerateEnrollmentProfileMobileconfig(orgName, mobiusURL, scepChallenge, topic string) ([]byte, error) {
	scepURL, err := ResolveAppleSCEPURL(mobiusURL)
	if err != nil {
		return nil, fmt.Errorf("resolve Apple SCEP url: %w", err)
	}
	serverURL, err := ResolveAppleMDMURL(mobiusURL)
	if err != nil {
...
func AddEnrollmentRefToMobiusURL(mobiusURL, reference string) (string, error) {
	if reference == "" {
		return mobiusURL, nil
	}

	u, err := url.Parse(mobiusURL)
	if err != nil {
		return "", fmt.Errorf("parsing configured server URL: %w", err)
	}
	q := u.Query()
	q.Add(mobileconfig.MobiusEnrollReferenceKey, reference)
	u.RawQuery = q.Encode()
	return u.String(), nil
}
...
type ProfileBimap struct {
	wantedState  map[*mobius.MDMAppleProfilePayload]*mobius.MDMAppleProfilePayload
	currentState map[*mobius.MDMAppleProfilePayload]*mobius.MDMAppleProfilePayload
}

// NewProfileBimap retuns a new ProfileBimap
func NewProfileBimap() *ProfileBimap {
	return &ProfileBimap{
		map[*mobius.MDMAppleProfilePayload]*mobius.MDMAppleProfilePayload{},
		map[*mobius.MDMAppleProfilePayload]*mobius.MDMAppleProfilePayload{},
	}
}

// GetMatchingProfileInDesiredState returns the addition key that matches the given removal
func (pb *ProfileBimap) GetMatchingProfileInDesiredState(removal *mobius.MDMAppleProfilePayload) (*mobius.MDMAppleProfilePayload, bool) {
	value, ok := pb.currentState[removal]
	return value, ok
}

// GetMatchingProfileInCurrentState returns the removal key that matches the given addition
func (pb *ProfileBimap) GetMatchingProfileInCurrentState(addition *mobius.MDMAppleProfilePayload) (*mobius.MDMAppleProfilePayload, bool) {
...
func (pb *ProfileBimap) IntersectByIdentifierAndHostUUID(wantedProfiles, currentProfiles []*mobius.MDMAppleProfilePayload) {
	key := func(p *mobius.MDMAppleProfilePayload) string {
		return fmt.Sprintf("%s-%s", p.ProfileIdentifier, p.HostUUID)
	}

	removeProfs := map[string]*mobius.MDMAppleProfilePayload{}
...
func (pb *ProfileBimap) add(wantedProfile, currentProfile *mobius.MDMAppleProfilePayload) {
	pb.wantedState[wantedProfile] = currentProfile
	pb.currentState[currentProfile] = wantedProfile
}

func IOSiPadOSRefetch(ctx context.Context, ds mobius.Datastore, commander *MDMAppleCommander, logger kitlog.Logger) error {
	...
	hostMDMCommands := make([]mobius.HostMDMCommand, 0, 3*len(devices))
	installedAppsUUIDs := make([]string, 0, len(devices))
	for _, device := range devices {
		if !slices.Contains(device.CommandsAlreadySent, mobius.RefetchAppsCommandUUIDPrefix) {
	...
	if len(installedAppsUUIDs) > 0 {
		err = commander.InstalledApplicationList(ctx, installedAppsUUIDs, mobius.RefetchAppsCommandUUIDPrefix+commandUUID, false)
		if err != nil {
	...
	for _, device := range devices {
		if !slices.Contains(device.CommandsAlreadySent, mobius.RefetchCertsCommandUUIDPrefix) {
	...
	if len(certsListUUIDs) > 0 {
		err = commander.CertificateList(ctx, certsListUUIDs, mobius.RefetchCertsCommandUUIDPrefix+commandUUID)
		if err != nil {
	...
	for _, device := range devices {
		if !slices.Contains(device.CommandsAlreadySent, mobius.RefetchDeviceCommandUUIDPrefix) {
	...
	if len(deviceInfoUUIDs) > 0 {
		if err := commander.DeviceInformation(ctx, deviceInfoUUIDs, mobius.RefetchDeviceCommandUUIDPrefix+commandUUID); err != nil {
...
func GenerateOTAEnrollmentProfileMobileconfig(orgName, mobiusURL, enrollSecret string) ([]byte, error) {
	path, err := url.JoinPath(mobiusURL, "/api/v1/mobiuss/ota_enrollment")
	if err != nil {
...
func IOSiPadOSRevive(ctx context.Context, ds mobius.Datastore, commander *MDMAppleCommander, logger kitlog.Logger) error {
	appCfg, err := ds.AppConfig(ctx)
	if err != nil {
		return ctxerr.Wrap(ctx, err, "fetching app config")
	}
	...
	ids, err := ds.ListMDMAppleEnrolledIPhoneIpadDeletedFromMobius(ctx, 500)
	if err != nil {
