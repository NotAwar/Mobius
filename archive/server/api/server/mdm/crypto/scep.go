...
import (
	...
	"github.com/MobiusDM/mobius/server/api/server/mdm/nanomdm/http/mdm"
...
func NewSCEPVerifier(ds mobius.MDMAssetRetriever) *SCEPVerifier {
	return &SCEPVerifier{
		ds: ds,
	}
}
...
func (s *SCEPVerifier) Verify(ctx context.Context, cert *x509.Certificate) error {
	...
	rootCert, err := assets.X509Cert(ctx, s.ds, mobius.MDMAssetCACert)
	if err != nil {
		return fmt.Errorf("loading existing assets from the database: %w", err)
	}
	opts.Roots.AddCert(rootCert)

	// the default SCEP cert issued by mobius doesn't have any extra key
