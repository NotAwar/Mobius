...
func New(ds mobius.Datastore, logger kitlog.Logger) *HostLifecycle {
...
func (t *HostLifecycle) turnOnApple(ctx context.Context, opts HostOptions) error {
	...
	if info.DEPAssignedToMobius || info.InstalledFromDEP {
...
func (t *HostLifecycle) restorePendingDEPHost(ctx context.Context, host *mobius.Host, abmTokenID *uint) error {
	if abmTokenID == nil {
...
func (t *HostLifecycle) getDefaultTeamForABMToken(ctx context.Context, host *mobius.Host, abmTokenID uint) (*uint, error) {
	...
	default:
		return nil, ctxerr.NewWithData(ctx, "attempting to get default ABM team for host with invalid platform", map[string]any{"host_platform": host.MobiusPlatform(), "host_id": host.ID})
	}
