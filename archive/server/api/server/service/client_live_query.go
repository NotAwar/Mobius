...
import (
	...
	ws "github.com/MobiusDM/mobius/server/api/server/websocket"
)
...
type LiveQueryResultsHandler struct {
	errors  chan error
	results chan mobius.DistributedQueryResult
...
func NewLiveQueryResultsHandler() *LiveQueryResultsHandler {
	return &LiveQueryResultsHandler{
		...
		results: make(chan mobius.DistributedQueryResult),
	}
}
...
func (h *LiveQueryResultsHandler) Results() <-chan mobius.DistributedQueryResult {
	return h.results
}
...
func (c *Client) LiveQueryWithContext(
	...
	verb, path := "POST", "/api/latest/mobius/queries/run_by_identifiers"
	...
	wssURL.Path = c.urlPrefix + "/api/latest/mobius/results/websocket"
