...
import (
	...
	authzctx "github.com/MobiusDM/mobius/server/api/server/contexts/authz"
	...
	mdmlifecycle "github.com/MobiusDM/mobius/server/api/server/mdm/lifecycle"
	...
	"github.com/MobiusDM/mobius/server/api/server/worker"
)
...
type HostDetailResponse struct {
	...
	Geolocation *mobius.GeoLocation `json:"geolocation,omitempty"`
}

func hostDetailResponseForHost(ctx context.Context, svc mobius.Service, host *mobius.HostDetail) (*HostDetailResponse, error) {
...
type listHostsRequest struct {
	Opts mobius.HostListOptions `url:"host_options"`
}
...
type listHostsResponse struct {
	...
	MDMSolution *mobius.MDMSolution `json:"mobile_device_management_solution,omitempty"`
	...
	// payload.
	MunkiIssue *mobius.MunkiIssue `json:"munki_issue,omitempty"`
...
func listHostsEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
	...
	if req.Opts.SoftwareTitleIDFilter != nil {
		...
		if err != nil && !mobius.IsNotFound(err) { // ignore not found, just return nil for the software title in that case
			return listHostsResponse{Err: err}, nil
		}
	}
	...
	var munkiIssue *mobius.MunkiIssue
	if req.Opts.MunkiIssueIDFilter != nil {
		var err error
		munkiIssue, err = svc.GetMunkiIssue(ctx, *req.Opts.MunkiIssueIDFilter)
		if err != nil && !mobius.IsNotFound(err) { // ignore not found, just return nil for the munki issue in that case
			return listHostsResponse{Err: err}, nil
		}
	}
...
func (svc *Service) GetMDMSolution(ctx context.Context, mdmID uint) (*mobius.MDMSolution, error) {
...
func (svc *Service) GetMunkiIssue(ctx context.Context, munkiIssueID uint) (*mobius.MunkiIssue, error) {
	// require list hosts permission to view this information
	if err := svc.authz.Authorize(ctx, &mobius.Host{}, mobius.ActionList); err != nil {
		return nil, err
	}
...
func (svc *Service) ListHosts(ctx context.Context, opt mobius.HostListOptions) ([]*mobius.Host, error) {
	if err := svc.authz.Authorize(ctx, &mobius.Host{}, mobius.ActionList); err != nil {
		return nil, err
	}
...
func deleteHostsEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) DeleteHosts(ctx context.Context, ids []uint, filter *map[string]interface{}) error {
	...
	if len(ids) == 0 && lid == nil && opts == nil {
		return &mobius.BadRequestError{Message: "list of ids or filters must be specified"}
	}

	if len(ids) > 0 && (lid != nil || (opts != nil && !opts.Empty())) {
		return &mobius.BadRequestError{Message: "Cannot specify a list of ids and filters at the same time"}
	}

	doDelete := func(hostIDs []uint, hosts []*mobius.Host) error {
...
func countHostsEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) CountHosts(ctx context.Context, labelID *uint, opts mobius.HostListOptions) (int, error) {
	if err := svc.authz.Authorize(ctx, &mobius.Host{}, mobius.ActionList); err != nil {
		return 0, err
	}
...
func (svc *Service) countHostFromFilters(ctx context.Context, labelID *uint, opt mobius.HostListOptions) (int, error) {
	vc, ok := viewer.FromContext(ctx)
	if !ok {
		return 0, mobius.ErrNoContext
	}
	filter := mobius.TeamFilter{User: vc.User, IncludeObserver: true}
...
func searchHostsEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
	...
	results := []*mobius.HostResponse{}

	for _, h := range hosts {
		results = append(results, mobius.HostResponseForHostCheap(h))
	}
...
func (svc *Service) SearchHosts(ctx context.Context, matchQuery string, queryID *uint, excludedHostIDs []uint) ([]*mobius.Host, error) {
	if err := svc.authz.Authorize(ctx, &mobius.Host{}, mobius.ActionList); err != nil {
		return nil, err
	}
...
func getHostEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) GetHost(ctx context.Context, id uint, opts mobius.HostDetailOptions) (*mobius.HostDetail, error) {
...
func (svc *Service) checkWriteForHostIDs(ctx context.Context, ids []uint) error {
	for _, id := range ids {
		...
		// Authorize again with team loaded now that we have team_id
		if err := svc.authz.Authorize(ctx, host, mobius.ActionWrite); err != nil {
			return err
		}
	}
	return nil
...
// Get Host Lite
// //////////////////////////////////////////////////////////////////////////////
func (svc *Service) GetHostLite(ctx context.Context, id uint) (*mobius.Host, error) {
	if err := svc.authz.Authorize(ctx, &mobius.Host{}, mobius.ActionList); err != nil {
		return nil, err
	}
...
func getHostSummaryEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) GetHostSummary(ctx context.Context, teamID *uint, platform *string, lowDiskSpace *int) (*mobius.HostSummary, error) {
	if lowDiskSpace != nil {
	...
	var builtinLabels []*mobius.LabelSummary
	for _, l := range labelsSummary {
		if l.LabelType == mobius.LabelTypeBuiltIn {
...
func hostByIdentifierEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) HostByIdentifier(ctx context.Context, identifier string, opts mobius.HostDetailOptions) (*mobius.HostDetail, error) {
	...
	// Authorize again with team loaded now that we have team_id
	if err := svc.authz.Authorize(ctx, host, mobius.ActionSelectiveRead); err != nil {
		return nil, err
	}
...
func deleteHostEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func addHostsToTeamEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) AddHostsToTeam(ctx context.Context, teamID *uint, hostIDs []uint, skipBulkPending bool) error {
	...
	// teams.
	if err := svc.authz.Authorize(ctx, &mobius.Host{TeamID: teamID}, mobius.ActionWrite); err != nil {
		return err
	}
...
func (svc *Service) createTransferredHostsActivity(ctx context.Context, teamID *uint, hostIDs []uint, hostNames []string) error {
	...
	if len(hostNames) == 0 {
		...
		hostsByID := make(map[uint]*mobius.Host, len(hosts))
		for _, h := range hosts {
			hostsByID[h.ID] = h
		}
	...
	if err := svc.NewActivity(
		...
		mobius.ActivityTypeTransferredHostsToTeam{
			TeamID:           teamID,
			TeamName:         teamName,
			HostIDs:          hostIDs,
...
func addHostsToTeamByFilterEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) AddHostsToTeamByFilter(ctx context.Context, teamID *uint, filters *map[string]interface{}) error {
	...
	if opt == nil {
		return &mobius.BadRequestError{Message: "filters must be specified"}
	}
...
func refetchHostEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) RefetchHost(ctx context.Context, id uint) error {
	...
	// iOS and iPadOS refetch are not authenticated with device token because these devices do not have Mobius Desktop,
	...
	if !svc.authz.IsAuthenticatedWith(ctx, authzctx.AuthnDeviceToken) {
		...
		// We verify mobius.ActionRead instead of mobius.ActionWrite because we want to allow
	...
	if host != nil && (host.Platform == "ios" || host.Platform == "ipados") {
		...
		for _, cmd := range commands {
			...
			case mobius.RefetchDeviceCommandUUIDPrefix:
				doDeviceInfoRefetch = false
			case mobius.RefetchAppsCommandUUIDPrefix:
				doAppRefetch = false
			case mobius.RefetchCertsCommandUUIDPrefix:
		...
		hostMDMCommands := make([]mobius.HostMDMCommand, 0, 3)
		cmdUUID := uuid.NewString()
		if doAppRefetch {
			err = svc.mdmAppleCommander.InstalledApplicationList(ctx, []string{host.UUID}, mobius.RefetchAppsCommandUUIDPrefix+cmdUUID, false)
			if err != nil {
				return ctxerr.Wrap(ctx, err, "refetch apps with MDM")
			}
			hostMDMCommands = append(hostMDMCommands, mobius.HostMDMCommand{
				HostID:      host.ID,
				CommandType: mobius.RefetchAppsCommandUUIDPrefix,
			})
		}
		if doCertsRefetch {
			if err := svc.mdmAppleCommander.CertificateList(ctx, []string{host.UUID}, mobius.RefetchCertsCommandUUIDPrefix+cmdUUID); err != nil {
		...
		if doDeviceInfoRefetch {
			...
			err = svc.mdmAppleCommander.DeviceInformation(ctx, []string{host.UUID}, mobius.RefetchDeviceCommandUUIDPrefix+cmdUUID)
			if err != nil {
...
func (svc *Service) verifyMDMConfiguredAndConnected(ctx context.Context, host *mobius.Host) error {
	if err := svc.VerifyMDMAppleConfigured(ctx); err != nil {
		if errors.Is(err, mobius.ErrMDMNotConfigured) {
			err = mobius.NewInvalidArgumentError("id", mobius.AppleMDMNotConfiguredMessage).WithStatus(http.StatusBadRequest)
		}
	...
	if !connected {
		return ctxerr.Wrap(ctx,
			mobius.NewInvalidArgumentError("id", "Host does not have MDM turned on."))
	}
	return nil
}

func (svc *Service) getHostDetails(ctx context.Context, host *mobius.Host, opts mobius.HostDetailOptions) (*mobius.HostDetail, error) {
	...
	var nextMw *mobius.HostMaintenanceWindow
	if len(mws) > 0 {
		nextMw = mws[0]
	}
	...
	if opts.IncludePolicies {
		...
		if hp == nil {
			hp = []*mobius.HostPolicy{}
		}

		policies = &hp
	}

	// If Mobius MDM is enabled and configured, we want to include MDM profiles,
	...
	var profiles []mobius.HostMDMProfile
	...
	if ac.MDM.EnabledAndConfigured || ac.MDM.WindowsEnabledAndConfigured {
		host.MDM.OSSettings = &mobius.HostMDMOSSettings{}
		...
		case "windows":
			...
			if license.IsPremium(ctx) {
				...
				host.MDM.OSSettings.DiskEncryption = mobius.HostMDMDiskEncryption{}
			...
			if profs == nil {
				profs = []mobius.HostMDMWindowsProfile{}
			}
			for _, p := range profs {
				p.Detail = mobius.HostMDMProfileDetail(p.Detail).Message()
		...
		case "darwin", "ios", "ipados":
			if ac.MDM.EnabledAndConfigured {
				...
				host.MDM.PopulateOSSettingsAndMacOSSettings(profs, mobileconfig.MobiusFileVaultPayloadIdentifier)

				for _, p := range profs {
					if p.Identifier == mobileconfig.MobiusFileVaultPayloadIdentifier {
	...
	if host.IsLUKSSupported() {
		...
		if eDE {
			...
			if status.Status != nil && *status.Status == mobius.DiskEncryptionVerified {
				host.MDM.EncryptionKeyAvailable = true
			}
		}
	}

	var macOSSetup *mobius.HostMDMMacOSSetup
	if ac.MDM.EnabledAndConfigured && license.IsPremium(ctx) {
		...
		if err != nil {
			...
			macOSSetup = &mobius.HostMDMMacOSSetup{}
		}
	}
...
func getEndUsers(ctx context.Context, ds mobius.Datastore, hostID uint) ([]mobius.HostEndUser, error) {
	...
	var endUsers []mobius.HostEndUser
	if scimUser != nil {
		endUser := mobius.HostEndUser{
...
type getHostQueryReportResponse struct {
	...
	Results       []mobius.HostQueryReportResult `json:"results"`
	Err           error                          `json:"error,omitempty"`
}

func (r getHostQueryReportResponse) Error() error { return r.Err }

func getHostQueryReportEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) GetHostQueryReportResults(ctx context.Context, hostID uint, queryID uint) ([]mobius.HostQueryReportResult, *time.Time, error) {
	...
	if len(rows) == 0 {
		return []mobius.HostQueryReportResult{}, nil, nil
	}

	var lastFetched *time.Time
	result := make([]mobius.HostQueryReportResult, 0, len(rows))
	for _, row := range rows {
		...
		if row.Data != nil {
			...
			result = append(result, mobius.HostQueryReportResult{Columns: columns})
		}
	}
...
func (svc *Service) hostIDsAndNamesFromFilters(ctx context.Context, opt mobius.HostListOptions, lid *uint) ([]uint, []string, []*mobius.Host, error) {
	vc, ok := viewer.FromContext(ctx)
	if !ok {
		return nil, nil, nil, mobius.ErrNoContext
	}
	filter := mobius.TeamFilter{User: vc.User, IncludeObserver: true}
...
type listHostDeviceMappingResponse struct {
	HostID        uint                        `json:"host_id"`
	DeviceMapping []*mobius.HostDeviceMapping `json:"device_mapping"`
	Err           error                       `json:"error,omitempty"`
}
...
func listHostDeviceMappingEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) ListHostDeviceMapping(ctx context.Context, id uint) ([]*mobius.HostDeviceMapping, error) {
	if !svc.authz.IsAuthenticatedWith(ctx, authzctx.AuthnDeviceToken) {
		if err := svc.authz.Authorize(ctx, &mobius.Host{}, mobius.ActionList); err != nil {
			return nil, err
		}
...
func putHostDeviceMappingEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) SetCustomHostDeviceMapping(ctx context.Context, hostID uint, email string) ([]*mobius.HostDeviceMapping, error) {
	...
	source := mobius.DeviceMappingCustomOverride
	if isInstallerSource {
		source = mobius.DeviceMappingCustomInstaller
	}
...
func getHostMDM(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func getHostMDMSummary(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
type getMacadminsDataResponse struct {
	Err       error                 `json:"error,omitempty"`
	Macadmins *mobius.MacadminsData `json:"macadmins"`
}

func (r getMacadminsDataResponse) Error() error { return r.Err }

func getMacadminsDataEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) MacadminsData(ctx context.Context, id uint) (*mobius.MacadminsData, error) {
	if !svc.authz.IsAuthenticatedWith(ctx, authzctx.AuthnDeviceToken) {
		if err := svc.authz.Authorize(ctx, &mobius.Host{}, mobius.ActionList); err != nil {
			return nil, err
		}
	...
	var munkiInfo *mobius.HostMunkiInfo
	switch version, err := svc.ds.GetHostMunkiVersion(ctx, id); {
	case err != nil && !mobius.IsNotFound(err):
		return nil, err
	case err == nil:
		munkiInfo = &mobius.HostMunkiInfo{Version: version}
	}

	var mdm *mobius.HostMDM
	...
	var munkiIssues []*mobius.HostMunkiIssue
...
type getAggregatedMacadminsDataResponse struct {
	Err       error                           `json:"error,omitempty"`
	Macadmins *mobius.AggregatedMacadminsData `json:"macadmins"`
}

func (r getAggregatedMacadminsDataResponse) Error() error { return r.Err }

func getAggregatedMacadminsDataEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) AggregatedMacadminsData(ctx context.Context, teamID *uint) (*mobius.AggregatedMacadminsData, error) {
	if err := svc.authz.Authorize(ctx, &mobius.Host{TeamID: teamID}, mobius.ActionList); err != nil {
		return nil, err
	}
	...
	agg := &mobius.AggregatedMacadminsData{}
...
func (svc *Service) MDMData(ctx context.Context, id uint) (*mobius.HostMDM, error) {
	if err := svc.authz.Authorize(ctx, &mobius.Host{}, mobius.ActionList); err != nil {
		return nil, err
	}
...
func (svc *Service) AggregatedMDMData(ctx context.Context, teamID *uint, platform string) (mobius.AggregatedMDMData, error) {
	if err := svc.authz.Authorize(ctx, &mobius.Host{TeamID: teamID}, mobius.ActionList); err != nil {
		return mobius.AggregatedMDMData{}, err
	}
	...
	return mobius.AggregatedMDMData{
		MDMStatus:       mdmStatus,
...
type hostsReportResponse struct {
	...
	Hosts   []*mobius.HostResponse `json:"-"` // they get rendered explicitly, in csv
	Err     error                  `json:"error,omitempty"`
}
			...
			for i, rec := range recs {
				for _, col := range r.Columns {
					...
					if !ok {
						...
						endpoint_utils.EncodeError(ctx, &mobius.BadRequestError{Message: fmt.Sprintf("invalid column name: %q", col)}, w)
						return
					}
...
func hostsReportEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
	...
	if req.Format != "csv" {
		...
		err := ctxerr.Wrap(ctx, mobius.NewInvalidArgumentError("format", "unsupported or unspecified report format").
	...
	hostResps := make([]*mobius.HostResponse, len(hosts))
	for i, h := range hosts {
		hr := mobius.HostResponseForHost(ctx, svc, h)
		hostResps[i] = hr
	}
	return hostsReportResponse{Columns: cols, Hosts: hostResps}, nil
}

func (svc *Service) ListLabelsForHost(ctx context.Context, hostID uint) ([]*mobius.Label, error) {
	// require list hosts permission to view this information
	if err := svc.authz.Authorize(ctx, &mobius.Host{}, mobius.ActionList); err != nil {
		return nil, err
	}
...
type osVersionsResponse struct {
	...
	OSVersions      []mobius.OSVersion         `json:"os_versions"`
	Err             error                      `json:"error,omitempty"`
}

func (r osVersionsResponse) Error() error { return r.Err }

func osVersionsEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) OSVersions(ctx context.Context, teamID *uint, platform *string, name *string, version *string, opts mobius.ListOptions, includeCVSS bool) (*mobius.OSVersions, int, *mobius.PaginationMetadata, error) {
	var count int
	if err := svc.authz.Authorize(ctx, &mobius.Host{TeamID: teamID}, mobius.ActionList); err != nil {
	...
	if name != nil && version == nil {
		return nil, count, nil, &mobius.BadRequestError{Message: "Cannot specify os_name without os_version"}
	}

	if name == nil && version != nil {
		return nil, count, nil, &mobius.BadRequestError{Message: "Cannot specify os_version without os_name"}
	}

	if opts.OrderKey != "" && opts.OrderKey != "hosts_count" {
		return nil, count, nil, &mobius.BadRequestError{Message: "Invalid order key"}
	}
	...
	if teamID != nil {
		...
		if *teamID != 0 {
			...
			if err != nil {
				...
				return nil, count, nil, mobius.NewInvalidArgumentError("team_id", fmt.Sprintf("team %d does not exist", *teamID)).
	...
	vc, ok := viewer.FromContext(ctx)
	if !ok {
		return nil, count, nil, mobius.ErrNoContext
	}
	osVersions, err := svc.ds.OSVersions(
		ctx, &mobius.TeamFilter{
			User:            vc.User,
	...
	if opts.OrderKey == "hosts_count" && opts.OrderDirection == mobius.OrderAscending {
...
func paginateOSVersions(slice []mobius.OSVersion, opts mobius.ListOptions) ([]mobius.OSVersion, *mobius.PaginationMetadata) {
	metaData := &mobius.PaginationMetadata{
	...
	if start >= uint(len(slice)) {
		return []mobius.OSVersion{}, metaData
	}
...
type getOSVersionResponse struct {
	CountsUpdatedAt *time.Time        `json:"counts_updated_at"`
	OSVersion       *mobius.OSVersion `json:"os_version"`
	Err             error             `json:"error,omitempty"`
}

func (r getOSVersionResponse) Error() error { return r.Err }

func getOSVersionEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) OSVersion(ctx context.Context, osID uint, teamID *uint, includeCVSS bool) (*mobius.OSVersion, *time.Time, error) {
	if err := svc.authz.Authorize(ctx, &mobius.Host{TeamID: teamID}, mobius.ActionList); err != nil {
		return nil, nil, err
	}
	...
	if teamID != nil && *teamID != 0 {
		...
		if err != nil {
			...
			return nil, nil, mobius.NewInvalidArgumentError("team_id", fmt.Sprintf("team %d does not exist", *teamID)).
				WithStatus(http.StatusNotFound)
		}
	}
	...
	if err != nil {
		if mobius.IsNotFound(err) {
			// We return an empty result here to be consistent with the mobius/os_versions behavior.
...
func (svc *Service) populateOSVersionDetails(ctx context.Context, osVersion *mobius.OSVersion, includeCVSS bool) error {
	...
	osVersion.Vulnerabilities = make(mobius.Vulnerabilities, 0) // avoid null in JSON
...
type getHostEncryptionKeyResponse struct {
	Err           error                         `json:"error,omitempty"`
	EncryptionKey *mobius.HostDiskEncryptionKey `json:"encryption_key,omitempty"`
	HostID        uint                          `json:"host_id,omitempty"`
}

func (r getHostEncryptionKeyResponse) Error() error { return r.Err }

func getHostEncryptionKey(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) HostEncryptionKey(ctx context.Context, id uint) (*mobius.HostDiskEncryptionKey, error) {
	if err := svc.authz.Authorize(ctx, &mobius.Host{}, mobius.ActionList); err != nil {
		return nil, err
	}
	...
	var key *mobius.HostDiskEncryptionKey
	if host.IsLUKSSupported() {
	...
	err = svc.NewActivity(
		ctx,
		authz.UserFromContext(ctx),
		mobius.ActivityTypeReadHostDiskEncryptionKey{
			HostID:          host.ID,
...
func (svc *Service) decryptForMDMPlatform(ctx context.Context, host *mobius.Host) (*mobius.HostDiskEncryptionKey, error) {
	// Here we must check if the appropriate MDM is enabled for that particular host's platform.
	var decryptCert *tls.Certificate
	if host.MobiusPlatform() == "windows" {
...
type getHostHealthResponse struct {
	Err        error              `json:"error,omitempty"`
	HostID     uint               `json:"host_id,omitempty"`
	HostHealth *mobius.HostHealth `json:"health,omitempty"`
}

func (r getHostHealthResponse) Error() error { return r.Err }

func getHostHealthEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) GetHostHealth(ctx context.Context, id uint) (*mobius.HostHealth, error) {
	svc.authz.SkipAuthorization(ctx)
	...
	if err := svc.authz.Authorize(ctx, hh, mobius.ActionRead); err != nil {
		return nil, err
	}
...
func (svc *Service) HostLiteByIdentifier(ctx context.Context, identifier string) (*mobius.HostLite, error) {
	if err := svc.authz.Authorize(ctx, &mobius.Host{}, mobius.ActionList); err != nil {
		return nil, err
	}
	...
	if err := svc.authz.Authorize(ctx, mobius.Host{
		TeamID: host.TeamID,
...
func (svc *Service) HostLiteByID(ctx context.Context, id uint) (*mobius.HostLite, error) {
	if err := svc.authz.Authorize(ctx, &mobius.Host{}, mobius.ActionList); err != nil {
		return nil, err
	}
...
func hostListOptionsFromFilters(filter *map[string]interface{}) (*mobius.HostListOptions, *uint, error) {
	var labelID *uint
	...
	for k, v := range *filter {
		...
		case "status":
			...
			if !mobius.HostStatus(status).IsValid() {
				return nil, nil, badRequest("status must be one of: new, online, offline, missing")
			}
			opt.StatusFilter = mobius.HostStatus(status)
		case "query":
			query, ok := v.(string)
			if !ok {
...
func addLabelsToHostEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) AddLabelsToHost(ctx context.Context, id uint, labelNames []string) error {
	...
	if err := svc.authz.Authorize(ctx, host, mobius.ActionWriteHostLabel); err != nil {
...
func removeLabelsFromHostEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func getHostSoftwareEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) ListHostSoftware(ctx context.Context, hostID uint, opts mobius.HostSoftwareTitleListOptions) ([]*mobius.HostSoftwareWithInstaller, *mobius.PaginationMetadata, error) {
	...
	if !svc.authz.IsAuthenticatedWith(ctx, authzctx.AuthnDeviceToken) {
		...
		if !ok {
			return nil, nil, ctxerr.Wrap(ctx, mobius.NewAuthRequiredError("internal error: missing host from request context"))
		}
		host = h
	}

	mdmEnrolled, err := svc.ds.IsHostConnectedToMobiusMDM(ctx, host)
	if err != nil {
	...
	if len(software) > 0 {
		var titleIDs []uint
		softwareByTitleID := make(map[uint]*mobius.HostSoftwareWithInstaller)
...
func listHostCertificatesEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) ListHostCertificates(ctx context.Context, hostID uint, opts mobius.ListOptions) ([]*mobius.HostCertificatePayload, *mobius.PaginationMetadata, error) {
	if !svc.authz.IsAuthenticatedWith(ctx, authzctx.AuthnDeviceToken) {
		host, err := svc.ds.HostLite(ctx, hostID)
		if err != nil {
			svc.authz.SkipAuthorization(ctx)
	...
	payload := make([]*mobius.HostCertificatePayload, 0, len(certs))
