...
import (
	...
	"github.com/MobiusDM/mobius/server/api/server/service/middleware/authzcheck"
	"github.com/MobiusDM/mobius/server/api/server/service/middleware/ratelimit"
)
...
func DecodeQueryTagValue(r *http.Request, fp fieldPair) error {
	...
	if ok {
		...
		if queryVal == "" {
			...
			return &mobius.BadRequestError{Message: fmt.Sprintf("Param %s is required", fp.Sf.Name)}
		}
		field := fp.V
		...
		case reflect.Int:
			...
			case "order_direction", "inherited_order_direction":
				switch queryVal {
				case "desc":
					queryValInt = int(mobius.OrderDescending)
				case "asc":
					queryValInt = int(mobius.OrderAscending)
				default:
					return &mobius.BadRequestError{Message: "unknown order_direction: " + queryVal}
				}
			default:
...
// a `url` tag with value list_options it'll gather mobius.ListOptions from the
	...
	return func(ctx context.Context, r *http.Request) (interface{}, error) {
		...
		for _, fp := range fields {
			...
			if jsonExpected && nilBody {
				return nil, &mobius.BadRequestError{Message: "Expected JSON Body"}
			}
			...
			if jsonExpected && isCrossSite && !isContentJson {
				return nil, mobius.NewUserMessageError(errors.New("Expected Content-Type \"application/json\""), http.StatusUnsupportedMediaType)
			}
		...
		if !license.IsPremium(ctx) {
			for _, fp := range fields {
				if prem, ok := fp.Sf.Tag.Lookup("premium"); ok {
					...
					if val && !fp.V.IsZero() {
						return nil, &mobius.BadRequestError{Message: fmt.Sprintf(
...
type HandlerFunc func(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error)

type AndroidFunc func(ctx context.Context, request interface{}, svc android.Service) mobius.Errorer
...
type Endpointer[H HandlerFunc | AndroidFunc] interface {
	CallHandlerFunc(f H, ctx context.Context, request interface{}, svc interface{}) (mobius.Errorer, error)
	Service() interface{}
}
...
func (e *CommonEndpointer[H]) makeEndpoint(f H, v interface{}) http.Handler {
	...
	endp := e.AuthFunc(e.MobiusService, next)
...
func getNameFromPathAndVerb(verb, path, startAt string) string {
	...
	return prefix + pathReplacer.Replace(strings.TrimPrefix(strings.TrimRight(path, "/"), "/api/_version_/mobiuss/"))
}
...
func (e *CommonEndpointer[H]) HandlePathHandler(path string, pathHandler func(path string) http.Handler, verb string) {
	...
	versionedPath := strings.Replace(path, "/_version_/", fmt.Sprintf("/{mobiusversion:(?:%s)}/", strings.Join(versions, "|")), 1)
	...
	for _, alias := range e.alternativePaths {
		...
		versionedPath := strings.Replace(alias, "/_version_/", fmt.Sprintf("/{mobiusversion:(?:%s)}/", strings.Join(versions, "|")), 1)
		if e.usePathPrefix {
			e.Router.PathPrefix(versionedPath).Handler(pathHandler(versionedPath)).Name(nameAndVerb).Methods(verb)
		} else {
			e.Router.Handle(versionedPath, pathHandler(versionedPath)).Name(nameAndVerb).Methods(verb)
...
func EncodeCommonResponse(
	...
	if e, ok := response.(mobius.Errorer); ok && e.Error() != nil {
