...
import (
	...
	"github.com/MobiusDM/mobius/server/api/server/websocket"
)
...
type statsTracker struct {
	...
	stats             []statsToSave
	lastStatsEntry    *mobius.LiveQueryStats
}
...
func (svc Service) StreamCampaignResults(ctx context.Context, conn *websocket.Conn, campaignID uint) {
	...
	if err := svc.authz.Authorize(ctx, &mobius.TargetedQuery{Query: &mobius.Query{ObserverCanRun: true}}, mobius.ActionRun); err != nil {
	...
	var campaign *mobius.DistributedQueryCampaign
	go func() {
		var err error
		for {
			select {
	...
	mapHostnameRows := func(res *mobius.DistributedQueryResult) {
	...
	for {
		...
		case res := <-readChan:
			...
			switch res := res.(type) {
			case mobius.DistributedQueryResult: // includes error host responses, though they don't contribute to the "result" nor "no result" counts reported in status messages
				...
				if res.Error == nil {
					// Mobius considers a host-reported error to be neither a "result" nor "no result"
					// The Mobius UI currently tracks errors as they stream in, though the server doesn't
...
func (svc Service) addLiveQueryActivity(
	...
	activityData := mobius.ActivityTypeLiveQuery{
...
func calculateOutputSize(perfStatsTracker *statsTracker, res *mobius.DistributedQueryResult) uint64 {
	outputSize := uint64(0)
...
func (svc Service) updateStats(
	...
	if len(tracker.stats) > 0 {
		...
		statsMap := make(map[uint]*mobius.LiveQueryStats)
		...
		for _, gatheredStats := range tracker.stats {
			...
			if !ok {
				newStats := mobius.LiveQueryStats{
	...
	if aggregateStats && tracker.aggregationNeeded {
		...
		if tracker.lastStatsEntry != nil { // This check is just to be safe. It should never be nil.
			...
			stop := make(chan struct{}, 1)
			go func() {
				var stats []*mobius.LiveQueryStats
				var err error
				for {
					select {
					case <-stop:
		...
		err := svc.ds.CalculateAggregatedPerfStatsPercentiles(ctx, mobius.AggregatedStatsTypeScheduledQuery, queryID)
		if err != nil {
