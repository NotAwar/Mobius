...
import (
	...
	"github.com/MobiusDM/mobius/shared/pkg/rawjson"
...
func printLabel(c *cli.Context, label *mobius.LabelSpec) error {
	spec := specGeneric{
		Kind:    mobius.LabelKind,
...
func printQuerySpec(c *cli.Context, query *mobius.QuerySpec) error {
	spec := specGeneric{
		Kind:    mobius.QueryKind,
		Version: mobius.ApiVersion,
		Spec:    query,
	}
...
func printPack(c *cli.Context, pack *mobius.PackSpec) error {
	spec := specGeneric{
		Kind:    mobius.PackKind,
		Version: mobius.ApiVersion,
		Spec:    pack,
	}
...
func printSecret(c *cli.Context, secret *mobius.EnrollSecretSpec) error {
	spec := specGeneric{
		Kind:    mobius.EnrollSecretKind,
		Version: mobius.ApiVersion,
		Spec:    secret,
	}
...
func printHost(c *cli.Context, host *mobius.HostResponse) error {
	spec := specGeneric{
		Kind:    mobius.HostKind,
		Version: mobius.ApiVersion,
		Spec:    host,
	}
...
type enrichedAppConfigPresenter mobius.EnrichedAppConfig
...
func usersToUserRoles(users []mobius.User) UserRoles {
...
func printUserRoles(c *cli.Context, users []mobius.User) error {
	spec := specGeneric{
		Kind:    mobius.UserRolesKind,
		Version: mobius.ApiVersion,
...
func printTeams(c *cli.Context, teams []mobius.Team) error {
	for _, team := range teams {
		var teamItem interface{} = team
		if c.Bool(yamlFlagName) {
			teamSpec, err := mobius.TeamSpecFromTeam(&team)
			if err != nil {
				return err
			}
			teamItem = teamSpec
...
func queryToTableRow(query mobius.Query, teamName string) []string {
	platform := "all"
	if query.Platform != "" {
	...
	return &cli.Command{
		...
		Action: func(c *cli.Context) error {
			...
			if name == "" {
				...
				if me == nil {
					return errors.New("/api/latest/mobius/me returned an empty user")
				}
				...
				if c.Bool(yamlFlagName) || c.Bool(jsonFlagName) {
					for _, query := range queries {
						...
						if err := printQuerySpec(c, &mobius.QuerySpec{
							Name:        query.Name,
...
func userIsObserver(user mobius.User) (bool, error) {
	if user.GlobalRole != nil {
		return *user.GlobalRole == mobius.RoleObserver || *user.GlobalRole == mobius.RoleObserverPlus, nil
	} // Team user
	if len(user.Teams) == 0 {
		return false, errUserNoRoles
	}
	for _, team := range user.Teams {
		if team.Role != mobius.RoleObserver && team.Role != mobius.RoleObserverPlus {
			return false, nil
		}
	}
	return true, nil
...
func getPacksCommand() *cli.Command {
	return &cli.Command{
		...
		Action: func(c *cli.Context) error {
			...
			addQueries := func(pack *mobius.PackSpec) {
...
func getAppConfigCommand() *cli.Command {
	return &cli.Command{
		Name:  "config",
		Usage: "Retrieve the Mobius app configuration",
		Flags: []cli.Flag{
			jsonFlag(),
			yamlFlag(),
			configFlag(),
...
func getHostsCommand() *cli.Command {
	return &cli.Command{
		...
		Flags: []cli.Flag{
			...
			&cli.BoolFlag{
				Name:  "mdm",
				Usage: "Filters hosts by hosts that have MDM turned on in Mobius and are connected to Mobius's MDM server.",
			},
			&cli.BoolFlag{
				Name:  "mdm-pending",
				Usage: "Filters hosts by hosts ordered via Apple Business Manager (ABM). These will automatically enroll to Mobius and turn on MDM when they're unboxed.",
			},
		},
		Action: func(c *cli.Context) error {
			client, err := clientFromCLI(c)
			...
			if identifier == "" {
				...
				if c.Bool("mdm") || c.Bool("mdm-pending") {
					...
					if c.Bool("mdm") {
						query.Set("connected_to_mobius", "true")
					}
					if c.Bool("mdm-pending") {
						// hosts pending enrollment in Mobius's MDM server
						query.Set("mdm_name", mobius.WellKnownMDMMobius)
						query.Set("mdm_enrollment_status", string(mobius.MDMEnrollStatusPending))
					}
				}
...
func getCarvesCommand() *cli.Command {
	return &cli.Command{
		...
		Action: func(c *cli.Context) error {
			...
			carves, err := client.ListCarves(mobius.CarveListOptions{Expired: expired})
			if err != nil {
				return err
			}
...
func getTeamsYAMLFlag() cli.Flag {
	return &cli.BoolFlag{
		Name:  yamlFlagName,
		Usage: "Output team configuration in yaml format. Intended for use with \"mobiuscli apply -f\"",
	}
}
		...
		Action: func(c *cli.Context) error {
			...
			if err != nil {
				...
				if errors.As(err, &nfe) {
					log(c, "Error: No Apple Push Notification service (APNs) certificate found. Use `mobiuscli generate mdm-apple` and then `mobius serve` with `mdm` configuration to turn on MDM features.\n")
					return nil
				}
			...
			if mdm.RenewDate.Before(time.Now()) {
				...
				color.New(color.FgRed).Fprintln(c.App.Writer, "\nERROR: Your Apple Push Notification service (APNs) certificate is expired. MDM features are turned off. To renew your APNs certificate, follow these instructions: https://mobiusmdm.com/learn-more-about/renew-apns")
			} else if mdm.RenewDate.Before(warnDate) {
				// certificate will soon expire, print a warning
				color.New(color.FgYellow).Fprintln(c.App.Writer, "\nWARNING: Your Apple Push Notification service (APNs) certificate is less than 30 days from expiration. If it expires, MDM features will be turned off. To renew your APNs certificate, follow these instructions: https://mobiusmdm.com/learn-more-about/renew-apns")
			}
	...
	return &cli.Command{
		...
		Action: func(c *cli.Context) error {
			...
			if err != nil {
				var nfe service.NotFoundErr
				if errors.As(err, &nfe) {
					log(c, "Error: No Apple Business Manager server token found. Use `mobiuscli generate mdm-apple-bm` and then `mobius serve` with `mdm` configuration to automatically enroll macOS hosts to Mobius.\n")
					return nil
				}
			...
			if bm.RenewDate.Before(time.Now()) {
				// certificate is expired, print an error
				color.New(color.FgRed).Fprintln(c.App.Writer, "\nERROR: Your Apple Business Manager (ABM) server token is expired. Laptops newly purchased via ABM will not automatically enroll in Mobius. To renew your ABM server token, follow these instructions: https://mobiusmdm.com/docs/using-mobius/faq#how-can-i-renew-my-apple-business-manager-server-token")
			} else if bm.RenewDate.Before(warnDate) {
				// certificate will soon expire, print a warning
				color.New(color.FgYellow).Fprintln(c.App.Writer, "\nWARNING: Your Apple Business Manager (ABM) server token is less than 30 days from expiration. If it expires, laptops newly purchased via ABM will not automatically enroll in Mobius. To renew your ABM server token, follow these instructions: https://mobiusmdm.com/docs/using-mobius/faq#how-can-i-renew-my-apple-business-manager-server-token")
			}
	...
	return &cli.Command{
		...
		Action: func(c *cli.Context) error {
			...
			if err != nil {
				var nfe service.NotFoundErr
				if errors.As(err, &nfe) {
					return errors.New("The command doesn't exist. Please provide a valid command ID. To see a list of commands that were run, run `mobiuscli get mdm-commands`.")
				}
...
func getMDMCommandsCommand() *cli.Command {
	return &cli.Command{
		...
		Action: func(c *cli.Context) error {
			...
			opts := mobius.MDMCommandListOptions{
			...
			if err != nil {
				if strings.Contains(err.Error(), mobius.HostIdentiferNotFound) {
					return errors.New(mobius.HostIdentiferNotFound)
				}
				return err
			}
			if len(results) == 0 && opts.Filters.HostIdentifier == "" && opts.Filters.RequestType == "" {
				log(c, "You haven't run any MDM commands. Run MDM commands with the `mobiuscli mdm run-command` command.\n")
				return nil
			}
