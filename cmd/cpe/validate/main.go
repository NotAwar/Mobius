...
func checkMacOfficeNotes(vulnPath string) {
	// Iterate over each file in the vulnPath directory that starts with `mobius_macoffice`
	files, err := os.ReadDir(vulnPath)
	if err != nil {
		panic(fmt.Sprintf("failed to read directory: %v", err))
	}
	for _, file := range files {
		if strings.HasPrefix(file.Name(), "mobius_macoffice") && strings.HasSuffix(file.Name(), ".json") {
...
func checkMSRCVulnerabilities(vulnPath string) {
	// Iterate over each file in the vulnPath directory that starts with `mobius_msrc`
	files, err := os.ReadDir(vulnPath)
	if err != nil {
		panic(fmt.Sprintf("failed to read directory: %v", err))
	}
	for _, file := range files {
		if strings.HasPrefix(file.Name(), "mobius_msrc") && strings.HasSuffix(file.Name(), ".json") {
			filePath := filepath.Join(vulnPath, file.Name())
