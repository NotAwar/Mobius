...
import (
	...
	"github.com/notawar/mobius set/v4/server/service/schedule"
...
func (s *safeStore) AppConfig(ctx context.Context) (*mobius.AppConfig, error) {
	s.mu.Lock()
...
func TestMaybeSendStatistics(t *testing.T) {
	...
	mobiusConfig := config.MobiusConfig{Osquery: config.OsqueryConfig{DetailUpdateInterval: 1 * time.Hour}}
	...
	ds.AppConfigFunc = func(ctx context.Context) (*mobius.AppConfig, error) {
		return &mobius.AppConfig{ServerSettings: mobius.ServerSettings{EnableAnalytics: true}}, nil
	}

	ds.ShouldSendStatisticsFunc = func(ctx context.Context, frequency time.Duration, config config.MobiusConfig) (mobius.StatisticsPayload, bool, error) {
		return mobius.StatisticsPayload{
			...
			HostsEnrolledByOperatingSystem: map[string][]mobius.HostsCountByOSVersion{
				"linux": {
					mobius.HostsCountByOSVersion{Version: "1.2.3", NumEnrolled: 22},
				},
			},
			HostsEnrolledByOrbitVersion:   []mobius.HostsCountByOrbitVersion{},
			HostsEnrolledByOsqueryVersion: []mobius.HostsCountByOsqueryVersion{},
			StoredErrors:                  []byte(`[]`),
			Organization:                  "Mobius",
			AIFeaturesDisabled:            true,
			MaintenanceWindowsEnabled:     true,
			MaintenanceWindowsConfigured:  true,
			NumHostsMobiusDesktopEnabled:   1984,
		}, true, nil
	}
	recorded := false
...
func TestMaybeSendStatisticsSkipsSendingIfNotNeeded(t *testing.T) {
	...
	ds.ShouldSendStatisticsFunc = func(ctx context.Context, frequency time.Duration, cfg config.MobiusConfig) (mobius.StatisticsPayload, bool, error) {
...
func TestMaybeSendStatisticsSendsIfNotConfiguredForPremium(t *testing.T) {
	...
	ds.ShouldSendStatisticsFunc = func(ctx context.Context, frequency time.Duration, config config.MobiusConfig) (mobius.StatisticsPayload, bool, error) {
		return mobius.StatisticsPayload{}, true, nil
	}
...
func TestAutomationsSchedule(t *testing.T) {
	...
	ds.TeamsSummaryFunc = func(ctx context.Context) ([]*mobius.TeamSummary, error) {
		return []*mobius.TeamSummary{}, nil
	}

	ds.OutdatedAutomationBatchFunc = func(ctx context.Context) ([]mobius.PolicyFailure, error) {
		return []mobius.PolicyFailure{}, nil
	}
	...
	ds.AppConfigFunc = func(ctx context.Context) (*mobius.AppConfig, error) {
		return &mobius.AppConfig{
			WebhookSettings: mobius.WebhookSettings{
				...
				Interval: mobius.Duration{Duration: 2 * time.Second},
			},
		}, nil
	}
...
func TestCronVulnerabilitiesCreatesDatabasesPath(t *testing.T) {
	...
	ds.AppConfigFunc = func(ctx context.Context) (*mobius.AppConfig, error) {
		return &mobius.AppConfig{
			Features: mobius.Features{EnableSoftwareInventory: true},
		}, nil
	}
	ds.InsertCVEMetaFunc = func(ctx context.Context, x []mobius.CVEMeta) error {
		return nil
	}
	ds.AllSoftwareIteratorFunc = func(ctx context.Context, query mobius.SoftwareIterQueryOptions) (mobius.SoftwareIterator, error) {
...
func (f *softwareIterator) Value() (*mobius.Software, error) {
...
`
	...
	appConfig := &mobius.AppConfig{
		...
		WebhookSettings: mobius.WebhookSettings{
			VulnerabilitiesWebhook: mobius.VulnerabilitiesWebhookSettings{
				Enable:         true,
	...
	ds.ListSoftwareCPEsFunc = func(ctx context.Context) ([]mobius.SoftwareCPE, error) {
		return []mobius.SoftwareCPE{
			{
				ID:         1,
				SoftwareID: 1,
	...
	ds.InsertSoftwareVulnerabilityFunc = func(ctx context.Context, vuln mobius.SoftwareVulnerability, src mobius.VulnerabilitySource) (bool, error) {
		return true, nil
	}
	ds.UpsertSoftwareCPEsFunc = func(ctx context.Context, cpes []mobius.SoftwareCPE) (int64, error) {
		return int64(0), nil
	}
	ds.DeleteSoftwareCPEsFunc = func(ctx context.Context, cpes []mobius.SoftwareCPE) (int64, error) {
		return int64(0), nil
	}
	ds.DeleteOutOfDateVulnerabilitiesFunc = func(ctx context.Context, source mobius.VulnerabilitySource, duration time.Duration) error {
		return nil
	}
	ds.OSVersionsFunc = func(
		ctx context.Context, teamFilter *mobius.TeamFilter, platform *string, name *string, version *string,
	) (*mobius.OSVersions, error) {
		return &mobius.OSVersions{
	...
	ds.HostIDsByOSVersionFunc = func(ctx context.Context, osVersion mobius.OSVersion, offset int, limit int) ([]uint, error) {
		if offset == 0 {
			return []uint{1}, nil
		}
		return []uint{}, nil
	}
	ds.ListSoftwareForVulnDetectionFunc = func(ctx context.Context, filter mobius.VulnSoftwareFilter) ([]mobius.Software, error) {
		return []mobius.Software{
			{
				ID:               1,
				Name:             "Twisted",
				Version:          "22.2.0",
	...
	ds.ListSoftwareVulnerabilitiesByHostIDsSourceFunc = func(ctx context.Context, hostIDs []uint, source mobius.VulnerabilitySource) (map[uint][]mobius.SoftwareVulnerability, error) {
		require.Equal(t, []uint{1}, hostIDs)
		require.Equal(t, mobius.UbuntuOVALSource, source)
		return map[uint][]mobius.SoftwareVulnerability{}, nil
	}
	ds.ListOperatingSystemsFunc = func(ctx context.Context) ([]mobius.OperatingSystem, error) {
		return []mobius.OperatingSystem{
			{
				ID:            1,
				Name:          "Ubuntu",
				Version:       "22.04.1 LTS",
	...
	ds.ListOperatingSystemsForPlatformFunc = func(ctx context.Context, platform string) ([]mobius.OperatingSystem, error) {
		return []mobius.OperatingSystem{}, nil
	}

	ds.DeleteOutOfDateOSVulnerabilitiesFunc = func(ctx context.Context, src mobius.VulnerabilitySource, d time.Duration) error {
		return nil
	}

	ds.ListCVEsFunc = func(ctx context.Context, maxAge time.Duration) ([]mobius.CVEMeta, error) {
	...
	ds.HostVulnSummariesBySoftwareIDsFunc = func(ctx context.Context, softwareIDs []uint) ([]mobius.HostVulnerabilitySummary, error) {
...
func TestAutomationsScheduleLockDuration(t *testing.T) {
	...
	ds.AppConfigFunc = func(ctx context.Context) (*mobius.AppConfig, error) {
		ac := mobius.AppConfig{
			WebhookSettings: mobius.WebhookSettings{
				Interval: mobius.Duration{Duration: 1 * time.Hour},
			},
		}
		select {
		case <-intitalConfigLoaded:
			ac.WebhookSettings.Interval = mobius.Duration{Duration: expectedInterval}
		default:
			// initial config
...
func TestAutomationsScheduleIntervalChange(t *testing.T) {
	...
	ds.AppConfigFunc = func(ctx context.Context) (*mobius.AppConfig, error) {
		...
		return &mobius.AppConfig{
			WebhookSettings: mobius.WebhookSettings{
				Interval: mobius.Duration{Duration: interval.value},
			},
		}, nil
	}
...
func TestDebugMux(t *testing.T) {
	...
	cases := []struct {
		...
		{
			"only mobius auth handler, no token",
			debugMux{mobiusAuthenticatedHandler: h1},
			"",
			200,
		},
		{
			"only mobius auth handler, with token",
			debugMux{mobiusAuthenticatedHandler: h1},
			"token",
			200,
		},
		{
			...
			debugMux{mobiusAuthenticatedHandler: h1, tokenAuthenticatedHandler: h2},
			"",
			200,
		},
		{
...
func TestCronActivitiesStreaming(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		as := []*mobius.Activity{a1, a2, a3}

		ds.ListActivitiesFunc = func(ctx context.Context, opt mobius.ListActivitiesOptions) ([]*mobius.Activity, *mobius.PaginationMetadata, error) {
			return as, nil, nil
		}
	...
	t.Run("bigger_than_batch", func(t *testing.T) {
		...
		as := make([]*mobius.Activity, ActivitiesToStreamBatchCount*2+1)
		for i := range as {
...
func TestVerifyDiskEncryptionKeysJob(t *testing.T) {
	...
	assets := map[mobius.MDMAssetName]mobius.MDMConfigAsset{
		mobius.MDMAssetCACert: {Value: testCertPEM},
		mobius.MDMAssetCAKey:  {Value: testKeyPEM},
	}
	...
	t.Run("able to decrypt", func(t *testing.T) {
		ds.GetUnverifiedDiskEncryptionKeysFunc = func(ctx context.Context) ([]mobius.HostDiskEncryptionKey, error) {
	...
	t.Run("unable to decrypt", func(t *testing.T) {
		ds.GetUnverifiedDiskEncryptionKeysFunc = func(ctx context.Context) ([]mobius.HostDiskEncryptionKey, error) {
			return []mobius.HostDiskEncryptionKey{{HostID: 1, Base64Encrypted: "RANDOM"}}, nil
		}
