...
func TestGitOpsTeamSofwareInstallers(t *testing.T) {
	...
	cases := []struct {
		...
		{"testdata/gitops/team_software_installer_not_found.yml", "Please make sure that URLs are reachable from your Mobius server."},
	...
	for _, c := range cases {
		c.file = filepath.Join("../../mobiuscli", c.file)
		t.Run(filepath.Base(c.file), func(t *testing.T) {
			ds, _, _ := testing_utils.SetupFullGitOpsPremiumServer(t)
			tokExpire := time.Now().Add(time.Hour)
			token, err := test.CreateVPPTokenEncoded(tokExpire, "mobius", "ca")
			...
			_, err = mobiuscli.RunAppNoChecks([]string{"gitops", "-f", c.file})
			if c.wantErr == "" {
				require.NoError(t, err)
			} else {
...
func TestGitOpsTeamSoftwareInstallersQueryEnv(t *testing.T) {
	...
	_, err := mobiuscli.RunAppNoChecks([]string{"gitops", "-f", "../../mobiuscli/testdata/gitops/team_software_installer_valid_env_query.yml"})
	require.NoError(t, err)
}
	...
	}{
		{
			...
			vppApps: []mobius.VPPAppResponse{
				...
				},
				{
					...
					AppStoreID: "1",
					Platform:   mobius.IPadOSPlatform,
				},
			},
		},
	}
	for _, c := range cases {
		c.noTeamFile = filepath.Join("../../mobiuscli", c.noTeamFile)
		t.Run(filepath.Base(c.noTeamFile), func(t *testing.T) {
			...
			globalFile := "../../mobiuscli/testdata/gitops/global_config_no_paths.yml"
			...
			require.NoError(t, err)
			_, err = mobiuscli.RunAppNoChecks([]string{"gitops", "-f", globalFile, "-f", dstPath})
			if c.wantErr == "" {
				require.NoError(t, err)
			} else {
				require.ErrorContains(t, err, c.wantErr)
...
func TestGitOpsNoTeamSoftwareInstallers(t *testing.T) {
	...
	cases := []struct {
		...
		{"testdata/gitops/no_team_software_installer_not_found.yml", "Please make sure that URLs are reachable from your Mobius server."},
	...
	for _, c := range cases {
		...
		t.Run(filepath.Base(c.noTeamFile), func(t *testing.T) {
			...
			if strings.HasPrefix(filepath.Base(c.noTeamFile), "no_team_setup_software") {
				...
				globalFile = "../../mobiuscli/testdata/gitops/global_config_no_paths_no_controls.yml"
			}
			dstPath := filepath.Join(filepath.Dir(c.noTeamFile), "no-team.yml")
			t.Cleanup(func() {
				os.Remove(dstPath)
...
func TestGitOpsTeamVPPApps(t *testing.T) {
	...
	for _, c := range cases {
		...
		t.Run(filepath.Base(c.file), func(t *testing.T) {
			ds, _, _ := testing_utils.SetupFullGitOpsPremiumServer(t)
			token, err := test.CreateVPPTokenEncoded(c.tokenExpiration, "mobius", "ca")
			require.NoError(t, err)
...
func TestGitOpsTeamVPPAndApp(t *testing.T) {
	...
	token, err := test.CreateVPPTokenEncoded(renewDate, "mobius", "ca")
	require.NoError(t, err)
	...
	tokensByTeams := make(map[uint]*mobius.VPPTokenDB)
	...
	buf, err := mobiuscli.RunAppNoChecks([]string{
		"gitops", "-f", "../../mobiuscli/testdata/gitops/global_config_vpp.yml", "-f",
		"../../mobiuscli/testdata/gitops/team_vpp_valid_app.yml",
	})
	require.NoError(t, err)
	...
	assert.Contains(t, buf.String(), fmt.Sprintf(mobiuscli.ReapplyingTeamForVPPAppsMsg, teamName))
}
...
`, name, name)
	...
	}{
		{
			...
			realRunAssertion: func(t *testing.T, appCfg *mobius.AppConfig, ds mobius.Datastore, out string, err error) {
				...
				assert.ElementsMatch(
					...
					[]mobius.MDMAppleVolumePurchasingProgramInfo{
						{
							Location: "Mobius Device Management Inc.",
							Teams: []string{
								"💻 Workstations",
	...
	for _, tt := range cases {
		t.Run(tt.name, func(t *testing.T) {
			...
			ds.ListVPPTokensFunc = func(ctx context.Context) ([]*mobius.VPPTokenDB, error) {
				return []*mobius.VPPTokenDB{{Location: "Mobius Device Management Inc."}, {Location: "Acme Inc."}}, nil
			}
			...
			// Dry run
			out, err := mobiuscli.RunAppNoChecks(append(args, "--dry-run"))
			...
			// Real run
			out, err = mobiuscli.RunAppNoChecks(args)
