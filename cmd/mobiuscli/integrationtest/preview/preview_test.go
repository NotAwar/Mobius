...
func TestPreviewFailsOnInvalidLicenseKey(t *testing.T) {
	_, err := mobiuscli.RunAppNoChecks([]string{"preview", "--license-key", "0xDEADBEEF"})
...
func TestIntegrationsPreview(t *testing.T) {
	...
	t.Setenv("MOBIUS_SERVER_ADDRESS", "https://localhost:8412")
	mobiuscli.TestOverridePreviewDirectory = t.TempDir()
	configPath := filepath.Join(t.TempDir(), "config")
	t.Log("config path: ", configPath)

	t.Cleanup(func() {
		require.Equal(t, "", mobiuscli.RunAppForTest(t, []string{"preview", "--config", configPath, "stop"}))
	})

	var output *bytes.Buffer
	require.NoError(t, nettest.RunWithNetRetry(t, func() error {
		var err error
		output, err = mobiuscli.RunAppNoChecks([]string{
			"preview",
			"--config", configPath,
	...
	queries := mobiuscli.RunAppForTest(t, []string{"get", "queries", "--config", configPath, "--json"})
	n := strings.Count(queries, `"kind":"query"`)
	require.Greater(t, n, 10)

	// app configuration must disable analytics
	appConf := mobiuscli.RunAppForTest(t, []string{"get", "config", "--include-server-config", "--config", configPath, "--yaml"})
