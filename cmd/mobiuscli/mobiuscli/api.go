...
func clientFromCLI(c *cli.Context) (*service.Client, error) {
	mobiusClient, err := unauthenticatedClientFromCLI(c)
	if err != nil {
		return nil, err
	}
	...
	if !c.IsSet("config") && flag.Lookup("test.v") != nil {
		mobiusClient.SetToken("AAAA")
		return mobiusClient, nil
	}
	...
	token, ok := t.(string)
	if !ok {
		fmt.Fprintln(os.Stderr, "Token invalid. Please log in with: mobiuscli login")
		return nil, fmt.Errorf("token config value expected type %T, got %T: %+v", "", t, t)
	}
	if token == "" {
		fmt.Fprintln(os.Stderr, "Token missing. Please log in with: mobiuscli login")
		return nil, errors.New("token config value missing")
	}
	mobiusClient.SetToken(token)

	// Check if version matches mobius server. Also ensures that the token is valid.
	clientInfo := version.Version()

	serverInfo, err := mobiusClient.Version()
	if err != nil {
		if errors.Is(err, service.ErrUnauthenticated) {
			fmt.Fprintln(os.Stderr, "Token invalid or session expired. Please log in with: mobiuscli login")
		}
		return nil, err
	}
	...
	switch appCfg, err := mobiusClient.GetAppConfig(); {
	case err == nil:
		if appCfg.MDM.AppleBMTermsExpired {
			mobius.WriteAppleBMTermsExpiredBanner(os.Stderr)
			// This is just a warning, continue ...
		}
...
func unauthenticatedClientFromConfig(cc Context, debug bool, outputWriter io.Writer, errWriter io.Writer) (*service.Client, error) {
	...
	if flag.Lookup("test.v") != nil {
		return service.NewClient(
			os.Getenv("MOBIUS_SERVER_ADDRESS"), true, "", "", options...)
	}

	if cc.Address == "" {
		return nil, errors.New("set the Mobius API address with: mobiuscli config set --address https://localhost:8080")
	}
	...
	if err != nil {
		return nil, fmt.Errorf("error creating Mobius API client handler: %w", err)
	}
...
func clientConfigFromCLI(c *cli.Context) (Context, error) {
	...
	if !c.IsSet("config") && flag.Lookup("test.v") != nil {
		return Context{
			Address:       os.Getenv("MOBIUS_SERVER_ADDRESS"),
			TLSSkipVerify: true,
		}, nil
	}
...
// apiCommand mobiuscli api [options] uri
// -F, --field <key=value>
...
func apiCommand() *cli.Command {
	...
	return &cli.Command{
		...
		UsageText: `mobiuscli api [options] [url]`,
		Flags: []cli.Flag{
			&cli.StringSliceFlag{
				Name:    "F",
		...
		Action: func(c *cli.Context) error {
			...
			if !strings.HasPrefix(uriString, "/api/v1/mobius") {
				uriString = fmt.Sprintf("/api/v1/mobius%s", uriString)
			}
			...
			resp, err := mobiusClient.AuthenticatedDoCustomHeaders(method, uriString, params.Encode(), nil, headers)
			if err != nil {
				return err
			}
