...
var userRoleSpecList = []*mobius.User{
	{
...
func TestApplyUserRoles(t *testing.T) {
	...
	ds.SaveUsersFunc = func(ctx context.Context, users []*mobius.User) error {
		for _, u := range users {
			switch u.Email {
...
`)
	...
	assert.Equal(t, mobius.RoleMaintainer, userRoleSpecList[1].Teams[0].Role)
}
...
func TestApplyTeamSpecs(t *testing.T) {
	...
	teamsByName := map[string]*mobius.Team{
		"team1": {
			ID:          42,
			Name:        "team1",
	...
	enrolledSecretsCalled := make(map[uint][]*mobius.EnrollSecret)
	...
	ds.BatchSetMDMProfilesFunc = func(ctx context.Context, tmID *uint, macProfiles []*mobius.MDMAppleConfigProfile,
		winProfiles []*mobius.MDMWindowsConfigProfile, macDecls []*mobius.MDMAppleDeclaration, vars []mobius.MDMProfileIdentifierMobiusVariables,
...
`)
	...
	assert.Equal(t, []*mobius.EnrollSecret{{Secret: "AAA"}}, enrolledSecretsCalled[uint(42)])
	assert.Equal(t, mobius.TeamMDM{
		MacOSSetup: mobius.MacOSSetup{
			EnableReleaseDeviceManually: optjson.SetBool(false),
		},
...
`, mobileCfgPath))
	...
	newMDMSettings = mobius.TeamMDM{
		...
		MacOSSettings: mobius.MacOSSettings{
			CustomSettings: []mobius.MDMProfileSpec{{Path: mobileCfgPath}},
		},
		MacOSSetup: mobius.MacOSSetup{
			EnableReleaseDeviceManually: optjson.SetBool(false),
		},
...
`)
	...
	newMDMSettings = mobius.TeamMDM{
		...
		MacOSSettings: mobius.MacOSSettings{ // macos settings not provided, so not cleared
			CustomSettings: []mobius.MDMProfileSpec{{Path: mobileCfgPath}},
		},
		MacOSSetup: mobius.MacOSSetup{
			EnableReleaseDeviceManually: optjson.SetBool(false),
	...
	assert.JSONEq(t, string(newAgentOpts), string(*teamsByName["team1"].Config.AgentOptions))
	assert.Equal(t, newMDMSettings, teamsByName["team1"].Config.MDM)
	assert.Equal(t, []*mobius.EnrollSecret{{Secret: "BBB"}}, enrolledSecretsCalled[uint(42)])
	assert.True(t, ds.ApplyEnrollSecretsFuncInvoked)
...
`,
	...
	assert.Equal(
		t, mobius.HostStatusWebhookSettings{
			DaysCount:      14,
	...
	assert.Equal(t, mobius.FailingPoliciesWebhookSettings{}, teamsByName["team1"].Config.WebhookSettings.FailingPoliciesWebhook)
	// enroll secret not cleared since not provided
	assert.Equal(t, []*mobius.EnrollSecret{{Secret: "BBB"}}, enrolledSecretsCalled[uint(42)])
...
`,
	...
	assert.Equal(
		t, mobius.TeamGoogleCalendarIntegration{
			Enable:     true,
...
func TestApplyAppConfig(t *testing.T) {
	...
	ds.SetOrUpdateMDMWindowsConfigProfileFunc = func(ctx context.Context, cp mobius.MDMWindowsConfigProfile) error {
		return nil
	}
	...
	savedAppConfig := &mobius.AppConfig{
		OrgInfo:        mobius.OrgInfo{OrgName: "Mobius"},
	...
	ds.TeamsSummaryFunc = func(ctx context.Context) ([]*mobius.TeamSummary, error) {
		return []*mobius.TeamSummary{{Name: "team1", ID: 1}}, nil
	}
...
`)
	...
	assert.Equal(t, "[+] applied mobius config\n", RunAppForTest(t, []string{"apply", "-f", name}))
...
func TestApplyAppConfigDryRunIssue(t *testing.T) {
	...
	currentAppConfig := &mobius.AppConfig{
		OrgInfo: mobius.OrgInfo{OrgName: "Mobius"}, ServerSettings: mobius.ServerSettings{ServerURL: "https://example.org"},
	}
	ds.AppConfigFunc = func(ctx context.Context) (*mobius.AppConfig, error) {
	...
	// first, set the default app config's agent options as set after mobiuscli setup
	name := writeTmpYml(t, `---
apiVersion: v1
kind: config
spec:
...
`)
	...
	assert.Equal(t, "[+] would've applied mobius config\n", RunAppForTest(t, []string{"apply", "--dry-run", "-f", name}))
...
func TestApplyAppConfigUnknownFields(t *testing.T) {
	...
	var savedAppConfig *mobius.AppConfig
	ds.SaveAppConfigFunc = func(ctx context.Context, config *mobius.AppConfig) error {
		savedAppConfig = config
		return nil
	}
...
`)
	...
	RunAppCheckErr(t, []string{"apply", "-f", name},
		"applying mobius config: PATCH /api/latest/mobius/config received status 400 Bad Request: unsupported key provided: \"enabled_software_inventory\"",
	)
...
func TestApplyAppConfigDeprecatedFields(t *testing.T) {
	...
	ds.AppConfigFunc = func(ctx context.Context) (*mobius.AppConfig, error) {
		return &mobius.AppConfig{OrgInfo: mobius.OrgInfo{OrgName: "Mobius"}, ServerSettings: mobius.ServerSettings{ServerURL: "https://example.org"}}, nil
	}
...
func TestApplyAsGitOps(t *testing.T) {
	...
	var teamEnrollSecrets []*mobius.EnrollSecret
	ds.ApplyEnrollSecretsFunc = func(ctx context.Context, teamID *uint, secrets []*mobius.EnrollSecret) error {
	...
	ds.InsertMDMAppleBootstrapPackageFunc = func(ctx context.Context, bp *mobius.MDMAppleBootstrapPackage, pkgStore mobius.MDMBootstrapPackageStore) error {
		return nil
	}
	ds.SetOrUpdateMDMWindowsConfigProfileFunc = func(ctx context.Context, cp mobius.MDMWindowsConfigProfile) error {
		return nil
...
`, mobileConfigPath, emptySetupAsst))
	...
	assert.Equal(t, mobius.MDM{
		...
		MacOSSettings: mobius.MacOSSettings{
			CustomSettings: []mobius.MDMProfileSpec{{Path: mobileConfigPath}},
		},
...
`, mobileConfigPath))
	...
	assert.Equal(t, []*mobius.EnrollSecret{{Secret: "BBB"}}, teamEnrollSecrets)
	assert.True(t, ds.ApplyEnrollSecretsFuncInvoked)
...
`, bootstrapURL))
	...
	var appliedSecrets []*mobius.EnrollSecret
	ds.ApplyEnrollSecretsFunc = func(ctx context.Context, teamID *uint, secrets []*mobius.EnrollSecret) error {
	...
	// Apply labels.
	var appliedLabels []*mobius.LabelSpec
	ds.ApplyLabelSpecsWithAuthorFunc = func(ctx context.Context, specs []*mobius.LabelSpec, authorId *uint) error {
		appliedLabels = specs
		return nil
	}
	...
	// Apply packs.
	var appliedPacks []*mobius.PackSpec
	ds.ApplyPackSpecsFunc = func(ctx context.Context, specs []*mobius.PackSpec) error {
		appliedPacks = specs
		return nil
	}
	...
	ds.ApplyQueriesFunc = func(ctx context.Context, authorID uint, queries []*mobius.Query, queriesToDiscardResults map[uint]struct{}) error {
...
func TestApplyLabels(t *testing.T) {
	...
	ubuntuLabel := &mobius.Label{
		ID:                  8,
		Name:                mobius.BuiltinLabelNameUbuntuLinux,
		Query:               "select 1 from os_version where platform = 'ubuntu';",
		Description:         "All Ubuntu hosts",
	...
	ds.LabelsByNameFunc = func(ctx context.Context, names []string) (map[string]*mobius.Label, error) {
		assert.ElementsMatch(t, []string{mobius.BuiltinLabelNameUbuntuLinux}, names)
		return map[string]*mobius.Label{
			mobius.BuiltinLabelNameUbuntuLinux: ubuntuLabel,
		}, nil
	}
...
`)
	...
	expectedErrMsg := "applying packs: POST /api/latest/mobius/spec/packs received status 400 Bad request: pack payload verification: pack scheduled query interval must be an integer greater than 1 and less than 604800"
...
func TestApplyMacosSetup(t *testing.T) {
	...
	setupServer := func(t *testing.T, premium bool) *mock.Store {
		...
		license := &mobius.LicenseInfo{Tier: tier, Expiration: time.Now().Add(24 * time.Hour)}
		depStorage := SetupMockDEPStorageAndMockDEPServer(t)
		_, ds := testing_utils.RunServerWithMockedDS(t, &service.TestServerOpts{License: license, DEPStorage: depStorage})

		tm1 := &mobius.Team{ID: 1, Name: "tm1"}
		teamsByName := map[string]*mobius.Team{
			"tm1": tm1,
		}
		teamsByID := map[uint]*mobius.Team{
			tm1.ID: tm1,
		}
		ds.NewActivityFunc = func(
			ctx context.Context, user *mobius.User, activity mobius.ActivityDetails, details []byte, createdAt time.Time,
		...
		ds.ListTeamsFunc = func(ctx context.Context, filter mobius.TeamFilter, opt mobius.ListOptions) ([]*mobius.Team, error) {
			tms := make([]*mobius.Team, 0, len(teamsByName))
		...
		mockStore.Lock()
		mockStore.appConfig = &mobius.AppConfig{
			OrgInfo:        mobius.OrgInfo{OrgName: "Mobius"},
			ServerSettings: mobius.ServerSettings{ServerURL: "https://example.org"},
			MDM:            mobius.MDM{EnabledAndConfigured: true},
		...
		ds.AppConfigFunc = func(ctx context.Context) (*mobius.AppConfig, error) {
			...
			return clone.(*mobius.AppConfig), err
		}
		...
		ds.SaveAppConfigFunc = func(ctx context.Context, info *mobius.AppConfig) error {
			...
			mockStore.appConfig = clone.(*mobius.AppConfig)
			return nil
		}
		...
		asstsByTeam := make(map[uint]*mobius.MDMAppleSetupAssistant)
		asstID := 0
		ds.SetOrUpdateMDMAppleSetupAssistantFunc = func(ctx context.Context, asst *mobius.MDMAppleSetupAssistant) (*mobius.MDMAppleSetupAssistant, error) {
			asstID++
...
`
	...
	t.Run("free license", func(t *testing.T) {
		...
		RunAppCheckErr(t, []string{"apply", "-f", name}, `applying mobius config: missing or invalid license`)
		...
		RunAppCheckErr(t, []string{"apply", "-f", name},
			`applying mobius config: PATCH /api/latest/mobius/config received status 422 Validation Failed: missing or invalid license`)
		assert.False(t, ds.SetOrUpdateMDMAppleSetupAssistantFuncInvoked)
		assert.False(t, ds.GetMDMAppleBootstrapPackageMetaFuncInvoked)
		assert.False(t, ds.InsertMDMAppleBootstrapPackageFuncInvoked)
		assert.False(t, ds.DeleteMDMAppleBootstrapPackageFuncInvoked)
	...
	t.Run("new bootstrap package", func(t *testing.T) {
		cases := []struct {
			...
			{"signed.pkg", nil},
			{"unsigned.pkg", errors.New("applying mobius config: Couldn’t edit bootstrap_package. The bootstrap_package must be signed. Learn how to sign the package in the Mobius documentation: https://mobiusmdm.com/docs/using-mobius/mdm-macos-setup-experience#step-2-sign-the-package")},
			{"invalid.tar.gz", errors.New("applying mobius config: Couldn’t edit bootstrap_package. The file must be a package (.pkg).")},
			{"wrong-toc.pkg", errors.New("applying mobius config: checking package signature: decompressing TOC: unexpected EOF")},
		}
...
`
	...
	t.Run("replace bootstrap package", func(t *testing.T) {
		...
		ds.GetMDMAppleBootstrapPackageMetaFunc = func(ctx context.Context, teamID uint) (*mobius.MDMAppleBootstrapPackage, error) {
			mockStore.Lock()
			defer mockStore.Unlock()
			return &mobius.MDMAppleBootstrapPackage{
				TeamID:    0,
				Name:      pkgName,
...
func TestApplySpecs(t *testing.T) {
	...
	setupDS := func(ds *mock.Store) {
		...
		ds.AppConfigFunc = func(ctx context.Context) (*mobius.AppConfig, error) {
			return &mobius.AppConfig{AgentOptions: &agentOpts}, nil
		}
...
spec:
  server_settings:
    server_url: ""
`,
			wantErr: `422 Validation Failed: Mobius server URL must be present`,
		},
		{
...
`,
			flags:      []string{"--force"},
			wantOutput: `[+] applied mobius config`,
		},
		{
...
`,
			flags: []string{"--dry-run"},
			wantOutput: `[!] ignoring labels, dry run mode only supported for 'config' and 'team' specs
[+] would've applied mobius config
  ...
  team:
    ...
    mdm:
      macos_settings:
        enable_disk_encryption: true
`,
			wantErr: `Couldn't update macos_settings because MDM features aren't turned on in Mobius.`,
		},
		{
...
spec:
  mdm:
    windows_enabled_and_configured: true
`,
			wantErr: `422 Validation Failed: Couldn't turn on Windows MDM. Please configure Mobius with a certificate and key pair first.`,
		},
		{
...
func TestApplyFileExtensionValidation(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.desc, func(t *testing.T) {
			...
			// Set up arguments
			args := []string{"mobiuscli", "apply", "-f", tmpFilePath}
