...
func configFlag() cli.Flag {
	...
	defaultConfigPath := filepath.Join(homeDir, ".mobius", "config")
	return &cli.StringFlag{
		Name:    "config",
		Value:   defaultConfigPath,
		EnvVars: []string{"CONFIG"},
		Usage:   "Path to the mobiuscli config file",
	}
}
...
func contextFlag() cli.Flag {
	return &cli.StringFlag{
		...
		Usage:   "Name of mobiuscli config context to use",
	}
}
...
func configSetCommand() *cli.Command {
	...
	return &cli.Command{
		...
		UsageText: `mobiuscli config set [options]`,
		Flags: []cli.Flag{
			configFlag(),
			contextFlag(),
			&cli.StringFlag{
				...
				Usage:       "Address of the Mobius server",
			},
			&cli.StringFlag{
				Name:        "email",
				...
				Value:       "",
				Destination: &flEmail,
				Usage:       "Email to use when connecting to the Mobius server",
			},
			&cli.StringFlag{
				Name:        "token",
			...
			&cli.StringFlag{
				...
				Usage:       "Specify RootCA chain used to communicate with Mobius",
			},
			&cli.StringFlag{
				Name:        "url-prefix",
				...
				Usage:       "Specify URL Prefix to use with Mobius server (copy from server configuration)",
			},
			&cli.StringSliceFlag{
				Name:        "custom-header",
...
func configGetCommand() *cli.Command {
	return &cli.Command{
		...
		UsageText: `mobiuscli config get [options]`,
		Flags: []cli.Flag{
			configFlag(),
			contextFlag(),
		},
