...
func TestCustomHeadersConfig(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		called = true
		require.Equal(t, "custom", r.Header.Get("X-Mobius-Test"))
		require.Equal(t, "another", r.Header.Get("X-Mobius-MoreTest"))
	...
	RunAppForTest(t, []string{
		...
		"--config", configFile,
		"--token", "abcd",
		"--custom-header", "X-Mobius-Test:custom",
		"--custom-header", "X-Mobius-MoreTest:another",
		"--address", srv.URL,
	})
