...
func debugCommand() *cli.Command {
	return &cli.Command{
		Name:  "debug",
		Usage: "Tools for debugging Mobius",
		Flags: []cli.Flag{
			configFlag(),
			contextFlag(),
			debugFlag(),
...
func outfileName(name string) string {
	return fmt.Sprintf("mobius-%s-%s", name, nowFn().Format("20060102150405Z"))
}
...
func debugProfileCommand() *cli.Command {
	return &cli.Command{
		Name:      "profile",
		Usage:     "Record a CPU profile from the Mobius server.",
		...
		Action: func(c *cli.Context) error {
			...
			profile, err := mobius.DebugPprof("profile")
			if err != nil {
				return err
			}
...
func debugCmdlineCommand() *cli.Command {
	return &cli.Command{
		Name:  "cmdline",
		Usage: "Get the command line used to invoke the Mobius server.",
		Flags: []cli.Flag{
			outfileFlag(),
			configFlag(),
			contextFlag(),
		...
		Action: func(c *cli.Context) error {
			...
			cmdline, err := mobius.DebugPprof("cmdline")
			if err != nil {
				return err
			}
...
func debugHeapCommand() *cli.Command {
	...
	return &cli.Command{
		Name:      name,
		Usage:     "Report the allocated memory in the Mobius server.",
		...
		Action: func(c *cli.Context) error {
			...
			profile, err := mobius.DebugPprof(name)
			if err != nil {
				return err
			}
...
func debugGoroutineCommand() *cli.Command {
	...
	return &cli.Command{
		Name:      name,
		Usage:     "Get stack traces of all goroutines (threads) in the Mobius server.",
...
func debugTraceCommand() *cli.Command {
	...
	return &cli.Command{
		Name:      name,
		Usage:     "Record an execution trace on the Mobius server.",
		...
		Action: func(c *cli.Context) error {
			...
			for _, profile := range profiles {
				...
				case "errors":
					...
					ext = jsonExtension
					err = mobius.DebugErrors(&buf, false)
					if err == nil {
						res = buf.Bytes()
					}
				...
				case "db-locks":
					ext = jsonExtension
					res, err = mobius.DebugDBLocks()
				case "db-innodb-status":
					ext = jsonExtension
					res, err = mobius.DebugInnoDBStatus()
				case "db-process-list":
					ext = jsonExtension
					res, err = mobius.DebugProcessList()

				default:
					ext = profileExtension
					res, err = mobius.DebugPprof(profile)
				}
...
func debugConnectionCommand() *cli.Command {
	...
	return &cli.Command{
		Name:      "connection",
		ArgsUsage: "[<address>]",
		Usage:     "Investigate the cause of a connection failure to the Mobius server.",
		Description: `Run a number of checks to debug a connection failure to the Mobius
server.
...
the default context used if none is explicitly specified.`,
		...
		Action: func(c *cli.Context) error {
			...
			} else if cert := getMobiusCertificate(c); cert != "" {
				return errors.New("the --mobius-certificate flag can only be set when an <address> is provided")
			}
			...
			if cc.Address == "" {
				return errors.New(`set the Mobius API address with: mobiuscli config set --address https://localhost:8080
or provide an <address> argument to debug: mobiuscli debug connection localhost:8080`)
			}
			...
			//
			// Scenarios:
			// 	- If a --mobius-certificate is provided, use it as root CA.
			// 	- If a --mobius-certificate is not provided, but a cc.RootCA is set in the configuration, use it as root CA.
			// 	- If a --mobius-certificate is not provided and there isn't a cc.RootCA set in the configuration, use the embedded certs as root CA.
			//
			usingEmbeddedCA := false
			if usingHTTPS {
				certPath := getMobiusCertificate(c)
				if certPath != "" {
					// if a certificate is provided, use it as root CA
					cc.RootCA = certPath
					cc.TLSSkipVerify = false
				} else if cc.RootCA == "" { // --mobius-certificate is not set
					// If a certificate is not provided and a cc.RootCA is not set in the configuration,
					// then use the embedded root CA which is used by osquery to connect to Mobius.
			...
			if usingHTTPS {
				rootCA := cc.RootCA
				if usingEmbeddedCA {
					rootCA += " (embedded certs used by default to generate mobiusdaemon packages)"
				}
			...
			if usingHTTPS {
				if err := checkMobiusCert(c.Context, timeoutPerCheck, cc.RootCA, baseURL.Host); err != nil {
...
func debugMigrations() *cli.Command {
	return &cli.Command{
		...
		Description: `Run a check for database migrations on the mobius server.

It returns the list of migrations that are missing.
Such migrations can be applied via "mobius prepare db" before running "mobius serve".
`,
		Flags: []cli.Flag{
			configFlag(),
			contextFlag(),
		...
		Action: func(c *cli.Context) error {
			...
			case mobius.NoMigrationsCompleted:
				...
				fmt.Println("Your Mobius database is not initialized. Mobius cannot start up.\n" +
					"Mobius server must be run with \"prepare db\" to perform the migrations.")
...
func debugErrorsCommand() *cli.Command {
	...
	return &cli.Command{
		Name:      name,
		Usage:     "Save the recorded mobius server errors to a file.",
		UsageText: "Recording of errors and their retention period is controlled via the --logging_error_retention_period mobius command flag.",
		Flags: []cli.Flag{
			outfileFlag(),
			configFlag(),
			contextFlag(),
		...
		Action: func(c *cli.Context) error {
			...
			if err := mobius.DebugErrors(out, flush); err != nil {
				return err
			}
...
func checkMobiusCert(ctx context.Context, timeout time.Duration, certPath, addr string) error {
	ctx, cancel := context.WithTimeout(ctx, timeout)
	defer cancel()
