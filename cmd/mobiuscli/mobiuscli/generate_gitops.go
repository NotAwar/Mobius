...
type generateGitopsClient interface {
	GetAppConfig() (*mobius.EnrichedAppConfig, error)
	GetEnrollSecretSpec() (*mobius.EnrollSecretSpec, error)
	ListTeams(query string) ([]mobius.Team, error)
	ListScripts(query string) ([]*mobius.Script, error)
	ListConfigurationProfiles(teamID *uint) ([]*mobius.MDMConfigProfilePayload, error)
	GetScriptContents(scriptID uint) ([]byte, error)
	GetProfileContents(profileID string) ([]byte, error)
	GetTeam(teamID uint) (*mobius.Team, error)
	ListSoftwareTitles(query string) ([]mobius.SoftwareTitleListResult, error)
	GetSoftwareTitleByID(ID uint, teamID *uint) (*mobius.SoftwareTitle, error)
	GetPolicies(teamID *uint) ([]*mobius.Policy, error)
	GetQueries(teamID *uint, name *string) ([]mobius.Query, error)
	GetLabels() ([]*mobius.LabelSpec, error)
	Me() (*mobius.User, error)
}
...
func generateGitopsCommand() *cli.Command {
	return &cli.Command{
		...
		Usage:       "Generate GitOps configuration files for Mobius.",
		Description: "This command generates GitOps configuration files for Mobius.",
...
// Create the action for the generate-gitops command, using a provided mobiusClient.
func createGenerateGitopsAction(mobiusClient generateGitopsClient) func(*cli.Context) error {
...
func (cmd *GenerateGitopsCommand) Run() error {
	...
	if me.GlobalRole != nil && *me.GlobalRole != mobius.RoleAdmin {
...
func generateProfileFilename(profile *mobius.MDMConfigProfilePayload, profileContentsString string) string {
...
func (cmd *GenerateGitopsCommand) generateSSOSettings(ssoSettings *mobius.SSOSettings) (map[string]interface{}, error) {
	t := reflect.TypeOf(mobius.SSOSettings{})
...
type GlobalOrTeamIntegrations struct {
	GlobalIntegrations *mobius.Integrations     `json:"global_integrations,omitempty"`
	TeamIntegrations   *mobius.TeamIntegrations `json:"team_integrations,omitempty"`
}
...
func (cmd *GenerateGitopsCommand) generateMDM(mdm *mobius.MDM) (map[string]interface{}, error) {
	t := reflect.TypeOf(mobius.MDM{})
	result := map[string]interface{}{
	...
	if !cmd.CLI.Bool("insecure") {
		if auth, ok := result[jsonFieldName(t, "EndUserAuthentication")]; ok {
			endUserAuth := auth.(mobius.MDMEndUserAuthentication)
...
func (cmd *GenerateGitopsCommand) generateControls(teamId *uint, teamName string, teamMdm *mobius.TeamMDM) (map[string]interface{}, error) {
	...
	if teamMdm != nil && cmd.AppConfig.License.IsPremium() {
		mdmT := reflect.TypeOf(mobius.TeamMDM{})
		...
		if teamId == nil || *teamId == 0 {
			mdmT := reflect.TypeOf(mobius.MDM{})
		...
		if teamMdm.MacOSSetup.BootstrapPackage.Value != "" || teamMdm.MacOSSetup.EnableEndUserAuthentication || teamMdm.MacOSSetup.MacOSSetupAssistant.Value != "" || teamMdm.MacOSSetup.Script.Value != "" || (teamMdm.MacOSSetup.Software.Valid && len(teamMdm.MacOSSetup.Software.Value) > 0) {
			...
			cmd.Messages.Notes = append(cmd.Messages.Notes, Note{
				Filename: teamName,
				Note:     "The macos_setup configuration is not supported by this tool yet.  To configure it, please follow the Mobius documentation at https://mobiusmdm.com/docs/configuration/yaml-files#macos-setup",
			})
		}
	}
...
func (cmd *GenerateGitopsCommand) generateProfiles(teamId *uint, teamName string) (map[string]interface{}, error) {
	...
	if err != nil {
		if strings.Contains(err.Error(), mobius.MDMNotConfiguredMessage) {
			return nil, nil
		}
...
func (cmd *GenerateGitopsCommand) generatePolicies(teamId *uint, filePath string) ([]map[string]interface{}, error) {
	...
	t := reflect.TypeOf(mobius.Policy{})
...
func (cmd *GenerateGitopsCommand) generateLabels() ([]map[string]interface{}, error) {
	...
	t := reflect.TypeOf(mobius.LabelSpec{})
	result := make([]map[string]interface{}, 0)
	for _, label := range labels {
		if label.LabelType != mobius.LabelTypeRegular {
			continue
		}
		...
		if label.LabelMembershipType == mobius.LabelMembershipTypeDynamic {
