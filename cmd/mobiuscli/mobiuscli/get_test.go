...
import (
	...
	"github.com/notawar/mobius set/v4/pkg/spec"
...
var setCurrentUserSession = func(t *testing.T, ds *mock.Store, user *mobius.User) {
...
func TestGetTeams(t *testing.T) {
	...
	mobius.WriteExpiredLicenseBanner(&expiredBanner)
	require.Contains(t, expiredBanner.String(), "Your license for Mobius Premium is about to expire")
	...
	testCases := []struct {
		...
		{
			"not expired license",
			&mobius.LicenseInfo{Tier: mobius.TierPremium, Expiration: time.Now().Add(24 * time.Hour)},
			false,
		},
		{
			"expired license",
...
func TestGetTeamsByName(t *testing.T) {
	_, ds := testing_utils.RunServerWithMockedDS(t,
		&service.TestServerOpts{License: &mobius.LicenseInfo{Tier: mobius.TierPremium, Expiration: time.Now().Add(24 * time.Hour)}})
...
func TestGetHosts(t *testing.T) {
	...
	// this func is called when no host is specified i.e. `mobiuscli get hosts --json`
	ds.ListHostsFunc = func(ctx context.Context, filter mobius.TeamFilter, opt mobius.HostListOptions) ([]*mobius.Host, error) {
		...
		hosts := []*mobius.Host{
			{
				UpdateCreateTimestamps: mobius.UpdateCreateTimestamps{
					CreateTimestamp: mobius.CreateTimestamp{CreatedAt: time.Time{}},
					UpdateTimestamp: mobius.UpdateTimestamp{UpdatedAt: time.Time{}},
				},
				HostSoftware:    mobius.HostSoftware{},
	...
	// these are run when host is specified `mobiuscli get hosts --json test_host`
	...
	ds.ListLabelsForHostFunc = func(ctx context.Context, hid uint) ([]*mobius.Label, error) {
		return make([]*mobius.Label, 0), nil
	}
	ds.ListPacksForHostFunc = func(ctx context.Context, hid uint) (packs []*mobius.Pack, err error) {
		return make([]*mobius.Pack, 0), nil
	}
	ds.ListHostBatteriesFunc = func(ctx context.Context, hid uint) (batteries []*mobius.HostBattery, err error) {
		return nil, nil
	}
...
func TestGetConfig(t *testing.T) {
	...
	ds.AppConfigFunc = func(ctx context.Context) (*mobius.AppConfig, error) {
		return &mobius.AppConfig{
			Features:              mobius.Features{EnableHostUsers: true},
			VulnerabilitySettings: mobius.VulnerabilitySettings{DatabasesPath: "/some/path"},
	...
	t.Run("AppConfigAsTeamUsers", func(t *testing.T) {
		// test as team admin
		setCurrentUserSession(t, ds, &mobius.User{
			ID:         5,
		...
		return []mobius.SoftwareTitleListResult{
			{
				...
				Versions: []mobius.SoftwareVersion{
					{
						Version:         "0.0.1",
						Vulnerabilities: &mobius.SliceString{"cve-123-456-001", "cve-123-456-002"},
					},
					{
						Version:         "0.0.2",
						Vulnerabilities: &mobius.SliceString{"cve-123-456-001"},
					},
					{
						Version:         "0.0.3",
						Vulnerabilities: &mobius.SliceString{"cve-123-456-003"},
					},
				},
			},
			{
...
func TestGetSoftwareVersions(t *testing.T) {
	...
	foo002 := mobius.Software{Name: "foo", Version: "0.0.2", Source: "chrome_extensions", ExtensionID: "xyz", Browser: "edge"}
	foo003 := mobius.Software{Name: "foo", Version: "0.0.3", Source: "chrome_extensions", GenerateCPE: "someothercpewithoutvulns"}
	bar003 := mobius.Software{Name: "bar", Version: "0.0.3", Source: "deb_packages", BundleIdentifier: "bundle"}
	...
	ds.ListSoftwareFunc = func(ctx context.Context, opt mobius.SoftwareListOptions) ([]mobius.Software, *mobius.PaginationMetadata, error) {
		gotTeamID = opt.TeamID
		return []mobius.Software{foo001, foo002, foo003, bar003}, &mobius.PaginationMetadata{}, nil
	}
...
func TestGetPack(t *testing.T) {
	...
	ds.GetPackSpecFunc = func(ctx context.Context, name string) (*mobius.PackSpec, error) {
		if name != "pack1" {
			return nil, nil
		}
		return &mobius.PackSpec{
	...
	for _, tc := range []struct {
		...
		},
		{
			...
			user: &mobius.User{
				...
				GlobalRole: nil,
				Teams:      []mobius.UserTeam{{Role: mobius.RoleObserver}},
			},
		},
		{
...
func TestEnrichedAppConfig(t *testing.T) {
    ...
    `)
		...
		var enriched mobius.EnrichedAppConfig
...
func TestGetAppleBM(t *testing.T) {
	...
	t.Run("premium license, single token", func(t *testing.T) {
		_, ds := testing_utils.RunServerWithMockedDS(t,
			&service.TestServerOpts{License: &mobius.LicenseInfo{Tier: mobius.TierPremium}, DEPStorage: depStorage})
		ds.ListABMTokensFunc = func(ctx context.Context) ([]*mobius.ABMToken, error) {
			return []*mobius.ABMToken{
				{ID: 1},
			}, nil
		}
...
func TestGetCarves(t *testing.T) {
	...
	require.NoError(t, err)
	ds.ListCarvesFunc = func(ctx context.Context, opts mobius.CarveListOptions) ([]*mobius.CarveMetadata, error) {
		return []*mobius.CarveMetadata{
			{
				HostId:     1,
				Name:       "foobar",
				BlockCount: 10,
...
func TestGetCarve(t *testing.T) {
	...
	createdAt, err := time.Parse(time.RFC3339, "1999-03-10T02:45:06.371Z")
	require.NoError(t, err)
	ds.CarveFunc = func(ctx context.Context, carveID int64) (*mobius.CarveMetadata, error) {
		return &mobius.CarveMetadata{
			HostId:     1,
			Name:       "foobar",
			BlockCount: 10,
	...
	agentOpts := json.RawMessage(`
{
  ...
  "overrides": {
	...
	ds.ListTeamsFunc = func(ctx context.Context, filter mobius.TeamFilter, opt mobius.ListOptions) ([]*mobius.Team, error) {
		return []*mobius.Team{team1, team2}, nil
	}
	ds.AppConfigFunc = func(ctx context.Context) (*mobius.AppConfig, error) {
		return &mobius.AppConfig{AgentOptions: &agentOpts, MDM: mobius.MDM{EnabledAndConfigured: true}}, nil
	}
	...
	ds.BatchSetSoftwareInstallersFunc = func(ctx context.Context, tmID *uint, installers []*mobius.UploadSoftwareInstallerPayload) error {
		return nil
	}
...
<plist version="1.0">
  ...
  <SyncHdr>
    ...
    <MsgID>2</MsgID>
    <Target>
      <LocURI>https://roperzh-mobius.ngrok.io/api/mdm/microsoft/management</LocURI>
    </Target>
    <Source>
  ...
  <SyncBody>
	...
	ds.GetMDMAppleCommandResultsFunc = func(ctx context.Context, commandUUID string) ([]*mobius.MDMCommandResult, error) {
		...
		case "fail-cmd":
			return nil, io.EOF
		default:
			return []*mobius.MDMCommandResult{
				{
					HostUUID:    "device1",
	...
	ds.GetMDMWindowsCommandResultsFunc = func(ctx context.Context, commandUUID string) ([]*mobius.MDMCommandResult, error) {
		switch commandUUID {
		case "empty-cmd":
			return nil, nil
		case "fail-cmd":
...
|           |                      |                |        |          |   <!-- CmdID generated by Mobius -->                                                         |   <SyncHdr>                                                                        |
|           |                      |                |        |          |   <CmdID>90dbfca8-d4ac-40c9-bf57-ba5b8cbf1ce0</CmdID>                                       |     <VerDTD>1.2</VerDTD>                                                           |
|           |                      |                |        |          |   <Replace>                                                                                 |     <VerProto>DM/1.2</VerProto>                                                    |
|           |                      |                |        |          |     <!-- CmdID generated by Mobius -->                                                       |     <SessionID>48</SessionID>                                                      |
|           |                      |                |        |          |     <CmdID>81a141b2-5064-4dc3-a51a-128b8caa5438</CmdID>                                     |     <MsgID>2</MsgID>                                                               |
|           |                      |                |        |          |     <Item>                                                                                  |     <Target>                                                                       |
|           |                      |                |        |          |       <Target>                                                                              |       <LocURI>https://roperzh-mobius.ngrok.io/api/mdm/microsoft/management</LocURI> |
...
func TestGetMDMCommands(t *testing.T) {
	...
	ds.ListMDMCommandsFunc = func(ctx context.Context, tmFilter mobius.TeamFilter, listOpts *mobius.MDMCommandListOptions) ([]*mobius.MDMCommand, error) {
		if empty || listErr != nil {
			return nil, listErr
		}

		if noHostErr != nil {
			return nil, errors.New(mobius.HostIdentiferNotFound)
		}
		...
		return []*mobius.MDMCommand{
			...
			// This represents a command generated by mobius as part of a Windows profile
			{
				HostUUID:    "h2",
				CommandUUID: "u3",
				UpdatedAt:   time.Date(2023, 4, 11, 9, 5, 0, 0, time.UTC),
	...
	require.NoError(t, err)
	require.Contains(t, buf.String(), "You haven't run any MDM commands. Run MDM commands with the `mobiuscli mdm run-command` command.")
...
`))
	...
	expectIdentifier = true
	noHostErr = errors.New(mobius.HostIdentiferNotFound)
	_, err = RunAppNoChecks([]string{"get", "mdm-commands", "--host", "foo"})
	require.Error(t, err)
	require.ErrorContains(t, err, mobius.HostIdentiferNotFound)
}
