...
func gitopsCommand() *cli.Command {
	...
	return &cli.Command{
		Name:      "gitops",
		Usage:     "Synchronize Mobius configuration with provided file. This command is intended to be used in a GitOps workflow.",
		UsageText: `mobiuscli gitops [options]`,
		Flags: []cli.Flag{
			&cli.StringSliceFlag{
				Name:        "f",
				Required:    true,
		...
		Action: func(c *cli.Context) error {
			...
			var teamDryRunAssumptions *mobius.TeamSpecsDryRunAssumptions
			...
			type missingVPPTeamWithApps struct {
				config   *spec.GitOps
				vppApps  []*mobius.TeamSpecAppStoreApp
				filename string
			}
			...
			// We keep track of the environment MOBIUS_SECRET_* variables
			allMobiusSecrets := make(map[string]string)
			...
			for _, flFilename := range flFilenames.Value() {
				...
				if isGlobalConfig {
					if globalConfigLoaded {
						return errors.New("only one global config file may be provided to mobiuscli gitops")
					}
			...
			for _, configFile := range configs {
				...
				if isGlobalConfig {
					...
					logf("[!] skipping team config %s since teams are only supported for premium Mobius users\n", flFilename)
					continue
				}
				...
				if proposedLabelNames == nil {
					proposedLabelNames = make([]string, 0)
					persistedLabels, err := mobiusClient.GetLabels()
					if err != nil {
						return err
					}
					for _, persistedLabel := range persistedLabels {
						if persistedLabel.LabelType == mobius.LabelTypeRegular {
				...
				if isGlobalConfig && totalFilenames > 1 && !(totalFilenames == 2 && noTeamPresent) && appConfig.License.IsPremium() {
					abmTeams, hasMissingABMTeam, usesLegacyABMConfig, err = checkABMTeamAssignments(config, mobiusClient)
					if err != nil {
						return err
					}

					vppTeams, missingVPPTeams, err = checkVPPTeamAssignments(config, mobiusClient)
					if err != nil {
						return err
					}
				...
				if flDryRun {
					incomingSecrets := mobiusClient.GetGitOpsSecrets(config)
				...
				err = mobiusClient.SaveEnvSecrets(allMobiusSecrets, config.MobiusSecrets, flDryRun)
				if err != nil {
					return err
				}
				assumptions, postOps, err := mobiusClient.DoGitOps(c.Context, config, flFilename, logf, flDryRun, teamDryRunAssumptions, appConfig,
			...
			if len(abmTeams) > 0 && hasMissingABMTeam {
				if err = applyABMTokenAssignmentIfNeeded(c, teamNames, abmTeams, originalABMConfig, usesLegacyABMConfig, flDryRun,
					mobiusClient); err != nil {
					return err
				}
			}
			if len(missingVPPTeams) > 0 {
				if err = applyVPPTokenAssignmentIfNeeded(c, teamNames, vppTeams, originalVPPConfig, flDryRun, mobiusClient); err != nil {
					return err
				}
			}
			...
			for _, teamWithApps := range missingVPPTeamsWithApps {
				...
				_, postOps, err := mobiusClient.DoGitOps(c.Context, teamWithApps.config, teamWithApps.filename, logf, flDryRun, teamDryRunAssumptions, appConfig,
					teamsSoftwareInstallers, teamsVPPApps, teamsScripts)
				if err != nil {
					return err
				}
			...
			if flDeleteOtherTeams && appConfig.License.IsPremium() { // skip team deletion for non-premium users
				teams, err := mobiusClient.ListTeams("")
				if err != nil {
					return err
				}
				for _, team := range teams {
					if !slices.Contains(teamNames, team.Name) {
						...
						if flDryRun {
							...
							if err := mobiusClient.DeleteTeam(team.ID); err != nil {
								return err
							}
						}
					}
			...
			if globalConfigLoaded && !noTeamPresent {
				...
				defaultNoTeamConfig.TeamName = ptr.String(mobius.TeamNameNoTeam)
				_, postOps, err := mobiusClient.DoGitOps(c.Context, defaultNoTeamConfig, "no-team.yml", logf, flDryRun, nil, appConfig,
					map[string][]mobius.SoftwarePackageResponse{}, map[string][]mobius.VPPAppResponse{}, map[string][]mobius.ScriptResponse{})
				if err != nil {
					return err
				}
...
func getLabelUsage(config *spec.GitOps) (map[string][]LabelUsage, error) {
	...
	for _, setting := range config.Software.MobiusMaintainedApps {
		var labels []string
		if len(setting.LabelsIncludeAny) > 0 {
			labels = setting.LabelsIncludeAny
		}
		if len(setting.LabelsExcludeAny) > 0 {
			if len(labels) > 0 {
				return nil, fmt.Errorf("Mobius Maintained App '%s' has multiple label keys; please choose one of `labels_include_any`, `labels_exclude_any`.", setting.Slug)
			}
			labels = setting.LabelsExcludeAny
		}
		updateLabelUsage(labels, setting.Slug, "Mobius Maintained App", result)
	}
...
func getCustomSettings(osSettings interface{}) ([]mobius.MDMProfileSpec, bool) {
	if settingsMap, ok := osSettings.(mobius.WithMDMProfileSpecs); ok {
		return settingsMap.GetMDMProfileSpecs(), true
	}
	return nil, false
}

func extractControlsForNoTeam(flFilenames cli.StringSlice, appConfig *mobius.EnrichedAppConfig) (spec.GitOpsControls, bool, error) {
	for _, flFilename := range flFilenames.Value() {
...
func checkABMTeamAssignments(config *spec.GitOps, mobiusClient *service.Client) (
	...
	if mdm, ok := config.OrgSettings["mdm"]; ok {
		if mdmMap, ok := mdm.(map[string]any); ok {
			...
			if hasLegacyConfig && hasNewConfig {
				return nil, false, false, errors.New(mobius.AppleABMDefaultTeamDeprecatedMessage)
			}

			abmToks, err := mobiusClient.CountABMTokens()
			if err != nil {
...
func applyABMTokenAssignmentIfNeeded(
	...
	flDryRun bool,
	mobiusClient *service.Client,
) error {
	if usesLegacyConfig && len(abmTeamNames) > 1 {
		return errors.New(mobius.AppleABMDefaultTeamDeprecatedMessage)
	}
	...
	if err := mobiusClient.ApplyAppConfig(appConfigUpdate, mobius.ApplySpecOptions{}); err != nil {
		return fmt.Errorf("applying mobius config: %w", err)
	}
	return nil
}

func checkVPPTeamAssignments(config *spec.GitOps, mobiusClient *service.Client) (
...
func applyVPPTokenAssignmentIfNeeded(
	...
	for _, vppTeam := range vppTeamNames {
		if !mobius.IsReservedTeamName(vppTeam) && !slices.Contains(teamNames, vppTeam) {
	...
	if err := mobiusClient.ApplyAppConfig(appConfigUpdate, mobius.ApplySpecOptions{}); err != nil {
		return fmt.Errorf("applying mobius config for volume_purchasing_program teams: %w", err)
	}
	return nil
}
