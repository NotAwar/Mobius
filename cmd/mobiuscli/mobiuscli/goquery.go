...
type goqueryClient struct {
	...
	// query against Mobius. Keep a mapping so that we know how to target
	// the host.
	hostnameByUUID map[string]string
}

func newGoqueryClient(mobiusClient *service.Client) *goqueryClient {
...
func goqueryCommand() *cli.Command {
	return &cli.Command{
		...
		Action: func(c *cli.Context) error {
			...
			goquery.Run(newGoqueryClient(mobius), gqconfig.Config{})
			return nil
		},
	}
}
