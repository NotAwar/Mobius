...
func loginCommand() *cli.Command {
	...
	return &cli.Command{
		Name:  "login",
		Usage: "Login to Mobius",
		UsageText: `
mobiuscli login [options]

Interactively prompts for email and password if not specified in the flags or environment variables.

Trying to login with SSO or MFA? First, login to the Mobius UI and retrieve your API token from the "My account" page. Then set your API token with the mobiuscli config set --token <your-api-token-here> command. You're now logged in to mobiuscli.
`,
		Flags: []cli.Flag{
			&cli.StringFlag{
				Name:        "email",
		...
		Action: func(c *cli.Context) error {
			...
			// CLI history.
			if flEmail == "" {
				fmt.Println("Log in using the standard Mobius credentials.")
				fmt.Print("Email: ")
			...
			token, err := mobius.Login(flEmail, flPassword)
			if err != nil {
			...
			fmt.Printf("[+] Mobius login successful and context configured!\n")
