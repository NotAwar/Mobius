	...
	return &cli.Command{
		...
		Action: func(c *cli.Context) error {
			...
			for _, ident := range hostIdents {
				...
				mdmHostPlatform := mobius.MDMPlatform(host.Platform)
				...
				if host.MDM.ConnectedToMobius == nil || !*host.MDM.ConnectedToMobius {
					return errors.New(`Can't run the MDM command because one or more hosts have MDM turned off. Run the following command to see a list of hosts with MDM on: mobiuscli get hosts --mdm.`)
				}
			...
			if len(hostUUIDs) == 0 || notFoundCount > 0 {
				...
				return errors.New(mobius.TargetedHostsDontExistErrMsg)
			}
			...
			if err != nil {
				if errors.Is(err, service.ErrMissingLicense) && mdmPlatform == "windows" {
					return errors.New(mobius.WindowsMDMRequiresPremiumCmdMessage)
				}
...
Hosts will run the command the next time they check into Mobius.

Copy and run this command to see results:

mobiuscli get mdm-command-results --id=%v
`, result.CommandUUID)
...
func mdmUnlockCommand() *cli.Command {
	return &cli.Command{
		...
		Action: func(c *cli.Context) error {
			...
			if mobius.MDMPlatform(host.Platform) == "darwin" {
				fmt.Fprintf(c.App.Writer, `
...
mobiuscli get host %s`, hostIdent)
...
func hostMdmActionSetup(c *cli.Context, hostIdent string, actionType string) (client *service.Client, host *service.HostDetailResponse, err error) {
	...
	if err != nil {
		var nfe service.NotFoundErr
		if errors.As(err, &nfe) {
			return nil, nil, errors.New(mobius.HostNotFoundErrMsg)
		}
