	...
	hosts := []testhost{
		{
			host: &mobius.Host{
				...
				UUID:     "mac-enrolled",
				Platform: "darwin",
				MDM:      mobius.MDMHostData{Name: mobius.WellKnownMDMMobius, EnrollmentStatus: ptr.String("On (manual)"), ConnectedToMobius: ptr.Bool(true)},
			},
			mdmInfo: &mobius.HostMDM{Enrolled: true, Name: mobius.WellKnownMDMMobius},
		},
		{
			host: &mobius.Host{
				ID:       2,
		...
		},
		{
			host: &mobius.Host{
				ID:       8,
				UUID:     "mac-non-mobius-enrolled",
				Platform: "darwin",
				MDM:      mobius.MDMHostData{Name: mobius.WellKnownMDMJamf, EnrollmentStatus: ptr.String("On (manual)"), ConnectedToMobius: ptr.Bool(false)},
			},
			mdmInfo: &mobius.HostMDM{Enrolled: true, Name: mobius.WellKnownMDMJamf},
		},
		{
			host: &mobius.Host{
				ID:       9,
				UUID:     "win-non-mobius-enrolled",
				Platform: "windows",
				MDM:      mobius.MDMHostData{Name: mobius.WellKnownMDMIntune, EnrollmentStatus: ptr.String("On (manual)"), ConnectedToMobius: ptr.Bool(false)},
			},
			mdmInfo: &mobius.HostMDM{Enrolled: true, Name: mobius.WellKnownMDMIntune},
		},
		{
			host: &mobius.Host{
				ID:       10,
				UUID:     "mac-pending",
				Platform: "darwin",
				MDM:      mobius.MDMHostData{Name: mobius.WellKnownMDMMobius, EnrollmentStatus: ptr.String("Pending"), ConnectedToMobius: ptr.Bool(false)},
			},
			mdmInfo: &mobius.HostMDM{Enrolled: false, Name: mobius.WellKnownMDMMobius},
		},
		{
			host: &mobius.Host{
				ID:       11,
...
<plist version="1.0">
	...
	appCfgAllMDM := &mobius.AppConfig{MDM: mobius.MDM{EnabledAndConfigured: true, WindowsEnabledAndConfigured: true}}
	appCfgWinMDM := &mobius.AppConfig{MDM: mobius.MDM{WindowsEnabledAndConfigured: true}}
	appCfgMacMDM := &mobius.AppConfig{MDM: mobius.MDM{EnabledAndConfigured: true}}
	appCfgNoMDM := &mobius.AppConfig{MDM: mobius.MDM{}}

	for _, lic := range []string{mobius.TierFree, mobius.TierPremium} {
		t.Run(lic, func(t *testing.T) {
			enqueuer := new(mdmmock.MDMAppleStore)
			license := &mobius.LicenseInfo{Tier: lic, Expiration: time.Now().Add(24 * time.Hour)}
			...
			ds.AreHostsConnectedToMobiusMDMFunc = func(ctx context.Context, hosts []*mobius.Host) (map[string]bool, error) {
				...
				for _, h := range hosts {
					res[h.UUID] = h.MDM.ConnectedToMobius != nil && *h.MDM.ConnectedToMobius
				}
				return res, nil
			}
			...
			cases := []struct {
				...
				{"single host not found", []string{"--hosts", "no-such-host", "--payload", appleCmdFilePath}, appCfgAllMDM, mobius.TargetedHostsDontExistErrMsg},
				...
				{"macOS non-mobius host", []string{"--hosts", "mac-non-mobius-enrolled", "--payload", appleCmdFilePath}, appCfgAllMDM, `Can't run the MDM command because one or more hosts have MDM turned off.`},
				{"windows non-mobius host", []string{"--hosts", "win-non-mobius-enrolled", "--payload", winCmdFilePath}, appCfgAllMDM, `Can't run the MDM command because one or more hosts have MDM turned off.`},
				...
				{"no mdm enabled", []string{"--hosts", "win-enrolled", "--payload", winCmdFilePath}, appCfgNoMDM, "MDM features aren't turned on in Mobius."},
				...
				{"windows premium cmd", []string{"--hosts", "win-enrolled", "--payload", winPremiumCmdFilePath}, appCfgAllMDM, func() string {
					if lic == mobius.TierFree {
						return `Missing or invalid license. Wipe command is available in Mobius Premium only.`
					}
					return ""
				}()},
				...
				{"all hosts not found", []string{"--hosts", "no-such-1,no-such-2,no-such-3", "--payload", winCmdFilePath}, appCfgAllMDM, mobius.TargetedHostsDontExistErrMsg},
			...
			for _, c := range cases {
				t.Run(c.desc, func(t *testing.T) {
					...
					if c.wantErr != "" {
						...
						require.NoError(t, err)
						require.Contains(t, buf.String(), `Hosts will run the command the next time they check into Mobius.`)
						require.Contains(t, buf.String(), `mobiuscli get mdm-command-results --id=`)
					}
				})
			}
		})
...
func TestMDMLockCommand(t *testing.T) {
	...
	ds.GetHostLockWipeStatusFunc = func(ctx context.Context, host *mobius.Host) (*mobius.HostLockWipeStatus, error) {
		...
		var status mobius.HostLockWipeStatus
		status.HostMobiusPlatform = mobiusPlatform

		if _, ok := unlockPending[host.ID]; ok {
			if mobiusPlatform == "darwin" {
				return nil, errors.New("apple devices do not have an unlock pending state")
			}

			status.UnlockScript = &mobius.HostScriptResult{}
		}

		if _, ok := lockPending[host.ID]; ok {
			if mobiusPlatform == "darwin" {
				status.LockMDMCommand = &mobius.MDMCommand{}
				return &status, nil
			}

			status.LockScript = &mobius.HostScriptResult{}
		}

		if _, ok := wipePending[host.ID]; ok {
			if mobiusPlatform == "linux" {
				status.WipeScript = &mobius.HostScriptResult{ExitCode: nil}
				return &status, nil
			}

			status.WipeMDMCommand = &mobius.MDMCommand{}
	...
	ds.GetHostOrbitInfoFunc = func(ctx context.Context, hostID uint) (*mobius.HostOrbitInfo, error) {
		hostIDMod := hostID % 3
		switch hostIDMod {
		case 0:
			return nil, &notFoundError{}
		case 1:
			return &mobius.HostOrbitInfo{}, nil
		case 2:
			return &mobius.HostOrbitInfo{ScriptsEnabled: ptr.Bool(true)}, nil
		default:
	...
	ds.IsHostConnectedToMobiusMDMFunc = func(ctx context.Context, host *mobius.Host) (bool, error) {
		...
		return mdmInfo != nil && mdmInfo.Enrolled == true && mdmInfo.Name == mobius.WellKnownMDMMobius, nil
	}
...
`, ident, ident)
	...
	cases := []struct {
		...
		{appCfgAllMDM, "lock non-existent host", []string{"--host", "notfound"}, mobius.HostNotFoundErrMsg},
...
func TestMDMUnlockCommand(t *testing.T) {
	...
	ds.GetHostLockWipeStatusFunc = func(ctx context.Context, host *mobius.Host) (*mobius.HostLockWipeStatus, error) {
		...
		if _, ok := locked[host.ID]; ok {
			if mobiusPlatform == "darwin" {
				status.LockMDMCommand = &mobius.MDMCommand{}
				status.LockMDMCommandResult = &mobius.MDMCommandResult{Status: mobius.MDMAppleStatusAcknowledged}
				return &status, nil
			}

			status.LockScript = &mobius.HostScriptResult{ExitCode: ptr.Int64(0)}
		}
	...
	ds.UnlockHostViaScriptFunc = func(ctx context.Context, request *mobius.HostScriptRequestPayload, platform string) error {
		return nil
	}
	...
	ds.IsHostConnectedToMobiusMDMFunc = func(ctx context.Context, host *mobius.Host) (bool, error) {
		return host.MDM.ConnectedToMobius != nil && *host.MDM.ConnectedToMobius, nil
	}
	ds.ListUpcomingHostMaintenanceWindowsFunc = func(ctx context.Context, hid uint) ([]*mobius.HostMaintenanceWindow, error) {
		return nil, nil
	}
...
`, ident)
	...
	cases := []struct {
		...
		{appCfgAllMDM, "no flags", nil, `Required flag "host" not set`},
		{appCfgAllMDM, "host flag empty", []string{"--host", ""}, `No host targeted. Please provide --host.`},
		{appCfgAllMDM, "unlock non-existent host", []string{"--host", "notfound"}, mobius.HostNotFoundErrMsg},
		{appCfgMacMDM, "valid windows but only macos mdm", []string{"--host", winEnrolled.host.UUID}, `Windows MDM isn't turned on.`},
		{appCfgAllMDM, "valid windows", []string{"--host", winEnrolled.host.UUID}, ""},
		{appCfgAllMDM, "valid macos", []string{"--host", macEnrolled.host.UUID}, ""},
		{appCfgNoMDM, "valid linux", []string{"--host", linuxEnrolled.host.UUID}, ""},
...
func TestMDMWipeCommand(t *testing.T) {
	...
	ds.GetHostLockWipeStatusFunc = func(ctx context.Context, host *mobius.Host) (*mobius.HostLockWipeStatus, error) {
		...
		if _, ok := wiped[host.ID]; ok {
			if mobiusPlatform == "linux" {
				status.WipeScript = &mobius.HostScriptResult{ExitCode: ptr.Int64(0)}
			}

			if mobiusPlatform == "darwin" {
				status.WipeMDMCommand = &mobius.MDMCommand{}
				status.WipeMDMCommandResult = &mobius.MDMCommandResult{
	...
	ds.UnlockHostViaScriptFunc = func(ctx context.Context, request *mobius.HostScriptRequestPayload, hostMobiusPlatform string) error {
		return nil
	}
	ds.UnlockHostManuallyFunc = func(ctx context.Context, hostID uint, hostMobiusPlatform string, ts time.Time) error {
		return nil
	}
	ds.WipeHostViaWindowsMDMFunc = func(ctx context.Context, host *mobius.Host, cmd *mobius.MDMWindowsCommand) error {
		return nil
	}

	ds.WipeHostViaScriptFunc = func(ctx context.Context, request *mobius.HostScriptRequestPayload, hostMobiusPlatform string) error {
		return nil
	}
	...
	appCfgAllMDM, appCfgWinMDM, appCfgMacMDM, appCfgNoMDM := setupAppConigs()
	appCfgScriptsDisabled := &mobius.AppConfig{ServerSettings: mobius.ServerSettings{ScriptsDisabled: true}}
	...
	cases := []struct {
		...
		{appCfgAllMDM, "no flags", nil, `Required flag "host" not set`},
		{appCfgAllMDM, "host flag empty", []string{"--host", ""}, `No host targeted. Please provide --host.`},
		{appCfgAllMDM, "wipe non-existent host", []string{"--host", "notfound"}, mobius.HostNotFoundErrMsg},
		{appCfgMacMDM, "valid windows but only macos mdm", []string{"--host", winEnrolled.host.UUID}, `Windows MDM isn't turned on.`},
		{appCfgAllMDM, "valid windows", []string{"--host", winEnrolled.host.UUID}, ""},
		{appCfgAllMDM, "valid macos", []string{"--host", macEnrolled.host.UUID}, ""},
		{appCfgNoMDM, "valid linux", []string{"--host", linuxEnrolled.host.UUID}, ""},
...
func setupTestServer(t *testing.T) *mock.Store {
	...
	enqueuer.EnqueueDeviceLockCommandFunc = func(ctx context.Context, host *mobius.Host, cmd *mdm.Command, pin string) error {
		return nil
	}

	enqueuer.EnqueueDeviceWipeCommandFunc = func(ctx context.Context, host *mobius.Host, cmd *mdm.Command) error {
		return nil
	}
...
func setupAppConigs() (*mobius.AppConfig, *mobius.AppConfig, *mobius.AppConfig, *mobius.AppConfig) {
	appCfgAllMDM := &mobius.AppConfig{MDM: mobius.MDM{EnabledAndConfigured: true, WindowsEnabledAndConfigured: true}}
	appCfgWinMDM := &mobius.AppConfig{MDM: mobius.MDM{WindowsEnabledAndConfigured: true}}
	appCfgMacMDM := &mobius.AppConfig{MDM: mobius.MDM{EnabledAndConfigured: true}}
	appCfgNoMDM := &mobius.AppConfig{MDM: mobius.MDM{}}
