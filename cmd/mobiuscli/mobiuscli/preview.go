...
import (
	...
	"github.com/notawar/mobius/v4/ee/server/licensing"
...
const (
	standardQueryLibraryUrl   = "https://raw.githubusercontent.com/mobiusmdm/mobius/main/docs/01-Using-Mobius/standard-query-library/standard-query-library.yml"
	...
	licenseError = "License key is invalid. Please check your license key and try again, or run `mobiuscli preview` without the --license-key option to try Mobius Free."
...
func newDockerCompose() (dockerCompose, error) {
	...
	return dockerCompose{}, errors.New("`docker compose` is required for the mobiuscli preview experience.\n\nPlease install `docker compose` (https://docs.docker.com/compose/install/).")
}
...
func previewCommand() *cli.Command {
	return &cli.Command{
		...
		Usage:   "Start a sandbox deployment of the Mobius server",
		Description: `Start a sandbox deployment of the Mobius server using Docker and docker compose. Docker tools must be available in the environment.
...
Use the stop and reset subcommands to manage the server and dependencies once started.`,
		...
		Flags: []cli.Flag{
			...
			&cli.StringFlag{
				...
				Usage: "License key to enable Mobius Premium (optional)",
			},
			&cli.StringFlag{
				Name:  tagFlagName,
				Usage: "Run a specific version of Mobius",
				Value: "latest",
			},
			&cli.StringFlag{
			...
			&cli.StringFlag{
				...
				Usage:  "Use custom local directory for the pulling configuration from mobiusmdm/mobius (used for development/testing)",
				Value:  "",
				Hidden: true,
			},
			&cli.BoolFlag{
		...
		Action: func(c *cli.Context) error {
			...
			if previewConfigDir := c.String(previewConfigPathFlagName); previewConfigDir != "" {
				...
				mobiusBranch := c.String(previewConfigFlagName)
				fmt.Printf("Downloading dependencies from mobiusmdm/mobius:%s into %s...\n", mobiusBranch, previewDir)
				githubHTTPClient := mobiushttp.NewGithubClient()
				githubClient := github.NewClient(githubHTTPClient)
				if err := downloadFromMobiusRepo(
					context.Background(),
					githubHTTPClient,
					githubClient,
			...
			// Make sure directories are writable, otherwise the Mobius
			...
			if err := os.Setenv("MOBIUS_SERVER_PRIVATE_KEY", pk); err != nil {
				return fmt.Errorf("failed to set private key: %w", err)
			}

			if err := os.Setenv("MOBIUS_VERSION", c.String(tagFlagName)); err != nil {
				return fmt.Errorf("failed to set Mobius version: %w", err)
			}
			...
			cmd := compose.Command("up", "-d", "--remove-orphans", "mysql01", "redis01", "mobius01")
			cmd.Env = append(os.Environ(), "MOBIUS_LICENSE_KEY="+licenseKey)
			...
			// Start mobius02 (UI server) after mobius01 (agent/mobiuscli server)
			// has finished starting up so that there is no conflict with
			// running database migrations.
			cmd = compose.Command("up", "-d", "--remove-orphans", "mobius02")
			cmd.Env = append(os.Environ(), "MOBIUS_LICENSE_KEY="+licenseKey)
			out, err = cmd.CombinedOutput()
			if err != nil {
				fmt.Println(string(out))
			...
			mobiusClient, err := service.NewClient(address, true, "", "")
			if err != nil {
				return fmt.Errorf("Error creating Mobius API client handler: %w", err)
			}

			token, err := mobiusClient.Setup(email, "Admin", password, "Mobius for osquery")
			if err != nil {
			...
			if err != nil {
				...
				fmt.Println("Configured mobiuscli in the 'preview' context to avoid overwriting existing config.")
				context = "preview"
			...
			if err := writeConfig(configPath, config); err != nil {
				return fmt.Errorf("Error writing mobiuscli configuration: %w", err)
			}

			// Create client and get enroll secret
			client, err := unauthenticatedClientFromCLI(c)
			if err != nil {
				return fmt.Errorf("Error making mobiuscli client: %w", err)
			}

			token, err = client.Login(email, password)
			if err != nil {
				return fmt.Errorf("mobiuscli login failed: %w", err)
			}
			...
			_, _, _, _, err = client.ApplyGroup(c.Context, false, specs, ".", logf, nil, mobius.ApplyClientSpecOptions{}, teamsSoftwareInstallers, teamsVPPApps, teamsScripts)
			if err != nil {
				return err
			}
			...
			// Docker image used for preview (mobiusmdm/mobiuscli:latest) is released
			...
			}, mobius.ApplySpecOptions{}); err != nil {
			...
			fmt.Println("Mobius will now log you into the UI automatically.")
			...
			fmt.Println("Preview environment complete. Enjoy using Mobius!")
...
func previewDirectory() string {
	...
	return filepath.Join(homeDir, ".mobius", "preview")
}

// downloadFromMobiusRepo downloads a specific directory from the mobiusmdm/mobius repository.
// It creates files and folders as world writable as this is mainly used for testing.
func downloadFromMobiusRepo(
	ctx context.Context,
	client *http.Client,
	...
	for _, content := range contents {
		if content.GetType() == "file" {
			...
			return downloadFromMobiusRepo(ctx,
				client,
				githubClient,
...
func checkDocker() error {
	...
	if _, err := exec.LookPath("docker"); err != nil {
		return errors.New("Docker is required for the mobiuscli preview experience.\n\nPlease install Docker (https://docs.docker.com/get-docker/).")
	}

	// Check running
	if err := exec.Command("docker", "info").Run(); err != nil {
		return errors.New("Please start Docker daemon before running mobiuscli preview.")
	}
...
func previewStopCommand() *cli.Command {
	return &cli.Command{
		Name:  "stop",
		Usage: "Stop the Mobius preview server and dependencies",
		Flags: []cli.Flag{
			configFlag(),
			contextFlag(),
			debugFlag(),
		...
		Action: func(c *cli.Context) error {
			...
			if err != nil {
				fmt.Println(string(out))
				return fmt.Errorf("Failed to run %s stop for Mobius server and dependencies", compose)
			}
...
func previewResetCommand() *cli.Command {
	return &cli.Command{
		Name:  "reset",
		Usage: "Reset the Mobius preview server and dependencies",
		Flags: []cli.Flag{
			configFlag(),
			contextFlag(),
			debugFlag(),
		...
		Action: func(c *cli.Context) error {
			...
			if err != nil {
				fmt.Println(string(out))
				return fmt.Errorf("Failed to run %s rm -sf for Mobius server and dependencies.", compose)
			}
