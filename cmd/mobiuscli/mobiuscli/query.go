...
func queryCommand() *cli.Command {
	...
	return &cli.Command{
		...
		UsageText: `mobiuscli query [options]`,
...
be run on the union of the hosts and hosts with matching labels.
		...
		Action: func(c *cli.Context) error {
			...
			if flQueryName != "" {
				...
				// For backwards compatibility with older mobius server, we explicitly find the query in the result array
			...
			if err != nil {
				if strings.Contains(err.Error(), "no hosts targeted") {
					return errors.New(mobius.NoHostsTargetedErrMsg)
				}
				if strings.Contains(err.Error(), mobius.InvalidLabelSpecifiedErrMsg) {
					pattern := fmt.Sprintf("(%s.*)$", regexp.QuoteMeta(mobius.InvalidLabelSpecifiedErrMsg))
