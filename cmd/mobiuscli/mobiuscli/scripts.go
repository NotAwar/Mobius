...
func runScriptCommand() *cli.Command {
	return &cli.Command{
		...
		UsageText: `mobiuscli run-script [options]`,
		Flags: []cli.Flag{
			&cli.StringFlag{
				Name:     "script-path",
			...
			&cli.UintFlag{
				Name:     "team",
				Usage:    `Available in Mobius Premium. ID of the team that the saved script belongs to. 0 targets hosts assigned to “No team” (default: 0).`,
				Required: false,
			},
			&cli.BoolFlag{
				Name:     "async",
		...
		Action: func(c *cli.Context) error {
			...
			if appCfg.ServerSettings.ScriptsDisabled {
				return errors.New(mobius.RunScriptScriptsDisabledGloballyErrMsg)
			}
			...
			if err != nil {
				...
				if errors.As(err, &nfe) {
					return errors.New(mobius.HostNotFoundErrMsg)
				}
				var sce mobius.ErrWithStatusCode
				if errors.As(err, &sce) {
					if sce.StatusCode() == http.StatusForbidden {
						return errors.New(mobius.RunScriptForbiddenErrMsg)
					}
				}
				return err
			}

			if h.Status != mobius.StatusOnline {
				return errors.New(mobius.RunScriptHostOfflineErrMsg)
			}
			...
			if path != "" || args > 0 {
				...
				// for the shorter
				if err := mobius.ValidateHostScriptContents(string(b), false); err != nil {
					if err.Error() == mobius.RunScripUnsavedMaxLenErrMsg {
						return errors.New("Script is too large. Script referenced by '--script-path' is limited to 10,000 characters. To run larger script save it to Mobius and use '--script-name'.")
					}
					return err
				}
			}
...
func renderScriptResult(c *cli.Context, res *mobius.HostScriptResult) error {
...
`))
	...
	if len(res.Output) >= mobius.UnsavedScriptMaxRuneLen && utf8.RuneCountInString(res.Output) >= mobius.UnsavedScriptMaxRuneLen {
		data.Output = "Mobius records the last 10,000 characters to prevent downtime.\n\n" + res.Output
	} else {
...
func validateScriptPath(path string) error {
	...
	return errors.New(mobius.RunScriptInvalidTypeErrMsg)
}
