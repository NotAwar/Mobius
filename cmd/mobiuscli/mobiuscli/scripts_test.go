...
import (
	...
	"github.com/notawar/mobius set/v4/pkg/scripts"
...
func TestRunScriptCommand(t *testing.T) {
	...
	ds.AppConfigFunc = func(ctx context.Context) (*mobius.AppConfig, error) {
		return &mobius.AppConfig{ServerSettings: mobius.ServerSettings{ScriptsDisabled: false}}, nil
	}
	...
	exceedsMaxCharsUnsaved := strings.Repeat("a", mobius.UnsavedScriptMaxRuneLen+1)
	exceedsMaxCharsSaved := strings.Repeat("a", mobius.SavedScriptMaxRuneLen+1)
...
`
	...
	type testCase struct {
		...
		scriptResult        *mobius.HostScriptResult
		quiet               bool
		async               bool
		expectOutput        string
		expectErrMsg        string
	...
	cases := []testCase{
		{
			...
			expectErrMsg:   mobius.HostNotFoundErrMsg,
			expectNotFound: true,
		},
		{
			name:         "invalid file type",
			scriptPath:   func() string { return writeTmpScriptContents(t, "echo hello world", ".txt") },
			expectErrMsg: mobius.RunScriptInvalidTypeErrMsg,
		},
		{
		...
		},
		{
			...
			expectErrMsg: "Script is too large. Script referenced by '--script-path' is limited to 10,000 characters. To run larger script save it to Mobius and use '--script-name'.",
		},
		{
			name: "script not too long (unsaved)",
			scriptPath: func() string {
				return writeTmpScriptContents(t, exceedsMaxCharsUnsaved[:mobius.UnsavedScriptMaxRuneLen], ".sh")
			},
			scriptResult: &mobius.HostScriptResult{
				ExitCode: ptr.Int64(0),
				Output:   "hello world",
...
`,
		},
		{
			...
			scriptResult: &mobius.HostScriptResult{
				ExitCode: ptr.Int64(-1),
				Output:   "Oh no!",
				Message:  mobius.HostScriptTimeoutMessage(ptr.Int(int(scripts.MaxHostExecutionTime.Seconds()))),
			},
			expectOutput: `
Error: Timeout. Mobius stopped the script after 300 seconds to protect host performance.
...
`,
		},
		{
			...
			scriptResult: &mobius.HostScriptResult{
				...
				Output:   "",
				Message:  mobius.RunScriptDisabledErrMsg,
			},
			expectOutput: `
Error: Scripts are disabled for this host. To run scripts, deploy the mobiusdaemon agent with scripts enabled.
...
Mobius records the last 10,000 characters to prevent downtime.
...
`, exceedsMaxCharsUnsaved),
		...
		//	name:         "host timeout",
		//	scriptPath:   generateValidPath,
		//	expectErrMsg: mobius.RunScriptHostTimeoutErrMsg,
		// },
		{name: "disabled scripts globally", scriptPath: generateValidPath, expectErrMsg: mobius.RunScriptScriptsDisabledGloballyErrMsg},
	}

	setupDS := func(t *testing.T, c testCase) {
		ds.HostByIdentifierFunc = func(ctx context.Context, ident string) (*mobius.Host, error) {
			if ident != "host1" || c.expectNotFound {
				return nil, &notFoundError{}
			}
			return &mobius.Host{ID: 42, SeenTime: time.Now(), OrbitNodeKey: ptr.String("abc")}, nil
		}
		ds.HostFunc = func(ctx context.Context, hid uint) (*mobius.Host, error) {
			if hid != 42 || c.expectNotFound {
				return nil, &notFoundError{}
			}
			h := mobius.Host{ID: hid, SeenTime: time.Now(), OrbitNodeKey: ptr.String("abc")}
			if c.expectOffline {
				h.SeenTime = time.Now().Add(-time.Hour)
			}
			return &h, nil
		}
		ds.ListPendingHostScriptExecutionsFunc = func(ctx context.Context, hid uint, onlyShowInternal bool) ([]*mobius.HostScriptResult, error) {
			require.Equal(t, uint(42), hid)
			if c.expectPending {
				return []*mobius.HostScriptResult{{HostID: uint(42)}}, nil
			}
			return nil, nil
		}
		ds.GetHostScriptExecutionResultFunc = func(ctx context.Context, execID string) (*mobius.HostScriptResult, error) {
			if c.scriptResult != nil {
				return c.scriptResult, nil
			}
			return &mobius.HostScriptResult{}, nil
		}
		...
		ds.NewHostScriptExecutionRequestFunc = func(ctx context.Context, req *mobius.HostScriptRequestPayload) (*mobius.HostScriptResult, error) {
			require.Equal(t, uint(42), req.HostID)
			return &mobius.HostScriptResult{
				Hostname:       "host1",
				HostID:         req.HostID,
		...
		if c.name == "disabled scripts globally" {
			ds.AppConfigFunc = func(ctx context.Context) (*mobius.AppConfig, error) {
				return &mobius.AppConfig{ServerSettings: mobius.ServerSettings{ScriptsDisabled: true}}, nil
			}
		} else {
			ds.AppConfigFunc = func(ctx context.Context) (*mobius.AppConfig, error) {
				return &mobius.AppConfig{ServerSettings: mobius.ServerSettings{ScriptsDisabled: false}}, nil
		...
		if c.savedScriptContents != nil {
			...
			ds.ScriptFunc = func(ctx context.Context, id uint) (*mobius.Script, error) {
				return &mobius.Script{ID: id, Name: "foo"}, nil
			}
		} else {
			ds.GetScriptContentsFunc = func(ctx context.Context, id uint) ([]byte, error) {
