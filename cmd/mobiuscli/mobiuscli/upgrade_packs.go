...
func upgradePacksCommand() *cli.Command {
	...
	return &cli.Command{
		...
		UsageText: `mobiuscli upgrade-packs [options]`,
		Flags: []cli.Flag{
			configFlag(),
			contextFlag(),
			debugFlag(),
		...
		Action: func(c *cli.Context) error {
			...
			packsByID := make(map[uint]*mobius.Pack, len(packsDB))
...
To import these queries to Mobius, you can merge the data in the output file with your existing query configuration and run `+"`mobiuscli apply`"+`.

Note that existing 2017 "Packs" have been left intact. To avoid running duplicate queries on your hosts, visit %s/packs/manage and disable all 2017 "Packs" after upgrading. Mobius will continue to support these until the next major version release, when 2017 "Packs" will be automatically converted to queries.
...
func writeQuerySpecsToFile(filename string, specs []*mobius.QuerySpec) error {
	...
	for _, spec := range specs {
		qYaml := mobius.QueryObject{
			ObjectMetadata: mobius.ObjectMetadata{
				ApiVersion: mobius.ApiVersion,
...
func mapQueriesToPacks(packs []*mobius.PackSpec, queries []mobius.Query) map[*mobius.PackSpec][]*mobius.Query {
	queriesByName := make(map[string]*mobius.Query, len(queries))
	for _, q := range queries {
		q := q // avoid taking address of iteration var
		queriesByName[q.Name] = &q
	}

	queriesByPack := make(map[*mobius.PackSpec][]*mobius.Query, len(packs))
...
// converted.
func upgradePackToQueriesSpecs(packSpec *mobius.PackSpec, packDB *mobius.Pack, packQueries []*mobius.Query, ts time.Time) ([]*mobius.QuerySpec, int) {
	...
	schedByName := make(map[string]*mobius.PackSpecQuery, len(packSpec.Queries))
	...
	for _, pq := range packQueries {
		...
		for _, tm := range packSpec.Targets.Teams {
			...
			newSpecs = append(newSpecs, &mobius.QuerySpec{
				Name:               newQueryName,
				Description:        desc,
				Query:              pq.Query,
				TeamName:           tm,
