...
func createUserCommand() *cli.Command {
	return &cli.Command{
		...
		UsageText: `This command will create a new user in Mobius. By default, the user will authenticate with a password and will be a global observer.
   ...
   If a password is required and not provided by flag, the command will prompt for password input through stdin.`,
		...
		Action: func(c *cli.Context) error {
			...
			if globalRoleString != "" && len(teamStrings) > 0 { //nolint:gocritic // ignore ifElseChain
				...
				if !mobius.ValidGlobalRole(globalRoleString) {
				...
				for _, t := range teamStrings {
					...
					if !mobius.ValidTeamRole(parts[1]) {
						return fmt.Errorf("'%s' is not a valid team role", parts[1])
					}

					teams = append(teams, mobius.UserTeam{Team: mobius.Team{ID: uint(teamID)}, Role: parts[1]}) //nolint:gosec // dismiss G115
				}
			}
			...
			sessionKey, err := client.CreateUser(mobius.UserPayload{
				Password:                 &password,
				Email:                    &email,
				Name:                     &name,
				SSOEnabled:               &sso,
...
func createBulkUsersCommand() *cli.Command {
	return &cli.Command{
		...
		UsageText: `This command will create a set of users in Mobius by importing a CSV file. Expected columns are: Name,Email,SSO,API Only,Global Role,Teams. Created Users by default get random password and Observer Role.`,
		Flags: []cli.Flag{
			&cli.StringFlag{
				Name:     csvFlagName,
			...
			for _, record := range csvLines[1:] {
				...
				if globalRoleString != "" && len(teamStrings) > 0 && teamStrings[0] != "" { //nolint:gocritic // ignore ifElseChain
					...
					for _, t := range teamStrings {
						...
						teams = append(teams,
							mobius.UserTeam{Team: mobius.Team{ID: uint(teamID)}, Role: parts[1]}) //nolint:gosec // dismiss G115
					}
				}
				...
				force_reset := !sso
				users = append(users, mobius.UserPayload{
					Password:                 &password,
					Email:                    &email,
					Name:                     &name,
					SSOEnabled:               &sso,
...
func deleteUserCommand() *cli.Command {
	return &cli.Command{
		Name:      "delete",
		Usage:     "Delete a user",
		UsageText: `This command will delete a user specified by their email in Mobius.`,
		Flags: []cli.Flag{
			&cli.StringFlag{
				Name:     emailFlagName,
