...
import (
	...
	"github.com/notawar/mobius/v4/cmd/osquery-perf/installer_cache"
	"github.com/notawar/mobius set/v4/cmd/osquery-perf/osquery_perf"
	"github.com/notawar/mobius set/v4/pkg/file"
	"github.com/notawar/mobius set/v4/pkg/mdm/mdmtest"
...
var (
	...
	macosVulnerableSoftware            []mobius.Software
	vsCodeExtensionsVulnerableSoftware []mobius.Software
...
func loadMacOSVulnerableSoftware() {
	...
	type vulnerableSoftware struct {
		Software []mobius.Software `json:"software"`
	}
...
func loadExtraVulnerableSoftware() {
	...
	for _, line := range lines {
		...
		vsCodeExtensionsVulnerableSoftware = append(vsCodeExtensionsVulnerableSoftware, mobius.Software{
...
type agent struct {
	...
	os                            string
	// deviceAuthToken holds Mobius Desktop device authentication token.
	//
	...
	// (client side) against Mobius MDM.
	...
	// Software installed on the host via Mobius. Key is the software name + version + bundle identifier.
	...
	// /api/v1/osquery/log requests to the Mobius server fail.
	//
...
func newAgent(
	...
	return &agent{
		...
		disableMobiusDesktop:      disableMobiusDesktop,
...
func (a *agent) runOrbitLoop() {
	...
	// checkToken is used to simulate Mobius Desktop polling until a token is
	...
	// Mobius Desktop performs a burst of check token requests when it's initialized
	if !a.disableMobiusDesktop {
		checkToken()
	}
...
func (a *agent) doDeclarativeManagement(cmd *mdm.Command) {
	...
	for _, d := range items.Declarations.Configurations {
		...
		report.StatusItems.Management.Declarations.Configurations = []mobius.MDMAppleDDMStatusDeclaration{
			{Active: true, Valid: mobius.MDMAppleDeclarationValid, Identifier: d.Identifier, ServerToken: d.ServerToken},
		}
	...
	return []map[string]string{
		{
			...
			"provider_id":           mobius.WellKnownMDMMobius,
...
func (a *agent) runLiveQuery(query string) (results []map[string]string, status *mobius.OsqueryStatus, message *string, stats *mobius.Stats) {
	if a.liveQueryFailProb > 0.0 && rand.Float64() <= a.liveQueryFailProb {
		ss := mobius.OsqueryStatus(1)
...
func (a *agent) runLiveYaraQuery(query string) (results []map[string]string, status *mobius.OsqueryStatus, message *string, stats *mobius.Stats) {
...
func (a *agent) processQuery(name, query string, cachedResults *cachedResults) (
	...
	status *mobius.OsqueryStatus, message *string, stats *mobius.Stats,
) {
	const (
		hostPolicyQueryPrefix = "mobius_policy_query_"
		hostDetailQueryPrefix = "mobius_detail_query_"
		liveQueryPrefix       = "mobius_distributed_query_"
	)
	statusOK := mobius.StatusOK
	statusNotOK := mobius.OsqueryStatus(1)
	...
	case name == hostDetailQueryPrefix+"munki_info":
		ss := mobius.OsqueryStatus(rand.Intn(2))
		if ss == mobius.StatusOK {
			results = a.munkiInfo()
		}
		return true, results, &ss, nil, nil
...
func (a *agent) DistributedWrite(queries map[string]string) error {
	r := service.SubmitDistributedQueryResultsRequest{
		Results:  make(mobius.OsqueryDistributedQueryResults),
		Statuses: make(map[string]mobius.OsqueryStatus),
		Messages: make(map[string]string),
		Stats:    make(map[string]*mobius.Stats),
	}
	r.NodeKey = a.nodeKey
	...
	for _, name := range queryNames {
		...
		if !handled {
			...
			r.Statuses[name] = mobius.StatusOK
		} else {
			if results != nil {
...
func main() {
	...
	var (
		...
		commonSoftwareCount                          = flag.Int("common_software_count", 10, "Number of common installed applications reported to mobius")
		commonVSCodeExtensionsSoftwareCount          = flag.Int("common_vscode_extensions_software_count", 5, "Number of common vscode_extensions installed applications reported to mobius")
		...
		// report software to Mobius, so the initial reads/inserts can be expensive).
		...
		vulnerableSoftwareCount     = flag.Int("vulnerable_software_count", 10, "Number of vulnerable installed applications reported to mobius.  Use -1 to load all vulnerable software.")
		withLastOpenedSoftwareCount = flag.Int("with_last_opened_software_count", 10, "Number of applications that may report a last opened timestamp to mobius")
		...
		commonUserCount             = flag.Int("common_user_count", 10, "Number of common host users reported to mobius")
		uniqueUserCount             = flag.Int("unique_user_count", 10, "Number of unique host users reported to mobius")
		...
		mdmProb               = flag.Float64("mdm_prob", 0.0, "Probability of a host enrolling via Mobius MDM (applies for macOS and Windows hosts, implies orbit enrollment on Windows) [0, 1]")
		...
		disableMobiusDesktop = flag.Bool("disable_mobius_desktop", false, "Disable Mobius Desktop")
