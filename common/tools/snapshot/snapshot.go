...
func restore(homedir string) error {
	...
	cmd := exec.Command("./common/tools/backup_db/restore.sh", snapshots[index].Path)
...
func snapshot(homedir string) error {
	...
	cmd := exec.Command("./common/tools/backup_db/backup.sh", filepath.Join(snapshotPath, "db.sql.gz"))
