...
- Note: Mobius server uses SQL prepared statements, and the default setting of MySQL DB server's [max_prepared_stmt_count](https://dev.mysql.com/doc/refman/8.0/en/server-system-variables.html#sysvar_max_prepared_stmt_count)
...
> Wondering where to get a license key?  You can [grab a license](https://mobiusmdm.com/new-license) or if you have questions, [schedule a demo](https://mobiusmdm.com/contact).
...
> Available in Mobius Premium. Activity information is available for all Mobius Free and Mobius Premium instances using the [Activities API](https://mobiusmdm.com/docs/using-mobius/rest-api#activities).
...
To use webhook logging for query results, the following two Mobius config values must *both* be set:
...
If `firehose_access_key_id` and `firehose_secret_access_key` are omitted, Mobius will try to use [AWS STS](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_credentials_temp.html) credentials.
...
This flag only has effect if `email.backend` or `MOBIUS_EMAIL_BACKEND` is set to `ses`. This configuration **is
...
When `disable_schedule` is set to `false` (the default), Mobius instances will try to create the `databases_path` if it doesn't exist.
...
To download the data streams, you can use `mobiuscli vulnerability-data-stream --dir ./somedir`. The contents downloaded can then be reviewed, and finally uploaded to the defined `databases_path` in the mobius instance(s) doing the vulnerability processing.
...
Mobius Sandbox no longer exists. Mobius might use this later to enable one-click, downloaded agents (mobiusdaemon) (noahtalerman 2024-06-26)

These configurations control how Mobius interacts with the
packaging server (coming soon).  These features are currently only intended to be used within
Mobius sandbox, but this is subject to change.
...
- Default value: `""`
- Environment variable: `MOBIUS_PACKAGING_GLOBAL_ENROLL_SECRET`
- Config file format:
  ```yaml
  packaging:
...
This is the name of the S3 bucket to store pre-built Mobius agent (mobiusdaemon) installers.

- Default value: ""
- Environment variable: `MOBIUS_PACKAGING_S3_BUCKET`
- Config file format:
  ```yaml
  packaging:
    s3:
...
- Default value: ""
- Environment variable: `MOBIUS_PACKAGING_S3_PREFIX`
- Config file format:
  ```yaml
  packaging:
    s3:
...
- Default value: ""
- Environment variable: `MOBIUS_PACKAGING_S3_ACCESS_KEY_ID`
- Config file format:
  ```yaml
  packaging:
    s3:
...
- Default value: ""
- Environment variable: `MOBIUS_PACKAGING_S3_SECRET_ACCESS_KEY`
- Config file format:
  ```yaml
  packaging:
    s3:
...
- Default value: ""
- Environment variable: `MOBIUS_PACKAGING_S3_STS_ASSUME_ROLE_ARN`
- Config file format:
  ```yaml
  packaging:
    s3:
...
- Default value: ""
- Environment variable: `MOBIUS_PACKAGING_S3_STS_EXTERNAL_ID`
- Config file format:
  ```yaml
  packaging:
    s3:
...
- Default value: ""
- Environment variable: `MOBIUS_PACKAGING_S3_ENDPOINT_URL`
- Config file format:
  ```yaml
  packaging:
    s3:
...
- Default value: false
- Environment variable: `MOBIUS_PACKAGING_S3_DISABLE_SSL`
- Config file format:
  ```yaml
  packaging:
    s3:
...
- Default value: false
- Environment variable: `MOBIUS_PACKAGING_S3_FORCE_S3_PATH_STYLE`
- Config file format:
  ```yaml
  packaging:
    s3:
...
- Default value: ""
- Environment variable: `MOBIUS_PACKAGING_S3_REGION`
- Config file format:
  ```yaml
  packaging:
    s3:
