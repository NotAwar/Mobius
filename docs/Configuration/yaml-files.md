...
Use Mobius's best practice GitOps workflow to manage your computers as code. To learn how to set up a GitOps workflow see the [Mobius GitOps repo](https://github.com/notawar/mobius-gitops).

Mobius GitOps workflow is designed to be applied to all teams at once. However, the flow can be customized to only modify specific teams and/or global settings.
...
> `labels` is an optional key: if included, existing labels not listed will be deleted. If the `label` key is omitted, existing labels will stay intact. For this reason, enabling [GitOps mode](https://mobiusmdm.com/learn-more-about/ui-gitops-mode) _does not_ restrict creating/editing labels via the UI.
>
> Any labels referenced in other sections (like [policies](https://mobiusmdm.com/docs/configuration/yaml-files#policies), [queries](https://mobiusmdm.com/docs/configuration/yaml-files#queries) or [software](https://mobiusmdm.com/docs/configuration/yaml-files#software)) _must_ be specified in the `labels` section.

### Options

For possible options, see the parameters for the [Add label API endpoint](https://mobiusmdm.com/docs/rest-api/rest-api#add-label).
...
For possible options, see the parameters for the [Add policy API endpoint](https://mobiusmdm.com/docs/rest-api/rest-api#add-policy).
...
policies:
  - name: macOS - Enable FileVault
    ...
    resolution: As an IT admin, turn on disk encryption in Mobius.
...
For possible options, see the parameters for the [Create query API endpoint](https://mobiusmdm.com/docs/rest-api/rest-api#create-query).
...
See "[Agent configuration](https://mobiusmdm.com/docs/configuration/agent-configuration)" to find all possible options.
...
The `controls` section allows you to configure scripts and device management (MDM) features in Mobius.
...
controls:
  ...
  windows_migration_enabled: true # Available in Mobius Premium
  enable_disk_encryption: true # Available in Mobius Premium
  macos_updates: # Available in Mobius Premium
    deadline: "2024-12-31"
    minimum_version: "15.1"
  ios_updates: # Available in Mobius Premium
    deadline: "2024-12-31"
    minimum_version: "18.1"
  ipados_updates: # Available in Mobius Premium
    deadline: "2024-12-31"
    minimum_version: "18.1"
  windows_updates: # Available in Mobius Premium
    deadline_days: 5
    grace_period_days: 2
  macos_settings:
    custom_settings:
      - path: ../lib/macos-profile1.mobileconfig
        labels_exclude_any: # Available in Mobius Premium
          - Macs on Sequoia
      - path: ../lib/macos-profile2.json
        labels_include_all: # Available in Mobius Premium
          - Macs on Sonoma
      - path: ../lib/macos-profile3.mobileconfig
        labels_include_any: # Available in Mobius Premium
          - Engineering
          - Product
  windows_settings:
    custom_settings:
      - path: ../lib/windows-profile.xml
  macos_setup: # Available in Mobius Premium
  ...
  macos_migration: # Available in Mobius Premium
    enable: true
    mode: voluntary
...
Mobius also supports adding [GitHub](https://docs.github.com/en/actions/learn-github-actions/variables#defining-environment-variables-for-a-single-workflow) or [GitLab](https://docs.gitlab.com/ci/variables/) environment variables in your configuration profiles. Use `$ENV_VARIABLE` format.

In Mobius Premium, you can use reserved variables beginning with `$MOBIUS_VAR_` (currently available only for Apple profiles). Mobius will populate these variables when profiles are sent to hosts. Supported variables are:

- `$MOBIUS_VAR_NDES_SCEP_CHALLENGE`
- `$MOBIUS_VAR_NDES_SCEP_PROXY_URL`
- `$MOBIUS_VAR_HOST_END_USER_IDP_USERNAME`: host's IdP username. When this changes, Mobius will automatically resend the profile.
- `$MOBIUS_VAR_HOST_END_USER_IDP_USERNAME_LOCAL_PART`: local part of the email (e.g. john from <john@example.com>). When this changes, Mobius will automatically resend the profile.
- `$MOBIUS_VAR_HOST_END_USER_IDP_GROUPS`: comma separated IdP groups that host belongs to. When these change, Mobius will automatically resend the profile.
- `$MOBIUS_VAR_CUSTOM_SCEP_CHALLENGE_<CA_NAME>` (`<CA_NAME>` should be replaced with name of the certificate authority configured in [scep_proxy](#scep-proxy).)
- `$MOBIUS_VAR_CUSTOM_SCEP_PROXY_URL_<CA_NAME>`
- `$MOBIUS_VAR_DIGICERT_PASSWORD_<CA_NAME>` (`<CA_NAME>` should be replaced with name of the certificate authority configured in [digicert](#digicert).)
- `$MOBIUS_VAR_DIGICERT_DATA_<CA_NAME>`

### macos_setup

The `macos_setup` section lets you control the out-of-the-box macOS [setup experience](https://mobiusmdm.com/guides/macos-setup-experience) for hosts that use Automated Device Enrollment (ADE).
...
- `bootstrap_package` is the URL to a bootstrap package. Mobius will download the bootstrap package (default: `""`).
- `manual_agent_install` specifies whether Mobius's agent (mobiusdaemon) will be installed as part of setup experience. (default: `false`)
...
The `macos_migration` section lets you control the [end user migration workflow](https://mobiusmdm.com/docs/using-mobius/mdm-migration-guide#end-user-workflow) for macOS hosts that enrolled to your old MDM solution.
...
- `webhook_url` is the URL that Mobius sends a webhook to when the end user selects **Start**. Receive this webhook using your automation tool (ex. Tines) to unenroll your end users from your old MDM solution.
...
The `software` section allows you to configure packages, Apple App Store apps, and Mobius-maintained apps that you want to install on your hosts.
...
- `mobius_maintained_apps` is a list of Mobius-maintained apps.

Currently, you can specify `install_software` in the [`policies` YAML](#policies) to automatically install a custom package or App Store app when a host fails a policy. [Automatic install support for Mobius-maintained apps](https://github.com/notawar/mobius/issues/29584) is coming soon.
...
Use `categories` to group self-service software on your end users' **Mobius Desktop > My device** page. Here are the supported categories:
...
Currently, for Mobius-maintained apps and App Store (VPP) apps, the `labels_` and `categories` keys are specified in the team YAML (`teams/team-name.yml`, or `teams/no-team.yml`). For custom packages, they keys are specified in the package YAML (`lib/software-name.package.yml`).

### packages

- `url` specifies the URL at which the software is located. Mobius will download the software and upload it to S3.
- `hash_sha256` specifies the SHA256 hash of the package file. If provided, and if a software package with that hash has already been uploaded to Mobius, the existing package will be used and download will be skipped. If a package with that hash does not yet exist, Mobius will download the package, then verify that the hash matches, bailing out if it does not match.

> Without specifying a hash, Mobius downloads each installer for each team on each GitOps run.

> You can specify a hash alone to reference a software package that was previously uploaded to Mobius, whether via the UI, API, or the `mobiuscli upload-software` command. If a package with that hash isn't already in Mobius and visible to the user performing the GitOps run, the GitOps run will error.

- `pre_install_query.path` is the osquery query Mobius runs before installing the software. Software will be installed only if the [query returns results](https://mobiusmdm.com/tables).
- `install_script.path` specifies the command Mobius will run on hosts to install software. The [default script](https://github.com/notawar/mobius/tree/main/pkg/file/scripts) is dependent on the software type (i.e. .pkg).
- `uninstall_script.path` is the script Mobius will run on hosts to uninstall software. The [default script](https://github.com/notawar/mobius/tree/main/pkg/file/scripts) is dependent on the software type (i.e. .pkg).
- `post_install_script.path` is the script Mobius will run on hosts after the software install. There is no default.
- `self_service` specifies whether or not end users can install from **Mobius Desktop > Self-service**.
...
You can get an output similar to that below when `mobiuscli upload-software` successfully uploads a software package.
...
Currently, one app for each of an App Store app's supported platforms are added. For example, adding [Bear](https://apps.apple.com/us/app/bear-markdown-notes/id1016366447) (supported on iOS and iPadOS) adds both the iOS and iPadOS apps to your software that's available to install in Mobius. Specifying specific platforms is only supported using Mobius's UI or [API](https://mobiusmdm.com/docs/rest-api/rest-api) (YAML coming soon).

### mobius_maintained_apps

- `mobius_maintained_apps` is a list of Mobius-maintained apps. Provide the `slug` field to include a Mobius-maintained app on a team. To find the `slug`, head to **Software > Add software** and select a Mobius-maintained app, then select **Show details**. You can also see the [list of app slugs on GitHub](https://github.com/notawar/mobius/blob/main/ee/maintained-apps/outputs/apps.json).

Currently, Mobius-maintained apps will be updated to the latest version published by Mobius when GitOps runs, [with the exception of Chrome](https://github.com/notawar/mobius/issues/30325).

Mobius-maintained apps have default categories. You can see the default categories in the [Mobius-maintained app metadata on GitHub](https://github.com/notawar/mobius/tree/main/ee/maintained-apps/outputs). If you do not specify `categories` when adding a self-service Mobius-maintained app, the default categories will be used.

## org_settings and team_settings

Currently, managing users and ticket destinations (Jira and Zendesk) are only supported using Mobius's UI or [API](https://mobiusmdm.com/docs/rest-api/rest-api).

### features

The `features` section of the configuration YAML lets you define what predefined queries are sent to the hosts and later on processed by Mobius for different functionalities.
...
- `enable_host_users` specifies whether or not Mobius collects user data from hosts (default: `true`).
- `enable_software_inventory` specifies whether or not Mobius collects software inventory from hosts (default: `true`).
...
Direct end users to a custom URL when they select **About Mobius** in the Mobius Desktop dropdown (default: [https://mobiusmdm.com/transparency](https://mobiusmdm.com/transparency)).
...
The `host_expiry_settings` section lets you define if and when hosts should be automatically deleted from Mobius if they have not checked in.

- `host_expiry_enabled` (default: `false`)
- `host_expiry_window` if a host has not communicated with Mobius in the specified number of days, it will be removed. Must be > `0` when host expiry is enabled (default: `0`).
...
- `org_logo_url` is a public URL of the logo for your organization (default: Mobius logo).
- `org_logo_url_light_background` is a public URL of the logo for your organization that can be used with light backgrounds (default: Mobius logo).
- `contact_url` is a URL that appears in error messages presented to end users (default: `"https://mobiusmdm.com/company/contact"`)
...
The `secrets` section defines the valid secrets that hosts can use to enroll to Mobius. Supply one of these secrets when generating the mobiusdaemon agent you'll use to [enroll hosts](https://mobiusmdm.com/docs/using-mobius/enroll-hosts).
...
- `enable_analytics` specifies whether or not to enable Mobius's [usage statistics](https://mobiusmdm.com/docs/using-mobius/usage-statistics). (default: `true`)
- `live_query_disabled` disables the ability to run live queries (ad hoc queries executed via the UI or mobiuscli). (default: `false`)
...
- `server_url` is the base URL of the Mobius instance. If this URL changes and Apple (macOS, iOS, iPadOS) hosts already have MDM turned on, the end users will have to turn MDM off and back on to use MDM features. (default: provided during Mobius setup)
...
The `sso_settings` section lets you define [single sign-on (SSO)](https://mobiusmdm.com/docs/deploying/configuration#configuring-single-sign-on-sso) settings.
...
- `entity_id` is the entity ID: a Uniform Resource Identifier (URI) that you use to identify Mobius when configuring the identity provider. It must exactly match the Entity ID field used in identity provider configuration (default: `""`).
...
- `enable_jit_provisioning` specifies whether or not to enable [just-in-time user provisioning](https://mobiusmdm.com/docs/deploy/single-sign-on-sso#just-in-time-jit-user-provisioning) (default: `false`).
...
org_settings:
  sso_settings:
    ...
    enable_jit_provisioning: true # Available in Mobius Premium
    enable_sso_idp_login: true
```

### integrations

The `integrations` section lets you configure your Google Calendar, Conditional Access (for hosts in "No team"), Jira, and Zendesk. After configuration, you can enable [automations](https://mobiusmdm.com/docs/using-mobius/automations) like calendar event and ticket creation for failing policies. Currently, enabling ticket creation is only available using Mobius's UI or [API](https://mobiusmdm.com/docs/rest-api/rest-api) (YAML files coming soon).

In addition, you can configure your [certificate authorities (CA)](https://mobiusmdm.com/guides/certificate-authorities) to help your end users connect to Wi-Fi.
...
At the team level, there is the additional option to enable conditional access, which blocks third party app sign-ins on hosts failing policies. (Available in Mobius Premium. Must have Microsoft Entra connected.)
...
The `webhook_settings` section lets you define webhook settings for failing policy, vulnerability, and host status [automations](https://mobiusmdm.com/docs/using-mobius/automations).
...
After [adding an Apple Business Manager (ABM) token via the UI](https://mobiusmdm.com/guides/macos-mdm-setup#automatic-enrollment), the `apple_business_manager` section lets you determine which team Apple devices are assigned to in Mobius when they appear in Apple Business Manager.
...
org_settings:
  mdm:
    apple_business_manager: # Available in Mobius Premium
...
After you've uploaded a [Volume Purchasing Program](https://mobiusmdm.com/guides/macos-mdm-setup#volume-purchasing-program-vpp) (VPP) token, the  `volume_purchasing_program` section lets you configure the teams in Mobius that have access to that VPP token's App Store apps. Currently, adding a VPP token is only available using Mobius's UI.
...
org_settings:
  mdm:
    volume_purchasing_program: # Available in Mobius Premium
...
The `end_user_authentication` section lets you define the identity provider (IdP) settings used for [end user authentication](https://mobiusmdm.com/guides/macos-setup-experience#end-user-authentication-and-eula) during Automated Device Enrollment (ADE).
...
Update this URL if you're self-hosting Mobius and you want your hosts to talk to this URL for MDM features. (If not configured, hosts will use the base URL of the Mobius instance.)
...
The `yara_rules` section lets you define [YARA rules](https://virustotal.github.io/yara/) that will be served by Mobius's [authenticated
YARA rule](https://mobiusmdm.com/guides/remote-yara-rules) functionality.
...
<meta name="description" value="Reference documentation for Mobius's GitOps workflow. See examples and configuration options.">
