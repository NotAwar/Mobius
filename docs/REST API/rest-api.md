...
- [Mobius configuration](#mobius-configuration)
...
Use the Mobius APIs to automate Mobius.
...
All API requests to the Mobius server require API token authentication unless noted in the documentation. API tokens are tied to your Mobius user account.

To get an API token, retrieve it from "My account" > "Get API token" in the Mobius UI (`/profile`). Or, you can send a request to the [login API endpoint](#log-in) to get your token.
...
Sends a password reset email to the specified email. Requires that SMTP or SES is configured for your Mobius server.

`POST /api/v1/mobius/forgot_password`
...
`POST /api/v1/mobius/change_password`
...
`POST /api/v1/mobius/reset_password`
...
`POST /api/v1/mobius/perform_required_password_reset`
...
This is the callback endpoint that the identity provider will use to send security assertions to Mobius. This is where Mobius receives and processes the response from the identify provider.

`POST /api/v1/mobius/sso/callback`
...
Returns a list of the activities that have been performed in Mobius. For a comprehensive list of activity types and detailed information, please see the [audit logs](https://mobiusmdm.com/docs/using-mobius/audit-activities) page.

`GET /api/v1/mobius/activities`
...
`GET /api/v1/mobius/activities?page=0&per_page=10&order_key=created_at&order_direction=desc`
...
Mobius supports osquery's file carving functionality as of Mobius 3.3.0. This allows the Mobius server to request files (and sets of files) from Mobius's agent (mobiusdaemon), returning the full contents to Mobius.

To initiate a file carve using the Mobius API, you can use the [live query](#run-live-query) endpoint to run a query against the `carves` table.

For more information on executing a file carve in Mobius, go to the [File carving with Mobius docs](https://github.com/notawar/mobius/blob/main/docs/Contributing/product-groups/orchestration/file-carving.md).
  ...
  "carves": [
    {
      ...
      "host_id": 7,
      "name": "macbook-pro.local-2021-02-23T22:52:01Z-mobius_distributed_query_30",
      "block_count": 1,
      "block_size": 2000000,
      "carve_size": 2048,
      "carve_id": "c6958b5f-4c10-4dc8-bc10-60aad5b20dc8",
      "request_id": "mobius_distributed_query_30",
    ...
    },
    {
      ...
      "host_id": 7,
      "name": "macbook-pro.local-2021-02-23T22:53:03Z-mobius_distributed_query_31",
      "block_count": 2,
      "block_size": 2000000,
      "carve_size": 3400704,
      "carve_id": "2b9170b9-4e11-4569-a97c-2f18d18bec7a",
      "request_id": "mobius_distributed_query_31",
...
`GET /api/v1/mobius/carves/:id/block/:block_id`
...
`GET /api/v1/mobius/carves/1/block/0`
...
The Mobius server exposes API endpoints that handle the configuration of Mobius as well as endpoints that manage enroll secret operations. These endpoints require prior authentication, you so you'll need to log in before calling any of the endpoints documented below.

### Get certificate

Returns the Mobius certificate.

`GET /api/v1/mobius/config/certificate`
...
Returns all information about the Mobius's configuration.

The `agent_options`, `sso_settings` and `smtp_settings` fields are only returned for admin and GitOps users with global access (see the [Role-based access guide](https://mobiusmdm.com/guides/role-based-access)).

`mdm.macos_settings.custom_settings`, `mdm.windows_settings.custom_settings`, `scripts`, and `mdm.macos_setup` only include the configuration profiles, scripts, and setup experience settings applied using [Mobius's YAML](https://mobiusmdm.com/docs/configuration/yaml-files). To list profiles, scripts, or setup experience settings added in the UI or API, use the [List configuration profiles](https://mobiusmdm.com/docs/rest-api/rest-api#list-custom-os-settings-configuration-profiles), [List scripts](https://mobiusmdm.com/docs/rest-api/rest-api#list-scripts), or GET endpoints from [Setup experience](https://mobiusmdm.com/docs/rest-api/rest-api#setup-experience) instead.

`GET /api/v1/mobius/config`
...
```json
{
  ...
  "server_settings": {
    "server_url": "https://instance.mobius.com",
  ...
  "mdm": {
    ...
    "client_url": "https://instance.mobius.com"
  },
  "agent_options": {
    "spec": {
      "config": {
    ...
    "google_calendar": [
      {
        ...
        "api_key_json": {
           ...
           "project_id": "mobius-in-your-calendar",
           "private_key_id": "<private key id>",
           "private_key": "-----BEGIN PRIVATE KEY-----\n<private key>\n-----END PRIVATE KEY-----\n",
           "client_email": "mobius-calendar-events@mobius-in-your-calendar.iam.gserviceaccount.com",
           "client_id": "<client id>",
           "auth_uri": "https://accounts.google.com/o/oauth2/auth",
           "token_uri": "https://oauth2.googleapis.com/token",
           "auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs",
           "client_x509_cert_url": "https://www.googleapis.com/robot/v1/metadata/x509/mobius-calendar-events%40mobius-in-your-calendar.iam.gserviceaccount.com",
    ...
    "digicert": [
      {
        ...
        "certificate_common_name": "$MOBIUS_VAR_HOST_HARDWARE_SERIAL@example.com",
        "certificate_user_principal_names": [
          "$MOBIUS_VAR_HOST_HARDWARE_SERIAL@example.com",
        ],
        "certificate_seat_id": "$MOBIUS_VAR_HOST_HARDWARE_SERIAL@example.com"
      }
    ],
    "ndes_scep_proxy": {
...
Modifies the Mobius's configuration with the supplied information.

`PATCH /api/v1/mobius/config`
...
| agent_options            | objects | body  | The agent_options spec that is applied to all hosts. In Mobius 4.0.0 the `api/v1/mobius/spec/osquery_options` endpoints were removed.  |
| mobius_desktop            | object  | body  | See [mobius_desktop](#mobius-desktop).                                                                                                 |
...
```json
{
  "org_info": {
    ...
    "org_logo_url": "https://mobiusmdm.com/logo.png",
    "org_logo_url_light_background": "https://mobiusmdm.com/logo-light.png",
    "contact_url": "https://mobiusmdm.com/company/contact"
  },
  "server_settings": {
    "server_url": "https://instance.mobius.com",
  ...
  "mdm": {
    ...
    "apple_server_url": "https://instance.mobius.com"
  },
  "agent_options": {
    "config": {
      "options": {
  ...
  "mobius_desktop": {
    "transparency_url": "https://mobiusmdm.com/better"
  },
  "gitops": {
    "gitops_mode_enabled": false,
    "repository_url": "",
...
| org_logo_url_light_background     | string  | The URL for the organization logo displayed in Mobius on top of light backgrounds.   |
...
| Name                              | Type    | Description   |
| ---------------------             | ------- | ------------------------------------------------------------------------------------------- |
| server_url                        | string  | The Mobius server URL.                                                                       |
| enable_analytics                  | boolean | Whether to send anonymous usage statistics. Always enabled for Mobius Premium customers.     |
...
| Name                              | Type    | Description   |
| ---------------------             | ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| enable_smtp                       | boolean | Whether SMTP is enabled for the Mobius app.                                                                                                                            |
| sender_address                    | string  | The sender email address for the Mobius app. An invitation email is an example of the emails that may use this sender address                                          |
| server                            | string  | The SMTP server for the Mobius app.                                                                                                                                    |
| port                              | integer | The SMTP port for the Mobius app.                                                                                                                                      |
...
| Name                              | Type    | Description   |
| ---------------------             | ------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| enable_sso                        | boolean | Whether or not SSO is enabled for the Mobius application. If this value is true, you must also include most of the SSO settings parameters below.                       |
| entity_id                         | string  | The required entity ID is a URI that you use to identify Mobius when configuring the identity provider. Must be 5 or more characters.                                   |
...
| enable_sso_idp_login              | boolean | Determines whether Identity Provider (IdP) initiated login for Single sign-on (SSO) is enabled for the Mobius application.                                              |
| enable_jit_provisioning           | boolean | _Available in Mobius Premium._ When enabled, allows [just-in-time user provisioning](https://mobiusmdm.com/docs/deploy/single-sign-on-sso#just-in-time-jit-user-provisioning). |
...
| Name                              | Type    | Description   |
| ---------------------             | ------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| host_expiry_enabled               | boolean | When enabled, allows automatic cleanup of hosts that have not communicated with Mobius in some number of days.                                                  |
| host_expiry_window                | integer | If a host has not communicated with Mobius in the specified number of days, it will be removed. Must be greater than 0 if host_expiry_enabled is set to true.   |
...
_Available in Mobius Premium._

| Name                              | Type    | Description   |
| ---------------------             | ------- | -------------------------------------------------------------------------------- |
| transparency_url                  | string  | The URL used to display transparency information to users of Mobius Desktop.      |
...
| Name                              | Type    | Description   |
| ---------------------             | ------- | --------------------------------------------------------------------------------------------------------------------------------- |
| gitops_mode_enabled               | boolean | Whether to enable "GitOps mode", which restricts making changes via the UI that would be overridden by running `mobiuscli-gitops`. (Default: `false`) |
| repository_url                    | string  | The URL for the repository where changes are managed, for Mobius instances using GitOps. Users will be sent here when GitOps mode is enabled. |
...
| host_percentage                   | integer | The minimum percentage of hosts that must fail to check in to Mobius in order to trigger the webhook request.                                |
| days_count                        | integer | The minimum number of days that the configured `host_percentage` must fail to check in to Mobius in order to trigger the webhook request.    |
    ...
    "digicert": [
      {
        ...
        "profile_id": "7ed77396-9186-4bfa-9fa7-63dddc46b8a3",
        "certificate_common_name": "$MOBIUS_VAR_HOST_HARDWARE_SERIAL@example.com",
        "certificate_subject_alternative_name": "$MOBIUS_VAR_HOST_HARDWARE_SERIAL@example.com",
        "certificate_seat_id": "$MOBIUS_VAR_HOST_HARDWARE_SERIAL@example.com"
      }
    ],
    "ndes_scep_proxy": {
...
| enable_disk_encryption            | boolean | _Available in Mobius Premium._ Hosts that belong to no team will have disk encryption enabled if set to true. |
...
| apple_server_url         | string  | Update this URL if you're self-hosting Mobius and you want your hosts to talk to this URL for MDM features. (If not configured, hosts will use the base URL of the Mobius instance.)  |
...
| Name                              | Type    | Description   |
| ---------------------             | ------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| minimum_version                   | string  | Hosts that belong to no team and are enrolled into Mobius's MDM will be prompted to update when their OS is below this version. |
| deadline                          | string  | Hosts that belong to no team and are enrolled into Mobius's MDM will be forced to update their OS after this deadline (noon local time for hosts already on macOS 14 or above, 20:00 UTC for hosts on earlier macOS versions). |
...
| Name                              | Type    | Description   |
| ---------------------             | ------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| deadline_days                     | integer | Hosts that belong to no team and are enrolled into Mobius's MDM will have this number of days before updates are installed on Windows. |
| grace_period_days                 | integer | Hosts that belong to no team and are enrolled into Mobius's MDM will have this number of days before Windows restarts to install updates. |
...
| Name                              | Type    | Description   |
| ---------------------             | ------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| enable_end_user_authentication    | boolean | If set to true, end user authentication will be required during automatic MDM enrollment of new macOS devices. Settings for your IdP provider must also be [configured](https://mobiusmdm.com/docs/using-mobius/mdm-macos-setup-experience#end-user-authentication-and-eula). |
...
| Name                              | Type    | Description   |
| ---------------------             | ------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| custom_settings                   | array   | Only intended to be used by [Mobius's YAML](https://mobiusmdm.com/docs/configuration/yaml-files). To add macOS configuration profiles using Mobius's API, use the [Add configuration profile endpoint](https://mobiusmdm.com/docs/rest-api/rest-api#add-custom-os-setting-configuration-profile) instead. |
...
| Name                              | Type    | Description   |
| ---------------------             | ------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| entity_id                         | string  | **Required**. The entity ID is a URI that you use to identify Mobius when configuring the identity provider. Must be 5 or more characters.                                   |
...
| Name                              | Type    | Description   |
| ---------------------             | ------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| enable_host_users                 | boolean | Whether to enable the users feature in Mobius. (Default: `true`)                                                                          |
| enable_software_inventory         | boolean | Whether to enable the software inventory feature in Mobius. (Default: `true`)                                                             |
...
`GET /api/v1/mobius/spec/enroll_secret`
...
`POST /api/v1/mobius/spec/enroll_secret`
...
`GET /api/v1/mobius/teams/:id/secrets`
...
`GET /api/v1/mobius/teams/1/secrets`
...
`PATCH /api/v1/mobius/teams/:id/secrets`
...
`PATCH /api/v1/mobius/teams/2/secrets`
...
Get version and build information from the Mobius server.

`GET /api/v1/mobius/version`
...
- `detail_updated_at`: the last time Mobius updated host data, based on the results from the detail queries (this includes updates to host associated tables, e.g. `host_users`).
- `label_updated_at`: the last time Mobius updated the label membership for the host based on the results from the queries ran.
- `last_enrolled_at`: the last time the host enrolled to Mobius.
- `policy_updated_at`: the last time we updated the policy results for the host based on the queries ran.
- `seen_time`: the last time the host contacted the mobius server, regardless of what operation it was for.
...
| team_id                 | integer | query | _Available in Mobius Premium_. Filters to only include hosts in the specified team. Use `0` to filter by hosts assigned to "No team".                                                                                                                                                                                                                                                |
...
| connected_to_mobius   | boolean  | query | Filter hosts that are talking to this Mobius server for MDM features. In rare cases, hosts can be enrolled to one Mobius server but talk to a different Mobius server for MDM features. In this case, the value would be `false`. Always `false` for Linux hosts.                                                                                                                           |
| macos_settings          | string  | query | Filters the hosts by the status of the _mobile device management_ (MDM) profiles applied to hosts. Valid options are 'verified', 'verifying', 'pending', or 'failed'. **Note: If this filter is used in Mobius Premium without a team ID filter, the results include only hosts that are not assigned to any team.**                                                                                                                                                                                                             |
| munki_issue_id          | integer | query | The ID of the _munki issue_ (a Munki-reported error or warning message) to filter hosts by (that is, filter hosts that are affected by that corresponding error or warning message).                                                                                                                                                        |
| low_disk_space          | integer | query | _Available in Mobius Premium_. Filters the hosts to only include hosts with less GB of disk space available than this value. Must be a number between 1-100.                                                                                                                                                                                  |
...
| bootstrap_package       | string | query | _Available in Mobius Premium_. Filters the hosts by the status of the MDM bootstrap package on the host. Valid options are 'installed', 'pending', or 'failed'. |
| os_settings          | string  | query | Filters the hosts by the status of the operating system settings applied to the hosts. Valid options are 'verified', 'verifying', 'pending', or 'failed'. **Note: If this filter is used in Mobius Premium without a team ID filter, the results include only hosts that are not assigned to any team.** |
| os_settings_disk_encryption | string | query | Filters the hosts by disk encryption status. Valid options are 'verified', 'verifying', 'action_required', 'enforcing', 'failed', or 'removing_enforcement'.  **Note: If this filter is used in Mobius Premium without a team ID filter, the results include only hosts that are not assigned to any team.** |
| populate_software     | string | query | If `false` (or omitted), omits installed software details for each host. If `"without_vulnerability_details"`, include a list of installed software for each host, including which CVEs apply to the installed software versions. `true` adds vulnerability description, CVSS score, and other details when using Mobius Premium. See notes below on performance. |
| populate_policies     | boolean | query | If `true`, the response will include policy data for each host, including Mobius-maintained policies. |
...
> `software_id` is deprecated as of Mobius 4.42. It is maintained for backwards compatibility. Please use the `software_version_id` instead.

> `populate_software` returns a lot of data per host when set, and drastically more data when set to `true` on Mobius Premium. If you need vulnerability details for a large number of hosts, consider setting `populate_software` to `without_vulnerability_details` and pulling vulnerability details from the [Get vulnerability](#get-vulnerability) endpoint, as this returns details once per vulnerability rather than once per vulnerability per host.
...
`GET /api/v1/mobius/hosts?page=0&per_page=100&order_key=hostname&query=2ce&populate_software=true&populate_policies=true&populate_users=true&populate_labels=true`
  ...
  "hosts": [
    {
      ...
      "issues": {
        ...
        "critical_vulnerabilities_count": 2, // Mobius Premium only
      ...
      "mdm": {
        ...
        "name": "Mobius",
        "server_url": "https://example.mobiusmdm.com/mdm/apple/mdm"
      },
      "software": [
        {
          "id": 1,
          ...
          "vulnerabilities": [
            {
              ...
              "cvss_score": 7.5, // Mobius Premium only
              "epss_probability": 0.01537, // Mobius Premium only
              "cisa_known_exploit": false, // Mobius Premium only
              "cve_published": "2022-01-01 12:32:00", // Mobius Premium only
              "cve_description": "In the GNU C Library (aka glibc or libc6) before 2.28, parse_reg_exp in posix/regcomp.c misparses alternatives, which allows attackers to cause a denial of service (assertion failure and application exit) or trigger an incorrect result by attempting a regular-expression match.", // Mobius Premium only
              "resolved_in_version": "2.28" // Mobius Premium only
            }
          ],
...
> Note: the response above assumes a [GeoIP database is configured](https://mobiusmdm.com/docs/deploying/configuration#geoip), otherwise the `geolocation` object won't be included.
...
| status                  | string  | query | Indicates the status of the hosts to return. Can either be 'new', 'online', 'offline', 'mia' or 'missing'.                                                                                                                                                                                                                                  |
| query                   | string  | query | Search query keywords. Searchable fields include `hostname`, `hardware_serial`, `uuid`, `ipv4` and the hosts' email addresses (only searched if the query looks like an email address, i.e. contains an '@', no space, etc.).                                                                                                                |
| team_id                 | integer | query | _Available in Mobius Premium_. Filters the hosts to only include hosts in the specified team.                                                                                                                                                                                                                                                 |
| policy_id               | integer | query | The ID of the policy to filter hosts by.                                                                                                                                                                                                                                                                                                    |
| policy_response         | string  | query | **Requires `policy_id`**. Valid options are 'passing' or 'failing'.                                                                                                                                                                                                                                       |
| software_version_id     | integer | query | The ID of the software version to filter hosts by.                                                                                                            |
| software_title_id       | integer | query | The ID of the software title to filter hosts by.                                                                                                              |
...
| low_disk_space          | integer | query | _Available in Mobius Premium_. Filters the hosts to only include hosts with less GB of disk space available than this value. Must be a number between 1-100.                                                                                                                                                                                  |
| macos_settings_disk_encryption | string | query | Filters the hosts by disk encryption status. Valid options are 'verified', 'verifying', 'action_required', 'enforcing', 'failed', or 'removing_enforcement'. |
| bootstrap_package       | string | query | _Available in Mobius Premium_. Filters the hosts by the status of the MDM bootstrap package on the host. Valid options are 'installed', 'pending', or 'failed'. **Note: If this filter is used in Mobius Premium without a team ID filter, the results include only hosts that are not assigned to any team.** |
| os_settings          | string  | query | Filters the hosts by the status of the operating system settings applied to the hosts. Valid options are 'verified', 'verifying', 'pending', or 'failed'. **Note: If this filter is used in Mobius Premium without a team ID filter, the results include only hosts that are not assigned to any team.** |
| os_settings_disk_encryption | string | query | Filters the hosts by disk encryption status. Valid options are 'verified', 'verifying', 'action_required', 'enforcing', 'failed', or 'removing_enforcement'.  **Note: If this filter is used in Mobius Premium without a team ID filter, the results include only hosts that are not assigned to any team.** |
...
`GET /api/v1/mobius/hosts/count?page=0&per_page=100&order_key=hostname&query=2ce`
...
Returns the count of all hosts organized by status. `online_count` includes all hosts currently enrolled in Mobius. `offline_count` includes all hosts that haven't checked into Mobius recently. `mia_count` includes all hosts that haven't been seen by Mobius in more than 30 days. `new_count` includes the hosts that have been enrolled to Mobius in the last 24 hours.

`GET /api/v1/mobius/host_summary`

#### Parameters

| Name            | Type    | In    | Description                                                                     |
| --------------- | ------- | ----  | ------------------------------------------------------------------------------- |
| team_id         | integer | query | _Available in Mobius Premium_. The ID of the team whose host counts should be included. Defaults to all teams. |
| platform        | string  | query | Platform to filter by when counting. Defaults to all platforms.                 |
| low_disk_space  | integer | query | _Available in Mobius Premium_. Returns the count of hosts with less GB of disk space available than this value. Must be a number between 1-100. |

#### Example

`GET /api/v1/mobius/host_summary?team_id=1&low_disk_space=32`
...
| exclude_mobius_maintained_policies | boolean | query | If `true`, will omit Mobius-maintained policies from the policies list. |

#### Example

`GET /api/v1/mobius/hosts/121`
...
```json
{
  "host": {
    ...
    "issues": {
      "failing_policies_count": 1,
      "critical_vulnerabilities_count": 2, // Available in Mobius Premium
      "total_issues_count": 3
    },
    "batteries": [
      {
...
> Note:
>
> - `orbit_version: null` means this agent is not a mobiusdaemon agent
> - `mobius_desktop_version: null` means this agent is not a mobiusdaemon agent, or this agent is version <=1.23.0 which is not collecting the desktop version
> - `mobius_desktop_version: ""` means this agent is a mobiusdaemon agent but does not have mobius desktop
> - `scripts_enabled: null` means this agent is not a mobiusdaemon agent, or this agent is version <=1.23.0 which is not collecting the scripts enabled info

> Note: [Get human-device mapping](https://github.com/notawar/mobius/blob/62dc32454f6a40e81fe229abdfc370d3bf7a56c6/docs/REST%20API/rest-api.md?plain=1#L3518) is deprecated as of Mobius 4.67.0. It is maintained for backwards compatibility. Please use the [Get host](#get-host) endpoint to get human-device mapping.

### Get host by identifier

Returns the information of the host specified using the `hostname`, `uuid`, or `hardware_serial` as an identifier.

If `hostname` is specified when there is more than one host with the same hostname, the endpoint returns the first matching host. In Mobius, hostnames are fully qualified domain names (FQDNs). `hostname` (e.g. johns-macbook-air.local) is not the same as `display_name` (e.g. John's MacBook Air).

`GET /api/v1/mobius/hosts/identifier/:identifier`
...
`GET /api/v1/mobius/hosts/identifier/392547dc-0000-0000-a87a-d701ff75bc65`
...
This is the API route used by the **My device** page in Mobius desktop to display information about the host to the end user.

`GET /api/v1/mobius/device/:token`
...
`GET /api/v1/mobius/device/abcdef012456789`
...
Deletes the specified host from Mobius. Note that a deleted host will fail authentication with the previous node key, and in most osquery configurations will attempt to re-enroll automatically. If the host still has a valid enroll secret, it will re-enroll successfully.

`DELETE /api/v1/mobius/hosts/:id`
...
`DELETE /api/v1/mobius/hosts/121`
...
`POST /api/v1/mobius/hosts/:id/refetch`
...
`POST /api/v1/mobius/hosts/121/refetch`
...
`POST /api/v1/mobius/hosts/transfer`
...
`POST /api/v1/mobius/hosts/transfer/filter`
...
`DELETE /api/v1/mobius/hosts/:id/mdm`

#### Parameters

| Name | Type    | In   | Description                           |
| ---- | ------- | ---- | ------------------------------------- |
| id   | integer | path | **Required.** The host's ID in Mobius. |

#### Example

`DELETE /api/v1/mobius/hosts/42/mdm`
...
`POST /api/v1/mobius/hosts/delete`
...
`PUT /api/v1/mobius/hosts/:id/device_mapping`
...
`PUT /api/v1/mobius/hosts/1/device_mapping`
...
`GET /api/v1/mobius/hosts/:id/health`
...
`GET /api/v1/mobius/hosts/1/health`
...
```json
{
  ...
  "health": {
    ...
    "disk_encryption_enabled": true,
    "failing_policies_count": 1,
    "failing_critical_policies_count": 1, // Mobius Premium only
    "failing_policies": [
      {
        "id": 123,
        "name": "Google Chrome is up to date",
        "critical": true, // Mobius Premium only
...
in [Mobius's agent (mobiusdaemon)](https://mobiusmdm.com/docs/get-started/anatomy#mobiusdaemon).
...
`GET /api/v1/mobius/hosts/:id/mdm`
...
`GET /api/v1/mobius/hosts/32/mdm`
...
`GET /api/v1/mobius/hosts/summary/mdm`

#### Parameters

| Name     | Type    | In    | Description                                                                                                                                                                                                                                                                                                                        |
| -------- | ------- | ----- | -------------------------------------------------------------------------------- |
| team_id  | integer | query | _Available in Mobius Premium_. Filter by team                                      |
...
`GET /api/v1/mobius/hosts/summary/mdm?team_id=1&platform=windows`
...
`GET /api/v1/mobius/hosts/:id/macadmins`
...
`GET /api/v1/mobius/hosts/32/macadmins`
...
`GET /api/v1/mobius/macadmins`

#### Parameters

| Name    | Type    | In    | Description                                                                                                                                                                                                                                                                                                                        |
| ------- | ------- | ----- | ---------------------------------------------------------------------------------------------------------------- |
| team_id | integer | query | _Available in Mobius Premium_. Filters the aggregate host information to only include hosts in the specified team. |                           |
...
`GET /api/v1/mobius/hosts/:id/software`
...
| order_direction | string | query | **Requires `order_key`**. The direction of the order given the order key. Options include `"asc"` and `"desc"`. Default is `"asc"`. |
| min_cvss_score | integer | query | _Available in Mobius Premium_. Filters to include only software with vulnerabilities that have a CVSS version 3.x base score higher than the specified value.   |
| max_cvss_score | integer | query | _Available in Mobius Premium_. Filters to only include software with vulnerabilities that have a CVSS version 3.x base score lower than what's specified.   |
| exploit | boolean | query | _Available in Mobius Premium_. If `true`, filters to only include software with vulnerabilities that have been actively exploited in the wild (`cisa_known_exploit: true`). Default is `false`.  |

#### Example

`GET /api/v1/mobius/hosts/123/software`
...
`GET /api/v1/mobius/hosts/report`
...
`GET /api/v1/mobius/hosts/report?software_id=123&format=csv&columns=hostname,primary_ip,platform`
...
`GET /api/v1/mobius/hosts/:id/encryption_key`
...
`GET /api/v1/mobius/hosts/8/encryption_key`
...
Available for macOS, iOS, and iPadOS hosts only. Requires Mobius's MDM properly [enabled and configured](https://mobiusmdm.com/docs/using-mobius/mdm-setup).

Retrieves the certificates installed on a host.

`GET /api/v1/mobius/hosts/:id/certificates`
...
`GET /api/v1/mobius/hosts/8/certificates`
  ...
  "certificates": [
    {
      ...
      "subject": {
        ...
        "organizational_unit": "Mobius Device Management Inc.",
        "common_name": "MobiusDM"
      },
      "issuer": {
        "country": "US",
...
Requires Mobius's MDM properly [enabled and configured](https://mobiusmdm.com/docs/using-mobius/mdm-setup).

Retrieves a list of the configuration profiles assigned to a host.

`GET /api/v1/mobius/hosts/:id/configuration_profiles`
...
`GET /api/v1/mobius/hosts/8/configuration_profiles`
...
To lock a macOS host, the host must have MDM turned on. To lock a Windows or Linux host, the host must have [scripts enabled](https://mobiusmdm.com/docs/using-mobius/scripts).

`POST /api/v1/mobius/hosts/:id/lock`
...
`POST /api/v1/mobius/hosts/123/lock`
...
`POST /api/v1/mobius/hosts/123/lock?view_pin=true`
...
> To verify the host successfully locked, you can use the [Get host](https://mobiusmdm.com/docs/rest-api/rest-api#get-host) endpoint to retrieve the host's `mdm.device_status`.
...
To unlock a Windows or Linux host, the host must have [scripts enabled](https://mobiusmdm.com/docs/using-mobius/scripts).

`POST /api/v1/mobius/hosts/:id/unlock`
...
> To verify the host successfully unlocked, you can use the [Get host](https://mobiusmdm.com/docs/rest-api/rest-api#get-host) endpoint to retrieve the host's `mdm.device_status`. macOS hosts require entering `unlock_pin` to unlock.

### Wipe host

Sends a command to wipe the specified macOS, iOS, iPadOS, Windows, or Linux host. The host is wiped once it comes online.

To wipe a macOS, iOS, iPadOS, or Windows host, the host must have MDM turned on. To lock a Linux host, the host must have [scripts enabled](https://mobiusmdm.com/docs/using-mobius/scripts).

`POST /api/v1/mobius/hosts/:id/wipe`
...
`POST /api/v1/mobius/hosts/123/wipe`
...
> To verify the host was successfully wiped, you can use the [Get host](https://mobiusmdm.com/docs/rest-api/rest-api#get-host) endpoint to retrieve the host's `mdm.device_status`.

### Get host's past activity

`GET /api/v1/mobius/hosts/:id/activities`
...
`GET /api/v1/mobius/hosts/12/activities`
...
`GET /api/v1/mobius/hosts/:id/activities/upcoming`
...
`GET /api/v1/mobius/hosts/12/activities/upcoming`
...
`DELETE /api/v1/mobius/hosts/:id/activities/upcoming/:activity_id`
...
`DELETE /api/v1/mobius/hosts/12/activities/upcoming/81e10a70-730b-4c45-9b40-b14373e04757`
...
`POST /api/v1/mobius/hosts/:id/labels`
...
`POST /api/v1/mobius/hosts/12/labels`
...
`DELETE /api/v1/mobius/hosts/:id/labels`
...
`DELETE /api/v1/mobius/hosts/12/labels`
...
The live query will stop if the targeted host is offline, or if the query times out. Timeouts happen if the host hasn't responded after the configured `MOBIUS_LIVE_QUERY_REST_PERIOD` (default 25 seconds) or if the `distributed_interval` agent option (default 10 seconds) is higher than the `MOBIUS_LIVE_QUERY_REST_PERIOD`.

`POST /api/v1/mobius/hosts/:id/query`
...
`POST /api/v1/mobius/hosts/123/query`
...
`POST /api/v1/mobius/hosts/identifier/:identifier/query`
...
`POST /api/v1/mobius/hosts/identifier/392547dc-0000-0000-a87a-d701ff75bc65/query`
...
| host_ids    | array | body | The list of Mobius host IDs the label will apply to.  Only one of either `query` (to create a dynamic label) or `hosts`/`host_ids` (to create a manual label)  can be included in the request.
...
| host_ids    | array   | body | If updating a manual label: the list of Mobius host IDs the label will apply to. The provided list fully replaces the previous list. Only one of either `hosts` or `host_ids` can be included in the request.

#### Example

`PATCH /api/v1/mobius/labels/21`
...
Returns a list of all the labels in Mobius.

`GET /api/v1/mobius/labels/summary`
...
`GET /api/v1/mobius/labels/:id/hosts`
...
`GET /api/v1/mobius/labels/6/hosts&query=floobar`
...
`DELETE /api/v1/mobius/labels/:name`
...
`DELETE /api/v1/mobius/labels/ubuntu_label`
...
`DELETE /api/v1/mobius/labels/id/:id`
...
`DELETE /api/v1/mobius/labels/id/13`
...
> [Add custom macOS setting](https://github.com/notawar/mobius/blob/mobius-v4.40.0/docs/REST%20API/rest-api.md#add-custom-macos-setting-configuration-profile) (`POST /api/v1/mobius/mdm/apple/profiles`) API endpoint is deprecated as of Mobius 4.41. It is maintained for backwards compatibility. Please use the below API endpoint instead.

Add a configuration profile to enforce custom settings on macOS and Windows hosts.

`POST /api/v1/mobius/configuration_profiles`
...
| team_id                   | string   | form | _Available in Mobius Premium_. The team ID for the profile. If specified, the profile is applied to only hosts that are assigned to the specified team. If not specified, the profile is applied to only to hosts that are not assigned to any team. |
| labels_include_all        | array     | form | _Available in Mobius Premium_. Target hosts that have all labels in the array. |
| labels_include_any      | array     | form | _Available in Mobius Premium_. Target hosts that have any label in the array. |
| labels_exclude_any | array | form | _Available in Mobius Premium_. Target hosts that that don’t have any label in the array. |
...
> [List custom macOS settings](https://github.com/notawar/mobius/blob/mobius-v4.40.0/docs/REST%20API/rest-api.md#list-custom-macos-settings-configuration-profiles) (`GET /api/v1/mobius/mdm/apple/profiles`) API endpoint is deprecated as of Mobius 4.41. It is maintained for backwards compatibility. Please use the below API endpoint instead.

Get a list of the configuration profiles in Mobius.

For Mobius Premium, the list can
...
`GET /api/v1/mobius/configuration_profiles`

#### Parameters

| Name                      | Type   | In    | Description                                                               |
| ------------------------- | ------ | ----- | ------------------------------------------------------------------------- |
| team_id                   | string | query | _Available in Mobius Premium_. The team id to filter profiles.              |
| page                      | integer | query | Page number of the results to fetch.                                     |
| per_page                  | integer | query | Results per page.                                                        |

#### Example

List all configuration profiles for macOS and Windows hosts enrolled to Mobius's MDM that are not assigned to any team.
...
> [Download custom macOS setting](https://github.com/notawar/mobius/blob/mobius-v4.40.0/docs/REST%20API/rest-api.md#download-custom-macos-setting-configuration-profile) (`GET /api/v1/mobius/mdm/apple/profiles/:profile_id`) API endpoint is deprecated as of Mobius 4.41. It is maintained for backwards compatibility. Please use the API endpoint below instead.

`GET /api/v1/mobius/configuration_profiles/:profile_uuid`
...
`GET /api/v1/mobius/configuration_profiles/f663713f-04ee-40f0-a95a-7af428c351a9`
...
`GET /api/v1/mobius/configuration_profiles/f663713f-04ee-40f0-a95a-7af428c351a9?alt=media`
...
> [Delete custom macOS setting](https://github.com/notawar/mobius/blob/mobius-v4.40.0/docs/REST%20API/rest-api.md#delete-custom-macos-setting-configuration-profile) (`DELETE /api/v1/mobius/mdm/apple/profiles/:profile_id`) API endpoint is deprecated as of Mobius 4.41. It is maintained for backwards compatibility. Please use the below API endpoint instead.

`DELETE /api/v1/mobius/configuration_profiles/:profile_uuid`
...
`DELETE /api/v1/mobius/configuration_profiles/f663713f-04ee-40f0-a95a-7af428c351a9`
...
`POST /api/v1/mobius/hosts/:id/configuration_profiles/:profile_uuid/resend`
...
`POST /api/v1/mobius/hosts/233/configuration_profiles/fc14a20-84a2-42d8-9257-a425f62bb54d/resend`
...
`POST /api/v1/mobius/configuration_profiles/resend/batch`
...
`POST /api/v1/mobius/configuration_profiles/batch/resend`
...
> The `PATCH /api/v1/mobius/mdm/apple/settings` API endpoint is deprecated as of Mobius 4.45. It is maintained for backward compatibility. Please use the new API endpoint below. You can view [archived docuementation for the deprecated endpoint](https://github.com/iansltx/mobius/blob/d1791518a43c9d290192dbf992bcea290c8158a3/docs/REST%20API/rest-api.md#update-disk-encryption-enforcement).

_Available in Mobius Premium_

`POST /api/v1/mobius/disk_encryption`
...
`GET /api/v1/mobius/disk_encryption`

#### Parameters

| Name                      | Type   | In    | Description                                                               |
| ------------------------- | ------ | ----- | ------------------------------------------------------------------------- |
| team_id                   | string | query | _Available in Mobius Premium_. The team ID to filter the summary.           |
...
> [Get macOS settings statistics](https://github.com/notawar/mobius/blob/mobius-v4.40.0/docs/REST%20API/rest-api.md#get-macos-settings-statistics) (`GET /api/v1/mobius/mdm/apple/profiles/summary`) API endpoint is deprecated as of Mobius 4.41. It is maintained for backwards compatibility. Please use the below API endpoint instead.

Get aggregate status counts of all OS settings (configuration profiles and disk encryption) enforced on hosts.

For Mobius Premium users, the counts can
...
`GET /api/v1/mobius/configuration_profiles/summary`
...
`GET /api/v1/mobius/configuration_profile/:profile_uuid/status`
...
`GET /api/v1/mobius/configuration_profile/f663713f-04ee-40f0-a95a-7af428c351a9/status`
...
`POST /api/v1/mobius/enrollment_profiles/automatic`
...
`GET /api/v1/mobius/enrollment_profiles/automatic`
...
`GET /api/v1/mobius/enrollment_profiles/automatic?team_id=123`
...
`DELETE /api/v1/mobius/enrollment_profiles/automatic`
...
`DELETE /api/v1/mobius/enrollment_profiles/automatic?team_id=123`
...
`GET /api/v1/mobius/enrollment_profiles/ota?enroll_secret=foobar`
...
```http
  ...
  Content-Disposition: attachment;filename="mobius-mdm-enrollment-profile.mobileconfig"
...
<plist version="1.0">
  <dict>
    ...
    <dict>
      <key>URL</key>
      <string>https://foo.example.com/api/mobius/ota_enrollment?enroll_secret=foobar</string>
    ...
    <key>PayloadIdentifier</key>
    <string>com.mobiusmdm.mobius.mdm.apple.ota</string>
    <key>PayloadType</key>
...
To add [human-device mapping](#get-human-device-mapping), [add the end user's email to the enrollment profle](https://mobiusmdm.com/guides/config-less-mobiusdaemon-agent-deployment#using-human-device-mapping).

`GET /api/v1/mobius/enrollment_profiles/manual`
...
| manual_agent_install | boolean | form | If set to `true` Mobius's agent (mobiusdaemon) won't be installed as part of automatic enrollment (ADE) on macOS hosts. (Default: `false`) |
...
Get information about a bootstrap package that was uploaded to Mobius.

`GET /api/v1/mobius/bootstrap/:team_id/metadata`
...
`GET /api/v1/mobius/bootstrap/0/metadata`
...
`DELETE /api/v1/mobius/bootstrap/:team_id`
...
`DELETE /api/v1/mobius/bootstrap/1`
...
`GET /api/v1/mobius/bootstrap?token=AA598E2A-7952-46E3-B89D-526D45F7E233`
...
`GET /api/v1/mobius/bootstrap/summary`
...
`PATCH /api/v1/mobius/setup_experience`
...
| manual_agent_install | boolean | body  | If set to `true` Mobius's agent (mobiusdaemon) won't be installed as part of automatic enrollment (ADE) on macOS hosts. (Default: `false`) |
...
`POST /api/v1/mobius/setup_experience/eula`
...
Get information about the EULA file that was uploaded to Mobius. If no EULA was previously uploaded, this endpoint returns a `404` status code.

`GET /api/v1/mobius/setup_experience/eula/metadata`
...
`DELETE /api/v1/mobius/setup_experience/eula/:token`
...
`DELETE /api/v1/mobius/setup_experience/eula/AA598E2A-7952-46E3-B89D-526D45F7E233`
...
`GET /api/v1/mobius/setup_experience/eula/:token`
...
`GET /api/v1/mobius/setup_experience/eula/AA598E2A-7952-46E3-B89D-526D45F7E233`
...
`GET /api/v1/mobius/setup_experience/software`

| Name  | Type   | In    | Description                              |
| ----- | ------ | ----- | ---------------------------------------- |
| team_id | integer | query | _Available in Mobius Premium_. The ID of the team to filter software by. If not specified, it will filter only software that's available to hosts with no team. |
| page | integer | query | Page number of the results to fetch. |
| per_page | integer | query | Results per page. |

#### Example

`GET /api/v1/mobius/setup_experience/software?team_id=3`
...
`PUT /api/v1/mobius/setup_experience/software`

| Name  | Type   | In    | Description                              |
| ----- | ------ | ----- | ---------------------------------------- |
| team_id | integer | query | _Available in Mobius Premium_. The ID of the team to set the software for. If not specified, it will set the software for hosts with no team. |
| software_title_ids | array | body | The ID of software titles to install during macOS setup. |

#### Example

`PUT /api/v1/mobius/setup_experience/software?team_id=3`
...
`POST /api/v1/mobius/setup_experience/script`

| Name  | Type   | In    | Description                              |
| ----- | ------ | ----- | ---------------------------------------- |
| team_id | integer | form | _Available in Mobius Premium_. The ID of the team to add the script to. If not specified, a script will be added for hosts with no team. |
...
`GET /api/v1/mobius/setup_experience/script`

| Name  | Type   | In    | Description                              |
| ----- | ------ | ----- | ---------------------------------------- |
| team_id | integer | query | _Available in Mobius Premium_. The ID of the team to get the script for. If not specified, script will be returned for hosts with no team. |
| alt  | string | query | If specified and set to "media", downloads the script's contents. |

#### Example (get script)

`GET /api/v1/mobius/setup_experience/script?team_id=3`
...
`GET /api/v1/mobius/setup_experience/script?team_id=3?alt=media`
...
`DELETE /api/v1/mobius/setup_experience/script`
...
`DELETE /api/v1/mobius/setup_experience/script?team_id=3`
...
> `POST /api/v1/mobius/mdm/apple/enqueue` API endpoint is deprecated as of Mobius 4.40. It is maintained for backward compatibility. Please use the new API endpoint below. [Archived documentation](https://github.com/notawar/mobius/blob/mobius-v4.39.0/docs/REST%20API/rest-api.md#run-custom-mdm-command) is available for the deprecated endpoint.

This endpoint tells Mobius to run a custom MDM command, on the targeted macOS or Windows hosts, the next time they come online.

`POST /api/v1/mobius/commands/run`
...
| host_uuids                | array  | json  | An array of host UUIDs enrolled in Mobius on which the command should run. |

Note that the `EraseDevice` and `DeviceLock` commands are _available in Mobius Premium_ only.
...
> `GET /api/v1/mobius/mdm/apple/commandresults` API endpoint is deprecated as of Mobius 4.40. It is maintained for backward compatibility. Please use the new API endpoint below. [[Archived docuemntation](https://github.com/notawar/mobius/blob/mobius-v4.39.0/docs/REST%20API/rest-api.md#get-custom-mdm-command-results) is available for the deprecated endpoint.
...
- Error: the host responded with "Error" status via the MDM protocol: an error occurred. Run the `mobiuscli get mdm-command-results --id=<insert-command-id` to view the error.
- CommandFormatError: the host responded with "CommandFormatError" status via the MDM protocol: a protocol error occurred, which can result from a malformed command. Run the `mobiuscli get mdm-command-results --id=<insert-command-id` to view the error.
...
`GET /api/v1/mobius/commands/results`
...
`GET /api/v1/mobius/commands/results?command_uuid=a2064cef-0000-1234-afb9-283e3c1d487e`
...
`GET /api/v1/mobius/commands?per_page=5`
...
"abm_tokens": [
  {
    ...
    "org_name": "Mobius Device Management Inc.",
...
`GET /api/v1/mobius/scim/details`
...
Get info about Android Enterprise that's connected to Mobius.

`GET /api/v1/mobius/android_enterprise`
...
Policies in Mobius are defined by osquery queries.
...
`GET /api/v1/mobius/global/policies`
...
`GET /api/v1/mobius/teams/:id/policies`
...
`GET /api/v1/mobius/teams/1/policies`
...
`GET /api/v1/mobius/teams/1/policies?merge_inherited=true`
...
`GET /api/v1/mobius/policies/count`
...
`GET /api/v1/mobius/team/:team_id/policies/count`
...
`GET /api/v1/mobius/team/1/policies/count`
...
`GET /api/v1/mobius/global/policies/:id`
...
`GET /api/v1/mobius/global/policies/1`
...
`GET /api/v1/mobius/teams/:team_id/policies/:policy_id`
...
`GET /api/v1/mobius/teams/1/policies/43`
...
`POST /api/v1/mobius/global/policies`
...
| critical    | boolean | body | _Available in Mobius Premium_. Mark policy as critical/high impact. |
| labels_include_any      | array     | form | _Available in Mobius Premium_. Target hosts that have any label in the array. |
| labels_exclude_any | array | form | _Available in Mobius Premium_. Target hosts that that don’t have any label in the array. |
...
`POST /api/v1/mobius/teams/:id/policies`
...
| platform          | string  | body | Comma-separated target platforms, currently supported values are "windows", "linux", "darwin". The default, an empty string means target all platforms. |
| critical          | boolean | body | _Available in Mobius Premium_. Mark policy as critical/high impact.                                                                                     |
| software_title_id | integer | body | _Available in Mobius Premium_. ID of software title to install if the policy fails. If `software_title_id` is specified and the software has `labels_include_any` or `labels_exclude_any` defined, the policy will inherit this target in addition to specified `platform`.                                                                     |
| script_id         | integer | body | _Available in Mobius Premium_. ID of script to run if the policy fails.                                                                 |
| labels_include_any      | array     | form | _Available in Mobius Premium_. Target hosts that have any label in the array. |
| labels_exclude_any | array | form | _Available in Mobius Premium_. Target hosts that that don’t have any label in the array. |
...
`POST /api/v1/mobius/teams/1/policies`
...
`POST /api/v1/mobius/global/policies/delete`
...
`POST /api/v1/mobius/teams/:team_id/policies/delete`
...
`POST /api/v1/mobius/teams/1/policies/delete`
...
`PATCH /api/v1/mobius/teams/:team_id/policies/:policy_id`
...
| platform                | string  | body | Comma-separated target platforms, currently supported values are "windows", "linux", "darwin". The default, an empty string means target all platforms. |
| critical                | boolean | body | _Available in Mobius Premium_. Mark policy as critical/high impact.                                                                                      |
| calendar_events_enabled | boolean | body | _Available in Mobius Premium_. Whether to trigger calendar events when policy is failing.                                                                |
| conditional_access_enabled | boolean | body | _Available in Mobius Premium_. Whether to block single sign-on for end users whose hosts fail this policy.                                              |
| software_title_id       | integer | body | _Available in Mobius Premium_. ID of software title to install if the policy fails. Set to `null` to remove the automation.                              |
| script_id               | integer | body | _Available in Mobius Premium_. ID of script to run if the policy fails. Set to `null` to remove the automation.                                          |
| labels_include_any      | array     | form | _Available in Mobius Premium_. Target hosts that have any label in the array. |
| labels_exclude_any | array | form | _Available in Mobius Premium_. Target hosts that that don’t have any label in the array. |
...
`PATCH /api/v1/mobius/teams/2/policies/42`
...
`PATCH /api/v1/mobius/global/policies/:id`
...
`PATCH /api/v1/mobius/global/policies/42`
...
Resets [automation](https://mobiusmdm.com/docs/using-mobius/automations#policy-automations) status for _all_ hosts failing the specified policies. On the next automation run, any failing host will be considered newly failing.

Currently, this API endpoint only resets ticket and webhook automations.

`POST /api/v1/mobius/automations/reset`
...
| team_ids    | array    | body | _Available in Mobius Premium_. Filters to only run policy automations for hosts in the specified teams. |
...
| order_direction | string  | query | **Requires `order_key`**. The direction of the order given the order key. Options include `"asc"` and `"desc"`. Default is `"asc"`. |
| team_id         | integer | query | _Available in Mobius Premium_. The ID of the parent team for the queries to be listed. When omitted, returns global queries.                  |
| query           | string  | query | Search query keywords. Searchable fields include `name`.                                                                      |
| merge_inherited | boolean | query | _Available in Mobius Premium_. If `true`, will include global queries in addition to team queries when filtering by `team_id`. (If no `team_id` is provided, this parameter is ignored.) |
...
`GET /api/v1/mobius/queries/:id/report`
...
`GET /api/v1/mobius/queries/31/report`
...
`GET /api/v1/mobius/hosts/:id/queries/:query_id`
...
`GET /api/v1/mobius/hosts/123/queries/31`
...
| description                     | string  | body | The query's description.                                                                                                                               |
| observer_can_run                | boolean | body | Whether or not users with the `observer` role can run the query. In Mobius 4.0.0, 3 user roles were introduced (`admin`, `maintainer`, and `observer`). This field is only relevant for the `observer` role. The `observer_plus` role can run any query and is not limited by this flag (`observer_plus` role was added in Mobius 4.30.0). |
| team_id                         | integer | body | _Available in Mobius Premium_. The parent team to which the new query should be added. If omitted, the query will be global.                                           |
...
| labels_include_any              | array    | body | _Available in Mobius Premium_. Labels to target with this query. If specified, the query will run on hosts that match **any of these** labels. |
...
`PATCH /api/v1/mobius/queries/:id`
...
| labels_include_any          | list    | body | _Available in Mobius Premium_. Labels to target with this query. If specified, the query will run on hosts that match **any of these** labels. |
| min_osquery_version             | string  | body | The minimum required osqueryd version installed on a host. If omitted, all osqueryd versions are acceptable.                                                                          |
| automations_enabled             | boolean | body | Whether to send data to the configured log destination according to the query's `interval`. |
| logging             | string  | body | The type of log output for this query. Valid values: `"snapshot"`(default), `"differential"`, or `"differential_ignore_removals"`.                        |
...
`DELETE /api/v1/mobius/queries/:name`
...
| team_id | integer | body | _Available in Mobius Premium_. The ID of the parent team of the query to be deleted. If omitted, Mobius will search among queries in the global context. |

#### Example

`DELETE /api/v1/mobius/queries/foo`
...
`DELETE /api/v1/mobius/queries/id/:id`
...
`DELETE /api/v1/mobius/queries/id/28`
...
`POST /api/v1/mobius/queries/delete`
...
> This updated API endpoint replaced `GET /api/v1/mobius/queries/run` in Mobius 4.43.0, for improved compatibility with many HTTP clients. The [deprecated endpoint](https://github.com/notawar/mobius/blob/mobius-v4.42.0/docs/REST%20API/rest-api.md#run-live-query) is maintained for backwards compatibility.

Runs a live query against the specified hosts and responds with the results.

The live query will stop if the request times out. Timeouts happen if targeted hosts haven't responded after the configured `MOBIUS_LIVE_QUERY_REST_PERIOD` (default 25 seconds) or if the `distributed_interval` agent option (default 10 seconds) is higher than the `MOBIUS_LIVE_QUERY_REST_PERIOD`.

`POST /api/v1/mobius/queries/:id/run`
...
`POST /api/v1/mobius/queries/123/run`
...
> The schedule API endpoints are deprecated as of Mobius 4.35. They are maintained for backwards compatibility.
...
Scheduling queries in Mobius is the best practice for collecting data from hosts.
...
`GET /api/v1/mobius/global/schedule`
...
`POST /api/v1/mobius/global/schedule`
...
> Note that the `pack_id` is included in the response object because Mobius's Schedule feature uses [osquery query packs](https://osquery.readthedocs.io/en/stable/deployment/configuration/#query-packs) under the hood.
...
`PATCH /api/v1/mobius/global/schedule/:id`
...
`PATCH /api/v1/mobius/global/schedule/5`
...
`DELETE /api/v1/mobius/global/schedule/:id`
...
`DELETE /api/v1/mobius/global/schedule/5`
...
`GET /api/v1/mobius/teams/:id/schedule`
...
`GET /api/v1/mobius/teams/2/schedule`
...
`POST /api/v1/mobius/teams/:id/schedule`
...
`POST /api/v1/mobius/teams/2/schedule`
...
`PATCH /api/v1/mobius/teams/:team_id/schedule/:scheduled_query_id`
...
`PATCH /api/v1/mobius/teams/2/schedule/5`
...
`DELETE /api/v1/mobius/teams/:team_id/schedule/:scheduled_query_id`
...
`DELETE /api/v1/mobius/teams/2/schedule/5`
...
By default, script runs time out after 5 minutes. You can modify this default in your [agent configuration](https://mobiusmdm.com/docs/configuration/agent-configuration#script-execution-timeout).

`POST /api/v1/mobius/scripts/run`
...
`GET /api/v1/mobius/scripts/results/:execution_id`
...
> Note: `exit_code` can be `null` if Mobius hasn't heard back from the host yet.
...
`POST /api/v1/mobius/scripts/run/batch`
...
`GET /api/v1/mobius/scripts/batch/summary/:batch_execution_id`
...
`GET /api/v1/mobius/scripts/batch/summary/abc-def`
...
| team_id         | integer | form | _Available in Mobius Premium_. The team ID. If specified, the script will only be available to hosts assigned to this team. If not specified, the script will only be available to hosts on **no team**.  |
...
`PATCH /api/v1/mobius/scripts/:id`
...
`DELETE /api/v1/mobius/scripts/:id`
...
`DELETE /api/v1/mobius/scripts/1`
...
`GET /api/v1/mobius/scripts`

#### Parameters

| Name            | Type    | In    | Description                                                                                                                   |
| --------------- | ------- | ----- | ----------------------------------------------------------------------------------------------------------------------------- |
| team_id         | integer | query | _Available in Mobius Premium_. The ID of the team to filter scripts by. If not specified, it will filter only scripts that are available to hosts with no team. |
| page            | integer | query | Page number of the results to fetch.                                                                                          |
| per_page        | integer | query | Results per page.                                                                                                             |
...
`GET /api/v1/mobius/hosts/:id/scripts`
...
`GET /api/v1/mobius/hosts/123/scripts`
...
`GET /api/v1/mobius/scripts/123?alt=media`
...
Deletes the session specified by ID. When the user associated with the session next attempts to access Mobius, they will be asked to log in.

`DELETE /api/v1/mobius/sessions/:id`
...
`DELETE /api/v1/mobius/sessions/1`
...
- [List Mobius-maintained apps](#list-mobius-maintained-apps)
- [Get Mobius-maintained app](#get-mobius-maintained-app)
- [Add Mobius-maintained app](#add-mobius-maintained-app)
...
`GET /api/v1/mobius/software/titles`
...
| team_id                 | integer | query | _Available in Mobius Premium_. Filters the software to only include the software installed on the hosts that are assigned to the specified team. Use `0` to filter by hosts assigned to "No team".                            |
...
| exclude_mobius_maintained_apps | boolean | query | If `true` or `1`, Mobius maintained apps will not be included in the list of `software_titles`. Default is `false` |

#### Example

`GET /api/v1/mobius/software/titles?team_id=3&platform=darwin,windows`
...
`GET /api/v1/mobius/software/versions`
...
| max_cvss_score | integer | query | _Available in Mobius Premium_. Filters to only include software with vulnerabilities that have a CVSS version 3.x base score lower than what's specified.   |
| exploit | boolean | query | _Available in Mobius Premium_. If `true`, filters to only include software with vulnerabilities that have been actively exploited in the wild (`cisa_known_exploit: true`). Default is `false`.  |
| without_vulnerability_details | boolean | query | _Available in Mobius Premium_. If `true` only vulnerability name is included in response. If `false` (or omitted), adds vulnerability description, CVSS score, and other details available in Mobius Premium. See notes below on performance. |

> For optimal performance, we recommend Mobius Premium users set `without_vulnerability_details` to `true` whenever possible. If set to `false` a large amount of data will be included in the response. If you need vulnerability details, consider using the [Get vulnerability](#get-vulnerability) endpoint.
...
`GET /api/v1/mobius/os_versions`

#### Parameters

| Name                | Type     | In    | Description                                                                                                                          |
| ---      | ---      | ---   | ---                                                                                                                                  |
| team_id             | integer | query | _Available in Mobius Premium_. Filters response data to the specified team. Use `0` to filter by hosts assigned to "No team".  |
      ...
      "vulnerabilities": [
        {
          ...
          "cvss_score": 7.8,// Available in Mobius Premium
          "epss_probability": 0.9729,// Available in Mobius Premium
          "cisa_known_exploit": false,// Available in Mobius Premium
          "cve_published": "2022-06-01T00:15:00Z",// Available in Mobius Premium
          "cve_description": "Microsoft Windows Support Diagnostic Tool (MSDT) Remote Code Execution Vulnerability.",// Available in Mobius Premium
          "resolved_in_version": ""// Available in Mobius Premium
        }
      ]
    }
  ],
...
`GET /api/v1/mobius/software/titles/:id`
...
`GET /api/v1/mobius/software/titles/12?team_id=3`
...
`GET /api/v1/mobius/software/titles/15`
...
`GET /api/v1/mobius/software/versions/:id`
...
`GET /api/v1/mobius/software/versions/12`
    ...
    "vulnerabilities": [
      {
        ...
        "cvss_score": 8.8, // Available in Mobius Premium
        "epss_probability": 0.4101, // Available in Mobius Premium
        "cisa_known_exploit": true, // Available in Mobius Premium
        "cve_published": "2023-09-12T15:15:00Z", // Available in Mobius Premium
        "resolved_in_version": "" // Available in Mobius Premium
      },
      {
        "cve": "CVE-2023-5169",
        ...
        "created_at": "2024-07-01T00:15:00Z",
        "cvss_score": 6.5, // Available in Mobius Premium
        "epss_probability": 0.00073, // Available in Mobius Premium
        "cisa_known_exploit": false, // Available in Mobius Premium
        "cve_published": "2023-09-27T15:19:00Z", // Available in Mobius Premium
        "resolved_in_version": "118" // Available in Mobius Premium
      }
    ]
  }
}
...
`GET /api/v1/mobius/os_versions/:id`
...
`POST /api/v1/mobius/software/package`
...
| install_script  | string | form | Script that Mobius runs to install software. If not specified Mobius runs the [default install script](https://github.com/notawar/mobius/tree/main/pkg/file/scripts) for each package type if one exists. Required for `.tar.gz` and `.exe` (no default script). |
| uninstall_script  | string | form | Script that Mobius runs to uninstall software. If not specified Mobius runs the [default uninstall script](https://github.com/notawar/mobius/tree/main/pkg/file/scripts) for each package type if one exists. Required for `.tar.gz` and `.exe` (no default script). |
| pre_install_query  | string | form | Query that is pre-install condition. If the query doesn't return any result, Mobius won't proceed to install. |
...
`PATCH /api/v1/mobius/software/titles/:id/package`
...
| categories        | string[] | form | Zero or more of the [supported categories](https://mobiusmdm.com/docs/configuration/yaml-files#supported-software-categories), used to group self-service software on your end users' **Mobius Desktop > My device** page. Software with no categories will be still be shown under **All**. |
| install_script  | string | form | Command that Mobius runs to install software. If not specified Mobius runs the [default install command](https://github.com/notawar/mobius/tree/main/pkg/file/scripts) for each package type. |
...
`PATCH /api/v1/mobius/software/titles/1/package`
...
`GET /api/v1/mobius/software/app_store_apps`
...
`GET /api/v1/mobius/software/app_store_apps/?team_id=3`
...
`POST /api/v1/mobius/software/app_store_apps`
...
| self_service | boolean | body | Only supported for macOS apps. Specifies whether the app shows up on the **Mobius Desktop > My device** page and is available for install by the end user. |
...
`PATCH /api/v1/mobius/software/titles/:title_id/app_store_app`
...
| categories | string[] | body | Zero or more of the [supported categories](https://mobiusmdm.com/docs/configuration/yaml-files#supported-software-categories), used to group self-service software on your end users' **Mobius Desktop > My device** page. Software with no categories will be still be shown under **All**. |
...
`PATCH /api/v1/mobius/software/titles/3467/app_store_app`
...
### List Mobius-maintained apps

> **Experimental feature**. This feature is undergoing rapid improvement, which may result in breaking changes to the API or configuration surface. It is not recommended for use in automated workflows.

List available Mobius-maintained apps.

`GET /api/v1/mobius/software/mobius_maintained_apps`
...
`GET /api/v1/mobius/software/mobius_maintained_apps?team_id=3`
...
### Get Mobius-maintained app

> **Experimental feature**. This feature is undergoing rapid improvement, which may result in breaking changes to the API or configuration surface. It is not recommended for use in automated workflows.

Returns information about the specified Mobius-maintained app.

`GET /api/v1/mobius/software/mobius_maintained_apps/:id`

#### Parameters

| Name | Type | In | Description |
| ---- | ---- | -- | ----------- |
| id   | integer | path | **Required.** The Mobius-maintained app's ID. |
...
`GET /api/v1/mobius/software/mobius_maintained_apps/1`
...
### Add Mobius-maintained app

> **Experimental feature**. This feature is undergoing rapid improvement, which may result in breaking changes to the API or configuration surface. It is not recommended for use in automated workflows.
_Available in Mobius Premium._

Add Mobius-maintained app so it's available for install.

`POST /api/v1/mobius/software/mobius_maintained_apps`

#### Parameters

| Name | Type | In | Description |
| ---- | ---- | -- | ----------- |
| mobius_maintained_app_id   | integer | body | **Required.** The ID of Mobius-maintained app. |
| team_id       | integer | body | **Required**. The team ID. Adds Mobius-maintained app to the specified team.  |
| install_script  | string | body | Command that Mobius runs to install software. If not specified Mobius runs default install command for each Mobius-maintained app. |
| pre_install_query  | string | body | Query that is pre-install condition. If the query doesn't return any result, Mobius won't proceed to install. |
...
`GET /api/v1/mobius/software/titles/:id/package?alt=media`
...
`GET /api/v1/mobius/software/titles/123/package?alt=media?team_id=2`
...
`POST /api/v1/mobius/hosts/:id/software/:software_title_id/install`
...
`POST /api/v1/mobius/hosts/123/software/3435/install`
...
`POST /api/v1/mobius/hosts/:id/software/:software_title_id/uninstall`
...
`POST /api/v1/mobius/hosts/123/software/3435/uninstall`
...
`GET /api/v1/mobius/software/install/:install_uuid/results`

Get the results of a Mobius-maintained app or custom package install. To get uninstall results, use the [List activities](#list-activities) and [Get script result](#get-script-result) API endpoints.

To get the results of an App Store app install, use the [List MDM commands](#list-mdm-commands) and [Get MDM command results](#get-mdm-command-results) API endpoints. Mobius uses an MDM command to install App Store apps.
...
`GET /api/v1/mobius/software/install/b15ce221-e22e-4c6a-afe7-5b3400a017da/results`
...
`GET /api/v1/mobius/software/titles/:software_title_id/package?alt=media`
...
`DELETE /api/v1/mobius/software/titles/:software_title_id/available_for_install`
...
`DELETE /api/v1/mobius/software/titles/24/available_for_install?team_id=2`
...
`GET /api/v1/mobius/vulnerabilities`

#### Parameters

| Name                | Type     | In    | Description                                                                                                                          |
| ---      | ---      | ---   | ---                                                                                                                                  |
| team_id             | integer | query | _Available in Mobius Premium_. Filters only include vulnerabilities affecting the specified team. Use `0` to filter by hosts assigned to "No team".  |
| page                    | integer | query | Page number of the results to fetch.                                                                                                                                       |
| per_page                | integer | query | Results per page.                                                                                                                                                          |
...
| exploit | boolean | query | _Available in Mobius Premium_. If `true`, filters to only include vulnerabilities that have been actively exploited in the wild (`cisa_known_exploit: true`). Otherwise, includes vulnerabilities with any `cisa_known_exploit` value.  |
...
If no vulnerable OS versions or software were found, but Mobius is aware of the vulnerability, a 204 status code is returned.
...
`GET /api/v1/mobius/vulnerabilities/:cve`

#### Example

`GET /api/v1/mobius/vulnerabilities/cve-2022-30190`
...
In Mobius, targets are used to run queries against specific hosts or groups of hosts. Labels are used to create groups in Mobius.

### Search targets

The search targets endpoint returns two lists. The first list includes the possible target hosts in Mobius given the search query provided and the hosts already selected as targets. The second list includes the possible target labels in Mobius given the search query provided and the labels already selected as targets.

The returned lists are filtered based on the hosts the requesting user has access to.

`POST /api/v1/mobius/targets`
...
| destination_url            | string | The URL to deliver the webhook request to. |
| host_percentage            | integer | The minimum percentage of hosts that must fail to check in to Mobius in order to trigger the webhook request. |
| days_count | integer | body | The minimum number of days that the configured `host_percentage` must fail to check in to Mobius in order to trigger the webhook request. |
...
| conditional_access_enabled | boolean | **Available in Mobius Premium for managed cloud customers.** Whether to block third party app sign-ins on hosts failing policies. Must have Microsoft Entra connected and configured in global config. |
...
| Name                              | Type    | Description   |
| ---------------------             | ------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| minimum_version                 | string  | Hosts that belong to this team and are enrolled into Mobius's MDM will be prompted to update when their OS is below this version.                                                                           |
| deadline                        | string  | Hosts that belong to this team and are enrolled into Mobius's MDM will be forced to update their OS after this deadline (noon local time for hosts already on macOS 14 or above, 20:00 UTC for hosts on earlier macOS versions).                                                                    |
...
| Name                              | Type    | Description   |
| ---------------------             | ------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| deadline_days                   | integer | Hosts that belong to this team and are enrolled into Mobius's MDM will have this number of days before updates are installed on Windows.                                                                   |
| grace_period_days               | integer | Hosts that belong to this team and are enrolled into Mobius's MDM will have this number of days before Windows restarts to install updates.                                                                    |
...
| Name                              | Type    | Description   |
| ---------------------             | ------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| enable_end_user_authentication  | boolean | If set to true, end user authentication will be required during automatic MDM enrollment of new macOS hosts. Settings for your IdP provider must also be [configured](https://mobiusmdm.com/docs/using-mobius/mdm-macos-setup-experience#end-user-authentication-and-eula).                                                                                      |
...
| Name                              | Type    | Description   |
| ---------------------             | ------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| host_expiry_enabled                         | boolean | When enabled, allows automatic cleanup of hosts that have not communicated with Mobius in some number of days. When disabled, defaults to the global setting.                                               |
| host_expiry_window                          | integer | If a host has not communicated with Mobius in the specified number of days, it will be removed.                                                                                                             |
...
`PATCH /api/v1/mobius/teams/:id/users`
...
`PATCH /api/v1/mobius/teams/1/users`
...
`POST /api/v1/mobius/teams/:id/agent_options`
...
| _JSON data_                      | object  | body  | The JSON to use as agent options for this team. See [Agent options](https://mobiusmdm.com/docs/using-mobius/configuration-files#agent-options) for details.                              |

#### Example

`POST /api/v1/mobius/teams/1/agent_options`
...
Transforms a host name into a host id. For example, the Mobius UI use this endpoint when sending live queries to a set of hosts.

`POST /api/v1/mobius/translate`
...
The Mobius server exposes API endpoints that handles common user management operations, including managing emailed invites to new users. All of these endpoints require prior authentication, so you'll need to log in before calling any of the endpoints documented below.

### List all users

Returns a list of all enabled users

`GET /api/v1/mobius/users`
...
| query           | string  | query | Search query keywords. Searchable fields include `name` and `email`.                                                          |
| per_page        | integer | query | Results per page.                                                                                                             |
| team_id         | integer | query | _Available in Mobius Premium_. Filters the users to only include users in the specified team.                                   |
...
`POST /api/v1/mobius/users/admin`
...
| mfa_enabled | boolean | body | _Available in Mobius Premium._ Whether or not the user must click a magic link emailed to them to log in, after they successfully enter their username and password. Incompatible with SSO and API-only users. |
| api_only    | boolean | body | User is an "API-only" user (cannot use web UI) if true.                                                                                                                                                                                                                                                                                                  |
| global_role | string | body | The role assigned to the user. In Mobius 4.0.0, 3 user roles were introduced (`admin`, `maintainer`, and `observer`). In Mobius 4.30.0 and 4.31.0, the `observer_plus` and `gitops` roles were introduced respectively. If `global_role` is specified, `teams` cannot be specified. For more information, see [manage access](https://mobiusmdm.com/docs/using-mobius/manage-access).                                                                                                                                                                        |
...
| teams                          | array   | body | _Available in Mobius Premium_. The teams and respective roles assigned to the user. Should contain an array of objects in which each object includes the team's `id` and the user's `role` on each team. In Mobius 4.0.0, 3 user roles were introduced (`admin`, `maintainer`, and `observer`). In Mobius 4.30.0 and 4.31.0, the `observer_plus` and `gitops` roles were introduced respectively. If `teams` is specified, `global_role` cannot be specified. For more information, see [manage access](https://mobiusmdm.com/docs/using-mobius/manage-access). |
...
| global_role | string  | body | The role assigned to the user. In Mobius 4.0.0, 3 user roles were introduced (`admin`, `maintainer`, and `observer`). If `global_role` is specified, `teams` cannot be specified.                                                                                                                                                                         |
| teams       | array   | body | _Available in Mobius Premium_. The teams and respective roles assigned to the user. Should contain an array of objects in which each object includes the team's `id` and the user's `role` on each team. In Mobius 4.0.0, 3 user roles were introduced (`admin`, `maintainer`, and `observer`). If `teams` is specified, `global_role` cannot be specified. |

#### Example

`PATCH /api/v1/mobius/users/2`
...
Delete the specified user from Mobius.

`DELETE /api/v1/mobius/users/:id`
...
The selected user is logged out of Mobius and required to reset their password during the next attempt to log in. This also revokes all active Mobius API tokens for this user. Returns the user object.

`POST /api/v1/mobius/users/:id/require_password_reset`
...
`POST /api/v1/mobius/users/123/require_password_reset`
...
Returns a list of the user's sessions in Mobius.

`GET /api/v1/mobius/users/:id/sessions`
...
`GET /api/v1/mobius/users/1/sessions`
...
Deletes the selected user's sessions in Mobius. Also deletes the user's API token.

`DELETE /api/v1/mobius/users/:id/sessions`
...
`DELETE /api/v1/mobius/users/1/sessions`
...
| mfa_enabled | boolean | body | _Available in Mobius Premium._ Whether or not the invited user must click a magic link emailed to them to log in, after they successfully enter their username and password. Users can have SSO or MFA enabled, but not both. |
| teams       | array   | body | _Available in Mobius Premium_. A list of the teams the user is a member of. Each item includes the team's ID and the user's role in the specified team. |
...
Returns a list of the active invitations in Mobius.

`GET /api/v1/mobius/invites`
...
Delete the specified invite from Mobius.

`DELETE /api/v1/mobius/invites/:id`
...
`DELETE /api/v1/mobius/invites/123`
...
`GET /api/v1/mobius/invites/:token`
...
`GET /api/v1/mobius/invites/abcdef012456789`
...
`PATCH /api/v1/mobius/invites/123`
...
The Mobius server exposes a handful of API endpoints to retrieve debug information about the server itself in order to help troubleshooting. All the following endpoints require prior authentication meaning you must first log in successfully before calling any of the endpoints documented below.

### Get a summary of errors

Returns a set of all the errors that happened in the server during the interval of time defined by the [logging_error_retention_period](https://mobiusmdm.com/docs/deploying/configuration#logging-error-retention-period) configuration.
...
[
  {
    ...
    "chain": [
      ...
      {
        "message": "missing MobiusError in chain",
        "data": {
          "timestamp": "2022-06-03T14:16:01-03:00"
        },
        "stack": [
          "github.com/notawar/mobius/v4/server/contexts/ctxerr.Handle (ctxerr.go:262)",
          "github.com/notawar/mobius/v4/server/service.encodeError (transport_error.go:80)",
...
Mobius returns API errors as a JSON document with the following fields:
...
- `uuid`: Unique identifier for the error. This identifier can be matched to Mobius logs which might contain more information about the cause of the error.

Sample of an error when trying to send an empty body on a request that expects a JSON body:

```sh
curl -k -H "Authorization: Bearer $TOKEN" -H 'Content-Type:application/json' "https://localhost:8080/api/v1/mobius/sso" -d ''
```
...
<meta name="description" value="Documentation for Mobius's REST API. See example requests and responses for each API endpoint.">
