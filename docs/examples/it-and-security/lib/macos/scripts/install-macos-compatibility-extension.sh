...
# Safe for deployment via Mobius (schedules orbit restart to avoid script termination)
#
# Usage:
#   sudo ./install_macos_compatibility_extension.sh          # Default: scheduled restart (Mobius-safe)
...
EXTENSION_DIR="/var/mobius/extensions"
...
handle_orbit_restart() {
    if [[ "$IMMEDIATE_RESTART" == "immediate" ]]; then
        ...
        log "Scheduling orbit service restart (safe for Mobius deployment)..."
...
restart_orbit_immediate() {
    ...
    launchctl kickstart -k system/com.mobiusmdm.orbit
...
handle_detached_restart() {
    ...
    launchctl kickstart -k system/com.mobiusmdm.orbit >> /var/log/macos_compatibility_installer.log 2>&1
...
main() {
    ...
    if [[ "$IMMEDIATE_RESTART" == "immediate" ]]; then
        ...
        log "Mode: Scheduled restart (safe for Mobius deployment)"
    fi
    ...
    # Handle orbit restart (scheduled for Mobius deployment, immediate for manual)
    handle_orbit_restart
