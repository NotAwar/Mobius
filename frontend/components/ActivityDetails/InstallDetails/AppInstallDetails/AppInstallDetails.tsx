...
export const AppInstallDetails = ({
  ...
  if (isStatusNotNow) {
    ...
    subordinate =
      " but couldn’t because the host was locked or was running on battery power while in Power Nap. Mobius will try again";
  } else {
    ...
    <>
      <div className={`${baseClass}__software-install-details`}>
        <div className={`${baseClass}__status-message`}>
          ...
          <span>
            Mobius {predicate} <b>{software_title}</b> on {formattedHost}
            {subordinate}.
          </span>
        </div>
        {showCommandPayload && (
