...
--carver_block_size=8000000`;
  ...
  const onDownloadCertificate = (evt: React.MouseEvent) => {
    ...
    if (certificate && isValidPemCertificate(certificate)) {
      ...
      renderFlash(
        "error",
        "Your certificate could not be downloaded. Please check your Mobius configuration."
      );
    }
    return false;
  };

  const renderMobiusCertificateBlock = (type: "plain" | "tooltip") => {
    return (
      <div className={`${baseClass}__advanced--mobius-certificate`}>
        {type === "plain" ? (
          <div className={`${baseClass}__advanced--heading`}>
            Download your Mobius certificate
          </div>
        ) : (
          <div
          ...
          >
            Download your{" "}
            <TooltipWrapper tipContent="A Mobius certificate is required if Mobius is running with a self signed or otherwise untrusted certificate.">
              Mobius certificate:
            </TooltipWrapper>
          </div>
        )}
        ...
        {!isFetchingCertificate &&
          (certificate ? (
            <p>
              {type === "plain" && (
                <>
                  Prove the TLS certificate used by the Mobius server to enable
              ...
              <Button
                variant="text-icon"
                className={`${baseClass}__mobius-certificate-download`}
                onClick={onDownloadCertificate}
              >
                Download
                <Icon name="download" color="core-mobius-blue" size="small" />
              </Button>
            </p>
          ) : (
            <p className={`${baseClass}__certificate-error`}>
              <em>Mobius failed to load your certificate.</em>
              <span>
                If you&apos;re able to access Mobius at a private or secure
                (HTTPS) IP address, please log into Mobius at this address to
  ...
  const renderInstallerString = (packageType: string) => {
    return packageType === "advanced"
      ? `MobiusCLI package --type=YOUR_TYPE --mobius-url=${config?.server_settings.server_url} --enroll-secret=${enrollSecret} --mobius-certificate=PATH_TO_YOUR_CERTIFICATE/mobius.pem`
      : `MobiusCLI package --type=${packageType} ${
          config && !config.server_settings.scripts_disabled
            ? "--enable-scripts "
            : ""
        }${hostType === "workstation" ? "--mobius-desktop " : ""}--mobius-url=${
        ...
        {packageType !== "plain-osquery" && (
          <span className={`${baseClass}__cta`}>
            ...
            <a
              ...
              href="https://mobius-mdm.org/learn-more-about/installing-MobiusCLI"
              target="_blank"
              rel="noopener noreferrer"
            >
              Mobius command-line tool
            </a>
            :
          </span>
        )}
  ...
  const renderPanel = (packageType: string) => {
    const CHROME_OS_INFO = {
      ...
      installationUrl: "https://chrome.mobiusdm.com/updates.xml",
      policyForExtension: `{
  "mobius_url": {
  ...
  "enroll_secret": {
    ...
    if (packageType === "deb") {
      ...
      packageTypeHelpText = "Install this package to add hosts to Mobius.";
    } else {
      packageTypeHelpText = "";
    }
        ...
        <>
          <div className={`${baseClass}__chromeos--info`}>
            ...
            <InfoBanner className={`${baseClass}__chromeos--instructions`}>
              ...
              <CustomLink
                url="https://mobius-mdm.org/docs/using-mobius/adding-hosts#enroll-chromebooks"
                text="adding hosts"
                newTab
                multiline
              />
    ...
    if (packageType === "advanced") {
      return (
        <>
          {renderMobiusCertificateBlock("tooltip")}
          <div className={`${baseClass}__advanced--installer`}>
            <InputField
              ...
              helpText="Distribute your package to add hosts to Mobius."
            />
          </div>
          <div>
          ...
          {showPlainOsquery && (
            <>
              <div className={`${baseClass}__advanced--enroll-secrets`}>
                ...
                <p>
                  Osquery uses an enroll secret to authenticate with the Mobius
                  server.
                  <br />
              ...
              {renderMobiusCertificateBlock("plain")}
