...
import { IMobiusMaintainedApp } from "interfaces/software";

import { softwareAlreadyAddedTipContent } from "pages/SoftwarePage/SoftwareAddPage/SoftwareMobiusMaintained/MobiusMaintainedAppDetailsPage/MobiusAppDetailsForm/MobiusAppDetailsForm";
...
interface IInstallerActionCellProps {
  value?: Omit<IMobiusMaintainedApp, "name" | "version"> | null;
...
const InstallerActionCell = ({
  ...
  const onClick = () => {
    const path = getPathWithQueryParams(
      PATHS.SOFTWARE_MOBIUS_MAINTAINED_DETAILS(id),
      { team_id: teamId }
    );
    if (router && path) {
      router?.push(path);
