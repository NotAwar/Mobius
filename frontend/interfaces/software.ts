...
export interface ISoftwarePackage {
  ...
  mobius_maintained_app_id?: number | null;
...
export const SOURCE_TYPE_CONVERSION = {
  ...
  chrome_extensions: "Browser plugin", // chrome_extensions can include any chrome-based browser (e.g., edge), so we rely instead on the `browser` field computed by Mobius server and fallback to this value if it is not present.
...
> = {
  ...
  pending_install: "told Mobius to install",
  failed_install: "failed to install",
  pending_uninstall: "told Mobius to uninstall",
...
export interface IMobiusMaintainedApp {
  id: number;
  name: string;
  version: string;
  platform: MobiusMaintainedAppPlatform;
  software_title_id?: number; // null unless the team already has the software added (as a Mobius-maintained app, App Store (app), or custom package)
}

export type MobiusMaintainedAppPlatform = Extract<
  Platform,
  "darwin" | "windows"
>;
...
export interface ICombinedFMA {
  name: string;
  macos: Omit<IMobiusMaintainedApp, "name"> | null;
  windows: Omit<IMobiusMaintainedApp, "name"> | null;
}
export interface IMobiusMaintainedAppDetails {
  id: number;
  name: string;
  version: string;
  platform: MobiusMaintainedAppPlatform;
