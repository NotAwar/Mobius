...
const TAGGED_TEMPLATES = {
  ...
  editPackCtlActivityTemplate: () => {
    return "edited a pack using MobiusCLI.";
  },
  editPolicyCtlActivityTemplate: () => {
    return "edited policies using MobiusCLI.";
  },
  editQueryCtlActivityTemplate: (activity: IActivity) => {
    ...
    return typeof count === "undefined" || count === 1
      ? "edited a query using MobiusCLI."
      : "edited queries using MobiusCLI.";
  },
  editSoftwareCtlActivityTemplate: () => {
    return "edited software using MobiusCLI.";
  },
  ...
  userAddedBySSOTempalte: () => {
    return "was added to Mobius by SSO.";
  },
  ...
  mobiusEnrolled: (activity: IActivity) => {
    const hostDisplayName = activity.details?.host_display_name ? (
      <b>{activity.details.host_display_name}</b>
    ) : (
      "A host"
    );
    return <>{hostDisplayName} enrolled in Mobius.</>;
  },
    ...
    return (
      <>
        {activity.actor_full_name
          ? " told Mobius to turn off mobile device management (MDM) for"
  ...
  enabledWindowsMdm: () => {
    return (
      <>
        {" "}
        told Mobius to turn on MDM features for all Windows hosts (servers
        excluded).
      </>
    );
  },
  disabledWindowsMdm: () => {
    return <> told Mobius to turn off Windows MDM features.</>;
  },
  ...
  enabledWindowsMdmMigration: () => {
    return (
      <>
        {" "}
        told Mobius to automatically migrate Windows hosts connected to another
        MDM solution.
      </>
    );
  },
  disabledWindowsMdmMigration: () => {
    return (
      <>
        {" "}
        told Mobius to stop migrating Windows hosts connected to another MDM
        solution.
      </>
    );
  },
...
const getDetail = (activity: IActivity, isPremiumTier: boolean) => {
  switch (activity.type) {
    ...
    case ActivityType.MobiusEnrolled: {
      return TAGGED_TEMPLATES.mobiusEnrolled(activity);
    }
...
const GlobalActivityItem = ({
  ...
  const renderActivityPrefix = () => {
    const DEFAULT_ACTOR_DISPLAY = (
      <b>{activity.mobius_initiated ? "Mobius" : activity.actor_full_name} </b>
    );
