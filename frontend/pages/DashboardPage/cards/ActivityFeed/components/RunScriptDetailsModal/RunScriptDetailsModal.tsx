  ...
  switch (exitCode) {
    case null:
      return !hostTimeout ? (
        ...
        // Expected API message: "Mobius hasn’t heard from the host in over 1 minute. Mobius doesn’t know if the script ran because the host went offline."
        <StatusMessageError message={message} />
      );
    case -2:
      // Expected API message: "Scripts are disabled for this host. To run scripts, deploy the mobiusdaemon agent with scripts enabled."
      return <StatusMessageError message={message} />;
    case -1: {
      // message should look like: "Timeout. Mobius stopped the script after 600 seconds to protect host performance.";
      ...
      const modMessage = (
        <>
          Timeout. Mobius stopped the script {varText}to protect host
          performance.
        </>
      );
    ...
    <Textarea
      label={
        <>
          The{" "}
          <TooltipWrapper
            tipContent="Mobius records the last 10,000 characters to prevent downtime."
