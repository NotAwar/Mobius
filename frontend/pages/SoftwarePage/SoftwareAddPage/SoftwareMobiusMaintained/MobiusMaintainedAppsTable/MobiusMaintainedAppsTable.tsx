...
import { ISoftwareMobiusMaintainedAppsResponse } from "services/entities/software";
import { getNextLocationPath } from "utilities/helpers";
import {
  MobiusMaintainedAppPlatform,
  ICombinedFMA,
...
import { generateTableConfig } from "./MobiusMaintainedAppsTableConfig";

const baseClass = "mobius-maintained-apps-table";

const EmptyMobiusAppsTable = () => (
  <EmptyTable
    ...
    info={
      <>
        ...
        <CustomLink
          newTab
          url="https://mobius-mdm.org/feature-request"
...
/** Used to convert MobiusMaintainedApp API response which has separate entries
...
const combineAppsByPlatform = (
  fmaList: IMobiusMaintainedApp[]
): ICombinedFMA[] => {
  const combinedApps: { [name: string]: ICombinedFMA } = {};

  fmaList.forEach((app: IMobiusMaintainedApp) => {
    ...
    if (platform === "darwin") {
      combinedApps[name].macos = {
        platform: platform as MobiusMaintainedAppPlatform,
        ...rest,
      };
...
interface IMobiusMaintainedAppsTableProps {
  teamId: number;
  isLoading: boolean;
  query: string;
  perPage: number;
  ...
  data?: ISoftwareMobiusMaintainedAppsResponse;
}

interface IRowProps {
  original: IMobiusMaintainedApp;
}

const MobiusMaintainedAppsTable = ({
  teamId,
  isLoading,
  data,
  router,
  ...
  orderKey,
  currentPage,
}: IMobiusMaintainedAppsTableProps) => {
  ...
  const onQueryChange = useCallback(
    (newTableQuery: ITableQueryData) => {
      ...
      const newRoute = getNextLocationPath({
        pathPrefix: PATHS.SOFTWARE_ADD_MOBIUS_MAINTAINED,
        routeTemplate: "",
  ...
  const combinedAppsByPlatform =
    (data && combineAppsByPlatform(data.mobius_maintained_apps ?? [])) ?? [];
  ...
  return (
    <TableContainer<IRowProps>
      ...
      isLoading={isLoading}
      resultsTitle="items"
      emptyComponent={EmptyMobiusAppsTable}
...
export default MobiusMaintainedAppsTable;
