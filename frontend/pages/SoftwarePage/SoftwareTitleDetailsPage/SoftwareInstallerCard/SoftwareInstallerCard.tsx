...
> = {
  ...
  pending: {
    ...
    tooltip: (
      <>
        Mobius is installing/uninstalling or will
        <br />
      ...
      {options.map((option) => {
        const ButtonContent = (
          <Button
            ...
            variant="icon"
          >
            <Icon name={option.iconName} color="core-mobius-blue" />
          </Button>
        );
...
const SoftwareInstallerCard = ({
  ...
  const installerType = isSoftwarePackage(softwareInstaller)
    ? "package"
    : "vpp";
  const isMobiusMaintainedApp =
    "mobius_maintained_app_id" in softwareInstaller &&
    !!softwareInstaller.mobius_maintained_app_id;
  const isCustomPackage = installerType === "package" && !isMobiusMaintainedApp;
  const sha256 =
  ...
  if (installerType === "package" && !version) {
    versionInfo = (
      <TooltipWrapper
        tipContent={
          <span>
            Mobius couldn&apos;t read the version from {name}.{" "}
            <CustomLink
              newTab
