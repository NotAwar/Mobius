...
import { IMobiusMaintainedAppFormData } from "pages/SoftwarePage/SoftwareAddPage/SoftwareMobiusMaintained/MobiusMaintainedAppDetailsPage/MobiusAppDetailsForm/MobiusAppDetailsForm";
...
const SoftwareOptionsSelector = ({
  ...
  const getAutomaticInstallTooltip = (): JSX.Element => {
    if (isExePackage || isTarballPackage) {
      return (
        <>
          Mobius can&apos;t create a policy to detect existing installations for{" "}
    ...
    <div className={`form-field ${classNames}`}>
      ...
      {formData.automaticInstall && isCustomPackage && (
        <InfoBanner color="yellow">
          ...
          Mobius&apos;s policy may not detect these existing installations.
          Please create a test team in Mobius to verify a smooth installation.{" "}
          <CustomLink
