...
import { normalizeEmptyValues, wrapMobiusHelper } from "utilities/helpers";
...
const HostDetailsPage = ({
    ...
    () => hostAPI.loadHostDetails(hostIdFromURL),
    {
      ...
      onSuccess: (returnedHost) => {
        ...
        if (
          ...
          // If the API reports that a Mobius refetch request is pending, we want to check back for fresh
          // host details. Here we set a one second timeout and poll the API again using
          // fullyReloadHost. We will repeat this process with each onSuccess cycle for a total of
          // 60 seconds or until the API reports that the Mobius refetch request has been resolved
  ...
  useEffect(() => {
    if (host?.display_name) {
      // e.g., Rachel's Macbook Pro | Hosts | Mobius
  ...
  const renderActionDropdown = () => {
    ...
    return (
      <HostActionsDropdown
        ...
        isConnectedToMobiusMdm={host.mdm?.connected_to_mobius}
          ...
          <>
            ...
            <TabPanel>
              <SoftwareLibraryCard
                ...
                hostTeamId={host.team_id || 0}
                hostMDMEnrolled={host.mdm.connected_to_mobius}
    ...
    <MainContent className={baseClass}>
      <>
        <HostDetailsBanners
          ...
          connectedToMobiusMdm={host?.mdm.connected_to_mobius}
            ...
            <TabPanel className={`${baseClass}__details-panel`}>
              ...
              {showAgentOptionsCard && (
                <AgentOptionsCard
                  ...
                  wrapMobiusHelper={wrapMobiusHelper}
