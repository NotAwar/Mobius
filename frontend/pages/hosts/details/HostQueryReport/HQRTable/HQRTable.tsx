...
import { HumanTimeDiffWithMobiusLaunchCutoff } from "components/HumanTimeDiffWithDateTip";
    ...
    if (!lastFetched) {
      ...
      return (
        <EmptyTable
          ...
          info={`Mobius is collecting query results from ${hostName}. Check back later.`}
        />
      );
    }
    if (reportClipped) {
      return (
        <EmptyTable
          ...
          info="This query has paused reporting in Mobius, and no results were saved for this host."
        />
      );
    }
    return (
    ...
    return (
      <>
        ...
        <span className="last-fetched">
          Last fetched{" "}
          <HumanTimeDiffWithMobiusLaunchCutoff timeString={lastFetched ?? ""} />
        </span>
      </>
    );
