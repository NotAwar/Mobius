import {
  MOBIUS_FILEVAULT_PROFILE_DISPLAY_NAME,
...
export const isDiskEncryptionProfile = (profileName: string) => {
  return profileName === MOBIUS_FILEVAULT_PROFILE_DISPLAY_NAME;
};
...
const APPLE_PROFILE_VERIFIED_DISPLAY_CONFIG: ProfileDisplayOption = {
  ...
  tooltip: (innerProps) =>
    innerProps.isDiskEncryptionProfile
      ? "The host turned disk encryption on and sent the key to Mobius. " +
        "Mobius verified."
      : "The host applied the setting. Mobius verified.",
} as const;
  ...
  tooltip: (innerProps) =>
    innerProps.isDiskEncryptionProfile
      ? "The host acknowledged the MDM command to turn on disk encryption. " +
        "Mobius is verifying with osquery and retrieving the disk encryption key. " +
        "This may take up to one hour."
      : "The host acknowledged the MDM command to apply the setting. Mobius is " +
...
export const WINDOWS_DISK_ENCRYPTION_DISPLAY_CONFIG: WindowsDiskEncryptionDisplayConfig = {
  verified: {
    ...
    iconName: "success",
    tooltip: () =>
      "The host turned disk encryption on and sent the key to Mobius. Mobius verified.",
  },
  verifying: {
    statusText: "Verifying",
    iconName: "success-outline",
    tooltip: () =>
      "The host acknowledged the MDM command to turn on disk encryption. Mobius is verifying with " +
