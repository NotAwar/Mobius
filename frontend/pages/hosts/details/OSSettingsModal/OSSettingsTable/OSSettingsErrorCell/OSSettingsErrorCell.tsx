...
const RefetchButton = ({ isFetching, onClick }: IRefetchButtonProps) => {
  ...
  return (
    ...
    >
      <Icon name="refresh" color="core-mobius-blue" size="small" />
      {buttonText}
    </Button>
  );
};
...
const formatDetailCertificateError = (detail: IHostMdmProfile["detail"]) => {
  ...
  const matchMobiusVarErr = detail.match(
    /populate (?<field>.*) because (?<ca>.*) certificate authority does(?:n.t| not) exist/
  );
  if (matchMobiusVarErr?.groups) {
    return (
      <>
        Mobius couldn&apos;t populate {matchMobiusVarErr.groups.field} because{" "}
        <b>{matchMobiusVarErr.groups.ca}</b> certificate authority doesn&apos;t
        exist. Please go to{" "}
        <b>
          Settings {">"} Integrations {">"} Certificates
        </b>
  ...
  if (detail.includes("There is no IdP email for this host.")) {
    return (
      <>
        ...
        <br />
        Mobius couldn&apos;t populate
        <br />
        $MOBIUS_VAR_HOST_END_USER_EMAIL_IDP.
        <br />
        <CustomLink
          text="Learn more"
          url="https://mobius-mdm.org/learn-more-about/idp-email"
          newTab
