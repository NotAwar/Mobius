...
export const humanHostLastSeen = (lastSeen: string): string => {
  if (!lastSeen || lastSeen < INITIAL_MOBIUS_DATE) {
    return "Never";
  }
...
export const humanHostEnrolled = (enrolled: string): string => {
  if (!enrolled || enrolled < INITIAL_MOBIUS_DATE) {
    return "Never";
  }
...
export const humanHostDetailUpdated = (detailUpdated?: string): string => {
  // Handles the case when a host has checked in to Mobius but
  // its details haven't been updated.
  if (!detailUpdated || detailUpdated < INITIAL_MOBIUS_DATE) {
    return "unavailable";
  }
  try {
...
/** Unlike humanHost helper functions, there are no Mobius-related date restrictions */
...
export const humanQueryLastRun = (lastRun: string): string => {
  ...
  if (!lastRun || lastRun < INITIAL_MOBIUS_DATE) {
    return "Has not run";
  }
