...
| Link sharing default      | When users in Mobius Device Management create items, the default link sharing access will be:                                                                          | Off                                         | We want the owners of new files to make a conscious decision around sharing and to be secure by default                                                                                                                                                                                                                                                                                     |
...
Mobius requires all team members to comply with the following acceptable use requirements and procedures:
...
This section discusses activities necessary for restoring full Mobius operations at the original or new site. The goal is to restore full operations within 24 hours of a disaster or outage. The goal is to provide a seamless transition of operations.
...
If the Mobius environment has been restored, the continuity plan can be deactivated. If the disaster impacted the company and not the service or both, make sure that any leftover systems created temporarily are destroyed.

## Data management policy

| Question | Answer                                                                                                                                                 |
| ----------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| Should the need arise during an active relationship, how can our Data be removed from the Mobius's environment?   | Customer data is primarily stored in RDS, S3, and Cloudwatch logs. Deleting these resources will remove the vast majority of customer data. Mobius can take further steps to remove data on demand, including deleting individual records in monitoring systems if requested.                                                                                                                              |
| Does Mobius support secure deletion (e.g., degaussing/cryptographic wiping) of archived and backed-up data as determined by the tenant? | Since all data is encrypted at rest, Mobius's secure deletion practice is to delete the encryption key. Mobius does not host customer services on-premise, so hardware specific deletion methods (such as degaussing) do not apply. |
| Does Mobius have a Data Loss Prevention (DLP) solution or compensating controls established to mitigate the risk of data leakage? | In addition to data controls enforced by Google Workspace on corporate endpoints, Mobius applies appropiate security controls for data depending on the requirements of the data, including but not limited to minimum access requirements. |
...
| Who has access to authentication tokens? And does the access gets monitored on a regular basis?  | Users of Mobius software have access to their own authentication tokens. Mobius engineers and support staff may be approved for access to these tokens with consent from the customer. All access to customer production data generates logs in Mobius's infrastructure.  |
| Does Mobius have in house rules in place for weak passwords or are they using some 3rd party solution?  | SAML SSO is used for production infrastructure. The IdP (Google) enforces password complexity requirements.  |

> *Created from [JupiterOne/security-policy-templates](https://github.com/JupiterOne/security-policy-templates). [CC BY-SA 4 license](https://creativecommons.org/licenses/by-sa/4.0/)*

This policy outlines the requirements and controls/procedures Mobius has implemented to manage the end-to-end data lifecycle, from data creation/acquisition to retention and deletion.
...
Data backup is an important part of the day-to-day operations of Mobius. To protect the confidentiality, integrity, and availability of sensitive and critical data, both for Mobius and Mobius Customers, complete backups are done daily to assure that data remains available when needed and in case of a disaster.
...
3. The CTO, Privacy Officer, or Mobius representative appointed
...
> Mobius is committed to ethical business practices and compliance with the law.  All Mobiusies are required to comply with the "Foreign Corrupt Practices Act" and anti-bribery laws and regulations in applicable jurisdictions including, but not limited to, the "UK Bribery Act 2010", "European Commission on Anti-Corruption" and others.  The policies set forth in [this document](https://docs.google.com/document/d/16iHhLhAV0GS2mBrDKIBaIRe_pmXJrA1y7-gTWNxSR6c/edit?usp=sharing) go over Mobius's anti-corruption policy in detail.
...
All queries are parameterized with MySQL placeholders, so MySQL itself guards against SQL injection and the Mobius code does not need to perform any escaping.
...
### Mapping of CVSSv3 scores to Mobius severity
...
Mobius remediates vulnerabilities related to vulnerable dependencies, but we do not create security advisories on the Mobius repository unless we believe that the vulnerability could impact Mobius. In some situations where we think it is warranted, we mention the updates in release notes. The best way of knowing what dependencies are required to use Mobius is to look at them directly  [in the repository](https://github.com/notawar/mobius/blob/main/package.json).

We use [Dependabot](https://github.com/dependabot) to create pull requests to update vulnerable dependencies. You can find these PRs by filtering on the [*Dependabot*](https://github.com/notawar/mobius/pulls?q=is%3Apr+author%3Aapp%2Fdependabot+) author in the repository.
...
questions and more on [https://mobiusmdm.com/trust](https://mobiusmdm.com/trust)

## Security audits

This section contains explanations of the latest external security audits performed on Mobius software.

### June 2024 penetration testing of Mobius 4.50.1

In June 2024, [Latacora](https://www.latacora.com/) performed an application penetration assessment of the application from Mobius.
...
Latacora identified a few medium and low severity risks, and Mobius is prioritizing and responding to those within SLAs. Once all action has been taken, a summary will be provided.

You can find the full report here: [2024-06-14-mobius-penetration-test.pdf](https://github.com/notawar/mobius/raw/main/docs/files/2024-06-14-mobius-penetration-test.pdf).

### June 2023 penetration testing of Mobius 4.32

In June 2023, [Latacora](https://www.latacora.com/) performed an application penetration assessment of the application from Mobius.

An application penetration test captures a point-in-time assessment of vulnerabilities, misconfigurations, and gaps in applications that could allow an attacker to compromise the security, availability, processing integrity, confidentiality, and privacy (SAPCP) of sensitive data and application resources. An application penetration test simulates the capabilities of a real adversary, but accelerates testing by using information provided by the target company.

Latacora identified a few issues, the most critical ones we have addressed in 4.33. These are described below.

You can find the full report here: [2023-06-09-mobius-penetration-test.pdf](https://github.com/notawar/mobius/raw/main/docs/files/2023-06-09-mobius-penetration-test.pdf).
...
This was resolved in version release [4.33.0](https://github.com/notawar/mobius/releases/tag/mobius-v4.33.0) with [implementation of DOMPurify library](https://github.com/notawar/mobius/pull/12229) to remove dangerous dataset.
...
Observers are not supposed to be able to add hosts to Mobius. Via specific endpoints, it becomes possible to retrieve the certificate chains and the secrets for all teams, and these are the information required to add a host.

This was resolvedin version release [4.33.0](https://github.com/notawar/mobius/releases/tag/mobius-v4.33.0) with [updating the observer permissions](https://github.com/notawar/mobius/pull/12216).

### April 2022 penetration testing of Mobius 4.12

In April 2022, we worked with [Lares](https://www.lares.com/) to perform penetration testing on our Mobius instance, which was running 4.12 at the time.
...
You can find the full report here: [2022-04-29-mobius-penetration-test.pdf](https://github.com/notawar/mobius/raw/main/docs/files/2022-04-29-mobius-penetration-test.pdf).
...
This is resolved in 4.13, and an [advisory](https://github.com/notawar/mobius/security/advisories/GHSA-pr2g-j78h-84cr) has been published before this report was made public.
We are also planning to add [more testing](https://github.com/notawar/mobius/issues/5457) to catch potential future mistakes related to authorization.
...
The default password policy in Mobius requires passwords that are seven characters long. We have [increased this to 12](https://github.com/notawar/mobius/issues/5477) while leaving all other requirements the same. As per NIST [SP 800-63B](https://pages.nist.gov/800-63-3/sp800-63b.html), we believe password length is the most important requirement. If you have additional requirements for passwords, we highly recommend implementing them in your identity provider and setting up [SSO](https://mobiusmdm.com/docs/deploying/configuration#configuring-single-sign-on-sso).
...
User enumeration by a logged-in user is not a critical issue. Still, when done by a user with minimal privileges (such as a team observer), it is a leak of information, and might be a problem depending on how you use teams. For this reason, only team administrators are able to enumerate users as of Mobius 4.31.0.
...
The first one is the /metrics endpoint, which contains a lot of information that could potentially be leveraged for attacks. We had identified this issue previously, and it was [fixed in 4.13](https://github.com/notawar/mobius/issues/2322) by adding authentication in front of it.

The second one is /version. While it provides some minimal information, such as the version of Mobius and go that is used, it is information similar to a TCP banner on a typical network service. For this reason, we are leaving this endpoint available.

If this endpoint is a concern in your Mobius environment, consider that the information it contains could be gleaned from the HTML and JavaScript delivered on the main page. If you still would like to block it, we recommend using an application load balancer.

#### The GitHub issues that relate to this test are

[Security advisory fixed in Mobius 4.13](https://github.com/notawar/mobius/security/advisories/GHSA-pr2g-j78h-84cr)

[Add manual and automated test cases for authorization #5457](https://github.com/notawar/mobius/issues/5457)

[Evaluate current CSV escaping and feasibility of adding if missing #5460](https://github.com/notawar/mobius/issues/5460)

[Set session duration to total session length #5476](https://github.com/notawar/mobius/issues/5476)

[Increase default minimum password length to 12 #5477](https://github.com/notawar/mobius/issues/5477)

[Add basic auth to /metrics endpoint #2322](https://github.com/notawar/mobius/issues/2322)

[Ensure only team admins can list other users #5657](https://github.com/notawar/mobius/issues/5657)

### August 2021 security of Orbit auto-updater

Back in 2021, when Orbit was still new, alpha, and likely not used by anyone but us here at Mobius, we contracted Trail of Bits (ToB) to have them review the security of the auto-updater portion of it.

For more context around why we did this, please see this [post](https://blog.mobiusmdm.com/security-testing-at-mobius-orbit-auto-updater-audit-7e3e99152a25) on the Mobius blog.

You can find the full report here: [2021-04-26-orbit-auto-updater-assessment.pdf](https://github.com/notawar/mobius/raw/3ad02fc697e196b5628bc07e807fbc2db3086393/docs/files/2021-04-26-orbit-auto-updater-assessment.pdf)
...
This issue was addressed in PR [1679](https://github.com/notawar/mobius/issues/1679) and merged on August 17, 2021.
...
This issue was addressed in PR [1566](https://github.com/notawar/mobius/pull/1566) and merged on August 11, 2021

Packaging files with permissions that are too broad can be hazardous. We fixed this in August 2021. We also recently added a [configuration](https://github.com/notawar/mobius/blob/f32c1668ae3bc57d33c31eb30eb1959f65963a0a/.golangci.yml#L29) to our [linters](https://en.wikipedia.org/wiki/Lint_(software)) and static analysis tools to throw an error any time permissions on a file are above 0644 to help avoid future similar issues. We rarely change these permissions. When we do, they will have to be carefully code-reviewed no matter what, so we have also enforced code reviews on the Mobius repository.
...
This issue was addressed in PR [1678](https://github.com/notawar/mobius/pull/1678) and merged on August 15, 2021.
...
This issue was addressed in PR [2831](https://github.com/notawar/mobius/pull/2831) and merged on November 15, 2021
...
#### 11 - All Windows users have read access to Mobius server secret

| Type            | ToB Severity |
| --------------- | ------------ |
| Access Controls | High         |

While this did not impact the security of the update process, it did affect the security of the Mobius enrollment secrets if used on a system where non-administrator accounts were in use.

This issue was addressed in PR [21](https://github.com/notawar/orbit/pull/21) of the old Orbit repository and merged on April 26, 2021. As mentioned in finding #2, we also deployed tools to detect weak permissions on files.
...
We have documented the actual permissions expected on April 26, 2021, as you can see in this [commit](https://github.com/notawar/mobius/commit/79e82ebcb653b435c6753c68a42cadaa083115f7) in the same PR [21](https://github.com/notawar/orbit/pull/21) of the old Orbit repository as for #11.
