...
resource "google_secret_manager_secret" "secret" {
  secret_id = "mobius-db-password-${random_pet.suffix.id}"
  replication {
    automatic = true
  }
}
...
resource "google_secret_manager_secret" "private_key" {
  secret_id = "mobius-private-key-${random_pet.suffix.id}"
  replication {
    automatic = true
  }
}
...
resource "google_secret_manager_secret_version" "secret-version-data" {
  ...
  secret_data = module.mobius-mysql.generated_user_password
}
  ...
  template {
    spec {
      containers {
        ...
        env {
          name  = "MOBIUS_S3_SOFTWARE_INSTALLERS_ACCESS_KEY_ID"
          value = google_storage_hmac_key.key.access_id
        }
        env {
          name  = "MOBIUS_S3_SOFTWARE_INSTALLERS_SECRET_ACCESS_KEY"
          value = google_storage_hmac_key.key.secret
        }
        env {
          name  = "MOBIUS_S3_SOFTWARE_INSTALLERS_ENDPOINT_URL"
          value = "https://storage.googleapis.com"
        }
        env {
          // software_installers_force_s3_path_style
          name  = "MOBIUS_S3_SOFTWARE_INSTALLERS_FORCE_S3_PATH_STYLE"
          value = "true"
        }
        env {
          name  = "MOBIUS_S3_SOFTWARE_INSTALLERS_REGION"
          value = "auto"
        }
        env {
        ...
        env {
          ...
          value = module.mobius-mysql.private_ip_address
        }
        env {
        ...
        args = [
          "-c",
          "mobius prepare --no-prompt=true db; exec mobius serve"
        ]
      }
    }
    ...
    metadata {
      annotations = {
        ...
        "run.googleapis.com/cloudsql-instances"   = module.mobius-mysql.instance_connection_name
