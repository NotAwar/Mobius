module "s3_bucket_for_logs" {
  ...
  bucket = "mobius-loadtesting-alb-logs"
...
module "athena-s3-bucket" {
  ...
  bucket = "mobius-loadtesting-alb-logs-athena"
...
resource "aws_athena_database" "logs" {
  name   = "mobius_loadtesting_alb_logs"
  bucket = module.athena-s3-bucket.s3_bucket_id
}

resource "aws_athena_workgroup" "logs" {
  name = "mobius-loadtesting-logs"
...
resource "aws_alb_listener" "https-mobiusmdm" {
  ...
  default_action {
    ...
    fixed_response {
      ...
      message_body = "moved to subdomains, try https://default.loadtest.mobiusmdm.com"
      status_code  = "404"
    }
  }
}
