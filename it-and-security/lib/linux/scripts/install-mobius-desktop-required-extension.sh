...
if [ ! -d "/home/$username/.local/share/gnome-shell/extensions/$extension_name" ]; then
	...
	sudo -i -u $username -H DBUS_SESSION_BUS_ADDRESS=unix:path=/run/user/$uid/bus \
		...
		"Mobius Desktop" 0 \"\" "Mobius Desktop" "Install a GNOME extension to enable Mobius Desktop. This lets you see what your organization is doing on your computer." "[]" '{"urgency": <2>}' 0
