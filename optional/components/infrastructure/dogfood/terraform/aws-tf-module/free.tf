locals {
  ...
  extra_environment_variables_free = {
    ...
    MOBIUS_MYSQL_MAX_OPEN_CONNS                 = "25"
...
module "free" {
  source = "github.com/notawar/mobius-terraform//byo-vpc?ref=tf-mod-byo-vpc-v1.13.0"
  vpc_config = {
  ...
  mobius_config = {
    ...
    extra_iam_policies          = module.ses-free.mobius_extra_iam_policies
    extra_environment_variables = merge(module.ses-free.mobius_extra_environment_variables, local.extra_environment_variables_free, module.geolite2.extra_environment_variables)
    private_key_secret_name     = "${local.customer_free}-mobius-server-private-key"
  }
  alb_config = {
    name            = local.customer_free
...
module "acm-free" {
  ...
  domain_name = "free.mobiusmdm.com"
