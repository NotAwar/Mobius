provider "aws" {
  default_tags {
    tags = {
      environment = "dogfood"
      terraform   = "https://github.com/notawar/mobius/main/infrastructure/dogfood/terraform"
      state       = "s3://mobius-terraform-remote-state/mobius"
    }
  }
}
...
terraform {
  ...
  backend "s3" {
    bucket         = "mobius-terraform-remote-state"
    region         = "us-east-2"
    key            = "mobius"
    dynamodb_table = "mobius-terraform-state-lock"
  }
  required_providers {
    aws = {
...
variable "mobius_image" {
  default = "160035666661.dkr.ecr.us-east-2.amazonaws.com/mobius:1f68e7a5e39339d763da26a0c8ae3e459b2e1f016538d7962312310493381f7c"
}
variable "geolite2_license" {}
variable "mobius_sentry_dsn" {}
variable "elastic_url" {}
variable "elastic_token" {}
variable "mobius_calendar_periodicity" {
  default     = "30s"
...
locals {
  customer       = "mobius-dogfood"
  mobius_image   = var.mobius_image # Set this to the version of mobius to be deployed
  geolite2_image = "${aws_ecr_repository.mobius.repository_url}:${split(":", var.mobius_image)[1]}-geolite2-${formatdate("YYYYMMDDhhmm", timestamp())}"
  extra_environment_variables = {
    MOBIUS_LICENSE_KEY   = var.mobius_license
    ...
    MOBIUS_MYSQL_MAX_OPEN_CONNS                 = "10"
    MOBIUS_MYSQL_READ_REPLICA_MAX_OPEN_CONNS    = "10"
    ...
    MOBIUS_CALENDAR_PERIODICITY            = var.mobius_calendar_periodicity
    MOBIUS_DEV_ANDROID_ENABLED             = "1"
    MOBIUS_DEV_ANDROID_SERVICE_CREDENTIALS = var.android_service_credentials
    # Webhook Results & Status Logging Destination
    MOBIUS_WEBHOOK_STATUS_URL        = var.webhook_url
    MOBIUS_WEBHOOK_RESULT_URL        = var.webhook_url
    MOBIUS_OSQUERY_RESULT_LOG_PLUGIN = var.webhook_url != "" ? "webhook" : ""
  }
  sentry_secrets = {
    MOBIUS_SENTRY_DSN = "${aws_secretsmanager_secret.sentry.arn}:MOBIUS_SENTRY_DSN::"
  }
  # idp_metadata_file = "${path.module}/files/idp-metadata.xml"
}

module "main" {
  source          = "github.com/notawar/mobius-terraform?ref=tf-mod-root-v1.15.2"
  ...
  mobius_config = {
    ...
    extra_iam_policies = concat(module.firehose-logging.mobius_extra_iam_policies, module.osquery-carve.mobius_extra_iam_policies, module.ses.mobius_extra_iam_policies)
    extra_environment_variables = merge(
      module.osquery-carve.mobius_extra_environment_variables,
      module.ses.mobius_extra_environment_variables,
    ...
    ) #, module.saml_auth_proxy.mobius_extra_execution_policies)
    extra_secrets = merge(
      module.mdm.extra_secrets,
      local.sentry_secrets,
      module.cloudfront-software-installers.extra_secrets
    )
    private_key_secret_name = "${local.customer}-mobius-server-private-key"
    ...
    #     command = [
    #       "osqueryd",
    #       "--tls_hostname=dogfood.mobiusmdm.com",
    #       "--force=true",
  ...
  alb_config = {
    ...
    #      }]
    #      conditions = [{
    #        path_patterns = ["/device/*", "/api/*/mobius/device/*", "/saml/*"]
    #      }]
    #      }, {
    #      https_listener_index = 0
    #      priority             = 1
    ...
    #      }]
    #      conditions = [{
    #        path_patterns = ["/api/*/mobius/device/*/migrate_mdm", "/api/*/mobius/device/*/rotate_encryption_key"]
    #      }]
    #      }, {
    #      https_listener_index = 0
    #      priority             = 2
    ...
    #      }]
    #      conditions = [{
    #        path_patterns = ["/api/*/mobius/device/*/debug/errors", "/api/*/mobius/device/*/desktop"]
    #      }]
    #      }, {
    #      https_listener_index = 0
    #      priority             = 3
    ...
    #      }]
    #      conditions = [{
    #        path_patterns = ["/api/*/mobius/device/*/refetch", "/api/*/mobius/device/*/transparency"]
    #      }]
    #    }]
  }
}
...
module "acm" {
  ...
  domain_name = "dogfood.mobiusmdm.com"
...
resource "aws_secretsmanager_secret_version" "sentry" {
  ...
  secret_string = jsonencode({
    MOBIUS_SENTRY_DSN = var.mobius_sentry_dsn
  })
}

resource "aws_iam_policy" "sentry" {
  name   = "mobius-sentry-secret-policy"
...
module "migrations" {
  ...
  source                   = "github.com/notawar/mobius-terraform//addons/migrations?ref=tf-mod-addon-migrations-v2.0.1"
...
module "mdm" {
  source             = "github.com/notawar/mobius-terraform//addons/mdm?ref=tf-mod-addon-mdm-v2.0.0"
...
module "firehose-logging" {
  source                = "github.com/notawar/mobius-terraform//addons/byo-firehose-logging-destination/firehose?ref=tf-mod-addon-byo-firehose-logging-destination-firehose-v2.0.3"
  firehose_results_name = "osquery_results"
  firehose_status_name  = "osquery_status"
  firehose_audit_name   = "mobius_audit"
  iam_role_arn          = "arn:aws:iam::273354660820:role/terraform-20250115232230102400000003"
  region                = data.aws_region.current.name
}

module "osquery-carve" {
  source = "github.com/notawar/mobius-terraform//addons/osquery-carve?ref=tf-mod-addon-osquery-carve-v1.1.0"
  osquery_carve_s3_bucket = {
    name = "mobius-${local.customer}-osquery-carve"
  }
}

module "monitoring" {
  source                  = "github.com/notawar/mobius-terraform//addons/monitoring?ref=tf-mod-addon-monitoring-v1.5.1"
  customer_prefix         = local.customer
  mobius_ecs_service_name = module.main.byo-vpc.byo-db.byo-ecs.service.name
  albs = [
    {
      name                    = module.main.byo-vpc.byo-db.alb.lb_dns_name,
...
module "logging_alb" {
  source        = "github.com/notawar/mobius-terraform//addons/logging-alb?ref=tf-mod-addon-logging-alb-v1.3.0"
  prefix        = local.customer
  enable_athena = true
}

resource "aws_iam_policy" "ecr" {
  name   = "mobius-ecr-policy"
...
resource "aws_ecr_repository" "mobius" {
  name                 = "mobius"
...
module "notify_slack" {
  ...
  sns_topic_name = "mobius-dogfood-p1-alerts"
...
module "notify_slack_p2" {
  ...
  sns_topic_name       = "mobius-dogfood-p2-alerts"
...
module "ses" {
  source  = "github.com/notawar/mobius-terraform//addons/ses?ref=tf-mod-addon-ses-v1.3.0"
  zone_id = aws_route53_zone.main.zone_id
  domain  = "dogfood.mobiusmdm.com"
}

# module "saml_auth_proxy" {
#   # source                       = "github.com/notawar/mobius-terraform//addons/saml-auth-proxy?ref=main"
...
#   security_groups              = module.main.byo-vpc.byo-db.byo-ecs.service.network_configuration[0].security_groups
#   base_url                     = "https://dogfood.mobiusmdm.com/"
#   subnets                      = module.main.byo-vpc.byo-db.byo-ecs.service.network_configuration[0].subnets
#   vpc_id                       = module.main.vpc.vpc_id
...
#   version                               = "3.15.1"
#   bucket                                = "mobius-dogfood-idp-metadata"
...
module "geolite2" {
  source            = "github.com/notawar/mobius-terraform//addons/geolite2?ref=tf-mod-addon-geolite2-v1.0.0"
  mobius_image      = var.mobius_image
  destination_image = local.geolite2_image
  license_key       = var.geolite2_license
}

module "vuln-processing" {
  source                               = "github.com/notawar/mobius-terraform//addons/external-vuln-scans?ref=tf-mod-addon-external-vuln-scans-v2.2.0"
  ecs_cluster                          = module.main.byo-vpc.byo-db.byo-ecs.service.cluster
  execution_iam_role_arn               = module.main.byo-vpc.byo-db.byo-ecs.execution_iam_role_arn
  subnets                              = module.main.byo-vpc.byo-db.byo-ecs.service.network_configuration[0].subnets
  security_groups                      = module.main.byo-vpc.byo-db.byo-ecs.service.network_configuration[0].security_groups
  mobius_config                        = module.main.byo-vpc.byo-db.byo-ecs.mobius_config
  task_role_arn                        = module.main.byo-vpc.byo-db.byo-ecs.iam_role_arn
  mobius_server_private_key_secret_arn = module.main.byo-vpc.byo-db.byo-ecs.mobius_server_private_key_secret_arn
  awslogs_config = {
    group  = module.main.byo-vpc.byo-db.byo-ecs.mobius_config.awslogs.name
    region = module.main.byo-vpc.byo-db.byo-ecs.mobius_config.awslogs.region
    prefix = module.main.byo-vpc.byo-db.byo-ecs.mobius_config.awslogs.prefix
  }
  mobius_s3_software_installers_config = module.main.byo-vpc.byo-db.byo-ecs.mobius_s3_software_installers_config
}
...
module "cloudfront-software-installers" {
  source            = "github.com/notawar/mobius-terraform//addons/cloudfront-software-installers?ref=tf-mod-addon-cloudfront-software-installers-v1.0.0"
  customer          = local.customer
  s3_bucket         = module.main.byo-vpc.byo-db.byo-ecs.mobius_s3_software_installers_config.bucket_name
  s3_kms_key_id     = module.main.byo-vpc.byo-db.byo-ecs.mobius_s3_software_installers_config.kms_key_id
