...
resource "aws_autoscaling_group" "elasticstack" {
  ...
  tag {
    ...
    value               = "https://github.com/notawar/mobius.git"
    propagate_at_launch = true
  }
...
data "aws_iam_policy_document" "elasticstack" {
  statement {
    ...
    resources = ["arn:aws:secretsmanager:us-east-2:917007347864:secret:/mobius/ssh/keys-7iQNe1"]
  }
...
resource "aws_alb_listener" "elasticsearch" {
  ...
  certificate_arn   = aws_acm_certificate_validation.mobiusdm_com.certificate_arn
