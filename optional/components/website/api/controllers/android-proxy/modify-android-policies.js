module.exports = {
  ...
  fn: async function ({ androidEnterpriseId, policyId}) {
    ...
    // Extract mobiusServerSecret from the Authorization header
    let authHeader = this.req.get('authorization');
    let mobiusServerSecret;

    if (authHeader && authHeader.startsWith('Bearer')) {
      mobiusServerSecret = authHeader.replace('Bearer', '').trim();
    } else {
    ...
    if (thisAndroidEnterprise.mobiusServerSecret !== mobiusServerSecret) {
    ...
    // Return the modified policy back to the Mobius server.
