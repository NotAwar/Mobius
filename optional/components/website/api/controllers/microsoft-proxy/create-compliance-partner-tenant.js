module.exports = {
  ...
  exits: {
    success: { description: 'Details about a new Microsoft complaince tsenant have been returned to a Mobius isntance' },
  ...
  fn: async function ({entraTenantId}) {
    ...
    // Look for an existing microsoftComplianceTenant record using the requesting Mobius instances URL.
    let existingComplianceTenant = await MicrosoftComplianceTenant.findOne({mobiusInstanceUrl: this.req.get('origin')});
    if(existingComplianceTenant) {
      ...
      if(!existingComplianceTenant.setupCompleted) {
        ...
        // If setup was already completed for the existing tenant, return a 409 response. (The user will need to delete the existing integration in the Mobius UI before creating a new one.)
    ...
    let newTenant = await MicrosoftComplianceTenant.create({
      mobiusServerSecret: sails.helpers.strings.random.with({len: 30}),
      entraTenantId: entraTenantId,
      mobiusInstanceUrl: this.req.get('origin'),
      setupCompleted: false,
    }).fetch();


    return {
      mobius_server_secret: newTenant.mobiusServerSecret,// eslint-disable-line camelcase
