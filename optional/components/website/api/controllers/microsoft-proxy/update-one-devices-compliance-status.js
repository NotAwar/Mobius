module.exports = {
  ...
  inputs: {
    ...
    deviceName: {
      type: 'string',
      description: 'The device\'s  display name in Mobius.',
      required: true,
    },
    os: {
      type: 'string',
    ...
    lastCheckInTime: {
      type: 'number',
      description: 'The device\'s last sync with Mobius in Unix seconds',
      required: true,
    },
  },
  ...
  fn: async function ({entraTenantId, mobiusServerSecret, deviceId, deviceManagementState, deviceName, os, osVersion, userPrincipalName, compliant, lastCheckInTime}) {


    let informationAboutThisTenant = await MicrosoftComplianceTenant.findOne({entraTenantId: entraTenantId, mobiusServerSecret: mobiusServerSecret});
    if(!informationAboutThisTenant) {
      return new Error({error: 'No MicrosoftComplianceTenant record was found that matches the provided entra_tenant_id and mobius_server_secret combination.'});
    }
