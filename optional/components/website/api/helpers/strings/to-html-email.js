module.exports = {
  ...
  description: 'Compile a Markdown string into an HTML string with styles added inline for the Mobius newsletter.',
  ...
  fn: function(inputs, exits) {
    ...
    customRenderer.link = function(href, title, textHTML) {
      (href)=>{
        let isExternal = ! href.match(/^https?:\/\/([^\.|blog]+\.)*mobiusmdm\.com/g);// « FUTURE: make this smarter with sails.config.baseUrl + _.escapeRegExp()
        // Check if this link is to mobiusmdm.com or www.mobiusmdm.com.
        let isBaseUrl = href.match(/^(https?:\/\/)([^\.]+\.)*mobiusmdm\.com$/g);
        if (isExternal) {
    ...
    customRenderer.image = function(href, title) {
      let linkToImageInAssetsFolder = href.replace(/^(\.\.\/website\/assets)/gi, 'https://mobiusmdm.com');
