parasails.registerPage('basic-documentation', {
  ...
  mounted: async function() {
    ...
    // Set a currentDocsSection value to display different Mobius premium CTAs based on what section is being viewed.
    this.currentDocsSection = this.thisPage.url.split(/\//).slice(-2)[0];

    // Handle hashes in urls when coming from an external page.
    if(window.location.hash){
      // If a hash was provided, we'll remove the # and any query parameters from it. (e.g., #create-an-api-only-user?utm_medium=mobiusui&utm_campaign=get-api-token » create-an-api-only-user)
  ...
  methods: {
    ...
    findPagesByUrl: function (url='') {
      let slug;
      // if no url is passed, use the base url as the slug (e.g., 'docs' )
      if (!url) {
    ...
    //     'using-mobius':['The basics', 'Device management', 'Vuln management', 'Security compliance', 'Osquery management', 'Dig deeper'],
    //   };
    ...
    getTitleFromUrl: function (url) {
      return _
        ...
        .last()
        .split(/-/)
        .map((str) => str === 'mobius' ? 'Mobius' : str === 'rest' ? 'REST' : str === 'api' ? 'API' : str)
        .join(' ')
        .capitalize()
        .value();
    },
