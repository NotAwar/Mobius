parasails.registerPage('signup', {
  ...
  beforeMount: function() {
    ...
    if(window.location.hash){
      ...
      } else if(window.location.hash === '#trymobius') {
        this.loginSlug = '/login#trymobius';
        this.pageToRedirectToAfterRegistration = '/try-mobius';
        window.location.hash = '';
      }
    }
  },
  ...
  methods: {
    ...
    submittedSignUpForm: async function() {// When the server says everything worked…
      ...
      // > "Website - Sign up" becomes "mobius_website__sign_up"  (double-underscore representing hierarchy)
      if(window.gtag !== undefined){
        window.gtag('event','mobius_website__sign_up');
      }
