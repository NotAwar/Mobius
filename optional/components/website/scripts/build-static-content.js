  ...
  fn: async function ({ dry, githubAccessToken }) {
    ...
    await sails.helpers.flow.simultaneously([
      ...
      async()=>{
        ...
        let appsJsonData = await sails.helpers.fs.readJson(path.join(topLvlRepoPath, '/maintained-apps/outputs/apps.json'));
        ...
        await sails.helpers.flow.simultaneouslyForEach(appsJsonData.apps, async(app)=>{
          ...
          let detailedInformationAboutThisApp = await sails.helpers.fs.readJson(path.join(topLvlRepoPath, '/maintained-apps/outputs/'+app.slug+'.json'))
          .intercept('doesNotExist', ()=>{
            return new Error(`Could not build app library configuration from the ee/maintained-apps folder. When attempting to read a JSON configuration file for ${appInformation.identifier}, no file was found at ${path.join(topLvlRepoPath, '/maintained-apps/outputs/'+app.slug+'.json')}. Was it moved?')}.`);
          });
