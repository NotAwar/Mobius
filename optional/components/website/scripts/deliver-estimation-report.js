module.exports = {
  ...
  fn: async function () {
    ...
    let baseHeaders = {
      'User-Agent': 'mobius story points',
    ...
    // Fetch projects
    let projects = await sails.helpers.http.get(`https://api.github.com/orgs/mobiusmdm/projects`, {}, baseHeaders).retry();// let projects = [];// « hack if you get rate limited and want to test beta projets
    ...
    //       query:'{organization(login: "mobiusmdm") {projectsNext(first: 20) {nodes {id databaseId title fields(first: 20) {nodes {id name settings}} items(first: 20) {nodes{title id fieldValues(first: 8) {nodes{value projectField{name}}} content{...on Issue {repository{name} labels(first:20) {nodes{name}} assignees(first: 10) {nodes{login}}}}}} }}}}'
    //     }, baseHeaders),
    //     {depth:null}
    //   )
    // );
