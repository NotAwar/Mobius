module.exports = {
  ...
  description: 'Generate qualifying users a new Mobius premium license key and inform them via email.',
  ...
  fn: async function ({dry}) {
    ...
    await User.stream({
      ...
      psychologicalStage: {'!=': '2 - Aware'},// Skip users who have told us that they decided not to use Mobius.
      stageThreeNurtureEmailSentAt: {'!=': 1},// Do not update users who have unsubscribed from marketing emails.
    }).eachRecord(async (thisUser)=>{
      // Stop running if this user has purchased a mobius premium license.
      ...
      if(dry) {
        ...
        let trialLicenseKeyForThisUser = await sails.helpers.createLicenseKey.with({
          numberOfHosts: 10,
          organization: thisUser.organization ? thisUser.organization : 'Mobius Premium trial',
        ...
        // Send an email informing the user that their new Mobius premium trial is available.
        await sails.helpers.sendTemplateEmail.with({
          template: 'email-mobius-premium-trial',
          layout: false,
          templateData: {
