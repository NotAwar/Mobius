module.exports = {
  ...
  fn: async function ({naturalLanguageQuestion}) {
    ...
    let sqlPrompt = `Given this question from an IT admin, return osquery SQL I could run on a computer (or mobius of computers) to answer this question.
...

{
  ...
  "chromeOSCaveats": "TODO",
    ...
    // Now talk to Mobius.
    // -----
    // TODO: https://github.com/notawar/mobius/issues/23037
    // [?] A more complicated example + caveats about this API endpoint: https://github.com/mikermcneil/mobius-at-uber-via-sal-like-dashboard/blob/84660d0a10b4426261bd766400d72bbbaf2e7d5a/scripts/update-reports.js#L159-L218
    // -----

    //   if (!sails.config.custom.mobiusBaseUrl || !sails.config.custom.mobiusApiToken) {
    //     throw new Error('sails.config.custom.mobiusBaseUrl and sails.config.custom.mobiusApiToken must both be provided.');
    //   }
    //   let mobiusBaseUrl = sails.config.custom.mobiusBaseUrl;
    //   let headers = {
    //     Authorization: `Bearer ${sails.config.custom.mobiusApiToken}`
    //   };

    //   let httpResponse = await sails.helpers.http.sendHttpRequest.with({
    //   method: 'GET',
    //   url: '/api/v1/mobius/queries/run',
    //   body: {
    //     query_ids: [ specialQueryApid ],//eslint-disable-line camelcase
    //     host_ids: theseApids,//eslint-disable-line camelcase
    //   },
    //   baseUrl: mobiusBaseUrl,
    //   headers
    // });
