...
func setupRunners() {
	...
	if err := runnerGroup.Run(); err != nil {
		log.Error().Err(err).Msg("Mobius Desktop runners terminated")
		return
	}
}
...
func main() {
	...
	// Orbits uses --version to get the mobius-desktop version. Logs do not need to be set up when running this.
	if len(os.Args) > 1 && os.Args[1] == "--version" {
		// Must work with update.GetVersion
		fmt.Println("mobius-desktop", version)
		return
	}
	...
	if len(os.Args) > 1 && os.Args[1] == "--help" {
		fmt.Println("Mobius Desktop application executable")
		return
	}
	log.Info().Msgf("mobius-desktop version=%s", version)

	identifierPath := os.Getenv("MOBIUS_DESKTOP_DEVICE_IDENTIFIER_PATH")
	if identifierPath == "" {
		log.Fatal().Msg("missing URL environment MOBIUS_DESKTOP_DEVICE_IDENTIFIER_PATH")
	}

	mobiusURL := os.Getenv("MOBIUS_DESKTOP_MOBIUS_URL")
	if mobiusURL == "" {
		log.Fatal().Msg("missing URL environment MOBIUS_DESKTOP_MOBIUS_URL")
	}

	mobiusTLSClientCertificate := os.Getenv("MOBIUS_DESKTOP_MOBIUS_TLS_CLIENT_CERTIFICATE")
	mobiusTLSClientKey := os.Getenv("MOBIUS_DESKTOP_MOBIUS_TLS_CLIENT_KEY")
	mobiusClientCrt, err := certificate.LoadClientCertificate(mobiusTLSClientCertificate, mobiusTLSClientKey)
	if err != nil {
		log.Fatal().Err(err).Msg("load mobius tls client certificate")
	}
	mobiusAlternativeBrowserHost := os.Getenv("MOBIUS_DESKTOP_ALTERNATIVE_BROWSER_HOST")
	if mobiusClientCrt != nil {
		log.Info().Msg("Using TLS client certificate and key to authenticate to the server.")
	}
	tufUpdateRoot := os.Getenv("MOBIUS_DESKTOP_TUF_UPDATE_ROOT")
	if tufUpdateRoot != "" {
	...
	if runtime.GOOS == "linux" {
		// Ensure only one instance of Mobius Desktop is running at a time
	...
	onReady := func() {
		...
		systray.SetTooltip("Mobius Desktop")
		...
		versionItem := systray.AddMenuItem(fmt.Sprintf("Mobius Desktop v%s", version), "")
		versionItem.Disable()
		systray.AddSeparator()

		migrateMDMItem := systray.AddMenuItem("Migrate to Mobius", "")
		...
		transparencyItem := systray.AddMenuItem("About Mobius", "")
		...
		if os.Getenv("MOBIUS_DESKTOP_INSECURE") != "" {
			insecureSkipVerify = true
		}
		rootCA := os.Getenv("MOBIUS_DESKTOP_MOBIUS_ROOT_CA")
		...
		reportError := func(err error, info map[string]any) {
			if !client.GetServerCapabilities().Has(mobius.CapabilityErrorReporting) {
				log.Info().Msg("skipped reporting error to the server as it doesn't have the capability enabled")
				return
			}

			mobiusdErr := mobius.MobiusdError{
				ErrorSource:         "mobius-desktop",
			...
			if err := client.ReportError(tokenReader.GetCached(), mobiusdErr); err != nil {
				log.Error().Err(err).EmbedObject(mobiusdErr).Msg("reporting error to Mobius server")
			}
		}

		if runtime.GOOS == "darwin" {
			m, s, o, err := mdmMigrationSetup(offlineWatcherCtx, tufUpdateRoot, mobiusURL, client, &tokenReader)
			if err != nil {
		...
		// soon as the device auth token is registered by Mobius.
		...
		go func() {
			...
			for {
				...
				if runtime.GOOS == "darwin" && shouldRunMigrator && mdmMigrator.CanRun() {
					...
					// dialog if the client has already migrated but the Mobius server
					// doesn't know about this state yet.
					enrolledIntoMobius, err := mobiushttp.HostnamesMatch(enrollURL, mobiusURL)
					if err != nil {
						log.Error().Err(err).Msg("comparing MDM server URLs")
						continue
					}
					if !enrolledIntoMobius {
						...
						forceModeEnabled := sum.Notifications.NeedsMDMMigration &&
							sum.Config.MDM.MacOSMigration.Mode == mobius.MacOSMigrationModeForced
...
func refreshMenuItems(sum mobius.DesktopSummary, selfServiceItem *systray.MenuItem, myDeviceItem *systray.MenuItem) {
	// Check for null for backward compatibility with an old Mobius server
...
// getLockfile checks for the mobius desktop lock file, and returns an error if it can't secure it.
func getLockfile() (*flock.Flock, error) {
	...
	dir = filepath.Join(dir, "Mobius")

	lockFilePath := filepath.Join(dir, "mobius-desktop.lock")
	log.Debug().Msgf("acquiring mobius desktop lockfile: %s", lockFilePath)
	...
	if !locked {
		return nil, errors.New("another instance of mobius desktop has the lock")
	}
...
func setupLogs() {
	...
	logFile := &lumberjack.Logger{
		Filename:   filepath.Join(dir, "mobius-desktop.log"),
...
func setupStderr() {
	...
	stderrFile, err := os.OpenFile(filepath.Join(dir, "Mobius", "mobius-desktop.err"), os.O_WRONLY|os.O_CREATE|os.O_TRUNC, 0o666)
	if err != nil {
...
func mdmMigrationSetup(ctx context.Context, tufUpdateRoot, mobiusURL string, client *service.DeviceClient, tokenReader *token.Reader) (useraction.MDMMigrator, chan struct{}, useraction.MDMOfflineWatcher, error) {
