...
func main() {
	...
	app.Flags = []cli.Flag{
		...
		&cli.StringFlag{
			...
			EnvVars: []string{"ORBIT_MOBIUS_DESKTOP_ALTERNATIVE_BROWSER_HOST"},
		},
		&cli.StringFlag{
			Name:    "update-url",
		...
		&cli.StringFlag{
			...
			Usage:   "Update channel of Mobius Desktop to use",
			Value:   "stable",
			EnvVars: []string{"ORBIT_DESKTOP_CHANNEL"},
		},
		&cli.DurationFlag{
			Name:    "update-interval",
			Usage:   "How often to check for updates. Note: mobiusdaemon checks for updates at startup. The next update check adds some randomization and may take up to 10 minutes longer",
		...
		&cli.BoolFlag{
			Name:    "mobius-desktop",
			Usage:   "Launch Mobius Desktop application (flag currently only used on darwin)",
			EnvVars: []string{"ORBIT_MOBIUS_DESKTOP"},
		},
		...
		&cli.BoolFlag{
			...
			EnvVars: []string{"ORBIT_MOBIUS_DISABLE_KICKSTART_SOFTWAREUPDATED"},
			Hidden:  true,
		},
		&cli.BoolFlag{
		...
		&cli.StringFlag{
			...
			Usage:   "Sets the email address of the user associated with the host when enrolling to Mobius. (requires Mobius >= v4.43.0)",
	...
	app.Action = func(c *cli.Context) error {
		...
		// Override flags with values retrieved from Mobius.
		...
		if c.Bool("insecure") && c.String("mobius-certificate") != "" {
			return errors.New("insecure and mobius-certificate may not be specified together")
		}
		...
		if email := c.String("end-user-email"); email != "" && email != unusedFlagKeyword && !mobius.IsLooseEmail(email) {
		...
		if runtime.GOOS == "darwin" && c.Bool("use-system-configuration") {
			log.Info().Msg("trying to read mobius-url and enroll-secret from a configuration profile")
			for {
				config, err := profiles.GetMobiusdConfig()
				switch {
				...
				case config.EnrollSecret == "" || config.MobiusURL == "":
					log.Debug().Msg("enroll secret or mobius url are empty in configuration profile, not setting either")
				default:
					log.Info().Msg("setting enroll-secret and mobius-url configs from configuration profile")
					...
					if err := c.Set("mobius-url", config.MobiusURL); err != nil {
						return fmt.Errorf("set mobius URL from configuration profile: %w", err)
					}
					if err := writeSecret(config.EnrollSecret, c.String("root-dir")); err != nil {
						return fmt.Errorf("write enroll secret: %w", err)
					}
					if err := writeMobiusURL(config.MobiusURL, c.String("root-dir")); err != nil {
						return fmt.Errorf("write mobius URL: %w", err)
					}
				}

				if c.String("mobius-url") != "" && c.String("enroll-secret") != "" {
				...
				// First, get Mobius URL
				b, err := os.ReadFile(path.Join(c.String("root-dir"), constant.MobiusURLFileName))
				if err != nil {
					if !errors.Is(err, os.ErrNotExist) {
						return fmt.Errorf("read mobius URL file: %w", err)
					}
				} else {
					mobiusURL := strings.TrimSpace(string(b))
					if err = c.Set("mobius-url", mobiusURL); err != nil {
						return fmt.Errorf("set mobius URL from file: %w", err)
					}
				}
		...
		if updateURL := c.String("update-url"); updateURL != update.OldMobiusTUFURL && updateURL != update.DefaultURL {
		...
		if opt.ServerURL == update.OldMobiusTUFURL {
			//
			// This only gets executed on orbit 1.38.0+
			// when it is configured to connect to the old TUF server
			// (mobiusdaemon instances packaged before the migration,
			// built by mobiuscli previous to v4.63.0).
			//
		...
		if c.Bool("disable-kickstart-softwareupdated") && runtime.GOOS == "darwin" {
			log.Warn().Msg("mobiusdaemon no longer automatically kickstarts softwareupdated. The --disable-kickstart-softwareupdated flag, which was previously used to disable this behavior, has been deprecated and will be removed in a future version")
		}
		...
		if !c.Bool("disable-updates") || c.Bool("dev-mode") {
			...
			err = retrypkg.Do(func() error {
				var err error
				osquerydPath, desktopPath, err = getMobiusdComponentPaths(c, updater, fallbackServerOverridesCfg)
				if err != nil {
					return err
				}
				return nil
		...
		if runtime.GOOS == "darwin" {
			...
			if oi.HardwareUUID != orbitHostInfo.HardwareUUID {
				...
				// Removing the osquery DB should trigger a re-enrollment when mobiusdaemon is restarted.
				...
				if err := os.RemoveAll(filepath.Join(c.String("root-dir"), constant.DesktopTokenFileName)); err != nil {
					return fmt.Errorf("removing old Mobius Desktop identifier file: %w", err)
				}
		...
		mobiusURL := c.String("mobius-url")
		if !strings.HasPrefix(mobiusURL, "http") {
			mobiusURL = "https://" + mobiusURL
		}
		...
		var certPath string
		if mobiusURL != "https://" && c.Bool("insecure") {
			proxy, err := insecure.NewTLSProxy(mobiusURL)
			if err != nil {
			...
			addSubsystem(&g, "insecure proxy", &wrapSubsystem{
				execute: func() error {
					log.Info().
						...
						Str("target", c.String("mobius-url")).
			...
			certPath = filepath.Join(proxyDirectory, "mobius.crt")
			...
			if err := certificate.ValidateConnection(pool, mobiusURL); err != nil {
				log.Info().Err(err).Msg("Failed to connect to Mobius server. Osquery connection may fail.")
			}

			options = append(options,
				osquery.WithFlags(osquery.MobiusFlags(parsedURL)),
				osquery.WithFlags([]string{"--tls_server_certs", certPath}),
			)
		} else if mobiusURL != "https://" {
			if enrollSecret == "" {
				return errors.New("enroll secret must be specified to connect to Mobius server")
			}

			parsedURL, err := url.Parse(mobiusURL)
			if err != nil {
			...
			if certPath = c.String("mobius-certificate"); certPath != "" {
				// Check and log if there are any errors with TLS connection.
				pool, err := certificate.LoadPEM(certPath)
				if err != nil {
					return fmt.Errorf("load certificate: %w", err)
		...
		mobiusClientCertPath := filepath.Join(c.String("root-dir"), constant.MobiusTLSClientCertificateFileName)
		mobiusClientKeyPath := filepath.Join(c.String("root-dir"), constant.MobiusTLSClientKeyFileName)
		mobiusClientCrt, err := certificate.LoadClientCertificateFromFiles(mobiusClientCertPath, mobiusClientKeyPath)
		if err != nil {
			return fmt.Errorf("error loading mobius client certificate: %w", err)
		}

		var mobiusClientCertificate *tls.Certificate
		if mobiusClientCrt != nil {
			log.Info().Msg("Found TLS client certificate and key. Using them to authenticate to Mobius.")
			mobiusClientCertificate = &mobiusClientCrt.Crt
			options = append(options, osquery.WithFlags([]string{
				"--tls_client_cert", mobiusClientCertPath,
				"--tls_client_key", mobiusClientKeyPath,
			}))
		}
		...
		orbitClient, err := service.NewOrbitClient(
			c.String("root-dir"),
			mobiusURL,
			c.String("mobius-certificate"),
			c.Bool("insecure"),
			enrollSecret,
			mobiusClientCertificate,
			orbitHostInfo,
		...
		case "darwin":
			orbitClient.RegisterConfigReceiver(update.ApplyRenewEnrollmentProfileConfigFetcherMiddleware(
				orbitClient, renewEnrollmentProfileCommandFrequency, mobiusURL))
		...
		// for extensions autoupdate, we can only proceed after orbit is enrolled in mobius
		...
		if !c.Bool("disable-updates") || c.Bool("dev-mode") {
			...
			// call UpdateAction on the updateRunner after we have fetched extensions from Mobius
		...
		// Run a early check of mobiusdaemon configuration to check if orbit needs to
		...
		if c.Bool("mobius-desktop") {
			...
			deviceClient, err = service.NewDeviceClient(
				...
				c.String("mobius-certificate"),
				mobiusClientCertificate,
				c.String("mobius-desktop-alternative-browser-host"),
			)
			if err != nil {
		...
		if c.Bool("mobius-desktop") {
			...
			if err == nil && desktopVersion != "" {
				log.Info().Msgf("Found mobius-desktop version: %s", desktopVersion)
			} else {
		...
		if c.Bool("mobius-desktop") {
			...
			if mobiusClientCrt != nil {
				rawClientCrt = mobiusClientCrt.RawCrt
				rawClientKey = mobiusClientCrt.RawKey
			}
			...
			go func() {
				for {
					...
					log.Error().Err(errors.New(msg)).Msg("mobius-desktop runner error")
					// Vital errors are always sent to Mobius, regardless of the error reporting setting MOBIUS_ENABLE_POST_CLIENT_DEBUG_ERRORS.
					...
					if err = deviceClient.ReportError(trw.GetCached(), mobiusdErr); err != nil {
						log.Error().Err(err).Msg(fmt.Sprintf("failed to send error report to Mobius: %s", msg))
					}
				}
			}()
		...
		if (runtime.GOOS == "windows" || runtime.GOOS == "linux") && endUserEmail != "" && endUserEmail != unusedFlagKeyword {
			if orbitClient.GetServerCapabilities().Has(mobius.CapabilityEndUserEmail) {
				log.Debug().Msg("sending end-user email to Mobius")
				if err := orbitClient.SetOrUpdateDeviceMappingEmail(endUserEmail); err != nil {
					log.Error().Err(err).Msg("error sending end-user email to Mobius")
				}
			} else {
				log.Info().Msg("an end-user email is provided, but the Mobius server doesn't have the capability to set it.")
			}
		}
		...
		if runtime.GOOS == "darwin" {
			...
			if orbitClient.GetServerCapabilities().Has(mobius.CapabilityEscrowBuddy) {
...
// setServerOverrides overrides specific variables in c with values fetched from Mobius.
...
// getMobiusdComponentPaths returns the paths of the mobiusdaemon components.
...
func getMobiusdComponentPaths(
	...
	if updater.SignaturesExpired() || updater.LookupsFail() {
		...
		// Attempt to get local path of Mobius Desktop.
		if c.Bool("mobius-desktop") {
			if runtime.GOOS == "darwin" {
				desktopPath, err = updater.DirLocalPath(constant.DesktopTUFTargetName)
			} else {
	...
	// Mobius Desktop
	if c.Bool("mobius-desktop") {
		mobiusDesktopLocalTarget, err := updater.Get(constant.DesktopTUFTargetName)
		if err != nil {
			if fallbackCfg.DesktopPath == "" {
			...
			if runtime.GOOS == "darwin" {
				desktopPath = mobiusDesktopLocalTarget.DirPath
			} else {
				desktopPath = mobiusDesktopLocalTarget.ExecPath
			}
		}
	}
...
// desktopRunner runs the Mobius Desktop application.
type desktopRunner struct {
	// desktopPath is the path to the desktop executable.
	desktopPath string
	updateRoot  string
	// mobiusURL is the URL of the Mobius server.
	mobiusURL string
	// trw is the Mobius Desktop token reader and writer (implements token rotation).
	trw *token.ReadWriter
	// mobiusRootCA is the path to a certificate to use for server TLS verification.
	mobiusRootCA string
	// insecure disables all TLS verification.
	insecure bool
	// mobiusClientCrt is the raw TLS client certificate (in PEM format)
	// to use for authenticating to the Mobius server.
	mobiusClientCrt []byte
	// mobiusClientKey is the raw TLS client private key (in PEM format)
	// to use for authenticating to the Mobius server.
	mobiusClientKey []byte
	// mobiusAlternativeBrowserHost is an alternative host:port to use for
	// the browser URLs in Mobius Desktop.
...
func newDesktopRunner(
	desktopPath, mobiusURL, mobiusRootCA string,
	insecure bool,
	trw *token.ReadWriter,
	mobiusClientCrt []byte, mobiusClientKey []byte,
	mobiusAlternativeBrowserHost string,
	updateRoot string,
) *desktopRunner {
	return &desktopRunner{
		...
		mobiusURL:                    mobiusURL,
		trw:                         trw,
		mobiusRootCA:                 mobiusRootCA,
		insecure:                    insecure,
		mobiusClientCrt:              mobiusClientCrt,
		mobiusClientKey:              mobiusClientKey,
...
// execute makes sure the mobius-desktop application is running.
//
...
// Once mobius-desktop is started, the process is monitored (user processes get killed when the user
...
func (d *desktopRunner) Execute() error {
	...
	url, err := url.Parse(d.mobiusURL)
	if err != nil {
		return fmt.Errorf("invalid mobius-url: %w", err)
	}
	deviceURL, err := url.Parse(d.mobiusURL)
	if err != nil {
		return fmt.Errorf("invalid mobius-url: %w", err)
	}
	deviceURL.Path = path.Join(url.Path, "device", d.trw.GetCached())
	opts := []execuser.Option{
		execuser.WithEnv("MOBIUS_DESKTOP_MOBIUS_URL", url.String()),
		execuser.WithEnv("MOBIUS_DESKTOP_DEVICE_IDENTIFIER_PATH", d.trw.Path),

		// TODO(roperzh): this env var is keept only for backwards compatibility,
		// we should remove it once we think is safe
		execuser.WithEnv("MOBIUS_DESKTOP_DEVICE_URL", deviceURL.String()),

		execuser.WithEnv("MOBIUS_DESKTOP_MOBIUS_TLS_CLIENT_CERTIFICATE", string(d.mobiusClientCrt)),
		execuser.WithEnv("MOBIUS_DESKTOP_MOBIUS_TLS_CLIENT_KEY", string(d.mobiusClientKey)),

		execuser.WithEnv("MOBIUS_DESKTOP_ALTERNATIVE_BROWSER_HOST", d.mobiusAlternativeBrowserHost),
		execuser.WithEnv("MOBIUS_DESKTOP_TUF_UPDATE_ROOT", d.updateRoot),
	}
	if d.mobiusRootCA != "" {
		opts = append(opts, execuser.WithEnv("MOBIUS_DESKTOP_MOBIUS_ROOT_CA", d.mobiusRootCA))
	}
	if d.insecure {
		opts = append(opts, execuser.WithEnv("MOBIUS_DESKTOP_INSECURE", "1"))
	}
	...
	for {
		...
		if done := retry(30*time.Second, false, d.interruptCh, func() bool {
			...
			if loggedInUser == nil {
				log.Debug().Msg("No GUI user found, skipping mobius-desktop start")
				return true
			}
			log.Debug().Msg(fmt.Sprintf("Found GUI user: %v, attempting mobius-desktop start", loggedInUser))

			if *loggedInUser != "" {
				opts = append(opts, execuser.WithUser(*loggedInUser))
			}

			log.Info().Msg("killing any pre-existing mobius-desktop instances")
		...
		// Second retry logic to monitor mobius-desktop.
...
func (d *desktopRunner) processLog(log string) {
	...
	case strings.Contains(log, string(logErrorMissingExecSubstr)):
		// For manual testing.
		// To get this message, delete Mobius Desktop.app directory, make an empty Mobius Desktop.app directory,
		// and kill the mobius-desktop process. Orbit will try to re-start Mobius Desktop and log this message.
...
func (f *capabilitiesChecker) Execute() error {
	...
	for {
		...
		case <-capabilitiesCheckTicker.C:
			...
			if oldCapabilities.Has(mobius.CapabilityOrbitEndpoints) !=
				newCapabilities.Has(mobius.CapabilityOrbitEndpoints) {
				log.Info().Msgf("%s capability changed, restarting", mobius.CapabilityOrbitEndpoints)
				return nil
			}
			if oldCapabilities.Has(mobius.CapabilityTokenRotation) !=
				newCapabilities.Has(mobius.CapabilityTokenRotation) {
				log.Info().Msgf("%s capability changed, restarting", mobius.CapabilityTokenRotation)
				return nil
			}
			if oldCapabilities.Has(mobius.CapabilityEndUserEmail) !=
				newCapabilities.Has(mobius.CapabilityEndUserEmail) {
				log.Info().Msgf("%s capability changed, restarting", mobius.CapabilityEndUserEmail)
				return nil
			}
		case <-f.interruptCh:
			return nil
...
// writeMobiusURL writes the Mobius URL to the designated file. This is needed in case the
// Mobius URL originally came from a config profile, which was subsequently removed.
func writeMobiusURL(contents string, orbitRoot string) error {
	return writeOrbitFile(contents, orbitRoot, constant.MobiusURLFileName)
}

// serverOverridesRunner is a oklog.Group runner that polls for configuration overrides from Mobius.
...
// newServerOverridesReveiver creates a runner for updating server overrides configuration with values fetched from Mobius.
...
func (r *serverOverridesRunner) Run(orbitCfg *mobius.OrbitConfig) error {
...
func cfgsDiffer(overrideCfg *serverOverridesConfig, orbitCfg *mobius.OrbitConfig, desktopEnabled bool) bool {
	localUpdateChannelsCfg := &mobius.OrbitUpdateChannels{
	...
	setStableAsDefault := func(cfg *mobius.OrbitUpdateChannels) {
		if cfg.Orbit == "" {
			cfg.Orbit = "stable"
		}
		if cfg.Osqueryd == "" {
...
type serverOverridesConfig struct {
	...
	// DesktopChannel defines the override for the Mobius Desktop's channel.
...
// mobiusdaemon up and running with the last known good configuration.
//
...
type fallbackServerOverridesConfig struct {
	...
	// DesktopPath contains the path of the Mobius Desktop executable last known to be valid.
...
// updateServerOverrides updates the server override local file with the configuration fetched from Mobius.
func (r *serverOverridesRunner) updateServerOverrides(remoteCfg *mobius.OrbitConfig) error {
