...
import (
	...
	pkgscripts "github.com/notawar/mobius set/v4/pkg/scripts"
...
type Client interface {
	GetInstallerDetails(installID string) (*mobius.SoftwareInstallDetails, error)
	...
	SaveInstallerResult(payload *mobius.HostSoftwareInstallResultPayload) error
}
...
func (r *Runner) Run(config *mobius.OrbitConfig) error {
...
func (r *Runner) run(ctx context.Context, config *mobius.OrbitConfig) error {
...
func (r *Runner) installSoftware(ctx context.Context, installID string, logger zerolog.Logger) (*mobius.HostSoftwareInstallResultPayload, error) {
	...
	if !r.scriptsEnabled() {
		// Mobius knows that -2 means script was disabled on host
		logger.Info().Msg("scripts are disabled for this host, stopping installation")
		payload.InstallScriptExitCode = ptr.Int(mobius.ExitCodeScriptsDisabled)
	...
	if installer.SoftwareInstallerURL != nil && installer.SoftwareInstallerURL.URL != "" {
		...
		if err != nil {
			...
			// If download fails, we will fall back to downloading the installer directly from Mobius server
			installerPath = ""
		}
	}

	if installerPath == "" {
		logger.Info().Msg("about to download software installer from Mobius")
		...
		if err != nil {
			...
			// Set a special exit code to indicate that the installer download failed, so that Mobius
			// will mark this installation as failed.
			payload.InstallScriptExitCode = ptr.Int(mobius.ExitCodeInstallerDownloadFailed)
	...
	if installer.PostInstallScript != "" {
		...
		if postErr != nil || postExitCode != 0 {
			...
			if uninstallScript == "" {
				// The Mobius server is < v4.57.0, so we need to use the old method.
