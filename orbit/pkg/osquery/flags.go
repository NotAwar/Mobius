...
// MobiusFlags is the set of flags to pass to osquery when connecting to Mobius.
func MobiusFlags(mobiusURL *url.URL) []string {
	hostname, prefix := mobiusURL.Host, mobiusURL.Path
	return []string{
		...
		// a client connected to Mobius. Users can always override this in the
		// config they serve via Mobius.
		"--config_refresh=60",
