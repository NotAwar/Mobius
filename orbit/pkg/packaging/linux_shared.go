...
func buildNFPM(opt Options, pkger nfpm.Packager) (string, error) {
	...
	info := &nfpm.Info{
		Name:        "mobius-osquery",
		Version:     opt.Version,
		Description: "Mobius osquery -- runtime and autoupdater",
...
{{ if .Desktop }}
ORBIT_MOBIUS_DESKTOP=true
ORBIT_DESKTOP_CHANNEL={{ .DesktopChannel }}
{{ if .MobiusDesktopAlternativeBrowserHost }}
ORBIT_MOBIUS_DESKTOP_ALTERNATIVE_BROWSER_HOST={{ .MobiusDesktopAlternativeBrowserHost }}
{{ end }}
{{ end }}
{{ if .Insecure }}ORBIT_INSECURE=true{{ end }}
{{ if .DisableUpdates }}ORBIT_DISABLE_UPDATES=true{{ end }}
{{ if .MobiusURL }}ORBIT_MOBIUS_URL={{.MobiusURL}}{{ end }}
{{ if .MobiusCertificate }}ORBIT_MOBIUS_CERTIFICATE=/opt/orbit/mobius.pem{{ end }}
...
func writePreRemove(opt Options, path string) error {
	...
	//
	// "pkill mobius-desktop" is required because the application
