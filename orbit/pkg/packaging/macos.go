...
func BuildPkg(opt Options) (string, error) {
	...
	if opt.MobiusCertificate != "" {
		if err := writeMobiusServerCertificate(opt, orbitRoot); err != nil {
			return "", fmt.Errorf("write mobius server certificate: %w", err)
		}
	}

	if opt.MobiusTLSClientCertificate != "" {
		if err := writeMobiusClientCertificate(opt, orbitRoot); err != nil {
			return "", fmt.Errorf("write mobius client certificate: %w", err)
		}
	}
	...
	if opt.Notarize {
		switch {
		case isDarwin:
			if err := NotarizeStaple(generatedPath, "com.mobiusmdm.orbit"); err != nil {
				return "", err
			}
		case isLinuxNative:
...
func writeLaunchd(opt Options, rootPath string) error {
	...
	path := filepath.Join(rootPath, "Library", "LaunchDaemons", "com.mobiusmdm.orbit.plist")
	if err := secure.MkdirAll(filepath.Dir(path), constant.DefaultDirMode); err != nil {
		return fmt.Errorf("mkdir: %w", err)
	}
...
func writeMobiusServerCertificate(opt Options, orbitRoot string) error {
	dstPath := filepath.Join(orbitRoot, "mobius.pem")

	if err := file.Copy(opt.MobiusCertificate, dstPath, 0o644); err != nil {
		return fmt.Errorf("write mobius server certificate: %w", err)
	}
...
func writeMobiusClientCertificate(opt Options, orbitRoot string) error {
	dstPath := filepath.Join(orbitRoot, constant.MobiusTLSClientCertificateFileName)
	if err := file.Copy(opt.MobiusTLSClientCertificate, dstPath, constant.DefaultFileMode); err != nil {
		return fmt.Errorf("write mobius certificate file: %w", err)
	}
	dstPath = filepath.Join(orbitRoot, constant.MobiusTLSClientKeyFileName)
	if err := file.Copy(opt.MobiusTLSClientKey, dstPath, constant.DefaultFileMode); err != nil {
		return fmt.Errorf("write mobius key file: %w", err)
	}
	return nil
}
...
func xarBom(opt Options, rootPath string) error {
	...
	default:
		...
		cmdMkbom = exec.Command(
			"docker", "run", "--rm", "-v", rootPath+":/root", "mobiusmdm/bomutils",
			"mkbom", "-u", "0", "-g", "80",
	...
	default:
		cmdXar = exec.Command(
			"docker", "run", "--rm", "-v", rootPath+":/root", "-w", "/root/flat", "mobiusmdm/bomutils",
			"xar",
		)
