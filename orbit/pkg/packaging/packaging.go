...
type Options struct {
	// MobiusURL is the URL to the Mobius server.
	MobiusURL string
	// EnrollSecret is the enroll secret used to authenticate to the Mobius
	// server.
	EnrollSecret string
	// Version is the version number for this package.
	Version string
	// Identifier is the identifier (eg. com.mobiusmdm.orbit) for the package product.
	Identifier string
	...
	Notarize bool
	// MobiusCertificate is a file path to a Mobius server certificate to include in the package.
	MobiusCertificate string
	// MobiusTLSClientCertificate is a file path to a client certificate to use when
	// connecting to the Mobius server.
	//
	// If set, then MobiusTLSClientKey must be set too.
	MobiusTLSClientCertificate string
	// MobiusTLSClientKey is a file path to a client private key to use when
	// connecting to the Mobius server.
	//
	// If set, then MobiusTLSClientCertificate must be set too.
	MobiusTLSClientKey string
	// MobiusDesktopAlternativeBrowserHost is an alternative host:port to use for Mobius Desktop in the browser.
	//
	// This may be required when using TLS client authentication for connecting to Mobius via a proxy.
	// Otherwise users would need to configure client certificates on their browsers.
	//
	// If not set, then MobiusURL is used instead.
	MobiusDesktopAlternativeBrowserHost string
	...
	// DesktopChannel is the update channel to use for the Mobius Desktop application.
	...
	Debug bool
	// Desktop determines whether to package the Mobius Desktop application.
	Desktop bool
	...
	// UseSystemConfiguration tells mobiusdaemon to try to read MobiusURL and
	...
	// emulation to build an MSI mobiusdaemon installer
	LocalWixDir string
...
func InitializeUpdates(updateOpt update.Options) (*UpdatesData, error) {
	...
	//
	// Once https://tuf.mobiuscli.com is brought down (which means downgrades to 1.37.0 or
