...
func BuildMSI(opt Options) (string, error) {
	...
	// We are using native MSI build on macOS arm64, instead of Docker, because the current mobiusmdm/wix Docker image is unreliable on macOS arm64.
	// We are looking into creating a new Docker image for macOS arm64.
	if runtime.GOOS == "darwin" && runtime.GOARCH == "arm64" && absWixDir == "" {
		fmt.Println("Detected macOS arm64. mobiuscli must use locally installed wine and wix to build the MSI package.")
	...
	if opt.Architecture == ArchArm64 {
		filename = "mobius-osquery-arm64.msi"
	}
...
func checkWine(wineChecked bool) error {
	if !wineChecked && runtime.GOOS == "darwin" {
		...
		if err := cmd.Run(); err != nil {
			return fmt.Errorf(
				"%s failed. Is Wine installed? Creating a mobiusdaemon agent for Windows (.msi) requires Wine. To install Wine see the script here: https://mobiusmdm.com/install-wine %w",
				wix.WineCmd, err,
			)
		}
	}
...
func createVersionInfo(vParts []string, manifestPath string) (*goversioninfo.VersionInfo, error) {
	...
	version := strings.Join(vParts, ".")
	copyright := fmt.Sprintf("%d Mobius Device Management Inc.", time.Now().Year())
	...
	result := goversioninfo.VersionInfo{
		...
		StringFileInfo: goversioninfo.StringFileInfo{
			Comments:         "Mobius osquery",
			CompanyName:      "Mobius Device Management (mobiusmdm.com)",
			FileDescription:  "Mobius osquery installer",
			FileVersion:      version,
			InternalName:     "",
