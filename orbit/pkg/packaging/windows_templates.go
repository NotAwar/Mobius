...
<Wix xmlns="http://schemas.microsoft.com/wix/2006/wi" xmlns:util="http://schemas.microsoft.com/wix/UtilExtension">
  <Product
    ...
    Language="1033"
    Version="{{.Version}}"
    Manufacturer="Mobius Device Management (mobiusmdm.com)"
    UpgradeCode="B681CB20-107E-428A-9B14-2D3C1AFED244" >

    <Package
      Keywords='Mobius osquery'
      Description="Mobius osquery"
    ...
    <Property Id="APPLICATIONFOLDER">
      <RegistrySearch Key="SOFTWARE\MobiusDM\Orbit" Root="HKLM" Type="raw" Id="APPLICATIONFOLDER_REGSEARCH" Name="Path" />
    </Property>

    <Property Id="ARPNOREPAIR" Value="yes" Secure="yes" />
    <Property Id="ARPNOMODIFY" Value="yes" Secure="yes" />

    <Property Id="MOBIUS_URL" Value="{{ if .MobiusURL }}{{ .MobiusURL }}{{ end }}"/>
    <Property Id="MOBIUS_SECRET" Value="dummy"/>
    <Property Id="ENABLE_SCRIPTS" Value="{{ if .EnableScripts }}True{{ else }}False{{ end }}"/>
	<Property Id="MOBIUS_DESKTOP" Value="{{ if .Desktop }}True{{ else }}False{{ end }}"/>
	{{ $endUserEmailArg := "" }}
            ...
            <Directory Id="ORBITBINORBIT" Name="orbit">
              <Component Id="C_ORBITBIN" Guid="AF347B4E-B84B-4DD4-9C4D-133BE17B613D">
                ...
                <ServiceInstall
                  ...
                  Start="auto"
                  Type="ownProcess"
                  Description="This service runs Mobius's osquery runtime and autoupdater (Orbit)."
                  Arguments='--root-dir "[ORBITROOT]." --log-file "[System64Folder]config\systemprofile\AppData\Local\MobiusDM\Orbit\Logs\orbit-osquery.log" --mobius-url "[MOBIUS_URL]"{{ if .MobiusCertificate }} --mobius-certificate "[ORBITROOT]mobius.pem"{{ end }}{{ if .EnrollSecret }} --enroll-secret-path "[ORBITROOT]secret.txt"{{ end }}{{if .Insecure }} --insecure{{ end }}{{ if .Debug }} --debug{{ end }}{{ if .UpdateURL }} --update-url "{{ .UpdateURL }}"{{ end }}{{ if .UpdateTLSServerCertificate }} --update-tls-certificate "[ORBITROOT]update.pem"{{ end }}{{ if .DisableUpdates }} --disable-updates{{ end }} --mobius-desktop="[MOBIUS_DESKTOP]" --desktop-channel {{ .DesktopChannel }}{{ if .MobiusDesktopAlternativeBrowserHost }} --mobius-desktop-alternative-browser-host {{ .MobiusDesktopAlternativeBrowserHost }}{{ end }} --orbit-channel "{{ .OrbitChannel }}" --osqueryd-channel "{{ .OsquerydChannel }}" --enable-scripts="[ENABLE_SCRIPTS]" {{ if and (ne .HostIdentifier "") (ne .HostIdentifier "uuid") }}--host-identifier={{ .HostIdentifier }}{{ end }}{{ $endUserEmailArg }}{{ if .OsqueryDB }} --osquery-db="{{ .OsqueryDB }}"{{ end }}'
                >
                  <util:ServiceConfig
   ...
   <SetProperty Id="CA_UpdateSecret"
                 ...
                 Sequence="execute"
                 Value='&quot;[POWERSHELLEXE]&quot; -NoLogo -NonInteractive -NoProfile -ExecutionPolicy Bypass -File "[ORBITROOT]installer_utils.ps1" -updateSecret "[MOBIUS_SECRET]"' />
   ...
   <SetProperty Id="CA_RemoveRebootPending"
    ...
    <Feature Id="Orbit" Title="Mobius osquery" Level="1" Display="hidden">
...
<instrumentationManifest xsi:schemaLocation="http://schemas.microsoft.com/win/2004/08/events eventman.xsd" xmlns="http://schemas.microsoft.com/win/2004/08/events" xmlns:win="http://manifests.microsoft.com/win/2004/08/windows/events" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:trace="http://schemas.microsoft.com/win/2004/08/events/trace">
	<instrumentation>
		<events>
			<provider name="MobiusDM" guid="{F7740E18-3259-434F-9759-976319968900}" symbol="OsqueryWindowsEventLogProvider" resourceFileName="%systemdrive%\Program Files\Orbit\bin\osqueryd\windows\{{ .OsquerydChannel }}\osqueryd.exe" messageFileName="%systemdrive%\Program Files\Orbit\bin\osqueryd\windows\{{ .OsquerydChannel }}\osqueryd.exe">
				<events>
...
function Force-Remove-Orbit {
  ...
  try {
    ...
    Get-ChildItem "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall" -Recurse  -ErrorAction "SilentlyContinue" |  Where-Object {($_.ValueCount -gt 0)} | ForEach-Object {
      ...
      $properties = Get-ItemProperty -LiteralPath $_.PSPath  -ErrorAction "SilentlyContinue" |  Where-Object {($_.DisplayName -eq "Mobius osquery")}
      if ($properties) {
...
function Graceful-Product-Uninstall($productName) {
  ...
  try {
    ...
    if ($productName -eq "Mobius osquery") {
      Stop-Orbit
...
function Main {
  ...
  try {
    ...
    if ($uninstallOsquery) {
      ...
      #if (Graceful-Product-Uninstall("Mobius osquery")) {
      if ($false) {
