...
func TestCreateVersionInfo(t *testing.T) {
	...
	t.Run("creates a VersionInfo struct", func(t *testing.T) {
		...
		require.Equal(t, result.StringFileInfo.Comments, "Mobius osquery")
		require.Equal(t, result.StringFileInfo.CompanyName, "Mobius Device Management (mobiusmdm.com)")
		require.Equal(t, result.StringFileInfo.FileDescription, "Mobius osquery installer")
		require.Equal(t, result.StringFileInfo.FileVersion, "1.2.3.0")
		require.Equal(t, result.StringFileInfo.LegalCopyright, fmt.Sprintf("%d Mobius Device Management Inc.", time.Now().Year()))
		require.Equal(t, result.StringFileInfo.ProductName, "Mobius osquery")
