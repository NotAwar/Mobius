...
const (
	...
	imageName          = "mobiusmdm/wix:latest"
...
func darwinWineExecutable() (string, error) {
	...
	if !found {
		return "", fmt.Errorf("Unknown wine version: %q. Is Wine installed? Creating a mobiusdaemon agent for Windows (.msi) requires Wine. To install Wine see the script here: https://mobiusmdm.com/install-wine ", string(cmdOut))
	}
	wineVersion, err := strconv.ParseInt(strings.Split(wineVerStr, ".")[0], 10, 64)
	if err != nil {
		return "", fmt.Errorf("Unable to parse wine version: %q. Is Wine installed? Creating a mobiusdaemon agent for Windows (.msi) requires Wine. To install Wine see the script here: https://mobiusmdm.com/install-wine ", wineVerStr)
	}
	if wineVersion < 10 {
