...
// GetMobiusdConfig searches and parses a device level configuration profile
// with Mobius's payload identifier.
func GetMobiusdConfig() (*mobius.MDMAppleMobiusdConfig, error) {
	pc, err := getProfilePayloadContent[mobius.MDMAppleMobiusdConfig](mobileconfig.MobiusdConfigPayloadIdentifier)
	if err != nil {
		if err == ErrNotFound {
			return &mobius.MDMAppleMobiusdConfig{}, nil
		}
...
func GetCustomEnrollmentProfileEndUserEmail() (string, error) {
	pc, err := getProfilePayloadContent[mobius.MDMCustomEnrollmentProfileItem](mobileconfig.MobiusEnrollmentPayloadIdentifier)
	if err != nil {
		return "", err
	}
