...
func TestGetMobiusdConfig(t *testing.T) {
	testErr := errors.New("test error")
	cases := []struct {
		cmdOut  *string
		cmdErr  error
		wantOut *mobius.MDMAppleMobiusdConfig
		wantErr error
	}{
		{nil, testErr, nil, testErr},
		{ptr.String("invalid-xml"), nil, nil, io.EOF},
		{&emptyOutput, nil, &mobius.MDMAppleMobiusdConfig{}, nil},
		{&withMobiusdConfig, nil, &mobius.MDMAppleMobiusdConfig{EnrollSecret: "ENROLL_SECRET", MobiusURL: "https://test.example.com"}, nil},
	}
...
<dict>
	...
	<array>
		<dict>
			...
			<string>com.mobiusmdm.mobiusdaemon.config</string>
			...
			<array>
				<dict>
					...
					<string>com.mobiusmdm.mobiusdaemon.config</string>
					<key>PayloadType</key>
					<string>com.mobiusmdm.mobiusdaemon</string>
					<key>PayloadUUID</key>
	...
	withMobiusdConfigAndEnrollment = `
<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0">
<dict>
	...
	<array>
		...
		<dict>
			...
			<key>ProfileIdentifier</key>
			<string>com.mobiusmdm.mobius.mdm.apple</string>
			<key>ProfileInstallDate</key>
			<string>2023-02-27 18:55:07 +0000</string>
			<key>ProfileItems</key>
			<array>
...
func TestCustomInstallerWorkflow(t *testing.T) {
	...
	for _, c := range []struct {
		...
		{"happy path", withMobiusdConfigAndEnrollment, "user@example.com", nil},
		{"empty profiles", emptyOutput, "", ErrNotFound},
		{"no enrollment payload", withMobiusdConfig, "", ErrNotFound},
		{"wrong payload identifier", strings.Replace(withMobiusdConfigAndEnrollment, mobileconfig.MobiusEnrollmentPayloadIdentifier, "wrong-identifier", 1), "", ErrNotFound},
		{"no end user email key", strings.Replace(withMobiusdConfigAndEnrollment, "EndUserEmail", "WrongKey", 1), "", ErrNotFound},
	} {
			...
			ptr.String(`Device Enrollment configuration:
{
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			err := CheckAssignedEnrollmentProfile(mobiusURL)
			if c.wantErr != nil {
...
func TestGetProfilePayloadContent(t *testing.T) {
	...
	execProfileCmd = func() (*bytes.Buffer, error) {
		var buf bytes.Buffer
		buf.WriteString(withMobiusdConfigAndEnrollment)
		return &buf, nil
	}

	// mismatched int type is not acceptable
	_, err := getProfilePayloadContent[int]("com.mobiusmdm.mobius.mdm.apple.mdm")
	...
	_, err = getProfilePayloadContent[string]("com.mobiusmdm.mobius.mdm.apple.mdm")
	...
	_, err = getProfilePayloadContent[bool]("com.mobiusmdm.mobius.mdm.apple.mdm")
	...
	_, err = getProfilePayloadContent[[]byte]("com.mobiusmdm.mobius.mdm.apple.mdm")
	...
	ws, err := getProfilePayloadContent[wrongStruct]("com.mobiusmdm.mobius.mdm.apple.mdm")
	require.NoError(t, err)
	require.NotNil(t, ws)
	...
	c, err := getProfilePayloadContent[mobius.MDMAppleMobiusdConfig]("com.mobiusmdm.mobiusdaemon.config")
	require.NoError(t, err)
	require.NotNil(t, c)
	require.Equal(t, *c, mobius.MDMAppleMobiusdConfig{EnrollSecret: "ENROLL_SECRET", MobiusURL: "https://test.example.com"})

	// struct type is acceptable and returns the expected value type corresponds to the payload identifier
	e, err := getProfilePayloadContent[mobius.MDMCustomEnrollmentProfileItem]("com.mobiusmdm.mobius.mdm.apple.mdm")
	require.NoError(t, err)
	require.NotNil(t, e)
	require.Equal(t, *e, mobius.MDMCustomEnrollmentProfileItem{EndUserEmail: "user@example.com"})

	// map type is acceptable
	m, err := getProfilePayloadContent[map[string]any]("com.mobiusmdm.mobius.mdm.apple.mdm")
	require.NoError(t, err)
	require.NotNil(t, m)
	gotMap := *m
	...
	require.False(t, ok)
	_, ok = gotMap["MobiusURL"]
	require.False(t, ok)

	// map type is acceptable
	m2, err := getProfilePayloadContent[map[string]any]("com.mobiusmdm.mobiusdaemon.config")
	require.NoError(t, err)
	require.NotNil(t, m)
	gotMap = *m2
