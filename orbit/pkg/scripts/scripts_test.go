...
func TestRunner(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			client:    &mockClient{scripts: map[string]*mobius.HostScriptResult{"a": {}}},
			execer:    &mockExecCmd{},
			enabled:   true,
			execIDs:   []string{"a"},
			execCalls: 1,
		...
		},
		{
			...
			client:    &mockClient{scripts: map[string]*mobius.HostScriptResult{"a": {}, "b": {}, "c": {}}},
			execer:    &mockExecCmd{},
			enabled:   true,
		...
		},
		{
			...
			client:      &mockClient{getErr: io.ErrUnexpectedEOF, scripts: map[string]*mobius.HostScriptResult{"a": {}}},
			execer:      &mockExecCmd{},
			enabled:     true,
			execIDs:     []string{"a"},
			execCalls:   0,
		...
		},
		{
			...
			client:      &mockClient{saveErr: io.ErrUnexpectedEOF, scripts: map[string]*mobius.HostScriptResult{"a": {}}},
			execer:      &mockExecCmd{},
			enabled:     true,
			execIDs:     []string{"a"},
			execCalls:   1,
		...
		},
		{
			...
			client:      &mockClient{scripts: map[string]*mobius.HostScriptResult{"a": {ExitCode: ptr.Int64(0)}}},
			execer:      &mockExecCmd{},
			enabled:     true,
			execIDs:     []string{"a"},
			execCalls:   0,
		...
		},
		{
			...
			client:      &mockClient{getErr: errFailOnce, scripts: map[string]*mobius.HostScriptResult{"a": {ExitCode: ptr.Int64(0)}}},
			execer:      &mockExecCmd{},
			enabled:     true,
			execIDs:     []string{"a", "b"},
			execCalls:   0,
		...
		},
		{
			...
			client:      &mockClient{scripts: map[string]*mobius.HostScriptResult{"a": {ExitCode: ptr.Int64(0)}, "b": {}, "c": {}}, erroredScripts: map[string]error{"b": errFailOnce}},
			execer:      &mockExecCmd{},
			enabled:     true,
			execIDs:     []string{"a", "b", "c"},
			execCalls:   0,
...
func TestRunnerTempDir(t *testing.T) {
	t.Run("deletes temp dir", func(t *testing.T) {
		...
		client := &mockClient{scripts: map[string]*mobius.HostScriptResult{"a": {ScriptContents: "echo 'Hi'", ExecutionID: "a"}}}
	...
	t.Run("remove fails, returns original error", func(t *testing.T) {
		...
		client := &mockClient{saveErr: io.ErrUnexpectedEOF, scripts: map[string]*mobius.HostScriptResult{"a": {ScriptContents: "echo 'Hi'"}}}
		execer := &mockExecCmd{output: []byte("output"), exitCode: 0, err: nil}
	...
	t.Run("remove fails, returns this error if the rest succeeded", func(t *testing.T) {
		...
		client := &mockClient{scripts: map[string]*mobius.HostScriptResult{"a": {ScriptContents: "echo 'Hi'"}}}
		execer := &mockExecCmd{output: []byte("output"), exitCode: 0, err: nil}
	...
	t.Run("keeps temp dir", func(t *testing.T) {
		tempDir := t.TempDir()
		t.Setenv("MOBIUS_PREVENT_SCRIPT_TEMPDIR_DELETION", "1")
		...
		require.Contains(t, entries[0].Name(), "mobius-a-")
...
type mockClient struct {
	scripts        map[string]*mobius.HostScriptResult
	results        map[string]*mobius.HostScriptResultPayload
	getErr         error
	saveErr        error
	erroredScripts map[string]error
}

func (m *mockClient) GetHostScript(execID string) (*mobius.HostScriptResult, error) {
	if m.getErr != nil {
		err := m.getErr
		if err == errFailOnce {
			m.getErr = nil
...
func (m *mockClient) SaveHostScriptResult(result *mobius.HostScriptResultPayload) error {
	if m.results == nil {
		m.results = make(map[string]*mobius.HostScriptResultPayload)
	}
