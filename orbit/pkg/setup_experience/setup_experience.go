...
// Client is the minimal interface needed to communicate with the Mobius server.
type Client interface {
	GetSetupExperienceStatus() (*mobius.SetupExperienceStatusPayload, error)
}

// SetupExperiencer is the type that manages the Mobius setup experience flow during macOS Setup
...
func (s *SetupExperiencer) Run(oc *mobius.OrbitConfig) error {
	...
	if orgLogo == "" {
		orgLogo = "https://mobiusmdm.com/images/permanent/mobius-mark-color-40x40@4x.png"
	}
	...
	if payload.BootstrapPackage != nil {
		if payload.BootstrapPackage.Status != mobius.MDMBootstrapPackageFailed && payload.BootstrapPackage.Status != mobius.MDMBootstrapPackageInstalled {
	...
	if payload.AccountConfiguration != nil {
		if payload.AccountConfiguration.Status != mobius.MDMAppleStatusAcknowledged &&
			payload.AccountConfiguration.Status != mobius.MDMAppleStatusError &&
			payload.AccountConfiguration.Status != mobius.MDMAppleStatusCommandFormatError {
	...
	if len(payload.Software) > 0 || payload.Script != nil {
		...
		var stepsDone int
		var prog uint
		var steps []*mobius.SetupExperienceStatusResult
		...
		for _, step := range steps {
			...
			if step.Status == mobius.SetupExperienceStatusFailure || step.Status == mobius.SetupExperienceStatusSuccess {
				stepsDone++
...
func anyProfilePending(profiles []*mobius.SetupExperienceConfigurationProfileResult) (bool, string) {
	for _, p := range profiles {
		if p.Status == mobius.MDMDeliveryPending {
			return true, p.Name
		}
	}
...
func (s *SetupExperiencer) startSwiftDialog(binaryPath, orgLogo string) error {
	...
	go func() {
		initOpts := &swiftdialog.SwiftDialogOptions{
			Title:            "none",
			Message:          "### Setting up your Mac...\n\nYour Mac is being configured by your organization using Mobius. This process may take some time to complete. Please don't attempt to restart or shut down the computer unless prompted to do so.",
			Icon:             orgLogo,
...
func resultToListItem(result *mobius.SetupExperienceStatusResult) swiftdialog.ListItem {
	statusText := "Pending"
	status := swiftdialog.StatusWait

	switch result.Status {
	case mobius.SetupExperienceStatusFailure:
		status = swiftdialog.StatusFail
		statusText = "Failed"
	case mobius.SetupExperienceStatusSuccess:
