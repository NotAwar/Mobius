...
func TablePlugin() *table.Plugin {
	...
	return table.NewPlugin("mobiusdaemon_logs", columns, generate)
}
...
func (l *logger) Write(event []byte) (int, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("mobius_logs.Write: %w", err)
	}
...
func (l *logger) WriteLevel(level zerolog.Level, event []byte) (int, error) {
	msgs, err := processLogEntry(event)
	if err != nil {
		return 0, fmt.Errorf("mobius_logs.WriteLevel: %w", err)
	}
