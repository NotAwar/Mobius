...
// FlagRunner is a specialized runner to periodically check and update flags from Mobius
...
// with FlagUpdateOptions to connect to Mobius
...
// DoFlagsUpdate checks for update of flags from Mobius
// It gets the flags from the Mobius server, and compares them to locally stored flagfile (if it exists)
// If the flag comparison from disk and server are not equal, it writes the flags to disk, and returns true
func (r *FlagRunner) Run(config *mobius.OrbitConfig) error {
	flagFileExists := true
	...
	osqueryFlagMapFromMobius, err := getFlagsFromJSON(config.Flags)
	if err != nil {
		return fmt.Errorf("error parsing flags: %w", err)
	}

	// compare both flags, if they are equal, nothing to do
	if flagFileExists && reflect.DeepEqual(osqueryFlagMapFromFile, osqueryFlagMapFromMobius) {
		return nil
	}

	// flags are not equal, write the mobius flags to disk
	err = writeFlagFile(r.opt.RootDir, osqueryFlagMapFromMobius)
	if err != nil {
...
// ExtensionRunner is a specialized runner to periodically check and update flags from Mobius
// It is designed with Execute and Interrupt functions to be compatible with oklog/run
//
// It uses an an OrbitConfigFetcher (which may be the OrbitClient with additional middleware), along
// with ExtensionUpdateOptions and updateRunner to connect to Mobius.
...
// DoExtensionConfigUpdate calls the /config API endpoint to grab extensions from Mobius
...
func (r *ExtensionRunner) Run(config *mobius.OrbitConfig) error {
	extensionAutoLoadFile := filepath.Join(r.opt.RootDir, "extensions.load")
	if len(config.Extensions) == 0 {
		// Extensions from Mobius is empty
	...
	var extensions mobius.Extensions
	err := json.Unmarshal(config.Extensions, &extensions)
	if err != nil {
		return fmt.Errorf("error unmarshing json extensions config from mobius: %w", err)
	}
