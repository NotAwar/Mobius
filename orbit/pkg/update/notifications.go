...
// OrbitConfigFetcher and detects if the mobius server sent a notification to
...
type renewEnrollmentProfileConfigReceiver struct {
	...
	// for tests, to be able to mock the function that checks for Mobius
	// enrollment
...
func ApplyRenewEnrollmentProfileConfigFetcherMiddleware(fetcher OrbitConfigFetcher, frequency time.Duration, mobiusURL string) mobius.OrbitConfigReceiver {
	return &renewEnrollmentProfileConfigReceiver{Frequency: frequency, mobiusURL: mobiusURL}
}

func (h *renewEnrollmentProfileConfigReceiver) Run(config *mobius.OrbitConfig) error {
	if config.Notifications.RenewEnrollmentProfile {
		if h.cmdMu.TryLock() {
			...
			if time.Since(h.lastRun) >= h.Frequency {
				...
				// we perform this check locally on the client too to avoid showing the
				// dialog if the Mobius enrollment profile has not been assigned to the device in
				// Apple Business Manager.
				assignedFn := h.checkAssignedEnrollmentProfileFn
				if assignedFn == nil {
					assignedFn = profiles.CheckAssignedEnrollmentProfile
				}
				if err := assignedFn(h.mobiusURL); err != nil {
...
// Run checks if the mobius server set the "needs windows {un}enrollment" flag
...
// enrolled in Mobius MDM.
func (w *windowsMDMEnrollmentConfigReceiver) Run(cfg *mobius.OrbitConfig) error {
	switch {
...
func (w *windowsMDMEnrollmentConfigReceiver) attemptEnrollment(notifs mobius.OrbitConfigNotifications) {
...
type runScriptsConfigReceiver struct {
	...
	// Mobius server so it knows the agent processed the request. Note that this
	...
	// for tests, if set will use this instead of profiles.GetMobiusdConfig.
	testGetMobiusdConfig func() (*mobius.MDMAppleMobiusdConfig, error)
...
) (mobius.OrbitConfigReceiver, func() bool) {
...
func (h *runScriptsConfigReceiver) runDynamicScriptsEnabledCheck() {
	getMobiusdConfig := h.testGetMobiusdConfig
	if getMobiusdConfig == nil {
		getMobiusdConfig = profiles.GetMobiusdConfig
	}

	// only run on macos and only if scripts are disabled by default for the
	// agent (but always run if a test get mobiusdaemon config function is set).
	if (runtime.GOOS == "darwin" && !h.ScriptsExecutionEnabled) || (h.testGetMobiusdConfig != nil) {
		go func() {
			runCheck := func() {
				cfg, err := getMobiusdConfig()
				if err != nil {
					if err != profiles.ErrNotImplemented {
						...
						log.Info().Err(err).Msg("get mobiusdaemon configuration failed")
					}
					return
				}
...
// GetConfig calls the wrapped Fetcher's GetConfig method, and if the mobius
// server sent a list of scripts to execute, starts a goroutine to execute
// them.
func (h *runScriptsConfigReceiver) Run(cfg *mobius.OrbitConfig) error {
	timeout := mobiusscripts.MaxHostExecutionTime
...
type DiskEncryptionKeySetter interface {
	SetOrUpdateDiskEncryptionKey(diskEncryptionStatus mobius.OrbitHostDiskEncryptionKeyPayload) error
}
...
func (w *windowsMDMBitlockerConfigReceiver) Run(cfg *mobius.OrbitConfig) error {
...
func (w *windowsMDMBitlockerConfigReceiver) attemptBitlockerEncryption(notifs mobius.OrbitConfigNotifications) {
	...
	if encryptionStatus != nil &&
		...
		if err := w.decryptVolume(targetVolume); err != nil {
			...
			if serverErr := w.updateMobiusServer("", err); serverErr != nil {
				log.Error().Err(serverErr).Msg("failed to send decryption failure to Mobius Server")
				return
			}
		}
	...
	if serverErr := w.updateMobiusServer(recoveryKey, encryptionErr); serverErr != nil {
		log.Error().Err(serverErr).Msg("failed to send encryption result to Mobius Server")
		return
	}
...
func (w *windowsMDMBitlockerConfigReceiver) updateMobiusServer(key string, err error) error {
	// Getting Bitlocker encryption operation error message if any
	// This is going to be sent to Mobius Server
	bitlockerError := ""
	if err != nil {
		bitlockerError = err.Error()
	}

	// Update Mobius Server with encryption result
	payload := mobius.OrbitHostDiskEncryptionKeyPayload{
