...
func TestRenewEnrollmentProfile(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.desc, func(t *testing.T) {
			...
			testConfig := &mobius.OrbitConfig{Notifications: mobius.OrbitConfigNotifications{RenewEnrollmentProfile: c.renewFlag}}
...
func TestRenewEnrollmentProfilePrevented(t *testing.T) {
	...
	testConfig := &mobius.OrbitConfig{Notifications: mobius.OrbitConfigNotifications{RenewEnrollmentProfile: true}}
...
func TestWindowsMDMEnrollment(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.desc, func(t *testing.T) {
			...
			testConfig := &mobius.OrbitConfig{Notifications: mobius.OrbitConfigNotifications{
...
func TestWindowsMDMEnrollmentPrevented(t *testing.T) {
	...
	cfgs := []mobius.OrbitConfigNotifications{
		{
	...
	for _, cfg := range cfgs {
		t.Run(fmt.Sprintf("%+v", cfg), func(t *testing.T) {
			testConfig := &mobius.OrbitConfig{Notifications: cfg}
	...
	t.Run("dynamic enabling of scripts", func(t *testing.T) {
		...
		runner := &runScriptsConfigReceiver{
			...
			testGetMobiusdConfig: func() (*mobius.MDMAppleMobiusdConfig, error) {
				return &mobius.MDMAppleMobiusdConfig{
...
type mockDiskEncryptionKeySetter struct {
	SetOrUpdateDiskEncryptionKeyImpl    func(diskEncryptionStatus mobius.OrbitHostDiskEncryptionKeyPayload) error
	SetOrUpdateDiskEncryptionKeyInvoked bool
}

func (m *mockDiskEncryptionKeySetter) SetOrUpdateDiskEncryptionKey(diskEncryptionStatus mobius.OrbitHostDiskEncryptionKeyPayload) error {
...
func TestBitlockerOperations(t *testing.T) {
	...
	testConfig := &mobius.OrbitConfig{
		Notifications: mobius.OrbitConfigNotifications{
			EnforceBitLockerEncryption: shouldEncrypt,
		},
	}

	clientMock := &mockDiskEncryptionKeySetter{}
	clientMock.SetOrUpdateDiskEncryptionKeyImpl = func(diskEncryptionStatus mobius.OrbitHostDiskEncryptionKeyPayload) error {
	...
	t.Run("successful mobius server update", func(t *testing.T) {
		setupTest()
		shouldFailEncryption = false
		mockStatus := &bitlocker.EncryptionStatus{ConversionStatus: bitlocker.ConversionStatusFullyDecrypted}
		enrollReceiver.execGetEncryptionStatusFn = func() ([]bitlocker.VolumeStatus, error) {
	...
	t.Run("failed mobius server update", func(t *testing.T) {
		setupTest()
		shouldFailEncryption = false
		...
		err := enrollReceiver.Run(testConfig)
		require.NoError(t, err)
		require.Contains(t, logBuf.String(), "failed to send encryption result to Mobius Server")
		require.True(t, clientMock.SetOrUpdateDiskEncryptionKeyInvoked)
		require.True(t, encryptFnCalled, "encryption function should have been called")
		require.False(t, decryptFnCalled, "decryption function should not be called")
	})
