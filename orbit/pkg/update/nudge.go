...
// It checks the config supplied by the wrapped OrbitConfigFetcher to detects whether the Mobius
...
func ApplyNudgeConfigReceiverMiddleware(opt NudgeConfigFetcherOptions) mobius.OrbitConfigReceiver {
	return &NudgeConfigReceiver{opt: opt}
}

// GetConfig calls the wrapped Fetcher's GetConfig method, and detects if the
// Mobius server has supplied a Nudge config.
//
...
// provided config.
func (n *NudgeConfigReceiver) Run(cfg *mobius.OrbitConfig) error {
...
func (n *NudgeConfigReceiver) configure(nudgeCfg mobius.NudgeConfig) error {
