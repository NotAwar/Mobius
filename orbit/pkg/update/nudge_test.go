...
func (s *nudgeTestSuite) TestUpdatesDisabled() {
	...
	cfg.NudgeConfig, err = mobius.NewNudgeConfig(mobius.AppleOSUpdateSettings{MinimumVersion: optjson.SetString("11"), Deadline: optjson.SetString("2022-01-04")})
...
func (s *nudgeTestSuite) TestNudgeConfigFetcherAddNudge() {
	...
	require.NoError(t, err)
	var savedConfig mobius.NudgeConfig
	...
	configBytes, err = os.ReadFile(configPath)
	require.NoError(t, err)
	savedConfig = mobius.NudgeConfig{}
	err = json.Unmarshal(configBytes, &savedConfig)
	require.NoError(t, err)
	require.Equal(t, cfg.NudgeConfig, &savedConfig)
