...
func NewRunner(updater *Updater, opt RunnerOptions) (*Runner, error) {
	...
	if _, err := updater.Lookup(constant.OrbitTUFTargetName); errors.Is(err, client.ErrNoLocalSnapshot) {
		...
		// (which may be the case during the migration from https://tuf.mobiuscli.com to
		// https://updates.mobiusmdm.com).
		return runner, nil
	}
...
func (r *Runner) updateTarget(target string) error {
	...
	oVC := compareVersion(newVersion, build.Version, "mobiusdaemon")
	...
	if oVC != nil && *oVC != 0 && runtime.GOOS == "windows" {
		if err := updateUninstallMobiusdRegistryVersion(newVersion); err != nil {
