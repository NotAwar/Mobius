...
const (
	//
	// For users using Mobius's TUF:
	// 	- orbit 1.38.0+ we migrate TUF from https://tuf.mobiuscli.com to https://updates.mobiusmdm.com.
	...
	//
	// For both Mobius's TUF and custom TUF, mobiusdaemon packages built with mobiuscli 4.63.0+ will contain both files
	// `updates-metadata.json` and `tuf-metadata.json` (same content) to support downgrades to orbit 1.37.0 or lower.

	//
	// The following variables are used by `mobiuscli package` and by orbit in the migration to the new TUF repository.
	//

	DefaultURL          = `https://updates.mobiusmdm.com`
	...
	OldMobiusTUFURL      = "https://tuf.mobiuscli.com"
...
func NewUpdater(opt Options) (*Updater, error) {
	...
	//
	// NewUpdater is used when packaging Orbit (`mobiuscli package`) and by Orbit
	...
	if meta["root.json"] == nil {
		// NOTE: This path is currently only used when (1) packaging Orbit (`mobiuscli package`), or
		// (2) in the edge-case when orbit's metadata JSON local file is removed for some reason, or
		// (3) first run on TUF migration from https://tuf.mobiuscli.com to https://updates.mobiusmdm.com.
		//
		// When edge-case (2) happens, orbit will attempt to use Mobius DM's root JSON
...
func (u *Updater) checkExec(target, tmpPath string, customCheckExec func(execPath string) error) error {
	...
	if !containsArch && strings.HasSuffix(os.Args[0], "mobiuscli") {
...
func createTUFRemoteStore(opt Options, serverURL string) (client.RemoteStore, error) {
	...
	httpClient := mobiushttp.NewClient(mobiushttp.WithTLSClientConfig(tlsConfig))
