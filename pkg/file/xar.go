...
// in the XAR format.
func ExtractXARMetadata(tfr *mobius.TempFileReader) (*InstallerMetadata, error) {
	h := sha256.New()
...
func getPackageInfo(p *packageInfoXML) (name string, identifier string, version string, packageIDs []string) {
	...
	for _, bundle := range p.Bundles {
		...
		if base, isValid := isValidAppFilePath(installPath); isValid {
			identifier = mobius.Preprocess(bundle.ID)
			name = base
			version = mobius.Preprocess(bundle.CFBundleShortVersionString)
		}
		bundleID := mobius.Preprocess(bundle.ID)
		if bundleID != "" {
