...
type TestAppleMDMClient struct {
	...
	// mobiusServerURL is the URL of the Mobius server, used to fetch the enrollment profile.
	...
	// will fetch the enrollment profile from Mobius as if it were a device running
	// Mobius Desktop.
	fetchEnrollmentProfileFromDesktop bool
	// desktopURLToken is the Mobius Desktop token used to fetch the enrollment profile
	// from Mobius as if it were a device running Mobius Desktop.
	desktopURLToken string

	// fetchEnrollmentProfileFromDEP indicates whether this simulated device will fetch
	// the enrollment profile from Mobius as if it were a device running the DEP flow.
	...
	// the enrollment profile from Mobius as if it were a device running the OTA flow.
	...
	// from Mobius as if it were a device running the DEP flow.
	depURLToken string
...
// enrollment profile from Mobius as if it were a device running Mobius Desktop.
...
// enrollment profile from Mobius as if it were a device running the DEP flow.
...
// profile from Mobius. The enrollment information is to be provided in the enrollInfo.
...
// enrollment profile from Mobius as if it were a device running the Over The
// Air (OTA) flow.
...
// profile from the Mobius server and then runs the SCEP enrollment, Authenticate and TokenUpdate
// steps.
...
func (c *TestAppleMDMClient) fetchEnrollmentProfileFromDEPURLUsingPost() error {
	buf, err := MachineInfoAsPKCS7(mobius.MDMAppleMachineInfo{
		Serial: c.SerialNumber,
		UDID:   c.UUID,
	})
	if err != nil {
...
func (c *TestAppleMDMClient) fetchEnrollmentProfileFromOTAURL() error {
	return c.fetchOTAProfile(
		"/api/latest/mobius/enrollment_profiles/ota?enroll_secret=" + url.QueryEscape(c.otaEnrollSecret),
	)
}

func (c *TestAppleMDMClient) fetchOTAProfile(url string) error {
	request, err := http.NewRequest("GET", c.mobiusServerURL+url, nil)
	if err != nil {
...
<dict>
	...
	os.Unsetenv("MOBIUS_DEV_MDM_APPLE_DISABLE_DEVICE_INFO_CERT_VERIFY")
...
func (c *TestAppleMDMClient) fetchEnrollmentProfile(path string, body []byte) (err error) {
	...
	if len(body) > 0 {
		request, err = http.NewRequest("POST", c.mobiusServerURL+path, bytes.NewReader(body))
		if err != nil {
			return fmt.Errorf("create request: %w", err)
		}
		request.Header.Set("Content-Type", "application/pkcs7-signature")
	} else {
		request, err = http.NewRequest("GET", c.mobiusServerURL+path, nil)
		if err != nil {
			return fmt.Errorf("create request: %w", err)
		}
	}
...
func (c *TestAppleMDMClient) doSCEP(url, challenge string) (*x509.Certificate, *rsa.PrivateKey, error) {
	...
	// (3). Generate CSR.
	cn := fmt.Sprintf("mobius-testdevice-%s", c.UUID)
	csrTemplate := x509util.CertificateRequest{
		CertificateRequest: x509.CertificateRequest{
			Subject: pkix.Name{
				CommonName:   cn,
				Organization: []string{"mobius-organization"},
			},
...
func (c *TestAppleMDMClient) DeclarativeManagement(endpoint string, data ...mobius.MDMAppleDDMStatusReport) (*http.Response, error) {
	payload := map[string]any{
...
func (c *TestAppleMDMClient) AcknowledgeInstalledApplicationList(udid, cmdUUID string, software []mobius.Software) (*mdm.Command, error) {
...
func (c *TestAppleMDMClient) AcknowledgeCertificateList(udid, cmdUUID string, certTemplates []*x509.Certificate) (*mdm.Command, error) {
	var certList []mobius.MDMAppleCertificateListItem
	for _, cert := range certTemplates {
		b, _, err := mysql.GenerateTestCertBytes(cert)
		if err != nil {
			return nil, err
		}
		certList = append(certList, mobius.MDMAppleCertificateListItem{
...
func makeClientSCEPEndpoints(instance string) (*scepserver.Endpoints, error) {
	...
	// #nosec (this client is used for testing only)
	c := mobiushttp.NewClient(mobiushttp.WithTLSClientConfig(&tls.Config{
		InsecureSkipVerify: true,
	}))
...
func EncodeDeviceInfo(machineInfo mobius.MDMAppleMachineInfo) (string, error) {
...
func MachineInfoAsPKCS7(machineInfo mobius.MDMAppleMachineInfo) ([]byte, error) {
