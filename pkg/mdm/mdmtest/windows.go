...
type TestWindowsMDMClient struct {
	...
	HardwareID string
	// mobiusServerURL is the URL of the Mobius server, used to ping the MDM endpoints.
	...
	enrollmentType mobius.WindowsMDMEnrollmentType
...
func NewTestMDMClientWindowsProgramatic(serverURL string, orbitNodeKey string, opts ...TestWindowsMDMClientOption) *TestWindowsMDMClient {
	c := TestWindowsMDMClient{
		...
		enrollmentType:  mobius.WindowsMDMProgrammaticEnrollmentType,
...
func NewTestMDMClientWindowsAutomatic(serverURL string, email string, opts ...TestWindowsMDMClientOption) *TestWindowsMDMClient {
	c := TestWindowsMDMClient{
		mobiusServerURL:  serverURL,
		DeviceID:        uuid.NewString(),
		enrollmentType:  mobius.WindowsMDMAutomaticEnrollmentType,
...
func (c *TestWindowsMDMClient) StartManagementSession() (map[string]mobius.ProtoCmdOperation, error) {
	// Get SessionID
	sessionIDInt := 0
...
<SyncHdr>
	...
	<MsgID>1</MsgID>
	<Target>
	<LocURI>` + c.mobiusServerURL + microsoft_mdm.MDE2ManagementPath + `</LocURI>
	</Target>
	<Source>
...
func (c *TestWindowsMDMClient) doManagementReq(rawXMLReq []byte) (map[string]mobius.ProtoCmdOperation, error) {
	if c.debug {
	...
	c.queuedCommandResponses = make(map[string]mobius.SyncMLCmd)
	return cmds, nil
}

func (c *TestWindowsMDMClient) SendResponse() (map[string]mobius.ProtoCmdOperation, error) {
	// Get SessionID
	sessionID, err := c.lastManagementResp.GetSessionID()
	if err != nil {
		return nil, fmt.Errorf("session ID processing error %w", err)
	...
	var msg mobius.SyncML
	msg.Xmlns = syncml.SyncCmdNamespace
	msg.SyncHdr = mobius.SyncHdr{
		VerDTD:    syncml.SyncMLSupportedVersion,
		VerProto:  syncml.SyncMLVerProto,
		SessionID: sessionID,
		MsgID:     fmt.Sprint(messageIDInt + 1),
		Source:    &mobius.LocURI{LocURI: &c.DeviceID},
		Target: &mobius.LocURI{
			LocURI: ptr.String(c.mobiusServerURL + microsoft_mdm.MDE2ManagementPath),
		},
	}
...
func (c *TestWindowsMDMClient) AppendResponse(op mobius.SyncMLCmd) {
...
<s:Envelope
    ...
    <s:Header>
        ...
        <a:To s:mustUnderstand="1">` + c.mobiusServerURL + microsoft_mdm.MDE2EnrollPath + `</a:To>
...
<s:Envelope
    ...
    <s:Header>
        ...
        <a:To s:mustUnderstand="1">` + c.mobiusServerURL + microsoft_mdm.MDE2DiscoveryPath + `</a:To>
    </s:Header>
    <s:Body>
        <Discover
...
<s:Envelope
    ...
    <s:Header>
        ...
        <a:To s:mustUnderstand="1">` + c.mobiusServerURL + microsoft_mdm.MDE2PolicyPath + `</a:To>
        <wsse:Security s:mustUnderstand="1">
            <wsse:BinarySecurityToken ValueType="` + tokenValueType + `" EncodingType="http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-secext-1.0.xsd#base64binary">` + binarySecToken + `</wsse:BinarySecurityToken>
        </wsse:Security>
    </s:Header>
...
func (c *TestWindowsMDMClient) request(path string, reqBody []byte) (*http.Response, error) {
	request, err := http.NewRequest("POST", c.mobiusServerURL+path, bytes.NewReader(reqBody))
	if err != nil {
...
func (c *TestWindowsMDMClient) getToken() (binarySecToken string, tokenValueType string, err error) {
	...
	case mobius.WindowsMDMAutomaticEnrollmentType:
	...
	case mobius.WindowsMDMProgrammaticEnrollmentType:
		var err error
		tokenValueType = syncml.BinarySecurityDeviceEnroll
		binarySecToken, err = mobius.GetEncodedBinarySecurityToken(c.enrollmentType, c.TokenIdentifier)
		if err != nil {
