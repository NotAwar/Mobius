...
func TestValidGitOpsYaml(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"global_config_no_paths": {
			environment: map[string]string{
				"MOBIUS_SECRET_MOBIUS_SECRET_": "mobius_secret",
				"MOBIUS_SECRET_NAME":          "secret_name",
				"MOBIUS_SECRET_length":        "10",
				"MOBIUS_SECRET_BANANA":        "bread",
			},
		...
		t.Run(
			name, func(t *testing.T) {
				...
				if test.isTeam {
					...
					appConfig.License = &mobius.LicenseInfo{
				...
				if test.isTeam {
					...
					require.Len(t, secrets.([]*mobius.EnrollSecret), 2)
					assert.Equal(t, "SampleSecret123", secrets.([]*mobius.EnrollSecret)[0].Secret)
					assert.Equal(t, "ABC", secrets.([]*mobius.EnrollSecret)[1].Secret)
					...
					require.Len(t, gitops.Software.MobiusMaintainedApps, 2)
					for _, fma := range gitops.Software.MobiusMaintainedApps {
						switch fma.Slug {
						case "slack/darwin":
					...
					assert.Equal(t, "https://mobius.example.com", serverSettings.(map[string]interface{})["server_url"])
					...
					assert.True(t, ok)
					assert.Equal(t, "Mobius Device Management", orgInfo["org_name"])
					...
					assert.Contains(t, gitops.OrgSettings, "mobius_desktop")
				...
				// Check controls
				_, ok := gitops.Controls.MacOSSettings.(mobius.MacOSSettings)
				assert.True(t, ok, "macos_settings not found")
				_, ok = gitops.Controls.WindowsSettings.(mobius.WindowsSettings)
				...
				require.Len(t, gitops.MobiusSecrets, 4)
				assert.Equal(t, "mobius_secret", gitops.MobiusSecrets["MOBIUS_SECRET_MOBIUS_SECRET_"])
				assert.Equal(t, "secret_name", gitops.MobiusSecrets["MOBIUS_SECRET_NAME"])
				assert.Equal(t, "10", gitops.MobiusSecrets["MOBIUS_SECRET_length"])
				assert.Equal(t, "bread", gitops.MobiusSecrets["MOBIUS_SECRET_BANANA"])
				...
				if test.isTeam {
					...
					assert.Equal(t, "./lib/collect-mobiusdaemon-logs.sh", gitops.Policies[7].RunScript.Path)
					...
					assert.Contains(t, gitops.Policies[8].RunScript.Path, "./lib/collect-mobiusdaemon-logs.sh")
				}
			},
		)
	}
...
- name: orbit_info \$NOT_EXPANDED \\\$ALSO_NOT_EXPANDED
  query: "SELECT * from orbit_info; -- double quotes are escaped by YAML after Mobius's escaping of backslashes \\\\\$NOT_EXPANDED"
  interval: 0
...
`
	...
	require.Equal(t, `SELECT * from orbit_info; -- double quotes are escaped by YAML after Mobius's escaping of backslashes \$NOT_EXPANDED`, gitOps.Queries[0].Query)
...
`
	...
	require.NoError(t, err)
	err = file.Copy(
		filepath.Join("testdata", "lib", "collect-mobiusdaemon-logs.sh"),
		filepath.Join(basePath, "lib", "collect-mobiusdaemon-logs.sh"),
		0o755,
	)
	require.NoError(t, err)
	appConfig := mobius.EnrichedAppConfig{}
...
func TestIllegalMobiusSecret(t *testing.T) {
	t.Parallel()
	config := getGlobalConfig([]string{"policies"})
	config += `
policies:
  - name: $MOBIUS_SECRET_POLICY
    platform: linux
    query: SELECT 1 FROM osquery_info WHERE start_time < 0;
  - name: My policy
    platform: windows
...
`
	_, err := gitOpsFromString(t, config)
	assert.ErrorContains(t, err, "variables with \"MOBIUS_SECRET_\" prefix are only allowed")
}
