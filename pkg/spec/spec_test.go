...
func TestExpandEnv(t *testing.T) {
	for _, tc := range []struct {
		...
		{map[string]string{"foo": "1"}, `$foo $MOBIUS_VAR_BAR ${MOBIUS_VAR_BAR}x ${foo}`, `1 $MOBIUS_VAR_BAR ${MOBIUS_VAR_BAR}x 1`, nil},
		...
		{map[string]string{}, `foo PREVENT_ESCAPING_bar $ MOBIUS_VAR_`, `foo PREVENT_ESCAPING_bar $ MOBIUS_VAR_`, nil}, // nothing to replace
		{
			map[string]string{"foo": "BAR"},
			`\$MOBIUS_VAR_$foo \${MOBIUS_VAR_$foo} \${MOBIUS_VAR_${foo}2}`,
			`$MOBIUS_VAR_BAR ${MOBIUS_VAR_BAR} ${MOBIUS_VAR_BAR2}`, nil,
		}, // nested variables
	} {
		os.Clearenv()
...
func TestLookupEnvSecrets(t *testing.T) {
	for _, tc := range []struct {
		...
		{map[string]string{"MOBIUS_SECRET_foo": "1"}, `$MOBIUS_SECRET_foo`, map[string]string{"MOBIUS_SECRET_foo": "1"}, nil},
		{
			map[string]string{"foo": "1"},
			`$MOBIUS_SECRET_foo`,
			map[string]string{},
			checkMultiErrors(t, "environment variable \"MOBIUS_SECRET_foo\" not set"),
		},
	} {
		os.Clearenv()
		for k, v := range tc.environment {
