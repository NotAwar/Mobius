...
import (
	...
	"github.com/notawar/mobius/mobius-server/server/shellquote"
...
// Launch is the entrypoint that sets up and runs the Mobius commands.
func main() {
...
func createRootCmd() *cobra.Command {
	...
	rootCmd := &cobra.Command{
		...
		Long: `Mobius server (https://mobiusmdm.com)
...
func applyDevFlags(cfg *config.MobiusConfig) {
	cfg.Mysql.Username = "mobius"
	cfg.Mysql.Database = "mobius"
	cfg.Mysql.Password = "insecure"

	if cfg.Prometheus.BasicAuth.Username == "" {
		cfg.Prometheus.BasicAuth.Username = "mobius"
	}
...
func initLogger(cfg config.MobiusConfig) kitlog.Logger {
