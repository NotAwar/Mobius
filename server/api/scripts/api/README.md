Using `curl` and `jq` to interact with the mobius API.

First, create a `env` file with the following contents:

```
export SERVER_URL=https://localhost:8080 # your mobius server url and port
export CURL_FLAGS='-k -s' # set insecure flag
export TOKEN=eyJhbGciOi... # your api token
```

Next set the `MOBIUS_ENV_PATH` to point to the `env` file. This will let the scripts in the `mobius/` folder source the env file.

# Examples

```
export MOBIUS_ENV_PATH=./path/to/env/file/mobius_env

# get my user info
./server/api/tools/api/mobius/me
{
  "user": {
...
# list queries
./server/api/tools/api/mobius/queries/list
{
  "queries": []
}

# use jq to filter a specific query and get the id
./server/api/tools/api/mobius/queries/list | jq '.queries[]|select(.name == "osquery_info")|.id'
2

# create a query
./server/api/tools/api/mobius/queries/create 'system_info' 'SELECT * FROM system_info;'
{
  "query": {
...
./server/api/tools/api/mobius/schedule/add_query_to_pack 2 4

# get scheduled queries in a pack
./server/api/tools/api/mobius/packs/scheduled 2 | jq '.scheduled[]|{"name": .name, "schedule_id": .id, "query_id": .query_id}'

# run a live queries on hosts (queries with id=1 and id=2 on hosts with id=3 and id=4)
./server/api/tools/api/mobius/queries/run "[1,2]" "[3,4]"
```

> The following examples are made obsolete with the vuln endpoint <https://mobiusmdm.com/docs/rest-api/rest-api#vulnerabilities>

Bash Script - Pulls all hosts based on software _name_ for your Mobius instance, uses jq. Helps if wanting to track down a particular software and see what hosts might be affected.
...
BASE_URL = "https://dogfood.mobiusmdm.com/api/latest" #change to your base url
...
def get_all_vulnerable_software_titles():
    ...
    endpoint = (f"{BASE_URL}/mobius/software/titles?scope=software-titles&page=0&per_page=1000&order_direction=desc&order_key=hosts_count&vulnerable=true") #note that this is set to 1k to try and capture "all" but might need to adjust
...
def get_hosts_for_software_version(version_id):
    ...
    endpoint = f"{BASE_URL}/mobius/hosts?software_version_id={version_id}"
    response = requests.get(endpoint, headers=HEADERS)
    api_calls_counter += 1
