...
# gitops as this is not something that relates to mobiuscli apply).
allow {
...
# teams (not gitops as this is not something that relates to mobiuscli apply).
allow {
  object.type == "host_software_installer_result"
...
# Any logged in user can read Mobius's version
allow {
  object.type == "version"
  not is_null(subject)
  action == read
