...
import (
	...
	"github.com/notawar/mobius/mobius-server/server/mdm/nanodep/tokenpki"
...
// ServerConfig defines configs related to the Mobius server
...
// LicenseConfig defines configs related to licensing Mobius.
type LicenseConfig struct {
	Key              string `yaml:"key"`
	EnforceHostLimit bool   `yaml:"enforce_host_limit"`
}

// VulnerabilitiesConfig defines configs related to vulnerability processing within Mobius.
...
// UpgradesConfig defines configs related to mobius server upgrades.
...
// PrometheusConfig holds the configuration for Mobius's prometheus metrics.
...
// PackagingConfig holds configuration to build and retrieve Mobius packages
...
// MobiusConfig stores the application configuration. Each subcategory is
...
type MicrosoftCompliancePartnerConfig struct {
	// ProxyAPIKey is a shared key required to use the Microsoft Compliance Partner proxy API (mobiusmdm.com).
...
type MDMConfig struct {
	...
	// AppleEnable enables Apple MDM functionality on Mobius.
...
// filled into the MobiusConfig struct
func (man Manager) addConfigs() {
	addMysqlConfig := func(prefix, defaultAddr, usageSuffix string) {
		...
		man.addConfigString(prefix+".username", "mobius",
		...
		man.addConfigString(prefix+".database", "mobius",
	...
	man.addConfigString("server.address", "0.0.0.0:8080",
		"Mobius server address (host:port)")
	man.addConfigString("server.cert", "./tools/osquery/mobius.crt",
		"Mobius TLS certificate path")
	man.addConfigString("server.key", "./tools/osquery/mobius.key",
		"Mobius TLS key path")
	...
	man.addConfigBool("server.sandbox_enabled", false,
		"When enabled, Mobius limits some features for the Sandbox")
	man.addConfigBool("server.websockets_allow_unsafe_origin", false, "Disable checking the origin header on websocket connections, this is sometimes necessary when proxies rewrite origin headers between the client and the Mobius webserver")
	man.addConfigBool("server.frequent_cleanups_enabled", false, "Enable frequent cleanups of expired data (15 minute interval)")
	man.addConfigBool("server.force_h2c", false, "Force the mobius server to use HTTP2 cleartext aka h2c (ignored if using TLS)")
	...
	// License
	man.addConfigString("license.key", "", "Mobius license key (to enable Mobius Premium features)")
	...
	man.addConfigString("vulnerabilities.databases_path", "/tmp/vulndbs",
		"Path where Mobius will download the data feeds to check CVEs")
	...
	man.addConfigString("microsoft_compliance_partner.proxy_uri", "https://mobiusmdm.com", "URI of the Microsoft Compliance Partner proxy (for development/testing)")
...
// MobiusConfig struct
func (man Manager) LoadConfig() MobiusConfig {
	man.loadConfigFile()
...
// Manager manages the addition and retrieval of config values for Mobius
// configs. It's only public API method is LoadConfig, which will return the
// populated MobiusConfig struct.
type Manager struct {
	viper    *viper.Viper
...
// decrypting the encrypted value that is usually provided via the mobius
// server's flags.
func SetTestMDMConfig(t testing.TB, cfg *MobiusConfig, cert, key []byte, wstepCertAndKeyDir string) {
