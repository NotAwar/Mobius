...
import (
	...
	"github.com/notawar/mobius/mobius-server/server/service/calendar"
	"github.com/notawar/mobius/mobius-server/server/service/schedule"
)
...
func NewCalendarSchedule(
	...
	instanceID string,
	ds mobius.Datastore,
	distributedLock mobius.Lock,
	serverConfig config.CalendarConfig,
	logger kitlog.Logger,
) (*schedule.Schedule, error) {
	const (
		name = string(mobius.CronCalendar)
	)
...
func cronCalendarEvents(ctx context.Context, ds mobius.Datastore, distributedLock mobius.Lock, serverConfig config.CalendarConfig,
...
func cronCalendarEventsForTeam(
	...
	var (
		passingHosts                       []mobius.HostPolicyMembershipData
		failingHosts                       []mobius.HostPolicyMembershipData
		failingHostsWithoutAssociatedEmail []mobius.HostPolicyMembershipData
	)
...
func processCalendarFailingHosts(
	...
	calendarConfig *calendar.Config,
	orgName string,
	hosts []mobius.HostPolicyMembershipData,
	logger kitlog.Logger,
) {
	hosts = filterHostsWithSameEmail(hosts)

	hostsCh := make(chan mobius.HostPolicyMembershipData)
		...
		go func() {
			...
			for host := range hostsCh {
				...
				if err == nil {
					...
					if hostCalendarEvent.WebhookStatus == mobius.CalendarWebhookStatusPending || hostCalendarEvent.WebhookStatus == mobius.CalendarWebhookStatusRetry {
						// This can happen if the host went offline (and never returned results)
						// after setting the webhook as pending.
						continue // continue with next host
					}
					now := time.Now()
					webhookAlreadyFired := hostCalendarEvent.WebhookStatus == mobius.CalendarWebhookStatusSent
				...
				case mobius.IsNotFound(err) || expiredEvent:
...
func filterHostsWithSameEmail(hosts []mobius.HostPolicyMembershipData) []mobius.HostPolicyMembershipData {
	minHostPerEmail := make(map[string]mobius.HostPolicyMembershipData)
	for _, host := range hosts {
	...
	filtered := make([]mobius.HostPolicyMembershipData, 0, len(minHostPerEmail))
...
func processFailingHostExistingCalendarEvent(
	...
	ds mobius.Datastore,
	distributedLock mobius.Lock,
	userCalendar mobius.UserCalendar,
	orgName string,
	hostCalendarEvent *mobius.HostCalendarEvent,
	calendarEvent *mobius.CalendarEvent,
	host mobius.HostPolicyMembershipData,
	policyIDtoPolicy *sync.Map,
	calendarConfig *calendar.Config,
	logger kitlog.Logger,
) error {
	...
	var newETag string
	var genBodyFn mobius.CalendarGenBodyFn = func(conflict bool) (string, bool, error) {
		var body string
	...
	if calendarConfig.AlwaysReloadEvent() || shouldReloadCalendarEvent(now, calendarEvent, hostCalendarEvent) {
		...
		updatedEvent, _, err = userCalendar.GetAndUpdateEvent(calendarEvent, genBodyFn,
			mobius.CalendarGetAndUpdateEventOpts{UpdateTimezone: true})
		if err != nil {
	...
	if hostCalendarEvent.WebhookStatus == mobius.CalendarWebhookStatusSent {
		return nil
	}
	...
	if err := ds.UpdateHostCalendarWebhookStatus(ctx, host.HostID, mobius.CalendarWebhookStatusPending); err != nil {
...
func getBodyTag(ctx context.Context, ds mobius.Datastore, host mobius.HostPolicyMembershipData, policyIDtoPolicy *sync.Map,
...
func shouldReloadCalendarEvent(now time.Time, calendarEvent *mobius.CalendarEvent, hostCalendarEvent *mobius.HostCalendarEvent) bool {
	// Check the user calendar regularly (but not every cron run)
	// to reduce load on both Mobius and the calendar service.
	...
	if eventHappeningNow(now, calendarEvent) && hostCalendarEvent.WebhookStatus == mobius.CalendarWebhookStatusNone {
		return true
	}
	return false
}

func eventHappeningNow(now time.Time, calendarEvent *mobius.CalendarEvent) bool {
...
func processFailingHostCreateCalendarEvent(
	...
	if _, err := ds.CreateOrUpdateCalendarEvent(
		...
		host.HostID, mobius.CalendarWebhookStatusNone,
	); err != nil {
	...
	for {
		...
		calendarEvent, err := userCalendar.CreateEvent(
			preferredDate, func(conflict bool) (string, bool, error) {
				...
				body, generatedTag = calendar.GenerateCalendarEventBody(ctx, ds, orgName, host, policyIDtoPolicy, conflict, logger)
				return body, true, nil
			}, mobius.CalendarCreateEventOpts{},
		)
		var dee mobius.DayEndedError
		switch {
		case err == nil:
...
func isHostOnline(ctx context.Context, ds mobius.Datastore, hostID uint) (bool, error) {
	...
	switch status {
	case mobius.StatusOnline, mobius.StatusNew:
		return true, nil
	case mobius.StatusOffline, mobius.StatusMIA, mobius.StatusMissing:
		return false, nil
	default:
...
func cronCalendarEventsCleanup(ctx context.Context, ds mobius.Datastore, logger kitlog.Logger) error {
	appConfig, err := ds.AppConfig(ctx)
	if err != nil {
		return fmt.Errorf("load app config: %w", err)
	}

	var userCalendar mobius.UserCalendar
...
func deleteCalendarEventsInParallel(
	ctx context.Context, ds mobius.Datastore, calendarConfig *calendar.Config, calendarEvents []*mobius.CalendarEvent,
	logger kitlog.Logger,
) {
	if len(calendarEvents) > 0 {
		calendarEventCh := make(chan *mobius.CalendarEvent)
		var wg sync.WaitGroup
		for i := 0; i < calendarConsumers; i++ {
			wg.Add(+1)
			go func() {
...
func deleteCalendarEvent(
	ctx context.Context, ds mobius.Datastore, userCalendar mobius.UserCalendar, calendarEvent *mobius.CalendarEvent,
) error {
	if userCalendar != nil {
