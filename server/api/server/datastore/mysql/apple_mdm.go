...
import (
	...
	mobiusmdm "github.com/MobiusDM/mobius/server/api/server/mdm"
...
func isAppleHostConnectedToMobiusMDM(ctx context.Context, q sqlx.QueryerContext, h *mobius.Host) (bool, error) {
	var uuid string
...
func (ds *Datastore) NewMDMAppleConfigProfile(ctx context.Context, cp mobius.MDMAppleConfigProfile, usesMobiusVars []string) (*mobius.MDMAppleConfigProfile, error) {
...
(SELECT ?, ?, ?, ?, ?, ?, UNHEX(MD5(?)), CURRENT_TIMESTAMP(), ? FROM DUAL WHERE
	...
	err := ds.withTx(ctx, func(tx sqlx.ExtContext) error {
		...
		// record the ID as we want to return a mobius.Profile instance with it
		// filled in.
		profileID, _ = res.LastInsertId()

		labels := make([]mobius.ConfigurationProfileLabel, 0, len(cp.LabelsIncludeAll)+len(cp.LabelsIncludeAny)+len(cp.LabelsExcludeAny))
		...
		if err := batchSetProfileVariableAssociationsDB(ctx, tx, []mobius.MDMProfileUUIDMobiusVariables{
			{ProfileUUID: profUUID, MobiusVariables: usesMobiusVars},
		}, "darwin"); err != nil {
...
func formatErrorDuplicateConfigProfile(err error, cp *mobius.MDMAppleConfigProfile) error {
	switch {
...
func formatErrorDuplicateDeclaration(err error, decl *mobius.MDMAppleDeclaration) error {
	switch {
...
func (ds *Datastore) ListMDMAppleConfigProfiles(ctx context.Context, teamID *uint) ([]*mobius.MDMAppleConfigProfile, error) {
	stmt := `
SELECT
	profile_uuid,
	profile_id,
...
ORDER BY name`
	...
	mobiusIdentifiers := []string{}
	for idf := range mobileconfig.MobiusPayloadIdentifiers() {
		mobiusIdentifiers = append(mobiusIdentifiers, idf)
	}
	stmt, args, err := sqlx.In(stmt, teamID, mobiusIdentifiers)
	if err != nil {
		return nil, ctxerr.Wrap(ctx, err, "sqlx.In ListMDMAppleConfigProfiles")
	}

	var res []*mobius.MDMAppleConfigProfile
...
func (ds *Datastore) GetMDMAppleConfigProfileByDeprecatedID(ctx context.Context, profileID uint) (*mobius.MDMAppleConfigProfile, error) {
	return ds.getMDMAppleConfigProfileByIDOrUUID(ctx, profileID, "")
}

func (ds *Datastore) GetMDMAppleConfigProfile(ctx context.Context, profileUUID string) (*mobius.MDMAppleConfigProfile, error) {
	return ds.getMDMAppleConfigProfileByIDOrUUID(ctx, 0, profileUUID)
}

func (ds *Datastore) getMDMAppleConfigProfileByIDOrUUID(ctx context.Context, id uint, uuid string) (*mobius.MDMAppleConfigProfile, error) {
	stmt := `
SELECT
	profile_uuid,
	profile_id,
...
func (ds *Datastore) GetMDMAppleDeclaration(ctx context.Context, declUUID string) (*mobius.MDMAppleDeclaration, error) {
	stmt := `
SELECT
	declaration_uuid,
	team_id,
...
WHERE
	...
	var res mobius.MDMAppleDeclaration
...
func cancelAppleHostInstallsForDeletedMDMProfiles(ctx context.Context, tx sqlx.ExtContext, profileUUIDs []string) error {
	...
	stmt, args, err := sqlx.In(delStmt, profileUUIDs, mobius.MDMOperationTypeInstall)
	if err != nil {
	...
	stmt, args, err = sqlx.In(deactivateNanoStmt, profileUUIDs, mobius.MDMDeliveryPending, mobius.MDMOperationTypeInstall)
	if err != nil {
		return ctxerr.Wrap(ctx, err, "building in statement")
	}
	if _, err := tx.ExecContext(ctx, stmt, args...); err != nil {
	...
	stmt, args, err = sqlx.In(updStmt, mobius.MDMOperationTypeRemove,
		[]mobius.MDMDeliveryStatus{mobius.MDMDeliveryPending, mobius.MDMDeliveryFailed}, profileUUIDs, mobius.MDMOperationTypeInstall)
	if err != nil {
		return ctxerr.Wrap(ctx, err, "building in statement")
	}
...
func cancelAppleHostInstallsForDeletedMDMDeclarations(ctx context.Context, tx sqlx.ExtContext, declUUIDs []string) error {
	...
	stmt, args, err := sqlx.In(delStmt, declUUIDs, mobius.MDMOperationTypeInstall)
	if err != nil {
		return ctxerr.Wrap(ctx, err, "building in statement")
	}
	...
	stmt, args, err = sqlx.In(updStmt, mobius.MDMOperationTypeRemove, declUUIDs, mobius.MDMOperationTypeInstall)
	if err != nil {
		return ctxerr.Wrap(ctx, err, "building in statement")
	}
...
func (ds *Datastore) GetHostMDMAppleProfiles(ctx context.Context, hostUUID string) ([]mobius.HostMDMAppleProfile, error) {
	stmt := fmt.Sprintf(`
SELECT
	profile_uuid,
...
WHERE
	...
	stmt, args, err := sqlx.In(stmt, hostUUID, hostUUID, mobiusmdm.ListMobiusReservedMacOSDeclarationNames())
	if err != nil {
		return nil, ctxerr.Wrap(ctx, err, "building in statement")
	}

	var profiles []mobius.HostMDMAppleProfile
...
func (ds *Datastore) GetHostMDMCertificateProfile(ctx context.Context, hostUUID string,
...
) (*mobius.HostMDMCertificateProfile, error) {
	stmt := `
	SELECT
		hmap.host_uuid,
		hmap.profile_uuid,
	...
	var profile mobius.HostMDMCertificateProfile
...
func (ds *Datastore) RenewMDMManagedCertificates(ctx context.Context) error {
	...
	hostProfileClause := ``
	values := []interface{}{mobius.MDMOperationTypeInstall}

	totalHostCertsToRenew := 0
	hostCertTypesToRenew := []mobius.CAConfigAssetType{mobius.CAConfigDigiCert, mobius.CAConfigCustomSCEPProxy, mobius.CAConfigNDES}
	...
	LIMIT 1000`, hostCertType, mobius.MDMOperationTypeInstall)
		if err != nil {
...
WHERE
	...
	return ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		...
		res, err = tx.ExecContext(ctx, updateStmt, profUUID, hostUUID, mobius.MDMOperationTypeInstall)
		if err != nil {
...
func (ds *Datastore) NewMDMAppleEnrollmentProfile(
	ctx context.Context,
	payload mobius.MDMAppleEnrollmentProfilePayload,
) (*mobius.MDMAppleEnrollmentProfile, error) {
...
func (ds *Datastore) ListMDMAppleEnrollmentProfiles(ctx context.Context) ([]*mobius.MDMAppleEnrollmentProfile, error) {
	var enrollmentProfiles []*mobius.MDMAppleEnrollmentProfile
...
func (ds *Datastore) GetMDMAppleEnrollmentProfileByToken(ctx context.Context, token string) (*mobius.MDMAppleEnrollmentProfile, error) {
	var enrollment mobius.MDMAppleEnrollmentProfile
...
func (ds *Datastore) GetMDMAppleEnrollmentProfileByType(ctx context.Context, typ mobius.MDMAppleEnrollmentType) (*mobius.MDMAppleEnrollmentProfile, error) {
	var enrollment mobius.MDMAppleEnrollmentProfile
...
func (ds *Datastore) GetVPPCommandResults(ctx context.Context, commandUUID string, hostUUID string) ([]*mobius.MDMCommandResult, error) {
	var results []*mobius.MDMCommandResult
...
func (ds *Datastore) GetMDMAppleCommandResults(ctx context.Context, commandUUID string) ([]*mobius.MDMCommandResult, error) {
	query := `
SELECT
    ncr.id as host_uuid,
    ncr.command_uuid,
...
func (ds *Datastore) ListMDMAppleCommands(
	...
	listOpts *mobius.MDMCommandListOptions,
) ([]*mobius.MDMAppleCommand, error) {
...
`, ds.whereFilterHostsByTeams(tmFilter, "h"))
	...
	var results []*mobius.MDMAppleCommand
...
func (ds *Datastore) NewMDMAppleInstaller(ctx context.Context, name string, size int64, manifest string, installer []byte, urlToken string) (*mobius.MDMAppleInstaller, error) {
	...
	id, _ := res.LastInsertId()
	return &mobius.MDMAppleInstaller{
		ID:        uint(id), //nolint:gosec // dismiss G115
		Size:      size,
		Name:      name,
		Manifest:  manifest,
...
func (ds *Datastore) MDMAppleInstaller(ctx context.Context, token string) (*mobius.MDMAppleInstaller, error) {
	var installer mobius.MDMAppleInstaller
...
func (ds *Datastore) MDMAppleInstallerDetailsByID(ctx context.Context, id uint) (*mobius.MDMAppleInstaller, error) {
	var installer mobius.MDMAppleInstaller
	if err := sqlx.GetContext(
		ctx,
		ds.writer(ctx),
...
func (ds *Datastore) MDMAppleInstallerDetailsByToken(ctx context.Context, token string) (*mobius.MDMAppleInstaller, error) {
	var installer mobius.MDMAppleInstaller
	if err := sqlx.GetContext(
		ctx,
		ds.writer(ctx),
...
func (ds *Datastore) ListMDMAppleInstallers(ctx context.Context) ([]mobius.MDMAppleInstaller, error) {
	var installers []mobius.MDMAppleInstaller
...
func (ds *Datastore) MDMAppleListDevices(ctx context.Context) ([]mobius.MDMAppleDevice, error) {
	var devices []mobius.MDMAppleDevice
	if err := sqlx.SelectContext(
		ctx,
		ds.writer(ctx),
		&devices,
...
func (ds *Datastore) MDMAppleUpsertHost(ctx context.Context, mdmHost *mobius.Host) error {
...
func mdmHostEnrollFields(mdmHost *mobius.Host) (refetchRequested bool, lastEnrolledAt time.Time) {
...
func createHostFromMDMDB(
	...
	// third-party MDM, but gets assigned in ABM to Mobius (during
...
func (ds *Datastore) IngestMDMAppleDevicesFromDEPSync(
	...
	abmTokenID uint,
	macOSTeam, iosTeam, ipadTeam *mobius.Team,
	...
	var teamIDs []*uint
	for _, team := range []*mobius.Team{macOSTeam, iosTeam, ipadTeam} {
		if team == nil {
...
func (ds *Datastore) UpsertMDMAppleHostDEPAssignments(ctx context.Context, hosts []mobius.Host, abmTokenID uint) error {
...
func upsertHostDEPAssignmentsDB(ctx context.Context, tx sqlx.ExtContext, hosts []mobius.Host, abmTokenID uint) error {
	if len(hosts) == 0 {
		return nil
	}
...
func upsertHostDisplayNames(ctx context.Context, tx sqlx.ExtContext, hosts ...mobius.Host) error {
	var args []interface{}
	var parts []string
	for _, h := range hosts {
...
func upsertMDMAppleHostMDMInfoDB(ctx context.Context, tx sqlx.ExtContext, appCfg *mobius.AppConfig, fromSync bool, hostIDs ...uint) error {
	if len(hostIDs) == 0 {
		return nil
	}
	...
	if err != nil {
		return ctxerr.Wrap(ctx, err, "resolve Mobius MDM URL")
	}
	...
	result, err := tx.ExecContext(ctx, `
		...
		mobius.WellKnownMDMMobius, serverURL)
	if err != nil {
	...
	if insertOnDuplicateDidInsertOrUpdate(result) {
		...
		if err := sqlx.GetContext(ctx, tx, &mdmID, stmt, mobius.WellKnownMDMMobius, serverURL); err != nil {
...
func upsertMDMAppleHostLabelMembershipDB(ctx context.Context, tx sqlx.ExtContext, logger log.Logger, hosts ...mobius.Host) error {
...
func (ds *Datastore) MDMTurnOff(ctx context.Context, uuid string) error {
	return ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		...
		if !mobius.MDMSupported(host.Platform) {
...
func unionSelectDevices(devices []hostToCreateFromMDM) (stmt string, args []interface{}) {
	for i, d := range devices {
		...
		// map the platform hint to Mobius's hosts.platform field.
		normalizedHint := strings.ToLower(d.PlatformHint)
		platform := string(mobius.MacOSPlatform)
		switch {
		case strings.Contains(normalizedHint, "iphone"):
			platform = string(mobius.IOSPlatform)
		case strings.Contains(normalizedHint, "ipad"):
			platform = string(mobius.IPadOSPlatform)
		}
...
func (ds *Datastore) GetHostDEPAssignment(ctx context.Context, hostID uint) (*mobius.HostDEPAssignment, error) {
	var res mobius.HostDEPAssignment
...
func (ds *Datastore) DeleteHostDEPAssignments(ctx context.Context, abmTokenID uint, serials []string) error {
	...
	return ds.withTx(ctx, func(tx sqlx.ExtContext) error {
		...
		// If pending host is no longer in ABM, we should delete it because it will never enroll in Mobius.
...
func (ds *Datastore) RestoreMDMApplePendingDEPHost(ctx context.Context, host *mobius.Host) error {
...
func (ds *Datastore) GetNanoMDMEnrollment(ctx context.Context, id string) (*mobius.NanoEnrollment, error) {
	var nanoEnroll mobius.NanoEnrollment
...
func (ds *Datastore) GetNanoMDMUserEnrollment(ctx context.Context, deviceId string) (*mobius.NanoEnrollment, error) {
	var nanoEnroll mobius.NanoEnrollment
	// use writer as it is used just after creation in some cases
	err := sqlx.GetContext(ctx, ds.writer(ctx), &nanoEnroll, `SELECT id, device_id, type, enabled, token_update_tally
...
// Mobius variables in profiles.
func (ds *Datastore) BatchSetMDMAppleProfiles(ctx context.Context, tmID *uint, profiles []*mobius.MDMAppleConfigProfile) error {
	return ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
...
func (ds *Datastore) batchSetMDMAppleProfilesDB(
	...
	profilesVariablesByIdentifier []mobius.MDMProfileIdentifierMobiusVariables,
...
`
	...
	// or processing
	incomingProfs := make(map[string]*mobius.MDMAppleConfigProfile, len(profiles))
	for i, p := range profiles {
		incomingIdents[i] = p.Identifier
		incomingProfs[p.Identifier] = p
	}

	var existingProfiles []*mobius.MDMAppleConfigProfile
	...
	// profiles that are managed and delivered by Mobius
	mobiusIdents := []string{}
	for ident := range mobileconfig.MobiusPayloadIdentifiers() {
		mobiusIdents = append(mobiusIdents, ident)
	}
	...
	stmt, args, err = sqlx.In(loadToBeDeletedProfilesNotInList, profTeamID, append(keepIdents, mobiusIdents...))
	if err != nil {
	...
	// delete the obsolete profiles (all those that are not in keepIdents or delivered by Mobius)
	var result sql.Result
	stmt, args, err = sqlx.In(deleteProfilesNotInList, profTeamID, append(keepIdents, mobiusIdents...))
	...
	if len(profileIdentsInsertedOrUpdated) > 0 {
		var insertedUpdatedProfs []*mobius.MDMAppleConfigProfile
		...
		for _, p := range profilesVariablesByIdentifier {
			lookupVariablesByIdentifier[p.Identifier] = p.MobiusVariables
		}
	...
	incomingLabels := []mobius.ConfigurationProfileLabel{}
	var profsWithoutLabels []string
	if len(incomingIdents) > 0 {
		var newlyInsertedProfs []*mobius.MDMAppleConfigProfile
...
func (ds *Datastore) BulkDeleteMDMAppleHostsConfigProfiles(ctx context.Context, profs []*mobius.MDMAppleProfilePayload) error {
	return ds.withTx(ctx, func(tx sqlx.ExtContext) error {
		return ds.bulkDeleteMDMAppleHostsConfigProfilesDB(ctx, tx, profs)
	})
}

func (ds *Datastore) bulkDeleteMDMAppleHostsConfigProfilesDB(ctx context.Context, tx sqlx.ExtContext, profs []*mobius.MDMAppleProfilePayload) error {
	if len(profs) == 0 {
		return nil
	}
...
`, fmt.Sprintf(appleMDMProfilesDesiredStateQuery, profileHostIn, profileHostIn, profileHostIn, profileHostIn))
	...
	var wantedProfiles []*mobius.MDMAppleProfilePayload
	for i := 0; i < selectProfilesTotalBatches; i++ {
		...
		if len(onlyProfileUUIDs) > 0 {
			...
			stmt, args, err = sqlx.In(toInstallStmt, batchUUIDs, batchUUIDs, batchUUIDs, batchUUIDs, mobius.MDMOperationTypeRemove)
		}
		...
		var partialResult []*mobius.MDMAppleProfilePayload
	...
	wantedProfiles = mobius.FilterMacOSOnlyProfilesFromIOSIPadOS(wantedProfiles)
...
`, fmt.Sprintf(appleMDMProfilesDesiredStateQuery, profileHostIn, profileHostIn, profileHostIn, profileHostIn), narrowByProfiles)
	...
	var currentProfiles []*mobius.MDMAppleProfilePayload
	for i := 0; i < selectProfilesTotalBatches; i++ {
		start := i * selectProfilesBatchSize
		end := start + selectProfilesBatchSize
		if end > len(hostUUIDs) {
		...
		if len(onlyProfileUUIDs) > 0 {
			...
			stmt, args, err = sqlx.In(toRemoveStmt, batchUUIDs, batchUUIDs, batchUUIDs, batchUUIDs, batchUUIDs, mobius.MDMOperationTypeRemove)
		}
	...
	var hostProfilesToClean []*mobius.MDMAppleProfilePayload
	...
	profilesToInsert := make(map[string]*mobius.MDMAppleProfilePayload)
	...
	for _, p := range wantedProfiles {
		if pp, ok := profileIntersection.GetMatchingProfileInCurrentState(p); ok {
			if pp.Status != &mobius.MDMDeliveryFailed && bytes.Equal(pp.Checksum, p.Checksum) {
				profilesToInsert[fmt.Sprintf("%s\n%s", p.HostUUID, p.ProfileUUID)] = &mobius.MDMAppleProfilePayload{
					ProfileUUID:       p.ProfileUUID,
		...
		pargs = append(pargs, p.ProfileUUID, p.HostUUID, p.ProfileIdentifier, p.ProfileName, p.Checksum, p.SecretsUpdatedAt,
			mobius.MDMOperationTypeInstall, nil, "", "", p.Scope)
		psb.WriteString("(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?),")
		batchCount++
	...
	for _, p := range currentProfiles {
		...
		// Doing so will result in Mobius attempting to remove a profile that doesn't exist on the
		// host (since the installation failed). Skipping it here will lead to it being removed from
		// the host in Mobius during profile reconciliation, which is what we want.
		...
		pargs = append(pargs, p.ProfileUUID, p.HostUUID, p.ProfileIdentifier, p.ProfileName, p.Checksum, p.SecretsUpdatedAt,
			mobius.MDMOperationTypeRemove, nil, "", "", p.Scope)
		psb.WriteString("(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?),")
		batchCount++
...
//     to independent verification by Mobius (verifying), or has reached a terminal
...
//     but is still subject to independent verification by Mobius (verifying)
//     or the operation has been completed and independenly verified by Mobius (verified).
//
...
func (ds *Datastore) ListMDMAppleProfilesToInstall(ctx context.Context) ([]*mobius.MDMAppleProfilePayload, error) {
	query := fmt.Sprintf(`
	SELECT
		ds.profile_uuid,
		ds.host_uuid,
	...
	var profiles []*mobius.MDMAppleProfilePayload
	err := sqlx.SelectContext(ctx, ds.reader(ctx), &profiles, query, mobius.MDMOperationTypeRemove, mobius.MDMOperationTypeInstall)
	return profiles, err
}

func (ds *Datastore) ListMDMAppleProfilesToRemove(ctx context.Context) ([]*mobius.MDMAppleProfilePayload, error) {
	...
	var profiles []*mobius.MDMAppleProfilePayload
	err := sqlx.SelectContext(ctx, ds.reader(ctx), &profiles, query, mobius.MDMOperationTypeRemove)
...
func (ds *Datastore) BulkUpsertMDMAppleHostProfiles(ctx context.Context, payload []*mobius.MDMAppleBulkUpsertHostProfilePayload) error {
	if len(payload) == 0 {
		return nil
	}
	...
	executeUpsertBatch := func(valuePart string, args []any) error {
	    ...
	    INSERT INTO host_mdm_apple_profiles (
            ...
            ON DUPLICATE KEY UPDATE
			  ...
			  scope = VALUES(scope)`,
			strings.TrimSuffix(valuePart, ","), mobius.MDMOperationTypeRemove,
		)
	...
	generateValueArgs := func(p *mobius.MDMAppleBulkUpsertHostProfilePayload) (string, []any) {
		valuePart := "(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?),"
		args := []any{
...
func (ds *Datastore) UpdateOrDeleteHostMDMAppleProfile(ctx context.Context, profile *mobius.HostMDMAppleProfile) error {
	if profile.OperationType == mobius.MDMOperationTypeRemove && profile.Status != nil {
		var ignoreError bool
		if *profile.Status == mobius.MDMDeliveryFailed {
		...
		if ignoreError ||
			(*profile.Status == mobius.MDMDeliveryVerifying || *profile.Status == mobius.MDMDeliveryVerified) {
			_, err := ds.writer(ctx).ExecContext(ctx, `
	...
	if profile.OperationType == mobius.MDMOperationTypeRemove && profile.Status != nil && *profile.Status == mobius.MDMDeliveryFailed {
	...
	if profile.OperationType == mobius.MDMOperationTypeInstall && profile.Status != nil && *profile.Status == mobius.MDMDeliveryVerifying {
...
func sqlCaseMDMAppleStatus() string {
	...
	var (
		failed    = fmt.Sprintf("'%s'", string(mobius.MDMDeliveryFailed))
		pending   = fmt.Sprintf("'%s'", string(mobius.MDMDeliveryPending))
		verifying = fmt.Sprintf("'%s'", string(mobius.MDMDeliveryVerifying))
		verified  = fmt.Sprintf("'%s'", string(mobius.MDMDeliveryVerified))
	)
	return `
...
func sqlJoinMDMAppleProfilesStatus() string {
	...
	var (
		...
		verifying = fmt.Sprintf("'%s'", string(mobius.MDMDeliveryVerifying))
		verified  = fmt.Sprintf("'%s'", string(mobius.MDMDeliveryVerified))
		install   = fmt.Sprintf("'%s'", string(mobius.MDMOperationTypeInstall))
		filevault = fmt.Sprintf("'%s'", mobileconfig.MobiusFileVaultPayloadIdentifier)
	)
	return `
	LEFT JOIN (
...
func sqlJoinMDMAppleDeclarationsStatus() string {
	...
	var (
		...
		verified          = fmt.Sprintf("'%s'", string(mobius.MDMDeliveryVerified))
		install           = fmt.Sprintf("'%s'", string(mobius.MDMOperationTypeInstall))
		reservedDeclNames = fmt.Sprintf("'%s', '%s', '%s'", mobiusmdm.MobiusMacOSUpdatesProfileName, mobiusmdm.MobiusIOSUpdatesProfileName, mobiusmdm.MobiusIPadOSUpdatesProfileName)
	)
	return `
	LEFT JOIN (
...
func (ds *Datastore) GetMDMAppleProfilesSummary(ctx context.Context, teamID *uint) (*mobius.MDMProfilesSummary, error) {
	stmt := `
SELECT
	COUNT(id) AS count,
	%s AS status
...
GROUP BY
	...
	var res mobius.MDMProfilesSummary
	for s, c := range byStatus {
		switch mobius.MDMDeliveryStatus(s) {
		case mobius.MDMDeliveryFailed:
			res.Failed = c
		case mobius.MDMDeliveryPending:
			res.Pending = c
		case mobius.MDMDeliveryVerifying:
			res.Verifying = c
		case mobius.MDMDeliveryVerified:
			res.Verified = c
		default:
...
func (ds *Datastore) InsertMDMIdPAccount(ctx context.Context, account *mobius.MDMIdPAccount) error {
	stmt := `
...
func (ds *Datastore) GetMDMIdPAccountByEmail(ctx context.Context, email string) (*mobius.MDMIdPAccount, error) {
	stmt := `SELECT uuid, username, fullname, email FROM mdm_idp_accounts WHERE email = ?`
	var acct mobius.MDMIdPAccount
...
func (ds *Datastore) GetMDMIdPAccountByUUID(ctx context.Context, uuid string) (*mobius.MDMIdPAccount, error) {
...
func subqueryFileVaultVerifying() (string, []interface{}) {
	...
	args := []interface{}{
		mobileconfig.MobiusFileVaultPayloadIdentifier,
...
func subqueryFileVaultFailed() (string, []interface{}) {
	sql := `
            ...
            WHERE
                ...
                AND hmap.profile_identifier = ?
                AND hmap.status = ?`
	args := []interface{}{mobileconfig.MobiusFileVaultPayloadIdentifier, mobius.MDMDeliveryFailed}
	return sql, args
}
...
func subqueryFileVaultRemovingEnforcement() (string, []interface{}) {
	sql := `
            ...
            WHERE
                ...
                AND (hmap.status IS NULL OR hmap.status = ?)
                AND hmap.operation_type = ?`
	args := []interface{}{mobileconfig.MobiusFileVaultPayloadIdentifier, mobius.MDMDeliveryPending, mobius.MDMOperationTypeRemove}
	return sql, args
}

func (ds *Datastore) GetMDMAppleFileVaultSummary(ctx context.Context, teamID *uint) (*mobius.MDMAppleFileVaultSummary, error) {
	sqlFmt := `
SELECT
    COUNT(
        CASE WHEN EXISTS (%s)
...
WHERE
	...
	var res mobius.MDMAppleFileVaultSummary
...
// NOTE: this method is only used internally to upsert Mobius-controlled
// profiles (called in service.ensureMobiusProfiles with the Mobius configuration
// profiles). Those are known not to use any Mobius variables. This method must
// not be used for any profile that uses Mobius variables.
func (ds *Datastore) BulkUpsertMDMAppleConfigProfiles(ctx context.Context, payload []*mobius.MDMAppleConfigProfile) error {
	if len(payload) == 0 {
		return nil
	}
...
func (ds *Datastore) InsertMDMAppleBootstrapPackage(ctx context.Context, bp *mobius.MDMAppleBootstrapPackage, pkgStore mobius.MDMBootstrapPackageStore) error {
...
func (ds *Datastore) CopyDefaultMDMAppleBootstrapPackage(ctx context.Context, ac *mobius.AppConfig, toTeamID uint) error {
	if ac == nil {
...
`
		...
		// mobiuscli in a gitops workflow).
...
func (ds *Datastore) GetMDMAppleBootstrapPackageBytes(ctx context.Context, token string, pkgStore mobius.MDMBootstrapPackageStore) (*mobius.MDMAppleBootstrapPackage, error) {
	const stmt = `SELECT name, bytes, sha256 FROM mdm_apple_bootstrap_packages WHERE token = ?`

	var bp mobius.MDMAppleBootstrapPackage
...
func (ds *Datastore) GetMDMAppleBootstrapPackageSummary(ctx context.Context, teamID uint) (*mobius.MDMAppleBootstrapPackageSummary, error) {
	...
	// assigned to Mobius in ABM but still manually enroll. We should probably keep using host_mdm,
	...
	var bp mobius.MDMAppleBootstrapPackageSummary
...
func (ds *Datastore) GetHostMDMMacOSSetup(ctx context.Context, hostID uint) (*mobius.HostMDMMacOSSetup, error) {
	// NOTE: Consider joining on host_dep_assignments instead of host_mdm so DEP hosts that
	// manually enroll or re-enroll are included in the results so long as they are not unassigned
	// in Apple Business Manager. The problem with using host_dep_assignments is that a host can be
	// assigned to Mobius in ABM but still manually enroll. We should probably keep using host_mdm,
...
WHERE
	...
	args := []interface{}{mobius.MDMBootstrapPackageInstalled, mobius.MDMBootstrapPackageFailed, mobius.MDMBootstrapPackagePending, hostID}

	var dest mobius.HostMDMMacOSSetup
	...
	if dest.BootstrapPackageStatus == mobius.MDMBootstrapPackageFailed {
...
func (ds *Datastore) GetMDMAppleBootstrapPackageMeta(ctx context.Context, teamID uint) (*mobius.MDMAppleBootstrapPackage, error) {
...
func (ds *Datastore) CleanupUnusedBootstrapPackages(ctx context.Context, pkgStore mobius.MDMBootstrapPackageStore, removeCreatedBefore time.Time) error {
	if pkgStore == nil {
...
func getMDMAppleConfigProfileByTeamAndIdentifierDB(ctx context.Context, tx sqlx.QueryerContext, teamID *uint, profileIdentifier string) (*mobius.MDMAppleConfigProfile, error) {
	if teamID == nil {
		teamID = ptr.Uint(0)
	}
...
WHERE
	...
	var profile mobius.MDMAppleConfigProfile
	err := sqlx.GetContext(ctx, tx, &profile, stmt, teamID, profileIdentifier)
	if err != nil {
		if err == sql.ErrNoRows {
			return &mobius.MDMAppleConfigProfile{}, ctxerr.Wrap(ctx, notFound("MDMAppleConfigProfile").WithName(profileIdentifier))
		}
		return &mobius.MDMAppleConfigProfile{}, ctxerr.Wrap(ctx, err, "get mdm apple config profile by team and identifier")
	}
	return &profile, nil
}

func (ds *Datastore) SetOrUpdateMDMAppleSetupAssistant(ctx context.Context, asst *mobius.MDMAppleSetupAssistant) (*mobius.MDMAppleSetupAssistant, error) {
	const stmt = `
		INSERT INTO
...
func (ds *Datastore) GetMDMAppleSetupAssistant(ctx context.Context, teamID *uint) (*mobius.MDMAppleSetupAssistant, error) {
	return ds.getMDMAppleSetupAssistant(ctx, ds.reader(ctx), teamID)
}

func (ds *Datastore) getMDMAppleSetupAssistant(ctx context.Context, q sqlx.QueryerContext, teamID *uint) (*mobius.MDMAppleSetupAssistant, error) {
	const stmt = `
	SELECT
		id,
		team_id,
	...
	var asst mobius.MDMAppleSetupAssistant
	var globalOrTmID uint
	if teamID != nil {
		globalOrTmID = *teamID
	}
...
func (ds *Datastore) updateHostDEPAssignProfileResponses(ctx context.Context, payload *godep.ProfileResponse, abmTokenID *uint) error {
	...
	for serial, status := range payload.Devices {
		switch status {
		case string(mobius.DEPAssignProfileResponseSuccess):
			success = append(success, serial)
		case string(mobius.DEPAssignProfileResponseNotAccessible):
			notAccessible = append(notAccessible, serial)
		case string(mobius.DEPAssignProfileResponseFailed):
	...
	return ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		if err := updateHostDEPAssignProfileResponses(ctx, tx, ds.logger, payload.ProfileUUID, success,
			string(mobius.DEPAssignProfileResponseSuccess), abmTokenID); err != nil {
			return err
		}
		if err := updateHostDEPAssignProfileResponses(ctx, tx, ds.logger, payload.ProfileUUID, notAccessible,
			string(mobius.DEPAssignProfileResponseNotAccessible), abmTokenID); err != nil {
			return err
		}
		if err := updateHostDEPAssignProfileResponses(ctx, tx, ds.logger, payload.ProfileUUID, failed,
			string(mobius.DEPAssignProfileResponseFailed), abmTokenID); err != nil {
			return err
		}
		return nil
	})
...
`
	...
	stmt, args, err := sqlx.In(stmt, string(mobius.DEPAssignProfileResponseFailed), depCooldownPeriod.Seconds(), serials)
	if err != nil {
...
WHERE
	...
	if err := sqlx.SelectContext(ctx, ds.reader(ctx), &rows, stmt, string(mobius.DEPAssignProfileResponseFailed), string(mobius.JobStateFailure), depCooldownPeriod.Seconds()); err != nil {
...
func (ds *Datastore) MDMResetEnrollment(ctx context.Context, hostUUID string, scepRenewalInProgress bool) error {
	return ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		...
		// stage if the host is enrolling with Mobius, SCEP renewals are
...
func (ds *Datastore) batchSetMDMAppleDeclarations(ctx context.Context, tx sqlx.ExtContext, tmID *uint,
	incomingDeclarations []*mobius.MDMAppleDeclaration,
) (updatedDB bool, err error) {
	...
	incomingDeclarationsMap := make(map[string]*mobius.MDMAppleDeclaration, len(incomingDeclarations))
	...
	keepNames := make([]string, 0, len(existingDecls)+len(mobiusmdm.ListMobiusReservedMacOSDeclarationNames()))
	for _, p := range existingDecls {
		if newP := incomingDeclarationsMap[p.Name]; newP != nil {
			keepNames = append(keepNames, p.Name)
		}
	}
	keepNames = append(keepNames, mobiusmdm.ListMobiusReservedMacOSDeclarationNames()...)
...
func (ds *Datastore) updateDeclarationsLabelAssociations(ctx context.Context, tx sqlx.ExtContext,
	incomingDeclarationsMap map[string]*mobius.MDMAppleDeclaration, teamID uint,
) (updatedDB bool, err error) {
	var incomingLabels []mobius.ConfigurationProfileLabel
...
func (ds *Datastore) insertOrUpdateDeclarations(ctx context.Context, tx sqlx.ExtContext, incomingDeclarations []*mobius.MDMAppleDeclaration,
	teamID uint,
) (updatedDB bool, err error) {
	const insertStmt = `
...
`
	...
	for _, d := range incomingDeclarations {
		declUUID := mobius.MDMAppleDeclarationUUIDPrefix + uuid.NewString()
		var result sql.Result
...
WHERE
  ...
  name IN (?)
`
	var existingDecls []*mobius.MDMAppleDeclaration
...
func (ds *Datastore) NewMDMAppleDeclaration(ctx context.Context, declaration *mobius.MDMAppleDeclaration) (*mobius.MDMAppleDeclaration, error) {
	const stmt = `
INSERT INTO mdm_apple_declarations (
	declaration_uuid,
	team_id,
...
func (ds *Datastore) SetOrUpdateMDMAppleDeclaration(ctx context.Context, declaration *mobius.MDMAppleDeclaration) (*mobius.MDMAppleDeclaration, error) {
	const stmt = `
INSERT INTO mdm_apple_declarations (
	declaration_uuid,
	team_id,
...
func (ds *Datastore) insertOrUpsertMDMAppleDeclaration(ctx context.Context, insOrUpsertStmt string, declaration *mobius.MDMAppleDeclaration) (*mobius.MDMAppleDeclaration, error) {
	declUUID := mobius.MDMAppleDeclarationUUIDPrefix + uuid.NewString()
	...
	err := ds.withTx(ctx, func(tx sqlx.ExtContext) error {
		...
		labels := make([]mobius.ConfigurationProfileLabel, 0,
...
func batchSetDeclarationLabelAssociationsDB(ctx context.Context, tx sqlx.ExtContext,
	declarationLabels []mobius.ConfigurationProfileLabel, declUUIDsWithoutLabels []string,
) (updatedDB bool, err error) {
	...
	var (
		...
		labelsToInsert  = make(map[string]*mobius.ConfigurationProfileLabel, len(declarationLabels))
	)
	...
	var existingProfileLabels []mobius.ConfigurationProfileLabel
	...
	if len(existingProfileLabels) == len(labelsToInsert) {
		for _, existing := range existingProfileLabels {
			...
			// The mobius.ConfigurationProfileLabel struct has no pointers, so we can use standard cmp.Equal
...
func (ds *Datastore) MDMAppleDDMDeclarationsToken(ctx context.Context, hostUUID string) (*mobius.MDMAppleDDMDeclarationsToken, error) {
	const stmt = `
SELECT
...
WHERE
	...
	var res mobius.MDMAppleDDMDeclarationsToken
	if err := sqlx.GetContext(ctx, ds.reader(ctx), &res, stmt, hostUUID, mobius.MDMOperationTypeInstall); err != nil {
...
func (ds *Datastore) MDMAppleDDMDeclarationItems(ctx context.Context, hostUUID string) ([]mobius.MDMAppleDDMDeclarationItem, error) {
	const stmt = `
SELECT
...
WHERE
	...
	var res []mobius.MDMAppleDDMDeclarationItem
...
func (ds *Datastore) MDMAppleDDMDeclarationsResponse(ctx context.Context, identifier string, hostUUID string) (*mobius.MDMAppleDeclaration, error) {
...
WHERE
	...
	var res mobius.MDMAppleDeclaration
	if err := sqlx.GetContext(ctx, ds.reader(ctx), &res, stmt, hostUUID, identifier, mobius.MDMOperationTypeInstall); err != nil {
		if err == sql.ErrNoRows {
...
func (ds *Datastore) MDMAppleBatchSetHostDeclarationState(ctx context.Context) ([]string, error) {
	...
	err := ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		var err error
		uuids, _, err = mdmAppleBatchSetHostDeclarationStateDB(ctx, tx, batchSize, &mobius.MDMDeliveryPending)
		return err
	})
...
func mdmAppleBatchSetPendingHostDeclarationsDB(
	...
	tx sqlx.ExtContext,
	batchSize int,
	changedDeclarations []*mobius.MDMAppleHostDeclaration,
	status *mobius.MDMDeliveryStatus,
) (updatedDB bool, err error) {
	baseStmt := `
	...
	profilesToInsert := make(map[string]*mobius.MDMAppleHostDeclaration)
	...
	executeUpsertBatch := func(valuePart string, args []any) error {
		...
		var existingProfiles []mobius.MDMAppleHostDeclaration
	...
	generateValueArgs := func(d *mobius.MDMAppleHostDeclaration) (string, []any) {
		profilesToInsert[fmt.Sprintf("%s\n%s", d.HostUUID, d.DeclarationUUID)] = &mobius.MDMAppleHostDeclaration{
			HostUUID:         d.HostUUID,
...
func cleanUpDuplicateRemoveInstall(ctx context.Context, tx sqlx.ExtContext, profilesToInsert map[string]*mobius.MDMAppleHostDeclaration) error {
	...
	findRemoveProfilesArgs = append(findRemoveProfilesArgs, mobius.MDMOperationTypeRemove, mobius.MDMDeliveryPending)
	...
	if len(tokensToMarkVerified) > 0 {
		...
		markVerifiedArgs = append(markVerifiedArgs, mobius.MDMDeliveryVerified)
		...
		removeProfilesArgs = append(removeProfilesArgs, mobius.MDMOperationTypeRemove, mobius.MDMDeliveryPending)
		...
		markInstallProfilesVerified := fmt.Sprintf(`
			...
			AND operation_type = ?
			`, strings.TrimSuffix(strings.Repeat("(?,?),", len(tokensToMarkVerified)), ","))
		markVerifiedArgs = append(markVerifiedArgs, mobius.MDMOperationTypeInstall)
...
func mdmAppleGetHostsWithChangedDeclarationsDB(ctx context.Context, tx sqlx.ExtContext) ([]*mobius.MDMAppleHostDeclaration, error) {
	stmt := fmt.Sprintf(`
        (
            SELECT
                ds.host_uuid,
	...
	var decls []*mobius.MDMAppleHostDeclaration
	if err := sqlx.SelectContext(ctx, tx, &decls, stmt, mobius.MDMOperationTypeRemove, mobius.MDMOperationTypeInstall, mobius.MDMOperationTypeRemove); err != nil {
...
func (ds *Datastore) MDMAppleStoreDDMStatusReport(ctx context.Context, hostUUID string, updates []*mobius.MDMAppleHostDeclaration) error {
...
ON DUPLICATE KEY UPDATE
  ...
  `
	...
	var current []*mobius.MDMAppleHostDeclaration
	...
	updatesByToken := make(map[string]*mobius.MDMAppleHostDeclaration, len(updates))
	...
	for _, c := range current {
		...
		if u, ok := updatesByToken[c.Token]; ok && u.OperationType != mobius.MDMOperationTypeRemove {
...
func (ds *Datastore) MDMAppleSetPendingDeclarationsAs(ctx context.Context, hostUUID string, status *mobius.MDMDeliveryStatus, detail string) error {
	stmt := `
  UPDATE host_mdm_apple_declarations
  SET
    status = ?,
  ...
  `
	...
	_, err := ds.writer(ctx).ExecContext(
		...
		status, detail,
		// WHERE ...
		mobius.MDMOperationTypeInstall, mobius.MDMDeliveryPending, hostUUID,
	)
...
func (ds *Datastore) MDMAppleSetRemoveDeclarationsAsPending(ctx context.Context, hostUUID string, declarationUUIDs []string) error {
  ...
  `
	...
	stmt, args, err := sqlx.In(stmt, mobius.MDMDeliveryPending, hostUUID, declarationUUIDs, mobius.MDMOperationTypeRemove)
	if err != nil {
...
func (ds *Datastore) InsertMDMConfigAssets(ctx context.Context, assets []mobius.MDMConfigAsset, tx sqlx.ExtContext) error {
...
func (ds *Datastore) GetAllMDMConfigAssetsByName(ctx context.Context, assetNames []mobius.MDMAssetName,
...
WHERE
	...
	assetMap := make(map[mobius.MDMAssetName]mobius.MDMConfigAsset, len(res))
	for _, asset := range res {
		...
		assetMap[asset.Name] = mobius.MDMConfigAsset{Name: asset.Name, Value: decryptedVal}
	}
...
func (ds *Datastore) GetAllMDMConfigAssetsHashes(ctx context.Context, assetNames []mobius.MDMAssetName) (map[mobius.MDMAssetName]string, error) {
	if len(assetNames) == 0 {
		return nil, nil
	}
...
WHERE name IN (?) AND deletion_uuid = ''`
	...
	assetMap := make(map[mobius.MDMAssetName]string, len(res))
	for _, asset := range res {
...
func (ds *Datastore) DeleteMDMConfigAssetsByName(ctx context.Context, assetNames []mobius.MDMAssetName) error {
	return ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
...
func (ds *Datastore) HardDeleteMDMConfigAsset(ctx context.Context, assetName mobius.MDMAssetName) error {
	stmt := `
...
func softDeleteMDMConfigAssetsByName(ctx context.Context, tx sqlx.ExtContext, assetNames []mobius.MDMAssetName) error {
	stmt := `
UPDATE
    mdm_config_assets
SET
...
func insertMDMConfigAssets(ctx context.Context, tx sqlx.ExtContext, assets []mobius.MDMConfigAsset, privateKey string) error {
	stmt := `
...
func (ds *Datastore) ReplaceMDMConfigAssets(ctx context.Context, assets []mobius.MDMConfigAsset, tx sqlx.ExtContext) error {
	replaceFunc := func(tx sqlx.ExtContext) error {
		var names []mobius.MDMAssetName
		for _, a := range assets {
...
func (ds *Datastore) ListIOSAndIPadOSToRefetch(ctx context.Context, interval time.Duration) (devices []mobius.AppleDevicesToRefetch,
	err error,
) {
	hostsStmt := `
...
func (ds *Datastore) GetABMTokenByOrgName(ctx context.Context, orgName string) (*mobius.ABMToken, error) {
...
func (ds *Datastore) SaveABMToken(ctx context.Context, tok *mobius.ABMToken) error {
	const stmt = `
UPDATE
	abm_tokens
SET
...
func (ds *Datastore) InsertABMToken(ctx context.Context, tok *mobius.ABMToken) (*mobius.ABMToken, error) {
	const stmt = `
INSERT INTO
	abm_tokens
...
func (ds *Datastore) ListABMTokens(ctx context.Context) ([]*mobius.ABMToken, error) {
	stmt := `
SELECT
	abt.id,
	abt.organization_name,
...
LEFT OUTER JOIN
	...
	stmt, args, err := sqlx.Named(stmt, map[string]any{"no_team": mobius.TeamNameNoTeam})
	if err != nil {
		return nil, ctxerr.Wrap(ctx, err, "build list ABM tokens query from named args")
	}

	var tokens []*mobius.ABMToken
	...
	for _, tok := range tokens {
		...
		tok.MacOSTeam = mobius.ABMTokenTeam{Name: tok.MacOSTeamName, ID: macOSTeamID}
		tok.IOSTeam = mobius.ABMTokenTeam{Name: tok.IOSTeamName, ID: iOSTeamID}
		tok.IPadOSTeam = mobius.ABMTokenTeam{Name: tok.IPadOSTeamName, ID: iPadIOSTeamID}
...
func (ds *Datastore) GetABMTokenByID(ctx context.Context, tokenID uint) (*mobius.ABMToken, error) {
...
func (ds *Datastore) getABMToken(ctx context.Context, tokenID uint, orgName string) (*mobius.ABMToken, error) {
	stmt := `
SELECT
	abt.id,
	abt.organization_name,
...
func (ds *Datastore) AddHostMDMCommands(ctx context.Context, commands []mobius.HostMDMCommand) error {
	const baseStmt = `
...
func (ds *Datastore) GetHostMDMCommands(ctx context.Context, hostID uint) (commands []mobius.HostMDMCommand, err error) {
...
func (ds *Datastore) RemoveHostMDMCommand(ctx context.Context, command mobius.HostMDMCommand) error {
	const stmt = `
		DELETE FROM host_mdm_commands
...
func (ds *Datastore) GetMDMAppleOSUpdatesSettingsByHostSerial(ctx context.Context, serial string) (*mobius.AppleOSUpdateSettings, error) {
	stmt := `
SELECT
	team_id, platform
FROM
...
func (ds *Datastore) BulkUpsertMDMManagedCertificates(ctx context.Context, payload []*mobius.MDMManagedCertificate) error {
	if len(payload) == 0 {
		return nil
	}
	...
	generateValueArgs := func(p *mobius.MDMManagedCertificate) (string, []any) {
		valuePart := "(?, ?, ?, ?, ?, ?, ?, ?),"
...
func (ds *Datastore) ListHostMDMManagedCertificates(ctx context.Context, hostUUID string) ([]*mobius.MDMManagedCertificate, error) {
	hostCertsToRenew := []*mobius.MDMManagedCertificate{}
	err := sqlx.SelectContext(ctx, ds.reader(ctx), &hostCertsToRenew, `
...
func (ds *Datastore) GetMDMAppleEnrolledDeviceDeletedFromMobius(ctx context.Context, hostUUID string) (*mobius.MDMAppleEnrolledDeviceInfo, error) {
	const stmt = `
SELECT
	d.id,
...
`
	...
	var res mobius.MDMAppleEnrolledDeviceInfo
...
func (ds *Datastore) ListMDMAppleEnrolledIPhoneIpadDeletedFromMobius(ctx context.Context, limit int) ([]string, error) {
	const stmt = `
SELECT
	d.id
FROM
...
func (ds *Datastore) ReconcileMDMAppleEnrollRef(ctx context.Context, enrollRef string, machineInfo *mobius.MDMAppleMachineInfo) (string, error) {
	if machineInfo == nil {
...
func reconcileHostEmailsFromMdmIdpAccountsDB(ctx context.Context, tx sqlx.ExtContext, logger log.Logger, hostID uint) (*mobius.MDMIdPAccount, error) {
	idp, err := getMDMIdPAccountByHostID(ctx, tx, logger, hostID)
	if err != nil {
		return nil, ctxerr.Wrap(ctx, err, "get host mdm idp account email")
	}

	var hostEmails []mobius.HostDeviceMapping
	selectStmt := `SELECT id, host_id, email, source FROM host_emails WHERE host_id = ? AND source = ?`
	if err := sqlx.SelectContext(ctx, tx, &hostEmails, selectStmt, hostID, mobius.DeviceMappingMDMIdpAccounts); err != nil {
	...
	if idpEmail == "" {
		...
		if _, err := tx.ExecContext(ctx, delStmt, hostID, mobius.DeviceMappingMDMIdpAccounts); err != nil {
	...
	hits, misses := []mobius.HostDeviceMapping{}, []mobius.HostDeviceMapping{}
	...
	if _, err := tx.ExecContext(ctx, insStmt, idpEmail, hostID, mobius.DeviceMappingMDMIdpAccounts); err != nil {
...
func getMDMIdPAccountByHostID(ctx context.Context, q sqlx.QueryerContext, logger log.Logger, hostID uint) (*mobius.MDMIdPAccount, error) {
...
func (ds *Datastore) GetMDMIdPAccountByHostUUID(ctx context.Context, hostUUID string) (*mobius.MDMIdPAccount, error) {
	return getMDMIdPAccountByHostUUID(ctx, ds.reader(ctx), hostUUID)
}

func getMDMIdPAccountByHostUUID(ctx context.Context, q sqlx.QueryerContext, hostUUID string) (*mobius.MDMIdPAccount, error) {
	stmt := `
SELECT
	uuid, username, fullname, email
FROM
