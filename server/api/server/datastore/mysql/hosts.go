...
func (ds *Datastore) NewHost(ctx context.Context, host *mobius.Host) (*mobius.Host, error) {
...
func (ds *Datastore) SerialUpdateHost(ctx context.Context, host *mobius.Host) error {
...
func (ds *Datastore) SaveHostPackStats(ctx context.Context, teamID *uint, hostID uint, stats []mobius.PackStats) error {
	return saveHostPackStatsDB(ctx, ds.writer(ctx), teamID, hostID, stats)
}

func saveHostPackStatsDB(ctx context.Context, db *sqlx.DB, teamID *uint, hostID uint, stats []mobius.PackStats) error {
...
func loadHostPackStatsDB(ctx context.Context, db sqlx.QueryerContext, hid uint, hostPlatform string) ([]mobius.PackStats, error) {
	...
	ds := dialect.From(goqu.I("scheduled_queries").As("sq")).Select(
		...
		goqu.Or(
			...
			goqu.L("FIND_IN_SET(?, sq.platform)", mobius.PlatformFromHost(hostPlatform)).Neq(0),
		),
	)
...
func loadHostScheduledQueryStatsDB(ctx context.Context, db sqlx.QueryerContext, hid uint, hostPlatform string, teamID *uint) ([]mobius.QueryStats, error) {
	var teamID_ uint
	if teamID != nil {
		teamID_ = *teamID
	}
	...
	args := []interface{}{
		...
		hid,
		hid,
		mobius.PlatformFromHost(hostPlatform),
...
func loadHostUsersDB(ctx context.Context, db sqlx.QueryerContext, hostID uint) ([]mobius.HostUser, error) {
	sql := `SELECT username, groupname, uid, user_type, shell FROM host_users WHERE host_id = ? and removed_at IS NULL`
	var users []mobius.HostUser
...
func (ds *Datastore) ListHostUsers(ctx context.Context, hostID uint) ([]mobius.HostUser, error) {
...
// deleted from when a host is deleted in Mobius:
...
func deleteHosts(ctx context.Context, tx sqlx.ExtContext, hostIDs []uint) error {
	...
	stmt, args, err = sqlx.In(`DELETE FROM pack_targets WHERE type = ? AND target_id IN (?)`, mobius.TargetHost, hostIDs)
	if err != nil {
...
func (ds *Datastore) Host(ctx context.Context, id uint) (*mobius.Host, error) {
	sqlStatement := `
SELECT
  h.id,
  h.osquery_host_id,
  ...
  hoi.desktop_version AS mobius_desktop_version,
...
`
	...
	if host.DiskEncryptionEnabled != nil && !(*host.DiskEncryptionEnabled) && mobius.IsLinux(host.Platform) {
		...
		// https://github.com/MobiusDM/Mobius/issues/3906).
	...
	var (
		globalQueriesStats   []mobius.QueryStats
		hostTeamQueriesStats []mobius.QueryStats
	)
	...
	if len(globalQueriesStats) > 0 {
		host.PackStats = append(host.PackStats, mobius.PackStats{
			PackName:   "Global",
			Type:       "global",
...
func queryStatsToScheduledQueryStats(queriesStats []mobius.QueryStats, packName string) []mobius.ScheduledQueryStats {
	scheduledQueriesStats := make([]mobius.ScheduledQueryStats, 0, len(queriesStats))
	for _, queryStats := range queriesStats {
		scheduledQueriesStats = append(scheduledQueriesStats, mobius.ScheduledQueryStats{
...
const hostMDMSelect = `,
	JSON_OBJECT(
		...
		'dep_profile_error',
		CASE
			WHEN hdep.assign_profile_response = '` + string(mobius.DEPAssignProfileResponseFailed) + `' THEN CAST(TRUE AS JSON)
	...
	// query, please update the AreHostsConnectedToMobiusMDM
...
func amountEnrolledHostsByOSDB(ctx context.Context, db sqlx.QueryerContext) (byOS map[string][]mobius.HostsCountByOSVersion, totalCount int, err error) {
  ...
  `
	...
	byOS = make(map[string][]mobius.HostsCountByOSVersion)
	for _, h := range hostsByOS {
		totalCount += h.NumHosts
		byVersion := byOS[h.Platform]
		byVersion = append(byVersion, mobius.HostsCountByOSVersion{
			Version:     h.OSVersion,
...
func (ds *Datastore) ListHosts(ctx context.Context, filter mobius.TeamFilter, opt mobius.HostListOptions) ([]*mobius.Host, error) {
	sql := `SELECT
    h.id,
    h.osquery_host_id,
    h.created_at,
...
func (ds *Datastore) applyHostFilters(
	ctx context.Context, opt mobius.HostListOptions, sqlStmt string, filter mobius.TeamFilter, selectParams []interface{},
	...
	if softwareIDFilter != nil {
		...
		if opt.SoftwareStatusFilter != nil {
			...
			default:
				...
				// [1] https://github.com/MobiusDM/Mobius/blob/8aecae4d853829cb6e7f828099a4f0953643cf18/server/datastore/mysql/hosts.go#L1088-L1089
	...
	connectedToMobiusJoin := ""
	if opt.ConnectedToMobiusFilter != nil && *opt.ConnectedToMobiusFilter ||
	...
	if opt.BatchScriptExecutionIDFilter != nil {
		...
		if opt.BatchScriptExecutionStatusFilter.IsValid() {
			...
			case mobius.BatchScriptExecutionRan:
				batchScriptExecutionIDFilter += ` AND hsr.exit_code = 0`
			case mobius.BatchScriptExecutionPending:
			...
			case mobius.BatchScriptExecutionErrored:
			...
			case mobius.BatchScriptExecutionCancelled:
	...
	sqlStmt, whereParams = filterHostsByConnectedToMobius(sqlStmt, opt, whereParams)
	var err error
...
func filterHostsByTeam(sql string, opt mobius.HostListOptions, params []interface{}) (string, []interface{}) {
...
func filterHostsByMDM(sql string, opt mobius.HostListOptions, params []interface{}) (string, []interface{}) {
...
func filterHostsByConnectedToMobius(sql string, opt mobius.HostListOptions, params []any) (string, []any) {
	if opt.ConnectedToMobiusFilter != nil && *opt.ConnectedToMobiusFilter {
...
func filterHostsByOS(sql string, opt mobius.HostListOptions, params []interface{}) (string, []interface{}) {
...
func filterHostsByPolicy(sql string, opt mobius.HostListOptions, params []interface{}) (string, []interface{}) {
...
func filterHostsByStatus(now time.Time, sql string, opt mobius.HostListOptions, params []interface{}) (string, []interface{}) {
	switch opt.StatusFilter {
	case mobius.StatusNew:
		sql += "AND DATE_ADD(h.created_at, INTERVAL 1 DAY) >= ?"
		params = append(params, now)
	case mobius.StatusOnline:
		sql += fmt.Sprintf("AND DATE_ADD(COALESCE(hst.seen_time, h.created_at), INTERVAL LEAST(h.distributed_interval, h.config_tls_refresh) + %d SECOND) > ?", mobius.OnlineIntervalBuffer)
		params = append(params, now)
	case mobius.StatusOffline:
		sql += fmt.Sprintf("AND DATE_ADD(COALESCE(hst.seen_time, h.created_at), INTERVAL LEAST(h.distributed_interval, h.config_tls_refresh) + %d SECOND) <= ?", mobius.OnlineIntervalBuffer)
		params = append(params, now)
	case mobius.StatusMIA, mobius.StatusMissing:
...
func filterHostsByMacOSSettingsStatus(sql string, opt mobius.HostListOptions, params []any) (string, []any, error) {
...
func filterHostsByMacOSDiskEncryptionStatus(sql string, opt mobius.HostListOptions, params []interface{}) (string, []interface{}) {
	...
	case mobius.DiskEncryptionActionRequired:
		subquery, subqueryParams = subqueryFileVaultActionRequired()
	case mobius.DiskEncryptionEnforcing:
	...
	case mobius.DiskEncryptionRemovingEnforcement:
...
func (ds *Datastore) filterHostsByOSSettingsStatus(sql string, opt mobius.HostListOptions, params []interface{}, isDiskEncryptionEnabled bool) (string, []interface{}, error) {
	if !opt.OSSettingsFilter.IsValid() {
		return sql, params, nil
	}
	...
	// and adds extra where clauses to filter out Windows hosts that are not enrolled to Mobius MDM
	...
	if isDiskEncryptionEnabled {
		bitlockerStatus = fmt.Sprintf(`
            ...
            ELSE
                ''
            END`,
			ds.whereBitLockerStatus(mobius.DiskEncryptionVerified),
			ds.whereBitLockerStatus(mobius.DiskEncryptionVerifying),
			ds.whereBitLockerStatus(mobius.DiskEncryptionEnforcing),
			ds.whereBitLockerStatus(mobius.DiskEncryptionFailed),
		)
	}
...
func (ds *Datastore) filterHostsByOSSettingsDiskEncryptionStatus(sql string, opt mobius.HostListOptions, params []interface{}, enableDiskEncryption bool) (string, []interface{}) {
	...
	case mobius.DiskEncryptionVerified:
		if enableDiskEncryption {
			whereWindows = ds.whereBitLockerStatus(mobius.DiskEncryptionVerified)
		}
		subqueryMacOS, subqueryParams = subqueryFileVaultVerified()

	case mobius.DiskEncryptionVerifying:
		if enableDiskEncryption {
			whereWindows = ds.whereBitLockerStatus(mobius.DiskEncryptionVerifying)
		}
	...
	case mobius.DiskEncryptionEnforcing:
		if enableDiskEncryption {
			whereWindows = ds.whereBitLockerStatus(mobius.DiskEncryptionEnforcing)
		}
		subqueryMacOS, subqueryParams = subqueryFileVaultEnforcing()

	case mobius.DiskEncryptionFailed:
		if enableDiskEncryption {
			whereWindows = ds.whereBitLockerStatus(mobius.DiskEncryptionFailed)
		}
...
func filterHostsByMDMBootstrapPackageStatus(sql string, opt mobius.HostListOptions, params []interface{}) (string, []interface{}) {
	...
	case mobius.MDMBootstrapPackageFailed:
		subquery += ` AND ncr.status = 'Error'`
	case mobius.MDMBootstrapPackagePending:
		subquery += ` AND (ncr.status IS NULL OR (ncr.status != 'Acknowledged' AND ncr.status != 'Error'))`
	case mobius.MDMBootstrapPackageInstalled:
...
func filterHostsByVulnerability(sqlstmt string, opt mobius.HostListOptions, params []interface{}) (string, []interface{}) {
...
func filterHostsByProfileStatus(sqlstmt string, opt mobius.HostListOptions, params []interface{}) (string, []interface{}) {
	if opt.ProfileUUIDFilter != nil && opt.ProfileStatusFilter != nil {

		switch {
		case strings.HasPrefix(*opt.ProfileUUIDFilter, mobius.MDMAppleProfileUUIDPrefix):
			sqlstmt += ` AND EXISTS (
		SELECT
			1
		FROM
		...
		WHERE
			hmap.host_uuid = h.uuid
				...
				AND hmap.status = ?)`
		case strings.HasPrefix(*opt.ProfileUUIDFilter, mobius.MDMAppleDeclarationUUIDPrefix):
			sqlstmt += ` AND EXISTS (
		SELECT
			1
		FROM
		...
		WHERE
			had.host_uuid = h.uuid
				...
				AND had.status = ?)`
		case strings.HasPrefix(*opt.ProfileUUIDFilter, mobius.MDMWindowsProfileUUIDPrefix):
			sqlstmt += ` AND EXISTS (
		SELECT
			1
		FROM
...
func (ds *Datastore) CountHosts(ctx context.Context, filter mobius.TeamFilter, opt mobius.HostListOptions) (int, error) {
	sql := `SELECT count(*) `
...
func (ds *Datastore) GenerateHostStatusStatistics(ctx context.Context, filter mobius.TeamFilter, now time.Time, platform *string, lowDiskSpace *int) (*mobius.HostSummary, error) {
	...
	if platform != nil {
		...
		args = append(args, mobius.ExpandPlatform(*platform))
	}
	...
	sqlStatement := fmt.Sprintf(`
			...
			WHERE %s
			LIMIT 1;
		`, mobius.OnlineIntervalBuffer, mobius.OnlineIntervalBuffer, lowDiskSelect, hostDisksJoin, whereClause)

	stmt, args, err := sqlx.In(sqlStatement, args...)
	if err != nil {
		return nil, ctxerr.Wrap(ctx, err, "generating host statistics statement")
	}
	summary := mobius.HostSummary{TeamID: filter.TeamID}
	...
	var platforms []*mobius.HostSummaryPlatform
	stmt, args, err = sqlx.In(sqlStatement, args...)
	if err != nil {
...
// serial number lookup if Mobius MDM is enabled on the server (as we must be
		...
		default:
			...
			if enrolledHostInfo.NodeKeySet {
				...
				level.Warn(ds.logger).Log(
					"msg", "osquery host with duplicate identifier has enrolled in Mobius and will overwrite existing host data",
...
func (ds *Datastore) getContextTryStmt(ctx context.Context, dest interface{}, query string, args ...interface{}) error {
	...
	if stmt := ds.loadOrPrepareStmt(ctx, query); stmt != nil {
		...
		//
		// - see https://github.com/MobiusDM/Mobius/issues/20781 for an
...
func (ds *Datastore) LoadHostByNodeKey(ctx context.Context, nodeKey string) (*mobius.Host, error) {
	query := `
    SELECT
      h.id,
      h.osquery_host_id,
...
// If the node key is invalid it returns a NotFoundError.
func (ds *Datastore) LoadHostByOrbitNodeKey(ctx context.Context, nodeKey string) (*mobius.Host, error) {
	query := `
    SELECT
      h.id,
      h.osquery_host_id,
...
func (ds *Datastore) LoadHostByDeviceAuthToken(ctx context.Context, authToken string, tokenTTL time.Duration) (*mobius.Host, error) {
	const query = `
    SELECT
      h.id,
      h.osquery_host_id,
...
`
	...
	if err != nil {
		if IsDuplicate(err) {
			return mobius.ConflictError{Message: "auth token conflicts with another host"}
		}
...
func (ds *Datastore) SearchHosts(ctx context.Context, filter mobius.TeamFilter, matchQuery string, omit ...uint) ([]*mobius.Host, error) {
	query := `SELECT
    h.id,
    h.osquery_host_id,
    h.created_at,
...
func (ds *Datastore) HostIDsByIdentifier(ctx context.Context, filter mobius.TeamFilter, hostIdentifiers []string) ([]uint, error) {
...
func (ds *Datastore) ListHostsLiteByUUIDs(ctx context.Context, filter mobius.TeamFilter, uuids []string) ([]*mobius.Host, error) {
	if len(uuids) == 0 {
		return nil, nil
	}
...
func (ds *Datastore) ListHostsLiteByIDs(ctx context.Context, ids []uint) ([]*mobius.Host, error) {
	if len(ids) == 0 {
		return nil, nil
	}
...
func (ds *Datastore) HostByIdentifier(ctx context.Context, identifier string) (*mobius.Host, error) {
	stmt := `
    SELECT
      h.id,
      h.osquery_host_id,
...
func (ds *Datastore) SaveHostUsers(ctx context.Context, hostID uint, users []mobius.HostUser) error {
	return ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		return saveHostUsersDB(ctx, tx, hostID, users)
	})
}

func saveHostUsersDB(ctx context.Context, tx sqlx.ExtContext, hostID uint, users []mobius.HostUser) error {
	currentHostUsers, err := loadHostUsersDB(ctx, tx, hostID)
	if err != nil {
		return err
	}

	keyForUser := func(u *mobius.HostUser) string { return fmt.Sprintf("%d\x00%s", u.Uid, u.Username) }
...
func (ds *Datastore) FailingPoliciesCount(ctx context.Context, host *mobius.Host) (uint, error) {
	if host.MobiusPlatform() == "" {
...
func (ds *Datastore) ListPoliciesForHost(ctx context.Context, host *mobius.Host) ([]*mobius.HostPolicy, error) {
	if host.MobiusPlatform() == "" {
		// We log to help troubleshooting in case this happens.
		level.Error(ds.logger).Log("err", "unrecognized platform", "hostID", host.ID, "platform", host.Platform) //nolint:errcheck
	}
	...
	var policies []*mobius.HostPolicy
	if err := sqlx.SelectContext(ctx, ds.reader(ctx), &policies, query, host.ID, host.ID, host.MobiusPlatform(), host.ID, host.ID); err != nil {
...
func (ds *Datastore) ListHostDeviceMapping(ctx context.Context, id uint) ([]*mobius.HostDeviceMapping, error) {
...
func deviceMappingTranslateSourceColumn(hostEmailsTableAlias string) string {
	...
	return fmt.Sprintf(` CASE WHEN %ssource LIKE '%s%%' THEN '%s' ELSE %[1]ssource END `,
		hostEmailsTableAlias, mobius.DeviceMappingCustomPrefix, mobius.DeviceMappingCustomReplacement)
}

func (ds *Datastore) listHostDeviceMappingDB(ctx context.Context, q sqlx.QueryerContext, hostID uint) ([]*mobius.HostDeviceMapping, error) {
	stmt := fmt.Sprintf(`
    SELECT
      id,
      host_id,
...
func (ds *Datastore) ReplaceHostDeviceMapping(ctx context.Context, hid uint, mappings []*mobius.HostDeviceMapping, source string) error {
	...
	return ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		...
		toIns := make(map[string]*mobius.HostDeviceMapping)
		for _, m := range mappings {
			toIns[m.Email+"\n"+m.Source] = m
		}

		var prevMappings []*mobius.HostDeviceMapping
...
func (ds *Datastore) SetOrUpdateCustomHostDeviceMapping(ctx context.Context, hostID uint, email, source string) ([]*mobius.HostDeviceMapping, error) {
	const (
	...
	err := ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		if source == mobius.DeviceMappingCustomOverride {
			// must delete any existing custom installer
			if _, err := tx.ExecContext(ctx, delStmt, hostID, mobius.DeviceMappingCustomInstaller); err != nil {
		...
		if rowsAffected == 0 {
			...
			if source == mobius.DeviceMappingCustomInstaller {
				// custom installer can only insert if there's no custom override
				stmt = insInstallerStmt
				params = append(params, hostID, mobius.DeviceMappingCustomOverride)
			}
...
func (ds *Datastore) ReplaceHostBatteries(ctx context.Context, hid uint, mappings []*mobius.HostBattery) error {
	for _, m := range mappings {
		if hid != m.HostID {
	...
	keyFn := func(b *mobius.HostBattery) string {
	...
	return ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		...
		// need to be deleted and inserted.
		toIns := make(map[string]*mobius.HostBattery)
		...
		var prevMappings []*mobius.HostBattery
...
func (ds *Datastore) SetOrUpdateMunkiInfo(ctx context.Context, hostID uint, version string, errors, warnings []string) error {
	...
	msgToID, err := ds.getOrInsertMunkiIssues(ctx, errors, warnings, mobius.DefaultMunkiIssuesBatchSize)
	if err != nil {
		return err
	}
...
func (ds *Datastore) SetOrUpdateMDMData(
	...
	return ds.updateOrInsert(
		ctx,
		`UPDATE host_mdm SET enrolled = ?, server_url = ?, installed_from_dep = ?, mdm_id = ?, is_server = ?, mobius_enroll_ref = ? WHERE host_id = ?`,
		`INSERT INTO host_mdm (enrolled, server_url, installed_from_dep, mdm_id, is_server, mobius_enroll_ref, host_id) VALUES (?, ?, ?, ?, ?, ?, ?)`,
		enrolled, serverURL, installedFromDep, mdmID, isServer, mobiusEnrollmentRef, hostID,
	)
}
...
func maybeAssociateHostMDMIdPWithScimUser(ctx context.Context, tx sqlx.ExtContext, logger log.Logger, hostID uint, idp *mobius.MDMIdPAccount) error {
	if idp == nil {
	...
	switch {
	case err != nil && !mobius.IsNotFound(err):
...
func (ds *Datastore) GetHostMDM(ctx context.Context, hostID uint) (*mobius.HostMDM, error) {
	...
	err := sqlx.GetContext(ctx, ds.reader(ctx), &hmdm, `
		...
		WHERE hm.host_id = ?`, mobius.UnknownMDMName, hostID)
	if err != nil {
		if err == sql.ErrNoRows {
...
func (ds *Datastore) GetHostMDMCheckinInfo(ctx context.Context, hostUUID string) (*mobius.HostMDMCheckinInfo, error) {
	var hmdm mobius.HostMDMCheckinInfo
	...
	err := sqlx.GetContext(ctx, ds.writer(ctx), &hmdm, `
		SELECT
			...
			hda.host_id IS NOT NULL AND hda.deleted_at IS NULL as dep_assigned_to_mobius,
...
func (ds *Datastore) GetMDMSolution(ctx context.Context, mdmID uint) (*mobius.MDMSolution, error) {
	var solution mobius.MDMSolution
...
func (ds *Datastore) GetHostMunkiIssues(ctx context.Context, hostID uint) ([]*mobius.HostMunkiIssue, error) {
	var issues []*mobius.HostMunkiIssue
...
func (ds *Datastore) GetMunkiIssue(ctx context.Context, munkiIssueID uint) (*mobius.MunkiIssue, error) {
	var issue mobius.MunkiIssue
...
func (ds *Datastore) AggregatedMunkiVersion(ctx context.Context, teamID *uint) ([]mobius.AggregatedMunkiVersion, time.Time, error) {
	id := uint(0)
	globalStats := true
	...
	var versions []mobius.AggregatedMunkiVersion
...
func (ds *Datastore) AggregatedMunkiIssues(ctx context.Context, teamID *uint) ([]mobius.AggregatedMunkiIssue, time.Time, error) {
	id := uint(0)
	globalStats := true
	...
	var result []mobius.AggregatedMunkiIssue
...
func (ds *Datastore) AggregatedMDMStatus(ctx context.Context, teamID *uint, platform string) (mobius.AggregatedMDMStatus, time.Time, error) {
	id := uint(0)
	globalStats := true
	...
	var status mobius.AggregatedMDMStatus
	...
	if err != nil {
		if err == sql.ErrNoRows {
			// not having stats is not an error
			return mobius.AggregatedMDMStatus{}, time.Time{}, nil
		}
		return mobius.AggregatedMDMStatus{}, time.Time{}, ctxerr.Wrap(ctx, err, "selecting mdm status")
	}
	if err := json.Unmarshal(statusJson.JsonValue, &status); err != nil {
		return mobius.AggregatedMDMStatus{}, time.Time{}, ctxerr.Wrap(ctx, err, "unmarshaling mdm status")
	}
	return status, statusJson.UpdatedAt, nil
}

func platformKey(key mobius.AggregatedStatsType, platform string) mobius.AggregatedStatsType {
	if platform == "" {
		return key
	}
	return key + "_" + mobius.AggregatedStatsType(platform)
}

func (ds *Datastore) AggregatedMDMSolutions(ctx context.Context, teamID *uint, platform string) ([]mobius.AggregatedMDMSolutions, time.Time, error) {
	id := uint(0)
	globalStats := true
	...
	var result []mobius.AggregatedMDMSolutions
	var resultJSON struct {
		JsonValue []byte    `db:"json_value"`
		UpdatedAt time.Time `db:"updated_at"`
	}
...
func (ds *Datastore) generateAggregatedMunkiIssues(ctx context.Context, teamID *uint) error {
	...
	var issues []mobius.AggregatedMunkiIssue
	query := `
  SELECT
...
func (ds *Datastore) generateAggregatedMDMStatus(ctx context.Context, teamID *uint, platform string) error {
	var (
		id          = uint(0)
		globalStats = true
		status      mobius.AggregatedMDMStatus
	)
	// NOTE: ds.UpdateHostTablesOnMDMUnenroll sets installed_from_dep = 0 so DEP hosts are not counted as pending after unenrollment
	query := `SELECT
...
func (ds *Datastore) generateAggregatedMDMSolutions(ctx context.Context, teamID *uint, platform string) error {
	var (
		id          = uint(0)
		globalStats = true
		results     []mobius.AggregatedMDMSolutions
		whereAnd    = "WHERE"
	)
	query := `SELECT
				mdms.id,
...
func (ds *Datastore) HostLite(ctx context.Context, id uint) (*mobius.Host, error) {
...
func (ds *Datastore) UpdateHostOsqueryIntervals(ctx context.Context, id uint, intervals mobius.HostOsqueryIntervals) error {
	sqlStatement := `
		UPDATE hosts SET
			distributed_interval = ?,
			config_tls_refresh = ?,
...
func (ds *Datastore) OSVersion(ctx context.Context, osVersionID uint, teamFilter *mobius.TeamFilter) (
	*mobius.OSVersion, *time.Time, error,
) {
...
func (ds *Datastore) OSVersions(
	...
	var filtered []mobius.OSVersion
	for _, os := range counts {
		if (platform == nil || *platform == os.Platform || (*platform == "linux" && mobius.IsLinux(os.Platform))) && (name == nil || version == nil || (*name == os.NameOnly && *version == os.Version)) {
			filtered = append(filtered, os)
		}
	}
	counts = filtered
	...
	// aggregate counts by name and version
	byNameVers := make(map[string]mobius.OSVersion)
	for _, os := range counts {
		key := fmt.Sprintf("%s %s", os.NameOnly, os.Version)
		val, ok := byNameVers[key]
		if !ok {
			// omit os id
			byNameVers[key] = mobius.OSVersion{Name: os.Name, NameOnly: os.NameOnly, Version: os.Version, Platform: os.Platform, HostsCount: os.HostsCount, OSVersionID: os.OSVersionID}
		} else {
			newVal := val
...
func (ds *Datastore) executeOSVersionQuery(ctx context.Context, teamFilter *mobius.TeamFilter) (
...
func (ds *Datastore) unmarshalOSVersions(jsonValue *json.RawMessage) ([]mobius.OSVersion, error) {
	var versions []mobius.OSVersion
	if jsonValue != nil {
...
func (ds *Datastore) UpdateOSVersions(ctx context.Context) error {
	...
	statsByTeamID := make(map[uint][]mobius.OSVersion)
	// stats are also aggregated globally per os version
	globalStats := make(map[uint]mobius.OSVersion)
	...
	for _, id := range teamIDs {
		if _, ok := statsByTeamID[id]; !ok {
			statsByTeamID[id] = []mobius.OSVersion{}
		}
	}
	// same for "no team"
	if _, ok := statsByTeamID[0]; !ok {
		statsByTeamID[0] = []mobius.OSVersion{}
	}
	...
	globalArray := make([]mobius.OSVersion, 0, len(globalStats))
...
// count. See https://github.com/MobiusDM/Mobius/pull/6782#discussion_r926103758.
...
`
	...
	var batteries []*mobius.HostBattery
...
func (ds *Datastore) ListUpcomingHostMaintenanceWindows(ctx context.Context, hid uint) ([]*mobius.HostMaintenanceWindow, error) {
	stmt := `
		SELECT
			ce.start_time,
			ce.timezone
	...
	var mws []*mobius.HostMaintenanceWindow
...
//
// Notes:
//   - We use `2 * interval`, because of the artificial jitter added to the intervals in Mobius.
//   - Default values for:
...
func countHostsNotRespondingDB(ctx context.Context, db sqlx.QueryerContext, logger log.Logger, config config.MobiusConfig) (int, error,
) {
	interval := config.Osquery.DetailUpdateInterval.Seconds()

	// The primary `WHERE` clause is intended to capture where Mobius hasn't received a distributed write
...
func amountHostsByOsqueryVersionDB(ctx context.Context, db sqlx.QueryerContext) ([]mobius.HostsCountByOsqueryVersion, error) {
	counts := make([]mobius.HostsCountByOsqueryVersion, 0)
...
func numHostsMobiusDesktopEnabledDB(ctx context.Context, db sqlx.QueryerContext) (int, error) {
	var count int
	const stmt = `
...
func (ds *Datastore) GetMatchingHostSerials(ctx context.Context, serials []string) (map[string]*mobius.Host, error) {
	result := map[string]*mobius.Host{}
	if len(serials) == 0 {
		return result, nil
	}
...
func (ds *Datastore) GetHostHealth(ctx context.Context, id uint) (*mobius.HostHealth, error) {
	sqlStmt := `
	...
	host := &mobius.Host{ID: id, Platform: hh.Platform}
	...
	for _, s := range host.Software {
		if len(s.Vulnerabilities) > 0 {
			hh.VulnerableSoftware = append(hh.VulnerableSoftware, mobius.HostHealthVulnerableSoftware{
				ID:      s.ID,
				Name:    s.Name,
				Version: s.Version,
			})
	...
	for _, p := range policies {
		if p.Response == "fail" {
			...
			hh.FailingPolicies = append(hh.FailingPolicies, &mobius.HostHealthFailingPolicy{
				ID:         p.ID,
				Name:       p.Name,
...
func (ds *Datastore) HostLiteByIdentifier(ctx context.Context, identifier string) (*mobius.HostLite, error) {
	return ds.loadHostLite(ctx, nil, &identifier)
}

func (ds *Datastore) HostLiteByID(ctx context.Context, id uint) (*mobius.HostLite, error) {
	return ds.loadHostLite(ctx, &id, nil)
}

func (ds *Datastore) loadHostLite(ctx context.Context, id *uint, identifier *string) (*mobius.HostLite, error) {
...
// Specified in story https://github.com/MobiusDM/Mobius/issues/18115
