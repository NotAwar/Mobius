...
func (ds *Datastore) ApplyLabelSpecsWithAuthor(ctx context.Context, specs []*mobius.LabelSpec, authorID *uint) (err error) {
	err = ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		...
		for _, s := range specs {
			...
			if s.LabelType == mobius.LabelTypeBuiltIn ||
				s.LabelMembershipType != mobius.LabelMembershipTypeManual {
...
func (ds *Datastore) UpdateLabelMembershipByHostIDs(ctx context.Context, labelID uint, hostIds []uint, teamFilter mobius.TeamFilter) (*mobius.Label, []uint, error) {
	err := ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
...
func (ds *Datastore) GetLabelSpecs(ctx context.Context) ([]*mobius.LabelSpec, error) {
	var specs []*mobius.LabelSpec
	// Get basic specs
	...
	for _, spec := range specs {
		if spec.LabelType != mobius.LabelTypeBuiltIn &&
			spec.LabelMembershipType == mobius.LabelMembershipTypeManual {
...
func (ds *Datastore) GetLabelSpec(ctx context.Context, name string) (*mobius.LabelSpec, error) {
	var specs []*mobius.LabelSpec
	query := `
...
func (ds *Datastore) getLabelHostnames(ctx context.Context, label *mobius.LabelSpec) error {
	sql := `
		SELECT hostname
		FROM hosts
		WHERE id IN
...
// NewLabel creates a new mobius.Label
func (ds *Datastore) NewLabel(ctx context.Context, label *mobius.Label, opts ...mobius.OptionalArg) (*mobius.Label, error) {
	query := `
	INSERT INTO labels (
		name,
		description,
...
func (ds *Datastore) SaveLabel(ctx context.Context, label *mobius.Label, teamFilter mobius.TeamFilter) (*mobius.Label, []uint, error) {
...
// DeleteLabel deletes a mobius.Label
func (ds *Datastore) DeleteLabel(ctx context.Context, name string) error {
	return ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		...
		_, err = tx.ExecContext(ctx, `DELETE FROM pack_targets WHERE type=? AND target_id=?`, mobius.TargetLabel, labelID)
		if err != nil {
...
// Label returns a mobius.Label identified by lid if one exists.
func (ds *Datastore) Label(ctx context.Context, lid uint, teamFilter mobius.TeamFilter) (*mobius.Label, []uint, error) {
	return ds.labelDB(ctx, lid, teamFilter, ds.reader(ctx))
}

func (ds *Datastore) labelDB(ctx context.Context, lid uint, teamFilter mobius.TeamFilter, q sqlx.QueryerContext) (*mobius.Label, []uint, error) {
	stmt := fmt.Sprintf(`
		SELECT
		       l.*,
	...
	var hostIDs []uint
	if label.LabelMembershipType == mobius.LabelMembershipTypeManual && label.HostCount > 0 {
...
// ListLabels returns all labels limited or sorted by mobius.ListOptions.
// MatchQuery not supported
func (ds *Datastore) ListLabels(ctx context.Context, filter mobius.TeamFilter, opt mobius.ListOptions) ([]*mobius.Label, error) {
	if opt.After != "" {
		return nil, &mobius.BadRequestError{Message: "parameter 'after' is not supported"}
	}
	if opt.MatchQuery != "" {
		return nil, &mobius.BadRequestError{Message: "parameter 'query' is not supported"}
	}
...
func platformForHost(host *mobius.Host) string {
...
func (ds *Datastore) LabelQueriesForHost(ctx context.Context, host *mobius.Host) (map[string]string, error) {
	var rows *sql.Rows
	var err error
	platform := platformForHost(host)
	query := `SELECT id, query FROM labels WHERE platform = ? OR platform = '' AND label_membership_type = ?`
	rows, err = ds.reader(ctx).QueryContext(ctx, query, platform, mobius.LabelMembershipTypeDynamic)
...
// ListLabelsForHost returns a list of mobius.Label for a given host id.
func (ds *Datastore) ListLabelsForHost(ctx context.Context, hid uint) ([]*mobius.Label, error) {
	sqlStatement := `
...
// ListHostsInLabel returns a list of mobius.Host that are associated
// with mobius.Label referenced by Label ID
func (ds *Datastore) ListHostsInLabel(ctx context.Context, filter mobius.TeamFilter, lid uint, opt mobius.HostListOptions) ([]*mobius.Host, error) {
	queryFmt := `
    SELECT
      h.id,
      h.osquery_host_id,
...
func (ds *Datastore) applyHostLabelFilters(ctx context.Context, filter mobius.TeamFilter, lid uint, query string, opt mobius.HostListOptions) (string, []interface{}, error) {
...
func (ds *Datastore) CountHostsInLabel(ctx context.Context, filter mobius.TeamFilter, lid uint, opt mobius.HostListOptions) (int, error) {
...
func (ds *Datastore) ListUniqueHostsInLabels(ctx context.Context, filter mobius.TeamFilter, labels []uint) ([]*mobius.Host, error) {
	if len(labels) == 0 {
...
func (ds *Datastore) searchLabelsWithOmits(ctx context.Context, filter mobius.TeamFilter, query string, omit ...uint) ([]*mobius.Label, error) {
...
func (ds *Datastore) addAllHostsLabelToList(ctx context.Context, filter mobius.TeamFilter, labels []*mobius.Label, omit ...uint) ([]*mobius.Label, error) {
	sql := fmt.Sprintf(`
			SELECT *,
				(SELECT COUNT(1)
					FROM label_membership lm JOIN hosts h ON (lm.host_id = h.id)
	...
	var allHosts mobius.Label
	if err := sqlx.GetContext(ctx, ds.reader(ctx), &allHosts, sql, mobius.LabelTypeBuiltIn); err != nil {
...
func (ds *Datastore) searchLabelsDefault(ctx context.Context, filter mobius.TeamFilter, omit ...uint) ([]*mobius.Label, error) {
	sql := fmt.Sprintf(`
			SELECT *,
				(SELECT COUNT(1)
					FROM label_membership lm JOIN hosts h ON (lm.host_id = h.id)
...
// SearchLabels performs wildcard searches on mobius.Label name
func (ds *Datastore) SearchLabels(ctx context.Context, filter mobius.TeamFilter, query string, omit ...uint) ([]*mobius.Label, error) {
	transformedQuery := transformQuery(query)
...
func (ds *Datastore) LabelsByName(ctx context.Context, names []string) (map[string]*mobius.Label, error) {
	if len(names) == 0 {
		return map[string]*mobius.Label{}, nil
	}
	...
	result := make(map[string]*mobius.Label, len(labels))
	for _, label := range labels {
...
func (ds *Datastore) LabelsSummary(ctx context.Context) ([]*mobius.LabelSummary, error) {
	labelsSummary := []*mobius.LabelSummary{}
