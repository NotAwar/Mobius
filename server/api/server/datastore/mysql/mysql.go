...
import (
	...
	"github.com/MobiusDM/mobius/server/api/server/datastore/mysql/migrations/data"
	"github.com/MobiusDM/mobius/server/api/server/datastore/mysql/migrations/tables"
	"github.com/MobiusDM/mobius/server/api/server/goose"
	"github.com/MobiusDM/mobius/server/api/server/mdm/android"
	android_mysql "github.com/MobiusDM/mobius/server/api/server/mdm/android/mysql"
	nano_push "github.com/MobiusDM/mobius/server/api/server/mdm/nanomdm/push"
	scep_depot "github.com/MobiusDM/mobius/server/api/server/mdm/scep/depot"
...
// Datastore is an implementation of mobius.Datastore interface backed by
// MySQL
type Datastore struct {
	...
	// This key is used to encrypt sensitive data stored in the Mobius DB, for example MDM
...
func (ds *Datastore) MigrationStatus(ctx context.Context) (*mobius.MigrationStatus, error) {
...
// It assumes some deployments may have performed migrations out of order.
func compareMigrations(knownTable goose.Migrations, knownData goose.Migrations, appliedTable, appliedData []int64) *mobius.MigrationStatus {
	if len(appliedTable) == 0 && len(appliedData) == 0 {
		return &mobius.MigrationStatus{
			StatusCode: mobius.NoMigrationsCompleted,
		}
	}
	...
	if equalData && equalTable {
		return &mobius.MigrationStatus{
			StatusCode: mobius.AllMigrationsCompleted,
		}
	}
	...
	if len(missingTable) > 0 || len(missingData) > 0 {
		return &mobius.MigrationStatus{
			StatusCode:   mobius.SomeMigrationsCompleted,
	...
	return &mobius.MigrationStatus{
		StatusCode:   mobius.UnknownMigrations,
...
var (
	knownUnknownTableMigrations = map[int64]struct{}{
		// This migration was introduced incorrectly in mobius-v4.4.0 and its
		// timestamp was changed in mobius-v4.4.1.
		20210924114500: {},
	}
...
func appendListOptionsToSelect(ds *goqu.SelectDataset, opts mobius.ListOptions) *goqu.SelectDataset {
	ds = appendOrderByToSelect(ds, opts)
	ds = appendLimitOffsetToSelect(ds, opts)
	return ds
}

func appendOrderByToSelect(ds *goqu.SelectDataset, opts mobius.ListOptions) *goqu.SelectDataset {
	if opts.OrderKey != "" {
...
func appendLimitOffsetToSelect(ds *goqu.SelectDataset, opts mobius.ListOptions) *goqu.SelectDataset {
...
func appendListOptionsToSQL(sql string, opts *mobius.ListOptions) (string, []interface{}) {
...
func appendListOptionsWithCursorToSQL(sql string, params []interface{}, opts *mobius.ListOptions) (string, []interface{}) {
	...
	if orderKey != "" {
		...
		if opts.TestSecondaryOrderKey != "" {
			direction := "ASC"
			if opts.TestSecondaryOrderDirection == mobius.OrderDescending {
				direction = "DESC"
			}
...
func (ds *Datastore) whereFilterHostsByTeams(filter mobius.TeamFilter, hostKey string) string {
	if filter.User == nil {
	...
	if filter.User.GlobalRole != nil {
		...
		case mobius.RoleAdmin, mobius.RoleMaintainer, mobius.RoleObserverPlus:
	...
	for _, team := range filter.User.Teams {
		if team.Role == mobius.RoleAdmin ||
			team.Role == mobius.RoleMaintainer ||
			team.Role == mobius.RoleObserverPlus ||
			(team.Role == mobius.RoleObserver && filter.IncludeObserver) {
...
// SQL.
func (ds *Datastore) whereFilterGlobalOrTeamIDByTeams(filter mobius.TeamFilter, filterTableAlias string) string {
...
func (ds *Datastore) whereFilterGlobalOrTeamIDByTeamsWithSqlFilter(
	filter mobius.TeamFilter, globalSqlFilter string, teamIDSqlFilter string,
) string {
	if filter.User == nil {
		// This is likely unintentional, however we would like to return no
		// results rather than panicking or returning some other error. At least
...
func (ds *Datastore) whereFilterTeams(filter mobius.TeamFilter, teamKey string) string {
	if filter.User == nil {
		// This is likely unintentional, however we would like to return no
		// results rather than panicking or returning some other error. At least
		// log.
	...
	if filter.User.GlobalRole != nil {
		switch *filter.User.GlobalRole {
		case mobius.RoleAdmin, mobius.RoleMaintainer, mobius.RoleGitOps, mobius.RoleObserverPlus:
			return "TRUE"
		case mobius.RoleObserver:
			if filter.IncludeObserver {
				return "TRUE"
...
func (ds *Datastore) InnoDBStatus(ctx context.Context) (string, error) {
	...
	if err != nil {
		...
		// This can be set by DB admin like: GRANT PROCESS,SELECT ON *.* TO 'mobius'@'%';
...
func (ds *Datastore) ProcessList(ctx context.Context) ([]mobius.MySQLProcess, error) {
	var processList []mobius.MySQLProcess
	// using the writer even when doing a read to get the data from the main db node
