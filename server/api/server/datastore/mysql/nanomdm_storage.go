...
import (
	...
	nanodep_mysql "github.com/MobiusDM/mobius/server/api/server/mdm/nanodep/storage/mysql"
	"github.com/MobiusDM/mobius/server/api/server/mdm/nanomdm/mdm"
	nanomdm_mysql "github.com/MobiusDM/mobius/server/api/server/mdm/nanomdm/storage/mysql"
...
//
// Always returns "0" as stale token because mobius.Datastore always returns a valid push certificate.
...
func (s *NanoMDMStorage) StorePushCert(ctx context.Context, pemCert, pemKey []byte) error {
	return errors.New("please use mobius.Datastore to manage MDM assets")
}
...
//   - The method performs mobius-specific actions after the command is enqueued.
func (s *NanoMDMStorage) EnqueueDeviceLockCommand(
	...
	return common_mysql.WithRetryTxx(ctx, s.db, func(tx sqlx.ExtContext) error {
		...
		if _, err := tx.ExecContext(ctx, stmt, host.ID, cmd.CommandUUID, pin, host.MobiusPlatform()); err != nil {
...
func (s *NanoMDMStorage) EnqueueDeviceWipeCommand(ctx context.Context, host *mobius.Host, cmd *mdm.Command) error {
	return common_mysql.WithRetryTxx(ctx, s.db, func(tx sqlx.ExtContext) error {
		if err := enqueueCommandDB(ctx, tx, []string{host.UUID}, cmd); err != nil {
			return err
		}
...
func (s *NanoMDMStorage) GetAllMDMConfigAssetsByName(ctx context.Context, assetNames []mobius.MDMAssetName,
...
func (s *NanoMDMStorage) GetABMTokenByOrgName(ctx context.Context, orgName string) (*mobius.ABMToken, error) {
...
// DEP auth tokens from the tables managed by Mobius.
