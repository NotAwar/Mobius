...
func (ds *Datastore) NewGlobalPolicy(ctx context.Context, authorID *uint, args mobius.PolicyPayload) (*mobius.Policy, error) {
	var newPolicy *mobius.Policy
...
func newGlobalPolicy(ctx context.Context, db sqlx.ExtContext, authorID *uint, args mobius.PolicyPayload) (*mobius.Policy, error) {
	...
	for _, labelInclude := range args.LabelsIncludeAny {
		dummyPolicy.LabelsIncludeAny = append(dummyPolicy.LabelsIncludeAny, mobius.LabelIdent{LabelName: labelInclude})
	}

	for _, labelExclude := range args.LabelsExcludeAny {
		dummyPolicy.LabelsExcludeAny = append(dummyPolicy.LabelsExcludeAny, mobius.LabelIdent{LabelName: labelExclude})
	}
...
func updatePolicyLabelsTx(ctx context.Context, tx sqlx.ExtContext, policy *mobius.Policy) error {
...
func loadLabelsForPolicies(ctx context.Context, db sqlx.QueryerContext, policies []*mobius.Policy) error {
	const sql = `
		SELECT
			pl.policy_id,
	...
	policyMap := make(map[uint]*mobius.Policy, len(policies))
	...
	for _, row := range rows {
		if row.Exclude {
			policyMap[row.PolicyID].LabelsExcludeAny = append(policyMap[row.PolicyID].LabelsExcludeAny, mobius.LabelIdent{LabelName: row.LabelName, LabelID: row.LabelID})
		} else {
			policyMap[row.PolicyID].LabelsIncludeAny = append(policyMap[row.PolicyID].LabelsIncludeAny, mobius.LabelIdent{LabelName: row.LabelName, LabelID: row.LabelID})
		}
	}
...
func (ds *Datastore) Policy(ctx context.Context, id uint) (*mobius.Policy, error) {
	return policyDB(ctx, ds.reader(ctx), id, nil)
}

func policyDB(ctx context.Context, q sqlx.QueryerContext, id uint, teamID *uint) (*mobius.Policy, error) {
	teamWhere := "TRUE"
	args := []interface{}{id}
	if teamID != nil {
		teamWhere = "team_id = ?"
	...
	if err := loadLabelsForPolicies(ctx, q, []*mobius.Policy{&policy}); err != nil {
...
func (ds *Datastore) PolicyLite(ctx context.Context, id uint) (*mobius.PolicyLite, error) {
	var policy mobius.PolicyLite
	err := sqlx.GetContext(
...
func (ds *Datastore) SavePolicy(ctx context.Context, p *mobius.Policy, shouldRemoveAllPolicyMemberships bool, removePolicyStats bool) error {
	if err := ds.withTx(ctx, func(tx sqlx.ExtContext) error {
...
func savePolicy(ctx context.Context, db sqlx.ExtContext, logger kitlog.Logger, p *mobius.Policy, shouldRemoveAllPolicyMemberships bool, removePolicyStats bool) error {
...
func (ds *Datastore) RecordPolicyQueryExecutions(ctx context.Context, host *mobius.Host, results map[uint]*bool, updated time.Time, deferredSaveHost bool) error {
	vals := []interface{}{}
	bindvars := []string{}
	if len(results) > 0 {
		// Sort the results to have generated SQL queries ordered to minimize
		// deadlocks. See https://github.com/MobiusDM/Mobius/issues/1146.
...
func (ds *Datastore) ListGlobalPolicies(ctx context.Context, opts mobius.ListOptions) ([]*mobius.Policy, error) {
...
func listPoliciesDB(ctx context.Context, q sqlx.QueryerContext, teamID *uint, opts mobius.ListOptions) ([]*mobius.Policy, error) {
	var args []interface{}
...
func getInheritedPoliciesForTeam(ctx context.Context, q sqlx.QueryerContext, teamID uint, opts mobius.ListOptions) ([]*mobius.Policy, error) {
	var args []interface{}
...
func (ds *Datastore) PoliciesByID(ctx context.Context, ids []uint) (map[uint]*mobius.Policy, error) {
	...
	policiesByID := make(map[uint]*mobius.Policy, len(ids))
...
func (ds *Datastore) PolicyQueriesForHost(ctx context.Context, host *mobius.Host) (map[string]string, error) {
...
`
	...
	if err := sqlx.SelectContext(ctx, ds.reader(ctx), &rows, stmt, host.TeamID, host.MobiusPlatform(), host.ID, host.ID); err != nil {
...
func (ds *Datastore) NewTeamPolicy(ctx context.Context, teamID uint, authorID *uint, args mobius.PolicyPayload) (policy *mobius.Policy, err error) {
	var newPolicy *mobius.Policy
	if err := ds.withTx(ctx, func(tx sqlx.ExtContext) error {
...
func newTeamPolicy(ctx context.Context, db sqlx.ExtContext, teamID uint, authorID *uint, args mobius.PolicyPayload) (*mobius.Policy, error) {
	if args.QueryID != nil {
		q, err := query(ctx, db, *args.QueryID)
		if err != nil {
			return nil, ctxerr.Wrap(ctx, err, "fetching query from id")
...
func (ds *Datastore) ListTeamPolicies(ctx context.Context, teamID uint, opts mobius.ListOptions, iopts mobius.ListOptions) (teamPolicies, inheritedPolicies []*mobius.Policy, err error) {
...
func (ds *Datastore) ListMergedTeamPolicies(ctx context.Context, teamID uint, opts mobius.ListOptions) ([]*mobius.Policy, error) {
	var args []interface{}
...
func (ds *Datastore) TeamPolicy(ctx context.Context, teamID uint, policyID uint) (*mobius.Policy, error) {
...
func (ds *Datastore) ApplyPolicySpecs(ctx context.Context, authorID uint, specs []*mobius.PolicySpec) error {
	...
	teamIDToPolicies := make(map[*uint][]*mobius.PolicySpec, 1)
	...
	return ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		...
		for teamID, teamPolicySpecs := range teamIDToPolicies {
			for _, spec := range teamPolicySpecs {
				...
				labelsIncludeAnyIdents := make([]mobius.LabelIdent, 0, len(spec.LabelsIncludeAny))
				for _, labelInclude := range spec.LabelsIncludeAny {
					labelsIncludeAnyIdents = append(labelsIncludeAnyIdents, mobius.LabelIdent{LabelName: labelInclude})
				}
				labelsExcludeAnyIdents := make([]mobius.LabelIdent, 0, len(spec.LabelsExcludeAny))
				for _, labelExclude := range spec.LabelsExcludeAny {
					labelsExcludeAnyIdents = append(labelsExcludeAnyIdents, mobius.LabelIdent{LabelName: labelExclude})
				}
				err = updatePolicyLabelsTx(ctx, tx, &mobius.Policy{
					PolicyData: mobius.PolicyData{
...
func (ds *Datastore) AsyncBatchInsertPolicyMembership(ctx context.Context, batch []mobius.PolicyMembershipResult) error {
...
func cleanupPolicyMembershipOnPolicyUpdate(
	...
	for _, platform := range splitPlatforms {
		expandedPlatforms = append(expandedPlatforms, mobius.ExpandPlatform(strings.TrimSpace(platform))...)
	}
...
func (ds *Datastore) OutdatedAutomationBatch(ctx context.Context) ([]mobius.PolicyFailure, error) {
	var failures []mobius.PolicyFailure
	err := ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
...
func (ds *Datastore) UpdateHostPolicyCounts(ctx context.Context) error {
	...
	// https://github.com/MobiusDM/Mobius/blob/main/server/datastore/mysql/migrations/tables/20231215122713_InsertPolicyStatsData.go#L12
...
func (ds *Datastore) GetCalendarPolicies(ctx context.Context, teamID uint) ([]mobius.PolicyCalendarData, error) {
	query := `SELECT id, name FROM policies WHERE team_id = ? AND calendar_events_enabled;`
	var policies []mobius.PolicyCalendarData
...
func (ds *Datastore) GetPoliciesWithAssociatedInstaller(ctx context.Context, teamID uint, policyIDs []uint) ([]mobius.PolicySoftwareInstallerData, error) {
	if len(policyIDs) == 0 {
		return nil, nil
	}
	...
	var policies []mobius.PolicySoftwareInstallerData
...
func (ds *Datastore) GetPoliciesWithAssociatedVPP(ctx context.Context, teamID uint, policyIDs []uint) ([]mobius.PolicyVPPData, error) {
	if len(policyIDs) == 0 {
		return nil, nil
	}
	...
	var policies []mobius.PolicyVPPData
	if err := sqlx.SelectContext(ctx, ds.reader(ctx), &policies, query, args...); err != nil {
		return nil, ctxerr.Wrap(ctx, err, "get policies with associated installer")
	}
	return policies, nil
...
func (ds *Datastore) GetPoliciesWithAssociatedScript(ctx context.Context, teamID uint, policyIDs []uint) ([]mobius.PolicyScriptData, error) {
	if len(policyIDs) == 0 {
		return nil, nil
	}
	...
	var policies []mobius.PolicyScriptData
	if err := sqlx.SelectContext(ctx, ds.reader(ctx), &policies, query, args...); err != nil {
...
`
	...
	var hosts []mobius.HostPolicyMembershipData
...
func (ds *Datastore) getPoliciesBySoftwareTitleIDs(
	...
	teamID *uint,
) ([]mobius.AutomaticInstallPolicy, error) {
...
`
	...
	batchSize := 32000 // see https://github.com/MobiusDM/Mobius/issues/26753 on the math behind this number
	...
	err := common_mysql.BatchProcessSimple(softwareTitleIDs, batchSize, func(softwareTitleIDsToProcess []uint) error {
		...
		var policyBatch []mobius.AutomaticInstallPolicy
