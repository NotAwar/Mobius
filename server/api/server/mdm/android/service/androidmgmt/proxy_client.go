...
const defaultProxyEndpoint = "https://mobiusmdm.com/api/android/"

// ProxyClient connects to Google's Android Management API via a proxy, which is hosted at mobiusmdm.com by default.
type ProxyClient struct {
	logger             kitlog.Logger
	mgmt               *androidmanagement.Service
	licenseKey         string
	proxyEndpoint      string
	mobiusServerSecret string
}
...
func NewProxyClient(ctx context.Context, logger kitlog.Logger, licenseKey string, getenv func(string) string) Client {
	proxyEndpoint := getenv("MOBIUS_DEV_ANDROID_PROXY_ENDPOINT")
	if proxyEndpoint == "" {
	...
	mgmt, err := androidmanagement.NewService(ctx,
		...
		// The API key is required to exist but not used by this client. Instead, we use the MobiusServerSecret as a bearer token.
		option.WithAPIKey("not_used"),
		option.WithHTTPClient(mobiushttp.NewClient()),
	)
	if err != nil {
...
func (p *ProxyClient) SignupURLsCreate(ctx context.Context, serverURL, callbackURL string) (*android.SignupDetails, error) {
	...
	case isErrorCode(err, http.StatusConflict):
		// This error message is displayed to users via the API.
		return nil, android.NewConflictError(fmt.Errorf("android enterprise already exists. For help, please contact Mobius support https://mobiusmdm.com/support: %w", err))
	case err != nil:
...
func (p *ProxyClient) EnterprisesCreate(ctx context.Context, req EnterprisesCreateRequest) (EnterprisesCreateResponse, error) {
	...
	type proxyEnterprise struct {
		MobiusLicenseKey string                       `json:"mobiusLicenseKey"`
	...
	type proxyEnterpriseResponse struct {
		MobiusServerSecret string `json:"mobiusServerSecret"`
	...
	return EnterprisesCreateResponse{
		...
		MobiusServerSecret: per.MobiusServerSecret,
	}, nil
}
...
func (p *ProxyClient) EnterprisesPoliciesPatch(ctx context.Context, policyName string, policy *androidmanagement.Policy) error {
	...
	call.Header().Set("Authorization", "Bearer "+p.mobiusServerSecret)
	_, err := call.Do()
	switch {
