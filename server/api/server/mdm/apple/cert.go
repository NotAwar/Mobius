...
type MobiusWebsiteError struct {
	Status  int
	message string
}

func (e MobiusWebsiteError) Error() string {
	if e.message != "" {
		return e.message
	}
...
// GetSignedAPNSCSR makes a request to the mobiusmdm.com API to get a signed APNs
...
func GetSignedAPNSCSR(client *http.Client, csr *x509.CertificateRequest) error {
	...
	// for testing
	baseURL := defaultMobiusDMAPIURL
	if x := os.Getenv("TEST_MOBIUSDAEMONM_API_URL"); x != "" {
	...
	if resp.StatusCode != http.StatusOK {
		...
		return MobiusWebsiteError{Status: resp.StatusCode, message: string(b)}
	}
	return nil
}
...
// GetSignedAPNSCSRNoEmail makes a request to the mobiusmdm.com API to get a signed APNs
...
func GetSignedAPNSCSRNoEmail(client *http.Client, csr *x509.CertificateRequest) ([]byte, error) {
	...
	req, err := http.NewRequest(http.MethodPost, u, bytes.NewReader(b))
	if err != nil {
		return nil, fmt.Errorf("creating csr signing request for mobiusmdm api: %w", err)
	}

	resp, err := client.Do(req)
	if err != nil {
		return nil, fmt.Errorf("sending csr signing request to mobiusmdm api: %w", err)
	}
	defer resp.Body.Close()

	respBytes, err := io.ReadAll(resp.Body)
	if err != nil {
		return nil, fmt.Errorf("parsing CSR body response from mobiusmdm api: %w", err)
	}
	if resp.StatusCode != http.StatusOK {
		return nil, MobiusWebsiteError{Status: resp.StatusCode, message: string(respBytes)}
	}

	var csrResp websiteSignCSRResponse
	if err := json.Unmarshal(respBytes, &csrResp); err != nil {
		return nil, fmt.Errorf("unmarshalling signed csr response from mobiusmdm api: %w", err)
	}
