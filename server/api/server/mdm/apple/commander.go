...
import (
	...
	nanomdm_push "github.com/notawar/mobius/mobius-server/server/mdm/nanomdm/push"
...
// MDMAppleCommander contains methods to enqueue commands managed by Mobius and
// send push notifications to hosts.
//
// It's intentionally decoupled from mobius.Service so it can be used internally
// in crons and other services, leaving authentication/permission handling to
// the caller.
type MDMAppleCommander struct {
	storage mobius.MDMAppleStore
	pusher  nanomdm_push.Pusher
}

// NewMDMAppleCommander creates a new commander instance.
func NewMDMAppleCommander(mdmStorage mobius.MDMAppleStore, mdmPushService nanomdm_push.Pusher) *MDMAppleCommander {
...
func (svc *MDMAppleCommander) DeviceLock(ctx context.Context, host *mobius.Host, uuid string) (unlockPIN string, err error) {
...
func (svc *MDMAppleCommander) EraseDevice(ctx context.Context, host *mobius.Host, uuid string) error {
