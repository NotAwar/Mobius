...
import (
	...
	"github.com/notawar/mobius/mobius-server/server/mdm/apple/rootcert"
...
type AssetSets struct {
	...
	// VisionOS []Asset `json:"visionOS"` // Mobius doesn't support visionOS yet
	// XROS     []Asset `json:"xrOS"`    // Mobius doesn't support xrOS yet
}
...
type APIResponse struct {
	...
	// PublicRapidSecurityResponses interface{} `json:"PublicRapidSecurityResponses"` // Mobius doesn't support PublicRapidSecurityResponses yet
}
...
// [1]: http://gdmf.apple.com/v2/pmv
// [2]: https://support.apple.com/guide/deployment/use-mdm-to-deploy-software-updates-depafd2fad80/web
func GetLatestOSVersion(device mobius.MDMAppleMachineInfo) (*Asset, error) {
	...
	for i, s := range assetSet {
		for _, d := range s.SupportedDevices {
			if d == device.Product || d == device.SoftwareUpdateDeviceID {
				...
				if mobius.CompareVersions(assetSet[latestIdx].ProductVersion, s.ProductVersion) < 0 {
...
func createClient() *http.Client {
	...
	return mobiushttp.NewClient(
		mobiushttp.WithTLSClientConfig(&tls.Config{
			RootCAs:    certPool,
			MinVersion: tls.VersionTLS12,
		}),
		mobiushttp.WithTimeout(10*time.Second),
	)
}
...
func GetAssetMetadata() (*APIResponse, error) {
	...
	req.Header.Set("User-Agent", "mobius-device-management")
...
func getBaseURL() string {
	devURL := os.Getenv("MOBIUS_DEV_GDMF_URL")
	if devURL != "" {
		return devURL
	}
	return baseURL
