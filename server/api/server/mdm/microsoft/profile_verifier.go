...
import (
	...
	"github.com/notawar/mobius/mobius-server/server/mdm/microsoft/admx"
	"github.com/notawar/mobius/mobius-server/server/mdm/microsoft/wlanxml"
...
func LoopOverExpectedHostProfiles(
	ctx context.Context,
	ds mobius.ProfileVerificationStore,
	host *mobius.Host,
	fn func(profile *mobius.ExpectedMDMProfile, hash, locURI, data string),
) error {
...
func VerifyHostMDMProfiles(ctx context.Context, logger log.Logger, ds mobius.ProfileVerificationStore, host *mobius.Host,
...
func splitMissingProfilesIntoFailAndRetryBuckets(ctx context.Context, ds mobius.ProfileVerificationStore, host *mobius.Host,
...
func compareResultsToExpectedProfiles(ctx context.Context, logger log.Logger, ds mobius.ProfileVerificationStore, host *mobius.Host,
	profileResults profileResultsTransform, existingProfiles []mobius.HostMDMWindowsProfile,
	...
	windowsProfilesByID := make(map[string]mobius.HostMDMWindowsProfile, len(existingProfiles))
	for _, existingProfile := range existingProfiles {
		windowsProfilesByID[existingProfile.ProfileUUID] = existingProfile
	}

	err = LoopOverExpectedHostProfiles(ctx, ds, host, func(profile *mobius.ExpectedMDMProfile, ref, locURI, wantData string) {
		...
		case !strings.HasPrefix(gotStatus, "2"):
			...
			if gotStatus == "404" && (IsADMXInstallConfigOperationCSP(locURI) || IsWin32OrDesktopBridgeADMXCSP(locURI)) {
				if existingProfile, ok := windowsProfilesByID[profile.ProfileUUID]; ok && existingProfile.Status != nil &&
					(*existingProfile.Status == mobius.MDMDeliveryVerified || *existingProfile.Status == mobius.MDMDeliveryVerifying) {
...
func transformProfileResults(rawProfileResultsSyncML []byte) (profileResultsTransform, error) {
	...
	for _, r := range syncML.GetOrderedCmds() {
		...
		ref := *r.Cmd.CmdRef
		if r.Verb == mobius.CmdStatus && r.Cmd.Data != nil {
			transform.cmdRefToStatus[ref] = *r.Cmd.Data
		}

		if r.Verb == mobius.CmdResults {
