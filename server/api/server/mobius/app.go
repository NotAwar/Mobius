...
type ConditionalAccessSettings struct {
	...
	// for "Conditional access" on Entra and Mobius.
...
type SMTPSettings struct {
	...
	// SMTPServer is the host name of the SMTP server Mobius will use to send mail
...
// VulnerabilitySettings is part of the AppConfig which defines how mobius will behave
// while scanning for vulnerabilities in the host software
type VulnerabilitySettings struct {
	// DatabasesPath is the directory where mobius will store the different databases
...
type MDM struct {
	...
	// requests from mobiusdaemon requests on customer networks.  AppleServerURL DNS should resolve to the
	// same IP as the Mobius Server URL.
	// If not set, the server will use Mobius server URL (recommended).
	...
	// AppleBMEnabledAndConfigured is set to true if Mobius has been
	...
	// EnabledAndConfigured is set to true if Mobius has been
	...
	// WindowsEnabledAndConfigured indicates if Mobius MDM is enabled for Windows.
	...
	// AndroidEnabledAndConfigured is set to true if Mobius successfully bound to an Android Management Enterprise
...
type AppConfig struct {
	...
	// MobiusDesktop holds settings for Mobius Desktop that can be changed via the API.
	MobiusDesktop MobiusDesktopSettings `json:"mobius_desktop"`

	// VulnerabilitySettings defines how mobius will behave while scanning for vulnerabilities in the host software
...
func (c *AppConfig) Copy() *AppConfig {
	...
	// MobiusDesktopSettings: nothing needs cloning
	...
	// MobiusDesktop: nothing needs cloning
...
// EnrichedAppConfig contains the AppConfig along with additional mobius
// instance configuration settings as returned by the
// "GET /api/latest/mobius/config" API endpoint (and mobiuscli get config).
...
// OrgInfo contains general info about the organization using Mobius.
type OrgInfo struct {
	...
	// https://mobiusmdm.com/company/contact is used.
	ContactURL string `json:"contact_url"`
}

const DefaultOrgInfoContactURL = "https://mobiusmdm.com/company/contact"

// ServerSettings contains general settings about the Mobius application.
...
func (f *Features) ApplyDefaultsForNewInstalls() {
	...
	// next in already running mobiuss
...
// MobiusDesktopSettings contains settings used to configure Mobius Desktop.
type MobiusDesktopSettings struct {
	// TransparencyURL is the URL used for the “About Mobius” link in the Mobius Desktop menu.
	TransparencyURL string `json:"transparency_url"`
}

// DefaultTransparencyURL is the default URL used for the “About Mobius” link in the Mobius Desktop menu.
const DefaultTransparencyURL = "https://mobiusmdm.com/transparency"

// SecureframeTransparencyURL is the URL used for the "About Mobius" link in Mobius Desktop when the Secureframe partnership config value is enabled
const SecureframeTransparencyURL = "https://mobiusmdm.com/better?utm_content=secureframe"
...
// EnrollSecretSpec is the mobiuscli spec type for enroll secrets.
...
const (
	...
	// TierPremium is Mobius Premium aka the paid license.
	TierPremium = "premium"
	// TierFree is Mobius Free aka the free license.
	TierFree = "free"
	// TierTrial is Mobius Premium but in trial mode
...
// Partnerships contains specialized configuration options for Mobius partners.
type Partnerships struct {
	EnablePrimo bool `json:"enable_primo,omitempty"`
}

// LicenseInfo contains information about the Mobius license.
type LicenseInfo struct {
	...
	// ManagedCloud indicates whether this Mobius instance is a cloud instance.
...
const (
	HeaderLicenseKey          = "X-Mobius-License"
...
// mobius instance (as configured for the cli, either via flags, env vars or the
