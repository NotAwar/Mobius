...
// OrbitConfigNotifications are notifications that the mobius server sends to
// mobiusdaemon (orbit) so that it can run commands or more generally react to this
// information.
type OrbitConfigNotifications struct {
	...
	// disabled and the device was enrolled in Mobius's MDM (see
	...
	// RunSetupExperience indicates whether Orbit should run the Mobius setup experience
...
type OrbitConfig struct {
	...
	// UpdateChannels contains the TUF channels to use on mobiusdaemon components.
	//
...
// OrbitUpdateChannels hold the update channels that can be configured in mobiusdaemon agents.
type OrbitUpdateChannels struct {
	...
	// Desktop holds the Mobius Desktop channel.
