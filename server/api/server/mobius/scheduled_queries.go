...
type ScheduledQueryStats struct {
	...
	// WallTimeMs should always be hidden (0-value) in the Mobius API response since it does not map to a field in the DB.
...
func ScheduledQueryFromQuery(query *Query) *ScheduledQuery {
	...
	switch query.Logging {
case "", "snapshot": //nolint:gocritic // ignore ifElseChain
		snapshot = ptr.Bool(true)
		removed = ptr.Bool(false)
	case "differential":
		snapshot = ptr.Bool(false)
		removed = ptr.Bool(true)
	default: // query.Logging == "differential_ignore_removals"
		snapshot = ptr.Bool(false)
		removed = ptr.Bool(false)
	}
