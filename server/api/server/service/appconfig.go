...
import (
	...
	"github.com/notawar/mobius/mobius-server/pkg/rawjson"
...
type appConfigResponseFields struct {
	UpdateInterval  *mobius.UpdateIntervalConfig  `json:"update_interval"`
	Vulnerabilities *mobius.VulnerabilitiesConfig `json:"vulnerabilities"`
	...
	Logging *mobius.Logging `json:"logging,omitempty"`
...
func getAppConfigEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
	...
	var conditionalAccessSettings *mobius.ConditionalAccessSettings
	conditionalAccessIntegration, err := svc.ConditionalAccessMicrosoftGet(ctx)
	if err != nil {
		return nil, err
	}
	if conditionalAccessIntegration != nil {
		conditionalAccessSettings = &mobius.ConditionalAccessSettings{
	...
	var ssoSettings *mobius.SSOSettings
	...
	if license.IsPremium() && appConfig.MobiusDesktop.TransparencyURL != "" {
		transparencyURL = appConfig.MobiusDesktop.TransparencyURL
	}
	mobiusDesktop := mobius.MobiusDesktopSettings{TransparencyURL: transparencyURL}

	if appConfig.OrgInfo.ContactURL == "" {
		appConfig.OrgInfo.ContactURL = mobius.DefaultOrgInfoContactURL
	}
...
func (svc *Service) AppConfigObfuscated(ctx context.Context) (*mobius.AppConfig, error) {
...
func modifyAppConfigEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
	req := request.(*modifyAppConfigRequest)
	appConfig, err := svc.ModifyAppConfig(ctx, req.RawMessage, mobius.ApplySpecOptions{
		Force:     req.Force,
		DryRun:    req.DryRun,
	...
	if (!license.IsPremium()) || response.MobiusDesktop.TransparencyURL == "" {
		response.MobiusDesktop.TransparencyURL = mobius.DefaultTransparencyURL
	}

	return response, nil
}

func (svc *Service) ModifyAppConfig(ctx context.Context, p []byte, applyOpts mobius.ApplySpecOptions) (*mobius.AppConfig, error) {
	...
	var oldSMTPSettings mobius.SMTPSettings
	...
	storedJiraByProjectKey, err := mobius.IndexJiraIntegrations(appConfig.Integrations.Jira)
	if err != nil {
		return nil, ctxerr.Wrap(ctx, err, "modify AppConfig")
	}

	storedZendeskByGroupID, err := mobius.IndexZendeskIntegrations(appConfig.Integrations.Zendesk)
	if err != nil {
		return nil, ctxerr.Wrap(ctx, err, "modify AppConfig")
	}

	invalid := &mobius.InvalidArgumentError{}
	var newAppConfig mobius.AppConfig
	...
	// default transparency URL is https://mobiusmdm.com/transparency so you are allowed to apply as long as it's not changing
	if newAppConfig.MobiusDesktop.TransparencyURL != "" && newAppConfig.MobiusDesktop.TransparencyURL != mobius.DefaultTransparencyURL {
		if !license.IsPremium() {
			invalid.Append("transparency_url", ErrMissingLicense.Error())
			return nil, ctxerr.Wrap(ctx, invalid)
		}
		if _, err := url.Parse(newAppConfig.MobiusDesktop.TransparencyURL); err != nil {
	...
	if newAppConfig.MDM.EnableDiskEncryption.Valid {
		if newAppConfig.MDM.EnableDiskEncryption.Value && svc.config.Server.PrivateKey == "" {
			return nil, ctxerr.New(ctx,
				"Missing required private key. Learn how to configure the private key here: https://mobiusmdm.com/learn-more-about/mobius-server-private-key")
		}
	...
	if legacyKeys := appConfig.DidUnmarshalLegacySettings(); len(legacyKeys) > 0 {
		...
		// were encountered.
		legacyUsedWarning = &mobius.BadRequestError{
			Message: fmt.Sprintf("warning: deprecated settings were used in the configuration: %v; consider updating to the new settings: https://mobiusmdm.com/docs/using-mobius/configuration-files#settings",
				legacyKeys),
		}
	}
	...
	if appConfig.ServerSettings.ServerURL == "" {
		invalid.Append("server_url", "Mobius server URL must be present")
	} else {
	...
	if newAppConfig.AgentOptions != nil {
		...
		if err := mobius.ValidateJSONAgentOptions(ctx, svc.ds, *appConfig.AgentOptions, license.IsPremium()); err != nil {
			err = mobius.SuggestAgentOptionsCorrection(err)
			err = mobius.NewUserMessageError(err, http.StatusBadRequest)
	...
	mobius.ValidateGoogleCalendarIntegrations(appConfig.Integrations.GoogleCalendar, invalid)
	mobius.ValidateEnabledVulnerabilitiesIntegrations(appConfig.WebhookSettings.VulnerabilitiesWebhook, appConfig.Integrations, invalid)
	mobius.ValidateEnabledFailingPoliciesIntegrations(appConfig.WebhookSettings.FailingPoliciesWebhook, appConfig.Integrations, invalid)
	mobius.ValidateEnabledHostStatusIntegrations(appConfig.WebhookSettings.HostStatusWebhook, invalid)
	mobius.ValidateEnabledActivitiesWebhook(appConfig.WebhookSettings.ActivitiesWebhook, invalid)

	var conditionalAccessNoTeamUpdated bool
	if newAppConfig.Integrations.ConditionalAccessEnabled.Set {
		if err := mobius.ValidateConditionalAccessIntegration(ctx, svc, oldConditionalAccessEnabled.Value, newAppConfig.Integrations.ConditionalAccessEnabled.Value); err != nil {
			return nil, err
		}
	...
	// prevent using the output of mobiuscli get config as input to mobiuscli apply or this endpoint.
		...
		if appConfig.SMTPSettings.SMTPEnabled {
			if oldSMTPSettings != *appConfig.SMTPSettings || !appConfig.SMTPSettings.SMTPConfigured {
				if err = svc.sendTestEmail(ctx, appConfig); err != nil {
					return nil, mobius.NewInvalidArgumentError("SMTP Options", err.Error())
				}
			}
	...
	if newAppConfig.Integrations.Jira != nil || newAppConfig.Integrations.Zendesk != nil {
		delJira, err := mobius.ValidateJiraIntegrations(ctx, storedJiraByProjectKey, newAppConfig.Integrations.Jira)
		if err != nil {
			if errors.As(err, &mobius.IntegrationTestError{}) {
				return nil, ctxerr.Wrap(ctx, &mobius.BadRequestError{
					Message: err.Error(),
				})
			}
			return nil, ctxerr.Wrap(ctx, mobius.NewInvalidArgumentError("Jira integration", err.Error()))
		}
		appConfig.Integrations.Jira = newAppConfig.Integrations.Jira

		delZendesk, err := mobius.ValidateZendeskIntegrations(ctx, storedZendeskByGroupID, newAppConfig.Integrations.Zendesk)
		if err != nil {
			if errors.As(err, &mobius.IntegrationTestError{}) {
				return nil, ctxerr.Wrap(ctx, &mobius.BadRequestError{
					Message: err.Error(),
			...
			return nil, ctxerr.Wrap(ctx, mobius.NewInvalidArgumentError("Zendesk integration", err.Error()))
		}
		...
		if len(delJira)+len(delZendesk) > 0 {
			if err := svc.ds.DeleteIntegrationsFromTeams(ctx, mobius.Integrations{Jira: delJira, Zendesk: delZendesk}); err != nil {
	...
	if gitopsModeEnabled {
		if !license.IsPremium() {
			return nil, mobius.NewInvalidArgumentError("UI GitOpsMode: ", ErrMissingLicense.Error())
		}
		if gitopsRepoURL == "" {
			return nil, mobius.NewInvalidArgumentError("UI GitOps Mode: ", "Repository URL is required when GitOps mode is enabled")
		}
	}
	...
	if oldAppConfig.UIGitOpsMode.GitopsModeEnabled != appConfig.UIGitOpsMode.GitopsModeEnabled {
		...
		if gitopsModeEnabled {
			act = mobius.ActivityTypeEnabledGitOpsMode{}
		} else {
			act = mobius.ActivityTypeDisabledGitOpsMode{}
		}
	...
	if !license.IsPremium() {
		...
		appConfig.MobiusDesktop.TransparencyURL = ""
	}
	...
	case appConfig.WebhookSettings.ActivitiesWebhook.Enable && !oldAppConfig.WebhookSettings.ActivitiesWebhook.Enable:
		act := mobius.ActivityTypeEnabledActivityAutomations{WebhookUrl: appConfig.WebhookSettings.ActivitiesWebhook.DestinationURL}
		if err := svc.NewActivity(ctx, authz.UserFromContext(ctx), act); err != nil {
			return nil, ctxerr.Wrap(ctx, err, "create activity for enabled activity automations")
		}
	case !appConfig.WebhookSettings.ActivitiesWebhook.Enable && oldAppConfig.WebhookSettings.ActivitiesWebhook.Enable:
		act := mobius.ActivityTypeDisabledActivityAutomations{}
		if err := svc.NewActivity(ctx, authz.UserFromContext(ctx), act); err != nil {
	...
	case appConfig.WebhookSettings.ActivitiesWebhook.Enable &&
		...
		act := mobius.ActivityTypeEditedActivityAutomations{
	...
	case caStatusAdded:
		if err = svc.NewActivity(ctx, authz.UserFromContext(ctx), mobius.ActivityAddedNDESSCEPProxy{}); err != nil {
			return nil, ctxerr.Wrap(ctx, err, "create activity for added NDES SCEP proxy")
		}
	case caStatusEdited:
		if err = svc.NewActivity(ctx, authz.UserFromContext(ctx), mobius.ActivityEditedNDESSCEPProxy{}); err != nil {
			return nil, ctxerr.Wrap(ctx, err, "create activity for edited NDES SCEP proxy")
		}
	case caStatusDeleted:
		// Delete stored password
		if err := svc.ds.HardDeleteMDMConfigAsset(ctx, mobius.MDMAssetNDESPassword); err != nil {
			return nil, ctxerr.Wrap(ctx, err, "delete NDES SCEP password")
		}
		if err = svc.NewActivity(ctx, authz.UserFromContext(ctx), mobius.ActivityDeletedNDESSCEPProxy{}); err != nil {
	...
	for caName, status := range caStatus.digicert {
		switch status {
		case caStatusAdded:
			if err = svc.NewActivity(ctx, authz.UserFromContext(ctx), mobius.ActivityAddedDigiCert{Name: caName}); err != nil {
				return nil, ctxerr.Wrap(ctx, err, "create activity for added DigiCert CA")
			}
		case caStatusEdited:
			if err = svc.NewActivity(ctx, authz.UserFromContext(ctx), mobius.ActivityEditedDigiCert{Name: caName}); err != nil {
		...
		case caStatusDeleted:
			...
			if err = svc.NewActivity(ctx, authz.UserFromContext(ctx), mobius.ActivityDeletedDigiCert{Name: caName}); err != nil {
	...
	for caName, status := range caStatus.customSCEPProxy {
		switch status {
		case caStatusAdded:
			if err = svc.NewActivity(ctx, authz.UserFromContext(ctx), mobius.ActivityAddedCustomSCEPProxy{Name: caName}); err != nil {
				return nil, ctxerr.Wrap(ctx, err, "create activity for added Custom SCEP Proxy")
			}
		case caStatusEdited:
			if err = svc.NewActivity(ctx, authz.UserFromContext(ctx), mobius.ActivityEditedCustomSCEPProxy{Name: caName}); err != nil {
		...
		case caStatusDeleted:
			...
			if err = svc.NewActivity(ctx, authz.UserFromContext(ctx), mobius.ActivityDeletedCustomSCEPProxy{Name: caName}); err != nil {
	...
	if vppAssignmentsDefined {
		// 1. Reset teams for VPP tokens that exist in Mobius but aren't present in the config being passed
		clear(tokensInCfg)
		...
		for tokenID, tokenTeams := range vppAssignments {
			if _, err := svc.ds.UpdateVPPTokenTeams(ctx, tokenID, tokenTeams); err != nil {
				var errTokConstraint mobius.ErrVPPTokenTeamConstraint
				if errors.As(err, &errTokConstraint) {
					return nil, ctxerr.Wrap(ctx, mobius.NewUserMessageError(errTokConstraint, http.StatusConflict))
				}
				return nil, ctxerr.Wrap(ctx, err, "saving ABM token assignments")
			}
		}
	...
	if oldAgentOptions != newAgentOptions {
		if err := svc.NewActivity(
			...
			mobius.ActivityTypeEditedAgentOptions{
				Global: true,
			},
		); err != nil {
	...
	//
	if err := svc.processAppleOSUpdateSettings(ctx, license, mobius.MacOS,
	...
	if err := svc.processAppleOSUpdateSettings(ctx, license, mobius.IOS,
		oldAppConfig.MDM.IOSUpdates,
		appConfig.MDM.IOSUpdates,
	); err != nil {
		return nil, ctxerr.Wrap(ctx, err, "process iOS OS updates config change")
	}
	if err := svc.processAppleOSUpdateSettings(ctx, license, mobius.IPadOS,
	...
	if !oldAppConfig.MDM.WindowsUpdates.Equal(appConfig.MDM.WindowsUpdates) {
		...
		if err := svc.NewActivity(
			ctx,
			authz.UserFromContext(ctx),
			mobius.ActivityTypeEditedWindowsUpdates{
	...
	if mdmEnableEndUserAuthChanged {
		...
		if appConfig.MDM.MacOSSetup.EnableEndUserAuthentication {
			act = mobius.ActivityTypeEnabledMacosSetupEndUserAuth{}
		} else {
			act = mobius.ActivityTypeDisabledMacosSetupEndUserAuth{}
		}
		if err := svc.NewActivity(ctx, authz.UserFromContext(ctx), act); err != nil {
	...
	if oldAppConfig.MDM.WindowsEnabledAndConfigured != appConfig.MDM.WindowsEnabledAndConfigured {
		...
		if appConfig.MDM.WindowsEnabledAndConfigured {
			act = mobius.ActivityTypeEnabledWindowsMDM{}
		} else {
			act = mobius.ActivityTypeDisabledWindowsMDM{}
		}
		if err := svc.NewActivity(ctx, authz.UserFromContext(ctx), act); err != nil {
			return nil, ctxerr.Wrapf(ctx, err, "create activity %s", act.ActivityName())
		}
	...
	if appConfig.MDM.WindowsEnabledAndConfigured && oldAppConfig.MDM.WindowsMigrationEnabled != appConfig.MDM.WindowsMigrationEnabled {
		...
		if appConfig.MDM.WindowsMigrationEnabled {
			act = mobius.ActivityTypeEnabledWindowsMDMMigration{}
		} else {
			act = mobius.ActivityTypeDisabledWindowsMDMMigration{}
		}
		if err := svc.NewActivity(ctx, authz.UserFromContext(ctx), act); err != nil {
			return nil, ctxerr.Wrapf(ctx, err, "create activity %s", act.ActivityName())
		}
	...
	if conditionalAccessNoTeamUpdated {
		if appConfig.Integrations.ConditionalAccessEnabled.Value {
			if err := svc.NewActivity(
				ctx,
				authz.UserFromContext(ctx),
				mobius.ActivityTypeEnabledConditionalAccessAutomations{
					TeamID:   nil,
					TeamName: "",
				},
			); err != nil {
			...
			if err := svc.NewActivity(
				ctx,
				authz.UserFromContext(ctx),
				mobius.ActivityTypeDisabledConditionalAccessAutomations{
					TeamID:   nil,
					TeamName: "",
				},
			); err != nil {
...
func (svc *Service) processAppConfigCAs(ctx context.Context, newAppConfig *mobius.AppConfig, oldAppConfig *mobius.AppConfig,
	appConfig *mobius.AppConfig, invalid *mobius.InvalidArgumentError,
) (appConfigCAStatus, error) {
	var invalidLicense bool
	mobiusLicense, _ := license.FromContext(ctx)
	if newAppConfig.Integrations.NDESSCEPProxy.Set && newAppConfig.Integrations.NDESSCEPProxy.Valid && !mobiusLicense.IsPremium() {
		invalid.Append("integrations.ndes_scep_proxy", ErrMissingLicense.Error())
		appConfig.Integrations.NDESSCEPProxy.Valid = false
		invalidLicense = true
	}
	if newAppConfig.Integrations.DigiCert.Set && newAppConfig.Integrations.DigiCert.Valid && !mobiusLicense.IsPremium() {
		invalid.Append("integrations.digicert", ErrMissingLicense.Error())
		appConfig.Integrations.DigiCert.Valid = false
		invalidLicense = true
	}
	if newAppConfig.Integrations.CustomSCEPProxy.Set && newAppConfig.Integrations.CustomSCEPProxy.Valid && !mobiusLicense.IsPremium() {
	...
	default:
		...
		appConfig.Integrations.NDESSCEPProxy.Value.URL = mobius.Preprocess(newAppConfig.Integrations.NDESSCEPProxy.Value.URL)
		appConfig.Integrations.NDESSCEPProxy.Value.AdminURL = mobius.Preprocess(newAppConfig.Integrations.NDESSCEPProxy.Value.AdminURL)
		appConfig.Integrations.NDESSCEPProxy.Value.Username = mobius.Preprocess(newAppConfig.Integrations.NDESSCEPProxy.Value.Username)
		// do not preprocess password
		if len(svc.config.Server.PrivateKey) == 0 {
			invalid.Append("integrations.ndes_scep_proxy",
				"Cannot encrypt NDES password. Missing required private key. Learn how to configure the private key here: https://mobiusmdm.com/learn-more-about/mobius-server-private-key")
		}
		...
		if !oldAppConfig.Integrations.NDESSCEPProxy.Valid {
			...
			if newSCEPProxy.AdminURL != oldSCEPProxy.AdminURL ||
				...
				(newSCEPProxy.Password != "" && newSCEPProxy.Password != mobius.MaskedPassword) {
				result.ndes = caStatusEdited
	...
	default:
		...
		if len(svc.config.Server.PrivateKey) == 0 {
			invalid.Append("integrations.digicert",
				"Cannot encrypt DigiCert API token. Missing required private key. Learn how to configure the private key here: https://mobiusmdm."+
					"com/learn-more-about/mobius-server-private-key")
			break
		}
		additionalDigiCertValidationNeeded = true
		for _, ca := range newAppConfig.Integrations.DigiCert.Value {
			ca.Name = mobius.Preprocess(ca.Name)
			...
			// Validate URL
			ca.URL = mobius.Preprocess(ca.URL)
			...
			ca.ProfileID = mobius.Preprocess(ca.ProfileID)
		...
		for i, newCA := range appConfig.Integrations.DigiCert.Value {
			...
			for _, oldCA := range remainingOldCAs {
				...
				case newCA.Equals(&oldCA):
					...
					appConfig.Integrations.DigiCert.Value[i].APIToken = mobius.MaskedPassword
					found = true
				case newCA.Name == oldCA.Name:
					// changed
					found = true
					needToVerify = newCA.NeedToVerify(&oldCA)
					if needToVerify && (len(newCA.APIToken) == 0 || newCA.APIToken == mobius.MaskedPassword) {
			...
			if !found {
				if len(newCA.APIToken) == 0 || newCA.APIToken == mobius.MaskedPassword {
					invalid.Append("integrations.digicert.api_token",
		...
		if len(svc.config.Server.PrivateKey) == 0 {
			invalid.Append("integrations.custom_scep_proxy",
				"Cannot encrypt SCEP challenge. Missing required private key. Learn how to configure the private key here: "+
					"https://mobiusmdm.com/learn-more-about/mobius-server-private-key")
			break
		}
		...
		for i, newCA := range appConfig.Integrations.CustomSCEPProxy.Value {
			...
			for _, oldCA := range remainingOldCAs {
				...
				case newCA.Equals(&oldCA):
					...
					appConfig.Integrations.CustomSCEPProxy.Value[i].Challenge = mobius.MaskedPassword
					found = true
				case newCA.Name == oldCA.Name:
					// changed
					if len(newCA.Challenge) == 0 || newCA.Challenge == mobius.MaskedPassword {
...
func (svc *Service) populateDigiCertAPITokens(ctx context.Context, remainingOldCAs []mobius.DigiCertIntegration) error {
	assets, err := svc.ds.GetAllCAConfigAssetsByType(ctx, mobius.CAConfigDigiCert)
...
func (svc *Service) populateCustomSCEPChallenges(ctx context.Context, remainingOldCAs []mobius.CustomSCEPProxyIntegration) error {
	assets, err := svc.ds.GetAllCAConfigAssetsByType(ctx, mobius.CAConfigCustomSCEPProxy)
	if err != nil && !mobius.IsNotFound(err) {
...
func filterDeletedDigiCertCAs(oldAppConfig *mobius.AppConfig, newAppConfig *mobius.AppConfig,
	result *appConfigCAStatus,
) []mobius.DigiCertIntegration {
	remainingOldCAs := make([]mobius.DigiCertIntegration, 0, len(oldAppConfig.Integrations.DigiCert.Value))
...
// It mutates the provided result to set a deleted status where applicable and returns a list of the remaining (non-deleted) integrations.
func filterDeletedCustomSCEPCAs(oldAppConfig *mobius.AppConfig, newAppConfig *mobius.AppConfig,
	result *appConfigCAStatus,
) []mobius.CustomSCEPProxyIntegration {
	remainingOldCAs := make([]mobius.CustomSCEPProxyIntegration, 0, len(oldAppConfig.Integrations.CustomSCEPProxy.Value))
...
func validateCAName(name string, caType string, allCANames map[string]struct{}, invalid *mobius.InvalidArgumentError) bool {
	if name == "NDES" {
...
func validateCACN(cn string, invalid *mobius.InvalidArgumentError) bool {
	...
	mobiusVars := findMobiusVariables(cn)
	for mobiusVar := range mobiusVars {
		switch mobiusVar {
		case mobius.MobiusVarHostEndUserEmailIDP, mobius.MobiusVarHostHardwareSerial:
			// ok
		default:
			invalid.Append("integrations.digicert.certificate_common_name", "MOBIUS_VAR_"+mobiusVar+" is not allowed in CA Common Name (CN)")
			return false
		}
	}
	return true
...
func validateSeatID(seatID string, invalid *mobius.InvalidArgumentError) bool {
	if len(strings.TrimSpace(seatID)) == 0 {
		invalid.Append("integrations.digicert.certificate_seat_id", "CA Seat ID cannot be empty")
		return false
	}
	mobiusVars := findMobiusVariables(seatID)
	for mobiusVar := range mobiusVars {
		switch mobiusVar {
		case mobius.MobiusVarHostEndUserEmailIDP, mobius.MobiusVarHostHardwareSerial:
			// ok
		default:
			invalid.Append("integrations.digicert.certificate_seat_id", "MOBIUS_VAR_"+mobiusVar+" is not allowed in DigiCert Seat ID")
			return false
		}
	}
	return true
...
func validateUserPrincipalNames(userPrincipalNames []string, invalid *mobius.InvalidArgumentError) bool {
	...
	mobiusVars := findMobiusVariables(userPrincipalNames[0])
	for mobiusVar := range mobiusVars {
		switch mobiusVar {
		case mobius.MobiusVarHostEndUserEmailIDP, mobius.MobiusVarHostHardwareSerial:
			// ok
		default:
			invalid.Append("integrations.digicert.certificate_user_principal_names",
				"MOBIUS_VAR_"+mobiusVar+" is not allowed in CA User Principal Name")
			return false
		}
	}
	return true
...
func (svc *Service) processAppleOSUpdateSettings(
	...
	appleDevice mobius.AppleDevice,
	oldOSUpdateSettings mobius.AppleOSUpdateSettings,
	newOSUpdateSettings mobius.AppleOSUpdateSettings,
) error {
	if oldOSUpdateSettings.MinimumVersion.Value != newOSUpdateSettings.MinimumVersion.Value ||
		...
		var activity mobius.ActivityDetails
		switch appleDevice {
		case mobius.MacOS:
			activity = mobius.ActivityTypeEditedMacOSMinVersion{
				MinimumVersion: newOSUpdateSettings.MinimumVersion.Value,
				Deadline:       newOSUpdateSettings.Deadline.Value,
			}
		case mobius.IOS:
			activity = mobius.ActivityTypeEditedIOSMinVersion{
				MinimumVersion: newOSUpdateSettings.MinimumVersion.Value,
				Deadline:       newOSUpdateSettings.Deadline.Value,
			}
		case mobius.IPadOS:
			activity = mobius.ActivityTypeEditedIPadOSMinVersion{
				MinimumVersion: newOSUpdateSettings.MinimumVersion.Value,
				Deadline:       newOSUpdateSettings.Deadline.Value,
			}
		}
...
func (svc *Service) HasCustomSetupAssistantConfigurationWebURL(ctx context.Context, teamID *uint) (bool, error) {
	...
	if !ok || !az.Checked() {
		return false, mobius.NewAuthRequiredError("method requires previous authorization")
	}
...
func (svc *Service) validateMDM(
	...
	// we want to use `oldMdm` here as this boolean is set by the mobius
	// server at startup and can't be modified by the user
	if !oldMdm.EnabledAndConfigured {
		if len(mdm.MacOSSettings.CustomSettings) > 0 && !mobius.MDMProfileSpecsMatch(mdm.MacOSSettings.CustomSettings, oldMdm.MacOSSettings.CustomSettings) {
			invalid.Append("macos_settings.custom_settings",
				`Couldn't update macos_settings because MDM features aren't turned on in Mobius. Use mobiuscli generate mdm-apple and then mobius serve with mdm configuration to turn on MDM features.`)
		}
		...
		if mdm.MacOSSetup.MacOSSetupAssistant.Value != "" && oldMdm.MacOSSetup.MacOSSetupAssistant.Value != mdm.MacOSSetup.MacOSSetupAssistant.Value {
			invalid.Append("macos_setup.macos_setup_assistant",
				`Couldn't update macos_setup because MDM features aren't turned on in Mobius. Use mobiuscli generate mdm-apple and then mobius serve with mdm configuration to turn on MDM features.`)
		}
	...
	checkCustomSettings := func(prefix string, customSettings []mobius.MDMProfileSpec) {
	...
	if !mdm.WindowsEnabledAndConfigured {
		if mdm.WindowsSettings.CustomSettings.Set &&
			...
			!mobius.MDMProfileSpecsMatch(mdm.WindowsSettings.CustomSettings.Value, oldMdm.WindowsSettings.CustomSettings.Value) {
			invalid.Append("windows_settings.custom_settings",
				`Couldn’t edit windows_settings.custom_settings. Windows MDM isn’t turned on. This can be enabled by setting "controls.windows_enabled_and_configured: true" in the default configuration. Visit https://mobiusmdm.com/guides/windows-mdm-setup and https://mobiusmdm.com/docs/configuration/yaml-files#controls to learn more about enabling MDM.`)
		}
	}
	...
	if mdm.MacOSSetup.EnableEndUserAuthentication != oldMdm.MacOSSetup.EnableEndUserAuthentication {
		...
		if hasCustomConfigurationWebURL {
			invalid.Append("end_user_authentication", mobius.EndUserAuthDEPWebURLConfiguredErrMsg)
		}
	}
	...
	if !svc.config.MDM.IsMicrosoftWSTEPSet() {
		if mdm.WindowsEnabledAndConfigured {
			invalid.Append("mdm.windows_enabled_and_configured", "Couldn't turn on Windows MDM. Please configure Mobius with a certificate and key pair first.")
			return nil
		}
	}
...
func (svc *Service) validateABMAssignments(
	...
	if name := mdm.DeprecatedAppleBMDefaultTeam; name != "" && name != oldMdm.DeprecatedAppleBMDefaultTeam {
		...
		return []*mobius.ABMToken{tok}, nil
	}
	...
	if mdm.AppleBusinessManager.Set && len(mdm.AppleBusinessManager.Value) > 0 {
		...
		tokensByName := map[string]*mobius.ABMToken{}
		...
		var tokensToSave []*mobius.ABMToken
...
func (svc *Service) validateVPPAssignments(
	ctx context.Context,
	volumePurchasingProgramInfo []mobius.MDMAppleVolumePurchasingProgramInfo,
	invalid *mobius.InvalidArgumentError,
	license *mobius.LicenseInfo,
) (map[uint][]uint, error) {
	...
	teamsByName := map[string]uint{mobius.TeamNameNoTeam: 0}
	for _, tm := range teams {
		teamsByName[tm.Name] = tm.ID
	}
	...
	tokensByLocation := map[string]*mobius.VPPTokenDB{}
	for _, token := range tokens {
		// The default assignments for all tokens is "no team"
		// (ie: team_id IS NULL), here we reset the assignments
		// for all tokens, those will be re-added below.
	...
	for _, vpp := range volumePurchasingProgramInfo {
		for _, tmName := range vpp.Teams {
			if _, ok := teamsByName[norm.NFC.String(tmName)]; !ok && tmName != mobius.TeamNameAllTeams {
		...
		for _, teamName := range vpp.Teams {
			if teamName == mobius.TeamNameAllTeams {
				if len(vpp.Teams) > 1 {
					invalid.Appendf("mdm.volume_purchasing_program", "token cannot belong to %s and other teams", mobius.TeamNameAllTeams)
					return nil, nil
				}
				tokenTeams = []uint{}
				break
...
func validateSSOProviderSettings(incoming, existing mobius.SSOProviderSettings, invalid *mobius.InvalidArgumentError) {
...
func validateSSOSettings(p mobius.AppConfig, existing *mobius.AppConfig, invalid *mobius.InvalidArgumentError, license *mobius.LicenseInfo) {
	if p.SSOSettings != nil && p.SSOSettings.EnableSSO {

		var existingSSOProviderSettings mobius.SSOProviderSettings
...
func applyEnrollSecretSpecEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
	req := request.(*applyEnrollSecretSpecRequest)
	err := svc.ApplyEnrollSecretSpec(
		ctx, req.Spec, mobius.ApplySpecOptions{
			DryRun: req.DryRun,
		},
	)
	if err != nil {
...
func (svc *Service) ApplyEnrollSecretSpec(ctx context.Context, spec *mobius.EnrollSecretSpec, applyOpts mobius.ApplySpecOptions) error {
	if err := svc.authz.Authorize(ctx, &mobius.EnrollSecret{}, mobius.ActionWrite); err != nil {
		return err
	}
	if len(spec.Secrets) > mobius.MaxEnrollSecretsCount {
		return ctxerr.Wrap(ctx, mobius.NewInvalidArgumentError("secrets", "too many secrets"))
	}
	...
	if svc.config.Packaging.GlobalEnrollSecret != "" {
		return ctxerr.New(ctx, "enroll secret cannot be changed when mobius_packaging.global_enroll_secret is set")
	}
	...
	if applyOpts.DryRun {
		for _, s := range spec.Secrets {
			...
			if !available {
				return ctxerr.Wrap(ctx, mobius.NewInvalidArgumentError("secrets", "a provided global enroll secret is already being used"))
			}
		}
		return nil
	}
...
func getEnrollSecretSpecEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) GetEnrollSecretSpec(ctx context.Context) (*mobius.EnrollSecretSpec, error) {
	if err := svc.authz.Authorize(ctx, &mobius.EnrollSecret{}, mobius.ActionRead); err != nil {
		return nil, err
	}
	...
	return &mobius.EnrollSecretSpec{Secrets: secrets}, nil
}
...
func versionEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) Version(ctx context.Context) (*version.Info, error) {
	if err := svc.authz.Authorize(ctx, &mobius.Version{}, mobius.ActionRead); err != nil {
		return nil, err
	}
...
func getCertificateEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) HostFeatures(ctx context.Context, host *mobius.Host) (*mobius.Features, error) {
