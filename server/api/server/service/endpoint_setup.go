...
const (
	starterLibraryURL = "https://raw.githubusercontent.com/MobiusDM/mobius/main/docs/01-Using-Mobius/starter-library/starter-library.yml"
	scriptsBaseURL    = "https://raw.githubusercontent.com/MobiusDM/mobius/main/"
)
...
func makeSetupEndpoint(svc mobius.Service, logger kitlog.Logger) endpoint.Endpoint {
	return func(ctx context.Context, request interface{}) (interface{}, error) {
		...
		adminPayload.GlobalRole = ptr.String(mobius.RoleAdmin)
...
func applyStarterLibrary(
	...
	httpClientFactory func(opts ...mobiushttp.ClientOpt) *http.Client,
	...
	httpClient := httpClientFactory(mobiushttp.WithTimeout(5 * time.Second))
	...
	tempDir, err := os.MkdirTemp("", "mobius-scripts-*")
	if err != nil {
...
func downloadAndUpdateScripts(ctx context.Context, specs *spec.Group, scriptNames []string, tempDir string, logger kitlog.Logger) error {
	...
	httpClient := mobiushttp.NewClient(mobiushttp.WithTimeout(5 * time.Second))
