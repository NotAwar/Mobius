...
import (
	...
	"github.com/MobiusDM/mobius/server/api/server/mdm/microsoft/syncml"
	nanomdm "github.com/MobiusDM/mobius/server/api/server/mdm/nanomdm/mdm"
...
func getAppleMDMEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) GetAppleMDM(ctx context.Context) (*mobius.AppleMDM, error) {
	if err := svc.authz.Authorize(ctx, &mobius.AppleMDM{}, mobius.ActionRead); err != nil {
		return nil, err
	}

	apns, err := assets.X509Cert(ctx, svc.ds, mobius.MDMAssetAPNSCert)
	if err != nil {
		return nil, ctxerr.Wrap(ctx, err, "parse certificate")
	}

	appleMDM := &mobius.AppleMDM{
...
func getAppleBMEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) GetAppleBM(ctx context.Context) (*mobius.AppleBM, error) {
...
func requestMDMAppleCSREndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) RequestMDMAppleCSR(ctx context.Context, email, org string) (*mobius.AppleCSR, error) {
	if err := svc.authz.Authorize(ctx, &mobius.AppleCSR{}, mobius.ActionWrite); err != nil {
		return nil, err
	}

	if err := mobius.ValidateEmail(email); err != nil {
		if strings.TrimSpace(email) == "" {
			return nil, ctxerr.Wrap(ctx, mobius.NewInvalidArgumentError("email_address", "missing email address"))
		}
		return nil, ctxerr.Wrap(ctx, mobius.NewInvalidArgumentError("email_address", fmt.Sprintf("invalid email address: %v", err)))
	}
	if strings.TrimSpace(org) == "" {
		return nil, ctxerr.Wrap(ctx, mobius.NewInvalidArgumentError("organization", "missing organization"))
	}
	...
	// request the signed APNs CSR from mobiusmdm.com
	client := mobiushttp.NewClient(mobiushttp.WithTimeout(10 * time.Second))
	if err := apple_mdm.GetSignedAPNSCSR(client, apnsCSR); err != nil {
		if ferr, ok := err.(apple_mdm.MobiusWebsiteError); ok {
			status := http.StatusBadGateway
			if ferr.Status >= 400 && ferr.Status <= 499 {
				// TODO: mobiusmdm.com returns a genereric "Bad
				// Request" message, we should coordinate and
				// stablish a response schema from which we can get
				// the invalid field and use
				// mobius.NewInvalidArgumentError instead
				//
			...
			return nil, ctxerr.Wrap(
				ctx,
				mobius.NewUserMessageError(
					fmt.Errorf("MobiusDM CSR request failed: %w", err),
					status,
				),
			)
		}
...
func (svc *Service) VerifyMDMAppleConfigured(ctx context.Context) error {
	...
	if !appCfg.MDM.EnabledAndConfigured {
		// skipauth: Authorization is currently for user endpoints only.
		svc.authz.SkipAuthorization(ctx)
		return mobius.ErrMDMNotConfigured
	}
...
func createMDMEULAEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func getMDMEULAEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) MDMGetEULABytes(ctx context.Context, token string) (*mobius.MDMEULA, error) {
	// skipauth: No authorization check needed due to implementation returning
	// only license error.
	svc.authz.SkipAuthorization(ctx)
...
func getMDMEULAMetadataEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) MDMGetEULAMetadata(ctx context.Context) (*mobius.MDMEULA, error) {
	// skipauth: No authorization check needed due to implementation returning
	// only license error.
	svc.authz.SkipAuthorization(ctx)
...
func deleteMDMEULAEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) VerifyMDMWindowsConfigured(ctx context.Context) error {
	...
	if !appCfg.MDM.WindowsEnabledAndConfigured {
		// skipauth: Authorization is currently for user endpoints only.
		svc.authz.SkipAuthorization(ctx)
		return mobius.ErrWindowsMDMNotConfigured
	}
...
func runMDMCommandEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) RunMDMCommand(ctx context.Context, rawBase64Cmd string, hostUUIDs []string) (result *mobius.CommandEnqueueResult, err error) {
	hosts, err := svc.authorizeAllHostsTeams(ctx, hostUUIDs, mobius.ActionWrite, &mobius.MDMCommandAuthz{})
	if err != nil {
		return nil, err
	}
	if len(hosts) == 0 {
		err := mobius.NewInvalidArgumentError("host_uuids", "No hosts targeted. Make sure you provide a valid UUID.").WithStatus(http.StatusNotFound)
		return nil, ctxerr.Wrap(ctx, err, "no host received")
	}

	connectedMap, err := svc.ds.AreHostsConnectedToMobiusMDM(ctx, hosts)
	if err != nil {
		return nil, ctxerr.Wrap(ctx, err, "checking if hosts are connected to Mobius")
	}

	platforms := make(map[string]bool)
	for _, h := range hosts {
		if !connectedMap[h.UUID] {
			err := mobius.NewInvalidArgumentError("host_uuids", "Can't run the MDM command because one or more hosts have MDM turned off. Run the following command to see a list of hosts with MDM on: mobiuscli get hosts --mdm.").WithStatus(http.StatusPreconditionFailed)
			return nil, ctxerr.Wrap(ctx, err, "check host mdm enrollment")
		}
		platforms[h.MobiusPlatform()] = true
	}
	if len(platforms) != 1 {
		err := mobius.NewInvalidArgumentError("host_uuids", "All hosts must be on the same platform.")
	...
	if !mobius.MDMSupported(commandPlatform) {
		err := mobius.NewInvalidArgumentError("host_uuids", "Invalid platform. You can only run MDM commands on Windows or Apple hosts.")
		return nil, ctxerr.Wrap(ctx, err, "check host platform")
	}
	...
	case "windows":
		if err := svc.VerifyMDMWindowsConfigured(ctx); err != nil {
			err := mobius.NewInvalidArgumentError("host_uuids", mobius.WindowsMDMNotConfiguredMessage).WithStatus(http.StatusBadRequest)
			return nil, ctxerr.Wrap(ctx, err, "check windows MDM enabled")
		}
	default:
		if err := svc.VerifyMDMAppleConfigured(ctx); err != nil {
			err := mobius.NewInvalidArgumentError("host_uuids", mobius.AppleMDMNotConfiguredMessage).WithStatus(http.StatusBadRequest)
...
func (svc *Service) enqueueAppleMDMCommand(ctx context.Context, rawXMLCmd []byte, deviceIDs []string) (result *mobius.CommandEnqueueResult, err error) {
	cmd, err := nanomdm.DecodeCommand(rawXMLCmd)
	if err != nil {
		err = mobius.NewInvalidArgumentError("command", "unable to decode plist command").WithStatus(http.StatusUnsupportedMediaType)
	...
	if err := svc.mdmAppleCommander.EnqueueCommand(ctx, deviceIDs, string(rawXMLCmd)); err != nil {
		...
		if errors.As(err, &apnsErr) {
			...
			if len(failedUUIDs) < len(deviceIDs) {
				...
				// of failed uuids.
				return &mobius.CommandEnqueueResult{
			...
			err := mobius.NewBadGatewayError("Apple push notificiation service", err)
			...
			if mysqlErr.Number == mysqlerr.ER_NO_REFERENCED_ROW_2 {
				err := mobius.NewInvalidArgumentError(
					"device_ids",
...
func (svc *Service) enqueueMicrosoftMDMCommand(ctx context.Context, rawXMLCmd []byte, deviceIDs []string) (result *mobius.CommandEnqueueResult, err error) {
	cmdMsg, err := mobius.ParseWindowsMDMCommand(rawXMLCmd)
	if err != nil {
		err = mobius.NewInvalidArgumentError("command", err.Error())
...
func getMDMCommandResultsEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) GetMDMCommandResults(ctx context.Context, commandUUID string) ([]*mobius.MDMCommandResult, error) {
...
func (svc *Service) getDeviceSoftwareMDMCommandResults(ctx context.Context, commandUUID string) ([]*mobius.MDMCommandResult, error) {
...
func listMDMCommandsEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
	req := request.(*listMDMCommandsRequest)
	results, err := svc.ListMDMCommands(ctx, &mobius.MDMCommandListOptions{
		ListOptions: req.ListOptions,
		Filters:     mobius.MDMCommandFilters{HostIdentifier: req.HostIdentifier, RequestType: req.RequestType},
	})
	if err != nil {
...
func (svc *Service) ListMDMCommands(ctx context.Context, opts *mobius.MDMCommandListOptions) ([]*mobius.MDMCommand, error) {
	// first, authorize that the user has the right to list hosts
	if err := svc.authz.Authorize(ctx, &mobius.Host{}, mobius.ActionList); err != nil {
		return nil, ctxerr.Wrap(ctx, err)
	}
	...
	// MDM Apple commands.
	results, err := svc.ds.ListMDMCommands(ctx, mobius.TeamFilter{
		User:            vc.User,
	...
	if authzErr != nil {
		...
		allowedResults := make([]*mobius.MDMCommand, 0, len(results))
		for _, res := range results {
			var id uint
			if res.TeamID != nil {
				id = *res.TeamID
	...
	if len(results) == 0 && opts.Filters.HostIdentifier != "" {
		...
		var nve mobius.NotFoundError
		if errors.As(err, &nve) {
			return nil, mobius.NewInvalidArgumentError("Invalid Host", mobius.HostIdentiferNotFound).WithStatus(http.StatusNotFound)
		}
	}
...
type getMDMDiskEncryptionSummaryResponse struct {
	*mobius.MDMDiskEncryptionSummary
	Err error `json:"error,omitempty"`
}

func (r getMDMDiskEncryptionSummaryResponse) Error() error { return r.Err }

func getMDMDiskEncryptionSummaryEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) GetMDMDiskEncryptionSummary(ctx context.Context, teamID *uint) (*mobius.MDMDiskEncryptionSummary, error) {
	// skipauth: No authorization check needed due to implementation returning
	// only license error.
	svc.authz.SkipAuthorization(ctx)
...
func getMDMProfilesSummaryEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
// the specified authzAction (e.g. mobius.ActionWrite) for all the hosts' teams
...
func (svc *Service) authorizeAllHostsTeams(ctx context.Context, hostUUIDs []string, authzAction any, authorizer mobius.TeamIDSetter) ([]*mobius.Host, error) {
	// load hosts (lite) by uuids, check that the user has the rights to run
	// commands for every affected team.
	if err := svc.authz.Authorize(ctx, &mobius.Host{}, mobius.ActionSelectiveList); err != nil {
		return nil, err
	}
...
type getMDMConfigProfileResponse struct {
	*mobius.MDMConfigProfilePayload
	Err error `json:"error,omitempty"`
}

func (r getMDMConfigProfileResponse) Error() error { return r.Err }

func getMDMConfigProfileEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
	...
	if isAppleProfileUUID(req.ProfileUUID) {
		...
		return &getMDMConfigProfileResponse{
			MDMConfigProfilePayload: mobius.NewMDMConfigProfilePayloadFromApple(cp),
		}, nil
	}
	...
	if isAppleDeclarationUUID(req.ProfileUUID) {
		...
		return &getMDMConfigProfileResponse{
			MDMConfigProfilePayload: mobius.NewMDMConfigProfilePayloadFromAppleDDM(decl),
		}, nil
	}
	...
	return &getMDMConfigProfileResponse{
		MDMConfigProfilePayload: mobius.NewMDMConfigProfilePayloadFromWindows(cp),
	}, nil
}

func (svc *Service) GetMDMWindowsConfigProfile(ctx context.Context, profileUUID string) (*mobius.MDMWindowsConfigProfile, error) {
...
func deleteMDMConfigProfileEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) DeleteMDMWindowsConfigProfile(ctx context.Context, profileUUID string) error {
	...
	if err := svc.VerifyMDMWindowsConfigured(ctx); err != nil {
		err := mobius.NewInvalidArgumentError("profile_uuid", mobius.WindowsMDMNotConfiguredMessage).WithStatus(http.StatusBadRequest)
	...
	if err := svc.authz.Authorize(ctx, &mobius.MDMConfigProfileAuthz{TeamID: prof.TeamID}, mobius.ActionWrite); err != nil {
		return ctxerr.Wrap(ctx, err)
	}

	// prevent deleting Mobius-managed profiles (e.g., Windows OS Updates profile controlled by the OS Updates settings)
	mobiusNames := mdm.MobiusReservedProfileNames()
	if _, ok := mobiusNames[prof.Name]; ok {
		err := &mobius.BadRequestError{Message: "Profiles managed by Mobius can't be deleted using this endpoint."}
	...
	if err := svc.NewActivity(
		ctx, authz.UserFromContext(ctx), &mobius.ActivityTypeDeletedWindowsProfile{
			TeamID:      actTeamID,
...
func isAppleDeclarationUUID(profileUUID string) bool {
	return strings.HasPrefix(profileUUID, mobius.MDMAppleDeclarationUUIDPrefix)
}
...
func (newMDMConfigProfileRequest) DecodeRequest(ctx context.Context, r *http.Request) (interface{}, error) {
	...
	if decoded.Profile.Size > 1024*1024 {
		return nil, mobius.NewInvalidArgumentError("mdm", "maximum configuration profile file size is 1 MB")
	}

	// add labels
	var existsInclAll, existsInclAny, existsExclAny, existsDepr bool
	var deprecatedLabels []string
	decoded.LabelsIncludeAll, existsInclAll = r.MultipartForm.Value[string(mobius.LabelsIncludeAll)]
	decoded.LabelsIncludeAny, existsInclAny = r.MultipartForm.Value[string(mobius.LabelsIncludeAny)]
	decoded.LabelsExcludeAny, existsExclAny = r.MultipartForm.Value[string(mobius.LabelsExcludeAny)]
	...
	if count > 1 {
		return nil, &mobius.BadRequestError{Message: `Only one of "labels_exclude_any", "labels_include_all", "labels_include_any", or "labels" can be included.`}
	}
	if existsDepr {
...
func newMDMConfigProfileEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
			...
			if err != nil {
				...
				if strings.Contains(errStr, "MDMAppleDeclaration.Name") && strings.Contains(errStr, "already exists") {
					return &newMDMConfigProfileResponse{
						Err: mobius.NewInvalidArgumentError("profile name", SameProfileNameUploadErrorMsg).WithStatus(http.StatusConflict),
					}, nil
				}
				return &newMDMConfigProfileResponse{Err: err}, nil
			}
...
func (svc *Service) NewMDMUnsupportedConfigProfile(ctx context.Context, teamID uint, filename string) error {
	...
	return &mobius.BadRequestError{Message: "Couldn't add profile. The file should be a .mobileconfig, XML, or JSON file."}
}

func (svc *Service) NewMDMWindowsConfigProfile(ctx context.Context, teamID uint, profileName string, r io.Reader, labels []string, labelsMembershipMode mobius.MDMLabelsMode) (*mobius.MDMWindowsConfigProfile, error) {
	if err := svc.authz.Authorize(ctx, &mobius.MDMConfigProfileAuthz{TeamID: &teamID}, mobius.ActionWrite); err != nil {
		return nil, ctxerr.Wrap(ctx, err)
	}
	...
	// only that any MDM is enabled, maybe it's just macOS
	if err := svc.VerifyMDMWindowsConfigured(ctx); err != nil {
		err := mobius.NewInvalidArgumentError("profile", mobius.WindowsMDMNotConfiguredMessage).WithStatus(http.StatusBadRequest)
		return nil, ctxerr.Wrap(ctx, err, "check windows MDM enabled")
	}
	...
	cp := mobius.MDMWindowsConfigProfile{
		TeamID: &teamID,
		Name:   profileName,
		SyncML: b,
	}
	if err := cp.ValidateUserProvided(); err != nil {
		...
		if strings.Contains(msg, syncml.DiskEncryptionProfileRestrictionErrMsg) {
			return nil, ctxerr.Wrap(ctx,
				&mobius.BadRequestError{Message: msg + " To control these settings use disk encryption endpoint."})
		}
		...
		err := &mobius.BadRequestError{Message: "Couldn't add. " + msg}
	...
	err = validateWindowsProfileMobiusVariables(string(cp.SyncML))
	if err != nil {
		return nil, ctxerr.Wrap(ctx, mobius.NewInvalidArgumentError("profile", err.Error()))
	}
	...
	if err != nil {
		...
		if errors.As(err, &existsErr) {
			err = mobius.NewInvalidArgumentError("profile", SameProfileNameUploadErrorMsg).
	...
	if err := svc.NewActivity(
		ctx, authz.UserFromContext(ctx), &mobius.ActivityTypeCreatedWindowsProfile{
			TeamID:      actTeamID,
			TeamName:    actTeamName,
...
func validateWindowsProfileMobiusVariables(contents string) error {
	if len(findMobiusVariables(contents)) > 0 {
		return &mobius.BadRequestError{Message: "Mobius variables ($MOBIUS_VAR_*) are not currently supported in Windows profiles"}
	}
	return nil
}

func (svc *Service) batchValidateProfileLabels(ctx context.Context, labelNames []string) (map[string]mobius.ConfigurationProfileLabel, error) {
	if len(labelNames) == 0 {
		return nil, nil
	}
...
func (svc *Service) validateProfileLabels(ctx context.Context, labelNames []string) ([]mobius.ConfigurationProfileLabel, error) {
	labelMap, err := svc.batchValidateProfileLabels(ctx, labelNames)
	if err != nil {
		return nil, ctxerr.Wrap(ctx, err, "validating profile labels")
	}

	var profLabels []mobius.ConfigurationProfileLabel
...
type backwardsCompatProfilesParam []mobius.MDMProfileBatchPayload
...
func (bcp *backwardsCompatProfilesParam) UnmarshalJSON(data []byte) error {
	...
	if lookAhead := bytes.TrimSpace(data); len(lookAhead) > 0 && lookAhead[0] == '[' {
		// use []mobius.MDMProfileBatchPayload to prevent infinite recursion if we
		// use `backwardsCompatProfileSlice`
		var profs []mobius.MDMProfileBatchPayload
	...
	*bcp = make(backwardsCompatProfilesParam, 0, len(backwardsCompat))
	for name, contents := range backwardsCompat {
		*bcp = append(*bcp, mobius.MDMProfileBatchPayload{Name: name, Contents: contents})
	}
	return nil
}
...
func batchSetMDMProfilesEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) BatchSetMDMProfiles(
	ctx context.Context, tmID *uint, tmName *string, profiles []mobius.MDMProfileBatchPayload, dryRun, skipBulkPending bool,
	...
	var labelMap map[string]mobius.ConfigurationProfileLabel
	if !dryRun {
	...
	if dryRun {
		var profilesWithoutSecrets []mobius.MDMProfileBatchPayload
		for _, p := range profiles {
			if len(mobius.ContainsPrefixVars(string(p.Contents), mobius.ServerSecretPrefix)) == 0 {
	...
	profilesWithSecrets := make(map[int]mobius.MDMProfileBatchPayload, len(profiles))
	...
	profilesVariablesByIdentifierMap, err := validateMobiusVariables(ctx, appCfg, appleProfiles, windowsProfiles, appleDecls)
	if err != nil {
		return err
	}
	...
	updates := mobius.MDMProfilesUpdates{
	...
	if updates.WindowsConfigProfile {
		if err := svc.NewActivity(
			ctx, authz.UserFromContext(ctx), &mobius.ActivityTypeEditedWindowsProfile{
				TeamID:   tmID,
				TeamName: tmName,
			}); err != nil {
	...
	if updates.AppleDeclaration {
		if err := svc.NewActivity(
			ctx, authz.UserFromContext(ctx), &mobius.ActivityTypeEditedDeclarationProfile{
				TeamID:   tmID,
				TeamName: tmName,
			}); err != nil {
...
func validateMobiusVariables(ctx context.Context, appConfig *mobius.AppConfig, appleProfiles map[int]*mobius.MDMAppleConfigProfile,
	windowsProfiles map[int]*mobius.MDMWindowsConfigProfile, appleDecls map[int]*mobius.MDMAppleDeclaration,
) (map[string]map[string]struct{}, error) {
	var err error

	profileVarsByProfIdentifier := make(map[string]map[string]struct{})
	for _, p := range appleProfiles {
		profileVars, err := validateConfigProfileMobiusVariables(appConfig, string(p.Mobileconfig))
		if err != nil {
			return nil, ctxerr.Wrap(ctx, err, "validating config profile Mobius variables")
		}
		profileVarsByProfIdentifier[p.Identifier] = profileVars
	}
	for _, p := range windowsProfiles {
		err = validateWindowsProfileMobiusVariables(string(p.SyncML))
		if err != nil {
			return nil, ctxerr.Wrap(ctx, err, "validating Windows profile Mobius variables")
		}
	}
	for _, p := range appleDecls {
		err = validateDeclarationMobiusVariables(string(p.RawJSON))
		if err != nil {
			return nil, ctxerr.Wrap(ctx, err, "validating declaration Mobius variables")
		}
	}
	return profileVarsByProfIdentifier, nil
}

func (svc *Service) validateCrossPlatformProfileNames(ctx context.Context, appleProfiles map[int]*mobius.MDMAppleConfigProfile,
	windowsProfiles map[int]*mobius.MDMWindowsConfigProfile, appleDecls map[int]*mobius.MDMAppleDeclaration,
) error {
	...
	for i, p := range appleProfiles {
		if _, ok := extByName[p.Name]; ok {
			err := mobius.NewInvalidArgumentError(fmt.Sprintf("appleProfiles[%d]", i), fmtDuplicateNameErrMsg(p.Name))
	...
	for i, p := range windowsProfiles {
		if _, ok := extByName[p.Name]; ok {
			err := mobius.NewInvalidArgumentError(fmt.Sprintf("windowsProfiles[%d]", i), fmtDuplicateNameErrMsg(p.Name))
	...
	for i, p := range appleDecls {
		if _, ok := extByName[p.Name]; ok {
			err := mobius.NewInvalidArgumentError(fmt.Sprintf("appleDecls[%d]", i), fmtDuplicateNameErrMsg(p.Name))
...
func (svc *Service) authorizeBatchProfiles(ctx context.Context, tmID *uint, tmName *string) (*uint, *string, error) {
	if tmID != nil && tmName != nil {
		...
		return nil, nil, ctxerr.Wrap(ctx, mobius.NewInvalidArgumentError("team_name", "cannot specify both team_id and team_name"))
	}
	if tmID != nil || tmName != nil {
		...
		if !license.IsPremium() {
			...
			svc.authz.SkipAuthorization(ctx) // so that the error message is not replaced by "forbidden"
			return nil, nil, ctxerr.Wrap(ctx, mobius.NewInvalidArgumentError(field, ErrMissingLicense.Error()))
		}
	}
	...
	if err := svc.authz.Authorize(ctx, &mobius.MDMConfigProfileAuthz{TeamID: tmID}, mobius.ActionWrite); err != nil {
...
func getAppleProfiles(
	...
	profiles map[int]mobius.MDMProfileBatchPayload,
	labelMap map[string]mobius.ConfigurationProfileLabel,
) (map[int]*mobius.MDMAppleConfigProfile, map[int]*mobius.MDMAppleDeclaration, error) {
	// any duplicate identifier or name in the provided set results in an error
	profs := make(map[int]*mobius.MDMAppleConfigProfile, len(profiles))
	decls := make(map[int]*mobius.MDMAppleDeclaration, len(profiles))
	...
	for i, prof := range profiles {
		...
		if mdm.GuessProfileExtension(prof.Contents) == "json" {
			rawDecl, err := mobius.GetRawDeclarationValues(prof.Contents)
			if err != nil {
				return nil, nil, err
			}
			...
			mdmDecl := mobius.NewMDMAppleDeclaration(prof.Contents, tmID, prof.Name, rawDecl.Type, rawDecl.Identifier)
			mdmDecl.SecretsUpdatedAt = prof.SecretsUpdatedAt
			for _, labelName := range prof.LabelsIncludeAll {
				if lbl, ok := labelMap[labelName]; ok {
					declLabel := mobius.ConfigurationProfileLabel{
			...
			case v == "mobileconfig":
				return nil, nil, ctxerr.Wrap(ctx,
					mobius.NewInvalidArgumentError(mdmDecl.Identifier, "A configuration profile with this identifier already exists."),
					"duplicate mobileconfig profile by identifier")
			case v == "declaration":
				return nil, nil, ctxerr.Wrap(ctx,
					mobius.NewInvalidArgumentError(mdmDecl.Identifier, "A declaration profile with this identifier already exists."),
					"duplicate declaration profile by identifier")
			default:
				// this should never happen but just in case
				return nil, nil, ctxerr.Wrap(ctx,
					mobius.NewInvalidArgumentError(mdmDecl.Identifier, "A profile with this identifier already exists."),
		...
		for _, labelName := range prof.LabelsIncludeAll {
			if lbl, ok := labelMap[labelName]; ok {
				mdmLabel := mobius.ConfigurationProfileLabel{
					LabelName:  lbl.LabelName,
					LabelID:    lbl.LabelID,
					RequireAll: true,
				}
		...
		if err := mdmProf.ValidateUserProvided(); err != nil {
			...
			if strings.Contains(err.Error(), mobileconfig.DiskEncryptionProfileRestrictionErrMsg) {
				iae = mobius.NewInvalidArgumentError(prof.Name,
				...
				iae = mobius.NewInvalidArgumentError(prof.Name, err.Error())
			}
		...
		if mdmProf.Name != prof.Name {
			return nil, nil, ctxerr.Wrap(ctx,
				mobius.NewInvalidArgumentError(prof.Name, fmt.Sprintf("Couldn't edit custom_settings. The name provided for the profile must match the profile PayloadDisplayName: %q", mdmProf.Name)),
				"duplicate mobileconfig profile by name")
		}

		if _, ok := byName[mdmProf.Name]; ok {
			return nil, nil, ctxerr.Wrap(ctx,
				mobius.NewInvalidArgumentError(prof.Name, fmt.Sprintf("Couldn't edit custom_settings. More than one configuration profile have the same name (PayloadDisplayName): %q", mdmProf.Name)),
				"duplicate mobileconfig profile by name")
		}
		...
		if _, ok := byIdent[mdmProf.Identifier]; ok {
			return nil, nil, ctxerr.Wrap(ctx,
				mobius.NewInvalidArgumentError(prof.Name, fmt.Sprintf("Couldn't edit custom_settings. More than one configuration profile have the same identifier (PayloadIdentifier): %q", mdmProf.Identifier)),
				"duplicate mobileconfig profile by identifier")
		}
	...
	if !appCfg.MDM.EnabledAndConfigured {
		...
		return nil, nil, ctxerr.Wrap(ctx, mobius.NewInvalidArgumentError("mdm", "cannot set custom settings: Mobius MDM is not configured"))
	}
...
func getWindowsProfiles(
	...
	profiles map[int]mobius.MDMProfileBatchPayload,
	labelMap map[string]mobius.ConfigurationProfileLabel,
) (map[int]*mobius.MDMWindowsConfigProfile, error) {
	profs := make(map[int]*mobius.MDMWindowsConfigProfile, len(profiles))
	...
	for i, profile := range profiles {
		...
		mdmProf := &mobius.MDMWindowsConfigProfile{
			TeamID: tmID,
			Name:   profile.Name,
		...
		if err := mdmProf.ValidateUserProvided(); err != nil {
			...
			return nil, ctxerr.Wrap(ctx,
				mobius.NewInvalidArgumentError(fmt.Sprintf("profiles[%s]", profile.Name), msg))
		}
	...
	if !appCfg.MDM.WindowsEnabledAndConfigured {
		...
		return nil, ctxerr.Wrap(ctx, mobius.NewInvalidArgumentError("mdm", "cannot set custom settings: Mobius MDM is not configured"))
	}

	return profs, nil
}

func validateProfiles(profiles map[int]mobius.MDMProfileBatchPayload) error {
	for _, profile := range profiles {
		...
		if count > 1 {
			return mobius.NewInvalidArgumentError("mdm", `Couldn't edit custom_settings. For each profile, only one of "labels_exclude_any", "labels_include_all", "labels_include_any" or "labels" can be included.`)
		}

		if len(profile.Contents) > 1024*1024 {
			return mobius.NewInvalidArgumentError("mdm", "maximum configuration profile file size is 1 MB")
		}
		...
		if platform != "darwin" && platform != "windows" {
			...
			return mobius.NewInvalidArgumentError("mdm", fmt.Sprintf(
...
func listMDMConfigProfilesEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
	...
	if profs == nil {
		...
		res.Profiles = []*mobius.MDMConfigProfilePayload{}
	}
	return &res, nil
}

func (svc *Service) ListMDMConfigProfiles(ctx context.Context, teamID *uint, opt mobius.ListOptions) ([]*mobius.MDMConfigProfilePayload, *mobius.PaginationMetadata, error) {
...
func updateDiskEncryptionEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) UpdateMDMDiskEncryption(ctx context.Context, teamID *uint, enableDiskEncryption *bool) error {
	...
	// `PATCH /api/v1/mobius/mdm/apple/settings` endpoint, but for now it's better
	...
	if err := svc.authz.Authorize(ctx, mobius.MDMAppleSettingsPayload{TeamID: teamID}, mobius.ActionWrite); err != nil {
		return ctxerr.Wrap(ctx, err)
	}
...
func resendHostMDMProfileEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) ResendHostMDMProfile(ctx context.Context, hostID uint, profileUUID string) error {
	...
	if err := svc.authz.Authorize(ctx, &mobius.MDMConfigProfileAuthz{TeamID: host.TeamID}, mobius.ActionWrite); err != nil {
		return ctxerr.Wrap(ctx, err)
	}
	...
	switch {
	case strings.HasPrefix(profileUUID, mobius.MDMAppleProfileUUIDPrefix):
		if err := svc.VerifyMDMAppleConfigured(ctx); err != nil {
			return ctxerr.Wrap(ctx, mobius.NewInvalidArgumentError("HostMDMProfile", mobius.AppleMDMNotConfiguredMessage).WithStatus(http.StatusBadRequest), "check apple mdm enabled")
		}
		if host.Platform != "darwin" && host.Platform != "ios" && host.Platform != "ipados" {
			return ctxerr.Wrap(ctx, mobius.NewInvalidArgumentError("HostMDMProfile", "Profile is not compatible with host platform."), "check host platform")
		}
	...
	case strings.HasPrefix(profileUUID, mobius.MDMAppleDeclarationUUIDPrefix):
		if err := svc.VerifyMDMAppleConfigured(ctx); err != nil {
			return ctxerr.Wrap(ctx, mobius.NewInvalidArgumentError("HostMDMProfile", mobius.AppleMDMNotConfiguredMessage).WithStatus(http.StatusBadRequest), "check apple mdm enabled")
		}
		if host.Platform != "darwin" && host.Platform != "ios" && host.Platform != "ipados" {
	...
	case strings.HasPrefix(profileUUID, mobius.MDMWindowsProfileUUIDPrefix):
		if err := svc.VerifyMDMWindowsConfigured(ctx); err != nil {
			return ctxerr.Wrap(ctx, mobius.NewInvalidArgumentError("HostMDMProfile", mobius.WindowsMDMNotConfiguredMessage).WithStatus(http.StatusBadRequest), "check windows mdm enabled")
		}
	...
	default:
		return ctxerr.Wrap(ctx, mobius.NewInvalidArgumentError("HostMDMProfile", "Invalid profile UUID prefix.").WithStatus(http.StatusNotFound), "check profile UUID prefix")
	}

	// check again based on team id of profile before we proceeding
	if err := svc.authz.Authorize(ctx, &mobius.MDMConfigProfileAuthz{TeamID: profileTeamID}, mobius.ActionWrite); err != nil {
	...
	if err != nil {
		if mobius.IsNotFound(err) {
			return ctxerr.Wrap(ctx, mobius.NewInvalidArgumentError("HostMDMProfile", "Unable to match profile to host.").WithStatus(http.StatusNotFound), "getting host mdm profile status")
		}
		return ctxerr.Wrap(ctx, err, "getting host mdm profile status")
	}
	if status == mobius.MDMDeliveryPending || status == mobius.MDMDeliveryVerifying {
		return ctxerr.Wrap(ctx, mobius.NewInvalidArgumentError("HostMDMProfile", "Couldn’t resend. Configuration profiles with “pending” or “verifying” status can’t be resent.").WithStatus(http.StatusConflict), "check profile status")
	}
	if status != mobius.MDMDeliveryFailed && status != mobius.MDMDeliveryVerified {
		// this should never happen, but just in case
	...
	if err := svc.NewActivity(
		ctx, authz.UserFromContext(ctx), &mobius.ActivityTypeResentConfigurationProfile{
			HostID:          &host.ID,
...
func getMDMAppleCSREndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) GetMDMAppleCSR(ctx context.Context) ([]byte, error) {
	...
	if len(privateKey) == 0 {
		return nil, ctxerr.New(ctx, "Couldn't download signed CSR. Missing required private key. Learn how to configure the private key here: https://mobiusmdm.com/learn-more-about/mobius-server-private-key")
	}
	...
	savedAssets, err := svc.ds.GetAllMDMConfigAssetsByName(ctx, []mobius.MDMAssetName{
	...
	if len(savedAssets) == 0 {
		...
		for k, v := range map[mobius.MDMAssetName][]byte{
			mobius.MDMAssetCACert:  apple_mdm.EncodeCertPEM(scepCert),
			mobius.MDMAssetCAKey:   apple_mdm.EncodePrivateKeyPEM(scepKey),
			mobius.MDMAssetAPNSKey: apple_mdm.EncodePrivateKeyPEM(apnsRSAKey),
		} {
			assets = append(assets, mobius.MDMConfigAsset{
				Name:  k,
				Value: v,
			})
		}
		...
		rawApnsKey := savedAssets[mobius.MDMAssetAPNSKey]
	...
	// Submit CSR to mobiusmdm.com for signing
	websiteClient := mobiushttp.NewClient(mobiushttp.WithTimeout(10 * time.Second))

	signedCSRB64, err := apple_mdm.GetSignedAPNSCSRNoEmail(websiteClient, apnsCSR)
	if err != nil {
		var fwe apple_mdm.MobiusWebsiteError
		if errors.As(err, &fwe) {
			// From svc.RequestMDMAppleCSR: mobiusmdm.com returns a bad request here if the email is invalid.
...
func uploadMDMAppleAPNSCertEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) UploadMDMAppleAPNSCert(ctx context.Context, cert io.ReadSeeker) error {
	...
	if len(privateKey) == 0 {
		return ctxerr.New(ctx, "Couldn't add APNs certificate. Missing required private key. Learn how to configure the private key here: https://mobiusmdm.com/learn-more-about/mobius-server-private-key")
	}

	if cert == nil {
		return ctxerr.Wrap(ctx, mobius.NewInvalidArgumentError("certificate", "Invalid certificate. Please provide a valid certificate from Apple Push Certificate Portal."))
	}
	...
	assets, err := svc.ds.GetAllMDMConfigAssetsByName(ctx, []mobius.MDMAssetName{mobius.MDMAssetAPNSKey}, nil)
	if err != nil {
		if mobius.IsNotFound(err) {
	...
	_, err = tls.X509KeyPair(certBytes, assets[mobius.MDMAssetAPNSKey].Value)
	if err != nil {
		return ctxerr.Wrap(ctx, mobius.NewInvalidArgumentError("certificate", "Invalid certificate. Please provide a valid certificate from Apple Push Certificate Portal."))
	}
	...
	err = svc.ds.DeleteMDMConfigAssetsByName(ctx, []mobius.MDMAssetName{mobius.MDMAssetAPNSCert})
	if err != nil {
		return ctxerr.Wrap(ctx, err, "deleting old apns cert from db")
	}
	err = svc.ds.InsertMDMConfigAssets(ctx, []mobius.MDMConfigAsset{
		{Name: mobius.MDMAssetAPNSCert, Value: certBytes},
	}, nil)
	if err != nil {
	...
	if appCfg.MDM.EnableDiskEncryption.Value {
		...
		if err := svc.NewActivity(ctx, authz.UserFromContext(ctx), mobius.ActivityTypeEnabledMacosDiskEncryption{}); err != nil {
	...
	for _, team := range teams {
		...
		if isEncryptionEnforced {
			...
			if err := svc.NewActivity(ctx, authz.UserFromContext(ctx), mobius.ActivityTypeEnabledMacosDiskEncryption{TeamID: &team.ID, TeamName: &team.Name}); err != nil {
...
func deleteMDMAppleAPNSCertEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) DeleteMDMAppleAPNSCert(ctx context.Context) error {
	...
	err := svc.ds.DeleteMDMConfigAssetsByName(ctx, []mobius.MDMAssetName{
...
func batchResendMDMProfileToHostsEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
	req := request.(*batchResendMDMProfileToHostsRequest)

	if err := svc.BatchResendMDMProfileToHosts(ctx, req.ProfileUUID, mobius.BatchResendMDMProfileFilters{
		ProfileStatus: mobius.MDMDeliveryStatus(req.Filters.ProfileStatus),
	}); err != nil {
...
func (svc *Service) BatchResendMDMProfileToHosts(ctx context.Context, profileUUID string, filters mobius.BatchResendMDMProfileFilters) error {
	...
	case strings.HasPrefix(profileUUID, mobius.MDMAppleProfileUUIDPrefix):
		if err := svc.VerifyMDMAppleConfigured(ctx); err != nil {
			return ctxerr.Wrap(ctx, mobius.NewInvalidArgumentError("profile_uuid", mobius.AppleMDMNotConfiguredMessage).WithStatus(http.StatusBadRequest), "check apple MDM enabled")
		}
	...
	default:
		return mobius.NewInvalidArgumentError("profile_uuid", "unknown profile").WithStatus(http.StatusNotFound)
	}
	...
	if count > 0 {
		if err := svc.NewActivity(
			ctx, authz.UserFromContext(ctx), &mobius.ActivityTypeResentConfigurationProfileBatch{
				ProfileName: profileName,
				HostCount:   count,
			}); err != nil {
...
type getMDMConfigProfileStatusResponse struct {
	mobius.MDMConfigProfileStatus
	Err error `json:"error,omitempty"`
}

func (r getMDMConfigProfileStatusResponse) Error() error { return r.Err }

func getMDMConfigProfileStatusEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) GetMDMConfigProfileStatus(ctx context.Context, profileUUID string) (mobius.MDMConfigProfileStatus, error) {
	// do a basic authz check that before we can make a more specific check based
	// on the team of the profile (just to ensure the user has _some_
	// authorization before loading the profile).
	if err := svc.authz.Authorize(ctx, &mobius.Host{}, mobius.ActionSelectiveList); err != nil {
		return mobius.MDMConfigProfileStatus{}, ctxerr.Wrap(ctx, err)
	}
	...
	case strings.HasPrefix(profileUUID, mobius.MDMAppleProfileUUIDPrefix):
		if err := svc.VerifyMDMAppleConfigured(ctx); err != nil {
			return mobius.MDMConfigProfileStatus{}, ctxerr.Wrap(ctx, mobius.NewInvalidArgumentError("profile_uuid", mobius.AppleMDMNotConfiguredMessage).WithStatus(http.StatusBadRequest), "check apple MDM enabled")
		}

		prof, err := svc.ds.GetMDMAppleConfigProfile(ctx, profileUUID)
		if err != nil {
			return mobius.MDMConfigProfileStatus{}, err
		}
		if _, ok := mobileconfig.MobiusPayloadIdentifiers()[prof.Identifier]; ok {
			// this endpoint is for custom settings, not mobius-controlled profiles,
			...
			// the profile).
			return mobius.MDMConfigProfileStatus{}, mobius.NewInvalidArgumentError("profile_uuid", "unknown profile").WithStatus(http.StatusNotFound)
		}
		teamID = prof.TeamID
	...
	case strings.HasPrefix(profileUUID, mobius.MDMWindowsProfileUUIDPrefix):
		if err := svc.VerifyMDMWindowsConfigured(ctx); err != nil {
			err := mobius.NewInvalidArgumentError("profile_uuid", mobius.WindowsMDMNotConfiguredMessage).WithStatus(http.StatusBadRequest)
			return mobius.MDMConfigProfileStatus{}, ctxerr.Wrap(ctx, err, "check windows MDM enabled")
		}
		...
		if ok := slices.Contains(mdm.ListMobiusReservedWindowsProfileNames(), prof.Name); ok {
			// this endpoint is for custom settings, not mobius-controlled profiles,
	...
	case strings.HasPrefix(profileUUID, mobius.MDMAppleDeclarationUUIDPrefix):
		...
		if ok := slices.Contains(mdm.ListMobiusReservedMacOSDeclarationNames(), decl.Name); ok {
			// this endpoint is for custom settings, not mobius-controlled profiles,
			// return not found if such a profile is requested here (status of e.g.
			// FileVault is considerably different than just checking the status of
			// the profile).
