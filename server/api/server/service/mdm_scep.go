...
func (svc *service) PKIOperation(ctx context.Context, data []byte) ([]byte, error) {
	if len(data) == 0 {
		return nil, &mobius.BadRequestError{Message: "missing data for PKIOperation"}
	}
...
func NewSCEPService(ds mobius.MDMAssetRetriever, signer scepserver.CSRSignerContext, logger log.Logger) scepserver.Service {
	return &service{
		signer:      signer,
