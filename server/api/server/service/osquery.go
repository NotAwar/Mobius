...
import (
	...
	"github.com/notawar/mobius/mobius-server/server/pubsub"
	"github.com/notawar/mobius/mobius-server/server/service/conditional_access_microsoft_proxy"
	"github.com/notawar/mobius/mobius-server/server/service/middleware/endpoint_utils"
	"github.com/notawar/mobius/mobius-server/server/service/osquery_utils"
...
func (svc *Service) AuthenticateHost(ctx context.Context, nodeKey string) (*mobius.Host, bool, error) {
...
func enrollAgentEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) serialUpdateHost(host *mobius.Host) {
...
func getClientConfigEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) getScheduledQueries(ctx context.Context, teamID *uint) (mobius.Queries, error) {
	appConfig, err := svc.ds.AppConfig(ctx)
	if err != nil {
	...
	config := make(mobius.Queries, len(queries))
...
func (svc *Service) GetClientConfig(ctx context.Context) (map[string]interface{}, error) {
	...
	for _, pack := range packs {
		...
		configQueries := mobius.Queries{}
		for _, query := range queries {
			queryContent := mobius.QueryContent{
				Query:    query.Query,
		...
		// the pack's queries
		packConfig[pack.Name] = mobius.PackContent{
	...
	if len(globalQueries) > 0 {
		packConfig["Global"] = mobius.PackContent{
	...
	if host.TeamID != nil {
		...
		if len(teamQueries) > 0 {
			...
			packConfig[packName] = mobius.PackContent{
				Queries: teamQueries,
			}
		}
	}
	...
	intervals := mobius.HostOsqueryIntervals{
...
func (svc *Service) AgentOptionsForHost(ctx context.Context, hostTeamID *uint, hostPlatform string) (json.RawMessage, error) {
	...
	if hostTeamID != nil {
		...
		if teamAgentOptions != nil && len(*teamAgentOptions) > 0 {
			var options mobius.AgentOptions
...
func getDistributedQueriesEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) detailQueriesForHost(ctx context.Context, host *mobius.Host) (queries map[string]string, discovery map[string]string, err error) {
...
func (svc *Service) hostRequiresConditionalAccessMicrosoftIngestion(ctx context.Context, host *mobius.Host) bool {
...
func (svc *Service) labelQueriesForHost(ctx context.Context, host *mobius.Host) (map[string]string, error) {
...
func (svc *Service) disablePoliciesDuringSetupExperience(ctx context.Context, host *mobius.Host) (bool, error) {
	if host.Platform != string(mobius.MacOSPlatform) {
		return false, nil
	}
...
func (svc *Service) policyQueriesForHost(ctx context.Context, host *mobius.Host) (policyQueries map[string]string, noPoliciesForHost bool, err error) {
...
func (shim *submitDistributedQueryResultsRequestShim) toRequest(ctx context.Context) (*SubmitDistributedQueryResultsRequest, error) {
	...
	for query, status := range shim.Statuses {
		...
		case string:
			...
			statuses[query] = mobius.OsqueryStatus(sint)
		case float64:
			statuses[query] = mobius.OsqueryStatus(s)
		default:
...
func submitDistributedQueryResultsEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
const (
	...
	hostAdditionalQueryPrefix = "mobius_additional_query_"
	...
	// osqueryd writes the distributed query results.
	hostPolicyQueryPrefix = "mobius_policy_query_"
	...
	// When Mobius receives results from such query then it will update the host's
...
func (svc *Service) SubmitDistributedQueryResults(
	...
	detailUpdated := false
	additionalResults := make(mobius.OsqueryDistributedQueryResults)
	...
	for query, rows := range results {
		...
		failed := ok && status != mobius.StatusOK
...
func processCalendarPolicies(
	...
	switch {
	case err == nil:
		if hostCalendarEvent.WebhookStatus != mobius.CalendarWebhookStatusPending {
			return nil
		}
	case mobius.IsNotFound(err):
		return nil
	...
	if now.Before(calendarEvent.StartTime) {
		...
		if err = ds.UpdateHostCalendarWebhookStatus(context.Background(), host.ID, mobius.CalendarWebhookStatusError); err != nil {
	...
	go func() {
		...
		err := backoff.Retry(
			func() error {
				if err := mobius.FireCalendarWebhook(
					...
					if errors.As(err, &statusCoder) && statusCoder.StatusCode() == http.StatusTooManyRequests {
						...
						if err := ds.UpdateHostCalendarWebhookStatus(
							context.Background(), host.ID, mobius.CalendarWebhookStatusRetry,
						); err != nil {
		...
		nextStatus := mobius.CalendarWebhookStatusSent
		if err != nil {
			level.Error(logger).Log("msg", "fire webhook", "err", err)
			nextStatus = mobius.CalendarWebhookStatusError
		}
...
func getFailingCalendarPolicies(policyResults map[uint]*bool, calendarPolicies []mobius.PolicyCalendarData) []mobius.PolicyCalendarData {
	var failingPolicies []mobius.PolicyCalendarData
...
func pythonPackageFilter(platform string, results *mobius.OsqueryDistributedQueryResults, statuses *map[string]mobius.OsqueryStatus) {
	...
	if status, ok := (*statuses)[linuxSoftware]; !ok || status != mobius.StatusOK {
		return
	}
...
func preProcessSoftwareExtraResults(
	...
	for _, query := range []string{
		...
		if status, ok := (*statuses)[query]; ok && status != mobius.StatusOK {
...
func globalPolicyAutomationsEnabled(webhookSettings mobius.WebhookSettings, integrations mobius.Integrations) bool {
...
func teamPolicyAutomationsEnabled(webhookSettings mobius.TeamWebhookSettings, integrations mobius.TeamIntegrations) bool {
	if webhookSettings.FailingPoliciesWebhook.Enable {
		return true
	}
	for _, j := range integrations.Jira {
...
func (svc *Service) ingestQueryResults(
	...
	policyResults map[uint]*bool,
	labelResults map[uint]*bool,
	additionalResults mobius.OsqueryDistributedQueryResults,
	stats *mobius.Stats,
) (bool, bool, error) {
...
func (svc *Service) directIngestDetailQuery(ctx context.Context, host *mobius.Host, name string, rows []map[string]string) (ingested bool, err error) {
	features, err := svc.HostFeatures(ctx, host)
	if err != nil {
...
// provided mobius.Host appropriately.
func (svc *Service) ingestDistributedQuery(
	ctx context.Context, host mobius.Host, name string, rows []map[string]string, errMsg string, stats *mobius.Stats,
) error {
	...
	res := mobius.DistributedQueryResult{
	...
	if err != nil {
		...
		if campaign.CreatedAt.After(svc.clock.Now().Add(-1 * time.Minute)) {
			...
			//	2. The client (UI/mobiuscli) starts listenting for query results.
			//
		...
		if campaign.Status != mobius.QueryComplete {
			campaign.Status = mobius.QueryComplete
...
// provided mobius.Host appropriately.
func (svc *Service) ingestDetailQuery(ctx context.Context, host *mobius.Host, name string, rows []map[string]string) error {
	features, err := svc.HostFeatures(ctx, host)
	if err != nil {
...
func (svc *Service) registerFlippedPolicies(_ context.Context, hostID uint, hostname, displayName string, newFailing, newPassing []uint) error {
	...
	for _, policyID := range newPassing {
		if err := svc.failingPolicySet.RemoveHosts(policyID, []mobius.PolicySetHost{host}); err != nil {
			return err
		}
	}
	return nil
...
func (svc *Service) processSoftwareForNewlyFailingPolicies(
	...
	if hostTeamID == nil {
		policyTeamID = mobius.PolicyNoTeamID
	} else {
	...
	policiesWithInstallersMap := make(map[uint]mobius.PolicySoftwareInstallerData)
	...
	var failingPoliciesWithInstaller []mobius.PolicySoftwareInstallerData
	for _, policyWithInstaller := range policiesWithInstaller {
	...
	for _, failingPolicyWithInstaller := range failingPoliciesWithInstaller {
		...
		if mobius.PlatformFromHost(hostPlatform) != installerMetadata.Platform {
		...
		// hostLastInstall.Status == nil can happen when a software is installed by Mobius and later removed.
		if hostLastInstall != nil && hostLastInstall.Status != nil &&
			*hostLastInstall.Status == mobius.SoftwareInstallPending {
		...
		// (in SaveHostSoftwareInstallResult) the author will be set to Mobius.
...
func (svc *Service) processVPPForNewlyFailingPolicies(
	...
	policiesWithVPPMap := make(map[uint]mobius.PolicyVPPData)
	...
	var failingPoliciesWithVPP []mobius.PolicyVPPData
	for _, policyWithVPP := range policiesWithVPP {
	...
	for _, failingPolicyWithVPP := range failingPoliciesWithVPP {
		...
		commandUUID, err := svc.EnterpriseOverrides.InstallVPPAppPostValidation(ctx, host, vppMetadata, vppToken, mobius.HostSoftwareInstallOptions{
			SelfService: false,
			PolicyID:    &policyID,
		})
		if err != nil {
...
func (svc *Service) processScriptsForNewlyFailingPolicies(
	...
	policiesWithScriptsMap := make(map[uint]mobius.PolicyScriptData)
	...
	var failingPoliciesWithScript []mobius.PolicyScriptData
	for _, policyWithScript := range policiesWithScript {
	...
	for _, failingPolicyWithScript := range failingPoliciesWithScript {
		...
		hostPlatform := mobius.PlatformFromHost(hostPlatform)
		...
		runScriptRequest := mobius.HostScriptRequestPayload{
			HostID:          hostID,
			...
			TeamID:          policyTeamID,
			PolicyID:        &policyID,
			// no user ID as scripts are executed by Mobius
		}
...
func (svc *Service) processConditionalAccessForNewlyFailingPolicies(
	...
	if err != nil {
		if mobius.IsNotFound(err) {
			// Nothing to do because Mobius hasn't ingested the Entra's "Device ID" or
	...
	for incomingPolicyID, incomingPolicyResult := range incomingPolicyResults {
		...
		if incomingPolicyResult != nil && !*incomingPolicyResult {
			hostIsCompliantInMobius = false
			break
		}
	}

	if hostConditionalAccessStatus.Managed != nil && mdmEnrolled == *hostConditionalAccessStatus.Managed &&
		hostConditionalAccessStatus.Compliant != nil && hostIsCompliantInMobius == *hostConditionalAccessStatus.Compliant {
		// Nothing to do, nothing has changed.
		return nil
	}

	svc.setHostConditionalAccessAsync(hostID, hostConditionalAccessStatus, mdmEnrolled, hostIsCompliantInMobius)
...
func (svc *Service) setHostConditionalAccessAsync(
	hostID uint,
	hostConditionalAccessStatus *mobius.HostConditionalAccessStatus,
	managed bool,
	compliant bool,
) {
	go func() {
...
func submitLogsEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
//   - `unmarshaledResults` with each result unmarshaled to `mobius.ScheduledQueryResult`s, where if an item is `nil` it means the corresponding
...
func (svc *Service) preProcessOsqueryResults(
...
) (unmarshaledResults []*mobius.ScheduledQueryResult, queriesDBData map[string]*mobius.Query) {
	// skipauth: Authorization is currently for user endpoints only.
	svc.authz.SkipAuthorization(ctx)
	...
	for _, raw := range osqueryResults {
		var result *mobius.ScheduledQueryResult
	...
	queriesDBData = make(map[string]*mobius.Query)
	for i, queryResult := range unmarshaledResults {
		...
		if errors.Is(err, mobius.ErrLegacyQueryPack) {
...
func (svc *Service) SubmitResultLogs(ctx context.Context, logs []json.RawMessage) error {
	...
	for i, unmarshaledResult := range unmarshaledResults {
		...
		if queryReportsDisabled {
			...
			// and thus Mobius won't receive any further results anymore.
		...
		if !ok {
			// If Mobius doesn't know of the query we write the logs to the logging destination
			...
			//
			// If a query was configured from Mobius but was recently removed, we may still write
			// the results for it here. Eventually the query will be removed from the host schedule
			// and thus Mobius won't receive any further results anymore.
			filteredLogs = append(filteredLogs, logs[i])
			continue
...
func (svc *Service) saveResultLogsToQueryReports(
	ctx context.Context,
	unmarshaledResults []*mobius.ScheduledQueryResult,
	queriesDBData map[string]*mobius.Query,
	...
	for _, result := range unmarshaledResultsFiltered {
		...
		if dbQuery.DiscardData || dbQuery.Logging != mobius.LoggingSnapshot {
...
//		}
//	]
func transformEventFormatToSnapshotFormat(results []*mobius.ScheduledQueryResult) []*mobius.ScheduledQueryResult {
	isEventFormat := func(result *mobius.ScheduledQueryResult) bool {
		return result != nil && result.Action == "snapshot" && len(result.Snapshot) == 0 && len(result.Columns) > 0
	}

	resultsInEventFormat := make(map[string]*mobius.ScheduledQueryResult)
	for _, result := range results {
		...
		if !ok {
			...
			resultsInEventFormat[result.QueryName] = &mobius.ScheduledQueryResult{
	...
	var filteredResults []*mobius.ScheduledQueryResult
	for _, result := range results {
		if isEventFormat(result) {
...
func (svc *Service) overwriteResultRows(ctx context.Context, result *mobius.ScheduledQueryResult, queryID, hostID uint, maxQueryReportRows int) error {
	fetchTime := time.Now()

	rows := make([]*mobius.ScheduledQueryResultRow, 0, len(result.Snapshot))

	// If the snapshot is empty, we still want to save a row with a null value
	// to capture LastFetched.
	if len(result.Snapshot) == 0 {
		rows = append(rows, &mobius.ScheduledQueryResultRow{
			QueryID:     queryID,
			HostID:      hostID,
			Data:        nil,
	...
	for _, snapshotItem := range result.Snapshot {
		row := &mobius.ScheduledQueryResultRow{
			QueryID:     queryID,
			HostID:      hostID,
			Data:        snapshotItem,
			LastFetched: fetchTime,
...
func getMostRecentResults(results []*mobius.ScheduledQueryResult) []*mobius.ScheduledQueryResult {
	// Use a map to track the most recent entry for each unique QueryName
	latestResults := make(map[string]*mobius.ScheduledQueryResult)
...
//
// The expected format of query names managed by Mobius is:
...
func getQueryNameAndTeamIDFromResult(path string) (*uint, string, error) {
	...
	if sep == "" {
		...
		if strings.HasPrefix(path, "pack/") && strings.Count(path, "/") == 2 {
			return nil, "", mobius.ErrLegacyQueryPack
		}
...
func (svc *Service) YaraRuleByName(ctx context.Context, name string) (*mobius.YaraRule, error) {
...
func getYaraEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
