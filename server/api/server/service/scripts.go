...
import (
	...
	"github.com/MobiusDM/mobius/server/api/pkg/scripts"
...
func runScriptEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
	req := request.(*runScriptRequest)

	var noWait time.Duration
	result, err := svc.RunHostScript(ctx, &mobius.HostScriptRequestPayload{
		HostID:         req.HostID,
...
func runScriptSyncEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) GetScriptIDByName(ctx context.Context, scriptName string, teamID *uint) (uint, error) {
	...
	if err != nil {
		if mobius.IsNotFound(err) {
			return 0, mobius.NewInvalidArgumentError("script_name", fmt.Sprintf(`Script '%s' doesn’t exist.`, scriptName))
		}
		return 0, err
	}
	return id, nil
...
func (svc *Service) RunHostScript(ctx context.Context, request *mobius.HostScriptRequestPayload, waitForResult time.Duration) (*mobius.HostScriptResult, error) {
	...
	if cfg.ServerSettings.ScriptsDisabled {
		svc.authz.SkipAuthorization(ctx)
		return nil, mobius.NewUserMessageError(errors.New(mobius.RunScriptScriptsDisabledGloballyErrMsg), http.StatusForbidden)
	}
	...
	if request.ScriptContents != "" {
		if err := svc.ds.ValidateEmbeddedSecrets(ctx, []string{request.ScriptContents}); err != nil {
			svc.authz.SkipAuthorization(ctx)
			return nil, mobius.NewInvalidArgumentError("script", err.Error())
		}
	}
	...
	if err != nil {
		...
		if mobius.IsNotFound(err) {
			if err := svc.authz.Authorize(ctx, &mobius.HostScriptResult{}, mobius.ActionWrite); err != nil {
				return nil, err
			}
		}
		svc.authz.SkipAuthorization(ctx)
	...
	if host.NodeKey == nil || *host.NodeKey == "" {
		...
		// an error
		svc.authz.SkipAuthorization(ctx)
		return nil, mobius.NewUserMessageError(errors.New(mobius.RunScriptDisabledErrMsg), http.StatusUnprocessableEntity)
	}
	...
	if err := svc.authz.Authorize(ctx, &mobius.HostScriptResult{TeamID: host.TeamID}, mobius.ActionWrite); err != nil {
		return nil, err
	}
	...
	if request.ScriptID != nil {
		...
		if err != nil {
			if mobius.IsNotFound(err) {
				return nil, mobius.NewInvalidArgumentError("script_id", `No script exists for the provided "script_id".`).
		...
		if scriptTmID != hostTmID {
			return nil, mobius.NewInvalidArgumentError("script_id", `The script does not belong to the same team (or no team) as the host.`)
		}
		...
		if isQueued {
			return nil, mobius.NewInvalidArgumentError("script_id", `The script is already queued on the given host.`).WithStatus(http.StatusConflict)
		}
	...
	if err := mobius.ValidateHostScriptContents(request.ScriptContents, isSavedScript); err != nil {
		return nil, mobius.NewInvalidArgumentError("script_contents", err.Error())
	}

	asyncExecution := waitForResult <= 0

	if !asyncExecution && host.Status(time.Now()) != mobius.StatusOnline {
		return nil, mobius.NewInvalidArgumentError("host_id", mobius.RunScriptHostOfflineErrMsg)
	}
	...
	if !asyncExecution && len(pending) > 0 {
		return nil, mobius.NewInvalidArgumentError("script_id", mobius.RunScriptAlreadyRunningErrMsg).WithStatus(http.StatusConflict)
	}
...
func getScriptResultEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func setUpGetScriptResultResponse(scriptResult *mobius.HostScriptResult) *getScriptResultResponse {
...
func (svc *Service) GetScriptResult(ctx context.Context, execID string) (*mobius.HostScriptResult, error) {
	scriptResult, err := svc.ds.GetHostScriptExecutionResult(ctx, execID)
	if err != nil {
		if mobius.IsNotFound(err) {
			if err := svc.authz.Authorize(ctx, &mobius.HostScriptResult{}, mobius.ActionRead); err != nil {
				return nil, err
			}
		}
		svc.authz.SkipAuthorization(ctx)
	...
	if scriptResult.HostDeletedAt == nil {
		...
		if err := svc.authz.Authorize(ctx, &mobius.HostScriptResult{TeamID: host.TeamID}, mobius.ActionRead); err != nil {
			return nil, err
		}
...
func createScriptEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) NewScript(ctx context.Context, teamID *uint, name string, r io.Reader) (*mobius.Script, error) {
	if err := svc.authz.Authorize(ctx, &mobius.Script{TeamID: teamID}, mobius.ActionWrite); err != nil {
		return nil, err
	}
	...
	if err != nil {
		var (
			existsErr mobius.AlreadyExistsError
			fkErr     mobius.ForeignKeyError
		)
		if errors.As(err, &existsErr) {
			err = mobius.NewInvalidArgumentError("script", "A script with this name already exists.").WithStatus(http.StatusConflict)
		} else if errors.As(err, &fkErr) {
			err = mobius.NewInvalidArgumentError("team_id", "The team does not exist.").WithStatus(http.StatusNotFound)
		}
	...
	if err := svc.NewActivity(
		...
		mobius.ActivityTypeAddedScript{
			TeamID:     teamID,
			TeamName:   teamName,
...
func deleteScriptEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) DeleteScript(ctx context.Context, scriptID uint) error {
	script, err := svc.authorizeScriptByID(ctx, scriptID, mobius.ActionWrite)
	if err != nil {
		return err
	}
	...
	if err := svc.NewActivity(
		ctx,
		authz.UserFromContext(ctx),
		mobius.ActivityTypeDeletedScript{
...
func listScriptsEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) ListScripts(ctx context.Context, teamID *uint, opt mobius.ListOptions) ([]*mobius.Script, *mobius.PaginationMetadata, error) {
	if err := svc.authz.Authorize(ctx, &mobius.Script{TeamID: teamID}, mobius.ActionRead); err != nil {
		return nil, nil, err
	}
...
func getScriptEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) GetScript(ctx context.Context, scriptID uint, withContent bool) (*mobius.Script, []byte, error) {
	script, err := svc.authorizeScriptByID(ctx, scriptID, mobius.ActionRead)
	if err != nil {
		return nil, nil, err
	}
...
func (updateScriptRequest) DecodeRequest(ctx context.Context, r *http.Request) (interface{}, error) {
	...
	if !ok {
		return nil, &mobius.BadRequestError{Message: "missing script id"}
	}
	scriptID, err := strconv.ParseUint(scriptIDStr, 10, 64)
	if err != nil {
		return nil, &mobius.BadRequestError{Message: "invalid script id"}
	}
	// Check if scriptID exceeds the maximum value for uint, code linter
	if scriptID > uint64(^uint(0)) {
		return nil, &mobius.BadRequestError{Message: "script id out of bounds"}
	}
...
func updateScriptEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) UpdateScript(ctx context.Context, scriptID uint, r io.Reader) (*mobius.Script, error) {
	...
	if err := svc.authz.Authorize(ctx, &mobius.Script{TeamID: script.TeamID}, mobius.ActionWrite); err != nil {
		return nil, err
	}
	...
	if err := mobius.ValidateHostScriptContents(scriptContents, true); err != nil {
		return nil, mobius.NewInvalidArgumentError("script", err.Error())
	}
	...
	if err := svc.NewActivity(
		ctx,
		authz.UserFromContext(ctx),
		mobius.ActivityTypeUpdatedScript{
			TeamID:     script.TeamID,
			TeamName:   teamName,
			ScriptName: script.Name,
		},
...
func getHostScriptDetailsEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) GetHostScriptDetails(ctx context.Context, hostID uint, opt mobius.ListOptions) ([]*mobius.HostScriptDetail, *mobius.PaginationMetadata, error) {
	h, err := svc.ds.HostLite(ctx, hostID)
	if err != nil {
		if mobius.IsNotFound(err) {
			// if error is because the host does not exist, check first if the user
			// had global access (to prevent leaking valid host ids).
			if err := svc.authz.Authorize(ctx, &mobius.Script{}, mobius.ActionRead); err != nil {
				return nil, nil, err
			}
		}
		return nil, nil, err
	...
	if err := svc.authz.Authorize(ctx, &mobius.Script{TeamID: h.TeamID}, mobius.ActionRead); err != nil {
		return nil, nil, err
	}
...
type batchScriptExecutionSummaryResponse struct {
	mobius.BatchExecutionSummary
	Err error `json:"error,omitempty"`
}

func (r batchSetScriptsResponse) Error() error { return r.Err }

func batchSetScriptsEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) BatchSetScripts(ctx context.Context, maybeTmID *uint, maybeTmName *string, payloads []mobius.ScriptPayload, dryRun bool) ([]mobius.ScriptResponse, error) {
	if maybeTmID != nil && maybeTmName != nil {
		svc.authz.SkipAuthorization(ctx) // so that the error message is not replaced by "forbidden"
		return nil, ctxerr.Wrap(ctx, mobius.NewInvalidArgumentError("team_name", "cannot specify both team_id and team_name"))
	}
	...
	if maybeTmID != nil || maybeTmName != nil {
		...
		if err != nil {
			...
			if dryRun && mobius.IsNotFound(err) {
				return nil, nil
			}
			return nil, err
		}
	...
	scripts := make([]*mobius.Script, 0, len(payloads))
	...
	for i, p := range payloads {
		...
		if err := script.ValidateNewScript(); err != nil {
			return nil, ctxerr.Wrap(ctx,
				mobius.NewInvalidArgumentError(fmt.Sprintf("scripts[%d]", i), err.Error()))
		}

		if byName[script.Name] {
			return nil, ctxerr.Wrap(ctx,
				mobius.NewInvalidArgumentError(fmt.Sprintf("scripts[%d]", i), fmt.Sprintf("Couldn’t edit scripts. More than one script has the same file name: %q", script.Name)),
	...
	if err := svc.NewActivity(
		ctx, authz.UserFromContext(ctx), &mobius.ActivityTypeEditedScript{
			TeamID:   teamID,
			TeamName: teamName,
		}); err != nil {
...
func batchScriptExecutionSummaryEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) BatchScriptExecutionSummary(ctx context.Context, batchExecutionID string) (*mobius.BatchExecutionSummary, error) {
	summary, err := svc.ds.BatchExecuteSummary(ctx, batchExecutionID)
	if err != nil {
		return nil, ctxerr.Wrap(ctx, err, "get batch script summary")
	}

	if err := svc.authz.Authorize(ctx, &mobius.Script{TeamID: summary.TeamID}, mobius.ActionRead); err != nil {
		return nil, err
	}
...
func (svc *Service) authorizeScriptByID(ctx context.Context, scriptID uint, authzAction string) (*mobius.Script, error) {
	...
	if err != nil {
		if mobius.IsNotFound(err) {
			...
			if err := svc.authz.Authorize(ctx, &mobius.Script{}, authzAction); err != nil {
				return nil, err
			}
		}
		svc.authz.SkipAuthorization(ctx)
...
func batchScriptRunEndpoint(ctx context.Context, request any, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) BatchScriptExecute(ctx context.Context, scriptID uint, hostIDs []uint, filters *map[string]interface{}) (string, error) {
	...
	if len(hostIDs) > 0 && filters != nil {
		return "", mobius.NewInvalidArgumentError("filters", "cannot specify both host_ids and filters")
	}
	...
	if cfg.ServerSettings.ScriptsDisabled {
		svc.authz.SkipAuthorization(ctx)
		return "", mobius.NewUserMessageError(errors.New(mobius.RunScriptScriptsDisabledGloballyErrMsg), http.StatusForbidden)
	}
	...
	if filters != nil {
		...
		if opt == nil {
			return "", mobius.NewInvalidArgumentError("filters", "filters must be a valid set of host list options")
		}

		if opt.TeamFilter == nil {
			return "", mobius.NewInvalidArgumentError("filters", "filters must include a team filter")
		}

		filter := mobius.TeamFilter{User: ctxUser, IncludeObserver: true}
	...
	if len(hosts) == 0 {
		return "", &mobius.BadRequestError{Message: "no hosts match the specified host IDs"}
	}

	if len(hosts) > MAX_BATCH_EXECUTION_HOSTS {
		return "", mobius.NewInvalidArgumentError("filters", "too_many_hosts")
	}
	...
	for _, host := range hosts {
		...
		if host.TeamID == nil || script.TeamID == nil || *host.TeamID != *script.TeamID {
			return "", mobius.NewInvalidArgumentError("host_ids", "all hosts must be on the same team as the script")
		}
	}

	batchID, err := svc.ds.BatchExecuteScript(ctx, userId, scriptID, hostIDsToExecute)
	if err != nil {
		return "", mobius.NewUserMessageError(err, http.StatusBadRequest)
	}

	if err := svc.NewActivity(ctx, ctxUser, mobius.ActivityTypeRanScriptBatch{
		ScriptName:       script.Name,
...
func lockHostEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
	...
	response := &lockHostResponse{DeviceStatus: mobius.DeviceStatusUnlocked, PendingAction: mobius.PendingActionLock}
...
func unlockHostEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
	...
	resp := unlockHostResponse{HostID: &req.HostID, DeviceStatus: mobius.DeviceStatusLocked, PendingAction: mobius.PendingActionUnlock}
...
func (req *wipeHostRequest) DecodeBody(ctx context.Context, r io.Reader, u url.Values, c []*x509.Certificate) error {
	...
	metadata := mobius.MDMWipeMetadata{}
...
func wipeHostEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
	...
	return wipeHostResponse{DeviceStatus: mobius.DeviceStatusUnlocked, PendingAction: mobius.PendingActionWipe}, nil
}

func (svc *Service) WipeHost(ctx context.Context, _ uint, _ *mobius.MDMWipeMetadata) error {
	// skipauth: No authorization check needed due to implementation returning
	// only license error.
	svc.authz.SkipAuthorization(ctx)
