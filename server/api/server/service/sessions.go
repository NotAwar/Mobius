...
import (
	...
	"github.com/MobiusDM/mobius/server/api/server/service/contract"
...
func getInfoAboutSessionEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) GetInfoAboutSession(ctx context.Context, id uint) (*mobius.Session, error) {
	...
	if err := svc.authz.Authorize(ctx, session, mobius.ActionRead); err != nil {
		return nil, err
	}
...
func deleteSessionEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) DeleteSession(ctx context.Context, id uint) error {
	...
	if err := svc.authz.Authorize(ctx, session, mobius.ActionWrite); err != nil {
		return err
	}
...
func loginEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) Login(ctx context.Context, email, password string, supportsEmailVerification bool) (*mobius.User, *mobius.Session, error) {
	...
	defer func(start time.Time) {
		if err != nil && !errors.Is(err, sendingMFAEmail) && !errors.Is(err, mfaNotSupportedForClient) {
			if err := svc.NewActivity(
				ctx, nil, mobius.ActivityTypeUserFailedLogin{
					Email:    email,
	...
	if errors.As(err, &nfe) {
		return nil, nil, mobius.NewAuthFailedError("user not found")
	}
	if err != nil {
		return nil, nil, mobius.NewAuthFailedError(err.Error())
	}

	if err = user.ValidatePassword(password); err != nil {
		return nil, nil, mobius.NewAuthFailedError("invalid password")
	}

	if user.SSOEnabled {
		return nil, nil, mobius.NewAuthFailedError("password login disabled for sso users")
	...
	if err := svc.NewActivity(
		ctx, user, mobius.ActivityTypeUserLoggedIn{
			PublicIP: publicip.FromContext(ctx),
		}); err != nil {
		return nil, nil, err
	}
...
func (svc *Service) makeSession(ctx context.Context, userID uint) (*mobius.Session, error) {
...
func sessionCreateEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) CompleteMFA(ctx context.Context, token string) (*mobius.Session, *mobius.User, error) {
	// skipauth: No user context available yet to authorize against.
	svc.authz.SkipAuthorization(ctx)
...
func logoutEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
	err := svc.Logout(ctx)
	if err != nil {
...
func (svc *Service) DestroySession(ctx context.Context) error {
	...
	if !ok {
		return mobius.NewAuthRequiredError(mobius.ErrNoContext.Error())
	}
...
func initiateSSOEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) InitiateSSO(ctx context.Context, redirectURL string) (string, error) {
	...
	if appConfig.SSOSettings == nil || !appConfig.SSOSettings.EnableSSO {
		err := &mobius.BadRequestError{Message: "organization not configured to use sso"}
	...
	settings := sso.Settings{
		...
		AssertionConsumerServiceURL: serverURL + svc.config.Server.URLPrefix + "/api/v1/mobius/sso/callback",
...
func makeCallbackSSOEndpoint(urlPrefix string) endpoint_utils.HandlerFunc {
	return func(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
		authResponse := request.(mobius.Auth)
		session, err := getSSOSession(ctx, svc, authResponse)
		var resp callbackSSOResponse
		if err != nil {
			if err := svc.NewActivity(ctx, nil, mobius.ActivityTypeUserFailedLogin{
     ...
     window.localStorage.setItem('MOBIUS::auth_token', '{{ .Token }}');
     window.location = redirectURL;
     </script>
     <body>
     Redirecting to Mobius at {{ .RedirectURL }} ...
     </body>
     </html>
    `
...
func getSSOSession(ctx context.Context, svc mobius.Service, auth mobius.Auth) (*mobius.SSOSession, error) {
...
func (svc *Service) InitSSOCallback(ctx context.Context, auth mobius.Auth) (string, error) {
	// skipauth: User context does not yet exist. Unauthenticated users may
	...
	err = sso.ValidateAudiences(
		*metadata,
		...
		appConfig.ServerSettings.ServerURL+svc.config.Server.URLPrefix+"/api/v1/mobius/sso/callback", // ACS
	)
	if err != nil {
...
func (svc *Service) GetSSOUser(ctx context.Context, auth mobius.Auth) (*mobius.User, error) {
...
func (svc *Service) LoginSSOUser(ctx context.Context, user *mobius.User, redirectURL string) (*mobius.SSOSession, error) {
	...
	err = svc.NewActivity(
		ctx,
		user,
		mobius.ActivityTypeUserLoggedIn{
			PublicIP: publicip.FromContext(ctx),
		},
	)
	if err != nil {
...
type ssoSettingsResponse struct {
	Settings *mobius.SessionSSOSettings `json:"settings,omitempty"`
	Err      error                      `json:"error,omitempty"`
}

func (r ssoSettingsResponse) Error() error { return r.Err }

func settingsSSOEndpoint(ctx context.Context, _ interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) SSOSettings(ctx context.Context) (*mobius.SessionSSOSettings, error) {
	...
	settings := &mobius.SessionSSOSettings{
...
func (svc *Service) makeMFAEmail(ctx context.Context, user mobius.User) error {
...
func (svc *Service) GetSessionByKey(ctx context.Context, key string) (*mobius.Session, error) {
...
func (svc *Service) validateSession(ctx context.Context, session *mobius.Session) error {
	if session == nil {
		return mobius.NewAuthRequiredError("active session not present")
	}
	...
	if sessionDuration != 0 && time.Since(session.AccessedAt) >= sessionDuration {
		...
		return mobius.NewAuthRequiredError("expired session")
	}
