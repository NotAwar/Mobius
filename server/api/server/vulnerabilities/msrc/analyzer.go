...
import (
	...
	msrc "github.com/notawar/mobius/mobius-server/server/vulnerabilities/msrc/parsed"
	utils "github.com/notawar/mobius/mobius-server/server/vulnerabilities/utils"
)
...
func Analyze(
	...
	toInsert := make(map[string]mobius.OSVulnerability)
	toDelete := make(map[string]mobius.OSVulnerability)

	// Run vulnerability detection for all hosts in this batch (hIDs)
	// and store the results in 'found'.
	var found []mobius.OSVulnerability
	for cve, v := range bulletin.Vulnerabities {
		...
		if isVuln {
			found = append(found, mobius.OSVulnerability{
				OSID:              os.ID,
				CVE:               cve,
	...
	var existing []mobius.OSVulnerability
	...
	err = utils.BatchProcess(toDelete, func(v []mobius.OSVulnerability) error {
	...
	var inserted []mobius.OSVulnerability
	if collectVulns {
		inserted = make([]mobius.OSVulnerability, 0, len(toInsert))
	}

	err = utils.BatchProcess(toInsert, func(v []mobius.OSVulnerability) error {
		n, err := ds.InsertOSVulnerabilities(ctx, v, mobius.MSRCSource)
		if err != nil {
			return err
		}
...
func loadBulletin(os mobius.OperatingSystem, dir string) (*msrc.SecurityBulletin, error) {
