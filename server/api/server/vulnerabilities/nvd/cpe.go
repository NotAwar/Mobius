...
func DownloadCPEDBFromGithub(vulnPath string, cpeDBURL string) error {
	...
	githubClient := mobiushttp.NewGithubClient()
...
func cpeGeneralSearchQuery(software *mobius.Software) (string, []interface{}, error) {
...
var (
	...
	softwareTransformers = []struct {
		matches func(*mobius.Software) bool
		mutate  func(*mobius.Software, log.Logger)
	}{
		{
			...
			matches: func(s *mobius.Software) bool {
			...
			mutate: func(s *mobius.Software, logger log.Logger) {
				// 243 -> 2024.3
		...
		},
		{
			...
			mutate: func(s *mobius.Software, logger kitlog.Logger) {
...
func mutateSoftware(software *mobius.Software, logger log.Logger) {
...
func CPEFromSoftware(logger log.Logger, db *sqlx.DB, software *mobius.Software, translations CPETranslations, reCache *regexpCache) (string, error) {
...
func consumeCPEBuffer(
	...
	batch []mobius.SoftwareCPE,
) error {
	var toDelete []mobius.SoftwareCPE
	var toUpsert []mobius.SoftwareCPE
...
func TranslateSoftwareToCPE(
	...
	nonOvalIterator, err := ds.AllSoftwareIterator(
		ctx,
		mobius.SoftwareIterQueryOptions{
			// Also exclude iOS and iPadOS apps until we enable vulnerabilities support for them.
			// TODO: Re-enable OVAL after fixing types
			ExcludedSources: []string{"ios_apps", "ipados_apps"}, // append(oval.SupportedSoftwareSources, "ios_apps", "ipados_apps"),
		},
	)
	if err != nil {
...
func translateSoftwareToCPEWithIterator(
	...
	vulnPath string,
	logger kitlog.Logger,
	iterator mobius.SoftwareIterator,
) error {
	...
	for iterator.Next() {
		...
		if software.Version == "" {
			...
			// generated by a previous version of Mobius.
		} else {
		...
		buffer = append(buffer, mobius.SoftwareCPE{SoftwareID: software.ID, CPE: cpe})
