...
import (
	...
	nvdsync "github.com/MobiusDM/mobius/server/api/server/vulnerabilities/nvd/sync"
	"github.com/MobiusDM/mobius/server/api/server/vulnerabilities/nvd/tools/cvefeed"
	feednvd "github.com/MobiusDM/mobius/server/api/server/vulnerabilities/nvd/tools/cvefeed/nvd"
	"github.com/MobiusDM/mobius/server/api/server/vulnerabilities/nvd/tools/cvefeed/nvd/schema"
	"github.com/MobiusDM/mobius/server/api/server/vulnerabilities/nvd/tools/providers/nvd"
...
func GetGitHubCVEAssetPath() (string, error) {
	...
	ghClient := github.NewClient(mobiushttp.NewGithubClient())
...
// vulnerabilities database folder to software CPEs in the mobius database.
...
func TranslateCPEToCVE(
	...
	// NVD feed file.
	softwareVulns := make(map[string]mobius.SoftwareVulnerability)
	osVulns := make(map[string]mobius.OSVulnerability)
	...
	var newVulns []mobius.SoftwareVulnerability
	for _, vuln := range softwareVulns {
		ok, err := ds.InsertSoftwareVulnerability(ctx, vuln, mobius.NVDSource)
		if err != nil {
	...
	for _, vuln := range osVulns {
		_, err := ds.InsertOSVulnerability(ctx, vuln, mobius.NVDSource)
		if err != nil {
			level.Error(logger).Log("cpe processing", "error", "err", err)
			continue
		}
	...
	if err = ds.DeleteOutOfDateVulnerabilities(ctx, mobius.NVDSource, 2*periodicity); err != nil {
		level.Error(logger).Log("msg", "error deleting out of date vulnerabilities", "err", err)
	}
	if err = ds.DeleteOutOfDateOSVulnerabilities(ctx, mobius.NVDSource, 2*periodicity); err != nil {
...
func GetMacOSCPEs(ctx context.Context, ds mobius.Datastore) ([]osCPEWithNVDMeta, error) {
...
func checkCVEs(
...
) ([]mobius.SoftwareVulnerability, []mobius.OSVulnerability, error) {
	...
	var foundSoftwareVulns []mobius.SoftwareVulnerability
	var foundOSVulns []mobius.OSVulnerability
