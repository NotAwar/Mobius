...
import (
	...
	"github.com/MobiusDM/mobius/server/api/server/vulnerabilities/nvd/tools/cpedict"
...
func applyCPEDatabaseSchema(db *sqlx.DB) error {
	...
	// with old mobius versions that use "select * from cpe ...". When creating the view, we need to
