...
// the github.com/MobiusDM/mobius/server/api/server/vulnerabilities/nvd/tools doesn't yet support parsing
...
func (s *CVE) initSync(ctx context.Context) error {
	// Remove any legacy feeds from previous versions of Mobius.
...
// removeLegacyFeeds removes all the legacy feed files downloaded by previous versions of Mobius.
...
func (s *CVE) updateYearFile(year int, cves []nvdapi.CVEItem) error {
	...
	// This is assumed by the github.com/MobiusDM/mobius/server/api/server/vulnerabilities/nvd/tools package.
	if year < 2002 {
		year = 2002
	}
