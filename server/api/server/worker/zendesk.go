...
import (
	...
	"github.com/MobiusDM/mobius/server/api/server/service/externalsvc"
...
* [{{ .DisplayName }}]({{ $.MobiusURL }}/hosts/{{ .ID }})
...
1. Go to the [Software]({{ .MobiusURL }}/software/manage) page in Mobius.
...
This ticket was created automatically by your Mobius Zendesk integration.
`)),
	...
	FailingPolicyDescription: template.Must(template.New("").Parse(
		`{{ if .PolicyCritical }}This policy is marked as **Critical** in Mobius.
...
View hosts that failed {{ .PolicyName }} on the [**Hosts**]({{ .MobiusURL }}/hosts/manage/?order_key=hostname&order_direction=asc&{{ if .TeamID }}team_id={{ .TeamID }}&{{ end }}policy_id={{ .PolicyID }}&policy_response=failing) page in Mobius.

----

This issue was created automatically by your Mobius Zendesk integration.
`)),
}
...
func (z *Zendesk) runVuln(ctx context.Context, cli ZendeskClient, args zendeskArgs) error {
	...
	var hosts []mobius.HostVulnerabilitySummary
	var err error
...
func (z *Zendesk) runFailingPolicy(ctx context.Context, cli ZendeskClient, args zendeskArgs) error {
	tplArgs := newFailingPoliciesTplArgs(z.MobiusURL, args.FailingPolicy)
...
func QueueZendeskVulnJobs(
	...
	recentVulns []mobius.SoftwareVulnerability,
	cveMeta map[string]mobius.CVEMeta,
) error {
...
func QueueZendeskFailingPolicyJob(ctx context.Context, ds mobius.Datastore, logger kitlog.Logger,
	policy *mobius.Policy, hosts []mobius.PolicySetHost,
) error {
	attrs := []interface{}{
		"enabled", "true",
