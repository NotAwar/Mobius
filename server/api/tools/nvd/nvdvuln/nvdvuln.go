...
import (
	...
	"github.com/notawar/mobius/mobius-server/server/vulnerabilities/nvd"
...
func main() {
	...
	softwareName := flag.String("software_name", "", "Name of the software as ingested by Mobius")
	softwareVersion := flag.String("software_version", "", "Version of the software as ingested by Mobius")
	softwareSource := flag.String("software_source", "", "Source for this software (e.g. 'apps' for macOS applications)")
	softwareBundleIdentifier := flag.String("software_bundle_identifier", "", "Bundle identifier of the software as ingested by Mobius (for macOS apps only)")

	// Fields to fetch software (and the found vulnerabilities) from a Mobius instance.
	// This mode of operation then compares the CVEs found by the Mobius instance with the CVEs found by this new run of vulnerability processing.
	...
	var softwareCPEs []mobius.SoftwareCPE
	...
	ds.InsertSoftwareVulnerabilityFunc = func(ctx context.Context, vuln mobius.SoftwareVulnerability, source mobius.VulnerabilitySource) (bool, error) {
		return true, nil
	}
	...
	ds.DeleteOutOfDateOSVulnerabilitiesFunc = func(ctx context.Context, source mobius.VulnerabilitySource, duration time.Duration) error {
		return nil
	}
...
func getSoftwareName(software []mobius.Software, softwareID uint) string {
	for _, s := range software {
		if s.ID == softwareID {
...
func (s *softwareIterator) Value() (*mobius.Software, error) {
	ss := &s.software[s.i]
	s.i += 1
	return ss, nil
}
...
func getSoftwareFromURL(url, apiToken string, debug bool) []mobius.Software {
	...
	var filteredSoftware []mobius.Software
	for _, s := range software {
