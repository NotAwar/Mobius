...
func main() {
	...
	dsn := "mobius:insecure@tcp(localhost:3306)/mobius"
	...
	macSoftware, err := readCSVFile("./server/api/tools/seed_data/software-macos.csv")
	if err != nil {
		log.Fatal(err)
	}
	...
	// Insert win software
	winSoftware, err := readCSVFile("./server/api/tools/seed_data/software-win.csv")
	if err != nil {
		log.Fatal(err)
	}
	...
	ubuntuSoftware, err := readCSVFile("./server/api/tools/seed_data/software-ubuntu.csv")
	if err != nil {
		log.Fatal(err)
	}
