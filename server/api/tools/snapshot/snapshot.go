...
func restore(homedir string) error {
	snapshotsDir := filepath.Join(homedir, ".mobius", "snapshots")
	...
	// Walk the ~/.mobius/snapshots directory if it exists.
	...
	cmd := exec.Command("./server/api/tools/backup_db/restore.sh", snapshots[index].Path)
...
func snapshot(homedir string) error {
	...
	cmd := exec.Command("./server/api/tools/backup_db/backup.sh", filepath.Join(snapshotPath, "db.sql.gz"))
