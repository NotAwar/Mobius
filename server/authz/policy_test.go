...
const (
	read               = mobius.ActionRead
	list               = mobius.ActionList
	write              = mobius.ActionWrite
	writeRole          = mobius.ActionWriteRole
	run                = mobius.ActionRun
	runNew             = mobius.ActionRunNew
	changePwd          = mobius.ActionChangePassword
	selectiveRead      = mobius.ActionSelectiveRead
	selectiveList      = mobius.ActionSelectiveList
	cancelHostActivity = mobius.ActionCancelHostActivity
)
...
func TestAuthorizeSession(t *testing.T) {
	...
	session := &mobius.Session{UserID: 42}
	runTestCases(t, []authTestCase{
		...
		{user: test.UserMaintainer, object: &mobius.Session{UserID: test.UserMaintainer.ID}, action: read, allow: true},
		{user: test.UserMaintainer, object: &mobius.Session{UserID: test.UserMaintainer.ID}, action: write, allow: true},
		...
		{user: test.UserNoRoles, object: &mobius.Session{UserID: test.UserNoRoles.ID}, action: read, allow: true},
		{user: test.UserNoRoles, object: &mobius.Session{UserID: test.UserNoRoles.ID}, action: write, allow: true},
		...
		{user: test.UserObserver, object: &mobius.Session{UserID: test.UserObserver.ID}, action: read, allow: true},
		{user: test.UserObserver, object: &mobius.Session{UserID: test.UserObserver.ID}, action: write, allow: true},
		...
		{user: test.UserObserverPlus, object: &mobius.Session{UserID: test.UserObserverPlus.ID}, action: read, allow: true},
		{user: test.UserObserverPlus, object: &mobius.Session{UserID: test.UserObserverPlus.ID}, action: write, allow: true},
	})
}
...
func TestAuthorizeUser(t *testing.T) {
	...
	newUser := &mobius.User{}
	user := &mobius.User{ID: 42}
	newTeamUser := &mobius.User{
		Teams: []mobius.UserTeam{{Team: mobius.Team{ID: 1}, Role: mobius.RoleObserver}},
	}
	teamAdmin := &mobius.User{
		ID:    101,
		Teams: []mobius.UserTeam{{Team: mobius.Team{ID: 1}, Role: mobius.RoleAdmin}},
	}
...
func TestAuthorizeEnrollSecret(t *testing.T) {
	...
	teamAdmin := &mobius.User{
		Teams: []mobius.UserTeam{
			{Team: mobius.Team{ID: 1}, Role: mobius.RoleAdmin},
		},
	}
	teamMaintainer := &mobius.User{
		Teams: []mobius.UserTeam{
			{Team: mobius.Team{ID: 1}, Role: mobius.RoleMaintainer},
		},
	}
	teamObserver := &mobius.User{
		Teams: []mobius.UserTeam{
			{Team: mobius.Team{ID: 1}, Role: mobius.RoleObserver},
		},
	}
	teamObserverPlus := &mobius.User{
		Teams: []mobius.UserTeam{
			{Team: mobius.Team{ID: 1}, Role: mobius.RoleObserverPlus},
		},
	}
	teamGitOps := &mobius.User{
		Teams: []mobius.UserTeam{
			{Team: mobius.Team{ID: 1}, Role: mobius.RoleGitOps},
		},
	}
	globalSecret := &mobius.EnrollSecret{TeamID: nil}
	teamSecret := &mobius.EnrollSecret{TeamID: ptr.Uint(1)}
	runTestCases(t, []authTestCase{
		// No access
...
func TestAuthorizeTeam(t *testing.T) {
	...
	team := &mobius.Team{} // Empty team is used to "list teams"
	team1 := &mobius.Team{ID: 1}
	team2 := &mobius.Team{ID: 2}
	runTestCases(t, []authTestCase{
...
func TestAuthorizeSoftwareInventory(t *testing.T) {
	...
	softwareInventory := &mobius.AuthzSoftwareInventory{}
	runTestCases(t, []authTestCase{
...
func TestAuthorizeSoftwareInstaller(t *testing.T) {
	...
	noTeamInstaller := &mobius.SoftwareInstaller{}
	team1Installer := &mobius.SoftwareInstaller{TeamID: ptr.Uint(1)}
	team2Installer := &mobius.SoftwareInstaller{TeamID: ptr.Uint(2)}
	runTestCases(t, []authTestCase{
...
func TestAuthorizeHostSoftwareInstallerResult(t *testing.T) {
	...
	noTeamInstallResult := &mobius.HostSoftwareInstallerResultAuthz{}
	team1InstallResult := &mobius.HostSoftwareInstallerResultAuthz{HostTeamID: ptr.Uint(1)}
	team2InstallResult := &mobius.HostSoftwareInstallerResultAuthz{HostTeamID: ptr.Uint(2)}
	runTestCases(t, []authTestCase{
		// Write permissions
...
func TestAuthorizeHost(t *testing.T) {
	...
	host := &mobius.Host{}
	hostTeam1 := &mobius.Host{TeamID: ptr.Uint(1)}
	hostTeam2 := &mobius.Host{TeamID: ptr.Uint(2)}
	runTestCases(t, []authTestCase{
		// No access
...
func TestAuthorizeQuery(t *testing.T) {
	...
	twoTeamsAdminObs := &mobius.User{
		ID: 103,
		Teams: []mobius.UserTeam{
			{Team: mobius.Team{ID: 1}, Role: mobius.RoleAdmin},
			{Team: mobius.Team{ID: 2}, Role: mobius.RoleObserver},
		},
	}
	teamObserverPlus := &mobius.User{
		ID: 104,
	...
	globalQueryNoTargets := &mobius.TargetedQuery{
		Query: globalQuery,
	}

	globalQueryTargetedToTeam1 := &mobius.TargetedQuery{
		HostTargets: mobius.HostTargets{TeamIDs: []uint{1}},
		Query:       globalQuery,
	}
	globalQueryTargetedToTeam1AndTeam2 := &mobius.TargetedQuery{
		HostTargets: mobius.HostTargets{TeamIDs: []uint{1, 2}},
		Query:       globalQuery,
	}
	globalQueryTargetedToTeam2 := &mobius.TargetedQuery{
		HostTargets: mobius.HostTargets{TeamIDs: []uint{2}},
		Query:       globalQuery,
	}
	globalQueryTargetedToTeam1AndTeam2AndTeam3 := &mobius.TargetedQuery{
		HostTargets: mobius.HostTargets{TeamIDs: []uint{1, 2, 3}},
		Query:       globalQuery,
	}

	globalObserverQuery := &mobius.Query{
		ObserverCanRun: true,
	}
	globalObserverQueryEmptyTargets := &mobius.TargetedQuery{
		Query: globalObserverQuery,
	}
	globalObserverQueryTargetedToTeam1 := &mobius.TargetedQuery{
		HostTargets: mobius.HostTargets{TeamIDs: []uint{1}},
		Query:       globalObserverQuery,
	}
	globalObserverQueryTargetedToTeam1AndTeam2 := &mobius.TargetedQuery{
		HostTargets: mobius.HostTargets{TeamIDs: []uint{1, 2}},
		Query:       globalObserverQuery,
	}
	globalObserverQueryTargetedToTeam2 := &mobius.TargetedQuery{
		HostTargets: mobius.HostTargets{TeamIDs: []uint{2}},
		Query:       globalObserverQuery,
	}
	globalObserverQueryTargetedToTeam1AndTeam2AndTeam3 := &mobius.TargetedQuery{
		HostTargets: mobius.HostTargets{TeamIDs: []uint{1, 2, 3}},
		Query:       globalObserverQuery,
	}

	teamAdminQuery := &mobius.Query{
		ID:             1,
		AuthorID:       ptr.Uint(teamAdmin.ID),
		ObserverCanRun: false,
		TeamID:         ptr.Uint(1),
	}
	teamMaintQuery := &mobius.Query{
		ID:             2,
		AuthorID:       ptr.Uint(teamMaintainer.ID),
		ObserverCanRun: false,
		TeamID:         ptr.Uint(1),
	}
	globalAdminQuery := &mobius.Query{ID: 3, AuthorID: ptr.Uint(test.UserAdmin.ID), ObserverCanRun: false}
	globalGitOpsQuery := &mobius.Query{ID: 4, AuthorID: ptr.Uint(test.UserGitOps.ID), ObserverCanRun: false}
	teamGitOpsQuery := &mobius.Query{
		ID: 5, AuthorID: ptr.Uint(teamGitOps.ID),
		ObserverCanRun: false,
		TeamID:         ptr.Uint(1),
	}
	observerQueryOnTeam3 := &mobius.Query{
		ID:             6,
		ObserverCanRun: true,
		TeamID:         ptr.Uint(3),
	}
	observerQueryOnTeam3TargetedToTeam3 := &mobius.TargetedQuery{
		HostTargets: mobius.HostTargets{TeamIDs: []uint{3}},
		Query:       observerQueryOnTeam3,
	}
	observerQueryOnTeam3TargetedToTeam2 := &mobius.TargetedQuery{
		HostTargets: mobius.HostTargets{TeamIDs: []uint{2}},
		Query:       observerQueryOnTeam3,
	}
	observerQueryOnTeam3TargetedToTeam1 := &mobius.TargetedQuery{
		HostTargets: mobius.HostTargets{TeamIDs: []uint{1}},
		Query:       observerQueryOnTeam3,
	}
	observerQueryOnTeam1 := &mobius.Query{
		ID:             7,
		ObserverCanRun: true,
		TeamID:         ptr.Uint(1),
	}
	observerQueryOnTeam1TargetedToTeam1 := &mobius.TargetedQuery{
		HostTargets: mobius.HostTargets{TeamIDs: []uint{1}},
		Query:       observerQueryOnTeam1,
	}
	observerQueryOnTeam1TargetedToTeam2 := &mobius.TargetedQuery{
		HostTargets: mobius.HostTargets{TeamIDs: []uint{2}},
		Query:       observerQueryOnTeam1,
	}
...
func TestAuthorizeTeamPolicy(t *testing.T) {
	...
	team1Policy := &mobius.Policy{
		PolicyData: mobius.PolicyData{
			TeamID: ptr.Uint(1),
		},
	}
	team2Policy := &mobius.Policy{
		PolicyData: mobius.PolicyData{
			TeamID: ptr.Uint(2),
		},
	}
...
func TestAuthorizeMDMConfigProfile(t *testing.T) {
	...
	globalProfile := &mobius.MDMConfigProfileAuthz{}
	team1Profile := &mobius.MDMConfigProfileAuthz{
		TeamID: ptr.Uint(1),
	}
	runTestCases(t, []authTestCase{
...
func TestAuthorizeMDMAppleSettings(t *testing.T) {
	...
	globalSettings := &mobius.MDMAppleSettingsPayload{}
	team1Settings := &mobius.MDMAppleSettingsPayload{
		TeamID: ptr.Uint(1),
	}
	runTestCases(t, []authTestCase{
...
func TestAuthorizeMDMAppleSetupAssistant(t *testing.T) {
	...
	globalSettings := &mobius.MDMAppleSetupAssistant{}
	team1Settings := &mobius.MDMAppleSetupAssistant{
		TeamID: ptr.Uint(1),
	}
	runTestCases(t, []authTestCase{
		{user: test.UserNoRoles, object: globalSettings, action: write, allow: false},
...
func TestAuthorizeMDMAppleBootstrapPackage(t *testing.T) {
	...
	globalSettings := &mobius.MDMAppleBootstrapPackage{}
	team1Settings := &mobius.MDMAppleBootstrapPackage{
		TeamID: 1,
	}
	runTestCases(t, []authTestCase{
		{user: test.UserNoRoles, object: globalSettings, action: write, allow: false},
...
func assertAuthorized(t *testing.T, user *mobius.User, object, action interface{}) {
	t.Helper()
...
func assertUnauthorized(t *testing.T, user *mobius.User, object, action interface{}) {
	t.Helper()
...
func TestAuthorizeMDMCommand(t *testing.T) {
	...
	globalCommand := &mobius.MDMCommandAuthz{}
	team1Command := &mobius.MDMCommandAuthz{
		TeamID: ptr.Uint(1),
	}
	runTestCases(t, []authTestCase{
...
func TestAuthorizeHostScriptResult(t *testing.T) {
	...
	globalScript := &mobius.HostScriptResult{}
	globalSavedScript := &mobius.HostScriptResult{ScriptID: ptr.Uint(1)}
	team1Script := &mobius.HostScriptResult{TeamID: ptr.Uint(1)}
	team1SavedScript := &mobius.HostScriptResult{TeamID: ptr.Uint(1), ScriptID: ptr.Uint(1)}
...
func TestAuthorizeScript(t *testing.T) {
	...
	globalScript := &mobius.Script{}
	team1Script := &mobius.Script{
		TeamID: ptr.Uint(1),
	}
	runTestCases(t, []authTestCase{
		{user: test.UserNoRoles, object: globalScript, action: write, allow: false},
...
func TestJSONToInterfaceUser(t *testing.T) {
	...
	subject, err := jsonToInterface(&mobius.User{GlobalRole: ptr.String(mobius.RoleAdmin)})
	require.NoError(t, err)
	{
		subject := subject.(map[string]interface{})
		assert.Equal(t, mobius.RoleAdmin, subject["global_role"])
		assert.Nil(t, subject["teams"])
	}

	subject, err = jsonToInterface(&mobius.User{
		Teams: []mobius.UserTeam{
			{Team: mobius.Team{ID: 3}, Role: mobius.RoleObserver},
			{Team: mobius.Team{ID: 42}, Role: mobius.RoleMaintainer},
		},
	})
	require.NoError(t, err)
	{
		...
		assert.Equal(t, mobius.RoleObserver, subject["teams"].([]interface{})[0].(map[string]interface{})["role"])
		assert.Equal(t, json.Number("3"), subject["teams"].([]interface{})[0].(map[string]interface{})["id"])
		assert.Equal(t, mobius.RoleMaintainer, subject["teams"].([]interface{})[1].(map[string]interface{})["role"])
...
func TestHostHealth(t *testing.T) {
	...
	hostHealth := &mobius.HostHealth{TeamID: ptr.Uint(1)}
	runTestCases(t, []authTestCase{
