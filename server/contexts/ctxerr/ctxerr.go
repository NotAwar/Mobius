...
// MobiusError is the error implementation used by this package.
type MobiusError struct {
...
func (e MobiusError) Error() string {
	if e.cause == nil {
		return e.msg
	}
...
func (e *MobiusError) Unwrap() error {
	return e.cause
}

// Stack returns a call stack for the error
func (e *MobiusError) Stack() []string {
	return e.stack.List()
}
...
func (e *MobiusError) StackTrace() *runtime.Frames {
...
func (e *MobiusError) StackFrames() []uintptr {
	return e.stack.(stack) // outside of tests, e.stack is always a stack type
}

// LogFields implements mobius.ErrWithLogFields, so attached error data can be
// logged along with the error
func (e *MobiusError) LogFields() []any {
	var fields []any
...
func newError(ctx context.Context, msg string, cause error, data map[string]interface{}) error {
	...
	return &MobiusError{msg, stack, cause, edata}
}
...
func wrapError(ctx context.Context, msg string, cause error, data map[string]interface{}) error {
	...
	isMobiusError := errors.As(cause, &ferr)

	// If the error is a MobiusError, don't add the full stack trace as it should
	// already be present.
	if isMobiusError {
		stack = stack[:1]
	}

	edata := encodeData(ctx, data, !isMobiusError)
	return &MobiusError{msg, stack, cause, edata}
}
...
func Cause(err error) error {
	return mobius.Cause(err)
}

// MobiusCause is similar to Cause, but returns the root-most
// MobiusError in the chain
func MobiusCause(err error) *MobiusError {
	var ferr, aux *MobiusError
	var ok bool

	for err != nil {
		if aux, ok = err.(*MobiusError); ok {
			ferr = aux
		}
		err = Unwrap(err)
	}
...
func MarshalJSON(err error) ([]byte, error) {
	chain := make([]mobiusErrorJSON, 0)

	for err != nil {
		switch v := err.(type) {
		case *MobiusError:
			chain = append(chain, mobiusErrorJSON{
				Message: v.msg,
				Data:    v.data,
				Stack:   v.stack.List(),
			})
		default:
			chain = append(chain, mobiusErrorJSON{Message: v.Error()})
		}
...
func Handle(ctx context.Context, err error) {
	...
	// a MobiusError in the chain
	var ferr *MobiusError
	if !errors.As(err, &ferr) {
		err = Wrap(ctx, err, "missing MobiusError in chain")
	}

	cause := err
	if ferr := MobiusCause(err); ferr != nil {
		// use the MobiusCause error so we send the most relevant stacktrace to APM
