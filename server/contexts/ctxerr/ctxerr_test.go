...
func TestNewWithData(t *testing.T) {
	t.Run("with valid data", func(t *testing.T) {
		...
		err := NewWithData(ctx, "new", data).(*MobiusError)
...
func TestErrorf(t *testing.T) {
	ctx, cleanup := setup()
	defer cleanup()
	err := Errorf(ctx, "%s %d", "new", 1).(*MobiusError)
...
func TestWrap(t *testing.T) {
	t.Run("with message provided", func(t *testing.T) {
		...
		err := Wrap(ctx, cause, "new").(*MobiusError)
	...
	t.Run("without message provided", func(t *testing.T) {
		...
		defer cleanup()
		cause := errors.New("cause")
		err := Wrap(ctx, cause).(*MobiusError)
...
func TestWrapNewWithData(t *testing.T) {
	t.Run("with valid data", func(t *testing.T) {
		...
		cause := errors.New("cause")
		data := map[string]interface{}{"foo": "bar"}
		err := WrapWithData(ctx, cause, "new", data).(*MobiusError)
	...
	t.Run("without message provided", func(t *testing.T) {
		...
		data := map[string]interface{}{"foo": make(chan int)}
		cause := errors.New("cause")
		err := WrapWithData(ctx, cause, "", data).(*MobiusError)
		require.Equal(t, err.msg, "")
		require.NotEmpty(t, err.stack.List())
		require.NotNil(t, err.cause)
		assert.Regexp(t, regexp.MustCompile(`{"error": ".+"}`), string(err.data))
...
func TestWrapf(t *testing.T) {
	...
	defer cleanup()
	cause := errors.New("cause")
	err := Wrapf(ctx, cause, "%s %d", "new", 1).(*MobiusError)
...
func TestMarshalJSON(t *testing.T) {
	...
	errWrap := Wrap(ctx, errNew, "b").(*MobiusError)
	errWrap.stack = mockStack{[]string{"sb"}}

	errNewWithData := NewWithData(ctx, "c", map[string]interface{}{"f": "c"}).(*MobiusError)
...
func TestMobiusCause(t *testing.T) {
	ctx, cleanup := setup()
	defer cleanup()

	var nilErr *MobiusError
	errNew := errors.New("a")
	...
	cases := []struct {
		...
		{"non-mobius, unwrapped errors returns nil", errNew, nilErr},
		{"mobius unwrapped errors returns the error itself", errWrapRoot, errWrapRoot},
		{"deeply nested errors return the root mobius error", errWrap1, errWrapRoot},
		{"deeply nested errors return the root mobius error", errWrap2, errWrapRoot},
	}

	for _, c := range cases {
		t.Run(c.msg, func(t *testing.T) {
			actual := MobiusCause(c.in)
...
func TestHandle(t *testing.T) {
	...
	t.Run("wraps when there's no MobiusError in the chain", func(t *testing.T) {
		ctx := context.Background()
		eh := MockHandler{}
		err := errors.New("new")
		eh.StoreImpl = func(serr error) {
			var ferr *MobiusError
...
func TestAdditionalMetadata(t *testing.T) {
	t.Run("saves additional data about the host if present", func(t *testing.T) {
		ctx, cleanup := setup()
		defer cleanup()
		hctx := host.NewContext(ctx, &mobius.Host{Platform: "test_platform", OsqueryVersion: "5.0"})
		err := New(hctx, "with host context").(*MobiusError)
	...
	t.Run("saves additional data about the viewer if present", func(t *testing.T) {
		ctx, cleanup := setup()
		defer cleanup()
		vctx := viewer.NewContext(ctx, viewer.Viewer{Session: &mobius.Session{ID: 1}, User: &mobius.User{SSOEnabled: true}})
		err := New(vctx, "with host context").(*MobiusError)
...
func TestLogFields(t *testing.T) {
	...
	cases := []struct {
		...
		{&MobiusError{data: json.RawMessage("{malformed: 1, }}")}, []any{"data", "{malformed: 1, }}"}},
	}

	for _, c := range cases {
		ferr, ok := c.err.(*MobiusError)
		require.True(t, ok)
		got := ferr.LogFields()
