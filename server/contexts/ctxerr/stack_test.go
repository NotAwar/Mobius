...
func TestElasticStack(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			causeStackContains: []string{
				// since it wraps a non-MobiusError, the full stack is collected
	...
	for _, c := range cases {
		t.Run(c.desc, func(t *testing.T) {
			...
			cause := MobiusCause(err)
			causeStack := cause.Stack()

			// if the mobius root error != mobius leaf error, then separate leaf +
			...
			_, _, apmErrs := apmtest.NewRecordingTracer().WithTransaction(func(ctx context.Context) {
				// APM should be able to capture that error (we use the MobiusCause error,
...
func TestSentryStack(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.desc, func(t *testing.T) {
			...
			// since we capture the MobiusCause error, the last entry in the exceptions
			// must be a MobiusError and contain the stacktrace we're looking for.
			rootCapturedErr := payload.Exceptions[len(payload.Exceptions)-1]
			require.Equal(t, "*ctxerr.MobiusError", rootCapturedErr.Type)
