...
//     mobius.ScheduledQueryList for an example, or packsList in this package
//     for an alternative approach).
//  4. Add the cached item to mobiuss/tools/cloner-check/main.go (in the
...
func (c *cloneCache) Get(ctx context.Context, k string) (mobius.Cloner, bool) {
...
func (c *cloneCache) Set(ctx context.Context, k string, x mobius.Cloner, d time.Duration) {
	clone, err := x.Clone()
	if err != nil {
...
func New(ds mobius.Datastore, opts ...Option) mobius.Datastore {
	c := &cachedMysql{
		Datastore:            ds,
...
func (ds *cachedMysql) NewAppConfig(ctx context.Context, info *mobius.AppConfig) (*mobius.AppConfig, error) {
...
func (ds *cachedMysql) AppConfig(ctx context.Context) (*mobius.AppConfig, error) {
	if x, found := ds.c.Get(ctx, appConfigKey); found {
		ac, ok := x.(*mobius.AppConfig)
		if ok {
			return ac, nil
		}
	}
...
func (ds *cachedMysql) SaveAppConfig(ctx context.Context, info *mobius.AppConfig) error {
...
func (ds *cachedMysql) ListPacksForHost(ctx context.Context, hid uint) ([]*mobius.Pack, error) {
...
func (ds *cachedMysql) ListScheduledQueriesInPack(ctx context.Context, packID uint) (mobius.ScheduledQueryList, error) {
	key := fmt.Sprintf(scheduledQueriesKey, packID)
	if x, found := ds.c.Get(ctx, key); found {
		scheduledQueries, ok := x.(mobius.ScheduledQueryList)
		if ok {
...
func (ds *cachedMysql) TeamFeatures(ctx context.Context, teamID uint) (*mobius.Features, error) {
	key := fmt.Sprintf(teamFeaturesKey, teamID)
	if x, found := ds.c.Get(ctx, key); found {
		if features, ok := x.(*mobius.Features); ok {
			return features, nil
		}
	}
...
func (ds *cachedMysql) TeamMDMConfig(ctx context.Context, teamID uint) (*mobius.TeamMDM, error) {
	key := fmt.Sprintf(teamMDMConfigKey, teamID)
	if x, found := ds.c.Get(ctx, key); found {
		if cfg, ok := x.(*mobius.TeamMDM); ok {
			return cfg, nil
		}
	}
...
func (ds *cachedMysql) SaveTeam(ctx context.Context, team *mobius.Team) (*mobius.Team, error) {
...
func (ds *cachedMysql) QueryByName(ctx context.Context, teamID *uint, name string) (*mobius.Query, error) {
...
func (ds *cachedMysql) GetAllMDMConfigAssetsByName(ctx context.Context, assetNames []mobius.MDMAssetName,
	...
	cachedAssets := make(map[mobius.MDMAssetName]mobius.MDMConfigAsset)
	var missingAssets []mobius.MDMAssetName

	for _, name := range assetNames {
		key := fmt.Sprintf(mdmConfigAssetKey, name, latestHashes[name])

		if x, found := ds.c.Get(ctx, key); found {
			asset, ok := x.(mobius.MDMConfigAsset)
			if ok {
