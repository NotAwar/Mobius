...
var (
	automationActivityAuthor = "Mobius"
...
func (ds *Datastore) NewActivity(
	...
	processed, _ := ctx.Value(mobius.ActivityWebhookContextKey).(bool)
	if !processed {
		return ctxerr.New(
			ctx, "activity webhook not processed. Please use svc.NewActivity instead of ds.NewActivity. This is a Mobius server bug.",
		)
	}
	...
	if automatableActivity, ok := activity.(mobius.AutomatableActivity); ok && automatableActivity.WasFromAutomation() {
		userName = &automationActivityAuthor
		mobiusInitiated = true
	}

	cols := []string{"mobius_initiated", "user_id", "user_name", "activity_type", "details", "created_at"}
	args := []any{
		mobiusInitiated,
		userID,
		userName,
	...
	vppPtrAct, okPtr := activity.(*mobius.ActivityInstalledAppStoreApp)
	vppAct, ok := activity.(mobius.ActivityInstalledAppStoreApp)
	if okPtr || ok {
		...
		activateNext := vppAct.Status != string(mobius.SoftwareInstalled)
		if vppPtrAct != nil {
			activateNext = vppPtrAct.Status != string(mobius.SoftwareInstalled)
		}
		...
		if activateNext {
			...
			// Mobius (wipe, lock results, setup experience results, etc. - see all
	...
	return ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		...
		// get in the 10K+.
		if ah, ok := activity.(mobius.ActivityHosts); ok {
...
func (ds *Datastore) ListActivities(ctx context.Context, opt mobius.ListActivitiesOptions) ([]*mobius.Activity, *mobius.PaginationMetadata, error) {
	// Fetch activities

	activities := []*mobius.Activity{}
	activitiesQ := `
		SELECT
			a.id,
			a.user_id,
...
// tasks.
func (ds *Datastore) ListHostUpcomingActivities(ctx context.Context, hostID uint, opt mobius.ListOptions) ([]*mobius.UpcomingActivity, *mobius.PaginationMetadata, error) {
	...
	if count == 0 {
		return []*mobius.UpcomingActivity{}, &mobius.PaginationMetadata{}, nil
	}
	...
	listStmts := []string{
		...
		`SELECT
			...
			IF(ua.mobius_initiated, 'Mobius', COALESCE(u.name, ua.payload->>'$.user.name')) as name,
			u.id as user_id,
			...
			ua.mobius_initiated as mobius_initiated
		FROM
...
`,
	...
	listStmt, args, err := sqlx.Named(listStmt, map[string]any{
		"host_id":                      hostID,
		"ran_script_type":              mobius.ActivityTypeRanScript{}.ActivityName(),
		"installed_software_type":      mobius.ActivityTypeInstalledSoftware{}.ActivityName(),
		"uninstalled_software_type":    mobius.ActivityTypeUninstalledSoftware{}.ActivityName(),
		"installed_app_store_app_type": mobius.ActivityInstalledAppStoreApp{}.ActivityName(),
	})
	if err != nil {
	...
	var activities []*mobius.UpcomingActivity
	...
	metaData := &mobius.PaginationMetadata{HasPreviousResults: opt.Page > 0, TotalResults: count}
...
func (ds *Datastore) ListHostPastActivities(ctx context.Context, hostID uint, opt mobius.ListOptions) ([]*mobius.Activity, *mobius.PaginationMetadata, error) {
	const listStmt = `
	SELECT
		...
		u.id as user_id,
		a.mobius_initiated as mobius_initiated
	FROM
		host_activities ha
		JOIN activities a
	...
	var activities []*mobius.Activity
	if err := sqlx.SelectContext(ctx, ds.reader(ctx), &activities, stmt, args...); err != nil {
		return nil, nil, ctxerr.Wrap(ctx, err, "select upcoming activities")
	}
...
func (ds *Datastore) CancelHostUpcomingActivity(ctx context.Context, hostID uint, executionID string) (mobius.ActivityDetails, error) {
	var details mobius.ActivityDetails
...
func (ds *Datastore) cancelHostUpcomingActivity(ctx context.Context, tx sqlx.ExtContext, hostID uint, executionID string) (mobius.ActivityDetails, error) {
	const (
...
`
	...
	var pastAct mobius.ActivityDetails
	...
	case "script":
		...
		if _, err := tx.ExecContext(ctx, failSetupExpStmt, mobius.SetupExperienceStatusFailure, hostUUID, executionID); err != nil {
		...
		pastAct = mobius.ActivityTypeCanceledRunScript{
			HostID:          act.HostID,
	...
	case "software_install":
		...
		pastAct = mobius.ActivityTypeCanceledInstallSoftware{
			HostID:          act.HostID,
			HostDisplayName: act.HostDisplayName,
	...
	case "software_uninstall":
		...
		pastAct = mobius.ActivityTypeCanceledUninstallSoftware{
			HostID:          act.HostID,
			HostDisplayName: act.HostDisplayName,
			SoftwareTitle:   act.CanceledName,
			SoftwareTitleID: titleID,
	...
	case "vpp_app_install":
		...
		pastAct = mobius.ActivityTypeCanceledInstallAppStoreApp{
			HostID:          act.HostID,
			HostDisplayName: act.HostDisplayName,
			SoftwareTitle:   act.CanceledName,
			SoftwareTitleID: titleID,
...
func clearLockWipeForCanceledActivity(ctx context.Context, tx sqlx.ExtContext, hostID uint, executionID string) error {
	...
	if lockCnt > 0 || wipeCnt > 0 {
		...
		actType := mobius.ActivityTypeLockedHost{}.ActivityName()
		if wipeCnt > 0 {
			actType = mobius.ActivityTypeWipedHost{}.ActivityName()
		}
...
func (ds *Datastore) GetHostUpcomingActivityMeta(ctx context.Context, hostID uint, executionID string) (*mobius.UpcomingActivityMeta, error) {
	const getStmt = `
	SELECT
		ua.execution_id,
		ua.activated_at,
...
`
	...
	namedArgs := map[string]any{
		...
		"lock_action":   mobius.WellKnownActionLock,
		"unlock_action": mobius.WellKnownActionUnlock,
		"wipe_action":   mobius.WellKnownActionWipe,
		"none_action":   mobius.WellKnownActionNone,
	}
	...
	var actMeta mobius.UpcomingActivityMeta
