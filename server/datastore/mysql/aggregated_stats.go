...
func getPercentileQuery(aggregate mobius.AggregatedStatsType, time string, percentile string) string {
	switch aggregate { //nolint:gocritic // ignore singleCaseSwitch
	case mobius.AggregatedStatsTypeScheduledQuery:
...
func setP50AndP95Map(
	ctx context.Context, tx sqlx.QueryerContext, aggregate mobius.AggregatedStatsType, time string, id uint, statsMap map[string]interface{},
) error {
	var p50, p95 float64
...
func (ds *Datastore) UpdateQueryAggregatedStats(ctx context.Context) error {
	err := walkIdsInTable(
		ctx, ds.reader(ctx), "queries", func(queryID uint) error {
			return ds.CalculateAggregatedPerfStatsPercentiles(ctx, mobius.AggregatedStatsTypeScheduledQuery, queryID)
		})
	if err != nil {
...
func (ds *Datastore) CalculateAggregatedPerfStatsPercentiles(ctx context.Context, aggregate mobius.AggregatedStatsType, queryID uint) error {
...
func getTotalExecutionsQuery(aggregate mobius.AggregatedStatsType) string {
	switch aggregate { //nolint:gocritic // ignore singleCaseSwitch
	case mobius.AggregatedStatsTypeScheduledQuery:
