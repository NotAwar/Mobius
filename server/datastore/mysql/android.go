...
func (ds *Datastore) setTimesToNonZero(host *mobius.AndroidHost) {
...
func (ds *Datastore) UpdateAndroidHost(ctx context.Context, host *mobius.AndroidHost, fromEnroll bool) error {
	if !host.IsValid() {
...
func (ds *Datastore) AndroidHostLite(ctx context.Context, enterpriseSpecificID string) (*mobius.AndroidHost, error) {
...
func (ds *Datastore) insertAndroidHostLabelMembershipTx(ctx context.Context, tx sqlx.ExtContext, hostID uint) error {
	...
	err := sqlx.SelectContext(ctx, tx, &labels, `SELECT id, name FROM labels WHERE label_type = 1 AND (name = ? OR name = ?)`,
		mobius.BuiltinLabelNameAllHosts, mobius.BuiltinLabelNameAndroid)
	switch {
	case err != nil:
...
// Android MDM is turned off for all Mobius).
