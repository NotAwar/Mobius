...
func testNewAndroidHost(t *testing.T, ds *Datastore) {
	...
	require.ElementsMatch(t, []string{mobius.BuiltinLabelNameAllHosts, mobius.BuiltinLabelNameAndroid}, names)
	...
	// This may occur when 2 Mobius servers received the same host information via pubsub.
	...
	ExecAdhocSQL(t, ds, func(q sqlx.ExtContext) error {
		_, err := q.ExecContext(testCtx(), `DELETE FROM labels WHERE name = ?`, mobius.BuiltinLabelNameAndroid)
		return err
	})
...
func createAndroidHost(enterpriseSpecificID string) *mobius.AndroidHost {
...
func testAndroidMDMStats(t *testing.T, ds *Datastore) {
	...
	var androidHost0 *mobius.AndroidHost
	for i := range hosts {
	...
	macHost, err := ds.NewHost(testCtx(), &mobius.Host{
	...
	linuxHost, err := ds.NewHost(testCtx(), &mobius.Host{
	...
	require.NoError(t, err)
	require.Equal(t, mobius.AggregatedMDMStatus{}, statusStats)
	require.Equal(t, []mobius.AggregatedMDMSolutions(nil), solutionsStats)
	...
	solutionsStats, _, err = ds.AggregatedMDMSolutions(testCtx(), nil, "")
	require.NoError(t, err)
	require.Equal(t, mobius.AggregatedMDMStatus{HostsCount: 4, EnrolledManualHostsCount: 4}, statusStats)
	require.Len(t, solutionsStats, 2)

	// both solutions are Mobius
	require.Equal(t, mobius.WellKnownMDMMobius, solutionsStats[0].Name)
	require.Equal(t, mobius.WellKnownMDMMobius, solutionsStats[1].Name)
	...
	require.NoError(t, err)
	require.Equal(t, mobius.AggregatedMDMStatus{HostsCount: 3, EnrolledManualHostsCount: 3}, statusStats)
	...
	solutionsStats, _, err = ds.AggregatedMDMSolutions(testCtx(), nil, "")
	require.NoError(t, err)
	require.Equal(t, mobius.AggregatedMDMStatus{HostsCount: 4, EnrolledManualHostsCount: 1, UnenrolledHostsCount: 3}, statusStats)
	require.Len(t, solutionsStats, 1)
	...
	solutionsStats, _, err = ds.AggregatedMDMSolutions(testCtx(), nil, "android")
	require.NoError(t, err)
	require.Equal(t, mobius.AggregatedMDMStatus{HostsCount: 3, UnenrolledHostsCount: 3}, statusStats)
	...
	solutionsStats, _, err = ds.AggregatedMDMSolutions(testCtx(), nil, "android")
	require.NoError(t, err)
	require.Equal(t, mobius.AggregatedMDMStatus{HostsCount: 3, UnenrolledHostsCount: 2, EnrolledManualHostsCount: 1}, statusStats)
	require.Len(t, solutionsStats, 1)
	require.Equal(t, 1, solutionsStats[0].HostsCount)
	require.Equal(t, serverURL, solutionsStats[0].ServerURL)
}
