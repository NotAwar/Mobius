...
func testAppConfigOrgInfo(t *testing.T, ds *Datastore) {
	...
	info2.SMTPSettings.SMTPAuthenticationType = mobius.AuthTypeNameUserNamePassword
	info2.SMTPSettings.SMTPUserName = "username"
	info2.SMTPSettings.SMTPPassword = "password"
	info2.SMTPSettings.SMTPEnableTLS = false
	info2.SMTPSettings.SMTPAuthenticationMethod = mobius.AuthMethodNameCramMD5
...
func testAppConfigEnrollSecrets(t *testing.T, ds *Datastore) {
	...
	// Remove team secrets
	err = ds.ApplyEnrollSecrets(ctx, &team1.ID, []*mobius.EnrollSecret{})
	assert.NoError(t, err)
	secret, err = ds.VerifyEnrollSecret(ctx, "one_secret")
	assert.Error(t, err)
	assert.Nil(t, secret)
...
func testAppConfigEnrollSecretRoundtrip(t *testing.T, ds *Datastore) {
	...
	expectedSecrets := []*mobius.EnrollSecret{
		{Secret: "one_secret"},
		{Secret: "two_secret"},
	}
...
func testAggregateEnrollSecretPerTeam(t *testing.T, ds *Datastore) {
	...
	team1, err := ds.NewTeam(context.Background(), &mobius.Team{Name: "team1"})
	require.NoError(t, err)
	err = ds.ApplyEnrollSecrets(context.Background(), &team1.ID, []*mobius.EnrollSecret{
	...
	_, err = ds.NewTeam(context.Background(), &mobius.Team{Name: "team2"})
	require.NoError(t, err)

	// a team with a single enroll secret
	team3, err := ds.NewTeam(context.Background(), &mobius.Team{Name: "team3"})
	require.NoError(t, err)
	err = ds.ApplyEnrollSecrets(context.Background(), &team3.ID, []*mobius.EnrollSecret{
	...
	require.ElementsMatch(t, []*mobius.EnrollSecret{
...
func testIsEnrollSecretAvailable(t *testing.T, ds *Datastore) {
	...
	require.NoError(t, ds.ApplyEnrollSecrets(ctx, nil, []*mobius.EnrollSecret{{Secret: "globalSecret"}}))
	require.NoError(t, ds.ApplyEnrollSecrets(ctx, &team1.ID, []*mobius.EnrollSecret{{Secret: "teamSecret"}}))
...
func testNDESSCEPProxyPassword(t *testing.T, ds *Datastore) {
	...
	ac.Integrations.NDESSCEPProxy = optjson.Any[mobius.NDESSCEPProxyIntegration]{
		Valid: true,
		Set:   true,
		Value: mobius.NDESSCEPProxyIntegration{
			AdminURL: adminURL,
			Username: username,
			Password: password,
			URL:      url,
	...
	checkProxyConfig := func() {
		...
		assert.Equal(t, mobius.MaskedPassword, result.Integrations.NDESSCEPProxy.Value.Password)
	}

	checkProxyConfig()

	checkPassword := func() {
		assets, err := ds.GetAllMDMConfigAssetsByName(ctx, []mobius.MDMAssetName{mobius.MDMAssetNDESPassword}, nil)
		require.NoError(t, err)
		require.Len(t, assets, 1)
		assert.Equal(t, password, string(assets[mobius.MDMAssetNDESPassword].Value))
	}
	checkPassword()

	// Set password to masked password -- should not update
	ac.Integrations.NDESSCEPProxy.Value.Password = mobius.MaskedPassword
	err = ds.SaveAppConfig(ctx, ac)
	require.NoError(t, err)
	checkProxyConfig()
	checkPassword()
