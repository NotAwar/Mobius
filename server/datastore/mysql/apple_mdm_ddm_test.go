...
func testMDMAppleBatchSetHostDeclarationState(t *testing.T, ds *Datastore) {
	t.Run("BasicTest", func(t *testing.T) {
		...
		declarations := make([]*mobius.MDMAppleDeclaration, 3)
		for i := 0; i < 3; i++ {
			declarations[i], err = ds.NewMDMAppleDeclaration(ctx, &mobius.MDMAppleDeclaration{
		...
		removeDeclarations := make([]*mobius.MDMAppleDeclaration, 3)
		for i := 0; i < 3; i++ {
			removeDeclarations[i] = &mobius.MDMAppleDeclaration{
	...
	t.Run("MultipleHostsSharedTokens", func(t *testing.T) {
		...
		for i := 0; i < 3; i++ {
			...
			var err error
			hosts[i], err = ds.NewHost(ctx, &mobius.Host{
		...
		installDeclarations := make([]*mobius.MDMAppleDeclaration, 3)
		for i := 0; i < 3; i++ {
			var err error
			installDeclarations[i], err = ds.NewMDMAppleDeclaration(ctx, &mobius.MDMAppleDeclaration{
