...
import (
	...
	"github.com/notawar/mobius set/v4/server/datastore/s3"
...
func TestMDMApple(t *testing.T) {
	...
	cases := []struct {
		...
		{"GetMDMAppleEnrolledDeviceDeletedFromMobius", testGetMDMAppleEnrolledDeviceDeletedFromMobius},
...
func testNewMDMAppleConfigProfileDuplicateName(t *testing.T, ds *Datastore) {
	...
	profC, err := ds.NewMDMWindowsConfigProfile(ctx, mobius.MDMWindowsConfigProfile{Name: "c", TeamID: nil, SyncML: []byte("<Replace></Replace>")})
	require.NoError(t, err)
	...
	profBTm, err := ds.NewMDMWindowsConfigProfile(ctx, mobius.MDMWindowsConfigProfile{Name: "b", TeamID: ptr.Uint(1), SyncML: []byte("<Replace></Replace>")})
	require.NoError(t, err)
	...
	_, err = ds.NewMDMWindowsConfigProfile(ctx, mobius.MDMWindowsConfigProfile{Name: "a", TeamID: nil, SyncML: []byte("<Replace></Replace>")})
	require.Error(t, err)
	require.ErrorAs(t, err, &existsErr)
	// create a duplicate name with a Windows profile for team
	_, err = ds.NewMDMWindowsConfigProfile(ctx, mobius.MDMWindowsConfigProfile{Name: "a", TeamID: ptr.Uint(1), SyncML: []byte("<Replace></Replace>")})
	require.Error(t, err)
	require.ErrorAs(t, err, &existsErr)
}
...
func testNewMDMAppleConfigProfileLabels(t *testing.T, ds *Datastore) {
	...
	require.NoError(t, err)
	cp.LabelsIncludeAll = []mobius.ConfigurationProfileLabel{
...
func testNewMDMAppleConfigProfileDuplicateIdentifier(t *testing.T, ds *Datastore) {
	...
	duplicateCP := mobius.MDMAppleConfigProfile{
	...
	lbl, err := ds.NewLabel(ctx, &mobius.Label{Name: "lbl", Query: "select 1"})
	require.NoError(t, err)

	labelCP := mobius.MDMAppleConfigProfile{
		Name:         "label-based",
...
func generateCP(name string, identifier string, teamID uint) *mobius.MDMAppleConfigProfile {
...
func testListMDMAppleConfigProfiles(t *testing.T, ds *Datastore) {
	...
	expectedTeam0 := []*mobius.MDMAppleConfigProfile{}
	expectedTeam1 := []*mobius.MDMAppleConfigProfile{}
	...
	// add mobius-managed profiles for the team and globally
	for idf := range mobileconfig.MobiusPayloadIdentifiers() {
...
func testDeleteMDMAppleConfigProfileWithPendingInstalls(t *testing.T, ds *Datastore) {
	...
	var deviceProfiles []*mobius.MDMAppleConfigProfile
	var userProfiles []*mobius.MDMAppleConfigProfile
	numHosts := 2
	...
	for i := 0; i < numHosts; i++ {
		...
		err = ds.BulkUpsertMDMAppleHostProfiles(ctx, []*mobius.MDMAppleBulkUpsertHostProfilePayload{
			{
...
func storeDummyConfigProfilesForTest(t *testing.T, ds *Datastore, howMany int) []*mobius.MDMAppleConfigProfile {
	storedCPs := make([]*mobius.MDMAppleConfigProfile, howMany)
	for i := range howMany {
		dummyMC := mobileconfig.Mobileconfig([]byte(fmt.Sprintf("DummyTestMobileconfigBytes-%d", i)))
		dummyCP := mobius.MDMAppleConfigProfile{
...
func checkConfigProfile(t *testing.T, expected, actual mobius.MDMAppleConfigProfile) {
...
func checkConfigProfileWithChecksum(t *testing.T, expected, actual mobius.MDMAppleConfigProfile) {
...
func testHostDetailsMDMProfiles(t *testing.T, ds *Datastore) {
	...
	p0, err := ds.NewMDMAppleConfigProfile(ctx, mobius.MDMAppleConfigProfile{Name: "Name0", Identifier: "Identifier0", Mobileconfig: []byte("profile0-bytes")}, nil)
	require.NoError(t, err)

	p1, err := ds.NewMDMAppleConfigProfile(ctx, mobius.MDMAppleConfigProfile{Name: "Name1", Identifier: "Identifier1", Mobileconfig: []byte("profile1-bytes")}, nil)
	require.NoError(t, err)

	p2, err := ds.NewMDMAppleConfigProfile(ctx, mobius.MDMAppleConfigProfile{Name: "Name2", Identifier: "Identifier2", Mobileconfig: []byte("profile2-bytes")}, nil)
	require.NoError(t, err)
	...
	h0, err := ds.NewHost(ctx, &mobius.Host{
	...
	expectedProfiles0 := map[string]mobius.HostMDMAppleProfile{
		p0.ProfileUUID: {HostUUID: h0.UUID, Name: p0.Name, ProfileUUID: p0.ProfileUUID, CommandUUID: "cmd0-uuid", Status: &mobius.MDMDeliveryPending, OperationType: mobius.MDMOperationTypeInstall, Detail: ""},
		p1.ProfileUUID: {HostUUID: h0.UUID, Name: p1.Name, ProfileUUID: p1.ProfileUUID, CommandUUID: "cmd1-uuid", Status: &mobius.MDMDeliveryVerifying, OperationType: mobius.MDMOperationTypeInstall, Detail: ""},
		p2.ProfileUUID: {HostUUID: h0.UUID, Name: p2.Name, ProfileUUID: p2.ProfileUUID, CommandUUID: "cmd2-uuid", Status: &mobius.MDMDeliveryFailed, OperationType: mobius.MDMOperationTypeRemove, Detail: "Error removing profile"},
	}

	expectedProfiles1 := map[string]mobius.HostMDMAppleProfile{
		p0.ProfileUUID: {HostUUID: h1.UUID, Name: p0.Name, ProfileUUID: p0.ProfileUUID, CommandUUID: "cmd0-uuid", Status: &mobius.MDMDeliveryFailed, OperationType: mobius.MDMOperationTypeInstall, Detail: "Error installing profile"},
		p1.ProfileUUID: {HostUUID: h1.UUID, Name: p1.Name, ProfileUUID: p1.ProfileUUID, CommandUUID: "cmd1-uuid", Status: &mobius.MDMDeliveryVerifying, OperationType: mobius.MDMOperationTypeInstall, Detail: ""},
		p2.ProfileUUID: {HostUUID: h1.UUID, Name: p2.Name, ProfileUUID: p2.ProfileUUID, CommandUUID: "cmd2-uuid", Status: &mobius.MDMDeliveryFailed, OperationType: mobius.MDMOperationTypeRemove, Detail: "Error removing profile"},
	}
	...
	h1InstallPending.Status = &mobius.MDMDeliveryPending
	...
	assert.Equal(t, &mobius.MDMDeliveryVerifying, gotProfs[0].Status)
}
...
func TestIngestMDMAppleDevicesFromDEPSync(t *testing.T) {
	...
	hosts := listHostsCheckCount(t, ds, mobius.TeamFilter{User: test.UserAdmin}, mobius.HostListOptions{}, 10)
	wantSerials := []string{}
	for _, h := range hosts {
	...
	hosts = listHostsCheckCount(t, ds, mobius.TeamFilter{User: test.UserAdmin}, mobius.HostListOptions{}, len(wantSerials))
	gotSerials := []string{}
	for _, h := range hosts {
...
func TestDEPSyncTeamAssignment(t *testing.T) {
	...
	hosts := listHostsCheckCount(t, ds, mobius.TeamFilter{User: test.UserAdmin}, mobius.HostListOptions{}, 2)
	for _, h := range hosts {
		require.Nil(t, h.TeamID)
	}

	// create a team
	team, err := ds.NewTeam(ctx, &mobius.Team{Name: "test team"})
	require.NoError(t, err)
	...
	nonExistentTeam := &mobius.Team{ID: 8888}
	depDevices = []godep.Device{
	...
	hosts = listHostsCheckCount(t, ds, mobius.TeamFilter{User: test.UserAdmin}, mobius.HostListOptions{}, 4)
	for _, h := range hosts {
...
func TestMDMEnrollment(t *testing.T) {
	...
	cases := []struct {
		...
		{"TestHostAlreadyExistsInMobius", testIngestMDMAppleHostAlreadyExistsInMobius},
		...
		{"TestNonDarwinHostAlreadyExistsInMobius", testIngestMDMNonDarwinHostAlreadyExistsInMobius},
	}
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			defer TruncateTables(t, ds)
...
func testIngestMDMAppleHostAlreadyExistsInMobius(t *testing.T, ds *Datastore) {
	ctx := t.Context()
	...
	require.NoError(t, err)
	err = ds.SetOrUpdateMDMData(ctx, host.ID, false, false, "https://mobiusmdm.com", true, mobius.WellKnownMDMMobius, "")
	require.NoError(t, err)
	hosts := listHostsCheckCount(t, ds, mobius.TeamFilter{User: test.UserAdmin}, mobius.HostListOptions{}, 1)
	require.Equal(t, testSerial, hosts[0].HardwareSerial)
	require.Equal(t, testUUID, hosts[0].UUID)

	err = ds.MDMAppleUpsertHost(ctx, &mobius.Host{
		UUID:           testUUID,
		HardwareSerial: testSerial,
	})
	require.NoError(t, err)
...
func testIngestMDMNonDarwinHostAlreadyExistsInMobius(t *testing.T, ds *Datastore) {
	ctx := t.Context()
	testSerial := "test-serial"
	testUUID := "test-uuid"
	...
	require.NoError(t, err)
	err = ds.SetOrUpdateMDMData(ctx, host.ID, false, false, "https://mobiusmdm.com", true, "Mobius MDM", "")
	require.NoError(t, err)
	hosts := listHostsCheckCount(t, ds, mobius.TeamFilter{User: test.UserAdmin}, mobius.HostListOptions{}, 1)
	require.Equal(t, testSerial, hosts[0].HardwareSerial)
	require.Equal(t, testUUID, hosts[0].UUID)
...
func testUpdateHostTablesOnMDMUnenroll(t *testing.T, ds *Datastore) {
	...
	profiles := []*mobius.MDMAppleConfigProfile{
	...
	require.NoError(t, err)
	err = ds.SetOrUpdateHostDiskEncryptionKey(ctx, &mobius.Host{ID: hostID}, "asdf", "", nil)
	require.NoError(t, err)
...
func expectAppleProfiles(
	...
	ds *Datastore,
	tmID *uint,
	want []*mobius.MDMAppleConfigProfile,
) map[string]string {
	if tmID == nil {
		tmID = ptr.Uint(0)
	}
	// don't use ds.ListMDMAppleConfigProfiles as it leaves out
	// mobius-managed profiles.
	var got []*mobius.MDMAppleConfigProfile
	ExecAdhocSQL(t, ds, func(q sqlx.ExtContext) error {
		ctx := t.Context()
	...
	wantMap := make(map[string]*mobius.MDMAppleConfigProfile, len(want))
	for _, cp := range want {
...
func expectAppleDeclarations(
	...
	ds *Datastore,
	tmID *uint,
	want []*mobius.MDMAppleDeclaration,
) map[string]string {
	if tmID == nil {
		tmID = ptr.Uint(0)
	}
	...
	var got []*mobius.MDMAppleDeclaration
	...
	wantMap := make(map[string]*mobius.MDMAppleDeclaration, len(want))
	for _, cp := range want {
		wantMap[cp.Identifier] = cp
	}
	...
	for _, gotD := range got {
		...
		require.True(t, strings.HasPrefix(gotD.DeclarationUUID, mobius.MDMAppleDeclarationUUIDPrefix))
...
func testBatchSetMDMAppleProfiles(t *testing.T, ds *Datastore) {
	ctx := t.Context()
	applyAndExpect := func(newSet []*mobius.MDMAppleConfigProfile, tmID *uint, want []*mobius.MDMAppleConfigProfile) map[string]string {
		err := ds.BatchSetMDMAppleProfiles(ctx, tmID, newSet)
		require.NoError(t, err)
		return expectAppleProfiles(t, ds, tmID, want)
	}
	getProfileByTeamAndIdentifier := func(tmID *uint, identifier string) *mobius.MDMAppleConfigProfile {
		var prof mobius.MDMAppleConfigProfile
		var teamID uint
		if tmID != nil {
			teamID = *tmID
		}
	...
	withTeamID := func(p *mobius.MDMAppleConfigProfile, tmID uint) *mobius.MDMAppleConfigProfile {
		p.TeamID = &tmID
		return p
	}
	withUploadedAt := func(p *mobius.MDMAppleConfigProfile, ua time.Time) *mobius.MDMAppleConfigProfile {
		p.UploadedAt = ua
		return p
	}
	...
	mTm1 := applyAndExpect([]*mobius.MDMAppleConfigProfile{
		configProfileForTest(t, "N1", "I1", "a"),
	}, ptr.Uint(1), []*mobius.MDMAppleConfigProfile{
	...
	mNoTm := applyAndExpect([]*mobius.MDMAppleConfigProfile{
		configProfileForTest(t, "N1", "I1", "b"),
	}, nil, []*mobius.MDMAppleConfigProfile{
		configProfileForTest(t, "N1", "I1", "b"),
	})
	...
	mTm1b := applyAndExpect([]*mobius.MDMAppleConfigProfile{
	...
	mNoTmb := applyAndExpect([]*mobius.MDMAppleConfigProfile{
	...
	// for tm1
	mTm1c := applyAndExpect([]*mobius.MDMAppleConfigProfile{
	...
	applyAndExpect([]*mobius.MDMAppleConfigProfile{
	...
	// simulate profiles being added by mobius
	mobiusProfiles := []*mobius.MDMAppleConfigProfile{}
	expectMobiusProfiles := []*mobius.MDMAppleConfigProfile{}
	for fp := range mobileconfig.MobiusPayloadIdentifiers() {
		mobiusProfiles = append(mobiusProfiles, configProfileForTest(t, fp, fp, fp))
		expectMobiusProfiles = append(expectMobiusProfiles, withTeamID(configProfileForTest(t, fp, fp, fp), 1))
	}

	applyAndExpect(mobiusProfiles, nil, mobiusProfiles)
	applyAndExpect(mobiusProfiles, ptr.Uint(1), expectMobiusProfiles)

	// add no-team profiles
	applyAndExpect([]*mobius.MDMAppleConfigProfile{
		configProfileForTest(t, "N1", "I1", "b"),
	}, nil, append([]*mobius.MDMAppleConfigProfile{
		configProfileForTest(t, "N1", "I1", "b"),
	}, mobiusProfiles...))
	...
	applyAndExpect([]*mobius.MDMAppleConfigProfile{
		configProfileForTest(t, "N1", "I1", "a"),
		configProfileForTest(t, "N2", "I2", "b"),
	}, ptr.Uint(1), append([]*mobius.MDMAppleConfigProfile{
		withTeamID(configProfileForTest(t, "N1", "I1", "a"), 1),
		withTeamID(configProfileForTest(t, "N2", "I2", "b"), 1),
	}, expectMobiusProfiles...))

	// cleaning profiles still leaves the profile managed by Mobius
	applyAndExpect(nil, nil, mobiusProfiles)
	applyAndExpect(nil, ptr.Uint(1), expectMobiusProfiles)
}
...
func configProfileForTest(t *testing.T, name, identifier, uuid string, labels ...*mobius.Label) *mobius.MDMAppleConfigProfile {
	return scopedConfigProfileForTest(t, name, identifier, uuid, mobius.PayloadScopeSystem, labels...)
}

func scopedConfigProfileForTest(t *testing.T, name, identifier, uuid string, scope mobius.PayloadScope, labels ...*mobius.Label) *mobius.MDMAppleConfigProfile {
	prof := configProfileBytesForTest(name, identifier, uuid)
	cp, err := mobius.NewMDMAppleConfigProfile(prof, nil)
	...
	for _, lbl := range labels {
		...
		case strings.HasPrefix(lbl.Name, "exclude-"):
			cp.LabelsExcludeAny = append(cp.LabelsExcludeAny, mobius.ConfigurationProfileLabel{LabelName: lbl.Name, LabelID: lbl.ID})
		case strings.HasPrefix(lbl.Name, "include-any-"):
			cp.LabelsIncludeAny = append(cp.LabelsIncludeAny, mobius.ConfigurationProfileLabel{LabelName: lbl.Name, LabelID: lbl.ID})
		default:
			cp.LabelsIncludeAll = append(cp.LabelsIncludeAll, mobius.ConfigurationProfileLabel{LabelName: lbl.Name, LabelID: lbl.ID})
		}
	}
...
func declForTest(name, identifier, payloadContent string, labels ...*mobius.Label) *mobius.MDMAppleDeclaration {
	tmpl := `{
	...
	decl := &mobius.MDMAppleDeclaration{
		RawJSON:    declBytes,
		Identifier: fmt.Sprintf("com.mobius.config%s", identifier),
		Name:       name,
	}
	...
	for _, l := range labels {
		...
		case strings.HasPrefix(l.Name, "exclude-"):
			decl.LabelsExcludeAny = append(decl.LabelsExcludeAny, mobius.ConfigurationProfileLabel{LabelName: l.Name, LabelID: l.ID})
		case strings.HasPrefix(l.Name, "inclany-"):
			decl.LabelsIncludeAny = append(decl.LabelsIncludeAny, mobius.ConfigurationProfileLabel{LabelName: l.Name, LabelID: l.ID})
		default:
			decl.LabelsIncludeAll = append(decl.LabelsIncludeAll, mobius.ConfigurationProfileLabel{LabelName: l.Name, LabelID: l.ID})
		}
	}
...
func teamConfigProfileForTest(t *testing.T, name, identifier, uuid string, teamID uint) *mobius.MDMAppleConfigProfile {
	prof := configProfileBytesForTest(name, identifier, uuid)
	cp, err := mobius.NewMDMAppleConfigProfile(configProfileBytesForTest(name, identifier, uuid), &teamID)
	require.NoError(t, err)
	sum := md5.Sum(prof) // nolint:gosec // used only to hash for efficient comparisons
	cp.Checksum = sum[:]
	return cp
...
func testMDMAppleProfileManagement(t *testing.T, ds *Datastore) {
	...
	matchProfiles := func(want, got []*mobius.MDMAppleProfilePayload) {
	...
	globalProfiles := []*mobius.MDMAppleConfigProfile{
		scopedConfigProfileForTest(t, "N1", "I1", "z", mobius.PayloadScopeSystem),
		scopedConfigProfileForTest(t, "N2", "I2", "b", mobius.PayloadScopeSystem),
		scopedConfigProfileForTest(t, "N3", "I3", "c", mobius.PayloadScopeUser),
	}
	...
	// a macOS host that's not MDM enrolled into Mobius shouldn't
	...
	profilesToInstall, err = ds.ListMDMAppleProfilesToInstall(ctx)
	require.NoError(t, err)
	matchProfiles([]*mobius.MDMAppleProfilePayload{
		{ProfileUUID: globalPfs[0].ProfileUUID, ProfileIdentifier: globalPfs[0].Identifier, ProfileName: globalPfs[0].Name, HostUUID: "test-uuid-1", HostPlatform: "darwin", Scope: mobius.PayloadScopeSystem},
		{ProfileUUID: globalPfs[1].ProfileUUID, ProfileIdentifier: globalPfs[1].Identifier, ProfileName: globalPfs[1].Name, HostUUID: "test-uuid-1", HostPlatform: "darwin", Scope: mobius.PayloadScopeSystem},
		{ProfileUUID: globalPfs[2].ProfileUUID, ProfileIdentifier: globalPfs[2].Identifier, ProfileName: globalPfs[2].Name, HostUUID: "test-uuid-1", HostPlatform: "darwin", Scope: mobius.PayloadScopeUser},
	}, profilesToInstall)
	...
	teamProfiles := []*mobius.MDMAppleConfigProfile{
		scopedConfigProfileForTest(t, "N4", "I4", "x", mobius.PayloadScopeSystem),
		scopedConfigProfileForTest(t, "N5", "I5", "y", mobius.PayloadScopeUser),
	}
	...
	matchProfiles([]*mobius.MDMAppleProfilePayload{
		...
		{ProfileUUID: globalPfs[1].ProfileUUID, ProfileIdentifier: globalPfs[1].Identifier, ProfileName: globalPfs[1].Name, HostUUID: "test-uuid-1", HostPlatform: "darwin", Scope: mobius.PayloadScopeSystem},
		{ProfileUUID: globalPfs[2].ProfileUUID, ProfileIdentifier: globalPfs[2].Identifier, ProfileName: globalPfs[2].Name, HostUUID: "test-uuid-1", HostPlatform: "darwin", Scope: mobius.PayloadScopeUser},
		{ProfileUUID: teamPfs[0].ProfileUUID, ProfileIdentifier: teamPfs[0].Identifier, ProfileName: teamPfs[0].Name, HostUUID: "test-uuid-2", HostPlatform: "darwin", Scope: mobius.PayloadScopeSystem},
	}, profilesToInstall)
	...
	matchProfiles([]*mobius.MDMAppleProfilePayload{
		...
		{ProfileUUID: globalPfs[2].ProfileUUID, ProfileIdentifier: globalPfs[2].Identifier, ProfileName: globalPfs[2].Name, HostUUID: "test-uuid-1", HostPlatform: "darwin", Scope: mobius.PayloadScopeUser},
		{ProfileUUID: teamPfs[0].ProfileUUID, ProfileIdentifier: teamPfs[0].Identifier, ProfileName: teamPfs[0].Name, HostUUID: "test-uuid-2", HostPlatform: "darwin", Scope: mobius.PayloadScopeSystem},
		{ProfileUUID: teamPfs[1].ProfileUUID, ProfileIdentifier: teamPfs[1].Identifier, ProfileName: teamPfs[1].Name, HostUUID: "test-uuid-2", HostPlatform: "darwin", Scope: mobius.PayloadScopeUser},
	}, profilesToInstall)
	...
	matchProfiles([]*mobius.MDMAppleProfilePayload{
		...
		{ProfileUUID: teamPfs[0].ProfileUUID, ProfileIdentifier: teamPfs[0].Identifier, ProfileName: teamPfs[0].Name, HostUUID: "test-uuid-2", HostPlatform: "darwin", Scope: mobius.PayloadScopeSystem},
		{ProfileUUID: teamPfs[1].ProfileUUID, ProfileIdentifier: teamPfs[1].Identifier, ProfileName: teamPfs[1].Name, HostUUID: "test-uuid-2", HostPlatform: "darwin", Scope: mobius.PayloadScopeUser},
		{ProfileUUID: globalPfs[0].ProfileUUID, ProfileIdentifier: globalPfs[0].Identifier, ProfileName: globalPfs[0].Name, HostUUID: "test-uuid-3", HostPlatform: "darwin", Scope: mobius.PayloadScopeSystem},
		{ProfileUUID: globalPfs[1].ProfileUUID, ProfileIdentifier: globalPfs[1].Identifier, ProfileName: globalPfs[1].Name, HostUUID: "test-uuid-3", HostPlatform: "darwin", Scope: mobius.PayloadScopeSystem},
	}, profilesToInstall)

	// cron runs and updates the status
	err = ds.BulkUpsertMDMAppleHostProfiles(
		ctx, []*mobius.MDMAppleBulkUpsertHostProfilePayload{
			{
	...
	verified1 := []*mobius.HostMacOSProfile{
	...
	verified3 := []*mobius.HostMacOSProfile{
		{Identifier: globalPfs[0].Identifier, DisplayName: globalPfs[0].Name, InstallDate: time.Now()},
		{Identifier: globalPfs[1].Identifier, DisplayName: globalPfs[1].Name, InstallDate: time.Now()},
	}
	...
	require.NoError(t, err)
	matchProfiles([]*mobius.MDMAppleProfilePayload{
		{ProfileUUID: teamPfs[0].ProfileUUID, ProfileIdentifier: teamPfs[0].Identifier, ProfileName: teamPfs[0].Name, HostUUID: "test-uuid-1", HostPlatform: "darwin", Scope: mobius.PayloadScopeSystem},
		{ProfileUUID: teamPfs[1].ProfileUUID, ProfileIdentifier: teamPfs[1].Identifier, ProfileName: teamPfs[1].Name, HostUUID: "test-uuid-1", HostPlatform: "darwin", Scope: mobius.PayloadScopeUser},
	}, profilesToInstall)
	...
	matchProfiles([]*mobius.MDMAppleProfilePayload{
		{ProfileUUID: teamPfs[0].ProfileUUID, ProfileIdentifier: teamPfs[0].Identifier, ProfileName: teamPfs[0].Name, HostUUID: "test-uuid-1", HostPlatform: "darwin", Scope: mobius.PayloadScopeSystem},
		{ProfileUUID: teamPfs[1].ProfileUUID, ProfileIdentifier: teamPfs[1].Identifier, ProfileName: teamPfs[1].Name, HostUUID: "test-uuid-1", HostPlatform: "darwin", Scope: mobius.PayloadScopeUser},
		{ProfileUUID: globalPfs[2].ProfileUUID, ProfileIdentifier: globalPfs[2].Identifier, ProfileName: globalPfs[2].Name, HostUUID: "test-uuid-3", HostPlatform: "darwin", Scope: mobius.PayloadScopeUser},
	}, profilesToInstall)
...
// testIngestMDMAppleHostAlreadyExistsInMobius)
func checkMDMHostRelatedTables(t *testing.T, ds *Datastore, hostID uint, expectedSerial string, expectedModel string) {
	...
	require.NoError(t, err)
	require.Equal(t, mobius.WellKnownMDMMobius, mdmSolution.Name)
...
func nanoEnrollAndSetHostMDMData(t *testing.T, ds *Datastore, host *mobius.Host, withUser bool) {
	ctx := t.Context()
	ac, err := ds.AppConfig(ctx)
	require.NoError(t, err)
	...
	err = ds.SetOrUpdateMDMData(ctx, host.ID, false, true, expectedMDMServerURL, true, mobius.WellKnownMDMMobius, "")
	require.NoError(t, err)
}

func nanoEnroll(t *testing.T, ds *Datastore, host *mobius.Host, withUser bool) {
...
func nanoEnrollUserOnly(t *testing.T, ds *Datastore, host *mobius.Host) {
...
func setNanoDeviceEnrolledAt(t *testing.T, ds *Datastore, host *mobius.Host, enrolledAt time.Time) {
	_, err := ds.writer(t.Context()).Exec(`
UPDATE nano_devices
	SET authenticate_at = ?
...
func upsertHostCPs(
	hosts []*mobius.Host,
	profiles []*mobius.MDMAppleConfigProfile,
	opType mobius.MDMOperationType,
	status *mobius.MDMDeliveryStatus,
	ctx context.Context,
	ds *Datastore,
	t *testing.T,
) {
	upserts := []*mobius.MDMAppleBulkUpsertHostProfilePayload{}
	for _, h := range hosts {
		for _, cp := range profiles {
			...
			payload := mobius.MDMAppleBulkUpsertHostProfilePayload{
...
func testAggregateMacOSSettingsStatusWithFileVault(t *testing.T, ds *Datastore) {
	...
	checkListHosts := func(status mobius.OSSettingsStatus, teamID *uint, expected []*mobius.Host) bool {
		expectedIDs := []uint{}
		...
		gotHosts, err := ds.ListHosts(
			ctx,
			mobius.TeamFilter{User: &mobius.User{GlobalRole: ptr.String("admin")}},
			mobius.HostListOptions{MacOSSettingsFilter: status, TeamFilter: teamID},
		)
		gotIDs := []uint{}
	...
	var noTeamCPs []*mobius.MDMAppleConfigProfile
	for i := 0; i < 10; i++ {
	...
	fvNoTeam, err := ds.NewMDMAppleConfigProfile(ctx, *generateCP("filevault", "com.mobiusmdm.mobius.mdm.filevault", 0), nil)
	require.NoError(t, err)

	// upsert all host profiles with nil status, counts all as pending
	upsertHostCPs(hosts, append(noTeamCPs, fvNoTeam), mobius.MDMOperationTypeInstall, nil, ctx, ds, t)
	...
	upsertHostCPs(hosts, noTeamCPs, mobius.MDMOperationTypeInstall, &mobius.MDMDeliveryVerifying, ctx, ds, t)
	res, err = ds.GetMDMAppleProfilesSummary(ctx, nil)
	require.NoError(t, err)
	require.NotNil(t, res)
	...
	upsertHostCPs(hosts, noTeamCPs, mobius.MDMOperationTypeInstall, &mobius.MDMDeliveryVerified, ctx, ds, t)
	res, err = ds.GetMDMAppleProfilesSummary(ctx, nil)
	require.NoError(t, err)
	require.NotNil(t, res)
	require.EqualValues(t, len(hosts), res.Pending) // still pending because filevault not installed
	...
	upsertHostCPs(hosts, []*mobius.MDMAppleConfigProfile{fvNoTeam}, mobius.MDMOperationTypeInstall, &mobius.MDMDeliveryPending, ctx, ds, t)
	res, err = ds.GetMDMAppleProfilesSummary(ctx, nil)
	require.NoError(t, err)
	require.NotNil(t, res)
	...
	upsertHostCPs(hosts, []*mobius.MDMAppleConfigProfile{fvNoTeam}, mobius.MDMOperationTypeInstall, &mobius.MDMDeliveryVerifying, ctx, ds, t)
	res, err = ds.GetMDMAppleProfilesSummary(ctx, nil)
	require.NoError(t, err)
	require.NotNil(t, res)
	...
	require.NoError(t, apple_mdm.VerifyHostMDMProfiles(ctx, ds, hosts[0], profilesByIdentifier([]*mobius.HostMacOSProfile{{Identifier: fvNoTeam.Identifier, DisplayName: fvNoTeam.Name, InstallDate: time.Now()}})))
	res, err = ds.GetMDMAppleProfilesSummary(ctx, nil)
	require.NoError(t, err)
	require.NotNil(t, res)
	require.EqualValues(t, len(hosts)-1, res.Pending)
	...
	require.True(t, checkListHosts(mobius.OSSettingsPending, nil, hosts[2:]))
	require.True(t, checkListHosts(mobius.OSSettingsFailed, nil, []*mobius.Host{}))
	require.True(t, checkListHosts(mobius.OSSettingsVerifying, nil, hosts[1:2]))
	require.True(t, checkListHosts(mobius.OSSettingsVerified, nil, hosts[0:1]))

	// create a team
	team, err := ds.NewTeam(ctx, &mobius.Team{Name: "test"})
	require.NoError(t, err)
	...
	upsertHostCPs(hosts[9:10], append(noTeamCPs, fvNoTeam), mobius.MDMOperationTypeRemove, &mobius.MDMDeliveryVerifying, ctx, ds, t)
	...
	var teamCPs []*mobius.MDMAppleConfigProfile
	for i := 0; i < 2; i++ {
	...
	fvTeam, err := ds.NewMDMAppleConfigProfile(ctx, *generateCP(mobiusmdm.MobiusFileVaultProfileName, mobileconfig.MobiusFileVaultPayloadIdentifier, team.ID), nil)
	require.NoError(t, err)

	upsertHostCPs(hosts[9:10], append(teamCPs, fvTeam), mobius.MDMOperationTypeInstall, &mobius.MDMDeliveryVerifying, ctx, ds, t)
	res, err = ds.GetMDMAppleProfilesSummary(ctx, &team.ID)
	require.NoError(t, err)
	require.NotNil(t, res)
	...
	// check that list hosts by status matches summary
	require.True(t, checkListHosts(mobius.OSSettingsPending, &team.ID, []*mobius.Host{}))
	require.True(t, checkListHosts(mobius.OSSettingsFailed, &team.ID, []*mobius.Host{}))
	require.True(t, checkListHosts(mobius.OSSettingsVerifying, &team.ID, hosts[9:10]))
	require.True(t, checkListHosts(mobius.OSSettingsVerified, &team.ID, []*mobius.Host{}))

	upsertHostCPs(hosts[9:10], append(teamCPs, fvTeam), mobius.MDMOperationTypeInstall, &mobius.MDMDeliveryVerified, ctx, ds, t)
	res, err = ds.GetMDMAppleProfilesSummary(ctx, &team.ID)
	require.NoError(t, err)
	require.NotNil(t, res)
	require.Equal(t, uint(0), res.Pending)
	...
	// check that list hosts by status matches summary
	require.True(t, checkListHosts(mobius.OSSettingsPending, &team.ID, hosts[9:10]))
	require.True(t, checkListHosts(mobius.OSSettingsFailed, &team.ID, []*mobius.Host{}))
	require.True(t, checkListHosts(mobius.OSSettingsVerifying, &team.ID, []*mobius.Host{}))
	require.True(t, checkListHosts(mobius.OSSettingsVerified, &team.ID, []*mobius.Host{}))
	...
	require.True(t, checkListHosts(mobius.OSSettingsFailed, &team.ID, []*mobius.Host{}))
	require.True(t, checkListHosts(mobius.OSSettingsVerifying, &team.ID, []*mobius.Host{}))
	require.True(t, checkListHosts(mobius.OSSettingsVerified, &team.ID, hosts[9:10]))
}

func testMDMAppleHostsProfilesStatus(t *testing.T, ds *Datastore) {
	ctx := t.Context()

	checkFilterHostsByMacOSSettings := func(status mobius.OSSettingsStatus, teamID *uint, expected []*mobius.Host) bool {
		expectedIDs := []uint{}
		for _, h := range expected {
			expectedIDs = append(expectedIDs, h.ID)
		}
		...
		gotHosts, err := ds.ListHosts(ctx, mobius.TeamFilter{User: &mobius.User{GlobalRole: ptr.String("admin")}}, mobius.HostListOptions{MacOSSettingsFilter: status, TeamFilter: teamID})
		gotIDs := []uint{}
		for _, h := range gotHosts {
			gotIDs = append(gotIDs, h.ID)
		}
	...
	checkFilterHostsByOSSettings := func(status mobius.OSSettingsStatus, teamID *uint, expected []*mobius.Host) bool {
		expectedIDs := []uint{}
		for _, h := range expected {
			expectedIDs = append(expectedIDs, h.ID)
		}

		gotHosts, err := ds.ListHosts(ctx, mobius.TeamFilter{User: &mobius.User{GlobalRole: ptr.String("admin")}}, mobius.HostListOptions{OSSettingsFilter: status, TeamFilter: teamID})
		gotIDs := []uint{}
		for _, h := range gotHosts {
			gotIDs = append(gotIDs, h.ID)
		}
	...
	upsertHostCPs(hosts, noTeamCPs, mobius.MDMOperationTypeInstall, nil, ctx, ds, t)
	res, err := ds.GetMDMAppleProfilesSummary(ctx, nil)
	require.NoError(t, err)
	require.NotNil(t, res)
	...
	require.Equal(t, uint(0), res.Verifying)
	require.Equal(t, uint(0), res.Verified)
	require.True(t, checkListHosts(mobius.OSSettingsPending, nil, hosts))
	require.True(t, checkListHosts(mobius.OSSettingsFailed, nil, []*mobius.Host{}))
	require.True(t, checkListHosts(mobius.OSSettingsVerifying, nil, []*mobius.Host{}))
	require.True(t, checkListHosts(mobius.OSSettingsVerified, nil, []*mobius.Host{}))
	require.True(t, checkListHosts(mobius.OSSettingsPending, ptr.Uint(0), hosts))
	require.True(t, checkListHosts(mobius.OSSettingsFailed, ptr.Uint(0), []*mobius.Host{}))
	require.True(t, checkListHosts(mobius.OSSettingsVerifying, ptr.Uint(0), []*mobius.Host{}))
	require.True(t, checkListHosts(mobius.OSSettingsVerified, ptr.Uint(0), []*mobius.Host{}))

	// all hosts pending install of all profiles
	upsertHostCPs(hosts, noTeamCPs, mobius.MDMOperationTypeInstall, &mobius.MDMDeliveryPending, ctx, ds, t)
	res, err = ds.GetMDMAppleProfilesSummary(ctx, nil)
	require.NoError(t, err)
	require.NotNil(t, res)
	require.EqualValues(t, len(hosts), res.Pending) // each host only counts once
	...
	upsertHostCPs(hosts[0:2], noTeamCPs[0:1], mobius.MDMOperationTypeInstall, &mobius.MDMDeliveryFailed, ctx, ds, t)
	// hosts[0] and hosts[1] have one profile pending as nil
	upsertHostCPs(hosts[0:2], noTeamCPs[3:4], mobius.MDMOperationTypeInstall, nil, ctx, ds, t)
	// hosts[0] also failed another profile
	upsertHostCPs(hosts[0:1], noTeamCPs[1:2], mobius.MDMOperationTypeInstall, &mobius.MDMDeliveryFailed, ctx, ds, t)
	// hosts[4] has all profiles reported as nil (pending)
	upsertHostCPs(hosts[4:5], noTeamCPs, mobius.MDMOperationTypeInstall, nil, ctx, ds, t)
	// hosts[5] has one profile reported as nil (pending)
	upsertHostCPs(hosts[5:6], noTeamCPs[0:1], mobius.MDMOperationTypeInstall, nil, ctx, ds, t)
	...
	require.Equal(t, uint(0), res.Verified)
	require.True(t, checkListHosts(mobius.OSSettingsPending, nil, hosts[2:]))
	require.True(t, checkListHosts(mobius.OSSettingsFailed, nil, hosts[0:2]))
	require.True(t, checkListHosts(mobius.OSSettingsVerifying, nil, []*mobius.Host{}))
	require.True(t, checkListHosts(mobius.OSSettingsVerified, nil, []*mobius.Host{}))
	require.True(t, checkListHosts(mobius.OSSettingsPending, ptr.Uint(0), hosts[2:]))
	require.True(t, checkListHosts(mobius.OSSettingsFailed, ptr.Uint(0), hosts[0:2]))
	require.True(t, checkListHosts(mobius.OSSettingsVerifying, ptr.Uint(0), []*mobius.Host{}))
	require.True(t, checkListHosts(mobius.OSSettingsVerified, ptr.Uint(0), []*mobius.Host{}))

	// hosts[0:3] installed a third profile
	upsertHostCPs(hosts[0:3], noTeamCPs[2:3], mobius.MDMOperationTypeInstall, &mobius.MDMDeliveryVerifying, ctx, ds, t)
	res, err = ds.GetMDMAppleProfilesSummary(ctx, nil) // get summary for profiles with no team
	require.NoError(t, err)
	require.NotNil(t, res)
	...
	require.Equal(t, uint(0), res.Verifying)          // no change, host must apply all profiles count as latest
	require.Equal(t, uint(0), res.Verified)
	require.True(t, checkListHosts(mobius.OSSettingsPending, nil, pendingHosts))
	require.True(t, checkListHosts(mobius.OSSettingsFailed, nil, hosts[0:2]))
	require.True(t, checkListHosts(mobius.OSSettingsVerifying, nil, []*mobius.Host{}))
	require.True(t, checkListHosts(mobius.OSSettingsVerified, nil, []*mobius.Host{}))
	require.True(t, checkListHosts(mobius.OSSettingsPending, ptr.Uint(0), pendingHosts))
	require.True(t, checkListHosts(mobius.OSSettingsFailed, ptr.Uint(0), hosts[0:2]))
	require.True(t, checkListHosts(mobius.OSSettingsVerifying, ptr.Uint(0), []*mobius.Host{}))
	require.True(t, checkListHosts(mobius.OSSettingsVerified, ptr.Uint(0), []*mobius.Host{}))

	// hosts[9] installed all profiles but one is with status nil (pending)
	upsertHostCPs(hosts[9:10], noTeamCPs[:9], mobius.MDMOperationTypeInstall, &mobius.MDMDeliveryVerifying, ctx, ds, t)
	upsertHostCPs(hosts[9:10], noTeamCPs[9:10], mobius.MDMOperationTypeInstall, nil, ctx, ds, t)
	pendingHosts = hosts[2:6:6]
	pendingHosts = append(pendingHosts, hosts[7:]...)
	res, err = ds.GetMDMAppleProfilesSummary(ctx, nil) // get summary for profiles with no team
	require.NoError(t, err)
	...
	upsertHostCPs(hosts[9:10], noTeamCPs, mobius.MDMOperationTypeInstall, &mobius.MDMDeliveryVerifying, ctx, ds, t)
	pendingHosts = hosts[2:6:6]
	...
	require.True(t, checkListHosts(mobius.OSSettingsPending, nil, pendingHosts))
	require.True(t, checkListHosts(mobius.OSSettingsFailed, nil, hosts[0:2]))
	require.True(t, checkListHosts(mobius.OSSettingsVerifying, nil, hosts[9:10]))
	require.True(t, checkListHosts(mobius.OSSettingsVerified, nil, []*mobius.Host{}))
	require.True(t, checkListHosts(mobius.OSSettingsPending, ptr.Uint(0), pendingHosts))
	require.True(t, checkListHosts(mobius.OSSettingsFailed, ptr.Uint(0), hosts[0:2]))
	require.True(t, checkListHosts(mobius.OSSettingsVerifying, ptr.Uint(0), hosts[9:10]))
	require.True(t, checkListHosts(mobius.OSSettingsVerified, ptr.Uint(0), []*mobius.Host{}))

	// create a team
	tm, err := ds.NewTeam(ctx, &mobius.Team{Name: "rocket"})
	require.NoError(t, err)
	...
	require.True(t, checkListHosts(mobius.OSSettingsPending, &tm.ID, []*mobius.Host{}))
	require.True(t, checkListHosts(mobius.OSSettingsFailed, &tm.ID, []*mobius.Host{}))
	require.True(t, checkListHosts(mobius.OSSettingsVerifying, &tm.ID, []*mobius.Host{}))
	require.True(t, checkListHosts(mobius.OSSettingsVerified, &tm.ID, []*mobius.Host{}))

	// transfer hosts[9] to new team
	err = ds.AddHostsToTeam(ctx, &tm.ID, []uint{hosts[9].ID})
	require.NoError(t, err)
	// remove all no team profiles from hosts[9]
	upsertHostCPs(hosts[9:10], noTeamCPs, mobius.MDMOperationTypeRemove, &mobius.MDMDeliveryPending, ctx, ds, t)
	...
	require.Equal(t, uint(0), res.Verifying)
	require.Equal(t, uint(0), res.Verified)
	require.True(t, checkListHosts(mobius.OSSettingsPending, &tm.ID, hosts[9:10]))
	require.True(t, checkListHosts(mobius.OSSettingsFailed, &tm.ID, []*mobius.Host{}))
	require.True(t, checkListHosts(mobius.OSSettingsVerifying, &tm.ID, []*mobius.Host{}))
	require.True(t, checkListHosts(mobius.OSSettingsVerified, &tm.ID, []*mobius.Host{}))
	...
	upsertHostCPs(hosts[9:10], teamCPs, mobius.MDMOperationTypeInstall, &mobius.MDMDeliveryVerifying, ctx, ds, t)
	...
	upsertHostCPs(hosts[9:10], noTeamCPs, mobius.MDMOperationTypeRemove, &mobius.MDMDeliveryVerifying, ctx, ds, t)
	res, err = ds.GetMDMAppleProfilesSummary(ctx, &tm.ID)
	require.NoError(t, err)
	require.NotNil(t, res)
	require.Equal(t, uint(0), res.Pending)
	...
	require.True(t, checkListHosts(mobius.OSSettingsPending, &tm.ID, []*mobius.Host{}))
	require.True(t, checkListHosts(mobius.OSSettingsFailed, &tm.ID, []*mobius.Host{}))
	require.True(t, checkListHosts(mobius.OSSettingsVerifying, &tm.ID, hosts[9:10]))
	require.True(t, checkListHosts(mobius.OSSettingsVerified, &tm.ID, []*mobius.Host{}))

	// verify one profile on hosts[9]
	upsertHostCPs(hosts[9:10], teamCPs[0:1], mobius.MDMOperationTypeInstall, &mobius.MDMDeliveryVerified, ctx, ds, t)
	res, err = ds.GetMDMAppleProfilesSummary(ctx, &tm.ID)
	require.NoError(t, err)
	require.NotNil(t, res)
	require.Equal(t, uint(0), res.Pending)
	...
	upsertHostCPs(hosts[9:10], teamCPs[1:], mobius.MDMOperationTypeInstall, &mobius.MDMDeliveryVerified, ctx, ds, t)
	res, err = ds.GetMDMAppleProfilesSummary(ctx, &tm.ID)
	require.NoError(t, err)
	require.NotNil(t, res)
	require.Equal(t, uint(0), res.Pending)
	...
	require.True(t, checkListHosts(mobius.OSSettingsFailed, &tm.ID, []*mobius.Host{}))
	require.True(t, checkListHosts(mobius.OSSettingsVerifying, &tm.ID, []*mobius.Host{}))
	require.True(t, checkListHosts(mobius.OSSettingsVerified, &tm.ID, hosts[9:10]))
...
func testDoNotIgnoreMDMClientError(t *testing.T, ds *Datastore) {
	...
	require.NoError(t, ds.BulkUpsertMDMAppleHostProfiles(ctx, []*mobius.MDMAppleBulkUpsertHostProfilePayload{{
	...
	require.Equal(t, mobius.MDMOperationTypeRemove, cps[0].OperationType)
	require.NotNil(t, cps[0].Status)
	require.Equal(t, mobius.MDMDeliveryPending, *cps[0].Status)

	// simulate remove failed with client error message
	require.NoError(t, ds.UpdateOrDeleteHostMDMAppleProfile(ctx, &mobius.HostMDMAppleProfile{
		CommandUUID:   "c1",
		HostUUID:      "h1",
	...
	require.Equal(t, mobius.MDMOperationTypeRemove, cps[0].OperationType)
	require.NotNil(t, cps[0].Status)
	require.Equal(t, mobius.MDMDeliveryFailed, *cps[0].Status)
...
func createDiskEncryptionRecord(ctx context.Context, ds *Datastore, t *testing.T, host *mobius.Host, key string, decryptable bool,
	threshold time.Time,
) {
...
func TestMDMAppleFileVaultSummary(t *testing.T) {
	...
	// no teams tests =====
	noTeamFVProfile, err := ds.NewMDMAppleConfigProfile(ctx, *generateCP(mobiusmdm.MobiusFileVaultProfileName, mobileconfig.MobiusFileVaultPayloadIdentifier, 0), nil)
	require.NoError(t, err)

	// verifying status
	verifyingHost := hosts[0]
	upsertHostCPs(
		[]*mobius.Host{verifyingHost},
	...
	upsertHostCPs(
		[]*mobius.Host{requiredActionHost},
		[]*mobius.MDMAppleConfigProfile{noTeamFVProfile},
		mobius.MDMOperationTypeInstall,
	...
	// failed status
	failedHost := hosts[3]
	upsertHostCPs([]*mobius.Host{failedHost}, []*mobius.MDMAppleConfigProfile{noTeamFVProfile}, mobius.MDMOperationTypeInstall, &mobius.MDMDeliveryFailed, ctx, ds, t)
	...
	upsertHostCPs([]*mobius.Host{removingEnforcementHost}, []*mobius.MDMAppleConfigProfile{noTeamFVProfile}, mobius.MDMOperationTypeRemove, &mobius.MDMDeliveryPending, ctx, ds, t)
	fvProfileSummary, err = ds.GetMDMAppleFileVaultSummary(ctx, nil)
	...
	tm, err := ds.NewTeam(ctx, &mobius.Team{Name: "team-1"})
	require.NoError(t, err)
	team1FVProfile, err := ds.NewMDMAppleConfigProfile(ctx, *generateCP(mobiusmdm.MobiusFileVaultProfileName, mobileconfig.MobiusFileVaultPayloadIdentifier, tm.ID), nil)
	require.NoError(t, err)
	err = ds.AddHostsToTeam(ctx, &tm.ID, []uint{verifyingTeam1Host.ID})
	require.NoError(t, err)

	upsertHostCPs([]*mobius.Host{verifyingTeam1Host}, []*mobius.MDMAppleConfigProfile{team1FVProfile}, mobius.MDMOperationTypeInstall, &mobius.MDMDeliveryVerifying, ctx, ds, t)
	...
	// verified status
	upsertHostCPs(
		[]*mobius.Host{verifyingTeam1Host},
		[]*mobius.MDMAppleConfigProfile{team1FVProfile},
		mobius.MDMOperationTypeInstall,
...
func testGetMDMAppleCommandResults(t *testing.T, ds *Datastore) {
	...
	enrolledHosts := make([]*mobius.Host, 3)
	for i := 0; i < 3; i++ {
		h, err := ds.NewHost(ctx, &mobius.Host{
	...
	i := 3
	unenrolledHost, err := ds.NewHost(ctx, &mobius.Host{
		Hostname:      fmt.Sprintf("test-host%d-name", i),
		OsqueryHostID: ptr.String(fmt.Sprintf("osquery-%d", i)),
		NodeKey:       ptr.String(fmt.Sprintf("nodekey-%d", i)),
		UUID:          fmt.Sprintf("test-uuid-%d", i),
	...
	require.Equal(t, res[0], &mobius.MDMCommandResult{
	...
	require.ElementsMatch(t, res, []*mobius.MDMCommandResult{
		{
			HostUUID:    enrolledHosts[0].UUID,
			CommandUUID: uuid2,
			Status:      "Acknowledged",
...
func testBulkUpsertMDMAppleConfigProfile(t *testing.T, ds *Datastore) {
	...
	globalCP := &mobius.MDMAppleConfigProfile{
		Name:         "DummyTestName",
		Identifier:   "DummyTestIdentifier",
		Mobileconfig: mc,
		TeamID:       nil,
	}
	teamCP := &mobius.MDMAppleConfigProfile{
		Name:         "DummyTestName",
		Identifier:   "DummyTestIdentifier",
		Mobileconfig: mc,
		TeamID:       ptr.Uint(1),
	}
	allProfiles := []*mobius.MDMAppleConfigProfile{globalCP, teamCP}
...
func testMDMAppleBootstrapPackageCRUD(t *testing.T, ds *Datastore) {
	ctx := t.Context()
	var nfe mobius.NotFoundError
	var aerr mobius.AlreadyExistsError

	err := ds.InsertMDMAppleBootstrapPackage(ctx, &mobius.MDMAppleBootstrapPackage{}, nil)
	require.Error(t, err)

	bp1 := &mobius.MDMAppleBootstrapPackage{
		TeamID: uint(0),
		Name:   t.Name(),
	...
	bp2 := &mobius.MDMAppleBootstrapPackage{
		TeamID: uint(2),
		Name:   t.Name(),
		Sha256: sha256.New().Sum(nil),
		Bytes:  []byte("content"),
...
func testListMDMAppleCommands(t *testing.T, ds *Datastore) {
	...
	// create a team
	tm1, err := ds.NewTeam(ctx, &mobius.Team{Name: "team1"})
	require.NoError(t, err)
	...
	require.ElementsMatch(t, res, []*mobius.MDMAppleCommand{
		{
	...
	res, err = ds.ListMDMAppleCommands(ctx, mobius.TeamFilter{User: test.UserAdmin}, &mobius.MDMCommandListOptions{
		ListOptions: mobius.ListOptions{Page: 0, PerPage: 3, OrderKey: "device_id", OrderDirection: mobius.OrderDescending},
	})
	require.NoError(t, err)
	require.Len(t, res, 3)
	...
	res, err = ds.ListMDMAppleCommands(ctx, mobius.TeamFilter{User: test.UserAdmin}, &mobius.MDMCommandListOptions{
		ListOptions: mobius.ListOptions{Page: 1, PerPage: 3, OrderKey: "device_id", OrderDirection: mobius.OrderDescending},
	})
	require.NoError(t, err)
	require.Len(t, res, 1)

	// filter by a user from team tm1, can only see that team's host
	u1, err := ds.NewUser(ctx, &mobius.User{
		...
		Teams: []mobius.UserTeam{
			{Team: *tm1, Role: mobius.RoleObserver},
		},
	})
	require.NoError(t, err)
	...
	res, err = ds.ListMDMAppleCommands(ctx, mobius.TeamFilter{User: u1}, &mobius.MDMCommandListOptions{
		ListOptions: mobius.ListOptions{PerPage: 3},
	})
	require.NoError(t, err)
	require.Len(t, res, 0)

	// now with IncludeObserver set to true
	res, err = ds.ListMDMAppleCommands(ctx, mobius.TeamFilter{User: u1, IncludeObserver: true}, &mobius.MDMCommandListOptions{
		ListOptions: mobius.ListOptions{PerPage: 3, OrderKey: "updated_at", OrderDirection: mobius.OrderDescending},
	})
	require.NoError(t, err)
	require.Len(t, res, 1)
	require.NotZero(t, res[0].UpdatedAt)
...
func testMDMAppleSetupAssistant(t *testing.T, ds *Datastore) {
	...
	// create for no team
	noTeamAsst, err := ds.SetOrUpdateMDMAppleSetupAssistant(ctx, &mobius.MDMAppleSetupAssistant{Name: "test", Profile: json.RawMessage("{}")})
	require.NoError(t, err)
	...
	_, err = ds.SetOrUpdateMDMAppleSetupAssistant(ctx, &mobius.MDMAppleSetupAssistant{TeamID: ptr.Uint(123), Name: "test", Profile: json.RawMessage("{}")})
	require.Error(t, err)
	require.ErrorContains(t, err, "foreign key constraint fails")

	// create a team
	tm, err := ds.NewTeam(ctx, &mobius.Team{Name: "tm"})
	require.NoError(t, err)

	// create for existing team
	tmAsst, err := ds.SetOrUpdateMDMAppleSetupAssistant(ctx, &mobius.MDMAppleSetupAssistant{TeamID: &tm.ID, Name: "test", Profile: json.RawMessage("{}")})
	require.NoError(t, err)
	...
	tok1, err := ds.InsertABMToken(ctx, &mobius.ABMToken{OrganizationName: "o1", EncryptedToken: []byte(uuid.NewString())})
	require.NoError(t, err)
	...
	// with both tokens
	tok2, err := ds.InsertABMToken(ctx, &mobius.ABMToken{OrganizationName: "o2", EncryptedToken: []byte(uuid.NewString())})
	require.NoError(t, err)
	...
	tmAsst2, err := ds.SetOrUpdateMDMAppleSetupAssistant(ctx, &mobius.MDMAppleSetupAssistant{TeamID: &tm.ID, Name: "test2", Profile: json.RawMessage(`{"x":2}`)})
	require.NoError(t, err)
	...
	noTeamAsst2, err := ds.SetOrUpdateMDMAppleSetupAssistant(ctx, &mobius.MDMAppleSetupAssistant{Name: "test3", Profile: json.RawMessage(`{"x": 3}`)})
	require.NoError(t, err)
	...
	tmAsst3, err := ds.SetOrUpdateMDMAppleSetupAssistant(ctx, &mobius.MDMAppleSetupAssistant{TeamID: &tm.ID, Name: "test2", Profile: json.RawMessage(`{"x":2}`)})
	require.NoError(t, err)
	...
	tmAsst4, err := ds.SetOrUpdateMDMAppleSetupAssistant(ctx, &mobius.MDMAppleSetupAssistant{TeamID: &tm.ID, Name: "test2", Profile: json.RawMessage(`{"x":3}`)})
	require.NoError(t, err)
...
func testMDMAppleEnrollmentProfile(t *testing.T, ds *Datastore) {
	...
	_, err := ds.GetMDMAppleEnrollmentProfileByType(ctx, mobius.MDMAppleEnrollmentTypeAutomatic)
	require.Error(t, err)
	require.ErrorIs(t, err, sql.ErrNoRows)
	...
	profAuto, err := ds.NewMDMAppleEnrollmentProfile(ctx, mobius.MDMAppleEnrollmentProfilePayload{
		Type:       "automatic",
		DEPProfile: &rawMsg,
		Token:      "abcd",
	})
	...
	profMan, err := ds.NewMDMAppleEnrollmentProfile(ctx, mobius.MDMAppleEnrollmentProfilePayload{
		Type:       "manual",
		DEPProfile: &rawMsg,
		Token:      "efgh",
	})
	...
	getProf, err := ds.GetMDMAppleEnrollmentProfileByType(ctx, mobius.MDMAppleEnrollmentTypeAutomatic)
	require.NoError(t, err)
	getProf.UpdateCreateTimestamps = mobius.UpdateCreateTimestamps{}
...
func testListMDMAppleSerials(t *testing.T, ds *Datastore) {
	...
	// create a mix of DEP-enrolled hosts, non-Mobius-MDM, pending DEP-enrollment
	hosts := make([]*mobius.Host, 7)
	for i := 0; i < len(hosts); i++ {
		...
		switch {
		case i <= 3:
			// assigned in ABM to Mobius
			err = ds.UpsertMDMAppleHostDEPAssignments(ctx, []mobius.Host{*h}, abmToken.ID)
			require.NoError(t, err)
		case i == 4:
			// not ABM assigned
		case i == 5:
	...
	tm1, err := ds.NewTeam(ctx, &mobius.Team{Name: "team1"})
	require.NoError(t, err)
	tm2, err := ds.NewTeam(ctx, &mobius.Team{Name: "team2"})
	require.NoError(t, err)
...
func testSetVerifiedMacOSProfiles(t *testing.T, ds *Datastore) {
	...
	expectedHostMDMStatus := make(map[uint]map[string]mobius.MDMDeliveryStatus)
	...
	require.NoError(t, err)
	require.Len(t, cps, 4)
	storedByIdentifier := make(map[string]*mobius.MDMAppleConfigProfile)
	for _, cp := range cps {
	...
	for i := 0; i < 3; i++ {
		...
		hosts = append(hosts, h)
		expectedHostMDMStatus[h.ID] = map[string]mobius.MDMDeliveryStatus{
			cp1.Identifier: mobius.MDMDeliveryPending,
			cp2.Identifier: mobius.MDMDeliveryVerifying,
			cp3.Identifier: mobius.MDMDeliveryVerified,
			cp4.Identifier: mobius.MDMDeliveryPending,
		}
	}
	...
	adHocSetVerifying := func(hostUUID, profileIndentifier string) {
		ExecAdhocSQL(t, ds, func(tx sqlx.ExtContext) error {
			_, err := tx.ExecContext(ctx,
				...
				mobius.MDMDeliveryVerifying, hostUUID, profileIndentifier)
			return err
		})
	}

	// initialize the host MDM profile statuses
	upsertHostCPs(hosts, []*mobius.MDMAppleConfigProfile{storedByIdentifier[cp1.Identifier]}, mobius.MDMOperationTypeInstall, &mobius.MDMDeliveryPending, ctx, ds, t)
	upsertHostCPs(hosts, []*mobius.MDMAppleConfigProfile{storedByIdentifier[cp2.Identifier]}, mobius.MDMOperationTypeInstall, &mobius.MDMDeliveryVerifying, ctx, ds, t)
	upsertHostCPs(hosts, []*mobius.MDMAppleConfigProfile{storedByIdentifier[cp3.Identifier]}, mobius.MDMOperationTypeInstall, &mobius.MDMDeliveryVerified, ctx, ds, t)
	upsertHostCPs(hosts, []*mobius.MDMAppleConfigProfile{storedByIdentifier[cp4.Identifier]}, mobius.MDMOperationTypeInstall, &mobius.MDMDeliveryPending, ctx, ds, t)
	checkHostMDMProfileStatuses()

	// statuses don't change during the grace period if profiles are missing (i.e. not installed)
	require.NoError(t, apple_mdm.VerifyHostMDMProfiles(ctx, ds, hosts[0], map[string]*mobius.HostMacOSProfile{}))
	checkHostMDMProfileStatuses()

	// if install date is before the updated at timestamp of the profile, statuses don't change
	// during the grace period
	require.NoError(t, apple_mdm.VerifyHostMDMProfiles(ctx, ds, hosts[1], profilesByIdentifier([]*mobius.HostMacOSProfile{
		{
	...
	require.NoError(t, apple_mdm.VerifyHostMDMProfiles(ctx, ds, hosts[2], profilesByIdentifier([]*mobius.HostMacOSProfile{
		{
			Identifier:  cp2.Identifier,
			DisplayName: cp2.Name,
	...
	expectedHostMDMStatus[hosts[2].ID][cp2.Identifier] = mobius.MDMDeliveryVerified
	expectedHostMDMStatus[hosts[2].ID][cp4.Identifier] = mobius.MDMDeliveryVerified
	checkHostMDMProfileStatuses()
	...
	expectedHostMDMStatus[hosts[2].ID][cp1.Identifier] = mobius.MDMDeliveryVerified // cp1 can go from pending to verified
	expectedHostMDMStatus[hosts[2].ID][cp3.Identifier] = mobius.MDMDeliveryPending  // first retry for cp3
	expectedHostMDMStatus[hosts[2].ID][cp4.Identifier] = mobius.MDMDeliveryPending  // first retry for cp4
	checkHostMDMProfileStatuses()
	...
	expectedHostMDMStatus[hosts[2].ID][cp3.Identifier] = mobius.MDMDeliveryFailed // still missing after retry so expect cp3 to fail
	expectedHostMDMStatus[hosts[2].ID][cp4.Identifier] = mobius.MDMDeliveryFailed // still missing after retry so expect cp4 to fail
	checkHostMDMProfileStatuses()
	...
	expectedHostMDMStatus[hosts[2].ID][cp2.Identifier] = mobius.MDMDeliveryPending // first retry for cp2
	checkHostMDMProfileStatuses()
	// simulate retry command acknowledged by setting status to "verifying"
	...
	expectedHostMDMStatus[hosts[2].ID][cp2.Identifier] = mobius.MDMDeliveryFailed // still outdated after retry so expect cp2 to fail
	checkHostMDMProfileStatuses()
}
...
func TestCopyDefaultMDMAppleBootstrapPackage(t *testing.T) {
	...
	checkStoredBP := func(teamID uint, wantErr error, wantNewToken bool, wantBP *mobius.MDMAppleBootstrapPackage) {
		var gotBP mobius.MDMAppleBootstrapPackage
	...
	tm, err := ds.NewTeam(ctx, &mobius.Team{Name: "test"})
	require.NoError(t, err)
	teamID := tm.ID
	noTeamID := uint(0)
	...
	defaultBP := &mobius.MDMAppleBootstrapPackage{
		TeamID: noTeamID,
		Name:   "name",
	...
	defaultBP2 := &mobius.MDMAppleBootstrapPackage{
		TeamID: noTeamID,
		Name:   "new name",
	...
	tc.Config.WebhookSettings.FailingPoliciesWebhook = mobius.FailingPoliciesWebhookSettings{
		Enable:         true,
...
func TestHostDEPAssignments(t *testing.T) {
	...
	t.Run("DEP enrollment", func(t *testing.T) {
		...
		require.Equal(t, mobius.WellKnownMDMMobius, getHostResp.MDM.Name)
		require.Nil(t, getHostResp.DEPAssignedToMobius) // always nil for get host
		...
		testHost, err := ds.EnrollOrbit(ctx, true, mobius.OrbitHostInfo{HardwareSerial: depSerial, Platform: "darwin", HardwareUUID: depUUID, Hostname: "dep-host"}, depOrbitNodeKey, nil)
		require.NoError(t, err)
		...
		require.NoError(t, err)
		require.True(t, *h.DEPAssignedToMobius)
		h, err = ds.LoadHostByDeviceAuthToken(ctx, depDeviceTok, 1*time.Hour)
		require.NoError(t, err)
		require.True(t, *h.DEPAssignedToMobius)

		// simulate osquery report of MDM detail query
		err = ds.SetOrUpdateMDMData(ctx, testHost.ID, false, true, expectedMDMServerURL, true, mobius.WellKnownMDMMobius, "")
		require.NoError(t, err)
	...
	t.Run("manual enrollment", func(t *testing.T) {
		...
		err = ds.MDMAppleUpsertHost(ctx, &mobius.Host{HardwareSerial: manualSerial, UUID: manualUUID})
		require.NoError(t, err)
		...
		// simulate initial osquery enrollment via Orbit
		manualHost, err := ds.EnrollOrbit(ctx, true, mobius.OrbitHostInfo{HardwareSerial: manualSerial, Platform: "darwin", HardwareUUID: manualUUID, Hostname: "maunual-host"}, manualOrbitNodeKey, nil)
		require.NoError(t, err)
		...
		require.NoError(t, err)
		require.False(t, *h.DEPAssignedToMobius)
...
%s
	...
	for i, c := range cases {
		t.Run(fmt.Sprintf("%q %q %q", c.prefix, c.content, c.suffix), func(t *testing.T) {
			...
			prof, err := ds.NewMDMAppleConfigProfile(ctx, mobius.MDMAppleConfigProfile{
			...
			goProf := mobius.MDMApplePreassignProfilePayload{Profile: mc}
...
func testMDMAppleResetEnrollment(t *testing.T, ds *Datastore) {
	...
	cp, err := ds.NewMDMAppleConfigProfile(ctx, *generateCP("name0", "identifier0", 0), nil)
	require.NoError(t, err)
	upsertHostCPs([]*mobius.Host{host}, []*mobius.MDMAppleConfigProfile{cp}, mobius.MDMOperationTypeInstall, &mobius.MDMDeliveryVerified, ctx, ds, t)
	...
	`, host.UUID)
	require.NoError(t, err)
	err = ds.InsertMDMAppleBootstrapPackage(ctx, &mobius.MDMAppleBootstrapPackage{
		TeamID: uint(0),
		Name:   t.Name(),
		Sha256: sha256.New().Sum(nil),
		Bytes:  []byte("content"),
...
func testLockUnlockWipeMacOS(t *testing.T, ds *Datastore) {
	...
	err = ds.UnlockHostManually(ctx, host.ID, host.MobiusPlatform(), time.Now().UTC())
	require.NoError(t, err)
...
func testMDMAppleDDMDeclarationsToken(t *testing.T, ds *Datastore) {
	...
	decl, err := ds.NewMDMAppleDeclaration(ctx, &mobius.MDMAppleDeclaration{
		Identifier: "decl-1",
		Name:       "decl-1",
	})
	require.NoError(t, err)
	...
	decl2, err := ds.NewMDMAppleDeclaration(ctx, &mobius.MDMAppleDeclaration{
		Identifier: "decl-2",
		Name:       "decl-2",
	})
	require.NoError(t, err)
...
func testMDMAppleSetPendingDeclarationsAs(t *testing.T, ds *Datastore) {
	...
	for i := 0; i < 10; i++ {
		_, err := ds.NewMDMAppleDeclaration(ctx, &mobius.MDMAppleDeclaration{
	...
	checkStatus := func(declarations []mobius.HostMDMAppleProfile, wantStatus mobius.MDMDeliveryStatus, wantDetail string) {
	...
	checkStatus(profs, mobius.MDMDeliveryPending, "")

	err = ds.MDMAppleSetPendingDeclarationsAs(ctx, h.UUID, &mobius.MDMDeliveryFailed, "mock error")
	require.NoError(t, err)
	profs, err = ds.GetHostMDMAppleProfiles(ctx, h.UUID)
	require.NoError(t, err)
	checkStatus(profs, mobius.MDMDeliveryFailed, "mock error")
}

func testSetOrUpdateMDMAppleDDMDeclaration(t *testing.T, ds *Datastore) {
	ctx := t.Context()
	l1, err := ds.NewLabel(ctx, &mobius.Label{Name: "l1", Query: "select 1"})
	require.NoError(t, err)
	l2, err := ds.NewLabel(ctx, &mobius.Label{Name: "l2", Query: "select 2"})
	require.NoError(t, err)
	tm1, err := ds.NewTeam(ctx, &mobius.Team{Name: "tm1"})
	require.NoError(t, err)

	d1, err := ds.NewMDMAppleDeclaration(ctx, &mobius.MDMAppleDeclaration{
		Identifier: "i1",
		Name:       "d1",
	...
	d1tm1, err := ds.SetOrUpdateMDMAppleDeclaration(ctx, &mobius.MDMAppleDeclaration{
		Identifier: "i1",
		Name:       "d1",
		TeamID:     &tm1.ID,
		RawJSON:    json.RawMessage(`{"Identifier": "i1"}`),
	...
	d1, err = ds.SetOrUpdateMDMAppleDeclaration(ctx, &mobius.MDMAppleDeclaration{
		Identifier:       "i1b",
		Name:             "d1",
		RawJSON:          json.RawMessage(`{"Identifier": "i1b"}`),
		LabelsIncludeAll: []mobius.ConfigurationProfileLabel{{LabelName: l1.Name, LabelID: l1.ID}},
	})
	require.NoError(t, err)
	...
	d1, err = ds.SetOrUpdateMDMAppleDeclaration(ctx, &mobius.MDMAppleDeclaration{
		...
		Name:             "d1",
		RawJSON:          json.RawMessage(`{"Identifier": "i1b"}`),
		LabelsIncludeAll: []mobius.ConfigurationProfileLabel{{LabelName: l2.Name, LabelID: l2.ID}},
	})
	require.NoError(t, err)
	require.Equal(t, d1.DeclarationUUID, d1Ori.DeclarationUUID)
	...
	d1tm1B, err := ds.SetOrUpdateMDMAppleDeclaration(ctx, &mobius.MDMAppleDeclaration{
		Identifier:       "i1b",
		Name:             "d1",
		TeamID:           &tm1.ID,
		RawJSON:          json.RawMessage(`{"Identifier": "i1b"}`),
...
func TestMDMAppleProfileVerification(t *testing.T) {
	...
	type testCase struct {
		name           string
		initialStatus  mobius.MDMDeliveryStatus
		expectedStatus mobius.MDMDeliveryStatus
		expectedDetail string
	}

	setupTestProfile := func(t *testing.T, suffix string) *mobius.MDMAppleConfigProfile {
	...
	setProfileUploadedAt := func(t *testing.T, cp *mobius.MDMAppleConfigProfile, ua time.Time) {
		ExecAdhocSQL(t, ds, func(tx sqlx.ExtContext) error {
	...
	checkHostStatus := func(t *testing.T, h *mobius.Host, expectedStatus mobius.MDMDeliveryStatus, expectedDetail string) error {
		gotProfs, err := ds.GetHostMDMAppleProfiles(ctx, h.UUID)
		if err != nil {
			return err
		}
	...
	initializeProfile := func(t *testing.T, h *mobius.Host, cp *mobius.MDMAppleConfigProfile, status mobius.MDMDeliveryStatus, prevRetries uint) {
		upsertHostCPs([]*mobius.Host{h}, []*mobius.MDMAppleConfigProfile{cp}, mobius.MDMOperationTypeInstall, &status, ctx, ds, t)
	...
	t.Run("MissingProfileWithRetry", func(t *testing.T) {
		...
		cases := []testCase{
			{
				...
				initialStatus:  mobius.MDMDeliveryPending,
				expectedStatus: mobius.MDMDeliveryPending, // no change
			},
			{
				name:           "VerifyingThenMissing",
				initialStatus:  mobius.MDMDeliveryVerifying,
				expectedStatus: mobius.MDMDeliveryFailed, // change to failed
			},
			{
				name:           "VerifiedThenMissing",
				initialStatus:  mobius.MDMDeliveryVerified,
				expectedStatus: mobius.MDMDeliveryFailed, // change to failed
			},
			{
				name:           "FailedThenMissing",
				initialStatus:  mobius.MDMDeliveryFailed,
				expectedStatus: mobius.MDMDeliveryFailed, // no change
			},
		}
		...
		for i, tc := range cases {
			...
			var reportedProfiles []*mobius.HostMacOSProfile // no profiles reported for this test
			...
			require.NoError(t, apple_mdm.VerifyHostMDMProfiles(ctx, ds, h, profilesByIdentifier(reportedProfiles)))
			if tc.expectedStatus == mobius.MDMDeliveryFailed {
				// grace period expired, first failure gets retried so status should be pending and empty detail
				require.NoError(t, checkHostStatus(t, h, mobius.MDMDeliveryPending, ""), tc.name)
			}

			if tc.initialStatus != mobius.MDMDeliveryPending {
				// after retry, assume successful install profile command so status should be verifying
				upsertHostCPs([]*mobius.Host{h}, []*mobius.MDMAppleConfigProfile{cp}, mobius.MDMOperationTypeInstall, &mobius.MDMDeliveryVerifying, ctx, ds, t)
				// report osquery results
				require.NoError(t, apple_mdm.VerifyHostMDMProfiles(ctx, ds, h, profilesByIdentifier(reportedProfiles)))
				// now we see the expected status
				require.NoError(t, checkHostStatus(t, h, tc.expectedStatus, string(mobius.HostMDMProfileDetailFailedWasVerifying)), tc.name) // grace period expired, max retries so check expected status
			}
		}
	})
	...
	t.Run("OutdatedProfile", func(t *testing.T) {
		...
		cases := []testCase{
			...
			},
			{
				...
				initialStatus:  mobius.MDMDeliveryVerifying,
				expectedStatus: mobius.MDMDeliveryFailed, // change to failed
				expectedDetail: string(mobius.HostMDMProfileDetailFailedWasVerifying),
			},
			{
				name:           "VerifiedThenFoundOutdated",
				initialStatus:  mobius.MDMDeliveryVerified,
				expectedStatus: mobius.MDMDeliveryFailed, // change to failed
				expectedDetail: string(mobius.HostMDMProfileDetailFailedWasVerified),
			},
			{
		...
		for i, tc := range cases {
			t.Run(tc.name, func(t *testing.T) {
				...
				h := test.NewHost(t, ds, tc.name, tc.name, tc.name, tc.name, twoMinutesAgo)
				cp := setupTestProfile(t, fmt.Sprintf("%s-%d", tc.name, i))
				reportedProfiles := []*mobius.HostMacOSProfile{
					{
						DisplayName: cp.Name,
		...
		cases := []testCase{
			{
				...
				initialStatus:  mobius.MDMDeliveryPending,
				expectedStatus: mobius.MDMDeliveryVerified, // pending can go to verified if found
				expectedDetail: "",
			},
			{
				name:           "VerifyingThenFoundExpected",
				initialStatus:  mobius.MDMDeliveryVerifying,
				expectedStatus: mobius.MDMDeliveryVerified, // change to verified
				expectedDetail: "",
			},
			{
				name:           "VerifiedThenFoundExpected",
				initialStatus:  mobius.MDMDeliveryVerified,
				expectedStatus: mobius.MDMDeliveryVerified, // no change
				expectedDetail: "",
			},
			{
				name:           "FailedThenFoundExpected",
				initialStatus:  mobius.MDMDeliveryFailed,
				expectedStatus: mobius.MDMDeliveryVerified, // failed can become verified if found later
				expectedDetail: "",
			},
		}
		...
		cases := []testCase{
			{
				...
				initialStatus:  mobius.MDMDeliveryPending,
				expectedStatus: mobius.MDMDeliveryVerified, // profile can go from pending to verified
				expectedDetail: "",
			},
			{
	...
	t.Run("EarliestInstallDate", func(t *testing.T) {
		...
		require.NoError(t, apple_mdm.VerifyHostMDMProfiles(ctx, ds, h, profilesByIdentifier(reportedProfiles)))
		require.NoError(t, checkHostStatus(t, h, mobius.MDMDeliveryVerifying, "")) // no change
		...
		require.NoError(t, apple_mdm.VerifyHostMDMProfiles(ctx, ds, h, profilesByIdentifier(reportedProfiles)))
		require.NoError(t, checkHostStatus(t, h, mobius.MDMDeliveryFailed, string(mobius.HostMDMProfileDetailFailedWasVerifying))) // set to failed
		...
		require.NoError(t, apple_mdm.VerifyHostMDMProfiles(ctx, ds, h, profilesByIdentifier(reportedProfiles)))
		require.NoError(t, checkHostStatus(t, h, mobius.MDMDeliveryVerified, "")) // set to verified based on earliest install date
	})
}

func profilesByIdentifier(profiles []*mobius.HostMacOSProfile) map[string]*mobius.HostMacOSProfile {
	byIdentifier := map[string]*mobius.HostMacOSProfile{}
...
func TestRestorePendingDEPHost(t *testing.T) {
	...
	t.Run("DEP enrollment", func(t *testing.T) {
		...
		checkStoredHost := func(t *testing.T, hostID uint, expectedHost *mobius.Host) {
		...
		setupTestHost := func(t *testing.T) (pendingHost, mdmEnrolledHost *mobius.Host) {
			depSerial := "dep-serial"
			depUUID := "dep-uuid"
			depOrbitNodeKey := "dep-orbit-node-key"
			...
			require.Equal(t, mobius.WellKnownMDMMobius, pendingHost.MDM.Name)
			...
			// simulate osquery report of MDM detail query
			err = ds.SetOrUpdateMDMData(ctx, depHostID, false, true, expectedMDMServerURL, true, mobius.WellKnownMDMMobius, "")
			require.NoError(t, err)
			...
			require.Equal(t, mobius.WellKnownMDMMobius, mdmEnrolledHost.MDM.Name)
...
func testMDMConfigAsset(t *testing.T, ds *Datastore) {
	...
	assets := []mobius.MDMConfigAsset{
		{
			Name:  mobius.MDMAssetCACert,
			Value: []byte("a"),
		},
		{
			Name:  mobius.MDMAssetCAKey,
			Value: []byte("b"),
		},
	}
	wantAssets := map[mobius.MDMAssetName]mobius.MDMConfigAsset{}
	...
	a, err := ds.GetAllMDMConfigAssetsByName(ctx, []mobius.MDMAssetName{mobius.MDMAssetCACert, mobius.MDMAssetCAKey}, nil)
	require.NoError(t, err)
	require.Equal(t, wantAssets, a)

	h, err := ds.GetAllMDMConfigAssetsHashes(ctx, []mobius.MDMAssetName{mobius.MDMAssetCACert, mobius.MDMAssetCAKey})
	require.NoError(t, err)
	require.Len(t, h, 2)
	require.NotEmpty(t, h[mobius.MDMAssetCACert])
	require.NotEmpty(t, h[mobius.MDMAssetCAKey])

	// try to fetch an asset that doesn't exist
	var nfe mobius.NotFoundError
	a, err = ds.GetAllMDMConfigAssetsByName(ctx, []mobius.MDMAssetName{mobius.MDMAssetABMCert}, ds.writer(ctx))
	require.ErrorAs(t, err, &nfe)
	require.Nil(t, a)

	h, err = ds.GetAllMDMConfigAssetsHashes(ctx, []mobius.MDMAssetName{mobius.MDMAssetABMCert})
	require.ErrorAs(t, err, &nfe)
	require.Nil(t, h)

	// try to fetch a mix of assets that exist and doesn't exist
	a, err = ds.GetAllMDMConfigAssetsByName(ctx, []mobius.MDMAssetName{mobius.MDMAssetCACert, mobius.MDMAssetABMCert}, nil)
	require.ErrorIs(t, err, ErrPartialResult)
	require.Len(t, a, 1)

	h, err = ds.GetAllMDMConfigAssetsHashes(ctx, []mobius.MDMAssetName{mobius.MDMAssetCACert, mobius.MDMAssetABMCert})
	require.ErrorIs(t, err, ErrPartialResult)
	require.Len(t, h, 1)
	require.NotEmpty(t, h[mobius.MDMAssetCACert])
	...
	newAssets := []mobius.MDMConfigAsset{
		{
			Name:  mobius.MDMAssetCACert,
			Value: []byte("c"),
		},
	...
	wantNewAssets := map[mobius.MDMAssetName]mobius.MDMConfigAsset{}
	...
	a, err = ds.GetAllMDMConfigAssetsByName(ctx, []mobius.MDMAssetName{mobius.MDMAssetCACert, mobius.MDMAssetCAKey}, ds.reader(ctx))
	require.NoError(t, err)
	...
	err = ds.DeleteMDMConfigAssetsByName(ctx, []mobius.MDMAssetName{mobius.MDMAssetCACert, mobius.MDMAssetCAKey})
	require.NoError(t, err)
	...
	expected := make(map[string]mobius.MDMConfigAsset)
	...
	for _, got := range ar {
		...
		require.NoError(t, err)
		require.Equal(t, expected[string(d)].Name, mobius.MDMAssetName(got.Name))
	...
	// Hard delete
	err = ds.HardDeleteMDMConfigAsset(ctx, mobius.MDMAssetCACert)
	require.NoError(t, err)
	a, err = ds.GetAllMDMConfigAssetsByName(ctx, []mobius.MDMAssetName{mobius.MDMAssetCACert, mobius.MDMAssetCAKey}, nil)
	require.ErrorAs(t, err, &nfe)
	require.Nil(t, a)
	...
	var result bool
	err = sqlx.GetContext(ctx, ds.reader(ctx), &result, "SELECT 1 FROM mdm_config_assets WHERE name = ?", mobius.MDMAssetCACert)
	assert.ErrorIs(t, err, sql.ErrNoRows)

	// other (non-hard deleted asset still present)
	err = sqlx.GetContext(ctx, ds.reader(ctx), &result, "SELECT 1 FROM mdm_config_assets WHERE name = ?", mobius.MDMAssetCAKey)
...
func testListIOSAndIPadOSToRefetch(t *testing.T, ds *Datastore) {
	...
	hostCount := 0
	newHost := func(platform string) *mobius.Host {
		h, err := ds.NewHost(ctx, &mobius.Host{
	...
	require.NoError(t, ds.AddHostMDMCommands(ctx, []mobius.HostMDMCommand{{
		HostID:      iOS0.ID,
...
func testIngestMDMAppleDevicesFromDEPSyncIOSIPadOS(t *testing.T, ds *Datastore) {
	...
	hosts, err := ds.ListHosts(ctx, mobius.TeamFilter{
		User: &mobius.User{
			GlobalRole: ptr.String(mobius.RoleAdmin),
		},
	}, mobius.HostListOptions{})
	require.NoError(t, err)
...
func testMDMAppleProfilesOnIOSIPadOS(t *testing.T, ds *Datastore) {
	...
	// Add the Mobiusd configuration and  profile that are only for macOS.
	params := mobileconfig.MobiusdProfileOptions{
		EnrollSecret: t.Name(),
		ServerURL:    "https://example.com",
		PayloadType:  mobileconfig.MobiusdConfigPayloadIdentifier,
		PayloadName:  mobiusmdm.MobiusdConfigProfileName,
	}
	var contents bytes.Buffer
	err := mobileconfig.MobiusdProfileTemplate.Execute(&contents, params)
	require.NoError(t, err)
	mobiusdConfigProfile, err := mobius.NewMDMAppleConfigProfile(contents.Bytes(), nil)
	require.NoError(t, err)
	_, err = ds.NewMDMAppleConfigProfile(ctx, *mobiusdConfigProfile, nil)
	require.NoError(t, err)

	// For the FileVault profile we re-use the MobiusdProfileTemplate
	// (because fileVaultProfileTemplate is not exported)
	var contents2 bytes.Buffer
	params.PayloadName = mobiusmdm.MobiusFileVaultProfileName
	params.PayloadType = mobileconfig.MobiusFileVaultPayloadIdentifier
	err = mobileconfig.MobiusdProfileTemplate.Execute(&contents2, params)
	require.NoError(t, err)
	fileVaultProfile, err := mobius.NewMDMAppleConfigProfile(contents2.Bytes(), nil)
	require.NoError(t, err)
...
func testHostDetailsMDMProfilesIOSIPadOS(t *testing.T, ds *Datastore) {
	...
	p0, err := ds.NewMDMAppleConfigProfile(ctx, mobius.MDMAppleConfigProfile{
		Name:         "Name0",
	...
	iOS, err := ds.NewHost(ctx, &mobius.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
		SeenTime:        time.Now(),
	...
	require.NoError(t, err)
	iPadOS, err := ds.NewHost(ctx, &mobius.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
		SeenTime:        time.Now(),
	...
	expectedProfilesIOS := map[string]mobius.HostMDMAppleProfile{
		p0.ProfileUUID: {
			HostUUID:      iOS.UUID,
			Name:          p0.Name,
	...
	expectedProfilesIPadOS := map[string]mobius.HostMDMAppleProfile{
		p0.ProfileUUID: {
	...
	for _, tc := range []struct {
		host             *mobius.Host
		expectedProfiles map[string]mobius.HostMDMAppleProfile
	}{
		{
			host:             iOS,
		...
		require.Equal(t, mobius.MDMDeliveryVerified, *gotProfs[0].Status)
	}
}
...
func testMDMAppleBootstrapPackageWithS3(t *testing.T, ds *Datastore) {
	...
	bpMatchesWithoutContent := func(want, got *mobius.MDMAppleBootstrapPackage) {
	...
	err := ds.InsertMDMAppleBootstrapPackage(ctx, &mobius.MDMAppleBootstrapPackage{}, pkgStore)
	require.Error(t, err)
	...
	err = ds.CopyDefaultMDMAppleBootstrapPackage(ctx, &mobius.AppConfig{}, 1)
	require.NoError(t, err)
	...
	bp3 := &mobius.MDMAppleBootstrapPackage{
		TeamID: uint(3),
		Name:   "bp3",
...
func testMDMAppleGetAndUpdateABMToken(t *testing.T, ds *Datastore) {
	...
	tm2, err := ds.NewTeam(ctx, &mobius.Team{Name: "team2"})
	require.NoError(t, err)
	tm3, err := ds.NewTeam(ctx, &mobius.Team{Name: "team3"})
	require.NoError(t, err)
	...
	t1, err := ds.InsertABMToken(ctx, &mobius.ABMToken{OrganizationName: "unused", EncryptedToken: []byte(encTok)})
	require.NoError(t, err)
	require.NotEmpty(t, t1.ID)
	t2, err := ds.InsertABMToken(ctx, &mobius.ABMToken{EncryptedToken: []byte(encTok)})
	require.NoError(t, err)
	...
	require.Equal(t, mobius.TeamNameNoTeam, tok.MacOSTeamName)
	require.Equal(t, mobius.TeamNameNoTeam, tok.IOSTeamName)
	require.Equal(t, mobius.TeamNameNoTeam, tok.IPadOSTeamName)
	...
	require.Equal(t, mobius.TeamNameNoTeam, tokReload.IPadOSTeamName)
	require.Equal(t, mobius.TeamNameNoTeam, tokReload.IPadOSTeam.Name)
	...
	require.Equal(t, "org-name", tokReload.OrganizationName)
	require.Equal(t, "name@example.com", tokReload.AppleID)
	require.Equal(t, mobius.TeamNameNoTeam, tokReload.MacOSTeamName)
	require.Equal(t, mobius.TeamNameNoTeam, tokReload.MacOSTeam.Name)
	...
	require.Equal(t, mobius.TeamNameNoTeam, expTok.MacOSTeamName)
	require.Equal(t, mobius.TeamNameNoTeam, expTok.MacOSTeam.Name)
...
func testMDMAppleABMTokensTermsExpired(t *testing.T, ds *Datastore) {
	...
	t1, err := ds.InsertABMToken(ctx, &mobius.ABMToken{OrganizationName: "abm1", EncryptedToken: []byte(encTok1)})
	require.NoError(t, err)
	require.NotEmpty(t, t1.ID)
	encTok2 := uuid.NewString()
	t2, err := ds.InsertABMToken(ctx, &mobius.ABMToken{OrganizationName: "abm2", EncryptedToken: []byte(encTok2)})
	require.NoError(t, err)
	require.NotEmpty(t, t2.ID)
	// this one simulates a mirated token - empty name
	encTok3 := uuid.NewString()
	t3, err := ds.InsertABMToken(ctx, &mobius.ABMToken{OrganizationName: "", EncryptedToken: []byte(encTok3)})
	require.NoError(t, err)
...
func testMDMGetABMTokenOrgNamesAssociatedWithTeam(t *testing.T, ds *Datastore) {
	...
	tok1, err := ds.InsertABMToken(ctx, &mobius.ABMToken{OrganizationName: "org1", EncryptedToken: []byte(encTok)})
	require.NoError(t, err)
	require.NotEmpty(t, tok1.ID)

	tok2, err := ds.InsertABMToken(ctx, &mobius.ABMToken{OrganizationName: "org2", EncryptedToken: []byte(encTok)})
	require.NoError(t, err)
	require.NotEmpty(t, tok1.ID)

	tok3, err := ds.InsertABMToken(ctx, &mobius.ABMToken{OrganizationName: "org3", EncryptedToken: []byte(encTok), MacOSDefaultTeamID: &tm2.ID})
	require.NoError(t, err)
	require.NotEmpty(t, tok1.ID)
	...
	h3, err := ds.NewHost(ctx, &mobius.Host{
		Hostname:      "test-host3-name",
	...
	h4, err := ds.NewHost(ctx, &mobius.Host{
	...
	require.NoError(t, ds.UpsertMDMAppleHostDEPAssignments(ctx, []mobius.Host{*h1, *h4}, tok1.ID))
	require.NoError(t, ds.UpsertMDMAppleHostDEPAssignments(ctx, []mobius.Host{*h2}, tok3.ID))
	require.NoError(t, ds.UpsertMDMAppleHostDEPAssignments(ctx, []mobius.Host{*h3}, tok2.ID))
...
func testHostMDMCommands(t *testing.T, ds *Datastore) {
	...
	hostCommands := []mobius.HostMDMCommand{
		{
			HostID:      h.ID,
	...
	allCommands = append(allCommands, mobius.HostMDMCommand{
		HostID:      badHostID,
		CommandType: "command-1",
	})
...
func testIngestMDMAppleDeviceFromOTAEnrollment(t *testing.T, ds *Datastore) {
	...
	otaDevices := []mobius.MDMAppleMachineInfo{
...
func TestGetMDMAppleOSUpdatesSettingsByHostSerial(t *testing.T) {
	...
	getConfigSettings := func(teamID uint, key string) *mobius.AppleOSUpdateSettings {
		var settings mobius.AppleOSUpdateSettings
		ExecAdhocSQL(t, ds, func(q sqlx.ExtContext) error {
	...
	checkExpectedVersion := func(t *testing.T, gotSettings *mobius.AppleOSUpdateSettings, expectedVersion string) {
		if expectedVersion == "" {
	...
	encTok := uuid.NewString()
	abmToken, err := ds.InsertABMToken(context.Background(), &mobius.ABMToken{OrganizationName: "unused", EncryptedToken: []byte(encTok)})
	require.NoError(t, err)
	require.NotEmpty(t, abmToken.ID)
...
func testMDMManagedSCEPCertificates(t *testing.T, ds *Datastore) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			err = ds.BulkUpsertMDMManagedCertificates(ctx, []*mobius.MDMManagedCertificate{
				{
					HostUUID:             host.UUID,
					ProfileUUID:          initialCP.ProfileUUID,
			...
			assert.Equal(t, mobius.MDMDeliveryPending, *profile.Status)
		...
		`, mobius.MDMDeliveryVerified, host.UUID, initialCP.ProfileUUID)
					if err != nil {
						return err
					}
					return nil
				...
				require.NoError(t, err)
				require.NotNil(t, profile.Status)
				assert.Equal(t, mobius.MDMDeliveryVerified, *profile.Status)
...
func testMDMManagedDigicertCertificates(t *testing.T, ds *Datastore) {
	...
	assert.Equal(t, &notValidBefore, profile.NotValidBefore)
	assert.Equal(t, &notValidAfter, profile.NotValidAfter)
	assert.Equal(t, mobius.CAConfigDigiCert, profile.Type)
	require.NotNil(t, profile.Serial)
	assert.Equal(t, serial, *profile.Serial)
...
func testAppleMDMSetBatchAsyncLastSeenAt(t *testing.T, ds *Datastore) {
	...
	// create some hosts, all enrolled
	enrolledHosts := make([]*mobius.Host, 2)
	...
	getHostLastSeenAt := func(h *mobius.Host) time.Time {
...
func testMDMAppleProfileLabels(t *testing.T, ds *Datastore) {
	...
	require.NoError(t, err)
	matchProfiles([]*mobius.MDMAppleProfilePayload{
		{ProfileUUID: globProf1.ProfileUUID, ProfileIdentifier: globProf1.Identifier, ProfileName: globProf1.Name, HostUUID: "test-uuid-1", HostPlatform: "darwin", Scope: mobius.PayloadScopeSystem},
		{ProfileUUID: globProf2.ProfileUUID, ProfileIdentifier: globProf2.Identifier, ProfileName: globProf2.Name, HostUUID: "test-uuid-1", HostPlatform: "darwin", Scope: mobius.PayloadScopeSystem},
	}, profilesToInstall)

	hostLabel, err := ds.NewHost(ctx, &mobius.Host{
	...
	// include-any labels
	l1, err := ds.NewLabel(ctx, &mobius.Label{Name: "include-any-1", Query: "select 1"})
	require.NoError(t, err)

	l2, err := ds.NewLabel(ctx, &mobius.Label{Name: "include-any-2", Query: "select 1"})
	require.NoError(t, err)

	l3, err := ds.NewLabel(ctx, &mobius.Label{Name: "include-any-3", Query: "select 1"})
	require.NoError(t, err)

	// include-all labels
	l4, err := ds.NewLabel(ctx, &mobius.Label{Name: "include-all-4", Query: "select 1"})
	require.NoError(t, err)

	l5, err := ds.NewLabel(ctx, &mobius.Label{Name: "include-all-5", Query: "select 1"})
	require.NoError(t, err)

	// exclude-any labels
	l6, err := ds.NewLabel(ctx, &mobius.Label{Name: "exclude-any-6", Query: "select 1"})
	require.NoError(t, err)

	l7, err := ds.NewLabel(ctx, &mobius.Label{Name: "exclude-any-7", Query: "select 1"})
	require.NoError(t, err)
	...
	require.NoError(t, err)
	matchProfiles([]*mobius.MDMAppleProfilePayload{
		{ProfileUUID: globProf1.ProfileUUID, ProfileIdentifier: globProf1.Identifier, ProfileName: globProf1.Name, HostUUID: host1.UUID, HostPlatform: "darwin", Scope: mobius.PayloadScopeSystem},
		{ProfileUUID: globProf2.ProfileUUID, ProfileIdentifier: globProf2.Identifier, ProfileName: globProf2.Name, HostUUID: host1.UUID, HostPlatform: "darwin", Scope: mobius.PayloadScopeSystem},
		{ProfileUUID: profExcludeAny.ProfileUUID, ProfileIdentifier: profExcludeAny.Identifier, ProfileName: profExcludeAny.Name, HostUUID: host1.UUID, HostPlatform: "darwin", Scope: mobius.PayloadScopeSystem},

		{ProfileUUID: globProf1.ProfileUUID, ProfileIdentifier: globProf1.Identifier, ProfileName: globProf1.Name, HostUUID: hostLabel.UUID, HostPlatform: "darwin", Scope: mobius.PayloadScopeSystem},
		{ProfileUUID: globProf2.ProfileUUID, ProfileIdentifier: globProf2.Identifier, ProfileName: globProf2.Name, HostUUID: hostLabel.UUID, HostPlatform: "darwin", Scope: mobius.PayloadScopeSystem},
		{ProfileUUID: profIncludeAny.ProfileUUID, ProfileIdentifier: profIncludeAny.Identifier, ProfileName: profIncludeAny.Name, HostUUID: hostLabel.UUID, HostPlatform: "darwin", Scope: mobius.PayloadScopeSystem},
		{ProfileUUID: profIncludeAll.ProfileUUID, ProfileIdentifier: profIncludeAll.Identifier, ProfileName: profIncludeAll.Name, HostUUID: hostLabel.UUID, HostPlatform: "darwin", Scope: mobius.PayloadScopeSystem},
		{ProfileUUID: profExcludeAny.ProfileUUID, ProfileIdentifier: profExcludeAny.Identifier, ProfileName: profExcludeAny.Name, HostUUID: hostLabel.UUID, HostPlatform: "darwin", Scope: mobius.PayloadScopeSystem},
	}, profilesToInstall)
...
func testAggregateMacOSSettingsAllPlatforms(t *testing.T, ds *Datastore) {
	...
	upsertHostCPs(hosts, []*mobius.MDMAppleConfigProfile{cp}, mobius.MDMOperationTypeInstall, nil, ctx, ds, t)
	res, err := ds.GetMDMAppleProfilesSummary(ctx, nil)
	require.NoError(t, err)
	require.NotNil(t, res)
	require.EqualValues(t, len(hosts), res.Pending)
...
func testGetMDMAppleEnrolledDeviceDeletedFromMobius(t *testing.T, ds *Datastore) {
	ctx := t.Context()
	...
	for _, h := range hosts {
		_, err := ds.GetMDMAppleEnrolledDeviceDeletedFromMobius(ctx, h.UUID)
		require.Error(t, err)
		require.True(t, errors.Is(err, sql.ErrNoRows))
	}
	ids, err := ds.ListMDMAppleEnrolledIPhoneIpadDeletedFromMobius(ctx, 10)
	require.NoError(t, err)
	require.Len(t, ids, 0)
	...
	info, err := ds.GetMDMAppleEnrolledDeviceDeletedFromMobius(ctx, hosts[0].UUID)
	require.NoError(t, err)
	...
	info, err = ds.GetMDMAppleEnrolledDeviceDeletedFromMobius(ctx, hosts[1].UUID)
	require.NoError(t, err)
	...
	for _, huuid := range []string{hosts[2].UUID, hosts[3].UUID, uuid.NewString()} {
		_, err := ds.GetMDMAppleEnrolledDeviceDeletedFromMobius(ctx, huuid)
		require.Error(t, err)
		require.True(t, errors.Is(err, sql.ErrNoRows))
	}
...
func testSetMDMAppleProfilesWithVariables(t *testing.T, ds *Datastore) {
		...
		ExecAdhocSQL(t, ds, func(q sqlx.ExtContext) error {
			return sqlx.SelectContext(ctx, q, &gotVars, `
				...
				FROM
					...
					INNER JOIN mobius_variables fv ON mcpv.mobius_variable_id = fv.id
				WHERE
					macp.identifier = ? AND
					macp.team_id = ?`, profIdent, teamID)
		})
		for i := range wantVars {
			wantVars[i] = "MOBIUS_VAR_" + wantVars[i]
		}
	...
	require.NoError(t, err)
	_, err = ds.NewMDMAppleConfigProfile(ctx, profB, []string{mobius.MobiusVarHostEndUserIDPUsername})
	require.NoError(t, err)
	_, err = ds.NewMDMAppleConfigProfile(ctx, profC, []string{mobius.MobiusVarHostEndUserIDPGroups, mobius.MobiusVarCustomSCEPChallengePrefix + "ZZZ"})
	require.NoError(t, err)

	checkProfileVariables(profA.Identifier, 0, []string{})
	checkProfileVariables(profB.Identifier, 0, []string{mobius.MobiusVarHostEndUserIDPUsername})
	checkProfileVariables(profC.Identifier, tm1.ID, []string{mobius.MobiusVarHostEndUserIDPGroups, mobius.MobiusVarCustomSCEPChallengePrefix})
	...
	updates, err := ds.BatchSetMDMProfiles(ctx, nil, []*mobius.MDMAppleConfigProfile{
		&profA,
		&profB,
	}, nil, nil, []mobius.MDMProfileIdentifierMobiusVariables{
		{Identifier: profA.Identifier, MobiusVariables: []string{mobius.MobiusVarHostEndUserIDPUsernameLocalPart}},
		{Identifier: profB.Identifier, MobiusVariables: []string{mobius.MobiusVarHostEndUserIDPUsername}},
	})
	require.NoError(t, err)
	require.Equal(t, mobius.MDMProfilesUpdates{AppleConfigProfile: true}, updates)

	checkProfileVariables(profA.Identifier, 0, []string{mobius.MobiusVarHostEndUserIDPUsernameLocalPart})
	checkProfileVariables(profB.Identifier, 0, []string{mobius.MobiusVarHostEndUserIDPUsername})
	...
	updates, err = ds.BatchSetMDMProfiles(ctx, nil, []*mobius.MDMAppleConfigProfile{
		...
		{Identifier: profA.Identifier, MobiusVariables: nil},
		{Identifier: profB.Identifier, MobiusVariables: []string{mobius.MobiusVarHostEndUserIDPGroups}},
		{Identifier: profD.Identifier, MobiusVariables: []string{mobius.MobiusVarHostEndUserIDPUsername, mobius.MobiusVarDigiCertDataPrefix + "ZZZ"}},
	})
	require.NoError(t, err)
	require.Equal(t, mobius.MDMProfilesUpdates{AppleConfigProfile: true}, updates)

	checkProfileVariables(profA.Identifier, 0, nil)
	checkProfileVariables(profB.Identifier, 0, []string{mobius.MobiusVarHostEndUserIDPGroups})
	checkProfileVariables(profD.Identifier, 0, []string{mobius.MobiusVarHostEndUserIDPUsername, mobius.MobiusVarDigiCertDataPrefix})
	...
	updates, err = ds.BatchSetMDMProfiles(ctx, nil,
		[]*mobius.MDMAppleConfigProfile{
			&profA,
			&profB,
			&profD,
		},
		[]*mobius.MDMWindowsConfigProfile{
			windowsConfigProfileForTest(t, "W1", "W1"),
		},
		[]*mobius.MDMAppleDeclaration{
			declForTest("D1", "D1", "foo"),
		},
		[]mobius.MDMProfileIdentifierMobiusVariables{
			{Identifier: profA.Identifier, MobiusVariables: nil},
			{Identifier: profB.Identifier, MobiusVariables: []string{mobius.MobiusVarHostEndUserIDPGroups}},
			{Identifier: profD.Identifier, MobiusVariables: []string{mobius.MobiusVarHostEndUserIDPUsername, mobius.MobiusVarDigiCertDataPrefix + "ZZZ"}},
		})
	require.NoError(t, err)
	require.Equal(t, mobius.MDMProfilesUpdates{AppleDeclaration: true, WindowsConfigProfile: true}, updates)
	...
	updates, err = ds.BatchSetMDMProfiles(ctx, &tm1.ID, []*mobius.MDMAppleConfigProfile{
		&profE,
	}, nil, nil, []mobius.MDMProfileIdentifierMobiusVariables{
		{Identifier: profE.Identifier, MobiusVariables: []string{mobius.MobiusVarHostEndUserIDPGroups, mobius.MobiusVarDigiCertDataPrefix + "ZZZ"}},
	})
	require.NoError(t, err)
	require.Equal(t, mobius.MDMProfilesUpdates{AppleConfigProfile: true}, updates)

	checkProfileVariables(profC.Identifier, tm1.ID, nil)
	checkProfileVariables(profE.Identifier, tm1.ID, []string{mobius.MobiusVarHostEndUserIDPGroups, mobius.MobiusVarDigiCertDataPrefix})
