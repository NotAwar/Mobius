...
func testGetAllCAConfigAssetsByType(t *testing.T, ds *Datastore) {
	...
	_, err := ds.GetAllCAConfigAssetsByType(ctx, mobius.CAConfigDigiCert)
	assert.Error(t, err)
	assert.True(t, mobius.IsNotFound(err))

	_, err = ds.GetAllCAConfigAssetsByType(ctx, mobius.CAConfigCustomSCEPProxy)
	assert.Error(t, err)
	assert.True(t, mobius.IsNotFound(err))

	// Insert some test assets
	testAssets := []mobius.CAConfigAsset{
		{Name: "asset1", Type: mobius.CAConfigDigiCert, Value: []byte("value1")},
		{Name: "asset2", Type: mobius.CAConfigCustomSCEPProxy, Value: []byte("value2")},
		{Name: "asset3", Type: mobius.CAConfigDigiCert, Value: []byte("value3")},
	}
	err = ds.SaveCAConfigAssets(ctx, testAssets)
	require.NoError(t, err)

	// Test retrieving assets by type
	digiCertAssets, err := ds.GetAllCAConfigAssetsByType(ctx, mobius.CAConfigDigiCert)
	require.NoError(t, err)
	...
	scepProxyAssets, err := ds.GetAllCAConfigAssetsByType(ctx, mobius.CAConfigCustomSCEPProxy)
	require.NoError(t, err)
...
func addAndVerifyTestAssets(t *testing.T, ds *Datastore, ctx context.Context, assets []mobius.CAConfigAsset) {
	err := ds.SaveCAConfigAssets(ctx, assets)
	require.NoError(t, err)

	// Group assets by type
	digiCertAssets := make([]mobius.CAConfigAsset, 0)
	scepProxyAssets := make([]mobius.CAConfigAsset, 0)
	...
	if len(digiCertAssets) > 0 {
		retrievedAssets, err := ds.GetAllCAConfigAssetsByType(ctx, mobius.CAConfigDigiCert)
		require.NoError(t, err)
	...
	if len(scepProxyAssets) > 0 {
		retrievedAssets, err := ds.GetAllCAConfigAssetsByType(ctx, mobius.CAConfigCustomSCEPProxy)
		require.NoError(t, err)
...
func testSaveCAConfigAssets(t *testing.T, ds *Datastore) {
	...
	err := ds.SaveCAConfigAssets(ctx, []mobius.CAConfigAsset{})
	...
	updatedAssets := []mobius.CAConfigAsset{
		{Name: "asset1", Type: mobius.CAConfigCustomSCEPProxy, Value: []byte("value1-updated")},
		{Name: "asset3", Type: mobius.CAConfigDigiCert, Value: []byte("value3")},
	}
	...
	assert.Equal(t, mobius.CAConfigDigiCert, digiCertAssets["asset3"].Type)
	...
	assert.Equal(t, mobius.CAConfigCustomSCEPProxy, scepProxyAssets["asset1"].Type)
	assert.Equal(t, []byte("value1-updated"), scepProxyAssets["asset1"].Value)

	// Check the unchanged asset
	assert.Equal(t, mobius.CAConfigCustomSCEPProxy, scepProxyAssets["asset2"].Type)
...
func testDeleteCAConfigAssets(t *testing.T, ds *Datastore) {
	...
	_, err = ds.GetAllCAConfigAssetsByType(ctx, mobius.CAConfigDigiCert)
	assert.Error(t, err)
	assert.True(t, mobius.IsNotFound(err), "Expected NotFound error for DigiCert assets, got: %v", err)

	_, err = ds.GetAllCAConfigAssetsByType(ctx, mobius.CAConfigCustomSCEPProxy)
	assert.Error(t, err)
	assert.True(t, mobius.IsNotFound(err), "Expected NotFound error for SCEP Proxy assets, got: %v", err)
...
func testGetCAConfigAsset(t *testing.T, ds *Datastore) {
	...
	asset, err := ds.GetCAConfigAsset(ctx, "non-existent-asset", mobius.CAConfigDigiCert)
	assert.Error(t, err)
	assert.Nil(t, asset)
	assert.True(t, mobius.IsNotFound(err))
	...
	asset, err = ds.GetCAConfigAsset(ctx, "asset1", mobius.CAConfigDigiCert)
	require.NoError(t, err)
	require.NotNil(t, asset)
	assert.Equal(t, "asset1", asset.Name)
	assert.Equal(t, mobius.CAConfigDigiCert, asset.Type)
	assert.Equal(t, []byte("value1"), asset.Value)

	// Test retrieving another existing asset
	asset, err = ds.GetCAConfigAsset(ctx, "asset2", mobius.CAConfigCustomSCEPProxy)
	require.NoError(t, err)
	require.NotNil(t, asset)
	assert.Equal(t, "asset2", asset.Name)
	assert.Equal(t, mobius.CAConfigCustomSCEPProxy, asset.Type)
	assert.Equal(t, []byte("value2"), asset.Value)

	// Test retrieving an asset with a matching name but different type
	asset, err = ds.GetCAConfigAsset(ctx, "asset1", mobius.CAConfigCustomSCEPProxy)
	assert.Error(t, err)
	assert.Nil(t, asset)
	assert.True(t, mobius.IsNotFound(err))
}
