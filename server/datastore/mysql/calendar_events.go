...
func getCalendarEventByID(ctx context.Context, q sqlx.QueryerContext, id uint) (*mobius.CalendarEvent, error) {
	const calendarEventsQuery = "SELECT " + calendarEventCols + " FROM calendar_events ce WHERE id = ?"
	var calendarEvent mobius.CalendarEvent
...
func (ds *Datastore) GetCalendarEvent(ctx context.Context, email string) (*mobius.CalendarEvent, error) {
...
func (ds *Datastore) GetCalendarEventDetailsByUUID(ctx context.Context, uuidStr string) (*mobius.CalendarEventDetails, error) {
	UUID, err := uuid.Parse(uuidStr)
	if err != nil {
		return nil, ctxerr.Wrap(ctx, err, "invalid uuid")
	}
	const calendarEventsByUUIDQuery = `
		...
		WHERE ce.uuid_bin = ?;
	`
	var calendarEvent mobius.CalendarEventDetails
...
func (ds *Datastore) GetHostCalendarEvent(ctx context.Context, hostID uint) (*mobius.HostCalendarEvent, *mobius.CalendarEvent, error) {
	const hostCalendarEventsQuery = `
		SELECT * FROM host_calendar_events WHERE host_id = ?
	`
	var hostCalendarEvent mobius.HostCalendarEvent
...
func (ds *Datastore) GetHostCalendarEventByEmail(ctx context.Context, email string) (*mobius.HostCalendarEvent, *mobius.CalendarEvent, error) {
	const calendarEventsQuery = `
...
func (ds *Datastore) UpdateHostCalendarWebhookStatus(ctx context.Context, hostID uint, status mobius.CalendarWebhookStatus) error {
	const calendarEventsQuery = `
...
func (ds *Datastore) ListCalendarEvents(ctx context.Context, teamID *uint) ([]*mobius.CalendarEvent, error) {
	calendarEventsQuery := `
	...
	var calendarEvents []*mobius.CalendarEvent
...
func (ds *Datastore) ListOutOfDateCalendarEvents(ctx context.Context, t time.Time) ([]*mobius.CalendarEvent, error) {
	calendarEventsQuery := `
