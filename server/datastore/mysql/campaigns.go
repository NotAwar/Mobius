...
func (ds *Datastore) DistributedQueryCampaign(ctx context.Context, id uint) (*mobius.DistributedQueryCampaign, error) {
	sql := `
...
func (ds *Datastore) SaveDistributedQueryCampaign(ctx context.Context, camp *mobius.DistributedQueryCampaign) error {
	sqlStatement := `
...
func (ds *Datastore) DistributedQueryCampaignsForQuery(ctx context.Context, queryID uint) ([]*mobius.DistributedQueryCampaign, error) {
	var campaigns []*mobius.DistributedQueryCampaign
...
func (ds *Datastore) DistributedQueryCampaignTargetIDs(ctx context.Context, id uint) (*mobius.HostTargets, error) {
	sqlStatement := `
		SELECT * FROM distributed_query_campaign_targets WHERE distributed_query_campaign_id = ?
	`
	targets := []mobius.DistributedQueryCampaignTarget{}
	...
	return &mobius.HostTargets{HostIDs: hostIDs, LabelIDs: labelIDs, TeamIDs: teamIDs}, nil
}

func (ds *Datastore) NewDistributedQueryCampaignTarget(ctx context.Context, target *mobius.DistributedQueryCampaignTarget) (*mobius.DistributedQueryCampaignTarget, error) {
	sqlStatement := `
...
func (ds *Datastore) CleanupDistributedQueryCampaigns(ctx context.Context, now time.Time) (expired uint, err error) {
	...
	`
	result, err := ds.writer(ctx).ExecContext(ctx, sqlStatement, mobius.QueryComplete,
		mobius.QueryWaiting, now.Add(-1*time.Minute),
		mobius.QueryRunning, now.Add(-24*time.Hour))
	if err != nil {
