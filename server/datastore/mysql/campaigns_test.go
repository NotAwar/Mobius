...
func testCampaignsDistributedQuery(t *testing.T, ds *Datastore) {
	...
	campaign := test.NewCampaign(t, ds, query.ID, mobius.QueryRunning, mockClock.Now())
	...
	checkTargets(t, ds, campaign.ID, mobius.HostTargets{})

	test.AddHostToCampaign(t, ds, campaign.ID, h1.ID)
	checkTargets(t, ds, campaign.ID, mobius.HostTargets{HostIDs: []uint{h1.ID}})

	test.AddLabelToCampaign(t, ds, campaign.ID, l1.ID)
	checkTargets(t, ds, campaign.ID, mobius.HostTargets{HostIDs: []uint{h1.ID}, LabelIDs: []uint{l1.ID}})

	test.AddLabelToCampaign(t, ds, campaign.ID, l2.ID)
	checkTargets(t, ds, campaign.ID, mobius.HostTargets{HostIDs: []uint{h1.ID}, LabelIDs: []uint{l1.ID, l2.ID}})

	test.AddHostToCampaign(t, ds, campaign.ID, h2.ID)
	test.AddHostToCampaign(t, ds, campaign.ID, h3.ID)

	checkTargets(t, ds, campaign.ID, mobius.HostTargets{HostIDs: []uint{h1.ID, h2.ID, h3.ID}, LabelIDs: []uint{l1.ID, l2.ID}})
}
...
func testCampaignsCleanupDistributedQuery(t *testing.T, ds *Datastore) {
	...
	c1 := test.NewCampaign(t, ds, query.ID, mobius.QueryWaiting, mockClock.Now())
	c2 := test.NewCampaign(t, ds, query.ID, mobius.QueryRunning, mockClock.Now())
	...
	assert.Equal(t, uint(1), expired)
	{
		...
		require.Nil(t, err)
		assert.Equal(t, c1.QueryID, retrieved.QueryID)
		assert.Equal(t, mobius.QueryComplete, retrieved.Status)
	}
	{
		retrieved, err := ds.DistributedQueryCampaign(ctx, c2.ID)
		require.Nil(t, err)
	...
	c3 := test.NewCampaign(t, ds, query.ID, mobius.QueryWaiting, mockClock.Now().AddDate(0, 0, -8))
	{
		retrieved, err := ds.DistributedQueryCampaign(ctx, c3.ID)
		require.Nil(t, err)
		assert.Equal(t, c3.QueryID, retrieved.QueryID)
		assert.Equal(t, mobius.QueryWaiting, retrieved.Status)
	}
...
func testCampaignsSaveDistributedQuery(t *testing.T, ds *Datastore) {
	user := test.NewUser(t, ds, t.Name(), t.Name()+"zwass@mobius.co", true)
	...
	require.Equal(t, mobius.QueryWaiting, gotC.Status)

	c1.Status = mobius.QueryComplete
	require.NoError(t, ds.SaveDistributedQueryCampaign(context.Background(), c1))

	gotC, err = ds.DistributedQueryCampaign(context.Background(), c1.ID)
	require.NoError(t, err)
	require.Equal(t, mobius.QueryComplete, gotC.Status)
}

func checkTargets(t *testing.T, ds mobius.Datastore, campaignID uint, expectedTargets mobius.HostTargets) {
