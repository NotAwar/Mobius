...
func (ds *Datastore) NewCarve(ctx context.Context, metadata *mobius.CarveMetadata) (*mobius.CarveMetadata, error) {
...
func (ds *Datastore) UpdateCarve(ctx context.Context, metadata *mobius.CarveMetadata) error {
	return updateCarveDB(ctx, ds.writer(ctx), metadata)
}

func updateCarveDB(ctx context.Context, exec sqlx.ExecerContext, metadata *mobius.CarveMetadata) error {
	stmt := `
...
func (ds *Datastore) Carve(ctx context.Context, carveId int64) (*mobius.CarveMetadata, error) {
	stmt := fmt.Sprintf(`
		SELECT %s
		FROM carve_metadata
		WHERE id = ?`,
	...
	var metadata mobius.CarveMetadata
...
func (ds *Datastore) CarveBySessionId(ctx context.Context, sessionId string) (*mobius.CarveMetadata, error) {
	stmt := fmt.Sprintf(`
		SELECT %s
		FROM carve_metadata
		WHERE session_id = ?`,
...
func (ds *Datastore) CarveByName(ctx context.Context, name string) (*mobius.CarveMetadata, error) {
	stmt := fmt.Sprintf(`
		SELECT %s
		FROM carve_metadata
		WHERE name = ?`,
...
func (ds *Datastore) ListCarves(ctx context.Context, opt mobius.CarveListOptions) ([]*mobius.CarveMetadata, error) {
	stmt := fmt.Sprintf(`
		SELECT %s
		FROM carve_metadata`,
		carveSelectFields,
...
func (ds *Datastore) NewBlock(ctx context.Context, metadata *mobius.CarveMetadata, blockId int64, data []byte) error {
...
func (ds *Datastore) GetBlock(ctx context.Context, metadata *mobius.CarveMetadata, blockId int64) ([]byte, error) {
	stmt := `
		SELECT data
		FROM carve_blocks
