...
func testCarvesMetadata(t *testing.T, ds *Datastore) {
	...
	expectedCarve := &mobius.CarveMetadata{
		HostId:     h.ID,
		Name:       "foobar",
		BlockCount: 10,
		BlockSize:  12,
...
func testCarvesList(t *testing.T, ds *Datastore) {
	...
	expectedCarve2 := &mobius.CarveMetadata{
		HostId:     h.ID,
		Name:       "foobar2",
		BlockCount: 42,
		BlockSize:  13,
	...
	carves, err := ds.ListCarves(context.Background(), mobius.CarveListOptions{Expired: true})
	require.NoError(t, err)
	assert.Equal(t, []*mobius.CarveMetadata{expectedCarve, expectedCarve2}, carves)

	// Expire the carves
	_, err = ds.CleanupCarves(context.Background(), time.Now().Add(24*time.Hour))
	require.NoError(t, err)

	carves, err = ds.ListCarves(context.Background(), mobius.CarveListOptions{Expired: false})
	require.NoError(t, err)
