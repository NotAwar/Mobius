...
func (ds *Datastore) ConsumeChallenge(ctx context.Context, challenge string) error {
	...
	err := ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		...
		// check expiry
		if time.Since(createdAt) <= mobius.OneTimeChallengeTTL {
			valid = true
		} else {
...
func (ds *Datastore) CleanupExpiredChallenges(ctx context.Context) (int64, error) {
	res, err := ds.writer(ctx).ExecContext(ctx, `DELETE FROM challenges WHERE created_at < ?`, time.Now().Add(-mobius.OneTimeChallengeTTL))
	if err != nil {
