...
func (ds *Datastore) ConditionalAccessMicrosoftGet(ctx context.Context) (*mobius.ConditionalAccessMicrosoftIntegration, error) {
	return getConditionalAccessMicrosoft(ctx, ds.reader(ctx))
}

func getConditionalAccessMicrosoft(ctx context.Context, q sqlx.QueryerContext) (*mobius.ConditionalAccessMicrosoftIntegration, error) {
	var integration mobius.ConditionalAccessMicrosoftIntegration
	err := sqlx.GetContext(
		ctx, q, &integration,
		// Currently only one global integration is supported.
...
func (ds *Datastore) LoadHostConditionalAccessStatus(ctx context.Context, hostID uint) (*mobius.HostConditionalAccessStatus, error) {
	var hostConditionalAccessStatus mobius.HostConditionalAccessStatus
