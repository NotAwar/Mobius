...
func (ds *Datastore) UpdateHostCertificates(ctx context.Context, hostID uint, hostUUID string, certs []*mobius.HostCertificateRecord) error {
	type certSourceToSet struct {
		Source   mobius.HostCertificateSource
		Username string
	}

	incomingBySHA1 := make(map[string]*mobius.HostCertificateRecord, len(certs))
	...
	existingCerts, _, err := listHostCertsDB(ctx, ds.reader(ctx), hostID, mobius.ListOptions{}) // requesting unpaginated results with default limit of 1 million
	if err != nil {
		return ctxerr.Wrap(ctx, err, "list host certificates for update")
	}

	existingBySHA1 := make(map[string]*mobius.HostCertificateRecord, len(existingCerts))
	...
	toInsert := make([]*mobius.HostCertificateRecord, 0, len(incomingBySHA1))
	...
	// Check if any of the certs to insert are managed by Mobius; if so, update the associated host_mdm_managed_certificates rows
	hostMDMManagedCertsToUpdate := make([]*mobius.MDMManagedCertificate, 0, len(toInsert))
	if len(toInsert) > 0 {
		...
		for _, hostMDMManagedCert := range hostMDMManagedCerts {
			...
			// by Mobius and stored in the DB directly, so we need not fetch them via osquery/MDM
			if hostMDMManagedCert.Type != mobius.CAConfigCustomSCEPProxy && hostMDMManagedCert.Type != mobius.CAConfigNDES {
				continue
			}
			for _, certToInsert := range toInsert {
				if strings.Contains(certToInsert.SubjectCommonName, "mobius-"+hostMDMManagedCert.ProfileUUID) {
					managedCertToUpdate := &mobius.MDMManagedCertificate{
	...
	return ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		...
		if len(toSetSourcesBySHA1) > 0 {
			...
			toReplaceSources := make([]*mobius.HostCertificateRecord, 0, len(toSetSourcesBySHA1))
			for sha1, sources := range toSetSourcesBySHA1 {
				for _, source := range sources {
					toReplaceSources = append(toReplaceSources, &mobius.HostCertificateRecord{
...
func loadHostCertIDsForSHA1DB(ctx context.Context, tx sqlx.QueryerContext, sha1s []string) (map[string]uint, error) {
	...
	var certs []*mobius.HostCertificateRecord
...
func listHostCertsDB(ctx context.Context, tx sqlx.QueryerContext, hostID uint, opts mobius.ListOptions) ([]*mobius.HostCertificateRecord, *mobius.PaginationMetadata, error) {
	stmt := `
SELECT
	hc.id,
	hc.sha1_sum,
...
WHERE
	...
	if opts.IncludeMetadata {
		metaData = &mobius.PaginationMetadata{HasPreviousResults: opts.Page > 0}
...
func replaceHostCertsSourcesDB(ctx context.Context, tx sqlx.ExtContext, toReplaceSources []*mobius.HostCertificateRecord) error {
...
func insertHostCertsDB(ctx context.Context, tx sqlx.ExtContext, certs []*mobius.HostCertificateRecord) error {
	if len(certs) == 0 {
		return nil
	}
...
func updateHostMDMManagedCertDetailsDB(ctx context.Context, tx sqlx.ExtContext, certs []*mobius.MDMManagedCertificate) error {
	if len(certs) == 0 {
		return nil
	}
