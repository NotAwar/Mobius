...
func testUpdateAndListHostCertificates(t *testing.T, ds *Datastore) {
	...
	payload := []*mobius.HostCertificateRecord{
	...
	certs, _, err := ds.ListHostCertificates(ctx, 1, mobius.ListOptions{OrderKey: "common_name"})
	...
	require.Equal(t, mobius.SystemHostCertificate, certs[0].Source)
	...
	require.Equal(t, mobius.SystemHostCertificate, certs[1].Source)

	// order by not_valid_after descending
	certs, _, err = ds.ListHostCertificates(ctx, 1, mobius.ListOptions{OrderKey: "not_valid_after", OrderDirection: mobius.OrderAscending})
	require.NoError(t, err)
	require.Len(t, certs, 2)
	...
	require.NoError(t, ds.UpdateHostCertificates(ctx, 1, "95816502-d8c0-462c-882f-39991cc89a0c", []*mobius.HostCertificateRecord{payload[1]}))
	certs, _, err = ds.ListHostCertificates(ctx, 1, mobius.ListOptions{OrderKey: "common_name"})
	require.NoError(t, err)
	...
	payload[0].Source = mobius.UserHostCertificate
	payload[0].Username = "A"
	require.NoError(t, ds.UpdateHostCertificates(ctx, 1, "95816502-d8c0-462c-882f-39991cc89a0c", []*mobius.HostCertificateRecord{payload[0], payload[1]}))
	certs, _, err = ds.ListHostCertificates(ctx, 1, mobius.ListOptions{OrderKey: "common_name"})
	require.NoError(t, err)
	require.Len(t, certs, 2)
	require.Equal(t, expected2.Subject.CommonName, certs[0].CommonName)
	...
	require.Equal(t, expected1.Subject.CommonName, certs[1].CommonName)
	require.Equal(t, expected1.Subject.CommonName, certs[1].SubjectCommonName)
	require.Equal(t, mobius.UserHostCertificate, certs[1].Source)
	...
	hostCert3SrcUserB.Source = mobius.UserHostCertificate
	hostCert3SrcUserB.Username = "B"
	cloneC := *hostCert3SrcUserB // copy to create a new record
	hostCert3SrcUserC := &cloneC
	hostCert3SrcUserC.Source = mobius.UserHostCertificate
	hostCert3SrcUserC.Username = "C"
	cloneD := *hostCert3SrcUserB // copy to create a new record
	hostCert3SrcUserD := &cloneD
	hostCert3SrcUserD.Source = mobius.UserHostCertificate
	hostCert3SrcUserD.Username = "D"
	cases := []struct {
		desc   string
		ingest []*mobius.HostCertificateRecord
	}{
		{desc: "nil slice", ingest: nil},
		{desc: "cert 1 and 2", ingest: []*mobius.HostCertificateRecord{hostCert2SrcSys, hostCert1SrcUserA}},
		{desc: "cert 2 and 3 (B, C)", ingest: []*mobius.HostCertificateRecord{hostCert2SrcSys, hostCert3SrcUserB, hostCert3SrcUserC}},
		{desc: "no change", ingest: []*mobius.HostCertificateRecord{hostCert2SrcSys, hostCert3SrcUserB, hostCert3SrcUserC}},
		{desc: "added cert 3 source (D)", ingest: []*mobius.HostCertificateRecord{hostCert2SrcSys, hostCert3SrcUserB, hostCert3SrcUserC, hostCert3SrcUserD}},
		{desc: "removed cert3 source (B)", ingest: []*mobius.HostCertificateRecord{hostCert2SrcSys, hostCert3SrcUserC, hostCert3SrcUserD}},
		{desc: "removed cert3 source (C)", ingest: []*mobius.HostCertificateRecord{hostCert2SrcSys, hostCert3SrcUserB, hostCert3SrcUserD}},
		{desc: "cleared, added cert 1", ingest: []*mobius.HostCertificateRecord{hostCert1SrcUserA}},
		{desc: "all cleared", ingest: []*mobius.HostCertificateRecord{}},
	}
	for _, c := range cases {
		t.Log(c.desc)
		...
		require.NoError(t, err)
		certs, _, err := ds.ListHostCertificates(ctx, 1, mobius.ListOptions{OrderKey: "common_name", TestSecondaryOrderKey: "username"})
		require.NoError(t, err)
...
func testUpdatingHostMDMManagedCertificates(t *testing.T, ds *Datastore) {
	...
	expected1 := x509.Certificate{
		Subject: pkix.Name{
			Country:      []string{"US"},
			CommonName:   "MYHWSERIAL mobius-" + initialCP.ProfileUUID,
	...
	// verify that we saved the records correctly
	certs, _, err := ds.ListHostCertificates(context.Background(), 1, mobius.ListOptions{OrderKey: "common_name"})
	require.NoError(t, err)
	require.Len(t, certs, 2)
	require.Equal(t, expected1.Subject.CommonName, certs[0].CommonName)
	require.Equal(t, expected1.Subject.CommonName, certs[0].SubjectCommonName)
	...
	assert.Equal(t, mobius.CAConfigCustomSCEPProxy, profile.Type)
	...
	// simulate removal of a certificate
	require.NoError(t, ds.UpdateHostCertificates(context.Background(), 1, "95816502-d8c0-462c-882f-39991cc89a0c", []*mobius.HostCertificateRecord{payload[1]}))
	certs3, _, err := ds.ListHostCertificates(context.Background(), 1, mobius.ListOptions{OrderKey: "common_name"})
	require.NoError(t, err)
...
func generateTestHostCertificateRecord(t *testing.T, hostID uint, template *x509.Certificate) *mobius.HostCertificateRecord {
	...
	return mobius.NewHostCertificateRecord(hostID, parsed)
}
