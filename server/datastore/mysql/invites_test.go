...
func testInvitesCreate(t *testing.T, ds *Datastore) {
	...
	invite := &mobius.Invite{
		...
		Teams: []mobius.UserTeam{
			...
			{Role: mobius.RoleMaintainer, Team: mobius.Team{ID: 3}},
		},
	}
	...
	_, err = ds.NewInvite(context.Background(), &mobius.Invite{
...
func setupTestInvites(t *testing.T, ds mobius.Datastore) {
	admin := &mobius.Invite{
		Email:      "admin@foo.com",
		Name:       "Xadmin",
		Token:      "admin",
...
func testInvitesUpdate(t *testing.T, ds *Datastore) {
	...
	sentinelInvite := &mobius.Invite{
		Email: "user@sentinel.com",
		Name:  "sentinel",
		Token: "some_sentinel",
		Teams: []mobius.UserTeam{
			{Role: mobius.RoleAdmin, Team: mobius.Team{ID: 1}},
		},
	}
	...
	invite.GlobalRole = null.StringFrom(mobius.RoleAdmin)
	invite.Teams = nil
	...
	require.NoError(t, err)
	assert.Equal(t, invite.ID, verify.ID)
	assert.Equal(t, null.StringFrom(mobius.RoleAdmin), verify.GlobalRole)
	...
	assert.Equal(t, mobius.RoleAdmin, verify.Teams[0].Role)
}
