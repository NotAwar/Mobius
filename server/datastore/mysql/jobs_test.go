...
func testQueueAndProcessJobs(t *testing.T, ds *Datastore) {
	...
	j1 := &mobius.Job{Name: "j1", State: mobius.JobStateQueued}
	j2 := &mobius.Job{Name: "j2", State: mobius.JobStateQueued, NotBefore: time.Now().Add(time.Hour)}
	...
	j1.State = mobius.JobStateSuccess
...
func testCleanupWorkerJobs(t *testing.T, ds *Datastore) {
	...
	setJobTimestamp := func(j *mobius.Job, subtract time.Duration) {
		j.NotBefore = time.Now().UTC().Add(-subtract)
		_, err = ds.UpdateJob(ctx, j.ID, j)
		require.NoError(t, err)
	}
	setJobStatus := func(j *mobius.Job, state mobius.JobState) {
		j.State = state
		_, err = ds.UpdateJob(ctx, j.ID, j)
		require.NoError(t, err)
	}
	...
	// mark it as done
	setJobStatus(j1, mobius.JobStateSuccess)
	...
	queuedJobs := make([]*mobius.Job, 0, 4)
	for i := 0; i < 4; i++ {
		j := &mobius.Job{Name: "j" + fmt.Sprint(i+1), State: mobius.JobStateQueued}
	...
	setJobStatus(queuedJobs[1], mobius.JobStateFailure) // failed 2m ago
	setJobStatus(queuedJobs[3], mobius.JobStateFailure) // failed 4m ago
	setJobStatus(queuedJobs[2], mobius.JobStateSuccess) // successful 3m ago
