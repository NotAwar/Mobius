...
func (ds *Datastore) GetLinuxDiskEncryptionSummary(ctx context.Context, teamID *uint) (mobius.MDMLinuxDiskEncryptionSummary, error) {
	...
	var counts []countRow
	summary := mobius.MDMLinuxDiskEncryptionSummary{}
...
func sqlCaseLinuxOSSettingsStatus() string {
	...
	CASE WHEN 
		...
		AND hdek.base64_encrypted != ''
		AND hdek.client_error = '' THEN
		'` + string(mobius.OSSettingsVerified) + `'
	WHEN hdek.client_error IS NOT NULL
	AND hdek.client_error != '' THEN
		'` + string(mobius.OSSettingsFailed) + `'
	WHEN hdek.base64_encrypted IS NULL
		OR (hdek.base64_encrypted = ''
		AND hdek.client_error = '') THEN
		'` + string(mobius.OSSettingsPending) + `'
	END`
}
...
func sqlCaseLinuxDiskEncryptionStatus() string {
	...
	CASE WHEN 
		...
		AND hdek.base64_encrypted != ''
		AND hdek.client_error = '' THEN
		'` + string(mobius.DiskEncryptionVerified) + `'
	WHEN hdek.client_error IS NOT NULL
	AND hdek.client_error != '' THEN
		'` + string(mobius.DiskEncryptionFailed) + `'
	WHEN hdek.base64_encrypted IS NULL
		OR (hdek.base64_encrypted = ''
		AND hdek.client_error = '') THEN
		'` + string(mobius.DiskEncryptionActionRequired) + `'
	END`
}
