...
func (ds *Datastore) DBLocks(ctx context.Context) ([]*mobius.DBLock, error) {
	...
	if err := ds.writer(ctx).SelectContext(ctx, &locks, stmt); err != nil {
		...
		//	GRANT PROCESS,SELECT ON *.* TO 'mobius'@'%';
		//	FLUSH PRIVILEGES;
		// Make sure to restart mobius after running the commands above.
