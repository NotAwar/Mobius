...
func testUpsertMaintainedApps(t *testing.T, ds *Datastore) {
	...
	listSavedApps := func() []mobius.MaintainedApp {
		var apps []mobius.MaintainedApp
		ExecAdhocSQL(t, ds, func(q sqlx.ExtContext) error {
			return sqlx.SelectContext(ctx, q, &apps, "SELECT name, platform, slug FROM mobius_maintained_apps ORDER BY slug")
		})
		return apps
	}

	expectedApps := maintained_apps.SyncApps(t, ds)
	var expectedAppsBaseInfo []mobius.MaintainedApp
	for _, app := range expectedApps {
		expectedAppsBaseInfo = append(expectedAppsBaseInfo, mobius.MaintainedApp{
			Name:     app.Name,
	...
	_, err := ds.UpsertMaintainedApp(ctx, &mobius.MaintainedApp{
		Name:     "Figma 2",
		Slug:     "figma/darwin",
		Platform: "darwin",
	})
...
func testSync(t *testing.T, ds *Datastore) {
	...
	ExecAdhocSQL(t, ds, func(q sqlx.ExtContext) error {
		return sqlx.SelectContext(context.Background(), q, &actualSlugs, "SELECT slug FROM mobius_maintained_apps ORDER BY slug")
	})
...
func testListAndGetAvailableApps(t *testing.T, ds *Datastore) {
	...
	_, _, err = ds.ListAvailableMobiusMaintainedApps(ctx, &team1.ID, mobius.ListOptions{IncludeMetadata: true})
	require.ErrorIs(t, err, &mobius.NoMaintainedAppsInDatabaseError{})

	maintained1, err := ds.UpsertMaintainedApp(ctx, &mobius.MaintainedApp{
		Name:             "Maintained1",
		Slug:             "maintained1",
		Platform:         "darwin",
	...
	require.NoError(t, err)
	maintained2, err := ds.UpsertMaintainedApp(ctx, &mobius.MaintainedApp{
		Name:             "Maintained2",
		Slug:             "maintained2",
		Platform:         "darwin",
		UniqueIdentifier: "mobius.maintained2",
	})
	require.NoError(t, err)
	maintained3, err := ds.UpsertMaintainedApp(ctx, &mobius.MaintainedApp{
		Name:             "Maintained3",
		Slug:             "maintained3",
		Platform:         "darwin",
		UniqueIdentifier: "mobius.maintained3",
	})
	require.NoError(t, err)
	maintained4, err := ds.UpsertMaintainedApp(ctx, &mobius.MaintainedApp{
		Name:             "Maintained4",
		Slug:             "maintained4",
		Platform:         "windows",
	...
	expectedApps := []mobius.MaintainedApp{
		{
			ID:       maintained1.ID,
	...
	// Testing pagination
	apps, meta, err := ds.ListAvailableMobiusMaintainedApps(ctx, &team1.ID, mobius.ListOptions{IncludeMetadata: true})
	require.NoError(t, err)
	...
	apps, meta, err = ds.ListAvailableMobiusMaintainedApps(ctx, &team1.ID, mobius.ListOptions{PerPage: 1, IncludeMetadata: true})
	require.NoError(t, err)
	...
	apps, meta, err = ds.ListAvailableMobiusMaintainedApps(ctx, &team1.ID, mobius.ListOptions{PerPage: 1, Page: 1, IncludeMetadata: true})
	require.NoError(t, err)
	require.Len(t, apps, 1)
	require.EqualValues(t, meta.TotalResults, 4)
	...
	apps, meta, err = ds.ListAvailableMobiusMaintainedApps(ctx, &team1.ID, mobius.ListOptions{PerPage: 1, Page: 2, IncludeMetadata: true})
	require.NoError(t, err)
	require.Len(t, apps, 1)
	require.EqualValues(t, meta.TotalResults, 4)
	...
	apps, meta, err = ds.ListAvailableMobiusMaintainedApps(ctx, &team1.ID, mobius.ListOptions{PerPage: 1, Page: 3, IncludeMetadata: true})
	require.NoError(t, err)
	require.Len(t, apps, 1)
	require.EqualValues(t, meta.TotalResults, 4)
	...
	// Testing search
	apps, meta, err = ds.ListAvailableMobiusMaintainedApps(ctx, &team1.ID, mobius.ListOptions{MatchQuery: "Maintained4", IncludeMetadata: true})
	require.NoError(t, err)
	require.Len(t, apps, 1)
	...
	apps, meta, err = ds.ListAvailableMobiusMaintainedApps(ctx, &team1.ID, mobius.ListOptions{MatchQuery: "Maintained5", IncludeMetadata: true})
	require.NoError(t, err)
	...
	_, _, err = ds.MatchOrCreateSoftwareInstaller(ctx, &mobius.UploadSoftwareInstallerPayload{
		...
		UserID:           user.ID,
		Platform:         string(mobius.MacOSPlatform),
		BundleIdentifier: "mobius.maintained1",
		ValidatedLabels:  &mobius.LabelIdentsWithScope{},
	})
	require.NoError(t, err)
	...
	_, titleID, err := ds.MatchOrCreateSoftwareInstaller(ctx, &mobius.UploadSoftwareInstallerPayload{
		Title:            "Maintained1",
		TeamID:           &team1.ID,
		InstallScript:    "nothing",
		Filename:         "foo.pkg",
	...
	ExecAdhocSQL(t, ds, func(q sqlx.ExtContext) error {
		_, err := q.ExecContext(ctx, "UPDATE software_installers SET platform = ? WHERE platform = ?", mobius.MacOSPlatform, mobius.IOSPlatform)
		return err
	})
	...
	// add Windows app
	_, windowsTitleID, err := ds.MatchOrCreateSoftwareInstaller(ctx, &mobius.UploadSoftwareInstallerPayload{
	...
	vppMaintained2 := &mobius.VPPApp{
		...
		BundleIdentifier: "mobius.maintained2",
	}
	...
	vppMaintained3 := &mobius.VPPApp{
		Name: "Maintained 3",
		VPPAppTeam: mobius.VPPAppTeam{
			VPPAppID: mobius.VPPAppID{
				AdamID:   "3",
				Platform: mobius.IOSPlatform,
			},
		},
		BundleIdentifier: "mobius.maintained3",
	}
	...
	apps, meta, err = ds.ListAvailableMobiusMaintainedApps(ctx, nil, mobius.ListOptions{IncludeMetadata: true})
	require.NoError(t, err)
	require.Len(t, apps, 4)
	require.EqualValues(t, meta.TotalResults, 4)
...
func testGetMaintainedAppBySlug(t *testing.T, ds *Datastore) {
	...
	require.NoError(t, err)
	_, titleId1, err := ds.MatchOrCreateSoftwareInstaller(ctx, &mobius.UploadSoftwareInstallerPayload{
	...
	// maintained app 2
	maintainedApp2, err := ds.UpsertMaintainedApp(ctx, &mobius.MaintainedApp{
		Name:             "Maintained2",
		Slug:             "maintained2",
		Platform:         "darwin",
		UniqueIdentifier: "mobius.maintained2",
	...
	require.NoError(t, err)
	require.Equal(t, &mobius.MaintainedApp{
		ID:               maintainedApp.ID,
		Name:             "Maintained1",
		Slug:             "maintained1",
		Platform:         "darwin",
