...
func addRequestTypeFilter(stmt string, filter *mobius.MDMCommandFilters, params []interface{}) (string, []interface{}) {
...
func (ds *Datastore) getMDMCommand(ctx context.Context, q sqlx.QueryerContext, cmdUUID string) (*mobius.MDMCommand, error) {
	stmt, _ := getCombinedMDMCommandsQuery(ds, "")
	stmt += "command_uuid = ?"

	var cmd mobius.MDMCommand
...
func (ds *Datastore) BatchSetMDMProfiles(ctx context.Context, tmID *uint, macProfiles []*mobius.MDMAppleConfigProfile,
	winProfiles []*mobius.MDMWindowsConfigProfile, macDeclarations []*mobius.MDMAppleDeclaration, profilesVariablesByIdentifier []mobius.MDMProfileIdentifierMobiusVariables,
	...
	err = ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		...
		// for now, only apple profiles support Mobius variables
...
func (ds *Datastore) ListMDMConfigProfiles(ctx context.Context, teamID *uint, opt mobius.ListOptions) ([]*mobius.MDMConfigProfilePayload, *mobius.PaginationMetadata, error) {
	// this lists custom profiles, it explicitly filters out the mobius-reserved
	// ones (reserved identifiers for Apple profiles, reserved names for Windows).

	var profs []*mobius.MDMConfigProfilePayload
...
`
	...
	mobiusIdentsMap := mobileconfig.MobiusPayloadIdentifiers()
	mobiusIdentifiers := make([]string, 0, len(mobiusIdentsMap))
	for k := range mobiusIdentsMap {
		mobiusIdentifiers = append(mobiusIdentifiers, k)
	}
	mobiusNamesMap := mdm.MobiusReservedProfileNames()
	mobiusNames := make([]string, 0, len(mobiusNamesMap))
	for k := range mobiusNamesMap {
		mobiusNames = append(mobiusNames, k)
	}

	args := []any{globalOrTeamID, mobiusIdentifiers, globalOrTeamID, mobiusNames, globalOrTeamID, mobiusNames}
	...
	if opt.IncludeMetadata {
		metaData = &mobius.PaginationMetadata{HasPreviousResults: opt.Page > 0}
	...
	for _, prof := range profs {
		if prof.Platform == "windows" {
			...
			if strings.HasPrefix(prof.ProfileUUID, mobius.MDMAppleDeclarationUUIDPrefix) {
	...
	profMap := make(map[string]*mobius.MDMConfigProfilePayload, len(profs))
	for _, prof := range profs {
...
func (ds *Datastore) listProfileLabelsForProfiles(ctx context.Context, winProfUUIDs, macProfUUIDs, macDeclUUIDs []string) ([]mobius.ConfigurationProfileLabel, error) {
	// load the labels associated with those profiles
	const labelsStmt = `
SELECT
...
`
	...
	var labels []mobius.ConfigurationProfileLabel
...
func (ds *Datastore) bulkSetPendingMDMHostProfilesDB(
	...
	if len(profileUUIDs) > 0 {
		...
		for _, puid := range profileUUIDs {
			if strings.HasPrefix(puid, mobius.MDMAppleProfileUUIDPrefix) { //nolint:gocritic // ignore ifElseChain
				macProfUUIDs = append(macProfUUIDs, puid)
			} else if strings.HasPrefix(puid, mobius.MDMAppleDeclarationUUIDPrefix) {
				hasAppleDecls = true
			} else {
...
func (ds *Datastore) UpdateHostMDMProfilesVerification(ctx context.Context, host *mobius.Host, toVerify, toFail, toRetry []string) error {
...
func setMDMProfilesRetryDB(ctx context.Context, tx sqlx.ExtContext, host *mobius.Host, identifiersOrNames []string) error {
...
func setMDMProfilesFailedDB(ctx context.Context, tx sqlx.ExtContext, host *mobius.Host, identifiersOrNames []string) error {
	if len(identifiersOrNames) == 0 {
		return nil
	}
...
WHERE
	...
	args := []interface{}{
		...
		mobius.HostMDMProfileDetailFailedWasVerifying,
		mobius.HostMDMProfileDetailFailedWasVerified,
...
func setMDMProfilesVerifiedDB(ctx context.Context, tx sqlx.ExtContext, host *mobius.Host, identifiersOrNames []string) error {
	if len(identifiersOrNames) == 0 {
		return nil
	}
...
func (ds *Datastore) GetHostMDMProfilesExpectedForVerification(ctx context.Context, host *mobius.Host) (map[string]*mobius.ExpectedMDMProfile, error) {
	var teamID uint
	if host.TeamID != nil {
		teamID = *host.TeamID
	}
...
func (ds *Datastore) getHostMDMWindowsProfilesExpectedForVerification(ctx context.Context, teamID, hostID uint) (map[string]*mobius.ExpectedMDMProfile, error) {
	stmt := `
...
HAVING
	...
	count_host_labels > 0
`
	var profiles []*mobius.ExpectedMDMProfile
	...
	byName := make(map[string]*mobius.ExpectedMDMProfile, len(profiles))
...
func (ds *Datastore) getHostMDMAppleProfilesExpectedForVerification(ctx context.Context, teamID uint, host *mobius.Host) (map[string]*mobius.ExpectedMDMProfile, error) {
...
`
	...
	var rows []*mobius.ExpectedMDMProfile
	...
	expectedProfilesByIdentifier := make(map[string]*mobius.ExpectedMDMProfile, len(rows))
	for _, r := range rows {
...
func (ds *Datastore) GetHostMDMProfilesRetryCounts(ctx context.Context, host *mobius.Host) ([]mobius.HostMDMProfileRetryCount, error) {
	const darwinStmt = `
SELECT
	profile_identifier,
	retries
...
WHERE
	...
	var dest []mobius.HostMDMProfileRetryCount
...
func (ds *Datastore) GetHostMDMProfileRetryCountByCommandUUID(ctx context.Context, host *mobius.Host, cmdUUID string) (mobius.HostMDMProfileRetryCount, error) {
	const darwinStmt = `
SELECT
...
WHERE
	...
	case "windows":
		stmt = windowsStmt
	default:
		return mobius.HostMDMProfileRetryCount{}, fmt.Errorf("unsupported platform %s", host.Platform)
	}
...
func batchSetProfileLabelAssociationsDB(
	...
	labelsToInsert := make(map[string]*mobius.ConfigurationProfileLabel, len(profileLabels))
...
func (ds *Datastore) MDMGetEULAMetadata(ctx context.Context) (*mobius.MDMEULA, error) {
...
func (ds *Datastore) MDMGetEULABytes(ctx context.Context, token string) (*mobius.MDMEULA, error) {
...
func (ds *Datastore) MDMInsertEULA(ctx context.Context, eula *mobius.MDMEULA) error {
...
func (ds *Datastore) GetHostCertAssociationsToExpire(ctx context.Context, expiryDays, limit int) ([]mobius.SCEPIdentityAssociation, error) {
...
SELECT
    ...
    COALESCE(MAX(hm.mobius_enroll_ref), '') AS enroll_reference,
...
LIMIT ?`, expiryDays, limit)
	...
	var uuids []mobius.SCEPIdentityAssociation
...
func (ds *Datastore) SetCommandForPendingSCEPRenewal(ctx context.Context, assocs []mobius.SCEPIdentityAssociation, cmdUUID string) error {
	if len(assocs) == 0 {
		return nil
	}
...
func (ds *Datastore) GetHostMDMProfileInstallStatus(ctx context.Context, hostUUID string, profUUID string) (mobius.MDMDeliveryStatus, error) {
...
`, table, column)
	...
	if err := sqlx.GetContext(ctx, ds.writer(ctx), &status, selectStmt, mobius.MDMDeliveryPending, mobius.MDMOperationTypeInstall, hostUUID, profUUID); err != nil {
		if err == sql.ErrNoRows {
...
func getTableAndColumnNameForHostMDMProfileUUID(profUUID string) (table, column string, err error) {
	switch {
	case strings.HasPrefix(profUUID, mobius.MDMAppleDeclarationUUIDPrefix):
		return "host_mdm_apple_declarations", "declaration_uuid", nil
	case strings.HasPrefix(profUUID, mobius.MDMAppleProfileUUIDPrefix):
		return "host_mdm_apple_profiles", "profile_uuid", nil
	case strings.HasPrefix(profUUID, mobius.MDMWindowsProfileUUIDPrefix):
...
func (ds *Datastore) AreHostsConnectedToMobiusMDM(ctx context.Context, hosts []*mobius.Host) (map[string]bool, error) {
	var (
		appleUUIDs []any
		winUUIDs   []any
	)
	...
	setConnectedUUIDs := func(stmt string, uuids []any, mp map[string]bool) error {
		...
		if len(uuids) > 0 {
			...
			if err != nil {
				return ctxerr.Wrap(ctx, err, "retrieving hosts connected to mobius")
			}
		}
	...
	// `connected_to_mobius` condition, any relevant filters, and the
	// query used in isAppleHostConnectedToMobiusMDM.
	const appleStmt = `
	  SELECT ne.id
	...
	// NOTE: if you change any of the conditions in this query, please
	// update the `hostMDMSelect` constant too, which has a
	// `connected_to_mobius` condition, and any relevant filters, and the
	// query used in isWindowsHostConnectedToMobiusMDM.
	const winStmt = `
...
func batchSetProfileVariableAssociationsDB(
	ctx context.Context,
	tx sqlx.ExtContext,
	profileVariablesByUUID []mobius.MDMProfileUUIDMobiusVariables,
	platform string,
) error {
	...
	for _, profVars := range profileVariablesByUUID {
		...
		if len(profVars.MobiusVariables) > 0 {
			varsToSet = true
		}
	}
	...
	// load mobius variables to map them to their IDs
	type varDef struct {
		ID       uint   `db:"id"`
		Name     string `db:"name"`
	...
	var varDefs []varDef
	const varsStmt = `SELECT id, name, is_prefix FROM mobius_variables`
	if err := sqlx.SelectContext(ctx, tx, &varDefs, varsStmt); err != nil {
		return fmt.Errorf("failed to load mobius variables: %w", err)
	}
	...
	for _, pv := range profileVariablesByUUID {
		for _, v := range pv.MobiusVariables {
			// variables received here do not have the MOBIUS_VAR_ prefix, but variables
			// in the mobius_variables table do.
			v = "MOBIUS_VAR_" + v
	...
	executeUpsertBatch := func(valuePart string, args []any) error {
		stmt := fmt.Sprintf(`
			...
			VALUES %s
			ON DUPLICATE KEY UPDATE
				mobius_variable_id = VALUES(mobius_variable_id)
...
func (ds *Datastore) BatchResendMDMProfileToHosts(ctx context.Context, profileUUID string, filters mobius.BatchResendMDMProfileFilters) (int64, error) {
...
func (ds *Datastore) GetMDMConfigProfileStatus(ctx context.Context, profileUUID string) (mobius.MDMConfigProfileStatus, error) {
	switch {
	...
	default:
		return mobius.MDMConfigProfileStatus{}, ctxerr.Wrap(ctx, notFound("ConfigurationProfile").WithName(profileUUID))
	}
}

func (ds *Datastore) getWindowsMDMConfigProfileStatus(ctx context.Context, profileUUID string) (mobius.MDMConfigProfileStatus, error) {
	var counts mobius.MDMConfigProfileStatus
...
GROUP BY
	...
	stmt, args, err := sqlx.Named(stmt, map[string]any{
		"status_failed":         mobius.MDMDeliveryFailed,
		"status_pending":        mobius.MDMDeliveryPending,
		"status_verifying":      mobius.MDMDeliveryVerifying,
		"status_verified":       mobius.MDMDeliveryVerified,
...
func (ds *Datastore) getAppleMDMConfigProfileStatus(ctx context.Context, profileUUID string) (mobius.MDMConfigProfileStatus, error) {
	var counts mobius.MDMConfigProfileStatus
...
GROUP BY
	...
	stmt, args, err := sqlx.Named(stmt, map[string]any{
		...
		"status_verifying":  mobius.MDMDeliveryVerifying,
		"status_verified":   mobius.MDMDeliveryVerified,
		"operation_install": mobius.MDMOperationTypeInstall,
		"profile_uuid":      profileUUID,
	})
	if err != nil {
		return counts, ctxerr.Wrap(ctx, err, "prepare arguments with sqlx.Named")
...
func (ds *Datastore) getAppleMDMDeclarationStatus(ctx context.Context, declUUID string) (mobius.MDMConfigProfileStatus, error) {
	var counts mobius.MDMConfigProfileStatus
