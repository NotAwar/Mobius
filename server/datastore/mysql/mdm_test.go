...
func TestMDMShared(t *testing.T) {
	...
	cases := []struct {
		...
		{"TestIsHostConnectedToMobiusMDM", testIsHostConnectedToMobiusMDM},
		{"TestAreHostsConnectedToMobiusMDM", testAreHostsConnectedToMobiusMDM},
...
func testMDMCommands(t *testing.T, ds *Datastore) {
	...
	cmds, err := ds.ListMDMCommands(ctx, mobius.TeamFilter{}, &mobius.MDMCommandListOptions{})
	require.NoError(t, err)
	require.Empty(t, cmds)

	// enroll a windows device
	windowsH, err := ds.NewHost(ctx, &mobius.Host{
	...
	require.NoError(t, err)
	windowsEnrollment := &mobius.MDMWindowsEnrolledDevice{
	...
	macH, err := ds.NewHost(ctx, &mobius.Host{
		Hostname:       "macos-test",
	...
	cmds, err = ds.ListMDMCommands(
		...
		&mobius.MDMCommandListOptions{},
	)
	require.NoError(t, err)
	require.Empty(t, cmds)

	// insert a windows command
	winCmd := &mobius.MDMWindowsCommand{
	...
	cmds, err = ds.ListMDMCommands(
		...
		mobius.TeamFilter{User: test.UserAdmin},
		&mobius.MDMCommandListOptions{
			ListOptions: mobius.ListOptions{OrderKey: "hostname"},
		})
	require.NoError(t, err)
	...
	winCmd2 := &mobius.MDMWindowsCommand{
		CommandUUID:  uuid.NewString(),
		RawCommand:   []byte("<Exec></Exec>"),
		TargetLocURI: "./test/uri2",
	}
	winCmd3 := &mobius.MDMWindowsCommand{
		CommandUUID:  uuid.NewString(),
		RawCommand:   []byte("<Exec></Exec>"),
	...
	// add macos host
	macH2, err := ds.NewHost(ctx, &mobius.Host{
	...
	cmds, err = ds.ListMDMCommands(
		...
		&mobius.MDMCommandListOptions{
			...
			ListOptions: mobius.ListOptions{OrderKey: "hostname", OrderDirection: mobius.OrderAscending},
		},
	)
	require.NoError(t, err)
	require.Len(t, cmds, 2)
...
func testBatchSetMDMProfiles(t *testing.T, ds *Datastore) {
	...
	applyAndExpect := func(
		newAppleSet []*mobius.MDMAppleConfigProfile,
		newWindowsSet []*mobius.MDMWindowsConfigProfile,
		newAppleDeclSet []*mobius.MDMAppleDeclaration,
		tmID *uint,
		wantApple []*mobius.MDMAppleConfigProfile,
		wantWindows []*mobius.MDMWindowsConfigProfile,
		wantAppleDecl []*mobius.MDMAppleDeclaration,
		wantUpdates mobius.MDMProfilesUpdates,
	) {
	...
	withTeamIDApple := func(p *mobius.MDMAppleConfigProfile, tmID uint) *mobius.MDMAppleConfigProfile {
		p.TeamID = &tmID
		return p
	}

	withTeamIDDecl := func(d *mobius.MDMAppleDeclaration, tmID uint) *mobius.MDMAppleDeclaration {
		d.TeamID = &tmID
		return d
	}

	withTeamIDWindows := func(p *mobius.MDMWindowsConfigProfile, tmID uint) *mobius.MDMWindowsConfigProfile {
		p.TeamID = &tmID
		return p
	}

	// empty set for no team (both Apple and Windows)
	applyAndExpect(nil, nil, nil, nil, nil, nil, nil, mobius.MDMProfilesUpdates{})

	// single Apple and Windows profile set for a specific team
	applyAndExpect(
		[]*mobius.MDMAppleConfigProfile{configProfileForTest(t, "N1", "I1", "a")},
		[]*mobius.MDMWindowsConfigProfile{windowsConfigProfileForTest(t, "W1", "l1")},
		[]*mobius.MDMAppleDeclaration{declForTest("D1", "D1", "foo")},
		ptr.Uint(1),
		...
		[]*mobius.MDMAppleDeclaration{withTeamIDDecl(declForTest("D1", "D1", "foo"), 1)},
		mobius.MDMProfilesUpdates{AppleConfigProfile: true, WindowsConfigProfile: true, AppleDeclaration: true},
	)
	...
	applyAndExpect(
		...
		mobius.MDMProfilesUpdates{AppleConfigProfile: false, WindowsConfigProfile: false, AppleDeclaration: false},
	)
	...
	lblExcl, err := ds.NewLabel(ctx, &mobius.Label{Name: "exclude-label-1", Query: "select 1"})
	require.NoError(t, err)
	lblExcl2, err := ds.NewLabel(ctx, &mobius.Label{Name: "exclude-label-2", Query: "select 2"})
	require.NoError(t, err)
	lblInclAny, err := ds.NewLabel(ctx, &mobius.Label{Name: "include-label-3", Query: "select 3"})
	require.NoError(t, err)
	lblInclAny2, err := ds.NewLabel(ctx, &mobius.Label{Name: "include-label-4", Query: "select 4"})
	require.NoError(t, err)
	lblInclAll, err := ds.NewLabel(ctx, &mobius.Label{Name: "inclall-label-5", Query: "select 5"})
	require.NoError(t, err)
	lblInclAll2, err := ds.NewLabel(ctx, &mobius.Label{Name: "inclall-label-6", Query: "select 6"})
	require.NoError(t, err)

	// we only care about declarations here, as batch-setting labels for profiles
	// is tested elsewhere.
	applyAndExpect(nil, nil, []*mobius.MDMAppleDeclaration{
		declForTest("D1", "D1", "foo", lblExcl, lblExcl2),
		declForTest("D2", "D2", "foo", lblInclAll, lblInclAll2),
	}, nil,
		nil, nil, []*mobius.MDMAppleDeclaration{
			declForTest("D1", "D1", "foo", lblExcl, lblExcl2),
			declForTest("D2", "D2", "foo", lblInclAll, lblInclAll2),
		},
	...
	applyAndExpect(nil, nil, []*mobius.MDMAppleDeclaration{
		...
		mobius.MDMProfilesUpdates{AppleConfigProfile: false, WindowsConfigProfile: false, AppleDeclaration: true},
	)
	applyAndExpect(nil, nil, []*mobius.MDMAppleDeclaration{
...
func testListMDMConfigProfiles(t *testing.T, ds *Datastore) {
	...
	opts := mobius.ListOptions{OrderKey: "name", IncludeMetadata: true}
	...
	require.Equal(t, *meta, mobius.PaginationMetadata{})

	// add mobius-managed Apple profiles for the team and globally
	...
	// add mobius-managed Windows profiles for the team and globally
	for name := range mdm_types.MobiusReservedProfileNames() {
		_, err = ds.NewMDMWindowsConfigProfile(
			ctx,
			mobius.MDMWindowsConfigProfile{Name: name, TeamID: &team.ID, SyncML: winProf},
		)
		require.NoError(t, err)
		_, err = ds.NewMDMWindowsConfigProfile(
			ctx,
			mobius.MDMWindowsConfigProfile{Name: name, TeamID: nil, SyncML: winProf},
		)
		require.NoError(t, err)
	}
	...
	profB, err := ds.NewMDMWindowsConfigProfile(
		ctx,
		mobius.MDMWindowsConfigProfile{Name: "B", TeamID: &team.ID, SyncML: winProf},
	)
	require.NoError(t, err)
	...
	var labels []*mobius.Label
	for i := 0; i < 8; i++ {
		lbl, err := ds.NewLabel(ctx, &mobius.Label{Name: "l" + strconv.Itoa(i), Query: "select 1"})
		require.NoError(t, err)
	...
	for i := 0; i < 3; i++ {
		...
		if i == 0 {
			acp.LabelsIncludeAll = []mobius.ConfigurationProfileLabel{
		...
		wcp := mobius.MDMWindowsConfigProfile{
			Name:   string(rune('C' + inc + 2)),
			TeamID: nil,
			SyncML: winProf,
		}
		if i == 0 {
			wcp.LabelsIncludeAll = []mobius.ConfigurationProfileLabel{
	...
	profLabels := map[string][]mobius.ConfigurationProfileLabel{
		"C": {
	...
	cases := []struct {
	...
	}{
		{
			"all global",
			nil,
			mobius.ListOptions{OrderKey: "name", IncludeMetadata: true},
			[]string{"A", "C", "E", "G", "I", "K", "M"},
			mobius.PaginationMetadata{},
		},
		{
			"all team",
			&team.ID,
...

		{
			...
			nil,
			mobius.ListOptions{OrderKey: "name", IncludeMetadata: true, PerPage: 2},
			[]string{"A", "C"},
			mobius.PaginationMetadata{HasNextResults: true},
		},
		{
			"page 1 per page 2, global",
			nil,
			mobius.ListOptions{OrderKey: "name", IncludeMetadata: true, PerPage: 2, Page: 1},
			[]string{"E", "G"},
			mobius.PaginationMetadata{HasPreviousResults: true, HasNextResults: true},
		},
		{
			"page 2 per page 2, global",
			nil,
			mobius.ListOptions{OrderKey: "name", IncludeMetadata: true, PerPage: 2, Page: 2},
			[]string{"I", "K"},
			mobius.PaginationMetadata{HasPreviousResults: true, HasNextResults: true},
		},
		{
			...
			nil,
			mobius.ListOptions{OrderKey: "name", IncludeMetadata: true, PerPage: 2, Page: 3},
			[]string{"M"},
			mobius.PaginationMetadata{HasPreviousResults: true},
		},
		{
			"page 4 per page 2, global",
			nil,
			mobius.ListOptions{OrderKey: "name", IncludeMetadata: true, PerPage: 2, Page: 4},
			[]string{},
			mobius.PaginationMetadata{HasPreviousResults: true},
		},
...

		{
			...
			nil,
			mobius.ListOptions{OrderKey: "name", IncludeMetadata: true, PerPage: 3},
			[]string{"A", "C", "E"},
			mobius.PaginationMetadata{HasNextResults: true},
		},
		{
			...
			nil,
			mobius.ListOptions{OrderKey: "name", IncludeMetadata: true, PerPage: 3, Page: 1},
			[]string{"G", "I", "K"},
			mobius.PaginationMetadata{HasPreviousResults: true, HasNextResults: true},
		},
		{
			...
			nil,
			mobius.ListOptions{OrderKey: "name", IncludeMetadata: true, PerPage: 3, Page: 2},
			[]string{"M"},
			mobius.PaginationMetadata{HasPreviousResults: true},
		},
		{
			...
			nil,
			mobius.ListOptions{OrderKey: "name", IncludeMetadata: true, PerPage: 3, Page: 3},
			[]string{},
			mobius.PaginationMetadata{HasPreviousResults: true},
		},
...

		{
			"no metadata, global",
			nil,
			mobius.ListOptions{OrderKey: "name", IncludeMetadata: false, PerPage: 2, Page: 1},
			[]string{"E", "G"},
			mobius.PaginationMetadata{},
		},
		{
	...
	for _, c := range cases {
		t.Run(c.desc, func(t *testing.T) {
			...
			var gotMeta mobius.PaginationMetadata
			if meta != nil {
				gotMeta = *meta
			}
...
type anyProfile struct {
	...
	OperationType    mobius.MDMOperationType
	IdentifierOrName string
}

// only asserts the profile ID, status and operation
func assertHostProfiles(t *testing.T, ds *Datastore, want map[*mobius.Host][]anyProfile) {
	ctx := context.Background()
...
func testBulkSetPendingMDMHostProfiles(t *testing.T, ds *Datastore) {
	...
	hostIDsFromHosts := func(hosts ...*mobius.Host) []uint {
	...
	getProfs := func(teamID *uint) []*mobius.MDMConfigProfilePayload {
		// TODO(roberto): the docs says that you can pass a comma separated
		// list of columns to OrderKey, but that doesn't seem to work
		profs, _, err := ds.ListMDMConfigProfiles(ctx, teamID, mobius.ListOptions{})
		require.NoError(t, err)
		sort.Slice(profs, func(i, j int) bool {
			l, r := profs[i], profs[j]
	...
	var darwinHosts []*mobius.Host // not pre-allocating, causes gosec false positive
	for i := 0; i < 3; i++ {
	...
	i = 4
	linuxHost, err := ds.NewHost(ctx, &mobius.Host{
		Hostname:      fmt.Sprintf("test-host%d-name", i),
		OsqueryHostID: ptr.String(fmt.Sprintf("osquery-%d", i)),
		NodeKey:       ptr.String(fmt.Sprintf("nodekey-%d", i)),
		UUID:          fmt.Sprintf("test-uuid-%d", i),
	...
	i = 5
	var windowsHosts []*mobius.Host // not preallocating, causes gosec false positive
	for j := 0; j < 3; j++ {
		h, err := ds.NewHost(ctx, &mobius.Host{
	...
	assert.False(t, updates.AppleDeclaration)
	assert.False(t, updates.WindowsConfigProfile)
	assertHostProfiles(t, ds, map[*mobius.Host][]anyProfile{
		darwinHosts[0]:  {},
		darwinHosts[1]:  {},
		darwinHosts[2]:  {},
		unenrolledHost:  {},
	...
	macGlobalProfiles := []*mobius.MDMAppleConfigProfile{
		configProfileForTest(t, "G1a", "G1a", "a"),
		configProfileForTest(t, "G2a", "G2a", "b"),
		configProfileForTest(t, "G3a", "G3a", "c"),
	}
	macGlobalDeclarations := []*mobius.MDMAppleDeclaration{
		declForTest("G1d", "G1d", "foo"),
		declForTest("G2d", "G2d", "bar"),
	}
	winGlobalProfiles := []*mobius.MDMWindowsConfigProfile{
	...
	tm1DarwinProfiles := []*mobius.MDMAppleConfigProfile{
		configProfileForTest(t, "T1.1a", "T1.1a", "d"),
		configProfileForTest(t, "T1.2a", "T1.2a", "e"),
	}
	tm1WindowsProfiles := []*mobius.MDMWindowsConfigProfile{
	...
	err = ds.BulkUpsertMDMAppleHostProfiles(ctx, []*mobius.MDMAppleBulkUpsertHostProfilePayload{
		{
			...
			Status: &mobius.MDMDeliveryVerifying, OperationType: mobius.MDMOperationTypeRemove, Checksum: []byte("csum"), Scope: mobius.PayloadScopeSystem,
		},
		{
		...
		},
		{
			...
			Status: &mobius.MDMDeliveryFailed, OperationType: mobius.MDMOperationTypeRemove, Checksum: []byte("csum"), Scope: mobius.PayloadScopeSystem,
		},
	})
	require.NoError(t, err)

	// add a profile to team1, and remove profile T1.1 on Apple, T1.2 on Windows
	newTm1DarwinProfiles := []*mobius.MDMAppleConfigProfile{
		configProfileForTest(t, "T1.2a", "T1.2a", "e"),
		configProfileForTest(t, "T1.3a", "T1.3a", "f"),
	}
	newTm1WindowsProfiles := []*mobius.MDMWindowsConfigProfile{
		windowsConfigProfileForTest(t, "T1.1w", "T1.1"),
	...
	newDarwinGlobalProfiles := []*mobius.MDMAppleConfigProfile{
		configProfileForTest(t, "G2a", "G2a", "b"),
		configProfileForTest(t, "G3a", "G3a", "c"),
		configProfileForTest(t, "G4a", "G4a", "d"),
	}
	newWindowsGlobalProfiles := []*mobius.MDMWindowsConfigProfile{
	...
	tm2DarwinProfiles := []*mobius.MDMAppleConfigProfile{
		configProfileForTest(t, "T2.1a", "T2.1a", "a"),
	}

	tm2WindowsProfiles := []*mobius.MDMWindowsConfigProfile{
	...
	setProfileLabels := func(t *testing.T, p *mobius.MDMConfigProfilePayload, labels ...*mobius.Label) {
...
func testGetHostMDMProfilesExpectedForVerification(t *testing.T, ds *Datastore) {
	...
	macosBasicTeamProfNoLabelsSetup := func() (uint, *mobius.Host) {
		...
		// create a team
		team, err := ds.NewTeam(ctx, &mobius.Team{Name: "macos team 1"})
		require.NoError(t, err)
		...
		profs, _, err := ds.ListMDMConfigProfiles(ctx, &team.ID, mobius.ListOptions{})
		require.NoError(t, err)
	...
	macosLabeledTeamProfSetup := func() (uint, *mobius.Host) {
		host, err := ds.NewHost(ctx, &mobius.Host{
			Hostname:      "macos-test-2",
			OsqueryHostID: ptr.String("osquery-macos-2"),
			NodeKey:       ptr.String("node-key-macos-2"),
		...
		// create a team
		team, err := ds.NewTeam(ctx, &mobius.Team{Name: "macos team 2"})
		require.NoError(t, err)
		...
		label, err := ds.NewLabel(ctx, &mobius.Label{Name: "test_label_1"})
		require.NoError(t, err)
	...
	macosLabeledTeamProfWithAdditionalLabeledProfSetup := func() (uint, *mobius.Host) {
		host, err := ds.NewHost(ctx, &mobius.Host{
		...
		// create a team
		team, err := ds.NewTeam(ctx, &mobius.Team{Name: "macos team 3"})
		require.NoError(t, err)
		...
		testLabel2, err := ds.NewLabel(ctx, &mobius.Label{Name: "test_label_2"})
		require.NoError(t, err)

		testLabel3, err := ds.NewLabel(ctx, &mobius.Label{Name: "test_label_3"})
		require.NoError(t, err)
	...
	macosProfWithBrokenLabelSetup := func() (uint, *mobius.Host) {
		host, err := ds.NewHost(ctx, &mobius.Host{
		...
		// create a team
		team, err := ds.NewTeam(ctx, &mobius.Team{Name: "macos team 4"})
		require.NoError(t, err)
		...
		testLabel4, err := ds.NewLabel(ctx, &mobius.Label{Name: "test_label_4"})
		require.NoError(t, err)
	...
	macosInstallDateOverrideSetup := func() (uint, *mobius.Host) {
		host, err := ds.NewHost(ctx, &mobius.Host{
		...
		// create a team
		team, err := ds.NewTeam(ctx, &mobius.Team{Name: "macos team 5"})
		require.NoError(t, err)
	...
	macosLabeledProfileRulesSetup := func() (uint, *mobius.Host) {
		host, err := ds.NewHost(ctx, &mobius.Host{
		...
		// create a team
		team, err := ds.NewTeam(ctx, &mobius.Team{Name: "macos team 6"})
		require.NoError(t, err)
		...
		// Include any labels
		includeAnyMatchedLabel1, err := ds.NewLabel(ctx, &mobius.Label{Name: "include-any-macos-matched-label-1"})
		require.NoError(t, err)
		includeAnyMatchedLabel2, err := ds.NewLabel(ctx, &mobius.Label{Name: "include-any-macos-matched-label-2"})
		require.NoError(t, err)
		includeAnyUnmatchedLabel, err := ds.NewLabel(ctx, &mobius.Label{Name: "include-any-macos-unmatched-label"})
		require.NoError(t, err)

		// Include all labels
		includeAllMatchedLabel1, err := ds.NewLabel(ctx, &mobius.Label{Name: "include-all-macos-matched-label-1"})
		require.NoError(t, err)
		includeAllMatchedLabel2, err := ds.NewLabel(ctx, &mobius.Label{Name: "include-all-macos-matched-label-2"})
		require.NoError(t, err)
		includeAllUnmatchedLabel, err := ds.NewLabel(ctx, &mobius.Label{Name: "include-all-macos-unmatched-label"})
		require.NoError(t, err)

		// Exclude labels
		excludeMatchedLabel1, err := ds.NewLabel(ctx, &mobius.Label{Name: "exclude-macos-matched-label-1"})
		require.NoError(t, err)
		excludeMatchedLabel2, err := ds.NewLabel(ctx, &mobius.Label{Name: "exclude-macos-matched-label-2"})
		require.NoError(t, err)
		excludeUnmatchedLabel, err := ds.NewLabel(ctx, &mobius.Label{Name: "exclude-macos-unmatched-label"})
		require.NoError(t, err)
	...
	windowsBasicTeamProfNoLabelsSetup := func() (uint, *mobius.Host) {
		host, err := ds.NewHost(ctx, &mobius.Host{
			Hostname:      "windows-test",
			OsqueryHostID: ptr.String("osquery-windows"),
			NodeKey:       ptr.String("node-key-windows"),
		...
		// create a team
		team, err := ds.NewTeam(ctx, &mobius.Team{Name: "windows team 1"})
		require.NoError(t, err)
		...
		// create profiles for team 1
		profiles := []*mobius.MDMWindowsConfigProfile{
	...
	windowsLabeledTeamProfSetup := func() (uint, *mobius.Host) {
		host, err := ds.NewHost(ctx, &mobius.Host{
		...
		// create a team
		team, err := ds.NewTeam(ctx, &mobius.Team{Name: "windows team 2"})
		require.NoError(t, err)
		...
		label, err := ds.NewLabel(ctx, &mobius.Label{Name: "test_label_6"})
		require.NoError(t, err)
	...
	windowsLabeledTeamProfWithAdditionalLabeledProfSetup := func() (uint, *mobius.Host) {
		host, err := ds.NewHost(ctx, &mobius.Host{
		...
		// create a team
		team, err := ds.NewTeam(ctx, &mobius.Team{Name: "windows team 3"})
		require.NoError(t, err)
		...
		testLabel2, err := ds.NewLabel(ctx, &mobius.Label{Name: uuid.NewString()})
		require.NoError(t, err)

		testLabel3, err := ds.NewLabel(ctx, &mobius.Label{Name: uuid.NewString()})
		require.NoError(t, err)
	...
	windowsProfWithBrokenLabelSetup := func() (uint, *mobius.Host) {
		host, err := ds.NewHost(ctx, &mobius.Host{
		...
		// create a team
		team, err := ds.NewTeam(ctx, &mobius.Team{Name: "windows team 4"})
		require.NoError(t, err)
		...
		label, err := ds.NewLabel(ctx, &mobius.Label{Name: uuid.NewString()})
		require.NoError(t, err)
	...
	windowsLabeledProfileRulesSetup := func() (uint, *mobius.Host) {
		host, err := ds.NewHost(ctx, &mobius.Host{
		...
		// create a team
		team, err := ds.NewTeam(ctx, &mobius.Team{Name: "windows team 5"})
		require.NoError(t, err)
		...
		// Include any labels
		includeAnyMatchedLabel1, err := ds.NewLabel(ctx, &mobius.Label{Name: "include-any-windows-matched-label-1"})
		require.NoError(t, err)
		includeAnyMatchedLabel2, err := ds.NewLabel(ctx, &mobius.Label{Name: "include-any-windows-matched-label-2"})
		require.NoError(t, err)
		includeAnyUnmatchedLabel, err := ds.NewLabel(ctx, &mobius.Label{Name: "include-any-windows-unmatched-label"})
		require.NoError(t, err)

		// Include all labels
		includeAllMatchedLabel1, err := ds.NewLabel(ctx, &mobius.Label{Name: "include-all-windows-matched-label-1"})
		require.NoError(t, err)
		includeAllMatchedLabel2, err := ds.NewLabel(ctx, &mobius.Label{Name: "include-all-windows-matched-label-2"})
		require.NoError(t, err)
		includeAllUnmatchedLabel, err := ds.NewLabel(ctx, &mobius.Label{Name: "include-all-windows-unmatched-label"})
		require.NoError(t, err)

		// Exclude labels
		excludeMatchedLabel1, err := ds.NewLabel(ctx, &mobius.Label{Name: "exclude-windows-matched-label-1"})
		require.NoError(t, err)
		excludeMatchedLabel2, err := ds.NewLabel(ctx, &mobius.Label{Name: "exclude-windows-matched-label-2"})
		require.NoError(t, err)
		excludeUnmatchedLabel, err := ds.NewLabel(ctx, &mobius.Label{Name: "exclude-windows-unmatched-label"})
		require.NoError(t, err)
	...
	tests := []struct {
		name        string
		setupFunc   func() (uint, *mobius.Host)
		wantMac     map[string]*mobius.ExpectedMDMProfile
		wantWindows map[string]*mobius.ExpectedMDMProfile
		os          string
	}{
		{
...
func testBatchSetProfileLabelAssociations(t *testing.T, ds *Datastore) {
	...
	wantOtherWin := []mobius.ConfigurationProfileLabel{
	...
	wantOtherMac := []mobius.ConfigurationProfileLabel{
	...
	for platform, uuid := range platforms {
		expectLabels := func(t *testing.T, profUUID, platform string, want []mobius.ConfigurationProfileLabel) {
			p := platform
			if p == "darwin" {
				p = "apple"
			}
			...
			var got []mobius.ConfigurationProfileLabel
			ExecAdhocSQL(t, ds, func(tx sqlx.ExtContext) error {
		...
		t.Run("empty input "+platform, func(t *testing.T) {
			want := []mobius.ConfigurationProfileLabel{}
		...
		t.Run("valid input "+platform, func(t *testing.T) {
			profileLabels := []mobius.ConfigurationProfileLabel{
		...
		t.Run("invalid profile UUID "+platform, func(t *testing.T) {
			invalidProfileLabels := []mobius.ConfigurationProfileLabel{
	...
	t.Run("unsupported platform", func(t *testing.T) {
		err := ds.withTx(ctx, func(tx sqlx.ExtContext) error {
			_, err := batchSetProfileLabelAssociationsDB(
				ctx,
				tx,
				[]mobius.ConfigurationProfileLabel{{}},
				nil,
				"unsupported",
			)
			return err
...
func testBatchSetMDMProfilesTransactionError(t *testing.T, ds *Datastore) {
	...
	lbl, err := ds.NewLabel(ctx, &mobius.Label{Name: "label", Query: "select 1"})
	require.NoError(t, err)
	...
	for _, c := range cases {
		t.Run(c.windowsErr+" "+c.appleErr, func(t *testing.T) {
			...
			appleProfs := []*mobius.MDMAppleConfigProfile{
				configProfileForTest(t, "N1", "I1", "a"),
				configProfileForTest(t, "N2", "I2", "b"),
			}
			winProfs := []*mobius.MDMWindowsConfigProfile{
...
func testMDMEULA(t *testing.T, ds *Datastore) {
	...
	var ae mobius.AlreadyExistsError
	err = ds.MDMInsertEULA(ctx, eula)
...
func testSCEPRenewalHelpers(t *testing.T, ds *Datastore) {
	...
	addCert := func(notAfter time.Time, h *mobius.Host) {
		serial, err := scepDepot.Serial()
		require.NoError(t, err)
		cert := &x509.Certificate{
			SerialNumber: serial,
			Subject: pkix.Name{
				CommonName: "Mobius Identity",
			},
			NotAfter: notAfter,
	...
	var i int
	setHost := func(notAfter time.Time) *mobius.Host {
		i++
		h, err := ds.NewHost(ctx, &mobius.Host{
			Hostname:      fmt.Sprintf("test-host%d-name", i),
			OsqueryHostID: ptr.String(fmt.Sprintf("osquery-%d", i)),
	...
	checkSCEPRenew := func(assoc mobius.SCEPIdentityAssociation, want *string) {
		var got *string
		ExecAdhocSQL(t, ds, func(q sqlx.ExtContext) error {
			return sqlx.GetContext(
				ctx,
	...
	err = ds.SetCommandForPendingSCEPRenewal(ctx, []mobius.SCEPIdentityAssociation{}, "foo")
	...
	err = ds.SetCommandForPendingSCEPRenewal(ctx, []mobius.SCEPIdentityAssociation{assocs[0]}, "foo")
	require.NoError(t, err)
	...
	err = ds.SetCommandForPendingSCEPRenewal(
		ctx,
		[]mobius.SCEPIdentityAssociation{{HostUUID: "foo", SHA256: "bar"}},
		"bar",
	)
...
func testMDMProfilesSummaryAndHostFilters(t *testing.T, ds *Datastore) {
	...
	checkSummaryWindows := func(t *testing.T, teamID *uint, expected mobius.MDMProfilesSummary) {
	...
	checkSummaryMac := func(t *testing.T, teamID *uint, expected mobius.MDMProfilesSummary) {
	...
	checkListHostsFilterOSSettings := func(t *testing.T, teamID *uint, status mobius.OSSettingsStatus, expectedIDs []uint) {
		gotHosts, err := ds.ListHosts(
			ctx,
			mobius.TeamFilter{User: test.UserAdmin},
			mobius.HostListOptions{TeamFilter: teamID, OSSettingsFilter: status},
		)
		require.NoError(t, err)
	...
	type hostIDsByProfileStatus map[mobius.MDMDeliveryStatus][]uint

	checkExpected := func(t *testing.T, teamID *uint, ep hostIDsByProfileStatus) {
		expectSummaryWindows := map[mobius.MDMDeliveryStatus]uint{}
		expectSummaryMac := map[mobius.MDMDeliveryStatus]uint{}
		...
		checkSummaryMac(t, teamID, mobius.MDMProfilesSummary{
			Pending:   expectSummaryMac[mobius.MDMDeliveryPending],
			Failed:    expectSummaryMac[mobius.MDMDeliveryFailed],
			Verifying: expectSummaryMac[mobius.MDMDeliveryVerifying],
			Verified:  expectSummaryMac[mobius.MDMDeliveryVerified],
		})

		checkSummaryWindows(t, teamID, mobius.MDMProfilesSummary{
			Pending:   expectSummaryWindows[mobius.MDMDeliveryPending],
			Failed:    expectSummaryWindows[mobius.MDMDeliveryFailed],
			Verifying: expectSummaryWindows[mobius.MDMDeliveryVerifying],
			Verified:  expectSummaryWindows[mobius.MDMDeliveryVerified],
		})

		checkListHostsFilterOSSettings(
			t,
			teamID,
			mobius.OSSettingsVerified,
			ep[mobius.MDMDeliveryVerified],
		)
		checkListHostsFilterOSSettings(
			t,
			teamID,
			mobius.OSSettingsVerifying,
			ep[mobius.MDMDeliveryVerifying],
		)
		checkListHostsFilterOSSettings(
			t,
			teamID,
			mobius.OSSettingsFailed,
			ep[mobius.MDMDeliveryFailed],
		)
		checkListHostsFilterOSSettings(
			t,
			teamID,
			mobius.OSSettingsPending,
			ep[mobius.MDMDeliveryPending],
		)
	}
	...
	checkMacHostProfiles := func(t *testing.T, hostUUID string, statusByProfUUID map[string]string) {
		...
		for _, prof := range profs {
			...
			require.Equal(t, mobius.MDMDeliveryStatus(ep), *prof.Status)
		}
	}

	upsertHostProfileStatus := func(t *testing.T, hostUUID string, profUUID string, status *mobius.MDMDeliveryStatus) {
		ExecAdhocSQL(t, ds, func(q sqlx.ExtContext) error {
			var table string
			var profType string
			switch {
			...
			_, err = q.ExecContext(ctx, stmt, mobius.MDMOperationTypeInstall, hostUUID, profUUID)
			require.NoError(t, err)
			return err
		})
	}
	...
	macHostsByID := make(map[uint]*mobius.Host, 5)
	winHostsByID := make(map[uint]*mobius.Host, 5)
	for i := 0; i < 10; i++ {
		p := "windows"
		if i >= 5 {
			p = "darwin"
	...
	upsertHostProfileStatus(t, hosts[0].UUID, "w1", &mobius.MDMDeliveryPending)

	checkExpected(t, nil, hostIDsByProfileStatus{
		mobius.MDMDeliveryPending: []uint{hosts[0].ID},
	})

	// add some mac profiles with different statuses
	upsertHostProfileStatus(t, hosts[9].UUID, "a1", &mobius.MDMDeliveryFailed)
	upsertHostProfileStatus(t, hosts[9].UUID, "a2", &mobius.MDMDeliveryPending)
	upsertHostProfileStatus(t, hosts[9].UUID, "a3", &mobius.MDMDeliveryVerifying)
	upsertHostProfileStatus(t, hosts[9].UUID, "a4", &mobius.MDMDeliveryVerified)

	// add some mac declarations with different statuses
	upsertHostProfileStatus(t, hosts[9].UUID, "d1", &mobius.MDMDeliveryFailed)
	upsertHostProfileStatus(t, hosts[9].UUID, "d2", &mobius.MDMDeliveryPending)
	upsertHostProfileStatus(t, hosts[9].UUID, "d3", &mobius.MDMDeliveryVerifying)
	upsertHostProfileStatus(t, hosts[9].UUID, "d4", &mobius.MDMDeliveryVerified)

	checkExpected(t, nil, hostIDsByProfileStatus{
		mobius.MDMDeliveryPending: []uint{hosts[0].ID},
		mobius.MDMDeliveryFailed:  []uint{hosts[9].ID},
	})
	...
	upsertHostProfileStatus(t, hosts[9].UUID, "a1", &mobius.MDMDeliveryPending)
	...
	upsertHostProfileStatus(t, hosts[9].UUID, "d1", &mobius.MDMDeliveryPending)
	expectedHostProfiles["d1"] = "pending"
	checkExpected(t, nil, hostIDsByProfileStatus{
		mobius.MDMDeliveryPending: []uint{hosts[0].ID, hosts[9].ID},
	})
	checkMacHostProfiles(t, hosts[9].UUID, expectedHostProfiles)

	// set pending mac declaration to failed, host status is now failed
	upsertHostProfileStatus(t, hosts[9].UUID, "d2", &mobius.MDMDeliveryFailed)
	...
	upsertHostProfileStatus(t, hosts[9].UUID, "d2", &mobius.MDMDeliveryVerifying)
	...
	upsertHostProfileStatus(t, hosts[9].UUID, "a1", &mobius.MDMDeliveryVerifying)
	expectedHostProfiles["a1"] = "verifying"
	upsertHostProfileStatus(t, hosts[9].UUID, "a2", &mobius.MDMDeliveryVerifying)
	...
	upsertHostProfileStatus(t, hosts[9].UUID, "d1", &mobius.MDMDeliveryVerifying)
	expectedHostProfiles["d1"] = "verifying"
	checkExpected(t, nil, hostIDsByProfileStatus{
		mobius.MDMDeliveryPending:   []uint{hosts[0].ID},
		mobius.MDMDeliveryVerifying: []uint{hosts[9].ID},
	})
	checkMacHostProfiles(t, hosts[9].UUID, expectedHostProfiles)
	...
	// verifying
	upsertHostProfileStatus(t, hosts[9].UUID, "a1", &mobius.MDMDeliveryVerified)
	expectedHostProfiles["a1"] = "verified"
	upsertHostProfileStatus(t, hosts[9].UUID, "a2", &mobius.MDMDeliveryVerified)
	expectedHostProfiles["a2"] = "verified"
	upsertHostProfileStatus(t, hosts[9].UUID, "a3", &mobius.MDMDeliveryVerified)
	...
	upsertHostProfileStatus(t, hosts[9].UUID, "d1", &mobius.MDMDeliveryVerified)
	expectedHostProfiles["d1"] = "verified"
	upsertHostProfileStatus(t, hosts[9].UUID, "d2", &mobius.MDMDeliveryVerified)
	expectedHostProfiles["d2"] = "verified"
	upsertHostProfileStatus(t, hosts[9].UUID, "d3", &mobius.MDMDeliveryVerified)
	expectedHostProfiles["d3"] = "verified"
	checkExpected(t, nil, hostIDsByProfileStatus{
		mobius.MDMDeliveryPending:  []uint{hosts[0].ID},
		mobius.MDMDeliveryVerified: []uint{hosts[9].ID},
	})
	checkMacHostProfiles(t, hosts[9].UUID, expectedHostProfiles)
	...
	checkExpected(t, nil, hostIDsByProfileStatus{
		mobius.MDMDeliveryPending:  []uint{hosts[0].ID},
		mobius.MDMDeliveryVerified: []uint{hosts[9].ID}, // all profiles were verified
	})
	checkMacHostProfiles(t, hosts[9].UUID, expectedHostProfiles)
...
func testAreHostsConnectedToMobiusMDM(t *testing.T, ds *Datastore) {
	ctx := context.Background()

	notConnectedMac, err := ds.NewHost(ctx, &mobius.Host{
		Hostname:      "macos-test",
	...
	connectedMac, err := ds.NewHost(ctx, &mobius.Host{
		Hostname:      "macos-test",
		OsqueryHostID: ptr.String("osquery-macos"),
		NodeKey:       ptr.String("node-key-macos"),
		UUID:          uuid.NewString(),
	...
	disconnectedWithoutCheckoutMac, err := ds.NewHost(ctx, &mobius.Host{
	...
	notConnectedWin, err := ds.NewHost(ctx, &mobius.Host{
		Hostname:      "windows-test",
	...
	connectedWin, err := ds.NewHost(ctx, &mobius.Host{
		Hostname:      "windows-test",
		OsqueryHostID: ptr.String("osquery-windows"),
		NodeKey:       ptr.String("node-key-windows"),
		UUID:          uuid.NewString(),
	...
	disconnectedWithoutCheckoutWin, err := ds.NewHost(ctx, &mobius.Host{
	...
	require.NoError(t, err)
	windowsEnrollmentDisconnectedWithoutCheckout := &mobius.MDMWindowsEnrolledDevice{
		MDMDeviceID:            uuid.New().String(),
		MDMHardwareID:          uuid.New().String() + uuid.New().String(),
		MDMDeviceState:         microsoft_mdm.MDMDeviceStateEnrolled,
		MDMDeviceType:          "CIMClient_Windows",
	...
	connectedMap, err := ds.AreHostsConnectedToMobiusMDM(ctx, []*mobius.Host{
		notConnectedMac,
		connectedMac,
		connectedWin,
		notConnectedWin,
...
func testIsHostConnectedToMobiusMDM(t *testing.T, ds *Datastore) {
	ctx := context.Background()
	macH, err := ds.NewHost(ctx, &mobius.Host{
		Hostname:      "macos-test",
		OsqueryHostID: ptr.String("osquery-macos"),
	...
	connected, err := ds.IsHostConnectedToMobiusMDM(ctx, macH)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	connected, err = ds.IsHostConnectedToMobiusMDM(ctx, windowsH)
	require.NoError(t, err)
	require.False(t, connected)
...
func testBulkSetPendingMDMHostProfilesExcludeAny(t *testing.T, ds *Datastore) {
	...
	var labels []*mobius.Label
	for i := 0; i < 6; i++ {
		lbl, err := ds.NewLabel(ctx, &mobius.Label{Name: "exclude-label-" + strconv.Itoa(i), Query: "select 1"})
		require.NoError(t, err)
		labels = append(labels, lbl)
	}
	...
	windowsProfs := []*mobius.MDMWindowsConfigProfile{
		windowsConfigProfileForTest(t, "W1", "W1", labels[2]),
	}
	appleDecls := []*mobius.MDMAppleDeclaration{
	...
	var i int
	winHost, err := ds.NewHost(ctx, &mobius.Host{
	...
	i++
	appleHost, err := ds.NewHost(ctx, &mobius.Host{
	...
	i++
	winHost2, err := ds.NewHost(ctx, &mobius.Host{
		Hostname:      fmt.Sprintf("win-host%d-name", i),
		OsqueryHostID: ptr.String(fmt.Sprintf("osquery-%d", i)),
		NodeKey:       ptr.String(fmt.Sprintf("nodekey-%d", i)),
		UUID:          fmt.Sprintf("win-uuid-%d", i),
	...
	i++
	appleHost2, err := ds.NewHost(ctx, &mobius.Host{
		Hostname:      fmt.Sprintf("apple-host%d-name", i),
		OsqueryHostID: ptr.String(fmt.Sprintf("osquery-%d", i)),
		NodeKey:       ptr.String(fmt.Sprintf("nodekey-%d", i)),
		UUID:          fmt.Sprintf("apple-uuid-%d", i),
...
func testBatchResendProfileToHosts(t *testing.T, ds *Datastore) {
	...
	team, err := ds.NewTeam(ctx, &mobius.Team{Name: "team1"})
	require.NoError(t, err)
	...
	forceSetAppleHostProfileStatus(t, ds, host1.UUID, profA, mobius.MDMOperationTypeInstall, mobius.MDMDeliveryVerifying)
	forceSetAppleHostProfileStatus(t, ds, host2.UUID, profA, mobius.MDMOperationTypeInstall, mobius.MDMDeliveryVerifying)
	forceSetAppleHostProfileStatus(t, ds, host3.UUID, profA, mobius.MDMOperationTypeInstall, mobius.MDMDeliveryVerifying)

	// batch-resend profile A, does not impact any host
	n, err := ds.BatchResendMDMProfileToHosts(ctx, profA.ProfileUUID, mobius.BatchResendMDMProfileFilters{ProfileStatus: mobius.MDMDeliveryFailed})
	require.NoError(t, err)
	require.EqualValues(t, 0, n)

	assertHostProfileStatus(t, ds, host1.UUID,
		hostProfileStatus{profA.ProfileUUID, mobius.MDMDeliveryVerifying})
	assertHostProfileStatus(t, ds, host2.UUID,
		hostProfileStatus{profA.ProfileUUID, mobius.MDMDeliveryVerifying})
	assertHostProfileStatus(t, ds, host3.UUID,
		hostProfileStatus{profA.ProfileUUID, mobius.MDMDeliveryVerifying})
	...
	n, err = ds.BatchResendMDMProfileToHosts(ctx, profB.ProfileUUID, mobius.BatchResendMDMProfileFilters{ProfileStatus: mobius.MDMDeliveryFailed})
	require.NoError(t, err)
	require.EqualValues(t, 0, n)
	...
	forceSetAppleHostProfileStatus(t, ds, host1.UUID, profA, mobius.MDMOperationTypeInstall, mobius.MDMDeliveryFailed)
	forceSetAppleHostProfileStatus(t, ds, host2.UUID, profA, mobius.MDMOperationTypeInstall, mobius.MDMDeliveryFailed)
	forceSetAppleHostProfileStatus(t, ds, host3.UUID, profA, mobius.MDMOperationTypeInstall, mobius.MDMDeliveryVerified)
	...
	assertHostProfileStatus(t, ds, host1.UUID,
		hostProfileStatus{profA.ProfileUUID, mobius.MDMDeliveryPending})
	assertHostProfileStatus(t, ds, host2.UUID,
		hostProfileStatus{profA.ProfileUUID, mobius.MDMDeliveryPending})
	assertHostProfileStatus(t, ds, host3.UUID,
		hostProfileStatus{profA.ProfileUUID, mobius.MDMDeliveryVerified})
	assertHostProfileStatus(t, ds, host4.UUID)
	...
	forceSetAppleHostProfileStatus(t, ds, host1.UUID, profB, mobius.MDMOperationTypeInstall, mobius.MDMDeliveryFailed)
	forceSetAppleHostProfileStatus(t, ds, host2.UUID, profB, mobius.MDMOperationTypeInstall, mobius.MDMDeliveryFailed)
	forceSetAppleHostProfileStatus(t, ds, host3.UUID, profB, mobius.MDMOperationTypeInstall, mobius.MDMDeliveryFailed)

	assertHostProfileStatus(t, ds, host1.UUID,
		hostProfileStatus{profA.ProfileUUID, mobius.MDMDeliveryPending},
		hostProfileStatus{profB.ProfileUUID, mobius.MDMDeliveryFailed})
	assertHostProfileStatus(t, ds, host2.UUID,
		hostProfileStatus{profA.ProfileUUID, mobius.MDMDeliveryPending},
		hostProfileStatus{profB.ProfileUUID, mobius.MDMDeliveryFailed})
	assertHostProfileStatus(t, ds, host3.UUID,
	...
	assertHostProfileStatus(t, ds, host1.UUID,
		hostProfileStatus{profA.ProfileUUID, mobius.MDMDeliveryPending},
		hostProfileStatus{profB.ProfileUUID, mobius.MDMDeliveryPending})
	assertHostProfileStatus(t, ds, host2.UUID,
		hostProfileStatus{profA.ProfileUUID, mobius.MDMDeliveryPending},
		hostProfileStatus{profB.ProfileUUID, mobius.MDMDeliveryPending})
	assertHostProfileStatus(t, ds, host3.UUID,
	...
	forceSetAppleHostProfileStatus(t, ds, host4.UUID, profC, mobius.MDMOperationTypeInstall, mobius.MDMDeliveryFailed)
	forceSetAppleHostProfileStatus(t, ds, host1.UUID, profA, mobius.MDMOperationTypeInstall, mobius.MDMDeliveryVerified)
	forceSetAppleHostProfileStatus(t, ds, host2.UUID, profA, mobius.MDMOperationTypeInstall, mobius.MDMDeliveryVerified)
	forceSetAppleHostProfileStatus(t, ds, host1.UUID, profB, mobius.MDMOperationTypeInstall, mobius.MDMDeliveryVerified)
	forceSetAppleHostProfileStatus(t, ds, host2.UUID, profB, mobius.MDMOperationTypeInstall, mobius.MDMDeliveryVerified)
	forceSetAppleHostProfileStatus(t, ds, host3.UUID, profB, mobius.MDMOperationTypeInstall, mobius.MDMDeliveryVerified)

	// batch-resend profile C, host 4 affected
	n, err = ds.BatchResendMDMProfileToHosts(ctx, profC.ProfileUUID, mobius.BatchResendMDMProfileFilters{ProfileStatus: mobius.MDMDeliveryFailed})
	require.NoError(t, err)
	require.EqualValues(t, 1, n)

	assertHostProfileStatus(t, ds, host1.UUID,
		hostProfileStatus{profA.ProfileUUID, mobius.MDMDeliveryVerified},
		hostProfileStatus{profB.ProfileUUID, mobius.MDMDeliveryVerified})
	assertHostProfileStatus(t, ds, host2.UUID,
		hostProfileStatus{profA.ProfileUUID, mobius.MDMDeliveryVerified},
		hostProfileStatus{profB.ProfileUUID, mobius.MDMDeliveryVerified})
	assertHostProfileStatus(t, ds, host3.UUID,
		...
		hostProfileStatus{profB.ProfileUUID, mobius.MDMDeliveryVerified})
	assertHostProfileStatus(t, ds, host4.UUID,
		hostProfileStatus{profC.ProfileUUID, mobius.MDMDeliveryPending})
...
func testGetMDMConfigProfileStatus(t *testing.T, ds *Datastore) {
	...
	appleProfsNoTm := []*mobius.MDMAppleConfigProfile{
		configProfileForTest(t, "A1", "A1", uuid.NewString()),
	}
	windowsProfsNoTm := []*mobius.MDMWindowsConfigProfile{
		windowsConfigProfileForTest(t, "W1", "W1"),
	}
	appleDeclsNoTm := []*mobius.MDMAppleDeclaration{
		declForTest("D1", "D1", "{}"),
	}
	...
	appleProfsTm := []*mobius.MDMAppleConfigProfile{
		configProfileForTest(t, "A2", "A2", uuid.NewString()),
	}
	windowsProfsTm := []*mobius.MDMWindowsConfigProfile{
		windowsConfigProfileForTest(t, "W2", "W2"),
	}
	appleDeclsTm := []*mobius.MDMAppleDeclaration{
		declForTest("D2", "D2", "{}"),
	}
	...
	profNameToProf := make(map[string]*mobius.MDMConfigProfilePayload)
	profs, _, err := ds.ListMDMConfigProfiles(ctx, nil, mobius.ListOptions{})
	require.NoError(t, err)
	for _, prof := range profs {
	...
	for _, h := range []*mobius.Host{host1, host2, host3, host4, host5, host6, host7, host8} {
		err = ds.SetOrUpdateMDMData(ctx, h.ID, false, true, "https://mobiusmdm.com", false, mobius.WellKnownMDMMobius, "")
		require.NoError(t, err)
	}
	...
	for _, name := range []string{"A1", "W1", "D1", "A2", "W2", "D2"} {
		...
		require.Equal(t, mobius.MDMConfigProfileStatus{}, status, name)
	}
	...
	cases := []struct {
		...
		want        mobius.MDMConfigProfileStatus
	}{
		{
			...
			setup: func(t *testing.T) {
				...
				forceSetAppleHostProfileStatus(t, ds, host1.UUID, appleA1, mobius.MDMOperationTypeInstall, mobius.MDMDeliveryPending)
				forceSetAppleHostProfileStatus(t, ds, host2.UUID, appleA1, mobius.MDMOperationTypeInstall, mobius.MDMDeliveryPending)
				forceSetAppleHostProfileStatus(t, ds, host3.UUID, appleA1, mobius.MDMOperationTypeInstall, "")
			},
			want: mobius.MDMConfigProfileStatus{Pending: 3},
		},
		{
			...
			setup: func(t *testing.T) {
				...
				forceSetWindowsHostProfileStatus(t, ds, host6.UUID, winW1, mobius.MDMOperationTypeInstall, mobius.MDMDeliveryPending)
				forceSetWindowsHostProfileStatus(t, ds, host7.UUID, winW1, mobius.MDMOperationTypeInstall, "")
			},
			want: mobius.MDMConfigProfileStatus{Pending: 2},
		},
		{
			...
			setup: func(t *testing.T) {
				winW1 := test.ToMDMWindowsConfigProfile(profNameToProf["W1"])
				forceSetWindowsHostProfileStatus(t, ds, host6.UUID, winW1, mobius.MDMOperationTypeInstall, mobius.MDMDeliveryPending)
				forceSetWindowsHostProfileStatus(t, ds, host7.UUID, winW1, mobius.MDMOperationTypeInstall, mobius.MDMDeliveryFailed)
			},
			want: mobius.MDMConfigProfileStatus{Pending: 1, Failed: 1},
		},
		{
			...
			setup: func(t *testing.T) {
				...
				forceSetAppleHostDeclarationStatus(t, ds, host1.UUID, declD1, mobius.MDMOperationTypeInstall, mobius.MDMDeliveryPending)
				forceSetAppleHostDeclarationStatus(t, ds, host2.UUID, declD1, mobius.MDMOperationTypeInstall, mobius.MDMDeliveryFailed)
				forceSetAppleHostDeclarationStatus(t, ds, host3.UUID, declD1, mobius.MDMOperationTypeInstall, mobius.MDMDeliveryVerified)
			},
			want: mobius.MDMConfigProfileStatus{Pending: 1, Failed: 1, Verified: 1},
		},
		{
			...
			setup: func(t *testing.T) {
				...
				forceSetAppleHostProfileStatus(t, ds, host4.UUID, appleA2, mobius.MDMOperationTypeInstall, mobius.MDMDeliveryVerifying)
				forceSetAppleHostProfileStatus(t, ds, host5.UUID, appleA2, mobius.MDMOperationTypeInstall, mobius.MDMDeliveryVerified)
			},
			want: mobius.MDMConfigProfileStatus{Verifying: 1, Verified: 1},
		},
		{
			...
			setup: func(t *testing.T) {
				...
				forceSetAppleHostDeclarationStatus(t, ds, host4.UUID, declD2, mobius.MDMOperationTypeInstall, mobius.MDMDeliveryFailed)
				forceSetAppleHostDeclarationStatus(t, ds, host5.UUID, declD2, mobius.MDMOperationTypeInstall, mobius.MDMDeliveryFailed)
			},
			want: mobius.MDMConfigProfileStatus{Failed: 2},
		},
		{
			...
			setup: func(t *testing.T) {
				...
				forceSetWindowsHostProfileStatus(t, ds, host8.UUID, winW2, mobius.MDMOperationTypeInstall, mobius.MDMDeliveryPending)
			},
			want: mobius.MDMConfigProfileStatus{Pending: 1},
		},
	}
	for _, c := range cases {
		t.Run(c.desc, func(t *testing.T) {
...
func testDeleteMDMProfilesCancelsInstalls(t *testing.T, ds *Datastore) {
	...
	for _, h := range []*mobius.Host{host1, host2, host3, host4} {
		err = ds.SetOrUpdateMDMData(ctx, h.ID, false, true, "https://mobiusmdm.com", false, mobius.WellKnownMDMMobius, "")
		require.NoError(t, err)
	}

	// set the declaration as pending install on host1, installed on host2
	forceSetAppleHostDeclarationStatus(t, ds, host1.UUID, test.ToMDMAppleDecl(profNameToProf["D2"]), mobius.MDMOperationTypeInstall, "")
	forceSetAppleHostDeclarationStatus(t, ds, host2.UUID, test.ToMDMAppleDecl(profNameToProf["D2"]), mobius.MDMOperationTypeInstall, mobius.MDMDeliveryVerified)
	assertHostProfileOpStatus(t, ds, host1.UUID,
		hostProfileOpStatus{profNameToProf["D2"].ProfileUUID, mobius.MDMDeliveryPending, mobius.MDMOperationTypeInstall})
	assertHostProfileOpStatus(t, ds, host2.UUID,
		hostProfileOpStatus{profNameToProf["D2"].ProfileUUID, mobius.MDMDeliveryVerified, mobius.MDMOperationTypeInstall})
	...
	assertHostProfileOpStatus(t, ds, host1.UUID)
	assertHostProfileOpStatus(t, ds, host2.UUID,
		hostProfileOpStatus{profNameToProf["D2"].ProfileUUID, mobius.MDMDeliveryPending, mobius.MDMOperationTypeRemove})

	// set the Windows profile as pending install on host3, installed on host4
	forceSetWindowsHostProfileStatus(t, ds, host3.UUID, test.ToMDMWindowsConfigProfile(profNameToProf["W2"]), mobius.MDMOperationTypeInstall, mobius.MDMDeliveryPending)
	forceSetWindowsHostProfileStatus(t, ds, host4.UUID, test.ToMDMWindowsConfigProfile(profNameToProf["W2"]), mobius.MDMOperationTypeInstall, mobius.MDMDeliveryVerified)
	assertHostProfileOpStatus(t, ds, host3.UUID,
		hostProfileOpStatus{profNameToProf["W2"].ProfileUUID, mobius.MDMDeliveryPending, mobius.MDMOperationTypeInstall})
	assertHostProfileOpStatus(t, ds, host4.UUID,
		hostProfileOpStatus{profNameToProf["W2"].ProfileUUID, mobius.MDMDeliveryVerified, mobius.MDMOperationTypeInstall})
	...
	forceSetAppleHostProfileStatus(t, ds, host1.UUID, test.ToMDMAppleConfigProfile(profNameToProf["A2"]), mobius.MDMOperationTypeInstall, "")
	forceSetAppleHostProfileStatus(t, ds, host2.UUID, test.ToMDMAppleConfigProfile(profNameToProf["A2"]), mobius.MDMOperationTypeInstall, mobius.MDMDeliveryVerifying)
	...
	assertHostProfileOpStatus(t, ds, host1.UUID,
		hostProfileOpStatus{profNameToProf["A2"].ProfileUUID, mobius.MDMDeliveryPending, mobius.MDMOperationTypeInstall})
	assertHostProfileOpStatus(t, ds, host2.UUID,
		hostProfileOpStatus{profNameToProf["A2"].ProfileUUID, mobius.MDMDeliveryVerifying, mobius.MDMOperationTypeInstall},
		hostProfileOpStatus{profNameToProf["D2"].ProfileUUID, mobius.MDMDeliveryPending, mobius.MDMOperationTypeRemove})
	...
	assertHostProfileOpStatus(t, ds, host1.UUID)
	assertHostProfileOpStatus(t, ds, host2.UUID,
		hostProfileOpStatus{profNameToProf["A2"].ProfileUUID, mobius.MDMDeliveryPending, mobius.MDMOperationTypeRemove},
		hostProfileOpStatus{profNameToProf["D2"].ProfileUUID, mobius.MDMDeliveryPending, mobius.MDMOperationTypeRemove})
	...
	forceSetAppleHostProfileStatus(t, ds, host1.UUID, test.ToMDMAppleConfigProfile(profNameToProf["A3"]), mobius.MDMOperationTypeInstall, mobius.MDMDeliveryPending)
	// enqueue the corresponding command for the installed profile
	cmdUUID = uuid.New().String()
	...
	assertHostProfileOpStatus(t, ds, host1.UUID,
		hostProfileOpStatus{profNameToProf["A3"].ProfileUUID, mobius.MDMDeliveryPending, mobius.MDMOperationTypeInstall})
	assertHostProfileOpStatus(t, ds, host2.UUID,
		hostProfileOpStatus{profNameToProf["A2"].ProfileUUID, mobius.MDMDeliveryPending, mobius.MDMOperationTypeRemove},
		hostProfileOpStatus{profNameToProf["D2"].ProfileUUID, mobius.MDMDeliveryPending, mobius.MDMOperationTypeRemove})
	...
	assertHostProfileOpStatus(t, ds, host1.UUID,
		hostProfileOpStatus{profNameToProf["A3"].ProfileUUID, mobius.MDMDeliveryPending, mobius.MDMOperationTypeRemove})
	assertHostProfileOpStatus(t, ds, host2.UUID,
		hostProfileOpStatus{profNameToProf["A2"].ProfileUUID, mobius.MDMDeliveryPending, mobius.MDMOperationTypeRemove},
		hostProfileOpStatus{profNameToProf["D2"].ProfileUUID, mobius.MDMDeliveryPending, mobius.MDMOperationTypeRemove})
	...
	cmds, err := ds.ListMDMCommands(ctx, mobius.TeamFilter{
		User:            test.UserAdmin,
		IncludeObserver: true,
	}, &mobius.MDMCommandListOptions{Filters: mobius.MDMCommandFilters{HostIdentifier: host1.UUID}})
	require.NoError(t, err)
	require.Len(t, cmds, 0)
}
