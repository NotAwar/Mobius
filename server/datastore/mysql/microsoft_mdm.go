...
func (ds *Datastore) MDMWindowsGetEnrolledDeviceWithDeviceID(ctx context.Context, mdmDeviceID string) (*mobius.MDMWindowsEnrolledDevice, error) {
	stmt := `SELECT
		id,
		mdm_device_id,
		mdm_hardware_id,
	...
	var winMDMDevice mobius.MDMWindowsEnrolledDevice
...
// database.
func (ds *Datastore) MDMWindowsInsertEnrolledDevice(ctx context.Context, device *mobius.MDMWindowsEnrolledDevice) error {
	stmt := `
...
func (ds *Datastore) MDMWindowsInsertCommandForHosts(ctx context.Context, hostUUIDsOrDeviceIDs []string, cmd *mobius.MDMWindowsCommand) error {
...
func (ds *Datastore) mdmWindowsInsertCommandForHostsDB(ctx context.Context, tx sqlx.ExecerContext, hostUUIDsOrDeviceIDs []string, cmd *mobius.MDMWindowsCommand) error {
...
// given device ID.
func (ds *Datastore) MDMWindowsGetPendingCommands(ctx context.Context, deviceID string) ([]*mobius.MDMWindowsCommand, error) {
	var commands []*mobius.MDMWindowsCommand
...
func (ds *Datastore) MDMWindowsSaveResponse(ctx context.Context, deviceID string, enrichedSyncML mobius.EnrichedSyncML) error {
	...
	return ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		...
		var matchingCmds []mobius.MDMWindowsCommand
		...
		var (
			args                     []any
			sb                       strings.Builder
			potentialProfilePayloads []*mobius.MDMWindowsProfilePayload
		...
		for _, cmd := range matchingCmds {
			...
			if status, ok := enrichedSyncML.CmdRefUUIDToStatus[cmd.CommandUUID]; ok && status.Data != nil {
				...
				if status.Cmd != nil && *status.Cmd == mobius.CmdAtomic {
					// The raw MDM command may contain a $MOBIUS_SECRET_XXX, which should never be exposed or stored unencrypted.
					...
					pp, err := mobius.BuildMDMWindowsProfilePayloadFromMDMResponse(cmdWithSecret, enrichedSyncML.CmdRefUUIDToStatus,
...
func updateMDMWindowsHostProfileStatusFromResponseDB(
	ctx context.Context,
	tx sqlx.ExtContext,
	payloads []*mobius.MDMWindowsProfilePayload,
) error {
	if len(payloads) == 0 {
		return nil
	}
	...
	// command later on.
	uuidsToPayloads := make(map[string]*mobius.MDMWindowsProfilePayload, len(payloads))
	...
	var matchingHostProfiles []mobius.MDMWindowsProfilePayload
	...
	for _, hp := range matchingHostProfiles {
		...
		if payload.Status != nil && *payload.Status == mobius.MDMDeliveryFailed {
...
func (ds *Datastore) GetMDMWindowsCommandResults(ctx context.Context, commandUUID string) ([]*mobius.MDMCommandResult, error) {
	query := `
SELECT
    mwe.host_uuid,
    wmcr.command_uuid,
...
// - host_mdm: hmdm
// - host_disks: hd
func (ds *Datastore) whereBitLockerStatus(status mobius.DiskEncryptionStatus) string {
	const (
...
func (ds *Datastore) GetMDMWindowsBitLockerSummary(ctx context.Context, teamID *uint) (*mobius.MDMWindowsBitLockerSummary, error) {
...
WHERE
	...
	var res mobius.MDMWindowsBitLockerSummary
	stmt := fmt.Sprintf(
		sqlFmt,
...
func (ds *Datastore) GetMDMWindowsBitLockerStatus(ctx context.Context, host *mobius.Host) (*mobius.HostMDMDiskEncryption, error) {
	if host == nil {
...
WHERE
	...
	if err := sqlx.GetContext(ctx, ds.reader(ctx), &dest, stmt, host.ID); err != nil {
		...
		dest.Status = mobius.DiskEncryptionEnforcing
	}
	...
	if dest.Status == "" {
		...
		dest.Status = mobius.DiskEncryptionFailed
	}
...
func (ds *Datastore) GetMDMWindowsConfigProfile(ctx context.Context, profileUUID string) (*mobius.MDMWindowsConfigProfile, error) {
	stmt := `
SELECT
	profile_uuid,
	team_id,
...
WHERE
	...
	var res mobius.MDMWindowsConfigProfile
...
func subqueryHostsMDMWindowsOSSettingsStatusFailed() (string, []interface{}, error) {
	...
	args := []interface{}{
		...
		mdm.ListMobiusReservedWindowsProfileNames(),
	}
...
func subqueryHostsMDMWindowsOSSettingsStatusVerifying() (string, []interface{}, error) {
	...
	args := []interface{}{
		...
		mobius.MDMOperationTypeInstall,
		mdm.ListMobiusReservedWindowsProfileNames(),
		[]interface{}{mobius.MDMDeliveryVerifying, mobius.MDMDeliveryVerified},
	}
	return sqlx.In(sql, args...)
}
...
func (ds *Datastore) GetMDMWindowsProfilesSummary(ctx context.Context, teamID *uint) (*mobius.MDMProfilesSummary, error) {
...
func (ds *Datastore) ListMDMWindowsProfilesToInstall(ctx context.Context) ([]*mobius.MDMWindowsProfilePayload, error) {
	var result []*mobius.MDMWindowsProfilePayload
...
func (ds *Datastore) listAllMDMWindowsProfilesToInstallDB(ctx context.Context, tx sqlx.ExtContext) ([]*mobius.MDMWindowsProfilePayload, error) {
	var profiles []*mobius.MDMWindowsProfilePayload
	err := sqlx.SelectContext(ctx, tx, &profiles, fmt.Sprintf(windowsProfilesToInstallQuery, "TRUE", "TRUE", "TRUE", "TRUE"), mobius.MDMOperationTypeInstall)
	if err != nil {
...
func (ds *Datastore) listMDMWindowsProfilesToInstallDB(
...
) (profiles []*mobius.MDMWindowsProfilePayload, err error) {
	if len(hostUUIDs) == 0 {
		return profiles, nil
	}
	...
	for i := range selectProfilesTotalBatches {
		...
		if len(onlyProfileUUIDs) > 0 {
			...
			stmt, args, err = sqlx.In(toInstallQuery, batchUUIDs, batchUUIDs, batchUUIDs, batchUUIDs, mobius.MDMOperationTypeInstall)
		}
		if err != nil {
		...
		var partialResult []*mobius.MDMWindowsProfilePayload
...
func (ds *Datastore) ListMDMWindowsProfilesToRemove(ctx context.Context) ([]*mobius.MDMWindowsProfilePayload, error) {
	var result []*mobius.MDMWindowsProfilePayload
	err := ds.withTx(ctx, func(tx sqlx.ExtContext) error {
		var err error
...
func (ds *Datastore) listAllMDMWindowsProfilesToRemoveDB(ctx context.Context, tx sqlx.ExtContext) (profiles []*mobius.MDMWindowsProfilePayload, err error) {
...
func (ds *Datastore) BulkUpsertMDMWindowsHostProfiles(ctx context.Context, payload []*mobius.MDMWindowsBulkUpsertHostProfilePayload) error {
	if len(payload) == 0 {
		return nil
	}
...
func (ds *Datastore) GetMDMWindowsProfilesContents(ctx context.Context, uuids []string) (map[string]mobius.MDMWindowsProfileContents, error) {
	if len(uuids) == 0 {
		return nil, nil
	}
	...
	results := make(map[string]mobius.MDMWindowsProfileContents, len(profs))
	for _, p := range profs {
		results[p.ProfileUUID] = mobius.MDMWindowsProfileContents{
			SyncML:   p.SyncML,
			Checksum: p.Checksum,
		}
	}
...
func (ds *Datastore) BulkDeleteMDMWindowsHostsConfigProfiles(ctx context.Context, profs []*mobius.MDMWindowsProfilePayload) error {
...
func (ds *Datastore) bulkDeleteMDMWindowsHostsConfigProfilesDB(
	ctx context.Context,
	tx sqlx.ExtContext,
	profs []*mobius.MDMWindowsProfilePayload,
) error {
	if len(profs) == 0 {
		return nil
	}
...
func (ds *Datastore) NewMDMWindowsConfigProfile(ctx context.Context, cp mobius.MDMWindowsConfigProfile) (*mobius.MDMWindowsConfigProfile, error) {
...
func (ds *Datastore) SetOrUpdateMDMWindowsConfigProfile(ctx context.Context, cp mobius.MDMWindowsConfigProfile) error {
	profileUUID := "w" + uuid.New().String()
	stmt := `
INSERT INTO
	mdm_windows_configuration_profiles (profile_uuid, team_id, name, syncml, uploaded_at)
...
`
	...
	// or processing
	incomingProfs := make(map[string]*mobius.MDMWindowsConfigProfile, len(profiles))
	for i, p := range profiles {
		incomingNames[i] = p.Name
		incomingProfs[p.Name] = p
	}

	var existingProfiles []*mobius.MDMWindowsConfigProfile
	...
	for n := range mdm.MobiusReservedProfileNames() {
	...
	var profsWithoutLabel []string
	if len(incomingNames) > 0 {
		var newlyInsertedProfs []*mobius.MDMWindowsConfigProfile
...
func (ds *Datastore) bulkSetPendingMDMWindowsHostProfilesDB(
	...
	var (
		pargs            []any
		profilesToInsert = make(map[string]*mobius.MDMWindowsProfilePayload)
		psb              strings.Builder
		batchCount       int
	)
	...
	executeUpsertBatch := func(valuePart string, args []any) error {
		...
		var existingProfiles []mobius.MDMWindowsProfilePayload
	...
	for _, p := range profilesToInstall {
		profilesToInsert[fmt.Sprintf("%s\n%s", p.ProfileUUID, p.HostUUID)] = &mobius.MDMWindowsProfilePayload{
		...
		pargs = append(
			...
			mobius.MDMOperationTypeInstall, p.Checksum)
...
func (ds *Datastore) GetHostMDMWindowsProfiles(ctx context.Context, hostUUID string) ([]mobius.HostMDMWindowsProfile, error) {
	stmt := fmt.Sprintf(`
SELECT
	profile_uuid,
...
host_uuid = ? AND profile_name NOT IN(?) AND NOT (operation_type = '%s' AND COALESCE(status, '%s') IN('%s', '%s'))`,
	...
	stmt, args, err := sqlx.In(stmt, hostUUID, mdm.ListMobiusReservedWindowsProfileNames())
	if err != nil {
		return nil, ctxerr.Wrap(ctx, err, "building in statement")
	}

	var profiles []mobius.HostMDMWindowsProfile
...
func (ds *Datastore) WipeHostViaWindowsMDM(ctx context.Context, host *mobius.Host, cmd *mobius.MDMWindowsCommand) error {
	return ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		...
		if _, err := tx.ExecContext(ctx, stmt, host.ID, cmd.CommandUUID, host.MobiusPlatform()); err != nil {
