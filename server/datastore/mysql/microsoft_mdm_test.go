...
func testMDMWindowsDiskEncryption(t *testing.T, ds *Datastore) {
	...
	checkBitLockerSummary := func(t *testing.T, teamID *uint, expected mobius.MDMWindowsBitLockerSummary) {
	...
	checkMDMProfilesSummary := func(t *testing.T, teamID *uint, expected mobius.MDMProfilesSummary) {
	...
	checkListHostsFilterOSSettings := func(t *testing.T, teamID *uint, status mobius.OSSettingsStatus, expectedIDs []uint) {
		gotHosts, err := ds.ListHosts(ctx, mobius.TeamFilter{User: test.UserAdmin}, mobius.HostListOptions{TeamFilter: teamID, OSSettingsFilter: status})
		require.NoError(t, err)
	...
	checkListHostsFilterDiskEncryption := func(t *testing.T, teamID *uint, status mobius.DiskEncryptionStatus, expectedIDs []uint) {
		gotHosts, err := ds.ListHosts(ctx, mobius.TeamFilter{User: test.UserAdmin}, mobius.HostListOptions{TeamFilter: teamID, OSSettingsDiskEncryptionFilter: status})
		require.NoError(t, err)
		...
		count, err := ds.CountHosts(ctx, mobius.TeamFilter{User: test.UserAdmin}, mobius.HostListOptions{TeamFilter: teamID, OSSettingsDiskEncryptionFilter: status})
		require.NoError(t, err)
		require.Equal(t, len(expectedIDs), count, fmt.Sprintf("status: %s", status))
	}

	checkHostBitLockerStatus := func(t *testing.T, expected mobius.DiskEncryptionStatus, hostIDs []uint) {
	...
	type hostIDsByDEStatus map[mobius.DiskEncryptionStatus][]uint
	...
	expectedProfilesFromDE := func(expectedDE hostIDsByDEStatus) hostIDsByProfileStatus {
		...
		expectedProfiles[mobius.MDMDeliveryPending] = []uint{}
		for status, hostIDs := range expectedDE {
			switch status {
			case mobius.DiskEncryptionVerified:
				expectedProfiles[mobius.MDMDeliveryVerified] = hostIDs
			case mobius.DiskEncryptionVerifying:
				expectedProfiles[mobius.MDMDeliveryVerifying] = hostIDs
			case mobius.DiskEncryptionFailed:
				expectedProfiles[mobius.MDMDeliveryFailed] = hostIDs
			case mobius.DiskEncryptionEnforcing, mobius.DiskEncryptionRemovingEnforcement, mobius.DiskEncryptionActionRequired:
				expectedProfiles[mobius.MDMDeliveryPending] = append(expectedProfiles[mobius.MDMDeliveryPending], hostIDs...)
			}
		}
	...
	checkExpected := func(t *testing.T, teamID *uint, expectedDE hostIDsByDEStatus, expectedProfiles ...hostIDsByProfileStatus) {
		...
		for _, status := range []mobius.DiskEncryptionStatus{
		...
		checkBitLockerSummary(t, teamID, mobius.MDMWindowsBitLockerSummary{
			Verified:            uint(len(expectedDE[mobius.DiskEncryptionVerified])),
			Verifying:           uint(len(expectedDE[mobius.DiskEncryptionVerifying])),
			Failed:              uint(len(expectedDE[mobius.DiskEncryptionFailed])),
			Enforcing:           uint(len(expectedDE[mobius.DiskEncryptionEnforcing])),
			RemovingEnforcement: uint(len(expectedDE[mobius.DiskEncryptionRemovingEnforcement])),
			ActionRequired:      uint(len(expectedDE[mobius.DiskEncryptionActionRequired])),
		})

		checkMDMProfilesSummary(t, teamID, mobius.MDMProfilesSummary{
			Pending:   uint(len(ep[mobius.MDMDeliveryPending])),
			Failed:    uint(len(ep[mobius.MDMDeliveryFailed])),
			Verifying: uint(len(ep[mobius.MDMDeliveryVerifying])),
			Verified:  uint(len(ep[mobius.MDMDeliveryVerified])),
		})

		checkListHostsFilterOSSettings(t, teamID, mobius.OSSettingsVerified, ep[mobius.MDMDeliveryVerified])
		checkListHostsFilterOSSettings(t, teamID, mobius.OSSettingsVerifying, ep[mobius.MDMDeliveryVerifying])
		checkListHostsFilterOSSettings(t, teamID, mobius.OSSettingsFailed, ep[mobius.MDMDeliveryFailed])
		checkListHostsFilterOSSettings(t, teamID, mobius.OSSettingsPending, ep[mobius.MDMDeliveryPending])
	}
	...
	for i := 0; i < 10; i++ {
		...
		require.NoError(t, ds.SetOrUpdateMDMData(ctx, h.ID, false, true, "https://example.com", false, mobius.WellKnownMDMMobius, ""))
	...
	t.Run("Disk encryption enabled", func(t *testing.T) {
		...
		t.Run("Bitlocker enforcing-verifying-verified", func(t *testing.T) {
			...
			checkExpected(t, nil, hostIDsByDEStatus{
				mobius.DiskEncryptionEnforcing: []uint{hosts[0].ID, hosts[1].ID, hosts[2].ID, hosts[3].ID, hosts[4].ID},
			})
			...
			checkExpected(t, nil, hostIDsByDEStatus{
				mobius.DiskEncryptionVerified:  []uint{hosts[0].ID},
				mobius.DiskEncryptionEnforcing: []uint{hosts[1].ID, hosts[2].ID, hosts[3].ID, hosts[4].ID},
			})
			...
			cases := []struct {
				...
				expectedWithinGracePeriod  mobius.DiskEncryptionStatus
				expectedOutsideGracePeriod mobius.DiskEncryptionStatus
			}{
				{
					name:                       "encrypted reported after key",
					hostDisksEncrypted:         true,
					reportedAfterKey:           true,
					expectedWithinGracePeriod:  mobius.DiskEncryptionVerified,
					expectedOutsideGracePeriod: mobius.DiskEncryptionVerified,
				},
				{
					name:                       "encrypted reported before key",
					hostDisksEncrypted:         true,
					reportedAfterKey:           false,
					expectedWithinGracePeriod:  mobius.DiskEncryptionVerifying,
					expectedOutsideGracePeriod: mobius.DiskEncryptionVerifying,
				},
				{
					name:                       "not encrypted reported before key",
					hostDisksEncrypted:         false,
					reportedAfterKey:           false,
					expectedWithinGracePeriod:  mobius.DiskEncryptionEnforcing,
					expectedOutsideGracePeriod: mobius.DiskEncryptionEnforcing,
				},
				{
			...
			for _, c := range cases {
				t.Run(c.name, func(t *testing.T) {
					...
					t.Run("within grace period", func(t *testing.T) {
						...
						if c.expectedWithinGracePeriod == mobius.DiskEncryptionEnforcing {
							expected[mobius.DiskEncryptionEnforcing] = append([]uint{testHostID}, otherWindowsHostIDs...)
						} else {
							expected[c.expectedWithinGracePeriod] = []uint{testHostID}
							expected[mobius.DiskEncryptionEnforcing] = otherWindowsHostIDs
						}
					...
					t.Run("outside grace period", func(t *testing.T) {
						expected := make(hostIDsByDEStatus)
						if c.expectedOutsideGracePeriod == mobius.DiskEncryptionEnforcing {
							expected[mobius.DiskEncryptionEnforcing] = append([]uint{testHostID}, otherWindowsHostIDs...)
						} else {
		...
		t.Run("BitLocker failed status", func(t *testing.T) {
			...
			expected := hostIDsByDEStatus{
				mobius.DiskEncryptionVerified:  []uint{hosts[0].ID},
				mobius.DiskEncryptionFailed:    []uint{hosts[1].ID},
				mobius.DiskEncryptionEnforcing: []uint{hosts[2].ID, hosts[3].ID, hosts[4].ID},
			}

			checkExpected(t, nil, expected)

			// bitlocker failed status determines MDM aggregate status (profiles status is ignored)
			upsertHostProfileStatus(t, hosts[1].UUID, "some-windows-profile", mobius.MDMDeliveryFailed)
			checkExpected(t, nil, expected)
			upsertHostProfileStatus(t, hosts[1].UUID, "some-windows-profile", mobius.MDMDeliveryPending)
			checkExpected(t, nil, expected)
			upsertHostProfileStatus(t, hosts[1].UUID, "some-windows-profile", mobius.MDMDeliveryVerifying)
			checkExpected(t, nil, expected)
			upsertHostProfileStatus(t, hosts[1].UUID, "some-windows-profile", mobius.MDMDeliveryVerified)
			checkExpected(t, nil, expected)

			// profiles failed status determines MDM aggregate status (bitlocker status is ignored)
			upsertHostProfileStatus(t, hosts[0].UUID, "some-windows-profile", mobius.MDMDeliveryFailed)
			expectedProfiles := expectedProfilesFromDE(expected)
			expectedProfiles[mobius.MDMDeliveryFailed] = append(expectedProfiles[mobius.MDMDeliveryFailed], hosts[0].ID)
			expectedProfiles[mobius.MDMDeliveryVerified] = []uint{}
		...
		t.Run("BitLocker team filtering", func(t *testing.T) {
			// Test team filtering
			team, err := ds.NewTeam(ctx, &mobius.Team{Name: "team"})
			require.NoError(t, err)
			...
			checkExpected(t, nil, hostIDsByDEStatus{
				mobius.DiskEncryptionVerified:  []uint{hosts[0].ID},
				mobius.DiskEncryptionFailed:    []uint{hosts[1].ID},
				mobius.DiskEncryptionEnforcing: []uint{hosts[3].ID, hosts[4].ID}, // hosts[2] is no longer included in the no team summary
			})
			...
			checkExpected(t, &team.ID, hostIDsByDEStatus{
				mobius.DiskEncryptionEnforcing: []uint{hosts[2].ID}, // disk encryption is enabled for team so hosts[2] is counted
			})
			...
			checkExpected(t, nil, hostIDsByDEStatus{
				mobius.DiskEncryptionVerified:  []uint{hosts[0].ID},
				mobius.DiskEncryptionFailed:    []uint{hosts[1].ID},
				mobius.DiskEncryptionEnforcing: []uint{hosts[3].ID, hosts[4].ID},
			})
		})
		...
		t.Run("BitLocker Windows server excluded", func(t *testing.T) {
			require.NoError(t, ds.SetOrUpdateMDMData(ctx,
				...
				true, "https://example.com", false, mobius.WellKnownMDMMobius, ""))

			// Check Windows servers not counted
			checkExpected(t, nil, hostIDsByDEStatus{
				mobius.DiskEncryptionVerified:  []uint{hosts[0].ID},
				mobius.DiskEncryptionFailed:    []uint{hosts[1].ID},
				mobius.DiskEncryptionEnforcing: []uint{hosts[4].ID}, // hosts[3] is not counted
			})
		})
		...
		t.Run("OS settings filters include Windows and macOS hosts", func(t *testing.T) {
			...
			checkBitLockerSummary(t, nil, mobius.MDMWindowsBitLockerSummary{
				Verified:            1,
				Verifying:           0,
				Failed:              1,
				Enforcing:           1,
			...
			checkListHostsFilterDiskEncryption(t, nil, mobius.DiskEncryptionFailed, []uint{hosts[1].ID, hosts[5].ID})
			checkListHostsFilterOSSettings(t, nil, mobius.OSSettingsFailed, []uint{hosts[1].ID, hosts[5].ID})

			// delete the macOS host profile
			ExecAdhocSQL(t, ds, func(q sqlx.ExtContext) error {
				_, err := q.ExecContext(ctx, `DELETE FROM host_mdm_apple_profiles WHERE host_uuid = ? AND profile_identifier = ?`, hosts[5].UUID, mobileconfig.MobiusFileVaultPayloadIdentifier)
				return err
			})
		})
		...
		t.Run("BitLocker host disks must update to transition from Verifying to Verified", func(t *testing.T) {
			...
			checkExpected(t, nil, hostIDsByDEStatus{
				mobius.DiskEncryptionVerified:  []uint{hosts[0].ID},
				mobius.DiskEncryptionFailed:    []uint{hosts[1].ID},
				mobius.DiskEncryptionEnforcing: []uint{targetHost.ID}, // targetHost is initially enforcing
			})
			...
			checkExpected(t, nil, hostIDsByDEStatus{
				mobius.DiskEncryptionVerified:  []uint{hosts[0].ID},
				mobius.DiskEncryptionFailed:    []uint{hosts[1].ID},
				mobius.DiskEncryptionVerifying: []uint{targetHost.ID},
			})
			...
			checkExpected(t, nil, hostIDsByDEStatus{
				mobius.DiskEncryptionVerified: []uint{hosts[0].ID, targetHost.ID},
				mobius.DiskEncryptionFailed:   []uint{hosts[1].ID},
			})
		})
	})
...
func testMDMWindowsProfilesSummary(t *testing.T, ds *Datastore) {
	...
	checkListHostsFilterOSSettings := func(t *testing.T, teamID *uint, status mobius.OSSettingsStatus, expectedIDs []uint) {
		...
		count, err := ds.CountHosts(ctx, mobius.TeamFilter{User: test.UserAdmin}, mobius.HostListOptions{TeamFilter: teamID, OSSettingsFilter: status})
		require.NoError(t, err)
	...
	t.Run("profiles summary accounts for bitlocker status", func(t *testing.T) {
		t.Run("bitlocker disabled", func(t *testing.T) {
			...
			upsertHostProfileStatus(t, hosts[0].UUID, "some-windows-profile", &mobius.MDMDeliveryPending)
			expected[mobius.MDMDeliveryPending] = []uint{hosts[0].ID}
			checkExpected(t, nil, expected)

			upsertHostProfileStatus(t, hosts[1].UUID, "some-windows-profile", &mobius.MDMDeliveryFailed)
			expected[mobius.MDMDeliveryFailed] = []uint{hosts[1].ID}
			checkExpected(t, nil, expected)

			upsertHostProfileStatus(t, hosts[2].UUID, "some-windows-profile", &mobius.MDMDeliveryVerifying)
			expected[mobius.MDMDeliveryVerifying] = []uint{hosts[2].ID}
			checkExpected(t, nil, expected)

			upsertHostProfileStatus(t, hosts[3].UUID, "some-windows-profile", &mobius.MDMDeliveryVerified)
			expected[mobius.MDMDeliveryVerified] = []uint{hosts[3].ID}
			checkExpected(t, nil, expected)

			upsertHostProfileStatus(t, hosts[4].UUID, "some-windows-profile", nil)
			// nil status is treated as pending
			expected[mobius.MDMDeliveryPending] = append(expected[mobius.MDMDeliveryPending], hosts[4].ID)
			checkExpected(t, nil, expected)
		...
		t.Run("bitlocker enabled", func(t *testing.T) {
			...
			t.Run("bitlocker pending", func(t *testing.T) {
				expected := hostIDsByProfileStatus{
					mobius.MDMDeliveryPending: []uint{hosts[0].ID, hosts[1].ID, hosts[2].ID, hosts[3].ID, hosts[4].ID},
				}
				...
				expected[mobius.MDMDeliveryFailed] = []uint{hosts[1].ID}
				expected[mobius.MDMDeliveryPending] = []uint{hosts[0].ID, hosts[2].ID, hosts[3].ID, hosts[4].ID}
				checkExpected(t, nil, expected)
			...
			t.Run("bitlocker verifying", func(t *testing.T) {
				...
				// no profiles
				expected[mobius.MDMDeliveryVerifying] = []uint{hosts[0].ID}
				expected[mobius.MDMDeliveryPending] = []uint{hosts[1].ID, hosts[2].ID, hosts[3].ID, hosts[4].ID}
				checkExpected(t, nil, expected)

				upsertHostProfileStatus(t, hosts[0].UUID, "some-windows-profile", &mobius.MDMDeliveryFailed)
				// status for hosts[0] now failed because any failed status takes precedence
				expected = hostIDsByProfileStatus{
					mobius.MDMDeliveryFailed:    []uint{hosts[0].ID},
					mobius.MDMDeliveryVerifying: []uint{},
					mobius.MDMDeliveryPending:   []uint{hosts[1].ID, hosts[2].ID, hosts[3].ID, hosts[4].ID},
				}
				checkExpected(t, nil, expected)

				upsertHostProfileStatus(t, hosts[0].UUID, "some-windows-profile", &mobius.MDMDeliveryPending)
				// status for hosts[0] now pending because profiles status pendiing takes precedence over bitlocker status verifying
				expected[mobius.MDMDeliveryFailed] = []uint{}
				expected[mobius.MDMDeliveryPending] = []uint{hosts[0].ID, hosts[1].ID, hosts[2].ID, hosts[3].ID, hosts[4].ID}
				checkExpected(t, nil, expected)

				upsertHostProfileStatus(t, hosts[0].UUID, "some-windows-profile", &mobius.MDMDeliveryVerifying)
				// status for hosts[0] now verifying because both profiles status and bitlocker status are verifying
				expected[mobius.MDMDeliveryPending] = []uint{hosts[1].ID, hosts[2].ID, hosts[3].ID, hosts[4].ID}
				expected[mobius.MDMDeliveryVerifying] = []uint{hosts[0].ID}
				checkExpected(t, nil, expected)

				upsertHostProfileStatus(t, hosts[0].UUID, "some-windows-profile", &mobius.MDMDeliveryVerified)
				...
				expected[mobius.MDMDeliveryVerifying] = []uint{}
				expected[mobius.MDMDeliveryPending] = []uint{hosts[0].ID, hosts[1].ID, hosts[2].ID, hosts[3].ID, hosts[4].ID}
				checkExpected(t, nil, expected)
			...
			t.Run("bitlocker verified", func(t *testing.T) {
				...
				checkExpected(t, nil, hostIDsByProfileStatus{
					mobius.MDMDeliveryVerified: []uint{hosts[0].ID},
					mobius.MDMDeliveryPending:  []uint{hosts[1].ID, hosts[2].ID, hosts[3].ID, hosts[4].ID},
				})
				...
				expected = hostIDsByProfileStatus{
					mobius.MDMDeliveryFailed:   []uint{hosts[0].ID},
					mobius.MDMDeliveryVerified: []uint{},
					mobius.MDMDeliveryPending:  []uint{hosts[1].ID, hosts[2].ID, hosts[3].ID, hosts[4].ID},
				}
				checkExpected(t, nil, expected)
				...
				expected[mobius.MDMDeliveryPending] = []uint{hosts[1].ID, hosts[2].ID, hosts[3].ID, hosts[4].ID}
				expected[mobius.MDMDeliveryVerified] = []uint{hosts[0].ID}
				expected[mobius.MDMDeliveryVerifying] = []uint{}
				checkExpected(t, nil, expected)
			...
			t.Run("BitLocker host disks must update to transition from Verifying to Verified", func(t *testing.T) {
				...
				checkExpected(t, nil, hostIDsByProfileStatus{
					mobius.MDMDeliveryVerifying: []uint{hosts[0].ID},
					mobius.MDMDeliveryPending:   []uint{hosts[1].ID, hosts[2].ID, hosts[3].ID, hosts[4].ID},
				})
	...
	t.Run("profiles summary accounts for host profiles with mixed statuses", func(t *testing.T) {
		for i := 0; i < 5; i++ {
			...
			upsertHostProfileStatus(t, hosts[1].UUID, fmt.Sprintf("some-windows-profile-%d", i), &mobius.MDMDeliveryPending)
			// upsert five profiles for hosts[2] with verifying statuses
			upsertHostProfileStatus(t, hosts[2].UUID, fmt.Sprintf("some-windows-profile-%d", i), &mobius.MDMDeliveryVerifying)
			// upsert five profiles for hosts[3] with verified statuses
			upsertHostProfileStatus(t, hosts[3].UUID, fmt.Sprintf("some-windows-profile-%d", i), &mobius.MDMDeliveryVerified)
			// upsert five profiles for hosts[4] with failed statuses
			upsertHostProfileStatus(t, hosts[4].UUID, fmt.Sprintf("some-windows-profile-%d", i), &mobius.MDMDeliveryFailed)
		}

		expected := hostIDsByProfileStatus{
			mobius.MDMDeliveryPending:   []uint{hosts[0].ID, hosts[1].ID},
			mobius.MDMDeliveryVerifying: []uint{hosts[2].ID},
			mobius.MDMDeliveryVerified:  []uint{hosts[3].ID},
			mobius.MDMDeliveryFailed:    []uint{hosts[4].ID},
		}
		checkExpected(t, nil, expected)

		// add some other windows hosts that won't be be assigned any profiles
		otherHosts := make([]*mobius.Host, 0, 5)
		for i := 0; i < 5; i++ {
			u := uuid.New().String()
			h, err := ds.NewHost(ctx, &mobius.Host{
				DetailUpdatedAt: time.Now(),
		...
		for i := 0; i < 5; i++ {
			upsertHostProfileStatus(t, hosts[i].UUID, "some-windows-profile-0", &mobius.MDMDeliveryFailed)
		}
		expected = hostIDsByProfileStatus{
			mobius.MDMDeliveryPending:   []uint{},
			mobius.MDMDeliveryVerifying: []uint{},
			mobius.MDMDeliveryVerified:  []uint{},
			mobius.MDMDeliveryFailed:    []uint{hosts[0].ID, hosts[1].ID, hosts[2].ID, hosts[3].ID, hosts[4].ID},
		}
		checkExpected(t, nil, expected)

		// upsert some-profile-0 to pending status for hosts[0:4]
		for i := 0; i < 5; i++ {
			upsertHostProfileStatus(t, hosts[i].UUID, "some-windows-profile-0", &mobius.MDMDeliveryPending)
		}
		expected = hostIDsByProfileStatus{
			mobius.MDMDeliveryPending:   []uint{hosts[0].ID, hosts[1].ID, hosts[2].ID, hosts[3].ID},
			mobius.MDMDeliveryVerifying: []uint{},
			mobius.MDMDeliveryVerified:  []uint{},
			mobius.MDMDeliveryFailed:    []uint{hosts[4].ID},
		}
		...
		for i := 0; i < 5; i++ {
			upsertHostProfileStatus(t, hosts[i].UUID, "some-windows-profile-0", &mobius.MDMDeliveryVerifying)
		}
		expected = hostIDsByProfileStatus{
			mobius.MDMDeliveryPending:   []uint{hosts[0].ID, hosts[1].ID},
			mobius.MDMDeliveryVerifying: []uint{hosts[2].ID, hosts[3].ID},
			mobius.MDMDeliveryVerified:  []uint{},
			mobius.MDMDeliveryFailed:    []uint{hosts[4].ID},
		}
		checkExpected(t, nil, expected)
		...
		for i := 0; i < 5; i++ {
			upsertHostProfileStatus(t, hosts[i].UUID, "some-windows-profile-0", &mobius.MDMDeliveryVerified)
		}
		expected = hostIDsByProfileStatus{
			mobius.MDMDeliveryPending:   []uint{hosts[0].ID, hosts[1].ID},
			mobius.MDMDeliveryVerifying: []uint{hosts[2].ID},
		...
		expected = hostIDsByProfileStatus{
			mobius.MDMDeliveryPending:   []uint{hosts[0].ID, hosts[1].ID, hosts[2].ID, hosts[3].ID, otherHosts[0].ID, otherHosts[1].ID, otherHosts[2].ID, otherHosts[3].ID, otherHosts[4].ID},
			mobius.MDMDeliveryVerifying: []uint{},
			mobius.MDMDeliveryVerified:  []uint{},
			mobius.MDMDeliveryFailed:    []uint{hosts[4].ID},
		}
		...
		// create a new team
		t1, err := ds.NewTeam(ctx, &mobius.Team{Name: uuid.NewString()})
		require.NoError(t, err)
		require.NotNil(t, t1)
		...
		expectedTeam1 := hostIDsByProfileStatus{
			mobius.MDMDeliveryPending:   []uint{hosts[1].ID},
			mobius.MDMDeliveryVerifying: []uint{hosts[2].ID},
		}
		checkExpected(t, &t1.ID, expectedTeam1)
		// hosts[1:2] are not counted for no team
		expected = hostIDsByProfileStatus{
			mobius.MDMDeliveryPending: []uint{hosts[0].ID, hosts[3].ID, otherHosts[0].ID, otherHosts[1].ID, otherHosts[2].ID, otherHosts[3].ID, otherHosts[4].ID},
			mobius.MDMDeliveryFailed:  []uint{hosts[4].ID},
		}
		checkExpected(t, nil, expected)

		// report otherHosts[0] as a server
		require.NoError(t, ds.SetOrUpdateMDMData(ctx, otherHosts[0].ID, true, true, "https://example.com", false, mobius.WellKnownMDMMobius, ""))
		// otherHosts[0] is no longer counted
		expected = hostIDsByProfileStatus{
			mobius.MDMDeliveryPending: []uint{hosts[0].ID, hosts[3].ID, otherHosts[1].ID, otherHosts[2].ID, otherHosts[3].ID, otherHosts[4].ID},
			mobius.MDMDeliveryFailed:  []uint{hosts[4].ID},
		}
		checkExpected(t, nil, expected)

		// report hosts[0] as a server
		require.NoError(t, ds.SetOrUpdateMDMData(ctx, hosts[0].ID, true, true, "https://example.com", false, mobius.WellKnownMDMMobius, ""))
		// hosts[0] is no longer counted
		expected = hostIDsByProfileStatus{
			mobius.MDMDeliveryPending: []uint{hosts[3].ID, otherHosts[1].ID, otherHosts[2].ID, otherHosts[3].ID, otherHosts[4].ID},
			mobius.MDMDeliveryFailed:  []uint{hosts[4].ID},
		}
		checkExpected(t, nil, expected)
		...
		expected = hostIDsByProfileStatus{
			mobius.MDMDeliveryPending: []uint{otherHosts[1].ID, otherHosts[2].ID, otherHosts[3].ID, otherHosts[4].ID},
			mobius.MDMDeliveryFailed:  []uint{hosts[4].ID},
		}
		checkExpected(t, nil, expected)
...
func testMDMWindowsInsertCommandForHosts(t *testing.T, ds *Datastore) {
	...
	d1 := &mobius.MDMWindowsEnrolledDevice{
		MDMDeviceID:            uuid.New().String(),
		MDMHardwareID:          uuid.New().String() + uuid.New().String(),
		MDMDeviceState:         microsoft_mdm.MDMDeviceStateEnrolled,
		MDMDeviceType:          "CIMClient_Windows",
	...
	d2 := &mobius.MDMWindowsEnrolledDevice{
		MDMDeviceID:            uuid.New().String(),
		MDMHardwareID:          uuid.New().String() + uuid.New().String(),
		MDMDeviceState:         microsoft_mdm.MDMDeviceStateEnrolled,
		MDMDeviceType:          "CIMClient_Windows",
	...
	d3 := &mobius.MDMWindowsEnrolledDevice{
...
func testMDMWindowsGetPendingCommands(t *testing.T, ds *Datastore) {
	ctx := context.Background()
	d := &mobius.MDMWindowsEnrolledDevice{
		MDMDeviceID:            uuid.New().String(),
		MDMHardwareID:          uuid.New().String() + uuid.New().String(),
		MDMDeviceState:         microsoft_mdm.MDMDeviceStateEnrolled,
		MDMDeviceType:          "CIMClient_Windows",
...
func testMDMWindowsCommandResults(t *testing.T, ds *Datastore) {
	...
	dev := &mobius.MDMWindowsEnrolledDevice{
...
func windowsEnroll(t *testing.T, ds mobius.Datastore, h *mobius.Host) string {
	ctx := context.Background()
	d1 := &mobius.MDMWindowsEnrolledDevice{
		MDMDeviceID:            uuid.New().String(),
		MDMHardwareID:          uuid.New().String() + uuid.New().String(),
...
func testMDMWindowsProfileManagement(t *testing.T, ds *Datastore) {
	...
	// a windows host that's not MDM enrolled into Mobius shouldn't
	...
	profilesMatch := func(t *testing.T, want []string, profs []*mobius.MDMWindowsProfilePayload) {
		got := []string{}
	...
	profileByUUID := make(map[string]*mobius.MDMWindowsProfilePayload, len(profiles))
	...
	err = ds.BulkUpsertMDMWindowsHostProfiles(
		ctx, []*mobius.MDMWindowsBulkUpsertHostProfilePayload{
			{
...
func testBulkOperationsMDMWindowsHostProfiles(t *testing.T, ds *Datastore) {
	...
	getAllHostProfiles := func() []*mobius.MDMWindowsProfilePayload {
		var hostProfiles []*mobius.MDMWindowsProfilePayload
		ExecAdhocSQL(t, ds, func(q sqlx.ExtContext) error {
	...
	err := ds.BulkUpsertMDMWindowsHostProfiles(ctx, []*mobius.MDMWindowsBulkUpsertHostProfilePayload{})
	require.NoError(t, err)
	...
	for i, p := range hostsProfs {
		...
		require.Equal(t, mobius.MDMOperationTypeInstall, p.OperationType)
		require.Equal(t, &mobius.MDMDeliveryVerifying, p.Status)
	}
	...
	for i, p := range hostsProfs {
		require.Equal(t, profiles[i], p.ProfileUUID)
		require.Equal(t, mobius.MDMOperationTypeInstall, p.OperationType)
		require.Equal(t, &mobius.MDMDeliveryVerified, p.Status)
	}

	// empty payload
	err = ds.BulkDeleteMDMWindowsHostsConfigProfiles(ctx, []*mobius.MDMWindowsProfilePayload{})
	require.NoError(t, err)
	hostsProfs = getAllHostProfiles()
	require.Len(t, hostsProfs, 5)
...
func testGetMDMWindowsProfilesContents(t *testing.T, ds *Datastore) {
	...
	cases := []struct {
		ids  []string
		want map[string]mobius.MDMWindowsProfileContents
	}{
		{[]string{}, nil},
		{nil, nil},
		{
			[]string{profileUUIDs[0]},
			map[string]mobius.MDMWindowsProfileContents{profileUUIDs[0]: generateDummyWindowsProfileContents(profileUUIDs[0])},
		},
		{
			[]string{profileUUIDs[0], profileUUIDs[1], profileUUIDs[2]},
			map[string]mobius.MDMWindowsProfileContents{
...
func testMDMWindowsConfigProfiles(t *testing.T, ds *Datastore) {
	...
	profA, err := ds.NewMDMWindowsConfigProfile(ctx, mobius.MDMWindowsConfigProfile{Name: "a", TeamID: nil, SyncML: []byte("<Replace></Replace>")})
	require.NoError(t, err)
	require.NotEmpty(t, profA.ProfileUUID)
	profB, err := ds.NewMDMWindowsConfigProfile(ctx, mobius.MDMWindowsConfigProfile{Name: "b", TeamID: ptr.Uint(0), SyncML: []byte("<Replace></Replace>")})
	require.NoError(t, err)
	...
	profATm, err := ds.NewMDMWindowsConfigProfile(ctx, mobius.MDMWindowsConfigProfile{Name: "a", TeamID: ptr.Uint(1), SyncML: []byte("<Replace></Replace>")})
	require.NoError(t, err)
	...
	_, err = ds.NewMDMWindowsConfigProfile(ctx, mobius.MDMWindowsConfigProfile{Name: "b", TeamID: nil, SyncML: []byte("<Replace></Replace>")})
	require.Error(t, err)
	require.ErrorAs(t, err, &existsErr)
	// create a duplicate of Apple for no-team
	_, err = ds.NewMDMWindowsConfigProfile(ctx, mobius.MDMWindowsConfigProfile{Name: "c", TeamID: nil, SyncML: []byte("<Replace></Replace>")})
	require.Error(t, err)
	require.ErrorAs(t, err, &existsErr)
	...
	_, err = ds.NewMDMWindowsConfigProfile(ctx, mobius.MDMWindowsConfigProfile{Name: "b", TeamID: ptr.Uint(1), SyncML: []byte("<Replace></Replace>")})
	require.Error(t, err)
	require.ErrorAs(t, err, &existsErr)
	...
	_, err = ds.NewMDMWindowsConfigProfile(
		...
		mobius.MDMWindowsConfigProfile{
			...
			LabelsIncludeAll: []mobius.ConfigurationProfileLabel{{LabelName: "foo", LabelID: 1}},
		})
	...
	profWithLabel, err := ds.NewMDMWindowsConfigProfile(
		...
		mobius.MDMWindowsConfigProfile{
			...
			TeamID:           nil,
			SyncML:           []byte("<Replace></Replace>"),
			LabelsIncludeAll: []mobius.ConfigurationProfileLabel{{LabelName: label.Name, LabelID: label.ID}},
		})
	require.NoError(t, err)
...
func testSetOrReplaceMDMWindowsConfigProfile(t *testing.T, ds *Datastore) {
	...
	getProfileByTeamAndName := func(tmID *uint, name string) *mobius.MDMWindowsConfigProfile {
	...
	profs1 := expectWindowsProfiles(t, ds, nil, []*mobius.MDMWindowsConfigProfile{&cp1})
	...
	profs2 := expectWindowsProfiles(t, ds, nil, []*mobius.MDMWindowsConfigProfile{&cp2})
	...
	expectWindowsProfiles(t, ds, nil, []*mobius.MDMWindowsConfigProfile{&cp2})
	...
	profs3 := expectWindowsProfiles(t, ds, ptr.Uint(2), []*mobius.MDMWindowsConfigProfile{&cp4})
	...
	expectWindowsProfiles(t, ds, nil, []*mobius.MDMWindowsConfigProfile{&cp2, &cp5})
	...
	expectWindowsProfiles(t, ds, nil, []*mobius.MDMWindowsConfigProfile{&cp2, &cp6})
}
...
func testMDMWindowsProfileLabels(t *testing.T, ds *Datastore) {
	...
	// "include-any" labels
	l1, err := ds.NewLabel(ctx, &mobius.Label{
	...
	l2, err := ds.NewLabel(ctx, &mobius.Label{
	...
	l3, err := ds.NewLabel(ctx, &mobius.Label{
	...
	// include-all labels
	l4, err := ds.NewLabel(ctx, &mobius.Label{
	...
	l5, err := ds.NewLabel(ctx, &mobius.Label{
	...
	// exclude-all labels
	l6, err := ds.NewLabel(ctx, &mobius.Label{
	...
	l7, err := ds.NewLabel(ctx, &mobius.Label{
	...
	require.NoError(t, err)
	require.ElementsMatch(t, []*mobius.MDMWindowsProfilePayload{
		{
...
func expectWindowsProfiles(
	...
	ds *Datastore,
	tmID *uint,
	want []*mobius.MDMWindowsConfigProfile,
) map[string]string {
	if tmID == nil {
		tmID = ptr.Uint(0)
	}

	var got []*mobius.MDMWindowsConfigProfile
	ExecAdhocSQL(t, ds, func(q sqlx.ExtContext) error {
		ctx := context.Background()
	...
	wantMap := make(map[string]*mobius.MDMWindowsConfigProfile, len(want))
	for _, cp := range want {
		wantMap[cp.Name] = cp
	}
...
func testBatchSetMDMWindowsProfiles(t *testing.T, ds *Datastore) {
	...
	applyAndExpect := func(newSet []*mobius.MDMWindowsConfigProfile, tmID *uint, want []*mobius.MDMWindowsConfigProfile,
		wantUpdated bool,
	) map[string]string {
	...
	withTeamID := func(p *mobius.MDMWindowsConfigProfile, tmID uint) *mobius.MDMWindowsConfigProfile {
		p.TeamID = &tmID
		return p
	}
	withUploadedAt := func(p *mobius.MDMWindowsConfigProfile, ua time.Time) *mobius.MDMWindowsConfigProfile {
		p.UploadedAt = ua
		return p
	}
	...
	mTm1 := applyAndExpect([]*mobius.MDMWindowsConfigProfile{
		windowsConfigProfileForTest(t, "N1", "l1"),
	}, ptr.Uint(1), []*mobius.MDMWindowsConfigProfile{
	...
	applyAndExpect([]*mobius.MDMWindowsConfigProfile{
		windowsConfigProfileForTest(t, "N1", "l1"),
	}, nil, []*mobius.MDMWindowsConfigProfile{
		windowsConfigProfileForTest(t, "N1", "l1"),
	}, true)
	...
	mTm1b := applyAndExpect([]*mobius.MDMWindowsConfigProfile{
	...
	// for tm1
	mTm1c := applyAndExpect([]*mobius.MDMWindowsConfigProfile{
...
func windowsConfigProfileForTest(t *testing.T, name, locURI string, labels ...*mobius.Label) *mobius.MDMWindowsConfigProfile {
	prof := &mobius.MDMWindowsConfigProfile{
		Name: name,
	...
	for _, lbl := range labels {
		...
		case strings.HasPrefix(lbl.Name, "exclude-"):
			prof.LabelsExcludeAny = append(prof.LabelsExcludeAny, mobius.ConfigurationProfileLabel{LabelName: lbl.Name, LabelID: lbl.ID})
		case strings.HasPrefix(lbl.Name, "include-any-"):
			prof.LabelsIncludeAny = append(prof.LabelsIncludeAny, mobius.ConfigurationProfileLabel{LabelName: lbl.Name, LabelID: lbl.ID})
		default:
			prof.LabelsIncludeAll = append(prof.LabelsIncludeAll, mobius.ConfigurationProfileLabel{LabelName: lbl.Name, LabelID: lbl.ID})
		}
	}
...
func createResponseAsEnrichedSyncML(t *testing.T, enrolledDevice *mobius.MDMWindowsEnrolledDevice, atomicCommandUUID string,
	replaceCommandUUID string,
) mobius.EnrichedSyncML {
...
`, enrolledDevice.MDMDeviceID, atomicCommandUUID, replaceCommandUUID)
	syncML := &mobius.SyncML{}
	err := xml.Unmarshal([]byte(rawResponse), syncML)
	require.NoError(t, err)
	syncML.Raw = []byte(rawResponse)
	enrichedSyncML := mobius.NewEnrichedSyncML(syncML)
	return enrichedSyncML
}

func createEnrolledDevice(t *testing.T, ds *Datastore) *mobius.MDMWindowsEnrolledDevice {
	enrolledDevice := &mobius.MDMWindowsEnrolledDevice{
		MDMDeviceID:            uuid.New().String(),
		MDMHardwareID:          uuid.New().String() + uuid.New().String(),
		MDMDeviceState:         microsoft_mdm.MDMDeviceStateEnrolled,
...
func testSetMDMWindowsProfilesWithVariables(t *testing.T, ds *Datastore) {
	// NOTE: as of this code being written, Mobius variables are not yet supported
	...
	err := batchSetProfileVariableAssociationsDB(ctx, ds.writer(ctx), []mobius.MDMProfileUUIDMobiusVariables{
		{ProfileUUID: globalProfiles[0], MobiusVariables: nil},
		{ProfileUUID: globalProfiles[1], MobiusVariables: nil},
	}, "windows")
	require.NoError(t, err)
	...
	// add some variables
	err = batchSetProfileVariableAssociationsDB(ctx, ds.writer(ctx), []mobius.MDMProfileUUIDMobiusVariables{
		{ProfileUUID: globalProfiles[0], MobiusVariables: []string{mobius.MobiusVarHostEndUserIDPUsername, mobius.MobiusVarDigiCertDataPrefix + "ZZZ"}},
		{ProfileUUID: globalProfiles[1], MobiusVariables: []string{mobius.MobiusVarHostEndUserIDPGroups}},
	}, "windows")
	require.NoError(t, err)

	checkProfileVariables(globalProfiles[0], 0, []string{mobius.MobiusVarHostEndUserIDPUsername, mobius.MobiusVarDigiCertDataPrefix})
	checkProfileVariables(globalProfiles[1], 0, []string{mobius.MobiusVarHostEndUserIDPGroups})
}
