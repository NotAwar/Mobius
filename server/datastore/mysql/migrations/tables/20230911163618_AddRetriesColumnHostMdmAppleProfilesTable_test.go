...
VALUES
	...
	var hmap struct {
		...
		Status            *mobius.MDMDeliveryStatus `db:"status"`
		OperationType     mobius.MDMOperationType   `db:"operation_type"`
	...
	require.Equal(t, mobius.MDMDeliveryVerified, *hmap.Status)
	require.Equal(t, mobius.MDMOperationTypeInstall, hmap.OperationType)
