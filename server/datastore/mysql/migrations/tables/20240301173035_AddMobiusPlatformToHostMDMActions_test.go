...
func TestUp_20240301173035(t *testing.T) {
	...
	// mobius platform is left empty for pre-existing rows
	err = sqlx.Select(db, &hostActions, `SELECT host_id, lock_ref, mobius_platform FROM host_mdm_actions`)
	require.NoError(t, err)
	...
	require.Empty(t, hostActions[0].MobiusPlatform)
}
