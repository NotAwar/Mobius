	...
	if err != nil {
		if driverErr, ok := err.(*mysql.MySQLError); ok {
			if driverErr.Number == mysqlerr.ER_DUP_ENTRY {
				...
				// Mobius-reserved name is unique? All label names need to be unique
				...
				return fmt.Errorf("a label with the name %q already exists, please rename it before applying this migration: %w", mobius.BuiltinLabelMacOS14Plus, err)
			}
		}
		return err
	}
