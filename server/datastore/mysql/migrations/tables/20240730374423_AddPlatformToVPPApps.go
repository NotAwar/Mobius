...
func Up_20240730374423(tx *sql.Tx) error {
	...
	_, err = tx.Exec(`UPDATE vpp_apps SET platform = ?, updated_at = updated_at`, mobius.MacOSPlatform)
	if err != nil {
	...
	_, err = tx.Exec(`UPDATE vpp_apps_teams SET platform = ?`, mobius.MacOSPlatform)
	if err != nil {
