...
CREATE TABLE abm_tokens (
	...
	-- with the MobiusConfig.Server.PrivateKey
	token                  blob NOT NULL,
...
`
	...
	var token []byte
	if err := txx.Get(&token, getABM, mobius.MDMAssetABMTokenDeprecated); err != nil {
...
`
	...
	if _, err = tx.Exec(delABM, deletionUUID, mobius.MDMAssetABMTokenDeprecated); err != nil {
