...
CREATE TABLE vpp_tokens (
	...
	-- encrypted token, encrypted with the MobiusConfig.Server.PrivateKey value
	token                  blob NOT NULL,
	...
	-- the Go code.
	-- [1]: https://mobiusmdm.slack.com/archives/C03C41L5YEL/p1724073772972669
...
`
	...
	var token []byte
	if err := txx.Get(&token, getVPP, mobius.MDMAssetVPPTokenDeprecated); err != nil {
...
`
	...
	if _, err = tx.Exec(delVPP, deletionUUID, mobius.MDMAssetVPPTokenDeprecated); err != nil {
