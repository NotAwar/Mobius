...
`
	...
	_, err := tx.Exec(
		stmt,
		mobius.BuiltinLabelFedoraLinux,
		"All Fedora hosts",
	...
	if err != nil {
		if driverErr, ok := err.(*mysql.MySQLError); ok {
			if driverErr.Number == mysqlerr.ER_DUP_ENTRY {
				return fmt.Errorf("a label with the name %q already exists, please rename it before applying this migration: %w", mobius.BuiltinLabelFedoraLinux, err)
			}
		}
		return err
	}
