...
func Up_20241003145349(tx *sql.Tx) error {
	_, err := tx.Exec(`
CREATE TABLE mobius_library_apps (
	...
	-- it is expected to be provided by the hard-coded JSON list of apps in the Mobius library.
	...
	-- this new version should update the app in the Mobius library, not create a new entry.
	UNIQUE KEY idx_mobius_library_apps_token (token),
	CONSTRAINT fk_mobius_library_apps_install_script_content FOREIGN KEY (install_script_content_id)
		REFERENCES script_contents (id) ON DELETE CASCADE,
	CONSTRAINT fk_mobius_library_apps_uninstall_script_content FOREIGN KEY (uninstall_script_content_id)
		REFERENCES script_contents (id) ON DELETE CASCADE
)`)
	if err != nil {
		return fmt.Errorf("creating mobius_library_apps table: %w", err)
	}

	// New column mobius_library_app_id in software_installers allows to keep
	// track of which Mobius library app the installer comes from, if any.
	//
	// NOTE: it is not _crucial_ to have this now, as even if a user adds the
	// same installer _name_ in software_installers (without passing by the Mobius
	// Library, so this mobius_library_app_id would be NULL), it shouldn't see the
	// same _name_ from the Mobius library as available. But it's probably good in
	// any case to keep track of this, even if not obviously useful now.
	_, err = tx.Exec(`
ALTER TABLE software_installers
	ADD COLUMN mobius_library_app_id int unsigned DEFAULT NULL,
	ADD FOREIGN KEY fk_software_installers_mobius_library_app_id (mobius_library_app_id)
		REFERENCES mobius_library_apps (id) ON DELETE SET NULL
	`)
	if err != nil {
		return fmt.Errorf("failed to alter software_installers to add mobius_library_app_id: %w", err)
	}
	return nil
}
