...
func TestUp_20241003145349(t *testing.T) {
	...
	var count int
	err := db.Get(&count, "SELECT COUNT(*) FROM mobius_library_apps")
	require.NoError(t, err)
	require.Zero(t, count)

	// column was added and value is NULL
	var mobiusLibraryAppID *uint
	err = db.Get(&mobiusLibraryAppID, "SELECT mobius_library_app_id FROM software_installers WHERE id = ?", swiID)
	require.NoError(t, err)
	require.Nil(t, mobiusLibraryAppID)
}
