...
func TestUp_20250124194347(t *testing.T) {
	...
	_, err := db.Exec(insertSql, "name1", "", "", "com.mobius1")
	require.NoError(t, err)
	_, err = db.Exec(insertSql, "name1", "", "", "com.mobius2")
	require.Error(t, err, "Expected software insert to fail because of unique key")

	applyNext(t, db)

	_, err = db.Exec(insertSql, "name2", "", "", "com.mobiusdm1")
	require.NoError(t, err)
	_, err = db.Exec(insertSql, "name2", "", "", "com.mobiusdm2")
