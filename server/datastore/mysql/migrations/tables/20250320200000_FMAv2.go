...
ALTER TABLE software_installers
	CHANGE COLUMN mobius_library_app_id mobius_maintained_app_id INT unsigned DEFAULT NULL
`)
	if err != nil {
		return fmt.Errorf("failed to rename mobius_library_app_id column: %w", err)
	}

	_, err = tx.Exec(`RENAME TABLE mobius_library_apps TO mobius_maintained_apps`)
	if err != nil {
		return fmt.Errorf("failed to rename mobius_library_apps: %w", err)
	}

	_, err = tx.Exec(`UPDATE software_installers si
		JOIN mobius_maintained_apps fma ON fma.id = si.mobius_maintained_app_id
		SET mobius_maintained_app_id = NULL
	...
	if err != nil {
		return fmt.Errorf("failed to unlink diverged Mobius-maintained apps: %w", err)
	}

	_, err = tx.Exec(`
ALTER TABLE mobius_maintained_apps
    DROP CONSTRAINT fk_mobius_library_apps_install_script_content,
    DROP CONSTRAINT fk_mobius_library_apps_uninstall_script_content
`)
	if err != nil {
		return fmt.Errorf("failed to drop obsolete indexed from mobius_maintained_apps: %w", err)
	}
...
`)
	if err != nil {
		return fmt.Errorf("failed to alter mobius_maintained_apps: %w", err)
	}
...
WHERE
  ...
  AND NOT EXISTS (
	...
	_, err = tx.Exec(`UPDATE mobius_maintained_apps SET slug = concat(slug, '/', platform)`)
	if err != nil {
	...
	var slugs []string
	err = txx.Select(&slugs, `SELECT slug FROM mobius_maintained_apps WHERE slug in ('zoom/darwin', 'zoom-for-it-admins/darwin')`)
	if err != nil {
	...
	if len(slugs) > 1 || (len(slugs) == 1 && slugs[0] == "zoom/darwin") {
		_, err = tx.Exec(`DELETE FROM mobius_maintained_apps WHERE slug = 'zoom/darwin'`)
		if err != nil {
			return fmt.Errorf("failed to remove duplicate Zoom FMA: %w", err)
		}
	}

	_, err = tx.Exec(`UPDATE mobius_maintained_apps SET slug = 'zoom/darwin', name = 'Zoom' WHERE slug = 'zoom-for-it-admins/darwin'`)
	if err != nil {
