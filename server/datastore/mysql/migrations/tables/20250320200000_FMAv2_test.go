...
func TestUp_20250320200000(t *testing.T) {
	...
	execNoErr(t, db, `INSERT INTO cron_stats (name, instance, stats_type, status) VALUES (?, 'foo', ?, ?)`, mobius.CronMaintainedApps, mobius.CronStatsTypeScheduled, mobius.CronStatsStatusCompleted)
	execNoErr(t, db, `INSERT INTO cron_stats (name, instance, stats_type, status) VALUES (?, 'foo', ?, ?)`, mobius.CronMaintainedApps, mobius.CronStatsTypeTriggered, mobius.CronStatsStatusCompleted)
	...
	execNoErr(
		t,
		db,
		`INSERT INTO mobius_library_apps (name, token, version, platform, installer_url, sha256, bundle_identifier, install_script_content_id, uninstall_script_content_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		"Zoom",
		"zoom",
		"6.2.11.43613",
		"darwin",
	...
	noTeamBox := execNoErrLastID(t, db, `
		INSERT INTO software_installers
			(filename, version, platform, install_script_content_id, storage_id, package_ids, uninstall_script_content_id, mobius_library_app_id)
		VALUES
	...
	var fmas []mobius.MaintainedApp
	err = db.Select(&fmas, `SELECT id, name, slug, unique_identifier FROM mobius_maintained_apps ORDER BY name`)
	require.NoError(t, err)
	...
	err = db.Get(&linkedFMAID, `SELECT mobius_maintained_app_id FROM software_installers WHERE id = ?`, noTeamBox)
	require.NoError(t, err)
	require.Equal(t, boxFMAID, *linkedFMAID)

	// FMA ID for Box software installer on other team should be null
	err = db.Get(&linkedFMAID, `SELECT mobius_maintained_app_id FROM software_installers WHERE id = ?`, otherTeamBox)
	require.NoError(t, err)
	require.Nil(t, linkedFMAID)

	// Only the triggered job record should remain in the cron_stats table
	var stats []mobius.CronStats
	err = db.Select(&stats, `SELECT name, instance, stats_type, status FROM cron_stats`)
	require.NoError(t, err)
	require.Len(t, stats, 1)
	require.Equal(t, string(mobius.CronMaintainedApps), stats[0].Name)
	require.Equal(t, mobius.CronStatsTypeTriggered, stats[0].StatsType)
	require.Equal(t, mobius.CronStatsStatusCompleted, stats[0].Status)
}
