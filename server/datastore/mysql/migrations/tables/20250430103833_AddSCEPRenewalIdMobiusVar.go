...
func Up_20250430103833(tx *sql.Tx) error {
	...
	INSERT INTO mobius_variables (
		...
		('MOBIUS_VAR_SCEP_RENEWAL_ID', 0, :created_at)
	`
	...
	if err != nil {
		return fmt.Errorf("Failed to prepare insert for MOBIUS_VAR_SCEP_RENEWAL_ID: %s", err)
	}
	_, err = tx.Exec(stmt, args...)
	if err != nil {
		return fmt.Errorf("failed to insert MOBIUS_VAR_SCEP_RENEWAL_ID into mobius_variables: %s", err)
	}
	return nil
}
