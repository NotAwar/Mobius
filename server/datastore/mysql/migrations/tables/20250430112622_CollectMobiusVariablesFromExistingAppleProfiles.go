...
func Up_20250430112622(tx *sql.Tx) error {
	// For now, we only collect used mobius variables from existing Apple
	...
	for {
		...
		for _, profile := range existingProfiles {
			vars := findMobiusVariables(profile.MobileConfig)
			if len(vars) > 0 {
				if err := saveConfigProfileVars(tx, profile.ProfileUUID, vars, varDefs); err != nil {
					return fmt.Errorf("failed to save mobius variables for profile %s: %w", profile.ProfileUUID, err)
				}
			}
...
func saveConfigProfileVars(tx *sql.Tx, profileUUID string, vars map[string]any, varDefs []varDef) error {
	// map the profiles' variables to the mobius variable IDs
	var varIDs []uint
	for v := range vars {
	...
	stmt := strings.TrimSuffix(fmt.Sprintf(`INSERT INTO mdm_configuration_profile_variables (
		apple_profile_uuid, mobius_variable_id
...
func findMobiusVariables(contents string) map[string]interface{} {
	...
	for _, v := range resultSlice {
		// re-add the "MOBIUS_VAR_" prefix as it is needed to map with the entries in
		// mobius_variables table.
		result["MOBIUS_VAR_"+v] = struct{}{}
	}
	return result
}
