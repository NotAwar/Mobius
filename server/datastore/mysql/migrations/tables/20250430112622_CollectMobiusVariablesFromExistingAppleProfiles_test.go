...
type profileVarTuple struct {
	...
	VarID       uint   `db:"mobius_variable_id"`
}

func TestUp_20250430112622_SingleProfileWithVar(t *testing.T) {
	db := applyUpToPrev(t)

	prof1 := insertAppleConfigProfile(t, db, "N1", "I1", "MOBIUS_VAR_HOST_END_USER_IDP_USERNAME")
	var varID uint
	err := db.Get(&varID, `SELECT id FROM mobius_variables WHERE name = 'MOBIUS_VAR_HOST_END_USER_IDP_USERNAME'`)
	...
	err = db.Select(&profs, `SELECT apple_profile_uuid, mobius_variable_id FROM mdm_configuration_profile_variables`)
	require.NoError(t, err)
...
func TestUp_20250430112622_ProfilesUnknownVariable(t *testing.T) {
	...
	insertAppleConfigProfile(t, db, "N1", "I1", "MOBIUS_VAR_NO_SUCH_VARIABLE")
	insertAppleConfigProfile(t, db, "N2", "I2", "MOBIUS_VAR_WHAT_I_CANT_EVEN")
...
func runWithNProfiles(t *testing.T, n int) {
	...
	var defs []varDef
	err := db.Select(&defs, `SELECT id, name, is_prefix FROM mobius_variables`)
	require.NoError(t, err)
