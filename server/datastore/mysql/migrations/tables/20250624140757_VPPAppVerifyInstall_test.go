...
VALUES (?, ?, ?, ?, ?, ?, ?)`, hostUUID, hostUUID, "device", "topic", "magic", "hex", time.Now())
	...
	hvsi1 := insertVPPAppInstall(mobius.MDMAppleStatusAcknowledged)
	hvsi2 := insertVPPAppInstall(mobius.MDMAppleStatusError)
	hvsi3 := insertVPPAppInstall(mobius.MDMAppleStatusCommandFormatError)
	hvsi4 := insertVPPAppInstall(mobius.MDMAppleStatusNotNow)
	hvsi5 := insertVPPAppInstall(mobius.MDMAppleStatusIdle)
