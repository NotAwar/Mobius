...
func Up_20250626130239(tx *sql.Tx) error {
	...
	insStmt := `INSERT INTO mobius_variables
	(name, is_prefix, created_at)
	VALUES ('MOBIUS_VAR_HOST_END_USER_IDP_DEPARTMENT', 0, :created_at)`
	...
	if err != nil {
		return fmt.Errorf("failed to prepare insert of MOBIUS_VAR_HOST_END_USER_IDP_DEPARTMENT to mobius_variables: %s", err)
	}
	_, err = tx.Exec(stmt, args...)
	if err != nil {
		return fmt.Errorf("failed to insert MOBIUS_VAR_HOST_END_USER_IDP_DEPARTMENT into mobius_variables: %s", err)
	}
	return nil
}
