...
func (ds *Datastore) ListVulnsByOsNameAndVersion(ctx context.Context, name, version string, includeCVSS bool) (mobius.Vulnerabilities, error) {
	r := mobius.Vulnerabilities{}
...
func (ds *Datastore) InsertOSVulnerabilities(ctx context.Context, vulnerabilities []mobius.OSVulnerability, source mobius.VulnerabilitySource) (int64, error) {
	var args []interface{}
...
func (ds *Datastore) InsertOSVulnerability(ctx context.Context, v mobius.OSVulnerability, s mobius.VulnerabilitySource) (bool, error) {
	if v.CVE == "" {
...
func (ds *Datastore) DeleteOSVulnerabilities(ctx context.Context, vulnerabilities []mobius.OSVulnerability) error {
	if len(vulnerabilities) == 0 {
		return nil
	}
...
func (ds *Datastore) DeleteOutOfDateOSVulnerabilities(ctx context.Context, src mobius.VulnerabilitySource, d time.Duration) error {
	deleteStmt := `
