...
func TestListOperatingSystems(t *testing.T) {
	...
	osByID := make(map[uint]mobius.OperatingSystem)
	for _, os := range list {
		osByID[os.ID] = os
	}
...
func TestCleanupHostOperatingSystems(t *testing.T) {
	...
	testHosts := make([]*mobius.Host, 10)
	osByHostID := make(map[uint]mobius.OperatingSystem)
	...
	assertDeletedOS := func(expectDeletedIDs []uint) {
		...
		byID := make(map[uint]mobius.OperatingSystem)
		for _, os := range list {
			byID[os.ID] = os
		}
...
func seedOperatingSystems(t *testing.T, ds *Datastore) map[uint]mobius.OperatingSystem {
	osSeeds := []mobius.OperatingSystem{
		{
	...
	storedById := make(map[uint]mobius.OperatingSystem)
...
func isSameOS(t *testing.T, os1 mobius.OperatingSystem, os2 mobius.OperatingSystem) bool {
