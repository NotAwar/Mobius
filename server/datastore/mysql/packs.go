...
func applyPackSpecDB(ctx context.Context, tx sqlx.ExtContext, spec *mobius.PackSpec) error {
	if spec.Name == "" {
	...
	for _, l := range spec.Targets.Labels {
		if _, err := tx.ExecContext(ctx, query, packID, mobius.TargetLabel, l); err != nil {
	...
	for _, t := range spec.Targets.Teams {
		if _, err := tx.ExecContext(ctx, query, packID, mobius.TargetTeam, t); err != nil {
...
func (ds *Datastore) GetPackSpecs(ctx context.Context) ([]*mobius.PackSpec, error) {
	var specs []*mobius.PackSpec
	err := ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		// Get basic specs
...
`
			if err := sqlx.SelectContext(ctx, tx, &spec.Targets.Labels, query, spec.ID, mobius.TargetLabel); err != nil {
...
`
			if err := sqlx.SelectContext(ctx, tx, &spec.Targets.Teams, query, spec.ID, mobius.TargetTeam); err != nil {
...
func (ds *Datastore) GetPackSpec(ctx context.Context, name string) (*mobius.PackSpec, error) {
	spec := &mobius.PackSpec{}
	err := ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		// Get basic spec
...
func (ds *Datastore) PackByName(ctx context.Context, name string, opts ...mobius.OptionalArg) (*mobius.Pack, bool, error) {
	sqlStatement := `
		SELECT *
			FROM packs
			WHERE name = ?
...
func (ds *Datastore) NewPack(ctx context.Context, pack *mobius.Pack, opts ...mobius.OptionalArg) (*mobius.Pack, error) {
...
func replacePackTargetsDB(ctx context.Context, tx sqlx.ExecerContext, pack *mobius.Pack) error {
	...
	if len(pack.LabelIDs) > 0 {
		...
		for _, id := range pack.LabelIDs {
			args = append(args, pack.ID, mobius.TargetLabel, id)
		}
	...
	if len(pack.HostIDs) > 0 {
		...
		for _, id := range pack.HostIDs {
			args = append(args, pack.ID, mobius.TargetHost, id)
		}
		values := strings.TrimSuffix(
	...
	if len(pack.TeamIDs) > 0 {
		...
		for _, id := range pack.TeamIDs {
			args = append(args, pack.ID, mobius.TargetTeam, id)
		}
		values := strings.TrimSuffix(
...
func loadPackTargetsDB(ctx context.Context, q sqlx.QueryerContext, pack *mobius.Pack) error {
	var targets []mobius.Target
	sql := `
...
func (ds *Datastore) SavePack(ctx context.Context, pack *mobius.Pack) error {
...
// DeletePack deletes a mobius.Pack so that it won't show up in results.
func (ds *Datastore) DeletePack(ctx context.Context, name string) error {
	return ds.deleteEntityByName(ctx, packsTable, name)
}

// Pack fetch mobius.Pack with matching ID
func (ds *Datastore) Pack(ctx context.Context, pid uint) (*mobius.Pack, error) {
	return packDB(ctx, ds.reader(ctx), pid)
}

func packDB(ctx context.Context, q sqlx.QueryerContext, pid uint) (*mobius.Pack, error) {
...
func (ds *Datastore) ListPacksForHost(ctx context.Context, hid uint) ([]*mobius.Pack, error) {
...
func listPacksForHost(ctx context.Context, db sqlx.QueryerContext, hid uint) ([]*mobius.Pack, error) {
	query := `
	...
	if err := sqlx.SelectContext(ctx, db, &packs, query,
		mobius.TargetLabel, hid, mobius.TargetHost, hid, mobius.TargetTeam, hid,
