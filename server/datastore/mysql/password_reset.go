...
func (ds *Datastore) NewPasswordResetRequest(ctx context.Context, req *mobius.PasswordResetRequest) (*mobius.PasswordResetRequest, error) {
	sqlStatement := `
...
func (ds *Datastore) FindPasswordResetByToken(ctx context.Context, token string) (*mobius.PasswordResetRequest, error) {
	sqlStatement := `
		SELECT * FROM password_reset_requests
		WHERE token = ? AND CURRENT_TIMESTAMP < expires_at LIMIT 1
    `
	passwordResetRequest := &mobius.PasswordResetRequest{}
