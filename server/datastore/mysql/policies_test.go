...
func testPoliciesNewGlobalPolicyLegacy(t *testing.T, ds *Datastore) {
	...
	q, err := ds.NewQuery(context.Background(), &mobius.Query{
		Name:        "query1",
	...
	require.NoError(t, err)
	_, err = ds.NewGlobalPolicy(context.Background(), &user1.ID, mobius.PolicyPayload{
		QueryID: &q2.ID,
	})
	require.NoError(t, err)

	policies, err := ds.ListGlobalPolicies(context.Background(), mobius.ListOptions{})
	require.NoError(t, err)
...
func testPoliciesNewGlobalPolicyProprietary(t *testing.T, ds *Datastore) {
	...
	p, err := ds.NewGlobalPolicy(ctx, &user1.ID, mobius.PolicyPayload{
		Name:        "query1",
		Query:       "select 1;",
		Description: "query1 desc",
	...
	_, err = ds.NewGlobalPolicy(ctx, &user1.ID, mobius.PolicyPayload{
		Name:        "query2",
		Query:       "select 2;",
		Description: "query2 desc",
	...
	policies, err := ds.ListGlobalPolicies(ctx, mobius.ListOptions{})
	require.NoError(t, err)
	require.Len(t, policies, 2)
	...
	p3, err := ds.NewGlobalPolicy(ctx, &user1.ID, mobius.PolicyPayload{
		Name:        "query1",
		Query:       "select 3;",
...
func testGlobalPolicyPendingScriptsAndInstalls(t *testing.T, ds *Datastore) {
	...
	require.NoError(t, err)
	policy1, err := ds.NewGlobalPolicy(ctx, &user.ID, mobius.PolicyPayload{
		Name:        "query1",
		Query:       "select 1;",
		Description: "query1 desc",
		Resolution:  "query1 resolution",
	...
	_, err = ds.NewHostScriptExecutionRequest(ctx, &mobius.HostScriptRequestPayload{
		HostID:         host1.ID,
		ScriptContents: "echo",
		UserID:         &user.ID,
		PolicyID:       &policy1.ID,
	...
	require.NoError(t, err)
	policy2, err := ds.NewGlobalPolicy(ctx, &user.ID, mobius.PolicyPayload{
		Name:        "query2",
		Query:       "select 1;",
		Description: "query2 desc",
		Resolution:  "query2 resolution",
	...
	_, err = ds.InsertSoftwareInstallRequest(ctx, host2.ID, installerID, mobius.HostSoftwareInstallOptions{PolicyID: &policy2.ID})
	require.NoError(t, err)
...
func testPoliciesListOptions(t *testing.T, ds *Datastore) {
	...
	policies, err := ds.ListGlobalPolicies(ctx, mobius.ListOptions{MatchQuery: "apple", OrderKey: "name", OrderDirection: mobius.OrderAscending})
	require.NoError(t, err)
...
func testPoliciesMembershipView(deferred bool, t *testing.T, ds *Datastore) {
	...
	q2, err := ds.NewQuery(ctx, &mobius.Query{
		Name:        "query2",
		Description: "query2 desc",
		Query:       "select 42;",
		Saved:       true,
	...
	require.NoError(t, err)
	p2, err := ds.NewGlobalPolicy(ctx, &user1.ID, mobius.PolicyPayload{
		QueryID: &q2.ID,
	})
	require.NoError(t, err)
	...
	t1pol, err := ds.NewTeamPolicy(ctx, team1.ID, &user1.ID, mobius.PolicyPayload{
		Name:  "team1pol",
		Query: "SELECT 1",
	})
	require.NoError(t, err)
	t2pol, err := ds.NewTeamPolicy(ctx, team2.ID, &user1.ID, mobius.PolicyPayload{
		Name:  "team2pol",
		Query: "SELECT 2",
	})
	require.NoError(t, err)
	t2pol2, err := ds.NewTeamPolicy(ctx, team2.ID, &user1.ID, mobius.PolicyPayload{
		Name:  "team2pol2",
		Query: "SELECT 3",
	})
	require.NoError(t, err)
	...
	t1Pols, t1Inherited, err := ds.ListTeamPolicies(ctx, team1.ID, mobius.ListOptions{}, mobius.ListOptions{})
	require.NoError(t, err)
	...
	t2Pols, t2Inherited, err := ds.ListTeamPolicies(ctx, team2.ID, mobius.ListOptions{}, mobius.ListOptions{})
	require.NoError(t, err)
...
func testTeamPolicyLegacy(t *testing.T, ds *Datastore) {
	...
	prevPolicies, err := ds.ListGlobalPolicies(ctx, mobius.ListOptions{})
	require.NoError(t, err)
	require.Len(t, prevPolicies, 0)

	_, err = ds.NewTeamPolicy(ctx, 99999999, &user1.ID, mobius.PolicyPayload{
		QueryID: &q.ID,
	})
	require.Error(t, err)

	p, err := ds.NewTeamPolicy(ctx, team1.ID, &user1.ID, mobius.PolicyPayload{
		QueryID:    &q.ID,
	...
	gpol, err := ds.NewGlobalPolicy(ctx, &user1.ID, mobius.PolicyPayload{
		Name:  "global_1",
		Query: "SELECT 1",
	})
	require.NoError(t, err)

	globalPolicies, err := ds.ListGlobalPolicies(ctx, mobius.ListOptions{})
	require.NoError(t, err)
	require.Len(t, globalPolicies, 1)

	p2, err := ds.NewTeamPolicy(ctx, team2.ID, &user1.ID, mobius.PolicyPayload{
		QueryID: &q2.ID,
	})
	require.NoError(t, err)
	...
	teamPolicies, inherited1, err := ds.ListTeamPolicies(ctx, team1.ID, mobius.ListOptions{}, mobius.ListOptions{})
	require.NoError(t, err)
	...
	team2Policies, inherited2, err := ds.ListTeamPolicies(ctx, team2.ID, mobius.ListOptions{}, mobius.ListOptions{})
	require.NoError(t, err)
...
func testTeamPolicyProprietary(t *testing.T, ds *Datastore) {
	...
	requireLabels := func(t *testing.T, expected []string, actual []mobius.LabelIdent) {
	...
	gpol1, err := ds.NewGlobalPolicy(ctx, &user1.ID, mobius.PolicyPayload{
	...
	_, err = ds.NewTeamPolicy(ctx, team1.ID, &user1.ID, mobius.PolicyPayload{
		Name:  "query1",
		Query: "select 1;",
	})
	require.Error(t, err)
	...
	p3, err := ds.NewTeamPolicy(ctx, team1.ID, &user1.ID, mobius.PolicyPayload{
		Name:        "query1",
		Query:       "select 2;",
	...
	teamPolicies, _, err = ds.ListTeamPolicies(ctx, team1.ID, mobius.ListOptions{}, mobius.ListOptions{})
	require.NoError(t, err)
	require.Len(t, teamPolicies, 1)
	assert.Equal(t, "query1", teamPolicies[0].Name)
...
func testTeamPolicyPendingScriptsAndInstalls(t *testing.T, ds *Datastore) {
	...
	require.NoError(t, err)
	policy1, err := ds.NewTeamPolicy(ctx, team1.ID, nil, mobius.PolicyPayload{
		Name:        "team1",
		Query:       "select 1;",
	...
	require.NoError(t, err)
	policy2, err := ds.NewTeamPolicy(ctx, team2.ID, nil, mobius.PolicyPayload{
		Name:                "team2",
		Query:               "select 1;",
...
func testListMergedTeamPolicies(t *testing.T, ds *Datastore) {
	ctx := context.Background()
	gpol, err := ds.NewGlobalPolicy(ctx, nil, mobius.PolicyPayload{
		Name:        "query1 global",
		Query:       "select 1;",
		Description: "query1 desc",
		Resolution:  "query1 resolution",
	...
	team1policy, err := ds.NewTeamPolicy(ctx, team1.ID, nil, mobius.PolicyPayload{
		Name:        "query2 team1",
		Query:       "select 1;",
		Description: "query1 desc",
		Resolution:  "query1 resolution",
	...
	team2policy, err := ds.NewTeamPolicy(ctx, team2.ID, nil, mobius.PolicyPayload{
		Name:        "query3 team2",
		Query:       "select 2;",
		Description: "query2 desc",
		Resolution:  "query2 resolution",
	...
	merged, err := ds.ListMergedTeamPolicies(ctx, team1.ID, mobius.ListOptions{
		OrderKey:       "name",
	...
	host, err := ds.NewHost(context.Background(),
		&mobius.Host{OsqueryHostID: ptr.String("host1"), NodeKey: ptr.String(fmt.Sprint("host1", 1)), TeamID: nil})
	require.NoError(t, err)
	...
	merged, err = ds.ListMergedTeamPolicies(ctx, team2.ID, mobius.ListOptions{
		OrderKey: "name",
	})
	require.NoError(t, err)
	require.Len(t, merged, 2)
...
func newTestHostWithPlatform(t *testing.T, ds *Datastore, hostname, platform string, teamID *uint) *mobius.Host {
...
func newTestPolicy(t *testing.T, ds *Datastore, user *mobius.User, name, platforms string, teamID *uint) *mobius.Policy {
	query := fmt.Sprintf("select %s;", name)
	if teamID == nil {
		gp, err := ds.NewGlobalPolicy(context.Background(), &user.ID, mobius.PolicyPayload{
			Name:     name,
			Query:    query,
			Platform: platforms,
		})
	...
	tp, err := ds.NewTeamPolicy(context.Background(), *teamID, &user.ID, mobius.PolicyPayload{
		Name:     name,
		Query:    query,
		Platform: platforms,
	})
...
type expectedPolicyResults struct {
	...
	hostPolicies  []*mobius.HostPolicy
}

func expectedPolicyQueries(policies ...*mobius.Policy) expectedPolicyResults {
	queries := make(map[string]string)
	for _, policy := range policies {
		queries[fmt.Sprint(policy.ID)] = policy.Query
	}
	hostPolicies := make([]*mobius.HostPolicy, len(policies))
	for i := range policies {
		hostPolicies[i] = &mobius.HostPolicy{
...
func testPolicyQueriesForHostPlatforms(t *testing.T, ds *Datastore) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.host.Hostname, func(t *testing.T) {
			...
			// ListPoliciesForHost is the endpoint used by mobius UI/API clients.
...
func testPolicyQueriesForHost(t *testing.T, ds *Datastore) {
	...
	require.NoError(t, err)
	gp, err := ds.NewGlobalPolicy(context.Background(), &user1.ID, mobius.PolicyPayload{
		QueryID:    &q.ID,
	...
	require.NoError(t, err)
	tp, err := ds.NewTeamPolicy(context.Background(), team1.ID, &user1.ID, mobius.PolicyPayload{
		QueryID:    &q2.ID,
	...
	checkGlobaPolicy := func(policy *mobius.HostPolicy) {
...
func testPoliciesByID(t *testing.T, ds *Datastore) {
	...
	installer, err := mobius.NewTempFileReader(strings.NewReader("hello"), t.TempDir)
	require.NoError(t, err)
	installerID, _, err := ds.MatchOrCreateSoftwareInstaller(context.Background(), &mobius.UploadSoftwareInstallerPayload{
		InstallScript:     "hello",
		PreInstallQuery:   "SELECT 1",
		PostInstallScript: "world",
...
func testTeamPolicyTransfer(t *testing.T, ds *Datastore) {
	ctx := context.Background()
	user1 := test.NewUser(t, ds, "Alice", "alice@example.com", true)
	team1, err := ds.NewTeam(ctx, &mobius.Team{Name: t.Name() + "team1"})
	require.NoError(t, err)

	team2, err := ds.NewTeam(ctx, &mobius.Team{Name: t.Name() + "team2"})
	require.NoError(t, err)
	...
	tq, err := ds.NewQuery(ctx, &mobius.Query{
		Name:        "query1",
		Description: "query1 desc",
		Query:       "select 1;",
		Saved:       true,
	...
	require.NoError(t, err)
	team1Policy, err := ds.NewTeamPolicy(ctx, team1.ID, &user1.ID, mobius.PolicyPayload{
		QueryID: &tq.ID,
	})
	require.NoError(t, err)

	gq, err := ds.NewQuery(ctx, &mobius.Query{
		Name:        "query2",
		Description: "query2 desc",
		Query:       "select 2;",
		Saved:       true,
	...
	require.NoError(t, err)
	globalPolicy, err := ds.NewGlobalPolicy(ctx, &user1.ID, mobius.PolicyPayload{
		QueryID: &gq.ID,
	})
	require.NoError(t, err)
	...
	checkPassingCount := func(tm1, tm1Inherited, tm2Inherited, global uint) {
		t.Helper()
		require.NoError(t, ds.UpdateHostPolicyCounts(ctx))
		policies, inherited, err := ds.ListTeamPolicies(ctx, team1.ID, mobius.ListOptions{}, mobius.ListOptions{})
		require.NoError(t, err)
		require.Len(t, policies, 1)
		...
		policies, inherited, err = ds.ListTeamPolicies(ctx, team2.ID, mobius.ListOptions{}, mobius.ListOptions{})
		require.NoError(t, err)
...
func testApplyPolicySpec(t *testing.T, ds *Datastore) {
	...
	require.NoError(t, ds.ApplyPolicySpecs(ctx, user1.ID, []*mobius.PolicySpec{
		{
	...
	assert.Equal(t, []mobius.LabelIdent{{
	...
	noTeamPolicies, _, err := ds.ListTeamPolicies(ctx, mobius.PolicyNoTeamID, mobius.ListOptions{}, mobius.ListOptions{})
	require.NoError(t, err)
	...
	assert.Contains(t, policies[0].LabelsExcludeAny, mobius.LabelIdent{
		LabelName: fooLabel.Name,
		LabelID:   fooLabel.ID,
	})
	assert.Contains(t, policies[0].LabelsExcludeAny, mobius.LabelIdent{
	...
	require.NoError(
		t, ds.ApplyPolicySpecs(
			ctx, user1.ID, []*mobius.PolicySpec{
				{
					Name:        "query1" + unicode,
...
func updatePolicyFailureCountsForHosts(ctx context.Context, ds *Datastore, hosts []*mobius.Host) ([]*mobius.Host, error) {
	if len(hosts) == 0 {
		return hosts, nil
	}
...
func testApplyPolicySpecWithQueryPlatformChanges(t *testing.T, ds *Datastore) {
	...
	user1 := test.NewUser(t, ds, "User1", "user1@example.com", true)
	team1, err := ds.NewTeam(ctx, &mobius.Team{Name: "team1" + unicode})
	require.NoError(t, err)
	...
	teamHosts := make([]*mobius.Host, len(platforms))
	...
	globalHosts := make([]*mobius.Host, len(platforms))
	for i, pl := range platforms {
	...
	gPolicies, err := ds.ListGlobalPolicies(ctx, mobius.ListOptions{})
	require.NoError(t, err)
	require.Len(t, gPolicies, 3)
	// load the team policies
	tPolicies, _, err := ds.ListTeamPolicies(ctx, team1.ID, mobius.ListOptions{}, mobius.ListOptions{})
	require.NoError(t, err)
	require.Len(t, tPolicies, 3)

	// index the policies by name for easier access in the rest of the test
	polsByName := make(map[string]*mobius.Policy, len(gPolicies)+len(tPolicies))
	globalPolsByName := make(map[string]*mobius.Policy, len(gPolicies))
...
func testPoliciesSave(t *testing.T, ds *Datastore) {
	...
	err = ds.SavePolicy(ctx, &mobius.Policy{
	...
	computeChecksum := func(policy mobius.Policy) string {
	...
	gp2.LabelsExcludeAny = []mobius.LabelIdent{{LabelName: label1.Name}}
	gp2.LabelsIncludeAny = []mobius.LabelIdent{{LabelName: label2.Name}}
	err = ds.SavePolicy(ctx, &gp2, false, false)
	require.Error(t, err)
...
func testCachedPolicyCountDeletesOnPolicyChange(t *testing.T, ds *Datastore) {
	...
	teamHost, err := ds.NewHost(ctx, &mobius.Host{
	...
	globalHost, err := ds.NewHost(ctx, &mobius.Host{
	...
	teamPolicy, err := ds.NewTeamPolicy(ctx, team1.ID, &user1.ID, mobius.PolicyPayload{
		Name:        "team query",
		Query:       "select 1;",
	...
	teamPolicies, inheritedPolicies, err := ds.ListTeamPolicies(ctx, team1.ID, mobius.ListOptions{}, mobius.ListOptions{})
	require.NoError(t, err)
	require.Len(t, teamPolicies, 1)
...
func testPoliciesDelUser(t *testing.T, ds *Datastore) {
	...
	gp, err := ds.NewGlobalPolicy(ctx, &user1.ID, mobius.PolicyPayload{
		Name:        "global query",
		Query:       "select 1;",
		Description: "global query desc",
		Resolution:  "global query resolution",
	...
	require.NoError(t, err)
	tp, err := ds.NewTeamPolicy(ctx, team1.ID, &user2.ID, mobius.PolicyPayload{
		Name:        "team1 query",
		Query:       "select 2;",
		Description: "team1 query desc",
		Resolution:  "team1 query resolution",
...
func testFlippingPoliciesForHost(t *testing.T, ds *Datastore) {
	...
	team1, err := ds.NewTeam(ctx, &mobius.Team{Name: "team1"})
	require.NoError(t, err)
	p1, err := ds.NewGlobalPolicy(ctx, &user1.ID, mobius.PolicyPayload{
		Name:  "policy1",
		Query: "select 41;",
	})
	require.NoError(t, err)
	p2, err := ds.NewTeamPolicy(ctx, team1.ID, &user1.ID, mobius.PolicyPayload{
		Name:  "policy2",
		Query: "select 42;",
	})
	require.NoError(t, err)
	...
	require.NoError(t, err)
	pfailed, err := ds.NewTeamPolicy(ctx, team1.ID, &user1.ID, mobius.PolicyPayload{
		Name:  "policy_failed",
		Query: "select * from unexistent_table;",
	})
	require.NoError(t, err)
	p4, err := ds.NewTeamPolicy(ctx, team1.ID, &user1.ID, mobius.PolicyPayload{
		Name:  "policy_failed_to_run_then_pass",
		Query: "select 42;",
	})
	require.NoError(t, err)
	p5, err := ds.NewTeamPolicy(ctx, team1.ID, &user1.ID, mobius.PolicyPayload{
...
func testPolicyPlatformUpdate(t *testing.T, ds *Datastore) {
	ctx := context.Background()
	user := test.NewUser(t, ds, "Alice", "alice@example.com", true)
	tm, err := ds.NewTeam(ctx, &mobius.Team{Name: t.Name()})
	require.NoError(t, err)
	...
	_, err = ds.NewGlobalPolicy(ctx, ptr.Uint(user.ID), mobius.PolicyPayload{Name: "g1", Query: "select 1", Platform: ""})
	require.NoError(t, err)
	// new team policy for any platform
	_, err = ds.NewTeamPolicy(ctx, tm.ID, ptr.Uint(user.ID), mobius.PolicyPayload{Name: "t1", Query: "select 1", Platform: ""})
	require.NoError(t, err)

	// new global and team policies for Linux, via apply spec
	err = ds.ApplyPolicySpecs(ctx, user.ID, []*mobius.PolicySpec{
	...
	// load the global policies
	gpols, err := ds.ListGlobalPolicies(ctx, mobius.ListOptions{})
	require.NoError(t, err)
	require.Len(t, gpols, 2)
	// load the team policies
	tpols, _, err := ds.ListTeamPolicies(ctx, tm.ID, mobius.ListOptions{}, mobius.ListOptions{})
	require.NoError(t, err)
	require.Len(t, tpols, 2)

	// index the policies by name for easier access in the rest of the test
	polsByName := make(map[string]*mobius.Policy, len(gpols)+len(tpols))
...
func assertPolicyMembership(t *testing.T, ds *Datastore, polsByName map[string]*mobius.Policy, wantPolNameToHostIDs map[string][]uint) {
...
func testPolicyCleanupPolicyMembership(t *testing.T, ds *Datastore) {
	...
	platforms := []string{"windows", "darwin", "debian", "linux"}
	hosts := make([]*mobius.Host, len(platforms))
	for i, pl := range platforms {
		id := fmt.Sprintf("%s-%d", strings.ReplaceAll(t.Name(), "/", "_"), i)
		h, err := ds.NewHost(ctx, &mobius.Host{
			OsqueryHostID:   &id,
	...
	// index the policies by name for easier access in the rest of the test
	polsByName := make(map[string]*mobius.Policy, len(pols))
...
func updatePolicyWithTimestamp(t *testing.T, ds *Datastore, p *mobius.Policy, ts time.Time) {
	sqlStmt := `
		UPDATE policies
...
func testIncreasePolicyAutomationIteration(t *testing.T, ds *Datastore) {
	ctx := context.Background()
	pol1, err := ds.NewGlobalPolicy(ctx, nil, mobius.PolicyPayload{Name: "policy1"})
	require.NoError(t, err)
	pol2, err := ds.NewGlobalPolicy(ctx, nil, mobius.PolicyPayload{Name: "policy2"})
	require.NoError(t, err)
...
func testOutdatedAutomationBatch(t *testing.T, ds *Datastore) {
	...
	h1, err := ds.NewHost(ctx, &mobius.Host{OsqueryHostID: ptr.String("host1"), NodeKey: ptr.String("host1")})
	require.NoError(t, err)
	h2, err := ds.NewHost(ctx, &mobius.Host{OsqueryHostID: ptr.String("host2"), NodeKey: ptr.String("host2")})
	require.NoError(t, err)
	...
	require.NoError(t, err)
	require.ElementsMatch(t, batch, []mobius.PolicyFailure{})
...
func testListGlobalPoliciesCanPaginate(t *testing.T, ds *Datastore) {
	// create 30 policies
	for i := 0; i < 30; i++ {
		_, err := ds.NewGlobalPolicy(context.Background(), nil, mobius.PolicyPayload{Name: fmt.Sprintf("global policy %d", i)})
		require.NoError(t, err)
	}

	// create 30 team policies
	tm, err := ds.NewTeam(context.Background(), &mobius.Team{Name: "team1"})
	require.NoError(t, err)
	for i := 0; i < 30; i++ {
		_, err := ds.NewTeamPolicy(context.Background(), tm.ID, nil, mobius.PolicyPayload{Name: fmt.Sprintf("team policy %d", i)})
		require.NoError(t, err)
	}
...
func testListTeamPoliciesCanPaginate(t *testing.T, ds *Datastore) {
	...
	// Page 0 contains 20 policies
	policies, _, err := ds.ListTeamPolicies(context.Background(), tm.ID, mobius.ListOptions{
		Page:    0,
		PerPage: 20,
	}, mobius.ListOptions{})
	...
	// No list options returns all policies
	policies, _, err = ds.ListTeamPolicies(context.Background(), 1, mobius.ListOptions{}, mobius.ListOptions{})
	assert.Len(t, policies, 30)
	require.NoError(t, err)
}
...
func testCountPolicies(t *testing.T, ds *Datastore) {
	...
	// 10 global policies
	for i := 0; i < 10; i++ {
		_, err := ds.NewGlobalPolicy(ctx, nil, mobius.PolicyPayload{Name: fmt.Sprintf("global policy %d", i)})
		require.NoError(t, err)
	}
	...
	// add 5 team policies
	for i := 0; i < 5; i++ {
		_, err := ds.NewTeamPolicy(ctx, tm.ID, nil, mobius.PolicyPayload{Name: fmt.Sprintf("team policy %d", i)})
		require.NoError(t, err)
	}
...
func testUpdatePolicyHostCounts(t *testing.T, ds *Datastore) {
	// new policy
	policy, err := ds.NewGlobalPolicy(context.Background(), nil, mobius.PolicyPayload{Name: "global policy 1"})
	require.NoError(t, err)

	// create 4 global hosts
	var globalHosts []*mobius.Host
	for i := 100; i < 104; i++ {
		h, err := ds.NewHost(
			context.Background(),
			&mobius.Host{OsqueryHostID: ptr.String(fmt.Sprintf("host%d", i)), NodeKey: ptr.String(fmt.Sprintf("host%d", i)), TeamID: nil},
		)
		require.NoError(t, err)
	...
	// create 4 team hosts
	var teamHosts []*mobius.Host
	for i := 0; i < 4; i++ {
		h, err := ds.NewHost(context.Background(), &mobius.Host{OsqueryHostID: ptr.String(fmt.Sprintf("host%d", i)), NodeKey: ptr.String(fmt.Sprintf("host%d", i)), TeamID: &team.ID})
		require.NoError(t, err)
	...
	// new global policy
	policy2, err := ds.NewGlobalPolicy(context.Background(), nil, mobius.PolicyPayload{Name: "global policy 2"})
	require.NoError(t, err)
...
func testPoliciesNameUnicode(t *testing.T, ds *Datastore) {
	...
	// Save policy
	policy, err := ds.NewGlobalPolicy(context.Background(), nil, mobius.PolicyPayload{Name: equivalentNames[0]})
	require.NoError(t, err)
	assert.Equal(t, equivalentNames[0], policy.Name)

	// Try to create policy with equivalent name
	_, err = ds.NewGlobalPolicy(context.Background(), nil, mobius.PolicyPayload{Name: equivalentNames[1]})
	var existsErr *existsError
	assert.ErrorAs(t, err, &existsErr)

	// Try to update a different policy with equivalent name -- not allowed
	policyEmoji, err := ds.NewGlobalPolicy(context.Background(), nil, mobius.PolicyPayload{Name: "💻"})
	require.NoError(t, err)
	err = ds.SavePolicy(
		context.Background(), &mobius.Policy{PolicyData: mobius.PolicyData{ID: policyEmoji.ID, Name: equivalentNames[1]}}, false, false,
	)
	assert.True(t, IsDuplicate(err), err)

	// Try to find policy with equivalent name
	policies, err := ds.ListGlobalPolicies(context.Background(), mobius.ListOptions{MatchQuery: equivalentNames[1]})
	...
	// Create team policy
	teamPolicy, err := ds.NewTeamPolicy(context.Background(), team.ID, nil, mobius.PolicyPayload{Name: equivalentNames[0]})
	require.NoError(t, err)
	assert.Equal(t, equivalentNames[0], teamPolicy.Name)

	// Try to create another team policy with equivalent name -- not allowed
	_, err = ds.NewTeamPolicy(context.Background(), team.ID, nil, mobius.PolicyPayload{Name: equivalentNames[1]})
	assert.ErrorAs(t, err, &existsErr)

	// ListTeamPolicies, including inherited policy
	teamPolicies, inheritedPolicies, err := ds.ListTeamPolicies(
		context.Background(), team.ID, mobius.ListOptions{MatchQuery: equivalentNames[1]}, mobius.ListOptions{MatchQuery: equivalentNames[1]},
	)
	assert.NoError(t, err)
	require.Len(t, teamPolicies, 1)
...
func testPoliciesNameEmoji(t *testing.T, ds *Datastore) {
	...
	emoji0 := "🔥"
	_, err := ds.NewGlobalPolicy(context.Background(), nil, mobius.PolicyPayload{Name: emoji0})
	require.NoError(t, err)
	emoji1 := "💻"
	policyEmoji, err := ds.NewGlobalPolicy(context.Background(), nil, mobius.PolicyPayload{Name: emoji1})
	require.NoError(t, err)
	assert.Equal(t, emoji1, policyEmoji.Name)

	// Try to find policy with emoji0
	policies, err := ds.ListGlobalPolicies(context.Background(), mobius.ListOptions{MatchQuery: emoji0})
	assert.NoError(t, err)
	require.Len(t, policies, 1)
	assert.Equal(t, emoji0, policies[0].Name)

	// Try to find policy with emoji1
	policies, err = ds.ListGlobalPolicies(context.Background(), mobius.ListOptions{MatchQuery: emoji1})
	assert.NoError(t, err)
	require.Len(t, policies, 1)
...
func testPoliciesNameSort(t *testing.T, ds *Datastore) {
	var policies [3]*mobius.Policy
	var err error
	// Save policy
	policies[1], err = ds.NewGlobalPolicy(context.Background(), nil, mobius.PolicyPayload{Name: "В"})
	require.NoError(t, err)
	policies[2], err = ds.NewGlobalPolicy(context.Background(), nil, mobius.PolicyPayload{Name: "о"})
	require.NoError(t, err)
	policies[0], err = ds.NewGlobalPolicy(context.Background(), nil, mobius.PolicyPayload{Name: "а"})
	require.NoError(t, err)

	policiesResult, err := ds.ListGlobalPolicies(context.Background(), mobius.ListOptions{OrderKey: "name"})
	assert.NoError(t, err)
	require.Len(t, policies, 3)
...
func testGetCalendarPolicies(t *testing.T, ds *Datastore) {
	...
	team, err := ds.NewTeam(ctx, &mobius.Team{
		Name: "Foobar",
	})
	require.NoError(t, err)
	...
	_, err = ds.NewGlobalPolicy(ctx, nil, mobius.PolicyPayload{
		Name:  "Global Policy",
		Query: "SELECT * FROM time;",
	})
	require.NoError(t, err)

	_, err = ds.NewTeamPolicy(ctx, team.ID, nil, mobius.PolicyPayload{
		Name:                  "Team Policy 1",
	...
	teamPolicy2, err := ds.NewTeamPolicy(ctx, team.ID, nil, mobius.PolicyPayload{
		Name:                  "Team Policy 2",
		Query:                 "SELECT * FROM osquery_info;",
		CalendarEventsEnabled: true,
	})
	require.NoError(t, err)
	teamPolicy3, err := ds.NewTeamPolicy(ctx, team.ID, nil, mobius.PolicyPayload{
		Name:                  "Team Policy 3",
...
func testGetTeamHostsPolicyMemberships(t *testing.T, ds *Datastore) {
	...
	team1Policy1, err := ds.NewTeamPolicy(ctx, team1.ID, nil, mobius.PolicyPayload{
		Name:                  "Team 1 Policy 1",
		Query:                 "SELECT * FROM osquery_info;",
		CalendarEventsEnabled: true,
	})
	require.NoError(t, err)
	team1Policy2, err := ds.NewTeamPolicy(ctx, team1.ID, nil, mobius.PolicyPayload{
		Name:                  "Team 1 Policy 2",
		Query:                 "SELECT * FROM system_info;",
		CalendarEventsEnabled: false,
	})
	require.NoError(t, err)
	team2Policy1, err := ds.NewTeamPolicy(ctx, team2.ID, nil, mobius.PolicyPayload{
		Name:                  "Team 2 Policy 1",
		Query:                 "SELECT * FROM os_version;",
		CalendarEventsEnabled: true,
	})
	require.NoError(t, err)
	team2Policy2, err := ds.NewTeamPolicy(ctx, team2.ID, nil, mobius.PolicyPayload{
	...
	require.NoError(t, err)
	host4, err := ds.NewHost(ctx, &mobius.Host{
	...
	require.NoError(t, err)
	host5, err := ds.NewHost(ctx, &mobius.Host{
	...
	require.NoError(t, err)
	host6, err := ds.NewHost(ctx, &mobius.Host{
	...
	err = ds.ReplaceHostDeviceMapping(ctx, host1.ID, []*mobius.HostDeviceMapping{
	...
	require.NoError(t, err)
	err = ds.ReplaceHostDeviceMapping(ctx, host2.ID, []*mobius.HostDeviceMapping{
	...
	require.NoError(t, err)
	err = ds.ReplaceHostDeviceMapping(ctx, host3.ID, []*mobius.HostDeviceMapping{
		{HostID: host3.ID, Email: "zoo@example.com", Source: "google_chrome_profiles"},
	}, "google_chrome_profiles")
	require.NoError(t, err)
	err = ds.ReplaceHostDeviceMapping(ctx, host4.ID, []*mobius.HostDeviceMapping{
		{HostID: host4.ID, Email: "foo@example.com", Source: "google_chrome_profiles"},
	}, "google_chrome_profiles")
	require.NoError(t, err)
	err = ds.ReplaceHostDeviceMapping(ctx, host5.ID, []*mobius.HostDeviceMapping{
		{HostID: host5.ID, Email: "foo@other.com", Source: "google_chrome_profiles"},
	}, "google_chrome_profiles")
	require.NoError(t, err)
	err = ds.ReplaceHostDeviceMapping(ctx, host6.ID, []*mobius.HostDeviceMapping{
	...
	_, err = ds.CreateOrUpdateCalendarEvent(ctx, eventUUID1, "foo@example.com", now, now.Add(30*time.Minute), []byte(`{"foo": "bar"}`), &tZ,
		host1.ID, mobius.CalendarWebhookStatusPending)
	require.NoError(t, err)
	...
	_, err = ds.CreateOrUpdateCalendarEvent(ctx, eventUUID2, "bar@example.com", now, now.Add(30*time.Minute), []byte(`{"foo": "bar"}`), &tZ,
		host6.ID, mobius.CalendarWebhookStatusPending)
	require.NoError(t, err)
	...
	now = time.Now()
	_, err = ds.CreateOrUpdateCalendarEvent(ctx, eventUUID1, "foo@example.com", now, now.Add(30*time.Minute), []byte(`{"foo": "bar"}`), &tZ,
		host2.ID, mobius.CalendarWebhookStatusPending)
	require.NoError(t, err)
	...
	calendarEventHost3, err := ds.CreateOrUpdateCalendarEvent(ctx, eventUUID3, "zoo@example.com", now, now.Add(30*time.Minute),
		[]byte(`{"foo": "bar"}`), &tZ, host3.ID, mobius.CalendarWebhookStatusPending)
	require.NoError(t, err)
...
func testTeamPoliciesWithInstaller(t *testing.T, ds *Datastore) {
	...
	team1, err := ds.NewTeam(ctx, &mobius.Team{Name: "team1"})
	require.NoError(t, err)
	team2, err := ds.NewTeam(ctx, &mobius.Team{Name: "team2"}) // team2 has no policies
	require.NoError(t, err)

	// Policy p1 has no associated installer.
	p1, err := ds.NewTeamPolicy(ctx, team1.ID, &user1.ID, mobius.PolicyPayload{
		Name:                "p1",
		Query:               "SELECT 1;",
	...
	installer1, err := mobius.NewTempFileReader(strings.NewReader("hello"), t.TempDir)
	require.NoError(t, err)
	noTeamInstallerID, _, err := ds.MatchOrCreateSoftwareInstaller(context.Background(), &mobius.UploadSoftwareInstallerPayload{
		InstallScript:     "hello",
		PreInstallQuery:   "SELECT 1",
		PostInstallScript: "world",
		...
		Source:            "apps",
		UserID:            user1.ID,
		TeamID:            ptr.Uint(mobius.PolicyNoTeamID),
		ValidatedLabels:   &mobius.LabelIdentsWithScope{},
	})
	require.NoError(t, err)
	p4, err := ds.NewTeamPolicy(ctx, mobius.PolicyNoTeamID, &user1.ID, mobius.PolicyPayload{
		Name:                "p4",
		Query:               "SELECT 4;",
		SoftwareInstallerID: ptr.Uint(noTeamInstallerID),
	})
	require.NoError(t, err)
	_, err = ds.NewTeamPolicy(ctx, mobius.PolicyNoTeamID, &user1.ID, mobius.PolicyPayload{
		Name:                "p4",
		Query:               "SELECT 4;",
		SoftwareInstallerID: ptr.Uint(installerID),
	})
	...
	policiesWithInstallers, err := ds.GetPoliciesWithAssociatedInstaller(ctx, mobius.PolicyNoTeamID, []uint{p4.ID})
	require.NoError(t, err)
...
func testTeamPoliciesWithVPP(t *testing.T, ds *Datastore) {
	...
	// create team1 app
	team1App, err := ds.InsertVPPAppWithTeam(ctx, &mobius.VPPApp{
		Name: "vpp1", BundleIdentifier: "com.app.appy",
		VPPAppTeam: mobius.VPPAppTeam{VPPAppID: mobius.VPPAppID{AdamID: "adam_app", Platform: mobius.MacOSPlatform}},
	}, &team1.ID)
	require.NoError(t, err)
	...
	// create no-team app
	noTeamApp, err := ds.InsertVPPAppWithTeam(ctx, &mobius.VPPApp{
		Name: "vpp1", BundleIdentifier: "com.app.appy",
		VPPAppTeam: mobius.VPPAppTeam{VPPAppID: mobius.VPPAppID{AdamID: "adam_app", Platform: mobius.MacOSPlatform}},
	}, nil)
	require.NoError(t, err)
	...
	team1App2, err := ds.InsertVPPAppWithTeam(ctx, &mobius.VPPApp{
		Name: "vpp2", BundleIdentifier: "com.app.vpp2",
		VPPAppTeam: mobius.VPPAppTeam{VPPAppID: mobius.VPPAppID{AdamID: "adam_vpp2", Platform: mobius.MacOSPlatform}},
	}, &team1.ID)
	require.NoError(t, err)
	...
	policiesWithVPPs, err := ds.GetPoliciesWithAssociatedVPP(ctx, mobius.PolicyNoTeamID, []uint{p4.ID})
	require.NoError(t, err)
	...
	team1App3, err := ds.InsertVPPAppWithTeam(ctx, &mobius.VPPApp{
		Name: "vpp3", BundleIdentifier: "com.app.vpp3",
		VPPAppTeam: mobius.VPPAppTeam{VPPAppID: mobius.VPPAppID{AdamID: "adam_vpp3", Platform: mobius.MacOSPlatform}, AddAutoInstallPolicy: true},
	}, &team1.ID)
	require.NoError(t, err)
...
func testTeamPoliciesWithScript(t *testing.T, ds *Datastore) {
	...
	script, err := ds.NewScript(context.Background(), &mobius.Script{
		TeamID:         &team1.ID,
		Name:           "hello-world.sh",
	...
	noTeamScript, err := ds.NewScript(context.Background(), &mobius.Script{
		Name:           "hello-world.sh",
	...
	policiesWithScripts, err := ds.GetPoliciesWithAssociatedScript(ctx, mobius.PolicyNoTeamID, []uint{p4.ID})
	require.NoError(t, err)
...
func testApplyPolicySpecWithInstallers(t *testing.T, ds *Datastore) {
	...
	tfr1, err := mobius.NewTempFileReader(strings.NewReader("hello1"), t.TempDir)
	require.NoError(t, err)
	installer1ID, _, err := ds.MatchOrCreateSoftwareInstaller(ctx, &mobius.UploadSoftwareInstallerPayload{
		InstallScript:     "hello",
		PreInstallQuery:   "SELECT 1;",
	...
	tfr2, err := mobius.NewTempFileReader(strings.NewReader("hello2"), t.TempDir)
	require.NoError(t, err)
	installer2ID, _, err := ds.MatchOrCreateSoftwareInstaller(ctx, &mobius.UploadSoftwareInstallerPayload{
	...
	tfr3, err := mobius.NewTempFileReader(strings.NewReader("hello3"), t.TempDir)
	require.NoError(t, err)
	installer3ID, _, err := ds.MatchOrCreateSoftwareInstaller(ctx, &mobius.UploadSoftwareInstallerPayload{
	...
	tfr5, err := mobius.NewTempFileReader(strings.NewReader("hello5"), t.TempDir)
	require.NoError(t, err)
	installer5ID, _, err := ds.MatchOrCreateSoftwareInstaller(ctx, &mobius.UploadSoftwareInstallerPayload{
	...
	// create VPP apps
	va1, err := ds.InsertVPPAppWithTeam(ctx, &mobius.VPPApp{
		Name: "vpp1", BundleIdentifier: "com.app.vpp1",
		VPPAppTeam: mobius.VPPAppTeam{VPPAppID: mobius.VPPAppID{AdamID: "adam_vpp_app_1", Platform: mobius.MacOSPlatform}},
	}, &team1.ID)
	require.NoError(t, err)
	va2, err := ds.InsertVPPAppWithTeam(ctx, &mobius.VPPApp{
		Name: "vpp2", BundleIdentifier: "com.app.vpp2",
		VPPAppTeam: mobius.VPPAppTeam{VPPAppID: mobius.VPPAppID{AdamID: "adam_vpp_app_2", Platform: mobius.MacOSPlatform}},
	}, &team2.ID)
	require.NoError(t, err)
	va1NoTeam, err := ds.InsertVPPAppWithTeam(ctx, &mobius.VPPApp{
		Name: "vpp1", BundleIdentifier: "com.app.vpp1",
		VPPAppTeam: mobius.VPPAppTeam{VPPAppID: mobius.VPPAppID{AdamID: "adam_vpp_app_1", Platform: mobius.MacOSPlatform}},
	}, nil)
	require.NoError(t, err)
	...
	err = ds.ApplyPolicySpecs(ctx, user1.ID, []*mobius.PolicySpec{
		{
			Name:            "Global policy",
			Query:           "SELECT 1;",
			Description:     "Description",
	...
	require.NoError(t, err)
	team1Policies, _, err := ds.ListTeamPolicies(ctx, team1.ID, mobius.ListOptions{}, mobius.ListOptions{})
	require.NoError(t, err)
	...
	team2Policies, _, err := ds.ListTeamPolicies(ctx, team2.ID, mobius.ListOptions{}, mobius.ListOptions{})
	require.NoError(t, err)
	require.Len(t, team2Policies, 2)
	...
	tfr4, err := mobius.NewTempFileReader(strings.NewReader("hello3"), t.TempDir)
	require.NoError(t, err)
	installer4ID, _, err := ds.MatchOrCreateSoftwareInstaller(ctx, &mobius.UploadSoftwareInstallerPayload{
		InstallScript:     "hello3",
		PreInstallQuery:   "SELECT 3;",
		PostInstallScript: "world3",
		InstallerFile:     tfr4,
	...
	require.NoError(t, err)
	require.NotNil(t, installer2.TitleID)
	va4Team2, err := ds.InsertVPPAppWithTeam(ctx, &mobius.VPPApp{
		Name: "vpp4", BundleIdentifier: "com.app.vpp4",
		VPPAppTeam: mobius.VPPAppTeam{VPPAppID: mobius.VPPAppID{AdamID: "adam_vpp_app_4", Platform: mobius.MacOSPlatform}},
	}, &team2.ID)
	require.NoError(t, err)
	...
	va4Team1, err := ds.InsertVPPAppWithTeam(ctx, &mobius.VPPApp{
		Name: "vpp4", BundleIdentifier: "com.app.vpp4",
		VPPAppTeam: mobius.VPPAppTeam{VPPAppID: mobius.VPPAppID{AdamID: "adam_vpp_app_4", Platform: mobius.MacOSPlatform}},
	}, &team1.ID)
	require.NoError(t, err)
...
func testTeamPoliciesNoTeam(t *testing.T, ds *Datastore) {
	...
	policy0NoTeam, err := ds.NewTeamPolicy(ctx, mobius.PolicyNoTeamID, &user1.ID, mobius.PolicyPayload{
		Name:  "policy0NoTeam",
		Query: "SELECT 0;",
	})
	require.NoError(t, err)
	require.NotNil(t, policy0NoTeam.TeamID)
	require.Equal(t, mobius.PolicyNoTeamID, *policy0NoTeam.TeamID)
	tp, err := ds.TeamPolicy(ctx, mobius.PolicyNoTeamID, policy0NoTeam.ID)
	require.NoError(t, err)
	require.Equal(t, tp, policy0NoTeam)

	policy1Team1, err := ds.NewTeamPolicy(ctx, team1.ID, &user1.ID, mobius.PolicyPayload{
		Name:  "policy1Team1",
		Query: "SELECT 1;",
	})
	require.NoError(t, err)
	policy2Team2, err := ds.NewTeamPolicy(ctx, team2.ID, &user1.ID, mobius.PolicyPayload{
		Name:  "policy2Team2",
		Query: "SELECT 2;",
	})
	require.NoError(t, err)
	policy3NoTeam, err := ds.NewTeamPolicy(ctx, mobius.PolicyNoTeamID, &user1.ID, mobius.PolicyPayload{
		Name:  "policy3NoTeam",
		Query: "SELECT 3;",
	})
	require.NoError(t, err)
	policy4Team2, err := ds.NewTeamPolicy(ctx, team2.ID, &user1.ID, mobius.PolicyPayload{
		Name:  "policy4Team2",
		Query: "SELECT 4;",
	})
	require.NoError(t, err)

	globalPolicy1, err := ds.NewGlobalPolicy(ctx, &user1.ID, mobius.PolicyPayload{
		Name:  "globalPolicy1",
		Query: "SELECT gp1;",
	})
	require.NoError(t, err)
	globalPolicy2, err := ds.NewGlobalPolicy(ctx, &user1.ID, mobius.PolicyPayload{
		Name:  "globalPolicy2",
		Query: "SELECT gp2;",
	})
	require.NoError(t, err)
	...
	require.NoError(t, err)
	require.Equal(t, 1, c)
	mergedTeamPolicies, err := ds.ListMergedTeamPolicies(ctx, team1.ID, mobius.ListOptions{})
	require.NoError(t, err)
	...
	teamPolicies, inheritedPolicies, err = ds.ListTeamPolicies(ctx, team2.ID, mobius.ListOptions{}, mobius.ListOptions{})
	require.NoError(t, err)
	require.Len(t, teamPolicies, 2)
	...
	require.NoError(t, err)
	require.Equal(t, 2, c)
	mergedTeamPolicies, err = ds.ListMergedTeamPolicies(ctx, team2.ID, mobius.ListOptions{})
	require.NoError(t, err)
	...
	teamPolicies, inheritedPolicies, err = ds.ListTeamPolicies(ctx, mobius.PolicyNoTeamID, mobius.ListOptions{}, mobius.ListOptions{})
	require.NoError(t, err)
	require.Len(t, teamPolicies, 2)
	...
	require.Equal(t, teamPoliciesByID[teamPolicies[0].ID], teamPolicies[0])
	require.Equal(t, teamPoliciesByID[teamPolicies[1].ID], teamPolicies[1])
	c, err = ds.CountMergedTeamPolicies(ctx, mobius.PolicyNoTeamID, "")
	require.NoError(t, err)
	require.Equal(t, 4, c)
	c, err = ds.CountPolicies(ctx, ptr.Uint(mobius.PolicyNoTeamID), "")
	require.NoError(t, err)
	require.Equal(t, 2, c)
	mergedTeamPolicies, err = ds.ListMergedTeamPolicies(ctx, mobius.PolicyNoTeamID, mobius.ListOptions{})
	require.NoError(t, err)
	require.Len(t, mergedTeamPolicies, 4)
...
func testPoliciesBySoftwareTitleID(t *testing.T, ds *Datastore) {
	...
	installer1ID, _, err := ds.MatchOrCreateSoftwareInstaller(context.Background(), &mobius.UploadSoftwareInstallerPayload{
		InstallScript:     "hello",
		PreInstallQuery:   "SELECT 1",
		PostInstallScript: "world",
		InstallerFile:     installer,
	...
	installer2ID, _, err := ds.MatchOrCreateSoftwareInstaller(context.Background(), &mobius.UploadSoftwareInstallerPayload{
		InstallScript:     "hello",
		PreInstallQuery:   "SELECT 1",
		PostInstallScript: "world",
		InstallerFile:     installer,
	...
	installer3ID, _, err := ds.MatchOrCreateSoftwareInstaller(context.Background(), &mobius.UploadSoftwareInstallerPayload{
	...
	installer4ID, _, err := ds.MatchOrCreateSoftwareInstaller(context.Background(), &mobius.UploadSoftwareInstallerPayload{
		InstallScript:     "hello noteam",
		PreInstallQuery:   "SELECT 1 from noteam",
		PostInstallScript: "world",
		InstallerFile:     installer,
	...
	require.NoError(t, err)
	require.Len(t, policies, 2)
	expected := map[uint]mobius.AutomaticInstallPolicy{
...
func testClearAutoInstallPolicyStatusForHost(t *testing.T, ds *Datastore) {
	...
	user1 := test.NewUser(t, ds, "Alice", "alice@example.com", true)
	team1, err := ds.NewTeam(ctx, &mobius.Team{Name: "team1" + t.Name()})
	require.NoError(t, err)
	test.CreateInsertGlobalVPPToken(t, ds)
	...
	err = ds.RecordPolicyQueryExecutions(ctx, host, map[uint]*bool{
		...
		policy2.ID: ptr.Bool(false), // software isn't installed on host, so Mobius should install it
		policy3.ID: ptr.Bool(false), // software isn't installed on host, so Mobius should install it
	}, time.Now(), false)
	require.NoError(t, err)
...
func testPolicyLabels(t *testing.T, ds *Datastore) {
	...
	assertPolicies := func(t *testing.T, havePolicies []*mobius.HostPolicy, wantPolicies []*mobius.Policy, hostName string) {
	...
	assertQueries := func(t *testing.T, havePolicies map[string]string, wantPolicies []*mobius.Policy, hostName string) {
		haveMap := map[uint]string{}
		wantMap := map[uint]string{}
		missingPolicies := []string{}
		extraPolicies := []string{}
	...
	policyIncludeLabel1.LabelsIncludeAny = []mobius.LabelIdent{{LabelName: label1.Name}}
	...
	policyExcludeLabel2.LabelsExcludeAny = []mobius.LabelIdent{{LabelName: label2.Name}}
	...
	policyIncludeBoth.LabelsIncludeAny = []mobius.LabelIdent{{LabelName: label1.Name}, {LabelName: label2.Name}}
	...
	policyExcludeBoth.LabelsExcludeAny = []mobius.LabelIdent{{LabelName: label1.Name}, {LabelName: label2.Name}}
...
func testDeletePolicyWithSoftwareActivatesNextActivity(t *testing.T, ds *Datastore) {
	...
	installer, err := mobius.NewTempFileReader(strings.NewReader("hello"), t.TempDir)
	require.NoError(t, err)
	installerIDTm, _, err := ds.MatchOrCreateSoftwareInstaller(ctx, &mobius.UploadSoftwareInstallerPayload{
		InstallScript:   "hello",
		InstallerFile:   installer,
		StorageID:       "storage1",
		Filename:        "file1",
	...
	require.NoError(t, err)
	installer, err = mobius.NewTempFileReader(strings.NewReader("hello2"), t.TempDir)
	require.NoError(t, err)
	installerIDNoTm, _, err := ds.MatchOrCreateSoftwareInstaller(ctx, &mobius.UploadSoftwareInstallerPayload{
		InstallScript:   "hello2",
		InstallerFile:   installer,
		StorageID:       "storage2",
		Filename:        "file2",
	...
	policyTm, err := ds.NewTeamPolicy(ctx, team1.ID, &u.ID, mobius.PolicyPayload{
		Name:                "p1",
		Query:               "SELECT 1;",
		SoftwareInstallerID: ptr.Uint(installerIDTm),
	})
	require.NoError(t, err)
	policyNoTm, err := ds.NewTeamPolicy(ctx, mobius.PolicyNoTeamID, &u.ID, mobius.PolicyPayload{
		Name:                "p2",
		Query:               "SELECT 2;",
	...
	scriptExec, err := ds.NewHostScriptExecutionRequest(ctx, &mobius.HostScriptRequestPayload{
		HostID:         hostNoTm.ID,
		ScriptContents: "echo",
		UserID:         &u.ID,
		SyncRequest:    true,
	...
	installUUIDNoTm, err := ds.InsertSoftwareInstallRequest(ctx, hostNoTm.ID, installerIDNoTm,
		mobius.HostSoftwareInstallOptions{
			SelfService: false,
	...
	activities, _, err := ds.ListHostUpcomingActivities(ctx, hostNoTm.ID, mobius.ListOptions{})
	require.NoError(t, err)
	require.Len(t, activities, 2)
	require.Equal(t, scriptExec.ExecutionID, activities[0].UUID)
	require.Equal(t, installUUIDNoTm, activities[1].UUID)

	activities, _, err = ds.ListHostUpcomingActivities(ctx, hostTm.ID, mobius.ListOptions{})
	require.NoError(t, err)
	...
	require.NoError(t, err)
	_, err = ds.DeleteTeamPolicies(ctx, mobius.PolicyNoTeamID, []uint{policyNoTm.ID})
	require.NoError(t, err)
...
func testDeletePolicyWithScriptActivatesNextActivity(t *testing.T, ds *Datastore) {
	...
	// one for no team
	scriptTm, err := ds.NewScript(ctx, &mobius.Script{
		Name:           "script1.sh",
		ScriptContents: "echo",
		TeamID:         &team1.ID,
	})
	require.NoError(t, err)
	scriptNoTm, err := ds.NewScript(ctx, &mobius.Script{
		Name:           "script2.sh",
		ScriptContents: "echo",
		TeamID:         nil,
	})
	...
	hsrPolicyNoTm, err := ds.NewHostScriptExecutionRequest(ctx, &mobius.HostScriptRequestPayload{
		HostID:         hostNoTm.ID,
		ScriptContents: "echo",
		UserID:         &u.ID,
		PolicyID:       &policyNoTm.ID,
	...
	require.NoError(t, err)
	hsrPolicyTm, err := ds.NewHostScriptExecutionRequest(ctx, &mobius.HostScriptRequestPayload{
		HostID:         hostTm.ID,
		ScriptContents: "echo",
		UserID:         &u.ID,
		PolicyID:       &policyTm.ID,
