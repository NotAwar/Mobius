...
func (ds *Datastore) OverwriteQueryResultRows(ctx context.Context, rows []*mobius.ScheduledQueryResultRow, maxQueryReportRows int) (err error) {
	if len(rows) == 0 {
		return nil
	}
...
func (ds *Datastore) QueryResultRows(ctx context.Context, queryID uint, filter mobius.TeamFilter) ([]*mobius.ScheduledQueryResultRow, error) {
	...
	results := []*mobius.ScheduledQueryResultRow{}
...
func (ds *Datastore) QueryResultRowsForHost(ctx context.Context, queryID, hostID uint) ([]*mobius.ScheduledQueryResultRow, error) {
	selectStmt := `
