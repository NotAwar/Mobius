...
func testGetQueryResultRows(t *testing.T, ds *Datastore) {
	...
	query1Rows := []*mobius.ScheduledQueryResultRow{
		{
			QueryID:     query.ID,
			HostID:      host.ID,
	...
	err := ds.OverwriteQueryResultRows(context.Background(), query1Rows, mobius.DefaultMaxQueryReportRows)
	require.NoError(t, err)

	// Insert Result Row for different Scheduled Query
	query2 := test.NewQuery(t, ds, nil, "New Query 2", "SELECT 1", user.ID, true)
	query2Rows := []*mobius.ScheduledQueryResultRow{
		{
			QueryID:     query2.ID,
			HostID:      host.ID,
			LastFetched: mockTime,
	...
	err = ds.OverwriteQueryResultRows(context.Background(), query2Rows, mobius.DefaultMaxQueryReportRows)
	require.NoError(t, err)

	results, err := ds.QueryResultRows(context.Background(), query.ID, mobius.TeamFilter{User: test.UserAdmin})
	require.NoError(t, err)
	...
	results, err = ds.QueryResultRows(context.Background(), query2.ID, mobius.TeamFilter{User: test.UserAdmin})
	require.NoError(t, err)
...
func testGetQueryResultRowsForHost(t *testing.T, ds *Datastore) {
	...
	host1ResultRows := []*mobius.ScheduledQueryResultRow{
		{
			QueryID:     query.ID,
			HostID:      host1.ID,
			LastFetched: mockTime,
	...
	err := ds.OverwriteQueryResultRows(context.Background(), host1ResultRows, mobius.DefaultMaxQueryReportRows)
	require.NoError(t, err)

	// Insert 1 Result Row for Query1 Host2
	host2ResultRows := []*mobius.ScheduledQueryResultRow{
		{
			QueryID:     query.ID,
			HostID:      host2.ID,
			LastFetched: mockTime,
	...
	err = ds.OverwriteQueryResultRows(context.Background(), host2ResultRows, mobius.DefaultMaxQueryReportRows)
	require.NoError(t, err)
...
func testQueryResultRowsTeamFilter(t *testing.T, ds *Datastore) {
	...
	require.NoError(t, err)
	observerTeam, err := ds.NewTeam(context.Background(), &mobius.Team{
		Name: "observerTeam",
	})
	require.NoError(t, err)

	teamUser, err := ds.NewUser(context.Background(), &mobius.User{
		Password:   []byte("foo"),
		Salt:       "bar",
		Name:       "teamUser",
	...
	globalRow := []*mobius.ScheduledQueryResultRow{
		{
			QueryID:     query.ID,
	...
	err = ds.OverwriteQueryResultRows(context.Background(), globalRow, mobius.DefaultMaxQueryReportRows)
	require.NoError(t, err)

	teamRow := []*mobius.ScheduledQueryResultRow{
		{
			QueryID:     query.ID,
			HostID:      teamHost.ID,
			LastFetched: mockTime,
	...
	err = ds.OverwriteQueryResultRows(context.Background(), teamRow, mobius.DefaultMaxQueryReportRows)
	require.NoError(t, err)

	observerTeamRow := []*mobius.ScheduledQueryResultRow{
		{
			QueryID:     query.ID,
	...
	err = ds.OverwriteQueryResultRows(context.Background(), observerTeamRow, mobius.DefaultMaxQueryReportRows)
	require.NoError(t, err)
...
func testCountResultsForQuery(t *testing.T, ds *Datastore) {
	...
	host1ResultRow := []*mobius.ScheduledQueryResultRow{
		{
			QueryID:     query1.ID,
			HostID:      host.ID,
			LastFetched: mockTime,
	...
	err := ds.OverwriteQueryResultRows(context.Background(), host1ResultRow, mobius.DefaultMaxQueryReportRows)
	require.NoError(t, err)

	// Insert Nil Result Row for Query1, nil data rows are not counted
	host2ResultRow := []*mobius.ScheduledQueryResultRow{
		{
			QueryID:     query1.ID,
			HostID:      host2.ID,
			LastFetched: mockTime,
	...
	err = ds.OverwriteQueryResultRows(context.Background(), host2ResultRow, mobius.DefaultMaxQueryReportRows)
	require.NoError(t, err)

	// Insert 5 Result Rows for Query2
	resultRow2 := &mobius.ScheduledQueryResultRow{
		QueryID:     query2.ID,
		HostID:      host.ID,
		LastFetched: mockTime,
		Data: ptr.RawMessage([]byte(`{
	...
	var resultRows []*mobius.ScheduledQueryResultRow
	for i := 0; i < 5; i++ {
		resultRows = append(resultRows, resultRow2)
	}

	err = ds.OverwriteQueryResultRows(context.Background(), resultRows, mobius.DefaultMaxQueryReportRows)
	require.NoError(t, err)
...
func testCountResultsForQueryAndHost(t *testing.T, ds *Datastore) {
	...
	host1Query2 := []*mobius.ScheduledQueryResultRow{
		{
			QueryID:     query2.ID,
			HostID:      host1.ID,
			LastFetched: mockTime,
	...
	err = ds.OverwriteQueryResultRows(context.Background(), host1Query2, mobius.DefaultMaxQueryReportRows)
	require.NoError(t, err)
	...
	host3ResultRow := []*mobius.ScheduledQueryResultRow{
		{
			QueryID:     query2.ID,
			HostID:      host3.ID,
			LastFetched: mockTime,
	...
	err = ds.OverwriteQueryResultRows(context.Background(), host3ResultRow, mobius.DefaultMaxQueryReportRows)
	require.NoError(t, err)
...
func testOverwriteQueryResultRows(t *testing.T, ds *Datastore) {
	...
	initialRow := []*mobius.ScheduledQueryResultRow{
		{
			QueryID:     query.ID,
			HostID:      host.ID,
			LastFetched: mockTime,
	...
	err := ds.OverwriteQueryResultRows(context.Background(), initialRow, mobius.DefaultMaxQueryReportRows)
	require.NoError(t, err)

	// Overwrite Result Rows with new data
	newMockTime := mockTime.Add(2 * time.Minute)
	overwriteRows := []*mobius.ScheduledQueryResultRow{
		{
			QueryID:     query.ID,
			HostID:      host.ID,
	...
	err = ds.OverwriteQueryResultRows(context.Background(), overwriteRows, mobius.DefaultMaxQueryReportRows)
	require.NoError(t, err)
...
func testQueryResultRowsDoNotExceedMaxRows(t *testing.T, ds *Datastore) {
	...
	// Generate max rows -1
	maxRows := mobius.DefaultMaxQueryReportRows - 1
	maxMinusOneRows := make([]*mobius.ScheduledQueryResultRow, maxRows)
	for i := 0; i < maxRows; i++ {
		maxMinusOneRows[i] = &mobius.ScheduledQueryResultRow{
			QueryID:     query.ID,
			HostID:      host1.ID,
			LastFetched: mockTime,
			Data:        ptr.RawMessage([]byte(`{"model": "USB Mouse", "vendor": "Logitech"}`)),
	...
	err := ds.OverwriteQueryResultRows(context.Background(), maxMinusOneRows, mobius.DefaultMaxQueryReportRows)
	require.NoError(t, err)

	// Add an empty data rows which do not count towards the max
	err = ds.OverwriteQueryResultRows(context.Background(), []*mobius.ScheduledQueryResultRow{
		{
			QueryID:     query.ID,
			HostID:      host2.ID,
			LastFetched: mockTime,
	...
	require.NoError(t, err)
	require.Equal(t, mobius.DefaultMaxQueryReportRows, count)
	...
	rows := mobius.DefaultMaxQueryReportRows + 50
	largeBatchRows := make([]*mobius.ScheduledQueryResultRow, rows)
	for i := 0; i < rows; i++ {
		largeBatchRows[i] = &mobius.ScheduledQueryResultRow{
			QueryID:     query2.ID,
			HostID:      host1.ID,
			LastFetched: mockTime,
			Data:        ptr.RawMessage([]byte(`{"model": "USB Mouse", "vendor": "Logitech"}`)),
	...
	err = ds.OverwriteQueryResultRows(context.Background(), largeBatchRows, mobius.DefaultMaxQueryReportRows)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	require.Len(t, allResults, mobius.DefaultMaxQueryReportRows)
...
func testQueryResultRows(t *testing.T, ds *Datastore) {
	...
	filter := mobius.TeamFilter{User: user, IncludeObserver: true}
...
func testCleanupQueryResultRows(t *testing.T, ds *Datastore) {
	...
	rows := []*mobius.ScheduledQueryResultRow{
		{
	...
	err = ds.OverwriteQueryResultRows(context.Background(), rows, mobius.DefaultMaxQueryReportRows)
	require.NoError(t, err)
	...
	results, err := ds.QueryResultRows(context.Background(), queryNoDiscard.ID, mobius.TeamFilter{User: user})
	require.NoError(t, err)
	require.Len(t, results, 2)

	// Verify that the rows with discard data set to true are removed
	results, err = ds.QueryResultRows(context.Background(), queryDiscardTrue.ID, mobius.TeamFilter{User: user})
	require.NoError(t, err)
	require.Len(t, results, 0)
}
