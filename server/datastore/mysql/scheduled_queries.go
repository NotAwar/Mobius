...
func (ds *Datastore) ListScheduledQueriesInPackWithStats(ctx context.Context, id uint, opts mobius.ListOptions) ([]*mobius.ScheduledQuery, error) {
	query := `
		SELECT
			sq.id,
			sq.pack_id,
		...
		WHERE sq.pack_id = ?
	`
	params := []interface{}{false, mobius.AggregatedStatsTypeScheduledQuery, id}
	query, params = appendListOptionsWithCursorToSQL(query, params, &opts)
	results := []*mobius.ScheduledQuery{}
...
func (ds *Datastore) ListScheduledQueriesInPack(ctx context.Context, id uint) (mobius.ScheduledQueryList, error) {
	query := `
		SELECT
			sq.id,
			sq.pack_id,
...
func (ds *Datastore) NewScheduledQuery(ctx context.Context, sq *mobius.ScheduledQuery, opts ...mobius.OptionalArg) (*mobius.ScheduledQuery, error) {
	return insertScheduledQueryDB(ctx, ds.writer(ctx), sq)
}

func insertScheduledQueryDB(ctx context.Context, q sqlx.ExtContext, sq *mobius.ScheduledQuery) (*mobius.ScheduledQuery, error) {
...
func (ds *Datastore) SaveScheduledQuery(ctx context.Context, sq *mobius.ScheduledQuery) (*mobius.ScheduledQuery, error) {
	return saveScheduledQueryDB(ctx, ds.writer(ctx), sq)
}

func saveScheduledQueryDB(ctx context.Context, exec sqlx.ExecerContext, sq *mobius.ScheduledQuery) (*mobius.ScheduledQuery, error) {
	query := `
...
func (ds *Datastore) ScheduledQuery(ctx context.Context, id uint) (*mobius.ScheduledQuery, error) {
	query := `
		SELECT
			sq.id,
			sq.created_at,
...
`
	...
	if batchSize <= 0 {
		batchSize = mobius.DefaultScheduledQueryIDsByNameBatchSize
	}
...
func (ds *Datastore) AsyncBatchSaveHostsScheduledQueryStats(ctx context.Context, stats map[uint][]mobius.ScheduledQueryStats, batchSize int) (int, error) {
