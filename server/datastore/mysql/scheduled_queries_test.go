...
func testScheduledQueriesListInPackWithStats(t *testing.T, ds *Datastore) {
	...
	queries := []*mobius.Query{
		...
		{Name: "bar", Description: "do some bars", Query: "select baz from bar", Logging: mobius.LoggingDifferential},
	}
	...
	specs := []*mobius.PackSpec{
		{
			Name:    "baz",
			Targets: mobius.PackSpecTargets{Labels: []string{}},
	...
	gotQueries, err := ds.ListScheduledQueriesInPackWithStats(context.Background(), 1, mobius.ListOptions{})
	require.NoError(t, err)
...
func testScheduledQueriesNew(t *testing.T, ds *Datastore) {
	u1 := test.NewUser(t, ds, "Admin", "admin@mobius.co", true)
	q1 := test.NewQuery(t, ds, nil, "foo", "select * from time;", u1.ID, true)
	p1 := test.NewPack(t, ds, "baz")

	query, err := ds.NewScheduledQuery(context.Background(), &mobius.ScheduledQuery{
		PackID:  p1.ID,
		QueryID: q1.ID,
		Name:    "foo-scheduled",
	})
...
func testScheduledQueriesIDsByName(t *testing.T, ds *Datastore) {
	...
	queries := []*mobius.Query{
		{Name: "foo", Description: "get the foos", Query: "select * from foo", Logging: mobius.LoggingSnapshot},
		{Name: "bar", Description: "do some bars", Query: "select * from bar", Logging: mobius.LoggingSnapshot},
		{Name: "foo2", Description: "get the foos", Query: "select * from foo2", Logging: mobius.LoggingSnapshot},
		{Name: "bar2", Description: "do some bars", Query: "select * from bar2", Logging: mobius.LoggingSnapshot},
	}
...
func testScheduledQueriesAsyncBatchSaveStats(t *testing.T, ds *Datastore) {
	...
	assertStats := func(m map[uint][]mobius.ScheduledQueryStats) {
	...
	m := map[uint][]mobius.ScheduledQueryStats{
		h1.ID: {
			{
