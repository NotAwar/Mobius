...
func (ds *Datastore) CreateScimUser(ctx context.Context, user *mobius.ScimUser) (uint, error) {
...
func (ds *Datastore) ScimUserByID(ctx context.Context, id uint) (*mobius.ScimUser, error) {
	const query = `
		SELECT
...
func (ds *Datastore) ScimUserByUserName(ctx context.Context, userName string) (*mobius.ScimUser, error) {
	return scimUserByUserName(ctx, ds.reader(ctx), userName)
}

func scimUserByUserName(ctx context.Context, q sqlx.QueryerContext, userName string) (*mobius.ScimUser, error) {
	const query = `
		SELECT
			id, external_id, user_name, given_name, family_name, department, active, updated_at
		FROM scim_users
...
func (ds *Datastore) ScimUserByUserNameOrEmail(ctx context.Context, userName string, email string) (*mobius.ScimUser, error) {
	return scimUserByUserNameOrEmail(ctx, ds.reader(ctx), ds.logger, userName, email)
}

func scimUserByUserNameOrEmail(ctx context.Context, q sqlx.QueryerContext, logger log.Logger, userName string, email string) (*mobius.ScimUser, error) {
...
func (ds *Datastore) ScimUserByHostID(ctx context.Context, hostID uint) (*mobius.ScimUser, error) {
...
func getScimUserLiteByHostID(ctx context.Context, q sqlx.QueryerContext, hostID uint) (*mobius.ScimUser, error) {
	const query = `
		SELECT
...
func (ds *Datastore) ReplaceScimUser(ctx context.Context, user *mobius.ScimUser) error {
	if err := validateScimUserFields(user); err != nil {
		return err
	}
...
func insertEmails(ctx context.Context, tx sqlx.ExtContext, user *mobius.ScimUser) error {
...
func (ds *Datastore) ListScimUsers(ctx context.Context, opts mobius.ScimUsersListOptions) (users []mobius.ScimUser, totalResults uint, err error) {
	...
	userMap := make(map[uint]*mobius.ScimUser, len(users))
	...
	var allEmails []mobius.ScimUserEmail
	...
	for _, ug := range allUserGroups {
		if user, ok := userMap[ug.UserID]; ok {
			user.Groups = append(user.Groups, mobius.ScimUserGroup{
				ID:          ug.ID,
...
func (ds *Datastore) getScimUserEmails(ctx context.Context, userID uint) ([]mobius.ScimUserEmail, error) {
	return getScimUserEmails(ctx, ds.reader(ctx), userID)
}

func getScimUserEmails(ctx context.Context, q sqlx.QueryerContext, userID uint) ([]mobius.ScimUserEmail, error) {
	const query = `
		SELECT
			scim_user_id, email, ` + "`primary`" + `, type
		FROM scim_user_emails
	...
	`
	var emails []mobius.ScimUserEmail
...
func (ds *Datastore) getScimUserGroups(ctx context.Context, userID uint) ([]mobius.ScimUserGroup, error) {
	return getScimUserGroups(ctx, ds.reader(ctx), userID)
}

func getScimUserGroups(ctx context.Context, q sqlx.QueryerContext, userID uint) ([]mobius.ScimUserGroup, error) {
	const query = `
		SELECT
	...
	`
	var groups []mobius.ScimUserGroup
...
func validateScimUserFields(user *mobius.ScimUser) error {
	if user.ExternalID != nil && len(*user.ExternalID) > mobius.SCIMMaxFieldLength {
		return fmt.Errorf("external_id exceeds maximum length of %d characters", mobius.SCIMMaxFieldLength)
	}
	if len(user.UserName) > mobius.SCIMMaxFieldLength {
		return fmt.Errorf("user_name exceeds maximum length of %d characters", mobius.SCIMMaxFieldLength)
	}
	if user.GivenName != nil && len(*user.GivenName) > mobius.SCIMMaxFieldLength {
		return fmt.Errorf("given_name exceeds maximum length of %d characters", mobius.SCIMMaxFieldLength)
	}
	if user.FamilyName != nil && len(*user.FamilyName) > mobius.SCIMMaxFieldLength {
		return fmt.Errorf("family_name exceeds maximum length of %d characters", mobius.SCIMMaxFieldLength)
	}
	if user.Department != nil && len(*user.Department) > mobius.SCIMMaxFieldLength {
		return fmt.Errorf("department exceeds maximum length of %d characters", mobius.SCIMMaxFieldLength)
	}
	return nil
}

// validateScimGroupFields checks if the group fields exceed the maximum allowed length
func validateScimGroupFields(group *mobius.ScimGroup) error {
	if group.ExternalID != nil && len(*group.ExternalID) > mobius.SCIMMaxFieldLength {
		return fmt.Errorf("external_id exceeds maximum length of %d characters", mobius.SCIMMaxFieldLength)
	}
	if len(group.DisplayName) > mobius.SCIMMaxFieldLength {
		return fmt.Errorf("display_name exceeds maximum length of %d characters", mobius.SCIMMaxFieldLength)
	}
	return nil
}

// CreateScimGroup creates a new SCIM group in the database
func (ds *Datastore) CreateScimGroup(ctx context.Context, group *mobius.ScimGroup) (uint, error) {
...
func (ds *Datastore) ScimGroupByID(ctx context.Context, id uint, excludeUsers bool) (*mobius.ScimGroup, error) {
	const query = `
		SELECT
...
func (ds *Datastore) ScimGroupByDisplayName(ctx context.Context, displayName string) (*mobius.ScimGroup, error) {
	const query = `
		SELECT
			id, external_id, display_name
		FROM scim_groups
...
func (ds *Datastore) ReplaceScimGroup(ctx context.Context, group *mobius.ScimGroup) error {
	if err := validateScimGroupFields(group); err != nil {
		return err
	}
...
func (ds *Datastore) ListScimGroups(ctx context.Context, opts mobius.ScimGroupsListOptions) (groups []mobius.ScimGroup, totalResults uint, err error) {
	// Default pagination values if not provided
	if opts.StartIndex == 0 {
		opts.StartIndex = 1
	}
	...
	groupMap := make(map[uint]*mobius.ScimGroup, len(groups))
...
func (ds *Datastore) ScimLastRequest(ctx context.Context) (*mobius.ScimLastRequest, error) {
	const query = `
				SELECT
				...
				ORDER BY id LIMIT 1
			`
	var lastRequest mobius.ScimLastRequest
...
func (ds *Datastore) UpdateScimLastRequest(ctx context.Context, lastRequest *mobius.ScimLastRequest) error {
	if lastRequest == nil {
		return nil
	}
	...
	if len(lastRequest.Details) > mobius.SCIMMaxFieldLength {
		return fmt.Errorf("details exceeds maximum length of %d characters", mobius.SCIMMaxFieldLength)
	}
...
func triggerResendProfilesForIDPUserChange(ctx context.Context, tx sqlx.ExtContext, updatedScimUserID uint) error {
	...
	return triggerResendProfilesUsingVariables(ctx, tx, hostIDs,
		[]string{
			mobius.MobiusVarHostEndUserIDPUsername,
			mobius.MobiusVarHostEndUserIDPUsernameLocalPart,
			mobius.MobiusVarHostEndUserIDPDepartment,
		})
}
...
func triggerResendProfilesForIDPUserDeleted(ctx context.Context, tx sqlx.ExtContext, deletedScimUserID uint) error {
	...
	return triggerResendProfilesUsingVariables(ctx, tx, hostIDs,
		[]string{
			mobius.MobiusVarHostEndUserIDPUsername,
			mobius.MobiusVarHostEndUserIDPUsernameLocalPart,
			mobius.MobiusVarHostEndUserIDPGroups,
			mobius.MobiusVarHostEndUserIDPDepartment,
		})
}
...
func triggerResendProfilesForIDPGroupChange(ctx context.Context, tx sqlx.ExtContext, updatedScimGroupID uint) error {
	...
	return triggerResendProfilesUsingVariables(ctx, tx, hostIDs,
		[]string{mobius.MobiusVarHostEndUserIDPGroups})
}
...
func triggerResendProfilesUsingVariables(ctx context.Context, tx sqlx.ExtContext, hostIDs []uint, affectedVars []string) error {
	...
	UPDATE
		...
		JOIN mobius_variables fv
			ON mcpv.mobius_variable_id = fv.id
	SET
		hmap.status = NULL
	WHERE
...
`
	...
	for i, v := range affectedVars {
		vars[i] = "MOBIUS_VAR_" + v
	}

	stmt, args, err := sqlx.Named(updateStatusQuery, map[string]any{
		"host_ids":               hostIDs,
		"operation_type_install": mobius.MDMOperationTypeInstall,
		"affected_vars":          vars,
	})
	if err != nil {
...
func emailsRequireUpdate(currentEmails, newEmails []mobius.ScimUserEmail) bool {
	if len(currentEmails) != len(newEmails) {
		return true
	}

	// Create maps for efficient comparison
	currentEmailMap := make(map[string]mobius.ScimUserEmail)
