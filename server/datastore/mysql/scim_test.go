...
func testScimUserByID(t *testing.T, ds *Datastore) {
	...
	assert.True(t, mobius.IsNotFound(err))
}
...
func createTestScimUsers(t *testing.T, ds *Datastore) []*mobius.ScimUser {
	createUsers := []mobius.ScimUser{
		{
			UserName:   "test-user1",
...
func testReplaceScimUser(t *testing.T, ds *Datastore) {
	...
	assert.Equal(t, mobius.ScimUserGroup{ID: group.ID, DisplayName: group.DisplayName}, createdUser.Groups[0])

	// Modify the user and attempt to modify the Groups field
	updatedUser := mobius.ScimUser{
		ID:         user.ID,
		...
		Emails: []mobius.ScimUserEmail{ // Changed emails
			{
		...
		Groups: []mobius.ScimUserGroup{{ID: 999, DisplayName: "Ignored Group"}}, // Attempt to modify Groups (should be ignored)
	}
	...
	nonExistentUser := mobius.ScimUser{
...
func testReplaceScimUserEmails(t *testing.T, ds *Datastore) {
	...
	sameEmailsUser := mobius.ScimUser{
		ID:         user.ID,
	...
	multiPrimaryUser := mobius.ScimUser{
		ID:         user.ID,
	...
	userWithAllFields := mobius.ScimUser{
		ID:         user.ID,
	...
	userWithNilPrimary := mobius.ScimUser{
		ID:         user.ID,
		UserName:   "all-fields@example.com",
		ExternalID: ptr.String("ext-all-fields"),
		GivenName:  ptr.String("All"),
	...
	var nilPrimaryUser *mobius.ScimUser
	...
	userWithNilType := mobius.ScimUser{
		ID:         user.ID,
		UserName:   "all-fields@example.com",
		ExternalID: ptr.String("ext-all-fields"),
		GivenName:  ptr.String("All"),
	...
	// Verify the email was updated
	var nilTypeUser *mobius.ScimUser
...
func testListScimUsers(t *testing.T, ds *Datastore) {
	...
	allUsers, totalResults, err := ds.ListScimUsers(t.Context(), mobius.ScimUsersListOptions{
		ScimListOptions: mobius.ScimListOptions{
			StartIndex: 1,
			PerPage:    10,
		},
	})
	...
	page1Users, totalPage1, err := ds.ListScimUsers(t.Context(), mobius.ScimUsersListOptions{
		ScimListOptions: mobius.ScimListOptions{
			StartIndex: 1,
			PerPage:    2,
		},
	...
	page2Users, totalPage2, err := ds.ListScimUsers(t.Context(), mobius.ScimUsersListOptions{
		ScimListOptions: mobius.ScimListOptions{
	...
	listUsers, totalListUsers, err := ds.ListScimUsers(t.Context(), mobius.ScimUsersListOptions{
		ScimListOptions: mobius.ScimListOptions{
			StartIndex: 1,
			PerPage:    10,
		},
	...
	homeEmailUsers, totalHomeEmailUsers, err := ds.ListScimUsers(t.Context(), mobius.ScimUsersListOptions{
		ScimListOptions: mobius.ScimListOptions{
			StartIndex: 1,
			PerPage:    10,
		},
	...
	workEmailUsers, totalWorkEmailUsers, err := ds.ListScimUsers(t.Context(), mobius.ScimUsersListOptions{
		ScimListOptions: mobius.ScimListOptions{
			StartIndex: 1,
			PerPage:    10,
		},
	...
	noUsers, totalNoUsers1, err := ds.ListScimUsers(t.Context(), mobius.ScimUsersListOptions{
		ScimListOptions: mobius.ScimListOptions{
			StartIndex: 1,
			PerPage:    10,
		},
	...
	noUsers, totalNoUsers2, err := ds.ListScimUsers(t.Context(), mobius.ScimUsersListOptions{
		ScimListOptions: mobius.ScimListOptions{
			StartIndex: 1,
			PerPage:    10,
		},
...
func testScimGroupCreate(t *testing.T, ds *Datastore) {
	...
	groupsToCreate := []mobius.ScimGroup{
		{
			DisplayName: "Group1",
			ExternalID:  nil,
			ScimUsers:   []uint{},
...
func testScimGroupCreateValidation(t *testing.T, ds *Datastore) {
	...
	longString := strings.Repeat("a", mobius.SCIMMaxFieldLength+1) // String longer than allowed

	// Test ExternalID validation
	groupWithLongExternalID := mobius.ScimGroup{
	...
	groupWithLongDisplayName := mobius.ScimGroup{
...
func createTestScimGroups(t *testing.T, ds *Datastore, userIDs []uint) []*mobius.ScimGroup {
	createGroups := []mobius.ScimGroup{
		{
...
func testReplaceScimGroup(t *testing.T, ds *Datastore) {
	...
	nonExistentGroup := mobius.ScimGroup{
		ID:          99999, // Non-existent ID
...
func testListScimGroups(t *testing.T, ds *Datastore) {
	...
	allGroups, totalResults, err := ds.ListScimGroups(t.Context(), mobius.ScimGroupsListOptions{
		ScimListOptions: mobius.ScimListOptions{
			StartIndex: 1,
			PerPage:    10,
		},
	...
	// Test 2: Pagination - first page with 2 items
	page1Groups, totalPage1, err := ds.ListScimGroups(t.Context(), mobius.ScimGroupsListOptions{
		ScimListOptions: mobius.ScimListOptions{
			StartIndex: 1,
			PerPage:    2,
		},
	...
	// Test 3: Pagination - second page with 2 items
	page2Groups, totalPage2, err := ds.ListScimGroups(t.Context(), mobius.ScimGroupsListOptions{
		ScimListOptions: mobius.ScimListOptions{
			StartIndex: 3, // StartIndex is 1-based, so for the second page with 2 items per page, we start at index 3
			PerPage:    2,
		},
	...
	displayName := "List Test Group 2"
	filteredGroups, totalFilteredResults, err := ds.ListScimGroups(t.Context(), mobius.ScimGroupsListOptions{
		ScimListOptions: mobius.ScimListOptions{
			StartIndex: 1,
			PerPage:    10,
		},
	...
	emptyResults, totalEmptyResults, err := ds.ListScimGroups(t.Context(), mobius.ScimGroupsListOptions{
		ScimListOptions: mobius.ScimListOptions{
			StartIndex: 1,
			PerPage:    10,
		},
	...
	groupsWithoutUsers, totalWithoutUsers, err := ds.ListScimGroups(t.Context(), mobius.ScimGroupsListOptions{
		ScimListOptions: mobius.ScimListOptions{
			StartIndex: 1,
			PerPage:    10,
		},
...
func testScimUserCreateValidation(t *testing.T, ds *Datastore) {
	// Test validation for ExternalID
	longString := strings.Repeat("a", mobius.SCIMMaxFieldLength+1) // String longer than SCIMMaxFieldLength

	// Test ExternalID validation
	userWithLongExternalID := mobius.ScimUser{
	...
	userWithLongUserName := mobius.ScimUser{
		UserName:   longString,
		ExternalID: ptr.String("valid-external-id"),
		GivenName:  ptr.String("Valid"),
		FamilyName: ptr.String("Name"),
	...
	userWithLongGivenName := mobius.ScimUser{
		UserName:   "valid-username",
		ExternalID: ptr.String("valid-external-id"),
	...
	userWithLongFamilyName := mobius.ScimUser{
		UserName:   "valid-username",
		ExternalID: ptr.String("valid-external-id"),
		GivenName:  ptr.String("Valid"),
...
func testScimUserReplaceValidation(t *testing.T, ds *Datastore) {
	...
	userWithLongDepartment := mobius.ScimUser{
		ID:         user.ID,
		UserName:   "valid-username",
		ExternalID: ptr.String("valid-external-id"),
		GivenName:  ptr.String("Valid"),
...
func testScimLastRequest(t *testing.T, ds *Datastore) {
	...
	invalidStatusRequest := &mobius.ScimLastRequest{
		Status:  longStatus,
	...
	longDetails := strings.Repeat("b", mobius.SCIMMaxFieldLength+1) // 256 characters
	invalidDetailsRequest := &mobius.ScimLastRequest{
		Status:  "valid",
		Details: longDetails,
	}
	...
	newRequest := &mobius.ScimLastRequest{
		Status:  "success",
	...
	updatedRequest := &mobius.ScimLastRequest{
		Status:  "error",
...
func testTriggerResendIdPProfiles(t *testing.T, ds *Datastore) {
	...
	varsPerProfile := map[string][]string{
		profUsername.ProfileUUID: {mobius.MobiusVarHostEndUserIDPUsername, mobius.MobiusVarHostEndUserIDPUsernameLocalPart},
		profGroup.ProfileUUID:    {mobius.MobiusVarHostEndUserIDPGroups},
		profAll.ProfileUUID:      {mobius.MobiusVarHostEndUserIDPUsername, mobius.MobiusVarHostEndUserIDPUsernameLocalPart, mobius.MobiusVarHostEndUserIDPGroups},
	}
	for profUUID, vars := range varsPerProfile {
		for _, v := range vars {
			ExecAdhocSQL(t, ds, func(q sqlx.ExtContext) error {
				_, err := q.ExecContext(ctx, `INSERT INTO mdm_configuration_profile_variables (apple_profile_uuid, mobius_variable_id)
					SELECT ?, id FROM mobius_variables WHERE name = ?`, profUUID, "MOBIUS_VAR_"+v)
				return err
			})
		}
	}
	...
	scimUser1, err := ds.CreateScimUser(ctx, &mobius.ScimUser{UserName: "a@example.com"})
	require.NoError(t, err)
	scimUser2, err := ds.CreateScimUser(ctx, &mobius.ScimUser{UserName: "b@example.com"})
	require.NoError(t, err)
	scimUser3, err := ds.CreateScimUser(ctx, &mobius.ScimUser{UserName: "c@example.com"})
	require.NoError(t, err)
	...
	forceSetAppleHostProfileStatus(t, ds, host1.UUID, profNone, mobius.MDMOperationTypeInstall, mobius.MDMDeliveryVerifying)
	forceSetAppleHostProfileStatus(t, ds, host2.UUID, profNone, mobius.MDMOperationTypeInstall, mobius.MDMDeliveryVerifying)
	forceSetAppleHostProfileStatus(t, ds, host3.UUID, profNone, mobius.MDMOperationTypeInstall, mobius.MDMDeliveryVerifying)
	forceSetAppleHostProfileStatus(t, ds, host1.UUID, profUsername, mobius.MDMOperationTypeInstall, mobius.MDMDeliveryVerifying)
	forceSetAppleHostProfileStatus(t, ds, host2.UUID, profUsername, mobius.MDMOperationTypeInstall, mobius.MDMDeliveryVerifying)
	forceSetAppleHostProfileStatus(t, ds, host3.UUID, profUsername, mobius.MDMOperationTypeInstall, mobius.MDMDeliveryVerifying)
	forceSetAppleHostProfileStatus(t, ds, host1.UUID, profGroup, mobius.MDMOperationTypeInstall, mobius.MDMDeliveryVerifying)
	forceSetAppleHostProfileStatus(t, ds, host2.UUID, profGroup, mobius.MDMOperationTypeInstall, mobius.MDMDeliveryVerifying)
	forceSetAppleHostProfileStatus(t, ds, host3.UUID, profGroup, mobius.MDMOperationTypeInstall, mobius.MDMDeliveryVerifying)
	forceSetAppleHostProfileStatus(t, ds, host1.UUID, profAll, mobius.MDMOperationTypeInstall, mobius.MDMDeliveryVerifying)
	forceSetAppleHostProfileStatus(t, ds, host2.UUID, profAll, mobius.MDMOperationTypeInstall, mobius.MDMDeliveryVerifying)
	forceSetAppleHostProfileStatus(t, ds, host3.UUID, profAll, mobius.MDMOperationTypeInstall, mobius.MDMDeliveryVerifying)

	// change username of scim user 1
	err = ds.ReplaceScimUser(ctx, &mobius.ScimUser{ID: scimUser1, UserName: "A@example.com"})
	require.NoError(t, err)

	// this triggered a resend of profUsername and profAll on host1
	assertHostProfileStatus(t, ds, host1.UUID,
		hostProfileStatus{profNone.ProfileUUID, mobius.MDMDeliveryVerifying},
		hostProfileStatus{profUsername.ProfileUUID, mobius.MDMDeliveryPending},
		hostProfileStatus{profGroup.ProfileUUID, mobius.MDMDeliveryVerifying},
		hostProfileStatus{profAll.ProfileUUID, mobius.MDMDeliveryPending})
	assertHostProfileStatus(t, ds, host2.UUID,
		hostProfileStatus{profNone.ProfileUUID, mobius.MDMDeliveryVerifying},
		hostProfileStatus{profUsername.ProfileUUID, mobius.MDMDeliveryVerifying},
		hostProfileStatus{profGroup.ProfileUUID, mobius.MDMDeliveryVerifying},
	...
	group1, err := ds.CreateScimGroup(ctx, &mobius.ScimGroup{DisplayName: "g1", ScimUsers: []uint{scimUser1, scimUser2}})
	require.NoError(t, err)
	...
	assertHostProfileStatus(t, ds, host1.UUID,
		hostProfileStatus{profNone.ProfileUUID, mobius.MDMDeliveryVerifying},
		hostProfileStatus{profUsername.ProfileUUID, mobius.MDMDeliveryVerifying},
		hostProfileStatus{profGroup.ProfileUUID, mobius.MDMDeliveryPending},
		hostProfileStatus{profAll.ProfileUUID, mobius.MDMDeliveryPending})
	assertHostProfileStatus(t, ds, host2.UUID,
		hostProfileStatus{profNone.ProfileUUID, mobius.MDMDeliveryVerifying},
		hostProfileStatus{profUsername.ProfileUUID, mobius.MDMDeliveryVerifying},
	...
	group2, err := ds.CreateScimGroup(ctx, &mobius.ScimGroup{DisplayName: "g2"})
	require.NoError(t, err)
	err = ds.ReplaceScimUser(ctx, &mobius.ScimUser{ID: scimUser1, UserName: "A@example.com", GivenName: ptr.String("A")})
	require.NoError(t, err)
	...
	err = ds.ReplaceScimGroup(ctx, &mobius.ScimGroup{ID: group1, DisplayName: "G1", ScimUsers: []uint{scimUser1, scimUser2}})
	require.NoError(t, err)
	...
	err = ds.ReplaceScimGroup(ctx, &mobius.ScimGroup{ID: group1, DisplayName: "G1", ScimUsers: []uint{scimUser1, scimUser3}})
	require.NoError(t, err)
	...
	forceSetAppleHostProfileStatus(t, ds, host1.UUID, profUsername, mobius.MDMOperationTypeRemove, mobius.MDMDeliveryVerifying)
	forceSetAppleHostProfileStatus(t, ds, host1.UUID, profGroup, mobius.MDMOperationTypeInstall, mobius.MDMDeliveryVerifying)
	forceSetAppleHostProfileStatus(t, ds, host1.UUID, profAll, mobius.MDMOperationTypeInstall, mobius.MDMDeliveryVerifying)
	...
	err = ds.ReplaceScimUser(ctx, &mobius.ScimUser{ID: scimUser2, UserName: "B@example.com", GivenName: ptr.String("B")})
	require.NoError(t, err)
...
func testTriggerResendIdPProfilesOnTeam(t *testing.T, ds *Datastore) {
	...
	profGroup, err := ds.NewMDMAppleConfigProfile(ctx, *generateCP("a", "a", team.ID), []string{mobius.MobiusVarHostEndUserIDPGroups})
	require.NoError(t, err)
	...
	err = ds.ReplaceScimGroup(ctx, &mobius.ScimGroup{ID: group1, DisplayName: "g1", ScimUsers: []uint{scimUser2}})
	require.NoError(t, err)
...
func assertHostProfileStatus(t *testing.T, ds *Datastore, hostUUID string, wantProfiles ...hostProfileStatus) {
	...
	for _, p := range wantProfiles {
		withOpProfiles = append(withOpProfiles, hostProfileOpStatus{
			...
			Status:      p.Status,
			OpType:      mobius.MDMOperationTypeInstall,
		})
	}
...
func assertHostProfileOpStatus(t *testing.T, ds *Datastore, hostUUID string, wantProfiles ...hostProfileOpStatus) {
	...
	profStatus := make(map[string]mobius.MDMDeliveryStatus, len(profs))
	profOpType := make(map[string]mobius.MDMOperationType, len(profs))
...
func forceSetAppleHostProfileStatus(t *testing.T, ds *Datastore, hostUUID string, profile *mobius.MDMAppleConfigProfile, operation mobius.MDMOperationType, status mobius.MDMDeliveryStatus) {
	ctx := t.Context()
...
func forceSetWindowsHostProfileStatus(t *testing.T, ds *Datastore, hostUUID string, profile *mobius.MDMWindowsConfigProfile, operation mobius.MDMOperationType, status mobius.MDMDeliveryStatus) {
	ctx := t.Context()
...
func forceSetAppleHostDeclarationStatus(t *testing.T, ds *Datastore, hostUUID string, profile *mobius.MDMAppleDeclaration, operation mobius.MDMOperationType, status mobius.MDMDeliveryStatus) {
	ctx := t.Context()
