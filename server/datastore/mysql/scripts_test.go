...
func testHostScriptResult(t *testing.T, ds *Datastore) {
	...
	createdScript1, err := ds.NewHostScriptExecutionRequest(ctx, &mobius.HostScriptRequestPayload{
		HostID:         1,
		ScriptContents: "echo",
		UserID:         &u.ID,
		SyncRequest:    true,
	...
	hsr, action, err := ds.SetHostScriptExecutionResult(ctx, &mobius.HostScriptResultPayload{
		HostID:      1,
	...
	hsr, _, err = ds.SetHostScriptExecutionResult(ctx, &mobius.HostScriptResultPayload{
		HostID:      1,
		ExecutionID: createdScript1.ExecutionID,
		Output:      "foobarbaz",
		Runtime:     22,
	...
	createdScript2, err := ds.NewHostScriptExecutionRequest(ctx, &mobius.HostScriptRequestPayload{
		HostID:         1,
	...
	time.Sleep(time.Millisecond) // ensure a different timestamp
	createdScript3, err := ds.NewHostScriptExecutionRequest(ctx, &mobius.HostScriptRequestPayload{
		HostID:         1,
	...
	createdUnsignedScript, err := ds.NewHostScriptExecutionRequest(ctx, &mobius.HostScriptRequestPayload{
		HostID:         1,
		ScriptContents: "echo",
		UserID:         &u.ID,
		SyncRequest:    true,
	...
	unsignedScriptResult, _, err := ds.SetHostScriptExecutionResult(ctx, &mobius.HostScriptResultPayload{
		HostID:      1,
...
func testListPendingScriptDEPRestoration(t *testing.T, ds *Datastore) {
	...
	// create a createdScript execution request (with a user)
	u := test.NewUser(t, ds, "Bob", "bob@example.com", true)
	createdScript, err := ds.NewHostScriptExecutionRequest(ctx, &mobius.HostScriptRequestPayload{
		HostID:         host.ID,
		ScriptContents: "echo",
		UserID:         &u.ID,
		SyncRequest:    true,
...
func testScripts(t *testing.T, ds *Datastore) {
	...
	scriptGlobal, err := ds.NewScript(ctx, &mobius.Script{
		Name:           "a",
		ScriptContents: "echo",
	})
	require.NoError(t, err)
	...
	_, err = ds.NewScript(ctx, &mobius.Script{
		Name:           "a",
		TeamID:         ptr.Uint(123),
		ScriptContents: "echo",
	})
	require.Error(t, err)
	var fkErr mobius.ForeignKeyError
	...
	require.NoError(t, err)
	scriptTeam, err := ds.NewScript(ctx, &mobius.Script{
		Name:           "a",
		TeamID:         &tm.ID,
	...
	require.Error(t, err)
	var existsErr mobius.AlreadyExistsError
	...
	require.NoError(t, err)
	scriptTeam2, err := ds.NewScript(ctx, &mobius.Script{
		Name:           "a",
		TeamID:         &tm.ID,
		ScriptContents: "echo",
	})
...
func testListScripts(t *testing.T, ds *Datastore) {
	...
	for i := 0; i < 5; i++ {
		...
		require.NoError(t, err)
		_, err = ds.NewScript(ctx, &mobius.Script{Name: string('a' + byte(i)), TeamID: &tm1.ID, ScriptContents: "echo"})
		require.NoError(t, err)
	}

	// create a single script for team 2
	_, err = ds.NewScript(ctx, &mobius.Script{Name: "a", TeamID: &tm2.ID, ScriptContents: "echo"})
	require.NoError(t, err)
	...
	cases := []struct {
		...
		{
			opts:      mobius.ListOptions{PerPage: 3},
			teamID:    &tm1.ID,
			wantNames: []string{"a", "b", "c"},
			wantMeta:  &mobius.PaginationMetadata{HasNextResults: true, HasPreviousResults: false},
		},
		{
			opts:      mobius.ListOptions{Page: 1, PerPage: 3},
			teamID:    &tm1.ID,
			wantNames: []string{"d", "e"},
			wantMeta:  &mobius.PaginationMetadata{HasNextResults: false, HasPreviousResults: true},
		},
		{
			opts:      mobius.ListOptions{Page: 2, PerPage: 3},
			teamID:    &tm1.ID,
			wantNames: nil,
			wantMeta:  &mobius.PaginationMetadata{HasNextResults: false, HasPreviousResults: true},
		},
...
func testGetHostScriptDetails(t *testing.T, ds *Datastore) {
	...
	scripts, _, err := ds.ListScripts(ctx, nil, mobius.ListOptions{})
	require.NoError(t, err)
	...
	t.Run("results match expected formatting and filtering", func(t *testing.T) {
		res, _, err := ds.GetHostScriptDetails(ctx, 42, nil, mobius.ListOptions{}, "")
		require.NoError(t, err)
		require.Len(t, res, 6)
		for _, r := range res {
			switch r.ScriptID {
	...
	t.Run("empty slice returned if no scripts", func(t *testing.T) {
		res, _, err := ds.GetHostScriptDetails(ctx, 42, ptr.Uint(1), mobius.ListOptions{}, "") // team 1 has no scripts
		require.NoError(t, err)
	...
	t.Run("windows ps1 scripts are supported", func(t *testing.T) {
		res, _, err := ds.GetHostScriptDetails(ctx, 42, nil, mobius.ListOptions{}, "windows")
		require.NoError(t, err)
		require.NotNil(t, res)
		require.Len(t, res, 1)
...
func testBatchSetScripts(t *testing.T, ds *Datastore) {
	...
	applyAndExpect := func(newSet []*mobius.Script, tmID *uint, want []*mobius.Script) map[string]uint {
		...
		got, _, err := ds.ListScripts(ctx, tmID, mobius.ListOptions{})
		require.NoError(t, err)
	...
	// create a team
	tm1, err := ds.NewTeam(ctx, &mobius.Team{Name: t.Name() + "_tm1"})
	require.NoError(t, err)

	// apply single script set for tm1
	sTm1 := applyAndExpect([]*mobius.Script{
		{Name: "N1", ScriptContents: "C1"},
	}, ptr.Uint(tm1.ID), []*mobius.Script{
		{Name: "N1", TeamID: ptr.Uint(tm1.ID)},
	})
	n1WithTeamID := sTm1["N1"]

	teamPolicy, err := ds.NewTeamPolicy(ctx, tm1.ID, nil, mobius.PolicyPayload{
	...
	sNoTm := applyAndExpect([]*mobius.Script{
		{Name: "N1", ScriptContents: "C1"},
	}, nil, []*mobius.Script{
		{Name: "N1", TeamID: nil},
	})
	...
	noTeamPolicy, err := ds.NewTeamPolicy(ctx, mobius.PolicyNoTeamID, nil, mobius.PolicyPayload{
		Name:     "No Team Policy",
		Query:    "SELECT 1",
		Platform: "darwin",
	...
	sTm1b := applyAndExpect([]*mobius.Script{
		{Name: "N1", ScriptContents: "C1"},
	...
	sNoTmb := applyAndExpect([]*mobius.Script{
	...
	// script for tm1
	sTm1c := applyAndExpect([]*mobius.Script{
...
func testLockHostViaScript(t *testing.T, ds *Datastore) {
	...
	err = ds.LockHostViaScript(ctx, &mobius.HostScriptRequestPayload{
	...
	status, err := ds.GetHostLockWipeStatus(ctx, &mobius.Host{ID: windowsHostID, Platform: "windows", UUID: "uuid"})
	require.NoError(t, err)
	require.Equal(t, "windows", status.HostMobiusPlatform)
	...
	_, action, err := ds.SetHostScriptExecutionResult(ctx, &mobius.HostScriptResultPayload{
		HostID:      s.HostID,
...
func testUnlockHostViaScript(t *testing.T, ds *Datastore) {
	...
	err = ds.UnlockHostViaScript(ctx, &mobius.HostScriptRequestPayload{
		HostID:         hostID,
		ScriptContents: script,
		UserID:         &user.ID,
		SyncRequest:    false,
	...
	// verify that we have created entries in host_mdm_actions and host_script_results
	status, err := ds.GetHostLockWipeStatus(ctx, &mobius.Host{ID: hostID, Platform: "windows", UUID: "uuid"})
	require.NoError(t, err)
	require.Equal(t, "windows", status.HostMobiusPlatform)
...
func testLockUnlockWipeViaScripts(t *testing.T, ds *Datastore) {
	...
	for i, platform := range []string{"windows", "linux"} {
		...
		t.Run(platform, func(t *testing.T) {
			status, err := ds.GetHostLockWipeStatus(ctx, &mobius.Host{ID: hostID, Platform: platform, UUID: "uuid"})
			require.NoError(t, err)
			...
			case "windows":
				...
				wipeCmd := &mobius.MDMWindowsCommand{
			...
			case "linux":
				// record a request to wipe the host
				err = ds.WipeHostViaScript(ctx, &mobius.HostScriptRequestPayload{
					HostID:         hostID,
					ScriptContents: "wipe",
					UserID:         &user.ID,
					SyncRequest:    false,
...
func testLockUnlockManually(t *testing.T, ds *Datastore) {
	...
	status, err := ds.GetHostLockWipeStatus(ctx, &mobius.Host{ID: 1, Platform: "darwin", UUID: "uuid"})
	require.NoError(t, err)
	...
	require.NoError(t, err)
	status, err = ds.GetHostLockWipeStatus(ctx, &mobius.Host{ID: 2, Platform: "darwin", UUID: "uuid"})
	require.NoError(t, err)
	require.False(t, status.UnlockRequestedAt.IsZero())
	require.WithinDuration(t, today, status.UnlockRequestedAt, 1*time.Second)
}

func checkLockWipeState(t *testing.T, status *mobius.HostLockWipeStatus, unlocked, locked, wiped, pendingUnlock, pendingLock, pendingWipe bool) {
...
func testCleanupUnusedScriptContents(t *testing.T, ds *Datastore) {
	...
	res, err := ds.NewHostScriptExecutionRequest(ctx, &mobius.HostScriptRequestPayload{ScriptContents: "echo something_else", SyncRequest: true})
	require.NoError(t, err)
...
func testDeleteScriptsAssignedToPolicy(t *testing.T, ds *Datastore) {
	...
	p1, err := ds.NewTeamPolicy(ctx, team1.ID, nil, mobius.PolicyPayload{
		Name:     "p1",
		Query:    "SELECT 1;",
		ScriptID: &script.ID,
	})
...
func testDeletePendingHostScriptExecutionsForPolicy(t *testing.T, ds *Datastore) {
	...
	team1, _ := ds.NewTeam(ctx, &mobius.Team{Name: "team1"})

	script1, err := ds.NewScript(ctx, &mobius.Script{
		Name:           "script1.sh",
		TeamID:         &team1.ID,
		ScriptContents: "hello world",
	})
	require.NoError(t, err)
	script2, err := ds.NewScript(ctx, &mobius.Script{
		Name:           "script2.sh",
		TeamID:         &team1.ID,
		ScriptContents: "hello world",
	})
	...
	p2, err := ds.NewTeamPolicy(ctx, team1.ID, nil, mobius.PolicyPayload{
		Name:     "p2",
		Query:    "SELECT 2;",
		ScriptID: &script2.ID,
	})
	...
	scriptExecution, err := ds.NewHostScriptExecutionRequest(ctx, &mobius.HostScriptRequestPayload{
		HostID:         1,
		ScriptContents: "echo",
		UserID:         &user.ID,
		PolicyID:       &p1.ID,
...
func testUpdateScriptContents(t *testing.T, ds *Datastore) {
	...
	originalScript, err := ds.NewScript(ctx, &mobius.Script{
		Name:           "script1",
		ScriptContents: "hello world",
	})
	require.NoError(t, err)
...
func testUpdateDeletingUpcomingScriptExecutions(t *testing.T, ds *Datastore) {
	...
	script3, err := ds.NewScript(ctx, &mobius.Script{
		Name:           "script3",
...
func testBatchExecute(t *testing.T, ds *Datastore) {
	...
	require.Equal(t, mobius.BatchExecuteIncompatiblePlatform, exec_error)
	...
	require.Equal(t, mobius.BatchExecuteIncompatibleMobiusd, exec_error)
...
func testDeleteScriptActivatesNextActivity(t *testing.T, ds *Datastore) {
	...
	scriptA, err := ds.NewScript(ctx, &mobius.Script{
		Name:           "a",
		ScriptContents: "echo 'a'",
	})
	require.NoError(t, err)
	scriptB, err := ds.NewScript(ctx, &mobius.Script{
		Name:           "b",
	...
	// * hosts[2]: b, a
	// * hosts[3]: b
	execHost0ScriptA, err := ds.NewHostScriptExecutionRequest(ctx, &mobius.HostScriptRequestPayload{
		HostID:      hosts[0].ID,
		ScriptID:    &scriptA.ID,
		UserID:      &u.ID,
		SyncRequest: true,
	...
	require.NoError(t, err)
	execHost0ScriptB, err := ds.NewHostScriptExecutionRequest(ctx, &mobius.HostScriptRequestPayload{
		HostID:      hosts[0].ID,
		ScriptID:    &scriptB.ID,
		UserID:      &u.ID,
		SyncRequest: true,
	...
	require.NoError(t, err)
	execHost1ScriptA, err := ds.NewHostScriptExecutionRequest(ctx, &mobius.HostScriptRequestPayload{
		HostID:      hosts[1].ID,
		ScriptID:    &scriptA.ID,
		UserID:      &u.ID,
		SyncRequest: true,
	...
	require.NoError(t, err)
	execHost1ScriptB, err := ds.NewHostScriptExecutionRequest(ctx, &mobius.HostScriptRequestPayload{
		HostID:      hosts[1].ID,
		ScriptID:    &scriptB.ID,
		UserID:      &u.ID,
		SyncRequest: true,
	...
	require.NoError(t, err)
	execHost2ScriptB, err := ds.NewHostScriptExecutionRequest(ctx, &mobius.HostScriptRequestPayload{
		HostID:      hosts[2].ID,
		ScriptID:    &scriptB.ID,
		UserID:      &u.ID,
		SyncRequest: true,
	...
	require.NoError(t, err)
	execHost2ScriptA, err := ds.NewHostScriptExecutionRequest(ctx, &mobius.HostScriptRequestPayload{
		HostID:      hosts[2].ID,
		ScriptID:    &scriptA.ID,
		UserID:      &u.ID,
		SyncRequest: true,
	...
	require.NoError(t, err)
	execHost3ScriptB, err := ds.NewHostScriptExecutionRequest(ctx, &mobius.HostScriptRequestPayload{
		HostID:      hosts[3].ID,
		ScriptID:    &scriptB.ID,
		UserID:      &u.ID,
		SyncRequest: true,
...
func testBatchSetScriptActivatesNextActivity(t *testing.T, ds *Datastore) {
	...
	scripts, err := ds.BatchSetScripts(ctx, nil, []*mobius.Script{
	...
	require.NoError(t, err)
	execHost0ScriptC, err := ds.NewHostScriptExecutionRequest(ctx, &mobius.HostScriptRequestPayload{
		HostID: hosts[0].ID, ScriptID: ptr.Uint(scriptByName["C"]), SyncRequest: true, ScriptContents: "C3",
	})
	require.NoError(t, err)
	execHost0ScriptA2, err := ds.NewHostScriptExecutionRequest(ctx, &mobius.HostScriptRequestPayload{
		HostID: hosts[0].ID, ScriptID: ptr.Uint(scriptByName["A"]), SyncRequest: true, ScriptContents: "C1",
	})
	require.NoError(t, err)
	execHost0ScriptB, err := ds.NewHostScriptExecutionRequest(ctx, &mobius.HostScriptRequestPayload{
	...
	require.NoError(t, err)
	execHost1ScriptB2, err := ds.NewHostScriptExecutionRequest(ctx, &mobius.HostScriptRequestPayload{
		HostID: hosts[1].ID, ScriptID: ptr.Uint(scriptByName["B"]), SyncRequest: true, ScriptContents: "C2",
	})
	require.NoError(t, err)
	execHost1ScriptC, err := ds.NewHostScriptExecutionRequest(ctx, &mobius.HostScriptRequestPayload{
		HostID: hosts[1].ID, ScriptID: ptr.Uint(scriptByName["C"]), SyncRequest: true, ScriptContents: "C3",
	})
	require.NoError(t, err)
	execHost2ScriptC, err := ds.NewHostScriptExecutionRequest(ctx, &mobius.HostScriptRequestPayload{
	...
	require.NoError(t, err)
	execHost3ScriptA, err := ds.NewHostScriptExecutionRequest(ctx, &mobius.HostScriptRequestPayload{
	...
	require.NoError(t, err)
	execHost3ScriptC, err := ds.NewHostScriptExecutionRequest(ctx, &mobius.HostScriptRequestPayload{
	...
	// no change
	_, err = ds.BatchSetScripts(ctx, nil, []*mobius.Script{
		{Name: "A", ScriptContents: "C1"},
		{Name: "B", ScriptContents: "C2"},
		{Name: "C", ScriptContents: "C3"},
	})
