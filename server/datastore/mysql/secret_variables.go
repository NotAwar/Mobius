...
func (ds *Datastore) UpsertSecretVariables(ctx context.Context, secretVariables []mobius.SecretVariable) error {
	...
	var variablesToInsert []mobius.SecretVariable
	var variablesToUpdate []mobius.SecretVariable
	for _, secretVariable := range secretVariables {
...
func (ds *Datastore) GetSecretVariables(ctx context.Context, names []string) ([]mobius.SecretVariable, error) {
	if len(names) == 0 {
		return nil, nil
	}
	...
	var secretVariables []mobius.SecretVariable
...
func (ds *Datastore) expandEmbeddedSecrets(ctx context.Context, document string) (string, []mobius.SecretVariable, error) {
	embeddedSecrets := mobius.ContainsPrefixVars(document, mobius.ServerSecretPrefix)
	...
	if len(missingSecrets) > 0 {
		return "", nil, mobius.MissingSecretsError{MissingSecrets: missingSecrets}
	}

	expanded := mobius.MaybeExpand(document, func(s string, startPos, endPos int) (string, bool) {
		if !strings.HasPrefix(s, mobius.ServerSecretPrefix) {
			return "", false
		}
		val, ok := secretMap[strings.TrimPrefix(s, mobius.ServerSecretPrefix)]
		return val, ok
	})
...
func (ds *Datastore) ValidateEmbeddedSecrets(ctx context.Context, documents []string) error {
	...
	for _, document := range documents {
		vars := mobius.ContainsPrefixVars(document, mobius.ServerSecretPrefix)
		for _, v := range vars {
	...
	if len(missingSecrets) > 0 {
		return &mobius.MissingSecretsError{MissingSecrets: missingSecrets}
	}
