...
func testUpsertSecretVariables(t *testing.T, ds *Datastore) {
	...
	createExpectedSecrets := func() []mobius.SecretVariable {
		secrets := make([]mobius.SecretVariable, 0, len(secretMap))
		for name, value := range secretMap {
			secrets = append(secrets, mobius.SecretVariable{Name: name, Value: value})
		}
		return secrets
	}
...
func testValidateEmbeddedSecrets(t *testing.T, ds *Datastore) {
	noSecrets := `
This document contains to mobius secrets.
$MOBIUS_VAR_XX $HOSTNAME ${SOMETHING_ELSE}
`

	validSecret := `
This document contains a valid ${MOBIUS_SECRET_VALID}.
Another $MOBIUS_SECRET_ALSO_VALID.
`

	invalidSecret := `
This document contains a secret not stored in the database.
Hello doc${MOBIUS_SECRET_INVALID}. $MOBIUS_SECRET_ALSO_INVALID
`
	ctx := context.Background()
	secretMap := map[string]string{
		"VALID":      "testValue1",
	...
	require.ErrorContains(t, err, "$MOBIUS_SECRET_INVALID")
	require.ErrorContains(t, err, "$MOBIUS_SECRET_ALSO_INVALID")
...
func testExpandEmbeddedSecrets(t *testing.T, ds *Datastore) {
	noSecrets := `
This document contains no mobius secrets.
$MOBIUS_VAR_XX $HOSTNAME ${SOMETHING_ELSE}
`
