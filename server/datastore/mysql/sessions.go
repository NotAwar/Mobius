...
func (ds *Datastore) SessionByMFAToken(ctx context.Context, token string, sessionKeySize int) (*mobius.Session, *mobius.User, error) {
	var userID uint
	err := sqlx.GetContext(
		ctx,
		ds.reader(ctx),
...
func (ds *Datastore) SessionByKey(ctx context.Context, key string) (*mobius.Session, error) {
	sqlStatement := `
...
func (ds *Datastore) SessionByID(ctx context.Context, id uint) (*mobius.Session, error) {
	return ds.sessionByID(ctx, ds.reader(ctx), id)
}

func (ds *Datastore) sessionByID(ctx context.Context, q sqlx.QueryerContext, id uint) (*mobius.Session, error) {
	sqlStatement := `
		SELECT s.*, u.api_only FROM sessions s
		LEFT JOIN users u
		ON s.user_id = u.id
...
func (ds *Datastore) ListSessionsForUser(ctx context.Context, id uint) ([]*mobius.Session, error) {
	sqlStatement := `
		SELECT s.*, u.api_only FROM sessions s
		INNER JOIN users u
		ON s.user_id = u.id
...
func (ds *Datastore) NewSession(ctx context.Context, userID uint, sessionKeySize int) (*mobius.Session, error) {
	return ds.makeSessionInTransaction(ctx, ds.writer(ctx), userID, sessionKeySize)
}

func (ds *Datastore) makeSessionInTransaction(ctx context.Context, tx sqlx.ExtContext, userID uint, sessionKeySize int) (*mobius.Session, error) {
...
func (ds *Datastore) DestroySession(ctx context.Context, session *mobius.Session) error {
...
func (ds *Datastore) MarkSessionAccessed(ctx context.Context, session *mobius.Session) error {
	sqlStatement := `
		UPDATE sessions SET
		accessed_at = ?
		WHERE id = ?
