...
WHERE id IN (%s)`
	...
	if err := ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		...
		for _, tuple := range softwareIDPlatforms {
			...
			if tuple.Platform != string(mobius.MacOSPlatform) {
...
func (ds *Datastore) ListSetupExperienceSoftwareTitles(ctx context.Context, teamID uint, opts mobius.ListOptions) ([]mobius.SoftwareTitleListResult, int, *mobius.PaginationMetadata, error) {
	opts.IncludeMetadata = true
	opts.After = ""

	titles, count, meta, err := ds.ListSoftwareTitles(ctx, mobius.SoftwareTitleListOptions{
		TeamID:              &teamID,
		ListOptions:         opts,
...
func (ds *Datastore) ListSetupExperienceResultsByHostUUID(ctx context.Context, hostUUID string) ([]*mobius.SetupExperienceStatusResult, error) {
	const stmt = `
SELECT
	sesr.id,
	sesr.host_uuid,
...
WHERE host_uuid = ?
	`
	var results []*mobius.SetupExperienceStatusResult
...
func (ds *Datastore) UpdateSetupExperienceStatusResult(ctx context.Context, status *mobius.SetupExperienceStatusResult) error {
	const stmt = `
...
func (ds *Datastore) GetSetupExperienceScript(ctx context.Context, teamID *uint) (*mobius.Script, error) {
	query := `
SELECT
  id,
  team_id,
...
func (ds *Datastore) GetSetupExperienceScriptByID(ctx context.Context, scriptID uint) (*mobius.Script, error) {
	query := `
SELECT
  id,
  team_id,
...
func (ds *Datastore) SetSetupExperienceScript(ctx context.Context, script *mobius.Script) error {
...
func insertSetupExperienceScript(ctx context.Context, tx sqlx.ExtContext, script *mobius.Script, scriptContentsID uint) (sql.Result, error) {
	const insertStmt = `
INSERT INTO
  setup_experience_scripts (
...
func (ds *Datastore) MaybeUpdateSetupExperienceVPPStatus(ctx context.Context, hostUUID string, nanoCommandUUID string, status mobius.SetupExperienceStatusResultStatus) (bool, error) {
...
func (ds *Datastore) MaybeUpdateSetupExperienceSoftwareInstallStatus(ctx context.Context, hostUUID string, executionID string, status mobius.SetupExperienceStatusResultStatus) (bool, error) {
...
func (ds *Datastore) MaybeUpdateSetupExperienceScriptStatus(ctx context.Context, hostUUID string, executionID string, status mobius.SetupExperienceStatusResultStatus) (bool, error) {
