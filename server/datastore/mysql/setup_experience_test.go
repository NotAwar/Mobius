...
func testEnqueueSetupExperienceItems(t *testing.T, ds *Datastore) {
	...
	err = ds.SetSetupExperienceScript(ctx, &mobius.Script{Name: "script1", ScriptContents: "SCRIPT 1", TeamID: &team1.ID})
	require.NoError(t, err)
	err = ds.SetSetupExperienceScript(ctx, &mobius.Script{Name: "script2", ScriptContents: "SCRIPT 2", TeamID: &team2.ID})
	require.NoError(t, err)
...
func testGetSetupExperienceTitles(t *testing.T, ds *Datastore) {
	...
	tfr4, err := mobius.NewTempFileReader(strings.NewReader("hello2"), t.TempDir)
	require.NoError(t, err)
	...
	titles, count, meta, err := ds.ListSetupExperienceSoftwareTitles(ctx, team1.ID, mobius.ListOptions{})
	require.NoError(t, err)
	...
	titles, count, meta, err = ds.ListSetupExperienceSoftwareTitles(ctx, team2.ID, mobius.ListOptions{})
	require.NoError(t, err)
	assert.Len(t, titles, 1)
	...
	app2 := &mobius.VPPApp{Name: "vpp_app_2", VPPAppTeam: mobius.VPPAppTeam{VPPAppID: mobius.VPPAppID{AdamID: "2", Platform: mobius.IOSPlatform}}, BundleIdentifier: "b2"}
...
func testSetSetupExperienceTitles(t *testing.T, ds *Datastore) {
	...
	app4 := &mobius.VPPApp{Name: "vpp_app_1: iOS", VPPAppTeam: mobius.VPPAppTeam{VPPAppID: mobius.VPPAppID{AdamID: "1", Platform: mobius.IOSPlatform}}, BundleIdentifier: "b1"}
	...
	softwareTitles, _, _, err := ds.ListSoftwareTitles(ctx, mobius.SoftwareTitleListOptions{TeamID: &team1.ID}, mobius.TeamFilter{TeamID: &team1.ID})
	require.NoError(t, err)
	...
	softwareTitles, _, _, err = ds.ListSoftwareTitles(ctx, mobius.SoftwareTitleListOptions{TeamID: &team2.ID}, mobius.TeamFilter{TeamID: &team2.ID})
	require.NoError(t, err)
...
func testSetupExperienceStatusResults(t *testing.T, ds *Datastore) {
	...
	user, err := ds.NewUser(ctx, &mobius.User{Name: "Foo", Email: "foo@example.com", GlobalRole: ptr.String("admin"), Password: []byte("12characterslong!")})
	require.NoError(t, err)
	installerID, _, err := ds.MatchOrCreateSoftwareInstaller(ctx, &mobius.UploadSoftwareInstallerPayload{Filename: "test.app", Version: "1.0.0", UserID: user.ID, ValidatedLabels: &mobius.LabelIdentsWithScope{}})
	require.NoError(t, err)
	...
	assert.NoError(t, err)
	vppApp, err := ds.InsertVPPAppWithTeam(ctx, &mobius.VPPApp{BundleIdentifier: "com.test.test", Name: "test.app", LatestVersion: "1.0.0"}, nil)
	require.NoError(t, err)
	...
	insertSetupExperienceStatusResult := func(sesr *mobius.SetupExperienceStatusResult) {
	...
	expRes := []*mobius.SetupExperienceStatusResult{
		{
			HostUUID:            hostUUID,
			Name:                "software",
			Status:              mobius.SetupExperienceStatusPending,
...
func testSetupExperienceScriptCRUD(t *testing.T, ds *Datastore) {
	...
	wantScriptNoTeam := &mobius.Script{
		Name:           "script",
		ScriptContents: "echo bar",
	}
	...
	err = ds.SetSetupExperienceScript(ctx, &mobius.Script{Name: "script", ScriptContents: "echo baz"})
	require.Error(t, err)
	require.ErrorAs(t, err, &existsErr)

	// try to create another script with no team id and a different name
	err = ds.SetSetupExperienceScript(ctx, &mobius.Script{Name: "script2", ScriptContents: "echo baz"})
	require.Error(t, err)
	require.ErrorAs(t, err, &existsErr)

	// try to add a script for a team that doesn't exist
	var fkErr mobius.ForeignKeyError
	err = ds.SetSetupExperienceScript(ctx, &mobius.Script{TeamID: ptr.Uint(42), Name: "script", ScriptContents: "echo baz"})
	require.Error(t, err)
