...
func softwareSliceToMap(softwareItems []mobius.Software) map[string]mobius.Software {
	result := make(map[string]mobius.Software, len(softwareItems))
...
func (ds *Datastore) UpdateHostSoftware(ctx context.Context, hostID uint, software []mobius.Software) (*mobius.UpdateHostSoftwareDBResult, error) {
...
func (ds *Datastore) UpdateHostSoftwareInstalledPaths(
	...
	mutationResults *mobius.UpdateHostSoftwareDBResult,
) error {
...
func (ds *Datastore) getHostSoftwareInstalledPaths(
	...
	[]mobius.HostSoftwareInstalledPath,
	error,
) {
	stmt := `
	...
	var result []mobius.HostSoftwareInstalledPath
...
func hostSoftwareInstalledPathsDelta(
	hostID uint,
	reported map[string]struct{},
	stored []mobius.HostSoftwareInstalledPath,
	hostSoftware []mobius.Software,
	logger log.Logger,
) (
	...
	sIDLookup := map[uint]mobius.Software{}
	for _, s := range hostSoftware {
		sIDLookup[s.ID] = s
	}

	sUnqStrLook := map[string]mobius.Software{}
	for _, s := range hostSoftware {
		sUnqStrLook[s.ToUniqueStr()] = s
	}

	iSPathLookup := make(map[string]mobius.HostSoftwareInstalledPath)
	for _, r := range stored {
		...
		key := fmt.Sprintf(
			"%s%s%s%s%s%s%s",
			r.InstalledPath, mobius.SoftwareFieldSeparator, r.TeamIdentifier, mobius.SoftwareFieldSeparator, sha256, mobius.SoftwareFieldSeparator, s.ToUniqueStr(),
		)
		iSPathLookup[key] = r
	...
	for key := range reported {
		parts := strings.SplitN(key, mobius.SoftwareFieldSeparator, 4)
		...
		toInsert = append(toInsert, mobius.HostSoftwareInstalledPath{
			HostID:           hostID,
			SoftwareID:       s.ID,
...
func nothingChanged(current, incoming []mobius.Software, minLastOpenedAtDiff time.Duration) (
	map[string]mobius.Software, map[string]mobius.Software, bool,
) {
	// Process incoming software to ensure there are no duplicates, since the same software can be installed at multiple paths.
	incomingMap := make(map[string]mobius.Software, len(current)) // setting len(current) as the length since that should be the common case
...
func (ds *Datastore) ListSoftwareByHostIDShort(ctx context.Context, hostID uint) ([]mobius.Software, error) {
...
func (ds *Datastore) applyChangesForNewSoftwareDB(
...
) (*mobius.UpdateHostSoftwareDBResult, error) {
	r := &mobius.UpdateHostSoftwareDBResult{}
	...
	err = ds.withRetryTxx(
		ctx, func(tx sqlx.ExtContext) error {
			...
			incomingByChecksumCopy := make(map[string]mobius.Software, len(incomingByChecksum))
...
func checkForDeletedInstalledSoftware(ctx context.Context, tx sqlx.ExtContext, deleted []mobius.Software, inserted []mobius.Software,
	...
	if len(deletedTitles) > 0 {
		installedTitles, err := getInstalledByMobiusSoftwareTitles(ctx, tx, hostID)
		if err != nil {
			return err
		}
...
func (ds *Datastore) getExistingSoftware(
	ctx context.Context, current map[string]mobius.Software, incoming map[string]mobius.Software,
) (
	currentSoftware []softwareIDChecksum,
	incomingChecksumToSoftware map[string]mobius.Software,
	incomingChecksumToTitle map[string]mobius.SoftwareTitle,
	existingBundleIDsToUpdate map[string]mobius.Software,
	err error,
) {
	// Compute checksums for all incoming software, which we will use for faster retrieval, since checksum is a unique index
	incomingChecksumToSoftware = make(map[string]mobius.Software, len(current))
	newSoftware := make(map[string]struct{})
	bundleIDsToChecksum := make(map[string]string)
	bundleIDsToNames := make(map[string]string)
	existingBundleIDsToUpdate = make(map[string]mobius.Software)
...
func (ds *Datastore) getIncomingSoftwareChecksumsToExistingTitles(
	...
	incomingChecksumToSoftware map[string]mobius.Software,
) (map[string]mobius.SoftwareTitle, map[string]mobius.Software, error) {
	var (
		incomingChecksumToTitle     = make(map[string]mobius.SoftwareTitle, len(newSoftwareChecksums))
	...
	if len(argsWithoutBundleIdentifier) > 0 {
		...
		var existingSoftwareTitlesForNewSoftwareWithoutBundleIdentifier []mobius.SoftwareTitle
	...
	if len(argsWithBundleIdentifier) > 0 {
		...
		var existingSoftwareTitlesForNewSoftwareWithBundleIdentifier []mobius.SoftwareTitle
...
func UniqueSoftwareTitleStr(values ...string) string {
	return strings.Join(values, mobius.SoftwareFieldSeparator)
}
...
func deleteUninstalledHostSoftwareDB(
	...
	hostID uint,
	currentMap map[string]mobius.Software,
	incomingMap map[string]mobius.Software,
) ([]mobius.Software, error) {
	var deletesHostSoftwareIDs []uint
	var deletedSoftware []mobius.Software
...
func (ds *Datastore) insertNewInstalledHostSoftwareDB(
	...
	softwareChecksums map[string]mobius.Software,
	existingTitlesForNewSoftware map[string]mobius.SoftwareTitle,
	existingBundleIDsToUpdate map[string]mobius.Software,
) ([]mobius.Software, error) {
	var insertsHostSoftware []interface{}
	var insertedSoftware []mobius.Software
	...
	if len(softwareChecksums) > 0 {
		...
		for i := 0; i < len(keys); i += softwareInsertBatchSize {
			...
			newTitlesNeeded := make(map[string]mobius.SoftwareTitle)
			for j := start; j < end; j++ {
				checksum := keys[j]
...
func selectSoftwareSQL(opts mobius.SoftwareListOptions) (string, []interface{}, error) {
	ds := dialect.
...
func countSoftwareDB(
	...
	opts.ListOptions = mobius.ListOptions{
...
func (ds *Datastore) LoadHostSoftware(ctx context.Context, host *mobius.Host, includeCVEScores bool) error {
	opts := mobius.SoftwareListOptions{
		HostID:           &host.ID,
	...
	pathSignatureInformation := make(map[uint][]mobius.PathSignatureInformation)
	for _, ip := range installedPaths {
		installedPathsList[ip.SoftwareID] = append(installedPathsList[ip.SoftwareID], ip.InstalledPath)
		pathSignatureInformation[ip.SoftwareID] = append(pathSignatureInformation[ip.SoftwareID], mobius.PathSignatureInformation{
			InstalledPath:  ip.InstalledPath,
			TeamIdentifier: ip.TeamIdentifier,
		})
	}

	host.Software = make([]mobius.HostSoftwareEntry, 0, len(software))
	for _, s := range software {
		host.Software = append(host.Software, mobius.HostSoftwareEntry{
			Software:                 s,
...
func (si *softwareIterator) Value() (*mobius.Software, error) {
	dest := mobius.Software{}
...
// iteration using the returned mobius.SoftwareIterator is done.
func (ds *Datastore) AllSoftwareIterator(
	ctx context.Context,
	query mobius.SoftwareIterQueryOptions,
) (mobius.SoftwareIterator, error) {
	if !query.IsValid() {
...
func (ds *Datastore) UpsertSoftwareCPEs(ctx context.Context, cpes []mobius.SoftwareCPE) (int64, error) {
	var args []interface{}
...
func (ds *Datastore) DeleteSoftwareCPEs(ctx context.Context, cpes []mobius.SoftwareCPE) (int64, error) {
	if len(cpes) == 0 {
		return 0, nil
	}
...
func (ds *Datastore) ListSoftwareCPEs(ctx context.Context) ([]mobius.SoftwareCPE, error) {
	var result []mobius.SoftwareCPE
...
func (ds *Datastore) ListSoftware(ctx context.Context, opt mobius.SoftwareListOptions) ([]mobius.Software, *mobius.PaginationMetadata, error) {
	if !opt.VulnerableOnly && (opt.MinimumCVSS > 0 || opt.MaximumCVSS > 0 || opt.KnownExploit) {
		return nil, nil, mobius.NewInvalidArgumentError(
...
func (ds *Datastore) CountSoftware(ctx context.Context, opt mobius.SoftwareListOptions) (int, error) {
...
func (ds *Datastore) DeleteSoftwareVulnerabilities(ctx context.Context, vulnerabilities []mobius.SoftwareVulnerability) error {
...
func (ds *Datastore) DeleteOutOfDateVulnerabilities(ctx context.Context, source mobius.VulnerabilitySource, duration time.Duration) error {
...
func (ds *Datastore) SoftwareByID(ctx context.Context, id uint, teamID *uint, includeCVEScores bool, tmFilter *mobius.TeamFilter) (*mobius.Software, error) {
...
func (ds *Datastore) HostVulnSummariesBySoftwareIDs(ctx context.Context, softwareIDs []uint) ([]mobius.HostVulnerabilitySummary, error) {
	stmt := `
		SELECT DISTINCT
			h.id,
			h.hostname,
	...
	var result []mobius.HostVulnerabilitySummary
	...
	for _, r := range qR {
		...
		mapped := mobius.HostVulnerabilitySummary{
			ID:          r.HostID,
			Hostname:    r.HostName,
...
func (ds *Datastore) HostsByCVE(ctx context.Context, cve string) ([]mobius.HostVulnerabilitySummary, error) {
	stmt := `
		SELECT DISTINCT
				(h.id),
				h.hostname,
...
func (ds *Datastore) InsertCVEMeta(ctx context.Context, cveMeta []mobius.CVEMeta) error {
	query := `
...
func (ds *Datastore) ListSoftwareVulnerabilitiesByHostIDsSource(
	...
	hostIDs []uint,
	source mobius.VulnerabilitySource,
) (map[uint][]mobius.SoftwareVulnerability, error) {
	result := make(map[uint][]mobius.SoftwareVulnerability)
...
func (ds *Datastore) ListSoftwareForVulnDetection(
	ctx context.Context,
	filters mobius.VulnSoftwareFilter,
) ([]mobius.Software, error) {
	var result []mobius.Software
	var sqlstmt string
	var args []interface{}
...
func (ds *Datastore) ListCVEs(ctx context.Context, maxAge time.Duration) ([]mobius.CVEMeta, error) {
	var result []mobius.CVEMeta
...
type hostSoftware struct {
	mobius.HostSoftwareWithInstaller
...
func filterVppAppsByLabel(
	...
	// This is the list of VPP apps that are installed on the host by mobius or the user
	...
	if len(vppAppIDsToCheck) > 0 {
		...
		// differentiate between VPP apps that were installed by Mobius (show install details +
		// ability to reinstall in self-service) vs. VPP apps that Mobius knows about but either
		// weren't installed by Mobius or were installed by Mobius but are no longer in scope
...
func hostVPPInstalls(ds *Datastore, ctx context.Context, hostID uint, globalOrTeamID uint, selfServiceOnly bool, isMDMEnrolled bool) ([]*hostSoftware, error) {
	...
	vppInstallsStmt, args, err := sqlx.Named(vppInstallsStmt, map[string]any{
		"host_id":                   hostID,
		"global_or_team_id":         globalOrTeamID,
		"software_status_installed": mobius.SoftwareInstalled,
		"mdm_status_acknowledged":   mobius.MDMAppleStatusAcknowledged,
		"mdm_status_error":          mobius.MDMAppleStatusError,
		"mdm_status_format_error":   mobius.MDMAppleStatusCommandFormatError,
		"software_status_failed":    mobius.SoftwareInstallFailed,
		"software_status_pending":   mobius.SoftwareInstallPending,
	})
	if err != nil {
...
func hydrateHostSoftwareRecordFromDb(hydrated *hostSoftware, softwareTitle *hostSoftware) {
	...
	hydrated.SoftwarePackage = &mobius.SoftwarePackageOrApp{
	...
	if softwareTitle.LastInstallInstallUUID != nil && *softwareTitle.LastInstallInstallUUID != "" {
		hydrated.SoftwarePackage.LastInstall = &mobius.HostSoftwareInstall{
	...
	if softwareTitle.LastUninstallScriptExecutionID != nil && *softwareTitle.LastUninstallScriptExecutionID != "" {
		hydrated.SoftwarePackage.LastUninstall = &mobius.HostSoftwareUninstall{
...
func promoteSoftwareTitleVPPApp(softwareTitleRecord *hostSoftware) {
	...
	softwareTitleRecord.AppStoreApp = &mobius.SoftwarePackageOrApp{
	...
	if softwareTitleRecord.LastInstallInstallUUID != nil && *softwareTitleRecord.LastInstallInstallUUID != "" {
		softwareTitleRecord.AppStoreApp.LastInstall = &mobius.HostSoftwareInstall{
...
func (ds *Datastore) ListHostSoftware(ctx context.Context, host *mobius.Host, opts mobius.HostSoftwareTitleListOptions) ([]*mobius.HostSoftwareWithInstaller, *mobius.PaginationMetadata, error) {
	...
	namedArgs := map[string]any{
		"host_id":               host.ID,
		"host_platform":         host.MobiusPlatform(),
		"global_or_team_id":     globalOrTeamID,
		...
		"vpp_apps_platforms":    mobius.VPPAppsPlatforms,
		"known_exploit":         1,
	}
	...
	if opts.OnlyAvailableForInstall || opts.IncludeAvailableForInstall {
		namedArgs["vpp_apps_platforms"] = mobius.VPPAppsPlatforms
		namedArgs["host_compatible_platforms"] = host.MobiusPlatform()
		...
		// software installed on the host not by mobius and there exists a software installer that matches this software
		...
		if !opts.SelfServiceOnly || (opts.SelfServiceOnly && opts.IsMDMEnrolled) {
			// software installed on the host not by mobius and there exists a vpp app that matches this software
			// so that makes it available for install
			installedVPPAppsSql := `
				SELECT
					vpp_apps.title_id AS id,
			...
			for _, s := range installedVPPAppIDs {
				...
				if VPPAppByMobius, ok := hostVPPInstalledTitles[s.ID]; ok {
					// Vpp app installed by mobius, so we need to copy over the status,
					// because all mobius installed apps show an installed status if available
					tmpByVPPAdamID[*s.VPPAppAdamID].Status = VPPAppByMobius.Status
				}
				// If a VPP app is installed on the host, but not by mobius
	...
	if len(softwareTitleIds) > 0 || len(vppAdamIDs) > 0 {
		...
		default:
			return []*mobius.HostSoftwareWithInstaller{}, &mobius.PaginationMetadata{}, nil
		}
		...
		for _, softwareTitleRecord := range hostSoftwareList {
			...
			if softwareTitleRecord.SoftwareIDList != nil {
				...
				for index, softwareIdStr := range softwareIDList {
					version := &mobius.HostSoftwareInstalledVersion{}
	...
	software := make([]*mobius.HostSoftwareWithInstaller, 0, len(hostSoftwareList))
...
func (ds *Datastore) SetHostSoftwareInstallResult(ctx context.Context, result *mobius.HostSoftwareInstallResultPayload) (wasCanceled bool, err error) {
	const stmt = `
		UPDATE
...
`
	...
	err = ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		...
		if result.Status() != mobius.SoftwareInstallPending {
...
func getInstalledByMobiusSoftwareTitles(ctx context.Context, qc sqlx.QueryerContext, hostID uint) ([]mobius.SoftwareTitle, error) {
...
`
	...
	var titles []mobius.SoftwareTitle
...
func (ds *Datastore) NewSoftwareCategory(ctx context.Context, name string) (*mobius.SoftwareCategory, error) {
	...
	return &mobius.SoftwareCategory{Name: name, ID: id}, nil
}
