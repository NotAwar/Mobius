...
func (ds *Datastore) GetSoftwareInstallDetails(ctx context.Context, executionId string) (*mobius.SoftwareInstallDetails, error) {
	const stmt = `
  SELECT
...
`
	...
	result := &mobius.SoftwareInstallDetails{}
...
func (ds *Datastore) MatchOrCreateSoftwareInstaller(ctx context.Context, payload *mobius.UploadSoftwareInstallerPayload) (installerID, titleID uint, err error) {
...
func (ds *Datastore) createAutomaticPolicy(ctx context.Context, tx sqlx.ExtContext, policyData automatic_policy.PolicyData, teamID *uint, softwareInstallerID *uint, vppAppsTeamsID *uint) (*mobius.Policy, error) {
	tmID := mobius.PolicyNoTeamID
	if teamID != nil {
		tmID = *teamID
	}
	...
	policy, err := newTeamPolicy(ctx, tx, tmID, userID, mobius.PolicyPayload{
...
func (ds *Datastore) getOrGenerateSoftwareInstallerTitleID(ctx context.Context, payload *mobius.UploadSoftwareInstallerPayload) (uint, error) {
...
func (ds *Datastore) addSoftwareTitleToMatchingSoftware(ctx context.Context, titleID uint, payload *mobius.UploadSoftwareInstallerPayload) error {
...
func setOrUpdateSoftwareInstallerLabelsDB(ctx context.Context, tx sqlx.ExtContext, installerID uint, labels mobius.LabelIdentsWithScope, softwareType softwareType) error {
	...
	if len(labelIds) > 0 {
		...
		case mobius.LabelScopeIncludeAny:
			exclude = false
		case mobius.LabelScopeExcludeAny:
			exclude = true
		default:
...
func (ds *Datastore) SaveInstallerUpdates(ctx context.Context, payload *mobius.UpdateSoftwareInstallerPayload) error {
	...
	var clearMobiusMaintainedAppID string // FMA becomes custom package when uploading a new installer file
	if payload.InstallerFile != nil {
		touchUploaded = ", uploaded_at = NOW()"
		clearMobiusMaintainedAppID = ", mobius_maintained_app_id = NULL"
	}
	...
	err = ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		stmt := fmt.Sprintf(`UPDATE software_installers SET
			...
			WHERE id = ?`, touchUploaded, clearMobiusMaintainedAppID)
...
func (ds *Datastore) GetSoftwareInstallerMetadataByID(ctx context.Context, id uint) (*mobius.SoftwareInstaller, error) {
	query := `
SELECT
	si.id,
	si.team_id,
...
WHERE
	...
	var dest mobius.SoftwareInstaller
...
func (ds *Datastore) GetSoftwareInstallerMetadataByTeamAndTitleID(ctx context.Context, teamID *uint, titleID uint, withScriptContents bool) (*mobius.SoftwareInstaller, error) {
...
func (ds *Datastore) getSoftwareInstallerLabels(ctx context.Context, installerID uint) ([]mobius.SoftwareScopeLabel, error) {
	query := `
SELECT
	label_id,
	exclude,
...
var (
	errDeleteInstallerWithAssociatedPolicy = &mobius.ConflictError{Message: "Couldn't delete. Policy automation uses this software. Please disable policy automation for this software and try again."}
	errDeleteInstallerInstalledDuringSetup = &mobius.ConflictError{Message: "Couldn't delete. This software is installed when new Macs boot. Please remove software in Controls > Setup experience and try again."}
)
...
func (ds *Datastore) deletePendingSoftwareInstallsForPolicy(ctx context.Context, teamID *uint, policyID uint) error {
	...
	`
	_, err := ds.writer(ctx).ExecContext(ctx, deleteStmt, policyID, mobius.SoftwareInstallPending, globalOrTeamID)
	if err != nil {
...
func (ds *Datastore) InsertSoftwareInstallRequest(ctx context.Context, hostID uint, softwareInstallerID uint, opts mobius.HostSoftwareInstallOptions) (string, error) {
	const (
		getInstallerStmt = `
SELECT
...
func (ds *Datastore) runInstallerUpdateSideEffectsInTransaction(ctx context.Context, tx sqlx.ExtContext, installerID uint, wasMetadataUpdated bool, wasPackageUpdated bool) (affectedHostIDs []uint, err error) {
	if wasMetadataUpdated || wasPackageUpdated { // cancel pending installs/uninstalls
		...
		)`, mobius.SetupExperienceStatusCancelled, mobius.SetupExperienceStatusPending, mobius.SetupExperienceStatusRunning, installerID, installerID)
		if err != nil {
...
func (ds *Datastore) GetSoftwareInstallResults(ctx context.Context, resultsUUID string) (*mobius.HostSoftwareInstallerResult, error) {
	query := `
SELECT
	hsi.execution_id AS execution_id,
...
`
	...
	var dest mobius.HostSoftwareInstallerResult
...
func (ds *Datastore) GetSummaryHostSoftwareInstalls(ctx context.Context, installerID uint) (*mobius.SoftwareInstallerStatusSummary, error) {
	var dest mobius.SoftwareInstallerStatusSummary
...
FROM upcoming
	...
	query, args, err := sqlx.Named(stmt, map[string]interface{}{
		...
		"software_status_pending_install":   mobius.SoftwareInstallPending,
		"software_status_failed_install":    mobius.SoftwareInstallFailed,
		"software_status_pending_uninstall": mobius.SoftwareUninstallPending,
		"software_status_failed_uninstall":  mobius.SoftwareUninstallFailed,
...
func (ds *Datastore) vppAppJoin(appID mobius.VPPAppID, status mobius.SoftwareInstallerStatus) (string, []interface{}, error) {
	// for pending status, we'll join through upcoming_activities
	if status == mobius.SoftwarePending || status == mobius.SoftwareInstallPending || status == mobius.SoftwareUninstallPending {
		stmt := `JOIN (
SELECT DISTINCT
	host_id
FROM
...
WHERE
	%s) hss ON hss.host_id = h.id`
		...
		switch status {
		case mobius.SoftwareInstallPending:
			filter += " AND ua.activity_type = 'vpp_app_install'"
		case mobius.SoftwareUninstallPending:
	...
	if status == mobius.SoftwareFailed {
		status = mobius.SoftwareInstallFailed // TODO: When VPP supports uninstall this should become STATUS IN ('failed_install', 'failed_uninstall')
	}
...
func (ds *Datastore) softwareInstallerJoin(titleID uint, status mobius.SoftwareInstallerStatus) (string, []interface{}, error) {
	// for pending status, we'll join through upcoming_activities
	if status == mobius.SoftwarePending || status == mobius.SoftwareInstallPending || status == mobius.SoftwareUninstallPending {
		stmt := `JOIN (
SELECT DISTINCT
...
`, statusFilter)
	...
	return sqlx.Named(stmt, map[string]interface{}{
		"status":          status,
		"installFailed":   mobius.SoftwareInstallFailed,
		"uninstallFailed": mobius.SoftwareUninstallFailed,
		"title_id":        titleID,
	})
}

func (ds *Datastore) GetHostLastInstallData(ctx context.Context, hostID, installerID uint) (*mobius.HostLastInstallData, error) {
...
func (ds *Datastore) getLatestUpcomingInstall(ctx context.Context, hostID, installerID uint) (*mobius.HostLastInstallData, error) {
	var hostLastInstall mobius.HostLastInstallData
	stmt := `
SELECT
	execution_id,
	'pending_install' AS status
...
func (ds *Datastore) getLatestPastInstall(ctx context.Context, hostID, installerID uint) (*mobius.HostLastInstallData, error) {
	var hostLastInstall mobius.HostLastInstallData
	stmt := `
SELECT
	execution_id,
...
func (ds *Datastore) CleanupUnusedSoftwareInstallers(ctx context.Context, softwareInstallStore mobius.SoftwareInstallerStore, removeCreatedBefore time.Time) error {
...
func (ds *Datastore) BatchSetSoftwareInstallers(ctx context.Context, tmID *uint, installers []*mobius.UploadSoftwareInstallerPayload) error {
...
`
	...
	err := ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		...
		if len(installers) == 0 {
			...
			if _, err := tx.ExecContext(ctx, cancelSetupExperienceStatusForAllDeletedPendingSoftwareInstalls, mobius.SetupExperienceStatusCancelled, mobius.SetupExperienceStatusPending, mobius.SetupExperienceStatusRunning,
		...
		stmt, args, err = sqlx.In(cancelSetupExperienceStatusForDeletedSoftwareInstalls, mobius.SetupExperienceStatusCancelled, mobius.SetupExperienceStatusPending, mobius.SetupExperienceStatusRunning,
		...
		for _, installer := range installers {
			...
			args := []interface{}{
				...
				installer.MobiusMaintainedAppID,
				installer.InstallDuringSetup,
			}
			...
			if len(installer.ValidatedLabels.ByName) == 0 {
				...
				excludeLabels := installer.ValidatedLabels.LabelScope == mobius.LabelScopeExcludeAny
...
func (ds *Datastore) HasSelfServiceSoftwareInstallers(ctx context.Context, hostPlatform string, hostTeamID *uint) (bool, error) {
	if mobius.IsLinux(hostPlatform) {
		hostPlatform = "linux"
	}
	stmt := `SELECT 1
		WHERE EXISTS (
...
func (ds *Datastore) GetSoftwareInstallers(ctx context.Context, teamID uint) ([]mobius.SoftwarePackageResponse, error) {
...
`
	var softwarePackages []mobius.SoftwarePackageResponse
...
func (ds *Datastore) GetTeamsWithInstallerByHash(ctx context.Context, sha256, url string) (map[uint]*mobius.ExistingSoftwareInstaller, error) {
	stmt := `
SELECT
...
WHERE
	...
	var installers []*mobius.ExistingSoftwareInstaller
	...
	set := make(map[uint]*mobius.ExistingSoftwareInstaller, len(installers))
	for _, installer := range installers {
