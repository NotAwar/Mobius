...
func TestSoftwareInstallers(t *testing.T) {
	...
	cases := []struct {
		...
		{"SaveInstallerUpdatesClearsMobiusMaintainedAppID", testSaveInstallerUpdatesClearsMobiusMaintainedAppID},
	}
...
func testListPendingSoftwareInstalls(t *testing.T, ds *Datastore) {
	...
	err := ds.UpsertSecretVariables(ctx, []mobius.SecretVariable{
		{
			Name:  "RUBBER",
			Value: "DUCKY",
		},
	...
	require.NoError(t, err)
	installerID1, _, err := ds.MatchOrCreateSoftwareInstaller(ctx, &mobius.UploadSoftwareInstallerPayload{
		InstallScript:     "hello $MOBIUS_SECRET_RUBBER",
		PreInstallQuery:   "SELECT 1",
		PostInstallScript: "world $MOBIUS_SECRET_BIG",
		UninstallScript:   "goodbye $MOBIUS_SECRET_COOKIE",
		InstallerFile:     tfr1,
		StorageID:         "storage1",
		Filename:          "file1",
		Title:             "file1",
	...
	require.NoError(t, err)
	installerID2, _, err := ds.MatchOrCreateSoftwareInstaller(ctx, &mobius.UploadSoftwareInstallerPayload{
		InstallScript:     "world",
	...
	tfr3, err := mobius.NewTempFileReader(strings.NewReader("hello"), t.TempDir)
	require.NoError(t, err)
	installerID3, _, err := ds.MatchOrCreateSoftwareInstaller(ctx, &mobius.UploadSoftwareInstallerPayload{
	...
	hostInstall1, err := ds.InsertSoftwareInstallRequest(ctx, host1.ID, installerID1, mobius.HostSoftwareInstallOptions{})
	require.NoError(t, err)

	time.Sleep(time.Millisecond)
	hostInstall2, err := ds.InsertSoftwareInstallRequest(ctx, host1.ID, installerID2, mobius.HostSoftwareInstallOptions{})
	require.NoError(t, err)

	time.Sleep(time.Millisecond)
	hostInstall3, err := ds.InsertSoftwareInstallRequest(ctx, host2.ID, installerID1, mobius.HostSoftwareInstallOptions{})
	require.NoError(t, err)

	time.Sleep(time.Millisecond)
	hostInstall4, err := ds.InsertSoftwareInstallRequest(ctx, host2.ID, installerID2, mobius.HostSoftwareInstallOptions{})
	require.NoError(t, err)
	...
	hostInstall5, err := ds.InsertSoftwareInstallRequest(ctx, host2.ID, installerID2, mobius.HostSoftwareInstallOptions{})
	require.NoError(t, err)
	...
	hostInstall6, err := ds.InsertSoftwareInstallRequest(ctx, host1.ID, installerID3, mobius.HostSoftwareInstallOptions{SelfService: true})
	require.NoError(t, err)
	...
	_, err = ds.InsertSoftwareInstallRequest(ctx, host3.ID, installerID1, mobius.HostSoftwareInstallOptions{})
	require.NoError(t, err)
...
func testSoftwareInstallRequests(t *testing.T, ds *Datastore) {
	...
	// create a team
	team, err := ds.NewTeam(ctx, &mobius.Team{Name: "team 2"})
	require.NoError(t, err)
	...
	for tc, teamID := range cases {
		t.Run(tc, func(t *testing.T) {
			...
			// non-existent host
			_, err = ds.InsertSoftwareInstallRequest(ctx, 12, si.InstallerID, mobius.HostSoftwareInstallOptions{})
			require.ErrorAs(t, err, &nfe)

			// Host with software install pending
			tag := "-pending_install"
			hostPendingInstall, err := ds.NewHost(ctx, &mobius.Host{
			...
			require.NoError(t, err)
			_, err = ds.InsertSoftwareInstallRequest(ctx, hostPendingInstall.ID, si.InstallerID, mobius.HostSoftwareInstallOptions{})
			require.NoError(t, err)

			// Host with software install failed
			tag = "-failed_install"
			hostFailedInstall, err := ds.NewHost(ctx, &mobius.Host{
				Hostname:      "macos-test" + tag + tc,
				OsqueryHostID: ptr.String("osquery-macos" + tag + tc),
				NodeKey:       ptr.String("node-key-macos" + tag + tc),
				UUID:          uuid.NewString(),
			...
			require.NoError(t, err)
			execID, err := ds.InsertSoftwareInstallRequest(ctx, hostFailedInstall.ID, si.InstallerID, mobius.HostSoftwareInstallOptions{})
			require.NoError(t, err)
			_, err = ds.SetHostSoftwareInstallResult(ctx, &mobius.HostSoftwareInstallResultPayload{
			...
			tag = "-installed"
			hostInstalled, err := ds.NewHost(ctx, &mobius.Host{
				Hostname:      "macos-test" + tag + tc,
				OsqueryHostID: ptr.String("osquery-macos" + tag + tc),
				NodeKey:       ptr.String("node-key-macos" + tag + tc),
				UUID:          uuid.NewString(),
			...
			require.NoError(t, err)
			execID, err = ds.InsertSoftwareInstallRequest(ctx, hostInstalled.ID, si.InstallerID, mobius.HostSoftwareInstallOptions{})
			require.NoError(t, err)
			_, err = ds.SetHostSoftwareInstallResult(ctx, &mobius.HostSoftwareInstallResultPayload{
			...
			hostPendingUninstall, err := ds.NewHost(ctx, &mobius.Host{
				Hostname:      "macos-test" + tag + tc,
				OsqueryHostID: ptr.String("osquery-macos" + tag + tc),
				NodeKey:       ptr.String("node-key-macos" + tag + tc),
				UUID:          uuid.NewString(),
			...
			hostFailedUninstall, err := ds.NewHost(ctx, &mobius.Host{
				Hostname:      "macos-test" + tag + tc,
				OsqueryHostID: ptr.String("osquery-macos" + tag + tc),
				NodeKey:       ptr.String("node-key-macos" + tag + tc),
				UUID:          uuid.NewString(),
			...
			tag = "-uninstalled"
			hostUninstalled, err := ds.NewHost(ctx, &mobius.Host{
				Hostname:      "macos-test" + tag + tc,
				OsqueryHostID: ptr.String("osquery-macos" + tag + tc),
				NodeKey:       ptr.String("node-key-macos" + tag + tc),
				UUID:          uuid.NewString(),
			...
			userTeamFilter := mobius.TeamFilter{
				User: &mobius.User{GlobalRole: ptr.String("admin")},
			}
			...
			expectStatus := mobius.SoftwareInstallPending
			hosts, err := ds.ListHosts(ctx, userTeamFilter, mobius.HostListOptions{
				ListOptions:           mobius.ListOptions{PerPage: 100},
			...
			getHostNames := func(hosts []*mobius.Host) []string {
			...
			expectStatus = mobius.SoftwarePending
			hosts, err = ds.ListHosts(ctx, userTeamFilter, mobius.HostListOptions{
				ListOptions:           mobius.ListOptions{PerPage: 100},
				SoftwareTitleIDFilter: installerMeta.TitleID,
				SoftwareStatusFilter:  &expectStatus,
			...
			expectStatus = mobius.SoftwareInstallFailed
			hosts, err = ds.ListHosts(ctx, userTeamFilter, mobius.HostListOptions{
				ListOptions:           mobius.ListOptions{PerPage: 100},
				SoftwareTitleIDFilter: installerMeta.TitleID,
				SoftwareStatusFilter:  &expectStatus,
			...
			expectStatus = mobius.SoftwareFailed
			hosts, err = ds.ListHosts(ctx, userTeamFilter, mobius.HostListOptions{
				ListOptions:           mobius.ListOptions{PerPage: 100},
				SoftwareTitleIDFilter: installerMeta.TitleID,
				SoftwareStatusFilter:  &expectStatus,
			...
			expectStatus = mobius.SoftwareInstalled
			hosts, err = ds.ListHosts(ctx, userTeamFilter, mobius.HostListOptions{
				ListOptions:           mobius.ListOptions{PerPage: 100},
				SoftwareTitleIDFilter: installerMeta.TitleID,
				SoftwareStatusFilter:  &expectStatus,
			...
			expectStatus = mobius.SoftwareUninstallPending
			hosts, err = ds.ListHosts(ctx, userTeamFilter, mobius.HostListOptions{
				ListOptions:           mobius.ListOptions{PerPage: 100},
				SoftwareTitleIDFilter: installerMeta.TitleID,
				SoftwareStatusFilter:  &expectStatus,
			...
			expectStatus = mobius.SoftwareUninstallFailed
			hosts, err = ds.ListHosts(ctx, userTeamFilter, mobius.HostListOptions{
				ListOptions:           mobius.ListOptions{PerPage: 100},
				SoftwareTitleIDFilter: installerMeta.TitleID,
				SoftwareStatusFilter:  &expectStatus,
			...
			// list all hosts with the software title that shows up in host_software (after mobiusdaemon software query is run)
			hosts, err = ds.ListHosts(ctx, userTeamFilter, mobius.HostListOptions{
				ListOptions:           mobius.ListOptions{PerPage: 100},
				SoftwareTitleIDFilter: installerMeta.TitleID,
				TeamFilter:            teamFilter,
...
func testGetSoftwareInstallResult(t *testing.T, ds *Datastore) {
	...
	for _, tc := range []struct {
		name                    string
		expectedStatus          mobius.SoftwareInstallerStatus
	...
	}{
		{
			...
			expectedStatus:          mobius.SoftwareInstallPending,
		...
		},
		{
			...
			expectedStatus:          mobius.SoftwareInstallFailed,
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			installUUID, err := ds.InsertSoftwareInstallRequest(ctx, host.ID, installerID, mobius.HostSoftwareInstallOptions{})
			require.NoError(t, err)
			...
			if tc.expectedStatus == mobius.SoftwareInstallPending { // expect pending to be deleted
...
func testCleanupUnusedSoftwareInstallers(t *testing.T, ds *Datastore) {
	...
	tfr0, err := mobius.NewTempFileReader(ins0File, t.TempDir)
	require.NoError(t, err)
	assertExisting([]string{ins0})

	swi, _, err := ds.MatchOrCreateSoftwareInstaller(ctx, &mobius.UploadSoftwareInstallerPayload{
...
func testBatchSetSoftwareInstallers(t *testing.T, ds *Datastore) {
	...
	// create a team
	team, err := ds.NewTeam(ctx, &mobius.Team{Name: t.Name()})
	require.NoError(t, err)
	...
	assertSoftware := func(wantTitles []mobius.SoftwareTitle) {
		tmFilter := mobius.TeamFilter{User: &mobius.User{GlobalRole: ptr.String(mobius.RoleAdmin)}}
		titles, _, _, err := ds.ListSoftwareTitles(
			ctx,
			mobius.SoftwareTitleListOptions{TeamID: &team.ID},
			tmFilter,
		)
		require.NoError(t, err)
	...
	assertSoftware(nil)
	err = ds.BatchSetSoftwareInstallers(ctx, &team.ID, []*mobius.UploadSoftwareInstallerPayload{})
	require.NoError(t, err)
	softwareInstallers, err = ds.GetSoftwareInstallers(ctx, team.ID)
	require.NoError(t, err)
	require.Empty(t, softwareInstallers)
	...
	tfr0, err := mobius.NewTempFileReader(ins0File, t.TempDir)
	require.NoError(t, err)
	maintainedApp, err := ds.UpsertMaintainedApp(ctx, &mobius.MaintainedApp{
		Name:             "Maintained1",
		Slug:             "maintained1",
		Platform:         "darwin",
		UniqueIdentifier: "mobius.maintained1",
	})
	require.NoError(t, err)
	err = ds.BatchSetSoftwareInstallers(ctx, &team.ID, []*mobius.UploadSoftwareInstallerPayload{{
		InstallScript:        "install",
		...
		MobiusMaintainedAppID: ptr.Uint(maintainedApp.ID),
	}})
	require.NoError(t, err)
	softwareInstallers, err = ds.GetSoftwareInstallers(ctx, team.ID)
	require.NoError(t, err)
	...
	require.Equal(t, maintainedApp.ID, *softwareInstallers[0].MobiusMaintainedAppID)
	assertSoftware([]mobius.SoftwareTitle{
	...
	tfr1, err := mobius.NewTempFileReader(ins1File, t.TempDir)
	require.NoError(t, err)
	err = ds.BatchSetSoftwareInstallers(ctx, &team.ID, []*mobius.UploadSoftwareInstallerPayload{
		{
			InstallScript:      "install",
	...
	_, err = ds.InsertSoftwareInstallRequest(ctx, host1.ID, instDetails1.InstallerID, mobius.HostSoftwareInstallOptions{})
	require.NoError(t, err)
	execID2, err := ds.InsertSoftwareInstallRequest(ctx, host2.ID, instDetails1.InstallerID, mobius.HostSoftwareInstallOptions{})
	require.NoError(t, err)
	_, err = ds.SetHostSoftwareInstallResult(ctx, &mobius.HostSoftwareInstallResultPayload{
		HostID:                host2.ID,
		InstallUUID:           execID2,
	...
	require.NoError(t, err)
	require.Equal(t, mobius.SoftwareInstallerStatusSummary{Installed: 1, PendingInstall: 1}, *summary)
	...
	summary, err = ds.GetSummaryHostSoftwareInstalls(ctx, instDetails1.InstallerID)
	require.NoError(t, err)
	require.Equal(t, mobius.SoftwareInstallerStatusSummary{Installed: 0, PendingInstall: 0}, *summary)
	pendingHost1, err := ds.ListPendingSoftwareInstalls(ctx, host1.ID)
	require.NoError(t, err)
	...
	_, err = ds.InsertSoftwareInstallRequest(ctx, host1.ID, instDetails0.InstallerID, mobius.HostSoftwareInstallOptions{})
	require.NoError(t, err)
	execID2b, err := ds.InsertSoftwareInstallRequest(ctx, host2.ID, instDetails0.InstallerID, mobius.HostSoftwareInstallOptions{})
	require.NoError(t, err)
	...
	require.NoError(t, err)
	require.Equal(t, mobius.SoftwareInstallerStatusSummary{FailedInstall: 1, PendingInstall: 1}, *summary)
	...
	summary, err = ds.GetSummaryHostSoftwareInstalls(ctx, instDetails0.InstallerID)
	require.NoError(t, err)
	require.Equal(t, mobius.SoftwareInstallerStatusSummary{FailedInstall: 0, PendingInstall: 0}, *summary)
	pendingHost1, err = ds.ListPendingSoftwareInstalls(ctx, host1.ID)
	require.NoError(t, err)
	require.Empty(t, pendingHost1)
}
...
func testBatchSetSoftwareInstallersSetupExperienceSideEffects(t *testing.T, ds *Datastore) {
	...
	for _, status := range statuses {
		execID, err := ds.InsertSoftwareInstallRequest(ctx, host1.ID, *status.SoftwareInstallerID, mobius.HostSoftwareInstallOptions{ForSetupExperience: true})
		require.NoError(t, err)
		status.HostSoftwareInstallsExecutionID = &execID
		status.Status = mobius.SetupExperienceStatusRunning
	...
	for _, status := range statuses {
		require.Equal(t, mobius.SetupExperienceStatusRunning, status.Status)
	}
	...
	for _, status := range statuses {
		if status.Name == ins0 {
			...
			ins0Found = true
			require.Equal(t, mobius.SetupExperienceStatusCancelled, status.Status)
		} else {
...
func testHasSelfServiceSoftwareInstallers(t *testing.T, ds *Datastore) {
	...
	_, _, err = ds.MatchOrCreateSoftwareInstaller(ctx, &mobius.UploadSoftwareInstallerPayload{
		Title:           "foo",
		Source:          "bar",
		InstallScript:   "echo install",
		TeamID:          &team.ID,
	...
	_, err = ds.InsertVPPAppWithTeam(ctx, &mobius.VPPApp{VPPAppTeam: mobius.VPPAppTeam{VPPAppID: mobius.VPPAppID{AdamID: "adam_vpp_1", Platform: platform}}, Name: "vpp1", BundleIdentifier: "com.app.vpp1"}, nil)
	require.NoError(t, err)
	hasSelfService, err = ds.HasSelfServiceSoftwareInstallers(ctx, platform, nil)
	require.NoError(t, err)
	assert.False(t, hasSelfService)
	...
	_, err = ds.InsertVPPAppWithTeam(ctx, &mobius.VPPApp{VPPAppTeam: mobius.VPPAppTeam{VPPAppID: mobius.VPPAppID{AdamID: "adam_vpp_2", Platform: platform}, SelfService: true}, Name: "vpp2", BundleIdentifier: "com.app.vpp2"}, nil)
	require.NoError(t, err)
	hasSelfService, err = ds.HasSelfServiceSoftwareInstallers(ctx, platform, nil)
	require.NoError(t, err)
	assert.True(t, hasSelfService)
	...
	_, err = ds.InsertVPPAppWithTeam(ctx, &mobius.VPPApp{VPPAppTeam: mobius.VPPAppTeam{VPPAppID: mobius.VPPAppID{AdamID: "adam_vpp_3", Platform: mobius.MacOSPlatform}, SelfService: true}, Name: "vpp3", BundleIdentifier: "com.app.vpp3"}, &team.ID)
	require.NoError(t, err)
	// Check darwin
...
func testDeleteSoftwareInstallers(t *testing.T, ds *Datastore) {
	...
	softwareInstallerID, _, err := ds.MatchOrCreateSoftwareInstaller(ctx, &mobius.UploadSoftwareInstallerPayload{
		InstallScript:   "install",
		InstallerFile:   tfr0,
		StorageID:       ins0,
...
func testDeletePendingSoftwareInstallsForPolicy(t *testing.T, ds *Datastore) {
	...
	policy1, err := ds.NewTeamPolicy(ctx, team1.ID, &user1.ID, mobius.PolicyPayload{
		Name:                "p1",
		Query:               "SELECT 1;",
	...
	policy2, err := ds.NewTeamPolicy(ctx, team1.ID, &user1.ID, mobius.PolicyPayload{
		Name:                "p2",
		Query:               "SELECT 2;",
	...
	executionID, err := ds.InsertSoftwareInstallRequest(ctx, host1.ID, installerID1, mobius.HostSoftwareInstallOptions{PolicyID: &policy1.ID})
	require.NoError(t, err)

	err = sqlx.GetContext(ctx, ds.reader(ctx), &count, hostSoftwareInstallsCount, mobius.SoftwareInstallPending, executionID)
	require.NoError(t, err)
	...
	executionID, err = ds.InsertSoftwareInstallRequest(ctx, host1.ID, installerID2, mobius.HostSoftwareInstallOptions{PolicyID: &policy2.ID})
	require.NoError(t, err)
	...
	executionID, err = ds.InsertSoftwareInstallRequest(ctx, host2.ID, installerID1, mobius.HostSoftwareInstallOptions{PolicyID: &policy1.ID})
	require.NoError(t, err)
...
func testGetHostLastInstallData(t *testing.T, ds *Datastore) {
	...
	softwareInstallerID1, _, err := ds.MatchOrCreateSoftwareInstaller(ctx, &mobius.UploadSoftwareInstallerPayload{
		InstallScript:   "install",
		InstallerFile:   tfr0,
		StorageID:       ins0,
		Filename:        "installer.pkg",
	...
	require.NoError(t, err)
	softwareInstallerID2, _, err := ds.MatchOrCreateSoftwareInstaller(ctx, &mobius.UploadSoftwareInstallerPayload{
		InstallScript:   "install2",
		InstallerFile:   tfr0,
		StorageID:       ins0,
	...
	installUUID1, err := ds.InsertSoftwareInstallRequest(ctx, host1.ID, softwareInstallerID1, mobius.HostSoftwareInstallOptions{})
	require.NoError(t, err)
	...
	require.Equal(t, installUUID1, host1LastInstall.ExecutionID)
	require.NotNil(t, host1LastInstall.Status)
	require.Equal(t, mobius.SoftwareInstalled, *host1LastInstall.Status)

	// Install installer2.pkg on host1.
	installUUID2, err := ds.InsertSoftwareInstallRequest(ctx, host1.ID, softwareInstallerID2, mobius.HostSoftwareInstallOptions{})
	require.NoError(t, err)
	...
	installUUID3, err := ds.InsertSoftwareInstallRequest(ctx, host1.ID, softwareInstallerID1, mobius.HostSoftwareInstallOptions{})
	require.NoError(t, err)
	...
	require.Equal(t, installUUID3, host1LastInstall.ExecutionID)
	require.NotNil(t, host1LastInstall.Status)
	require.Equal(t, mobius.SoftwareInstallFailed, *host1LastInstall.Status)
...
func testBatchSetSoftwareInstallersScopedViaLabels(t *testing.T, ds *Datastore) {
	...
	tm1, err := ds.NewTeam(ctx, &mobius.Team{Name: t.Name() + "1"})
	require.NoError(t, err)
	tm2, err := ds.NewTeam(ctx, &mobius.Team{Name: t.Name() + "2"})
	require.NoError(t, err)
	user := test.NewUser(t, ds, "Alice", "alice@example.com", true)

	// create some installer payloads to be used by test cases
	installers := make([]*mobius.UploadSoftwareInstallerPayload, 3)
	for i := range installers {
		file := bytes.NewReader([]byte("installer" + fmt.Sprint(i)))
		tfr, err := mobius.NewTempFileReader(file, t.TempDir)
		require.NoError(t, err)
		installers[i] = &mobius.UploadSoftwareInstallerPayload{
			InstallScript:   "install",
			InstallerFile:   tfr,
	...
	labels := make([]*mobius.Label, 4)
	for i := range labels {
		lbl, err := ds.NewLabel(ctx, &mobius.Label{Name: "label" + fmt.Sprint(i)})
		require.NoError(t, err)
		labels[i] = lbl
	}
	...
	cases := []struct {
		...
		},
		{
			...
			team: tm1,
			payload: []testPayload{
				{Installer: installers[0], Labels: []*mobius.Label{labels[0]}},
			},
		},
		{
			...
			payload: []testPayload{
				...
				{Installer: installers[1], Labels: []*mobius.Label{labels[1]}, Exclude: true},
			},
		},
		{
			...
			payload: []testPayload{
				{Installer: installers[0], ShouldCancelPending: ptr.Bool(false)},
				{Installer: installers[1], Labels: []*mobius.Label{labels[1]}, Exclude: false, ShouldCancelPending: ptr.Bool(true)},
			},
		},
		{
			...
			team: tm1,
			payload: []testPayload{
				{Installer: installers[0], Labels: []*mobius.Label{labels[0], labels[1]}, ShouldCancelPending: ptr.Bool(true)},
			},
		},
		{
			...
			team: tm1,
			payload: []testPayload{
				{Installer: installers[0], Labels: []*mobius.Label{labels[1]}, ShouldCancelPending: ptr.Bool(true)},
			},
		},
		{
			...
			team: tm1,
			payload: []testPayload{
				{Installer: installers[0], Labels: []*mobius.Label{labels[0], labels[2]}, ShouldCancelPending: ptr.Bool(true)},
			},
		},
		{
			...
			team: tm2,
			payload: []testPayload{
				{Installer: installers[0], Labels: []*mobius.Label{labels[0]}, Exclude: false},
				{Installer: installers[1], Labels: []*mobius.Label{labels[0], labels[1], labels[2]}, Exclude: true},
				{Installer: installers[2], Labels: []*mobius.Label{labels[1], labels[2]}, Exclude: false},
			},
		},
		{
			...
			team: tm1,
			payload: []testPayload{
				{Installer: installers[0], Labels: []*mobius.Label{labels[0], labels[2]}, ShouldCancelPending: ptr.Bool(false)},
		...
		},
		{
			...
			team: tm1,
			payload: []testPayload{
				{Installer: installers[0], Labels: []*mobius.Label{labels[1], labels[3]}, ShouldCancelPending: ptr.Bool(true)},
		...
		},
		{
			...
			payload: []testPayload{
				...
				{Installer: installers[2], Labels: []*mobius.Label{labels[1], labels[2]}, Exclude: false, ShouldCancelPending: ptr.Bool(false)},
			},
		},
		{
	...
	for _, c := range cases {
		...
		if len(c.payload) > 0 {
			...
			for i, payload := range c.payload {
				if payload.ShouldCancelPending != nil {
					...
					_, err = ds.InsertSoftwareInstallRequest(ctx, host.ID, swID, mobius.HostSoftwareInstallOptions{})
					require.NoError(t, err)
					installerIDs[i] = swID
				}
			}
		...
		payload := make([]*mobius.UploadSoftwareInstallerPayload, len(c.payload))
		for i, p := range c.payload {
			installer := *p.Installer
			installer.ValidatedLabels = &mobius.LabelIdentsWithScope{LabelScope: mobius.LabelScopeIncludeAny}
			if p.Exclude {
				installer.ValidatedLabels.LabelScope = mobius.LabelScopeExcludeAny
			}
			byName := make(map[string]mobius.LabelIdent, len(p.Labels))
			for _, lbl := range p.Labels {
				byName[lbl.Name] = mobius.LabelIdent{LabelName: lbl.Name, LabelID: lbl.ID}
			}
		...
		// expected ones.
		installersByFilename := make(map[string]*mobius.SoftwareInstaller, len(installers))
...
func testMatchOrCreateSoftwareInstallerWithAutomaticPolicies(t *testing.T, ds *Datastore) {
	...
	// Test Mac FMA
	fma, err := ds.UpsertMaintainedApp(ctx, &mobius.MaintainedApp{ID: 1})
	require.NoError(t, err)
	installerFMA, _, err := ds.MatchOrCreateSoftwareInstaller(ctx, &mobius.UploadSoftwareInstallerPayload{
		InstallerFile:         tfr1,
		BundleIdentifier:      "com.foo.fma",
		Platform:              "darwin",
		Extension:             "dmg",
		MobiusMaintainedAppID:  ptr.Uint(fma.ID),
		StorageID:             "storage1",
		Filename:              "foobar1",
		Title:                 "FooFMA",
		Version:               "1.0",
	...
	require.Equal(t, mobius.PolicyNoTeamID, *noTeamPolicies[0].TeamID)
...
Software won't be installed on Linux hosts with RPM-based distributions because this policy's query is written to always pass on these hosts.`, team2Policies[0].Description)
	...
	// Test rpm.
	installerID4, _, err := ds.MatchOrCreateSoftwareInstaller(ctx, &mobius.UploadSoftwareInstallerPayload{
		InstallerFile:    tfr1,
		Extension:        "rpm",
		StorageID:        "storage4",
		Filename:         "barzoo1",
...
Software won't be installed on Linux hosts with Debian-based distributions because this policy's query is written to always pass on these hosts.`, team2Policies[1].Description)
	...
	team3, err := ds.NewTeam(ctx, &mobius.Team{Name: "team 3"})
	require.NoError(t, err)

	_, err = ds.NewTeamPolicy(ctx, team3.ID, &user1.ID, mobius.PolicyPayload{
	...
	team3Policies, _, err := ds.ListTeamPolicies(ctx, team3.ID, mobius.ListOptions{}, mobius.ListOptions{})
	require.NoError(t, err)
...
func testGetDetailsForUninstallFromExecutionID(t *testing.T, ds *Datastore) {
	...
	installer1, _, err := ds.MatchOrCreateSoftwareInstaller(ctx, &mobius.UploadSoftwareInstallerPayload{
		InstallerFile:    tfr1,
	...
	installer2, _, err := ds.MatchOrCreateSoftwareInstaller(ctx, &mobius.UploadSoftwareInstallerPayload{
		InstallerFile:    tfr1,
	...
	req1, err := ds.InsertSoftwareInstallRequest(ctx, host.ID, installer1, mobius.HostSoftwareInstallOptions{})
	require.NoError(t, err)
	req2, err := ds.InsertSoftwareInstallRequest(ctx, host.ID, installer2, mobius.HostSoftwareInstallOptions{})
	require.NoError(t, err)
...
func testGetTeamsWithInstallerByHash(t *testing.T, ds *Datastore) {
	...
	err = ds.BatchSetSoftwareInstallers(ctx, nil, []*mobius.UploadSoftwareInstallerPayload{
		{
			InstallerFile:    tfr1,
	...
	err = ds.BatchSetSoftwareInstallers(ctx, &team1.ID, []*mobius.UploadSoftwareInstallerPayload{
		{
			InstallerFile:    tfr1,
			BundleIdentifier: "bid1",
			Extension:        "pkg",
...
func testEditDeleteSoftwareInstallersActivateNextActivity(t *testing.T, ds *Datastore) {
	...
	newInstallerFile := func(ident string) *mobius.TempFileReader {
		tfr, err := mobius.NewTempFileReader(strings.NewReader(ident), t.TempDir)
		require.NoError(t, err)
		return tfr
	}
	...
	host1Ins1, err := ds.InsertSoftwareInstallRequest(ctx, host1.ID, ins1.InstallerID, mobius.HostSoftwareInstallOptions{})
	require.NoError(t, err)
	host1Ins2, err := ds.InsertSoftwareInstallRequest(ctx, host1.ID, ins2.InstallerID, mobius.HostSoftwareInstallOptions{})
	require.NoError(t, err)
	// add a script exec as last activity for host1
	host1Script, err := ds.NewHostScriptExecutionRequest(ctx, &mobius.HostScriptRequestPayload{
		HostID: host1.ID, ScriptContents: "echo", UserID: &user.ID, SyncRequest: true,
	})
	require.NoError(t, err)
	host2Ins1, err := ds.InsertSoftwareInstallRequest(ctx, host2.ID, ins1.InstallerID, mobius.HostSoftwareInstallOptions{})
	require.NoError(t, err)
	host2Ins2, err := ds.InsertSoftwareInstallRequest(ctx, host2.ID, ins2.InstallerID, mobius.HostSoftwareInstallOptions{})
	require.NoError(t, err)
	// add a script exec as first activity for host3
	host3Script, err := ds.NewHostScriptExecutionRequest(ctx, &mobius.HostScriptRequestPayload{
		HostID: host3.ID, ScriptContents: "echo", UserID: &user.ID, SyncRequest: true,
	})
	require.NoError(t, err)
	host3Ins2, err := ds.InsertSoftwareInstallRequest(ctx, host3.ID, ins2.InstallerID, mobius.HostSoftwareInstallOptions{})
	require.NoError(t, err)
...
func testBatchSetSoftwareInstallersActivateNextActivity(t *testing.T, ds *Datastore) {
	...
	host1Ins2, err := ds.InsertSoftwareInstallRequest(ctx, host1.ID, ins2.InstallerID, mobius.HostSoftwareInstallOptions{})
	require.NoError(t, err)
	host1Ins3, err := ds.InsertSoftwareInstallRequest(ctx, host1.ID, ins3.InstallerID, mobius.HostSoftwareInstallOptions{})
	require.NoError(t, err)
	host2Ins2, err := ds.InsertSoftwareInstallRequest(ctx, host2.ID, ins2.InstallerID, mobius.HostSoftwareInstallOptions{})
	require.NoError(t, err)
	host2Ins1, err := ds.InsertSoftwareInstallRequest(ctx, host2.ID, ins1.InstallerID, mobius.HostSoftwareInstallOptions{})
	require.NoError(t, err)
	host2Ins3, err := ds.InsertSoftwareInstallRequest(ctx, host2.ID, ins3.InstallerID, mobius.HostSoftwareInstallOptions{})
	require.NoError(t, err)
	host3Ins3, err := ds.InsertSoftwareInstallRequest(ctx, host3.ID, ins3.InstallerID, mobius.HostSoftwareInstallOptions{})
	require.NoError(t, err)
	host3Ins2, err := ds.InsertSoftwareInstallRequest(ctx, host3.ID, ins2.InstallerID, mobius.HostSoftwareInstallOptions{})
	require.NoError(t, err)
	host3Ins1, err := ds.InsertSoftwareInstallRequest(ctx, host3.ID, ins1.InstallerID, mobius.HostSoftwareInstallOptions{})
	require.NoError(t, err)
	...
	require.NoError(t, err)
	host2Script, err := ds.NewHostScriptExecutionRequest(ctx, &mobius.HostScriptRequestPayload{
...
func testSaveInstallerUpdatesClearsMobiusMaintainedAppID(t *testing.T, ds *Datastore) {
	ctx := context.Background()
	user := test.NewUser(t, ds, "Test User", "test@example.com", true)
	tfr, err := mobius.NewTempFileReader(strings.NewReader("file contents"), t.TempDir)
	require.NoError(t, err)
	...
	// Create an installer with a non-NULL mobius_maintained_app_id
	installerID, _, err := ds.MatchOrCreateSoftwareInstaller(ctx, &mobius.UploadSoftwareInstallerPayload{
		Title:                "testpkg",
		Source:               "apps",
		InstallScript:        "echo install",
	...
	// Assert that mobius_maintained_app_id is now NULL
	var fmaID *uint
	err = sqlx.GetContext(ctx, ds.reader(ctx), &fmaID, `SELECT mobius_maintained_app_id FROM software_installers WHERE id = ?`, installerID)
	require.NoError(t, err)
	assert.Nil(t, fmaID, "mobius_maintained_app_id should be NULL after update")
}
