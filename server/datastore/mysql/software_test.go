...
func testSoftwareSaveHost(t *testing.T, ds *Datastore) {
	...
	software1 := []mobius.Software{
		{Name: "foo", Version: "0.0.1", Source: "chrome_extensions"},
		{Name: "foo", Version: "0.0.3", Source: "chrome_extensions"},
	}
	software2 := []mobius.Software{
	...
	getHostSoftware := func(h *mobius.Host) []mobius.Software {
...
func testSoftwareCPE(t *testing.T, ds *Datastore) {
	...
	q := mobius.SoftwareIterQueryOptions{ExcludedSources: oval.SupportedSoftwareSources}
...
func testSoftwareDifferentNameSameBundleIdentifier(t *testing.T, ds *Datastore) {
	...
	incoming := make(map[string]mobius.Software)
	sw, err := mobius.SoftwareFromOsqueryRow("GoLand.app", "2024.3", "apps", "", "", "", "", "com.jetbrains.goland", "", "", "")
	require.NoError(t, err)
	...
	currentSoftware, incomingChecksumToSoftware, incomingChecksumToTitle, existingBundleIDsToUpdate, err := ds.getExistingSoftware(
		ctx, make(map[string]mobius.Software), incoming,
	)
	require.NoError(t, err)
	...
	var softwareTitle []mobius.SoftwareTitle
	err = sqlx.SelectContext(ctx, ds.reader(ctx),
	...
	incoming = make(map[string]mobius.Software)
	sw, err = mobius.SoftwareFromOsqueryRow("GoLand 2.app", "2024.3", "apps", "", "", "", "", "com.jetbrains.goland", "", "", "")
	require.NoError(t, err)
	...
	sw, err = mobius.SoftwareFromOsqueryRow("GoLand 3.app", "2024.3", "apps", "", "", "", "", "com.jetbrains.goland", "", "", "")
	require.NoError(t, err)

	sw2, err := mobius.SoftwareFromOsqueryRow("SomeOther.app", "1.2.3", "apps", "", "", "", "", "com.some.other", "", "", "")
	require.NoError(t, err)

	_, err = ds.applyChangesForNewSoftwareDB(ctx, host2.ID, []mobius.Software{*sw, *sw2})
	require.NoError(t, err)
	...
	var goland mobius.SoftwareTitle
...
func testSoftwareDuplicateNameDifferentBundleIdentifier(t *testing.T, ds *Datastore) {
	...
	incoming := make(map[string]mobius.Software)
	sw, err := mobius.SoftwareFromOsqueryRow("a", "0.0.1", "chrome_extension", "", "", "", "", "bundle_id1", "", "", "")
	require.NoError(t, err)
	soft2Key := sw.ToUniqueStr()
	incoming[soft2Key] = *sw

	currentSoftware, incomingChecksumToSoftware, incomingChecksumToTitle, _, err := ds.getExistingSoftware(
		context.Background(), make(map[string]mobius.Software), incoming,
	)
	require.NoError(t, err)
	tx, err := ds.writer(context.Background()).Beginx()
	require.NoError(t, err)
	_, err = ds.insertNewInstalledHostSoftwareDB(
		context.Background(), tx, host1.ID, currentSoftware, incomingChecksumToSoftware, incomingChecksumToTitle, map[string]mobius.Software{},
	)
	require.NoError(t, err)
	require.NoError(t, tx.Commit())
	...
	incoming = make(map[string]mobius.Software)
	sw, err = mobius.SoftwareFromOsqueryRow("a", "0.0.1", "chrome_extension", "", "", "", "", "bundle_id2", "", "", "")
	require.NoError(t, err)
	soft3Key := sw.ToUniqueStr()
	incoming[soft3Key] = *sw
...
func testSoftwareHostDuplicates(t *testing.T, ds *Datastore) {
	...
	longName := strings.Repeat("a", mobius.SoftwareNameMaxLength+5)

	incoming := make(map[string]mobius.Software)
	sw, err := mobius.SoftwareFromOsqueryRow(longName+"b", "0.0.1", "chrome_extension", "", "", "", "", "", "", "", "")
	require.NoError(t, err)
	soft2Key := sw.ToUniqueStr()
	incoming[soft2Key] = *sw
	...
	require.NoError(t, err)
	_, err = ds.insertNewInstalledHostSoftwareDB(
		context.Background(), tx, host1.ID, incomingByChecksum, existingSoftware, existingTitlesForNewSoftware, map[string]mobius.Software{},
	)
	require.NoError(t, err)
	require.NoError(t, tx.Commit())
	...
	require.Equal(t, strings.Repeat("a", mobius.SoftwareNameMaxLength), software[0].Name)

	incoming = make(map[string]mobius.Software)
	sw, err = mobius.SoftwareFromOsqueryRow(longName+"c", "0.0.1", "chrome_extension", "", "", "", "", "", "", "", "")
	require.NoError(t, err)
	soft3Key := sw.ToUniqueStr()
	incoming[soft3Key] = *sw
	...
	require.Equal(t, strings.Repeat("a", mobius.SoftwareNameMaxLength), software2[0].Name)
...
func testSoftwareLoadVulnerabilities(t *testing.T, ds *Datastore) {
	...
	for _, v := range vulns {
		_, err = ds.InsertSoftwareVulnerability(context.Background(), v, mobius.NVDSource)
		require.NoError(t, err)
	}
	require.NoError(t, ds.LoadHostSoftware(context.Background(), host, false))
...
func testSoftwareNothingChanged(t *testing.T, ds *Datastore) {
	cases := []struct {
		desc     string
		current  []mobius.Software
		incoming []mobius.Software
		want     bool
	}{
		{"both nil", nil, nil, true},
		{"different len", nil, []mobius.Software{{}}, false},

		{
			"identical",
			[]mobius.Software{{Name: "A", Version: "1.0", Source: "ASD"}},
			[]mobius.Software{{Name: "A", Version: "1.0", Source: "ASD"}},
			true,
		},
		{
			"different version",
			[]mobius.Software{{Name: "A", Version: "1.1", Source: "ASD"}},
			[]mobius.Software{{Name: "A", Version: "1.0", Source: "ASD"}},
			false,
		},
		{
			"new software",
			[]mobius.Software{{Name: "A", Version: "1.0", Source: "ASD"}},
			[]mobius.Software{{Name: "A", Version: "1.0", Source: "ASD"}, {Name: "B", Version: "1.0", Source: "ASD"}},
			false,
		},
		{
			"removed software",
		...
		},
		{
			...
			[]mobius.Software{{Name: "A", Version: "1.0", Source: "ASD", LastOpenedAt: ptr.Time(time.Now())}},
			[]mobius.Software{{Name: "A", Version: "1.0", Source: "ASD", LastOpenedAt: ptr.Time(time.Now())}},
			true,
		},
		{
		...
		},
		{
			...
			[]mobius.Software{{Name: "A", Version: "1.0", Source: "ASD", LastOpenedAt: ptr.Time(time.Now().Add(-365 * 24 * time.Hour))}},
			[]mobius.Software{{Name: "A", Version: "1.0", Source: "ASD", LastOpenedAt: ptr.Time(time.Now())}},
			false,
		},
		{
			"identical but insignificantly changed last open",
			[]mobius.Software{{Name: "A", Version: "1.0", Source: "ASD", LastOpenedAt: ptr.Time(time.Now().Add(-time.Second))}},
			[]mobius.Software{{Name: "A", Version: "1.0", Source: "ASD", LastOpenedAt: ptr.Time(time.Now())}},
			true,
		},
		{
			...
			[]mobius.Software{{Name: "A", Version: "1.0", Source: "ASD"}},
			[]mobius.Software{{Name: "A", Version: "1.0", Source: "ASD"}, {Name: "A", Version: "1.0", Source: "ASD"}},
			true,
		},
		{
...
func generateCVEMeta(n int) mobius.CVEMeta {
...
func testSoftwareList(t *testing.T, ds *Datastore) {
	...
	foo002 := mobius.Software{Name: "foo", Version: "v0.0.2", Source: "chrome_extensions"}
	foo003 := mobius.Software{Name: "foo", Version: "0.0.3", Source: "chrome_extensions", GenerateCPE: "someothercpewithoutvulns"}
	bar003 := mobius.Software{Name: "bar", Version: "0.0.3", Source: "deb_packages"}
	baz001 := mobius.Software{
		Name:        "baz",
		Version:     "0.0.1",
		Source:      "deb_packages",
	...
	t.Run("lists everything", func(t *testing.T) {
		opts := mobius.SoftwareListOptions{
		...
		expected := []mobius.Software{bar003, baz001, foo001, foo002, foo003}
	...
	t.Run("filters by team", func(t *testing.T) {
		...
		expected := []mobius.Software{foo001, foo003}
		test.ElementsMatchSkipID(t, software, expected)
		...
		if targetSoftware.Name == "foo" && targetSoftware.Version == "0.0.3" {
			expected = []mobius.Software{foo003}
		}
	...
	t.Run("filters by no team (team 0)", func(t *testing.T) {
		...
		expected := []mobius.Software{bar003, baz001, foo002, foo003}
		test.ElementsMatchSkipID(t, software, expected)
	})
	...
	t.Run("filters vulnerable software", func(t *testing.T) {
		...
		software := listSoftwareCheckCount(t, ds, 2, 2, opts, true)
		expected := []mobius.Software{foo001, baz001}
		test.ElementsMatchSkipID(t, software, expected)
	})
	...
	t.Run("filters by query", func(t *testing.T) {
		...
		software := listSoftwareCheckCount(t, ds, 1, 1, opts, true)
		expected := []mobius.Software{bar003}
		test.ElementsMatchSkipID(t, software, expected)

		// query by version
		opts.ListOptions.MatchQuery = "0.0.3"
		software = listSoftwareCheckCount(t, ds, 2, 2, opts, true)
		expected = []mobius.Software{foo003, bar003}
		test.ElementsMatchSkipID(t, software, expected)

		// query by version (case insensitive)
		opts.ListOptions.MatchQuery = "V0.0.2"
		software = listSoftwareCheckCount(t, ds, 1, 1, opts, true)
		expected = []mobius.Software{foo002}
		test.ElementsMatchSkipID(t, software, expected)
	})
	...
	t.Run("order by hosts_count", func(t *testing.T) {
		software := listSoftwareCheckCount(t, ds, 5, 5, mobius.SoftwareListOptions{ListOptions: mobius.ListOptions{OrderKey: "hosts_count", OrderDirection: mobius.OrderDescending}, WithHostCounts: true}, false)
...
func listSoftwareCheckCount(t *testing.T, ds *Datastore, expectedListCount int, expectedFullCount int, opts mobius.SoftwareListOptions, returnSorted bool) []mobius.Software {
	t.Helper()
...
func testSoftwareSyncHostsSoftware(t *testing.T, ds *Datastore) {
	...
	cmpNameVersionCount := func(want, got []mobius.Software) {
		cmp := make([]mobius.Software, len(got))
		for i, sw := range got {
			cmp[i] = mobius.Software{Name: sw.Name, Version: sw.Version, HostsCount: sw.HostsCount}
		}
	...
	globalOpts := mobius.SoftwareListOptions{
		WithHostCounts: true, ListOptions: mobius.ListOptions{OrderKey: "hosts_count", OrderDirection: mobius.OrderDescending},
	}
	_ = listSoftwareCheckCount(t, ds, 0, 0, globalOpts, false)

	software0 := []mobius.Software{
	...
	softwareTemp := make([]mobius.Software, 0, 10)
	for i := 0; i < 10; i++ {
		softwareTemp = append(
			softwareTemp, mobius.Software{Name: fmt.Sprintf("foo%d", i), Version: fmt.Sprintf("%d.0.1", i), Source: "deb_packages"},
		)
	}
	...
	allSw := listSoftwareCheckCount(t, ds, 3, 3, mobius.SoftwareListOptions{}, false)
	want = []mobius.Software{
	...
	team1Opts := mobius.SoftwareListOptions{WithHostCounts: true, TeamID: ptr.Uint(team1.ID), ListOptions: mobius.ListOptions{OrderKey: "hosts_count", OrderDirection: mobius.OrderDescending}}
	...
	require.NoError(t, err)
	test.ElementsMatchSkipIDAndHostCount(t, software1, []mobius.Software{*soft1ByID, *soft2ByID})

	team2Opts := mobius.SoftwareListOptions{WithHostCounts: true, TeamID: ptr.Uint(team2.ID), ListOptions: mobius.ListOptions{OrderKey: "hosts_count", OrderDirection: mobius.OrderDescending}}
...
func insertVulnSoftwareForTest(t *testing.T, ds *Datastore) {
	...
	for _, s := range software1 {
		key := fmt.Sprintf("%s%s%s%s%s%s%s", fmt.Sprintf("/some/path/%s", s.Name), mobius.SoftwareFieldSeparator, "", mobius.SoftwareFieldSeparator, "", mobius.SoftwareFieldSeparator, s.ToUniqueStr())
		s1Paths[key] = struct{}{}
	}
	...
	inserted, err := ds.InsertSoftwareVulnerability(context.Background(), mobius.SoftwareVulnerability{
	...
	for _, v := range vulns {
		inserted, err := ds.InsertSoftwareVulnerability(context.Background(), v, mobius.NVDSource)
		require.NoError(t, err)
		require.True(t, inserted)
	}
...
func testDeleteSoftwareVulnerabilities(t *testing.T, ds *Datastore) {
	...
	err = ds.DeleteSoftwareVulnerabilities(ctx, []mobius.SoftwareVulnerability{
		{
...
func testHostsByCVE(t *testing.T, ds *Datastore) {
	...
	require.ElementsMatch(t, hosts, []mobius.HostVulnerabilitySummary{
		{
			ID:          1,
			Hostname:    "host1",
...
func testHostVulnSummariesBySoftwareIDs(t *testing.T, ds *Datastore) {
	...
	allSoftware, _, err := ds.ListSoftware(ctx, mobius.SoftwareListOptions{})
	require.NoError(t, err)

	var fooRpm mobius.Software
	var chrome3 mobius.Software
	var barRpm mobius.Software
...
func testUpdateHostSoftwareUpdatesSoftware(t *testing.T, ds *Datastore) {
	...
	cmpNameVersionCount := func(expected, got []mobius.Software) {
		cmp := make([]mobius.Software, len(got))
		for i, sw := range got {
			cmp[i] = mobius.Software{Name: sw.Name, Version: sw.Version, HostsCount: sw.HostsCount}
		}
	...
	opts := mobius.SoftwareListOptions{WithHostCounts: true}
...
func testUpdateHostSoftware(t *testing.T, ds *Datastore) {
	...
	genSortFn := func(sl []mobius.HostSoftwareEntry) func(l, r int) bool {
...
func testListSoftwareVulnerabilitiesByHostIDsSource(t *testing.T, ds *Datastore) {
	...
	for i, s := range host.Software {
		cve, ok := cveMap[i]
		if ok {
			inserted, err := ds.InsertSoftwareVulnerability(ctx, mobius.SoftwareVulnerability{
				SoftwareID: s.ID,
				CVE:        cve,
			}, mobius.NVDSource)
			require.NoError(t, err)
	...
	result, err := ds.ListSoftwareVulnerabilitiesByHostIDsSource(ctx, []uint{host.ID}, mobius.NVDSource)
	require.NoError(t, err)
...
func testInsertSoftwareVulnerability(t *testing.T, ds *Datastore) {
	...
	t.Run("no vulnerabilities to insert", func(t *testing.T) {
		inserted, err := ds.InsertSoftwareVulnerability(ctx, mobius.SoftwareVulnerability{}, mobius.UbuntuOVALSource)
		require.NoError(t, err)
	...
	t.Run("duplicated vulnerabilities", func(t *testing.T) {
		...
		_, err := ds.UpdateHostSoftware(ctx, host.ID, []mobius.Software{software})
		require.NoError(t, err)
		require.NoError(t, ds.LoadHostSoftware(ctx, host, false))
		cpes := []mobius.SoftwareCPE{
		...
		}, mobius.UbuntuOVALSource)
		require.NoError(t, err)
		require.True(t, inserted)

		// Sleep so that the updated_at timestamp is guaranteed to be updated.
		time.Sleep(1 * time.Second)
		insertedOrUpdated, err := ds.InsertSoftwareVulnerability(ctx, mobius.SoftwareVulnerability{
			SoftwareID: host.Software[0].ID, CVE: "cve-1",
		}, mobius.UbuntuOVALSource)
		require.NoError(t, err)
		...
		storedVulns, err := ds.ListSoftwareVulnerabilitiesByHostIDsSource(ctx, []uint{host.ID}, mobius.UbuntuOVALSource)
		require.NoError(t, err)
	...
	t.Run("a vulnerability already exists", func(t *testing.T) {
		...
		inserted, err := ds.InsertSoftwareVulnerability(ctx, vulns[0], mobius.UbuntuOVALSource)
		require.NoError(t, err)
		require.True(t, inserted)

		// Sleep so that the updated_at timestamp is guaranteed to be updated.
		time.Sleep(1 * time.Second)
		insertedOrUpdated, err := ds.InsertSoftwareVulnerability(ctx, vulns[0], mobius.UbuntuOVALSource)
		require.NoError(t, err)
		// This will always return true because we always update the timestamp
		assert.True(t, insertedOrUpdated)
	...
	t.Run("vulnerability includes version range", func(t *testing.T) {
		...
		inserted, err := ds.InsertSoftwareVulnerability(ctx, vuln, mobius.UbuntuOVALSource)
		require.NoError(t, err)
		require.True(t, inserted)
...
func testListSoftwareForVulnDetection(t *testing.T, ds *Datastore) {
	t.Run("returns software without CPE entries", func(t *testing.T) {
		...
		require.NoError(t, err)
		require.NoError(t, ds.LoadHostSoftware(ctx, host, false))
		_, err = ds.UpsertSoftwareCPEs(ctx, []mobius.SoftwareCPE{{SoftwareID: host.Software[0].ID, CPE: "cpe1"}})
		require.NoError(t, err)
		// Load software again so that CPE data is included.
		require.NoError(t, ds.LoadHostSoftware(ctx, host, false))

		filter := mobius.VulnSoftwareFilter{HostID: &host.ID}
		...
		// test name filter
		filter = mobius.VulnSoftwareFilter{Name: "fo"} // LIKE match
		result, err = ds.ListSoftwareForVulnDetection(ctx, filter)
		require.NoError(t, err)
		...
		// test source filter
		filter = mobius.VulnSoftwareFilter{Source: "deb_packages"}
		result, err = ds.ListSoftwareForVulnDetection(ctx, filter)
...
func testSoftwareByIDIncludesCVEPublishedDate(t *testing.T, ds *Datastore) {
	t.Run("software.vulnerabilities includes the published date", func(t *testing.T) {
		...
		for _, t := range testCases {
			software = append(software, mobius.Software{
				Name:             t.name,
				Version:          "0.0.1",
				Source:           "apps",
		...
		for _, tC := range testCases {
			...
			if tC.hasMeta {
				...
				meta = append(meta, mobius.CVEMeta{
					CVE:              fmt.Sprintf("cve-%s", tC.name),
...
func testAllSoftwareIterator(t *testing.T, ds *Datastore) {
	...
	foo_ce_v1 := slices.IndexFunc(host.Software, func(c mobius.HostSoftwareEntry) bool {
		return c.Name == "foo" && c.Version == "0.0.1" && c.Source == "chrome_extensions"
	})
	foo_app_v2 := slices.IndexFunc(host.Software, func(c mobius.HostSoftwareEntry) bool {
		return c.Name == "foo" && c.Version == "v0.0.2" && c.Source == "apps"
	})
	bar_v3 := slices.IndexFunc(host.Software, func(c mobius.HostSoftwareEntry) bool {
	...
	testCases := []struct {
		name     string
		q        mobius.SoftwareIterQueryOptions
		expected []mobius.Software
	}{
		{
			...
			q: mobius.SoftwareIterQueryOptions{IncludedSources: []string{"apps"}},
		},
		{
			...
			q: mobius.SoftwareIterQueryOptions{ExcludedSources: []string{"apps"}},
		},
		{
			name: "no filter",
			expected: []mobius.Software{
		...
		},
		{
			...
			q: mobius.SoftwareIterQueryOptions{NameMatch: `ba[r|f]`, IncludedSources: []string{"deb_packages"}},
		},
		{
			...
			q: mobius.SoftwareIterQueryOptions{NameMatch: "foo\\.*", IncludedSources: []string{"chrome_extensions"}},
		},
		{
			...
			q: mobius.SoftwareIterQueryOptions{NameMatch: "foo\\.*", NameExclude: "bar$", IncludedSources: []string{"chrome_extensions"}},
		},
	}

	for _, tC := range testCases {
		t.Run(tC.name, func(t *testing.T) {
			var actual []mobius.Software
...
func testSoftwareIteratorForLinuxKernelCustomImages(t *testing.T, ds *Datastore) {
	...
	opts := mobius.SoftwareIterQueryOptions{
...
func testDeleteOutOfDateVulnerabilities(t *testing.T, ds *Datastore) {
	...
	inserted, err := ds.InsertSoftwareVulnerability(ctx, vulns[0], mobius.NVDSource)
	require.NoError(t, err)
	require.True(t, inserted)

	inserted, err = ds.InsertSoftwareVulnerability(ctx, vulns[1], mobius.NVDSource)
	require.NoError(t, err)
	require.True(t, inserted)
	...
	insertedOrUpdated, err := ds.InsertSoftwareVulnerability(ctx, vulns[0], mobius.NVDSource)
	require.NoError(t, err)
	assert.True(t, insertedOrUpdated)

	err = ds.DeleteOutOfDateVulnerabilities(ctx, mobius.NVDSource, 2*time.Hour)
	require.NoError(t, err)
...
func testDeleteSoftwareCPEs(t *testing.T, ds *Datastore) {
	...
	t.Run("with invalid software id", func(t *testing.T) {
		toDelete := []mobius.SoftwareCPE{cpes[0], {
...
func testHostSoftwareInstalledPathsDelta(t *testing.T, ds *Datastore) {
	...
	t.Run("nothing reported from osquery", func(t *testing.T) {
		var stored []mobius.HostSoftwareInstalledPath
		for i, s := range software {
			...
			stored = append(stored, mobius.HostSoftwareInstalledPath{
				ID:               uint(i),
				HostID:           host.ID,
				SoftwareID:       s.ID,
	...
	t.Run("host has no software but some paths were reported", func(t *testing.T) {
		...
		reported[fmt.Sprintf("/some/path/%d%s%s%s%s", software[0].ID, mobius.SoftwareFieldSeparator, "", mobius.SoftwareFieldSeparator, software[0].ToUniqueStr())] = struct{}{}
		reported[fmt.Sprintf("/some/path/%d%s%s%s%s", software[1].ID+1, mobius.SoftwareFieldSeparator, "", mobius.SoftwareFieldSeparator, software[1].ToUniqueStr())] = struct{}{}
		reported[fmt.Sprintf("/some/path/%d%s%s%s%s", software[2].ID, mobius.SoftwareFieldSeparator, "", mobius.SoftwareFieldSeparator, software[2].ToUniqueStr())] = struct{}{}
	...
	t.Run("we have some deltas", func(t *testing.T) {
		...
		getKey := func(s mobius.Software, change uint) string {
			var key string
			...
			case 3:
				key = fmt.Sprintf(
					"%s%d%s%s%s%s%s%s",
					"/some/path/", s.ID+change, mobius.SoftwareFieldSeparator, "corp1", mobius.SoftwareFieldSeparator, hash1, mobius.SoftwareFieldSeparator, s.ToUniqueStr(),
				)
			case 5:
				key = fmt.Sprintf(
					"%s%d%s%s%s%s%s%s",
					"/some/path/", s.ID+change, mobius.SoftwareFieldSeparator, "corp1", mobius.SoftwareFieldSeparator, hash2, mobius.SoftwareFieldSeparator, s.ToUniqueStr(),
				)
			default:
				key = fmt.Sprintf(
					"%s%d%s%s%s%s%s%s",
					"/some/path/", s.ID+change, mobius.SoftwareFieldSeparator, "corp1", mobius.SoftwareFieldSeparator, "", mobius.SoftwareFieldSeparator, s.ToUniqueStr(),
				)
			}
...
func testDeleteHostSoftwareInstalledPaths(t *testing.T, ds *Datastore) {
	...
	host1 := mobius.Host{ID: 1}
	host2 := mobius.Host{ID: 2}
	...
	var actual []mobius.HostSoftwareInstalledPath
	...
	expected := []mobius.HostSoftwareInstalledPath{
		{
			HostID:        host1.ID,
...
func testInsertHostSoftwareInstalledPaths(t *testing.T, ds *Datastore) {
	...
	toInsert := []mobius.HostSoftwareInstalledPath{
		{
			HostID:           1,
			SoftwareID:       1,
			InstalledPath:    "1",
...
func TestReconcileSoftwareTitles(t *testing.T) {
	...
	expectedTitlesByNSB := map[string]mobius.SoftwareTitle{}
	for _, s := range expectedSoftware {
		expectedTitlesByNSB[s.Name+s.Source+s.Browser] = mobius.SoftwareTitle{
			Name:    s.Name,
			Source:  s.Source,
			Browser: s.Browser,
		}
	...
	software1 := []mobius.Software{expectedSoftware[0], expectedSoftware[2]}
	software2 := []mobius.Software{expectedSoftware[1], expectedSoftware[2], expectedSoftware[3]}
	software3 := []mobius.Software{expectedSoftware[4]}
	...
	getSoftware := func() ([]mobius.Software, error) {
		var sw []mobius.Software
	...
	getTitles := func() ([]mobius.SoftwareTitle, error) {
		var swt []mobius.SoftwareTitle
	...
	assertSoftware := func(t *testing.T, wantSoftware []mobius.Software) {
		gotSoftware, err := getSoftware()
		require.NoError(t, err)
		require.Len(t, gotSoftware, len(wantSoftware))

		byNSBV := map[string]mobius.Software{}
	...
	assertTitles := func(t *testing.T, gotTitles []mobius.SoftwareTitle, expectMissing []string) {
	...
	require.NoError(t, ds.SyncHostsSoftware(context.Background(), time.Now()))
	assertSoftware(t, []mobius.Software{expectedSoftware[0], expectedSoftware[1], expectedSoftware[2], expectedSoftware[4]})
	...
	// add bar to host 3
	_, err = ds.UpdateHostSoftware(context.Background(), host3.ID, []mobius.Software{expectedSoftware[3], expectedSoftware[4]})
	require.NoError(t, err)
	require.NoError(t, ds.SyncHostsSoftware(context.Background(), time.Now()))
	gotTitles, err = getTitles()
	require.NoError(t, err)
	...
	expectedSoftware = append(expectedSoftware, mobius.Software{Name: "foo", Version: "0.0.4", Source: "chrome_extensions"})
	...
	expectedSoftware = append(expectedSoftware, mobius.Software{Name: "foo", Version: "0.0.4", Source: "rpm_packages"})
	_, err = ds.UpdateHostSoftware(ctx, host3.ID, expectedSoftware[3:])
	require.NoError(t, err)
...
func testListHostSoftware(t *testing.T, ds *Datastore) {
	...
	opts := mobius.HostSoftwareTitleListOptions{ListOptions: mobius.ListOptions{PerPage: 11, IncludeMetadata: true, OrderKey: "name", TestSecondaryOrderKey: "source"}}
	...
	expectStatus := func(s mobius.SoftwareInstallerStatus) *mobius.SoftwareInstallerStatus {
		return &s
	}
	...
	require.NoError(t, err)
	require.Empty(t, sw)
	require.Equal(t, &mobius.PaginationMetadata{}, meta)
	...
	require.NoError(t, err)
	assert.Empty(t, sw)
	assert.Equal(t, &mobius.PaginationMetadata{}, meta)
	...
	byNSV := map[string]mobius.Software{}
	for _, s := range software {
	...
	otherSoftwareByNSV := map[string]mobius.Software{}
	...
	for _, v := range vulns {
		_, err = ds.InsertSoftwareVulnerability(ctx, v, mobius.NVDSource)
		require.NoError(t, err)
	}
	...
	for _, s := range software {
		...
		key := fmt.Sprintf("%s%s%s%s%s%s%s", path, mobius.SoftwareFieldSeparator, "", mobius.SoftwareFieldSeparator, "", mobius.SoftwareFieldSeparator, s.ToUniqueStr())
		swPaths[key] = struct{}{}
	...
	expected := map[string]mobius.HostSoftwareWithInstaller{
		byNSV[a1].Name + byNSV[a1].Source: {Name: byNSV[a1].Name, Source: byNSV[a1].Source, InstalledVersions: []*mobius.HostSoftwareInstalledVersion{
		...
		byNSV[a2].Name + byNSV[a2].Source: {Name: byNSV[a2].Name, Source: byNSV[a2].Source, InstalledVersions: []*mobius.HostSoftwareInstalledVersion{
			{Version: byNSV[a2].Version, InstalledPaths: []string{installPaths[1]}},
		}},
		byNSV[b].Name + byNSV[b].Source: {Name: byNSV[b].Name, Source: byNSV[b].Source, InstalledVersions: []*mobius.HostSoftwareInstalledVersion{
		...
		byNSV[c1].Name + byNSV[c1].Source: {Name: byNSV[c1].Name, Source: byNSV[c1].Source, InstalledVersions: []*mobius.HostSoftwareInstalledVersion{
		...
		byNSV[d].Name + byNSV[d].Source: {Name: byNSV[d].Name, Source: byNSV[d].Source, InstalledVersions: []*mobius.HostSoftwareInstalledVersion{
			{Version: byNSV[d].Version, InstalledPaths: []string{installPaths[5]}},
		}},
		byNSV[e2].Name + byNSV[e2].Source: {Name: byNSV[e2].Name, Source: byNSV[e2].Source, InstalledVersions: []*mobius.HostSoftwareInstalledVersion{
			{Version: byNSV[e2].Version, InstalledPaths: []string{installPaths[6]}},
		}},
	}

	compareResults := func(expected map[string]mobius.HostSoftwareWithInstaller, got []*mobius.HostSoftwareWithInstaller, expectAsc bool, expectOmitted ...string) {
		...
		for _, g := range got {
			...
			if len(e.InstalledVersions) > 0 {
				byVers := make(map[string]mobius.HostSoftwareInstalledVersion, len(e.InstalledVersions))
	...
	sw, meta, err = ds.ListHostSoftware(ctx, host, opts)
	require.NoError(t, err)
	require.Equal(t, &mobius.PaginationMetadata{TotalResults: 6}, meta)
	compareResults(expected, sw, true)

	opts.VulnerableOnly = true
	sw, meta, err = ds.ListHostSoftware(ctx, host, opts)
	require.NoError(t, err)
	require.Equal(t, &mobius.PaginationMetadata{TotalResults: 2}, meta)
	...
	// create some Mobius installers and map them to a software title,
	// including one for a team
	tm, err := ds.NewTeam(ctx, &mobius.Team{Name: "team1"})
	require.NoError(t, err)
	...
	ExecAdhocSQL(t, ds, func(q sqlx.ExtContext) error {
		...
		hostSwi1InstallUUID, err = ds.InsertSoftwareInstallRequest(ctx, host.ID, swi1Pending, mobius.HostSoftwareInstallOptions{})
		if err != nil {
			return err
		}

		// swi2 is installed successfully
		hostSwi2InstallUUID, err = ds.InsertSoftwareInstallRequest(ctx, host.ID, swi2Installed, mobius.HostSoftwareInstallOptions{})
		if err != nil {
			return err
		}
		...
		hostSwi3InstallUUID, err = ds.InsertSoftwareInstallRequest(ctx, host.ID, swi3Failed, mobius.HostSoftwareInstallOptions{})
		if err != nil {
			return err
		}
		...
		otherHostI1UUID, err = ds.InsertSoftwareInstallRequest(ctx, otherHost.ID, swi3Failed, mobius.HostSoftwareInstallOptions{})
		if err != nil {
			return err
		}

		// swi4 is available (no install request), but add a pending request on the other host
		otherHostI2UUID, err = ds.InsertSoftwareInstallRequest(ctx, otherHost.ID, swi4Available, mobius.HostSoftwareInstallOptions{})
		if err != nil {
			return err
		}
		...
		hostSwi6InstallUUID, err = ds.InsertSoftwareInstallRequest(ctx, host.ID, swi6PendingUninstall, mobius.HostSoftwareInstallOptions{})
		if err != nil {
			return err
		}
	...
	expected[byNSV[b].Name+byNSV[b].Source] = mobius.HostSoftwareWithInstaller{
		Name:            "b",
		Source:          "apps",
		Status:          expectStatus(mobius.SoftwareInstallPending),
		SoftwarePackage: &mobius.SoftwarePackageOrApp{Name: "installer-0.pkg", Version: "v0.0.0", Platform: "darwin", SelfService: ptr.Bool(true), LastInstall: &mobius.HostSoftwareInstall{InstallUUID: hostSwi1InstallUUID}},
		InstalledVersions: []*mobius.HostSoftwareInstalledVersion{
			{Version: byNSV[b].Version, Vulnerabilities: []string{vulns[3].CVE}, InstalledPaths: []string{installPaths[2]}},
		},
	}
	i0 := mobius.HostSoftwareWithInstaller{
		Name:            "i0",
		Source:          "apps",
		Status:          expectStatus(mobius.SoftwareInstalled),
		SoftwarePackage: &mobius.SoftwarePackageOrApp{Name: "installer-1.pkg", Version: "v1.0.0", Platform: "darwin", SelfService: ptr.Bool(true), LastInstall: &mobius.HostSoftwareInstall{InstallUUID: hostSwi2InstallUUID}},
	}
	expected[i0.Name+i0.Source] = i0

	i1 := mobius.HostSoftwareWithInstaller{
		Name:            "i1",
		Source:          "apps",
		Status:          expectStatus(mobius.SoftwareInstallFailed),
		SoftwarePackage: &mobius.SoftwarePackageOrApp{Name: "installer-2.pkg", Version: "v2.0.0", Platform: "darwin", SelfService: ptr.Bool(false), LastInstall: &mobius.HostSoftwareInstall{InstallUUID: hostSwi3InstallUUID}},
	}
	expected[i1.Name+i1.Source] = i1

	i4 := mobius.HostSoftwareWithInstaller{
		Name:   "i4",
		Source: "apps",
		Status: expectStatus(mobius.SoftwareUninstallPending),
		SoftwarePackage: &mobius.SoftwarePackageOrApp{
			...
			LastInstall:   &mobius.HostSoftwareInstall{InstallUUID: hostSwi6InstallUUID},
			LastUninstall: &mobius.HostSoftwareUninstall{ExecutionID: hostSwi6UninstallUUID},
		},
	}
	expected[i4.Name+i4.Source] = i4

	i5 := mobius.HostSoftwareWithInstaller{
		Name:   "i5",
		Source: "apps",
		Status: expectStatus(mobius.SoftwareUninstallFailed),
		SoftwarePackage: &mobius.SoftwarePackageOrApp{
			Name: "installer-6.pkg", Version: "v6.0.0", Platform: "darwin", SelfService: ptr.Bool(false),
			LastUninstall: &mobius.HostSoftwareUninstall{ExecutionID: hostSwi7UninstallUUID},
		},
	}
	expected[i5.Name+i5.Source] = i5

	i6 := mobius.HostSoftwareWithInstaller{
		Name:   "i6",
		Source: "apps",
		Status: nil,
		SoftwarePackage: &mobius.SoftwarePackageOrApp{
			Name: "installer-7.pkg", Version: "v7.0.0", Platform: "darwin", SelfService: ptr.Bool(false),
			LastUninstall: &mobius.HostSoftwareUninstall{ExecutionID: hostSwi8UninstallUUID},
		},
	}
	...
	sw, meta, err = ds.ListHostSoftware(ctx, host, opts)
	require.NoError(t, err)
	require.Equal(t, &mobius.PaginationMetadata{TotalResults: uint(len(expected))}, meta)
	compareResults(expected, sw, true)

	// request with available software
	i2 := mobius.HostSoftwareWithInstaller{
		Name:            "i2",
		Source:          "apps",
		Status:          nil,
		SoftwarePackage: &mobius.SoftwarePackageOrApp{Name: "installer-3.pkg", Version: "v3.0.0", Platform: "darwin", SelfService: ptr.Bool(false)},
	}
	expected[i2.Name+i2.Source] = i2

	i3 := mobius.HostSoftwareWithInstaller{
		Name:            "i3",
		Source:          "apps",
		Status:          nil,
		SoftwarePackage: &mobius.SoftwarePackageOrApp{Name: "installer-4.pkg", Version: "v4.0.0", Platform: "darwin", SelfService: ptr.Bool(false)},
	}
	...
	sw, meta, err = ds.ListHostSoftware(ctx, host, opts)
	require.NoError(t, err)
	require.Equal(t, &mobius.PaginationMetadata{TotalResults: uint(len(expected)) - 1}, meta)
	...
	expectedAvailableOnly := map[string]mobius.HostSoftwareWithInstaller{}
	...
	sw, meta, err = ds.ListHostSoftware(ctx, host, opts)
	require.NoError(t, err)
	assert.Equal(t, &mobius.PaginationMetadata{TotalResults: uint(len(expectedAvailableOnly))}, meta)
	compareResults(expectedAvailableOnly, sw, true)
	opts.OnlyAvailableForInstall = false

	// request in descending order
	opts.ListOptions.OrderDirection = mobius.OrderDescending
	opts.ListOptions.TestSecondaryOrderDirection = mobius.OrderDescending
	opts.IncludeAvailableForInstall = false
	sw, meta, err = ds.ListHostSoftware(ctx, host, opts)
	require.NoError(t, err)
	require.Equal(t, &mobius.PaginationMetadata{TotalResults: uint(len(expected)) - 2}, meta)
	compareResults(expected, sw, false, i2.Name+i2.Source, i3.Name+i3.Source)
	opts.ListOptions.OrderDirection = mobius.OrderAscending
	opts.ListOptions.TestSecondaryOrderDirection = mobius.OrderAscending
	...
	hostSwi3PendingInstallUUID, err := ds.InsertSoftwareInstallRequest(ctx, host.ID, swi3Failed, mobius.HostSoftwareInstallOptions{})
	require.NoError(t, err)
	...
	expected[i1.Name+i1.Source] = mobius.HostSoftwareWithInstaller{
		Name:            "i1",
		Source:          "apps",
		Status:          expectStatus(mobius.SoftwareInstallPending),
		SoftwarePackage: &mobius.SoftwarePackageOrApp{Name: "installer-2.pkg", Version: "v2.0.0", Platform: "darwin", SelfService: ptr.Bool(false), LastInstall: &mobius.HostSoftwareInstall{InstallUUID: hostSwi3PendingInstallUUID}},
	}
	expectedAvailableOnly[byNSV[b].Name+byNSV[b].Source] = expected[byNSV[b].Name+byNSV[b].Source]
	...
	sw, meta, err = ds.ListHostSoftware(ctx, tmHost, opts)
	require.NoError(t, err)
	require.Equal(t, &mobius.PaginationMetadata{TotalResults: 1}, meta)
	compareResults(map[string]mobius.HostSoftwareWithInstaller{
	...
	va1, err := ds.InsertVPPAppWithTeam(ctx,
		&mobius.VPPApp{
			VPPAppTeam: mobius.VPPAppTeam{VPPAppID: mobius.VPPAppID{AdamID: "adam_vpp_1", Platform: mobius.MacOSPlatform}}, Name: "vpp1",
			BundleIdentifier: "com.app.vpp1",
		}, nil)
	require.NoError(t, err)
	_, err = ds.InsertVPPAppWithTeam(ctx,
		&mobius.VPPApp{
			VPPAppTeam: mobius.VPPAppTeam{VPPAppID: mobius.VPPAppID{AdamID: "adam_vpp_1", Platform: mobius.IOSPlatform}}, Name: "vpp1",
			BundleIdentifier: "com.app.vpp1",
		}, nil)
	require.NoError(t, err)
	_, err = ds.InsertVPPAppWithTeam(ctx,
	...
	va2, err := ds.InsertVPPAppWithTeam(ctx,
		&mobius.VPPApp{
			VPPAppTeam: mobius.VPPAppTeam{VPPAppID: mobius.VPPAppID{AdamID: "adam_vpp_2", Platform: mobius.MacOSPlatform}}, Name: "vpp2",
	...
	va3, err := ds.InsertVPPAppWithTeam(ctx,
		&mobius.VPPApp{
			VPPAppTeam: mobius.VPPAppTeam{VPPAppID: mobius.VPPAppID{AdamID: "adam_vpp_3", Platform: mobius.MacOSPlatform}, SelfService: true}, Name: "vpp3",
	...
	require.Equal(t, ds.testActivateSpecificNextActivities, activated)
	ds.testActivateSpecificNextActivities = []string{"-"}
	createVPPAppInstallResult(t, ds, host, vpp1CmdUUID, mobius.MDMAppleStatusAcknowledged)
	createVPPAppInstallResult(t, ds, host, vpp2CmdUUID, mobius.MDMAppleStatusError)
	...
	expected["vpp1apps"] = mobius.HostSoftwareWithInstaller{
		Name:        "vpp1",
		Source:      "apps",
		Status:      expectStatus(mobius.SoftwareInstalled),
		AppStoreApp: &mobius.SoftwarePackageOrApp{AppStoreID: vpp1, Platform: "darwin", SelfService: ptr.Bool(false), LastInstall: &mobius.HostSoftwareInstall{CommandUUID: vpp1CmdUUID}},
	}
	expected["vpp2apps"] = mobius.HostSoftwareWithInstaller{
		Name:        "vpp2",
		Source:      "apps",
		Status:      expectStatus(mobius.SoftwareInstallPending),
		AppStoreApp: &mobius.SoftwarePackageOrApp{AppStoreID: vpp2, Platform: "darwin", SelfService: ptr.Bool(false), LastInstall: &mobius.HostSoftwareInstall{CommandUUID: vpp2bCmdUUID}},
	}
	...
	expected["vpp3apps"] = mobius.HostSoftwareWithInstaller{
		Name:        "vpp3",
		Source:      "apps",
		Status:      nil,
		AppStoreApp: &mobius.SoftwarePackageOrApp{AppStoreID: vpp3, Platform: "darwin", SelfService: ptr.Bool(true)},
	}
	...
	sw, meta, err = ds.ListHostSoftware(ctx, host, opts)
	require.NoError(t, err)
	require.Equal(t, &mobius.PaginationMetadata{TotalResults: uint(len(expectedAvailableOnly) - 1)}, meta)
	...
	compareResults(map[string]mobius.HostSoftwareWithInstaller{
		...
		"vpp1apps": {
			...
			Source:      "apps",
			Status:      expectStatus(mobius.SoftwareInstallPending),
			AppStoreApp: &mobius.SoftwarePackageOrApp{AppStoreID: vpp1, Platform: "darwin", SelfService: ptr.Bool(false), LastInstall: &mobius.HostSoftwareInstall{CommandUUID: vpp1TmCmdUUID}},
		},
	}, sw, true)
	...
	require.NoError(t, err)
	require.Equal(t, &mobius.PaginationMetadata{TotalResults: 5}, meta)

	expectedOther := map[string]mobius.HostSoftwareWithInstaller{
		otherSoftware[0].Name + otherSoftware[0].Source: {Name: otherSoftware[0].Name, Source: otherSoftware[0].Source, InstalledVersions: []*mobius.HostSoftwareInstalledVersion{
			{Version: otherSoftware[0].Version},
		}},
		otherSoftware[1].Name + otherSoftware[1].Source: {Name: otherSoftware[1].Name, Source: otherSoftware[1].Source, InstalledVersions: []*mobius.HostSoftwareInstalledVersion{
			{Version: otherSoftware[1].Version},
		}},
		otherSoftware[2].Name + otherSoftware[2].Source: {Name: otherSoftware[2].Name, Source: otherSoftware[2].Source, InstalledVersions: []*mobius.HostSoftwareInstalledVersion{
		...
		"i1apps": {
			...
			Source:          "apps",
			Status:          expectStatus(mobius.SoftwareInstallPending),
			SoftwarePackage: &mobius.SoftwarePackageOrApp{Name: "installer-2.pkg", Version: "v2.0.0", Platform: "darwin", SelfService: ptr.Bool(false), LastInstall: &mobius.HostSoftwareInstall{InstallUUID: otherHostI1UUID}},
		},
		"i2apps": {
			Name:            "i2",
			Source:          "apps",
			Status:          expectStatus(mobius.SoftwareInstallPending),
			SoftwarePackage: &mobius.SoftwarePackageOrApp{Name: "installer-3.pkg", Version: "v3.0.0", Platform: "darwin", SelfService: ptr.Bool(false), LastInstall: &mobius.HostSoftwareInstall{InstallUUID: otherHostI2UUID}},
		},
	}
	...
	cases := []struct {
		name      string
		opts      mobius.HostSoftwareTitleListOptions
		wantNames []string
		wantMeta  *mobius.PaginationMetadata
	}{
		{
			name:      "No available for install software, page 0",
			opts:      mobius.HostSoftwareTitleListOptions{ListOptions: mobius.ListOptions{PerPage: 5}, IncludeAvailableForInstall: false},
			wantNames: []string{byNSV[a1].Name, byNSV[a2].Name, byNSV[b].Name, byNSV[c1].Name, byNSV[d].Name},
			wantMeta:  &mobius.PaginationMetadata{HasNextResults: true, HasPreviousResults: false, TotalResults: 13},
		},
		{
			name:      "No available for install software, page 1",
			opts:      mobius.HostSoftwareTitleListOptions{ListOptions: mobius.ListOptions{Page: 1, PerPage: 5}, IncludeAvailableForInstall: false},
			wantNames: []string{byNSV[e2].Name, i0.Name, i1.Name, i4.Name, i5.Name},
			wantMeta:  &mobius.PaginationMetadata{HasNextResults: true, HasPreviousResults: true, TotalResults: 13},
		},
		{
			name:      "No available for install software, page 2",
			opts:      mobius.HostSoftwareTitleListOptions{ListOptions: mobius.ListOptions{Page: 2, PerPage: 5}, IncludeAvailableForInstall: false},
			wantNames: []string{i6.Name, "vpp1", "vpp2"},
			wantMeta:  &mobius.PaginationMetadata{HasNextResults: false, HasPreviousResults: true, TotalResults: 13},
		},
		{
			name:      "No available for install software, page 3",
			opts:      mobius.HostSoftwareTitleListOptions{ListOptions: mobius.ListOptions{Page: 3, PerPage: 5}, IncludeAvailableForInstall: false},
			wantNames: []string{},
			wantMeta:  &mobius.PaginationMetadata{HasNextResults: false, HasPreviousResults: true, TotalResults: 13},
		},
		{
			name:      "Include Available for install software with MDM on, page 0",
			opts:      mobius.HostSoftwareTitleListOptions{ListOptions: mobius.ListOptions{PerPage: 5}, IncludeAvailableForInstall: true, IsMDMEnrolled: true},
			wantNames: []string{byNSV[a1].Name, byNSV[a2].Name, byNSV[b].Name, byNSV[c1].Name, byNSV[d].Name},
			wantMeta:  &mobius.PaginationMetadata{HasNextResults: true, HasPreviousResults: false, TotalResults: 15},
		},
		{
			name:      "Include Available for install software with MDM on, page 1",
			opts:      mobius.HostSoftwareTitleListOptions{ListOptions: mobius.ListOptions{Page: 1, PerPage: 5}, IncludeAvailableForInstall: true, IsMDMEnrolled: true},
			wantNames: []string{byNSV[e2].Name, i0.Name, i1.Name, i2.Name, i4.Name},
			wantMeta:  &mobius.PaginationMetadata{HasNextResults: true, HasPreviousResults: true, TotalResults: 15},
		},
		{
			name: "Include Available for install software with MDM on, page 2",
			opts: mobius.HostSoftwareTitleListOptions{
				ListOptions:                mobius.ListOptions{Page: 2, PerPage: 5},
				IncludeAvailableForInstall: true,
				IsMDMEnrolled:              true,
			},
			wantNames: []string{i5.Name, i6.Name, "vpp1", "vpp2", "vpp3"},
			wantMeta:  &mobius.PaginationMetadata{HasNextResults: false, HasPreviousResults: true, TotalResults: 15},
		},

		{
			...
			wantMeta:  &mobius.PaginationMetadata{HasNextResults: false, HasPreviousResults: true, TotalResults: 14},
		},
		{
			name: "Include Available for install software with MDM on, page 3",
			opts: mobius.HostSoftwareTitleListOptions{
				ListOptions:                mobius.ListOptions{Page: 3, PerPage: 5},
				IncludeAvailableForInstall: true,
				IsMDMEnrolled:              true,
			},
			wantNames: []string{},
		...
		},
		{
			...
			opts:      mobius.HostSoftwareTitleListOptions{ListOptions: mobius.ListOptions{PerPage: 3}, IncludeAvailableForInstall: true, SelfServiceOnly: true, IsMDMEnrolled: true},
			wantNames: []string{byNSV[b].Name, i0.Name, "vpp3"},
			wantMeta:  &mobius.PaginationMetadata{HasNextResults: false, HasPreviousResults: false, TotalResults: 3},
		},
		{
			name:      "Available for install and self-service only software with MDM on, page 1",
			opts:      mobius.HostSoftwareTitleListOptions{ListOptions: mobius.ListOptions{Page: 1, PerPage: 3}, IncludeAvailableForInstall: true, SelfServiceOnly: true, IsMDMEnrolled: true},
			wantNames: []string{},
			wantMeta:  &mobius.PaginationMetadata{HasNextResults: false, HasPreviousResults: true, TotalResults: 3},
		},
		{
			name:      "Available for install and self-service only software with MDM off, page 0",
			opts:      mobius.HostSoftwareTitleListOptions{ListOptions: mobius.ListOptions{PerPage: 3}, IncludeAvailableForInstall: true, SelfServiceOnly: true},
			wantNames: []string{byNSV[b].Name, i0.Name},
			wantMeta:  &mobius.PaginationMetadata{HasNextResults: false, HasPreviousResults: false, TotalResults: 2},
		},
		{
			name:      "Only available for install software with MDM on, page 0",
			opts:      mobius.HostSoftwareTitleListOptions{ListOptions: mobius.ListOptions{Page: 0, PerPage: 4}, OnlyAvailableForInstall: true, IsMDMEnrolled: true},
			wantNames: []string{byNSV[b].Name, "i0", "i1", "i2"},
			wantMeta:  &mobius.PaginationMetadata{HasNextResults: true, HasPreviousResults: false, TotalResults: 10},
		},
		{
			name: "Only available for install with MDM on, page 2",
			opts: mobius.HostSoftwareTitleListOptions{
				ListOptions:             mobius.ListOptions{Page: 2, PerPage: 4},
				OnlyAvailableForInstall: true,
				IsMDMEnrolled:           true,
			},
			wantNames: []string{"vpp2", "vpp3"},
			wantMeta:  &mobius.PaginationMetadata{HasNextResults: false, HasPreviousResults: true, TotalResults: 10},
		},
	}
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
	...
	softwareAlreadyInstalled := mobius.Software{Name: "DummyApp.app", Version: "1.0.1", Source: "apps", BundleIdentifier: "com.example.dummy"}

	// Host has software installed, but not by Mobius, and there is no matching software installer.
	...
	_, err = ds.InsertSoftwareVulnerability(ctx, mobius.SoftwareVulnerability{SoftwareID: softwareAlreadyInstalled.ID, CVE: "CVE-2025-10101"}, mobius.NVDSource)
	require.NoError(t, err)
	opts = mobius.HostSoftwareTitleListOptions{ListOptions: mobius.ListOptions{PerPage: 11, IncludeMetadata: true, OrderKey: "name", TestSecondaryOrderKey: "source"}}
	opts.OnlyAvailableForInstall = true
	opts.VulnerableOnly = true
	...
	_, err = ds.InsertSoftwareInstallRequest(ctx, host.ID, SoftwareInstallerID, mobius.HostSoftwareInstallOptions{})
	require.NoError(t, err)
	opts = mobius.HostSoftwareTitleListOptions{ListOptions: mobius.ListOptions{PerPage: 11, IncludeMetadata: true, OrderKey: "name", TestSecondaryOrderKey: "source"}}
	opts.OnlyAvailableForInstall = true
	opts.VulnerableOnly = true
...
func testListIOSHostSoftware(t *testing.T, ds *Datastore) {
	...
	nanoEnroll(t, ds, host, false)
	opts := mobius.HostSoftwareTitleListOptions{ListOptions: mobius.ListOptions{
	...
	compareResults := func(expected map[string]mobius.HostSoftwareWithInstaller, got []*mobius.HostSoftwareWithInstaller, expectAsc bool,
	...
	sw, meta, err = ds.ListHostSoftware(ctx, host, opts)
	require.NoError(t, err)
	assert.Equal(t, &mobius.PaginationMetadata{TotalResults: uint(len(expected))}, meta)
	compareResults(expected, sw, true)

	opts.VulnerableOnly = true
	sw, meta, err = ds.ListHostSoftware(ctx, host, opts)
	require.NoError(t, err)
	assert.Equal(t, &mobius.PaginationMetadata{TotalResults: uint(len(expected) - 1)}, meta)
	...
	// Create a team
	tm, err := ds.NewTeam(ctx, &mobius.Team{Name: "mobile team"})
	require.NoError(t, err)
	...
	_, err = ds.InsertVPPAppWithTeam(ctx,
		&mobius.VPPApp{
			VPPAppTeam: mobius.VPPAppTeam{VPPAppID: mobius.VPPAppID{AdamID: "adam_vpp_1", Platform: mobius.IPadOSPlatform}}, Name: "vpp1",
			BundleIdentifier: "com.app.vpp1",
		}, nil)
	require.NoError(t, err)
	_, err = ds.InsertVPPAppWithTeam(ctx,
	...
	va2, err := ds.InsertVPPAppWithTeam(ctx,
		&mobius.VPPApp{
			VPPAppTeam: mobius.VPPAppTeam{VPPAppID: mobius.VPPAppID{AdamID: "adam_vpp_2", Platform: mobius.IOSPlatform}}, Name: "vpp2",
			BundleIdentifier: "com.app.vpp2",
		}, nil)
	require.NoError(t, err)
	va3, err := ds.InsertVPPAppWithTeam(ctx,
		&mobius.VPPApp{
			VPPAppTeam: mobius.VPPAppTeam{VPPAppID: mobius.VPPAppID{AdamID: "adam_vpp_3", Platform: mobius.IOSPlatform}}, Name: "vpp3",
			BundleIdentifier: "com.app.vpp3",
		}, nil)
	require.NoError(t, err)
	va4, err := ds.InsertVPPAppWithTeam(ctx,
		&mobius.VPPApp{
			VPPAppTeam: mobius.VPPAppTeam{VPPAppID: mobius.VPPAppID{AdamID: "adam_vpp_4", Platform: mobius.IOSPlatform}}, Name: "vpp4",
	...
	expected["vpp1ios_apps"] = mobius.HostSoftwareWithInstaller{
		Name:        "vpp1",
		Source:      "ios_apps",
		Status:      expectStatus(mobius.SoftwareInstalled),
		AppStoreApp: &mobius.SoftwarePackageOrApp{AppStoreID: vpp1, Platform: "ios", SelfService: ptr.Bool(false), LastInstall: &mobius.HostSoftwareInstall{CommandUUID: vpp1CmdUUID}},
	}
	expected["vpp2ios_apps"] = mobius.HostSoftwareWithInstaller{
		Name:        "vpp2",
		Source:      "ios_apps",
		Status:      expectStatus(mobius.SoftwareInstallPending),
		AppStoreApp: &mobius.SoftwarePackageOrApp{AppStoreID: vpp2, Platform: "ios", SelfService: ptr.Bool(false), LastInstall: &mobius.HostSoftwareInstall{CommandUUID: vpp2bCmdUUID}},
	}
	...
	expected["vpp3ios_apps"] = mobius.HostSoftwareWithInstaller{
		Name:        "vpp3",
		Source:      "ios_apps",
		Status:      nil,
		AppStoreApp: &mobius.SoftwarePackageOrApp{AppStoreID: vpp3, Platform: "ios", SelfService: ptr.Bool(false)},
	}
	expected["vpp4ios_apps"] = mobius.HostSoftwareWithInstaller{
		Name:        "vpp4",
		Source:      "ios_apps",
		Status:      nil,
		AppStoreApp: &mobius.SoftwarePackageOrApp{AppStoreID: vpp4, Platform: "ios", SelfService: ptr.Bool(false)},
	}
	expectedAvailableOnly := map[string]mobius.HostSoftwareWithInstaller{}
...
func testListHostSoftwareWithVPPApps(t *testing.T, ds *Datastore) {
	...
	vPPApp := &mobius.VPPApp{
		VPPAppTeam:       mobius.VPPAppTeam{VPPAppID: mobius.VPPAppID{AdamID: "adam_vpp_1", Platform: mobius.MacOSPlatform}},
		Name:             "vpp1",
		BundleIdentifier: "com.app.vpp1",
		LatestVersion:    "1.0.0",
	}
	...
	sw, _, err := ds.ListHostSoftware(
		...
		mobius.HostSoftwareTitleListOptions{
			OnlyAvailableForInstall: true,
			IsMDMEnrolled:           true,
			ListOptions:             mobius.ListOptions{PerPage: 10, IncludeMetadata: true, OrderKey: "name", TestSecondaryOrderKey: "source"},
		},
	)
	require.NoError(t, err)
	assert.Len(t, sw, 1)
	...
	opts := mobius.HostSoftwareTitleListOptions{ListOptions: mobius.ListOptions{PerPage: uint(numberOfApps - 1), IncludeMetadata: true, OrderKey: "name", TestSecondaryOrderKey: "source"}}
	sw, meta, err := ds.ListHostSoftware(ctx, host, opts)
	require.NoError(t, err)
	...
	// The vpp app is installed by mobius, and also has been inventoried by osquery
	...
	// have the second host install a vpp app, but not by mobius
	res, err = ds.writer(ctx).ExecContext(ctx, `
        INSERT INTO software (name, version, source, bundle_identifier, title_id, checksum)
        VALUES (?, ?, ?, ?, ?, ?)
	`,
	...
	opts = mobius.HostSoftwareTitleListOptions{OnlyAvailableForInstall: true, ListOptions: mobius.ListOptions{PerPage: uint(numberOfApps - 1), IncludeMetadata: true, OrderKey: "name", TestSecondaryOrderKey: "source"}}
...
func testListHostSoftwareVPPSelfService(t *testing.T, ds *Datastore) {
	...
	vPPApp := &mobius.VPPApp{
		VPPAppTeam:       mobius.VPPAppTeam{SelfService: true, VPPAppID: mobius.VPPAppID{AdamID: "adam_vpp_1", Platform: mobius.MacOSPlatform}},
		Name:             "vpp1",
		BundleIdentifier: "com.app.vpp1",
		LatestVersion:    "1.0.0",
	}
	...
	vPPApp2 := &mobius.VPPApp{
		VPPAppTeam:       mobius.VPPAppTeam{SelfService: true, VPPAppID: mobius.VPPAppID{AdamID: "adam_vpp_2", Platform: mobius.MacOSPlatform}},
		Name:             "vpp2",
		BundleIdentifier: "com.app.vpp2",
		LatestVersion:    "1.0.1",
	}
	...
	assert.Equal(t, mobius.SoftwareInstallerStatus("pending_install"), *sw[0].Status)
	assert.NotNil(t, sw[0].AppStoreApp.LastInstall)
	assert.Nil(t, sw[0].InstalledVersions)

	// installed but not by mobius, so status is nil
...
func testSetHostSoftwareInstallResult(t *testing.T, ds *Datastore) {
	...
	checkResults := func(want *mobius.HostSoftwareInstallResultPayload) {
		type result struct {
	...
	want := &mobius.HostSoftwareInstallResultPayload{
		HostID:                    host.ID,
		InstallUUID:               "uuid0",
...
func testListHostSoftwareInstallThenTransferTeam(t *testing.T, ds *Datastore) {
	...
	team1, err := ds.NewTeam(ctx, &mobius.Team{Name: "team 1"})
	require.NoError(t, err)
	team2, err := ds.NewTeam(ctx, &mobius.Team{Name: "team 2"})
	require.NoError(t, err)
	...
	tfr1, err := mobius.NewTempFileReader(strings.NewReader("hello"), t.TempDir)
	require.NoError(t, err)
	installerTm1, _, err := ds.MatchOrCreateSoftwareInstaller(ctx, &mobius.UploadSoftwareInstallerPayload{
		InstallScript:   "hello",
		InstallerFile:   tfr1,
		StorageID:       "storage1",
		Filename:        "file1",
	...
	hostInstall1, err := ds.InsertSoftwareInstallRequest(ctx, host.ID, installerTm1, mobius.HostSoftwareInstallOptions{})
	require.NoError(t, err)
	_, err = ds.SetHostSoftwareInstallResult(ctx, &mobius.HostSoftwareInstallResultPayload{
		HostID:                host.ID,
	...
	createVPPAppInstallResult(t, ds, host, vpp1CmdUUID, mobius.MDMAppleStatusError)
...
func testListSoftwareVersionsVulnerabilityFilters(t *testing.T, ds *Datastore) {
	...
	_, err = ds.InsertSoftwareVulnerability(ctx, mobius.SoftwareVulnerability{
	...
	err = ds.InsertCVEMeta(ctx, []mobius.CVEMeta{
		{
			// chrome
			CVE:              "CVE-2024-1234",
	...
	tc := []struct {
	...
	}{
		{
			...
			opts: mobius.SoftwareListOptions{
				ListOptions:    mobius.ListOptions{OrderKey: "name"},
				VulnerableOnly: true,
			},
			expected: []swVersion{
				{
		...
		},
		{
			...
			opts: mobius.SoftwareListOptions{
				ListOptions:      mobius.ListOptions{OrderKey: "name", OrderDirection: mobius.OrderAscending},
				IncludeCVEScores: true,
				VulnerableOnly:   true,
				KnownExploit:     true,
			},
		...
		},
		{
			...
			err: mobius.NewInvalidArgumentError("query", "min_cvss_score, max_cvss_score, and exploit can only be provided with vulnerable=true"),
		},
		{
...
func testListHostSoftwareWithLabelScoping(t *testing.T, ds *Datastore) {
	...
	tfr1, err := mobius.NewTempFileReader(strings.NewReader("hello"), t.TempDir)
	require.NoError(t, err)
	installer1 := &mobius.UploadSoftwareInstallerPayload{
		InstallScript:     "hello",
	...
	tfr2, err := mobius.NewTempFileReader(strings.NewReader("hello"), t.TempDir)
	require.NoError(t, err)
	selfServiceinstaller := &mobius.UploadSoftwareInstallerPayload{
	...
	// install software installer on host via mobius
	hostInstall1, err := ds.InsertSoftwareInstallRequest(ctx, thirdHost.ID, selfServiceInstallerID, mobius.HostSoftwareInstallOptions{})
	require.NoError(t, err)
	_, err = ds.SetHostSoftwareInstallResult(ctx, &mobius.HostSoftwareInstallResultPayload{
		HostID:                thirdHost.ID,
		InstallUUID:           hostInstall1,
	...
	require.NoError(t, err)
	installedSoftware := []mobius.Software{
		{
			Name:    "file2",
			Version: "1.0",
			Source:  "apps",
	...
	softwareAlreadyInstalled := mobius.Software{Name: "file1", Version: "1.0.1", Source: "apps", BundleIdentifier: "bi1"}
	// Host has software installed, but not by Mobius, that matches the software installer available
	ExecAdhocSQL(t, ds, func(q sqlx.ExtContext) error {
		titleIDUint := titleID
		softwareAlreadyInstalled.TitleID = &titleIDUint
		res, err := q.ExecContext(ctx, `INSERT INTO software (name, source, bundle_identifier, version, title_id) VALUES (?, ?, ?, ?, ?)`,
	...
	expectedInstallers := map[string]*mobius.SoftwarePackageOrApp{
		installer1.Filename: {
	...
	checkSoftware := func(swList []*mobius.HostSoftwareWithInstaller, excludeNames ...string) {
	...
	label1, err := ds.NewLabel(ctx, &mobius.Label{Name: "label1" + t.Name()})
	require.NoError(t, err)
	...
	err = setOrUpdateSoftwareInstallerLabelsDB(ctx, ds.writer(ctx), installerID1, mobius.LabelIdentsWithScope{
		LabelScope: mobius.LabelScopeExcludeAny,
		ByName:     map[string]mobius.LabelIdent{label1.Name: {LabelName: label1.Name, LabelID: label1.ID}},
	}, softwareTypeInstaller)
	require.NoError(t, err)
	err = setOrUpdateSoftwareInstallerLabelsDB(ctx, ds.writer(ctx), selfServiceInstallerID, mobius.LabelIdentsWithScope{
		LabelScope: mobius.LabelScopeExcludeAny,
		ByName:     map[string]mobius.LabelIdent{label1.Name: {LabelName: label1.Name, LabelID: label1.ID}},
	}, softwareTypeInstaller)
	require.NoError(t, err)
	...
	// self-service flag
	selfServiceOpts := mobius.HostSoftwareTitleListOptions{ListOptions: mobius.ListOptions{PerPage: 10}, IncludeAvailableForInstall: true, SelfServiceOnly: true, IsMDMEnrolled: true}
	...
	installer2 := &mobius.UploadSoftwareInstallerPayload{
		InstallScript:     "hello",
		PreInstallQuery:   "SELECT 1",
		PostInstallScript: "world",
		UninstallScript:   "goodbye",
	...
	expectedInstallers[installer2.Filename] = &mobius.SoftwarePackageOrApp{
	...
	label2, err := ds.NewLabel(ctx, &mobius.Label{Name: "label2" + t.Name()})
	require.NoError(t, err)

	label3, err := ds.NewLabel(ctx, &mobius.Label{Name: "label3" + t.Name()})
	require.NoError(t, err)

	err = setOrUpdateSoftwareInstallerLabelsDB(ctx, ds.writer(ctx), installerID2, mobius.LabelIdentsWithScope{
		LabelScope: mobius.LabelScopeExcludeAny,
		ByName: map[string]mobius.LabelIdent{
	...
	// Add an installer. No label yet.
	installer3 := &mobius.UploadSoftwareInstallerPayload{
		InstallScript:     "hello",
		PreInstallQuery:   "SELECT 1",
		PostInstallScript: "world",
		UninstallScript:   "goodbye",
	...
	time.Sleep(time.Second)
	expectedInstallers[installer3.Filename] = &mobius.SoftwarePackageOrApp{
	...
	label4, err := ds.NewLabel(ctx, &mobius.Label{Name: "label4" + t.Name(), LabelMembershipType: mobius.LabelMembershipTypeDynamic})
	require.NoError(t, err)

	err = setOrUpdateSoftwareInstallerLabelsDB(ctx, ds.writer(ctx), installerID3, mobius.LabelIdentsWithScope{
		LabelScope: mobius.LabelScopeExcludeAny,
		ByName:     map[string]mobius.LabelIdent{label4.Name: {LabelName: label4.Name, LabelID: label4.ID}},
	}, softwareTypeInstaller)
	require.NoError(t, err)
...
func testListHostSoftwareVulnerabileAndVPP(t *testing.T, ds *Datastore) {
	...
	vulnerableOnlyOpts := mobius.HostSoftwareTitleListOptions{
		ListOptions: mobius.ListOptions{
			OrderKey: "name",
		},
		VulnerableOnly:             true,
	...
	knownExploitOpts := mobius.HostSoftwareTitleListOptions{
		ListOptions: mobius.ListOptions{
			OrderKey: "name",
		},
		VulnerableOnly: true,
	...
	minCVSSScoreOpts := mobius.HostSoftwareTitleListOptions{
		ListOptions: mobius.ListOptions{
			OrderKey: "name",
		},
		VulnerableOnly: true,
	...
	maxCVSSScoreOpts := mobius.HostSoftwareTitleListOptions{
		ListOptions: mobius.ListOptions{
			OrderKey: "name",
		},
		VulnerableOnly: true,
	...
	_, err = ds.InsertSoftwareVulnerability(
		ctx,
		mobius.SoftwareVulnerability{SoftwareID: uint(insertedID), CVE: "CVE-c-00c1"},
		mobius.NVDSource,
	)
	require.NoError(t, err)
	...
	tfr1, err := mobius.NewTempFileReader(strings.NewReader("hello"), t.TempDir)
	require.NoError(t, err)
	installerTm1, installerTitleID, err := ds.MatchOrCreateSoftwareInstaller(ctx, &mobius.UploadSoftwareInstallerPayload{
		InstallScript:   "hello",
		InstallerFile:   tfr1,
		StorageID:       "storage1",
		Filename:        "file1",
	...
	require.NoError(t, err)
	hostInstall1, err := ds.InsertSoftwareInstallRequest(ctx, tmHost.ID, installerTm1, mobius.HostSoftwareInstallOptions{})
	require.NoError(t, err)
	_, err = ds.SetHostSoftwareInstallResult(ctx, &mobius.HostSoftwareInstallResultPayload{
		HostID:                tmHost.ID,
		InstallUUID:           hostInstall1,
	...
	require.NoError(t, err)
	createVPPAppInstallResult(t, ds, tmHost, vpp1CmdUUID, mobius.MDMAppleStatusAcknowledged)
	// Insert software entry for vpp app
	res, err := ds.writer(ctx).ExecContext(ctx, `
        INSERT INTO software (name, version, source, bundle_identifier, title_id, checksum)
        VALUES (?, ?, ?, ?, ?, ?)
	...
	_, err = ds.InsertSoftwareVulnerability(
		ctx,
		mobius.SoftwareVulnerability{SoftwareID: uint(vppSoftwareID), CVE: "CVE-vpp1-0001"},
		mobius.NVDSource,
	)
	require.NoError(t, err)
	...
	installerID, titleID, err := ds.MatchOrCreateSoftwareInstaller(ctx, &mobius.UploadSoftwareInstallerPayload{
		Title:           "foo",
		Source:          "bar",
		InstallScript:   "echo",
		TeamID:          &tm.ID,
	...
	_, err = ds.InsertSoftwareVulnerability(
		ctx,
		mobius.SoftwareVulnerability{SoftwareID: uint(insertedID), CVE: "CVE-bar-0001"},
		mobius.NVDSource,
	)
	require.NoError(t, err)

	// pending install request for foo1.0 (non-vulnerable version)
	_, err = ds.InsertSoftwareInstallRequest(ctx, tmHost.ID, installerID, mobius.HostSoftwareInstallOptions{})
	require.NoError(t, err)
	require.NoError(t, ds.ReconcileSoftwareTitles(ctx))
	// Ensure that software "a" & "b" are returned as they are the only vulnerable apps at this point
	sw, _, err = ds.ListHostSoftware(ctx, tmHost, vulnerableOnlyOpts)
	...
	pendingVPPApp := &mobius.VPPApp{
		VPPAppTeam:       mobius.VPPAppTeam{VPPAppID: mobius.VPPAppID{AdamID: "adam_vpp_2", Platform: mobius.MacOSPlatform}},
		Name:             "vpp2",
		BundleIdentifier: "com.app.vpp2",
	}
	...
	// host has vulnerable software installed (not by mobius) that happens to match a vpp app in the mobius catalog
	hostInstalledVpps := &mobius.VPPApp{
		VPPAppTeam:       mobius.VPPAppTeam{VPPAppID: mobius.VPPAppID{AdamID: "adam_vpp_3", Platform: mobius.MacOSPlatform}},
		Name:             "vpp3",
		BundleIdentifier: "com.app.vpp3",
	}
	...
	sw, _, err = ds.ListHostSoftware(ctx, tmHost, mobius.HostSoftwareTitleListOptions{
		ListOptions: mobius.ListOptions{
			OrderKey: "name",
		},
		VulnerableOnly:             true,
	...
	matchingsOpts := mobius.HostSoftwareTitleListOptions{
		ListOptions: mobius.ListOptions{
			OrderKey: "name",
		},
		IncludeAvailableForInstall: true,
...
func testListHostSoftwareQuerySearching(t *testing.T, ds *Datastore) {
	...
	byName := map[string]mobius.Software{}
	for _, s := range software {
		byName[s.Name] = s
	}
	...
	require.NoError(t, err)
	require.Equal(t, &mobius.PaginationMetadata{TotalResults: 3}, meta)
	require.Equal(t, software[2].Name, sw[0].Name)
	require.Equal(t, software[0].Name, sw[1].Name)
	require.Equal(t, vPPApp.Name, sw[2].Name)

	// search with self-service
	vPPAppSlack := &mobius.VPPApp{
		VPPAppTeam:       mobius.VPPAppTeam{SelfService: true, VPPAppID: mobius.VPPAppID{AdamID: "adam_vpp_2", Platform: mobius.MacOSPlatform}},
		Name:             "slack",
	...
	vPPApp1Password := &mobius.VPPApp{
		VPPAppTeam:       mobius.VPPAppTeam{SelfService: true, VPPAppID: mobius.VPPAppID{AdamID: "adam_vpp_3", Platform: mobius.MacOSPlatform}},
		Name:             "1password",
...
func testListHostSoftwareWithLabelScopingVPP(t *testing.T, ds *Datastore) {
	...
	vppApp := &mobius.VPPApp{Name: "vpp_app_1", VPPAppTeam: mobius.VPPAppTeam{SelfService: true, VPPAppID: mobius.VPPAppID{AdamID: "1", Platform: mobius.MacOSPlatform}}, BundleIdentifier: "b1"}
	...
	require.NoError(t, err)
	createVPPAppInstallResult(t, ds, anotherHost, vpp1CmdUUID, mobius.MDMAppleStatusAcknowledged)
	// Insert software entry for vpp app
	_, err = ds.writer(ctx).ExecContext(ctx, `
        INSERT INTO software (name, version, source, bundle_identifier, title_id, checksum)
        VALUES (?, ?, ?, ?, ?, ?)
	...
	err = setOrUpdateSoftwareInstallerLabelsDB(ctx, ds.writer(ctx), vppAppTeamID, mobius.LabelIdentsWithScope{
		LabelScope: mobius.LabelScopeExcludeAny,
		ByName:     map[string]mobius.LabelIdent{label1.Name: {LabelName: label1.Name, LabelID: label1.ID}},
	}, softwareTypeVPP)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	createVPPAppInstallResult(t, ds, fourthHost, forthHostVpp1CmdUUID, mobius.MDMAppleStatusAcknowledged)
	// but inventory has not been updated yet so no software/host software records exist
	software, _, err = ds.ListHostSoftware(
		ctx,
		fourthHost,
		mobius.HostSoftwareTitleListOptions{
			ListOptions:                mobius.ListOptions{PerPage: 10},
			IncludeAvailableForInstall: true,
			SelfServiceOnly:            true,
			IsMDMEnrolled:              true,
		},
	...
	expectedStatus := mobius.SoftwareInstallerStatus("installed")
	...
	err = setOrUpdateSoftwareInstallerLabelsDB(ctx, ds.writer(ctx), vppAppTeamID, mobius.LabelIdentsWithScope{
		LabelScope: mobius.LabelScopeIncludeAny,
		ByName:     map[string]mobius.LabelIdent{label2.Name: {LabelName: label2.Name, LabelID: label2.ID}},
	}, softwareTypeVPP)
	require.NoError(t, err)
	...
	err = setOrUpdateSoftwareInstallerLabelsDB(ctx, ds.writer(ctx), vppAppTeamID, mobius.LabelIdentsWithScope{
		LabelScope: mobius.LabelScopeIncludeAny,
		ByName:     map[string]mobius.LabelIdent{label2.Name: {LabelName: label2.Name, LabelID: label2.ID}, label1.Name: {LabelName: label1.Name, LabelID: label1.ID}},
	}, softwareTypeVPP)
	require.NoError(t, err)
	...
	err = setOrUpdateSoftwareInstallerLabelsDB(ctx, ds.writer(ctx), vppAppTeamID, mobius.LabelIdentsWithScope{
		LabelScope: mobius.LabelScopeExcludeAny,
		ByName:     map[string]mobius.LabelIdent{label3.Name: {LabelName: label3.Name, LabelID: label3.ID}},
	}, softwareTypeVPP)
	require.NoError(t, err)
...
func testListHostSoftwareSelfServiceWithLabelScopingHostInstalled(t *testing.T, ds *Datastore) {
	...
	vPPApp := &mobius.VPPApp{
		VPPAppTeam: mobius.VPPAppTeam{
			...
			VPPAppID: mobius.VPPAppID{
				AdamID: "adam_vpp_1", Platform: mobius.MacOSPlatform,
			},
		},
		Name:             "vpp1",
		BundleIdentifier: "com.app.vpp1",
	...
	// exclude label
	excludeLabel, err := ds.NewLabel(ctx, &mobius.Label{Name: "Exclude Label" + t.Name()})
	require.NoError(t, err)
	...
	err = setOrUpdateSoftwareInstallerLabelsDB(ctx, ds.writer(ctx), selfServiceInstallerID, mobius.LabelIdentsWithScope{
		LabelScope: mobius.LabelScopeExcludeAny,
		ByName:     map[string]mobius.LabelIdent{excludeLabel.Name: {LabelName: excludeLabel.Name, LabelID: excludeLabel.ID}},
	}, softwareTypeInstaller)
	require.NoError(t, err)
	// label vpp app
	err = setOrUpdateSoftwareInstallerLabelsDB(ctx, ds.writer(ctx), vPPApp.VPPAppTeam.AppTeamID, mobius.LabelIdentsWithScope{
		LabelScope: mobius.LabelScopeExcludeAny,
		ByName:     map[string]mobius.LabelIdent{excludeLabel.Name: {LabelName: excludeLabel.Name, LabelID: excludeLabel.ID}},
	}, softwareTypeVPP)
	require.NoError(t, err)
...
func testDeletedInstalledSoftware(t *testing.T, ds *Datastore) {
	...
	host1 := test.NewHost(t, ds, "host1", "", "host1key", "host1uuid", time.Now())
	user1 := test.NewUser(t, ds, "Alice", "alice@example.com", true)
	team, err := ds.NewTeam(ctx, &mobius.Team{Name: "team 1"})
	require.NoError(t, err)
	...
	require.NoError(t, err)
	_, err = ds.InsertSoftwareInstallRequest(ctx, host1.ID, installerID, mobius.HostSoftwareInstallOptions{})
	require.NoError(t, err)
...
func testSoftwareCategories(t *testing.T, ds *Datastore) {
	...
	tfr1, err := mobius.NewTempFileReader(strings.NewReader("hello"), t.TempDir)
	require.NoError(t, err)
	installerNoTeam, titleID, err := ds.MatchOrCreateSoftwareInstaller(ctx, &mobius.UploadSoftwareInstallerPayload{
		InstallScript:   "hello",
		InstallerFile:   tfr1,
		StorageID:       "storage1",
		Filename:        "file1",
	...
	err = ds.SaveInstallerUpdates(ctx, &mobius.UpdateSoftwareInstallerPayload{
		TitleID:         titleID,
	...
	vppApp := &mobius.VPPApp{Name: "vpp_app_1", VPPAppTeam: mobius.VPPAppTeam{VPPAppID: mobius.VPPAppID{AdamID: "1", Platform: mobius.MacOSPlatform}, CategoryIDs: ids}, BundleIdentifier: "b1"}
	vppApp, err = ds.InsertVPPAppWithTeam(ctx, vppApp, nil)
	require.NoError(t, err)
