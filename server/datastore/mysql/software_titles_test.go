...
func testSoftwareSyncHostsSoftwareTitles(t *testing.T, ds *Datastore) {
	...
	cmpNameVersionCount := func(want, got []mobius.SoftwareTitleListResult) {
		cmp := make([]mobius.SoftwareTitleListResult, len(got))
		for i, sw := range got {
			cmp[i] = mobius.SoftwareTitleListResult{Name: sw.Name, HostsCount: sw.HostsCount}
		}
	...
	globalOpts := mobius.SoftwareTitleListOptions{ListOptions: mobius.ListOptions{OrderKey: "hosts_count", OrderDirection: mobius.OrderDescending}}
	globalCounts := listSoftwareTitlesCheckCount(t, ds, 2, 2, globalOpts)

	want := []mobius.SoftwareTitleListResult{
	...
	allSw := listSoftwareTitlesCheckCount(t, ds, 1, 1, mobius.SoftwareTitleListOptions{})
	want = []mobius.SoftwareTitleListResult{
		{Name: "foo", HostsCount: 2},
	}
	...
	team1Opts := mobius.SoftwareTitleListOptions{
		TeamID:      ptr.Uint(team1.ID),
		ListOptions: mobius.ListOptions{OrderKey: "hosts_count", OrderDirection: mobius.OrderDescending},
	}
	...
	team2Opts := mobius.SoftwareTitleListOptions{
...
func testOrderSoftwareTitles(t *testing.T, ds *Datastore) {
	...
	require.NoError(t, err)
	_, err = ds.InsertSoftwareInstallRequest(ctx, host1.ID, installer2, mobius.HostSoftwareInstallOptions{})
	require.NoError(t, err)
	...
	_, err = ds.InsertVPPAppWithTeam(ctx, &mobius.VPPApp{
		...
		VPPAppTeam: mobius.VPPAppTeam{VPPAppID: mobius.VPPAppID{AdamID: "adam_vpp_app_1", Platform: mobius.IPadOSPlatform}},
	}, nil)
	require.NoError(t, err)
	...
	titles, _, _, err := ds.ListSoftwareTitles(ctx, mobius.SoftwareTitleListOptions{
		ListOptions: mobius.ListOptions{
			OrderKey:       "hosts_count",
			OrderDirection: mobius.OrderDescending,
		},
		TeamID: ptr.Uint(0),
	}, mobius.TeamFilter{User: &mobius.User{GlobalRole: ptr.String(mobius.RoleAdmin)}})
	require.NoError(t, err)
...
func listSoftwareTitlesCheckCount(t *testing.T, ds *Datastore, expectedListCount int, expectedFullCount int, opts mobius.SoftwareTitleListOptions) []mobius.SoftwareTitleListResult {
	titles, count, _, err := ds.ListSoftwareTitles(context.Background(), opts, mobius.TeamFilter{User: &mobius.User{GlobalRole: ptr.String(mobius.RoleAdmin)}})
	require.NoError(t, err)
...
func testTeamFilterSoftwareTitles(t *testing.T, ds *Datastore) {
	...
	userGlobalAdmin, err := ds.NewUser(ctx, &mobius.User{Name: "user1", Password: []byte("test"), Email: "test1@email.com", GlobalRole: ptr.String(mobius.RoleAdmin)})
	require.NoError(t, err)
	userTeam1Admin, err := ds.NewUser(ctx, &mobius.User{Name: "user2", Password: []byte("test"), Email: "test2@email.com", Teams: []mobius.UserTeam{{Team: mobius.Team{ID: team1.ID}, Role: mobius.RoleAdmin}}})
	require.NoError(t, err)
	userTeam2Admin, err := ds.NewUser(ctx, &mobius.User{Name: "user3", Password: []byte("test"), Email: "test3@email.com", Teams: []mobius.UserTeam{{Team: mobius.Team{ID: team2.ID}, Role: mobius.RoleAdmin}}})
	require.NoError(t, err)
	...
	_, err = ds.InsertVPPAppWithTeam(ctx, &mobius.VPPApp{
		...
		VPPAppTeam: mobius.VPPAppTeam{VPPAppID: mobius.VPPAppID{AdamID: "adam_vpp_app_2", Platform: mobius.IOSPlatform}},
	}, &team2.ID)
	require.NoError(t, err)
	...
	_, err = ds.InsertVPPAppWithTeam(ctx, &mobius.VPPApp{
		...
		VPPAppTeam: mobius.VPPAppTeam{VPPAppID: mobius.VPPAppID{AdamID: "adam_vpp_app_3", Platform: mobius.MacOSPlatform}, SelfService: true},
	}, ptr.Uint(0))
	require.NoError(t, err)
	...
	globalTeamFilter := mobius.TeamFilter{User: userGlobalAdmin, IncludeObserver: true}
	titles, count, _, err := ds.ListSoftwareTitles(
		context.Background(), mobius.SoftwareTitleListOptions{
			ListOptions: mobius.ListOptions{},
			TeamID:      nil,
		}, globalTeamFilter,
	)
	...
	team1TeamFilter := mobius.TeamFilter{User: userTeam1Admin, IncludeObserver: true}
	titles, count, _, err = ds.ListSoftwareTitles(
		context.Background(), mobius.SoftwareTitleListOptions{ListOptions: mobius.ListOptions{}, TeamID: &team1.ID}, team1TeamFilter,
	)
	...
	assert.Equal(t, titles[0], mobius.SoftwareTitleListResult{ID: title.ID, Name: title.Name, Source: title.Source, Browser: title.Browser, HostsCount: title.HostsCount, VersionsCount: title.VersionsCount, Versions: title.Versions, CountsUpdatedAt: title.CountsUpdatedAt})

	// Testing the team 2 user
	titles, count, _, err = ds.ListSoftwareTitles(context.Background(), mobius.SoftwareTitleListOptions{ListOptions: mobius.ListOptions{}, TeamID: &team2.ID}, mobius.TeamFilter{
		User:            userTeam2Admin,
	...
	titles, _, _, err = ds.ListSoftwareTitles(
		context.Background(), mobius.SoftwareTitleListOptions{ListOptions: mobius.ListOptions{}, SelfServiceOnly: true, TeamID: &team1.ID}, team1TeamFilter,
	)
	// installer1 is associated with team 1
	require.NoError(t, err)
	require.Len(t, titles, 1)
	...
	titles, _, _, err = ds.ListSoftwareTitles(context.Background(), mobius.SoftwareTitleListOptions{ListOptions: mobius.ListOptions{}, SelfServiceOnly: true, TeamID: &team2.ID}, mobius.TeamFilter{
		User:            userTeam2Admin,
		IncludeObserver: true,
	})
	require.NoError(t, err)
	...
	titles, _, _, err = ds.ListSoftwareTitles(context.Background(), mobius.SoftwareTitleListOptions{ListOptions: mobius.ListOptions{}, SelfServiceOnly: true, TeamID: ptr.Uint(0)}, mobius.TeamFilter{
...
func sortTitlesByName(titles []mobius.SoftwareTitleListResult) {
...
func testListSoftwareTitlesInstallersOnly(t *testing.T, ds *Datastore) {
	...
	titles, counts, _, err := ds.ListSoftwareTitles(ctx, mobius.SoftwareTitleListOptions{
		ListOptions: mobius.ListOptions{
			OrderKey:       "name",
			OrderDirection: mobius.OrderAscending,
		},
...
func testListSoftwareTitlesAvailableForInstallFilter(t *testing.T, ds *Datastore) {
	...
	_, err = ds.InsertVPPAppWithTeam(ctx, &mobius.VPPApp{
		...
		VPPAppTeam: mobius.VPPAppTeam{VPPAppID: mobius.VPPAppID{AdamID: "adam_vpp_app_2", Platform: mobius.IPadOSPlatform}},
	}, nil)
	require.NoError(t, err)
	_, err = ds.InsertVPPAppWithTeam(ctx, &mobius.VPPApp{
		Name: "vpp2", BundleIdentifier: "com.example.vpp2",
	...
	require.NoError(t, err)
	vpp2, err := ds.InsertVPPAppWithTeam(ctx, &mobius.VPPApp{
		Name: "vpp2", BundleIdentifier: "com.example.vpp2",
		VPPAppTeam: mobius.VPPAppTeam{VPPAppID: mobius.VPPAppID{AdamID: "adam_vpp_app_2", Platform: mobius.IOSPlatform}},
	}, nil)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	vppPolicy, err := ds.NewTeamPolicy(ctx, mobius.PolicyNoTeamID, nil, mobius.PolicyPayload{
		Name:           "VPP Policy",
		Query:          "SELECT 1;",
...
func testListSoftwareTitlesOverflow(t *testing.T, ds *Datastore) {
	...
	for i := uint(0); i < 40_000; i++ {
		software = append(software,
			mobius.Software{Name: fmt.Sprintf("%dname", i), Version: fmt.Sprintf("0.0.%d", i), Source: "deb_packages"},
		)
...
func testListSoftwareTitlesAllTeams(t *testing.T, ds *Datastore) {
	...
	macOSInstallerNoTeam, _, err := ds.MatchOrCreateSoftwareInstaller(ctx, &mobius.UploadSoftwareInstallerPayload{
		Title:            "foobar",
	...
	_, err = ds.InsertVPPAppWithTeam(ctx, &mobius.VPPApp{
		...
		VPPAppTeam: mobius.VPPAppTeam{VPPAppID: mobius.VPPAppID{AdamID: "adam_vpp_app_canva", Platform: mobius.IOSPlatform}},
	}, &team1.ID)
	require.NoError(t, err)
	...
	_, err = ds.InsertVPPAppWithTeam(ctx, &mobius.VPPApp{
		Name: "Canva", BundleIdentifier: "com.example.canva",
		VPPAppTeam: mobius.VPPAppTeam{VPPAppID: mobius.VPPAppID{AdamID: "adam_vpp_app_canva", Platform: mobius.MacOSPlatform}},
	}, &team1.ID)
	require.NoError(t, err)
	...
	_, err = ds.InsertVPPAppWithTeam(ctx, &mobius.VPPApp{
		Name: "Canva", BundleIdentifier: "com.example.canva",
		VPPAppTeam: mobius.VPPAppTeam{VPPAppID: mobius.VPPAppID{AdamID: "adam_vpp_app_canva", Platform: mobius.IPadOSPlatform}},
	}, &team2.ID)
	require.NoError(t, err)
...
func testUploadedSoftwareExists(t *testing.T, ds *Datastore) {
	...
	tm, err := ds.NewTeam(ctx, &mobius.Team{Name: "Team Foo"})
	require.NoError(t, err)
	user1 := test.NewUser(t, ds, "Alice", "alice@example.com", true)
...
func testListSoftwareTitlesVulnerabilityFilters(t *testing.T, ds *Datastore) {
	...
	globalUser := &mobius.User{GlobalRole: ptr.String(mobius.RoleAdmin)}

	tc := []struct {
		name           string
		opts           mobius.SoftwareTitleListOptions
	...
	assertTitles := func(t *testing.T, titles []mobius.SoftwareTitleListResult, expectedTitles []string) {
		t.Helper()
	...
	for _, tt := range tc {
		t.Run(tt.name, func(t *testing.T) {
			titles, _, _, err := ds.ListSoftwareTitles(ctx, tt.opts, mobius.TeamFilter{User: globalUser})
			if tt.err != nil {
				require.Error(t, err)
...
func testUpdateSoftwareTitleName(t *testing.T, ds *Datastore) {
	...
	title1, err := ds.SoftwareTitleByID(ctx, installer1, nil, mobius.TeamFilter{User: user1})
	require.NoError(t, err)
	...
	title2, err := ds.SoftwareTitleByID(ctx, installer2, &tm.ID, mobius.TeamFilter{User: user1})
	require.NoError(t, err)
...
func testListSoftwareTitlesDoesnotIncludeDuplicates(t *testing.T, ds *Datastore) {
	...
	_, err := ds.UpdateHostSoftware(ctx, host.ID, []mobius.Software{
...
func generateSelectSoftwareTitlesSQLFixture(t *testing.T) { //nolint: unused
	...
	combinations := make([]mobius.SoftwareTitleListOptions, 0)
...
func generateSoftwareTitleListOptionsCombinations(
	...
	combinations *[]mobius.SoftwareTitleListOptions,
) {
	t := v.Type()
