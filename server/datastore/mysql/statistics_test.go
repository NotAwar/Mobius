...
func testStatisticsShouldSend(t *testing.T, ds *Datastore) {
	...
	premiumLicense := &mobius.LicenseInfo{Tier: mobius.TierPremium, Organization: "Mobius"}
	freeLicense := &mobius.LicenseInfo{Tier: mobius.TierFree}
	...
	stats, shouldSend, err := ds.ShouldSendStatistics(license.NewContext(ctx, premiumLicense), time.Millisecond, mobiusConfig)
	require.NoError(t, err)
	assert.True(t, shouldSend)
	assert.Equal(t, "premium", stats.LicenseTier)
	assert.Equal(t, "Mobius", stats.Organization)
	...
	assert.Equal(t, []mobius.HostsCountByOsqueryVersion{}, stats.HostsEnrolledByOsqueryVersion) // should be empty slice instead of nil
	assert.Equal(t, []mobius.HostsCountByOrbitVersion{}, stats.HostsEnrolledByOrbitVersion)     // should be empty slice instead of nil
	...
	assert.Equal(t, 0, stats.NumHostsMobiusDesktopEnabled)
	...
	require.NoError(t, err)
	_, err = ds.NewUser(ctx, &mobius.User{
		Password:                 []byte("foobar"),
		AdminForcedPasswordReset: false,
	...
	_, err = ds.NewTeam(ctx, &mobius.Team{
		Name:        "footeam",
	...
	_, err = ds.NewGlobalPolicy(ctx, ptr.Uint(1), mobius.PolicyPayload{
		Name:        "testpolicy",
		Query:       "select 1;",
	...
	_, err = ds.NewLabel(ctx, &mobius.Label{
		Name:        "testlabel",
		Query:       "select 1;",
		Platform:    "darwin",
	...
	cfg, err := ds.NewAppConfig(ctx, &mobius.AppConfig{
	...
	assert.NotEmpty(t, stats.MobiusVersion)
	assert.Equal(t, "premium", stats.LicenseTier)
	assert.Equal(t, "Mobius", stats.Organization)
	...
	assert.Equal(t, `[{"count":10,"loc":["a","b","c"]}]`, string(stats.StoredErrors))
	assert.Equal(t, []mobius.HostsCountByOsqueryVersion{{OsqueryVersion: "4.9.0", NumHosts: 1}}, stats.HostsEnrolledByOsqueryVersion)
	assert.Equal(t, []mobius.HostsCountByOrbitVersion{{OrbitVersion: "1.1.0", NumHosts: 1}}, stats.HostsEnrolledByOrbitVersion)
	assert.Equal(t, false, stats.AIFeaturesDisabled)
	assert.Equal(t, false, stats.MaintenanceWindowsEnabled)
	assert.Equal(t, false, stats.MaintenanceWindowsConfigured)
	assert.Equal(t, 1, stats.NumHostsMobiusDesktopEnabled)

	err = ds.RecordStatisticsSent(ctx)
	require.NoError(t, err)

	// If we try right away, it shouldn't ask to send
	stats, shouldSend, err = ds.ShouldSendStatistics(license.NewContext(ctx, premiumLicense), mobius.StatisticsFrequency, mobiusConfig)
	require.NoError(t, err)
	...
	assert.Equal(t, 5, stats.NumWeeklyPolicyViolationDaysActual)
	require.ElementsMatch(t, []mobius.HostsCountByOSVersion{
	...
	time.Sleep(1100 * time.Millisecond) // ensure the DB timestamp is not in the same second
	stats, shouldSend, err = ds.ShouldSendStatistics(license.NewContext(ctx, freeLicense), time.Millisecond, mobiusConfig)
	require.NoError(t, err)
	assert.True(t, shouldSend)
	assert.Equal(t, firstIdentifier, stats.AnonymousIdentifier)
	...
	mobiusConfig.Vulnerabilities = config.VulnerabilitiesConfig{DatabasesPath: "some/path/vulns"}
	stats, shouldSend, err = ds.ShouldSendStatistics(license.NewContext(ctx, freeLicense), time.Millisecond, mobiusConfig)
	require.NoError(t, err)
	assert.True(t, shouldSend)
