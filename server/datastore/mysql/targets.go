...
func (ds *Datastore) CountHostsInTargets(ctx context.Context, filter mobius.TeamFilter, targets mobius.HostTargets, now time.Time) (mobius.TargetMetrics, error) {
	...
	// As of Mobius 4.15, mia hosts are also included in the total for offline hosts
	sql := fmt.Sprintf(`
		SELECT
			COUNT(*) total,
		...
		WHERE %s AND %s`,
		mobius.OnlineIntervalBuffer, mobius.OnlineIntervalBuffer,
	...
	if err != nil {
		return mobius.TargetMetrics{}, ctxerr.Wrap(ctx, err, "sqlx.In CountHostsInTargets")
	}

	res := mobius.TargetMetrics{}
	err = sqlx.GetContext(ctx, ds.reader(ctx), &res, query, args...)
	if err != nil {
		return mobius.TargetMetrics{}, ctxerr.Wrap(ctx, err, "sqlx.Get CountHostsInTargets")
	}
...
func targetSQLCondAndArgs(targets mobius.HostTargets) (sql string, args []interface{}) {
...
func (ds *Datastore) HostIDsInTargets(ctx context.Context, filter mobius.TeamFilter, targets mobius.HostTargets) ([]uint, error) {
	if len(targets.HostIDs) == 0 && len(targets.LabelIDs) == 0 && len(targets.TeamIDs) == 0 {
		// No need to query if no targets selected
		return []uint{}, nil
	}
