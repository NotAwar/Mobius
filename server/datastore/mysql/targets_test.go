...
func testTargetsCountHosts(t *testing.T, ds *Datastore) {
	...
	hostCount := 0
	initHost := func(seenTime time.Time, distributedInterval uint, configTLSRefresh uint, teamID *uint) *mobius.Host {
		hostCount += 1
	...
	require.NoError(t, ds.ApplyLabelSpecs(context.Background(), []*mobius.LabelSpec{&l1, &l2}))
	l1.ID = labelIDFromName(t, ds, l1.Name)
	l2.ID = labelIDFromName(t, ds, l2.Name)

	for _, h := range []*mobius.Host{h1, h2, h3, h6} {
		err = ds.RecordLabelQueryExecutions(context.Background(), h, map[uint]*bool{l1.ID: ptr.Bool(true)}, mockClock.Now(), false)
		assert.Nil(t, err)
	}

	for _, h := range []*mobius.Host{h3, h4, h5} {
		err = ds.RecordLabelQueryExecutions(context.Background(), h, map[uint]*bool{l2.ID: ptr.Bool(true)}, mockClock.Now(), false)
		assert.Nil(t, err)
	}

	metrics, err := ds.CountHostsInTargets(context.Background(), filter, mobius.HostTargets{LabelIDs: []uint{l1.ID, l2.ID}}, mockClock.Now())
	require.Nil(t, err)
	assert.Equal(t, uint(6), metrics.TotalHosts)
	assert.Equal(t, uint(3), metrics.OfflineHosts) // metrics.MissingInActionHosts are also included in offline hosts as of Mobius 4.15
	assert.Equal(t, uint(3), metrics.OnlineHosts)
	assert.Equal(t, uint(1), metrics.MissingInActionHosts)

	metrics, err = ds.CountHostsInTargets(context.Background(), filter, mobius.HostTargets{HostIDs: []uint{h1.ID, h2.ID}, LabelIDs: []uint{l1.ID, l2.ID}}, mockClock.Now())
	require.Nil(t, err)
	assert.Equal(t, uint(6), metrics.TotalHosts)
	assert.Equal(t, uint(3), metrics.OfflineHosts) // metrics.MissingInActionHosts are also included in offline hosts as of Mobius 4.15
	assert.Equal(t, uint(3), metrics.OnlineHosts)
	...
	metrics, err = ds.CountHostsInTargets(context.Background(), filter, mobius.HostTargets{HostIDs: []uint{h1.ID, h2.ID}, LabelIDs: []uint{l1.ID, l2.ID}, TeamIDs: []uint{team1.ID, team2.ID}}, mockClock.Now())
	require.Nil(t, err)
	assert.Equal(t, uint(4), metrics.TotalHosts)
	assert.Equal(t, uint(2), metrics.OfflineHosts) // metrics.MissingInActionHosts are also included in offline hosts as of Mobius 4.15
	assert.Equal(t, uint(2), metrics.OnlineHosts)
	assert.Equal(t, uint(0), metrics.MissingInActionHosts)

	metrics, err = ds.CountHostsInTargets(context.Background(), filter, mobius.HostTargets{HostIDs: []uint{h1.ID, h2.ID}}, mockClock.Now())
	require.Nil(t, err)
	...
	metrics, err = ds.CountHostsInTargets(context.Background(), filter, mobius.HostTargets{HostIDs: []uint{h1.ID, h2.ID}, TeamIDs: []uint{team2.ID}}, mockClock.Now())
	require.Nil(t, err)
	assert.Equal(t, uint(4), metrics.TotalHosts)
	assert.Equal(t, uint(2), metrics.OnlineHosts)
	...
	metrics, err = ds.CountHostsInTargets(context.Background(), filter, mobius.HostTargets{HostIDs: []uint{h1.ID}, LabelIDs: []uint{l2.ID}}, mockClock.Now())
	require.Nil(t, err)
	assert.Equal(t, uint(4), metrics.TotalHosts)
	assert.Equal(t, uint(3), metrics.OnlineHosts)
	assert.Equal(t, uint(1), metrics.OfflineHosts)
	...
	metrics, err = ds.CountHostsInTargets(context.Background(), filter, mobius.HostTargets{}, mockClock.Now())
	require.Nil(t, err)
	...
	metrics, err = ds.CountHostsInTargets(context.Background(), filter, mobius.HostTargets{HostIDs: []uint{}, LabelIDs: []uint{}, TeamIDs: []uint{}}, mockClock.Now())
	require.Nil(t, err)
	assert.Equal(t, uint(0), metrics.TotalHosts)
	assert.Equal(t, uint(0), metrics.OnlineHosts)
	assert.Equal(t, uint(0), metrics.OfflineHosts)
	...
	// Get 'No team' hosts
	metrics, err = ds.CountHostsInTargets(context.Background(), filter, mobius.HostTargets{TeamIDs: []uint{0}}, mockClock.Now())
	require.Nil(t, err)
	assert.Equal(t, uint(2), metrics.TotalHosts)
	assert.Equal(t, uint(1), metrics.OnlineHosts)
	assert.Equal(t, uint(1), metrics.OfflineHosts)
	...
	metrics, err = ds.CountHostsInTargets(
		context.Background(), filter, mobius.HostTargets{TeamIDs: []uint{team1.ID, team3.ID, 0}}, mockClock.Now(),
	)
	require.Nil(t, err)
	...
	metrics, err = ds.CountHostsInTargets(context.Background(), filter, mobius.HostTargets{TeamIDs: []uint{team1.ID, team3.ID}}, mockClock.Now())
	require.Nil(t, err)
	...
	metrics, err = ds.CountHostsInTargets(context.Background(), filter, mobius.HostTargets{TeamIDs: []uint{team2.ID}}, mockClock.Now())
	require.Nil(t, err)
	assert.Equal(t, uint(3), metrics.TotalHosts)
	assert.Equal(t, uint(1), metrics.OnlineHosts)
	assert.Equal(t, uint(2), metrics.OfflineHosts)
	...
	metrics, err = ds.CountHostsInTargets(context.Background(), filter, mobius.HostTargets{LabelIDs: []uint{l1.ID}, TeamIDs: []uint{team2.ID}}, mockClock.Now())
	require.Nil(t, err)
	assert.Equal(t, uint(2), metrics.TotalHosts)
	assert.Equal(t, uint(1), metrics.OnlineHosts)
	assert.Equal(t, uint(1), metrics.OfflineHosts)
	...
	metrics, err = ds.CountHostsInTargets(context.Background(), filter, mobius.HostTargets{TeamIDs: []uint{team3.ID}}, mockClock.Now())
	require.Nil(t, err)
	assert.Equal(t, uint(0), metrics.TotalHosts)
	assert.Equal(t, uint(0), metrics.OnlineHosts)
	assert.Equal(t, uint(0), metrics.OfflineHosts)
	...
	metrics, err = ds.CountHostsInTargets(context.Background(), filter, mobius.HostTargets{LabelIDs: []uint{l1.ID, l2.ID}, TeamIDs: []uint{team3.ID}}, mockClock.Now())
	require.Nil(t, err)
	assert.Equal(t, uint(0), metrics.TotalHosts)
	assert.Equal(t, uint(0), metrics.OnlineHosts)
	assert.Equal(t, uint(0), metrics.OfflineHosts)
	...
	metrics, err = ds.CountHostsInTargets(context.Background(), filter, mobius.HostTargets{LabelIDs: []uint{l1.ID}, TeamIDs: []uint{team1.ID}}, mockClock.Now())
	require.Nil(t, err)
	assert.Equal(t, uint(1), metrics.TotalHosts)
	assert.Equal(t, uint(1), metrics.OnlineHosts)
	assert.Equal(t, uint(0), metrics.OfflineHosts)
	...
	metrics, err = ds.CountHostsInTargets(context.Background(), filter, mobius.HostTargets{LabelIDs: []uint{l2.ID}, TeamIDs: []uint{team2.ID}}, mockClock.Now())
	require.Nil(t, err)
	assert.Equal(t, uint(2), metrics.TotalHosts)
	assert.Equal(t, uint(1), metrics.OnlineHosts)
	assert.Equal(t, uint(1), metrics.OfflineHosts)
	...
	metrics, err = ds.CountHostsInTargets(context.Background(), filter, mobius.HostTargets{HostIDs: []uint{h1.ID}, LabelIDs: []uint{l2.ID}, TeamIDs: []uint{team2.ID}}, mockClock.Now())
	require.Nil(t, err)
	assert.Equal(t, uint(3), metrics.TotalHosts)
	assert.Equal(t, uint(2), metrics.OnlineHosts)
	assert.Equal(t, uint(1), metrics.OfflineHosts)
	...
	metrics, err = ds.CountHostsInTargets(context.Background(), filter, mobius.HostTargets{TeamIDs: []uint{team1.ID, team2.ID}}, mockClock.Now())
	require.Nil(t, err)
	assert.Equal(t, uint(4), metrics.TotalHosts)
	assert.Equal(t, uint(2), metrics.OnlineHosts)
	assert.Equal(t, uint(2), metrics.OfflineHosts)
	...
	assert.Equal(t, uint(6), metrics.TotalHosts)
	assert.Equal(t, uint(0), metrics.OnlineHosts)
	assert.Equal(t, uint(6), metrics.OfflineHosts) // metrics.MissingInActionHosts are also included in offline hosts as of Mobius 4.15
	assert.Equal(t, uint(1), metrics.MissingInActionHosts)
	...
	userTeam2 := &mobius.User{Teams: []mobius.UserTeam{{Team: *team2, Role: mobius.RoleAdmin}}}
...
func testTargetsHostStatus(t *testing.T, ds *Datastore) {
	...
	expectOnline := mobius.TargetMetrics{TotalHosts: 1, OnlineHosts: 1}
	expectOffline := mobius.TargetMetrics{TotalHosts: 1, OfflineHosts: 1}
	expectMIA := mobius.TargetMetrics{
		TotalHosts:           1,
		OfflineHosts:         1, // MissingInActionHosts are also included in offline hosts as of Mobius 4.15
	...
	for _, tt := range testCases {
		t.Run("", func(t *testing.T) {
			...
			// Verify status
			metrics, err := ds.CountHostsInTargets(context.Background(), filter, mobius.HostTargets{HostIDs: []uint{h.ID}}, mockClock.Now())
			require.NoError(t, err)
...
func testTargetsHostIDsInTargets(t *testing.T, ds *Datastore) {
	...
	hostCount := 0
	initHost := func(teamID *uint, platform string) *mobius.Host {
		hostCount += 1
		h, err := ds.NewHost(context.Background(), &mobius.Host{
			OsqueryHostID:   ptr.String(strconv.Itoa(hostCount)),
			NodeKey:         ptr.String(strconv.Itoa(hostCount)),
	...
	t1, err := ds.NewTeam(context.Background(), &mobius.Team{Name: "team1"})
	require.NoError(t, err)
	t2, err := ds.NewTeam(context.Background(), &mobius.Team{Name: "team2"})
	require.NoError(t, err)
	t3, err := ds.NewTeam(context.Background(), &mobius.Team{Name: "team3"})
	require.NoError(t, err)
	...
	l1, err := ds.NewLabel(context.Background(), &mobius.Label{
		Name:      "label foo",
		Query:     "query foo",
		LabelType: mobius.LabelTypeRegular,
	})
	require.NoError(t, err)
	l2, err := ds.NewLabel(context.Background(), &mobius.Label{
		Name:      "label bar",
		Query:     "query bar",
		LabelType: mobius.LabelTypeRegular,
	})
	require.NoError(t, err)
	l3, err := ds.NewLabel(context.Background(), &mobius.Label{
		Name:      "label zoo",
		Query:     "query zoo",
		LabelType: mobius.LabelTypeRegular,
	})
	...
	for _, h := range []*mobius.Host{h1, h2, h3, h4, h5, h6} {
	...
	// observer not included
	ids, err := ds.HostIDsInTargets(context.Background(), filter, mobius.HostTargets{HostIDs: []uint{1, 6}, LabelIDs: []uint{l2.ID}})
	require.Nil(t, err)
	assert.Len(t, ids, 0)
...
func testTargetsHostIDsInTargetsTeam(t *testing.T, ds *Datastore) {
	...
	team1, err := ds.NewTeam(context.Background(), &mobius.Team{Name: t.Name() + "team1"})
	require.NoError(t, err)
	team2, err := ds.NewTeam(context.Background(), &mobius.Team{Name: t.Name() + "team2"})
	require.NoError(t, err)

	h1 := initHost(mockClock.Now().Add(-1*time.Second), 10, 60, &team1.ID)
	initHost(mockClock.Now().Add(-1*time.Second), 10, 60, &team2.ID)

	targets, err := ds.HostIDsInTargets(context.Background(), filter, mobius.HostTargets{TeamIDs: []uint{team1.ID}})
	require.NoError(t, err)
	...
	targets, err = ds.HostIDsInTargets(context.Background(), filter, mobius.HostTargets{TeamIDs: []uint{team1.ID, team2.ID}})
	require.NoError(t, err)
	assert.Equal(t, []uint{h1.ID}, targets)
}
