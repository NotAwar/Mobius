...
func (ds *Datastore) NewTeam(ctx context.Context, team *mobius.Team) (*mobius.Team, error) {
...
func (ds *Datastore) Team(ctx context.Context, tid uint) (*mobius.Team, error) {
	return teamDB(ctx, ds.reader(ctx), tid, true)
}

func (ds *Datastore) TeamWithoutExtras(ctx context.Context, tid uint) (*mobius.Team, error) {
	return teamDB(ctx, ds.reader(ctx), tid, false)
}

func teamDB(ctx context.Context, q sqlx.QueryerContext, tid uint, withExtras bool) (*mobius.Team, error) {
	stmt := `
	...
	if withExtras {
		if err := loadSecretsForTeamsDB(ctx, q, []*mobius.Team{team}); err != nil {
...
func saveTeamSecretsDB(ctx context.Context, q sqlx.ExtContext, team *mobius.Team) error {
	if team.Secrets == nil {
		return nil
	}
...
func (ds *Datastore) DeleteTeam(ctx context.Context, tid uint) error {
	return ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		...
		_, err = tx.ExecContext(ctx, `DELETE FROM pack_targets WHERE type=? AND target_id=?`, mobius.TargetTeam, tid)
		if err != nil {
...
func (ds *Datastore) TeamByName(ctx context.Context, name string) (*mobius.Team, error) {
	// We must normalize the name for full Unicode support (Unicode equivalence).
...
func (ds *Datastore) loadExtrasForTeam(ctx context.Context, team *mobius.Team) (*mobius.Team, error) {
	if err := loadSecretsForTeamsDB(ctx, ds.reader(ctx), []*mobius.Team{team}); err != nil {
		return nil, ctxerr.Wrap(ctx, err, "getting secrets for teams")
	}
...
func (ds *Datastore) TeamByFilename(ctx context.Context, filename string) (*mobius.Team, error) {
	stmt := `
		SELECT ` + teamColumns + ` FROM teams
			WHERE filename = ?
	`
...
func loadUsersForTeamDB(ctx context.Context, q sqlx.QueryerContext, team *mobius.Team) error {
	sql := `
		SELECT u.name, u.id, u.email, ut.role
		FROM user_teams ut JOIN users u ON (ut.user_id = u.id)
		WHERE ut.team_id = ?
	`
	rows := []mobius.TeamUser{}
...
func loadHostCountForTeamDB(ctx context.Context, q sqlx.QueryerContext, team *mobius.Team) error {
	stmt := `
...
func loadFeaturesForTeamDB(ctx context.Context, q sqlx.QueryerContext, team *mobius.Team) error {
...
func saveUsersForTeamDB(ctx context.Context, exec sqlx.ExecerContext, team *mobius.Team) error {
...
func (ds *Datastore) SaveTeam(ctx context.Context, team *mobius.Team) (*mobius.Team, error) {
	// We must normalize the name for full Unicode support (Unicode equivalence).
	team.Name = norm.NFC.String(team.Name)
	err := ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		query := `
...
// mobius.ListOptions
func (ds *Datastore) ListTeams(ctx context.Context, filter mobius.TeamFilter, opt mobius.ListOptions) ([]*mobius.Team, error) {
	query := fmt.Sprintf(`
			SELECT `+teamColumns+`,
...
func loadSecretsForTeamsDB(ctx context.Context, q sqlx.QueryerContext, teams []*mobius.Team) error {
...
func (ds *Datastore) TeamsSummary(ctx context.Context) ([]*mobius.TeamSummary, error) {
	teamsSummary := []*mobius.TeamSummary{}
...
func (ds *Datastore) SearchTeams(ctx context.Context, filter mobius.TeamFilter, matchQuery string, omit ...uint) ([]*mobius.Team, error) {
	sql := fmt.Sprintf(`
			SELECT %s,
				(SELECT count(*) FROM user_teams WHERE team_id = t.id) AS user_count,
				(SELECT count(*) FROM hosts WHERE team_id = t.id) AS host_count
...
func (ds *Datastore) TeamEnrollSecrets(ctx context.Context, teamID uint) ([]*mobius.EnrollSecret, error) {
	sql := `
...
func (ds *Datastore) TeamFeatures(ctx context.Context, tid uint) (*mobius.Features, error) {
	return teamFeaturesDB(ctx, ds.reader(ctx), tid)
}

func teamFeaturesDB(ctx context.Context, q sqlx.QueryerContext, tid uint) (*mobius.Features, error) {
...
func (ds *Datastore) TeamMDMConfig(ctx context.Context, tid uint) (*mobius.TeamMDM, error) {
...
// that uses it.
func (ds *Datastore) DeleteIntegrationsFromTeams(ctx context.Context, deletedIntgs mobius.Integrations) error {
	const (
	...
	for rows.Next() {
		...
		if matches, _ := tm.Config.Integrations.MatchWithIntegrations(deletedIntgs); len(matches.Jira)+len(matches.Zendesk) > 0 {
			delJira, _ := mobius.IndexJiraIntegrations(matches.Jira)
			delZendesk, _ := mobius.IndexZendeskIntegrations(matches.Zendesk)

			var keepJira []*mobius.TeamJiraIntegration
			...
			var keepZendesk []*mobius.TeamZendeskIntegration
