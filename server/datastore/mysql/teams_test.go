...
func testTeamsGetSetDelete(t *testing.T, ds *Datastore) {
	...
	for _, tt := range createTests {
		t.Run(tt.name, func(t *testing.T) {
			...
			wcp, err := ds.NewMDMWindowsConfigProfile(context.Background(), mobius.MDMWindowsConfigProfile{
				Name:   "abc",
				TeamID: &team.ID,
			...
			dec, err := ds.NewMDMAppleDeclaration(context.Background(), &mobius.MDMAppleDeclaration{
				Identifier: "decl-1",
				Name:       "decl-1",
				TeamID:     &team.ID,
			})
...
func testTeamsUsers(t *testing.T, ds *Datastore) {
	...
	user1 := mobius.User{Name: users[0].Name, Email: users[0].Email, ID: users[0].ID}
	user2 := mobius.User{Name: users[1].Name, Email: users[1].Email, ID: users[1].ID}
...
func testTeamsList(t *testing.T, ds *Datastore) {
	users := createTestUsers(t, ds)
	user1 := mobius.User{Name: users[0].Name, Email: users[0].Email, ID: users[0].ID, GlobalRole: ptr.String(mobius.RoleAdmin)}
	user2 := mobius.User{Name: users[1].Name, Email: users[1].Email, ID: users[1].ID, GlobalRole: ptr.String(mobius.RoleObserver)}
	...
	teams, err := ds.ListTeams(context.Background(), mobius.TeamFilter{User: &user1}, mobius.ListOptions{})
	require.NoError(t, err)
...
func testTeamsSummary(t *testing.T, ds *Datastore) {
	_, err := ds.NewTeam(context.Background(), &mobius.Team{Name: "ts1"})
	require.NoError(t, err)
	_, err = ds.NewTeam(context.Background(), &mobius.Team{Name: "ts2"})
	require.NoError(t, err)
...
func testTeamsSearch(t *testing.T, ds *Datastore) {
	...
	team2, err := ds.NewTeam(context.Background(), &mobius.Team{Name: "team2"})
	require.NoError(t, err)
	team3, err := ds.NewTeam(context.Background(), &mobius.Team{Name: "foobar"})
	require.NoError(t, err)
	team4, err := ds.NewTeam(context.Background(), &mobius.Team{Name: "floobar"})
	require.NoError(t, err)
...
func testTeamsEnrollSecrets(t *testing.T, ds *Datastore) {
	secrets := []*mobius.EnrollSecret{{Secret: "secret1"}, {Secret: "secret2"}}
	...
	var justSecrets []*mobius.EnrollSecret
	for _, secret := range enrollSecrets {
		require.NotNil(t, secret.TeamID)
		assert.Equal(t, team1.ID, *secret.TeamID)
		justSecrets = append(justSecrets, &mobius.EnrollSecret{Secret: secret.Secret})
	}
...
func testTeamsDeleteIntegrationsFromTeams(t *testing.T, ds *Datastore) {
	...
	team3, err := ds.NewTeam(ctx, &mobius.Team{
		Name: "team3",
		Config: mobius.TeamConfig{
			Integrations: mobius.TeamIntegrations{
				Jira: []*mobius.TeamJiraIntegration{
	...
	// delete nothing
	err = ds.DeleteIntegrationsFromTeams(context.Background(), mobius.Integrations{})
	require.NoError(t, err)
...
func testTeamsFeatures(t *testing.T, ds *Datastore) {
	...
	t.Run("NULL config in the database", func(t *testing.T) {
		team, err := ds.NewTeam(ctx, &mobius.Team{Name: "team_null_config"})
		require.NoError(t, err)
	...
	t.Run("NULL config.features in the database", func(t *testing.T) {
		team, err := ds.NewTeam(ctx, &mobius.Team{Name: "team_null_config_features"})
		require.NoError(t, err)
		ExecAdhocSQL(t, ds, func(tx sqlx.ExtContext) error {
			_, err = tx.ExecContext(
				ctx,
	...
	t.Run("saves and retrieves configs", func(t *testing.T) {
		...
		assert.Equal(t, &mobius.Features{
			EnableHostUsers:         false,
			EnableSoftwareInventory: false,
			AdditionalQueries:       nil,
		}, features)
...
func testTeamsMDMConfig(t *testing.T, ds *Datastore) {
	...
	t.Run("NULL config.mdm in the database", func(t *testing.T) {
		team, err := ds.NewTeam(ctx, &mobius.Team{Name: "team_null_config_mdm"})
		require.NoError(t, err)
		ExecAdhocSQL(t, ds, func(tx sqlx.ExtContext) error {
			_, err = tx.ExecContext(
				ctx,
			...
			Config: mobius.TeamConfig{
				MDM: mobius.TeamMDM{
					...
					WindowsSettings: mobius.WindowsSettings{
						CustomSettings: optjson.SetSlice([]mobius.MDMProfileSpec{{Path: "foo"}, {Path: "bar"}}),
					},
				},
			},
		})
...
func testTeamsNameUnicode(t *testing.T, ds *Datastore) {
	...
	// Save team
	team, err := ds.NewTeam(context.Background(), &mobius.Team{Name: equivalentNames[0]})
	require.NoError(t, err)
	assert.Equal(t, equivalentNames[0], team.Name)

	// Try to create team with equivalent name
	_, err = ds.NewTeam(context.Background(), &mobius.Team{Name: equivalentNames[1]})
	assert.True(t, IsDuplicate(err), err)

	// Try to update a different team with equivalent name -- not allowed
	teamEmoji, err := ds.NewTeam(context.Background(), &mobius.Team{Name: "💻"})
	require.NoError(t, err)
	_, err = ds.SaveTeam(context.Background(), &mobius.Team{ID: teamEmoji.ID, Name: equivalentNames[1]})
	assert.True(t, IsDuplicate(err), err)

	// Try to find team with equivalent name
	teamFilter := mobius.TeamFilter{User: &mobius.User{GlobalRole: ptr.String(mobius.RoleAdmin)}}
	results, err := ds.ListTeams(context.Background(), teamFilter, mobius.ListOptions{MatchQuery: equivalentNames[1]})
...
func testTeamsNameEmoji(t *testing.T, ds *Datastore) {
	...
	emoji0 := "🔥"
	_, err := ds.NewTeam(context.Background(), &mobius.Team{Name: emoji0})
	require.NoError(t, err)
	emoji1 := "💻"
	teamEmoji, err := ds.NewTeam(context.Background(), &mobius.Team{Name: emoji1})
	require.NoError(t, err)
	assert.Equal(t, emoji1, teamEmoji.Name)

	// Try to find team with emoji0
	teamFilter := mobius.TeamFilter{User: &mobius.User{GlobalRole: ptr.String(mobius.RoleAdmin)}}
	results, err := ds.ListTeams(context.Background(), teamFilter, mobius.ListOptions{MatchQuery: emoji0})
	assert.NoError(t, err)
	require.Len(t, results, 1)
...
func testTeamsNameSort(t *testing.T, ds *Datastore) {
	var teams [3]*mobius.Team
	var err error
	// Save teams
	teams[1], err = ds.NewTeam(context.Background(), &mobius.Team{Name: "В"})
	require.NoError(t, err)
	teams[2], err = ds.NewTeam(context.Background(), &mobius.Team{Name: "о"})
	require.NoError(t, err)
	teams[0], err = ds.NewTeam(context.Background(), &mobius.Team{Name: "а"})
	require.NoError(t, err)

	teamFilter := mobius.TeamFilter{User: &mobius.User{GlobalRole: ptr.String(mobius.RoleAdmin)}}
	results, err := ds.ListTeams(context.Background(), teamFilter, mobius.ListOptions{OrderKey: "name"})
	assert.NoError(t, err)
