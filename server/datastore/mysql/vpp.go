...
import (
	...
	"github.com/notawar/mobius/v4/pkg/automatic_policy"
	"github.com/notawar/mobius set/v4/server/authz"
...
WHERE
	...
	var exclAny, inclAny []mobius.SoftwareScopeLabel
...
func (ds *Datastore) getVPPAppLabels(ctx context.Context, vppAppsTeamsID uint) ([]mobius.SoftwareScopeLabel, error) {
	query := `
SELECT
	label_id,
	exclude,
...
`
	...
	var labels []mobius.SoftwareScopeLabel
...
func (ds *Datastore) GetSummaryHostVPPAppInstalls(ctx context.Context, teamID *uint, appID mobius.VPPAppID) (*mobius.VPPAppStatusSummary,
	error,
) {
	var dest mobius.VPPAppStatusSummary
...
func (ds *Datastore) BatchInsertVPPApps(ctx context.Context, apps []*mobius.VPPApp) error {
	return ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		for _, app := range apps {
			...
			if err := insertVPPApps(ctx, tx, []*mobius.VPPApp{app}); err != nil {
...
func (ds *Datastore) getExistingLabels(ctx context.Context, vppAppTeamID uint) (*mobius.LabelIdentsWithScope, error) {
	existingLabels, err := ds.getVPPAppLabels(ctx, vppAppTeamID)
	if err != nil {
		return nil, ctxerr.Wrap(ctx, err, "getting existing labels")
	}

	var labels mobius.LabelIdentsWithScope
	var exclAny, inclAny []mobius.SoftwareScopeLabel
	...
	switch {
	case len(exclAny) > 0:
		labels.LabelScope = mobius.LabelScopeExcludeAny
		labels.ByName = make(map[string]mobius.LabelIdent, len(exclAny))
		for _, l := range exclAny {
			labels.ByName[l.LabelName] = mobius.LabelIdent{LabelName: l.LabelName, LabelID: l.LabelID}
		}
		return &labels, nil

	case len(inclAny) > 0:
		labels.LabelScope = mobius.LabelScopeIncludeAny
		labels.ByName = make(map[string]mobius.LabelIdent, len(inclAny))
...
func (ds *Datastore) SetTeamVPPApps(ctx context.Context, teamID *uint, appMobiuss []mobius.VPPAppTeam) error {
	...
	// called from mobiuscli gitops, so it should always be the case anyway.
	var replacingInstallDuringSetup bool
	if len(appMobiuss) == 0 || appMobiuss[0].InstallDuringSetup != nil {
		replacingInstallDuringSetup = true
	}

	var toAddApps []mobius.VPPAppTeam
	var toRemoveApps []mobius.VPPAppID

	for existingApp, appTeamInfo := range existingApps {
		var found bool
		for _, appMobius := range appMobiuss {
			// Self service value doesn't matter for removing app from team
			if existingApp == appMobius.VPPAppID {
				found = true
			}
		}
		if !found {
	...
	for _, appMobius := range appMobiuss {
		...
		existingApp, isExistingApp := existingApps[appMobius.VPPAppID]
		appMobius.AppTeamID = existingApp.AppTeamID
		var labelsChanged, categoriesChanged bool
		if isExistingApp {
			existingLabels, err := ds.getExistingLabels(ctx, appMobius.AppTeamID)
			if err != nil {
				return ctxerr.Wrap(ctx, err, "getting existing labels for vpp app")
			}

			labelsChanged = !existingLabels.Equal(appMobius.ValidatedLabels)

			existingCatIDs, err := ds.getVPPAppTeamCategoryIDs(ctx, appMobius.AppTeamID)
			if err != nil {
				return ctxerr.Wrap(ctx, err, "getting existing categories for vpp app")
			}

			categoriesChanged = !slices.Equal(existingCatIDs, appMobius.CategoryIDs)

		}

		// Get the hosts that are NOT in label scope currently (before the update happens)
		if labelsChanged {
			hostsNotInScope, err := ds.GetExcludedHostIDMapForVPPApp(ctx, appMobius.AppTeamID)
			if err != nil {
				return ctxerr.Wrap(ctx, err, "getting hosts not in scope for vpp app")
			}
			appsWithChangedLabels[appMobius.AppTeamID] = hostsNotInScope

		}

		if !isExistingApp ||
			existingApp.SelfService != appMobius.SelfService ||
			labelsChanged ||
			categoriesChanged ||
			appMobius.InstallDuringSetup != nil &&
				existingApp.InstallDuringSetup != nil &&
				*appMobius.InstallDuringSetup != *existingApp.InstallDuringSetup {
			toAddApps = append(toAddApps, appMobius)
		}
	}

	var vppToken *mobius.VPPTokenDB
	if len(appMobiuss) > 0 {
...
func (ds *Datastore) InsertVPPAppWithTeam(ctx context.Context, app *mobius.VPPApp, teamID *uint) (*mobius.VPPApp, error) {
	vppToken, err := ds.GetVPPTokenByTeamID(ctx, teamID)
	if err != nil {
...
func (ds *Datastore) GetVPPApps(ctx context.Context, teamID *uint) ([]mobius.VPPAppResponse, error) {
	var tmID uint
	if teamID != nil {
		tmID = *teamID
	}
	var results []mobius.VPPAppResponse
...
func (ds *Datastore) GetAssignedVPPApps(ctx context.Context, teamID *uint) (map[mobius.VPPAppID]mobius.VPPAppTeam, error) {
	stmt := `
SELECT
...
WHERE
	...
	appSet := make(map[mobius.VPPAppID]mobius.VPPAppTeam)
...
func (ds *Datastore) InsertVPPApps(ctx context.Context, apps []*mobius.VPPApp) error {
	return ds.withRetryTxx(ctx, func(tx sqlx.ExtContext) error {
		return insertVPPApps(ctx, tx, apps)
	})
}

func insertVPPApps(ctx context.Context, tx sqlx.ExtContext, apps []*mobius.VPPApp) error {
	stmt := `
...
func insertVPPAppTeams(ctx context.Context, tx sqlx.ExtContext, appID mobius.VPPAppTeam, teamID *uint, vppTokenID uint) (uint, error) {
	stmt := `
...
func removeVPPAppTeams(ctx context.Context, tx sqlx.ExtContext, appID mobius.VPPAppID, teamID *uint) error {
...
func (ds *Datastore) getOrInsertSoftwareTitleForVPPApp(ctx context.Context, tx sqlx.ExtContext, app *mobius.VPPApp) (uint, error) {
	...
	var source string
	switch app.Platform {
	case mobius.IOSPlatform:
		source = "ios_apps"
	case mobius.IPadOSPlatform:
		source = "ipados_apps"
	default:
		source = "apps"
	}
...
func (ds *Datastore) DeleteVPPAppFromTeam(ctx context.Context, teamID *uint, appID mobius.VPPAppID) error {
...
func (ds *Datastore) GetTitleInfoFromVPPAppsTeamsID(ctx context.Context, vppAppsTeamsID uint) (*mobius.PolicySoftwareTitle, error) {
	var info mobius.PolicySoftwareTitle
...
func (ds *Datastore) GetVPPAppMetadataByAdamIDPlatformTeamID(ctx context.Context, adamID string, platform mobius.AppleDevicePlatform, teamID *uint) (*mobius.VPPApp, error) {
	stmt := `
	SELECT va.adam_id,
...
func (ds *Datastore) GetVPPAppByTeamAndTitleID(ctx context.Context, teamID *uint, titleID uint) (*mobius.VPPApp, error) {
	stmt := `
SELECT
  vat.id,
  va.adam_id,
...
func (ds *Datastore) InsertHostVPPSoftwareInstall(ctx context.Context, hostID uint, appID mobius.VPPAppID,
	commandUUID, associatedEventID string, opts mobius.HostSoftwareInstallOptions,
) error {
	const (
		insertUAStmt = `
INSERT INTO upcoming_activities
	(host_id, priority, user_id, mobius_initiated, activity_type, execution_id, payload)
VALUES
	(?, ?, ?, ?, 'vpp_app_install', ?,
		JSON_OBJECT(
			'self_service', ?,
...
func (ds *Datastore) GetPastActivityDataForVPPAppInstall(ctx context.Context, commandResults *mdm.CommandResults) (*mobius.User, *mobius.ActivityInstalledAppStoreApp, error) {
...
WHERE
	...
	listStmt, args, err := sqlx.Named(stmt, map[string]any{
		...
		"software_status_failed":    string(mobius.SoftwareInstallFailed),
		"software_status_installed": string(mobius.SoftwareInstalled),
	})
	if err != nil {
	...
	case mobius.MDMAppleStatusCommandFormatError:
	case mobius.MDMAppleStatusError:
	...
	default:
		...
		status = string(mobius.SoftwareInstallPending)
	}

	act := &mobius.ActivityInstalledAppStoreApp{
		HostID:          res.HostID,
...
func (ds *Datastore) GetVPPTokenByLocation(ctx context.Context, loc string) (*mobius.VPPTokenDB, error) {
...
func (ds *Datastore) InsertVPPToken(ctx context.Context, tok *mobius.VPPTokenData) (*mobius.VPPTokenDB, error) {
	insertStmt := `
	INSERT INTO
		vpp_tokens (
			organization_name,
...
func (ds *Datastore) UpdateVPPToken(ctx context.Context, tokenID uint, tok *mobius.VPPTokenData) (*mobius.VPPTokenDB, error) {
	stmt := `
	UPDATE vpp_tokens
	SET
		organization_name = ?,
...
func (ds *Datastore) GetVPPToken(ctx context.Context, tokenID uint) (*mobius.VPPTokenDB, error) {
	stmt := `
	SELECT
		id,
		organization_name,
...
TEAMLOOP:
	for _, team := range tokTeams {
		...
		case mobius.NullTeamAllTeams:
			...
			tok.Teams = []mobius.TeamTuple{}
			break TEAMLOOP
		case mobius.NullTeamNoTeam:
			tok.Teams = append(tok.Teams, mobius.TeamTuple{
				ID:   0,
				Name: mobius.TeamNameNoTeam,
			})
		case mobius.NullTeamNone:
			// Regular team
			tok.Teams = append(tok.Teams, mobius.TeamTuple{
				ID:   *team.TeamID,
				Name: team.Name,
...
func (ds *Datastore) UpdateVPPTokenTeams(ctx context.Context, id uint, teams []uint) (*mobius.VPPTokenDB, error) {
	...
	var nullTeamCheck mobius.NullTeamType
	...
	if len(teams) > 0 {
		for _, team := range teams {
			...
			var teamptr *uint
			nullTeam := mobius.NullTeamNone
			if team != 0 {
				// Regular team
				teamptr = &team
			} else {
				// NoTeam team
				nullTeam = mobius.NullTeamNoTeam
				nullTeamCheck = mobius.NullTeamNoTeam
			}
		...
		values = stmtValues
		args = append(args, id, nil, mobius.NullTeamAllTeams)
		nullTeamCheck = mobius.NullTeamAllTeams
	}
	...
	if err != nil {
		...
		if errors.As(err, &mysqlErr) && IsDuplicate(err) {
			...
			return nil, ctxerr.Wrap(ctx, mobius.ErrVPPTokenTeamConstraint{Name: dupeTeamName, ID: &dupeTeamID})
		}
...
func (ds *Datastore) ListVPPTokens(ctx context.Context) ([]*mobius.VPPTokenDB, error) {
...
`
	...
	tokens := map[uint]*mobius.VPPTokenDB{}
	...
	for _, tokEnc := range tokEncs {
		...
		tokens[tokEnc.ID] = &mobius.VPPTokenDB{
			ID:        tokEnc.ID,
			OrgName:   tokEnc.OrgName,
			Location:  tokEnc.Location,
			RenewDate: tokEnc.RenewDate,
	...
	for _, team := range teams {
		...
		case mobius.NullTeamAllTeams:
			...
			token.Teams = []mobius.TeamTuple{}
		case mobius.NullTeamNoTeam:
			token.Teams = append(token.Teams, mobius.TeamTuple{ID: 0, Name: mobius.TeamNameNoTeam})
		case mobius.NullTeamNone:
			// Regular team
			token.Teams = append(token.Teams, mobius.TeamTuple{ID: *team.TeamID, Name: team.TeamName})
		}
	}

	var outTokens []*mobius.VPPTokenDB
	for _, token := range tokens {
		outTokens = append(outTokens, token)
	}

	slices.SortFunc(outTokens, func(a, b *mobius.VPPTokenDB) int {
...
func (ds *Datastore) GetVPPTokenByTeamID(ctx context.Context, teamID *uint) (*mobius.VPPTokenDB, error) {
	stmtTeam := `
	SELECT
		v.id,
		v.organization_name,
...
`
	...
	if teamID != nil && *teamID != 0 {
		...
		err = sqlx.GetContext(ctx, ds.reader(ctx), &tokEnc, stmtNullTeam, mobius.NullTeamNoTeam)
	}
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			if err := sqlx.GetContext(ctx, ds.reader(ctx), &tokEnc, stmtNullTeam, mobius.NullTeamAllTeams); err != nil {
				if errors.Is(err, sql.ErrNoRows) {
...
func checkVPPNullTeam(ctx context.Context, tx sqlx.ExtContext, currentID *uint, nullTeam mobius.NullTeamType) error {
	...
	if nullTeam == mobius.NullTeamAllTeams {
		var ids []uint
	...
	var id uint
	allTeamsFound := true
	if err := sqlx.GetContext(ctx, tx, &id, nullTeamStmt, mobius.NullTeamAllTeams); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			allTeamsFound = false
		} else {
			return ctxerr.Wrap(ctx, err, "scanning row in check vpp token null team")
	...
	if allTeamsFound && currentID != nil && *currentID != id {
		return ctxerr.Wrap(ctx, mobius.ErrVPPTokenTeamConstraint{Name: mobius.ReservedNameAllTeams})
	}

	if nullTeam != mobius.NullTeamNone {
		var id uint
		...
		if currentID == nil || *currentID != id {
			return ctxerr.Wrap(ctx, mobius.ErrVPPTokenTeamConstraint{Name: nullTeam.PrettyName()})
		}
	}
...
func (ds *Datastore) GetAllVPPApps(ctx context.Context) ([]*mobius.VPPApp, error) {
	query := `
SELECT
    adam_id,
	title_id,
...
func (ds *Datastore) GetVPPInstallsByVerificationUUID(ctx context.Context, verificationUUID string) ([]*mobius.HostVPPSoftwareInstall, error) {
	stmt := `
SELECT
	hvsi.command_uuid AS command_uuid,
	hvsi.host_id AS host_id,
...
WHERE hvsi.verification_command_uuid = ?
	...
	var result []*mobius.HostVPPSoftwareInstall
