...
func testVPPAppMetadata(t *testing.T, ds *Datastore) {
	...
	va1, err := ds.InsertVPPAppWithTeam(ctx, &mobius.VPPApp{
		...
		VPPAppTeam: mobius.VPPAppTeam{VPPAppID: mobius.VPPAppID{AdamID: "adam_vpp_app_1", Platform: mobius.MacOSPlatform}, SelfService: true},
	}, nil)
	require.NoError(t, err)
	...
	require.Equal(t, &mobius.VPPAppStoreApp{Name: "vpp1", VPPAppID: vpp1, BundleIdentifier: "com.app.vpp1", SelfService: true}, meta)
	...
	require.Equal(t, mobius.VPPAppID{Platform: mobius.MacOSPlatform, AdamID: meta.AdamID}, gotMeta.VPPAppTeam.VPPAppID)
	require.True(t, gotMeta.SelfService)

	// try to add the same app again, update self_service field
	_, err = ds.InsertVPPAppWithTeam(ctx, &mobius.VPPApp{
		Name: "vpp1", BundleIdentifier: "com.app.vpp1",
		VPPAppTeam: mobius.VPPAppTeam{VPPAppID: mobius.VPPAppID{AdamID: "adam_vpp_app_1", Platform: mobius.MacOSPlatform}, SelfService: true},
	}, nil)
	require.NoError(t, err)
	...
	meta.AddedAt = time.Time{} // we don't care about AddedAt here
	require.Equal(t, &mobius.VPPAppStoreApp{Name: "vpp1", VPPAppID: vpp1, SelfService: true, BundleIdentifier: "com.app.vpp1"}, meta)
	...
	require.Equal(t, &mobius.VPPAppStoreApp{Name: "vpp2", VPPAppID: vpp2, BundleIdentifier: "com.app.vpp2"}, meta)
	...
	_, err = ds.InsertVPPAppWithTeam(ctx, &mobius.VPPApp{
		Name: "vpp2", BundleIdentifier: "com.app.vpp2",
		VPPAppTeam: mobius.VPPAppTeam{VPPAppID: mobius.VPPAppID{AdamID: "adam_vpp_app_2", Platform: mobius.MacOSPlatform}, SelfService: true},
	}, &team1.ID)
	require.NoError(t, err)
	...
	meta.VPPAppsTeamsID = 0
	meta.AddedAt = time.Time{}
	require.Equal(t, &mobius.VPPAppStoreApp{Name: "vpp2", VPPAppID: vpp2, SelfService: true, BundleIdentifier: "com.app.vpp2"}, meta)
	...
	meta.VPPAppsTeamsID = 0
	meta.AddedAt = time.Time{}
	require.Equal(t, &mobius.VPPAppStoreApp{Name: "vpp2", VPPAppID: vpp2, BundleIdentifier: "com.app.vpp2", SelfService: true}, meta)

	// create another no-team app
	va3, err := ds.InsertVPPAppWithTeam(ctx, &mobius.VPPApp{
		Name: "vpp3", BundleIdentifier: "com.app.vpp3",
		VPPAppTeam: mobius.VPPAppTeam{VPPAppID: mobius.VPPAppID{AdamID: "adam_vpp_app_3", Platform: mobius.MacOSPlatform}},
	}, nil)
	require.NoError(t, err)
	...
	meta.VPPAppsTeamsID = 0
	meta.AddedAt = time.Time{}
	require.Equal(t, &mobius.VPPAppStoreApp{Name: "vpp3", VPPAppID: vpp3, BundleIdentifier: "com.app.vpp3"}, meta)
...
func testVPPAppStatus(t *testing.T, ds *Datastore) {
	...
	require.NoError(t, err)
	require.Equal(t, &mobius.VPPAppStatusSummary{Pending: 0, Failed: 0, Installed: 0}, summary)
	...
	summary, err = ds.GetSummaryHostVPPAppInstalls(ctx, nil, vpp1)
	require.NoError(t, err)
	require.Equal(t, &mobius.VPPAppStatusSummary{Pending: 1, Failed: 0, Installed: 0}, summary)

	// record a failed result
	createVPPAppInstallResult(t, ds, h1, cmd1, mobius.MDMAppleStatusError)

	summary, err = ds.GetSummaryHostVPPAppInstalls(ctx, nil, vpp1)
	require.NoError(t, err)
	require.Equal(t, &mobius.VPPAppStatusSummary{Pending: 0, Failed: 1, Installed: 0}, summary)
	...
	createVPPAppInstallResult(t, ds, h2, cmd3, mobius.MDMAppleStatusAcknowledged)
	...
	summary, err = ds.GetSummaryHostVPPAppInstalls(ctx, nil, vpp1)
	require.NoError(t, err)
	require.Equal(t, &mobius.VPPAppStatusSummary{Pending: 1, Failed: 0, Installed: 1}, summary)

	// mark the pending request as successful too
	createVPPAppInstallResult(t, ds, h1, cmd2, mobius.MDMAppleStatusAcknowledged)

	summary, err = ds.GetSummaryHostVPPAppInstalls(ctx, nil, vpp1)
	require.NoError(t, err)
	require.Equal(t, &mobius.VPPAppStatusSummary{Pending: 0, Failed: 0, Installed: 2}, summary)
	...
	createVPPAppInstallResult(t, ds, h3, cmd4, mobius.MDMAppleStatusAcknowledged)

	summary, err = ds.GetSummaryHostVPPAppInstalls(ctx, &team1.ID, vpp2)
	require.NoError(t, err)
	require.Equal(t, &mobius.VPPAppStatusSummary{Pending: 0, Failed: 0, Installed: 1}, summary)
	...
	createVPPAppInstallResult(t, ds, h3, cmd5, mobius.MDMAppleStatusAcknowledged)
	cmd6 := createVPPAppInstallRequest(t, ds, h1, vpp3.AdamID, user)
	createVPPAppInstallResult(t, ds, h1, cmd6, mobius.MDMAppleStatusCommandFormatError)
	createVPPAppInstallRequest(t, ds, h2, vpp3.AdamID, user)

	// for no team, it sees the failed and pending counts
	summary, err = ds.GetSummaryHostVPPAppInstalls(ctx, nil, vpp3)
	require.NoError(t, err)
	require.Equal(t, &mobius.VPPAppStatusSummary{Pending: 1, Failed: 1, Installed: 0}, summary)
...
func createVPPAppInstallRequest(t *testing.T, ds *Datastore, host *mobius.Host, adamID string, user *mobius.User) string {
	ctx := context.Background()
	...
	err := ds.InsertHostVPPSoftwareInstall(ctx, host.ID, mobius.VPPAppID{
		AdamID:   adamID,
		Platform: mobius.AppleDevicePlatform(host.Platform),
	}, cmdUUID, eventID, mobius.HostSoftwareInstallOptions{})
	require.NoError(t, err)
	return cmdUUID
}

func createVPPAppInstallResult(t *testing.T, ds *Datastore, host *mobius.Host, cmdUUID string, status string) {
	ctx := context.Background()
...
func testVPPApps(t *testing.T, ds *Datastore) {
	...
	team, err := ds.NewTeam(ctx, &mobius.Team{Name: "foobar"})
	require.NoError(t, err)

	test.CreateInsertGlobalVPPToken(t, ds)

	t.Run("vpp apps with labels", func(t *testing.T) {
		teamWithLabels, err := ds.NewTeam(ctx, &mobius.Team{Name: "labels" + t.Name()})
		require.NoError(t, err)
		...
		labeledApp = &mobius.VPPApp{
			...
			VPPAppTeam: mobius.VPPAppTeam{
				VPPAppID: mobius.VPPAppID{AdamID: "6", Platform: mobius.MacOSPlatform},
				ValidatedLabels: &mobius.LabelIdentsWithScope{
	...
	app1 := &mobius.VPPApp{Name: "vpp_app_1", VPPAppTeam: mobius.VPPAppTeam{VPPAppID: mobius.VPPAppID{AdamID: "1", Platform: mobius.MacOSPlatform}}, BundleIdentifier: "b1"}
	app2 := &mobius.VPPApp{Name: "vpp_app_2", VPPAppTeam: mobius.VPPAppTeam{VPPAppID: mobius.VPPAppID{AdamID: "2", Platform: mobius.MacOSPlatform}}, BundleIdentifier: "b2"}
	...
	appNoTeam1 := &mobius.VPPApp{
		Name: "vpp_no_team_app_1", VPPAppTeam: mobius.VPPAppTeam{VPPAppID: mobius.VPPAppID{AdamID: "3", Platform: mobius.MacOSPlatform}},
		BundleIdentifier: "b3",
	}
	appNoTeam2 := &mobius.VPPApp{
		Name: "vpp_no_team_app_2", VPPAppTeam: mobius.VPPAppTeam{VPPAppID: mobius.VPPAppID{AdamID: "4", Platform: mobius.MacOSPlatform}},
		BundleIdentifier: "b4",
	}
	...
	u, err := ds.NewUser(ctx, &mobius.User{
		Password:   []byte("p4ssw0rd.123"),
		Name:       "user1",
		Email:      "user1@example.com",
		GlobalRole: ptr.String(mobius.RoleAdmin),
	...
	err = ds.InsertHostVPPSoftwareInstall(ctx, h1.ID, app1.VPPAppID, "a", "b", mobius.HostSoftwareInstallOptions{})
	require.NoError(t, err)

	// non-existing host
	err = ds.InsertHostVPPSoftwareInstall(ctx, h1.ID+1, app2.VPPAppID, "c", "d", mobius.HostSoftwareInstallOptions{SelfService: true})
	require.Error(t, err)
	var nfe mobius.NotFoundError
	require.ErrorAs(t, err, &nfe)
	...
	err = ds.InsertHostVPPSoftwareInstall(ctx, h2.ID, app2.VPPAppID, "c", "d", mobius.HostSoftwareInstallOptions{SelfService: true})
	require.NoError(t, err)

	acts, _, err := ds.ListHostUpcomingActivities(ctx, h1.ID, mobius.ListOptions{})
	require.NoError(t, err)
	...
	acts, _, err = ds.ListHostUpcomingActivities(ctx, h2.ID, mobius.ListOptions{})
	require.NoError(t, err)
	require.Len(t, acts, 1)
	require.NotNil(t, acts[0].ActorFullName)
	require.Equal(t, u.Name, *acts[0].ActorFullName)
	...
	assert.Equal(t, map[mobius.VPPAppID]mobius.VPPAppTeam{
	...
	var appTitles []mobius.SoftwareTitle
...
func testSetTeamVPPApps(t *testing.T, ds *Datastore) {
	...
	// Create a team
	team, err := ds.NewTeam(ctx, &mobius.Team{Name: "vpp gang"})
	require.NoError(t, err)
	...
	require.NoError(t, err)
	app3 := &mobius.VPPApp{Name: "vpp_app_3", VPPAppTeam: mobius.VPPAppTeam{VPPAppID: mobius.VPPAppID{AdamID: "3", Platform: mobius.MacOSPlatform}}, BundleIdentifier: "b3"}
	_, err = ds.InsertVPPAppWithTeam(ctx, app3, nil)
	require.NoError(t, err)
	app4 := &mobius.VPPApp{Name: "vpp_app_4", VPPAppTeam: mobius.VPPAppTeam{VPPAppID: mobius.VPPAppID{AdamID: "4", Platform: mobius.MacOSPlatform}}, BundleIdentifier: "b4"}
	...
	err = ds.SetTeamVPPApps(ctx, &team.ID, []mobius.VPPAppTeam{
	...
	require.NoError(t, err)
	policy1, err := ds.NewTeamPolicy(ctx, team.ID, nil, mobius.PolicyPayload{
		Name:           "Policy 1",
		Query:          "SELECT 1;",
		Platform:       "darwin",
	...
	require.NoError(t, err)
	policy2, err := ds.NewTeamPolicy(ctx, team.ID, nil, mobius.PolicyPayload{
		Name:           "Policy 2",
		Query:          "SELECT 1;",
		Platform:       "darwin",
...
func testGetVPPAppByTeamAndTitleID(t *testing.T, ds *Datastore) {
	...
	fooApp, err := ds.InsertVPPAppWithTeam(ctx,
		&mobius.VPPApp{VPPAppTeam: mobius.VPPAppTeam{VPPAppID: mobius.VPPAppID{AdamID: "foo", Platform: mobius.MacOSPlatform}}, BundleIdentifier: "b1", Name: "Foo"},
		&team.ID)
	require.NoError(t, err)
	...
	barApp, err := ds.InsertVPPAppWithTeam(ctx,
		&mobius.VPPApp{VPPAppTeam: mobius.VPPAppTeam{VPPAppID: mobius.VPPAppID{AdamID: "bar", Platform: mobius.MacOSPlatform}}, BundleIdentifier: "b2", Name: "Bar"}, nil)
	require.NoError(t, err)
...
func testVPPTokensCRUD(t *testing.T, ds *Datastore) {
	...
	team, err := ds.NewTeam(ctx, &mobius.Team{Name: "Kritters"})
	assert.NoError(t, err)

	team2, err := ds.NewTeam(ctx, &mobius.Team{Name: "Zingers"})
	assert.NoError(t, err)
	...
	assert.Equal(t, mobius.TeamNameNoTeam, upTok.Teams[0].Name)
	...
	assert.Equal(t, mobius.TeamNameNoTeam, tok.Teams[0].Name)
	...
	assert.Equal(t, mobius.TeamNameNoTeam, toks[0].Teams[0].Name)
	...
	assert.Equal(t, mobius.TeamNameNoTeam, teamTok.Teams[0].Name)
	...
	t1app, err := ds.InsertVPPAppWithTeam(ctx, &mobius.VPPApp{
		Name: "vpp1", BundleIdentifier: "com.app.vpp1",
		VPPAppTeam: mobius.VPPAppTeam{VPPAppID: mobius.VPPAppID{AdamID: "adam_vpp_app_1", Platform: mobius.MacOSPlatform}},
	}, &team.ID)
	require.NoError(t, err)
	...
	t1Policy, err := ds.NewTeamPolicy(ctx, team.ID, nil, mobius.PolicyPayload{
		Name:           "p1",
		Query:          "SELECT 1;",
	...
	t2app, err := ds.InsertVPPAppWithTeam(ctx, &mobius.VPPApp{
		Name: "vpp1", BundleIdentifier: "com.app.vpp1",
		VPPAppTeam: mobius.VPPAppTeam{VPPAppID: mobius.VPPAppID{AdamID: "adam_vpp_app_1", Platform: mobius.MacOSPlatform}},
	}, &team2.ID)
	require.NoError(t, err)
	...
	t2Policy, err := ds.NewTeamPolicy(ctx, team2.ID, nil, mobius.PolicyPayload{
		Name:           "p1",
		Query:          "SELECT 1;",
	...
	assert.Contains(t, tokTeams.Teams, mobius.TeamTuple{ID: team2.ID, Name: team2.Name})
	assert.Contains(t, tokTeams.Teams, mobius.TeamTuple{ID: 0, Name: mobius.TeamNameNoTeam})
	...
	assert.Contains(t, tokTeam2.Teams, mobius.TeamTuple{ID: team2.ID, Name: team2.Name})
	assert.Contains(t, tokTeam2.Teams, mobius.TeamTuple{ID: 0, Name: mobius.TeamNameNoTeam})
	...
	t1Policy2, err := ds.NewTeamPolicy(ctx, team.ID, nil, mobius.PolicyPayload{
		Name:           "t1p2",
		Query:          "SELECT 1;",
		VPPAppsTeamsID: &t1meta.VPPAppsTeamsID,
	})
...
func testVPPTokenAppTeamAssociations(t *testing.T, ds *Datastore) {
	...
	team1, err := ds.NewTeam(ctx, &mobius.Team{Name: "Kritters"})
	assert.NoError(t, err)
...
func testGetOrInsertSoftwareTitleForVPPApp(t *testing.T, ds *Datastore) {
	...
	sw, _, _, err := ds.ListSoftwareTitles(ctx, mobius.SoftwareTitleListOptions{}, mobius.TeamFilter{TeamID: host2.TeamID})
	require.NoError(t, err)
	require.Len(t, sw, 3)
	var tid uint
	for _, s := range sw {
	...
	for _, platform := range mobius.VPPAppsPlatforms {
...
func testDeleteVPPAssignedToPolicy(t *testing.T, ds *Datastore) {
	...
	p1, err := ds.NewTeamPolicy(ctx, mobius.PolicyNoTeamID, nil, mobius.PolicyPayload{
		Name:           "p1",
		Query:          "SELECT 1;",
	...
	_, err = ds.DeleteTeamPolicies(ctx, mobius.PolicyNoTeamID, []uint{p1.ID})
	require.NoError(t, err)
...
func testVPPTokenTeamAssignment(t *testing.T, ds *Datastore) {
	...
	team2, err := ds.NewTeam(ctx, &mobius.Team{Name: "team2" + t.Name()})
	require.NoError(t, err)
	...
	err = ds.SetTeamVPPApps(ctx, &team1.ID, []mobius.VPPAppTeam{
		{VPPAppID: app1.VPPAppID, InstallDuringSetup: ptr.Bool(true)},
		{VPPAppID: app2.VPPAppID, SelfService: true},
	})
	require.NoError(t, err)
...
func testSetTeamVPPAppsWithLabels(t *testing.T, ds *Datastore) {
	...
	// Create a team
	team, err := ds.NewTeam(ctx, &mobius.Team{Name: "vpp_app_labels" + t.Name()})
	require.NoError(t, err)
	...
	app1.VPPAppTeam = mobius.VPPAppTeam{VPPAppID: app1.VPPAppID, ValidatedLabels: &mobius.LabelIdentsWithScope{
		LabelScope: mobius.LabelScopeIncludeAny,
	...
	app2.VPPAppTeam = mobius.VPPAppTeam{VPPAppID: app2.VPPAppID, ValidatedLabels: &mobius.LabelIdentsWithScope{
		LabelScope: mobius.LabelScopeExcludeAny,
		ByName: map[string]mobius.LabelIdent{
			label1.Name: {
				LabelID:   label1.ID,
...
func testGetAllVPPApps(t *testing.T, ds *Datastore) {
	...
	require.Equal(t, apps, []*mobius.VPPApp{app1, app2, app3})
}
