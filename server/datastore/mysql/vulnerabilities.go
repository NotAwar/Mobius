...
func (ds *Datastore) Vulnerability(ctx context.Context, cve string, teamID *uint, includeCVEScores bool) (*mobius.VulnerabilityWithMetadata, error) {
	var vuln mobius.VulnerabilityWithMetadata
...
func (ds *Datastore) OSVersionsByCVE(ctx context.Context, cve string, teamID *uint) (vos []*mobius.VulnerableOS, updatedAt time.Time, err error) {
	var teamFilter *mobius.TeamFilter
	if teamID != nil {
		teamFilter = &mobius.TeamFilter{TeamID: teamID}
	}
	...
	for _, osv := range osvs.OSVersions {
		for _, id := range verResolvedDedup {
			if osv.OSVersionID == id.OSVersionID {
				vos = append(vos, &mobius.VulnerableOS{
					OSVersion:         osv,
...
func (ds *Datastore) SoftwareByCVE(ctx context.Context, cve string, teamID *uint) (vs []*mobius.VulnerableSoftware, updatedAt time.Time, err error) {
	var args []interface{}
	selectStmt := `
		SELECT
			s.id,
...
func (ds *Datastore) ListVulnerabilities(ctx context.Context, opt mobius.VulnListOptions) ([]mobius.VulnerabilityWithMetadata, *mobius.PaginationMetadata, error) {
	...
	// Execute the query
	var vulns []mobius.VulnerabilityWithMetadata
...
func (ds *Datastore) CountVulnerabilities(ctx context.Context, opt mobius.VulnListOptions) (uint, error) {
	selectStmt := `
		SELECT
			COUNT(*)
		FROM (
...
func (ds *Datastore) IsCVEKnownToMobius(ctx context.Context, cve string) (bool, error) {
	var count uint
