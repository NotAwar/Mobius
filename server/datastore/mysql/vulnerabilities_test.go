...
func testListVulnerabilities(t *testing.T, ds *Datastore) {
	...
	// insert OS Vuln
	_, err = ds.InsertOSVulnerabilities(context.Background(), []mobius.OSVulnerability{
		{
			OSID:              1,
			CVE:               "CVE-2020-1234",
	...
	_, err = ds.InsertSoftwareVulnerability(context.Background(), mobius.SoftwareVulnerability{
		SoftwareID: 1,
		CVE:        "CVE-2020-1236",
	}, mobius.NVDSource)
	require.NoError(t, err)
	...
	// insert CVEMeta
	err = ds.InsertCVEMeta(context.Background(), []mobius.CVEMeta{
		{
			CVE:              "CVE-2020-1234",
	...
	expected := map[string]mobius.VulnerabilityWithMetadata{
		"CVE-2020-1234": {
			CVE: mobius.CVE{
				CVE:              "CVE-2020-1234",
			...
			HostsCount: 10,
			Source:     mobius.MSRCSource,
		},
		"CVE-2020-1235": {
			CVE:        mobius.CVE{CVE: "CVE-2020-1235"},
			HostsCount: 15,
			Source:     mobius.MSRCSource,
		},
		"CVE-2020-1236": {
			CVE:        mobius.CVE{CVE: "CVE-2020-1236"},
			HostsCount: 20,
			Source:     mobius.NVDSource,
		},
	}
	list, _, err = ds.ListVulnerabilities(context.Background(), mobius.VulnListOptions{IsEE: true})
	require.NoError(t, err)
	...
	expected = map[string]mobius.VulnerabilityWithMetadata{
		"CVE-2020-1234": {
			CVE:        mobius.CVE{CVE: "CVE-2020-1234"},
			HostsCount: 10,
			Source:     mobius.MSRCSource,
		},
		"CVE-2020-1235": {
	...
	list, _, err = ds.ListVulnerabilities(context.Background(), mobius.VulnListOptions{})
	require.NoError(t, err)
	require.Len(t, list, 3)
	for _, vuln := range list {
		expectedVuln, ok := expected[vuln.CVE.CVE]
...
func testVulnerabilityWithOS(t *testing.T, ds *Datastore) {
	...
	expected := mobius.VulnerabilityWithMetadata{
		CVE: mobius.CVE{
			CVE: "CVE-2020-1234",
		},
		HostsCount: 10,
...
func testCountVulnerabilities(t *testing.T, ds *Datastore) {
	...
	// global count
	count, err := ds.CountVulnerabilities(context.Background(), mobius.VulnListOptions{})
	require.NoError(t, err)
	require.Equal(t, uint(7), count)

	// global count with exploit filter
	count, err = ds.CountVulnerabilities(context.Background(), mobius.VulnListOptions{KnownExploit: true})
	require.NoError(t, err)
	require.Equal(t, uint(3), count)

	// global count with match query
	count, err = ds.CountVulnerabilities(context.Background(), mobius.VulnListOptions{ListOptions: mobius.ListOptions{MatchQuery: "2020-1234"}})
	require.NoError(t, err)
	require.Equal(t, uint(1), count)

	// team count
	count, err = ds.CountVulnerabilities(context.Background(), mobius.VulnListOptions{TeamID: ptr.Uint(1)})
	require.NoError(t, err)
	require.Equal(t, uint(6), count)

	// team count with exploit filter
	count, err = ds.CountVulnerabilities(context.Background(), mobius.VulnListOptions{TeamID: ptr.Uint(1), KnownExploit: true})
	require.NoError(t, err)
	require.Equal(t, uint(3), count)

	// team count with match query
	count, err = ds.CountVulnerabilities(context.Background(), mobius.VulnListOptions{TeamID: ptr.Uint(1), ListOptions: mobius.ListOptions{MatchQuery: "2020-1234"}})
	require.NoError(t, err)
	require.Equal(t, uint(1), count)

	// team count with exploit filter and match query
	count, err = ds.CountVulnerabilities(context.Background(), mobius.VulnListOptions{TeamID: ptr.Uint(1), KnownExploit: true, ListOptions: mobius.ListOptions{MatchQuery: "2020-1234"}})
	require.NoError(t, err)
	require.Equal(t, uint(1), count)
}

func testInsertVulnerabilityCounts(t *testing.T, ds *Datastore) {
	windowsOS := mobius.OperatingSystem{
		Name:     "Windows 11 Pro",
	...
	_, err = ds.InsertOSVulnerability(context.Background(), mobius.OSVulnerability{
		OSID: 1,
		CVE:  "CVE-2020-1234",
	}, mobius.MSRCSource)
	require.NoError(t, err)
	...
	_, err = ds.UpdateHostSoftware(context.Background(), host1.ID, []mobius.Software{
		{
			Name:    "Chrome",
			Version: "1.0.0",
		},
	...
	list, _, err = ds.ListVulnerabilities(context.Background(), mobius.VulnListOptions{TeamID: ptr.Uint(team1.ID)})
	require.NoError(t, err)
	...
	list, _, err = ds.ListVulnerabilities(context.Background(), mobius.VulnListOptions{TeamID: ptr.Uint(0)})
	require.NoError(t, err)
	...
	// team2 counts
	list, _, err = ds.ListVulnerabilities(context.Background(), mobius.VulnListOptions{TeamID: ptr.Uint(team2.ID)})
	require.NoError(t, err)
	...
	// patch team2 hosts
	macOSPatched := mobius.OperatingSystem{
		Name:     "macOS",
		Version:  "14.2",
		Arch:     "arm64",
		Platform: "darwin",
...
func testVulnerabilityHostCountBatchInserts(t *testing.T, ds *Datastore) {
	...
	// create 200 OS vulns
	osVulns := make([]mobius.OSVulnerability, 200)
	for i := 0; i < 200; i++ {
		osVulns[i] = mobius.OSVulnerability{
			OSID: 1,
	...
	softwareVulns := make([]mobius.SoftwareVulnerability, 200)
	for i := 0; i < 200; i++ {
		softwareVulns[i] = mobius.SoftwareVulnerability{
			SoftwareID: 1,
	...
	// update host OS
	for i := 0; i < 5; i++ {
		err = ds.UpdateHostOperatingSystem(context.Background(), hosts[i].ID, mobius.OperatingSystem{
			Name:     "Windows 11 Pro",
			Version:  "10.0.22000.3007",
			Arch:     "x86_64",
			Platform: "windows",
	...
	for i := 0; i < 5; i++ {
		_, err = ds.UpdateHostSoftware(context.Background(), hosts[i].ID, []mobius.Software{
			{
				Name:    "Chrome",
				Version: "1.0.0",
			},
...
func testSoftwareByCVE(t *testing.T, ds *Datastore) {
	...
	expected := &mobius.VulnerableSoftware{
		ID:                1,
		Name:              "Chrome",
		Version:           "1.0.0",
		Source:            "programs",
...
func assertHostCounts(t *testing.T, expected []hostCount, actual []mobius.VulnerabilityWithMetadata) {
	t.Helper()
...
func seedVulnerabilities(t *testing.T, ds *Datastore) {
	...
	for i := 0; i < 10; i++ {
		...
		err := ds.UpdateHostOperatingSystem(context.Background(), hostids[i], mobius.OperatingSystem{
	...
	for i := 0; i < 5; i++ {
		_, err = ds.UpdateHostSoftware(context.Background(), hostids[i], []mobius.Software{
			{
				Name:    "Chrome",
				Version: "1.0.0",
				Source:  "programs",
	...
	_, err = ds.UpdateHostSoftware(context.Background(), hostids[7], []mobius.Software{
		{
			Name:    "Chrome",
			Version: "1.0.0",
			Source:  "programs",
	...
	_, err = ds.UpsertSoftwareCPEs(context.Background(), []mobius.SoftwareCPE{
		{
			SoftwareID: 1,
	...
	softwareVulns := []mobius.SoftwareVulnerability{
		{
			SoftwareID:        1,
			CVE:               "CVE-2020-1234",
			ResolvedInVersion: ptr.String("1.0.0"),
