...
func addHosts(ctx context.Context, pool mobius.RedisPool, hostIDs ...uint) error {
...
func removeHosts(ctx context.Context, pool mobius.RedisPool, hostIDs ...uint) error {
	conn := redis.ConfigureDoer(pool, pool.Get())
	defer conn.Close()
...
func (d *Datastore) NewHost(ctx context.Context, host *mobius.Host) (*mobius.Host, error) {
...
func (d *Datastore) EnrollHost(ctx context.Context, isMDMEnabled bool, osqueryHostID, hardwareUUID, hardwareSerial, nodeKey string, teamID *uint, cooldown time.Duration) (*mobius.Host, error) {
