...
func TestEnforceHostLimit(t *testing.T) {
	...
	runTest := func(t *testing.T, pool mobius.RedisPool) {
		...
		ds.NewHostFunc = func(ctx context.Context, host *mobius.Host) (*mobius.Host, error) {
			hostIDSeq++
			host.ID = hostIDSeq
			return host, nil
		}
		...
		h1, err := wrappedDS.NewHost(ctx, &mobius.Host{})
		require.NoError(t, err)
		require.NotNil(t, h1)
		...
		requireInvokedAndReset(&ds.EnrollHostFuncInvoked)
		requireCanEnroll(true)
		h6, err := wrappedDS.NewHost(ctx, &mobius.Host{})
		require.NoError(t, err)
		require.NotNil(t, h6)
		requireInvokedAndReset(&ds.NewHostFuncInvoked)
		requireCanEnroll(false)
		...
		requireInvokedAndReset(&ds.EnrollHostFuncInvoked)
		requireCanEnroll(true)
		h8, err := wrappedDS.NewHost(ctx, &mobius.Host{})
		require.NoError(t, err)
		require.NotNil(t, h8)
		requireInvokedAndReset(&ds.NewHostFuncInvoked)
		requireCanEnroll(false)
...
func TestSyncEnrolledHostIDs(t *testing.T) {
	runTest := func(t *testing.T, pool mobius.RedisPool) {
		...
		h1, err := wrappedDS.NewHost(ctx, &mobius.Host{})
		require.NoError(t, err)
		h2, err := wrappedDS.NewHost(ctx, &mobius.Host{})
		require.NoError(t, err)
		h3, err := wrappedDS.NewHost(ctx, &mobius.Host{})
		require.NoError(t, err)
