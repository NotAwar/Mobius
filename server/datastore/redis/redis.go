...
func (p *standalonePool) Mode() mobius.RedisMode {
	return mobius.RedisStandalone
}
...
func (p *clusterPool) Mode() mobius.RedisMode {
	return mobius.RedisCluster
}
...
func NewPool(config PoolConfig) (mobius.RedisPool, error) {
...
// commands.
func ReadOnlyConn(pool mobius.RedisPool, conn redis.Conn) redis.Conn {
...
// returned unaltered.
func ConfigureDoer(pool mobius.RedisPool, conn redis.Conn) redis.Conn {
...
// length of 1).
func SplitKeysBySlot(pool mobius.RedisPool, keys ...string) [][]string {
...
func EachNode(pool mobius.RedisPool, replicas bool, fn func(conn redis.Conn) error) error {
...
// keys.
func BindConn(pool mobius.RedisPool, conn redis.Conn, keys ...string) error {
	if _, isCluster := pool.(*clusterPool); isCluster {
...
func PublishHasListeners(pool mobius.RedisPool, conn redis.Conn, channel, message string) (bool, error) {
...
func ScanKeys(pool mobius.RedisPool, pattern string, count int) ([]string, error) {
	var keys []string
