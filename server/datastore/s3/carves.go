...
type CarveStore struct {
	*s3store
	metadatadb mobius.CarveStore
}

// NewCarveStore creates a new store with the given config
func NewCarveStore(config config.S3Config, metadatadb mobius.CarveStore) (*CarveStore, error) {
...
func (c *CarveStore) generateS3Key(metadata *mobius.CarveMetadata) string {
...
func (c *CarveStore) NewCarve(ctx context.Context, metadata *mobius.CarveMetadata) (*mobius.CarveMetadata, error) {
...
func (c *CarveStore) UpdateCarve(ctx context.Context, metadata *mobius.CarveMetadata) error {
...
func (c *CarveStore) CleanupCarves(ctx context.Context, now time.Time) (int, error) {
	...
	nonExpiredCarves, err := c.ListCarves(ctx, mobius.CarveListOptions{
		ListOptions: mobius.ListOptions{PerPage: cleanupSize},
		Expired:     false,
	})
	if err != nil {
...
func (c *CarveStore) Carve(ctx context.Context, carveID int64) (*mobius.CarveMetadata, error) {
	return c.metadatadb.Carve(ctx, carveID)
}

// CarveBySessionId returns carve metadata by session ID
func (c *CarveStore) CarveBySessionId(ctx context.Context, sessionID string) (*mobius.CarveMetadata, error) {
	return c.metadatadb.CarveBySessionId(ctx, sessionID)
}

// CarveByName returns carve metadata by name
func (c *CarveStore) CarveByName(ctx context.Context, name string) (*mobius.CarveMetadata, error) {
	return c.metadatadb.CarveByName(ctx, name)
}

// ListCarves returns a list of the currently available carves
func (c *CarveStore) ListCarves(ctx context.Context, opt mobius.CarveListOptions) ([]*mobius.CarveMetadata, error) {
...
func (c *CarveStore) NewBlock(ctx context.Context, metadata *mobius.CarveMetadata, blockID int64, data []byte) error {
	objectKey := c.generateS3Key(metadata)
...
func (c *CarveStore) GetBlock(ctx context.Context, metadata *mobius.CarveMetadata, blockID int64) ([]byte, error) {
	objectKey := c.generateS3Key(metadata)
