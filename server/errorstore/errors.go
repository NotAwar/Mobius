...
func NewHandler(ctx context.Context, pool mobius.RedisPool, logger kitlog.Logger, ttl time.Duration) *Handler {
	eh := &Handler{
		pool:   pool,
		logger: logger,
		ttl:    ttl,
...
func newTestHandler(ctx context.Context, pool mobius.RedisPool, logger kitlog.Logger, ttl time.Duration, onStart func(), onStore func(error)) *Handler {
	eh := &Handler{
		pool:   pool,
		logger: logger,
		ttl:    ttl,
...
func hashError(err error) string {
	...
	ferr := ctxerr.MobiusCause(err)

	var sb strings.Builder
	// hash the cause type and message (it might not be a MobiusError)
	fmt.Fprintf(&sb, "%T\n%s\n", cause, cause.Error())

	// hash the stack trace of the root MobiusError in the chain
	if ferr != nil {
		fmt.Fprint(&sb, strings.Join(ferr.Stack(), "\n"))
	}

	// hash the vital MobiusError fields
	if mobiusdErr, ok := cause.(mobius.MobiusdError); ok {
		_, _ = fmt.Fprint(&sb,
			mobiusdErr.ErrorSource,
			mobiusdErr.ErrorSourceVersion,
			mobiusdErr.ErrorAdditionalInfo,
		)
	}
