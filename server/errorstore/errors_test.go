...
func alwaysMobiusErrors() error { return ctxerr.New(ctx, "always mobius errors") }
...
func TestHashErrMobiusError(t *testing.T) {
	...
	t.Run("HashWrapped", func(t *testing.T) {
		// hashing a mobius error that wraps a root error hashes to the same
...
func TestUnwrapAll(t *testing.T) {
	...
	eerr := ctxerr.Wrap(ctx, gerr, "mobius wrap")
	eerr2 := ctxerr.Wrap(ctx, eerr, "mobius wrap 2")
...
func testErrorHandlerCollectsErrors(t *testing.T, pool mobius.RedisPool, wd string, flush bool) {
  ...
  \{
    ...
    \},
    "stack": \[
      "github\.com\/mobiusmdm\/mobius\/v4\/server\/errorstore\.alwaysNewError \(errors_test\.go\:\d+\)",
      "github\.com\/mobiusmdm\/mobius\/v4\/server\/errorstore\.testErrorHandlerCollectsErrors \(errors_test\.go\:\d+\)",
      "github\.com\/mobiusmdm\/mobius\/v4\/server\/errorstore\.TestErrorHandler\.func\d\.\d \(errors_test\.go\:\d+\)",
      ".+",
      ".+"
    \]
  \}
...
func testErrorHandlerCollectsDifferentErrors(t *testing.T, pool mobius.RedisPool, wd string, flush bool) {
	ctx, cancelFunc := context.WithCancel(context.Background())
	defer cancelFunc()
  ...
  \{
    ...
    \},
    "stack": \[
      "github\.com\/mobiusmdm\/mobius\/v4\/server\/errorstore\.alwaysNewErrorTwo \(errors_test\.go\:\d+\)",
      "github\.com\/mobiusmdm\/mobius\/v4\/server\/errorstore\.testErrorHandlerCollectsDifferentErrors \(errors_test\.go\:\d+\)",
      "github\.com\/mobiusmdm\/mobius\/v4\/server\/errorstore\.TestErrorHandler\.func\d\.\d \(errors_test\.go\:\d+\)",
      ".+",
      ".+"
    \]
