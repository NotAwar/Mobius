...
// clients to connect to Mobius. These clients use a gRPC transport to hit the equivalent API
// endpoints.
//
// Mobius intends to maintain support for Launcher as long as the Launcher API remains as closely
// mapped to the osquery API as it is currently, and Mobius users continue to use Launcher. In the
// future as more Mobius users migrate to Orbit, this may be deprecated.
package launcher
...
import (
	...
	"github.com/notawar/mobius set/v4/server/contexts/host"
...
func (svc *launcherWrapper) PublishResults(ctx context.Context, nodeKey string, results []distributed.Result) (string, string, bool, error) {
	...
	osqueryResults := make(mobius.OsqueryDistributedQueryResults, len(results))
	statuses := make(map[string]mobius.OsqueryStatus, len(results))
	stats := make(map[string]*mobius.Stats, len(results))

	for _, result := range results {
		statuses[result.QueryName] = mobius.OsqueryStatus(result.Status)
		osqueryResults[result.QueryName] = result.Rows
		if result.QueryStats != nil {
			stats[result.QueryName] = &mobius.Stats{
...
// In the mobius.OsqueryService authentication is done via endpoint middleware, but all launcher endpoints require
