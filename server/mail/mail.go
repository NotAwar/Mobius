// Package mail provides implementations of the Mobius MailService
package mail
...
type sender interface {
	sendMail(e mobius.Email, msg []byte) error
}

func Test(mailer mobius.MailService, e mobius.Email) error {
...
type fromFunc func(e mobius.Email) (string, error)

func getMessageBody(e mobius.Email, f fromFunc) ([]byte, error) {
...
func getFrom(e mobius.Email) (string, error) {
	return "From: " + e.SMTPSettings.SMTPSenderAddress + "\r\n", nil
}

func (m mailService) SendEmail(e mobius.Email) error {
...
func (m mailService) CanSendEmail(smtpSettings mobius.SMTPSettings) bool {
...
func smtpAuth(e mobius.Email) (smtp.Auth, error) {
	if e.SMTPSettings.SMTPAuthenticationType != mobius.AuthTypeNameUserNamePassword {
		return nil, nil
	}
	...
	var auth smtp.Auth
	switch authMethod {
	case mobius.AuthMethodNameCramMD5:
		auth = smtp.CRAMMD5Auth(username, password)
	case mobius.AuthMethodNamePlain:
		auth = smtp.PlainAuth("", username, password, server)
	case mobius.AuthMethodNameLogin:
...
func (m mailService) sendMail(e mobius.Email, msg []byte) error {
	...
	if e.SMTPSettings.SMTPAuthenticationMethod == mobius.AuthMethodNameCramMD5 {
...
func dialTimeout(addr string, tlsConfig *tls.Config) (client *smtp.Client, err error) {
	...
	// port scan using the email functionality in Mobius)
