...
// To enable, set the following env vars: MOBIUS_DEV_ANDROID_GOOGLE_CLIENT=1 and MOBIUS_DEV_ANDROID_GOOGLE_SERVICE_CREDENTIALS=$(cat credentials.json)
...
func NewGoogleClient(ctx context.Context, logger kitlog.Logger, getenv func(string) string) Client {
	androidServiceCredentials := getenv("MOBIUS_DEV_ANDROID_GOOGLE_SERVICE_CREDENTIALS")
...
func (g *GoogleClient) createPubSub(ctx context.Context, pushURL string) (string, error) {
	...
	// Mobius server does an API call to Google to verify the message validity.
