...
func pubSubPushEndpoint(ctx context.Context, request interface{}, svc android.Service) mobius.Errorer {
...
func (svc *Service) authenticatePubSub(ctx context.Context, token string) error {
	...
	assets, err := svc.ds.GetAllMDMConfigAssetsByName(ctx, []mobius.MDMAssetName{mobius.MDMAssetAndroidPubSubToken}, nil)
	switch {
	case mobius.IsNotFound(err):
		return mobius.NewAuthFailedError("missing Android PubSub token in Mobius")
	case err != nil:
		return ctxerr.Wrap(ctx, err, "getting Android PubSub token")
	}
	goldenToken, ok := assets[mobius.MDMAssetAndroidPubSubToken]
	if !ok || string(goldenToken.Value) != token {
		return mobius.NewAuthFailedError("invalid Android PubSub token")
	}
	return nil
}

func (svc *Service) getClientAuthenticationSecret(ctx context.Context) (string, error) {
	assets, err := svc.ds.GetAllMDMConfigAssetsByName(ctx, []mobius.MDMAssetName{mobius.MDMAssetAndroidMobiusServerSecret}, nil)
	switch {
	case mobius.IsNotFound(err):
		return "", nil
	case err != nil:
	...
	return string(assets[mobius.MDMAssetAndroidMobiusServerSecret].Value), nil
}
...
func (svc *Service) handlePubSubStatusReport(ctx context.Context, token string, rawData []byte) error {
	...
	if device.AppliedState == string(android.DeviceStateDeleted) {
		...
		// TODO(mna): should that delete the host from Mobius? Or at least set host_mdm to unenrolled?
		return nil
	}
	...
	if host == nil {
		level.Debug(svc.logger).Log("msg", "Device not found in Mobius. Perhaps it was deleted, "+
...
func (svc *Service) enrollHost(ctx context.Context, device *androidmanagement.Device) error {
	...
	// Device may already be present in Mobius if device user removed the MDM profile and then re-enrolled
	host, err := svc.getExistingHost(ctx, device)
	if err != nil {
		return ctxerr.Wrap(ctx, err, "getting existing Android host")
	}
	...
	if host != nil {
		level.Debug(svc.logger).Log("msg", "The enrolling Android host is already present in Mobius. Updating team if needed",
	...
	// Device is new to Mobius
	return svc.addNewHost(ctx, device)
}

func (svc *Service) getExistingHost(ctx context.Context, device *androidmanagement.Device) (*mobius.AndroidHost, error) {
...
func (svc *Service) updateHost(ctx context.Context, device *androidmanagement.Device, host *mobius.AndroidHost, fromEnroll bool) error {
	err := svc.validateDevice(ctx, device)
	if err != nil {
		return err
	}
...
func (svc *Service) getHostIfPresent(ctx context.Context, enterpriseSpecificID string) (*mobius.AndroidHost, error) {
