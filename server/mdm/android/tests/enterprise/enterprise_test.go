...
import (
	...
	"github.com/notawar/mobius set/v4/server/mdm/android/service"
	"github.com/notawar/mobius set/v4/server/mdm/android/tests"
...
func (s *enterpriseTestSuite) TestEnterprise() {
	...
	s.DoJSON("GET", "/api/v1/mobiuss/android_enterprise", nil, http.StatusNotFound, &resp)

	// Create enterprise
	var signupResp android.EnterpriseSignupResponse
	s.DoJSON("GET", "/api/v1/mobiuss/android_enterprise/signup_url", nil, http.StatusOK, &signupResp)
	assert.Equal(s.T(), tests.EnterpriseSignupURL, signupResp.Url)
	s.T().Logf("callbackURL: %s", s.ProxyCallbackURL)

	s.MobiusSvc.On("NewActivity", mock.Anything, mock.Anything, mock.AnythingOfType("mobius.ActivityTypeEnabledAndroidMDM")).Return(nil)
	const enterpriseToken = "enterpriseToken"
	res := s.Do("GET", s.ProxyCallbackURL, nil, http.StatusOK, "enterpriseToken", enterpriseToken)
	s.MobiusSvc.AssertNumberOfCalls(s.T(), "NewActivity", 1)
	...
	s.DoJSON("GET", "/api/v1/mobiuss/android_enterprise", nil, http.StatusOK, &resp)
	assert.Equal(s.T(), tests.EnterpriseID, resp.EnterpriseID)

	// Delete enterprise and make sure we can't find it.
	s.MobiusSvc.On("NewActivity", mock.Anything, mock.Anything, mock.AnythingOfType("mobius.ActivityTypeDisabledAndroidMDM")).Return(nil)
	s.Do("DELETE", "/api/v1/mobiuss/android_enterprise", nil, http.StatusOK)
	s.MobiusSvc.AssertNumberOfCalls(s.T(), "NewActivity", 2)
	s.DoJSON("GET", "/api/v1/mobiuss/android_enterprise", nil, http.StatusNotFound, &resp)
}
...
func (s *enterpriseTestSuite) TestEnterpriseSSE() {
	...
	// Test happy path
	resp := s.Do("GET", "/api/v1/mobiuss/android_enterprise/signup_sse", nil, http.StatusOK)
	...
	// Test with error
	s.WithServer.DS.AppConfigFunc = func(_ context.Context) (*mobius.AppConfig, error) {
