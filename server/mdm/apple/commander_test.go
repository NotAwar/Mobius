...
import (
	...
	nanomdm_pushsvc "github.com/notawar/mobius set/v4/server/mdm/nanomdm/push/service"
	mdmmock "github.com/notawar/mobius set/v4/server/mock/mdm"
	svcmock "github.com/notawar/mobius set/v4/server/service/mock"
...
func TestMDMAppleCommander(t *testing.T) {
	...
	host := &mobius.Host{ID: 1, UUID: "A", Platform: "darwin"}
	cmdUUID = uuid.New().String()
	mdmStorage.EnqueueDeviceLockCommandFunc = func(ctx context.Context, gotHost *mobius.Host, cmd *mdm.Command, pin string) error {
	...
	cmdUUID = uuid.New().String()
	mdmStorage.EnqueueDeviceWipeCommandFunc = func(ctx context.Context, gotHost *mobius.Host, cmd *mdm.Command) error {
		require.NotNil(t, gotHost)
		require.Equal(t, host.ID, gotHost.ID)
		require.Equal(t, host.UUID, gotHost.UUID)
