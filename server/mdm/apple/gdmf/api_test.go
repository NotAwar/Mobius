...
func TestGetLatest(t *testing.T) {
	...
	defer srv.Close()
	t.Setenv("MOBIUS_DEV_GDMF_URL", srv.URL)

	// test the function
	d := mobius.MDMAppleMachineInfo{
...
func TestRetries(t *testing.T) {
	...
	os.Setenv("MOBIUS_DEV_GDMF_URL", srv.URL)
	t.Cleanup(func() {
		srv.Close()
		os.Unsetenv("MOBIUS_DEV_GDMF_URL")
	})

	latest, err := GetLatestOSVersion(mobius.MDMAppleMachineInfo{
		OSVersion:                "14.4.1",
		Product:                  "Mac15,7",
		Serial:                   "TESTSERIAL",
		SoftwareUpdateDeviceID:   "J516sAP",
