...
// Mobius users are not allowed to upload such payloads because Mobius fully manages disk encryption (FileVault).
func ContainsFDEFileVaultOptionsPayload(contents []byte) (bool, error) {
	...
	for _, p := range prof.PayloadContent {
		if p.PayloadType == MobiusCustomSettingsPayloadType && p.DestroyFVKeyOnStandby != nil {
			return true, nil
		}
	}
	return false, nil
