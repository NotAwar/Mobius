...
const (
	...
	// See https://github.com/notawar/mobius set/issues/12483 for more info.
...
func NewProfileMatcher(pool mobius.RedisPool) mobius.ProfileMatcher {
...
// the matching stage.
func (p *profileMatcher) PreassignProfile(ctx context.Context, payload mobius.MDMApplePreassignProfilePayload) error {
	var invArg mobius.InvalidArgumentError
	...
	if len(payload.Profile) == 0 {
		...
		if cp, err := mobius.NewMDMAppleConfigProfile(payload.Profile, nil); err != nil {
...
func (p *profileMatcher) RetrieveProfiles(ctx context.Context, externalHostIdentifier string) (mobius.MDMApplePreassignHostProfiles, error) {
	var hostProfs mobius.MDMApplePreassignHostProfiles
	...
	for k, v := range profs {
		...
		hostProfs.Profiles = append(hostProfs.Profiles, mobius.MDMApplePreassignProfile{
			Profile:    []byte(v),
