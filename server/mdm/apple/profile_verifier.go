...
// Profile verification is a set of related processes that run on the Mobius server to ensure that
// the MDM profiles installed on a host are the ones expected by the Mobius server. Expected profiles
// comprise the profiles that belong to the host's assigned team (or no
// team, as applicable).
//
// The Mobius server enqueues commands to install profiles on hosts via the MDM
// protocol. The Mobius server periodically runs a cron that enqueues install profile
...
// enrolls in Mobius, when a host's team membership changes, when a new profile is uploaded, when an
...
//
// - When the results of an install profile command are reported via the MDM protocol, the Mobius server
...
//
// - When host details are reported via osquery, the Mobius server ingests a list of installed
...
// updates the verification status in the datastore. It is intended to be called by Mobius osquery
// service when the Mobius server ingests host details.
func VerifyHostMDMProfiles(ctx context.Context, ds mobius.ProfileVerificationStore, host *mobius.Host, installedByProfIdentifier map[string]*mobius.HostMacOSProfile) error {
	...
	// mobius.ExpectedMDMProfile.IsWithinGracePeriod for actual value), so it will
...
// called by the Mobius MDM checkin and command service install profile request handler.
func HandleHostMDMProfileInstallResult(ctx context.Context, ds mobius.ProfileVerificationStore, hostUUID string, cmdUUID string, status *mobius.MDMDeliveryStatus, detail string) error {
	if status != nil && *status == mobius.MDMDeliveryFailed {
		...
		host := &mobius.Host{UUID: hostUUID, Platform: "darwin"}
