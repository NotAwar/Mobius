...
func FmtDDMError(reasons []mobius.MDMAppleDDMStatusErrorReason) string {
...
func EnrollURL(token string, appConfig *mobius.AppConfig) (string, error) {
...
func IsLessThanVersion(current string, target string) (bool, error) {
	cv, err := mobius.VersionToSemverVersion(current)
	if err != nil {
		return false, fmt.Errorf("invalid current version: %w", err)
	}
	tv, err := mobius.VersionToSemverVersion(target)
	if err != nil {
