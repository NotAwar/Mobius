...
// mobius.MaintainedApps.
func SyncApps(t *testing.T, ds mobius.Datastore) []mobius.MaintainedApp {
	...
	// this call
	os.Setenv("MOBIUS_DEV_MAINTAINED_APPS_BASE_URL", srv.URL)
	defer os.Unsetenv("MOBIUS_DEV_MAINTAINED_APPS_BASE_URL")

	err := Refresh(context.Background(), ds, log.NewNopLogger())
	require.NoError(t, err)

	apps, _, err := ds.ListAvailableMobiusMaintainedApps(context.Background(), nil, mobius.ListOptions{})
	require.NoError(t, err)
	return apps
}
...
func SyncAndRemoveApps(t *testing.T, ds mobius.Datastore) {
	_, filename, _, _ := runtime.Caller(0)
	base := filepath.Dir(filepath.Dir(filepath.Dir(filepath.Dir(filename))))
	outputsDir := filepath.Join(base, "ee/maintained-apps/outputs")
	...
	originalApps, _, err := ds.ListAvailableMobiusMaintainedApps(context.Background(), nil, mobius.ListOptions{})
	require.NoError(t, err)
	...
	modifiedApps, _, err := ds.ListAvailableMobiusMaintainedApps(context.Background(), nil, mobius.ListOptions{})
	require.NoError(t, err)
