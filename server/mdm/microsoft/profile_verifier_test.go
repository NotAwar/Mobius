...
func TestLoopHostMDMLocURIs(t *testing.T) {
	...
	ds.GetHostMDMProfilesExpectedForVerificationFunc = func(ctx context.Context, host *mobius.Host) (map[string]*mobius.ExpectedMDMProfile, error) {
		return map[string]*mobius.ExpectedMDMProfile{
	...
	got := []wantStruct{}
	err := LoopOverExpectedHostProfiles(ctx, ds, &mobius.Host{}, func(profile *mobius.ExpectedMDMProfile, hash, locURI, data string) {
...
func TestVerifyHostMDMProfilesHappyPaths(t *testing.T) {
	...
	cases := []struct {
		...
		existingProfiles  []mobius.HostMDMWindowsProfile
		...
		},
		{
			...
			hostProfiles: []hostProfile{
				{"N1", syncml.ForTestWithData([]syncml.TestCommand{{Verb: "Replace", LocURI: "L1", Data: "$MOBIUS_SECRET_VALUE"}}), 0},
			},
			report:   []osqueryReport{{"N1", "200", "L1", "D1"}},
			toVerify: []string{"N1"},
			toFail:   []string{},
		...
		},
		{
			...
			hostProfiles: []hostProfile{
				{"N1", syncml.ForTestWithData([]syncml.TestCommand{{Verb: "Add", LocURI: "L1", Data: "$MOBIUS_SECRET_VALUE"}}), 0},
			},
			report:   []osqueryReport{{"N1", "200", "L1", "D1"}},
			toVerify: []string{"N1"},
			toFail:   []string{},
	...
	for _, tt := range cases {
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, p := range tt.report {
				...
				msg.AppendCommand(mobius.MDMRaw, mobius.SyncMLCmd{
			...
			ds.GetHostMDMProfilesExpectedForVerificationFunc = func(ctx context.Context, host *mobius.Host) (map[string]*mobius.ExpectedMDMProfile, error) {
				...
				out := map[string]*mobius.ExpectedMDMProfile{}
				for _, profile := range tt.hostProfiles {
					out[profile.Name] = &mobius.ExpectedMDMProfile{
			...
			ds.UpdateHostMDMProfilesVerificationFunc = func(ctx context.Context, host *mobius.Host, toVerify []string, toFail []string, toRetry []string) error {
			...
			ds.GetHostMDMProfilesRetryCountsFunc = func(ctx context.Context, host *mobius.Host) ([]mobius.HostMDMProfileRetryCount, error) {
				out := []mobius.HostMDMProfileRetryCount{}
				for _, profile := range tt.hostProfiles {
					out = append(out, mobius.HostMDMProfileRetryCount{
			...
			ds.ExpandEmbeddedSecretsFunc = func(ctx context.Context, document string) (string, error) {
				return strings.ReplaceAll(document, "$MOBIUS_SECRET_VALUE", "D1"), nil
			}

			out, err := xml.Marshal(msg)
			require.NoError(t, err)
			require.NoError(t,
				VerifyHostMDMProfiles(context.Background(), log.NewNopLogger(), ds, &mobius.Host{DetailUpdatedAt: time.Now()}, out))
...
func TestIsWin32OrDesktopBridgeADMXCSP(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			locURI:   "./Vendor/MSFT/Policy/ConfigOperations/ADMXInstall/MobiusTestApp",
			expected: false,
		},
		{
			name:     "ADMXInstall node with ADMX policy filename",
			locURI:   "./Vendor/MSFT/Policy/ConfigOperations/ADMXInstall/MobiusTestApp/Policy/MobiusTestAppAdmxFilename",
			expected: false,
		},
	}
...
func TestIsADMXInstallConfigOperationCSP(t *testing.T) {
	testCases := []struct {
		...
		// User-scoped ConfigOperations are not supported per Microsoft documentation
		{
			...
			locURI:   "./User/Vendor/MSFT/Policy/ConfigOperations/ADMXInstall/MobiusTestApp",
			expected: false,
		},
