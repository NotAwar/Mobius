...
func MakeClientEndpoints(instance string, timeout *time.Duration) (*Endpoints, error) {
	...
	var mobiusOpts []mobiushttp.ClientOpt
	if timeout != nil {
		mobiusOpts = append(mobiusOpts, mobiushttp.WithTimeout(*timeout))
	}
	options := []httptransport.ClientOption{httptransport.SetClient(mobiushttp.NewClient(mobiusOpts...))}
