...
type AgentOptions struct {
	...
	// UpdateChannels holds the configured channels for mobiusdaemon components.
...
// at the time of the Mobius release.
func ValidateJSONAgentOptions(ctx context.Context, ds Datastore, rawJSON json.RawMessage, isPremium bool) error {
	...
	if len(opts.CommandLineStartUpFlags) > 0 {
		...
		// We prevent setting the following flags because they can break mobiusdaemon.
...
type osqueryAgentOptions struct {
	...
	// See https://github.com/notawar/mobius/issues/7871#issuecomment-1265531018
