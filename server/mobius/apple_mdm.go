...
// an enrollment profile in Mobius.
...
// MDMAppleEnrollmentProfile represents an Apple MDM enrollment profile in Mobius.
// Such enrollment profiles are used to enroll Apple devices to Mobius.
type MDMAppleEnrollmentProfile struct {
	// ID is the unique identifier of the enrollment in Mobius.
...
// MDMAppleManualEnrollmentProfile is used for authorization checks to get the standard Mobius manual
...
type MDMAppleInstaller struct {
	// ID is the unique identifier of the installer in Mobius.
	ID uint `json:"id" db:"id"`
...
type MDMAppleDevice struct {
	...
	// It's set to false when a device unenrolls from Mobius.
...
// MDMAppleConfigProfile represents an Apple MDM configuration profile in Mobius.
...
type MDMAppleConfigProfile struct {
	...
	// Mobius. For Apple profiles, it is the letter "a" followed by a uuid.
	ProfileUUID string `db:"profile_uuid" json:"profile_uuid"`
	// Deprecated: ProfileID is the old unique id of the configuration profile in
	// Mobius. It is still maintained and generated for new profiles, but only
	...
	// Mobius requires that Identifier must be unique in combination with the Name and TeamID.
	...
	// Mobius requires that Name must be unique in combination with the Identifier and TeamID.
...
func (cp MDMAppleConfigProfile) ValidateUserProvided() error {
	...
	if _, ok := mobiusNames[cp.Name]; ok {
...
// therefore is not, as far as Mobius knows, currently on the host).
// The profile in Pending status could be on the host, but Mobius has not received an Acknowledged status yet.
...
type HostDEPAssignment struct {
	// HostID is the id of the host in Mobius.
	HostID uint `db:"host_id"`
	// AddedAt is the timestamp when Mobius was notified that device was added to the Mobius MDM
	// server in Apple Busines Manager (ABM).
	AddedAt time.Time `db:"added_at"`
	// DeletedAt is the timestamp  when Mobius was notified that device was deleted from the Mobius
...
func (h *HostDEPAssignment) IsDEPAssignedToMobius() bool {
	if h == nil {
		return false
	}
...
// used as there are plans to evolve the `mobiuscli get mdm-commands` command
// output in the future to list one row per command instead of one per
// command-host combination, and this mobiuscli command is the only use of this
...
type MDMAppleDeclaration struct {
	...
	// Mobius. Since we use the same endpoints for declarations and profiles:
...
type MDMAppleHostDeclaration struct {
	...
	// Status represent the current state of the declaration, as known by the Mobius server.
	Status *MDMDeliveryStatus `db:"status" json:"status"`
	...
	// either by the MDM protocol or the Mobius server.
	Detail string `db:"detail" json:"detail"`
...
// package files. Mobius supports storing to the database and to an S3 bucket.
