...
// mobius-osquery installers. This was originally implemented to support the
// Mobius Sandbox and is not expected to be used outside of this:
// https://mobiusmdm.com/docs/configuration/mobius-server-configuration#packaging
...
// Datastore combines all the interfaces in the Mobius DAL
type Datastore interface {
	...
	// GetCompletedCampaigns returns the IDs of the campaigns that are in the mobius.QueryComplete state and that are in the
	...
	// IsHostConnectedToMobiusMDM verifies if the host has an active Mobius MDM enrollment with this server
	IsHostConnectedToMobiusMDM(ctx context.Context, host *Host) (bool, error)
	...
	// AreHostsConnectedToMobiusMDM checks each host MDM enrollment with
	...
	AreHostsConnectedToMobiusMDM(ctx context.Context, hosts []*Host) (map[string]bool, error)
	...
	// SetVPPInstallAsVerified marks the VPP app install attempt as "verified" (Mobius has validated
	...
	// SetVPPInstallAsFailed marks a VPP app install attempt as failed (Mobius couldn't validate that
	...
	// MDMTurnOff updates Mobius host information related to MDM when a
	...
	ShouldSendStatistics(ctx context.Context, frequency time.Duration, config config.MobiusConfig) (StatisticsPayload, bool, error)
	...
	SetOrUpdateMDMData(ctx context.Context, hostID uint, isServer, enrolled bool, serverURL string, installedFromDep bool, name string, mobiusEnrollRef string) error
	...
	// IsCVEKnownToMobius checks if the provided CVE is known to Mobius.
	IsCVEKnownToMobius(ctx context.Context, cve string) (bool, error)

	///////////////////////////////////////////////////////////////////////////////
	// Apple MDM

	// NewMDMAppleConfigProfile creates and returns a new configuration profile.
	NewMDMAppleConfigProfile(ctx context.Context, p MDMAppleConfigProfile, usesMobiusVars []string) (*MDMAppleConfigProfile, error)
	...
	// this is mainly aimed to internal usage within the Mobius server.
	...
	// Such enrollment profiles allow devices to enroll to Mobius MDM.
	...
	// NewMDMAppleInstaller creates and stores an Apple installer to Mobius.
	...
	// IngestMDMAppleDevicesFromDEPSync creates new Mobius host records for MDM-enrolled devices that are
	// not already enrolled in Mobius. It returns the number of hosts created, and an error.
	...
	// MDM-enrolled devices via OTA that are not already enrolled in Mobius.
	IngestMDMAppleDeviceFromOTAEnrollment(ctx context.Context, teamID *uint, deviceInfo MDMAppleMachineInfo) error

	// MDMAppleUpsertHost creates or matches a Mobius host record for an
	// MDM-enrolled device.
	MDMAppleUpsertHost(ctx context.Context, mdmHost *Host) error

	// RestoreMDMApplePendingDEPHost restores a host that was previously deleted from Mobius.
	...
	// that are enrolled or pending enrollment in Mobius's MDM via DEP for the
	...
	// that are enrolled or pending enrollment in Mobius's MDM via DEP in the
	...
	// for the host with the given serial number. The host must be DEP assigned to Mobius.
	...
	// GetMDMAppleEnrolledDeviceDeletedFromMobius returns the information of a
	// device that is still enrolled in Mobius MDM but the corresponding host has
	// been deleted from Mobius.
	GetMDMAppleEnrolledDeviceDeletedFromMobius(ctx context.Context, hostUUID string) (*MDMAppleEnrolledDeviceInfo, error)

	// ListMDMAppleEnrolledIphoneIpadDeletedFromMobius returns a list of nano
	// device IDs (host UUIDs) of iPhone and iPad that are enrolled in Mobius MDM
	// but deleted from Mobius.
	ListMDMAppleEnrolledIPhoneIpadDeletedFromMobius(ctx context.Context, limit int) ([]string, error)
	...
	BatchSetMDMProfiles(ctx context.Context, tmID *uint, macProfiles []*MDMAppleConfigProfile, winProfiles []*MDMWindowsConfigProfile,
		macDeclarations []*MDMAppleDeclaration, profilesVariables []MDMProfileIdentifierMobiusVariables) (updates MDMProfilesUpdates, err error)
	...
	LockHostViaScript(ctx context.Context, request *HostScriptRequestPayload, hostMobiusPlatform string) error

	// UnlockHostViaScript sends a script to unlock a host and updates the
	// states in host_mdm_actions
	UnlockHostViaScript(ctx context.Context, request *HostScriptRequestPayload, hostMobiusPlatform string) error
	...
	// pending.
	UnlockHostManually(ctx context.Context, hostID uint, hostMobiusPlatform string, ts time.Time) error
	...
	// states in host_mdm_actions.
	WipeHostViaScript(ctx context.Context, request *HostScriptRequestPayload, hostMobiusPlatform string) error
	...
	// GetAllVPPApps returns all the VPP apps in Mobius, across all teams.
	...
	// ListAvailableMobiusMaintainedApps returns a list of Mobius-maintained apps, including software title ID if
	...
	ListAvailableMobiusMaintainedApps(ctx context.Context, teamID *uint, opt ListOptions) ([]MaintainedApp, *PaginationMetadata, error)

	// ClearRemovedMobiusMaintainedApps deletes all Mobius-maintained apps that are not in the given
	// set of slugs.
	ClearRemovedMobiusMaintainedApps(ctx context.Context, slugsToKeep []string) error

	// GetMaintainedAppByID gets a Mobius-maintained app by its ID, including software title ID if
	// either the maintained app or a custom package/VPP app for the same app is installed on the specified team,
	// if a team is specified.
	GetMaintainedAppByID(ctx context.Context, appID uint, teamID *uint) (*MaintainedApp, error)

	// GetMaintainedAppBySlug gets a Mobius-maintained app by its slug
...
// status of a host's MDM profiles. The Mobius Datastore satisfies this interface.
