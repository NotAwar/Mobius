...
var (
	...
	MDMNotConfiguredMessage              = "MDM features aren't turned on in Mobius. For more information about setting up MDM, please visit https://mobiusmdm.com/docs/using-mobius"
	WindowsMDMNotConfiguredMessage       = "Windows MDM isn't turned on. For more information about setting up MDM, please visit https://mobiusmdm.com/learn-more-about/windows-mdm"
	AppleMDMNotConfiguredMessage         = "macOS MDM isn't turned on. Visit https://mobiusmdm.com/docs/using-mobius to learn how to turn on MDM."
	AppleABMDefaultTeamDeprecatedMessage = "mdm.apple_bm_default_team has been deprecated. Please use the new mdm.apple_business_manager key documented here: https://mobiusmdm.com/learn-more-about/apple-business-manager-gitops"
...
func (e licenseError) Error() string {
	return "Requires Mobius Premium license"
}
...
// related to mobius logic specifically. Other errors, such as mysql errors, shouldn't
...
// NewError returns a mobius error with the code and message specified
...
// NewErrorf returns a mobius error with the code, and message formatted
...
// MobiusdError is an error that can be reported by any of the mobiusdaemon
// components.
type MobiusdError struct {
	...
	// Vital errors are always reported to Mobius server.
	Vital bool `json:"vital"`
}

// Error implements the error interface
func (fe MobiusdError) Error() string {
...
// Mobius Desktop)
func (fe MobiusdError) MarshalZerologObject(e *zerolog.Event) {
...
func (fe MobiusdError) ToMap() map[string]any {
...
const (
	// Hosts, general
	HostNotFoundErrMsg           = "Host doesn't exist. Make sure you provide a valid hostname, UUID, or serial number. Learn more about host identifiers: https://mobiusmdm.com/learn-more-about/host-identifiers"
	NoHostsTargetedErrMsg        = "No hosts targeted. Make sure you provide a valid hostname, UUID, or serial number. Learn more about host identifiers: https://mobiusmdm.com/learn-more-about/host-identifiers"
	TargetedHostsDontExistErrMsg = "One or more targeted hosts don't exist. Make sure you provide a valid hostname, UUID, or serial number. Learn more about host identifiers: https://mobiusmdm.com/learn-more-about/host-identifiers"
	...
	RunScriptForbiddenErrMsg               = "You don't have the right permissions in Mobius to run the script."
	...
	RunScriptGatewayTimeoutErrMsg          = "Gateway timeout. Mobius didn't hear back from the host and doesn't know if the script ran. Please make sure your load balancer timeout isn't shorter than the Mobius server timeout."
	...
	// Config
	InvalidServerURLMsg = `Mobius server URL must use “https” or “http”.`

	// macOS setup experience
	BootstrapPkgNotDistributionErrMsg = "Couldn’t add. Bootstrap package must be a distribution package. Learn more at: https://mobiusmdm.com/learn-more-about/macos-distribution-packages"

	// NDES/SCEP validation
	MultipleSCEPPayloadsErrMsg          = "Add only one SCEP payload."
	SCEPVariablesNotInSCEPPayloadErrMsg = "Variables prefixed with \"$MOBIUS_VAR_SCEP_\", \"$MOBIUS_VAR_CUSTOM_SCEP_\" and \"$MOBIUS_VAR_NDES_SCEP\" must only be in the SCEP payload."
)

// Error message variables
var (
	NDESSCEPVariablesMissingErrMsg         = fmt.Sprintf("SCEP profile for NDES certificate authority requires: $MOBIUS_VAR_%s, $MOBIUS_VAR_%s, and $MOBIUS_VAR_%s variables.", MobiusVarNDESSCEPChallenge, MobiusVarNDESSCEPProxyURL, MobiusVarSCEPRenewalID)
	SCEPRenewalIDWithoutURLChallengeErrMsg = "Variable \"$MOBIUS_VAR_" + MobiusVarSCEPRenewalID + "\" can't be used if variables for SCEP URL and Challenge are not specified."
)
