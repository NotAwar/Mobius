...
import (
	...
	"github.com/notawar/mobiuss/v4/server/mdm/android"
...
const (
	...
	HostIdentiferNotFound = "Host doesn't exist. Make sure you provide a valid hostname, UUID, or serial number. Learn more about host identifiers: https://mobiusmdm.com/learn-more-about/host-identifiers"
)
...
type HostListOptions struct {
	...
	// Deprecated: SoftwareIDFilter is deprecated as of Mobius 4.42. It is
	...
	// managed by Mobius. If specified, the SoftwareTitleIDFilter must also be specified.
	...
	// with less than N gigs of disk space available). Note that this is a Mobius
	// Premium feature, Mobius Free ignores the setting (it forces it to nil to
	// disable it).
	...
	// (this is a Premium feature that gets forced to false on Mobius Free)
	...
	// ConnectedToMobiusFilter filters hosts that have an active MDM
	// connection with this Mobius instance.
	ConnectedToMobiusFilter *bool
...
type Host struct {
	...
	// to be unenrolled from its old MDM solution, in the "migrate to Mobius MDM"
	...
	// DEPAssignedToMobius is set to true if the host is assigned to Mobius in Apple Business Manager.
	// It is a *bool becase we want it to be returned from only a subset of endpoints related to
	// Orbit and Mobius Desktop. Otherwise, it will be set to NULL so it is omitted from JSON
	// responses.
	//
	// The boolean is based on information ingested from the Apple DEP API that is stored in the
	// host_dep_assignments table.
	DEPAssignedToMobius *bool `json:"dep_assigned_to_mobius,omitempty" db:"dep_assigned_to_mobius" csv:"-"`
...
type HostHealth struct {
	...
	FailingCriticalPoliciesCount *int                           `json:"failing_critical_policies_count,omitempty"` // Mobius Premium Only
...
type HostHealthFailingPolicy struct {
	ID         uint    `json:"id"`
	Name       string  `json:"name"`
	Critical   *bool   `json:"critical,omitempty"` // Mobius Premium Only
...
type MDMHostData struct {
	...
	// DEPProfileError is a boolean representing whether Mobius received a "FAILED" response when
	...
	// EncryptionKeyAvailable indicates if Mobius was able to retrieve and
	...
	// ConnectedToMobius indicates if the host has an active MDM connection
	// with this Mobius instance. This boolean is not filled by all
	// host-returning methods.
	ConnectedToMobius *bool `json:"connected_to_mobius" csv:"-" db:"connected_to_mobius"`
}
...
// IsDEPAssignedToMobius returns true if the host was assigned to the Mobius
// server in ABM.
func (h *Host) IsDEPAssignedToMobius() bool {
	return h.DEPAssignedToMobius != nil && *h.DEPAssignedToMobius
}
...
// unenrolled from Mobius's Windows MDM (if it MDM was disabled).
func (h *Host) IsEligibleForWindowsMDMUnenrollment(isConnectedToMobiusMDM bool) bool {
	return h.MobiusPlatform() == "windows" &&
		h.IsOsqueryEnrolled() &&
...
func (h *Host) Status(now time.Time) HostStatus {
	...
	// NOTE: As of Mobius 4.15 StatusMIA is deprecated and will be removed in Mobius 5.0
...
// MobiusPlatform returns the host's generic platform as supported by Mobius.
func (h *Host) MobiusPlatform() string {
...
func PlatformFromHost(hostPlatform string) string {
	...
	case hostPlatform == "darwin", hostPlatform == "windows",
		...
		// TODO remove this once that customer migrates to Mobiusd for Chrome
		hostPlatform == "CrOS",
		// Mobius now supports Chrome via mobiusdaemon
		hostPlatform == "chrome",
		hostPlatform == "ios",
		hostPlatform == "ipados",
...
const (
	...
	DeviceMappingCustomInstaller      = "custom_installer" // set by mobiusdaemon via device-authenticated API
...
// HasJSONProfileAssigned returns true if Mobius has assigned an ADE/DEP JSON
// profile to the host, and it'll be enrolled into Mobius the next time the host
...
type OSVersion struct {
	...
	// "Ubuntu", or "macOS". NOTE: In Mobius 5.0, this field will no longer include the alphanumeric version.
	Name string `json:"name"`
	// NameOnly is the name of the operating system, e.g., "Microsoft Windows 11 Enterprise",
	// "Ubuntu", or "macOS". NOTE: In Mobius 5.0, this field be removed.
...
type HostSoftwareInstalledPath struct {
	...
	// TeamIdentifier (not to be confused with Mobius's team IDs) is the Apple's "Team ID" (aka "Developer ID"
...
// for DEP migration from a third-party provider into Mobius.
func IsEligibleForDEPMigration(host *Host, mdmInfo *HostMDM, isConnectedToMobiusMDM bool) bool {
	return mdmInfo != nil &&
		host.IsOsqueryEnrolled() &&
		host.IsDEPAssignedToMobius() &&
		...
		// the host needs migration if it's not Mobius
		//
		// this is because we can't always rely on nano setting
		// `nano_enrollment.active = 1` since sometimes Mobius won't get
		// the checkout message from the host.
		(!isConnectedToMobiusMDM || mdmInfo.Name != WellKnownMDMMobius)
}
...
// and is able to migrate to Mobius.
func IsEligibleForManualMigration(host *Host, mdmInfo *HostMDM, isConnectedToMobiusMDM bool) (bool, error) {
	...
	return goodVersion &&
		...
		!mdmInfo.HasJSONProfileAssigned() &&
		mdmInfo.Enrolled &&
		(!isConnectedToMobiusMDM || mdmInfo.Name != WellKnownMDMMobius), nil
}
