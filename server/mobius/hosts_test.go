...
func TestHostStatus(t *testing.T) {
	...
	testCases := []struct {
		...
		{mockClock.Now().Add(-31 * 24 * time.Hour), 10, 10, StatusOffline}, // As of Mobius 4.15, StatusMIA is deprecated in favor of StatusOffline
		...
		{mockClock.Now().Add(-31 * 24 * time.Hour), 0, 0, StatusOffline}, // As of Mobius 4.15, StatusMIA is deprecated in favor of StatusOffline
	}
...
func TestHostStatusIsValid(t *testing.T) {
	for _, tt := range []struct {
		...
		{"mia", StatusMIA, true}, // As of Mobius 4.15, StatusMIA is deprecated in favor of StatusOffline
...
func TestPlatformFromHost(t *testing.T) {
	for _, tc := range []struct {
		...
		mobiusPlatform := PlatformFromHost(tc.host)
		require.Equal(t, tc.expPlatform, mobiusPlatform)
...
func TestIsEligibleForDEPMigration(t *testing.T) {
	testCases := []struct {
		...
		{
			name:                    "Not eligible - not DEP assigned to Mobius",
			osqueryHostID:           ptr.String("some-id"),
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			host := &Host{
				...
				DEPAssignedToMobius: tc.depAssignedToMobius,
				OSVersion:          tc.hostOS,
			}
