...
const (
	...
	// third-party MDM solution to Mobius.
	RefetchMDMUnenrollCriticalQueryDuration = 3 * time.Minute

	// MobiusVarNDESSCEPChallenge and other variables are used as $MOBIUS_VAR_<VARIABLE_NAME>.
	// For example: $MOBIUS_VAR_NDES_SCEP_CHALLENGE
	...
	// in the mobius_variables table. At some point we should refactor those constants/regexp
	// and the findMobiusVariables logic to use the DB table instead of hardcoding them here
	// (not doing it now because of time constraints to finish the story for the release).
	MobiusVarNDESSCEPChallenge               = "NDES_SCEP_CHALLENGE"
	MobiusVarNDESSCEPProxyURL                = "NDES_SCEP_PROXY_URL"
	MobiusVarHostEndUserEmailIDP             = "HOST_END_USER_EMAIL_IDP"
	MobiusVarHostHardwareSerial              = "HOST_HARDWARE_SERIAL"
	MobiusVarHostEndUserIDPUsername          = "HOST_END_USER_IDP_USERNAME"
	MobiusVarHostEndUserIDPUsernameLocalPart = "HOST_END_USER_IDP_USERNAME_LOCAL_PART"
	MobiusVarHostEndUserIDPGroups            = "HOST_END_USER_IDP_GROUPS"
	MobiusVarHostEndUserIDPDepartment        = "HOST_END_USER_IDP_DEPARTMENT"
	MobiusVarSCEPRenewalID                   = "SCEP_RENEWAL_ID"

	MobiusVarDigiCertDataPrefix        = "DIGICERT_DATA_"
	MobiusVarDigiCertPasswordPrefix    = "DIGICERT_PASSWORD_" // nolint:gosec // G101: Potential hardcoded credentials
	MobiusVarCustomSCEPChallengePrefix = "CUSTOM_SCEP_CHALLENGE_"
	MobiusVarCustomSCEPProxyURLPrefix  = "CUSTOM_SCEP_PROXY_URL_"
...
// accepted. The Mobius Datastore satisfies this interface.
...
func (bp *MDMAppleBootstrapPackage) URL(host string) (string, error) {
	...
	pkgURL.Path = "/api/latest/mobius/mdm/bootstrap"
...
// see https://github.com/notawar/mobius/blob/e9fd28717d474668ca626efbacdd0615d42b2e0a/server/service/osquery.go#L950
...
// HostMDMProfileRetryCount represents the number of times Mobius has attempted to install
...
//
// https://github.com/notawar/mobius/issues/11008#issuecomment-1503466119
...
type MDMProfilesSummary struct {
	// Verified includes each host where Mobius has verified the installation of all of the
...
//
//   - verified: the MDM command was successfully applied, and Mobius has
//     independently verified the status. This is a terminal state.
//
//   - verifying: the MDM command was successfully applied, but Mobius has not
...
const (
	...
	// NDES SCEP server. It is used by Mobius's SCEP proxy.
	...
	// MDMAssetAndroidMobiusServerSecret is the bearer token for Android requests sent to the mobiusmdm.com Android management proxy.
	MDMAssetAndroidMobiusServerSecret MDMAssetName = "android_mobius_server_secret" // nolint:gosec // Ignore G101: Potential hardcoded credentials
)
...
func FilterMacOSOnlyProfilesFromIOSIPadOS(profiles []*MDMAppleProfilePayload) []*MDMAppleProfilePayload {
	...
	for _, profilePayload := range profiles {
		if (profilePayload.HostPlatform == "ios" || profilePayload.HostPlatform == "ipados") &&
			(profilePayload.ProfileName == mdm_types.MobiusdConfigProfileName ||
				profilePayload.ProfileName == mdm_types.MobiusFileVaultProfileName) {
			continue
		}
...
// MDMProfileUUIDMobiusVariables represents the Mobius variables used by a
// profile identified by its UUID.
type MDMProfileUUIDMobiusVariables struct {
	// ProfileUUID is the UUID of the profile.
	ProfileUUID string
	// MobiusVariables is the (deduplicated) list of Mobius variables used by the
	// profile, without the "MOBIUS_VAR_" prefix (as returned by
	// findMobiusVariables).
	MobiusVariables []string
}

// MDMProfileIdentifierMobiusVariables represents the Mobius variables used by a
// profile identified by its identifier.
type MDMProfileIdentifierMobiusVariables struct {
