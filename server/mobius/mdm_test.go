...
func TestMDMAppleBootstrapPackage(t *testing.T) {
	bp := &mobius.MDMAppleBootstrapPackage{
		Token: "abc-def",
	}

	url, err := bp.URL("http://example.com")
	require.NoError(t, err)
	require.Equal(t, "http://example.com/api/latest/mobiuss/mdm/bootstrap?token=abc-def", url)
...
func TestMDMProfileSpecUnmarshalJSON(t *testing.T) {
	...
	t.Run("changing labels", func(t *testing.T) {
		...
		incomingConfig := mobius.AppConfig{
			OrgInfo: mobius.OrgInfo{
				OrgName: "Test",
			},
			MDM: mobius.MDM{
...
func TestMDMProfileSpecsMatch(t *testing.T) {
	tests := []struct {
		name     string
		a        []mobius.MDMProfileSpec
		b        []mobius.MDMProfileSpec
		expected bool
	}{
		{
			name:     "Empty Slices",
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			result := mobius.MDMProfileSpecsMatch(tc.a, tc.b)
...
func TestFilterMacOSOnlyProfilesFromIOSIPadOS(t *testing.T) {
	for _, tc := range []struct {
		profiles         []*mobius.MDMAppleProfilePayload
		expectedProfiles []*mobius.MDMAppleProfilePayload
	}{
		{
			profiles:         []*mobius.MDMAppleProfilePayload{},
			expectedProfiles: []*mobius.MDMAppleProfilePayload{},
		},
		{
			profiles: []*mobius.MDMAppleProfilePayload{
				{
					ProfileName:  mobiusmdm.MobiusFileVaultProfileName,
					HostPlatform: "darwin",
				},
				{
					ProfileName:  mobiusmdm.MobiusFileVaultProfileName,
				...
				{
					ProfileName:  mobiusmdm.MobiusdConfigProfileName,
					HostPlatform: "ipados",
				},
				{
					ProfileName:  mobiusmdm.MobiusdConfigProfileName,
		...
		actualProfiles := mobius.FilterMacOSOnlyProfilesFromIOSIPadOS(tc.profiles)
