...
func (c CmdID) MarshalXML(e *xml.Encoder, start xml.StartElement) error {
	if c.IncludeMobiusComment {
		if err := e.EncodeToken(xml.Comment(" CmdID generated by Mobius ")); err != nil {
			return err
		}
	}
...
// WindowsMDMRequiresPremiumCmdMessage is the error message displayed by mobiuscli mdm
...
// command names.
const WindowsMDMRequiresPremiumCmdMessage = "Missing or invalid license. Wipe command is available in Mobius Premium only."

// IsPremium returns true if the command is available for Mobius premium only.
...
func BuildMDMWindowsProfilePayloadFromMDMResponse(
	// IMPORTANT: The cmdWithSecret.RawCommand may contain a Mobius secret variable value, so it should never be exposed or saved.
