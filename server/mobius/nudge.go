...
func NewNudgeConfig(macOSUpdates AppleOSUpdateSettings) (*NudgeConfig, error) {
	...
	// when providing JSON config, so for compatibility with existing mobiusdaemon versions we're using a somewhat sane
	// UTC value here (noon Pacific Standard Time, no revision for DST). See https://github.com/notawar/mobius/pull/23320
	...
	return &NudgeConfig{
		OSVersionRequirements: []nudgeOSVersionRequirements{{
			...
			AboutUpdateURLs: []nudgeAboutUpdateURLs{{
				Language:       "en",
				AboutUpdateURL: "https://mobiusmdm.com/learn-more-about/os-updates",
			}},
		}},
