...
func RunFailing1000hosts(t *testing.T, r mobius.FailingPolicySet) {
	hosts := make([]mobius.PolicySetHost, 1000)
	for i := range hosts {
...
func RunFailingBasic(t *testing.T, r mobius.FailingPolicySet) {
	policyID1 := uint(1)
	...
	hostx := mobius.PolicySetHost{
		ID:       uint(999),
		Hostname: "hostx.example",
	}
	err = r.RemoveHosts(policyID1, []mobius.PolicySetHost{hostx})
	require.NoError(t, err)

	// Remove no hosts.
	err = r.RemoveHosts(policyID1, []mobius.PolicySetHost{})
	require.NoError(t, err)

	// Test adding a new host to a policy set.
	host2 := mobius.PolicySetHost{
		ID:       uint(2),
	...
	err = r.RemoveHosts(policyID1, []mobius.PolicySetHost{host2, host3})
	require.NoError(t, err)
	...
	err = r.RemoveHosts(policyID2, []mobius.PolicySetHost{host2})
	require.NoError(t, err)
	...
	err = r.RemoveHosts(policyID1, []mobius.PolicySetHost{host4})
	require.NoError(t, err)
