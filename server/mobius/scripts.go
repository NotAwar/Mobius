...
// for running a script synchronously (so that mobiuscli can display it) and to
...
func HostScriptTimeoutMessage(seconds *int) string {
	...
	return fmt.Sprintf("Timeout. Mobius stopped the script after %d seconds to protect host performance.", timeout)
}
...
type HostLockWipeStatus struct {
	// HostMobiusPlatform is the mobius-normalized platform of the host, i.e. the
	// result of host.MobiusPlatform().
	HostMobiusPlatform string
...
func (s *HostLockWipeStatus) IsPendingLock() bool {
	if s.HostMobiusPlatform == "darwin" || s.HostMobiusPlatform == "ios" || s.HostMobiusPlatform == "ipados" {
