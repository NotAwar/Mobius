...
type Service interface {
	...
	// GetTransparencyURL gets the URL to redirect to when an end user clicks About Mobius
	...
	// EnrollOrbit enrolls an orbit instance to Mobius by using the host information + enroll secret
	...
	// agent options. It also returns any notifications that mobius wants to surface
	// to mobiusdaemon (formerly orbit).
	GetOrbitConfig(ctx context.Context) (OrbitConfig, error)

	// LogMobiusdError logs an error report from a `mobiusdaemon` component
	LogMobiusdError(ctx context.Context, errData MobiusdError) error
	...
	// GetMobiusDesktopSummary returns a summary of the host used by Mobius Desktop to operate.
	GetMobiusDesktopSummary(ctx context.Context) (DesktopSummary, error)
	...
	// /////////////////////////////////////////////////////////////////////////////
	// UserService contains methods for managing a Mobius User.
	...
	// credentials are valid, then responds with an URL to the Mobius UI to
	...
	// the host, which is either set by the mobiusdaemon installer at startup (via a
	...
	// /////////////////////////////////////////////////////////////////////////////
	// AppConfigService provides methods for configuring  the Mobius application
	...
	// SetupRequired returns whether the app config setup needs to be performed (only when first initializing a Mobius
	// server).
	...
	// PartnershipsConfig returns Mobius partnership-specific configuration
	PartnershipsConfig(ctx context.Context) (*Partnerships, error)

	// LoggingConfig parses config.MobiusConfig instance and returns a Logging.
	LoggingConfig(ctx context.Context) (*Logging, error)

	// EmailConfig parses config.MobiusConfig and returns an EmailConfig
	...
	// InviteNewUser creates an invite for a new user to join Mobius.
	...
	// /////////////////////////////////////////////////////////////////////////////
	// TargetService **NOTE: SearchTargets will be removed in Mobius 5.0**
	...
	// UploadMDMAppleInstaller uploads an Apple installer to Mobius.
	...
	// DeleteMDMAppleInstaller deletes an Apple installer from Mobius.
	...
	// ListABMTokens lists all the ABM tokens in Mobius.
	ListABMTokens(ctx context.Context) ([]*ABMToken, error)

	// CountABMTokens counts the ABM tokens in Mobius.
	...
	// profile used for Mobius MDM enrollment from the specified device.
	...
	// given team that enables FileVault with a config that allows Mobius to
	...
	// AddMobiusMaintainedApp adds a Mobius-maintained app to the given team.
	AddMobiusMaintainedApp(ctx context.Context, teamID *uint, appID uint, installScript, preInstallQuery, postInstallScript, uninstallScript string, selfService bool, automaticInstall bool, labelsIncludeAny, labelsExcludeAny []string) (uint, error)
	// ListMobiusMaintainedApps lists Mobius-maintained apps, including associated software title for supplied team ID (if any)
	ListMobiusMaintainedApps(ctx context.Context, teamID *uint, opts ListOptions) ([]MaintainedApp, *PaginationMetadata, error)
	// GetMobiusMaintainedApp returns a Mobius-maintained app by ID, including associated software title for supplied team ID (if any)
	GetMobiusMaintainedApp(ctx context.Context, appID uint, teamID *uint) (*MaintainedApp, error)
	...
	// ScimDetails returns the details of last access to Mobius's SCIM endpoints
