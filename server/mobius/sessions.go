...
type Auth interface {
	...
	//
	// Mobius requires users to configure this value to be the email of the Subject
	UserID() string
...
const (
	globalUserRoleSSOAttrName     = "MOBIUS_JIT_USER_ROLE_GLOBAL"
	teamUserRoleSSOAttrNamePrefix = "MOBIUS_JIT_USER_ROLE_TEAM_"
	ssoAttrNullRoleValue          = "null"
)

// RolesFromSSOAttributes loads Global and Team roles from SAML custom attributes.
//   - Custom attribute `MOBIUS_JIT_USER_ROLE_GLOBAL` is used for setting global role.
//   - Custom attributes of the form `MOBIUS_JIT_USER_ROLE_TEAM_<TEAM_ID>` are used
