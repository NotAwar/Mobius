...
// installer files. Mobius supports storing to the local filesystem and to an
// S3 bucket.
...
// required for a client to pull in and install software from the mobius server
type SoftwareInstallDetails struct {
	...
	// InstallerID is the unique identifier for the software package metadata in Mobius.
...
type SoftwareInstaller struct {
	...
	// MobiusMaintainedAppID is the related Mobius-maintained app for this installer (if not nil).
	MobiusMaintainedAppID *uint `json:"mobius_maintained_app_id" db:"mobius_maintained_app_id"`
...
type SoftwarePackageResponse struct {
	...
	// ID of the Mobius Maintained App this package uses, if any
	MobiusMaintainedAppID *uint `json:"mobius_maintained_app_id" db:"mobius_maintained_app_id"`
}
...
type UploadSoftwareInstallerPayload struct {
	...
	// automatically created when a software installer is added to Mobius. This field should be set
...
type SoftwarePackageOrApp struct {
	...
	// AutomaticInstallPolicies is present for Mobius maintained apps and custom packages
...
type SoftwarePackageSpec struct {
	...
	// software package. Only present after parsing a GitOps file on the mobiuscli
...
type MobiusMaintainedAppsSpec struct {
	Slug             string   `json:"slug"`
	AutomaticInstall *bool    `json:"automatic_install"`
	SelfService      bool     `json:"self_service"`
	LabelsIncludeAny []string `json:"labels_include_any"`
...
// HostSoftwareInstallResultPayload is the payload provided by mobiusdaemon to record
...
const (
	// ExitCodeScriptsDisabled is a special exit code returned by mobiusdaemon in the
	// HostSoftwareInstallResultPayload when the install was attempted on a host with scripts
	// disabled.
	ExitCodeScriptsDisabled = -2
	// ExitCodeInstallerDownloadFailed is a special exit code returned by mobiusdaemon in the
	// HostSoftwareInstallResultPayload when mobiusdaemon failed to download the installer.
...
func NewTempFileReader(from io.Reader, tempDirFn func() string) (*TempFileReader, error) {
	...
	tempFile, err := os.CreateTemp(tempDirFn(), "mobius-temp-file-*")
	if err != nil {
		return nil, err
	}
...
// IsMobiusInitiated returns true if the software install is initiated by Mobius.
// Software installs initiated via a policy are mobius-initiated (and we also
// make sure SelfService is false, as this case is always user-initiated).
func (o HostSoftwareInstallOptions) IsMobiusInitiated() bool {
