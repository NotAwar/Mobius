...
// User is the model struct that represents a Mobius user.
type User struct {
...
func saltAndHashPassword(keySize int, plaintext string, cost int) (hashed []byte, salt string, err error) {
	...
	if err != nil {
		if errors.Is(err, bcrypt.ErrPasswordTooLong) {
			return nil, "", NewInvalidArgumentError("Could not create user. Password is over the 48 characters limit. If the password is under 48 characters, please check the auth_salt_key_size in your Mobius server config.", "password too long")
		}
		return nil, "", err
	}
