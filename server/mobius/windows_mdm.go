...
// zero because these states are not in Mobius's current implementation of BitLocker management.
...
// MDMWindowsConfigProfile represents a Windows MDM profile in Mobius.
type MDMWindowsConfigProfile struct {
	// ProfileUUID is the unique identifier of the configuration profile in
	// Mobius. For Windows profiles, it is the letter "w" followed by a uuid.
...
func (m *MDMWindowsConfigProfile) ValidateUserProvided() error {
	...
	// keep track of certain elements to perform Mobius-validations.
	//
	...
	for {
		...
		case xml.CharData:
			if inLocURI {
				if err := validateMobiusProvidedLocURI(string(t)); err != nil {
					return err
				}
			}
		}
...
var mobiusProvidedLocURIValidationMap = map[string][]string{
	syncml.MobiusBitLockerTargetLocURI: nil,
	syncml.MobiusOSUpdateTargetLocURI:  {"Windows updates", "mdm.windows_updates"},
}

func validateMobiusProvidedLocURI(locURI string) error {
	sanitizedLocURI := strings.TrimSpace(locURI)
	for mobiusLocURI, errHints := range mobiusProvidedLocURIValidationMap {
		if strings.Contains(sanitizedLocURI, mobiusLocURI) {
			if mobiusLocURI == syncml.MobiusBitLockerTargetLocURI {
