...
func TestTriggerFailingPolicies(t *testing.T) {
	...
	pols := map[uint]*mobius.PolicyData{
	...
	ds.PolicyFunc = func(ctx context.Context, id uint) (*mobius.Policy, error) {
		...
		return &mobius.Policy{PolicyData: *pd}, nil
	}

	teams := map[uint]*mobius.Team{
		1: {ID: 1, Name: "teamA", Config: mobius.TeamConfig{
		...
		2: {ID: 2, Name: "teamB", Config: mobius.TeamConfig{
			WebhookSettings: mobius.TeamWebhookSettings{
				FailingPoliciesWebhook: mobius.FailingPoliciesWebhookSettings{
					PolicyIDs: []uint{7, 8},
				},
		...
		4: {ID: 4, Name: "teamD", Config: mobius.TeamConfig{
			WebhookSettings: mobius.TeamWebhookSettings{
				FailingPoliciesWebhook: mobius.FailingPoliciesWebhookSettings{
					Enable:    true,
		...
		5: {ID: 5, Name: "teamE", Config: mobius.TeamConfig{
			WebhookSettings: mobius.TeamWebhookSettings{
				FailingPoliciesWebhook: mobius.FailingPoliciesWebhookSettings{
					PolicyIDs: []uint{15},
				},
	...
	for polID := range pols {
		err := failingPolicySet.AddHost(polID, mobius.PolicySetHost{
	...
	err := failingPolicySet.AddHost(11, mobius.PolicySetHost{
		ID:       11,
	...
	err = TriggerFailingPoliciesAutomation(context.Background(), ds, kitlog.NewNopLogger(), failingPolicySet, func(pol *mobius.Policy, cfg FailingPolicyAutomationConfig) error {
