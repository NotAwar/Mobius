...
func TestQueryResultsStore(t *testing.T) {
	runTest := func(t *testing.T, store *redisQueryResults) {
		...
		campaign1 := mobius.DistributedQueryCampaign{ID: 1}

		ctx1, cancel1 := context.WithCancel(context.Background())
		channel1, err := store.ReadChannel(ctx1, campaign1)
		require.NoError(t, err)

		expected1 := []mobius.DistributedQueryResult{
			{
		...
		campaign2 := mobius.DistributedQueryCampaign{ID: 2}

		ctx2, cancel2 := context.WithCancel(context.Background())
		channel2, err := store.ReadChannel(ctx2, campaign2)
		require.NoError(t, err)

		expected2 := []mobius.DistributedQueryResult{
			{
		...
		var results1, results2 []mobius.DistributedQueryResult
