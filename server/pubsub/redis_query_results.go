...
var _ mobius.QueryResultStore = &redisQueryResults{}
...
func NewRedisQueryResults(pool mobius.RedisPool, duplicateResults bool, logger log.Logger) *redisQueryResults {
...
func (r *redisQueryResults) Pool() mobius.RedisPool {
	return r.pool
}

func (r *redisQueryResults) WriteResult(result mobius.DistributedQueryResult) error {
...
func (r *redisQueryResults) ReadChannel(ctx context.Context, query mobius.DistributedQueryCampaign) (<-chan interface{}, error) {
		...
		for {
			...
			case msg, ok := <-msgChannel:
				if !ok {
					...
					// when Mobius's connection to Redis has been interrupted unexpectedly.
					//
				...
				switch msg := msg.(type) {
				case redigo.Message:
					var res mobius.DistributedQueryResult
