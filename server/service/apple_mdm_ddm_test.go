...
func TestDeclarativeManagement_DeclarationItems(t *testing.T) {
	...
	createDeclaration := func(t *testing.T, uuid, name, identifier string) *mobius.MDMAppleDeclaration {
		declaration := &mobius.MDMAppleDeclaration{
	...
	callDeclarativeManagementAndVerify := func(t *testing.T, hostUUID string,
	...
	) mobius.MDMAppleDDMDeclarationItemsResponse {
		req := mdm.Request{
			Context: ctx,
		...
		// Parse the response
		var declarationItemsResponse mobius.MDMAppleDDMDeclarationItemsResponse
