...
func NewTask(ds mobius.Datastore, pool mobius.RedisPool, clck clock.Clock, conf config.OsqueryConfig) *Task {
...
func storePurgeActiveHostID(pool mobius.RedisPool, zsetKey string, hid uint, reportedAt, purgeOlder time.Time) (int, error) {
	// KEYS[1]: the zsetKey
...
func loadActiveHostIDs(pool mobius.RedisPool, zsetKey string, scanCount int) ([]hostIDLastReported, error) {
...
func removeProcessedHostIDs(pool mobius.RedisPool, zsetKey string, batch []hostIDLastReported) (int, error) {
	...
	//   e.g. if the clocks are not exactly in sync between mobius instances, or
