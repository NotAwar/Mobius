...
func (t *Task) collectHostsLastSeen(ctx context.Context, ds mobius.Datastore, pool mobius.RedisPool, stats *collectorExecStats) error {
	cfg := t.taskConfigs[config.AsyncTaskHostLastSeen]
...
func (t *Task) loadSeenHostsIDs(ctx context.Context, pool mobius.RedisPool) ([]uint, error) {
	cfg := t.taskConfigs[config.AsyncTaskHostLastSeen]
