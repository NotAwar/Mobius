...
func (t *Task) RecordLabelQueryExecutions(ctx context.Context, host *mobius.Host, results map[uint]*bool, ts time.Time, deferred bool) error {
...
func (t *Task) collectLabelQueryExecutions(ctx context.Context, ds mobius.Datastore, pool mobius.RedisPool, stats *collectorExecStats) error {
	cfg := t.taskConfigs[config.AsyncTaskLabelMembership]
...
func (t *Task) GetHostLabelReportedAt(ctx context.Context, host *mobius.Host) time.Time {
	cfg := t.taskConfigs[config.AsyncTaskLabelMembership]
