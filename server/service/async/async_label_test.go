...
func testCollectLabelQueryExecutions(t *testing.T, ds *mysql.Datastore, pool mobius.RedisPool) {
...
func testRecordLabelQueryExecutionsSync(t *testing.T, ds *mock.Store, pool mobius.RedisPool) {
	ctx := context.Background()
	now := time.Now()
...
func testRecordLabelQueryExecutionsAsync(t *testing.T, ds *mock.Store, pool mobius.RedisPool) {
	ctx := context.Background()
	now := time.Now()
	lastYear := now.Add(-365 * 24 * time.Hour)
	host := &mobius.Host{
