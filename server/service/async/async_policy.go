...
func (t *Task) RecordPolicyQueryExecutions(ctx context.Context, host *mobius.Host, results map[uint]*bool, ts time.Time, deferred bool) error {
...
func (t *Task) collectPolicyQueryExecutions(ctx context.Context, ds mobius.Datastore, pool mobius.RedisPool, stats *collectorExecStats) error {
	cfg := t.taskConfigs[config.AsyncTaskPolicyMembership]
  ...
  `)
	...
	getKeyTuples := func(hostID uint) (inserts []mobius.PolicyMembershipResult, err error) {
		...
		inserts = make([]mobius.PolicyMembershipResult, 0, len(res))
		stats.Items += len(res)
		for _, item := range res {
			...
			var tup mobius.PolicyMembershipResult
	...
	runInsertBatch := func(batch []mobius.PolicyMembershipResult) error {
		stats.Inserts++
	...
	insertBatch := make([]mobius.PolicyMembershipResult, 0, cfg.InsertBatch)
...
func (t *Task) GetHostPolicyReportedAt(ctx context.Context, host *mobius.Host) time.Time {
	cfg := t.taskConfigs[config.AsyncTaskPolicyMembership]
