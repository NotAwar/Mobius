...
func testCollectPolicyQueryExecutions(t *testing.T, ds *mysql.Datastore, pool mobius.RedisPool) {
...
func testRecordPolicyQueryExecutionsSync(t *testing.T, ds *mock.Store, pool mobius.RedisPool) {
	ctx := context.Background()
	now := time.Now()
...
func testRecordPolicyQueryExecutionsAsync(t *testing.T, ds *mock.Store, pool mobius.RedisPool) {
	ctx := context.Background()
	now := time.Now()
	lastYear := now.Add(-365 * 24 * time.Hour)
	host := &mobius.Host{
...
func testRecordPolicyQueryExecutionsNoPoliciesSync(t *testing.T, ds *mock.Store, pool mobius.RedisPool) {
	ctx := context.Background()
	now := time.Now()
	lastYear := now.Add(-365 * 24 * time.Hour)
	host := &mobius.Host{
...
func testRecordPolicyQueryExecutionsNoPoliciesAsync(t *testing.T, ds *mock.Store, pool mobius.RedisPool) {
	ctx := context.Background()
	now := time.Now()
	lastYear := now.Add(-365 * 24 * time.Hour)
	host := &mobius.Host{
