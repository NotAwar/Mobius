...
func (t *Task) RecordScheduledQueryStats(ctx context.Context, teamID *uint, hostID uint, stats []mobius.PackStats, ts time.Time) error {
	...
	// mobius.ScheduledQueryStats struct. Using this structure ensures the fields
...
func (t *Task) collectScheduledQueryStats(ctx context.Context, ds mobius.Datastore, pool mobius.RedisPool, stats *collectorExecStats) error {
	cfg := t.taskConfigs[config.AsyncTaskScheduledQueryStats]
	...
	getHostStats := func(hostID uint) (sqStats []mobius.ScheduledQueryStats, schedQueryNames [][2]string, err error) {
	...
	schedIDs, err := ds.ScheduledQueryIDsByName(ctx, mobius.DefaultScheduledQueryIDsByNameBatchSize, schedNames...)
	if err != nil {
	...
	// scheduled queries
	batchStatsByHost := make(map[uint][]mobius.ScheduledQueryStats, len(hostsStats))
