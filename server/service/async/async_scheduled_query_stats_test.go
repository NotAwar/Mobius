...
func testCollectScheduledQueryStats(t *testing.T, ds *mysql.Datastore, pool mobius.RedisPool) {
	...
	cases := []struct {
		name      string
		hostStats map[uint][]mobius.PackStats // key is host ID
		...
		},
		{
			...
			map[uint][]mobius.PackStats{
				hid(1): {
					{PackName: p1.Name, QueryStats: []mobius.ScheduledQueryStats{{ScheduledQueryName: sq1.Name, Executions: 1}}},
				},
			},
			[]hostSQStats{
				{hid(1), sq1.ID, 1},
		...
		},
		{
			...
			map[uint][]mobius.PackStats{
				hid(2): {
					{PackName: p1.Name, QueryStats: []mobius.ScheduledQueryStats{{ScheduledQueryName: sq1.Name, Executions: 2}}},
					{PackName: p2.Name, QueryStats: []mobius.ScheduledQueryStats{{ScheduledQueryName: sq2.Name, Executions: 3}}},
				},
				hid(3): {
					{PackName: p2.Name, QueryStats: []mobius.ScheduledQueryStats{{ScheduledQueryName: sq2.Name, Executions: 4}}},
					{PackName: p3.Name, QueryStats: []mobius.ScheduledQueryStats{{ScheduledQueryName: sq3.Name, Executions: 5}}},
					{PackName: p3.Name, QueryStats: []mobius.ScheduledQueryStats{{ScheduledQueryName: sq4.Name, Executions: 6}}},
				},
			},
			[]hostSQStats{
				{hid(1), sq1.ID, 1},
		...
		},
		{
			...
			map[uint][]mobius.PackStats{
				hid(1): {
					{PackName: p1.Name, QueryStats: []mobius.ScheduledQueryStats{{ScheduledQueryName: sq1.Name, Executions: 7}}},
					{PackName: p2.Name, QueryStats: []mobius.ScheduledQueryStats{{ScheduledQueryName: sq2.Name, Executions: 8}}},
				},
				hid(3): {
					{PackName: p3.Name, QueryStats: []mobius.ScheduledQueryStats{{ScheduledQueryName: sq4.Name, Executions: 9}}},
				},
			},
			[]hostSQStats{
				{hid(1), sq1.ID, 7},
	...
	setupTest := func(t *testing.T, task *Task, data map[uint][]mobius.PackStats) collectorExecStats {
...
func testRecordScheduledQueryStatsSync(t *testing.T, ds *mock.Store, pool mobius.RedisPool) {
	ctx := context.Background()
	now := time.Now()
	host := &mobius.Host{ID: 1}

	stats := []mobius.PackStats{{PackName: "p1", QueryStats: []mobius.ScheduledQueryStats{{ScheduledQueryName: "sq1"}}}}
...
func testRecordScheduledQueryStatsAsync(t *testing.T, ds *mock.Store, pool mobius.RedisPool) {
	ctx := context.Background()
	now := time.Now()
	host := &mobius.Host{ID: 1}

	stats := []mobius.PackStats{
		{
			PackName: "p1", QueryStats: []mobius.ScheduledQueryStats{
				{ScheduledQueryName: "sq1", Executions: 1},
				{ScheduledQueryName: "sq2", Executions: 2},
			},
		},
		{
			PackName: "p2", QueryStats: []mobius.ScheduledQueryStats{
	...
	var sqStat mobius.ScheduledQueryStats
