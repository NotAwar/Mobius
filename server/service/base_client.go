...
type baseClient struct {
	...
	serverCapabilities mobius.CapabilityMap
	...
	// modified afterwards.
	clientCapabilities mobius.CapabilityMap
}
...
func (bc *baseClient) parseResponse(verb, path string, response *http.Response, responseDest interface{}) error {
	...
	if responseDest != nil {
		if e, ok := responseDest.(bodyHandler); ok {
			...
			if e, ok := responseDest.(mobius.Errorer); ok {
				if e.Error() != nil {
...
func (bc *baseClient) setServerCapabilities(response *http.Response) {
	capabilities := response.Header.Get(mobius.CapabilitiesHeader)
	bc.serverCapabilities.PopulateFromString(capabilities)
}

func (bc *baseClient) GetServerCapabilities() mobius.CapabilityMap {
...
func (bc *baseClient) setClientCapabilitiesHeader(req *http.Request) {
	...
	req.Header.Set(mobius.CapabilitiesHeader, bc.clientCapabilities.String())
}
...
func newBaseClient(
	...
	if mobiusClientCert != nil {
		tlsConfig.Certificates = []tls.Certificate{*mobiusClientCert}
	}
