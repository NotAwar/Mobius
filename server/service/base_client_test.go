...
func TestUrlGeneration(t *testing.T) {
	t.Run("without prefix", func(t *testing.T) {
		bc, err := newBaseClient("https://test.com", true, "", "", nil, mobius.CapabilityMap{})
	...
	t.Run("with prefix", func(t *testing.T) {
		bc, err := newBaseClient("https://test.com", true, "", "prefix/", nil, mobius.CapabilityMap{})
		require.NoError(t, err)
...
func TestNewBaseClient(t *testing.T) {
	t.Run("invalid addresses are an error", func(t *testing.T) {
		_, err := newBaseClient("http://foo\x7f.com/", true, "", "", nil, mobius.CapabilityMap{})
		require.Error(t, err)
	})
	...
	t.Run("http is only valid in development", func(t *testing.T) {
		...
		for _, c := range cases {
			_, err := newBaseClient(c.address, c.insecureSkipVerify, "", "", nil, mobius.CapabilityMap{})
...
func TestClientCapabilities(t *testing.T) {
	cases := []struct {
		...
		{"no capabilities", mobius.CapabilityMap{}, ""},
		{"one capability", mobius.CapabilityMap{mobius.Capability("test_capability"): {}}, "test_capability"},
		{
			"multiple capabilities",
			mobius.CapabilityMap{
				mobius.Capability("test_capability"):   {},
				mobius.Capability("test_capability_2"): {},
			},
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			require.ElementsMatch(t, strings.Split(c.expected, ","), strings.Split(req.Header.Get(mobius.CapabilitiesHeader), ","))
		})
	}
}
...
func TestServerCapabilities(t *testing.T) {
	...
	response := &http.Response{
		StatusCode: http.StatusOK,
		Body:       io.NopCloser(bytes.NewBufferString(`{}`)),
		Header:     http.Header{mobius.CapabilitiesHeader: []string{"test_capability"}},
	}
	bc, err := newBaseClient("https://test.com", true, "", "", nil, mobius.CapabilityMap{})
	require.NoError(t, err)
	testCapability := mobius.Capability("test_capability")
	...
	err = bc.parseResponse("", "", response, &struct{}{})
	require.NoError(t, err)
	require.Equal(t, mobius.CapabilityMap{}, bc.serverCapabilities)
	...
	response = &http.Response{
		StatusCode: http.StatusOK,
		Body:       io.NopCloser(bytes.NewBufferString(`{}`)),
		Header:     http.Header{mobius.CapabilitiesHeader: []string{"test_capability,test_capability_2"}},
	}
	err = bc.parseResponse("", "", response, &struct{}{})
	require.NoError(t, err)
	require.Equal(t, mobius.CapabilityMap{
		testCapability:                        {},
		mobius.Capability("test_capability_2"): {},
	}, bc.serverCapabilities)
	require.True(t, bc.GetServerCapabilities().Has(testCapability))
	require.True(t, bc.GetServerCapabilities().Has(mobius.Capability("test_capability")))
}
...
func TestClientCertificateAuth(t *testing.T) {
	...
	bc, err := newBaseClient(ts.URL, true, "", "", nil, mobius.CapabilityMap{})
	require.NoError(t, err)
	...
	bc, err = newBaseClient(ts.URL, true, "", "", &clientCrt.Crt, mobius.CapabilityMap{})
	require.NoError(t, err)
	request, err = http.NewRequest("GET", ts.URL, nil)
	require.NoError(t, err)
	_, err = bc.http.Do(request)
