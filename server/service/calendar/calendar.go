...
const (
	...
	// If a Mobius server gets the lock and is then shut down before releasing the lock, the next server may need to wait this long
...
type Config struct {
	...
	mobius.GoogleCalendarIntegration
	ServerURL string
}

// PolicyLiteWithMeta is a wrapper around mobius.PolicyLite that includes a tag for policy's description/resolution.
...
func CreateUserCalendarFromConfig(ctx context.Context, config *Config, logger kitlog.Logger) mobius.UserCalendar {
	// Enterprise calendar functionality removed - return nil stub
	return nil
}

func GenerateCalendarEventBody(ctx context.Context, ds mobius.Datastore, orgName string, host mobius.HostPolicyMembershipData,
	...
	conflictStr := ""
	if conflict {
		conflictStr = mobius.CalendarEventConflictText
	}
...
%s
`, orgName, mobius.CalendarBodyStaticHeader, conflictStr, host.HostDisplayName, description, resolution), tag
}

func getCalendarEventDescriptionAndResolution(ctx context.Context, ds mobius.Datastore, orgName string, host mobius.HostPolicyMembershipData,
	...
	getDefaultDescription := func() string {
		return fmt.Sprintf(`%s %s`, orgName, mobius.CalendarDefaultDescription)
	}
	...
	if len(policyIDs) == 1 && policyIDs[0] != "" {
		...
		if policyDescription == "" || policy.Resolution == nil || strings.TrimSpace(*policy.Resolution) == "" {
			...
			resolution = mobius.CalendarDefaultResolution
