...
func TestListCarves(t *testing.T) {
	...
	// admin user
	carves, err := svc.ListCarves(test.UserContext(ctx, test.UserAdmin), mobius.CarveListOptions{})
	require.NoError(t, err)
	require.Len(t, carves, 2)

	// only global admin can read carves
	_, err = svc.ListCarves(test.UserContext(ctx, test.UserNoRoles), mobius.CarveListOptions{})
	require.Error(t, err)
	require.Contains(t, err.Error(), authz.ForbiddenErrorMessage)

	// no user in context
	_, err = svc.ListCarves(ctx, mobius.CarveListOptions{})
	require.Error(t, err)
	require.Contains(t, err.Error(), authz.ForbiddenErrorMessage)
}
...
func TestGetCarve(t *testing.T) {
	...
	ds.CarveFunc = func(ctx context.Context, id int64) (*mobius.CarveMetadata, error) {
		return &mobius.CarveMetadata{
			ID: id,
		}, nil
	}
...
func TestCarveGetBlock(t *testing.T) {
	...
	ds.GetBlockFunc = func(ctx context.Context, carve *mobius.CarveMetadata, blockId int64) ([]byte, error) {
...
func TestCarveBegin(t *testing.T) {
	...
	payload := mobius.CarveBeginPayload{
		BlockCount: 23,
		BlockSize:  64,
		CarveSize:  23 * 64,
		RequestId:  "carve_request",
	...
	expectedMetadata := mobius.CarveMetadata{
		ID:         7,
		HostId:     host.ID,
		BlockCount: 23,
		BlockSize:  64,
	...
	ms.NewCarveFunc = func(ctx context.Context, metadata *mobius.CarveMetadata) (*mobius.CarveMetadata, error) {
		metadata.ID = 7
		return metadata, nil
	}
...
func TestCarveBeginEmptyError(t *testing.T) {
	...
	ctx := hostctx.NewContext(context.Background(), &mobius.Host{ID: 1})
	...
	_, err := svc.CarveBegin(ctx, mobius.CarveBeginPayload{})
	require.Error(t, err)
...
func TestCarveBeginMissingHostError(t *testing.T) {
	...
	_, err := svc.CarveBegin(context.Background(), mobius.CarveBeginPayload{})
	require.Error(t, err)
...
func TestCarveCarveBlockGetCarveError(t *testing.T) {
	...
	ms := new(mock.Store)
	svc := &Service{carveStore: ms}
	ms.CarveBySessionIdFunc = func(ctx context.Context, sessionId string) (*mobius.CarveMetadata, error) {
		return nil, errors.New("ouch!")
	}

	payload := mobius.CarveBlockPayload{
...
func TestCarveCarveBlockBlockCountExceedError(t *testing.T) {
	...
	ms.UpdateCarveFunc = func(ctx context.Context, carve *mobius.CarveMetadata) error {
...
func TestCarveCarveBlockNewBlockError(t *testing.T) {
	...
	ms.NewBlockFunc = func(ctx context.Context, carve *mobius.CarveMetadata, blockId int64, data []byte) error {
