...
// Client is used to consume Mobius APIs from Go code
type Client struct {
	*baseClient
	addr          string
	token         string
...
func NewClient(addr string, insecureSkipVerify bool, rootCA, urlPrefix string, options ...ClientOption) (*Client, error) {
	...
	baseClient, err := newBaseClient(addr, insecureSkipVerify, rootCA, urlPrefix, nil, mobius.CapabilityMap{})
	if err != nil {
		return nil, err
	}
...
func (c *Client) doContextWithBodyAndHeaders(ctx context.Context, verb, path, rawQuery string, bodyBytes []byte, headers map[string]string) (*http.Response, error) {
	...
	if resp.Header.Get(mobius.HeaderLicenseKey) == mobius.HeaderLicenseValueExpired {
		mobius.WriteExpiredLicenseBanner(c.errWriter)
	}
...
func (c *Client) CheckAnyMDMEnabled() error {
	return c.runAppConfigChecks(func(ac *mobius.EnrichedAppConfig) error {
		if !ac.MDM.EnabledAndConfigured && !ac.MDM.WindowsEnabledAndConfigured {
			return errors.New(mobius.MDMNotConfiguredMessage)
		}
		return nil
	})
}
...
func (c *Client) CheckAppleMDMEnabled() error {
	return c.runAppConfigChecks(func(ac *mobius.EnrichedAppConfig) error {
		if !ac.MDM.EnabledAndConfigured {
			return errors.New(mobius.AppleMDMNotConfiguredMessage)
		}
		return nil
	})
}
...
func (c *Client) runAppConfigChecks(fn func(ac *mobius.EnrichedAppConfig) error) error {
...
func getProfilesContents(baseDir string, macProfiles []mobius.MDMProfileSpec, windowsProfiles []mobius.MDMProfileSpec, expandEnv bool) ([]mobius.MDMProfileBatchPayload, error) {
	// map to check for duplicate names across all profiles
	extByName := make(map[string]string, len(macProfiles))
	result := make([]mobius.MDMProfileBatchPayload, 0, len(macProfiles))

	// iterate over the profiles for each platform
	for platform, profiles := range map[string][]mobius.MDMProfileSpec{
		"macos":   macProfiles,
		...
		for _, profile := range profiles {
			...
			if platform == "macos" {
				...
				case ".mobileconfig", ".xml": // allowing .xml for backwards compatibility
					mc, err := mobius.NewMDMAppleConfigProfile(fileContents, nil)
					if err != nil {
...
//
//	> from Noah: "We want to support existing features w/ mobiuscli apply for
//	> backwards compatibility GitOps but we don’t need to add new features."
//
// We should deprecate ApplyGroup and use it only for `mobiuscli apply` (and
...
// ApplyGroup applies the given spec group to Mobius.
func (c *Client) ApplyGroup(
	...
	appconfig *mobius.EnrichedAppConfig,
	opts mobius.ApplyClientSpecOptions,
	teamsSoftwareInstallers map[string][]mobius.SoftwarePackageResponse,
	teamsVPPApps map[string][]mobius.VPPAppResponse,
	teamsScripts map[string][]mobius.ScriptResponse,
) (map[string]uint, map[string][]mobius.SoftwarePackageResponse, map[string][]mobius.VPPAppResponse, map[string][]mobius.ScriptResponse, error) {
	...
	if specs.AppConfig != nil {
		...
		if macosSetup := extractAppCfgMacOSSetup(specs.AppConfig); macosSetup != nil {
			...
			case macosSetup.BootstrapPackage.Value != "":
				...
				if err != nil {
					return nil, nil, nil, nil, fmt.Errorf("applying mobius config: %w", err)
				}
		...
		if scripts := extractAppCfgScripts(specs.AppConfig); scripts != nil {
			scriptPayloads := make([]mobius.ScriptPayload, len(scripts))
			for i, f := range scripts {
				b, err := os.ReadFile(f)
				if err != nil {
					return nil, nil, nil, nil, fmt.Errorf("applying no-team scripts: %w", err)
				}
				scriptPayloads[i] = mobius.ScriptPayload{
					ScriptContents: b,
		...
		if rules != nil {
			rulePayloads := make([]mobius.YaraRule, len(rules))
			for i, f := range rules {
				...
				rulePayloads[i] = mobius.YaraRule{
					Contents: string(b),
		...
		if appconfig != nil {
			specs.AppConfig.(map[string]interface{})["gitops"] = mobius.UIGitOpsModeConfig{
		...
		if opts.DryRun {
			logfn("[+] would've applied mobius config\n")
		} else {
			logfn("[+] applied mobius config\n")
		}

		// We apply profiles after the main AppConfig org_settings because profiles may
		// contain Mobius variables that are set in org_settings, such as $MOBIUS_VAR_DIGICERT_PASSWORD_My_CA
		//
		...
		if (windowsCustomSettings != nil && macosCustomSettings != nil) || len(windowsCustomSettings)+len(macosCustomSettings) > 0 {
			...
			// this HTTP request gets routed to another Mobius server.
	...
	if len(specs.Teams) > 0 {
		...
		tmFileContents := make(map[string][]mobius.MDMProfileBatchPayload, len(tmMDMSettings))
		...
		tmBootstrapPackages := make(map[string]*mobius.MDMAppleBootstrapPackage, len(tmMacSetup))
		...
		tmMacSetupSoftware := make(map[string][]*mobius.MacOSSetupSoftware, len(tmMacSetup))
		...
		tmSoftwareMacOSSetup := make(map[string]map[mobius.MacOSSetupSoftware]struct{}, len(tmMacSetup))
		...
		tmScriptsPayloads := make(map[string][]mobius.ScriptPayload, len(tmScripts))
		for k, paths := range tmScripts {
			scriptPayloads := make([]mobius.ScriptPayload, len(paths))
			for i, f := range paths {
				b, err := os.ReadFile(f)
				if err != nil {
					return nil, nil, nil, nil, fmt.Errorf("applying mobius config: %w", err)
		...
		tmSoftwarePackagesPayloads := make(map[string][]mobius.SoftwareInstallerPayload, len(tmSoftwarePackages))
		tmSoftwarePackageByPath := make(map[string]map[string]mobius.SoftwarePackageSpec, len(tmSoftwarePackages))
			...
			for _, swSpec := range software {
				if swSpec.ReferencedYamlPath != "" {
					...
					if tmSoftwarePackageByPath[tmName] == nil {
						tmSoftwarePackageByPath[tmName] = make(map[string]mobius.SoftwarePackageSpec, len(software))
					}
		...
		tmMobiusMaintainedApps := extractTmSpecsMobiusMaintainedApps(specs.Teams)
		for tmName, apps := range tmMobiusMaintainedApps {
			installDuringSetupKeys := tmSoftwareMacOSSetup[tmName]
		...
		tmSoftwareAppsPayloads := make(map[string][]mobius.VPPBatchPayload)
		tmSoftwareAppsByAppID := make(map[string]map[string]mobius.TeamSpecAppStoreApp, len(tmSoftwareApps))
		for tmName, apps := range tmSoftwareApps {
			installDuringSetupKeys := tmSoftwareMacOSSetup[tmName]
			appPayloads := make([]mobius.VPPBatchPayload, 0, len(apps))
			for _, app := range apps {
				var installDuringSetup *bool
				if installDuringSetupKeys != nil {
					_, ok := installDuringSetupKeys[mobius.MacOSSetupSoftware{AppStoreID: app.AppStoreID}]
					installDuringSetup = &ok
				}
				appPayloads = append(appPayloads, mobius.VPPBatchPayload{
				...
				if tmSoftwareAppsByAppID[tmName] == nil {
					tmSoftwareAppsByAppID[tmName] = make(map[string]mobius.TeamSpecAppStoreApp, len(apps))
				}
		...
		var err error
		teamOpts := mobius.ApplyTeamSpecOptions{
						...
						if err := c.uploadMacOSSetupAssistant(b, &tmID, tmMacSetup[tmName].MacOSSetupAssistant.Value); err != nil {
							if strings.Contains(err.Error(), "Couldn't add") {
								...
								return nil, nil, nil, nil, fmt.Errorf("Couldn't edit macos_setup_assistant. Response from Apple: %s. Learn more at %s",
									strings.Trim(parts[1], " "), "https://mobiusmdm.com/learn-more-about/dep-profile")
							}
	...
	if len(specs.Policies) > 0 {
		...
		if policyName := mobius.FirstDuplicatePolicySpecName(specs.Policies); policyName != "" {
...
func extractTeamOrNoTeamMacOSSetupSoftware(baseDir string, software []*mobius.MacOSSetupSoftware) (map[mobius.MacOSSetupSoftware]struct{}, error) {
	m := make(map[mobius.MacOSSetupSoftware]struct{}, len(software))
...
func validateTeamOrNoTeamMacOSSetupSoftware(teamName string, macOSSetupSoftware []*mobius.MacOSSetupSoftware, packagesByPath map[string]mobius.SoftwarePackageSpec, vppAppsByAppID map[string]mobius.TeamSpecAppStoreApp) error {
	// if macos_setup.software has some values, they must exist in the software
	// packages or vpp apps.
...
func buildSoftwarePackagesPayload(specs []mobius.SoftwarePackageSpec, installDuringSetupKeys map[mobius.MacOSSetupSoftware]struct{}) ([]mobius.SoftwareInstallerPayload, error) {
	softwarePayloads := make([]mobius.SoftwareInstallerPayload, len(specs))
	for i, si := range specs {
		...
		if si.PreInstallQuery.Path != "" {
			...
			if _, ok := top.(map[any]any); ok {
				...
				// Gitops format
				var querySpecs []mobius.QuerySpec
		...
		var installDuringSetup *bool
		if installDuringSetupKeys != nil {
			_, ok := installDuringSetupKeys[mobius.MacOSSetupSoftware{PackagePath: si.ReferencedYamlPath}]
			installDuringSetup = &ok
		}
		softwarePayloads[i] = mobius.SoftwareInstallerPayload{
			URL:                si.URL,
...
func extractAppCfgMacOSSetup(appCfg any) *mobius.MacOSSetup {
	...
	// GitOps
	mosGitOps, ok := mmdm["macos_setup"].(*mobius.MacOSSetup)
	if ok {
		return mosGitOps
	}
...
func extractAppCfgCustomSettings(appCfg interface{}, platformKey string) []mobius.MDMProfileSpec {
	asMap, ok := appCfg.(map[string]interface{})
	if !ok {
		return nil
	}
	...
	mos, ok := mmdm[platformKey].(mobius.WithMDMProfileSpecs)
	if !ok || mos == nil {
...
// legacyExtractAppCfgCustomSettings is used to extract custom settings for legacy mobiuscli apply use case
func legacyExtractAppCfgCustomSettings(mmdm map[string]interface{}, platformKey string) []mobius.MDMProfileSpec {
	...
	csSpecs := make([]mobius.MDMProfileSpec, 0, len(csAny))
	for _, v := range csAny {
		if m, ok := v.(map[string]interface{}); ok {
			var profSpec mobius.MDMProfileSpec
			...
			if m != "" {
				csSpecs = append(csSpecs, mobius.MDMProfileSpec{Path: m})
			}
		}
	}
	return csSpecs
...
func extractAppCfgMacOSCustomSettings(appCfg interface{}) []mobius.MDMProfileSpec {
	return extractAppCfgCustomSettings(appCfg, "macos_settings")
}

func extractAppCfgWindowsCustomSettings(appCfg interface{}) []mobius.MDMProfileSpec {
...
func extractAppCfgYaraRules(appCfg interface{}) ([]mobius.YaraRuleSpec, error) {
	asMap, ok := appCfg.(map[string]interface{})
	if !ok {
	...
	if !ok {
		...
		return []mobius.YaraRuleSpec{}, nil
	}
	...
	ruleSpecs := make([]mobius.YaraRuleSpec, 0, len(rulesAny))
	for _, v := range rulesAny {
		...
		ruleSpecs = append(ruleSpecs, mobius.YaraRuleSpec{Path: path})
	}
...
func extractTmSpecsMDMCustomSettings(tmSpecs []json.RawMessage) map[string]profileSpecsByPlatform {
	...
	for _, tm := range tmSpecs {
		...
		if spec.Name != "" {
			var macOSSettings []mobius.MDMProfileSpec
			var windowsSettings []mobius.MDMProfileSpec
			...
			if len(spec.MDM.MacOSSettings.CustomSettings) > 0 {
				...
				if macOSSettings == nil {
					...
					macOSSettings = []mobius.MDMProfileSpec{}
				}
			}
			if len(spec.MDM.WindowsSettings.CustomSettings) > 0 {
				...
				if windowsSettings == nil {
					// to be consistent with the AppConfig custom settings, set it to an
					// empty slice if the provided custom settings are present but empty.
					windowsSettings = []mobius.MDMProfileSpec{}
				}
			}
...
func extractTmSpecsSoftwarePackages(tmSpecs []json.RawMessage) map[string][]mobius.SoftwarePackageSpec {
	var m map[string][]mobius.SoftwarePackageSpec
	for _, tm := range tmSpecs {
		var spec struct {
			Name     string          `json:"name"`
		...
		if spec.Name != "" && len(spec.Software) > 0 {
			if m == nil {
				m = make(map[string][]mobius.SoftwarePackageSpec)
			}
			var software mobius.SoftwareSpec
			var packages []mobius.SoftwarePackageSpec
...
func extractTmSpecsMobiusMaintainedApps(tmSpecs []json.RawMessage) map[string][]mobius.SoftwarePackageSpec {
	var m map[string][]mobius.SoftwarePackageSpec
	for _, tm := range tmSpecs {
		var spec struct {
			Name     string          `json:"name"`
		...
		if spec.Name != "" && len(spec.Software) > 0 {
			...
			if !software.MobiusMaintainedApps.Valid {
				// to be consistent with the AppConfig custom settings, set it to an
				// empty slice if the provided custom settings are present but empty.
				packages = []mobius.SoftwarePackageSpec{}
			} else {
				for _, app := range software.MobiusMaintainedApps.Value {
					packages = append(packages, mobius.SoftwarePackageSpec{
						Slug:             &app.Slug,
...
func extractTmSpecsSoftwareApps(tmSpecs []json.RawMessage) map[string][]mobius.TeamSpecAppStoreApp {
	var m map[string][]mobius.TeamSpecAppStoreApp
	for _, tm := range tmSpecs {
		var spec struct {
			Name     string          `json:"name"`
			Software json.RawMessage `json:"software"`
		...
		if spec.Name != "" && len(spec.Software) > 0 {
			if m == nil {
				m = make(map[string][]mobius.TeamSpecAppStoreApp)
			}
			var software mobius.SoftwareSpec
			var apps []mobius.TeamSpecAppStoreApp
			if err := json.Unmarshal(spec.Software, &software); err != nil {
				// ignore, will fail in apply team specs call
				continue
			}
			if !software.AppStoreApps.Valid {
				// to be consistent with the AppConfig custom settings, set it to an
				// empty slice if the provided custom settings are present but empty.
				apps = []mobius.TeamSpecAppStoreApp{}
			} else {
...
func extractTmSpecsMacOSSetup(tmSpecs []json.RawMessage) map[string]*mobius.MacOSSetup {
	var m map[string]*mobius.MacOSSetup
	for _, tm := range tmSpecs {
		var spec struct {
			Name string `json:"name"`
			MDM  struct {
		...
		if spec.Name != "" {
			if m == nil {
				m = make(map[string]*mobius.MacOSSetup)
			}
...
func (c *Client) SaveEnvSecrets(alreadySaved map[string]string, toSave map[string]string, dryRun bool) error {
	...
	var secretsToSave []mobius.SecretVariable
	for key, value := range toSave {
		if _, ok := alreadySaved[key]; !ok {
			secretsToSave = append(secretsToSave, mobius.SecretVariable{Name: key, Value: value})
...
// DoGitOps applies the GitOps config to Mobius.
func (c *Client) DoGitOps(
	...
	logf func(format string, args ...interface{}),
	dryRun bool,
	teamDryRunAssumptions *mobius.TeamSpecsDryRunAssumptions,
	appConfig *mobius.EnrichedAppConfig,
	// pass-by-ref to build lists
	teamsSoftwareInstallers map[string][]mobius.SoftwarePackageResponse,
	teamsVPPApps map[string][]mobius.VPPAppResponse,
	teamsScripts map[string][]mobius.ScriptResponse,
) (*mobius.TeamSpecsDryRunAssumptions, []func() error, error) {
	baseDir := filepath.Dir(fullFilename)
	filename := filepath.Base(fullFilename)
	var teamAssumptions *mobius.TeamSpecsDryRunAssumptions
	var err error
	logFn := func(format string, args ...interface{}) {
		if logf != nil {
			logf(format, args...)
	...
	if config.TeamName == nil {
		...
		group.EnrollSecret = &mobius.EnrollSecretSpec{Secrets: config.OrgSettings["secrets"].([]*mobius.EnrollSecret)}
		...
		if digicertIntegration, ok := integrations.(map[string]interface{})["digicert"]; !ok || digicertIntegration == nil {
			...
			var digicertData optjson.Slice[mobius.DigiCertIntegration]
		...
		if customSCEPIntegration, ok := integrations.(map[string]interface{})["custom_scep_proxy"]; !ok || customSCEPIntegration == nil {
			...
			var customSCEPData optjson.Slice[mobius.CustomSCEPProxyIntegration]
		...
		if windowsEnabledAndConfiguredAssumption, ok := mdmAppConfig["windows_enabled_and_configured"].(bool); ok {
			teamAssumptions = &mobius.TeamSpecsDryRunAssumptions{
		...
		team["software"].(map[string]any)["mobius_maintained_apps"] = config.Software.MobiusMaintainedApps
	...
	if !config.IsNoTeam() {
		...
		if config.Controls.MacOSSettings != nil {
			...
			mdmAppConfig["macos_settings"] = mobius.MacOSSettings{
				CustomSettings: []mobius.MDMProfileSpec{},
			}
		}
		...
		if config.Controls.MacOSSetup != nil {
			...
			mdmAppConfig["macos_setup"] = mobius.NewMacOSSetupWithDefaults()
		}
		...
		if config.Controls.WindowsSettings != nil {
			...
			mdmAppConfig["windows_settings"] = mobius.WindowsSettings{
				CustomSettings: optjson.Slice[mobius.MDMProfileSpec]{Value: []mobius.MDMProfileSpec{}},
			}
		}
	...
	teamIDsByName, teamsSoftwareInstallers, teamsVPPApps, teamsScripts, err := c.ApplyGroup(ctx, true, &group, baseDir, logf, appConfig, mobius.ApplyClientSpecOptions{
	...
	var teamSoftwareInstallers []mobius.SoftwarePackageResponse
	var teamVPPApps []mobius.VPPAppResponse
	var teamScripts []mobius.ScriptResponse
...
func (c *Client) doGitOpsNoTeamSetupAndSoftware(
	...
	logFn func(format string, args ...interface{}),
	dryRun bool,
) ([]mobius.SoftwarePackageResponse, []mobius.VPPAppResponse, error) {
	if !config.IsNoTeam() || appconfig == nil || !appconfig.License.IsPremium() {
		return nil, nil, nil
	}

	var macOSSetup mobius.MacOSSetup
	if config.Controls.MacOSSetup == nil {
	...
	var softwareInstallers []mobius.SoftwarePackageResponse

	packages := make([]mobius.SoftwarePackageSpec, 0, len(config.Software.Packages))
	packagesByPath := make(map[string]mobius.SoftwarePackageSpec, len(config.Software.Packages))
	...
	for _, software := range config.Software.MobiusMaintainedApps {
		if software != nil {
			packages = append(packages, mobius.SoftwarePackageSpec{
				Slug:             &software.Slug,
	...
	var appsPayload []mobius.VPPBatchPayload
	appsByAppID := make(map[string]mobius.TeamSpecAppStoreApp, len(config.Software.AppStoreApps))
	for _, vppApp := range config.Software.AppStoreApps {
		if vppApp != nil {
			// can be referenced by macos_setup.software
			appsByAppID[vppApp.AppStoreID] = *vppApp

			_, installDuringSetup := noTeamSoftwareMacOSSetup[mobius.MacOSSetupSoftware{AppStoreID: vppApp.AppStoreID}]
			appsPayload = append(appsPayload, mobius.VPPBatchPayload{
	...
	softwareInstallers, err = c.ApplyNoTeamSoftwareInstallers(swPkgPayload, mobius.ApplySpecOptions{DryRun: dryRun})
	if err != nil {
		return nil, nil, fmt.Errorf("applying software installers: %w", err)
	}
	logFn("[+] applying %d app store apps for 'No team'\n", len(appsPayload))
	vppApps, err := c.ApplyNoTeamAppStoreAppsAssociation(appsPayload, mobius.ApplySpecOptions{DryRun: dryRun})
	if err != nil {
...
func (c *Client) doGitOpsLabels(config *spec.GitOps, logFn func(format string, args ...interface{}), dryRun bool) ([]string, error) {
	...
	for _, persistedLabel := range persistedLabels {
		if persistedLabel.LabelType == mobius.LabelTypeBuiltIn {
			continue
		}
		if slices.IndexFunc(config.Labels, func(configLabel *mobius.LabelSpec) bool { return configLabel.Name == persistedLabel.Name }) == -1 {
...
func (c *Client) doGitOpsPolicies(config *spec.GitOps, teamSoftwareInstallers []mobius.SoftwarePackageResponse, teamVPPApps []mobius.VPPAppResponse, teamScripts []mobius.ScriptResponse, logFn func(format string, args ...interface{}), dryRun bool) error {
	...
	if teamID != nil {
		...
		for _, vppApp := range teamVPPApps {
			if vppApp.Platform != mobius.MacOSPlatform {
	...
	if len(config.Policies) > 0 {
		...
		if !dryRun {
			...
			for i := 0; i < len(config.Policies); i += batchSize {
				...
				policiesSpec := make([]*mobius.PolicySpec, len(policiesToApply))
...
func (c *Client) GetGitOpsSecrets(
	...
	if config.TeamName == nil {
		...
		if ok {
			secrets, ok := orgSecrets.([]*mobius.EnrollSecret)
			if ok {
		...
		if ok {
			secrets, ok := teamSecrets.([]*mobius.EnrollSecret)
			if ok {
				secretValues := make([]string, 0, len(secrets))
				for _, secret := range secrets {
					secretValues = append(secretValues, secret.Secret)
