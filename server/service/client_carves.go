...
func (c *Client) ListCarves(opt mobius.CarveListOptions) ([]*mobius.CarveMetadata, error) {
	endpoint := "/api/latest/mobius/carves"
	rawQuery := ""
	if opt.Expired {
		rawQuery = "expired=1"
	}
	...
	if err != nil {
		return nil, fmt.Errorf("GET /api/latest/mobius/carves: %w", err)
	}
...
func (c *Client) GetCarve(carveId int64) (*mobius.CarveMetadata, error) {
	endpoint := fmt.Sprintf("/api/latest/mobius/carves/%d", carveId)
...
func (c *Client) getCarveBlock(carveId, blockId int64) ([]byte, error) {
	path := fmt.Sprintf(
		"/api/latest/mobius/carves/%d/block/%d",
		carveId,
		blockId,
	)
...
func newCarveReader(carve mobius.CarveMetadata, client *Client) *carveReader {
	return &carveReader{
		carve:     carve,
		client:    client,
		bytesRead: 0,
...
func (c *Client) DownloadCarve(id int64) (io.Reader, error) {
	path := fmt.Sprintf("/api/latest/mobius/carves/%d", id)
	response, err := c.AuthenticatedDo("GET", path, "", nil)
	if err != nil {
		return nil, fmt.Errorf("GET %s: %w", path, err)
	}
