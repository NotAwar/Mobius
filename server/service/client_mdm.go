...
func (c *Client) GetAppleMDM() (*mobius.AppleMDM, error) {
	verb, path := "GET", "/api/latest/mobius/mdm/apple"
...
func (c *Client) GetAppleBM() (*mobius.AppleBM, error) {
	verb, path := "GET", "/api/latest/mobius/mdm/apple_bm"
...
func (c *Client) CountABMTokens() (int, error) {
	verb, path := "GET", "/api/latest/mobius/abm_tokens/count"
	var responseBody countABMTokensResponse
	err := c.authenticatedRequestWithQuery(nil, verb, path, &responseBody, "")
	return responseBody.Count, err
}

// RequestAppleCSR requests a signed CSR from the Mobius server and returns the
// CSR bytes
func (c *Client) RequestAppleCSR() ([]byte, error) {
	verb, path := "GET", "/api/latest/mobius/mdm/apple/request_csr"
	var resp getMDMAppleCSRResponse
	err := c.authenticatedRequest(nil, verb, path, &resp)
	return resp.CSR, err
}

// RequestAppleABM requests a signed CSR from the Mobius server and returns the
// public key bytes
func (c *Client) RequestAppleABM() ([]byte, error) {
	verb, path := "GET", "/api/latest/mobius/mdm/apple/abm_public_key"
	var resp generateABMKeyPairResponse
	err := c.authenticatedRequest(nil, verb, path, &resp)
	return resp.PublicKey, err
}

func (c *Client) GetBootstrapPackageMetadata(teamID uint, forUpdate bool) (*mobius.MDMAppleBootstrapPackage, error) {
	verb, path := "GET", fmt.Sprintf("/api/latest/mobius/mdm/bootstrap/%d/metadata", teamID)
...
func (c *Client) DeleteBootstrapPackage(teamID uint, dryRun bool) error {
	verb, path := "DELETE", fmt.Sprintf("/api/latest/mobius/mdm/bootstrap/%d", teamID)
...
func (c *Client) UploadBootstrapPackage(pkg *mobius.MDMAppleBootstrapPackage, dryRun bool) error {
	verb, path := "POST", "/api/latest/mobius/mdm/bootstrap"
...
func (c *Client) UploadBootstrapPackageIfNeeded(bp *mobius.MDMAppleBootstrapPackage, teamID uint, dryRun bool) error {
	isFirstTime := false
...
func (c *Client) ValidateBootstrapPackageFromURL(url string) (*mobius.MDMAppleBootstrapPackage, error) {
...
func downloadRemoteMacosBootstrapPackage(pkgURL string) (*mobius.MDMAppleBootstrapPackage, error) {
	...
	if err := file.CheckPKGSignature(pkgReader); err != nil {
		...
		case errors.Is(err, file.ErrNotSigned):
			return nil, errors.New("Couldn’t edit bootstrap_package. The bootstrap_package must be signed. Learn how to sign the package in the Mobius documentation: https://mobiusmdm.com/docs/using-mobius/mdm-macos-setup-experience#step-2-sign-the-package")
		default:
...
func (c *Client) uploadMacOSSetupAssistant(data []byte, teamID *uint, name string) error {
	verb, path := http.MethodPost, "/api/latest/mobius/enrollment_profiles/automatic"
...
func (c *Client) deleteMacOSSetupAssistant(teamID *uint) error {
	verb, path := http.MethodDelete, "/api/latest/mobius/enrollment_profiles/automatic"
...
func (c *Client) MDMListCommands(opts mobius.MDMCommandListOptions) ([]*mobius.MDMCommand, error) {
	const defaultCommandsPerPage = 20

	verb, path := http.MethodGet, "/api/latest/mobius/mdm/commands"
...
func (c *Client) MDMGetCommandResults(commandUUID string) ([]*mobius.MDMCommandResult, error) {
	verb, path := http.MethodGet, "/api/latest/mobius/mdm/commandresults"
...
func (c *Client) RunMDMCommand(hostUUIDs []string, rawCmd []byte, forPlatform string) (*mobius.CommandEnqueueResult, error) {
	...
	if err := c.authenticatedRequest(request, "POST", "/api/latest/mobius/mdm/commands/run", &response); err != nil {
...
func (c *Client) prepareWindowsMDMCommand(rawCmd []byte) ([]byte, error) {
	if _, err := mobius.ParseWindowsMDMCommand(rawCmd); err != nil {
		return nil, err
	}
...
func (c *Client) MDMLockHost(hostID uint) error {
	...
	if err := c.authenticatedRequest(nil, "POST", fmt.Sprintf("/api/latest/mobius/hosts/%d/lock", hostID), &response); err != nil {
...
func (c *Client) MDMUnlockHost(hostID uint) (string, error) {
	...
	if err := c.authenticatedRequest(nil, "POST", fmt.Sprintf("/api/latest/mobius/hosts/%d/unlock", hostID), &response); err != nil {
		return "", fmt.Errorf("lock host request: %w", err)
	}
...
func (c *Client) MDMWipeHost(hostID uint) error {
	...
	if err := c.authenticatedRequest(nil, "POST", fmt.Sprintf("/api/latest/mobius/hosts/%d/wipe", hostID), &response); err != nil {
