...
func (c *Client) DeleteProfile(profileID uint) error {
	verb, path := "DELETE", "/api/latest/mobius/mdm/apple/profiles/"+strconv.FormatUint(uint64(profileID), 10)
	var responseBody deleteMDMAppleConfigProfileResponse
	return c.authenticatedRequest(nil, verb, path, &responseBody)
}

func (c *Client) ListProfiles(teamID *uint) ([]*mobius.MDMAppleConfigProfile, error) {
	verb, path := "GET", "/api/latest/mobius/mdm/apple/profiles"
...
func (c *Client) ListConfigurationProfiles(teamID *uint) ([]*mobius.MDMConfigProfilePayload, error) {
	verb, path := "GET", "/api/latest/mobius/configuration_profiles"
	query := make(url.Values)
	if teamID != nil {
		query.Add("team_id", strconv.FormatUint(uint64(*teamID), 10))
	}
...
func (c *Client) GetProfileContents(profileID string) ([]byte, error) {
	verb, path := "GET", "/api/latest/mobius/mdm/profiles/"+profileID
...
func (c *Client) AddProfile(teamID uint, configurationProfile []byte) (uint, error) {
	...
	request, err := http.NewRequest(
		"POST",
		c.baseURL.String()+"/api/latest/mobius/mdm/apple/profiles",
		body,
	)
	if err != nil {
		return 0, err
	...
	if response.Header.Get(mobius.HeaderLicenseKey) == mobius.HeaderLicenseValueExpired {
...
func (c *Client) GetConfigProfilesSummary(teamID *uint) (*mobius.MDMProfilesSummary, error) {
	verb, path := "GET", "/api/latest/mobius/mdm/profiles/summary"
	query := make(url.Values)
	if teamID != nil {
		query.Add("team_id", strconv.FormatUint(uint64(*teamID), 10))
	}
