...
// Setup attempts to setup the current Mobius instance. If setup is successful,
// an auth token is returned.
func (c *Client) Setup(email, name, password, org string) (string, error) {
	params := setupRequest{
		Admin: &mobius.UserPayload{
			Email:    &email,
			Name:     &name,
			Password: &password,
		},
	...
	// If setup has already been completed, Mobius will not serve the setup
