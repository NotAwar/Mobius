// Package conditional_access_microsoft_proxy is the client HTTP package to operate on Entra through Mobius's MS proxy.
...
type CreateResponse struct {
	...
	Secret   string `json:"mobius_server_secret"`
}
...
func (p *Proxy) Get(ctx context.Context, tenantID string, secret string) (*GetResponse, error) {
	...
	if err := p.get(
		...
		fmt.Sprintf("entraTenantId=%s&mobiusServerSecret=%s", tenantID, secret),
		&getResponse,
	); err != nil {
...
type setComplianceStatusRequest struct {
	TenantID string `json:"entraTenantId"`
	Secret   string `json:"mobiusServerSecret"`
...
func (p *Proxy) GetMessageStatus(
	...
	if err := p.get(
		...
		fmt.Sprintf("entraTenantId=%s&mobiusServerSecret=%s&messageId=%s", tenantID, secret, messageID),
