...
var reVersion = regexp.MustCompile(`\{mobiusversion:\(\?:([^\}\)]+)\)\}`)

func makeStreamDistributedQueryCampaignResultsHandler(config config.ServerConfig, svc mobius.Service, logger kitlog.Logger) func(string) http.Handler {
	...
	return func(path string) http.Handler {
		...
		if len(matches) == 0 {
			panic("unexpected path, could not expand mobiusversion: " + path)
		}
