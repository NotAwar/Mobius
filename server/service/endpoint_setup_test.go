...
func TestDownloadAndUpdateScripts(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			tempDir, err := os.MkdirTemp("", "mobius-test-scripts-*")
			require.NoError(t, err)
...
func TestMobiusHTTPClientOverride(t *testing.T) {
	...
	// Create a mobiushttp client with our custom transport
	mobiusClient := mobiushttp.NewClient()
	// Replace the client's transport with our mock transport
	mobiusClient.Transport = client.Transport
...
func TestApplyStarterLibraryWithMockClient(t *testing.T) {
	...
	starterLibraryPath := "../../docs/01-Using-Mobius/starter-library/starter-library.yml"
	...
	mockRT := &testRoundTripper2{
		...
		RoundTripFunc: func(req *http.Request) (*http.Response, error) {
			...
			case strings.Contains(req.URL.String(), "uninstall-mobiusdaemon"):
	...
	httpClientFactory := func(opts ...mobiushttp.ClientOpt) *http.Client {
		client := mobiushttp.NewClient(opts...)
