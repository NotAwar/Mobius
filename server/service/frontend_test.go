...
func TestServeEndUserEnrollOTA(t *testing.T) {
	...
	for _, enabled := range []bool{true, false} {
		t.Run(fmt.Sprintf("MDM enabled: %t", enabled), func(t *testing.T) {
			...
			require.Contains(t, bodyString, "api/v1/mobius/enrollment_profiles/ota?enroll_secret=foo")
			require.Contains(t, bodyString, "/api/v1/mobius/android_enterprise/enrollment_token")
