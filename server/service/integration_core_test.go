...
import (
	...
	"github.com/notawar/mobius/v4/server/service/async"
...
`)
	...
	require.Equal(t, "SELECT * FROM foo;", dqResp.Queries["mobius_detail_query_software_macos"])
...
`)
	...
	require.Contains(t, dqResp.Queries, "mobius_detail_query_software_macos")
	require.Contains(t, dqResp.Queries["mobius_detail_query_software_macos"], "FROM apps")
	require.Contains(t, dqResp.Queries["mobius_detail_query_users"], "FROM users")
}

func (s *integrationTestSuite) TestDoubleUserCreationErrors() {
	t := s.T()

	params := mobius.UserPayload{
	...
	s.Do("POST", "/api/latest/mobius/users/admin", &params, http.StatusOK)
	respSecond := s.Do("POST", "/api/latest/mobius/users/admin", &params, http.StatusConflict)
...
func (s *integrationTestSuite) TestUserWithoutRoleErrors() {
	...
	resp := s.Do("POST", "/api/latest/mobius/users/admin", &params, http.StatusUnprocessableEntity)
	assertErrorCodeAndMessage(t, resp, mobius.ErrNoRoleNeeded, "either global role or team role needs to be defined")
}
...
func (s *integrationTestSuite) TestUserEmailValidation() {
	...
	s.Do("POST", "/api/latest/mobius/users/admin", &params, http.StatusUnprocessableEntity)
...
func (s *integrationTestSuite) TestUserPasswordLengthValidation() {
	...
	resp := s.Do("POST", "/api/latest/mobius/users/admin", &params, http.StatusUnprocessableEntity)
	assertBodyContains(s.T(), resp, "Could not create user. Password is over the 48 characters limit. If the password is under 48 characters, please check the auth_salt_key_size in your Mobius server config.")
}
...
func (s *integrationTestSuite) TestUserWithWrongRoleErrors() {
	...
	resp := s.Do("POST", "/api/latest/mobius/users/admin", &params, http.StatusUnprocessableEntity)
	assertErrorCodeAndMessage(t, resp, mobius.ErrNoRoleNeeded, "invalid global role: wrongrole")
}
...
func (s *integrationTestSuite) TestQueryCreationLogsActivity() {
	...
	s.DoJSON("GET", "/api/latest/mobius/activities", nil, http.StatusOK, &activities)
...
func (s *integrationTestSuite) TestCreatingAPIOnlyUserReturnsAPIToken() {
	...
	s.DoJSON("POST", "/api/latest/mobius/users/admin", params, http.StatusOK, &createResp)
	...
	params = mobius.UserPayload{
		...
		// AdminForcedPasswordReset is set to false when creating api-only users via `mobiuscli user create --api-only`.
	...
	s.DoJSON("GET", "/api/latest/mobius/hosts/count", countHostsRequest{}, http.StatusOK, &chr)
...
func (s *integrationTestSuite) TestActivityUserEmailPersistsAfterDeletion() {
	...
	s.DoJSON("POST", "/api/latest/mobius/login", params, http.StatusOK, &loginResp)
...
func (s *integrationTestSuite) TestPolicyDeletionLogsActivity() {
	...
	testPolicies := []mobius.PolicyPayload{{
		Name:  "policy1",
	...
	for _, policy := range testPolicies {
		...
		s.DoJSON("POST", "/api/latest/mobius/policies", policy, http.StatusOK, &resp)
		policyIDs = append(policyIDs, resp.Policy.PolicyData.ID)
	}

	// critical is premium only.
	s.DoJSON("POST", "/api/latest/mobius/policies", mobius.PolicyPayload{
		Name:     "policy3",
		Query:    "select * from time;",
		Critical: true,
...
`)
	...
	s.Do("PATCH", "/api/latest/mobius/config", appConfigSpec, http.StatusOK)
	config = s.getConfig()
	require.True(t, config.Features.EnableSoftwareInventory)
	...
	s.DoJSON("GET", "/api/latest/mobius/config", nil, http.StatusOK, &resp)
...
`,
	...
	s.Do("POST", "/api/latest/mobius/users/roles/spec", &userRoleSpec, http.StatusOK)

	user, err = s.ds.UserByEmail(context.Background(), email)
	require.NoError(t, err)
	require.Len(t, user.Teams, 1)
	assert.Equal(t, mobius.RoleMaintainer, user.Teams[0].Role)
...
`,
	...
	err = yaml.Unmarshal(spec, &userRoleSpec.Spec)
	require.NoError(t, err)
	s.Do("POST", "/api/latest/mobius/users/roles/spec", &userRoleSpec, http.StatusBadRequest)
}
...
func (s *integrationTestSuite) TestGlobalSchedule() {
	...
	gs := mobius.GlobalSchedulePayload{}
	s.DoJSON("GET", "/api/latest/mobius/schedule", nil, http.StatusOK, &gs)
	...
	// schedule that query
	gsParams := mobius.ScheduledQueryPayload{QueryID: ptr.Uint(qr.ID), Interval: ptr.Uint(42)}
	r := globalScheduleQueryResponse{}
	s.DoJSON("POST", "/api/latest/mobius/schedule", gsParams, http.StatusOK, &r)
	...
	s.DoJSON("GET", "/api/latest/mobius/schedule", nil, http.StatusOK, &gs, "page", "2", "per_page", "4")
	require.Len(t, gs.GlobalSchedule, 0)

	// update the scheduled query
	gs = mobius.GlobalSchedulePayload{}
	gsParams = mobius.ScheduledQueryPayload{Interval: ptr.Uint(55)}
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/schedule/%d", id), gsParams, http.StatusOK, &gs)

	// update a non-existing schedule
	gsParams = mobius.ScheduledQueryPayload{Interval: ptr.Uint(66)}
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/schedule/%d", id+1), gsParams, http.StatusNotFound, &gs)
	...
	r = globalScheduleQueryResponse{}
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobius/schedule/%d", id), nil, http.StatusOK, &r)

	// delete a non-existing schedule
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobius/schedule/%d", id+1), nil, http.StatusNotFound, &r)
	...
	params := translatorRequest{List: []mobius.TranslatePayload{
		{
			...
			Payload: mobius.StringIdentifierToIDPayload{Identifier: "admin1@example.com"},
		},
	}}
	s.DoJSON("POST", "/api/latest/mobius/translate", &params, http.StatusOK, &payload)
	require.Len(t, payload.List, 1)

	assert.Equal(t, s.users[payload.List[0].Payload.Identifier].ID, payload.List[0].Payload.ID)

	// empty body
	s.DoJSON("POST", "/api/latest/mobius/translate", &translatorRequest{}, http.StatusBadRequest, &payload)

	s.DoJSON("POST", "/api/latest/mobius/translate", &translatorRequest{List: []mobius.TranslatePayload{{Type: "notavalidtype", Payload: mobius.StringIdentifierToIDPayload{}}}}, http.StatusBadRequest, &payload)
}
...
func (s *integrationTestSuite) TestVulnerableSoftware() {
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d", host.ID), nil, http.StatusOK, &hostResponse)

	assertSoftware := func(t *testing.T, software []mobius.HostSoftwareEntry, contains *mobius.Software) {
		t.Helper()
		var found bool
	...
	expectedSoft2 := &mobius.Software{
		...
		Browser:     "chrome",
		GenerateCPE: "somecpe",
		Vulnerabilities: mobius.Vulnerabilities{
			{
				CVE:         "cve-123-123-132",
	...
	resp := s.Do("GET", "/api/latest/mobius/software", nil, http.StatusOK, "vulnerable", "true", "order_key", "generated_cpe", "order_direction", "desc")
	...
	resp = s.Do("GET", "/api/latest/mobius/software/versions", nil, http.StatusOK, "vulnerable", "true", "order_key", "generated_cpe", "order_direction", "desc")
	bodyBytes, err = io.ReadAll(resp.Body)
	require.NoError(t, err)
	assert.Contains(t, string(bodyBytes), `"counts_updated_at": null`)
	...
	s.DoJSON("GET", "/api/latest/mobius/software/count", countReq, http.StatusOK, &countResp)
	assert.Equal(t, 3, countResp.Count)

	// the software/count endpoint is different, it doesn't care about hosts counts
	countResp = countSoftwareResponse{}
	s.DoJSON("GET", "/api/latest/mobius/software/count", countReq, http.StatusOK, &countResp, "vulnerable", "true", "order_key", "generated_cpe", "order_direction", "desc")
	assert.Equal(t, 1, countResp.Count)

	// now the list software endpoint returns the software
	lsResp = listSoftwareResponse{}
	s.DoJSON("GET", "/api/latest/mobius/software", nil, http.StatusOK, &lsResp, "vulnerable", "true", "order_key", "generated_cpe", "order_direction", "desc")
	...
	s.DoJSON("GET", "/api/latest/mobius/software/versions", nil, http.StatusOK, &versionsResp, "vulnerable", "true", "order_key", "generated_cpe", "order_direction", "desc")
	...
	lsResp = listSoftwareResponse{}
	s.DoJSON("GET", "/api/latest/mobius/software", nil, http.StatusOK, &lsResp)
	require.True(t, len(lsResp.Software) >= len(software))
	require.NotNil(t, lsResp.CountsUpdatedAt)
	assert.WithinDuration(t, hostsCountTs, *lsResp.CountsUpdatedAt, time.Second)

	versionsResp = listSoftwareVersionsResponse{}
	s.DoJSON("GET", "/api/latest/mobius/software/versions", nil, http.StatusOK, &versionsResp)
	...
	lsResp = listSoftwareResponse{}
	s.DoJSON("GET", "/api/latest/mobius/software", nil, http.StatusOK, &lsResp, "page", "0", "per_page", "2", "order_key", "hosts_count", "order_direction", "desc")
	require.Len(t, lsResp.Software, 2)
	require.NotNil(t, lsResp.CountsUpdatedAt)
	assert.WithinDuration(t, hostsCountTs, *lsResp.CountsUpdatedAt, time.Second)

	versionsResp = listSoftwareVersionsResponse{}
	s.DoJSON("GET", "/api/latest/mobius/software/versions", nil, http.StatusOK, &versionsResp, "page", "0", "per_page", "2", "order_key", "hosts_count", "order_direction", "desc")
	...
	lsResp = listSoftwareResponse{}
	s.DoJSON("GET", "/api/latest/mobius/software", nil, http.StatusOK, &lsResp, "per_page", "2", "page", "1", "order_key", "hosts_count", "order_direction", "desc")
	require.Len(t, lsResp.Software, 1)
	require.NotNil(t, lsResp.CountsUpdatedAt)
	assert.WithinDuration(t, hostsCountTs, *lsResp.CountsUpdatedAt, time.Second)

	versionsResp = listSoftwareVersionsResponse{}
	s.DoJSON("GET", "/api/latest/mobius/software/versions", nil, http.StatusOK, &versionsResp, "per_page", "2", "page", "1", "order_key", "hosts_count", "order_direction", "desc")
	require.Len(t, versionsResp.Software, 1)
	require.True(t, versionsResp.Count >= 2)
	require.NotNil(t, versionsResp.CountsUpdatedAt)
	assert.WithinDuration(t, hostsCountTs, *versionsResp.CountsUpdatedAt, time.Second)
	...
	lsResp = listSoftwareResponse{}
	s.DoJSON("GET", "/api/latest/mobius/software", nil, http.StatusOK, &lsResp, "per_page", "2", "page", "2", "order_key", "hosts_count", "order_direction", "desc")
	require.Len(t, lsResp.Software, 0)
	require.Nil(t, lsResp.CountsUpdatedAt)

	versionsResp = listSoftwareVersionsResponse{}
	s.DoJSON("GET", "/api/latest/mobius/software/versions", nil, http.StatusOK, &versionsResp, "per_page", "2", "page", "2", "order_key", "hosts_count", "order_direction", "desc")
	require.Len(t, versionsResp.Software, 0)
	require.True(t, versionsResp.Count >= 2)
	...
	s.DoJSON("GET", "/api/latest/mobius/software", nil, http.StatusBadRequest, &lsResp, "per_page", "2", "page", "-10")
	s.DoJSON("GET", "/api/latest/mobius/software/versions", nil, http.StatusBadRequest, &lsResp, "per_page", "-2", "page", "2")
	s.DoJSON("GET", "/api/latest/mobius/software/count", nil, http.StatusBadRequest, &lsResp, "per_page", "-2", "page", "2")
}
...
func (s *integrationTestSuite) TestGlobalPolicies() {
	...
	// create 3 hosts
	for i := 0; i < 3; i++ {
		_, err := s.ds.NewHost(context.Background(), &mobius.Host{
			DetailUpdatedAt: time.Now(),
			LabelUpdatedAt:  time.Now(),
			PolicyUpdatedAt: time.Now(),
	...
	s.DoJSON("GET", "/api/latest/mobius/policies", nil, http.StatusOK, &policiesResponse)
	...
	s.Do("GET", fmt.Sprintf("/api/latest/mobius/policies/%d", 9999), nil, http.StatusNotFound)

	singlePolicyResponse := getPolicyByIDResponse{}
	singlePolicyURL := fmt.Sprintf("/api/latest/mobius/policies/%d", policiesResponse.Policies[0].ID)
	...
	listHostsURL := fmt.Sprintf("/api/latest/mobius/hosts?policy_id=%d", policiesResponse.Policies[0].ID)
	...
	listHostsURL = fmt.Sprintf("/api/latest/mobius/hosts?policy_id=%d&policy_response=passing", policiesResponse.Policies[0].ID)
	listHostsResp = listHostsResponse{}
	s.DoJSON("GET", listHostsURL, nil, http.StatusOK, &listHostsResp)
	...
	s.DoJSON("GET", "/api/latest/mobius/policies/count", nil, http.StatusOK, &cGPRes)
	assert.Equal(t, 1, cGPRes.Count)

	// count global policies with matching search query
	cGPRes = countGlobalPoliciesResponse{}
	s.DoJSON("GET", "/api/latest/mobius/policies/count", nil, http.StatusOK, &cGPRes, "query", "estQue")
	assert.Equal(t, 1, cGPRes.Count)
	...
	cGPRes = countGlobalPoliciesResponse{}
	s.DoJSON("GET", "/api/latest/mobius/policies/count", nil, http.StatusOK, &cGPRes, "query", "Query4")
	...
	s.DoJSON("POST", "/api/latest/mobius/policies/delete", deletePolicyParams, http.StatusOK, &deletePolicyResp)
...
func (s *integrationTestSuite) TestBulkDeleteHostsFromTeam() {
	...
	team1, err := s.ds.NewTeam(context.Background(), &mobius.Team{Name: t.Name() + "team1"})
	require.NoError(t, err)

	p, err := s.ds.NewPack(context.Background(), &mobius.Pack{
		Name: t.Name(),
		Hosts: []mobius.Target{
			{
	...
	s.DoJSON("POST", "/api/latest/mobius/hosts/delete", req, http.StatusOK, &resp)
...
func (s *integrationTestSuite) TestBulkDeleteHostByIDsWithTimeout() {
	...
	s.DoJSON("POST", "/api/latest/mobius/hosts/delete", req, http.StatusAccepted, &resp)
...
func (s *integrationTestSuite) createHosts(t *testing.T, platforms ...string) []*mobius.Host {
...
func (s *integrationTestSuite) TestBulkDeleteHostsErrors() {
	...
	resp := deleteHostsResponse{}
	s.DoJSON("POST", "/api/latest/mobius/hosts/delete", req, http.StatusBadRequest, &resp)
...
func (s *integrationTestSuite) TestHostsCount() {
	...
	s.DoJSON(
		"GET", "/api/latest/mobius/hosts/count", req, http.StatusOK, &resp,
	...
	s.DoJSON("GET", "/api/latest/mobius/hosts/count", nil, http.StatusOK, &resp, "low_disk_space", "32")
	...
	s.DoJSON("GET", "/api/latest/mobius/hosts/count", nil, http.StatusBadRequest, &resp, "low_disk_space", "123456")

	// filter by MDM criteria without any host having such information
	s.DoJSON("GET", "/api/latest/mobius/hosts/count", nil, http.StatusOK, &resp, "mdm_id", fmt.Sprint(999))
	require.Equal(t, 0, resp.Count)
	s.DoJSON("GET", "/api/latest/mobius/hosts/count", nil, http.StatusOK, &resp, "mdm_enrollment_status", "manual")
	require.Equal(t, 0, resp.Count)

	// set MDM information on a host
	require.NoError(t, s.ds.SetOrUpdateMDMData(context.Background(), hosts[1].ID, false, true, "https://simplemdm.com", false, mobius.WellKnownMDMSimpleMDM, ""))
	// also create server with MDM information, which is ignored.
	require.NoError(t, s.ds.SetOrUpdateMDMData(context.Background(), hosts[2].ID, true, true, "https://simplemdm.com", false, mobius.WellKnownMDMSimpleMDM, ""))
	var mdmID uint
	mysql.ExecAdhocSQL(t, s.ds, func(q sqlx.ExtContext) error {
		return sqlx.GetContext(context.Background(), q, &mdmID,
			`SELECT id FROM mobile_device_management_solutions WHERE name = ? AND server_url = ?`, mobius.WellKnownMDMSimpleMDM, "https://simplemdm.com")
	})

	// set MDM information for another host installed from DEP and pending enrollment to Mobius MDM
	pendingMDMHost, err := s.ds.NewHost(context.Background(), &mobius.Host{
		Platform:       "darwin",
		HardwareSerial: "532141num832",
		HardwareModel:  "MacBook Pro",
	})
	require.NoError(t, err)
	require.NoError(t, s.ds.SetOrUpdateMDMData(context.Background(), pendingMDMHost.ID, false, false, "https://mobiusmdm.com", true, mobius.WellKnownMDMMobius, ""))

	s.DoJSON("GET", "/api/latest/mobius/hosts/count", nil, http.StatusOK, &resp, "mdm_id", fmt.Sprint(mdmID))
	require.Equal(t, 1, resp.Count)
	s.DoJSON("GET", "/api/latest/mobius/hosts/count", nil, http.StatusOK, &resp, "mdm_enrollment_status", "manual")
	require.Equal(t, 1, resp.Count)
	s.DoJSON("GET", "/api/latest/mobius/hosts/count", nil, http.StatusOK, &resp, "mdm_enrollment_status", "automatic")
	require.Equal(t, 0, resp.Count)
	s.DoJSON("GET", "/api/latest/mobius/hosts/count", nil, http.StatusOK, &resp, "mdm_enrollment_status", "unenrolled")
	require.Equal(t, 0, resp.Count)
	s.DoJSON("GET", "/api/latest/mobius/hosts/count", nil, http.StatusOK, &resp, "mdm_enrollment_status", "manual", "mdm_id", fmt.Sprint(mdmID))
	require.Equal(t, 1, resp.Count)
	s.DoJSON("GET", "/api/latest/mobius/hosts/count", nil, http.StatusOK, &resp, "mdm_enrollment_status", "pending")
	require.Equal(t, 1, resp.Count)

	// get the host's MDM info
	var hostResp getHostResponse
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d", pendingMDMHost.ID), nil, http.StatusOK, &hostResp)
	require.Equal(t, pendingMDMHost.ID, hostResp.Host.ID)
	require.Equal(t, "Pending", *hostResp.Host.MDM.EnrollmentStatus)
	require.Equal(t, "https://mobiusmdm.com", *hostResp.Host.MDM.ServerURL)
...
func (s *integrationTestSuite) TestPacks() {
	...
	s.Do("GET", "/api/latest/mobius/packs/999", nil, http.StatusNotFound)

	// create some packs
	packs := make([]mobius.Pack, 3)
	for i := range packs {
		...
		s.DoJSON("POST", "/api/latest/mobius/packs", req, http.StatusOK, &createResp)
		packs[i] = createResp.Pack.Pack
	}

	// get existing pack
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/packs/%d", packs[0].ID), nil, http.StatusOK, &packResp)
	require.Equal(t, packs[0].ID, packResp.Pack.ID)

	// list packs
	var listResp listPacksResponse
	s.DoJSON("GET", "/api/latest/mobius/packs", nil, http.StatusOK, &listResp, "per_page", "2", "order_key", "name")
	require.Len(t, listResp.Packs, 2)
	assert.Equal(t, packs[0].ID, listResp.Packs[0].ID)
	assert.Equal(t, packs[1].ID, listResp.Packs[1].ID)

	// get page 1
	s.DoJSON("GET", "/api/latest/mobius/packs", nil, http.StatusOK, &listResp, "page", "1", "per_page", "2", "order_key", "name")
	require.Len(t, listResp.Packs, 1)
	assert.Equal(t, packs[2].ID, listResp.Packs[0].ID)

	// get page 2, empty
	s.DoJSON("GET", "/api/latest/mobius/packs", nil, http.StatusOK, &listResp, "page", "2", "per_page", "2", "order_key", "name")
	require.Len(t, listResp.Packs, 0)

	var delResp deletePackResponse
	// delete non-existing pack by name
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobius/packs/%s", "zzz"), nil, http.StatusNotFound, &delResp)

	// delete existing pack by name
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobius/packs/%s", url.PathEscape(packs[0].Name)), nil, http.StatusOK, &delResp)

	// delete non-existing pack by id
	var delIDResp deletePackByIDResponse
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobius/packs/id/%d", packs[2].ID+1), nil, http.StatusNotFound, &delIDResp)

	// delete existing pack by id
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobius/packs/id/%d", packs[1].ID), nil, http.StatusOK, &delIDResp)

	var modResp modifyPackResponse
	// modify non-existing pack
	req := &mobius.PackPayload{Name: ptr.String("updated_" + packs[2].Name)}
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/packs/%d", packs[2].ID+1), req, http.StatusNotFound, &modResp)

	// modify existing pack
	req = &mobius.PackPayload{Name: ptr.String("updated_" + packs[2].Name)}
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/packs/%d", packs[2].ID), req, http.StatusOK, &modResp)
...
func (s *integrationTestSuite) TestListHosts() {
	...
	resp = listHostsResponse{}
	s.DoJSON("GET", "/api/latest/mobius/hosts", nil, http.StatusOK, &resp, "per_page", "1")
	...
	resp = listHostsResponse{}
	s.DoJSON("GET", "/api/latest/mobius/hosts", nil, http.StatusOK, &resp, "order_key", "h.id", "after", fmt.Sprint(hosts[1].ID))
	...
	resp = listHostsResponse{}
	s.DoJSON("GET", "/api/latest/mobius/hosts", nil, http.StatusOK, &resp, "software_id", fmt.Sprint(fooV1ID))
	require.Len(t, resp.Hosts, 1)
	...
	s.DoJSON("GET", "/api/latest/mobius/hosts/count", nil, http.StatusOK, &countResp, "software_id", fmt.Sprint(fooV1ID))
	require.Equal(t, 1, countResp.Count)

	// foo v0.0.2 is installed on hosts 0 and 1
	resp = listHostsResponse{}
	s.DoJSON("GET", "/api/latest/mobius/hosts", nil, http.StatusOK, &resp, "software_version_id", fmt.Sprint(fooV2ID))
	require.Len(t, resp.Hosts, 2)
	require.ElementsMatch(t, []uint{host0.ID, host1.ID}, []uint{resp.Hosts[0].ID, resp.Hosts[1].ID})

	s.DoJSON("GET", "/api/latest/mobius/hosts/count", nil, http.StatusOK, &countResp, "software_version_id", fmt.Sprint(fooV2ID))
	require.Equal(t, 2, countResp.Count)

	// bar/application title is only on host1
	resp = listHostsResponse{}
	s.DoJSON("GET", "/api/latest/mobius/hosts", nil, http.StatusOK, &resp, "software_title_id", fmt.Sprint(barAppTitleID))
	require.Len(t, resp.Hosts, 1)
	...
	s.DoJSON("GET", "/api/latest/mobius/hosts/count", nil, http.StatusOK, &countResp, "software_title_id", fmt.Sprint(barAppTitleID))
	require.Equal(t, 1, countResp.Count)

	// foo title is on all 3 hosts
	resp = listHostsResponse{}
	s.DoJSON("GET", "/api/latest/mobius/hosts", nil, http.StatusOK, &resp, "software_title_id", fmt.Sprint(fooTitleID))
	...
	s.DoJSON("GET", "/api/latest/mobius/hosts/count", nil, http.StatusOK, &countResp, "software_title_id", fmt.Sprint(fooTitleID))
	require.Equal(t, 3, countResp.Count)

	// verify invalid combinations of software filters
	s.DoJSON("GET", "/api/latest/mobius/hosts", nil, http.StatusBadRequest, &resp, "software_title_id", fmt.Sprint(fooTitleID), "software_id", fmt.Sprint(fooV1ID))
	s.DoJSON("GET", "/api/latest/mobius/hosts", nil, http.StatusBadRequest, &resp, "software_title_id", fmt.Sprint(fooTitleID), "software_version_id", fmt.Sprint(fooV1ID))
	s.DoJSON("GET", "/api/latest/mobius/hosts", nil, http.StatusBadRequest, &resp, "software_id", fmt.Sprint(fooV1ID), "software_version_id", fmt.Sprint(fooV1ID))
	s.DoJSON("GET", "/api/latest/mobius/hosts", nil, http.StatusBadRequest, &resp, "software_id", fmt.Sprint(fooV1ID), "software_version_id", fmt.Sprint(fooV1ID), "software_title_id", fmt.Sprint(fooTitleID))
	s.DoJSON("GET", "/api/latest/mobius/hosts/count", nil, http.StatusBadRequest, &countResp, "software_title_id", fmt.Sprint(fooTitleID), "software_id", fmt.Sprint(fooV1ID))
	s.DoJSON("GET", "/api/latest/mobius/hosts/count", nil, http.StatusBadRequest, &countResp, "software_title_id", fmt.Sprint(fooTitleID), "software_version_id", fmt.Sprint(fooV1ID))
	s.DoJSON("GET", "/api/latest/mobius/hosts/count", nil, http.StatusBadRequest, &countResp, "software_id", fmt.Sprint(fooV1ID), "software_version_id", fmt.Sprint(fooV1ID))
	s.DoJSON("GET", "/api/latest/mobius/hosts/count", nil, http.StatusBadRequest, &countResp, "software_id", fmt.Sprint(fooV1ID), "software_version_id", fmt.Sprint(fooV1ID), "software_title_id", fmt.Sprint(fooTitleID))
	...
	globalPolicy0, err := s.ds.NewGlobalPolicy(
		context.Background(), &user1.ID, mobius.PolicyPayload{
			QueryID: &q.ID,
		})
	require.NoError(t, err)
	...
	s.DoJSON("GET", "/api/latest/mobius/hosts", nil, http.StatusOK, &resp, "software_version_id", fmt.Sprint(fooV1ID), "disable_failing_policies", "true")
	require.Len(t, resp.Hosts, 1)
	...
	resp = listHostsResponse{}
	s.DoJSON(
		"GET", "/api/latest/mobius/hosts", nil, http.StatusOK, &resp, "software_version_id", fmt.Sprint(fooV1ID), "disable_issues", "true",
	)
	require.Len(t, resp.Hosts, 1)
	assert.Zero(t, resp.Hosts[0].HostIssues.FailingPoliciesCount)
	assert.Zero(t, resp.Hosts[0].HostIssues.TotalIssuesCount)
	...
	// filter by MDM criteria without any host having such information
	resp = listHostsResponse{}
	s.DoJSON("GET", "/api/latest/mobius/hosts", nil, http.StatusOK, &resp, "mdm_id", fmt.Sprint(999))
	require.Len(t, resp.Hosts, 0)
	assert.Nil(t, resp.Software)
	assert.Nil(t, resp.MDMSolution)
	assert.Nil(t, resp.MunkiIssue)
	resp = listHostsResponse{}
	s.DoJSON("GET", "/api/latest/mobius/hosts", nil, http.StatusOK, &resp, "mdm_enrollment_status", "manual")
	require.Len(t, resp.Hosts, 0)
	assert.Nil(t, resp.Software)
	assert.Nil(t, resp.MDMSolution)
	assert.Nil(t, resp.MunkiIssue)
	...
	resp = listHostsResponse{}
	s.DoJSON("GET", "/api/latest/mobius/hosts", nil, http.StatusOK, &resp, "munki_issue_id", fmt.Sprint(999))
	require.Len(t, resp.Hosts, 0)
	assert.Nil(t, resp.Software)
	assert.Nil(t, resp.MDMSolution)
	assert.Nil(t, resp.MunkiIssue)
	...
	// set MDM information on a host
	require.NoError(t, s.ds.SetOrUpdateMDMData(context.Background(), host2.ID, false, true, "https://simplemdm.com", false, mobius.WellKnownMDMSimpleMDM, ""))
	var mdmID uint
	mysql.ExecAdhocSQL(t, s.ds, func(q sqlx.ExtContext) error {
		return sqlx.GetContext(context.Background(), q, &mdmID,
			`SELECT id FROM mobile_device_management_solutions WHERE name = ? AND server_url = ?`, mobius.WellKnownMDMSimpleMDM, "https://simplemdm.com")
	...
	require.NoError(t, err)
	mysql.ExecAdhocSQL(t, s.ds, func(q sqlx.ExtContext) error {
		_, err := q.ExecContext(context.Background(), "INSERT INTO mobile_device_management_solutions (name, server_url) VALUES ('https://mobiusmdm.com', 'Mobius')")
		require.NoError(t, err)
		return err
	})
	require.NoError(t, s.ds.SetOrUpdateMDMData(context.Background(), pendingMDMHost.ID, false, false, "https://mobiusmdm.com", true, mobius.WellKnownMDMMobius, ""))
	...
	s.DoJSON("GET", "/api/latest/mobius/hosts", nil, http.StatusOK, &resp, "mdm_enrollment_status", "pending")
	require.Len(t, resp.Hosts, 1)
	...
	resp = listHostsResponse{}
	s.DoJSON("GET", "/api/latest/mobius/hosts", nil, http.StatusOK, &resp, "mdm_enrollment_status", "automatic")
	require.Len(t, resp.Hosts, 0)
	assert.Nil(t, resp.Software)
	assert.Nil(t, resp.MDMSolution)
	assert.Nil(t, resp.MunkiIssue)
	...
	resp = listHostsResponse{}
	s.DoJSON("GET", "/api/latest/mobius/hosts", nil, http.StatusOK, &resp, "mdm_enrollment_status", "unenrolled")
	require.Len(t, resp.Hosts, 0)
	assert.Nil(t, resp.Software)
	assert.Nil(t, resp.MDMSolution)
	assert.Nil(t, resp.MunkiIssue)
	...
	resp = listHostsResponse{}
	s.DoJSON("GET", "/api/latest/mobius/hosts", nil, http.StatusOK, &resp, "mdm_id", fmt.Sprint(mdmID))
	require.Len(t, resp.Hosts, 1)
	assert.Nil(t, resp.Software)
	assert.Nil(t, resp.MunkiIssue)
	require.NotNil(t, resp.MDMSolution)
	assert.Equal(t, mdmID, resp.MDMSolution.ID)
	assert.Equal(t, mobius.WellKnownMDMSimpleMDM, resp.MDMSolution.Name)
	assert.Equal(t, "https://simplemdm.com", resp.MDMSolution.ServerURL)

	resp = listHostsResponse{}
	s.DoJSON("GET", "/api/latest/mobius/hosts", nil, http.StatusOK, &resp, "mdm_id", fmt.Sprint(mdmID), "mdm_enrollment_status", "manual")
	require.Len(t, resp.Hosts, 1)
	assert.Nil(t, resp.Software)
	assert.Nil(t, resp.MunkiIssue)
	...
	resp = listHostsResponse{}
	s.DoJSON("GET", "/api/latest/mobius/hosts", nil, http.StatusBadRequest, &resp, "mdm_enrollment_status", "invalid-status")

	// Filter by inexistent software.
	resp = listHostsResponse{}
	s.DoJSON("GET", "/api/latest/mobius/hosts", nil, http.StatusOK, &resp, "software_id", fmt.Sprint(9999))
	require.Len(t, resp.Hosts, 0)
	assert.Nil(t, resp.Software)

	resp = listHostsResponse{}
	s.DoJSON("GET", "/api/latest/mobius/hosts", nil, http.StatusOK, &resp, "software_version_id", fmt.Sprint(9999))
	require.Len(t, resp.Hosts, 0)
	assert.Nil(t, resp.Software)

	resp = listHostsResponse{}
	s.DoJSON("GET", "/api/latest/mobius/hosts", nil, http.StatusOK, &resp, "software_title_id", fmt.Sprint(9999))
	require.Len(t, resp.Hosts, 0)
	assert.Nil(t, resp.SoftwareTitle)

	// Filter by non-existent team.
	resp = listHostsResponse{}
	s.DoJSON("GET", "/api/latest/mobius/hosts", nil, http.StatusBadRequest, &resp, "team_id", fmt.Sprint(9999))
	...
	resp = listHostsResponse{}
	s.DoJSON("GET", "/api/latest/mobius/hosts", nil, http.StatusOK, &resp, "munki_issue_id", fmt.Sprint(errMunkiID))
	require.Len(t, resp.Hosts, 1)
	assert.Nil(t, resp.Software)
	assert.Nil(t, resp.MDMSolution)
	require.NotNil(t, resp.MunkiIssue)
	assert.Equal(t, mobius.MunkiIssue{
		ID:        errMunkiID,
		Name:      "err",
		IssueType: "error",
	}, *resp.MunkiIssue)
	...
	resp = listHostsResponse{}
	s.DoJSON("GET", "/api/latest/mobius/hosts", nil, http.StatusOK, &resp, "munki_issue_id", fmt.Sprint(errMunkiID), "mdm_id", fmt.Sprint(mdmID))
	require.Len(t, resp.Hosts, 1)
	assert.Nil(t, resp.Software)
	assert.NotNil(t, resp.MDMSolution)
	...
	testOS := mobius.OperatingSystem{Name: "fooOS", Version: "4.2", Arch: "64bit", KernelVersion: "13.37", Platform: "bar"}
	...
	resp = listHostsResponse{}
	s.DoJSON("GET", "/api/latest/mobius/hosts", nil, http.StatusOK, &resp, "os_id", fmt.Sprintf("%d", osID))
	require.Len(t, resp.Hosts, 1)
	require.Equal(t, expected, resp.Hosts[0])

	resp = listHostsResponse{}
	s.DoJSON("GET", "/api/latest/mobius/hosts", nil, http.StatusOK, &resp, "os_name", "unknownOS", "os_version", "4.2")
	require.Len(t, resp.Hosts, 0)

	resp = listHostsResponse{}
	s.DoJSON("GET", "/api/latest/mobius/hosts", nil, http.StatusOK, &resp, "os_id", fmt.Sprintf("%d", osID+1337))
	require.Len(t, resp.Hosts, 0)
	...
	require.NoError(t, s.ds.InsertCVEMeta(context.Background(), []mobius.CVEMeta{{
		CVE:              "cve-123-123-123",
	...
	resp = listHostsResponse{}
	s.DoJSON("GET", "/api/latest/mobius/hosts", nil, http.StatusOK, &resp, "populate_software", "false", "populate_policies", "false")
	require.Len(t, resp.Hosts, 4)
	for _, h := range resp.Hosts {
	...
	globalPolicy1, err := s.ds.NewGlobalPolicy(
		ctx, &test.UserAdmin.ID, mobius.PolicyPayload{
			Name:  "foobar0",
			Query: "SELECT 0;",
		},
	)
	...
	resp = listHostsResponse{}
	s.DoJSON("GET", "/api/latest/mobius/hosts", nil, http.StatusOK, &resp, "populate_policies", "true")
	...
	// there are 3 hosts, whos names end with ...local0, ...local1, ...local2
	resp = listHostsResponse{}
	s.DoJSON("GET", "/api/latest/mobius/hosts", nil, http.StatusOK, &resp, "query", "local0")
	require.Len(t, resp.Hosts, 1)
	...
	// Add labels to host
	label1, err := s.ds.NewLabel(ctx, &mobius.Label{Name: "First Label"})
	require.NoError(t, err)
	label2, err := s.ds.NewLabel(ctx, &mobius.Label{Name: "Second Label"})
	require.NoError(t, err)
	...
	resp = listHostsResponse{}
	s.DoJSON("GET", "/api/latest/mobius/hosts", nil, http.StatusOK, &resp, "query", "local0", "populate_users", "true")
	require.Len(t, resp.Hosts, 1)
	require.Contains(t, resp.Hosts[0].Hostname, "local0")
	...
	resp = listHostsResponse{}
	s.DoJSON("GET", "/api/latest/mobius/hosts", nil, http.StatusOK, &resp, "query", "local0", "populate_labels", "true")
	require.Len(t, resp.Hosts, 1)
	require.Contains(t, resp.Hosts[0].Hostname, "local0")
...
func (s *integrationTestSuite) TestInvites() {
	...
	s.DoJSON("GET", "/api/latest/mobius/invites", nil, http.StatusOK, &listResp)
	...
	s.DoJSON("GET", "/api/latest/mobius/invites/invalid", nil, http.StatusNotFound, &verifyInvResp)

	// create invite without an email
	createInviteReq = createInviteRequest{InvitePayload: mobius.InvitePayload{
		Email:      nil,
		Name:       ptr.String("some other name"),
		GlobalRole: null.StringFrom(mobius.RoleObserver),
	}}
	createInviteResp = createInviteResponse{}
	s.DoJSON("POST", "/api/latest/mobius/invites", createInviteReq, http.StatusUnprocessableEntity, &createInviteResp)
	...
	// matches name
	listResp = listInvitesResponse{}
	s.DoJSON("GET", "/api/latest/mobius/invites", nil, http.StatusOK, &listResp, "query", " some name                     ")
	require.Len(t, listResp.Invites, 1)
	require.Equal(t, validInvite.ID, listResp.Invites[0].ID)
	...
	// matches email
	listResp = listInvitesResponse{}
	s.DoJSON("GET", "/api/latest/mobius/invites", nil, http.StatusOK, &listResp, "query", " some email                     ")
	require.Len(t, listResp.Invites, 1)
	require.Equal(t, validInvite.ID, listResp.Invites[0].ID)
	...
	// matches nothing
	listResp = listInvitesResponse{}
	s.DoJSON("GET", "/api/latest/mobius/invites", nil, http.StatusOK, &listResp, "query", " no match                     ")
	require.Len(t, listResp.Invites, 0)

	// list invites, next page is empty
	listResp = listInvitesResponse{}
	s.DoJSON("GET", "/api/latest/mobius/invites", nil, http.StatusOK, &listResp, "page", "1", "per_page", "2")
	require.Len(t, listResp.Invites, 0)

	// update a non-existing invite
	updateInviteReq := updateInviteRequest{InvitePayload: mobius.InvitePayload{
		Teams: []mobius.UserTeam{
			{Team: mobius.Team{ID: team.ID}, Role: mobius.RoleObserver},
		},
		MFAEnabled: ptr.Bool(true),
	}}
	updateInviteResp := updateInviteResponse{}
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/invites/%d", validInvite.ID+1), updateInviteReq, http.StatusNotFound, &updateInviteResp)

	// update the valid invite created earlier, make it an observer of a team
	updateInviteResp = updateInviteResponse{}
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/invites/%d", validInvite.ID), updateInviteReq, http.StatusPaymentRequired, &updateInviteResp)
	updateInviteReq.MFAEnabled = nil
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/invites/%d", validInvite.ID), updateInviteReq, http.StatusOK, &updateInviteResp)

	// update the valid invite: set an email that already exists for a user
	updateInviteReq = updateInviteRequest{
		InvitePayload: mobius.InvitePayload{
	...
	updateInviteResp = updateInviteResponse{}
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/invites/%d", validInvite.ID), updateInviteReq, http.StatusConflict, &updateInviteResp)
	...
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobius/invites/%d", createInviteResp.Invite.ID), nil, http.StatusOK, &delResp)
	...
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobius/invites/%d", validInvite.ID), nil, http.StatusNotFound, &delResp)
...
func (s *integrationTestSuite) TestCrossOriginJSONSecurity() {
	...
	resp := s.DoRawWithHeaders("POST", "/api/latest/mobius/invites", req, http.StatusUnsupportedMediaType, map[string]string{
	...
	resp = s.DoRawWithHeaders("POST", "/api/latest/mobius/invites", req, http.StatusOK, map[string]string{
		"Authorization": fmt.Sprintf("Bearer %s", s.withServer.token),
		"Origin":        "example.com",
		"Referer":       "example.com",
...
func (s *integrationTestSuite) TestCreateUserFromInviteErrors() {
	...
	for _, c := range cases {
		t.Run(c.desc, func(t *testing.T) {
			...
			s.DoJSON("POST", "/api/latest/mobius/users", c.pld, c.want, &resp)
		})
	}
}
...
func (s *integrationTestSuite) TestGetHostSummary() {
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d", hosts[0].ID), nil, http.StatusOK, &getHostResp)
	...
	// no team filter
	s.DoJSON("GET", "/api/latest/mobius/host_summary", nil, http.StatusOK, &resp)
	...
	for _, lbl := range resp.BuiltinLabels {
		assert.Equal(t, mobius.LabelTypeBuiltIn, lbl.LabelType)
	}
	...
	s.DoJSON("GET", "/api/latest/mobius/labels", nil, http.StatusOK, &listResp)
	...
	s.DoJSON("GET", "/api/latest/mobius/labels", nil, http.StatusBadRequest, &listResp, "order_key", "id", "after", "1")

	// team filter, no host
	s.DoJSON("GET", "/api/latest/mobius/host_summary", nil, http.StatusOK, &resp, "team_id", fmt.Sprint(team2.ID))
	...
	s.DoJSON("GET", "/api/latest/mobius/host_summary", nil, http.StatusOK, &resp, "team_id", fmt.Sprint(team1.ID), "low_disk_space", "2")
	...
	s.DoJSON("GET", "/api/latest/mobius/host_summary", nil, http.StatusOK, &resp, "team_id", fmt.Sprint(team1.ID), "platform", "linux")
	require.Equal(t, resp.TotalsHostsCount, uint(1))
	require.Equal(t, "debian", resp.Platforms[0].Platform)
	require.Equal(t, uint(1), resp.AllLinuxCount)

	s.DoJSON("GET", "/api/latest/mobius/host_summary", nil, http.StatusOK, &resp, "platform", "rhel")
	require.Equal(t, resp.TotalsHostsCount, uint(1))
	require.Equal(t, "rhel", resp.Platforms[0].Platform)
	require.Equal(t, uint(1), resp.AllLinuxCount)

	s.DoJSON("GET", "/api/latest/mobius/host_summary", nil, http.StatusOK, &resp, "platform", "linux")
	...
	s.DoJSON("GET", "/api/latest/mobius/host_summary", nil, http.StatusOK, &resp, "platform", "darwin")
	require.Equal(t, resp.TotalsHostsCount, uint(0))
	require.Equal(t, resp.AllLinuxCount, uint(0))
	require.Len(t, resp.Platforms, 0)

	// invalid low_disk_space value is still validated and results in error
	s.DoJSON("GET", "/api/latest/mobius/host_summary", nil, http.StatusBadRequest, &resp, "low_disk_space", "1234")
}
...
func (s *integrationTestSuite) TestGlobalPoliciesProprietary() {
	...
	s.DoJSON("POST", "/api/latest/mobius/policies", gpParams0, http.StatusBadRequest, &gpResp0)
	...
	response := s.DoRaw("PATCH", fmt.Sprintf("/api/latest/mobius/policies/%d", gpResp.Policy.ID), []byte(`{
		"name": "TestQuery4",
		"query": "select * from osquery_info;",
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/policies/%d", gpResp.Policy.ID), getPolicyByIDRequest{}, http.StatusOK, &ggpResp)
	...
	listHostsURL = fmt.Sprintf("/api/latest/mobius/hosts?policy_id=%d&policy_response=failing", policiesResponse.Policies[0].ID)
	listHostsResp = listHostsResponse{}
	s.DoJSON("GET", listHostsURL, nil, http.StatusOK, &listHostsResp)
	require.Len(t, listHostsResp.Hosts, 0)
...
func (s *integrationTestSuite) TestTeamPoliciesProprietary() {
	...
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobius/teams/%d/policies", team1.ID), tpParams, http.StatusOK, &tpResp)
	...
	response := s.DoRaw("PATCH", fmt.Sprintf("/api/latest/mobius/teams/%d/policies/%d", team1.ID, tpResp.Policy.ID), []byte(fmt.Sprintf(`{
		"name": "%s",
		"query": "select * from osquery_info;",
		"description": "Some description updated",
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/teams/%d/policies/%d", team1.ID, tpResp.Policy.ID), getPolicyByIDRequest{}, http.StatusOK, &gtpResp)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/teams/%d/policies", team1.ID), nil, http.StatusOK, &policiesResponse)
	require.Len(t, policiesResponse.Policies, 1)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/teams/%d/policies/count", team1.ID), nil, http.StatusOK, &tpCountResp)
	assert.Equal(t, 1, tpCountResp.Count)

	tpCountResp = countTeamPoliciesResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/teams/%d/policies/count", team1.ID), nil, http.StatusOK, &tpCountResp, "query", tpNameNew)
	assert.Equal(t, 1, tpCountResp.Count)
	...
	tpCountResp = countTeamPoliciesResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/teams/%d/policies/count", team1.ID), nil, http.StatusOK, &tpCountResp, "query", " nomatch")
	...
	listHostsURL = fmt.Sprintf("/api/latest/mobius/hosts?team_id=%d&policy_id=%d&policy_response=passing", team1.ID, policiesResponse.Policies[0].ID)
	listHostsResp = listHostsResponse{}
	s.DoJSON("GET", listHostsURL, nil, http.StatusOK, &listHostsResp)
	require.Len(t, listHostsResp.Hosts, 0)
...
func (s *integrationTestSuite) TestTeamPoliciesProprietaryInvalid() {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.tname, func(t *testing.T) {
			...
			tpResp := teamPolicyResponse{}
			s.DoJSON("POST", fmt.Sprintf("/api/latest/mobius/teams/%d/policies", team1.ID), tpReq, http.StatusBadRequest, &tpResp)
			...
			if testUpdate {
				...
				s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/teams/%d/policies/%d", team1.ID, teamPolicyID), tpReq, http.StatusBadRequest, &tpResp)
				require.Nil(t, tpResp.Policy)
			}
			...
			gpResp := globalPolicyResponse{}
			s.DoJSON("POST", "/api/latest/mobius/policies", gpReq, http.StatusBadRequest, &gpResp)
			require.Nil(t, tpResp.Policy)
			...
			if testUpdate {
				...
				s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/policies/%d", globalPolicyID), gpReq, http.StatusBadRequest, &gpResp)
				require.Nil(t, tpResp.Policy)
			}
		})
	}
...
func (s *integrationTestSuite) TestHostDetailsUpdatesStaleHostIssues() {
	...
	s.DoJSON("GET", fmt.Sprintf("/api/v1/mobius/hosts/%d", host.ID), nil, http.StatusOK, &hostResp)
...
func (s *integrationTestSuite) TestHostDetailsPolicies() {
	...
	resp := s.Do("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d", host1.ID), nil, http.StatusOK)
	...
	s.DoJSON("POST", "/api/latest/mobius/policies", gpParams, http.StatusConflict, &gpResp)
	// Try to create a team policy with an existing name.
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobius/teams/%d/policies", team1.ID), tpParams, http.StatusConflict, &tpResp)
}
...
func (s *integrationTestSuite) TestListActivities() {
	...
	prevActivities, _, err := s.ds.ListActivities(ctx, mobius.ListActivitiesOptions{})
	require.NoError(t, err)

	timestamp := time.Now()
	ctx = context.WithValue(ctx, mobius.ActivityWebhookContextKey, true)
	err = s.ds.NewActivity(ctx, &u, mobius.ActivityTypeAppliedSpecPack{}, nil, timestamp)
	require.NoError(t, err)

	err = s.ds.NewActivity(ctx, &u, mobius.ActivityTypeDeletedPack{}, nil, timestamp)
	require.NoError(t, err)

	err = s.ds.NewActivity(ctx, &u, mobius.ActivityTypeEditedPack{}, nil, timestamp)
	require.NoError(t, err)

	lenPage := len(prevActivities) + 2

	var listResp listActivitiesResponse
	s.DoJSON("GET", "/api/latest/mobius/activities", nil, http.StatusOK, &listResp, "per_page", strconv.Itoa(lenPage), "order_key", "id")
	require.Len(t, listResp.Activities, lenPage)
	require.NotNil(t, listResp.Meta)
	assert.Equal(t, mobius.ActivityTypeAppliedSpecPack{}.ActivityName(), listResp.Activities[lenPage-2].Type)
	assert.Equal(t, mobius.ActivityTypeDeletedPack{}.ActivityName(), listResp.Activities[lenPage-1].Type)

	s.DoJSON("GET", "/api/latest/mobius/activities", nil, http.StatusOK, &listResp, "per_page", strconv.Itoa(lenPage), "order_key", "id", "page", "1")
	require.Len(t, listResp.Activities, 1)
	assert.Equal(t, mobius.ActivityTypeEditedPack{}.ActivityName(), listResp.Activities[0].Type)

	s.DoJSON("GET", "/api/latest/mobius/activities", nil, http.StatusOK, &listResp, "per_page", "1", "order_key", "id", "order_direction", "desc")
	require.Len(t, listResp.Activities, 1)
	assert.Equal(t, mobius.ActivityTypeEditedPack{}.ActivityName(), listResp.Activities[0].Type)

	listResp = listActivitiesResponse{}
	s.DoJSON("GET", "/api/latest/mobius/activities", nil, http.StatusOK, &listResp, "per_page", "1", "order_key", "a.id", "after", "0")
	require.Len(t, listResp.Activities, 1)
...
func (s *integrationTestSuite) TestListGetCarves() {
	...
	hosts := s.createHosts(t)
	c1, err := s.ds.NewCarve(ctx, &mobius.CarveMetadata{
		CreatedAt: time.Now(),
		HostId:    hosts[0].ID,
		Name:      t.Name() + "_1",
		SessionId: "ssn1",
	...
	require.NoError(t, err)
	c2, err := s.ds.NewCarve(ctx, &mobius.CarveMetadata{
		CreatedAt: time.Now(),
		HostId:    hosts[1].ID,
		Name:      t.Name() + "_2",
		SessionId: "ssn2",
	...
	require.NoError(t, err)
	c3, err := s.ds.NewCarve(ctx, &mobius.CarveMetadata{
		CreatedAt: time.Now(),
		HostId:    hosts[2].ID,
		Name:      t.Name() + "_3",
		SessionId: "ssn3",
	...
	s.DoJSON("GET", "/api/latest/mobius/carves", nil, http.StatusOK, &listResp, "per_page", "2", "order_key", "id")
	...
	// with 'after' param
	s.DoJSON(
		"GET", "/api/latest/mobius/carves", nil, http.StatusOK, &listResp, "per_page", "2", "order_key", "id", "after",
	...
	// include expired
	s.DoJSON("GET", "/api/latest/mobius/carves", nil, http.StatusOK, &listResp, "per_page", "2", "order_key", "id", "expired", "1")
	require.Len(t, listResp.Carves, 2)
	assert.Equal(t, c1.ID, listResp.Carves[0].ID)
	assert.Equal(t, c2.ID, listResp.Carves[1].ID)

	// empty page
	s.DoJSON("GET", "/api/latest/mobius/carves", nil, http.StatusOK, &listResp, "page", "3", "per_page", "2", "order_key", "id", "expired", "1")
	require.Len(t, listResp.Carves, 0)

	// get specific carve
	var getResp getCarveResponse
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/carves/%d", c2.ID), nil, http.StatusOK, &getResp)
	require.Equal(t, c2.ID, getResp.Carve.ID)
	require.True(t, getResp.Carve.Expired)

	// get non-existing carve
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/carves/%d", c3.ID+1), nil, http.StatusNotFound, &getResp)

	// get expired carve block
	var blkResp getCarveBlockResponse
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/carves/%d/block/%d", c2.ID, 1), nil, http.StatusInternalServerError, &blkResp)

	// get valid carve block, but block not inserted yet
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/carves/%d/block/%d", c1.ID, 1), nil, http.StatusNotFound, &blkResp)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/carves/%d/block/%d", c1.ID, 1), nil, http.StatusOK, &blkResp)
...
func (s *integrationTestSuite) TestHostsAddToTeam() {
	...
	tm1, err := s.ds.NewTeam(ctx, &mobius.Team{
		Name: uuid.New().String(),
	})
	require.NoError(t, err)
	tm2, err := s.ds.NewTeam(ctx, &mobius.Team{
		Name: uuid.New().String(),
	})
	require.NoError(t, err)
	...
	// refetch existing
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobius/hosts/%d/refetch", hosts[0].ID), nil, http.StatusOK, &refetchResp)
	require.NoError(t, refetchResp.Err)

	// refetch unknown
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobius/hosts/%d/refetch", hosts[2].ID+1), nil, http.StatusNotFound, &refetchResp)

	// get by identifier unknown
	var getResp getHostResponse
	s.DoJSON("GET", "/api/latest/mobius/hosts/identifier/no-such-host", nil, http.StatusNotFound, &getResp)

	// get by identifier valid
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/identifier/%s", hosts[0].UUID), nil, http.StatusOK, &getResp)
	...
	s.lastActivityOfTypeMatches(
		mobius.ActivityTypeTransferredHostsToTeam{}.ActivityName(),
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d", hosts[0].ID), nil, http.StatusOK, &getResp)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d", hosts[1].ID), nil, http.StatusOK, &getResp)
	require.NotNil(t, getResp.Host.TeamID)
	require.Equal(t, tm1.ID, *getResp.Host.TeamID)
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d", hosts[2].ID), nil, http.StatusOK, &getResp)
	require.Nil(t, getResp.Host.TeamID)
	...
	s.DoJSON("POST", "/api/latest/mobius/hosts/transfer/filter", req, http.StatusOK, &addfResp)
	s.lastActivityOfTypeMatches(
		mobius.ActivityTypeTransferredHostsToTeam{}.ActivityName(),
	...
	s.DoJSON("POST", "/api/latest/mobius/hosts/transfer/filter", req, http.StatusOK, &addfResp)
	s.DoJSON("GET", "/api/latest/mobius/hosts", nil, http.StatusOK, &hostsResp)
	...
	s.DoJSON("POST", "/api/latest/mobius/hosts/transfer/filter", req, http.StatusOK, &dreq)
	s.DoJSON("GET", "/api/latest/mobius/hosts", nil, http.StatusOK, &hostsResp)
	require.Len(t, hostsResp.Hosts, 3)
	...
	s.DoJSON("POST", "/api/latest/mobius/hosts/delete", dreq, http.StatusOK, &delHostsResp)
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d", hosts[0].ID), nil, http.StatusNotFound, &getResp)

	// delete non-existing host
	var delResp deleteHostResponse
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobius/hosts/%d", hosts[2].ID+1), nil, http.StatusNotFound, &delResp)
	...
	s.DoJSON("GET", "/api/latest/mobius/hosts", nil, http.StatusOK, &listResp, "per_page", "3")
...
func (s *integrationTestSuite) TestGetHostByIdentifier() {
	...
	s.DoJSON("GET", "/api/v1/mobius/hosts/identifier/osquery-1", nil, http.StatusOK, &resp)
	require.Equal(t, hosts[1].ID, resp.Host.ID)

	s.DoJSON("GET", "/api/v1/mobius/hosts/identifier/serial-2", nil, http.StatusOK, &resp)
	require.Equal(t, hosts[2].ID, resp.Host.ID)

	s.DoJSON("GET", "/api/v1/mobius/hosts/identifier/nodekey-3", nil, http.StatusOK, &resp)
	require.Equal(t, hosts[3].ID, resp.Host.ID)

	s.DoJSON("GET", "/api/v1/mobius/hosts/identifier/test-uuid-4", nil, http.StatusOK, &resp)
	require.Equal(t, hosts[4].ID, resp.Host.ID)

	s.DoJSON("GET", "/api/v1/mobius/hosts/identifier/test-host5-name", nil, http.StatusOK, &resp)
	require.Equal(t, hosts[5].ID, resp.Host.ID)

	s.DoJSON("GET", "/api/v1/mobius/hosts/identifier/no-such-host", nil, http.StatusNotFound, &resp)
}
...
func (s *integrationTestSuite) TestScheduledQueries() {
	...
	s.DoJSON("POST", "/api/latest/mobius/packs", reqPack, http.StatusOK, &createPackResp)
	pack := createPackResp.Pack.Pack

	// try a non existent query
	s.Do("GET", fmt.Sprintf("/api/latest/mobius/queries/%d", 9999), nil, http.StatusNotFound)

	// list queries
	var listQryResp listQueriesResponse
	s.DoJSON("GET", "/api/latest/mobius/queries", nil, http.StatusOK, &listQryResp)
	assert.Len(t, listQryResp.Queries, 0)
	assert.Equal(t, listQryResp.Count, 0)

	// create a query
	var createQueryResp createQueryResponse
	reqQuery := &mobius.QueryPayload{
		Name:  ptr.String(strings.ReplaceAll(t.Name(), "/", "_")),
		Query: ptr.String("select * from time;"),
	}
	s.DoJSON("POST", "/api/latest/mobius/queries", reqQuery, http.StatusOK, &createQueryResp)
	...
	s.DoJSON("GET", "/api/latest/mobius/queries", nil, http.StatusOK, &listQryResp, "query", query.Name)
	require.Len(t, listQryResp.Queries, 1)
	assert.Equal(t, query.Name, listQryResp.Queries[0].Name)
	...
	s.DoJSON("GET", "/api/latest/mobius/queries", nil, http.StatusOK, &listQryResp, "query", "  nomatch")
	require.Len(t, listQryResp.Queries, 0)

	// Return that query by name
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/queries?query=%s", query.Name), nil, http.StatusOK, &listQryResp)
	require.Len(t, listQryResp.Queries, 1)
	assert.Equal(t, query.Name, listQryResp.Queries[0].Name)

	// next page returns nothing, count and meta are correct
	s.DoJSON("GET", "/api/latest/mobius/queries", nil, http.StatusOK, &listQryResp, "per_page", "2", "page", "1")
	require.Len(t, listQryResp.Queries, 0)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/queries/%d", query.ID), nil, http.StatusOK, &getQryResp)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/packs/%d/scheduled", pack.ID), nil, http.StatusOK, &getInPackResp)
	assert.Len(t, getInPackResp.Scheduled, 0)

	// list scheduled queries in non-existing pack
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/packs/%d/scheduled", pack.ID+1), nil, http.StatusOK, &getInPackResp)
	assert.Len(t, getInPackResp.Scheduled, 0)
	...
	s.DoJSON("POST", "/api/latest/mobius/packs/schedule", reqSQ, http.StatusOK, &createResp)
	...
	s.DoJSON("POST", "/api/latest/mobius/packs/schedule", reqSQ, http.StatusUnprocessableEntity, &createResp)
	...
	s.DoJSON("POST", "/api/latest/mobius/packs/schedule", reqSQ, http.StatusNotFound, &createResp)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/packs/%d/scheduled", pack.ID), nil, http.StatusOK, &getInPackResp, "page", "1", "per_page", "2")
	require.Len(t, getInPackResp.Scheduled, 0)

	// get non-existing scheduled query
	var getResp getScheduledQueryResponse
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/schedule/%d", sq1.ID+1), nil, http.StatusNotFound, &getResp)

	// get existing scheduled query
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/schedule/%d", sq1.ID), nil, http.StatusOK, &getResp)
	...
	reqMod := mobius.ScheduledQueryPayload{
		Interval: ptr.Uint(4),
	}
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/packs/schedule/%d", sq1.ID), reqMod, http.StatusOK, &modResp)
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/packs/schedule/%d", sq1.ID+1), reqMod, http.StatusNotFound, &modResp)

	// delete non-existing scheduled query
	var delResp deleteScheduledQueryResponse
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobius/packs/schedule/%d", sq1.ID+1), nil, http.StatusNotFound, &delResp)

	// delete existing scheduled query
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobius/packs/schedule/%d", sq1.ID), nil, http.StatusOK, &delResp)

	// get the now-deleted scheduled query
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/schedule/%d", sq1.ID), nil, http.StatusNotFound, &getResp)

	// modify the query
	var modQryResp modifyQueryResponse
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/queries/%d", query.ID), mobius.QueryPayload{Description: ptr.String("updated")}, http.StatusOK, &modQryResp)
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/queries/%d", query.ID+1), mobius.QueryPayload{Description: ptr.String("updated")}, http.StatusNotFound, &modQryResp)

	// delete the query by name
	var delByNameResp deleteQueryResponse
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobius/queries/%s", query.Name), nil, http.StatusOK, &delByNameResp)

	// delete unknown query by name (i.e. the same, now deleted)
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobius/queries/%s", query.Name), nil, http.StatusNotFound, &delByNameResp)
	...
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobius/queries/id/%d", query2.ID), nil, http.StatusOK, &delByIDResp)

	// delete unknown query by id (same id just deleted)
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobius/queries/id/%d", query2.ID), nil, http.StatusNotFound, &delByIDResp)
	...
	s.DoJSON("POST", "/api/latest/mobius/queries/delete", map[string]interface{}{
...
func (s *integrationTestSuite) TestQueriesPaginationAndPlatformFilter() {
	...
	// middle of the pages
	s.DoJSON("GET", "/api/latest/mobius/queries", nil, http.StatusOK, &listQryResp, "query", queryNameToMatch, "per_page", "2", "page", "1")
	...
	// first and only page
	s.DoJSON("GET", "/api/latest/mobius/queries", nil, http.StatusOK, &listQryResp, "query", queryNameToMatch, "per_page", "10", "page", "0")
	...
	s.DoJSON("GET", "/api/latest/mobius/queries", nil, http.StatusOK, &listQryResp, "query", queryNameToMatch, "per_page", "2", "page", "0")
	require.Len(t, listQryResp.Queries, 2)
	require.Equal(t, listQryResp.Count, 10)
	require.False(t, listQryResp.Meta.HasPreviousResults)
	require.True(t, listQryResp.Meta.HasNextResults)
	...
	// last page
	s.DoJSON("GET", "/api/latest/mobius/queries", nil, http.StatusOK, &listQryResp, "query", queryNameToMatch, "per_page", "5", "page", "1")
	...
	// after last page
	s.DoJSON("GET", "/api/latest/mobius/queries", nil, http.StatusOK, &listQryResp, "query", queryNameToMatch, "per_page", "2", "page", "5")
	require.Len(t, listQryResp.Queries, 0)
	require.Equal(t, listQryResp.Count, 10)
	require.True(t, listQryResp.Meta.HasPreviousResults)
	require.False(t, listQryResp.Meta.HasNextResults)
	...
	s.DoJSON("GET", "/api/latest/mobius/queries", nil, http.StatusOK, &listQryResp, "platform", "macos")
	...
	s.DoJSON("GET", "/api/latest/mobius/queries", nil, http.StatusOK, &listQryResp, "platform", "windows")
	require.Len(t, listQryResp.Queries, 8)
	require.Equal(t, listQryResp.Count, 8)
	require.False(t, listQryResp.Meta.HasPreviousResults)
	require.False(t, listQryResp.Meta.HasNextResults)
	...
	s.DoJSON("GET", "/api/latest/mobius/queries", nil, http.StatusOK, &listQryResp, "platform", "linux")
	require.Len(t, listQryResp.Queries, 8)
	require.Equal(t, listQryResp.Count, 8)
	require.False(t, listQryResp.Meta.HasPreviousResults)
	require.False(t, listQryResp.Meta.HasNextResults)
	...
	s.DoJSON("GET", "/api/latest/mobius/queries", nil, http.StatusOK, &listQryResp, "platform", "linux", "per_page", "1", "page", "0")
	require.Len(t, listQryResp.Queries, 1)
	require.Equal(t, listQryResp.Count, 8)
	require.False(t, listQryResp.Meta.HasPreviousResults)
	require.True(t, listQryResp.Meta.HasNextResults)
	...
	s.DoJSON("GET", "/api/latest/mobius/queries", nil, http.StatusOK, &listQryResp, "platform", "linux", "per_page", "1", "page", "1")
	require.Len(t, listQryResp.Queries, 1)
	require.Equal(t, listQryResp.Count, 8)
	require.True(t, listQryResp.Meta.HasPreviousResults)
	require.True(t, listQryResp.Meta.HasNextResults)
	...
	s.DoJSON("GET", "/api/latest/mobius/queries", nil, http.StatusBadRequest, &listQryResp, "platform", "lucas", "per_page", "1", "page", "1")

	// delete them by name
	var delByNameResp deleteQueryResponse
	// for _, qId := range testQueryIds {
	for _, q := range testQueries {
		s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobius/queries/%s", q.Name), nil, http.StatusOK, &delByNameResp)
	}
}
...
func (s *integrationTestSuite) TestHostDeviceMapping() {
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/device_mapping", hosts[2].ID+1), nil, http.StatusNotFound, &listResp)

	// existing host but none yet
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/device_mapping", hosts[0].ID), nil, http.StatusOK, &listResp)
	require.Len(t, listResp.DeviceMapping, 0)
	var hostResp getHostResponse
	s.DoJSON("GET", "/api/v1/mobius/hosts/identifier/"+hosts[0].UUID, nil, http.StatusOK, &hostResp)
	assert.Len(t, hostResp.Host.EndUsers, 0)
	hostResp = getHostResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/v1/mobius/hosts/%d", hosts[0].ID), nil, http.StatusOK, &hostResp)
	assert.Len(t, hostResp.Host.EndUsers, 0)

	// create a custom mapping of a non-existing host
	var putResp putHostDeviceMappingResponse
	s.DoJSON("PUT", fmt.Sprintf("/api/latest/mobius/hosts/%d/device_mapping", hosts[2].ID+1), nil, http.StatusNotFound, &putResp)

	// create some google mappings
	require.NoError(t, s.ds.ReplaceHostDeviceMapping(ctx, hosts[0].ID, []*mobius.HostDeviceMapping{
		{HostID: hosts[0].ID, Email: "a@b.c", Source: mobius.DeviceMappingGoogleChromeProfiles},
		{HostID: hosts[0].ID, Email: "b@b.c", Source: mobius.DeviceMappingGoogleChromeProfiles},
	}, mobius.DeviceMappingGoogleChromeProfiles))

	// create a custom mapping
	s.DoJSON("PUT", fmt.Sprintf("/api/latest/mobius/hosts/%d/device_mapping", hosts[0].ID), putHostDeviceMappingRequest{Email: "c@b.c"}, http.StatusOK, &putResp)
	require.Equal(t, hosts[0].ID, putResp.HostID)
	require.ElementsMatch(t, putResp.DeviceMapping, []*mobius.HostDeviceMapping{
		{Email: "a@b.c", Source: mobius.DeviceMappingGoogleChromeProfiles},
		{Email: "b@b.c", Source: mobius.DeviceMappingGoogleChromeProfiles},
		{Email: "c@b.c", Source: mobius.DeviceMappingCustomReplacement},
	})

	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/device_mapping", hosts[0].ID), nil, http.StatusOK, &listResp)
	require.Equal(t, hosts[0].ID, listResp.HostID)
	require.ElementsMatch(t, listResp.DeviceMapping, []*mobius.HostDeviceMapping{
		{Email: "a@b.c", Source: mobius.DeviceMappingGoogleChromeProfiles},
		{Email: "b@b.c", Source: mobius.DeviceMappingGoogleChromeProfiles},
		{Email: "c@b.c", Source: mobius.DeviceMappingCustomReplacement},
	})
	...
	checkEndUser := func() {
		...
		require.True(t, ok)
		assert.Equal(t, mobius.DeviceMappingGoogleChromeProfiles, source)
		source, ok = othersByEmail["b@b.c"]
		require.True(t, ok)
		assert.Equal(t, mobius.DeviceMappingGoogleChromeProfiles, source)
		source, ok = othersByEmail["c@b.c"]
		require.True(t, ok)
		assert.Equal(t, mobius.DeviceMappingCustomReplacement, source)
	}
	checkEndUser()
	hostResp = getHostResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/v1/mobius/hosts/%d", hosts[0].ID), nil, http.StatusOK, &hostResp)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/device_mapping", hosts[1].ID), nil, http.StatusOK, &listResp)
	require.Len(t, listResp.DeviceMapping, 0)

	var listHosts listHostsResponse
	// list hosts response includes device mappings
	s.DoJSON("GET", "/api/latest/mobius/hosts?device_mapping=true", nil, http.StatusOK, &listHosts)
	require.Len(t, listHosts.Hosts, len(hosts)+1)
	hostsByID := make(map[uint]mobius.HostResponse)
	for _, h := range listHosts.Hosts {
		hostsByID[h.ID] = h
	}
	var dm []*mobius.HostDeviceMapping
	...
	require.NoError(t, err)
	require.ElementsMatch(t, dm, []*mobius.HostDeviceMapping{
		{Email: "a@b.c", Source: mobius.DeviceMappingGoogleChromeProfiles},
		{Email: "b@b.c", Source: mobius.DeviceMappingGoogleChromeProfiles},
		{Email: "c@b.c", Source: mobius.DeviceMappingCustomReplacement},
	})
	...
	s.DoJSON("PUT", fmt.Sprintf("/api/latest/mobius/hosts/%d/device_mapping", hosts[0].ID), putHostDeviceMappingRequest{Email: "d@b.c"}, http.StatusOK, &putResp)
	require.Equal(t, hosts[0].ID, putResp.HostID)
	require.ElementsMatch(t, putResp.DeviceMapping, []*mobius.HostDeviceMapping{
		{Email: "a@b.c", Source: mobius.DeviceMappingGoogleChromeProfiles},
		{Email: "b@b.c", Source: mobius.DeviceMappingGoogleChromeProfiles},
		{Email: "d@b.c", Source: mobius.DeviceMappingCustomReplacement},
	})

	// create a custom_installer email for orbit host
	s.Do("PUT", "/api/mobius/orbit/device_mapping", orbitPutDeviceMappingRequest{
		OrbitNodeKey: *orbitHost.OrbitNodeKey,
		Email:        "e@b.c",
	}, http.StatusOK)

	// search host by email address finds the corresponding host
	s.DoJSON("GET", "/api/latest/mobius/hosts?device_mapping=true", nil, http.StatusOK, &listHosts, "query", "a@b.c")
	...
	s.DoJSON("GET", "/api/latest/mobius/hosts?device_mapping=true", nil, http.StatusOK, &listHosts, "query", "d@b.c")
	require.Len(t, listHosts.Hosts, 1)
	require.Equal(t, hosts[0].ID, listHosts.Hosts[0].ID)

	s.DoJSON("GET", "/api/latest/mobius/hosts?device_mapping=true", nil, http.StatusOK, &listHosts, "query", "e@b.c")
	require.Len(t, listHosts.Hosts, 1)
	require.Equal(t, orbitHost.ID, listHosts.Hosts[0].ID)

	// override the custom email for the orbit host
	s.DoJSON("PUT", fmt.Sprintf("/api/latest/mobius/hosts/%d/device_mapping", orbitHost.ID), putHostDeviceMappingRequest{Email: "f@b.c"}, http.StatusOK, &putResp)
	...
	s.DoJSON("GET", "/api/latest/mobius/hosts?device_mapping=true", nil, http.StatusOK, &listHosts, "query", "f@b.c")
	require.Len(t, listHosts.Hosts, 1)
	require.Equal(t, orbitHost.ID, listHosts.Hosts[0].ID)

	// searching by a never-used email returns nothing
	s.DoJSON("GET", "/api/latest/mobius/hosts?device_mapping=true", nil, http.StatusOK, &listHosts, "query", "Z@b.c")
	require.Len(t, listHosts.Hosts, 0)
}
...
func (s *integrationTestSuite) TestListHostsDeviceMappingSize() {
	...
	testSize := 50
	var mappings []*mobius.HostDeviceMapping
	for i := 0; i < testSize; i++ {
		testEmail, _ := server.GenerateRandomText(14)
		mappings = append(mappings, &mobius.HostDeviceMapping{HostID: hosts[0].ID, Email: testEmail, Source: "google_chrome_profiles"})
	}
...
type macadminsDataResponse struct {
	Macadmins *struct {
		Munki       *mobius.HostMunkiInfo    `json:"munki"`
		MunkiIssues []*mobius.HostMunkiIssue `json:"munki_issues"`
		MDM         *struct {
...
func (s *integrationTestSuite) TestGetMacadminsData() {
	...
	hostAll, err := s.ds.NewHost(ctx, &mobius.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
		SeenTime:        time.Now(),
	...
	hostNothing, err := s.ds.NewHost(ctx, &mobius.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
		SeenTime:        time.Now(),
	...
	hostOnlyMunki, err := s.ds.NewHost(ctx, &mobius.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
		SeenTime:        time.Now(),
	...
	hostOnlyMDM, err := s.ds.NewHost(ctx, &mobius.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
		SeenTime:        time.Now(),
	...
	hostMDMNoID, err := s.ds.NewHost(ctx, &mobius.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
		SeenTime:        time.Now(),
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/macadmins", hostAll.ID), nil, http.StatusOK, &macadminsData)
	...
	require.NoError(t, s.ds.SetOrUpdateMDMData(ctx, hostAll.ID, false, true, "https://simplemdm.com", true, mobius.WellKnownMDMSimpleMDM, ""))
	...
	assert.Equal(t, mobius.WellKnownMDMSimpleMDM, *macadminsData.Macadmins.MDM.Name)
	require.NotNil(t, macadminsData.Macadmins.MDM.ID)
	assert.NotZero(t, *macadminsData.Macadmins.MDM.ID)
	...
	macadminsData = macadminsDataResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/macadmins", hostNothing.ID), nil, http.StatusOK, &macadminsData)
	...
	macadminsData = macadminsDataResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/macadmins", hostOnlyMunki.ID), nil, http.StatusOK, &macadminsData)
	require.NotNil(t, macadminsData.Macadmins)
	...
	require.NoError(t, s.ds.SetOrUpdateMDMData(ctx, hostOnlyMDM.ID, false, true, "https://kandji.io", true, mobius.WellKnownMDMKandji, ""))
	macadminsData = macadminsDataResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/macadmins", hostOnlyMDM.ID), nil, http.StatusOK, &macadminsData)
	require.NotNil(t, macadminsData.Macadmins)
	require.NotNil(t, macadminsData.Macadmins.MDM)
	require.NotNil(t, macadminsData.Macadmins.MDM.Name)
	assert.Equal(t, mobius.WellKnownMDMKandji, *macadminsData.Macadmins.MDM.Name)
	require.NotNil(t, macadminsData.Macadmins.MDM.ID)
	assert.NotZero(t, *macadminsData.Macadmins.MDM.ID)
	...
	macadminsData = macadminsDataResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/macadmins", hostMDMNoID.ID), nil, http.StatusOK, &macadminsData)
	require.NotNil(t, macadminsData.Macadmins)
	require.NotNil(t, macadminsData.Macadmins.MDM)
	assert.Nil(t, macadminsData.Macadmins.MDM.Name)
	...
	s.DoJSON("GET", "/api/latest/mobius/macadmins", nil, http.StatusOK, &agg)
	require.NotNil(t, agg.Macadmins)
	assert.NotZero(t, agg.Macadmins.CountsUpdatedAt)
	assert.Len(t, agg.Macadmins.MunkiVersions, 2)
	assert.ElementsMatch(t, agg.Macadmins.MunkiVersions, []mobius.AggregatedMunkiVersion{
		{
			HostMunkiInfo: mobius.HostMunkiInfo{Version: "1.5.0"},
			HostsCount:    1,
		},
		{
			HostMunkiInfo: mobius.HostMunkiInfo{Version: "3.2.0"},
			HostsCount:    1,
		},
	})
	...
	assert.ElementsMatch(t, agg.Macadmins.MunkiIssues, []mobius.AggregatedMunkiIssue{
		{
			MunkiIssue: mobius.MunkiIssue{
				Name:      "error1",
				IssueType: "error",
			},
			HostsCount: 1,
	...
	for _, sol := range agg.Macadmins.MDMSolutions {
		...
		case "url2":
			assert.Equal(t, mobius.UnknownMDMName, sol.Name)
			assert.Equal(t, 1, sol.HostsCount)
		case "https://kandji.io":
			assert.Equal(t, mobius.WellKnownMDMKandji, sol.Name)
			assert.Equal(t, 1, sol.HostsCount)
		default:
	...
	s.DoJSON("GET", "/api/latest/mobius/hosts/summary/mdm", nil, http.StatusOK, &mdmAgg)
	...
	agg = getAggregatedMacadminsDataResponse{}
	s.DoJSON("GET", "/api/latest/mobius/macadmins", nil, http.StatusOK, &agg, "team_id", fmt.Sprint(team.ID))
	require.NotNil(t, agg.Macadmins)
	...
	agg = getAggregatedMacadminsDataResponse{}
	s.DoJSON("GET", "/api/latest/mobius/macadmins", nil, http.StatusNotFound, &agg, "team_id", "9999999")
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/mdm", hostNothing.ID), nil, http.StatusOK, &ghr)
	require.Nil(t, ghr.HostMDM)

	ghr = getHostMDMResponseTest{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/mdm", 999999), nil, http.StatusNotFound, &ghr)
	require.Nil(t, ghr.HostMDM)
}
...
func (s *integrationTestSuite) TestLabels() {
	...
	builtinsMap := mobius.ReservedLabelNames()
	var listResp listLabelsResponse
	s.DoJSON("GET", "/api/latest/mobius/labels", nil, http.StatusOK, &listResp)
	assert.True(t, len(listResp.Labels) > 0)
	var builtinLbl mobius.Label
	for _, lbl := range listResp.Labels {
	...
	s.DoJSON("GET", "/api/latest/mobius/labels/summary", nil, http.StatusOK, &summaryResp)
	...
	s.DoJSON("POST", "/api/latest/mobius/labels", &mobius.LabelPayload{Query: "select 1"}, http.StatusUnprocessableEntity, &createResp)

	// create a label with both a query and hosts, error
	res := s.Do("POST", "/api/latest/mobius/labels", &mobius.LabelPayload{Name: t.Name(), Query: "select 1", Hosts: []string{manualHosts[0].UUID}}, http.StatusUnprocessableEntity)
	...
	for n := range builtinsMap {
		s.DoJSON("POST", "/api/latest/mobius/labels", &mobius.LabelPayload{Name: n, Query: "select 1"}, http.StatusUnprocessableEntity, &createResp)
	}

	// create a valid dynamic label
	s.DoJSON("POST", "/api/latest/mobius/labels", &mobius.LabelPayload{Name: t.Name(), Query: "select 1"}, http.StatusOK, &createResp)
	...
	s.DoJSON("POST", "/api/latest/mobius/labels", &mobius.LabelPayload{Name: lbl1.Name, Hosts: []string{manualHosts[0].UUID}}, http.StatusConflict, &createResp)
	// try to create a dynamic label with the same name
	s.DoJSON("POST", "/api/latest/mobius/labels", &mobius.LabelPayload{Name: lbl1.Name, Query: "select 2"}, http.StatusConflict, &createResp)

	// get the label
	var getResp getLabelResponse
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/labels/%d", lbl1.ID), nil, http.StatusOK, &getResp)
	assert.Equal(t, lbl1.ID, getResp.Label.ID)
	assert.Empty(t, getResp.Label.HostIDs)

	// get a non-existing label
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/labels/%d", lbl1.ID+1), nil, http.StatusNotFound, &getResp)

	// create a valid manual label
	createResp = createLabelResponse{}
	s.DoJSON("POST", "/api/latest/mobius/labels", &mobius.LabelPayload{Name: t.Name() + "manual", Hosts: []string{manualHosts[0].UUID, manualHosts[1].Hostname, *manualHosts[2].NodeKey}}, http.StatusOK, &createResp)
	assert.NotZero(t, createResp.Label.ID)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/labels/%d", manualLbl1.ID), nil, http.StatusOK, &getResp)
	assert.Equal(t, manualLbl1.ID, getResp.Label.ID)
	assert.Equal(t, mobius.LabelTypeRegular, getResp.Label.LabelType)
	assert.Equal(t, mobius.LabelMembershipTypeManual, getResp.Label.LabelMembershipType)
	...
	createResp = createLabelResponse{}
	s.DoJSON("POST", "/api/latest/mobius/labels", &mobius.LabelPayload{Name: strings.ReplaceAll(t.Name(), "/", "_") + "manual2"}, http.StatusOK, &createResp)
	assert.NotZero(t, createResp.Label.ID)
	...
	// try to create a manual label with the same name
	s.DoJSON("POST", "/api/latest/mobius/labels", &mobius.LabelPayload{Name: manualLbl2.Name, Hosts: []string{manualHosts[0].UUID}}, http.StatusConflict, &createResp)
	// try to create a dynamic label with the same name
	s.DoJSON("POST", "/api/latest/mobius/labels", &mobius.LabelPayload{Name: manualLbl2.Name, Query: "select 2"}, http.StatusConflict, &createResp)

	// get the label
	getResp = getLabelResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/labels/%d", manualLbl2.ID), nil, http.StatusOK, &getResp)
	...
	// get a non-existing label
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/labels/%d", 9999), nil, http.StatusNotFound, &getResp)

	// modify dynamic label lbl1
	var modResp modifyLabelResponse
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/labels/%d", lbl1.ID), &mobius.ModifyLabelPayload{Name: ptr.String(t.Name() + "zzz")}, http.StatusOK, &modResp)
	...
	for n := range builtinsMap {
		s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/labels/%d", lbl1.ID), &mobius.ModifyLabelPayload{Name: ptr.String(n)}, http.StatusUnprocessableEntity, &modResp)
	}

	// modify a non-existing label
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/labels/%d", 9999), &mobius.ModifyLabelPayload{Name: ptr.String("zzz")}, http.StatusNotFound, &modResp)
	// modify a built-in label
	res = s.Do("PATCH", fmt.Sprintf("/api/latest/mobius/labels/%d", builtinLbl.ID), &mobius.ModifyLabelPayload{Name: ptr.String("zzz")}, http.StatusUnprocessableEntity)
	errMsg = extractServerErrorText(res.Body)
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/labels/%d", manualLbl1.ID), &mobius.ModifyLabelPayload{Name: &newName}, http.StatusOK,
		&modResp)
	assert.Equal(t, manualLbl1.ID, modResp.Label.ID)
	assert.Equal(t, mobius.LabelTypeRegular, modResp.Label.LabelType)
	assert.Equal(t, mobius.LabelMembershipTypeManual, modResp.Label.LabelMembershipType)
	...
	sameName, err := s.ds.NewHost(context.Background(), &mobius.Host{
	...
	modResp = modifyLabelResponse{}
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/labels/%d", manualLbl2.ID),
		&mobius.ModifyLabelPayload{Hosts: []string{sameName.HardwareSerial}}, http.StatusOK, &modResp)
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/labels/%d", manualLbl2.ID),
		&mobius.ModifyLabelPayload{Name: &newName, Hosts: []string{manualHosts[0].UUID}}, http.StatusOK, &modResp)
	assert.Equal(t, manualLbl2.ID, modResp.Label.ID)
	assert.Equal(t, mobius.LabelTypeRegular, modResp.Label.LabelType)
	assert.Equal(t, mobius.LabelMembershipTypeManual, modResp.Label.LabelMembershipType)
	...
	newName = "modified_manual_label2"
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/labels/%d", manualLbl2.ID),
		&mobius.ModifyLabelPayload{Name: &newName, HostIDs: []uint{manualHosts[1].ID, manualHosts[2].ID}}, http.StatusOK, &modResp)
	assert.Equal(t, manualLbl2.ID, modResp.Label.ID)
	assert.Equal(t, mobius.LabelTypeRegular, modResp.Label.LabelType)
	assert.Equal(t, mobius.LabelMembershipTypeManual, modResp.Label.LabelMembershipType)
	...
	modResp = modifyLabelResponse{}
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/labels/%d", manualLbl2.ID), &mobius.ModifyLabelPayload{Hosts: []string{}, Description: ptr.String("desc")}, http.StatusOK, &modResp)
	assert.Equal(t, manualLbl2.ID, modResp.Label.ID)
	...
	// list labels
	dynamicLabels := []mobius.Label{lbl1}
	manualLabels := []mobius.Label{manualLbl1, manualLbl2}
	s.DoJSON("GET", "/api/latest/mobius/labels", nil, http.StatusOK, &listResp, "per_page", strconv.Itoa(100))
	...
	// next page is empty
	s.DoJSON("GET", "/api/latest/mobius/labels", nil, http.StatusOK, &listResp, "per_page", "100", "page", "1")
	assert.Len(t, listResp.Labels, 0)

	// list labels with invalid query params
	s.DoJSON("GET", "/api/latest/mobius/labels", nil, http.StatusBadRequest, &listResp, "per_page", strconv.Itoa(builtInsCount+1), "order_key", "id", "after", "1")
	s.DoJSON("GET", "/api/latest/mobius/labels", nil, http.StatusBadRequest, &listResp, "per_page", strconv.Itoa(builtInsCount+1), "query", "no match query for this endpoint")

	// create another dynamic label
	s.DoJSON("POST", "/api/latest/mobius/labels", &mobius.LabelPayload{Name: strings.ReplaceAll(t.Name(), "/", "_"), Query: "select 1"}, http.StatusOK, &createResp)
	assert.NotZero(t, createResp.Label.ID)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/labels/%d/hosts", lbl2.ID), nil, http.StatusOK, &listHostsResp)
	assert.Len(t, listHostsResp.Hosts, len(lbl2Hosts))

	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/labels/%d/hosts", lbl2.ID), nil, http.StatusOK, &listHostsResp, "order_key", "id", "after", fmt.Sprintf("%d", lbl2Hosts[0].ID))
	...
	listHostsResp = listHostsResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/labels/%d/hosts", manualLbl1.ID), nil, http.StatusOK, &listHostsResp, "order_key", "id")
	...
	listHostsResp = listHostsResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/labels/%d/hosts", manualLbl2.ID), nil, http.StatusOK, &listHostsResp, "order_key", "id")
	assert.Len(t, listHostsResp.Hosts, 0)

	// list hosts in dynamic label 2 searching by display_name
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/labels/%d/hosts", lbl2.ID), nil, http.StatusOK, &listHostsResp, "order_key", "display_name", "order_direction", "desc")
	assert.Len(t, listHostsResp.Hosts, len(lbl2Hosts))
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/labels/%d/hosts", lbl2.ID), nil, http.StatusOK, &listHostsResp, "query", "a@b.c")
	...
	var countResp countHostsResponse
	s.DoJSON("GET", "/api/latest/mobius/hosts/count", nil, http.StatusOK, &countResp, "label_id", fmt.Sprint(lbl2.ID), "order_key", "display_name", "order_direction", "desc")
	assert.Equal(t, len(lbl2Hosts), countResp.Count)

	// lists hosts in label without hosts
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/labels/%d/hosts", lbl1.ID), nil, http.StatusOK, &listHostsResp)
	assert.Len(t, listHostsResp.Hosts, 0)

	// count hosts in label
	s.DoJSON("GET", "/api/latest/mobius/hosts/count", nil, http.StatusOK, &countResp, "label_id", fmt.Sprint(lbl1.ID))
	assert.Equal(t, 0, countResp.Count)

	// lists hosts in invalid label
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/labels/%d/hosts", lbl2.ID+1), nil, http.StatusOK, &listHostsResp)
	assert.Len(t, listHostsResp.Hosts, 0)

	// set MDM information on a host
	require.NoError(t, s.ds.SetOrUpdateMDMData(context.Background(), lbl2Hosts[0].ID, false, true, "https://simplemdm.com", false, mobius.WellKnownMDMSimpleMDM, ""))
	var mdmID uint
	mysql.ExecAdhocSQL(t, s.ds, func(q sqlx.ExtContext) error {
		return sqlx.GetContext(context.Background(), q, &mdmID,
			`SELECT id FROM mobile_device_management_solutions WHERE name = ? AND server_url = ?`, mobius.WellKnownMDMSimpleMDM, "https://simplemdm.com")
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/labels/%d/hosts", lbl2.ID), nil, http.StatusOK, &listHostsResp, "mdm_id", fmt.Sprint(mdmID))
	require.Len(t, listHostsResp.Hosts, 1)
	...
	assert.Equal(t, mobius.WellKnownMDMSimpleMDM, listHostsResp.MDMSolution.Name)
	assert.Equal(t, "https://simplemdm.com", listHostsResp.MDMSolution.ServerURL)

	// delete a label by id
	var delIDResp deleteLabelByIDResponse
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobius/labels/id/%d", lbl1.ID), nil, http.StatusOK, &delIDResp)

	// delete a non-existing label by id
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobius/labels/id/%d", lbl2.ID+1), nil, http.StatusNotFound, &delIDResp)

	// delete a label by name
	var delResp deleteLabelResponse
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobius/labels/%s", url.PathEscape(lbl2.Name)), nil, http.StatusOK, &delResp)

	// delete a non-existing label by name
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobius/labels/%s", url.PathEscape(lbl2.Name)), nil, http.StatusNotFound, &delResp)

	// delete a manual label by id
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobius/labels/id/%d", manualLbl1.ID), nil, http.StatusOK, &delIDResp)

	// delete a manual label by name
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobius/labels/%s", url.PathEscape(manualLbl2.Name)), nil, http.StatusOK, &delResp)

	// list labels, only the built-ins remain
	s.DoJSON("GET", "/api/latest/mobius/labels", nil, http.StatusOK, &listResp, "per_page", strconv.Itoa(builtInsCount+1))
	...
	s.DoJSON("GET", "/api/latest/mobius/host_summary", nil, http.StatusOK, &hostSummaryResp)
	...
	for name := range builtinsMap {
		...
		s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobius/labels/%s", url.PathEscape(name)), nil, http.StatusUnprocessableEntity, &delResp)

		// attempt to delete by id
		s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobius/labels/id/%d", id), nil, http.StatusUnprocessableEntity, &delIDResp)
	}
}

// Sanity test to make sure mobius/labels/<all>/hosts and mobius/hosts return the same thing.
func (s *integrationTestSuite) TestListHostsByLabel() {
	...
	// Add pack
	_, err = s.ds.NewPack(
		context.Background(), &mobius.Pack{
			Name: t.Name(),
			Hosts: []mobius.Target{
				{
					Type:     mobius.TargetHost,
	...
	qr, err := s.ds.NewQuery(
		context.Background(), &mobius.Query{
			Name:           t.Name(),
			Description:    "Some description",
			Query:          "select * from osquery;",
			ObserverCanRun: true,
	...
	require.NoError(
		...
		s.ds.SetOrUpdateMDMData(
			context.Background(), host.ID, false, true, "https://simplemdm.com", false, mobius.WellKnownMDMSimpleMDM, "",
		),
	)
	...
	require.NoError(
		t, s.ds.ReplaceHostDeviceMapping(
			context.Background(), host.ID, []*mobius.HostDeviceMapping{
				{HostID: hosts[0].ID, Email: "a@b.c", Source: mobius.DeviceMappingGoogleChromeProfiles},
				{HostID: hosts[0].ID, Email: "b@b.c", Source: mobius.DeviceMappingGoogleChromeProfiles},
			}, mobius.DeviceMappingGoogleChromeProfiles,
		),
	...
	s.DoJSON("GET", "/api/latest/mobius/hosts", nil, http.StatusOK, &hostsResp, "device_mapping", "true")
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/labels/%d/hosts", labelID), nil, http.StatusOK, &labelsResp, "device_mapping", "true")
...
func (s *integrationTestSuite) TestLabelSpecs() {
	...
	s.DoJSON("GET", "/api/latest/mobius/spec/labels", nil, http.StatusOK, &listResp)
	assert.True(t, len(listResp.Specs) > 0)
	for _, spec := range listResp.Specs {
		assert.Equal(t, mobius.LabelTypeBuiltIn, spec.LabelType)
	}
	...
	s.DoJSON("POST", "/api/latest/mobius/spec/labels", applyLabelSpecsRequest{
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/spec/labels/%s", url.PathEscape(name)), nil, http.StatusOK, &getResp)
	assert.Equal(t, name, getResp.Spec.Name)
	assert.NotEqual(t, 0, getResp.Spec.ID)

	// get a non-existing label spec
	s.DoJSON("GET", "/api/latest/mobius/spec/labels/zzz", nil, http.StatusNotFound, &getResp)
}
...
func (s *integrationTestSuite) TestUsers() {
	...
	s.DoJSON("GET", "/api/latest/mobius/users", nil, http.StatusOK, &listResp)
	assert.Len(t, listResp.Users, len(s.users))

	// with non-matching query
	s.DoJSON("GET", "/api/latest/mobius/users", nil, http.StatusOK, &listResp, "query", "noone")
	assert.Len(t, listResp.Users, 0)

	// with matching query containing leading/trailing whitespaces
	s.DoJSON("GET", "/api/latest/mobius/users", nil, http.StatusOK, &listResp, "query", " user 	")
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/users/%d", 1), nil, http.StatusOK, &getResp, "include_ui_settings", "true")
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/users/%d", 1), json.RawMessage(`{
		"settings": {
	...
	assert.Equal(t, getResp.Settings, &mobius.UserSettings{HiddenHostColumns: []string{"osquery_version"}})
	...
	assert.Equal(t, uint(1), getResp.User.ID)
	assert.Nil(t, getResp.User.Settings)
	assert.Equal(t, getResp.Settings, &mobius.UserSettings{HiddenHostColumns: []string{"hostname", "osquery_version"}})
	...
	assert.NotNil(t, getMeResp.User.GlobalRole)
	assert.Nil(t, getMeResp.User.Settings)
	assert.Equal(t, getMeResp.Settings, &mobius.UserSettings{HiddenHostColumns: []string{"hostname", "osquery_version"}})
	...
	assert.Equal(t, uint(1), getResp.User.ID)
	assert.Nil(t, getResp.User.Settings)
	assert.Equal(t, getResp.Settings, &mobius.UserSettings{HiddenHostColumns: []string{}})
	...
	assert.NotNil(t, getMeResp.User.GlobalRole)
	assert.Nil(t, getMeResp.User.Settings)
	assert.Equal(t, getMeResp.Settings, &mobius.UserSettings{HiddenHostColumns: []string{}})
	...
	s.DoJSON("POST", "/api/latest/mobius/users/admin", params, http.StatusBadRequest, &createResp)
	...
	s.DoJSONWithoutAuth("POST", "/api/latest/mobius/sessions", sessionCreateRequest{Token: "foo"}, http.StatusUnauthorized, &loginResp)
	// MFA unsupported client
	s.DoJSONWithoutAuth("POST", "/api/latest/mobius/login", params, http.StatusBadRequest, &loginResp)
	// MFA supported; send email
	s.DoJSONWithoutAuth("POST", "/api/latest/mobius/login",
	...
	s.DoJSONWithoutAuth("POST", "/api/latest/mobius/sessions", sessionCreateRequest{Token: mfaToken}, http.StatusOK, &loginResp)
	// can't use the same MFA token twice
	s.DoJSONWithoutAuth("POST", "/api/latest/mobius/sessions", sessionCreateRequest{Token: mfaToken}, http.StatusUnauthorized, &loginResp)
	...
	// turn off MFA
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/users/%d", u.ID), mobius.UserPayload{MFAEnabled: ptr.Bool(false)}, http.StatusOK, &modResp)
	require.False(t, modResp.User.MFAEnabled)

	// can't turn MFA back on
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/users/%d", u.ID), mobius.UserPayload{MFAEnabled: ptr.Bool(true)}, http.StatusPaymentRequired, &modResp)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/users/%d", u.ID), nil, http.StatusOK, &getResp)
	assert.Equal(t, u.ID, getResp.User.ID)
	assert.Len(t, getResp.User.Teams, 0)
	assert.Len(t, getResp.AvailableTeams, 0)

	// get non-existing user
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/users/%d", u.ID+1), nil, http.StatusNotFound, &getResp)

	// modify that user - simple name change
	params = mobius.UserPayload{
		Name: ptr.String("extraz"),
	}
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/users/%d", u.ID), params, http.StatusOK, &modResp)
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/users/%d", u.ID), params, http.StatusConflict, &modResp)
	...
	createInviteResp := createInviteResponse{}
	s.DoJSON("POST", "/api/latest/mobius/invites", createInviteReq, http.StatusPaymentRequired, &createInviteResp)
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/users/%d", u.ID), params, http.StatusForbidden, &modResp)
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/users/%d", u.ID+1), params, http.StatusNotFound, &modResp)
	...
	s.DoJSON(
		"POST", "/api/latest/mobius/perform_required_password_reset", performRequiredPasswordResetRequest{
			Password: newRawPwd,
			ID:       u.ID,
	...
	s.DoJSON("POST", "/api/latest/mobius/perform_required_password_reset", performRequiredPasswordResetRequest{
		Password: newRawPwd,
		ID:       u.ID,
	...
	s.DoJSON("POST", "/api/latest/mobius/login", contract.LoginRequest{Email: u.Email, Password: userRawPwd}, http.StatusOK, &loginResp)
	require.Equal(t, loginResp.User.ID, u.ID)

	// logout for that user
	s.token = loginResp.Token
	var logoutResp logoutResponse
	s.DoJSON("POST", "/api/latest/mobius/logout", nil, http.StatusOK, &logoutResp)

	// logout again, even though not logged in
	s.DoJSON("POST", "/api/latest/mobius/logout", nil, http.StatusUnauthorized, &logoutResp)

	s.token = s.getTestAdminToken()

	// login as that user with previous pwd fails
	loginResp = loginResponse{}
	s.DoJSON("POST", "/api/latest/mobius/login", contract.LoginRequest{Email: u.Email, Password: oldUserRawPwd}, http.StatusUnauthorized, &loginResp)

	// require a password reset
	var reqResetResp requirePasswordResetResponse
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobius/users/%d/require_password_reset", u.ID), map[string]bool{"require": true}, http.StatusOK, &reqResetResp)
	...
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobius/users/%d/require_password_reset", u.ID+1), map[string]bool{"require": true}, http.StatusNotFound, &reqResetResp)

	// delete user
	var delResp deleteUserResponse
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobius/users/%d", u.ID), nil, http.StatusOK, &delResp)

	// delete invalid user
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobius/users/%d", u.ID), nil, http.StatusNotFound, &delResp)
}
...
func (s *integrationTestSuite) TestGlobalPoliciesAutomationConfig() {
	...
	s.DoRaw("PATCH", "/api/latest/mobius/config", []byte(fmt.Sprintf(`{
		"webhook_settings": {
...
func (s *integrationTestSuite) TestActivitiesWebhookConfig() {
	...
	s.DoRaw(
		"PATCH", "/api/latest/mobius/config", []byte(
			`{
		"webhook_settings": {
			"activities_webhook": {
	...
	s.lastActivityOfTypeMatches(
		mobius.ActivityTypeEnabledActivityAutomations{}.ActivityName(),
	...
	s.lastActivityOfTypeMatches(
		mobius.ActivityTypeEditedActivityAutomations{}.ActivityName(),
	...
	s.lastActivityOfTypeMatches(
		mobius.ActivityTypeDisabledActivityAutomations{}.ActivityName(),
		``,
		0,
	)
...
func (s *integrationTestSuite) TestExternalIntegrationsConfig() {
	...
	require.Equal(t, mobius.MaskedPassword, config.Integrations.Jira[0].APIToken)
	...
	require.Equal(t, mobius.MaskedPassword, config.Integrations.Jira[1].APIToken)
	...
	}`, srvURL, mobius.MaskedPassword)), http.StatusBadRequest)
	...
	}`, srvURL, mobius.MaskedPassword)), http.StatusOK)
	...
	require.Equal(t, mobius.MaskedPassword, config.Integrations.Zendesk[0].APIToken)
	...
	require.Equal(t, mobius.MaskedPassword, config.Integrations.Zendesk[1].APIToken)
...
func (s *integrationTestSuite) TestGoogleCalendarIntegrations() {
	...
	assert.Equal(t, email, appConfig.Integrations.GoogleCalendar[0].ApiKey[mobius.GoogleCalendarEmail])
	assert.Equal(t, privateKey, appConfig.Integrations.GoogleCalendar[0].ApiKey[mobius.GoogleCalendarPrivateKey])
	...
	var appCfgResp appConfigResponse
	s.DoJSON(
		"PATCH", "/api/v1/mobius/config", json.RawMessage(
			`{
		"org_info": {
...
func (s *integrationTestSuite) TestQueriesBadRequests() {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.tname, func(t *testing.T) {
			...
			createQueryResp := createQueryResponse{}
			s.DoJSON("POST", "/api/latest/mobius/queries", reqQuery, http.StatusBadRequest, &createQueryResp)
			require.Nil(t, createQueryResp.Query)

			payload := mobius.QueryPayload{
				Name:     ptr.String(tc.name),
				Query:    ptr.String(tc.query),
				Platform: ptr.String(tc.platform),
				Logging:  ptr.String(tc.logging),
			...
			s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/queries/%d", existingQueryID), &payload, http.StatusBadRequest, &mResp)
...
func (s *integrationTestSuite) TestPacksBadRequests() {
	...
	reqPacks := &mobius.PackPayload{
		Name: ptr.String("existing pack"),
	}
	createPackResp := createPackResponse{}
	s.DoJSON("POST", "/api/latest/mobius/packs", reqPacks, http.StatusOK, &createPackResp)
	...
	for _, tc := range []struct {
		...
		t.Run(tc.tname, func(t *testing.T) {
			reqQuery := &mobius.PackPayload{
				Name: ptr.String(tc.name),
			}
			createPackResp := createQueryResponse{}
			s.DoJSON("POST", "/api/latest/mobius/packs", reqQuery, http.StatusBadRequest, &createPackResp)

			payload := mobius.PackPayload{
				Name: ptr.String(tc.name),
			}
			mResp := modifyPackResponse{}
			s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/packs/%d", existingPackID), &payload, http.StatusBadRequest, &mResp)
		})
	}
}
...
func (s *integrationTestSuite) TestPremiumEndpointsWithoutLicense() {
	...
	s.DoJSON("GET", "/api/latest/mobius/teams", nil, http.StatusPaymentRequired, &listResp)
	assert.Len(t, listResp.Teams, 0)

	// get team
	var getResp getTeamResponse
	s.DoJSON("GET", "/api/latest/mobius/teams/123", nil, http.StatusPaymentRequired, &getResp)
	assert.Nil(t, getResp.Team)

	// create team
	var tmResp teamResponse
	s.DoJSON("POST", "/api/latest/mobius/teams", &createTeamRequest{}, http.StatusPaymentRequired, &tmResp)
	assert.Nil(t, tmResp.Team)

	// modify team
	s.DoJSON("PATCH", "/api/latest/mobius/teams/123", mobius.TeamPayload{}, http.StatusPaymentRequired, &tmResp)
	assert.Nil(t, tmResp.Team)

	// delete team
	var delResp deleteTeamResponse
	s.DoJSON("DELETE", "/api/latest/mobius/teams/123", nil, http.StatusPaymentRequired, &delResp)

	// apply team specs
	var specResp applyTeamSpecsResponse
	teamSpecs := applyTeamSpecsRequest{Specs: []*mobius.TeamSpec{{Name: "newteam", Secrets: &[]mobius.EnrollSecret{{Secret: "ABC"}}}}}
	s.DoJSON("POST", "/api/latest/mobius/spec/teams", teamSpecs, http.StatusPaymentRequired, &specResp)

	// modify team agent options
	s.DoJSON("POST", "/api/latest/mobius/teams/123/agent_options", nil, http.StatusPaymentRequired, &tmResp)
	assert.Nil(t, tmResp.Team)

	// list team users
	var usersResp listUsersResponse
	s.DoJSON("GET", "/api/latest/mobius/teams/123/users", nil, http.StatusPaymentRequired, &usersResp, "page", "1")
	assert.Len(t, usersResp.Users, 0)

	// add team users
	s.DoJSON("PATCH", "/api/latest/mobius/teams/123/users", modifyTeamUsersRequest{Users: []mobius.TeamUser{{User: mobius.User{ID: 1}}}}, http.StatusPaymentRequired, &tmResp)
	assert.Nil(t, tmResp.Team)

	// delete team users
	s.DoJSON("DELETE", "/api/latest/mobius/teams/123/users", modifyTeamUsersRequest{Users: []mobius.TeamUser{{User: mobius.User{ID: 1}}}}, http.StatusPaymentRequired, &tmResp)
	assert.Nil(t, tmResp.Team)

	// get team enroll secrets
	var secResp teamEnrollSecretsResponse
	s.DoJSON("GET", "/api/latest/mobius/teams/123/secrets", nil, http.StatusPaymentRequired, &secResp)
	assert.Len(t, secResp.Secrets, 0)

	// modify team enroll secrets
	s.DoJSON("PATCH", "/api/latest/mobius/teams/123/secrets", modifyTeamEnrollSecretsRequest{Secrets: []mobius.EnrollSecret{{Secret: "DEF"}}}, http.StatusPaymentRequired, &secResp)
	assert.Len(t, secResp.Secrets, 0)

	// get apple BM configuration
	var appleBMResp getAppleBMResponse
	s.DoJSON("GET", "/api/latest/mobius/mdm/apple_bm", nil, http.StatusPaymentRequired, &appleBMResp)
	...
	// case where `mobiuscli get config`'s output is used as input to `mobiuscli
	// apply`).
	s.Do("POST", "/api/latest/mobius/mdm/apple/profiles/batch", nil, http.StatusNoContent)

	// batch-apply a non-empty set of MDM profiles fails
	res := s.Do("POST", "/api/latest/mobius/mdm/apple/profiles/batch",
		map[string]interface{}{"profiles": [][]byte{[]byte(`xyz`)}}, http.StatusUnprocessableEntity)
	errMsg := extractServerErrorText(res.Body)
	require.Contains(t, errMsg, "Mobius MDM is not configured")

	// update MDM disk encryption
	_ = s.Do("POST", "/api/latest/mobius/disk_encryption", mobius.MDMAppleSettingsPayload{}, http.StatusPaymentRequired)

	// device migrate mdm endpoint returns an error if not premium
	createHostAndDeviceToken(t, s.ds, "some-token")
	s.Do("POST", fmt.Sprintf("/api/v1/mobius/device/%s/migrate_mdm", "some-token"), nil, http.StatusPaymentRequired)
	...
	s.DoJSON(
		"GET", "/api/latest/mobius/software/titles",
		listSoftwareTitlesRequest{mobius.SoftwareTitleListOptions{VulnerableOnly: true, MinimumCVSS: 7.5}}, http.StatusPaymentRequired, &resp,
	)
	verResp := listSoftwareVersionsResponse{}
	s.DoJSON(
		"GET", "/api/latest/mobius/software/versions",
		listSoftwareRequest{mobius.SoftwareListOptions{VulnerableOnly: true, MinimumCVSS: 7.5}}, http.StatusPaymentRequired, &verResp,
	)
	countResp := countSoftwareResponse{}
	s.DoJSON(
		"GET", "/api/latest/mobius/software/count",
		listSoftwareRequest{mobius.SoftwareListOptions{VulnerableOnly: true, MinimumCVSS: 7.5}}, http.StatusPaymentRequired, &countResp,
	)

	resp = listSoftwareTitlesResponse{}
	s.DoJSON(
		"GET", "/api/latest/mobius/software/titles",
		listSoftwareTitlesRequest{mobius.SoftwareTitleListOptions{VulnerableOnly: true, MaximumCVSS: 7.5}}, http.StatusPaymentRequired, &resp,
	)
	verResp = listSoftwareVersionsResponse{}
	s.DoJSON(
		"GET", "/api/latest/mobius/software/versions",
		listSoftwareRequest{mobius.SoftwareListOptions{VulnerableOnly: true, MaximumCVSS: 7.5}}, http.StatusPaymentRequired, &verResp,
	)
	countResp = countSoftwareResponse{}
	s.DoJSON(
		"GET", "/api/latest/mobius/software/count",
		listSoftwareRequest{mobius.SoftwareListOptions{VulnerableOnly: true, MaximumCVSS: 7.5}}, http.StatusPaymentRequired, &countResp,
	)

	resp = listSoftwareTitlesResponse{}
	s.DoJSON(
		"GET", "/api/latest/mobius/software/titles",
		listSoftwareTitlesRequest{mobius.SoftwareTitleListOptions{VulnerableOnly: true, KnownExploit: true}}, http.StatusPaymentRequired, &resp,
	)
	verResp = listSoftwareVersionsResponse{}
	s.DoJSON(
		"GET", "/api/latest/mobius/software/versions",
		listSoftwareRequest{mobius.SoftwareListOptions{VulnerableOnly: true, KnownExploit: true}}, http.StatusPaymentRequired, &verResp,
	)
	countResp = countSoftwareResponse{}
	s.DoJSON(
		"GET", "/api/latest/mobius/software/count",
		listSoftwareRequest{mobius.SoftwareListOptions{VulnerableOnly: true, KnownExploit: true}}, http.StatusPaymentRequired, &countResp,
	)

	// lock/unlock/wipe a host
	s.Do("POST", "/api/v1/mobius/hosts/123/lock", nil, http.StatusPaymentRequired)
	s.Do("POST", "/api/v1/mobius/hosts/123/unlock", nil, http.StatusPaymentRequired)
	s.Do("POST", "/api/v1/mobius/hosts/123/wipe", nil, http.StatusPaymentRequired)
...
func (s *integrationTestSuite) TestScriptsEndpointsWithoutLicense() {
	...
	s.DoJSON("POST", "/api/latest/mobius/scripts/run", mobius.HostScriptRequestPayload{HostID: 1, ScriptContents: "echo foo"}, http.StatusNotFound, &runResp)

	// run a script sync
	s.DoJSON("POST", "/api/latest/mobius/scripts/run/sync", mobius.HostScriptRequestPayload{HostID: 1, ScriptContents: "echo foo"}, http.StatusNotFound, &runResp)

	// get script result
	var scriptResultResp getScriptResultResponse
	s.DoJSON("GET", "/api/latest/mobius/scripts/results/test-id", nil, http.StatusNotFound, &scriptResultResp)

	// create a saved script
	body, headers := generateNewScriptMultipartRequest(t,
		"myscript.sh", []byte(`echo "hello"`), s.token, nil)
	s.DoRawWithHeaders("POST", "/api/latest/mobius/scripts", body.Bytes(), http.StatusOK, headers)

	// run a saved script by name without team id (should fail host not found)
	res := s.Do("POST", "/api/latest/mobius/scripts/run/sync", runScriptSyncRequest{ScriptName: "myscript.sh"}, http.StatusNotFound)
	errMsg := extractServerErrorText(res.Body)
	...
	res = s.Do("POST", "/api/latest/mobius/scripts/run/sync", runScriptSyncRequest{ScriptName: "myscript.sh", TeamID: 1}, http.StatusPaymentRequired)
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, "Requires Mobius Premium license")

	// delete a saved script
	var delScriptResp deleteScriptResponse
	s.DoJSON("DELETE", "/api/latest/mobius/scripts/123", nil, http.StatusNotFound, &delScriptResp)

	// list saved scripts
	var listScriptsResp listScriptsResponse
	s.DoJSON("GET", "/api/latest/mobius/scripts", nil, http.StatusOK, &listScriptsResp, "per_page", "10")

	// get a saved script
	var getScriptResp getScriptResponse
	s.DoJSON("GET", "/api/latest/mobius/scripts/123", nil, http.StatusNotFound, &getScriptResp)

	// get host script details
	var getHostScriptDetailsResp getHostScriptDetailsResponse
	s.DoJSON("GET", "/api/latest/mobius/hosts/123/scripts", nil, http.StatusNotFound, &getHostScriptDetailsResp)

	// batch set scripts
	s.Do("POST", "/api/v1/mobius/scripts/batch", batchSetScriptsRequest{Scripts: nil}, http.StatusOK)
}
...
func (s *integrationTestSuite) TestGlobalPoliciesBrowsing() {
	...
	gpResp0 := globalPolicyResponse{}
	s.DoJSON("POST", "/api/latest/mobius/policies", gpParams0, http.StatusOK, &gpResp0)
...
func (s *integrationTestSuite) TestTeamPoliciesTeamNotExists() {
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/teams/%d/policies", 9999999), nil, http.StatusNotFound, &teamPoliciesResponse)
	require.Len(t, teamPoliciesResponse.Policies, 0)

	deleteTeamPoliciesResponse := deleteTeamPoliciesResponse{}
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobius/teams/%d/policies/delete", 9999999), deleteTeamPoliciesRequest{IDs: []uint{1, 1000}}, http.StatusNotFound, &deleteTeamPoliciesResponse)
}
...
func (s *integrationTestSuite) TestSessionInfo() {
	...
	resp := s.DoRawWithHeaders("GET", "/api/latest/mobius/me", nil, http.StatusOK, map[string]string{
		"Authorization": fmt.Sprintf("Bearer %s", ssn.Key),
	})
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/sessions/%d", ssn.ID), nil, http.StatusOK, &getResp)
	assert.Equal(t, ssn.ID, getResp.SessionID)
	assert.Equal(t, uint(1), getResp.UserID)

	// get info about session
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/sessions/%d", ssn.ID+1), nil, http.StatusNotFound, &getResp)

	// delete session
	var delResp deleteSessionResponse
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobius/sessions/%d", ssn.ID), nil, http.StatusOK, &delResp)

	// delete session - non-existing
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobius/sessions/%d", ssn.ID), nil, http.StatusNotFound, &delResp)
}
...
func (s *integrationTestSuite) TestAppConfig() {
	...
	assert.Equal(t, "MobiusTest", acResp.OrgInfo.OrgName) // set in SetupSuite
	...
	s.DoJSON("GET", "/api/latest/mobius/activities", nil, http.StatusOK, &listActivities, "order_key", "id", "order_direction", "desc")
	if len(listActivities.Activities) > 1 {
		// if there is an activity, make sure it is not edited_agent_options
		require.NotEqual(t, mobius.ActivityTypeEditedAgentOptions{}.ActivityName(), listActivities.Activities[0].Type)
	}
	...
	acResp = appConfigResponse{}
	s.DoJSON(
		"PATCH", "/api/latest/mobius/config", json.RawMessage(
			`{
    "server_settings": {
	...
	require.Equal(t, mobius.ActivityTypeEditedAgentOptions{}.ActivityName(), listActivities.Activities[0].Type)
	...
	s.DoJSON("GET", "/api/latest/mobius/version", nil, http.StatusOK, &verResp)
	assert.NotEmpty(t, verResp.Branch)

	// get enroll secrets, none yet
	var specResp getEnrollSecretSpecResponse
	s.DoJSON("GET", "/api/latest/mobius/spec/enroll_secret", nil, http.StatusOK, &specResp)
	...
	errMsg = extractServerErrorText(res.Body)
	assert.Contains(t, errMsg, "Please configure Mobius with a certificate and key pair first.")
	...
	var defAppCfg mobius.AppConfig
	...
	s.DoRaw("PATCH", "/api/latest/mobius/config", jsonMustMarshal(t, defAppCfg), http.StatusOK)
...
func (s *integrationTestSuite) TestQuerySpecs() {
	...
	s.DoJSON("GET", "/api/latest/mobius/spec/queries", nil, http.StatusOK, &getSpecsResp)
	assert.Len(t, getSpecsResp.Specs, 0)

	// get unknown one
	var getSpecResp getQuerySpecResponse
	s.DoJSON("GET", "/api/latest/mobius/spec/queries/nonesuch", nil, http.StatusNotFound, &getSpecResp)

	// create some queries via apply specs
	q1 := strings.ReplaceAll(t.Name(), "/", "_")
	q2 := q1 + "_2"
	var applyResp applyQuerySpecsResponse
	s.DoJSON("POST", "/api/latest/mobius/spec/queries", applyQuerySpecsRequest{
		Specs: []*mobius.QuerySpec{
	...
	var listQryResp listQueriesResponse
	s.DoJSON("GET", "/api/latest/mobius/queries", nil, http.StatusOK, &listQryResp, "order_key", "name")
	require.Len(t, listQryResp.Queries, 2)
	...
	// get specific spec
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/spec/queries/%s", q1), nil, http.StatusOK, &getSpecResp)
...
func (s *integrationTestSuite) TestListSoftwareAndSoftwareDetails() {
	...
	sws := make([]mobius.Software, 20)
	for i := range sws {
		sw := mobius.Software{Name: fmt.Sprintf("sw%02d", i), Version: fmt.Sprintf("0.0.%02d", i), Source: "apps"}
		if i%2 == 0 {
	...
	sortByNameAlphanumeric := func(sw []mobius.Software, a, b int) bool {
		aNum, _ := strconv.Atoi(strings.TrimPrefix(sw[a].Name, "sw"))
		bNum, _ := strconv.Atoi(strings.TrimPrefix(sw[b].Name, "sw"))
		return aNum < bNum
	}
	sortEntryByNameAlphanumeric := func(sw []mobius.HostSoftwareEntry, a, b int) bool {
		aNum, _ := strconv.Atoi(strings.TrimPrefix(sw[a].Name, "sw"))
		bNum, _ := strconv.Atoi(strings.TrimPrefix(sw[b].Name, "sw"))
		return aNum < bNum
	}
	...
	for i, h := range hosts {
		...
		if i == 0 {
			...
			sws = make([]mobius.Software, 0, len(h.Software))
	...
	var cpes []mobius.SoftwareCPE
	for i, sw := range sws {
		cpes = append(cpes, mobius.SoftwareCPE{SoftwareID: sw.ID, CPE: "somecpe" + strconv.Itoa(i)})
	}
	...
	assertSoftwareDetails := func(expectedSoftware []mobius.Software, team string) {
		t.Helper()
		...
		for _, sw := range expectedSoftware {
			...
			s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/software/%d", sw.ID), nil, http.StatusOK, &detailsResp, "team_id", team)
			...
			s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/software/versions/%d", sw.ID), nil, http.StatusOK, &detailsResp, "team_id", team)
			assert.Equal(t, sw.ID, detailsResp.Software.ID)
			assert.Equal(t, sw.Name, detailsResp.Software.Name)
			assert.Equal(t, sw.Version, detailsResp.Software.Version)
			assert.Equal(t, sw.Source, detailsResp.Software.Source)
	...
	assertResp := func(resp listSoftwareResponse, want []mobius.Software, ts time.Time, team string, counts ...int) {
		t.Helper()
	...
	assertVersionsResp := func(
		resp listSoftwareVersionsResponse, want []mobius.Software, ts time.Time, team string, swCount int, hostCounts ...int,
	) {
	...
	// no software host counts have been calculated yet, so this returns nothing
	var lsResp listSoftwareResponse
	s.DoJSON("GET", "/api/latest/mobius/software", nil, http.StatusOK, &lsResp, "order_key", "hosts_count", "order_direction", "desc")
	assertResp(lsResp, nil, time.Time{}, "")
	var versResp listSoftwareVersionsResponse
	s.DoJSON("GET", "/api/latest/mobius/software/versions", nil, http.StatusOK, &versResp, "order_key", "hosts_count", "order_direction", "desc")
	...
	lsResp = listSoftwareResponse{}
	s.DoJSON(
		"GET", "/api/latest/mobius/software", nil, http.StatusOK, &lsResp, "order_key", "hosts_count", "order_direction", "desc", "team_id",
		teamStr,
	)
	assertResp(lsResp, nil, time.Time{}, teamStr)
	versResp = listSoftwareVersionsResponse{}
	s.DoJSON(
		"GET", "/api/latest/mobius/software/versions", nil, http.StatusOK, &versResp, "order_key", "hosts_count", "order_direction", "desc",
		"team_id", teamStr,
	)
	...
	lsResp = listSoftwareResponse{}
	s.DoJSON("GET", "/api/latest/mobius/software", nil, http.StatusOK, &lsResp, "per_page", "5", "page", "0", "order_key", "hosts_count", "order_direction", "desc")
	assertResp(lsResp, []mobius.Software{sws[19], sws[18], sws[17], sws[16], sws[15]}, hostsCountTs, "", 20, 19, 18, 17, 16)
	versResp = listSoftwareVersionsResponse{}
	s.DoJSON("GET", "/api/latest/mobius/software/versions", nil, http.StatusOK, &versResp, "per_page", "5", "page", "0", "order_key", "hosts_count", "order_direction", "desc")
	assertVersionsResp(
		versResp, []mobius.Software{sws[19], sws[18], sws[17], sws[16], sws[15]}, hostsCountTs, "", len(sws), 20, 19, 18, 17, 16,
	)
	...
	// second page (page=1)
	lsResp = listSoftwareResponse{}
	s.DoJSON("GET", "/api/latest/mobius/software", nil, http.StatusOK, &lsResp, "per_page", "5", "page", "1", "order_key", "hosts_count", "order_direction", "desc")
	assertResp(lsResp, []mobius.Software{sws[14], sws[13], sws[12], sws[11], sws[10]}, hostsCountTs, "", 15, 14, 13, 12, 11)
	versResp = listSoftwareVersionsResponse{}
	s.DoJSON("GET", "/api/latest/mobius/software/versions", nil, http.StatusOK, &versResp, "per_page", "5", "page", "1", "order_key", "hosts_count", "order_direction", "desc")
	assertVersionsResp(
		versResp, []mobius.Software{sws[14], sws[13], sws[12], sws[11], sws[10]}, hostsCountTs, "", len(sws), 15, 14, 13, 12, 11,
	)
	...
	// third page (page=2)
	lsResp = listSoftwareResponse{}
	s.DoJSON("GET", "/api/latest/mobius/software", nil, http.StatusOK, &lsResp, "per_page", "5", "page", "2", "order_key", "hosts_count", "order_direction", "desc")
	assertResp(lsResp, []mobius.Software{sws[9], sws[8], sws[7], sws[6], sws[5]}, hostsCountTs, "", 10, 9, 8, 7, 6)
	versResp = listSoftwareVersionsResponse{}
	s.DoJSON("GET", "/api/latest/mobius/software/versions", nil, http.StatusOK, &versResp, "per_page", "5", "page", "2", "order_key", "hosts_count", "order_direction", "desc")
	assertVersionsResp(versResp, []mobius.Software{sws[9], sws[8], sws[7], sws[6], sws[5]}, hostsCountTs, "", len(sws), 10, 9, 8, 7, 6)
	require.True(t, versResp.Meta.HasPreviousResults)
	require.True(t, versResp.Meta.HasNextResults)

	// last page (page=3)
	lsResp = listSoftwareResponse{}
	s.DoJSON("GET", "/api/latest/mobius/software", nil, http.StatusOK, &lsResp, "per_page", "5", "page", "3", "order_key", "hosts_count", "order_direction", "desc")
	assertResp(lsResp, []mobius.Software{sws[4], sws[3], sws[2], sws[1], sws[0]}, hostsCountTs, "", 5, 4, 3, 2, 1)
	versResp = listSoftwareVersionsResponse{}
	s.DoJSON("GET", "/api/latest/mobius/software/versions", nil, http.StatusOK, &versResp, "per_page", "5", "page", "3", "order_key", "hosts_count", "order_direction", "desc")
	assertVersionsResp(versResp, []mobius.Software{sws[4], sws[3], sws[2], sws[1], sws[0]}, hostsCountTs, "", len(sws), 5, 4, 3, 2, 1)
	require.True(t, versResp.Meta.HasPreviousResults)
	require.False(t, versResp.Meta.HasNextResults)

	// past the end
	lsResp = listSoftwareResponse{}
	s.DoJSON("GET", "/api/latest/mobius/software", nil, http.StatusOK, &lsResp, "per_page", "5", "page", "4", "order_key", "hosts_count", "order_direction", "desc")
	assertResp(lsResp, nil, time.Time{}, "")
	versResp = listSoftwareVersionsResponse{}
	s.DoJSON("GET", "/api/latest/mobius/software/versions", nil, http.StatusOK, &versResp, "per_page", "5", "page", "4", "order_key", "hosts_count", "order_direction", "desc")
	assertVersionsResp(versResp, nil, time.Time{}, "", len(sws))

	// no explicit sort order, defaults to hosts_count DESC
	lsResp = listSoftwareResponse{}
	s.DoJSON("GET", "/api/latest/mobius/software", nil, http.StatusOK, &lsResp, "per_page", "2", "page", "0")
	assertResp(lsResp, []mobius.Software{sws[19], sws[18]}, hostsCountTs, "", 20, 19)
	versResp = listSoftwareVersionsResponse{}
	s.DoJSON("GET", "/api/latest/mobius/software/versions", nil, http.StatusOK, &versResp, "per_page", "2", "page", "0")
	assertVersionsResp(versResp, []mobius.Software{sws[19], sws[18]}, hostsCountTs, "", len(sws), 20, 19)

	// hosts_count ascending
	lsResp = listSoftwareResponse{}
	s.DoJSON("GET", "/api/latest/mobius/software", nil, http.StatusOK, &lsResp, "per_page", "3", "page", "0", "order_key", "hosts_count", "order_direction", "asc")
	assertResp(lsResp, []mobius.Software{sws[0], sws[1], sws[2]}, hostsCountTs, "", 1, 2, 3)
	versResp = listSoftwareVersionsResponse{}
	s.DoJSON("GET", "/api/latest/mobius/software/versions", nil, http.StatusOK, &versResp, "per_page", "3", "page", "0", "order_key", "hosts_count", "order_direction", "asc")
	assertVersionsResp(versResp, []mobius.Software{sws[0], sws[1], sws[2]}, hostsCountTs, "", len(sws), 1, 2, 3)

	// vulnerable software only
	lsResp = listSoftwareResponse{}
	s.DoJSON("GET", "/api/latest/mobius/software", nil, http.StatusOK, &lsResp, "vulnerable", "true", "per_page", "5", "page", "0", "order_key", "hosts_count", "order_direction", "desc")
	assertResp(lsResp, []mobius.Software{sws[9], sws[8], sws[7], sws[6], sws[5]}, hostsCountTs, "", 10, 9, 8, 7, 6)
	versResp = listSoftwareVersionsResponse{}
	s.DoJSON("GET", "/api/latest/mobius/software/versions", nil, http.StatusOK, &versResp, "vulnerable", "true", "per_page", "5", "page", "0", "order_key", "hosts_count", "order_direction", "desc")
	assertVersionsResp(
		versResp, []mobius.Software{sws[9], sws[8], sws[7], sws[6], sws[5]}, hostsCountTs, "", expectedVulnVersionsCount, 10, 9, 8, 7, 6,
	)

	// vulnerable software only, next page
	lsResp = listSoftwareResponse{}
	s.DoJSON("GET", "/api/latest/mobius/software", nil, http.StatusOK, &lsResp, "vulnerable", "true", "per_page", "5", "page", "1", "order_key", "hosts_count", "order_direction", "desc")
	assertResp(lsResp, []mobius.Software{sws[4], sws[3], sws[2], sws[1], sws[0]}, hostsCountTs, "", 5, 4, 3, 2, 1)
	versResp = listSoftwareVersionsResponse{}
	s.DoJSON("GET", "/api/latest/mobius/software/versions", nil, http.StatusOK, &versResp, "vulnerable", "true", "per_page", "5", "page", "1", "order_key", "hosts_count", "order_direction", "desc")
	assertVersionsResp(
		versResp, []mobius.Software{sws[4], sws[3], sws[2], sws[1], sws[0]}, hostsCountTs, "", expectedVulnVersionsCount, 5, 4, 3, 2, 1,
	)

	// vulnerable software only, past last page
	lsResp = listSoftwareResponse{}
	s.DoJSON("GET", "/api/latest/mobius/software", nil, http.StatusOK, &lsResp, "vulnerable", "true", "per_page", "5", "page", "2", "order_key", "hosts_count", "order_direction", "desc")
	assertResp(lsResp, nil, time.Time{}, "")
	versResp = listSoftwareVersionsResponse{}
	s.DoJSON("GET", "/api/latest/mobius/software/versions", nil, http.StatusOK, &versResp, "vulnerable", "true", "per_page", "5", "page", "2", "order_key", "hosts_count", "order_direction", "desc")
	...
	versionsResp := listSoftwareVersionsResponse{}
	s.DoJSON("GET", "/api/latest/mobius/software/versions", nil, http.StatusOK, &versionsResp, "query", sws[0].Name)
	assertVersionsResp(versionsResp, []mobius.Software{sws[0]}, hostsCountTs, "", 1, 1)
	// with whitespace
	s.DoJSON("GET", "/api/latest/mobius/software/versions", nil, http.StatusOK, &versionsResp, "query", " "+sws[0].Name+"\n")
	assertVersionsResp(versionsResp, []mobius.Software{sws[0]}, hostsCountTs, "", 1, 1)

	s.DoJSON("GET", "/api/latest/mobius/software/versions", nil, http.StatusOK, &versionsResp, "query", sws[0].Version)
	assertVersionsResp(versionsResp, []mobius.Software{sws[0]}, hostsCountTs, "", 1, 1)
	// with whitespace
	s.DoJSON("GET", "/api/latest/mobius/software/versions", nil, http.StatusOK, &versionsResp, "query", "\n"+sws[0].Version+"  ")
	assertVersionsResp(versionsResp, []mobius.Software{sws[0]}, hostsCountTs, "", 1, 1)

	// All 10 CVEs added to the first 10 software have the same cvePrefix, so should return all
	// 10 vulnerable software versions
	s.DoJSON("GET", "/api/latest/mobius/software/versions", nil, http.StatusOK, &versionsResp, "query", testCvePrefix)
	...
	lsResp = listSoftwareResponse{}
	s.DoJSON(
		"GET", "/api/latest/mobius/software", nil, http.StatusOK, &lsResp, "per_page", "2", "page", "0", "order_key", "hosts_count",
		"order_direction", "desc", "team_id", teamStr,
	)
	assertResp(lsResp, []mobius.Software{sws[19], sws[18]}, hostsCountTs, teamStr, 3, 2)
	versResp = listSoftwareVersionsResponse{}
	s.DoJSON(
		"GET", "/api/latest/mobius/software/versions", nil, http.StatusOK, &versResp, "per_page", "2", "page", "0", "order_key",
		"hosts_count", "order_direction", "desc", "team_id", teamStr,
	)
	assertVersionsResp(versResp, []mobius.Software{sws[19], sws[18]}, hostsCountTs, teamStr, expectedTeamVersionsCount, 3, 2)

	// filter by the team, 2 by page, next page
	lsResp = listSoftwareResponse{}
	s.DoJSON(
		"GET", "/api/latest/mobius/software", nil, http.StatusOK, &lsResp, "per_page", "2", "page", "1", "order_key", "hosts_count",
		"order_direction", "desc", "team_id", teamStr,
	)
	assertResp(lsResp, []mobius.Software{sws[17]}, hostsCountTs, teamStr, 1)
	versResp = listSoftwareVersionsResponse{}
	s.DoJSON(
		"GET", "/api/latest/mobius/software/versions", nil, http.StatusOK, &versResp, "per_page", "2", "page", "1", "order_key",
		"hosts_count", "order_direction", "desc", "team_id", teamStr,
	)
	assertVersionsResp(versResp, []mobius.Software{sws[17]}, hostsCountTs, teamStr, expectedTeamVersionsCount, 1)

	// filter by no team, 2 by page
	lsResp = listSoftwareResponse{}
	s.DoJSON(
		"GET", "/api/latest/mobius/software", nil, http.StatusOK, &lsResp, "per_page", "2", "page", "0", "order_key", "name",
		"order_direction", "desc", "team_id", "0",
	)
	fmt.Printf("lsResp: %+v\n", lsResp)
	assertResp(lsResp, []mobius.Software{sws[19], sws[18]}, hostsCountTs, "", 17, 17)

	// Invalid software team -- admin gets a 404, team users get a 403
	detailsResp := getSoftwareResponse{}
	s.DoJSON(
		"GET", fmt.Sprintf("/api/latest/mobius/software/versions/%d", versResp.Software[0].ID), nil, http.StatusNotFound, &detailsResp,
		"team_id", "999999",
	)
...
func (s *integrationTestSuite) TestChangeUserEmail() {
	...
	s.DoJSON("GET", "/api/latest/mobius/email/change/invalidtoken", nil, http.StatusNotFound, &changeResp)
	...
	s.DoJSON("GET", "/api/latest/mobius/email/change/validtoken", nil, http.StatusNotFound, &changeResp)
	...
	changeResp = changeEmailResponse{}
	s.DoJSON("GET", "/api/latest/mobius/email/change/validtoken", nil, http.StatusOK, &changeResp)
...
func (s *integrationTestSuite) TestSearchTargets() {
	...
	s.DoJSON("POST", "/api/latest/mobius/targets", searchTargetsRequest{}, http.StatusOK, &searchResp)
	...
	s.DoJSON("POST", "/api/latest/mobius/targets", searchTargetsRequest{Selected: mobius.HostTargets{LabelIDs: lblIDs}}, http.StatusOK, &searchResp)
	require.Equal(t, uint(0), searchResp.TargetsCount)
	...
	searchResp = searchTargetsResponse{}
	s.DoJSON("POST", "/api/latest/mobius/targets", searchTargetsRequest{Selected: mobius.HostTargets{HostIDs: []uint{hosts[1].ID}}}, http.StatusOK, &searchResp)
	...
	searchResp = searchTargetsResponse{}
	s.DoJSON("POST", "/api/latest/mobius/targets", searchTargetsRequest{MatchQuery: "foo.local1"}, http.StatusOK, &searchResp)
	require.Equal(t, uint(0), searchResp.TargetsCount)
...
func (s *integrationTestSuite) TestSearchHosts() {
	...
	s.DoJSON("POST", "/api/latest/mobius/hosts/search", searchHostsRequest{}, http.StatusOK, &searchResp)
	...
	s.DoJSON("POST", "/api/latest/mobius/hosts/search", searchHostsRequest{ExcludedHostIDs: []uint{}}, http.StatusOK, &searchResp)
	require.Len(t, searchResp.Hosts, len(hosts)) // no omitted host id

	searchResp = searchHostsResponse{}
	s.DoJSON("POST", "/api/latest/mobius/hosts/search", searchHostsRequest{ExcludedHostIDs: []uint{hosts[1].ID}}, http.StatusOK, &searchResp)
	require.Len(t, searchResp.Hosts, len(hosts)-1) // one omitted host id

	searchResp = searchHostsResponse{}
	s.DoJSON("POST", "/api/latest/mobius/hosts/search", searchHostsRequest{MatchQuery: "foo.local1"}, http.StatusOK, &searchResp)
	...
	require.NoError(t, err)
	searchResp = searchHostsResponse{}
	s.DoJSON("POST", "/api/latest/mobius/hosts/search", searchHostsRequest{MatchQuery: "foo.local0"}, http.StatusOK, &searchResp)
	require.Len(t, searchResp.Hosts, 1)
	...
	s.DoJSON("POST", "/api/latest/mobius/hosts/search", searchHostsRequest{MatchQuery: "a@b.c"}, http.StatusOK, &searchResp)
	require.Len(t, searchResp.Hosts, 1)

	// search for non-existent email, shouldn't get anything back
	s.DoJSON("POST", "/api/latest/mobius/hosts/search", searchHostsRequest{MatchQuery: "not@found.com"}, http.StatusOK, &searchResp)
...
func (s *integrationTestSuite) TestCountTargets() {
	...
	team, err := s.ds.NewTeam(context.Background(), &mobius.Team{Name: "TestTeam"})
	require.NoError(t, err)
	...
	// none selected
	s.DoJSON("POST", "/api/latest/mobius/targets/count", countTargetsRequest{}, http.StatusOK, &countResp)
	...
	s.DoJSON("POST", "/api/latest/mobius/targets/count", countTargetsRequest{Selected: mobius.HostTargets{LabelIDs: lblIDs}}, http.StatusOK, &countResp)
	...
	// team selected
	countResp = countTargetsResponse{}
	s.DoJSON("POST", "/api/latest/mobius/targets/count", countTargetsRequest{Selected: mobius.HostTargets{TeamIDs: []uint{team.ID}}}, http.StatusOK, &countResp)
	...
	countResp = countTargetsResponse{}
	s.DoJSON(
		"POST", "/api/latest/mobius/targets/count", countTargetsRequest{Selected: mobius.HostTargets{TeamIDs: []uint{0}}},
	...
	// host id selected
	countResp = countTargetsResponse{}
	s.DoJSON("POST", "/api/latest/mobius/targets/count", countTargetsRequest{Selected: mobius.HostTargets{HostIDs: []uint{hosts[1].ID}}}, http.StatusOK, &countResp)
	require.Equal(t, uint(1), countResp.TargetsCount)
	require.Equal(t, uint(0), countResp.TargetsOnline)
...
func (s *integrationTestSuite) TestStatus() {
	...
	s.DoJSON("GET", "/api/latest/mobius/status/result_store", nil, http.StatusOK, &statusResp)
	s.DoJSON("GET", "/api/latest/mobius/status/live_query", nil, http.StatusOK, &statusResp)
}
...
func (s *integrationTestSuite) TestEnrollHost() {
	...
	s.DoJSON("POST", "/api/latest/mobius/spec/enroll_secret", applyEnrollSecretSpecRequest{
		Spec: &mobius.EnrollSecretSpec{
			Secrets: []*mobius.EnrollSecret{{Secret: t.Name()}},
		},
	}, http.StatusOK, &applyResp)
...
func (s *integrationTestSuite) TestReenrollHostCleansPolicies() {
	...
	var getHostResp getHostResponse
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d", host.ID), nil, http.StatusOK, &getHostResp)
	require.Empty(t, getHostResp.Host.Policies)

	// create a policy and make the host fail it
	pol, err := s.ds.NewGlobalPolicy(ctx, nil, mobius.PolicyPayload{Name: t.Name(), Query: "SELECT 1", Platform: host.MobiusPlatform()})
	require.NoError(t, err)
	err = s.ds.RecordPolicyQueryExecutions(ctx, &mobius.Host{ID: host.ID}, map[uint]*bool{pol.ID: ptr.Bool(false)}, time.Now(), false)
	require.NoError(t, err)
...
func (s *integrationTestSuite) TestCarve() {
	...
	checkCarveError := func(id uint, err string) {
		var getResp getCarveResponse
		s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/carves/%d", id), nil, http.StatusOK, &getResp)
...
func (s *integrationTestSuite) TestLogLoginAttempts() {
	...
	res := s.DoRawNoAuth("POST", "/api/latest/mobius/login",
	...
	require.Equal(t, activity.Type, mobius.ActivityTypeUserFailedLogin{}.ActivityName())
	require.NotNil(t, activity.Details)
	actDetails := mobius.ActivityTypeUserFailedLogin{}
	...
	activity = activitiesResp.Activities[len(activitiesResp.Activities)-1]
	require.Equal(t, activity.Type, mobius.ActivityTypeUserLoggedIn{}.ActivityName())
	require.NotNil(t, activity.Details)
	err = json.Unmarshal(*activity.Details, &mobius.ActivityTypeUserLoggedIn{})
	require.NoError(t, err)
}
...
func (s *integrationTestSuite) TestChangePassword() {
	...
	endpoint := "/api/latest/mobius/change_password"
...
func (s *integrationTestSuite) TestPasswordReset() {
	...
	res := s.DoRawNoAuth("POST", "/api/latest/mobius/forgot_password", jsonMustMarshal(t, forgotPasswordRequest{Email: "invalid@asd.com"}), http.StatusInternalServerError)
	res.Body.Close()

	// Configure SMTP
	var configResp appConfigResponse
	s.DoJSON("PATCH", "/api/latest/mobius/config", json.RawMessage("{\"smtp_settings\":{\"enable_smtp\":true,\"sender_address\":\"user@example.com\",\"server\":\"127.0.0.1\",\"port\":1025,\"authentication_type\":\"authtype_none\"}}"), http.StatusOK, &configResp)

	// request forgot password, invalid email
	res = s.DoRawNoAuth("POST", "/api/latest/mobius/forgot_password", jsonMustMarshal(t, forgotPasswordRequest{Email: "invalid@asd.com"}), http.StatusAccepted)
	res.Body.Close()
	...
	// for i := 0; i < 20; i++ {
	//	s.DoJSON("POST", "/api/latest/mobius/forgot_password", forgotPasswordRequest{Email: "invalid@asd.com"}, http.StatusAccepted, &forgotResp)
	// }

	// request forgot password, valid email
	res = s.DoRawNoAuth("POST", "/api/latest/mobius/forgot_password", jsonMustMarshal(t, forgotPasswordRequest{Email: u.Email}), http.StatusAccepted)
	res.Body.Close()
	...
	res = s.DoRawNoAuth("POST", "/api/latest/mobius/reset_password", jsonMustMarshal(t, resetPasswordRequest{PasswordResetToken: token, NewPassword: userNewPwd}), http.StatusOK)
	res.Body.Close()

	// attempt it again with already-used token
	userUnusedPwd := "unusedpassw0rd!"
	res = s.DoRawNoAuth("POST", "/api/latest/mobius/reset_password", jsonMustMarshal(t, resetPasswordRequest{PasswordResetToken: token, NewPassword: userUnusedPwd}), http.StatusUnauthorized)
	res.Body.Close()

	// login with the old password, should not succeed
	res = s.DoRawNoAuth("POST", "/api/latest/mobius/login", jsonMustMarshal(t, contract.LoginRequest{Email: u.Email, Password: userRawPwd}),
		http.StatusUnauthorized)
	res.Body.Close()

	// login with the new password, should succeed
	res = s.DoRawNoAuth("POST", "/api/latest/mobius/login", jsonMustMarshal(t, contract.LoginRequest{Email: u.Email, Password: userNewPwd}),
		http.StatusOK)
	res.Body.Close()
}
...
func (s *integrationTestSuite) TestModifyUser() {
	...
	var modResp modifyUserResponse
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/users/%d", u.ID), mobius.UserPayload{
	...
	var loginResp loginResponse
	resp := s.DoRawNoAuth("POST", "/api/latest/mobius/login", jsonMustMarshal(t, contract.LoginRequest{
	...
	res := s.Do("PATCH", fmt.Sprintf("/api/latest/mobius/users/%d", u.ID), mobius.UserPayload{
		SSOEnabled: ptr.Bool(false),
	}, http.StatusUnprocessableEntity)
	errMsg := extractServerErrorText(res.Body)
	...
	s.Do("PATCH", fmt.Sprintf("/api/latest/mobius/users/%d", u.ID), mobius.UserPayload{
		SSOEnabled:  ptr.Bool(false),
...
func (s *integrationTestSuite) TestGetHostLastOpenedAt() {
	...
	var listHostsResp listHostsResponse
	s.DoJSON("GET", "/api/latest/mobius/hosts", nil, http.StatusOK, &listHostsResp)
...
func (s *integrationTestSuite) TestGetHostSoftwareUpdatedAt() {
	...
	getHostResp = getHostResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d", host.ID), nil, http.StatusOK, &getHostResp, "exclude_software", "true")
	require.Equal(t, host.ID, getHostResp.Host.ID)
	require.Empty(t, getHostResp.Host.Software)
	require.Greater(t, getHostResp.Host.SoftwareUpdatedAt, getHostResp.Host.CreatedAt)

	getHostResp = getHostResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/identifier/%s", *host.NodeKey), nil, http.StatusOK, &getHostResp)
	require.Equal(t, host.ID, getHostResp.Host.ID)
	require.Len(t, getHostResp.Host.Software, len(software))
	require.Greater(t, getHostResp.Host.SoftwareUpdatedAt, getHostResp.Host.CreatedAt)

	getHostResp = getHostResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/identifier/%s", *host.NodeKey), nil, http.StatusOK, &getHostResp, "exclude_software", "true")
	require.Equal(t, host.ID, getHostResp.Host.ID)
	require.Empty(t, getHostResp.Host.Software)
	require.Greater(t, getHostResp.Host.SoftwareUpdatedAt, getHostResp.Host.CreatedAt)
}
...
func (s *integrationTestSuite) TestHostsReportDownload() {
	...
	hosts := s.createHosts(t)
	err := s.ds.ApplyLabelSpecs(context.Background(), []*mobius.LabelSpec{
		{Name: t.Name(), LabelMembershipType: mobius.LabelMembershipTypeManual, Query: "select 1", Hosts: []string{hosts[2].Hostname}},
	})
	require.NoError(t, err)
	...
	pol, err := s.ds.NewGlobalPolicy(ctx, nil, mobius.PolicyPayload{Name: t.Name(), Query: "SELECT 1"})
	require.NoError(t, err)
	...
	err = s.ds.ReplaceHostDeviceMapping(ctx, hosts[2].ID, []*mobius.HostDeviceMapping{
	...
	res := s.DoRaw("GET", "/api/latest/mobius/hosts/report", nil, http.StatusUnsupportedMediaType, "format", "gzip")
	...
	res = s.DoRaw("GET", "/api/latest/mobius/hosts/report", nil, http.StatusOK, "format", "csv")
	...
	res = s.DoRaw(
		"GET", "/api/latest/mobius/hosts/report", nil, http.StatusOK, "format", "csv",
	...
	res = s.DoRaw("GET", "/api/latest/mobius/hosts/report", nil, http.StatusOK, "format", "csv", "page", "1", "per_page", "2", "columns", "hostname")
	rows, err = csv.NewReader(res.Body).ReadAll()
	res.Body.Close()
	require.NoError(t, err)
	require.Len(t, rows, len(hosts)+1)
	...
	res = s.DoRaw("GET", "/api/latest/mobius/hosts/report", nil, http.StatusOK, "format", "csv", "query", "local0", "columns", "hostname")
	rows, err = csv.NewReader(res.Body).ReadAll()
	res.Body.Close()
	require.NoError(t, err)
	...
	res = s.DoRaw("GET", "/api/latest/mobius/hosts/report", nil, http.StatusOK, "format", "csv", "columns", "id,hostname,device_mapping")
	...
	// with a label id
	res = s.DoRaw("GET", "/api/latest/mobius/hosts/report", nil, http.StatusOK, "format", "csv", "columns", "hostname", "label_id", fmt.Sprintf("%d", customLabelID))
	rows, err = csv.NewReader(res.Body).ReadAll()
	res.Body.Close()
	require.NoError(t, err)
	...
	res = s.DoRaw("GET", "/api/latest/mobius/hosts/report", nil, http.StatusOK, "format", "csv", "columns", "hostname", "label_id", fmt.Sprintf("%d", customLabelID), "query", "  local2 ")
	rows, err = csv.NewReader(res.Body).ReadAll()
	res.Body.Close()
	require.NoError(t, err)
	require.Len(t, rows, 2) // headers + member host
	...
	res = s.DoRaw("GET", "/api/latest/mobius/hosts/report", nil, http.StatusOK, "format", "csv", "columns", "hostname", "software_version_id", fmt.Sprint(fooV1ID))
	rows, err = csv.NewReader(res.Body).ReadAll()
	res.Body.Close()
	require.NoError(t, err)
	require.Len(t, rows, 2) // headers + member host
	...
	res = s.DoRaw("GET", "/api/latest/mobius/hosts/report", nil, http.StatusOK, "format", "csv", "columns", "hostname", "software_title_id", fmt.Sprint(fooTitleID))
	rows, err = csv.NewReader(res.Body).ReadAll()
	res.Body.Close()
	require.NoError(t, err)
	require.Len(t, rows, 2) // headers + member host
	...
	res = s.DoRaw("GET", "/api/latest/mobius/hosts/report", nil, http.StatusBadRequest, "format", "csv", "columns", "memory,hostname,status,nosuchcolumn")
	require.NoError(t, json.NewDecoder(res.Body).Decode(&errs))
	res.Body.Close()
	require.Len(t, errs.Errors, 1)
	...
	res = s.DoRaw("GET", "/api/latest/mobius/hosts/report", nil, http.StatusOK, "format", "csv", "order_key", "hostname", "order_direction", "desc", "columns", "memory,hostname,status")
	rows, err = csv.NewReader(res.Body).ReadAll()
	res.Body.Close()
	require.NoError(t, err)
	require.Len(t, rows, len(hosts)+1)
	...
	s.DoRaw("GET", "/api/latest/mobius/hosts/report", nil, http.StatusBadRequest, "software_title_id", "123", "software_id", "456")
	s.DoRaw("GET", "/api/latest/mobius/hosts/report", nil, http.StatusBadRequest, "software_title_id", "123", "software_version_id", "456")
	s.DoRaw("GET", "/api/latest/mobius/hosts/report", nil, http.StatusBadRequest, "software_id", "123", "software_version_id", "456")
	s.DoRaw("GET", "/api/latest/mobius/hosts/report", nil, http.StatusBadRequest, "software_id", "123", "software_version_id", "456", "software_title_id", "789")
}
...
func (s *integrationTestSuite) TestSSODisabled() {
	...
	s.DoJSON("POST", "/api/v1/mobius/sso", struct{}{}, http.StatusBadRequest, &initiateResp)

	var callbackResp callbackSSOResponse
	// callback without SAML response
	s.DoJSON("POST", "/api/v1/mobius/sso/callback", nil, http.StatusBadRequest, &callbackResp)
	// callback with invalid SAML response
	s.DoJSON("POST", "/api/v1/mobius/sso/callback?SAMLResponse=zz", nil, http.StatusBadRequest, &callbackResp)
	// callback with valid SAML response (<samlp:AuthnRequest></samlp:AuthnRequest>)
	res := s.DoRaw("POST", "/api/v1/mobius/sso/callback?SAMLResponse=PHNhbWxwOkF1dGhuUmVxdWVzdD48L3NhbWxwOkF1dGhuUmVxdWVzdD4%3D", nil, http.StatusOK)
...
func (s *integrationTestSuite) TestGetHostBatteries() {
	...
	require.ElementsMatch(t, []*mobius.HostBattery{
...
func (s *integrationTestSuite) TestGetHostMaintenanceWindow() {
	...
	require.NoError(t, err)
	err = s.ds.ReplaceHostDeviceMapping(ctx, host.ID, []*mobius.HostDeviceMapping{
		{
			HostID: host.ID,
	...
	testEvent := mobius.CalendarEvent{
		Email:     "foo@example.com",
		StartTime: startTime,
		EndTime:   endTime,
		Data:      []byte(`{}`),
	...
	dsEvent, err := s.ds.CreateOrUpdateCalendarEvent(ctx, testEvent.UUID, testEvent.Email, testEvent.StartTime, testEvent.EndTime,
		testEvent.Data, testEvent.TimeZone, host.ID, mobius.CalendarWebhookStatusNone)
	require.NoError(t, err)
	...
	_, err = s.ds.CreateOrUpdateCalendarEvent(ctx, testEvent.UUID, testEvent.Email, testEvent.StartTime, testEvent.EndTime, testEvent.Data,
		&timeZone, host.ID, mobius.CalendarWebhookStatusNone)
	require.NoError(t, err)
...
func (s *integrationTestSuite) TestGetHostDiskEncryption() {
	...
	hostWin, err := s.ds.NewHost(context.Background(), &mobius.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
		SeenTime:        time.Now(),
	...
	hostMac, err := s.ds.NewHost(context.Background(), &mobius.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
		SeenTime:        time.Now(),
	...
	hostLin, err := s.ds.NewHost(context.Background(), &mobius.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
		SeenTime:        time.Now(),
	...
	var getHostResp getHostResponse
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d", hostWin.ID), nil, http.StatusOK, &getHostResp)
	require.Equal(t, hostWin.ID, getHostResp.Host.ID)
	require.Nil(t, getHostResp.Host.DiskEncryptionEnabled)

	getHostResp = getHostResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d", hostMac.ID), nil, http.StatusOK, &getHostResp)
	require.Equal(t, hostMac.ID, getHostResp.Host.ID)
	require.Nil(t, getHostResp.Host.DiskEncryptionEnabled)

	getHostResp = getHostResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d", hostLin.ID), nil, http.StatusOK, &getHostResp)
	...
	s.DoJSON("GET", "/api/latest/mobius/configuration_profiles/summary", getMDMProfilesSummaryRequest{}, http.StatusOK, &profiles)
	s.DoJSON("GET", "/api/latest/mobius/mdm/profiles/summary", getMDMProfilesSummaryRequest{}, http.StatusOK, &profiles)
	...
	res := s.Do("POST", "/api/mobius/orbit/disk_encryption_key", orbitPostDiskEncryptionKeyRequest{
		OrbitNodeKey:  *orbitHost.OrbitNodeKey,
		EncryptionKey: []byte("testkey"),
	}, http.StatusBadRequest)
	errMsg := extractServerErrorText(res.Body)
	require.Contains(t, errMsg, mobius.ErrWindowsMDMNotConfigured.Error())
}
...
func (s *integrationTestSuite) TestListVulnerabilities() {
	...
	s.DoJSON("GET", "/api/latest/mobius/vulnerabilities", nil, http.StatusBadRequest, &resp, "order_key", "cvss_score", "order_direction", "asc")

	// Exploit is an EE only filter
	s.DoJSON("GET", "/api/latest/mobius/vulnerabilities", nil, http.StatusPaymentRequired, &resp, "exploit", "true")
	...
	_, err = s.ds.UpsertSoftwareCPEs(context.Background(), []mobius.SoftwareCPE{
		{
			SoftwareID: sw.ID,
	...
	res2, err := s.ds.UpdateHostSoftware(context.Background(), host2.ID, []mobius.Software{
	...
	s.DoJSON("GET", "/api/latest/mobius/vulnerabilities", nil, http.StatusOK, &resp, "query", "  123	")
	require.Empty(t, resp.Err)
	...
	s.DoJSON("GET", "/api/latest/mobius/vulnerabilities", nil, http.StatusOK, &resp, "query", "CVB")
	require.Empty(t, resp.Err)
	require.Len(s.T(), resp.Vulnerabilities, 0)
	...
	s.DoJSON("GET", "/api/latest/mobius/vulnerabilities", nil, http.StatusOK, &resp, "query", knownCVE)
	require.Empty(t, resp.Err)
	...
	s.DoJSON("GET", "/api/latest/mobius/vulnerabilities", nil, http.StatusOK, &resp, "query", "CVE-2021-1234")
	require.Empty(t, resp.Err)
	assert.Len(s.T(), resp.Vulnerabilities, 1)
	assert.Equal(t, resp.Count, uint(1))
	assert.False(t, resp.Meta.HasPreviousResults)
	assert.False(t, resp.Meta.HasNextResults)
	_ = s.Do("GET", "/api/latest/mobius/vulnerabilities/CVE-2021-1234", nil, http.StatusNotFound)
	...
	// Team 0 Filter
	s.DoJSON("GET", "/api/latest/mobius/vulnerabilities", nil, http.StatusOK, &resp, "team_id", "0")
	...
	// invalid cve
	s.DoJSON("GET", "/api/latest/mobius/vulnerabilities/foobar", nil, http.StatusBadRequest, &gResp)

	// Valid CVE but not in team scope
	s.Do("GET", "/api/latest/mobius/vulnerabilities/CVE-2021-1246", nil, http.StatusNoContent, "team_id",
		fmt.Sprintf("%d", team.ID))

	// Valid CVE in "no team" scope
	s.DoJSON("GET", "/api/latest/mobius/vulnerabilities/CVE-2021-1246", nil, http.StatusOK, &gResp, "team_id", "0")

	// Valid CVE not in "no team" scope
	s.Do("GET", "/api/latest/mobius/vulnerabilities/CVE-2021-12345", nil, http.StatusNoContent, "team_id", "0")

	// Invalid TeamID
	s.Do("GET", "/api/latest/mobius/vulnerabilities/CVE-2021-12345", nil, http.StatusForbidden, "team_id", "100")

	// Valid Global Request
	s.DoJSON("GET", "/api/latest/mobius/vulnerabilities/CVE-2021-12345", nil, http.StatusOK, &gResp)
	...
	s.DoJSON("GET", "/api/latest/mobius/vulnerabilities/CVE-2021-1235", nil, http.StatusOK, &gResp)
	require.Empty(t, gResp.Err)
...
func (s *integrationTestSuite) TestOSVersions() {
	...
	testOSes := []mobius.OperatingSystem{
	...
	osvMap := make(map[string]mobius.OperatingSystem)
	for _, os := range osv {
	...
	resp = listHostsResponse{}
	s.DoJSON("GET", "/api/latest/mobius/hosts", nil, http.StatusOK, &resp, "os_name", testOSes[1].Name, "os_version", testOSes[1].Version)
	require.Len(t, resp.Hosts, 2)

	expected := hosts[1].Hostname
	resp = listHostsResponse{}
	s.DoJSON("GET", "/api/latest/mobius/hosts", nil, http.StatusOK, &resp, "os_version_id", fmt.Sprintf("%d", osvMap["macOS 13.2.1 64bit"].OSVersionID))
	require.Len(t, resp.Hosts, 2)
	require.Equal(t, expected, resp.Hosts[0].Hostname)

	countResp := countHostsResponse{}
	s.DoJSON("GET", "/api/latest/mobius/hosts/count", nil, http.StatusOK, &countResp, "os_version_id", fmt.Sprintf("%d", osvMap["macOS 13.2.1 64bit"].OSVersionID))
	require.Equal(t, 2, countResp.Count)
	...
	assertOSVersion := func(t *testing.T, expected mobius.OSVersion, actual mobius.OSVersion) {
	...
	expectedVersion := mobius.OSVersion{
		HostsCount:  4,
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/os_versions/%d", osvMap["Windows 11 Pro 21H2 10.0.22000.2 ARM64"].OSVersionID), nil, http.StatusOK, &osVersionResp)
	assertOSVersion(t, expectedVersion, *osVersionResp.OSVersion)

	// invalid id
	s.DoJSON("GET", "/api/latest/mobius/os_versions/999", nil, http.StatusOK, &osVersionResp)
	assert.Zero(t, osVersionResp.OSVersion.HostsCount)

	// name and version filters
	s.DoJSON("GET", "/api/latest/mobius/os_versions", nil, http.StatusOK, &osVersionsResp, "os_name", "Windows 11 Pro 21H2", "os_version", "10.0.22000.2")
	...
	s.DoJSON("GET", "/api/latest/mobius/os_versions", nil, http.StatusBadRequest, &osVersionsResp, "os_name", "Windows 11 Pro 21H2")

	// version without name
	s.DoJSON("GET", "/api/latest/mobius/os_versions", nil, http.StatusBadRequest, &osVersionsResp, "os_version", "10.0.22000.1")

	// invalid order key
	s.DoJSON("GET", "/api/latest/mobius/os_versions", nil, http.StatusBadRequest, &osVersionsResp, "order_key", "nosuchkey")

	// ascending order by hosts count
	s.DoJSON("GET", "/api/latest/mobius/os_versions", nil, http.StatusOK, &osVersionsResp, "order_key", "hosts_count", "order_direction", "asc")
	require.Equal(t, 1, osVersionsResp.OSVersions[0].HostsCount)
	require.Equal(t, "macOS 14.1.2", osVersionsResp.OSVersions[0].Name)

	// test pagination
	s.DoJSON("GET", "/api/latest/mobius/os_versions", nil, http.StatusOK, &osVersionsResp, "page", "0", "per_page", "2")
	...
	s.DoJSON("GET", "/api/latest/mobius/os_versions", nil, http.StatusOK, &osVersionsResp, "page", "1", "per_page", "2")
	require.Len(t, osVersionsResp.OSVersions, 2)
	...
	s.DoJSON("GET", "/api/latest/mobius/os_versions", nil, http.StatusOK, &osVersionsResp, "page", "1", "per_page", "2", "team_id", "0")
	require.Len(t, osVersionsResp.OSVersions, 2)
	require.Equal(t, "macOS 13.2.1", osVersionsResp.OSVersions[0].Name)
	require.Equal(t, "macOS 14.1.2", osVersionsResp.OSVersions[1].Name)
	require.Equal(t, 4, osVersionsResp.Count)
...
func (s *integrationTestSuite) TestPingEndpoints() {
	...
	s.DoRaw("HEAD", "/api/mobius/orbit/ping", nil, http.StatusOK)
	// unauthenticated works too
	s.DoRawNoAuth("HEAD", "/api/mobius/orbit/ping", nil, http.StatusOK)

	s.DoRaw("HEAD", "/api/mobius/device/ping", nil, http.StatusOK)
	// unauthenticated works too
	s.DoRawNoAuth("HEAD", "/api/mobius/device/ping", nil, http.StatusOK)

	// device authenticated ping
	createHostAndDeviceToken(t, s.ds, "ping-token")
	s.DoRaw("HEAD", fmt.Sprintf("/api/v1/mobius/device/%s/ping", "ping-token"), nil, http.StatusOK)
	s.DoRawNoAuth("HEAD", fmt.Sprintf("/api/v1/mobius/device/%s/ping", "ping-token"), nil, http.StatusOK)
	s.DoRaw("HEAD", fmt.Sprintf("/api/v1/mobius/device/%s/ping", "bozo-token"), nil, http.StatusUnauthorized)
	s.DoRawNoAuth("HEAD", fmt.Sprintf("/api/v1/mobius/device/%s/ping", "bozo-token"), nil, http.StatusUnauthorized)
}
...
func (s *integrationTestSuite) TestMDMNotConfiguredEndpoints() {
	...
	for _, route := range mdmConfigurationRequiredEndpoints() {
		...
		if route.premiumOnly && route.deviceAuthenticated {
			...
			expectedErr = mobius.ErrMissingLicense
		}
...
func (s *integrationTestSuite) TestOrbitConfigNotifications() {
	...
	// missing orbit key
	s.DoJSON("POST", "/api/mobius/orbit/config", nil, http.StatusUnauthorized, &resp)

	hNoMDM := createOrbitEnrolledHost(t, "darwin", "nomdm", s.ds)
	resp = orbitGetConfigResponse{}
	s.DoJSON("POST", "/api/mobius/orbit/config", json.RawMessage(fmt.Sprintf(`{"orbit_node_key": %q}`, *hNoMDM.OrbitNodeKey)), http.StatusOK, &resp)
	require.False(t, resp.Notifications.RenewEnrollmentProfile)

	hSimpleMDM := createOrbitEnrolledHost(t, "darwin", "simplemdm", s.ds)
	err = s.ds.SetOrUpdateMDMData(context.Background(), hSimpleMDM.ID, false, true, "https://simplemdm.com", false, mobius.WellKnownMDMSimpleMDM, "")
	require.NoError(t, err)
	resp = orbitGetConfigResponse{}
	s.DoJSON("POST", "/api/mobius/orbit/config", json.RawMessage(fmt.Sprintf(`{"orbit_node_key": %q}`, *hSimpleMDM.OrbitNodeKey)), http.StatusOK, &resp)
	require.False(t, resp.Notifications.RenewEnrollmentProfile)

	// not yet assigned in ABM
	hMobiusMDM := createOrbitEnrolledHost(t, "darwin", "mobiusmdm", s.ds)
	err = s.ds.SetOrUpdateMDMData(context.Background(), hMobiusMDM.ID, false, false, "https://mobiusmdm.com", true, mobius.WellKnownMDMMobius, "")
	require.NoError(t, err)

	resp = orbitGetConfigResponse{}
	s.DoJSON("POST", "/api/mobius/orbit/config", json.RawMessage(fmt.Sprintf(`{"orbit_node_key": %q}`, *hMobiusMDM.OrbitNodeKey)), http.StatusOK, &resp)
	require.False(t, resp.Notifications.RenewEnrollmentProfile)

	// simulate ABM assignment
	encTok := uuid.NewString()
	abmToken, err := s.ds.InsertABMToken(ctx, &mobius.ABMToken{OrganizationName: "unused", EncryptedToken: []byte(encTok)})
	require.NoError(t, err)
	require.NotEmpty(t, abmToken.ID)
	err = s.ds.UpsertMDMAppleHostDEPAssignments(ctx, []mobius.Host{*hMobiusMDM}, abmToken.ID)
	require.NoError(t, err)
	err = s.ds.SetOrUpdateMDMData(context.Background(), hSimpleMDM.ID, false, true, "https://simplemdm.com", false, mobius.WellKnownMDMSimpleMDM, "")
	require.NoError(t, err)
	resp = orbitGetConfigResponse{}
	...
	// if the mobius mdm host is fully enrolled (not pending anymore), then the notification is false
	err = s.ds.SetOrUpdateMDMData(context.Background(), hMobiusMDM.ID, false, true, "https://mobiusmdm.com", true, mobius.WellKnownMDMMobius, "")
	require.NoError(t, err)
	resp = orbitGetConfigResponse{}
	s.DoJSON("POST", "/api/mobius/orbit/config", json.RawMessage(fmt.Sprintf(`{"orbit_node_key": %q}`, *hMobiusMDM.OrbitNodeKey)), http.StatusOK, &resp)
	require.False(t, resp.Notifications.RenewEnrollmentProfile)
	...
	s.Do("POST", "/api/mobius/orbit/scripts/request", json.RawMessage(fmt.Sprintf(`{"orbit_node_key": %q}`, *hMobiusMDM.OrbitNodeKey)), http.StatusNotFound)
	s.Do("POST", "/api/mobius/orbit/scripts/result", json.RawMessage(fmt.Sprintf(`{"orbit_node_key": %q}`, *hMobiusMDM.OrbitNodeKey)), http.StatusBadRequest)
}
...
func (s *integrationTestSuite) TestTryingToEnrollWithTheWrongSecret() {
	...
	s.DoJSON("POST", "/api/mobius/orbit/enroll", EnrollOrbitRequest{
...
func (s *integrationTestSuite) TestEnrollOrbitExistingHostNoSerialMatch() {
	...
	s.DoJSON("POST", "/api/latest/mobius/spec/enroll_secret", applyEnrollSecretSpecRequest{
		Spec: &mobius.EnrollSecretSpec{
			Secrets: []*mobius.EnrollSecret{{Secret: secret}},
		},
	}, http.StatusOK, &applyResp)
	...
	// host identifier.
	// See https://github.com/notawar/mobius/issues/9033#issuecomment-1411150758
...
func (s *integrationTestSuite) TestAPIVersion_v1_2022_04() {
	...
	// in that version
	gsParams := mobius.ScheduledQueryPayload{QueryID: ptr.Uint(qr.ID), Interval: ptr.Uint(42)}
	res := s.DoRaw("POST", "/api/2022-04/mobius/global/schedule", jsonMustMarshal(t, gsParams), http.StatusNotFound)
	res.Body.Close()
	// use the correct version for that deprecated API
	createResp := globalScheduleQueryResponse{}
	s.DoJSON("POST", "/api/v1/mobius/global/schedule", gsParams, http.StatusOK, &createResp)
	require.NotZero(t, createResp.Scheduled.ID)

	// list the scheduled queries with the new endpoint, but the old version
	res = s.DoRaw("GET", "/api/v1/mobius/schedule", nil, http.StatusMethodNotAllowed)
	res.Body.Close()

	// list again, this time with the correct version
	gs := mobius.GlobalSchedulePayload{}
	s.DoJSON("GET", "/api/2022-04/mobius/schedule", nil, http.StatusOK, &gs)
	require.Len(t, gs.GlobalSchedule, 1)

	// delete using the old endpoint but on the wrong new version
	res = s.DoRaw("DELETE", fmt.Sprintf("/api/2022-04/mobius/global/schedule/%d", createResp.Scheduled.ID), nil, http.StatusNotFound)
	res.Body.Close()
	// properly delete with old endpoint and old version
	var delResp deleteGlobalScheduleResponse
	s.DoJSON("DELETE", fmt.Sprintf("/api/v1/mobius/global/schedule/%d", createResp.Scheduled.ID), nil, http.StatusOK, &delResp)
}
...
func createOrbitEnrolledHost(t *testing.T, os, suffix string, ds mobius.Datastore) *mobius.Host {
	name := t.Name() + suffix
...
func createSession(t *testing.T, uid uint, ds mobius.Datastore) *mobius.Session {
...
func (s *integrationTestSuite) TestDirectIngestScheduledQueryStats() {
	...
	require.NoError(t, err)
	globalHost, err := s.ds.NewHost(context.Background(), &mobius.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
		SeenTime:        time.Now().Add(-1 * time.Minute),
	...
	require.NoError(t, err)
	team1Host, err := s.ds.NewHost(context.Background(), &mobius.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
		SeenTime:        time.Now().Add(-1 * time.Minute),
	...
	require.NoError(t, err)
	scheduledGlobalQuery, err := s.ds.NewQuery(context.Background(), &mobius.Query{
	...
	require.NoError(t, err)
	nonScheduledGlobalQuery, err := s.ds.NewQuery(context.Background(), &mobius.Query{
	...
	require.NoError(t, err)
	scheduledTeam1Query1, err := s.ds.NewQuery(context.Background(), &mobius.Query{
	...
	require.NoError(t, err)
	scheduledTeam1Query2, err := s.ds.NewQuery(context.Background(), &mobius.Query{
	...
	_, err = s.ds.NewQuery(context.Background(), &mobius.Query{
	...
	userPack1TargetTeam1, err := s.ds.NewPack(context.Background(), &mobius.Pack{
		Name:    "2017 Pack",
		Type:    nil,
		Teams:   []mobius.Target{{TargetID: team1.ID, Type: mobius.TargetTeam}},
		TeamIDs: []uint{team1.ID},
	})
	require.NoError(t, err)
	scheduledQueryOnPack1, err := s.ds.NewScheduledQuery(context.Background(), &mobius.ScheduledQuery{
	...
	require.NoError(t, err)
	detailQueries := osquery_utils.GetDetailQueries(context.Background(), config.MobiusConfig{
	...
	var scheduledQueriesStats []mobius.ScheduledQueryStats
	mysql.ExecAdhocSQL(t, s.ds, func(q sqlx.ExtContext) error {
	...
	// Check that the received stats were stored in the DB as expected.
	scheduledQueriesStats = []mobius.ScheduledQueryStats{}
	mysql.ExecAdhocSQL(t, s.ds, func(q sqlx.ExtContext) error {
		return sqlx.SelectContext(context.Background(), q, &scheduledQueriesStats,
			`SELECT
				scheduled_query_id, q.name AS scheduled_query_name, average_memory, denylisted,
...
func (s *integrationTestSuite) TestDirectIngestSoftwareWithLongFields() {
	...
	detailQueries := osquery_utils.GetDetailQueries(context.Background(), config.MobiusConfig{}, appConfig, &appConfig.Features, osquery_utils.Integrations{})
	...
	var wiresharkSoftware mobius.Software
	mysql.ExecAdhocSQL(t, s.ds, func(q sqlx.ExtContext) error {
	...
	require.NoError(t, err)
	var wiresharkSoftware2 mobius.Software
	mysql.ExecAdhocSQL(t, s.ds, func(q sqlx.ExtContext) error {
	...
	rows = []map[string]string{
		{
			"name":           "Foobar" + strings.Repeat("A", mobius.SoftwareNameMaxLength),
			"version":        "4.0.8" + strings.Repeat("B", mobius.SoftwareVersionMaxLength),
			"type":           "Program (Windows)",
			"source":         "programs" + strings.Repeat("C", mobius.SoftwareSourceMaxLength),
			"vendor":         strings.Repeat("D", mobius.SoftwareVendorMaxLength+1),
			"installed_path": "C:\\Program Files\\Foobar",
			// Test UTF-8 encoded strings.
			"bundle_identifier": strings.Repeat("⌘", mobius.SoftwareBundleIdentifierMaxLength+1),
			"release":           strings.Repeat("F", mobius.SoftwareReleaseMaxLength-1) + "⌘⌘",
			"arch":              strings.Repeat("G", mobius.SoftwareArchMaxLength+1),
		},
	}
	...
	var foobarSoftware mobius.Software
	mysql.ExecAdhocSQL(t, s.ds, func(q sqlx.ExtContext) error {
		return sqlx.GetContext(context.Background(), q, &foobarSoftware, softwareQueryByName, "Foobar"+strings.Repeat("A", mobius.SoftwareNameMaxLength-6))
	})
	require.NotZero(t, foobarSoftware.ID)
	require.Equal(t, "Foobar"+strings.Repeat("A", mobius.SoftwareNameMaxLength-6), foobarSoftware.Name)
	require.Equal(t, "4.0.8"+strings.Repeat("B", mobius.SoftwareNameMaxLength-5), foobarSoftware.Version)
	require.Equal(t, "programs"+strings.Repeat("C", mobius.SoftwareSourceMaxLength-8), foobarSoftware.Source)
	// Vendor field is currenty trimmed with a different method (... appended at the end)
	require.Equal(t, strings.Repeat("D", mobius.SoftwareVendorMaxLength-3)+"...", foobarSoftware.Vendor)
	require.Equal(t, strings.Repeat("⌘", mobius.SoftwareBundleIdentifierMaxLength), foobarSoftware.BundleIdentifier)
	require.Equal(t, strings.Repeat("F", mobius.SoftwareReleaseMaxLength-1)+"⌘", foobarSoftware.Release)
	require.Equal(t, strings.Repeat("G", mobius.SoftwareArchMaxLength), foobarSoftware.Arch)
	...
	var foobarSoftware2 mobius.Software
	mysql.ExecAdhocSQL(t, s.ds, func(q sqlx.ExtContext) error {
		return sqlx.GetContext(context.Background(), q, &foobarSoftware2, softwareQueryByName, "Foobar"+strings.Repeat("A", mobius.SoftwareNameMaxLength-6))
	})
...
func (s *integrationTestSuite) TestHostsReportWithPolicyResults() {
	...
	hostCount := 10
	hosts := make([]*mobius.Host, 0, hostCount)
	for i := 0; i < hostCount; i++ {
		hosts = append(hosts, newHostFunc(fmt.Sprintf("h%d", i)))
	}

	globalPolicy0, err := s.ds.NewGlobalPolicy(ctx, &test.UserAdmin.ID, mobius.PolicyPayload{
		Name:  "foobar0",
		Query: "SELECT 0;",
	})
	require.NoError(t, err)
	globalPolicy1, err := s.ds.NewGlobalPolicy(ctx, &test.UserAdmin.ID, mobius.PolicyPayload{
		Name:  "foobar1",
		Query: "SELECT 1;",
	})
	require.NoError(t, err)
	globalPolicy2, err := s.ds.NewGlobalPolicy(ctx, &test.UserAdmin.ID, mobius.PolicyPayload{
		Name:  "foobar2",
		Query: "SELECT 2;",
	})
	require.NoError(t, err)
	...
	res = s.DoRaw(
		"GET", "/api/latest/mobius/hosts/report", nil, http.StatusOK, "format", "csv", "disable_failing_policies", "false", "disable_issues",
		"true",
	)
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			res = s.DoRaw("GET", "/api/latest/mobius/hosts/report", nil, http.StatusOK, append(tc.args, "format", "csv")...)
			rows, err := csv.NewReader(res.Body).ReadAll()
			res.Body.Close()
			require.NoError(t, err)
			tc.checkRows(t, rows)
			...
			res = s.DoRaw(
				"GET", "/api/latest/mobius/hosts/report", nil, http.StatusOK, append(tc.args, "format", "csv", "disable_issues", "true")...,
			)
			rows, err = csv.NewReader(res.Body).ReadAll()
			res.Body.Close()
			require.NoError(t, err)
...
func (s *integrationTestSuite) TestQueryReports() {
	...
	host1Global, err := s.ds.NewHost(ctx, &mobius.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
		SeenTime:        time.Now(),
	...
	host2Global, err := s.ds.NewHost(ctx, &mobius.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
		SeenTime:        time.Now(),
	...
	host2Team1, err := s.ds.NewHost(ctx, &mobius.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
		SeenTime:        time.Now(),
	...
	host1Team2, err := s.ds.NewHost(ctx, &mobius.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
		SeenTime:        time.Now(),
	...
	osqueryInfoQuery, err := s.ds.NewQuery(ctx, &mobius.Query{
		Name:               "Osquery info",
	...
	usbDevicesQuery, err := s.ds.NewQuery(ctx, &mobius.Query{
		Name:               "USB devices",
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/queries/%d/report", usbDevicesQuery.ID), getQueryReportRequest{}, http.StatusOK, &gqrr)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/queries/%d", host1Global.ID, usbDevicesQuery.ID), getHostQueryReportRequest{}, http.StatusOK, &ghqrr)
  ...
  "decorations": {
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/queries/%d", host2Team1.ID, usbDevicesQuery.ID), getHostQueryReportRequest{}, http.StatusOK, &ghqrr)
	require.NoError(t, ghqrr.Err)
	require.Equal(t, usbDevicesQuery.ID, ghqrr.QueryID)
	...
	gqrr = getQueryReportResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/queries/%d/report", osqueryInfoQuery.ID), getQueryReportRequest{}, http.StatusOK, &gqrr)
	require.NoError(t, gqrr.Err)
	...
	gqrr = getQueryReportResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/queries/%d/report?team_id=%d", osqueryInfoQuery.ID, team2.ID), getQueryReportRequest{}, http.StatusOK, &gqrr)
	require.NoError(t, gqrr.Err)
	require.Equal(t, osqueryInfoQuery.ID, gqrr.QueryID)
	require.Len(t, gqrr.Results, 1)

	ghqrr = getHostQueryReportResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/queries/%d", host1Global.ID, osqueryInfoQuery.ID), getHostQueryReportRequest{}, http.StatusOK, &ghqrr)
	require.NoError(t, ghqrr.Err)
	...
	ghqrr = getHostQueryReportResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/queries/%d", host2Global.ID, osqueryInfoQuery.ID), getHostQueryReportRequest{}, http.StatusOK, &ghqrr)
	require.NoError(t, ghqrr.Err)
	require.Equal(t, osqueryInfoQuery.ID, ghqrr.QueryID)
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/queries/%d", osqueryInfoQuery.ID), modifyQueryRequest{ID: osqueryInfoQuery.ID, QueryPayload: mobius.QueryPayload{Description: &updatedDesc}}, http.StatusOK, &modifyQueryResp)
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/queries/%d", osqueryInfoQuery.ID), modifyQueryRequest{ID: osqueryInfoQuery.ID, QueryPayload: mobius.QueryPayload{Query: &updatedQuery}}, http.StatusOK, &modifyQueryResp)
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/queries/%d", osqueryInfoQuery.ID), modifyQueryRequest{
	...
	osqueryInfoQuerySpec := &mobius.QuerySpec{
	...
	var applyResp applyQuerySpecsResponse
	s.DoJSON("POST", "/api/latest/mobius/spec/queries", applyQuerySpecsRequest{
		Specs: []*mobius.QuerySpec{osqueryInfoQuerySpec},
	}, http.StatusOK, &applyResp)
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/queries/%d/report", osqueryInfoQuery.ID), getQueryReportRequest{}, http.StatusOK, &gqrr)
	require.Len(t, gqrr.Results, 0)
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/queries/%d", usbDevicesQuery.ID), modifyQueryRequest{ID: usbDevicesQuery.ID, QueryPayload: mobius.QueryPayload{Logging: &mobius.LoggingDifferential}}, http.StatusOK, &modifyQueryResp)
	require.Equal(t, mobius.LoggingDifferential, modifyQueryResp.Query.Logging)
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/queries/%d/report", usbDevicesQuery.ID), getQueryReportRequest{}, http.StatusOK, &gqrr)
	require.Len(t, gqrr.Results, 0)
	require.False(t, gqrr.ReportClipped)
	...
	discardData := true
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/queries/%d", osqueryInfoQuery.ID), modifyQueryRequest{ID: osqueryInfoQuery.ID, QueryPayload: mobius.QueryPayload{DiscardData: &discardData}}, http.StatusOK, &modifyQueryResp)
	...
	slreq = submitLogsRequest{
		...
		Data: []json.RawMessage{
			json.RawMessage(`{
  "snapshot": [` + results(mobius.DefaultMaxQueryReportRows, host1Global.UUID) + `
  ],
  "action": "snapshot",
  "name": "pack/Global/` + osqueryInfoQuery.Name + `",
  "hostIdentifier": "` + *host1Global.OsqueryHostID + `",
  ...
  "decorations": {
	...
	require.NoError(t, slres.Err)
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/queries/%d/report", osqueryInfoQuery.ID), getQueryReportRequest{}, http.StatusOK, &gqrr)
	require.Len(t, gqrr.Results, mobius.DefaultMaxQueryReportRows)
	require.True(t, gqrr.ReportClipped)

	ghqrr = getHostQueryReportResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/queries/%d", host1Global.ID, osqueryInfoQuery.ID), getHostQueryReportRequest{}, http.StatusOK, &ghqrr)
	require.NoError(t, ghqrr.Err)
	require.Len(t, ghqrr.Results, mobius.DefaultMaxQueryReportRows)
  ...
  "decorations": {
	...
	appConfigSpec := map[string]map[string]int{
		"server_settings": {"query_report_cap": mobius.DefaultMaxQueryReportRows + 1},
	}
	s.Do("PATCH", "/api/latest/mobius/config", appConfigSpec, http.StatusOK)
  ...
  "decorations": {
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/queries/%d/report", osqueryInfoQuery.ID), getQueryReportRequest{}, http.StatusOK, &gqrr)
	require.Len(t, gqrr.Results, mobius.DefaultMaxQueryReportRows+1)
	require.True(t, gqrr.ReportClipped)
...
func (s *integrationTestSuite) TestHostHealth() {
	...
	passingPolicy, err := s.ds.NewGlobalPolicy(context.Background(), nil, mobius.PolicyPayload{
		Name:       "passing_policy",
		Query:      "select 1",
	...
	failingPolicy, err := s.ds.NewGlobalPolicy(context.Background(), nil, mobius.PolicyPayload{
		Name:       "failing_policy",
		Query:      "select 0",
		Resolution: "Run this command to fix it",
	})
	...
	assert.Equal(t, hh.HostHealth.VulnerableSoftware[0], mobius.HostHealthVulnerableSoftware{
		ID:      soft1.ID,
		Name:    soft1.Name,
	...
	assert.Equal(t, hh.HostHealth.FailingPolicies[0], &mobius.HostHealthFailingPolicy{
		ID:         failingPolicy.ID,
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/health", 0), nil, http.StatusNotFound, &hh)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/health", host1.ID), nil, http.StatusOK, &resp)
...
func (s *integrationTestSuite) TestHostDeviceToken() {
	...
	s.DoJSON("POST", "/api/mobius/orbit/device_token", body, http.StatusBadRequest, &response{})
	...
	s.DoJSON("POST", "/api/mobius/orbit/device_token", body, http.StatusUnauthorized, &response{})
	...
	s.DoJSON("POST", "/api/mobius/orbit/device_token", body, http.StatusOK, &response{})
	...
	s.DoJSON("POST", "/api/mobius/orbit/device_token", body, http.StatusConflict, &response{})
}
...
func (s *integrationTestSuite) TestHostPastActivities() {
	...
	getDetails := func(a *mobius.Activity) mobius.ActivityTypeRanScript {
		var details mobius.ActivityTypeRanScript
	...
	savedScript, err := s.ds.NewScript(ctx, &mobius.Script{
		TeamID:         nil,
		Name:           "saved.sh",
	...
	var runResp runScriptResponse
	s.DoJSON("POST", "/api/latest/mobius/scripts/run", mobius.HostScriptRequestPayload{HostID: host.ID, ScriptID: &savedScript.ID}, http.StatusAccepted, &runResp)
	...
	var listResp listActivitiesResponse
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/activities", host.ID), nil, http.StatusOK, &listResp)
	...
	s.DoJSON("POST", "/api/latest/mobius/scripts/run", mobius.HostScriptRequestPayload{HostID: host.ID, ScriptContents: "echo 'foobar'"}, http.StatusAccepted, &runResp)
	require.Equal(t, host.ID, runResp.HostID)
	require.NotEmpty(t, runResp.ExecutionID)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/activities", host.ID), nil, http.StatusOK, &listResp, "page", "0", "per_page", "1")
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/activities", host.ID), nil, http.StatusOK, &listResp, "page", "1", "per_page", "1")
...
func (s *integrationTestSuite) TestListHostUpcomingActivities() {
	...
	hsr, err := s.ds.NewHostScriptExecutionRequest(ctx, &mobius.HostScriptRequestPayload{HostID: host1.ID, ScriptContents: "A", SyncRequest: true})
	require.NoError(t, err)
	h1A := hsr.ExecutionID
	hsr, err = s.ds.NewHostScriptExecutionRequest(ctx, &mobius.HostScriptRequestPayload{HostID: host1.ID, ScriptContents: "B"})
	require.NoError(t, err)
	h1B := hsr.ExecutionID
	hsr, err = s.ds.NewHostScriptExecutionRequest(ctx, &mobius.HostScriptRequestPayload{HostID: host1.ID, ScriptContents: "C"})
	require.NoError(t, err)
	h1C := hsr.ExecutionID
	hsr, err = s.ds.NewHostScriptExecutionRequest(ctx, &mobius.HostScriptRequestPayload{HostID: host1.ID, ScriptContents: "D", SyncRequest: true})
	require.NoError(t, err)
	h1D := hsr.ExecutionID
	hsr, err = s.ds.NewHostScriptExecutionRequest(ctx, &mobius.HostScriptRequestPayload{HostID: host1.ID, ScriptContents: "E"})
	require.NoError(t, err)
	h1E := hsr.ExecutionID

	// create a software installation request
	tfr1, err := mobius.NewTempFileReader(strings.NewReader("echo"), t.TempDir)
	require.NoError(t, err)
	sw1, _, err := s.ds.MatchOrCreateSoftwareInstaller(ctx, &mobius.UploadSoftwareInstallerPayload{
	...
	require.NoError(t, err)
	h1Foo, err := s.ds.InsertSoftwareInstallRequest(ctx, host1.ID, s1Meta.InstallerID, mobius.HostSoftwareInstallOptions{})
	require.NoError(t, err)
	...
	for _, c := range cases {
		t.Run(fmt.Sprintf("%#v", c.queries), func(t *testing.T) {
			...
			queryArgs := c.queries
			s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/activities/upcoming", host1.ID), nil, http.StatusOK, &listResp, queryArgs...)
			...
			if len(listResp.Activities) > 0 {
				...
				for i, a := range listResp.Activities {
					...
					require.Contains(t, []string{
						...
						mobius.ActivityTypeInstalledSoftware{}.ActivityName(),
					}, a.Type)
					...
					switch a.Type {
					case mobius.ActivityTypeRanScript{}.ActivityName():
						gotExecs[i] = details["script_execution_id"].(string)
					case mobius.ActivityTypeInstalledSoftware{}.ActivityName():
	...
	var listResp listHostUpcomingActivitiesResponse
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/activities/upcoming", host2.ID), nil, http.StatusOK, &listResp)
	require.Equal(t, uint(0), listResp.Count)
	require.Empty(t, listResp.Activities)
	require.Equal(t, &mobius.PaginationMetadata{HasNextResults: false, HasPreviousResults: false}, listResp.Meta)
}
...
func (s *integrationTestSuite) TestAddingRemovingManualLabels() {
	...
	newGlobalUserFunc := func(email string, globalRole string) *mobius.User {
		user := &mobius.User{
			Name:       email,
			Email:      email,
	...
	newTeamUserFunc := func(email string, team *mobius.Team, teamRole string) *mobius.User {
		user := &mobius.User{
			Name:  email,
			Email: email,
			Teams: []mobius.UserTeam{
	...
	globalObserver := newGlobalUserFunc("global.observer@example.com", mobius.RoleObserver)
	teamAdmin := newTeamUserFunc("team.admin@example.com", team1, mobius.RoleAdmin)
	teamObserver := newTeamUserFunc("team.observer@example.com", team1, mobius.RoleObserver)

	newHostFunc := func(name string, teamID *uint) *mobius.Host {
		host, err := s.ds.NewHost(ctx, &mobius.Host{
			NodeKey:  ptr.String(name),
			UUID:     name,
			Hostname: "foo.local." + name,
	...
	dynamicLabel1, err := s.ds.NewLabel(ctx, &mobius.Label{
		Name:                "dynamicLabel1",
		Query:               "SELECT 1;",
		LabelMembershipType: mobius.LabelMembershipTypeDynamic,
	})
	...
	require.NoError(t, err)
	manualLabel2, err := s.ds.NewLabel(ctx, &mobius.Label{
		Name:                "manualLabel2",
		Query:               "SELECT 3;",
		LabelMembershipType: mobius.LabelMembershipTypeManual,
	})
	...
	getHostLabels := func(host *mobius.Host) []string {
		var hostResp getHostResponse
		s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d", host.ID), nil, http.StatusOK, &hostResp)
		var labels []string
	...
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobius/hosts/%d/labels", host1.ID),
		json.RawMessage(`{}`), http.StatusOK, &addLabelsToHostResp,
	)
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobius/hosts/%d/labels", host1.ID), addLabelsToHostRequest{
		Labels: []string{},
	}, http.StatusOK, &addLabelsToHostResp)
	var removeLabelsFromHostResp removeLabelsFromHostResponse
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobius/hosts/%d/labels", host1.ID), removeLabelsFromHostRequest{
		Labels: []string{},
	...
	res := s.Do("POST", fmt.Sprintf("/api/latest/mobius/hosts/%d/labels", host1.ID), addLabelsToHostRequest{
	...
	res = s.Do("DELETE", fmt.Sprintf("/api/latest/mobius/hosts/%d/labels", host1.ID), removeLabelsFromHostRequest{
		Labels: []string{"All Hosts"},
	}, http.StatusBadRequest)
	errMsg = extractServerErrorText(res.Body)
	...
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobius/hosts/%d/labels", 999), addLabelsToHostRequest{
		Labels: []string{manualLabel1.Name, manualLabel2.Name},
	}, http.StatusNotFound, &addLabelsToHostResp)
	// Delete from non-existent host
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobius/hosts/%d/labels", 999), removeLabelsFromHostRequest{
		Labels: []string{manualLabel1.Name, manualLabel2.Name},
	}, http.StatusNotFound, &removeLabelsFromHostResp)

	// Add labels to team host.
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobius/hosts/%d/labels", teamHost2.ID), addLabelsToHostRequest{
	...
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobius/hosts/%d/labels", teamHost2.ID), removeLabelsFromHostRequest{
		Labels: []string{manualLabel1.Name},
...
func (s *integrationTestSuite) TestAutofillPolicies() {
	...
	// empty sql
	resp := s.Do("POST", "/api/latest/mobius/autofill/policy", req, http.StatusBadRequest)
	...
	resp = s.Do("POST", "/api/latest/mobius/autofill/policy", req, http.StatusUnprocessableEntity)
...
func (s *integrationTestSuite) TestHostWithNoPoliciesClearsPolicyCounts() {
	...
	team, err := s.ds.NewTeam(ctx, &mobius.Team{Name: "Zoobar"})
	require.NoError(t, err)
	...
	policy, err := s.ds.NewTeamPolicy(ctx, team.ID, nil, mobius.PolicyPayload{
		Name:  "Barfoo",
		Query: "SELECT 1;",
	})
	require.NoError(t, err)
...
func (s *integrationTestSuite) TestHostSoftwareWithTeamIdentifier() {
	...
	safariApp := mobius.Software{
		Name:             "Safari.app",
		BundleIdentifier: "com.apple.safari",
		Version:          "18.1",
		Source:           "apps",
	}
	googleChromeApp := mobius.Software{
		Name:             "Google Chrome.app",
		BundleIdentifier: "com.google.Chrome",
		Version:          "130.0.6723.117",
		Source:           "apps",
	}
	ghCli := mobius.Software{
		Name:   "gh",
	...
	for _, s := range software {
		...
		for _, pathItem := range pathItems {
			...
			key := fmt.Sprintf(
				"%s%s%s%s%s%s%s",
				path, mobius.SoftwareFieldSeparator, teamIdentifier, mobius.SoftwareFieldSeparator, cdHash, mobius.SoftwareFieldSeparator, s.ToUniqueStr(),
			)
			swPaths[key] = struct{}{}
		}
	}
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/software", host.ID),
...
func (s *integrationTestSuite) TestSecretVariables() {
	...
	s.DoJSON("PUT", "/api/latest/mobius/spec/secret_variables", req, http.StatusOK, &resp)
	...
	httpResp := s.Do("PUT", "/api/latest/mobius/spec/secret_variables", req, http.StatusUnprocessableEntity)
...
func (s *integrationTestSuite) TestListAndroidHostsInLabel() {
	...
	for _, lbl := range listResp.Labels {
		switch lbl.Name {
		case mobius.BuiltinLabelNameAllHosts:
			allLblID = lbl.ID
		case mobius.BuiltinLabelNameAndroid:
			androidLblID = lbl.ID
		}
	}
	...
	pluckHostIDs := func(hosts []mobius.HostResponse) []uint {
	...
	var listHostsResp listHostsResponse
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/labels/%d/hosts", allLblID), nil, http.StatusOK, &listHostsResp)
	...
	// count hosts in label
	var countResp countHostsResponse
	s.DoJSON("GET", "/api/latest/mobius/hosts/count", nil, http.StatusOK, &countResp, "label_id", fmt.Sprint(allLblID))
	require.Equal(t, len(hostIDs)+1, countResp.Count)

	// list android hosts
	listHostsResp = listHostsResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/labels/%d/hosts", androidLblID), nil, http.StatusOK, &listHostsResp)
	require.Len(t, listHostsResp.Hosts, len(hostIDs))
	require.ElementsMatch(t, hostIDs, pluckHostIDs(listHostsResp.Hosts))

	countResp = countHostsResponse{}
	s.DoJSON("GET", "/api/latest/mobius/hosts/count", nil, http.StatusOK, &countResp, "label_id", fmt.Sprint(androidLblID))
...
func (s *integrationTestSuite) TestHostCertificates() {
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/certificates", host.ID), nil, http.StatusOK, &certResp)
	require.Empty(t, certResp.Certificates)

	certResp = listHostCertificatesResponse{}
	res := s.DoRawNoAuth("GET", "/api/latest/mobius/device/"+token+"/certificates", nil, http.StatusOK)
	...
	certs := make([]*mobius.HostCertificateRecord, 0, len(certNames))
	for i, name := range certNames {
		sha1Sum := sha1.Sum([]byte(name)) // nolint:gosec
		certs = append(certs, &mobius.HostCertificateRecord{
			HostID:         host.ID,
			CommonName:     name,
			SHA1Sum:        sha1Sum[:],
			...
			Source:         mobius.SystemHostCertificate,
		})
	}
	...
	for i, cert := range certResp.Certificates {
		...
		require.Equal(t, mobius.SystemHostCertificate, cert.Source)
	}
	...
	// non-existing host
	certResp = listHostCertificatesResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/certificates", host.ID+1000), nil, http.StatusNotFound, &certResp)
	// for the device endpoint, the token is the authentication so if it doesn't
	// exist, the endpoint is unauthorized.
	certResp = listHostCertificatesResponse{}
	s.DoRawNoAuth("GET", "/api/latest/mobius/device/NO-SUCH-TOKEN/certificates", nil, http.StatusUnauthorized)

	pluckCertNames := func(certs []*mobius.HostCertificatePayload) []string {
	...
	certResp = listHostCertificatesResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/certificates", host.ID), nil, http.StatusBadRequest, &certResp, "order_key", "no-such-column")

	certResp = listHostCertificatesResponse{}
	res = s.DoRawNoAuth("GET", "/api/latest/mobius/device/"+token+"/certificates", nil, http.StatusBadRequest, "order_key", "no-such-column")
	...
	cases := []struct {
		...
		{queryParams: []string{"page", "0", "per_page", "2"}, wantNames: []string{"a", "b"}, wantMeta: mobius.PaginationMetadata{HasNextResults: true}},
		{queryParams: []string{"page", "1", "per_page", "2"}, wantNames: []string{"c", "d"}, wantMeta: mobius.PaginationMetadata{HasNextResults: true, HasPreviousResults: true}},
		{queryParams: []string{"page", "2", "per_page", "2"}, wantNames: []string{"e"}, wantMeta: mobius.PaginationMetadata{HasNextResults: false, HasPreviousResults: true}},
		{queryParams: []string{"page", "3", "per_page", "2"}, wantNames: []string{}, wantMeta: mobius.PaginationMetadata{HasNextResults: false, HasPreviousResults: true}},
		{queryParams: []string{"page", "0", "per_page", "4", "order_direction", "desc"}, wantNames: []string{"e", "d", "c", "b"}, wantMeta: mobius.PaginationMetadata{HasNextResults: true}},
		{queryParams: []string{"page", "1", "per_page", "4", "order_direction", "desc"}, wantNames: []string{"a"}, wantMeta: mobius.PaginationMetadata{HasNextResults: false, HasPreviousResults: true}},
		{queryParams: []string{"page", "0", "per_page", "3", "order_key", "not_valid_after"}, wantNames: []string{"d", "c", "e"}, wantMeta: mobius.PaginationMetadata{HasNextResults: true}},
		{queryParams: []string{"page", "1", "per_page", "3", "order_key", "not_valid_after"}, wantNames: []string{"a", "b"}, wantMeta: mobius.PaginationMetadata{HasNextResults: false, HasPreviousResults: true}},
	}
	for _, c := range cases {
		t.Run(strings.Join(c.queryParams, "_"), func(t *testing.T) {
			certResp = listHostCertificatesResponse{}
			s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/certificates", host.ID), nil, http.StatusOK, &certResp, c.queryParams...)
			...
			certResp = listHostCertificatesResponse{}
			res = s.DoRawNoAuth("GET", "/api/latest/mobius/device/"+token+"/certificates", nil, http.StatusOK, c.queryParams...)
			err = json.NewDecoder(res.Body).Decode(&certResp)
			require.NoError(t, err)
			require.Len(t, certResp.Certificates, len(c.wantNames))
			require.Equal(t, c.wantNames, pluckCertNames(certResp.Certificates))
...
func (s *integrationTestSuite) TestHostReenrollWithSameHostRowRefetchOsquery() {
	...
	for i, h := range []*mobius.Host{host1, host2, host3} {
	...
	oldHosts := make([]mobius.Host, 3)
	for i, h := range []*mobius.Host{host1, host2, host3} {
		var hostResponse getHostResponse
		s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d", h.ID), nil, http.StatusOK, &hostResponse)
	...
	for _, h := range []*mobius.Host{host1, host2, host3} {
		distributedReq := submitDistributedQueryResultsRequestShim{
			NodeKey: *h.NodeKey,
			Results: map[string]json.RawMessage{
...
func (s *integrationTestSuite) TestConditionalAccessOnlyCloud() {
	...
	s.DoJSON("POST", "/api/latest/mobius/conditional-access/microsoft/confirm", conditionalAccessMicrosoftConfirmRequest{},
