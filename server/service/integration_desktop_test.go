...
func (s *integrationTestSuite) TestDeviceAuthenticatedEndpoints() {
	...
	require.NoError(t, s.ds.ReplaceHostDeviceMapping(context.Background(), hosts[0].ID, []*mobius.HostDeviceMapping{
	...
	_, err = s.ds.SetOrUpdateCustomHostDeviceMapping(context.Background(), hosts[0].ID, "c@b.c", mobius.DeviceMappingCustomInstaller)
	require.NoError(t, err)
	...
	require.NoError(t, s.ds.ReplaceHostBatteries(context.Background(), hosts[0].ID, []*mobius.HostBattery{
	...
	res := s.DoRawNoAuth("GET", "/api/latest/mobius/device/", nil, http.StatusNotFound)
	res.Body.Close()

	// get host with invalid token
	res = s.DoRawNoAuth("GET", "/api/latest/mobius/device/no_such_token", nil, http.StatusUnauthorized)
	res.Body.Close()
	...
	require.Equal(t, &mobius.HostBattery{CycleCount: 1, Health: "Normal"}, (*getHostResp.Host.Batteries)[0])
	...
	getHostResp.Host.DEPAssignedToMobius = ptr.Bool(false)
	require.Equal(t, hostDevResp, getHostResp.Host)

	// request a refetch for that valid host
	res = s.DoRawNoAuth("POST", "/api/latest/mobius/device/"+token+"/refetch", nil, http.StatusOK)
	res.Body.Close()
	...
	res = s.DoRawNoAuth("POST", "/api/latest/mobius/device/no_such_token/refetch", nil, http.StatusUnauthorized)
	require.NoError(t, res.Body.Close())

	// list device mappings for valid token
	var listDMResp listHostDeviceMappingResponse
	res = s.DoRawNoAuth("GET", "/api/latest/mobius/device/"+token+"/device_mapping", nil, http.StatusOK)
	...
	require.ElementsMatch(t, listDMResp.DeviceMapping, []*mobius.HostDeviceMapping{
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/device_mapping", hosts[0].ID), nil, http.StatusOK, &listDMResp)
	require.Equal(t, hosts[0].ID, listDMResp.HostID)
	require.Equal(t, devDMs, listDMResp.DeviceMapping)

	// list device mappings for invalid token
	res = s.DoRawNoAuth("GET", "/api/latest/mobius/device/no_such_token/device_mapping", nil, http.StatusUnauthorized)
	require.NoError(t, res.Body.Close())

	// get macadmins for valid token
	var getMacadm macadminsDataResponse
	res = s.DoRawNoAuth("GET", "/api/latest/mobius/device/"+token+"/macadmins", nil, http.StatusOK)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/macadmins", hosts[0].ID), nil, http.StatusOK, &getMacadm)
	require.Equal(t, devMacadm, getMacadm.Macadmins)

	// get macadmins for invalid token
	res = s.DoRawNoAuth("GET", "/api/latest/mobius/device/no_such_token/macadmins", nil, http.StatusUnauthorized)
	require.NoError(t, res.Body.Close())
	...
	res = s.DoRawNoAuth("GET", "/api/latest/mobius/device/"+token+"/policies", nil, http.StatusPaymentRequired)
	require.NoError(t, json.NewDecoder(res.Body).Decode(&getHostResp))
	require.NoError(t, res.Body.Close())
	...
	res = s.DoRawNoAuth("GET", "/api/latest/mobius/device/"+token+"/desktop", nil, http.StatusPaymentRequired)
	require.NoError(t, json.NewDecoder(res.Body).Decode(&getDesktopResp))
	require.NoError(t, res.Body.Close())
	require.Nil(t, getDesktopResp.FailingPolicies)
}

// TestDefaultTransparencyURL tests that Mobius Free licensees are restricted to the default transparency url.
func (s *integrationTestSuite) TestDefaultTransparencyURL() {
	...
	acResp = appConfigResponse{}
	s.DoJSON("PATCH", "/api/latest/mobius/config", mobius.AppConfig{MobiusDesktop: mobius.MobiusDesktopSettings{TransparencyURL: "customURL"}}, http.StatusUnprocessableEntity, &acResp)
...
func (s *integrationTestSuite) TestRateLimitOfEndpoints() {
	...
	testCases := []struct {
		...
		{
			endpoint: "/api/latest/mobius/forgot_password",
			verb:     "POST",
		...
		{
			endpoint: "/api/latest/mobius/device/" + uuid.NewString(),
			verb:     "GET",
...
func (s *integrationTestSuite) TestErrorReporting() {
	...
	res := s.DoRawNoAuth("POST", "/api/latest/mobius/device/no_such_token/debug/errors", []byte("{}"), http.StatusUnauthorized)
	res.Body.Close()

	// invalid request body is a bad request
	res = s.DoRawNoAuth("POST", "/api/latest/mobius/device/"+token+"/debug/errors", []byte("{},{}"), http.StatusBadRequest)
	res.Body.Close()

	data := make(map[string]interface{})
	for i := int64(0); i < (maxMobiusdErrorReportSize+1024)/20; i++ {
	...
	require.NoError(t, err)
	res = s.DoRawNoAuth("POST", "/api/latest/mobius/device/"+token+"/debug/errors", jsonData, http.StatusBadRequest)
	res.Body.Close()

	res = s.DoRawNoAuth("POST", "/api/latest/mobius/device/"+token+"/debug/errors", []byte("{}"), http.StatusOK)
	res.Body.Close()
	...
	require.NoError(t, err)
	res = s.DoRawNoAuth("POST", "/api/latest/mobius/device/"+token+"/debug/errors", errBytes, http.StatusOK)
	res.Body.Close()
	...
	checkError := func(errorItem map[string]interface{}, expectedCount int) {
		...
		// Check that the exact mobiusdaemon error can be retrieved.
		b, err := json.Marshal(errChain1["data"])
		require.NoError(t, err)
		var receivedErr mobius.MobiusdError
