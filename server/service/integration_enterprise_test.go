...
import (
	...
	ma "github.com/notawar/mobius/v4/ee/maintained-apps"
	"github.com/notawar/mobius/v4/ee/server/calendar"
	...
	"github.com/notawar/mobius/v4/server/cron"
	"github.com/notawar/mobius/v4/server/datastore/filesystem"
	"github.com/notawar/mobius/v4/server/datastore/mysql"
	"github.com/notawar/mobius/v4/server/datastore/redis/redistest"
	"github.com/notawar/mobius/v4/server/live_query/live_query_mock"
	"github.com/notawar/mobius/v4/server/mdm"
	maintained_apps "github.com/notawar/mobius/v4/server/mdm/maintainedapps"
	"github.com/notawar/mobius/v4/server/mobius"
	"github.com/notawar/mobius/v4/server/ptr"
	"github.com/notawar/mobius/v4/server/pubsub"
	commonCalendar "github.com/notawar/mobius/v4/server/service/calendar"
	"github.com/notawar/mobius/v4/server/service/conditional_access_microsoft_proxy"
	"github.com/notawar/mobius/v4/server/service/redis_lock"
	"github.com/notawar/mobius/v4/server/service/schedule"
...
func (s *integrationEnterpriseTestSuite) TestTeamSpecs() {
	...
	s.DoJSON("POST", "/api/latest/mobius/spec/teams", teamSpecs, http.StatusOK, &applyResp)
	...
	s.lastActivityMatches(mobius.ActivityTypeAppliedSpecTeam{}.ActivityName(), fmt.Sprintf(`{"teams": [{"id": %d, "name": %q}]}`, team.ID, team.Name), 0)

	// Create team policy
	teamPolicy, err := s.ds.NewTeamPolicy(
		context.Background(), team.ID, nil, mobius.PolicyPayload{Name: "TestSpecTeamPolicy", Query: "SELECT 1"},
	)
	require.NoError(t, err)
	defer func() {
	...
	res := s.Do("POST", "/api/latest/mobius/spec/teams", teamSpecs, http.StatusUnprocessableEntity, "dry_run", "true")
	...
	s.DoJSON("GET", "/api/latest/mobius/teams/"+fmt.Sprint(team.ID), nil, http.StatusOK, &getTmResp)
	require.Equal(t, mobius.TeamMDM{
		MacOSUpdates: mobius.AppleOSUpdateSettings{
			MinimumVersion: optjson.SetString("10.15.0"),
			Deadline:       optjson.SetString("2021-01-01"),
	...
	s.DoJSON("GET", "/api/latest/mobius/teams", nil, http.StatusOK, &listTmResp, "query", teamName)
	...
	s.Do("POST", "/api/latest/mobius/spec/teams", teamSpecs, http.StatusBadRequest, "dry_run", "true")

	// dry-run with empty body
	res = s.DoRaw("POST", "/api/latest/mobius/spec/teams", nil, http.StatusBadRequest, "force", "true")
	errBody, err := io.ReadAll(res.Body)
	require.NoError(t, err)
	require.Contains(t, string(errBody), `"Expected JSON Body"`)

	// dry-run with invalid top-level key
	s.Do("POST", "/api/latest/mobius/spec/teams", json.RawMessage(`{
		"specs": [
	...
	res = s.Do("POST", "/api/latest/mobius/spec/teams", teamSpecs, http.StatusUnprocessableEntity, "dry_run", "true")
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, "Couldn't update macos_settings because MDM features aren't turned on in Mobius.")
	...
	s.DoJSON("POST", "/api/latest/mobius/spec/teams", teamSpecs, http.StatusOK, &applyResp, "dry_run", "true")
	...
	res = s.Do("POST", "/api/latest/mobius/spec/teams", teamSpecs, http.StatusUnprocessableEntity, "dry_run", "true")
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, "Couldn't update macos_setup because MDM features aren't turned on in Mobius.")
	...
	s.Do("POST", "/api/latest/mobius/spec/teams", teamSpecs, http.StatusOK, "dry_run", "true")
	...
	s.Do("POST", "/api/latest/mobius/spec/teams", teamSpecs, http.StatusOK)
	...
	s.Do("POST", "/api/latest/mobius/spec/teams", teamSpecs, http.StatusOK, "force", "true")
	...
	s.Do("POST", "/api/latest/mobius/spec/teams", teamSpecs, http.StatusBadRequest)
	...
	teams, err := s.ds.ListTeams(context.Background(), mobius.TeamFilter{User: user}, mobius.ListOptions{})
	require.NoError(t, err)
	...
	teamSpecs = map[string]any{
		"specs": []any{
			map[string]any{
				"name":     "team2",
				"secrets":  []mobius.EnrollSecret{{Secret: "ABC"}},
				"features": nil,
			},
		},
	}
...
func (s *integrationEnterpriseTestSuite) TestTeamSpecsPermissions() {
	...
	team1, err := s.ds.NewTeam(context.Background(), &mobius.Team{
		ID:          42,
		Name:        "team1",
		Description: "desc team1",
	})
	require.NoError(t, err)
	team2, err := s.ds.NewTeam(context.Background(), &mobius.Team{
		ID:          43,
		Name:        "team2",
	...
	s.Do("POST", "/api/latest/mobius/spec/teams", editTeam1Spec, http.StatusOK)
	...
	s.Do("POST", "/api/latest/mobius/spec/teams", editTeam2Spec, http.StatusForbidden)
}
...
func (s *integrationEnterpriseTestSuite) TestTeamSchedule() {
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/teams/%d/schedule", team1.ID), nil, http.StatusOK, &ts)
	...
	gsParams := teamScheduleQueryRequest{ScheduledQueryPayload: mobius.ScheduledQueryPayload{
		QueryID:  &qr.ID,
		Interval: ptr.Uint(42),
	}}
	r := teamScheduleQueryResponse{}
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobius/teams/%d/schedule", team1.ID), gsParams, http.StatusOK, &r)
	...
	modifyParams := modifyTeamScheduleRequest{ScheduledQueryPayload: mobius.ScheduledQueryPayload{Interval: ptr.Uint(55)}}
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/teams/%d/schedule/%d", team1.ID, id), modifyParams, http.StatusOK, &modifyResp)

	// just to satisfy my paranoia, wanted to make sure the contents of the json would work
	s.DoRaw("PATCH", fmt.Sprintf("/api/latest/mobius/teams/%d/schedule/%d", team1.ID, id), []byte(`{"interval": 77}`), http.StatusOK)
	...
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobius/teams/%d/schedule/%d", team1.ID, id), nil, http.StatusOK, &deleteResp)
...
func (s *integrationEnterpriseTestSuite) TestTeamPolicies() {
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/teams/%d/policies", team1.ID), nil, http.StatusOK, &ts)
	require.Len(t, ts.Policies, 0)
	require.Len(t, ts.InheritedPolicies, 0)

	// create a global policy
	gpol, err := s.ds.NewGlobalPolicy(context.Background(), nil, mobius.PolicyPayload{Name: "TestGlobalPolicy", Query: "SELECT 1"})
	require.NoError(t, err)
	defer func() {
	...
	qr, err := s.ds.NewQuery(context.Background(), &mobius.Query{
		Name:           "TestQuery2",
		Description:    "Some description",
		Query:          "select * from osquery;",
		ObserverCanRun: true,
	...
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobius/teams/%d/policies", team1.ID), tpParams, http.StatusOK, &r)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/teams/%d/policies/count", team1.ID), nil, http.StatusOK, &tc)
	require.Nil(t, tc.Err)
	require.Equal(t, 1, tc.Count)

	gc := countGlobalPoliciesResponse{}
	s.DoJSON("GET", "/api/latest/mobius/policies/count", nil, http.StatusOK, &gc)
	require.Nil(t, gc.Err)
	require.Equal(t, 1, gc.Count)

	// Test merge inherited
	ts = listTeamPoliciesResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/teams/%d/policies", team1.ID), nil, http.StatusOK, &ts, "merge_inherited", "true", "order_key", "team_id", "order_direction", "desc")
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/teams/%d/policies/count", team1.ID), nil, http.StatusOK, &countResp, "merge_inherited", "true")
	...
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobius/teams/%d/policies/delete", team1.ID), deletePolicyParams, http.StatusOK, &deletePolicyResp)
...
func (s *integrationEnterpriseTestSuite) TestNoTeamPolicies() {
	...
	ts := listTeamPoliciesResponse{}
	s.DoJSON("GET", "/api/latest/mobius/teams/0/policies", nil, http.StatusOK, &ts)
	require.Len(t, ts.Policies, 0)
	require.Len(t, ts.InheritedPolicies, 0)
	// Create a placeholder global policy.
	_, err := s.ds.NewGlobalPolicy(ctx, nil, mobius.PolicyPayload{
		Name:  "globalPolicy1",
		Query: "SELECT 0;",
	})
	require.NoError(t, err)
	...
	r := teamPolicyResponse{}
	s.DoJSON("POST", "/api/latest/mobius/teams/0/policies", tpParams, http.StatusOK, &r)
	...
	s.DoJSON("POST", "/api/latest/mobius/teams/0/policies", tpParams, http.StatusConflict, &r)
	...
	tc := countTeamPoliciesResponse{}
	s.DoJSON("GET", "/api/latest/mobius/teams/0/policies/count", nil, http.StatusOK, &tc)
	require.Equal(t, 2, tc.Count)
	// Test merge inherited for "No team" policies.
	ts = listTeamPoliciesResponse{}
	s.DoJSON("GET", "/api/latest/mobius/teams/0/policies", nil, http.StatusOK, &ts, "merge_inherited", "true", "order_key", "team_id", "order_direction", "desc")
	...
	countResp := countTeamPoliciesResponse{}
	s.DoJSON("GET", "/api/latest/mobius/teams/0/policies/count", nil, http.StatusOK, &countResp, "merge_inherited", "true")
	require.Nil(t, countResp.Err)
	...
	deletePolicyResp := deleteTeamPoliciesResponse{}
	s.DoJSON("POST", "/api/latest/mobius/teams/0/policies/delete", deletePolicyParams, http.StatusOK, &deletePolicyResp)
	ts = listTeamPoliciesResponse{}
	s.DoJSON("GET", "/api/latest/mobius/teams/0/policies", nil, http.StatusOK, &ts)
	require.Len(t, ts.Policies, 1)
	...
	ts = listTeamPoliciesResponse{}
	s.DoJSON("GET", "/api/latest/mobius/teams/0/policies", nil, http.StatusForbidden, &ts)
	tpParams = teamPolicyRequest{
		Name:  "noTeamPolicy1",
		Query: "SELECT 1;",
	}
	r = teamPolicyResponse{}
	s.DoJSON("POST", "/api/latest/mobius/teams/0/policies", tpParams, http.StatusForbidden, &r)
	tc = countTeamPoliciesResponse{}
	s.DoJSON("GET", "/api/latest/mobius/teams/0/policies/count", nil, http.StatusForbidden, &tc)
	deletePolicyParams = deleteTeamPoliciesRequest{
		IDs: []uint{noTeamPolicy2.ID},
	}
	s.DoJSON("POST", "/api/latest/mobius/teams/0/policies/delete", deletePolicyParams, http.StatusForbidden, &deleteTeamPoliciesResponse{})
}
...
func (s *integrationEnterpriseTestSuite) TestTeamQueries() {
	...
	// create global query
	params := mobius.QueryPayload{
		Name:  ptr.String("global1"),
		Query: ptr.String("select * from time;"),
	}
	var createQueryResp createQueryResponse
	s.DoJSON("POST", "/api/latest/mobius/queries", &params, http.StatusOK, &createQueryResp)
	...
	s.DoJSON("GET", "/api/latest/mobius/queries", nil, http.StatusOK, &listQueriesResp, "team_id", fmt.Sprint(team1.ID))
	require.Len(t, listQueriesResp.Queries, 1)
	assert.Equal(t, "team1", listQueriesResp.Queries[0].Name)

	// list merged team queries
	s.DoJSON("GET", "/api/latest/mobius/queries", nil, http.StatusOK, &listQueriesResp, "team_id", fmt.Sprint(team1.ID), "merge_inherited", "true", "order_key", "team_id", "order_direction", "desc")
...
func (s *integrationEnterpriseTestSuite) TestModifyTeamEnrollSecrets() {
	...
	team := &mobius.Team{
		...
		Secrets:     []*mobius.EnrollSecret{{Secret: "initialSecret"}},
	}
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/teams/%d/secrets", team.ID), req, http.StatusOK, &resp)
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/teams/%d/secrets", team.ID), json.RawMessage(`{"secrets": []}`), http.StatusOK, &resp)
	require.Len(t, resp.Secrets, 0)

	// Test bad requests
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/teams/%d/secrets", team.ID), json.RawMessage(`{"foo": [{"secret": "testSecret3"}]}`), http.StatusUnprocessableEntity, &resp)
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/teams/%d/secrets", team.ID), json.RawMessage(`{}`), http.StatusUnprocessableEntity, &resp)

	// too many secrets
	secrets := createEnrollSecrets(t, mobius.MaxEnrollSecretsCount+1)
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/teams/%d/secrets", team.ID), json.RawMessage(`{"secrets": `+string(jsonMustMarshal(t, secrets))+`}`), http.StatusUnprocessableEntity, &resp)
}
...
func (s *integrationEnterpriseTestSuite) TestAvailableTeams() {
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/users/%d", user.ID), nil, http.StatusOK, &getResp)
	...
	user.GlobalRole = nil
	user.Teams = []mobius.UserTeam{{Team: *team, Role: "maintainer"}}
	...
	require.NoError(t, err)
	resp := s.DoRawWithHeaders("GET", "/api/latest/mobius/me", []byte(""), http.StatusOK, map[string]string{
...
func (s *integrationEnterpriseTestSuite) TestTeamEndpoints() {
	...
	team := &mobius.Team{
		...
		Secrets:     []*mobius.EnrollSecret{{Secret: "DEF"}},
	}

	var tmResp teamResponse
	s.DoJSON("POST", "/api/latest/mobius/teams", team, http.StatusOK, &tmResp)
	...
	team2 := &mobius.Team{
		...
		Secrets:     []*mobius.EnrollSecret{{Secret: "GHI"}},
	}
	tmResp.Team = nil
	s.DoJSON("POST", "/api/latest/mobius/teams", team2, http.StatusConflict, &tmResp)

	// create a team with reserved team names; should be case-insensitive
	teamReserved := &mobius.Team{
		Name:        "no TeAm",
		Description: "description",
		Secrets:     []*mobius.EnrollSecret{{Secret: "foobar"}},
	}

	r := s.Do("POST", "/api/latest/mobius/teams", teamReserved, http.StatusUnprocessableEntity)
	...
	tmResp.Team = nil
	s.DoJSON("POST", "/api/latest/mobius/teams", team3, http.StatusUnprocessableEntity, &tmResp)

	// create a team with invalid host expiry window
	team4 := &mobius.TeamPayload{
		Name:        ptr.String(name + "invalid host_expiry_window"),
		Description: ptr.String("Team4 description"),
		Secrets:     []*mobius.EnrollSecret{{Secret: "TEAM4"}},
		HostExpirySettings: &mobius.HostExpirySettings{
			HostExpiryEnabled: true,
			HostExpiryWindow:  -1,
		},
	}
	s.DoJSON("POST", "/api/latest/mobius/teams", team4, http.StatusUnprocessableEntity, &tmResp)

	// list teams matching name of one team
	var listResp listTeamsResponse
	s.DoJSON("GET", "/api/latest/mobius/teams", nil, http.StatusOK, &listResp, "query", name, "per_page", "2")
	...
	s.DoJSON("GET", "/api/latest/mobius/teams", nil, http.StatusOK, &listResp, "query", " nope ", "per_page", "2")
	require.Len(t, listResp.Teams, 0)

	// get team
	var getResp getTeamResponse
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/teams/%d", tm1ID), nil, http.StatusOK, &getResp)
	...
	tmResp.Team = nil
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/teams/%d", tm1ID), team, http.StatusOK, &tmResp)
	assert.Contains(t, tmResp.Team.Description, "Alt ")

	// modify team's disk encryption, impossible without mdm enabled
	res := s.Do("PATCH", fmt.Sprintf("/api/latest/mobius/teams/%d", tm1ID), mobius.TeamPayload{
		MDM: &mobius.TeamPayloadMDM{
			EnableDiskEncryption: optjson.SetBool(true),
		},
	}, http.StatusUnprocessableEntity)
	errMsg := extractServerErrorText(res.Body)
	assert.Contains(t, errMsg, `Couldn't update macos_settings because MDM features aren't turned on in Mobius.`)

	// modify a team with a NULL config
	defaultFeatures := mobius.Features{}
	...
	tmResp.Team = nil
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/teams/%d", tm1ID+1), team, http.StatusNotFound, &tmResp)

	// modify team host expiry
	modifyExpiry := mobius.TeamPayload{
		HostExpirySettings: &mobius.HostExpirySettings{
			HostExpiryEnabled: true,
			HostExpiryWindow:  10,
		},
	}
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/teams/%d", tm1ID), modifyExpiry, http.StatusOK, &tmResp)
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/teams/%d", tm1ID), modifyExpiry, http.StatusUnprocessableEntity, &tmResp)

	// try to rename to reserved names
	r = s.Do("PATCH", fmt.Sprintf("/api/latest/mobius/teams/%d", tm1ID), mobius.TeamPayload{Name: ptr.String("no TEAM")}, http.StatusUnprocessableEntity)
	require.Contains(t, extractServerErrorText(r.Body), `"No team" is a reserved team name`)

	r = s.Do("PATCH", fmt.Sprintf("/api/latest/mobius/teams/%d", tm1ID), mobius.TeamPayload{Name: ptr.String("ALL teAMs")}, http.StatusUnprocessableEntity)
	require.Contains(t, extractServerErrorText(r.Body), `"All teams" is a reserved team name`)

	// Modify team's calendar config
	modifyCalendar := mobius.TeamPayload{
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/teams/%d", tm1ID), modifyCalendar, http.StatusOK, &tmResp)
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/teams/%d", tm1ID), modifyCalendar, http.StatusUnprocessableEntity, &tmResp)

	// list team users
	var usersResp listUsersResponse
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/teams/%d/users", tm1ID), nil, http.StatusOK, &usersResp)
	assert.Len(t, usersResp.Users, 0)

	// list team users - non-existing team
	usersResp.Users = nil
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/teams/%d/users", tm1ID+1), nil, http.StatusNotFound, &usersResp)
	...
	// add a team user
	tmResp.Team = nil
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/teams/%d/users", tm1ID), modifyTeamUsersRequest{Users: []mobius.TeamUser{{User: *user, Role: mobius.RoleObserver}}}, http.StatusOK, &tmResp)
	require.Len(t, tmResp.Team.Users, 1)
	assert.Equal(t, user.ID, tmResp.Team.Users[0].ID)

	// add a team user - non-existing team
	tmResp.Team = nil
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/teams/%d/users", tm1ID+1), modifyTeamUsersRequest{Users: []mobius.TeamUser{{User: *user, Role: mobius.RoleObserver}}}, http.StatusNotFound, &tmResp)

	// add a team user - invalid user role
	tmResp.Team = nil
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/teams/%d/users", tm1ID), modifyTeamUsersRequest{Users: []mobius.TeamUser{{User: *user, Role: "foobar"}}}, http.StatusUnprocessableEntity, &tmResp)

	// search for that user
	usersResp.Users = nil
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/teams/%d/users", tm1ID), nil, http.StatusOK, &usersResp, "query", "user")
	require.Len(t, usersResp.Users, 1)
	assert.Equal(t, user.ID, usersResp.Users[0].ID)

	// search for unknown user
	usersResp.Users = nil
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/teams/%d/users", tm1ID), nil, http.StatusOK, &usersResp, "query", "notauser")
	require.Len(t, usersResp.Users, 0)

	// delete team user
	tmResp.Team = nil
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobius/teams/%d/users", tm1ID), modifyTeamUsersRequest{Users: []mobius.TeamUser{{User: mobius.User{ID: user.ID}}}}, http.StatusOK, &tmResp)
	require.Len(t, tmResp.Team.Users, 0)

	// delete team user - unknown user
	tmResp.Team = nil
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobius/teams/%d/users", tm1ID), modifyTeamUsersRequest{Users: []mobius.TeamUser{{User: mobius.User{ID: user.ID + 1}}}}, http.StatusOK, &tmResp)
	require.Len(t, tmResp.Team.Users, 0)

	// delete team user - unknown team
	tmResp.Team = nil
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobius/teams/%d/users", tm1ID+1), modifyTeamUsersRequest{Users: []mobius.TeamUser{{User: mobius.User{ID: user.ID}}}}, http.StatusNotFound, &tmResp)

	// modify team agent options with invalid options
	tmResp.Team = nil
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobius/teams/%d/agent_options", tm1ID), json.RawMessage(`{
		"x": "y"
	}`), http.StatusBadRequest, &tmResp)

	// modify team agent options with invalid key
	badRes := s.Do("POST", fmt.Sprintf("/api/latest/mobius/teams/%d/agent_options", tm1ID), json.RawMessage(`{
		"bad_key": 1
	...
	tmResp.Team = nil
	resp := s.DoRaw("POST", fmt.Sprintf("/api/latest/mobius/teams/%d/agent_options", tm1ID), json.RawMessage(`{
		"config": {
			"options": {
	...
	s.lastActivityMatches(mobius.ActivityTypeEditedAgentOptions{}.ActivityName(), fmt.Sprintf(`{"global": false, "team_id": %d, "team_name": %q}`, tm1ID, team.Name), 0)

	// modify team agent options - unknown team
	tmResp.Team = nil
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobius/teams/%d/agent_options", tm1ID+1), json.RawMessage(`{}`), http.StatusNotFound, &tmResp)

	// get team enroll secrets
	var secResp teamEnrollSecretsResponse
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/teams/%d/secrets", tm1ID), nil, http.StatusOK, &secResp)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/teams/%d/secrets", tm1ID+1), nil, http.StatusOK, &secResp)
	assert.Len(t, secResp.Secrets, 0)

	// delete team
	var delResp deleteTeamResponse
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobius/teams/%d", tm1ID), nil, http.StatusOK, &delResp)

	// delete team again, now an unknown team
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobius/teams/%d", tm1ID), nil, http.StatusNotFound, &delResp)
}
...
func (s *integrationEnterpriseTestSuite) TestTeamSecretsAreObfuscated() {
	...
	users := []*mobius.User{global_obs, global_obs_plus, team_obs, team_obs_plus}
	for _, u := range users {
	...
	// --------------------------------------------------------------------
	for _, u := range []*mobius.User{global_obs, global_obs_plus} {

		s.setTokenForTest(t, u.Email, test.GoodPassword)

		// list all teams
		var listResp listTeamsResponse
		s.DoJSON("GET", "/api/latest/mobius/teams", nil, http.StatusOK, &listResp)
		...
		for _, team := range listResp.Teams {
			for _, secret := range team.Secrets {
				require.Equal(t, mobius.MaskedPassword, secret.Secret)
			}
		}
		...
		for _, team := range teams {
			var getResp getTeamResponse
			s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/teams/%d", team.ID), nil, http.StatusOK, &getResp)
			...
			var secResp teamEnrollSecretsResponse
			s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/teams/%d/secrets", team.ID), nil, http.StatusOK, &secResp)
	...
	// --------------------------------------------------------------------
	for _, u := range []*mobius.User{team_obs, team_obs_plus} {
		...
		for _, team := range listResp.Teams {
			for _, secret := range team.Secrets {
				...
				if u.ID == team_obs.ID {
					require.Equal(t, mobius.MaskedPassword == secret.Secret, team.ID == teams[0].ID)
					require.Equal(t, mobius.MaskedPassword != secret.Secret, team.ID == teams[1].ID)
				}
...
func (s *integrationEnterpriseTestSuite) TestExternalIntegrationsTeamConfig() {
	...
	team := &mobius.Team{
		Name:        t.Name(),
		Description: "Team description",
		Secrets:     []*mobius.EnrollSecret{{Secret: "XYZ"}},
	}
	var tmResp teamResponse
	s.DoJSON("POST", "/api/latest/mobius/teams", team, http.StatusOK, &tmResp)
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/teams/%d", team.ID), mobius.TeamPayload{WebhookSettings: &mobius.TeamWebhookSettings{
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/teams/%d", team.ID), mobius.TeamPayload{Integrations: &mobius.TeamIntegrations{
	...
	s.DoRaw("PATCH", "/api/v1/mobius/config", []byte(fmt.Sprintf(`{
		"integrations": {
			"jira": [
				{
					"url": %q,
	...
	tmResp = teamResponse{}
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/teams/%d", team.ID), mobius.TeamPayload{
		Integrations: &mobius.TeamIntegrations{
			Jira: []*mobius.TeamJiraIntegration{
				{
					URL:                   srvURL,
	...
	s.DoJSON("PATCH", "/api/v1/mobius/config", json.RawMessage(`{
		"org_info": {
...
func (s *integrationEnterpriseTestSuite) TestWindowsUpdatesTeamConfig() {
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/teams/%d", team.ID), map[string]any{
		"mdm": map[string]any{
			"windows_updates": &mobius.WindowsUpdates{
	...
	s.lastActivityMatches(mobius.ActivityTypeEditedWindowsUpdates{}.ActivityName(), fmt.Sprintf(`{"team_id": %d, "team_name": %q, "deadline_days": 5, "grace_period_days": 2}`, team.ID, team.Name), 0)

	checkWindowsOSUpdatesProfile(t, s.ds, &team.ID, &mobius.WindowsUpdates{
		DeadlineDays:    optjson.SetInt(5),
		GracePeriodDays: optjson.SetInt(2),
	})
	...
	require.Equal(t, 2, tmResp.Team.Config.MDM.WindowsUpdates.GracePeriodDays.Value)
	lastActivity := s.lastActivityMatches(mobius.ActivityTypeEditedWindowsUpdates{}.ActivityName(), fmt.Sprintf(`{"team_id": %d, "team_name": %q, "deadline_days": 6, "grace_period_days": 2}`, team.ID, team.Name), 0)
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/teams/%d", team.ID), map[string]any{
		"mdm": map[string]any{
			"macos_updates": &mobius.AppleOSUpdateSettings{
				MinimumVersion: optjson.SetString("10.15.0"),
				Deadline:       optjson.SetString("2021-01-01"),
			},
		},
	...
	s.lastActivityOfTypeMatches(mobius.ActivityTypeEditedWindowsUpdates{}.ActivityName(), "", lastActivity)
	lastActivity = s.lastActivityMatches(mobius.ActivityTypeEditedMacOSMinVersion{}.ActivityName(), ``, 0)
	...
	s.lastActivityMatches(mobius.ActivityTypeEditedWindowsUpdates{}.ActivityName(), fmt.Sprintf(`{"team_id": %d, "team_name": %q, "deadline_days": null, "grace_period_days": null}`, team.ID, team.Name), 0)
...
func (s *integrationEnterpriseTestSuite) TestGitOpsModeConfig() {
	...
	res := s.Do("PATCH", "/api/latest/mobius/config", json.RawMessage(`{
	...
	s.Do("PATCH", "/api/latest/mobius/config", json.RawMessage(`{
			"gitops": { "gitops_mode_enabled": true, "repository_url": "a.b.cc" }
	  }`), http.StatusOK)
	s.lastActivityOfTypeMatches(mobius.ActivityTypeEnabledGitOpsMode{}.ActivityName(), "", 0)

	// turn off, persists repo url
	s.Do("PATCH", "/api/latest/mobius/config", json.RawMessage(`{
			"gitops": { "gitops_mode_enabled": false, "repository_url": "a.b.cc" }
	  }`), http.StatusOK)
	s.lastActivityOfTypeMatches(mobius.ActivityTypeDisabledGitOpsMode{}.ActivityName(), "", 0)
...
func (s *integrationEnterpriseTestSuite) assertAppleOSUpdatesDeclaration(teamID *uint, profileName string, expected *mobius.AppleOSUpdateSettings) {
	t := s.T()
	if teamID == nil {
		teamID = ptr.Uint(0)
	}
...
func (s *integrationEnterpriseTestSuite) TestAppleOSUpdatesTeamConfig() {
	...
	s.assertAppleOSUpdatesDeclaration(&team.ID, mdm.MobiusMacOSUpdatesProfileName, nil)
	s.assertAppleOSUpdatesDeclaration(&team.ID, mdm.MobiusIOSUpdatesProfileName, nil)
	s.assertAppleOSUpdatesDeclaration(&team.ID, mdm.MobiusIPadOSUpdatesProfileName, nil)
	...
	require.Equal(t, "10.15.0", tmResp.Team.Config.MDM.MacOSUpdates.MinimumVersion.Value)
	require.Equal(t, "2021-01-01", tmResp.Team.Config.MDM.MacOSUpdates.Deadline.Value)
	s.lastActivityMatches(mobius.ActivityTypeEditedMacOSMinVersion{}.ActivityName(), fmt.Sprintf(`{"team_id": %d, "team_name": %q, "minimum_version": "10.15.0", "deadline": "2021-01-01"}`, team.ID, team.Name), 0)

	s.assertAppleOSUpdatesDeclaration(&team.ID, mdm.MobiusMacOSUpdatesProfileName, macOSUpdates)
	s.assertAppleOSUpdatesDeclaration(&team.ID, mdm.MobiusIOSUpdatesProfileName, nil)
	s.assertAppleOSUpdatesDeclaration(&team.ID, mdm.MobiusIPadOSUpdatesProfileName, nil)
	...
	s.lastActivityOfTypeMatches(mobius.ActivityTypeEditedMacOSMinVersion{}.ActivityName(), fmt.Sprintf(`{"team_id": %d, "team_name": %q, "minimum_version": "10.15.0", "deadline": "2021-01-01"}`, team.ID, team.Name), 0)
	s.lastActivityOfTypeMatches(mobius.ActivityTypeEditedIOSMinVersion{}.ActivityName(), fmt.Sprintf(`{"team_id": %d, "team_name": %q, "minimum_version": "11.11.11", "deadline": "2022-02-02"}`, team.ID, team.Name), 0)
	s.lastActivityOfTypeMatches(mobius.ActivityTypeEditedIPadOSMinVersion{}.ActivityName(), fmt.Sprintf(`{"team_id": %d, "team_name": %q, "minimum_version": "12.12.12", "deadline": "2023-03-03"}`, team.ID, team.Name), 0)

	s.assertAppleOSUpdatesDeclaration(&team.ID, mdm.MobiusMacOSUpdatesProfileName, macOSUpdates)
	s.assertAppleOSUpdatesDeclaration(&team.ID, mdm.MobiusIOSUpdatesProfileName, iOSUpdates)
	s.assertAppleOSUpdatesDeclaration(&team.ID, mdm.MobiusIPadOSUpdatesProfileName, iPadOSUpdates)
	...
	macOSLastActivity := s.lastActivityOfTypeMatches(mobius.ActivityTypeEditedMacOSMinVersion{}.ActivityName(), fmt.Sprintf(`{"team_id": %d, "team_name": %q, "minimum_version": "10.15.0", "deadline": "2025-10-01"}`, team.ID, team.Name), 0)
	iOSLastActivity := s.lastActivityOfTypeMatches(mobius.ActivityTypeEditedIOSMinVersion{}.ActivityName(), fmt.Sprintf(`{"team_id": %d, "team_name": %q, "minimum_version": "11.11.11", "deadline": "2024-02-02"}`, team.ID, team.Name), 0)
	iPadOSLastActivity := s.lastActivityOfTypeMatches(mobius.ActivityTypeEditedIPadOSMinVersion{}.ActivityName(), fmt.Sprintf(`{"team_id": %d, "team_name": %q, "minimum_version": "12.12.12", "deadline": "2024-03-03"}`, team.ID, team.Name), 0)
	...
	s.lastActivityOfTypeMatches(mobius.ActivityTypeEditedMacOSMinVersion{}.ActivityName(), "", macOSLastActivity)
	s.lastActivityOfTypeMatches(mobius.ActivityTypeEditedIOSMinVersion{}.ActivityName(), "", iOSLastActivity)
	s.lastActivityOfTypeMatches(mobius.ActivityTypeEditedIPadOSMinVersion{}.ActivityName(), "", iPadOSLastActivity)
	lastActivity := s.lastActivityMatches(mobius.ActivityTypeEditedWindowsUpdates{}.ActivityName(), ``, 0)
	...
	s.lastActivityOfTypeMatches(mobius.ActivityTypeEditedMacOSMinVersion{}.ActivityName(), fmt.Sprintf(`{"team_id": %d, "team_name": %q, "minimum_version": "", "deadline": ""}`, team.ID, team.Name), 0)
	s.lastActivityOfTypeMatches(mobius.ActivityTypeEditedIOSMinVersion{}.ActivityName(), fmt.Sprintf(`{"team_id": %d, "team_name": %q, "minimum_version": "", "deadline": ""}`, team.ID, team.Name), 0)
	s.lastActivityOfTypeMatches(mobius.ActivityTypeEditedIPadOSMinVersion{}.ActivityName(), fmt.Sprintf(`{"team_id": %d, "team_name": %q, "minimum_version": "", "deadline": ""}`, team.ID, team.Name), 0)
...
func (s *integrationEnterpriseTestSuite) TestLinuxDiskEncryption() {
	...
	// create a Linux host
	noTeamHost, err := s.ds.NewHost(context.Background(), &mobius.Host{
	...
	team, err := s.ds.NewTeam(context.Background(), &mobius.Team{Name: "A team"})
	require.NoError(t, err)
	teamID := ptr.Uint(team.ID)
	teamHost, err := s.ds.NewHost(context.Background(), &mobius.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
		SeenTime:        time.Now(),
	...
	s.DoJSON("GET", "/api/latest/mobius/configuration_profiles/summary", getMDMProfilesSummaryRequest{}, http.StatusOK, &profileSummary)
	require.Equal(t, mobius.MDMProfilesSummary{}, profileSummary.MDMProfilesSummary)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d", noTeamHost.ID), nil, http.StatusOK, &getHostResp)
	...
	res := s.DoRawNoAuth("GET", "/api/latest/mobius/device/"+deviceToken, nil, http.StatusOK)
	err = json.NewDecoder(res.Body).Decode(&getDeviceHostResp)
	require.NoError(t, err)
	require.Nil(t, getHostResp.Host.MDM.OSSettings)

	// turn on disk encryption enforcement
	s.Do("POST", "/api/latest/mobius/disk_encryption", updateDiskEncryptionRequest{EnableDiskEncryption: true}, http.StatusNoContent)
	...
	s.DoJSON("GET", "/api/latest/mobius/configuration_profiles/summary", getMDMProfilesSummaryRequest{}, http.StatusOK, &profileSummary)
	require.Equal(t, mobius.MDMProfilesSummary{Pending: 1}, profileSummary.MDMProfilesSummary)

	// encryption summary should succeed (Linux encryption doesn't require MDM)
	var summary getMDMDiskEncryptionSummaryResponse
	s.DoJSON("GET", "/api/latest/mobius/mdm/disk_encryption/summary", getMDMDiskEncryptionSummaryRequest{}, http.StatusOK, &summary)
	s.DoJSON("GET", "/api/latest/mobius/disk_encryption", getMDMDiskEncryptionSummaryRequest{}, http.StatusOK, &summary)
	// disk is encrypted but key hasn't been escrowed yet
	require.Equal(t, mobius.MDMDiskEncryptionSummary{ActionRequired: mobius.MDMPlatformsCounts{Linux: 1}}, *summary.MDMDiskEncryptionSummary)

	// trigger escrow process from device
	// should fail because default Orbit version is too old
	res = s.DoRawNoAuth("POST", fmt.Sprintf("/api/latest/mobius/device/%s/mdm/linux/trigger_escrow", deviceToken), nil, http.StatusBadRequest)
	res.Body.Close()

	// should succeed now that Orbit version isn't too old
	require.NoError(t, s.ds.SetOrUpdateHostOrbitInfo(context.Background(), noTeamHost.ID, mobius.MinOrbitLUKSVersion, sql.NullString{}, sql.NullBool{}))
	res = s.DoRawNoAuth("POST", fmt.Sprintf("/api/latest/mobius/device/%s/mdm/linux/trigger_escrow", deviceToken), nil, http.StatusNoContent)
	res.Body.Close()

	// confirm that Orbit endpoint shows notification flag
	var orbitResponse orbitGetConfigResponse
	s.DoJSON("POST", "/api/mobius/orbit/config", orbitGetConfigRequest{OrbitNodeKey: orbitKey}, http.StatusOK, &orbitResponse)
	...
	s.DoJSON("POST", "/api/mobius/orbit/config", orbitGetConfigRequest{OrbitNodeKey: orbitKey}, http.StatusOK, &secondOrbitResponse)
	...
	s.Do("POST", "/api/mobius/orbit/luks_data", orbitPostLUKSRequest{
	...
	// confirm verified
	s.DoJSON("GET", "/api/latest/mobius/disk_encryption", getMDMDiskEncryptionSummaryRequest{}, http.StatusOK, &summary)
	require.Equal(t, mobius.MDMDiskEncryptionSummary{Verified: mobius.MDMPlatformsCounts{Linux: 1}}, *summary.MDMDiskEncryptionSummary)

	// get passphrase back
	var keyResponse getHostEncryptionKeyResponse
	s.DoJSON("GET", fmt.Sprintf(`/api/latest/mobius/mdm/hosts/%d/encryption_key`, noTeamHost.ID), getHostEncryptionKeyRequest{}, http.StatusOK, &keyResponse)
	s.DoJSON("GET", fmt.Sprintf(`/api/latest/mobius/hosts/%d/encryption_key`, noTeamHost.ID), getHostEncryptionKeyRequest{}, http.StatusOK, &keyResponse)
	require.Equal(t, "whale makes pail rise", keyResponse.EncryptionKey.DecryptedValue)

	// TEAM //
	s.DoJSON("GET", "/api/latest/mobius/configuration_profiles/summary", getMDMProfilesSummaryRequest{TeamID: teamID}, http.StatusOK, &profileSummary)
	require.Equal(t, mobius.MDMProfilesSummary{}, profileSummary.MDMProfilesSummary)
	...
	s.Do("POST", "/api/latest/mobius/disk_encryption", updateDiskEncryptionRequest{TeamID: teamID, EnableDiskEncryption: true}, http.StatusNoContent)
	...
	s.DoJSON("GET", "/api/latest/mobius/disk_encryption", getMDMDiskEncryptionSummaryRequest{TeamID: teamID}, http.StatusOK, &summary)
	require.Equal(t, mobius.MDMDiskEncryptionSummary{ActionRequired: mobius.MDMPlatformsCounts{Linux: 1}}, *summary.MDMDiskEncryptionSummary)
	...
	// get passphrase back
	s.DoJSON("GET", fmt.Sprintf(`/api/latest/mobius/hosts/%d/encryption_key`, teamHost.ID), getHostEncryptionKeyRequest{}, http.StatusOK, &keyResponse)
...
func (s *integrationEnterpriseTestSuite) TestListDevicePolicies() {
	...
	s.DoJSON("PATCH", "/api/latest/mobius/config",
		json.RawMessage(`{
		"org_info":{
	...
	team, err := s.ds.NewTeam(ctx, &mobius.Team{
		ID:          51,
		Name:        "team1-policies",
		Description: "desc team1",
	})
	...
	qr, err := s.ds.NewQuery(ctx, &mobius.Query{
	...
	s.DoJSON("POST", "/api/latest/mobius/policies", gpParams, http.StatusOK, &gpResp)
	...
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobius/teams/%d/policies", team.ID), tpParams, http.StatusOK, &tpResp)

	// try with invalid token
	res := s.DoRawNoAuth("GET", "/api/latest/mobius/device/invalid_token/policies", nil, http.StatusUnauthorized)
	err = res.Body.Close()
	require.NoError(t, err)

	// GET `/api/_version_/mobius/device/{token}/policies`
	listDevicePoliciesResp := listDevicePoliciesResponse{}
	res = s.DoRawNoAuth("GET", "/api/latest/mobius/device/"+token+"/policies", nil, http.StatusOK)
	...
	// GET `/api/_version_/mobius/device/{token}`
	getDeviceHostResp := getDeviceHostResponse{}
	res = s.DoRawNoAuth("GET", "/api/latest/mobius/device/"+token, nil, http.StatusOK)
	err = json.NewDecoder(res.Body).Decode(&getDeviceHostResp)
	require.NoError(t, err)
	err = res.Body.Close()
	require.NoError(t, err)
	...
	// GET `/api/_version_/mobius/device/{token}/desktop`
	getDesktopResp := mobiusDesktopResponse{}
	res = s.DoRawNoAuth("GET", "/api/latest/mobius/device/"+token+"/desktop", nil, http.StatusOK)
...
// TestCustomTransparencyURL tests that Mobius Premium licensees can use custom transparency urls.
func (s *integrationEnterpriseTestSuite) TestCustomTransparencyURL() {
	...
	s.DoJSON("GET", "/api/latest/mobius/config", nil, http.StatusOK, &acResp)
	require.NotNil(t, acResp)
	require.Equal(t, mobius.DefaultTransparencyURL, acResp.MobiusDesktop.TransparencyURL)

	// confirm device endpoint returns initial default url
	deviceResp := &transparencyURLResponse{}
	rawResp := s.DoRawNoAuth("GET", "/api/latest/mobius/device/"+token+"/transparency", nil, http.StatusTemporaryRedirect)
	...
	require.Equal(t, mobius.DefaultTransparencyURL, rawResp.Header.Get("Location"))

	// set custom url
	acResp = appConfigResponse{}
	s.DoJSON("PATCH", "/api/latest/mobius/config", json.RawMessage(`{"mobius_desktop":{"transparency_url": "customURL"}}`), http.StatusOK, &acResp)
	require.NotNil(t, acResp)
	require.Equal(t, "customURL", acResp.MobiusDesktop.TransparencyURL)
	...
	acResp = appConfigResponse{}
	s.DoJSON("PATCH", "/api/latest/mobius/config", json.RawMessage(`{"mobius_desktop":{"transparency_url": ""}}`), http.StatusOK, &acResp)
	require.NotNil(t, acResp)
	require.Equal(t, mobius.DefaultTransparencyURL, acResp.MobiusDesktop.TransparencyURL)
...
func (s *integrationEnterpriseTestSuite) TestMDMWindowsUpdates() {
	...
	s.DoJSON("GET", "/api/latest/mobius/activities", nil, http.StatusOK, &activitiesResp, "order_key", "a.id", "order_direction", "desc")
	...
	checkInvalidConfig := func(config string) {
		...
		acResp := appConfigResponse{}
		s.DoJSON("PATCH", "/api/latest/mobius/config", json.RawMessage(config), http.StatusUnprocessableEntity, &acResp)

		// get the appconfig, nothing changed
		acResp = appConfigResponse{}
		s.DoJSON("GET", "/api/latest/mobius/config", nil, http.StatusOK, &acResp)
		require.Equal(t, mobius.WindowsUpdates{DeadlineDays: optjson.Int{Set: true}, GracePeriodDays: optjson.Int{Set: true}}, acResp.MDM.WindowsUpdates)
	...
	// valid config
	acResp := appConfigResponse{}
	s.DoJSON("PATCH", "/api/latest/mobius/config", json.RawMessage(`{
			"mdm": {
				"windows_updates": {
					"deadline_days": 5,
					"grace_period_days": 1
	...
	checkWindowsOSUpdatesProfile(t, s.ds, nil, &mobius.WindowsUpdates{
		DeadlineDays:    optjson.SetInt(5),
		GracePeriodDays: optjson.SetInt(1),
	})

	// edited windows updates activity got created
	s.lastActivityMatches(mobius.ActivityTypeEditedWindowsUpdates{}.ActivityName(), `{"deadline_days":5, "grace_period_days":1, "team_id": null, "team_name": null}`, 0)
	...
	lastActivity = s.lastActivityMatches(mobius.ActivityTypeEditedWindowsUpdates{}.ActivityName(), `{"deadline_days":6, "grace_period_days":1, "team_id": null, "team_name": null}`, 0)
	...
	lastActivity = s.lastActivityMatches(mobius.ActivityTypeEditedWindowsUpdates{}.ActivityName(), `{"deadline_days":null, "grace_period_days":null, "team_id": null, "team_name": null}`, 0)
...
func (s *integrationEnterpriseTestSuite) TestMDMAppleOSUpdates() {
	...
	checkInvalidConfig := func(config string) {
		...
		acResp = appConfigResponse{}
		s.DoJSON("GET", "/api/latest/mobius/config", nil, http.StatusOK, &acResp)
		require.Equal(t, mobius.AppleOSUpdateSettings{MinimumVersion: optjson.String{Set: true}, Deadline: optjson.String{Set: true}}, acResp.MDM.MacOSUpdates)
	...
	s.lastActivityOfTypeMatches(mobius.ActivityTypeEditedMacOSMinVersion{}.ActivityName(), `{"deadline":"2022-01-01", "minimum_version":"12.3.1", "team_id": null, "team_name": null}`, 0)
	s.lastActivityOfTypeMatches(mobius.ActivityTypeEditedIOSMinVersion{}.ActivityName(), `{"deadline":"2023-03-03", "minimum_version":"13.13.13", "team_id": null, "team_name": null}`, 0)
	s.lastActivityOfTypeMatches(mobius.ActivityTypeEditedIPadOSMinVersion{}.ActivityName(), `{"deadline":"2024-04-04", "minimum_version":"14.14.14", "team_id": null, "team_name": null}`, 0)
	s.assertAppleOSUpdatesDeclaration(nil, mdm.MobiusMacOSUpdatesProfileName, &mobius.AppleOSUpdateSettings{
		MinimumVersion: optjson.SetString("12.3.1"), Deadline: optjson.SetString("2022-01-01"),
	})
	s.assertAppleOSUpdatesDeclaration(nil, mdm.MobiusIOSUpdatesProfileName, &mobius.AppleOSUpdateSettings{
		MinimumVersion: optjson.SetString("13.13.13"), Deadline: optjson.SetString("2023-03-03"),
	})
	s.assertAppleOSUpdatesDeclaration(nil, mdm.MobiusIPadOSUpdatesProfileName, &mobius.AppleOSUpdateSettings{
	...
	s.lastActivityOfTypeMatches(mobius.ActivityTypeEditedMacOSMinVersion{}.ActivityName(), `{"deadline":"2024-01-01", "minimum_version":"12.3.1", "team_id": null, "team_name": null}`, 0)
	s.lastActivityOfTypeMatches(mobius.ActivityTypeEditedIOSMinVersion{}.ActivityName(), `{"deadline":"2025-05-05", "minimum_version":"13.13.13", "team_id": null, "team_name": null}`, 0)
	lastActivity = s.lastActivityOfTypeMatches(mobius.ActivityTypeEditedIPadOSMinVersion{}.ActivityName(), `{"deadline":"2026-06-06", "minimum_version":"14.14.14", "team_id": null, "team_name": null}`, 0)
	s.assertAppleOSUpdatesDeclaration(nil, mdm.MobiusMacOSUpdatesProfileName, &mobius.AppleOSUpdateSettings{
	...
	s.lastActivityOfTypeMatches(mobius.ActivityTypeEditedMacOSMinVersion{}.ActivityName(), `{"deadline":"", "minimum_version":"", "team_id": null, "team_name": null}`, 0)
	s.lastActivityOfTypeMatches(mobius.ActivityTypeEditedIOSMinVersion{}.ActivityName(), `{"deadline":"", "minimum_version":"", "team_id": null, "team_name": null}`, 0)
	lastActivity = s.lastActivityOfTypeMatches(mobius.ActivityTypeEditedIPadOSMinVersion{}.ActivityName(), `{"deadline":"", "minimum_version":"", "team_id": null, "team_name": null}`, 0)

	// check DDM profiles were removed
	s.assertAppleOSUpdatesDeclaration(nil, mdm.MobiusMacOSUpdatesProfileName, nil)
	s.assertAppleOSUpdatesDeclaration(nil, mdm.MobiusIOSUpdatesProfileName, nil)
	s.assertAppleOSUpdatesDeclaration(nil, mdm.MobiusIPadOSUpdatesProfileName, nil)
...
func (s *integrationEnterpriseTestSuite) TestInvitedUserMFA() {
	...
	// create valid invite
	createInviteReq := createInviteRequest{InvitePayload: mobius.InvitePayload{
		Email:      ptr.String("some email"),
		Name:       ptr.String("some name"),
		GlobalRole: null.StringFrom(mobius.RoleAdmin),
		MFAEnabled: ptr.Bool(true),
		SSOEnabled: ptr.Bool(true),
	}}
	createInviteResp := createInviteResponse{}
	s.DoJSON("POST", "/api/latest/mobius/invites", createInviteReq, http.StatusConflict, &createInviteResp)
	createInviteReq.SSOEnabled = nil
	s.DoJSON("POST", "/api/latest/mobius/invites", createInviteReq, http.StatusOK, &createInviteResp)
	...
	s.DoJSON("GET", "/api/latest/mobius/invites/"+validInviteToken, nil, http.StatusOK, &verifyInvResp)
	require.Equal(t, validInvite.ID, verifyInvResp.Invite.ID)

	var createFromInviteResp createUserResponse
	s.DoJSON("POST", "/api/latest/mobius/users", mobius.UserPayload{
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/invites/%d", validInvite.ID), updateInviteRequest{
		InvitePayload: mobius.InvitePayload{MFAEnabled: ptr.Bool(true), SSOEnabled: ptr.Bool(false)},
...
func (s *integrationEnterpriseTestSuite) TestSSOJITProvisioning() {
	...
	require.Contains(t, body, "Redirecting to Mobius at  ...")
	...
	activitiesResp := listActivitiesResponse{}
	s.DoJSON("GET", "/api/latest/mobius/activities", nil, http.StatusOK, &activitiesResp)
	require.NoError(t, activitiesResp.Err)
	require.NotEmpty(t, activitiesResp.Activities)
	require.Condition(t, func() bool {
		for _, a := range activitiesResp.Activities {
			if (a.Type == mobius.ActivityTypeUserAddedBySSO{}.ActivityName()) && *a.ActorEmail == auth.UserID() {
				return true
			}
		}
		return false
	...
	assert.Contains(t, auth.AssertionAttributes(), mobius.SAMLAttribute{
	...
	_, err = s.ds.NewTeam(context.Background(), &mobius.Team{
		Name:        "team_" + t.Name(),
	...
	assert.Contains(t, auth.AssertionAttributes(), mobius.SAMLAttribute{
		Name: "MOBIUS_JIT_USER_ROLE_TEAM_1",
		Values: []mobius.SAMLAttributeValue{{
			Value: "maintainer",
		}},
	})
	...
	// MOBIUS_JIT_USER_ROLE_* attributes with value `null` are ignored by Mobius.
	assert.Contains(t, auth.AssertionAttributes(), mobius.SAMLAttribute{
		Name: "MOBIUS_JIT_USER_ROLE_GLOBAL",
		Values: []mobius.SAMLAttributeValue{{
			Value: "null",
...
`)
	...
	team, err := s.ds.NewTeam(context.Background(), &mobius.Team{
		ID:          8324,
		Name:        "team1_" + t.Name(),
	...
	host, err := s.ds.NewHost(context.Background(), &mobius.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
	...
	require.Contains(t, dqResp.Queries, "mobius_detail_query_users")
	require.Contains(t, dqResp.Queries, "mobius_detail_query_software_macos")
	require.NotContains(t, dqResp.Queries, "mobius_additional_query_time")
	...
	s.DoJSON("POST", "/api/osquery/distributed/read", req, http.StatusOK, &dqResp)
	require.NotContains(t, dqResp.Queries, "mobius_detail_query_users")
	require.NotContains(t, dqResp.Queries, "mobius_detail_query_software_macos")
	require.Contains(t, dqResp.Queries, "mobius_additional_query_time")
}
...
func (s *integrationEnterpriseTestSuite) TestListHosts() {
	...
	host1, err := s.ds.NewHost(context.Background(), &mobius.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
		SeenTime:        time.Now().Add(-1 * time.Minute),
	...
	require.NoError(t, err)
	host2, err := s.ds.NewHost(context.Background(), &mobius.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
		SeenTime:        time.Now().Add(-1 * time.Minute),
	...
	require.NoError(t, err)
	host3, err := s.ds.NewHost(context.Background(), &mobius.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
		SeenTime:        time.Now().Add(-1 * time.Minute),
	...
	s.DoJSON("GET", "/api/latest/mobius/hosts", nil, http.StatusOK, &resp)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/labels/%d/hosts", allHostsLabel.ID), nil, http.StatusOK, &resp, "low_disk_space", "32")
	require.Len(t, resp.Hosts, 1)

	resp = listHostsResponse{}
	s.DoJSON("GET", "/api/latest/mobius/hosts", nil, http.StatusOK, &resp, "low_disk_space", "32")
	require.Len(t, resp.Hosts, 1)
	assert.Equal(t, host1.ID, resp.Hosts[0].ID)
	assert.Equal(t, 10.0, resp.Hosts[0].GigsDiskSpaceAvailable)

	resp = listHostsResponse{}
	s.DoJSON("GET", "/api/latest/mobius/hosts", nil, http.StatusOK, &resp, "low_disk_space", "100")
	require.Len(t, resp.Hosts, 2)

	// returns an error when the low_disk_space value is invalid (outside 1-100)
	s.DoJSON("GET", "/api/latest/mobius/hosts/count", nil, http.StatusBadRequest, &resp, "low_disk_space", "101")
	s.DoJSON("GET", "/api/latest/mobius/hosts/count", nil, http.StatusBadRequest, &resp, "low_disk_space", "0")

	// counting hosts works with and without the filter too
	var countResp countHostsResponse
	s.DoJSON("GET", "/api/latest/mobius/hosts/count", nil, http.StatusOK, &countResp)
	require.Equal(t, 3, countResp.Count)
	s.DoJSON("GET", "/api/latest/mobius/hosts/count", nil, http.StatusOK, &countResp, "low_disk_space", "32")
	require.Equal(t, 1, countResp.Count)
	s.DoJSON("GET", "/api/latest/mobius/hosts/count", nil, http.StatusOK, &countResp, "low_disk_space", "100")
	require.Equal(t, 2, countResp.Count)

	// host summary returns counts for low disk space
	var summaryResp getHostSummaryResponse
	s.DoJSON("GET", "/api/latest/mobius/host_summary", nil, http.StatusOK, &summaryResp, "low_disk_space", "32")
	...
	s.DoJSON("GET", "/api/latest/mobius/host_summary", nil, http.StatusOK, &summaryResp, "platform", "windows", "low_disk_space", "32")
	...
	summaryResp = getHostSummaryResponse{}
	s.DoJSON("GET", "/api/latest/mobius/host_summary", nil, http.StatusOK, &summaryResp, "team_id", "1", "platform", "linux", "low_disk_space", "32")
	...
	summaryResp = getHostSummaryResponse{}
	s.DoJSON("GET", "/api/latest/mobius/host_summary", nil, http.StatusOK, &summaryResp, "team_id", "1", "platform", "linux")
	require.Equal(t, uint(0), summaryResp.TotalsHostsCount)
	...
	inserted, err := s.ds.InsertSoftwareVulnerability(context.Background(), mobius.SoftwareVulnerability{
	...
	resp = listHostsResponse{}
	s.DoJSON("GET", "/api/latest/mobius/hosts", nil, http.StatusOK, &resp, "populate_software", "true")
	require.Len(t, resp.Hosts, 3)
	for _, h := range resp.Hosts {
		if h.ID == host1.ID {
	...
	resp = listHostsResponse{}
	s.DoJSON("GET", "/api/latest/mobius/hosts", nil, http.StatusOK, &resp, "populate_software", "without_vulnerability_details")
	require.Len(t, resp.Hosts, 3)
	for _, h := range resp.Hosts {
		if h.ID == host1.ID {
			require.NotEmpty(t, h.Software)
	...
	resp = listHostsResponse{}
	s.DoJSON("GET", "/api/latest/mobius/hosts", nil, http.StatusOK, &resp, "populate_software", "false")
	require.Len(t, resp.Hosts, 3)
	for _, h := range resp.Hosts {
		require.Empty(t, h.Software)
	}

	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/labels/%d/hosts", allHostsLabel.ID), nil, http.StatusOK, &resp)
	...
	s.DoJSON(
		"GET", "/api/latest/mobius/hosts", nil, http.StatusOK, &resp, "order_key", "issues",
	) // defaults to ascending order (lowest issues to most issues)
	require.Len(t, resp.Hosts, 3)
	assert.Equal(t, host1.ID, resp.Hosts[2].ID)
	s.DoJSON(
		"GET", fmt.Sprintf("/api/latest/mobius/labels/%d/hosts", allHostsLabel.ID), nil, http.StatusOK, &resp, "order_key", "issues",
...
func (s *integrationEnterpriseTestSuite) TestHostHealth() {
	...
	passingTeamPolicy, err := s.ds.NewTeamPolicy(context.Background(), team.ID, nil, mobius.PolicyPayload{
	...
	failingTeamPolicy, err := s.ds.NewTeamPolicy(context.Background(), team.ID, nil, mobius.PolicyPayload{
	...
	passingGlobalPolicy, err := s.ds.NewGlobalPolicy(context.Background(), nil, mobius.PolicyPayload{
		Name:       "Passing Global Policy",
		Query:      "select 1",
		Resolution: "Run this command to fix it",
	})
	...
	failingGlobalPolicy, err := s.ds.NewGlobalPolicy(context.Background(), nil, mobius.PolicyPayload{
		Name:       "Failing Global Policy",
		Query:      "select 1",
		Resolution: "Run this command to fix it",
		Critical:   false,
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/health", host.ID), nil, http.StatusOK, &hh)
	...
	assert.Contains(t, hh.HostHealth.FailingPolicies, &mobius.HostHealthFailingPolicy{
...
func (s *integrationEnterpriseTestSuite) TestListVulnerabilities() {
	...
	s.DoJSON("GET", "/api/latest/mobius/vulnerabilities", nil, http.StatusOK, &resp)

	// Invalid Order Key
	s.DoJSON("GET", "/api/latest/mobius/vulnerabilities", nil, http.StatusBadRequest, &resp, "order_key", "foo", "order_direction", "asc")

	// EE Only Order Key
	s.DoJSON("GET", "/api/latest/mobius/vulnerabilities", nil, http.StatusOK, &resp, "order_key", "cvss_score", "order_direction", "asc")
	...
	err = s.ds.UpdateHostOperatingSystem(context.Background(), host.ID, mobius.OperatingSystem{
	...
	_, err = s.ds.InsertOSVulnerability(context.Background(), mobius.OSVulnerability{
		OSID:              os.ID,
		CVE:               "CVE-2021-1234",
	...
	res, err := s.ds.UpdateHostSoftware(context.Background(), host.ID, []mobius.Software{
		{Name: "foo", Version: "0.0.1", Source: "chrome_extensions"},
	})
	require.NoError(t, err)
	sw := res.Inserted[0]

	_, err = s.ds.InsertSoftwareVulnerability(context.Background(), mobius.SoftwareVulnerability{
		SoftwareID: sw.ID,
		CVE:        "CVE-2021-1235",
	}, mobius.NVDSource)
	require.NoError(t, err)
	...
	err = s.ds.InsertCVEMeta(context.Background(), []mobius.CVEMeta{
		{
			CVE:              "CVE-2021-1234",
	...
	expected := map[string]struct {
		...
		HostCount   uint
		DetailsLink string
		Source      mobius.VulnerabilitySource
	}{
		"CVE-2021-1234": {
			HostCount:   1,
	...
	// EE Exploit Filter
	s.DoJSON("GET", "/api/latest/mobius/vulnerabilities", nil, http.StatusOK, &resp, "exploit", "true")
	require.Len(t, resp.Vulnerabilities, 1)
	require.Equal(t, "CVE-2021-1234", resp.Vulnerabilities[0].CVE.CVE)

	// Test Team Filter
	s.DoJSON("GET", "/api/latest/mobius/vulnerabilities", nil, http.StatusOK, &resp, "team_id", "1")
	require.Len(s.T(), resp.Vulnerabilities, 0)

	team, err := s.ds.NewTeam(context.Background(), &mobius.Team{Name: "team1"})
	require.NoError(t, err)
	err = s.ds.AddHostsToTeam(context.Background(), &team.ID, []uint{host.ID})
	require.NoError(t, err)
	...
	s.DoJSON("GET", "/api/latest/mobius/vulnerabilities", nil, http.StatusOK, &resp, "team_id", fmt.Sprintf("%d", team.ID))
	...
	s.DoJSON("GET", "/api/latest/mobius/vulnerabilities/CVE-2021-1234", nil, http.StatusOK, &gResp)
...
func (s *integrationEnterpriseTestSuite) TestOSVersions() {
	...
	testOS := mobius.OperatingSystem{Name: "Windows 11 Pro", Version: "10.0.22621.2861", Arch: "x86_64", KernelVersion: "10.0.22621.2861", Platform: "windows"}
	...
	resp = listHostsResponse{}
	s.DoJSON("GET", "/api/latest/mobius/hosts", nil, http.StatusOK, &resp, "os_name", testOS.Name, "os_version", testOS.Version)
	require.Len(t, resp.Hosts, 1)

	expected := resp.Hosts[0]
	resp = listHostsResponse{}
	s.DoJSON("GET", "/api/latest/mobius/hosts", nil, http.StatusOK, &resp, "os_id", fmt.Sprintf("%d", osinfo.ID))
	require.Len(t, resp.Hosts, 1)
	...
	s.DoJSON("GET", "/api/latest/mobius/os_versions", nil, http.StatusOK, &osVersionsResp)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/os_versions/%d", osinfo.OSVersionID), nil, http.StatusOK, &osVersionResp)
	require.Equal(t, &expectedOSVersion, osVersionResp.OSVersion)

	// OS versions with invalid team
	s.DoJSON(
		"GET", fmt.Sprintf("/api/latest/mobius/os_versions/%d", osinfo.OSVersionID), nil, http.StatusNotFound, &osVersionResp, "team_id",
		"99999",
	)
	...
	tr := teamResponse{}
	s.DoJSON(
		"POST", "/api/latest/mobius/teams", createTeamRequest{
			TeamPayload: mobius.TeamPayload{
	...
	s.DoJSON(
		"GET", fmt.Sprintf("/api/latest/mobius/os_versions/%d", osinfo.OSVersionID), nil, http.StatusOK, &osVersionResp, "team_id",
	...
	team0, err := s.ds.NewTeam(context.Background(), &mobius.Team{Name: "new team"})
	require.NoError(t, err)
	require.NoError(t, s.ds.AddHostsToTeam(context.Background(), &team0.ID, []uint{hosts[0].ID}))
	s.DoJSON("GET", "/api/latest/mobius/os_versions", nil, http.StatusOK, &osVersionsResp, "team_id", fmt.Sprintf("%d", team0.ID))
	require.Len(t, osVersionsResp.OSVersions, 0)

	// return err if team_id is invalid
	s.DoJSON("GET", "/api/latest/mobius/os_versions", nil, http.StatusBadRequest, &osVersionsResp, "team_id", "invalid")

	// Create another team and a team user
	team1, err := s.ds.NewTeam(
		context.Background(), &mobius.Team{
			ID:          42,
	...
	s.DoJSON(
		"GET", fmt.Sprintf("/api/latest/mobius/os_versions/%d", osinfo.OSVersionID), nil, http.StatusForbidden, &osVersionResp, "team_id",
		"99999",
	)

	// team user doesn't have acess to "no team"
	osVersionsResp = osVersionsResponse{}
	s.DoJSON("GET", "/api/latest/mobius/os_versions", nil, http.StatusForbidden, &osVersionsResp, "team_id", "0")
	require.Len(t, osVersionsResp.OSVersions, 0)
	...
	osVersionsResp = osVersionsResponse{}
	s.DoJSON("GET", "/api/latest/mobius/os_versions", nil, http.StatusOK, &osVersionsResp, "team_id", "0")
	require.Len(t, osVersionsResp.OSVersions, 0)
	osVersionsResp = osVersionsResponse{}
	...
	require.Len(t, osVersionsResp.OSVersions, 1)
	osVersionResp = getOSVersionResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/os_versions/%d", osinfo.OSVersionID), nil, http.StatusOK, &osVersionResp, "team_id", "0")
	require.Equal(t, &expectedOSVersion, osVersionResp.OSVersion)
...
func (s *integrationEnterpriseTestSuite) TestMDMNotConfiguredEndpoints() {
	...
	for _, route := range mdmConfigurationRequiredEndpoints() {
		var expectedErr mobius.ErrWithStatusCode = mobius.ErrMDMNotConfigured
		path := route.path
		if slices.Contains(windowsOnly, path) {
			expectedErr = mobius.ErrWindowsMDMNotConfigured
		}
	...
	mobiusdmSrv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
	}))
	t.Setenv("TEST_MOBIUSDAEMONM_API_URL", mobiusdmSrv.URL)
	t.Cleanup(mobiusdmSrv.Close)

	// Always accessible
	var reqCSRResp requestMDMAppleCSRResponse
	s.DoJSON("POST", "/api/latest/mobius/mdm/apple/request_csr", requestMDMAppleCSRRequest{EmailAddress: "a@b.c", Organization: "test"}, http.StatusOK, &reqCSRResp)
	s.Do("POST", "/api/latest/mobius/mdm/apple/dep/key_pair", nil, http.StatusOK)

	// setting enable release device manually requires MDM
	res := s.Do("PATCH", "/api/v1/mobius/setup_experience", mobius.MDMAppleSetupPayload{EnableReleaseDeviceManually: ptr.Bool(true)}, http.StatusBadRequest)
	errMsg := extractServerErrorText(res.Body)
	require.Contains(t, errMsg, mobius.ErrMDMNotConfigured.Error())

	res = s.Do("PATCH", "/api/v1/mobius/config", json.RawMessage(`{
...
func (s *integrationEnterpriseTestSuite) TestGlobalPolicyCreateReadPatch() {
	...
	s.DoJSON("POST", "/api/latest/mobius/policies", createPol1Req, http.StatusOK, &createPol1)
	...
	s.DoJSON("POST", "/api/latest/mobius/policies", createPol2Req, http.StatusOK, &createPol2)
	allEqual(t, createPol2Req, createPol2.Policy, fields...)

	listPol := &listGlobalPoliciesResponse{}
	s.DoJSON("GET", "/api/latest/mobius/policies", nil, http.StatusOK, listPol)
	...
	s.DoJSON("GET", "/api/latest/mobius/policies", nil, http.StatusOK, listPolByName, "query", " name1 ")
	...
	patchPol1Req := &modifyGlobalPolicyRequest{
		ModifyPolicyPayload: mobius.ModifyPolicyPayload{
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/policies/%d", createPol1.Policy.ID), patchPol1Req, http.StatusOK, patchPol1)
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/policies/%d", createPol2.Policy.ID), patchPol2Req, http.StatusOK, patchPol2)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/policies/%d", createPol2.Policy.ID), nil, http.StatusOK, getPol2)
...
func (s *integrationEnterpriseTestSuite) TestTeamPolicyCreateReadPatch() {
	...
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobius/teams/%d/policies", team1.ID), createPol1Req, http.StatusOK, &createPol1)
	...
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobius/teams/%d/policies", team1.ID), createPol2Req, http.StatusOK, &createPol2)
	allEqual(s.T(), createPol2Req, createPol2.Policy, fields...)

	listPol := &listTeamPoliciesResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/teams/%d/policies", team1.ID), nil, http.StatusOK, listPol)
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/teams/%d/policies/%d", team1.ID, createPol1.Policy.ID), patchPol1Req, http.StatusOK, patchPol1)
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/teams/%d/policies/%d", team1.ID, createPol2.Policy.ID), patchPol2Req, http.StatusOK, patchPol2)
	...
	getPol2 := &getPolicyByIDResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/teams/%d/policies/%d", team1.ID, createPol2.Policy.ID), nil, http.StatusOK, getPol2)
...
func (s *integrationEnterpriseTestSuite) TestResetAutomation() {
	...
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobius/teams/%d/policies", team1.ID), createPol3Req, http.StatusOK, &createPol3)

	var tmResp teamResponse
	// modify the team's config - enable the webhook
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/teams/%d", team1.ID), mobius.TeamPayload{WebhookSettings: &mobius.TeamWebhookSettings{
		FailingPoliciesWebhook: mobius.FailingPoliciesWebhookSettings{
			Enable:         true,
	...
	h1, err := s.ds.NewHost(ctx, &mobius.Host{})
	require.NoError(s.T(), err)
	...
	s.DoJSON("POST", "/api/latest/mobius/automations/reset", resetAutomationRequest{
		TeamIDs:   nil,
		PolicyIDs: []uint{},
	}, http.StatusOK, &tmResp)
...
func createHostAndDeviceToken(t *testing.T, ds *mysql.Datastore, token string) *mobius.Host {
...
func (s *integrationEnterpriseTestSuite) TestListSoftware() {
	...
	host, err := s.ds.NewHost(ctx, &mobius.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
		SeenTime:        time.Now(),
	...
	inserted, err := s.ds.InsertSoftwareVulnerability(
		ctx, mobius.SoftwareVulnerability{
			SoftwareID:        bar.ID,
			CVE:               "cve-123",
	...
	require.NoError(t, s.ds.InsertCVEMeta(ctx, []mobius.CVEMeta{{
		CVE:              "cve-123",
		CVSSScore:        ptr.Float64(5.4),
		EPSSProbability:  ptr.Float64(0.5),
		CISAKnownExploit: ptr.Bool(true),
	...
	s.DoJSON("GET", "/api/latest/mobius/software", nil, http.StatusOK, &resp)
	require.NotNil(t, resp)

	var fooPayload, barPayload mobius.Software
	...
	s.DoJSON("GET", "/api/latest/mobius/software/versions", nil, http.StatusOK, &respVersions)
	require.NotNil(t, resp)
	...
	s.DoJSON(
		"GET", "/api/latest/mobius/software/versions",
		listSoftwareRequest{},
...
// TestGitOpsUserActions tests the MDM permissions listed in ../../docs/Using\ Mobius/manage-access.md
func (s *integrationEnterpriseTestSuite) TestGitOpsUserActions() {
	...
	require.NoError(t, err)
	t1, err := s.ds.NewTeam(ctx, &mobius.Team{
		Name: "Foo",
	})
	require.NoError(t, err)
	t2, err := s.ds.NewTeam(ctx, &mobius.Team{
		Name: "Bar",
	})
	require.NoError(t, err)
	t3, err := s.ds.NewTeam(ctx, &mobius.Team{
		Name: "Zoo",
	})
	require.NoError(t, err)
	team1Host, err := s.ds.NewHost(ctx, &mobius.Host{
		NodeKey:  ptr.String(t.Name() + "2"),
		UUID:     t.Name() + "2",
	...
	require.NoError(t, err)
	globalHost, err := s.ds.NewHost(ctx, &mobius.Host{
		NodeKey:  ptr.String(t.Name() + "3"),
		UUID:     t.Name() + "3",
	...
	s.DoJSON("GET", "/api/latest/mobius/config", nil, http.StatusOK, &acr)
	require.False(t, acr.WebhookSettings.VulnerabilitiesWebhook.Enable)
	q1, err := s.ds.NewQuery(ctx, &mobius.Query{
		Name:    "Foo",
	...
	s.DoJSON("GET", "/api/latest/mobius/schedule", nil, http.StatusOK, &ggsr)
	require.NoError(t, ggsr.Err)
	cpar := createPackResponse{}
	var userPackID uint
	s.DoJSON("POST", "/api/latest/mobius/packs", createPackRequest{
	...
	s.DoJSON("POST", "/api/latest/mobius/users/admin", createUserRequest{
		UserPayload: mobius.UserPayload{
	...
	s.DoJSON("GET", "/api/latest/mobius/carves", listCarvesRequest{}, http.StatusOK, &lcr)
	...
	require.NoError(t, err)
	gp2, err := s.ds.NewGlobalPolicy(ctx, &admin.ID, mobius.PolicyPayload{
		Name:  "Zoo",
		Query: "SELECT 0;",
	})
	require.NoError(t, err)
	t2p, err := s.ds.NewTeamPolicy(ctx, t2.ID, &admin.ID, mobius.PolicyPayload{
		Name:  "Zoo2",
		Query: "SELECT 2;",
	})
	require.NoError(t, err)
	...
	require.NoError(t, err)
	manualLabel1, err := s.ds.NewLabel(ctx, &mobius.Label{
		Name:                "manualLabel1",
		Query:               "SELECT 2;",
	...
	s.DoJSON("GET", "/api/latest/mobius/activities", nil, http.StatusForbidden, &listActivitiesResponse{})

	// Attempt to retrieve hosts, should fail.
	s.DoJSON("GET", "/api/latest/mobius/hosts", nil, http.StatusForbidden, &listHostsResponse{})

	// Attempt to retrieve a host by identifier should succeed
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/identifier/%s", h1.Hostname), hostByIdentifierRequest{}, http.StatusOK, &getHostResponse{})

	// Attempt to filter hosts using labels, should fail (label ID 6 is the builtin label "All Hosts")
	s.DoJSON("GET", "/api/latest/mobius/labels/6/hosts", nil, http.StatusOK, &listHostsResponse{})

	// Attempt to delete hosts, should fail.
	s.DoJSON("DELETE", "/api/latest/mobius/hosts/1", nil, http.StatusForbidden, &deleteHostResponse{})

	// Attempt to transfer host from global to a team, should allow.
	s.DoJSON("POST", "/api/latest/mobius/hosts/transfer", addHostsToTeamRequest{
		TeamID:  &t1.ID,
		HostIDs: []uint{h1.ID},
	...
	s.DoJSON("POST", "/api/latest/mobius/labels", createLabelRequest{
		LabelPayload: mobius.LabelPayload{
			Name:  "foo",
			Query: "SELECT 1;",
		},
	}, http.StatusOK, &clr)
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/labels/%d", clr.Label.ID), modifyLabelRequest{
		ModifyLabelPayload: mobius.ModifyLabelPayload{
			Name: ptr.String("foo2"),
		},
	}, http.StatusOK, &modifyLabelResponse{})

	// Attempt to get a label, should fail.
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/labels/%d", clr.Label.ID), getLabelRequest{}, http.StatusOK, &getLabelResponse{})

	// Attempt to list all labels, should fail.
	s.DoJSON("GET", "/api/latest/mobius/labels", listLabelsRequest{}, http.StatusOK, &listLabelsResponse{})

	// Attempt to delete a label, should allow.
	s.DoJSON("DELETE", "/api/latest/mobius/labels/foo2", deleteLabelRequest{}, http.StatusOK, &deleteLabelResponse{})

	// Attempt to list all software, should fail.
	s.DoJSON("GET", "/api/latest/mobius/software/versions", listSoftwareRequest{}, http.StatusForbidden, &listSoftwareVersionsResponse{})
	s.DoJSON("GET", "/api/latest/mobius/software", listSoftwareRequest{}, http.StatusForbidden, &listSoftwareResponse{})
	s.DoJSON("GET", "/api/latest/mobius/software/count", countSoftwareRequest{}, http.StatusForbidden, &countSoftwareResponse{})
	s.DoJSON("GET", "/api/latest/mobius/software/titles", listSoftwareTitlesRequest{}, http.StatusForbidden, &listSoftwareTitlesResponse{})
	s.DoJSON("GET", "/api/latest/mobius/software/titles/1", getSoftwareTitleRequest{}, http.StatusForbidden, &getSoftwareTitleResponse{})

	// Attempt to list a software, should fail.
	s.DoJSON("GET", "/api/latest/mobius/software/1", getSoftwareRequest{}, http.StatusForbidden, &getSoftwareResponse{})
	s.DoJSON("GET", "/api/latest/mobius/software/versions/1", getSoftwareRequest{}, http.StatusForbidden, &getSoftwareResponse{})

	// Attempt to read app config, should pass.
	s.DoJSON("GET", "/api/latest/mobius/config", nil, http.StatusOK, &appConfigResponse{})
	...
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobius/hosts/%d/labels", h1.ID), addLabelsToHostRequest{
	...
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobius/hosts/%d/labels", h1.ID), removeLabelsFromHostRequest{
		Labels: []string{manualLabel1.Name},
	}, http.StatusOK, &removeLabelsFromHostResp)

	// Attempt to run live queries synchronously, should fail.
	s.DoJSON("GET", "/api/latest/mobius/queries/run", runLiveQueryRequest{
		HostIDs:  []uint{h1.ID},
		QueryIDs: []uint{q1.ID},
	...
	s.DoJSON("POST", "/api/latest/mobius/queries/run", createDistributedQueryCampaignRequest{
	...
	s.DoJSON("POST", "/api/latest/mobius/queries", createQueryRequest{
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/queries/%d", cqr.Query.ID), modifyQueryRequest{
		QueryPayload: mobius.QueryPayload{
			Name:  ptr.String("foo4"),
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/queries/%d", cqr.Query.ID), getQueryRequest{}, http.StatusOK, &getQueryResponse{})

	// Attempt to list all queries, should work.
	s.DoJSON("GET", "/api/latest/mobius/queries", listQueriesRequest{}, http.StatusOK, &listQueriesResponse{})

	// Attempt to delete queries, should allow.
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobius/queries/id/%d", cqr.Query.ID), deleteQueryByIDRequest{}, http.StatusOK, &deleteQueryByIDResponse{})
	s.DoJSON("POST", "/api/latest/mobius/queries/delete", deleteQueriesRequest{IDs: []uint{cqr2.Query.ID}}, http.StatusOK, &deleteQueriesResponse{})
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobius/queries/%s", cqr3.Query.Name), deleteQueryRequest{}, http.StatusOK, &deleteQueryResponse{})

	// Attempt to add a query to a user pack, should allow.
	sqr := scheduleQueryResponse{}
	s.DoJSON("POST", "/api/latest/mobius/packs/schedule", scheduleQueryRequest{
		PackID:   userPackID,
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/packs/schedule/%d", sqr.Scheduled.ID), modifyScheduledQueryRequest{
		ScheduledQueryPayload: mobius.ScheduledQueryPayload{
			Interval: ptr.Uint(30),
		},
	}, http.StatusOK, &scheduleQueryResponse{})

	// Attempt to remove a query from the global schedule, should allow.
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobius/packs/schedule/%d", sqr.Scheduled.ID), deleteScheduledQueryRequest{}, http.StatusOK, &scheduleQueryResponse{})

	// Attempt to read the global schedule, should allow.
	s.DoJSON("GET", "/api/latest/mobius/schedule", nil, http.StatusOK, &getGlobalScheduleResponse{})
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/packs/%d", cpr.Pack.ID), modifyPackRequest{
		PackPayload: mobius.PackPayload{
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/packs/%d", cpr.Pack.ID), nil, http.StatusOK, &getPackResponse{})

	// Attempt to delete a pack, should allow.
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobius/packs/id/%d", cpr.Pack.ID), deletePackRequest{}, http.StatusOK, &deletePackResponse{})

	// Attempt to create a global policy, should allow.
	gplr := globalPolicyResponse{}
	s.DoJSON("POST", "/api/latest/mobius/policies", globalPolicyRequest{
		Name:  "foo9",
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/policies/%d", gplr.Policy.ID), modifyGlobalPolicyRequest{
		ModifyPolicyPayload: mobius.ModifyPolicyPayload{
	...
	s.DoJSON(
		"GET", fmt.Sprintf("/api/latest/mobius/policies/%d", gplr.Policy.ID), getPolicyByIDRequest{}, http.StatusOK,
		&getPolicyByIDResponse{},
	)

	// Attempt to delete a global policy, should allow.
	s.DoJSON("POST", "/api/latest/mobius/policies/delete", deleteGlobalPoliciesRequest{
	...
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobius/team/%d/policies", t1.ID), teamPolicyRequest{
		Name:  "foo10",
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/teams/%d/policies/%d", t1.ID, tplr.Policy.ID), modifyTeamPolicyRequest{
		ModifyPolicyPayload: mobius.ModifyPolicyPayload{
	...
	s.DoJSON(
		"GET", fmt.Sprintf("/api/latest/mobius/team/%d/policies/%d", t1.ID, tplr.Policy.ID), getTeamPolicyByIDRequest{}, http.StatusOK,
		&getTeamPolicyByIDResponse{},
	)

	// Attempt to delete a team policy, should allow.
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobius/teams/%d/policies/delete", t1.ID), deleteTeamPoliciesRequest{
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/users/%d", admin.ID), modifyUserRequest{
		UserPayload: mobius.UserPayload{
			GlobalRole: ptr.String("observer"),
		},
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/users/%d", admin.ID), getUserRequest{}, http.StatusForbidden, &getUserResponse{})

	// Attempt to delete a user, should fail.
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobius/users/%d", admin.ID), deleteUserRequest{}, http.StatusForbidden, &deleteUserResponse{})

	// Attempt to add users to team, should allow.
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/teams/%d/users", t1.ID), modifyTeamUsersRequest{
		Users: []mobius.TeamUser{
			{
				User: *maintainer,
				Role: "admin",
			},
	...
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobius/teams/%d/users", t1.ID), modifyTeamUsersRequest{
		Users: []mobius.TeamUser{
			{
				User: *maintainer,
				Role: "admin",
	...
	tr := teamResponse{}
	s.DoJSON("POST", "/api/latest/mobius/teams", createTeamRequest{
		TeamPayload: mobius.TeamPayload{
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/teams/%d", tr.Team.ID), modifyTeamRequest{
		TeamPayload: mobius.TeamPayload{
	...
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobius/teams/%d/agent_options", tr.Team.ID), json.RawMessage(`{
		"config": {
			"options": {
				"aws_debug": true
			}
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/teams/%d", tr.Team.ID), getTeamRequest{}, http.StatusForbidden, &teamResponse{})

	// Attempt to delete a team, should allow.
	dtr := deleteTeamResponse{}
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobius/teams/%d", tr.Team.ID), deleteTeamRequest{}, http.StatusOK, &dtr)

	// Attempt to create/edit enroll secrets, should allow.
	s.DoJSON("POST", "/api/latest/mobius/spec/enroll_secret", applyEnrollSecretSpecRequest{
	...
	s.DoJSON("GET", "/api/latest/mobius/spec/enroll_secret", nil, http.StatusForbidden, &getEnrollSecretSpecResponse{})

	// Attempt to get team enroll secret, should fail.
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/teams/%d/secrets", t1.ID), teamEnrollSecretsRequest{}, http.StatusForbidden, &teamEnrollSecretsResponse{})

	// Attempt to list carved files, should fail.
	s.DoJSON("GET", "/api/latest/mobius/carves", listCarvesRequest{}, http.StatusForbidden, &listCarvesResponse{})

	// Attempt to get a carved file, should fail.
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/carves/%d", carveID), listCarvesRequest{}, http.StatusForbidden, &listCarvesResponse{})

	// Attempt to search hosts, should fail.
	s.DoJSON("POST", "/api/latest/mobius/targets", searchTargetsRequest{
		MatchQuery: "foo",
		QueryID:    &q1.ID,
	}, http.StatusForbidden, &searchTargetsResponse{})

	// Attempt to count target hosts, should fail.
	s.DoJSON("POST", "/api/latest/mobius/targets/count", countTargetsRequest{
		Selected: mobius.HostTargets{
			HostIDs:  []uint{h1.ID},
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/queries/%d", tcqr.Query.ID), modifyQueryRequest{
		QueryPayload: mobius.QueryPayload{
			Name:  ptr.String("foo4"),
			Query: ptr.String("SELECT * FROM system_info;"),
		},
	...
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobius/queries/id/%d", tcqr.Query.ID), deleteQueryByIDRequest{}, http.StatusOK, &deleteQueryByIDResponse{})

	// Attempt to edit query created by somebody else, should fail.
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/queries/%d", cqr4.Query.ID), modifyQueryRequest{
		QueryPayload: mobius.QueryPayload{
			Name:  ptr.String("foo4"),
			Query: ptr.String("SELECT * FROM system_info;"),
		},
	...
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobius/queries/id/%d", cqr4.Query.ID), deleteQueryByIDRequest{}, http.StatusForbidden, &deleteQueryByIDResponse{})

	// Attempt to read the global schedule, should fail.
	s.DoJSON("GET", "/api/latest/mobius/schedule", nil, http.StatusForbidden, &getGlobalScheduleResponse{})

	// Attempt to read the team's schedule, should allow.
	s.DoJSON(
		"GET", fmt.Sprintf("/api/latest/mobius/teams/%d/schedule", t1.ID), getTeamScheduleRequest{}, http.StatusOK,
		&getTeamScheduleResponse{},
	)

	// Attempt to read other team's schedule, should fail.
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/teams/%d/schedule", t2.ID), getTeamScheduleRequest{}, http.StatusForbidden, &getTeamScheduleResponse{})
	...
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobius/teams/%d/schedule", t1.ID), teamScheduleQueryRequest{
		ScheduledQueryPayload: mobius.ScheduledQueryPayload{
			QueryID:  ptr.Uint(q1.ID),
	...
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobius/teams/%d/schedule/%d", t1.ID, ttsqr.Scheduled.ID), deleteTeamScheduleRequest{}, http.StatusOK, &deleteTeamScheduleResponse{})

	// Attempt to add/remove a manual label from a team host, should allow.
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobius/hosts/%d/labels", team1Host.ID), addLabelsToHostRequest{
		Labels: []string{manualLabel1.Name},
	}, http.StatusOK, &addLabelsToHostResp)
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobius/hosts/%d/labels", team1Host.ID), removeLabelsFromHostRequest{
		Labels: []string{manualLabel1.Name},
	}, http.StatusOK, &removeLabelsFromHostResp)

	// Attempt to add/remove a manual label from a global host, should not allow.
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobius/hosts/%d/labels", globalHost.ID), addLabelsToHostRequest{
		Labels: []string{manualLabel1.Name},
	}, http.StatusForbidden, &addLabelsToHostResp)
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobius/hosts/%d/labels", globalHost.ID), removeLabelsFromHostRequest{
		Labels: []string{manualLabel1.Name},
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/policies/%d", gp2.ID), getPolicyByIDRequest{}, http.StatusForbidden, &getPolicyByIDResponse{})
	...
	// Attempt to edit a team policy, should allow.
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/teams/%d/policies/%d", t1.ID, ttplr.Policy.ID), modifyTeamPolicyRequest{
		ModifyPolicyPayload: mobius.ModifyPolicyPayload{
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/teams/%d/policies/%d", t2.ID, t2p.ID), modifyTeamPolicyRequest{
		ModifyPolicyPayload: mobius.ModifyPolicyPayload{
			Query: ptr.String("SELECT * from file WHERE path = 'foobar';"),
		},
	...
	// Attempt to view a team policy, should allow.
	s.DoJSON(
		"GET", fmt.Sprintf("/api/latest/mobius/team/%d/policies/%d", t1.ID, ttplr.Policy.ID), getTeamPolicyByIDRequest{}, http.StatusOK,
		&getTeamPolicyByIDResponse{},
	)

	// Attempt to view another team's policy, should fail.
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/team/%d/policies/%d", t2.ID, t2p.ID), getTeamPolicyByIDRequest{}, http.StatusForbidden, &getTeamPolicyByIDResponse{})
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/teams/%d", t1.ID), modifyTeamRequest{
		TeamPayload: mobius.TeamPayload{
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/teams/%d", t2.ID), modifyTeamRequest{
		TeamPayload: mobius.TeamPayload{
			Name: ptr.String("foo123456"),
		},
	...
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobius/teams/%d/agent_options", t1.ID), json.RawMessage(`{
		"config": {
			"options": {
				"aws_debug": true
			}
	...
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobius/teams/%d/agent_options", t2.ID), json.RawMessage(`{
		"config": {
			"options": {
				"aws_debug": true
			}
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/teams/%d/users", t3.ID), modifyTeamUsersRequest{
		Users: []mobius.TeamUser{
			{
				User: *u3,
				Role: "maintainer",
	...
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobius/teams/%d/users", t3.ID), modifyTeamUsersRequest{
		Users: []mobius.TeamUser{
			{
				User: *u3,
			},
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/teams/%d/users", t2.ID), modifyTeamUsersRequest{
		Users: []mobius.TeamUser{
			{
				User: *u3,
				Role: "maintainer",
	...
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobius/teams/%d/users", t2.ID), modifyTeamUsersRequest{
		Users: []mobius.TeamUser{
			{
				User: *u2,
			},
...
func (s *integrationEnterpriseTestSuite) TestDesktopEndpointWithInvalidPolicy() {
	...
	policy, err := s.ds.NewGlobalPolicy(context.Background(), &admin.ID, mobius.PolicyPayload{
	...
	desktopRes := mobiusDesktopResponse{}
	res := s.DoRawNoAuth("GET", "/api/latest/mobius/device/"+token+"/desktop", nil, http.StatusOK)
...
func (s *integrationEnterpriseTestSuite) TestRunHostScript() {
	...
	s.DoJSON("POST", "/api/latest/mobius/scripts/run", mobius.HostScriptRequestPayload{HostID: host.ID + 100, ScriptContents: "echo"}, http.StatusNotFound, &runResp)

	// attempt to run an empty script
	res := s.Do("POST", "/api/latest/mobius/scripts/run", mobius.HostScriptRequestPayload{HostID: host.ID, ScriptContents: ""}, http.StatusUnprocessableEntity)
	errMsg := extractServerErrorText(res.Body)
	...
	res = s.Do("POST", "/api/latest/mobius/scripts/run", mobius.HostScriptRequestPayload{HostID: host.ID, ScriptContents: strings.Repeat("a", mobius.UnsavedScriptMaxRuneLen+1)}, http.StatusUnprocessableEntity)
	errMsg = extractServerErrorText(res.Body)
	...
	res = s.Do("POST", "/api/latest/mobius/scripts/run", mobius.HostScriptRequestPayload{HostID: host.ID, ScriptContents: "echo $MOBIUS_SECRET_INVALID"}, http.StatusUnprocessableEntity)
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, `$MOBIUS_SECRET_INVALID`)

	// Upload a valid secret
	secretValue := "abc123"
	req := secretVariablesRequest{
		SecretVariables: []mobius.SecretVariable{
			{
				Name:  "MOBIUS_SECRET_TestRunHostScript",
				Value: secretValue,
			},
		},
	}
	secretResp := secretVariablesResponse{}
	s.DoJSON("PUT", "/api/latest/mobius/spec/secret_variables", req, http.StatusOK, &secretResp)

	// create a valid script execution request
	expectedScriptContents := "echo ${MOBIUS_SECRET_TestRunHostScript}"
	expectedScriptContentsWithSecret := fmt.Sprintf("echo %s", secretValue)
	s.DoJSON("POST", "/api/latest/mobius/scripts/run",
		mobius.HostScriptRequestPayload{HostID: host.ID, ScriptContents: expectedScriptContents}, http.StatusAccepted, &runResp)
	...
	s.DoJSON("GET", "/api/latest/mobius/scripts/results/"+runResp.ExecutionID, nil, http.StatusOK, &scriptResultResp)
	...
	require.Contains(t, scriptResultResp.Message, mobius.RunScriptAsyncScriptEnqueuedMsg)
	...
	s.DoJSON("POST", "/api/mobius/orbit/config",
	...
	srResp := s.Do("POST", "/api/latest/mobius/scripts/run/sync", mobius.HostScriptRequestPayload{HostID: host.ID, ScriptContents: "echo"}, http.StatusForbidden)
	assertBodyContains(t, srResp, mobius.RunScriptScriptsDisabledGloballyErrMsg)

	srResp = s.Do("POST", "/api/latest/mobius/scripts/run", mobius.HostScriptRequestPayload{HostID: host.ID, ScriptContents: "echo"}, http.StatusForbidden)
	assertBodyContains(t, srResp, mobius.RunScriptScriptsDisabledGloballyErrMsg)
	...
	s.DoJSON("POST", "/api/mobius/orbit/scripts/request",
	...
	s.DoJSON("POST", "/api/latest/mobius/scripts/run/sync", mobius.HostScriptRequestPayload{HostID: host.ID + 100, ScriptContents: "echo"}, http.StatusNotFound, &runSyncResp)

	// attempt to sync run an empty script
	res = s.Do("POST", "/api/latest/mobius/scripts/run/sync", mobius.HostScriptRequestPayload{HostID: host.ID, ScriptContents: ""}, http.StatusUnprocessableEntity)
	errMsg = extractServerErrorText(res.Body)
	...
	res = s.Do("POST", "/api/latest/mobius/scripts/run/sync", mobius.HostScriptRequestPayload{HostID: host.ID, ScriptContents: strings.Repeat("a", mobius.UnsavedScriptMaxRuneLen+1)}, http.StatusUnprocessableEntity)
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, "Script is too large. It's limited to 10,000 characters")
	...
	res = s.Do("POST", "/api/latest/mobius/scripts/run/sync", mobius.HostScriptRequestPayload{HostID: host.ID, ScriptContents: "echo"}, http.StatusConflict)
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, mobius.RunScriptAlreadyRunningErrMsg)

	// save a result via the orbit endpoint
	var orbitPostScriptResp orbitPostScriptResultResponse
	s.DoJSON("POST", "/api/mobius/orbit/scripts/result",
	...
	s.DoJSON("POST", "/api/latest/mobius/scripts/run/sync", mobius.HostScriptRequestPayload{HostID: host.ID, ScriptContents: "echo"}, http.StatusRequestTimeout, &runSyncResp)
	...
	require.Contains(t, runSyncResp.Message, mobius.RunScriptHostTimeoutErrMsg)
	...
	resultsCh := make(chan *mobius.HostScriptResultPayload, 1)
	go func() {
	...
	resultsCh <- &mobius.HostScriptResultPayload{
		HostID:   host.ID,
		Output:   "ok",
		Runtime:  1,
		ExitCode: 0,
	...
	runSyncResp = runScriptSyncResponse{}
	s.DoJSON("POST", "/api/latest/mobius/scripts/run/sync", mobius.HostScriptRequestPayload{HostID: host.ID, ScriptContents: "echo"}, http.StatusOK, &runSyncResp)
	require.Equal(t, host.ID, runSyncResp.HostID)
	require.NotEmpty(t, runSyncResp.ExecutionID)
	...
	scriptResultResp = getScriptResultResponse{}
	s.DoJSON("GET", "/api/latest/mobius/scripts/results/"+runSyncResp.ExecutionID, nil, http.StatusOK, &scriptResultResp)
	require.Equal(t, host.ID, scriptResultResp.HostID)
	require.Equal(t, "echo", scriptResultResp.ScriptContents)
	require.Nil(t, scriptResultResp.ExitCode)
	require.True(t, scriptResultResp.HostTimeout)
	require.Contains(t, scriptResultResp.Message, mobius.RunScriptHostTimeoutErrMsg)
	...
	require.NoError(t, err)
	s.DoJSON(
		"POST", "/api/latest/mobius/scripts/run", mobius.HostScriptRequestPayload{HostID: host.ID, ScriptContents: "echo"},
		http.StatusUnprocessableEntity, &runResp,
	)
	s.DoJSON(
		"POST", "/api/latest/mobius/scripts/run/sync", mobius.HostScriptRequestPayload{HostID: host.ID, ScriptContents: "echo"},
		http.StatusUnprocessableEntity, &runResp,
	)
	...
	// is offline.
	res = s.Do("POST", "/api/latest/mobius/scripts/run/sync", mobius.HostScriptRequestPayload{HostID: host.ID, ScriptContents: "echo"}, http.StatusUnprocessableEntity)
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, mobius.RunScriptHostOfflineErrMsg)

	// attempt to create an async script execution request, succeeds because script is added to queue.
	s.Do("POST", "/api/latest/mobius/scripts/run", mobius.HostScriptRequestPayload{HostID: host.ID, ScriptContents: "echo"}, http.StatusAccepted)

	// attempt to run a script on a plain osquery host
	plainOsqueryHost, err := s.ds.NewHost(context.Background(), &mobius.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
	...
	require.NoError(t, err)
	res = s.Do("POST", "/api/latest/mobius/scripts/run", mobius.HostScriptRequestPayload{HostID: plainOsqueryHost.ID, ScriptContents: "echo"}, http.StatusUnprocessableEntity)
	require.Contains(t, extractServerErrorText(res.Body), mobius.RunScriptDisabledErrMsg)
	res = s.Do("POST", "/api/latest/mobius/scripts/run/sync", mobius.HostScriptRequestPayload{HostID: plainOsqueryHost.ID, ScriptContents: "echo"}, http.StatusUnprocessableEntity)
	require.Contains(t, extractServerErrorText(res.Body), mobius.RunScriptDisabledErrMsg)

	// create a execution request that will return a timeout
	s.DoJSON("POST", "/api/latest/mobius/scripts/run", mobius.HostScriptRequestPayload{HostID: host.ID, ScriptContents: "echo"}, http.StatusAccepted, &runResp)
	...
	require.Equal(t, "Timeout. Mobius stopped the script after 900 seconds to protect host performance.", scriptResultResp.Message)
...
func (s *integrationEnterpriseTestSuite) TestRunBatchScript() {
	...
	team1, err := s.ds.NewTeam(ctx, &mobius.Team{
		Name: "Team 1",
	})
	require.NoError(t, err)
	...
	script, err := s.ds.NewScript(ctx, &mobius.Script{
		Name:           "script.sh",
	...
	// Team mismatch
	s.DoJSON("POST", "/api/latest/mobius/scripts/run/batch", batchScriptRunRequest{
		ScriptID: script.ID,
	...
	s.lastActivityOfTypeMatches(
		mobius.ActivityTypeRanScriptBatch{}.ActivityName(),
...
func (s *integrationEnterpriseTestSuite) TestRunHostSavedScript() {
	...
	host := createOrbitEnrolledHost(t, "linux", "", s.ds)
	tm, err := s.ds.NewTeam(ctx, &mobius.Team{Name: "team 1"})
	require.NoError(t, err)
	savedNoTmScript, err := s.ds.NewScript(ctx, &mobius.Script{
		TeamID:         nil,
		Name:           "no_team_script.sh",
		ScriptContents: "echo 'no team'",
	})
	require.NoError(t, err)
	savedTmScript, err := s.ds.NewScript(ctx, &mobius.Script{
		TeamID:         &tm.ID,
		Name:           "team_script.sh",
	...
	// attempt to run a script on a non-existing host
	var runResp runScriptResponse
	s.DoJSON("POST", "/api/latest/mobius/scripts/run", mobius.HostScriptRequestPayload{HostID: host.ID + 100, ScriptID: &savedNoTmScript.ID}, http.StatusNotFound, &runResp)

	// attempt to run with both script contents and id
	res := s.Do("POST", "/api/latest/mobius/scripts/run", mobius.HostScriptRequestPayload{HostID: host.ID, ScriptContents: "echo", ScriptID: ptr.Uint(savedTmScript.ID + 999)}, http.StatusUnprocessableEntity)
	errMsg := extractServerErrorText(res.Body)
	require.Contains(t, errMsg, `Only one of 'script_id' or 'script_contents' is allowed.`)

	// attempt to run with unknown script id
	res = s.Do("POST", "/api/latest/mobius/scripts/run", mobius.HostScriptRequestPayload{HostID: host.ID, ScriptID: ptr.Uint(savedTmScript.ID + 999)}, http.StatusNotFound)
	errMsg = extractServerErrorText(res.Body)
	...
	res = s.Do("POST", "/api/latest/mobius/scripts/run", mobius.HostScriptRequestPayload{HostID: host.ID, ScriptID: &savedTmScript.ID}, http.StatusUnprocessableEntity)
	errMsg = extractServerErrorText(res.Body)
	...
	// create a valid script execution request
	s.DoJSON("POST", "/api/latest/mobius/scripts/run", mobius.HostScriptRequestPayload{HostID: host.ID, ScriptID: &savedNoTmScript.ID}, http.StatusAccepted, &runResp)
	require.Equal(t, host.ID, runResp.HostID)
	require.NotEmpty(t, runResp.ExecutionID)
	...
	s.lastActivityMatches(
		mobius.ActivityTypeRanScript{}.ActivityName(),
		fmt.Sprintf(
	...
	// request will time-out waiting for a result.
	var runSyncResp runScriptSyncResponse
	s.DoJSON("POST", "/api/latest/mobius/scripts/run/sync", mobius.HostScriptRequestPayload{HostID: host.ID, ScriptID: &savedNoTmScript.ID}, http.StatusRequestTimeout, &runSyncResp)
	require.Equal(t, host.ID, runSyncResp.HostID)
	require.NotEmpty(t, runSyncResp.ExecutionID)
	...
	res = s.Do("POST", "/api/latest/mobius/scripts/run/sync", mobius.HostScriptRequestPayload{HostID: host.ID, ScriptContents: "echo", ScriptID: ptr.Uint(savedTmScript.ID + 999)}, http.StatusUnprocessableEntity)
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, `Only one of 'script_id' or 'script_contents' is allowed.`)

	// attempt to run sync with both script contents and script name
	res = s.Do("POST", "/api/latest/mobius/scripts/run/sync", mobius.HostScriptRequestPayload{HostID: host.ID, ScriptContents: "echo", ScriptName: savedTmScript.Name}, http.StatusUnprocessableEntity)
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, `Only one of 'script_contents' or 'script_name' is allowed.`)

	res = s.Do("POST", "/api/latest/mobius/scripts/run", mobius.HostScriptRequestPayload{HostID: host.ID, ScriptContents: "echo", ScriptName: savedTmScript.Name}, http.StatusUnprocessableEntity)
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, `Only one of 'script_contents' or 'script_name' is allowed.`)

	// attempt to run sync with both script id and script name
	res = s.Do("POST", "/api/latest/mobius/scripts/run/sync", mobius.HostScriptRequestPayload{HostID: host.ID, ScriptID: ptr.Uint(savedTmScript.ID + 999), ScriptName: savedTmScript.Name}, http.StatusUnprocessableEntity)
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, `Only one of 'script_id' or 'script_name' is allowed.`)

	res = s.Do("POST", "/api/latest/mobius/scripts/run", mobius.HostScriptRequestPayload{HostID: host.ID, ScriptID: ptr.Uint(savedTmScript.ID + 999), ScriptName: savedTmScript.Name}, http.StatusUnprocessableEntity)
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, `Only one of 'script_id' or 'script_name' is allowed.`)

	// attempt to run sync with both script contents and team id
	res = s.Do("POST", "/api/latest/mobius/scripts/run/sync", mobius.HostScriptRequestPayload{HostID: host.ID, ScriptContents: "echo", TeamID: 1}, http.StatusUnprocessableEntity)
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, `Only one of 'script_contents' or 'team_id' is allowed.`)

	res = s.Do("POST", "/api/latest/mobius/scripts/run", mobius.HostScriptRequestPayload{HostID: host.ID, ScriptContents: "echo", TeamID: 1}, http.StatusUnprocessableEntity)
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, `Only one of 'script_contents' or 'team_id' is allowed.`)

	// attempt to run sync with both script id and team id
	res = s.Do("POST", "/api/latest/mobius/scripts/run/sync", mobius.HostScriptRequestPayload{HostID: host.ID, ScriptID: ptr.Uint(savedTmScript.ID + 999), TeamID: 1}, http.StatusUnprocessableEntity)
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, `Only one of 'script_id' or 'team_id' is allowed.`)

	res = s.Do("POST", "/api/latest/mobius/scripts/run", mobius.HostScriptRequestPayload{HostID: host.ID, ScriptID: ptr.Uint(savedTmScript.ID + 999), TeamID: 1}, http.StatusUnprocessableEntity)
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, `Only one of 'script_id' or 'team_id' is allowed.`)

	// attempt to run sync without script contents, script id, or script name
	res = s.Do("POST", "/api/latest/mobius/scripts/run/sync", mobius.HostScriptRequestPayload{HostID: host.ID}, http.StatusUnprocessableEntity)
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, `One of 'script_id', 'script_contents', or 'script_name' is required.`)

	res = s.Do("POST", "/api/latest/mobius/scripts/run", mobius.HostScriptRequestPayload{HostID: host.ID}, http.StatusUnprocessableEntity)
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, `One of 'script_id', 'script_contents', or 'script_name' is required.`)

	// deleting the saved script should delete the pending script
	s.Do("DELETE", fmt.Sprintf("/api/latest/mobius/scripts/%d", savedNoTmScript.ID), nil, http.StatusNoContent)
	scriptResultResp = getScriptResultResponse{}
	s.DoJSON("GET", "/api/latest/mobius/scripts/results/"+runSyncResp.ExecutionID, nil, http.StatusNotFound, &scriptResultResp)
	...
	for i := 1; i <= 1000; i++ {
		...
		_, err = s.ds.NewHostScriptExecutionRequest(ctx, &mobius.HostScriptRequestPayload{HostID: host.ID, ScriptID: &script.ID})
		require.NoError(t, err)
	}
	...
	s.DoJSON("POST", "/api/latest/mobius/scripts/run", mobius.HostScriptRequestPayload{HostID: host.ID, ScriptID: &script.ID}, http.StatusConflict, &runResp)

	// set up a new host, new team, and some new scripts
	host2 := createOrbitEnrolledHost(t, "linux", "f1337", s.ds)
	tm2, err := s.ds.NewTeam(ctx, &mobius.Team{Name: "team 2"})
	require.NoError(t, err)
	savedNoTmScript2, err := s.ds.NewScript(ctx, &mobius.Script{
		TeamID:         nil,
		Name:           "f1337.sh",
		ScriptContents: "echo 'ALL YOUR BASE ARE BELONG TO US'",
	})
	require.NoError(t, err)
	savedTmScript2, err := s.ds.NewScript(ctx, &mobius.Script{
		TeamID:         &tm2.ID,
		Name:           "f1337.sh",
		ScriptContents: "echo 'ALL YOUR BASE ARE BELONG TO US'",
	})
	...
	_, err = s.ds.NewScript(ctx, &mobius.Script{
		TeamID:         nil,
		Name:           "f13372.sh",
		ScriptContents: "echo 'ALL YOUR BASE ARE BELONG TO US'",
	})
	...
	res = s.Do("POST", "/api/latest/mobius/scripts/run/sync", mobius.HostScriptRequestPayload{HostID: host2.ID, ScriptName: "f1337.sh", TeamID: tm.ID}, http.StatusUnprocessableEntity)
	errMsg = extractServerErrorText(res.Body)
	...
	res = s.Do("POST", "/api/latest/mobius/scripts/run/sync", mobius.HostScriptRequestPayload{HostID: host2.ID, ScriptName: "f1337.sh", TeamID: tm2.ID}, http.StatusUnprocessableEntity)
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, `The script does not belong to the same team`)
	...
	s.DoJSON("POST", "/api/latest/mobius/scripts/run/sync", mobius.HostScriptRequestPayload{HostID: host2.ID, ScriptName: "f1337.sh"}, http.StatusRequestTimeout, &runSyncResp2)
	...
	require.Contains(t, runSyncResp2.Message, mobius.RunScriptHostTimeoutErrMsg)
	...
	require.NoError(t, err)
	res = s.Do("POST", "/api/latest/mobius/scripts/run", mobius.HostScriptRequestPayload{HostID: plainOsqueryHost.ID, ScriptID: &script.ID}, http.StatusUnprocessableEntity)
	require.Contains(t, extractServerErrorText(res.Body), mobius.RunScriptDisabledErrMsg)
	res = s.Do("POST", "/api/latest/mobius/scripts/run/sync", mobius.HostScriptRequestPayload{HostID: plainOsqueryHost.ID, ScriptID: &script.ID}, http.StatusUnprocessableEntity)
	require.Contains(t, extractServerErrorText(res.Body), mobius.RunScriptDisabledErrMsg)

	// Async Run Script by Name

	// attempt to run async with a script that does not exist on the specified team
	res = s.Do("POST", "/api/latest/mobius/scripts/run", mobius.HostScriptRequestPayload{HostID: host2.ID, ScriptName: "f1337.sh", TeamID: tm.ID}, http.StatusUnprocessableEntity)
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, `Script 'f1337.sh' doesn’t exist.`)

	// attempt to run async with an existing team script that belongs to a team different from the host's team
	res = s.Do("POST", "/api/latest/mobius/scripts/run", mobius.HostScriptRequestPayload{HostID: host2.ID, ScriptName: "f1337.sh", TeamID: tm2.ID}, http.StatusUnprocessableEntity)
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, `The script does not belong to the same team`)

	var runSyncResp3 runScriptSyncResponse
	s.DoJSON("POST", "/api/latest/mobius/scripts/run", mobius.HostScriptRequestPayload{HostID: host2.ID, ScriptName: "f13372.sh"}, http.StatusAccepted, &runSyncResp3)
	require.Equal(t, host2.ID, runSyncResp3.HostID)
	require.NotEmpty(t, runSyncResp3.ExecutionID)

	// verify pending result
	s.DoJSON("GET", "/api/latest/mobius/scripts/results/"+runSyncResp3.ExecutionID, nil, http.StatusOK, &scriptResultResp)
...
func (s *integrationEnterpriseTestSuite) TestEnqueueSameScriptTwice() {
	...
	var runResp runScriptResponse
	s.DoJSON("POST", "/api/latest/mobius/scripts/run", mobius.HostScriptRequestPayload{HostID: host.ID, ScriptID: &script.ID}, http.StatusAccepted, &runResp)
	require.Equal(t, host.ID, runResp.HostID)
	require.NotEmpty(t, runResp.ExecutionID)

	// Should fail because the same script is already enqueued for this host.
	resp := s.Do("POST", "/api/latest/mobius/scripts/run", mobius.HostScriptRequestPayload{HostID: host.ID, ScriptID: &script.ID}, http.StatusConflict)
...
func (s *integrationEnterpriseTestSuite) TestOrbitConfigExtensions() {
	...
	foobarLabel, err := s.ds.NewLabel(ctx, &mobius.Label{
		Name:  "Foobar",
		Query: "SELECT 1;",
	})
	require.NoError(t, err)
	zoobarLabel, err := s.ds.NewLabel(ctx, &mobius.Label{
		Name:  "Zoobar",
		Query: "SELECT 1;",
	})
	require.NoError(t, err)
	...
	s.DoRaw("PATCH", "/api/latest/mobius/config", []byte(`{
  "agent_options": {
	"config": {
		"options": {
		"pack_delimiter": "/",
	...
	s.DoJSON("POST", "/api/mobius/orbit/config", json.RawMessage(fmt.Sprintf(`{"orbit_node_key": %q}`, *orbitDarwinClient.OrbitNodeKey)), http.StatusOK, &resp)
	require.JSONEq(t, `{
	"hello_mars_macos": {
		"channel": "stable",
		"platform": "macos"
	...
	resp = orbitGetConfigResponse{}
	s.DoJSON("POST", "/api/mobius/orbit/config", json.RawMessage(fmt.Sprintf(`{"orbit_node_key": %q}`, *orbitLinuxClient.OrbitNodeKey)), http.StatusOK, &resp)
	require.JSONEq(t, `{
	"hello_world_linux": {
		"channel": "stable",
		"platform": "linux"
	...
	resp = orbitGetConfigResponse{}
	s.DoJSON("POST", "/api/mobius/orbit/config", json.RawMessage(fmt.Sprintf(`{"orbit_node_key": %q}`, *orbitWindowsClient.OrbitNodeKey)), http.StatusOK, &resp)
...
func (s *integrationEnterpriseTestSuite) TestSavedScripts() {
	...
	res := s.DoRawWithHeaders("POST", "/api/latest/mobius/scripts", body.Bytes(), http.StatusOK, headers)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/scripts/%d", noTeamScriptID), nil, http.StatusOK, &getScriptResp)
	...
	res = s.Do("GET", fmt.Sprintf("/api/latest/mobius/scripts/%d", noTeamScriptID), nil, http.StatusOK, "alt", "media")
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/scripts/%d", noTeamScriptID+999), nil, http.StatusNotFound, &getScriptResp)
	// download a non-existing script
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/scripts/%d", noTeamScriptID+999), nil, http.StatusNotFound, &getScriptResp, "alt", "media")

	// file name is empty
	body, headers = generateNewScriptMultipartRequest(t,
		"", []byte(`echo "hello"`), s.token, nil)
	res = s.DoRawWithHeaders("POST", "/api/latest/mobius/scripts", body.Bytes(), http.StatusBadRequest, headers)
	errMsg := extractServerErrorText(res.Body)
	require.Contains(t, errMsg, "no file headers for script")

	// contains invalid mobius secret
	body, headers = generateNewScriptMultipartRequest(t,
		"secrets.sh", []byte(`echo "$MOBIUS_SECRET_INVALID"`), s.token, nil)
	res = s.DoRawWithHeaders("POST", "/api/latest/mobius/scripts", body.Bytes(), http.StatusUnprocessableEntity, headers)
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, "$MOBIUS_SECRET_INVALID")
	...
	body, headers = generateNewScriptMultipartRequest(t,
		"script2.sh", []byte(strings.Repeat("a", mobius.SavedScriptMaxRuneLen+1)), s.token, nil)
	res = s.DoRawWithHeaders("POST", "/api/latest/mobius/scripts", body.Bytes(), http.StatusUnprocessableEntity, headers)
	errMsg = extractServerErrorText(res.Body)
	...
	body, headers = generateNewScriptMultipartRequest(t,
		"script1.sh", []byte(`echo "hello"`), s.token, nil)
	res = s.DoRawWithHeaders("POST", "/api/latest/mobius/scripts", body.Bytes(), http.StatusConflict, headers)
	errMsg = extractServerErrorText(res.Body)
	...
	res = s.DoRawWithHeaders("POST", "/api/latest/mobius/scripts", body.Bytes(), http.StatusNotFound, headers)
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, "The team does not exist.")

	// create a team
	tm, err := s.ds.NewTeam(ctx, &mobius.Team{Name: "team1"})
	require.NoError(t, err)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/scripts/%d", tmScriptID), nil, http.StatusOK, &getScriptResp)
	...
	res = s.Do("GET", fmt.Sprintf("/api/latest/mobius/scripts/%d", tmScriptID), nil, http.StatusOK, "alt", "media")
	b, err = io.ReadAll(res.Body)
	require.NoError(t, err)
	...
	res = s.DoRawWithHeaders("PATCH", fmt.Sprintf("/api/latest/mobius/scripts/%d", tmScriptID), body.Bytes(), http.StatusOK, headers)
	...
	body, headers = generateNewScriptMultipartRequest(t,
		"script1.sh", []byte(`echo "updated script"`), s.token, nil)
	res = s.DoRawWithHeaders("PATCH", fmt.Sprintf("/api/latest/mobius/scripts/%d", 999999999999), body.Bytes(), http.StatusNotFound, headers)
	err = json.NewDecoder(res.Body).Decode(&updateScriptRep)
	require.NoError(t, err)

	// delete the no-team script
	s.Do("DELETE", fmt.Sprintf("/api/latest/mobius/scripts/%d", noTeamScriptID), nil, http.StatusNoContent)
	...
	s.Do("DELETE", fmt.Sprintf("/api/latest/mobius/scripts/%d", tmScriptID), nil, http.StatusNoContent)
	...
	s.Do("DELETE", fmt.Sprintf("/api/latest/mobius/scripts/%d", noTeamScriptID), nil, http.StatusNotFound)
}
...
func (s *integrationEnterpriseTestSuite) TestListSavedScripts() {
	...
	// create some teams
	tm1, err := s.ds.NewTeam(ctx, &mobius.Team{Name: "team1"})
	require.NoError(t, err)
	tm2, err := s.ds.NewTeam(ctx, &mobius.Team{Name: "team2"})
	require.NoError(t, err)
	tm3, err := s.ds.NewTeam(ctx, &mobius.Team{Name: "team3"})
	require.NoError(t, err)
	...
	for i := 0; i < 5; i++ {
		...
		require.NoError(t, err)
		_, err = s.ds.NewScript(ctx, &mobius.Script{Name: string('a' + byte(i)), TeamID: &tm1.ID, ScriptContents: "echo"})
		require.NoError(t, err)
	}

	// create a single script for team 2
	_, err = s.ds.NewScript(ctx, &mobius.Script{Name: "a", TeamID: &tm2.ID, ScriptContents: "echo"})
	require.NoError(t, err)
	...
	cases := []struct {
		...
		teamID    *uint
		wantNames []string
		wantMeta  *mobius.PaginationMetadata
	}{
		{
			wantNames: []string{"a", "b", "c", "d", "e"},
			wantMeta:  &mobius.PaginationMetadata{HasNextResults: false, HasPreviousResults: false},
		},
		{
			queries:   []string{"per_page", "2"},
			wantNames: []string{"a", "b"},
			wantMeta:  &mobius.PaginationMetadata{HasNextResults: true, HasPreviousResults: false},
		},
		{
			queries:   []string{"per_page", "2", "page", "1"},
			wantNames: []string{"c", "d"},
			wantMeta:  &mobius.PaginationMetadata{HasNextResults: true, HasPreviousResults: true},
		},
		{
			queries:   []string{"per_page", "2", "page", "2"},
			wantNames: []string{"e"},
			wantMeta:  &mobius.PaginationMetadata{HasNextResults: false, HasPreviousResults: true},
		},
		{
	...
	for _, c := range cases {
		t.Run(fmt.Sprintf("%v: %#v", c.teamID, c.queries), func(t *testing.T) {
			...
			s.DoJSON("GET", "/api/latest/mobius/scripts", nil, http.StatusOK, &listResp, queryArgs...)
...
func (s *integrationEnterpriseTestSuite) TestHostScriptDetails() {
	...
	// create some teams
	tm1, err := s.ds.NewTeam(ctx, &mobius.Team{Name: "test-script-details-team1"})
	require.NoError(t, err)
	tm2, err := s.ds.NewTeam(ctx, &mobius.Team{Name: "test-script-details-team2"})
	require.NoError(t, err)
	tm3, err := s.ds.NewTeam(ctx, &mobius.Team{Name: "test-script-details-team3"})
	require.NoError(t, err)
	tm4, err := s.ds.NewTeam(ctx, &mobius.Team{Name: "test-script-details-team4-windows"})
	require.NoError(t, err)

	// create 5 scripts for no team and team 1
	for i := 0; i < 5; i++ {
		_, err = s.ds.NewScript(ctx, &mobius.Script{Name: fmt.Sprintf("test-script-details-%d.sh", i), ScriptContents: "echo"})
		require.NoError(t, err)
		_, err = s.ds.NewScript(ctx, &mobius.Script{Name: fmt.Sprintf("test-script-details-%d.sh", i), TeamID: &tm1.ID, ScriptContents: "echo"})
		require.NoError(t, err)
	}

	// add a windows script to team 4
	_, err = s.ds.NewScript(ctx, &mobius.Script{Name: "test-script-details-windows.ps1", TeamID: &tm4.ID, ScriptContents: `Write-Host "Hello, World!"`})
	require.NoError(t, err)

	// create a single script for team 2
	_, err = s.ds.NewScript(ctx, &mobius.Script{Name: "test-script-details-team-2.sh", TeamID: &tm2.ID, ScriptContents: "echo"})
	require.NoError(t, err)

	// create a host without a team
	host0, err := s.ds.NewHost(ctx, &mobius.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
		SeenTime:        time.Now().Add(-1 * time.Minute),
	...
	host1, err := s.ds.NewHost(ctx, &mobius.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
		SeenTime:        time.Now().Add(-1 * time.Minute),
	...
	host2, err := s.ds.NewHost(ctx, &mobius.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
		SeenTime:        time.Now().Add(-1 * time.Minute),
	...
	host3, err := s.ds.NewHost(ctx, &mobius.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
		SeenTime:        time.Now().Add(-1 * time.Minute),
	...
	// create a Linux host
	host4, err := s.ds.NewHost(ctx, &mobius.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
		SeenTime:        time.Now().Add(-1 * time.Minute),
	...
	host5, err := s.ds.NewHost(ctx, &mobius.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
		SeenTime:        time.Now().Add(-1 * time.Minute),
	...
	insertResults := func(t *testing.T, hostID uint, script *mobius.Script, createdAt time.Time, execID string, exitCode *int64) {
		stmt := `
INSERT INTO
...
VALUES
	...
	insertResults(t, host0.ID, &mobius.Script{Name: "ad hoc script", ScriptContents: "echo foo"}, now, "ad-hoc-0", ptr.Int64(0))
	insertResults(t, host1.ID, &mobius.Script{Name: "ad hoc script", ScriptContents: "echo foo"}, now.Add(-1*time.Hour), "ad-hoc-1", ptr.Int64(1))

	t.Run("no team", func(t *testing.T) {
		noTeamScripts, _, err := s.ds.ListScripts(ctx, nil, mobius.ListOptions{})
		require.NoError(t, err)
		...
		// insert some ad hoc script results, these are never included in the host script details
		insertResults(t, host0.ID, &mobius.Script{Name: "ad hoc script", ScriptContents: "echo foo"}, now.Add(-3*time.Hour), "exec0-3", ptr.Int64(0))

		// check host script details, should include all no team scripts
		var resp getHostScriptDetailsResponse
		s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/scripts", host0.ID), nil, http.StatusOK, &resp)
		require.Len(t, resp.Scripts, len(noTeamScripts))
		byScriptID := make(map[uint]*mobius.HostScriptDetail, len(resp.Scripts))
	...
	t.Run("team 1", func(t *testing.T) {
		tm1Scripts, _, err := s.ds.ListScripts(ctx, &tm1.ID, mobius.ListOptions{})
		require.NoError(t, err)
		...
		var resp getHostScriptDetailsResponse
		s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/scripts", host1.ID), nil, http.StatusOK, &resp)
	...
	t.Run("deleted script", func(t *testing.T) {
		...
		// delete a script
		s.Do("DELETE", fmt.Sprintf("/api/latest/mobius/scripts/%d", noTeamScripts[0].ID), nil, http.StatusNoContent)
	...
	t.Run("transfer team", func(t *testing.T) {
		...
		tm2Scripts, _, err := s.ds.ListScripts(ctx, &tm2.ID, mobius.ListOptions{})
		require.NoError(t, err)
	...
	t.Run("no scripts", func(t *testing.T) {
		var resp getHostScriptDetailsResponse
		s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/scripts", host2.ID), nil, http.StatusOK, &resp)
		require.NotNil(t, resp.Scripts)
		require.Len(t, resp.Scripts, 0)
	})

	t.Run("windows", func(t *testing.T) {
		team4Scripts, _, err := s.ds.ListScripts(ctx, &tm4.ID, mobius.ListOptions{})
		require.NoError(t, err)
		require.Len(t, team4Scripts, 1)

		var resp getHostScriptDetailsResponse
		s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/scripts", host3.ID), nil, http.StatusOK, &resp)
		require.NotNil(t, resp.Scripts)
	...
	t.Run("linux", func(t *testing.T) {
		...
		var resp getHostScriptDetailsResponse
		s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/scripts", host4.ID), nil, http.StatusOK, &resp)
		require.NotNil(t, resp.Scripts)
	...
	t.Run("unspecified platform", func(t *testing.T) {
		var resp getHostScriptDetailsResponse
		s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/scripts", host5.ID), nil, http.StatusOK, &resp)
		require.NotNil(t, resp.Scripts)
		require.Len(t, resp.Scripts, 4)
	})
	...
	(?,?,?,?)`,
		...
		for _, c := range []struct {
		...
		}{
			{
				...
				expected:   mobius.RunScriptHostTimeoutErrMsg,
			},
			{
				name:       "script-timeout",
				exitCode:   ptr.Int64(-1),
				executedAt: now.Add(-1 * time.Hour),
				expected:   mobius.HostScriptTimeoutMessage(ptr.Int(int(scripts.MaxHostExecutionTime.Seconds()))),
			},
			{
				name:       "pending",
				exitCode:   nil,
				executedAt: now.Add(-1 * time.Minute),
				expected:   mobius.RunScriptAlreadyRunningErrMsg,
			},
			{
				name:       "success",
			...
			},
			{
				...
				executedAt: now.Add(-1 * time.Hour),
				expected:   mobius.RunScriptDisabledErrMsg,
			},
		} {
			t.Run(c.name, func(t *testing.T) {
				insertResults(t, host0.ID, &mobius.Script{ID: oldScriptID, ScriptContentID: oldScriptContentsID, Name: "test-script-details-timeout.sh"}, c.executedAt, "test-user-message_"+c.name, c.exitCode)

				var resp getScriptResultResponse
				s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/scripts/results/%s", "test-user-message_"+c.name), nil, http.StatusOK, &resp)
...
// used via s.DoRawWithHeaders to POST /api/_version_/mobius/scripts.
...
func (s *integrationEnterpriseTestSuite) TestAppConfigScripts() {
	...
	acResp := appConfigResponse{}
	s.DoJSON("PATCH", "/api/latest/mobius/config", json.RawMessage(`{ "scripts": ["foo", "bar"] }`), http.StatusOK, &acResp)
	...
	acResp = appConfigResponse{}
	s.DoJSON("PATCH", "/api/latest/mobius/config", json.RawMessage(`{}`), http.StatusOK, &acResp)
	assert.ElementsMatch(t, []string{"foo", "bar"}, acResp.Scripts.Value)

	// patch with explicitly empty scripts fields, would remove
	// them but this is a dry-run
	acResp = appConfigResponse{}
	s.DoJSON("PATCH", "/api/latest/mobius/config", json.RawMessage(`{ "scripts": null }`), http.StatusOK, &acResp, "dry_run", "true")
	assert.ElementsMatch(t, []string{"foo", "bar"}, acResp.Scripts.Value)

	// patch with explicitly empty scripts fields, removes them
	acResp = appConfigResponse{}
	s.DoJSON("PATCH", "/api/latest/mobius/config", json.RawMessage(`{ "scripts": null }`), http.StatusOK, &acResp)
	...
	acResp = appConfigResponse{}
	s.DoJSON("PATCH", "/api/latest/mobius/config", json.RawMessage(`{ "scripts": [] }`), http.StatusOK, &acResp)
	assert.Empty(t, acResp.Scripts.Value)

	// patch with an invalid array returns an error
	acResp = appConfigResponse{}
	s.DoJSON("PATCH", "/api/latest/mobius/config", json.RawMessage(`{ "scripts": ["foo", 1] }`), http.StatusBadRequest, &acResp)
	assert.Empty(t, acResp.Scripts.Value)
}
...
func (s *integrationEnterpriseTestSuite) TestApplyTeamsScriptsConfig() {
	...
	s.DoJSON("POST", "/api/latest/mobius/teams", team, http.StatusOK, &createTeamResp)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/teams/%d", team.ID), nil, http.StatusOK, &teamResp)
...
func (s *integrationEnterpriseTestSuite) TestBatchApplyScriptsEndpoints() {
	...
	saveAndCheckScripts := func(team *mobius.Team, scripts []mobius.ScriptPayload) {
		var teamID *uint
		teamIDStr := ""
		...
		s.DoJSON("POST", "/api/v1/mobius/scripts/batch", batchSetScriptsRequest{Scripts: scripts}, http.StatusOK, &scriptsBatchResponse, "team_id", teamIDStr)
		s.lastActivityMatches(
			mobius.ActivityTypeEditedScript{}.ActivityName(),
			teamActivity,
			0,
		)
		...
		var listResp listScriptsResponse
		s.DoJSON("GET", "/api/latest/mobius/scripts", nil, http.StatusOK, &listResp, "team_id", teamIDStr)
		require.Len(t, listResp.Scripts, len(scripts))

		got := make([]mobius.ScriptPayload, len(scripts))
		for i, gotScript := range listResp.Scripts {
			// add the script contents
			res := s.Do("GET", fmt.Sprintf("/api/latest/mobius/scripts/%d", gotScript.ID), nil, http.StatusOK, "alt", "media")
			b, err := io.ReadAll(res.Body)
			require.NoError(t, err)
			got[i] = mobius.ScriptPayload{
				Name:           gotScript.Name,
				ScriptContents: b,
			}
	...
	// create a new team
	tm, err := s.ds.NewTeam(ctx, &mobius.Team{Name: "batch_set_scripts"})
	require.NoError(t, err)
	...
	s.Do("POST", "/api/v1/mobius/scripts/batch", batchSetScriptsRequest{Scripts: nil},
	...
	s.Do("POST", "/api/v1/mobius/scripts/batch", batchSetScriptsRequest{Scripts: []mobius.ScriptPayload{
	...
	// invalid secret
	s.Do("POST", "/api/v1/mobius/scripts/batch", batchSetScriptsRequest{Scripts: []mobius.ScriptPayload{
		{Name: "N2.sh", ScriptContents: []byte("echo $MOBIUS_SECRET_INVALID")},
	}}, http.StatusUnprocessableEntity, "team_id", fmt.Sprint(tm.ID))

	// successfully apply a scripts for the team
	saveAndCheckScripts(tm, []mobius.ScriptPayload{
		{Name: "N1.sh", ScriptContents: []byte("foo")},
		{Name: "N2.sh", ScriptContents: []byte("bar")},
	})

	// successfully apply scripts for "no team"
	saveAndCheckScripts(nil, []mobius.ScriptPayload{
		{Name: "N1.sh", ScriptContents: []byte("foo")},
		{Name: "N2.sh", ScriptContents: []byte("bar")},
	})
...
`, teamName))
	...
	// create a linux host
	linuxHost, err := s.ds.NewHost(context.Background(), &mobius.Host{
	...
	s.DoJSON("POST", "/api/osquery/distributed/read", req, http.StatusOK, &dqResp)
	require.NotContains(t, dqResp.Queries, "mobius_detail_query_users")
	require.NotContains(t, dqResp.Queries, "mobius_detail_query_disk_encryption_linux")
	require.Contains(t, dqResp.Queries, "mobius_detail_query_software_linux")
	require.Contains(t, dqResp.Queries, fmt.Sprintf("mobius_distributed_query_%s", t.Name()))
...
`, teamName))
	...
	s.DoJSON("POST", "/api/osquery/distributed/read", req, http.StatusOK, &dqResp)
	require.Contains(t, dqResp.Queries, "mobius_detail_query_users")
	require.Contains(t, dqResp.Queries, "mobius_detail_query_disk_encryption_linux")
	require.Contains(t, dqResp.Queries, "mobius_detail_query_software_linux")
	require.Contains(t, dqResp.Queries, fmt.Sprintf("mobius_distributed_query_%s", t.Name()))
}
...
func (s *integrationEnterpriseTestSuite) TestAllSoftwareTitles() {
	...
	softwareTitleListResultsMatch := func(want, got []mobius.SoftwareTitleListResult) {
	...
	softwareTitlesMatch := func(want, got []mobius.SoftwareTitle) {
		// compare only the fields we care about
		for i := range got {
			require.NotZero(t, got[i].ID)
	...
	tmHost, err := s.ds.NewHost(context.Background(), &mobius.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
		SeenTime:        time.Now().Add(-1 * time.Minute),
	...
	team1, err := s.ds.NewTeam(ctx, &mobius.Team{Name: t.Name() + "team1"})
	require.NoError(t, err)
	team2, err := s.ds.NewTeam(ctx, &mobius.Team{Name: t.Name() + "team2"})
	require.NoError(t, err)
	...
	cpes := []mobius.SoftwareCPE{{SoftwareID: soft1.ID, CPE: "somecpe"}}
	...
	// self-service flag is ignored if no team specified see https://github.com/notawar/mobius/issues/26375
	s.DoJSON("GET", "/api/latest/mobius/software/titles", listSoftwareTitlesRequest{}, http.StatusOK, &resp, "self_service", "1")
	require.Equal(t, 2, resp.Count)

	s.DoJSON("GET", "/api/latest/mobius/software/titles", listSoftwareTitlesRequest{}, http.StatusOK, &resp)
	require.Equal(t, 2, resp.Count)

	require.NotEmpty(t, resp.CountsUpdatedAt)
	softwareTitleListResultsMatch([]mobius.SoftwareTitleListResult{
		{
			Name:          "foo",
			Source:        "homebrew",
			VersionsCount: 2,
			HostsCount:    1,
			Versions: []mobius.SoftwareVersion{
		...
		},
		{
			...
			HostsCount:    1,
			Versions: []mobius.SoftwareVersion{
				{Version: "0.0.4", Vulnerabilities: &mobius.SliceString{"cve-123-123-132"}},
			},
		},
	}, resp.SoftwareTitles)
	...
	s.DoJSON(
		"GET", "/api/latest/mobius/software/titles",
	...
	softwareTitleListResultsMatch([]mobius.SoftwareTitleListResult{}, resp.SoftwareTitles)

	// asking for vulnerable only software returns the expected values
	expectedVulnSoftware := []mobius.SoftwareTitleListResult{
		{
			Name:          "bar",
			Source:        "apps",
			VersionsCount: 1,
	...
	s.DoJSON("GET", "/api/latest/mobius/software/titles/999", getSoftwareTitleRequest{}, http.StatusNotFound, &stResp)

	// valid title
	stResp = getSoftwareTitleResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/software/titles/%d", fooTitle.ID), getSoftwareTitleRequest{}, http.StatusOK, &stResp)
	s.NotZero(*stResp.SoftwareTitle.CountsUpdatedAt)
	softwareTitlesMatch([]mobius.SoftwareTitle{
		{
			Name:          "foo",
			Source:        "homebrew",
			VersionsCount: 2,
	...
	}, []mobius.SoftwareTitle{*stResp.SoftwareTitle})

	// valid title for team
	stResp = getSoftwareTitleResponse{}
	s.DoJSON(
		"GET", fmt.Sprintf("/api/latest/mobius/software/titles/%d", fooTitle.ID), getSoftwareTitleRequest{}, http.StatusOK, &stResp,
		"team_id", fmt.Sprintf("%d", team1.ID),
	)
	softwareTitlesMatch(
		[]mobius.SoftwareTitle{
			{
				Name:          "foo",
				Source:        "homebrew",
				VersionsCount: 1,
	...
	expected := []mobius.SoftwareTitle{
		{
			...
			Versions: []mobius.SoftwareVersion{
				{
					Version:         "0.0.4",
					Vulnerabilities: &mobius.SliceString{"cve-123-123-132"},
	...
	// Global
	stResp = getSoftwareTitleResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/software/titles/%d", barTitle.ID), getSoftwareTitleRequest{}, http.StatusOK, &stResp)
	softwareTitlesMatch(expected, []mobius.SoftwareTitle{*stResp.SoftwareTitle})
	...
	stResp = getSoftwareTitleResponse{}
	s.DoJSON(
		"GET", fmt.Sprintf("/api/latest/mobius/software/titles/%d", barTitle.ID), getSoftwareTitleRequest{}, http.StatusNotFound, &stResp,
		"team_id", fmt.Sprintf("%d", team1.ID),
	)
	...
	stResp = getSoftwareTitleResponse{}
	s.DoJSON(
		"GET", fmt.Sprintf("/api/latest/mobius/software/titles/%d", bazTitle.ID), getSoftwareTitleRequest{}, http.StatusNotFound, &stResp,
		"team_id", "0")
	...
	stResp = getSoftwareTitleResponse{}
	s.DoJSON(
		"GET", fmt.Sprintf("/api/latest/mobius/software/titles/%d", barTitle.ID), getSoftwareTitleRequest{}, http.StatusOK, &stResp,
		"team_id", fmt.Sprintf("%d", team1.ID),
	)
	softwareTitlesMatch(
		[]mobius.SoftwareTitle{
	...
	payloadRubyTm1 := &mobius.UploadSoftwareInstallerPayload{
	...
	payloadEmacsMissingSecret := &mobius.UploadSoftwareInstallerPayload{
		InstallScript:     "install $MOBIUS_SECRET_INVALID",
		Filename:          "emacs.deb",
		PostInstallScript: "d",
		SelfService:       true,
	}
	s.uploadSoftwareInstallerWithErrorNameReason(t, payloadEmacsMissingSecret, http.StatusUnprocessableEntity, "$MOBIUS_SECRET_INVALID",
		"install script")

	payloadEmacsMissingPostSecret := &mobius.UploadSoftwareInstallerPayload{
		InstallScript:     "install",
		Filename:          "emacs.deb",
		PostInstallScript: "d $MOBIUS_SECRET_INVALID",
		SelfService:       true,
	}
	s.uploadSoftwareInstallerWithErrorNameReason(t, payloadEmacsMissingPostSecret, http.StatusUnprocessableEntity, "$MOBIUS_SECRET_INVALID",
		"post-install script")

	payloadEmacsMissingUnSecret := &mobius.UploadSoftwareInstallerPayload{
		InstallScript:     "install",
		Filename:          "emacs.deb",
		PostInstallScript: "d",
		UninstallScript:   "delet $MOBIUS_SECRET_INVALID",
		SelfService:       true,
	}
	s.uploadSoftwareInstallerWithErrorNameReason(t, payloadEmacsMissingUnSecret, http.StatusUnprocessableEntity, "$MOBIUS_SECRET_INVALID",
		"uninstall script")

	payloadEmacs := &mobius.UploadSoftwareInstallerPayload{
		InstallScript: "install",
		Filename:      "emacs.deb",
		SelfService:   true,
	}
	...
	payloadVim := &mobius.UploadSoftwareInstallerPayload{
		InstallScript: "install",
		Filename:      "vim.deb",
		SelfService:   true,
		TeamID:        ptr.Uint(0),
	...
	payloadRubyTm2 := &mobius.UploadSoftwareInstallerPayload{
		InstallScript: "install",
	...
	// "All teams" returns all software regardless of self_service see https://github.com/notawar/mobius/issues/26375
	resp = listSoftwareTitlesResponse{}
	s.DoJSON(
		"GET", "/api/latest/mobius/software/titles",
		listSoftwareTitlesRequest{},
	...
	emacsPath := fmt.Sprintf("/api/latest/mobius/software/titles/%d", resp.SoftwareTitles[0].ID)
...
func (s *integrationEnterpriseTestSuite) TestLockUnlockWipeWindowsLinux() {
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d", winHost.ID), nil, http.StatusOK, &getHostResp)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d", linuxHost.ID), nil, http.StatusOK, &getHostResp)
	require.NotNil(t, getHostResp.Host.MDM.DeviceStatus)
	require.Equal(t, "unlocked", *getHostResp.Host.MDM.DeviceStatus)
	require.NotNil(t, getHostResp.Host.MDM.PendingAction)
	require.Equal(t, "", *getHostResp.Host.MDM.PendingAction)
	...
	res := s.DoRaw("POST", fmt.Sprintf("/api/latest/mobius/hosts/%d/lock", winHost.ID), nil, http.StatusBadRequest)
	errMsg := extractServerErrorText(res.Body)
	require.Contains(t, errMsg, "Windows MDM isn't turned on.")
	res = s.DoRaw("POST", fmt.Sprintf("/api/latest/mobius/hosts/%d/unlock", winHost.ID), nil, http.StatusBadRequest)
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, "Windows MDM isn't turned on.")
	res = s.DoRaw("POST", fmt.Sprintf("/api/latest/mobius/hosts/%d/wipe", winHost.ID), nil, http.StatusBadRequest)
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, "Windows MDM isn't turned on.")
	...
	res = s.DoRaw("POST", fmt.Sprintf("/api/latest/mobius/hosts/%d/lock", linuxHost.ID), nil, http.StatusUnprocessableEntity)
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, "Couldn't lock host. To lock, deploy the mobiusdaemon agent with --enable-scripts")
	res = s.DoRaw("POST", fmt.Sprintf("/api/latest/mobius/hosts/%d/unlock", linuxHost.ID), nil, http.StatusUnprocessableEntity)
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, "Couldn't unlock host. To unlock, deploy the mobiusdaemon agent with --enable-scripts")
	res = s.DoRaw("POST", fmt.Sprintf("/api/latest/mobius/hosts/%d/wipe", linuxHost.ID), nil, http.StatusUnprocessableEntity)
	errMsg = extractServerErrorText(res.Body)
	require.Contains(t, errMsg, "Couldn't wipe host. To wipe, deploy the mobiusdaemon agent with --enable-scripts")
	...
	s.Do("POST", fmt.Sprintf("/api/latest/mobius/hosts/%d/lock", linuxHost.ID), nil, http.StatusOK)
	...
	s.Do("POST", fmt.Sprintf("/api/latest/mobius/hosts/%d/unlock", linuxHost.ID), nil, http.StatusOK)
...
func checkWindowsOSUpdatesProfile(t *testing.T, ds *mysql.Datastore, teamID *uint, wantSettings *mobius.WindowsUpdates) string {
	ctx := context.Background()

	var prof mobius.MDMWindowsConfigProfile
	mysql.ExecAdhocSQL(t, ds, func(tx sqlx.ExtContext) error {
		var globalOrTeamID uint
		if teamID != nil {
			globalOrTeamID = *teamID
		}
		err := sqlx.GetContext(ctx, tx, &prof, `SELECT profile_uuid, syncml FROM mdm_windows_configuration_profiles WHERE team_id = ? AND name = ?`, globalOrTeamID, mdm.MobiusWindowsOSUpdatesProfileName)
...
func (s *integrationEnterpriseTestSuite) createHosts(t *testing.T, platforms ...string) []*mobius.Host {
...
func (s *integrationEnterpriseTestSuite) TestSoftwareAuth() {
	...
	tmHost, err := s.ds.NewHost(ctx, &mobius.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
		SeenTime:        time.Now().Add(-1 * time.Minute),
	...
	require.NoError(t, err)
	team2, err := s.ds.NewTeam(ctx, &mobius.Team{
		ID:          43,
		Name:        "team2",
		Description: "desc team2",
	})
	...
	_, err = s.ds.UpdateHostSoftware(ctx, tmHost.ID, []mobius.Software{allSoftware[0]})
	require.NoError(t, err)
	...
	extraTestUsers := make(map[string]mobius.User)
	...
	s.DoJSON("GET", "/api/latest/mobius/software/titles", listSoftwareTitlesRequest{}, http.StatusOK, &listSoftwareTitlesResp)

	var softwareFoo, softwareBar *mobius.SoftwareTitleListResult
	...
	var teamFooVersion *mobius.SoftwareVersion
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.shouldFailGlobalRead {
				...
				var listSoftwareTitlesResp listSoftwareTitlesResponse
				s.DoJSON("GET", "/api/latest/mobius/software/titles", listSoftwareTitlesRequest{}, http.StatusForbidden, &listSoftwareTitlesResp)

				// List all software versions
				var resp listSoftwareVersionsResponse
				s.DoJSON("GET", "/api/latest/mobius/software/versions", listSoftwareTitlesRequest{}, http.StatusForbidden, &resp)

				// Get a global software title
				var getSoftwareTitleResp getSoftwareTitleResponse
				s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/software/titles/%d", softwareBar.ID), getSoftwareTitleRequest{}, http.StatusForbidden, &getSoftwareTitleResp)

				// Get a global software version
				var getSoftwareResp getSoftwareResponse
				s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/software/versions/%d", softwareBar.Versions[0].ID), getSoftwareRequest{}, http.StatusForbidden, &getSoftwareResp)

				// Get a global software vesion using the deprecated endpoint
				getSoftwareResp = getSoftwareResponse{}
				s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/software/%d", softwareBar.Versions[0].ID), getSoftwareRequest{}, http.StatusForbidden, &getSoftwareResp)

				// Get a count of software vesions using the deprecated endpoint
				var countSoftwareResp countSoftwareResponse
				s.DoJSON("GET", "/api/latest/mobius/software/count", getSoftwareRequest{}, http.StatusForbidden, &countSoftwareResp)

				// List all software versions using the deprecated endpoint
				var softwareListResp listSoftwareResponse
				s.DoJSON("GET", "/api/latest/mobius/software", listSoftwareRequest{}, http.StatusForbidden, &softwareListResp)
			} else {
				// List all software titles
				var listSoftwareTitlesResp listSoftwareTitlesResponse
				s.DoJSON("GET", "/api/latest/mobius/software/titles", listSoftwareTitlesRequest{}, http.StatusOK, &listSoftwareTitlesResp)
				...
				// List all software versions
				var resp listSoftwareVersionsResponse
				s.DoJSON("GET", "/api/latest/mobius/software/versions", listSoftwareRequest{}, http.StatusOK, &resp)
				require.Equal(t, 3, resp.Count)
				require.NotEmpty(t, resp.CountsUpdatedAt)

				// Get a global software title
				var getSoftwareTitleResp getSoftwareTitleResponse
				s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/software/titles/%d", softwareBar.ID), getSoftwareTitleRequest{}, http.StatusOK, &getSoftwareTitleResp)

				// Get a global software version
				var getSoftwareResp getSoftwareResponse
				s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/software/versions/%d", softwareBar.Versions[0].ID), getSoftwareRequest{}, http.StatusOK, &getSoftwareResp)

				// Get a global software vesion using the deprecated endpoint
				getSoftwareResp = getSoftwareResponse{}
				s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/software/%d", softwareBar.Versions[0].ID), getSoftwareRequest{}, http.StatusOK, &getSoftwareResp)

				// Get a global count of software vesions using the deprecated endpoint
				var countSoftwareResp countSoftwareResponse
				s.DoJSON("GET", "/api/latest/mobius/software/count", countSoftwareRequest{}, http.StatusOK, &countSoftwareResp)
				require.Equal(t, 3, countSoftwareResp.Count)

				// List all software versions using the deprecated endpoint
				var softwareListResp listSoftwareResponse
				s.DoJSON("GET", "/api/latest/mobius/software", listSoftwareRequest{}, http.StatusOK, &softwareListResp)
			...
			if tc.shouldFailTeamRead {
				...
				var listSoftwareTitlesResp listSoftwareTitlesResponse
				s.DoJSON("GET", "/api/latest/mobius/software/titles", listSoftwareTitlesRequest{SoftwareTitleListOptions: mobius.SoftwareTitleListOptions{TeamID: &team1.ID}}, http.StatusForbidden, &listSoftwareTitlesResp)

				// List software versions for a team.
				var resp listSoftwareTitlesResponse
				s.DoJSON("GET", "/api/latest/mobius/software/versions", listSoftwareRequest{SoftwareListOptions: mobius.SoftwareListOptions{TeamID: &team1.ID}}, http.StatusForbidden, &resp)

				// Get a team software title
				var getSoftwareTitleResp getSoftwareTitleResponse
				s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/software/titles/%d", softwareFoo.ID), getSoftwareTitleRequest{}, http.StatusForbidden, &getSoftwareTitleResp)

				// Get a team software version
				var getSoftwareResp getSoftwareResponse
				s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/software/versions/%d", teamFooVersion.ID), getSoftwareRequest{}, http.StatusForbidden, &getSoftwareResp)

				// Get a team software vesion using the deprecated endpoint
				getSoftwareResp = getSoftwareResponse{}
				s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/software/%d", teamFooVersion.ID), getSoftwareRequest{}, http.StatusForbidden, &getSoftwareResp)
				...
				// List all software titles for a team
				var listSoftwareTitlesResp listSoftwareTitlesResponse
				s.DoJSON("GET", "/api/latest/mobius/software/titles", listSoftwareTitlesRequest{SoftwareTitleListOptions: mobius.SoftwareTitleListOptions{TeamID: &team1.ID}}, http.StatusOK, &listSoftwareTitlesResp)
				require.Equal(t, 1, listSoftwareTitlesResp.Count)
				require.NotEmpty(t, listSoftwareTitlesResp.CountsUpdatedAt)

				// List software versions for a team.
				var resp listSoftwareTitlesResponse
				s.DoJSON("GET", "/api/latest/mobius/software/versions", listSoftwareRequest{SoftwareListOptions: mobius.SoftwareListOptions{TeamID: &team1.ID}}, http.StatusOK, &resp)
				require.Equal(t, 1, resp.Count)
				require.NotEmpty(t, resp.CountsUpdatedAt)

				// Get a team software title
				var getSoftwareTitleResp getSoftwareTitleResponse
				s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/software/titles/%d", softwareFoo.ID), getSoftwareTitleRequest{}, http.StatusOK, &getSoftwareTitleResp)

				// Get a team software version
				var getSoftwareResp getSoftwareResponse
				s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/software/versions/%d", teamFooVersion.ID), getSoftwareRequest{}, http.StatusOK, &getSoftwareResp)

				// Get a team software vesion using the deprecated endpoint
				getSoftwareResp = getSoftwareResponse{}
				s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/software/%d", teamFooVersion.ID), getSoftwareRequest{}, http.StatusOK, &getSoftwareResp)

				// Get a team count of software vesions using the deprecated endpoint
				var countSoftwareResp countSoftwareResponse
				s.DoJSON("GET", "/api/latest/mobius/software/count", countSoftwareRequest{SoftwareListOptions: mobius.SoftwareListOptions{TeamID: &team1.ID}}, http.StatusOK, &countSoftwareResp)
				require.Equal(t, 1, countSoftwareResp.Count)

				// List all software versions using the deprecated endpoint for a team
				var softwareListResp listSoftwareResponse
				s.DoJSON("GET", "/api/latest/mobius/software", listSoftwareRequest{SoftwareListOptions: mobius.SoftwareListOptions{TeamID: &team1.ID}}, http.StatusOK, &softwareListResp)
...
func genDistributedReqWithPolicyResults(host *mobius.Host, policyResults map[uint]*bool) submitDistributedQueryResultsRequestShim {
	var (
...
func (s *integrationEnterpriseTestSuite) TestCalendarEvents() {
	...
	newHost := func(name string, teamID *uint) *mobius.Host {
		h, err := s.ds.NewHost(ctx, &mobius.Host{
			DetailUpdatedAt: time.Now(),
			LabelUpdatedAt:  time.Now(),
			PolicyUpdatedAt: time.Now(),
			SeenTime:        time.Now().Add(-1 * time.Minute),
	...
	team1Policy1Calendar, err := s.ds.NewTeamPolicy(
		ctx, team1.ID, nil, mobius.PolicyPayload{
	...
	appCfg, err := s.ds.AppConfig(ctx)
	require.NoError(t, err)
	appCfg.Integrations.GoogleCalendar = []*mobius.GoogleCalendarIntegration{
		{
			Domain: "example.com",
	...
	team1.Config.Integrations.GoogleCalendar = &mobius.TeamGoogleCalendarIntegration{
		Enable:     true,
	...
	team2.Config.Integrations.GoogleCalendar = &mobius.TeamGoogleCalendarIntegration{
		Enable:     true,
	...
	err = s.ds.ReplaceHostDeviceMapping(ctx, host1Team1.ID, []*mobius.HostDeviceMapping{
		{
...
func (s *integrationEnterpriseTestSuite) TestCalendarEventsTransferringHosts() {
	...
	err = s.ds.ReplaceHostDeviceMapping(ctx, host1.ID, []*mobius.HostDeviceMapping{
		{
			HostID: host1.ID,
			Email:  "user1@example.com",
			Source: "google_chrome_profiles",
	...
	team2Policy1, err := s.ds.NewTeamPolicy(
		ctx, team2.ID, nil, mobius.PolicyPayload{
			Name:                  "team2Policy1",
			Query:                 "SELECT 2;",
			CalendarEventsEnabled: true,
		},
...
func (s *integrationEnterpriseTestSuite) TestLabelsHostsCounts() {
	...
	users := []mobius.UserPayload{
		{
			...
			AdminForcedPasswordReset: ptr.Bool(false),
			Teams: &[]mobius.UserTeam{
				{Team: mobius.Team{ID: tm1.ID}, Role: mobius.RoleMaintainer},
			},
		},
		{
			...
			AdminForcedPasswordReset: ptr.Bool(false),
			Teams: &[]mobius.UserTeam{
				{Team: mobius.Team{ID: tm2.ID}, Role: mobius.RoleAdmin},
			},
		},
		{
			...
			AdminForcedPasswordReset: ptr.Bool(false),
			Teams: &[]mobius.UserTeam{
				{Team: mobius.Team{ID: tm1.ID}, Role: mobius.RoleObserver},
				{Team: mobius.Team{ID: tm2.ID}, Role: mobius.RoleObserverPlus},
			},
		},
	}
	for _, u := range users {
		var createResp createUserResponse
		s.DoJSON("POST", "/api/latest/mobius/users/admin", u, http.StatusOK, &createResp)
	}
	...
	adminUserPayload := mobius.UserPayload{
	...
	for _, c := range cases {
		t.Run(c.desc, func(t *testing.T) {
			...
			s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/labels/%d", c.lblID), nil, http.StatusOK, &getLbl)
			require.Equal(t, c.want, getLbl.Label.Count)

			var listLbls listLabelsResponse
			s.DoJSON("GET", "/api/latest/mobius/labels", nil, http.StatusOK, &listLbls)
			var found bool
			...
			if c.u != adminUserPayload && (*c.u.Teams)[0].Role != mobius.RoleMaintainer && (*c.u.Teams)[0].Role != mobius.RoleAdmin {
				s.DoJSON("POST", "/api/latest/mobius/labels", createLabelRequest{
					LabelPayload: mobius.LabelPayload{
						Name:  "will fail",
						Query: "select 3",
				...
				s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/labels/%d", c.lblID), modifyLabelRequest{
					ModifyLabelPayload: mobius.ModifyLabelPayload{
...
func (s *integrationEnterpriseTestSuite) TestListHostSoftware() {
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/software", host.ID), nil, http.StatusOK, &getHostSw)
	require.Len(t, getHostSw.Software, 0)
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/software", host.ID), nil, http.StatusOK, &getHostSw, "self_service", "true")
	require.Len(t, getHostSw.Software, 0)

	var getDeviceSw getDeviceSoftwareResponse
	res := s.DoRawNoAuth("GET", "/api/latest/mobius/device/"+token+"/software", nil, http.StatusOK)
	err := json.NewDecoder(res.Body).Decode(&getDeviceSw)
	require.NoError(t, err)
	require.Len(t, getDeviceSw.Software, 0)
	res = s.DoRawNoAuth("GET", "/api/latest/mobius/device/"+token+"/software?self_service=1", nil, http.StatusOK)
	err = json.NewDecoder(res.Body).Decode(&getDeviceSw)
	require.NoError(t, err)
	require.Len(t, getDeviceSw.Software, 0)
	...
	_, err = s.ds.InsertSoftwareVulnerability(ctx, mobius.SoftwareVulnerability{SoftwareID: barSoftwareID, CVE: "CVE-bar-1234"}, mobius.NVDSource)
	require.NoError(t, err)
	_, err = s.ds.InsertSoftwareVulnerability(ctx, mobius.SoftwareVulnerability{SoftwareID: barSoftwareID, CVE: "CVE-bar-5678"}, mobius.NVDSource)
	require.NoError(t, err)

	getHostSw = getHostSoftwareResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/software", host.ID), nil, http.StatusOK, &getHostSw, "vulnerable", "true")
	...
	s.DoJSON("POST", "/api/latest/mobius/labels", &createLabelRequest{mobius.LabelPayload{
		Name:  "label1",
	...
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobius/hosts/%d/software/%d/install",
	...
	require.Equal(t, mobius.SoftwareInstallPending, *getHostSw.Software[2].Status)
	require.NotNil(t, getHostSw.Software[2].SoftwarePackage.SelfService)
	require.True(t, *getHostSw.Software[2].SoftwarePackage.SelfService)
	...
	require.Equal(t, mobius.SoftwareInstallPending, *getDeviceSw.Software[2].Status)
	...
	res = s.DoRawNoAuth("GET", "/api/latest/mobius/device/"+token+"/software/install/"+getDeviceSw.Software[2].SoftwarePackage.LastInstall.InstallUUID+"/results", nil, http.StatusOK)
	...
	require.Equal(t, mobius.SoftwareInstallPending, getDeviceInstallResult.Results.Status)
	s.DoRawNoAuth("GET", "/api/latest/mobius/device/"+token+"/software/install/nope/results", nil, http.StatusNotFound)
	s.DoRawNoAuth("GET", "/api/latest/mobius/device/"+otherToken+"/software/install/"+getDeviceSw.Software[2].SoftwarePackage.LastInstall.InstallUUID+"/results", nil, http.StatusNotFound)
	...
	// test with a query
	getHostSw = getHostSoftwareResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/software", host.ID), nil, http.StatusOK, &getHostSw, "query", "foo")
	...
	res = s.DoRawNoAuth("GET", "/api/latest/mobius/device/"+token+"/software?query=bar", nil, http.StatusOK)
	getDeviceSw = getDeviceSoftwareResponse{}
	err = json.NewDecoder(res.Body).Decode(&getDeviceSw)
	require.NoError(t, err)
	...
	getHostSw = getHostSoftwareResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/software", host.ID), nil, http.StatusOK, &getHostSw, "available_for_install",
	...
	assert.Equal(t, *getHostSw.Software[0].Status, mobius.SoftwareInstallPending)
...
func (s *integrationEnterpriseTestSuite) TestSoftwareInstallerUploadDownloadAndDelete() {
	...
	checkSoftwareInstaller := func(t *testing.T, payload *mobius.UploadSoftwareInstallerPayload) (installerID uint, titleID uint) {
		var tid uint
	...
	t.Run("upload no team software installer", func(t *testing.T) {
		...
		s.lastActivityMatches(mobius.ActivityTypeAddedSoftware{}.ActivityName(), activityData, 0)

		// upload again fails
		s.uploadSoftwareInstaller(t, payload, http.StatusConflict, "already exists")

		// update should succeed
		s.updateSoftwareInstaller(t, &mobius.UpdateSoftwareInstallerPayload{
		...
		"team_id": null, "self_service": true, "software_title_id": %d, "labels_include_any": [{"id": %d, "name": %q}]}`,
			titleID, labelResp.Label.ID, t.Name())
		s.lastActivityMatches(mobius.ActivityTypeEditedSoftware{}.ActivityName(), activityData, 0)
		...
		s.DoRawWithHeaders("PATCH", fmt.Sprintf("/api/latest/mobius/software/titles/%d/package", titleID), body.Bytes(), http.StatusOK, headers)
		...
		resp := s.Do("POST", fmt.Sprintf("/api/latest/mobius/hosts/%d/software/%d/install", host.ID, titleID), nil, http.StatusBadRequest)
		...
		s.DoRawWithHeaders("PATCH", fmt.Sprintf("/api/latest/mobius/software/titles/%d/package", titleID), b3.Bytes(), http.StatusOK, headers)
		expectedPayload.PreInstallQuery = "some other pre install query"
		expectedPayload.LabelsIncludeAny = []string{labelResp.Label.Name}
		expectedPayload.LabelsExcludeAny = nil
		checkSoftwareInstaller(t, &expectedPayload)

		s.Do("POST", fmt.Sprintf("/api/latest/mobius/hosts/%d/software/%d/install", host.ID, titleID), nil, http.StatusAccepted)
		...
		s.Do("POST", "/api/mobius/orbit/software_install/package?alt=media", orbitDownloadSoftwareInstallerRequest{
			InstallerID:  123,
			OrbitNodeKey: uuid.NewString(),
		}, http.StatusUnauthorized)

		// download the installer
		s.Do("GET", fmt.Sprintf("/api/latest/mobius/software/titles/%d/package?alt=media", titleID), nil, http.StatusBadRequest)

		// delete the installer from nil team fails
		s.Do("DELETE", fmt.Sprintf("/api/latest/mobius/software/titles/%d/available_for_install", titleID), nil, http.StatusBadRequest)

		// delete from team 0 succeeds
		s.Do("DELETE", fmt.Sprintf("/api/latest/mobius/software/titles/%d/available_for_install", titleID), nil, http.StatusNoContent, "team_id", "0")
		activityData = fmt.Sprintf(`{"software_title": "ruby", "software_package": "ruby.deb", "team_name": null,
		"team_id": null, "self_service": true, "labels_include_any": [{"id": %d, "name": %q}]}`,
			labelResp.Label.ID, labelResp.Label.Name)
		s.lastActivityMatches(mobius.ActivityTypeDeletedSoftware{}.ActivityName(), activityData, 0)
	})

	t.Run("create team software installer", func(t *testing.T) {
		var createTeamResp teamResponse
		s.DoJSON("POST", "/api/latest/mobius/teams", &mobius.Team{
			Name: t.Name(),
		...
		s.lastActivityOfTypeMatches(mobius.ActivityTypeAddedSoftware{}.ActivityName(), activityData, 0)

		// upload again fails
		s.uploadSoftwareInstaller(t, payload, http.StatusConflict, "already exists")

		// download the installer
		r := s.Do("GET", fmt.Sprintf("/api/latest/mobius/software/titles/%d/package?alt=media", titleID), nil, http.StatusOK, "team_id", fmt.Sprintf("%d", *payload.TeamID))
		...
		s.DoJSON("POST", fmt.Sprintf("/api/latest/mobius/software/titles/%d/package/token?alt=media", titleID), nil, http.StatusOK,
			&tokenResp, "team_id", fmt.Sprintf("%d", *payload.TeamID))
		require.NotEmpty(t, tokenResp.Token)
		r = s.DoRawNoAuth("GET", fmt.Sprintf("/api/latest/mobius/software/titles/%d/package/token/%s", titleID, tokenResp.Token), nil,
			http.StatusOK)
		checkDownloadResponse(t, r, payload.Filename)

		// downloading a second time using the same token should fail
		_ = s.DoRawNoAuth("GET", fmt.Sprintf("/api/latest/mobius/software/titles/%d/package/token/%s", titleID, tokenResp.Token), nil,
			http.StatusForbidden)

		// alt != media should fail
		s.DoJSON("POST", fmt.Sprintf("/api/latest/mobius/software/titles/%d/package/token?alt=bozo", titleID), nil,
			http.StatusUnprocessableEntity,
			&tokenResp, "team_id", fmt.Sprintf("%d", *payload.TeamID))

		// missing team_id should fail
		s.DoJSON("POST", fmt.Sprintf("/api/latest/mobius/software/titles/%d/package/token?alt=media", titleID), nil,
			http.StatusBadRequest,
			&tokenResp)
		...
		s.Do("POST", fmt.Sprintf("/api/latest/mobius/hosts/%d/software/%d/install", hostInTeam.ID, titleID), installSoftwareRequest{}, http.StatusAccepted)

		// requesting download with alt != media fails
		r = s.Do("POST", "/api/mobius/orbit/software_install/package?alt=FOOBAR", orbitDownloadSoftwareInstallerRequest{
			InstallerID:  installerID,
		...
		// valid download
		r = s.Do("POST", "/api/mobius/orbit/software_install/package?alt=media", orbitDownloadSoftwareInstallerRequest{
			InstallerID:  installerID,
			OrbitNodeKey: *hostInTeam.OrbitNodeKey,
		}, http.StatusOK)
		checkDownloadResponse(t, r, payload.Filename)
		...
		s.Do("POST", "/api/mobius/orbit/software_install/result", orbitPostSoftwareInstallResultRequest{
			OrbitNodeKey: *hostInTeam.OrbitNodeKey,
			HostSoftwareInstallResultPayload: &mobius.HostSoftwareInstallResultPayload{
		...
		_ = s.Do("POST", "/api/mobius/orbit/software_install/package?alt=media", orbitDownloadSoftwareInstallerRequest{
			InstallerID:  installerID,
			OrbitNodeKey: *hostInTeam.OrbitNodeKey,
		}, http.StatusForbidden)

		// delete the installer
		s.Do("DELETE", fmt.Sprintf("/api/latest/mobius/software/titles/%d/available_for_install", titleID), nil, http.StatusNoContent, "team_id", fmt.Sprintf("%d", *payload.TeamID))

		// check activity
		s.lastActivityOfTypeMatches(mobius.ActivityTypeDeletedSoftware{}.ActivityName(), fmt.Sprintf(`{"software_title": "ruby", "software_package": "ruby.deb", "team_name": "%s", "team_id": %d, "self_service": true}`, createTeamResp.Team.Name, createTeamResp.Team.ID), 0)

		// download the installer, not found anymore
		s.Do("GET", fmt.Sprintf("/api/latest/mobius/software/titles/%d/package?alt=media", titleID), nil, http.StatusNotFound, "team_id", fmt.Sprintf("%d", *payload.TeamID))
	})
	...
	t.Run("create team 0 software installer", func(t *testing.T) {
		...
		// check activity
		s.lastActivityOfTypeMatches(mobius.ActivityTypeAddedSoftware{}.ActivityName(),
		...
		// download the installer
		r := s.Do("GET", fmt.Sprintf("/api/latest/mobius/software/titles/%d/package?alt=media", titleID), nil, http.StatusOK, "team_id", fmt.Sprintf("%d", 0))
		checkDownloadResponse(t, r, payload.Filename)
		...
		// check activity
		s.lastActivityOfTypeMatches(mobius.ActivityTypeDeletedSoftware{}.ActivityName(),
			`{"software_title": "ruby", "software_package": "ruby.deb", "team_name": null, "team_id": null, "self_service": true}`, 0)

		// download the installer, not found anymore
		s.Do("GET", fmt.Sprintf("/api/latest/mobius/software/titles/%d/package?alt=media", titleID), nil, http.StatusNotFound, "team_id", fmt.Sprintf("%d", 0))
	})
	...
	t.Run("uninstall migration for software installer", func(t *testing.T) {
		...
		respTitle := getSoftwareTitleResponse{}
		s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/software/titles/%d", titleID), nil, http.StatusOK, &respTitle, "team_id",
		...
		// delete the installer
		s.Do("DELETE", fmt.Sprintf("/api/latest/mobius/software/titles/%d/available_for_install", titleID), nil, http.StatusNoContent,
...
func (s *integrationEnterpriseTestSuite) TestApplyTeamsSoftwareConfig() {
	...
	wantSoftwarePackages := []mobius.SoftwarePackageSpec{
		{
			URL:               "http://foo.com",
			SelfService:       true,
			InstallScript:     mobius.TeamSpecSoftwareAsset{Path: "./foo/install-script.sh"},
			PostInstallScript: mobius.TeamSpecSoftwareAsset{Path: "./foo/post-install-script.sh"},
			PreInstallQuery:   mobius.TeamSpecSoftwareAsset{Path: "./foo/query.yaml"},
		},
		{
			URL:               "http://bar.com",
			SelfService:       false,
			InstallScript:     mobius.TeamSpecSoftwareAsset{Path: "./bar/install-script.sh"},
			PostInstallScript: mobius.TeamSpecSoftwareAsset{Path: "./bar/post-install-script.sh"},
			PreInstallQuery:   mobius.TeamSpecSoftwareAsset{Path: "./bar/query.yaml"},
		},
	}
	wantAppStoreApps := []mobius.TeamSpecAppStoreApp{
		{
			AppStoreID: "1234",
		},
		{
...
func (s *integrationEnterpriseTestSuite) TestBatchSetSoftwareInstallers() {
	...
	// non-existent team
	s.Do("POST", "/api/latest/mobius/software/batch", batchSetSoftwareInstallersRequest{}, http.StatusNotFound, "team_name", "foo")

	// create a team
	tm, err := s.ds.NewTeam(context.Background(), &mobius.Team{
		Name:        t.Name(),
		Description: "desc",
	})
	require.NoError(t, err)
	...
	softwareToInstall := []*mobius.SoftwareInstallerPayload{
		{URL: "."},
	}
	s.Do("POST", "/api/latest/mobius/software/batch", batchSetSoftwareInstallersRequest{Software: softwareToInstall}, http.StatusUnprocessableEntity, "team_name", tm.Name)
	...
	s.DoJSON("POST", "/api/latest/mobius/software/batch", batchSetSoftwareInstallersRequest{Software: softwareToInstall}, http.StatusAccepted, &batchResponse, "team_name", tm.Name)
	message := waitBatchSetSoftwareInstallersFailed(t, s, tm.Name, batchResponse.RequestUUID)
	require.NotEmpty(t, message)
	require.Contains(t, message, fmt.Sprintf("validation failed: software.url Couldn't edit software. URL (\"%s/not_found.pkg\") returned \"Not Found\". Please make sure that URLs are reachable from your Mobius server.", srv.URL))
	...
	softwareToInstallBadSecret := []*mobius.SoftwareInstallerPayload{
		{
			URL:           rubyURL,
			InstallScript: "echo $MOBIUS_SECRET_INVALID",
		},
	}
	resp := s.Do("POST", "/api/latest/mobius/software/batch", batchSetSoftwareInstallersRequest{Software: softwareToInstallBadSecret}, http.StatusUnprocessableEntity, "team_name", tm.Name)
	errMsg := extractServerErrorText(resp.Body)
	require.Contains(t, errMsg, "$MOBIUS_SECRET_INVALID")

	softwareToInstallBadSecret[0].InstallScript = ""
	softwareToInstallBadSecret[0].PostInstallScript = "echo $MOBIUS_SECRET_ALSO_INVALID"
	resp = s.Do("POST", "/api/latest/mobius/software/batch", batchSetSoftwareInstallersRequest{Software: softwareToInstallBadSecret}, http.StatusUnprocessableEntity, "team_name", tm.Name)
	errMsg = extractServerErrorText(resp.Body)
	require.Contains(t, errMsg, "$MOBIUS_SECRET_ALSO_INVALID")

	softwareToInstallBadSecret[0].PostInstallScript = ""
	softwareToInstallBadSecret[0].UninstallScript = "echo $MOBIUS_SECRET_THIRD_INVALID"
	resp = s.Do("POST", "/api/latest/mobius/software/batch", batchSetSoftwareInstallersRequest{Software: softwareToInstallBadSecret}, http.StatusUnprocessableEntity, "team_name", tm.Name)
	errMsg = extractServerErrorText(resp.Body)
	require.Contains(t, errMsg, "$MOBIUS_SECRET_THIRD_INVALID")

	// TODO(roberto): test with a variety of response codes

	// check the application status
	titlesResp := listSoftwareTitlesResponse{}
	s.DoJSON("GET", "/api/v1/mobius/software/titles", nil, http.StatusOK, &titlesResp, "available_for_install", "true", "team_id",
		fmt.Sprint(tm.ID))
	...
	s.DoJSON("GET", "/api/v1/mobius/software/titles", nil, http.StatusOK, &newTitlesResp, "available_for_install", "true", "team_id",
		fmt.Sprint(tm.ID))
	...
	s.DoJSON("POST", "/api/latest/mobius/software/batch", batchSetSoftwareInstallersRequest{Software: softwareToInstall}, http.StatusAccepted, &batchResponse)
	...
	titlesResp = listSoftwareTitlesResponse{}
	s.DoJSON("GET", "/api/v1/mobius/software/titles", nil, http.StatusOK, &titlesResp, "available_for_install", "true", "team_id", strconv.Itoa(int(0)))
	require.Equal(t, 1, titlesResp.Count)
	require.Len(t, titlesResp.SoftwareTitles, 1)
	...
	require.Nil(t, packages[0].TeamID)
	newTitlesResp = listSoftwareTitlesResponse{}
	s.DoJSON("GET", "/api/v1/mobius/software/titles", nil, http.StatusOK, &newTitlesResp, "available_for_install", "true", "team_id", strconv.Itoa(int(0)))
	require.Equal(t, titlesResp, newTitlesResp)
	...
	lblA, err := s.ds.NewLabel(ctx, &mobius.Label{Name: "A"})
	require.NoError(t, err)
	lblB, err := s.ds.NewLabel(ctx, &mobius.Label{Name: "B"})
	require.NoError(t, err)
	...
	res := s.Do("POST", "/api/latest/mobius/software/batch", batchSetSoftwareInstallersRequest{Software: softwareToInstall}, http.StatusBadRequest)
	...
	//////////////////////////
	// Do a request with a mobius maintained app
	//////////////////////////
	maintained1, err := s.ds.UpsertMaintainedApp(ctx, &mobius.MaintainedApp{
		Name:             "1Password",
...
func waitBatchSetSoftwareInstallersCompleted(t *testing.T, s *integrationEnterpriseTestSuite, teamName string, requestUUID string) []mobius.SoftwarePackageResponse {
	timeout := time.After(1 * time.Minute)
	for {
		var batchResultResponse batchSetSoftwareInstallersResultResponse
		s.DoJSON("GET", "/api/latest/mobius/software/batch/"+requestUUID, nil, http.StatusOK, &batchResultResponse, "team_name", teamName)
		if batchResultResponse.Status == mobius.BatchSetSoftwareInstallersStatusCompleted {
...
func waitBatchSetSoftwareInstallersFailed(t *testing.T, s *integrationEnterpriseTestSuite, teamName string, requestUUID string) string {
	...
	for {
		var batchResultResponse batchSetSoftwareInstallersResultResponse
		s.DoJSON("GET", "/api/latest/mobius/software/batch/"+requestUUID, nil, http.StatusOK, &batchResultResponse, "team_name", teamName)
		if batchResultResponse.Status == mobius.BatchSetSoftwareInstallersStatusFailed {
...
func (s *integrationEnterpriseTestSuite) TestBatchSetSoftwareInstallersSideEffects() {
	...
	s.DoJSON("GET", fmt.Sprintf("/api/v1/mobius/software/titles/%d", titlesResp.SoftwareTitles[0].ID), nil, http.StatusOK, &titleResponse,
		"team_id", fmt.Sprint(tm.ID))
	uploadedAt := titleResponse.SoftwareTitle.SoftwarePackage.UploadedAt

	// create a host that doesn't have mobiusdaemon installed
	h, err := s.ds.NewHost(context.Background(), &mobius.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
		SeenTime:        time.Now().Add(-1 * time.Minute),
	...
	// host installs mobiusdaemon
	orbitKey := setOrbitEnrollment(t, h, s.ds)
	h.OrbitNodeKey = &orbitKey

	// create another host that doesn't have mobiusdaemon installed
	h2, err := s.ds.NewHost(context.Background(), &mobius.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
		SeenTime:        time.Now().Add(-1 * time.Minute),
	...
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobius/hosts/%d/software/%d/install", h.ID, titlesResp.SoftwareTitles[0].ID), nil, http.StatusAccepted, &installResp)

	// Get the install response, should be pending
	getHostSoftwareResp := getHostSoftwareResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/software", h.ID), nil, http.StatusOK, &getHostSoftwareResp)
	require.Equal(t, mobius.SoftwareInstallPending, *getHostSoftwareResp.Software[0].Status)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/software", h.ID), nil, http.StatusOK, &afterSelfServiceHostResp)
	require.Equal(t, mobius.SoftwareInstallPending, *getHostSoftwareResp.Software[0].Status)

	// update pre-install query
	withUpdatedPreinstallQuery := []*mobius.SoftwareInstallerPayload{
		{URL: srv.URL, PreInstallQuery: "SELECT * FROM os_version"},
	}
	s.DoJSON("POST", "/api/latest/mobius/software/batch", batchSetSoftwareInstallersRequest{Software: withUpdatedPreinstallQuery}, http.StatusAccepted, &batchResponse, "team_name", tm.Name)
	packages = waitBatchSetSoftwareInstallersCompleted(t, s, tm.Name, batchResponse.RequestUUID)
	require.Len(t, packages, 1)
	require.NotNil(t, packages[0].TitleID)
	require.NotNil(t, packages[0].TeamID)
	...
	require.Equal(t, srv.URL, packages[0].URL)
	titleResponse = getSoftwareTitleResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/v1/mobius/software/titles/%d", newTitlesResp.SoftwareTitles[0].ID), nil, http.StatusOK, &titleResponse,
		"team_id", fmt.Sprint(tm.ID))
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/software", h.ID), nil, http.StatusOK, &afterPreinstallHostResp)
	...
	s.Do("POST", "/api/mobius/orbit/software_install/result", json.RawMessage(fmt.Sprintf(`{
			"orbit_node_key": %q,
			"install_uuid": %q,
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/software", h.ID), nil, http.StatusOK, &hostResp)
	require.Equal(t, mobius.SoftwareInstalled, *hostResp.Software[0].Status)

	// update install script
	withUpdatedInstallScript := []*mobius.SoftwareInstallerPayload{
		{URL: srv.URL, InstallScript: "apt install ruby"},
	}
	s.DoJSON("POST", "/api/latest/mobius/software/batch", batchSetSoftwareInstallersRequest{Software: withUpdatedInstallScript}, http.StatusAccepted, &batchResponse, "team_name", tm.Name)
	packages = waitBatchSetSoftwareInstallersCompleted(t, s, tm.Name, batchResponse.RequestUUID)
	require.Len(t, packages, 1)
	require.NotNil(t, packages[0].TitleID)
	require.NotNil(t, packages[0].TeamID)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/software/install/%s/results", installUUID), nil, http.StatusOK, &installDetailsResp)
	require.Equal(t, mobius.SoftwareInstalled, installDetailsResp.Results.Status)

	// queue another install before we delete the installer via batch
	pendingResp := installSoftwareResponse{}
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobius/hosts/%d/software/%d/install", h.ID, titlesResp.SoftwareTitles[0].ID), nil, http.StatusAccepted, &pendingResp)

	// install should show as pending
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/software", h.ID), nil, http.StatusOK, &afterPreinstallHostResp)
	require.Equal(t, mobius.SoftwareInstallPending, *afterPreinstallHostResp.Software[0].Status)
	...
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobius/hosts/%d/software/%d/uninstall", h2.ID, titlesResp.SoftwareTitles[0].ID), nil, http.StatusAccepted, &uninstallResp)

	// uninstall should show as pending
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/software", h2.ID), nil, http.StatusOK, &afterPreinstallHostResp)
	require.Equal(t, mobius.SoftwareUninstallPending, *afterPreinstallHostResp.Software[0].Status)

	// delete all installers
	s.DoJSON("POST", "/api/latest/mobius/software/batch", batchSetSoftwareInstallersRequest{Software: []*mobius.SoftwareInstallerPayload{}}, http.StatusAccepted, &batchResponse, "team_name", tm.Name)
	packages = waitBatchSetSoftwareInstallersCompleted(t, s, tm.Name, batchResponse.RequestUUID)
	...
	installDetailsResp = getSoftwareInstallResultsResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/software/install/%s/results", installUUID), nil, http.StatusNotFound, &installDetailsResp)
}
...
func (s *integrationEnterpriseTestSuite) TestBatchSetSoftwareInstallersWithPoliciesAssociated() {
	...
	team1, err := s.ds.NewTeam(ctx, &mobius.Team{Name: "team1"})
	require.NoError(t, err)
	team2, err := s.ds.NewTeam(ctx, &mobius.Team{Name: "team2"})
	require.NoError(t, err)
	...
	var batchResponse batchSetSoftwareInstallersResponse
	s.DoJSON("POST", "/api/latest/mobius/software/batch", batchSetSoftwareInstallersRequest{Software: softwareToInstall}, http.StatusAccepted, &batchResponse, "team_name", team1.Name)
	...
	s.DoJSON("POST", "/api/latest/mobius/software/batch", batchSetSoftwareInstallersRequest{Software: softwareToInstall}, http.StatusAccepted, &batchResponse, "team_name", team2.Name)
	...
	mtplr := modifyTeamPolicyResponse{}
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/teams/%d/policies/%d", team1.ID, policy1Team1.ID), modifyTeamPolicyRequest{
		ModifyPolicyPayload: mobius.ModifyPolicyPayload{
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/teams/%d/policies/%d", team2.ID, policy2Team2.ID), modifyTeamPolicyRequest{
		ModifyPolicyPayload: mobius.ModifyPolicyPayload{
			SoftwareTitleID: optjson.Any[uint]{Set: true, Valid: true, Value: rubyDebTitleID},
		},
	}, http.StatusOK, &mtplr)
...
func (s *integrationEnterpriseTestSuite) TestSoftwareInstallerNewInstallRequestPlatformValidation() {
	...
	hostsByPlatform := map[string]*mobius.Host{
	...
	for _, tc := range testCases {
		for platform, host := range hostsByPlatform {
			for _, kind := range tc.supportedInstallers {
				...
				s.DoJSON("POST", fmt.Sprintf("/api/latest/mobius/hosts/%d/software/%d/install", host.ID, softwareTitles[kind]), nil,
					wantStatus, &resp)
			}
		}
	}
...
func (s *integrationEnterpriseTestSuite) TestSoftwareInstallerHostRequests() {
	...
	var resp installSoftwareResponse
	// non-existent host
	s.DoJSON("POST", "/api/latest/mobius/hosts/1/software/1/install", nil, http.StatusNotFound, &resp)
	s.DoJSON("POST", "/api/latest/mobius/hosts/1/software/1/uninstall", nil, http.StatusNotFound, &resp)
	...
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobius/hosts/%d/software/1/install", h.ID), nil, http.StatusUnprocessableEntity, &resp)
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobius/hosts/%d/software/1/uninstall", h.ID), nil, http.StatusUnprocessableEntity, &resp)
	...
	resp = installSoftwareResponse{}
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobius/hosts/%d/software/1/install", h.ID), nil, http.StatusBadRequest, &resp)
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobius/hosts/%d/software/1/uninstall", h.ID), nil, http.StatusBadRequest, &resp)
	...
	payloadDummy := &mobius.UploadSoftwareInstallerPayload{
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/software/titles/%d", pkgTitleID), nil, http.StatusOK, &respTitle, "team_id",
		fmt.Sprintf("%d", *teamID))
	require.NotNil(t, respTitle.SoftwareTitle.SoftwarePackage)
	...
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobius/hosts/%d/software/%d/install", h.ID, pkgTitleID), nil, http.StatusBadRequest, &resp)
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobius/hosts/%d/software/%d/uninstall", h.ID, pkgTitleID), nil, http.StatusBadRequest, &resp)

	// delete software installer which we will not use
	s.Do("DELETE", fmt.Sprintf("/api/latest/mobius/software/titles/%d/available_for_install", pkgTitleID), nil, http.StatusNoContent,
		"team_id", fmt.Sprintf("%d", *teamID))

	// install software request succeeds
	resp = installSoftwareResponse{}
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobius/hosts/%d/software/%d/install", h.ID, titleID), nil, http.StatusAccepted, &resp)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/software/install/%s/results", installUUID), nil, http.StatusOK, &gsirr)
	...
	require.Equal(t, mobius.SoftwareInstallPending, results.Status)

	// Can't install/uninstall if software install is pending
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobius/hosts/%d/software/%d/install", h.ID, titleID), nil, http.StatusBadRequest, &resp)
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobius/hosts/%d/software/%d/uninstall", h.ID, titleID), nil, http.StatusBadRequest, &resp)
	...
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobius/hosts/%d/software/%d/install", h2.ID, titleID), nil, http.StatusAccepted, &resp)
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/software", h2.ID), nil, http.StatusOK, &getHostSoftwareResp)
	require.Len(t, getHostSoftwareResp.Software, 1)
	...
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobius/hosts/%d/software/%d/install", h3.ID, titleID), nil, http.StatusAccepted, &resp)
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/software", h3.ID), nil, http.StatusOK, &getHostSoftwareResp)
	require.Len(t, getHostSoftwareResp.Software, 1)
	...
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobius/hosts/%d/software/%d/install", h4.ID, titleID), nil, http.StatusAccepted, &resp)
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/software", h4.ID), nil, http.StatusOK, &getHostSoftwareResp)
	require.Len(t, getHostSoftwareResp.Software, 1)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/software/titles/%d", titleID), nil, http.StatusOK, &titleResp, "team_id",
		fmt.Sprint(*teamID))
	...
	require.Equal(t, mobius.SoftwareInstallerStatusSummary{
		Installed:      1,
		PendingInstall: 2,
		FailedInstall:  1,
	...
	for _, c := range cases {
		t.Run(c.status, func(t *testing.T) {
			...
			s.DoJSON("GET", "/api/latest/mobius/hosts", nil, http.StatusOK, &listResp, "software_status", c.status, "team_id",
			...
			var countResp countHostsResponse
			s.DoJSON("GET", "/api/latest/mobius/hosts/count", nil, http.StatusOK, &countResp, "software_status", c.status, "team_id",
				fmt.Sprint(*teamID), "software_title_id", fmt.Sprint(titleID))
			...
			s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/labels/%d/hosts", labelResp.Label.ID), nil, http.StatusOK, &listResp,
	...
	// filter validations
	r := s.Do("GET", "/api/latest/mobius/hosts", nil, http.StatusBadRequest, "software_status", "uninstalled")
	require.Contains(t, extractServerErrorText(r.Body), "Invalid software_status")
	r = s.Do("GET", "/api/latest/mobius/hosts", nil, http.StatusBadRequest, "software_status", "installed")
	require.Contains(t, extractServerErrorText(r.Body), "Missing software_title_id")
	r = s.Do("GET", "/api/latest/mobius/hosts", nil, http.StatusBadRequest, "software_status", "installed", "software_title_id", "1")
	require.Contains(t, extractServerErrorText(r.Body), "Missing team_id")
	r = s.Do("GET", "/api/latest/mobius/hosts", nil, http.StatusBadRequest, "software_status", "installed", "team_id", "1")
	require.Contains(t, extractServerErrorText(r.Body), "Missing software_title_id")
	r = s.Do("GET", "/api/latest/mobius/hosts", nil, http.StatusBadRequest, "software_status", "installed", "team_id", "1", "software_title_id", "1", "software_version_id", "1")
	...
	r = s.Do("GET", "/api/latest/mobius/hosts", nil, http.StatusBadRequest, "software_status", "installed", "team_id", "1", "software_title_id", "1", "software_id", "1")
	...
	for _, status := range validStatuses {
		t.Run("nonexistent_title_id_"+status, func(t *testing.T) {
			// List hosts
			var listResp listHostsResponse
			s.DoJSON("GET", "/api/latest/mobius/hosts", nil, http.StatusOK, &listResp,
	...
	s.Do("POST", fmt.Sprintf("/api/latest/mobius/hosts/%d/lock", h.ID), nil, http.StatusOK)
	...
	s.DoJSON("POST", "/api/mobius/orbit/scripts/result",
		...
		http.StatusOK, &orbitScriptResp)
	s.Do("POST", fmt.Sprintf("/api/latest/mobius/hosts/%d/unlock", h.ID), nil, http.StatusOK)
	status, err = s.ds.GetHostLockWipeStatus(context.Background(), h)
	require.NoError(t, err)
	s.DoJSON("POST", "/api/mobius/orbit/scripts/result",
	...
	// Do uninstall on h
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobius/hosts/%d/software/%d/uninstall", h.ID, titleID), nil, http.StatusAccepted, &resp)
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/software", h.ID), nil, http.StatusOK, &getHostSoftwareResp)
	require.Len(t, getHostSoftwareResp.Software, 1)
	assert.NotNil(t, getHostSoftwareResp.Software[0].SoftwarePackage.LastInstall)
	assert.Equal(t, mobius.SoftwareUninstallPending, *getHostSoftwareResp.Software[0].Status)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/activities/upcoming", h.ID), nil, http.StatusOK, &listUpcomingAct)
	require.Len(t, listUpcomingAct.Activities, 1)
	assert.Equal(t, mobius.ActivityTypeUninstalledSoftware{}.ActivityName(), listUpcomingAct.Activities[0].Type)
	details := make(map[string]interface{}, 5)
	require.NoError(t, json.Unmarshal(*listUpcomingAct.Activities[0].Details, &details))
	assert.EqualValues(t, mobius.SoftwareUninstallPending, details["status"])

	// should be visible from My device
	s.DoRawNoAuth("GET", fmt.Sprintf("/api/v1/mobius/device/%s/software/uninstall/%s/results", token, uninstallExecutionID), nil, http.StatusOK)
	...
	require.NotNil(t, titleResp.SoftwareTitle.SoftwarePackage.Status)
	assert.Equal(t, mobius.SoftwareInstallerStatusSummary{
		PendingInstall:   1,
		FailedInstall:    1,
		PendingUninstall: 1,
	}, *titleResp.SoftwareTitle.SoftwarePackage.Status)
	...
	// Check activity feed
	var activitiesResp listActivitiesResponse
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/activities", h.ID), nil, http.StatusOK, &activitiesResp, "order_key", "a.id",
		"order_direction", "desc")
	require.NotEmpty(t, activitiesResp.Activities)
	assert.Equal(t, mobius.ActivityTypeUninstalledSoftware{}.ActivityName(), activitiesResp.Activities[0].Type)
	details = make(map[string]interface{}, 5)
	...
	results = gsirr.Results
	require.Equal(t, installUUID, results.InstallUUID)
	require.Equal(t, mobius.SoftwareInstalled, results.Status)

	var scriptResultResp getScriptResultResponse
	s.DoJSON("GET", "/api/latest/mobius/scripts/results/"+uninstallExecutionID, nil, http.StatusOK, &scriptResultResp)
...
func (s *integrationEnterpriseTestSuite) TestSelfServiceSoftwareInstallUninstall() {
	...
	payloadNoSS := &mobius.UploadSoftwareInstallerPayload{
	...
	payloadSS := &mobius.UploadSoftwareInstallerPayload{
	...
	res := s.DoRawNoAuth("POST", fmt.Sprintf("/api/v1/mobius/device/%s/software/install/%d", token, titleIDNoSS), nil, http.StatusBadRequest)
	errMsg := extractServerErrorText(res.Body)
	...
	payloadLabelSS := &mobius.UploadSoftwareInstallerPayload{
	...
	resp := s.DoRawNoAuth("POST", fmt.Sprintf("/api/v1/mobius/device/%s/software/install/%d", token, titleIDLabelSS), nil, http.StatusBadRequest)
	require.Contains(t, extractServerErrorText(resp.Body), "Couldn't install. Host isn't member of the labels defined for this software title.")

	// request self-install of software that allows it (is self-service + label scoped)
	s.DoRawNoAuth("POST", fmt.Sprintf("/api/v1/mobius/device/%s/software/install/%d", token, titleIDSS), nil, http.StatusAccepted)

	// it shows up as "self-installed" in the upcoming activities of the host
	var listUpcomingAct listHostUpcomingActivitiesResponse
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/activities/upcoming", host1.ID), nil, http.StatusOK, &listUpcomingAct)
	require.Len(t, listUpcomingAct.Activities, 1)
	require.Nil(t, listUpcomingAct.Activities[0].ActorID)
	require.False(t, listUpcomingAct.Activities[0].MobiusInitiated)

	var details mobius.ActivityTypeInstalledSoftware
	...
	require.EqualValues(t, mobius.SoftwareInstallPending, details.Status)
	installID := details.InstallUUID

	// record the installation results
	s.Do("POST", "/api/mobius/orbit/software_install/result",
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/activities", host1.ID), nil, http.StatusOK, &listPastAct)
	require.Len(t, listPastAct.Activities, 1)
	require.Nil(t, listPastAct.Activities[0].ActorID)
	require.False(t, listPastAct.Activities[0].MobiusInitiated)
	...
	require.Equal(t, details.SoftwareTitle, payloadSS.Title)
	require.True(t, details.SelfService)
	require.EqualValues(t, mobius.SoftwareInstalled, details.Status)

	// Do uninstall on host
	s.DoRawNoAuth("POST", fmt.Sprintf("/api/v1/mobius/device/%s/software/uninstall/%d", token, titleIDSS), nil, http.StatusAccepted)
	var getHostSoftwareResp getHostSoftwareResponse
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/software", host1.ID), nil, http.StatusOK, &getHostSoftwareResp)
	...
	assert.EqualValues(t, mobius.SoftwareUninstallPending, uninstallDetails["status"])
	assert.EqualValues(t, true, uninstallDetails["self_service"])
	assert.Nil(t, listUpcomingAct.Activities[0].ActorID)
	assert.False(t, listUpcomingAct.Activities[0].MobiusInitiated)

	// Check uninstall results via device endpoint before execution
	res = s.DoRawNoAuth("GET", fmt.Sprintf("/api/v1/mobius/device/%s/software/uninstall/%s/results", token, uninstallExecutionID), nil, http.StatusOK)
	...
	// Check activity feed
	var activitiesResp listActivitiesResponse
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/activities", host1.ID), nil, http.StatusOK, &activitiesResp, "order_key", "a.id",
		"order_direction", "desc")
	require.NotEmpty(t, activitiesResp.Activities)
	assert.Equal(t, mobius.ActivityTypeUninstalledSoftware{}.ActivityName(), activitiesResp.Activities[0].Type)
	uninstallDetails = make(map[string]interface{}, 5)
	...
	s.DoRawNoAuth("GET", fmt.Sprintf("/api/v1/mobius/device/%s/software/uninstall/%s/results", token2, uninstallExecutionID), nil, http.StatusNotFound)
	s.DoRawNoAuth("GET", fmt.Sprintf("/api/v1/mobius/device/%s/software/uninstall/%s/results", token, uninstallExecutionID+`f`), nil, http.StatusNotFound)
}
...
func (s *integrationEnterpriseTestSuite) TestHostSoftwareInstallResult() {
	...
	s.uploadSoftwareInstaller(t, payload, http.StatusOK, "")
	titleID := getSoftwareTitleID(t, s.ds, payload.Title, "deb_packages")
	payload2 := &mobius.UploadSoftwareInstallerPayload{
	...
	payload3 := &mobius.UploadSoftwareInstallerPayload{
	...
	for i := 0; i < len(installUUIDs); i++ {
		resp := installSoftwareResponse{}
		s.DoJSON("POST", fmt.Sprintf("/api/v1/mobius/hosts/%d/software/%d/install", host.ID, titleIDs[i]), nil, http.StatusAccepted, &resp)
	...
	checkResults := func(want result) {
		...
		s.DoJSON("GET", fmt.Sprintf("/api/v1/mobius/software/install/%s/results", want.InstallUUID), nil, http.StatusOK, &resp)
	...
	checkResults(result{
		...
		PreInstallQueryOutput: ptr.String(mobius.SoftwareInstallerQuerySuccessCopy),
		Output:                ptr.String(fmt.Sprintf(mobius.SoftwareInstallerInstallFailCopy, "failed")),
	})
	wantAct := mobius.ActivityTypeInstalledSoftware{
		HostID:          host.ID,
		HostDisplayName: host.DisplayName(),
		SoftwareTitle:   payload.Title,
		SoftwarePackage: payload.Filename,
		InstallUUID:     installUUIDs[0],
		Status:          string(mobius.SoftwareInstallFailed),
	}
	...
	checkResults(result{
		...
		Status:                mobius.SoftwareInstallFailed,
		PreInstallQueryOutput: ptr.String(mobius.SoftwareInstallerQueryFailCopy),
	})
	wantAct = mobius.ActivityTypeInstalledSoftware{
		HostID:          host.ID,
		HostDisplayName: host.DisplayName(),
	...
	checkResults(result{
		...
		PreInstallQueryOutput:   ptr.String(mobius.SoftwareInstallerQuerySuccessCopy),
		Output:                  ptr.String(fmt.Sprintf(mobius.SoftwareInstallerInstallSuccessCopy, "success")),
		PostInstallScriptOutput: ptr.String(fmt.Sprintf(mobius.SoftwareInstallerPostInstallSuccessCopy, "ok")),
	})
	wantAct = mobius.ActivityTypeInstalledSoftware{
		HostID:          host.ID,
		HostDisplayName: host.DisplayName(),
		...
		InstallUUID:     installUUIDs[2],
		Status:          string(mobius.SoftwareInstalled),
	}
	...
	s.Do("POST", "/api/mobius/orbit/software_install/result",
		...
		}`, *host.OrbitNodeKey, installUUIDs[2], mobius.ExitCodeInstallerDownloadFailed)),
		http.StatusNoContent)
	checkResults(result{
		HostID:      host.ID,
		InstallUUID: installUUIDs[2],
		Status:      mobius.SoftwareInstallFailed,
		Output:      ptr.String(mobius.SoftwareInstallerDownloadFailedCopy),
	})
	wantAct = mobius.ActivityTypeInstalledSoftware{
		HostID:          host.ID,
		HostDisplayName: host.DisplayName(),
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/activities", host.ID), nil, http.StatusOK, &hostActivityResp)
	require.Len(t, hostActivityResp.Activities, 4)
	require.NotNil(t, hostActivityResp.Activities[0].Details)
	var actDetails mobius.ActivityTypeInstalledSoftware
...
func (s *integrationEnterpriseTestSuite) TestHostScriptSoftDelete() {
	...
	tm, err := s.ds.NewTeam(ctx, &mobius.Team{Name: "host_soft_delete_team"})
	require.NoError(t, err)
	host := createOrbitEnrolledHost(t, "linux", "", s.ds)
	...
	s.Do("POST", "/api/mobius/orbit/scripts/result",
	...
	s.DoJSON("POST", "/api/latest/mobius/scripts/run", mobius.HostScriptRequestPayload{HostID: host.ID, ScriptID: &savedScriptID}, http.StatusAccepted, &runResp)
	...
	s.DoJSON("GET", "/api/latest/mobius/scripts/results/"+scriptExecID, nil, http.StatusOK, &scriptRes)
	...
	s.DoJSON("GET", "/api/latest/mobius/scripts/results/"+savedScriptExecID, nil, http.StatusOK, &scriptRes)
	...
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobius/hosts/%d", host.ID), nil, http.StatusOK, &deleteResp)
	...
	s.Do("DELETE", fmt.Sprintf("/api/latest/mobius/scripts/%d", savedScriptID), nil, http.StatusNoContent)
...
func genDistributedReqWithEntraIDDetails(host *mobius.Host, deviceID, userPrincipalName string) submitDistributedQueryResultsRequestShim {
	results := make(map[string]json.RawMessage)
	results["mobius_detail_query_conditional_access_microsoft_device_id"] = json.RawMessage(fmt.Sprintf(`[{"device_id": "%s", "user_principal_name": "%s"}]`, deviceID, userPrincipalName))
	return submitDistributedQueryResultsRequestShim{
		NodeKey:  *host.NodeKey,
		Results:  results,
		Statuses: make(map[string]interface{}),
...
func triggerAndWait(ctx context.Context, t *testing.T, ds mobius.Datastore, s *schedule.Schedule, timeout time.Duration) {
	...
	for {
		stats, err := ds.GetLatestCronStats(ctx, s.Name())
		require.NoError(t, err)
		if len(stats) > 0 && stats[0].ID > previousRunID && stats[0].Status == mobius.CronStatsStatusCompleted {
...
func (s *integrationEnterpriseTestSuite) cleanupQuery(queryID uint) {
	...
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobius/queries/id/%d", queryID), nil, http.StatusOK, &delResp)
}
...
func (s *integrationEnterpriseTestSuite) TestAutofillPoliciesAuthTeamUser() {
	...
	tests := []struct {
		...
		{role: mobius.RoleAdmin, pass: true},
		{role: mobius.RoleMaintainer, pass: true},
		{role: mobius.RoleGitOps, pass: true},
		{role: mobius.RoleObserver, pass: false},
		{role: mobius.RoleObserverPlus, pass: false},
	}
		...
		t.Run(
			tt.role, func(t *testing.T) {
				...
				if tt.pass {
					...
					s.DoJSON("POST", "/api/latest/mobius/autofill/policy", req, http.StatusOK, &res)
					assert.Equal(t, "description", res.Description)
					assert.Equal(t, "resolution", res.Resolution)
				} else {
					_ = s.Do("POST", "/api/latest/mobius/autofill/policy", req, http.StatusForbidden)
				}
			},
		)
	}
...
func (s *integrationEnterpriseTestSuite) TestPKGNewSoftwareTitleFlow() {
	...
	team, err := s.ds.NewTeam(ctx, &mobius.Team{Name: t.Name() + "team1"})
	require.NoError(t, err)
...
func (s *integrationEnterpriseTestSuite) TestEXEPackageUploads() {
	...
	s.uploadSoftwareInstaller(t, payload, http.StatusBadRequest, "Couldn't add. Mobius can't create a policy to detect existing installations for .exe packages. Please add the software, add a custom policy, and enable the install software policy automation.")
...
func (s *integrationEnterpriseTestSuite) TestCalendarCallback() {
	...
	genDistributedReqWithPolicyResults := func(host *mobius.Host, policyResults map[uint]*bool) submitDistributedQueryResultsRequestShim {
		var (
			results  = make(map[string]json.RawMessage)
			statuses = make(map[string]interface{})
			messages = make(map[string]string)
	...
	appCfg.Integrations.GoogleCalendar = []*mobius.GoogleCalendarIntegration{
		{
			Domain: "example.com",
			ApiKey: map[string]string{
				mobius.GoogleCalendarEmail: calendar.MockEmail,
			},
		},
	}
	err = s.ds.SaveAppConfig(ctx, appCfg)
	...
	// Send a sync command
	_ = s.DoRawWithHeaders("POST", "/api/v1/mobius/calendar/webhook/"+event.UUID, []byte(""), http.StatusOK, map[string]string{
		"X-Goog-Channel-Id":     details.ChannelID,
		"X-Goog-Resource-State": "sync",
	})

	// Send a regular callback with bad channel ID
	_ = s.DoRawWithHeaders("POST", "/api/v1/mobius/calendar/webhook/"+event.UUID, []byte(""), http.StatusForbidden, map[string]string{
	...
	_ = s.DoRawWithHeaders("POST", "/api/v1/mobius/calendar/webhook/"+eventRecreated.UUID, []byte(""), http.StatusOK,
		map[string]string{
			"X-Goog-Channel-Id":     details.ChannelID,
			"X-Goog-Resource-State": "exists",
		})
...
func (s *integrationEnterpriseTestSuite) TestCalendarEventBodyUpdate() {
	...
	calEvents = getEvents()
	require.Len(t, calEvents, 1)
	assert.Contains(t, calEvents[0].Description, mobius.CalendarDefaultDescription)
	assert.Contains(t, calEvents[0].Description, mobius.CalendarDefaultResolution)
...
func (s *integrationEnterpriseTestSuite) TestVPPAppsWithoutMDM() {
	...
	s.DoJSON("POST", "/api/latest/mobius/teams", &createTeamRequest{TeamPayload: mobius.TeamPayload{Name: ptr.String("Team 1")}}, http.StatusOK, &newTeamResp)
	team := newTeamResp.Team

	s.Do("POST", "/api/latest/mobius/hosts/transfer", &addHostsToTeamRequest{HostIDs: []uint{orbitHost.ID}, TeamID: &team.ID}, http.StatusOK)

	// Add an app so that we don't get a not found error
	app, err := s.ds.InsertVPPAppWithTeam(ctx, &mobius.VPPApp{
		Name:             "App " + t.Name(),
		BundleIdentifier: "bid_" + t.Name(),
		VPPAppTeam: mobius.VPPAppTeam{
			VPPAppID: mobius.VPPAppID{
				AdamID:   "adam_" + t.Name(),
				Platform: mobius.MacOSPlatform,
			},
		},
	}, &team.ID)
	require.NoError(t, err)

	pkgPayload := &mobius.UploadSoftwareInstallerPayload{
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/software", orbitHost.ID), getHostSoftwareRequest{}, http.StatusOK, &resp)
	assert.Len(t, resp.Software, 1)
	assert.NotNil(t, resp.Software[0].SoftwarePackage)
	assert.Nil(t, resp.Software[0].AppStoreApp)

	r := s.Do("POST", fmt.Sprintf("/api/latest/mobius/hosts/%d/software/%d/install", orbitHost.ID, app.TitleID), &installSoftwareRequest{},
		http.StatusUnprocessableEntity)
...
func (s *integrationEnterpriseTestSuite) TestPolicyAutomationsSoftwareInstallers() {
	...
	newHost := func(name string, teamID *uint, platform string) *mobius.Host {
		h, err := s.ds.NewHost(ctx, &mobius.Host{
			DetailUpdatedAt: time.Now(),
			LabelUpdatedAt:  time.Now(),
			PolicyUpdatedAt: time.Now(),
	...
	newMobiusdHost := func(name string, teamID *uint, platform string) *mobius.Host {
	...
	host0NoTeam := newMobiusdHost("host1NoTeam", nil, "darwin")
	host1Team1 := newMobiusdHost("host1Team1", &team1.ID, "darwin")
	host2Team1 := newMobiusdHost("host2Team1", &team1.ID, "ubuntu")
	host3Team2 := newMobiusdHost("host3Team2", &team2.ID, "windows")
	...
	require.NoError(t, err)
	rubyPayload := &mobius.UploadSoftwareInstallerPayload{
	...
	// Upload mobius-osquery.msi to team2.
	mobiusOsqueryPayload := &mobius.UploadSoftwareInstallerPayload{
		InstallScript: "some msi install script",
		Filename:      "mobius-osquery.msi",
		TeamID:        &team2.ID,
	...
	s.uploadSoftwareInstaller(t, mobiusOsqueryPayload, http.StatusOK, "")
	// Get software title ID of the uploaded installer.
	resp = listSoftwareTitlesResponse{}
	s.DoJSON(
		"GET", "/api/latest/mobius/software/titles",
		listSoftwareTitlesRequest{},
		http.StatusOK, &resp,
		"query", "Mobius osquery",
		"team_id", fmt.Sprintf("%d", team2.ID),
	)
	require.Len(t, resp.SoftwareTitles, 1)
	require.NotNil(t, resp.SoftwareTitles[0].SoftwarePackage)
	mobiusOsqueryMSITitleID := resp.SoftwareTitles[0].ID
	var mobiusOsqueryMSIInstallerID uint
	mysql.ExecAdhocSQL(t, s.ds, func(q sqlx.ExtContext) error {
		return sqlx.GetContext(ctx, q,
			&mobiusOsqueryMSIInstallerID,
			`SELECT id FROM software_installers WHERE global_or_team_id = ? AND filename = ?`,
			team2.ID, "mobius-osquery.msi",
		)
	})
	require.NotZero(t, mobiusOsqueryMSIInstallerID)

	// Create a VPP app to test that policies *can* be assigned to them (VPP automation is tested in MDM integration test)
	_, err = s.ds.InsertVPPAppWithTeam(ctx, &mobius.VPPApp{
		Name:             "App123 " + t.Name(),
		BundleIdentifier: "bid_" + t.Name(),
		VPPAppTeam: mobius.VPPAppTeam{
			VPPAppID: mobius.VPPAppID{
	...
	policy0AllTeams, err := s.ds.NewGlobalPolicy(ctx, nil, mobius.PolicyPayload{
	...
	policy1Team1, err := s.ds.NewTeamPolicy(ctx, team1.ID, nil, mobius.PolicyPayload{
		Name:     "policy1Team1",
		Query:    "SELECT 1;",
		Platform: "darwin",
	})
	...
	policy2Team1, err := s.ds.NewTeamPolicy(ctx, team1.ID, nil, mobius.PolicyPayload{
		Name:     "policy2Team1",
		Query:    "SELECT 2;",
	...
	policy3Team1, err := s.ds.NewTeamPolicy(ctx, team1.ID, nil, mobius.PolicyPayload{
		Name:  "policy3Team1",
		Query: "SELECT 3;",
	})
	require.NoError(t, err)
	// policy4Team2 runs on Windows devices.
	policy4Team2, err := s.ds.NewTeamPolicy(ctx, team2.ID, nil, mobius.PolicyPayload{
		Name:     "policy4Team2",
		Query:    "SELECT 4;",
		Platform: "windows",
	})
	...
	mtplr = modifyTeamPolicyResponse{}
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/teams/%d/policies/%d", team1.ID, policy1Team1.ID),
	...
	mtplr = modifyTeamPolicyResponse{}
	s.DoRaw("PATCH", fmt.Sprintf("/api/latest/mobius/teams/%d/policies/%d", team1.ID, policy1Team1.ID), []byte(`{
	...
	mtplr = modifyTeamPolicyResponse{}
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/teams/%d/policies/%d", team1.ID, policy2Team1.ID), modifyTeamPolicyRequest{
		ModifyPolicyPayload: mobius.ModifyPolicyPayload{
			SoftwareTitleID: optjson.Any[uint]{Set: true, Valid: true, Value: rubyDebTitleID},
		},
	}, http.StatusOK, &mtplr)
	...
	require.Equal(t, mobius.SoftwareInstallPending, *host1LastInstall.Status)
	...
	require.Equal(t, mobius.SoftwareInstallPending, *host2LastInstall.Status)

	// Associate mobius-osquery.msi to policy4Team2.
	mtplr = modifyTeamPolicyResponse{}
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/teams/%d/policies/%d", team2.ID, policy4Team2.ID), modifyTeamPolicyRequest{
		ModifyPolicyPayload: mobius.ModifyPolicyPayload{
			SoftwareTitleID: optjson.Any[uint]{Set: true, Valid: true, Value: mobiusOsqueryMSITitleID},
		},
	}, http.StatusOK, &mtplr)
	...
	host3LastInstall, err := s.ds.GetHostLastInstallData(ctx, host3Team2.ID, mobiusOsqueryMSIInstallerID)
	require.NoError(t, err)
	require.NotNil(t, host3LastInstall)
	require.NotEmpty(t, host3LastInstall.ExecutionID)
	require.NotNil(t, host3LastInstall.Status)
	require.Equal(t, mobius.SoftwareInstallPending, *host3LastInstall.Status)
	host3LastInstallDetails, err := s.ds.GetSoftwareInstallDetails(ctx, host3LastInstall.ExecutionID)
	require.NoError(t, err)
	// Even if mobius-osquery.msi was uploaded as Self-service, it was installed by Mobius, so
	...
	var listUpcomingAct listHostUpcomingActivitiesResponse
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/activities/upcoming", host1Team1.ID), nil, http.StatusOK, &listUpcomingAct)
	require.Len(t, listUpcomingAct.Activities, 1)
	require.Nil(t, listUpcomingAct.Activities[0].ActorID)
	require.Equal(t, "Mobius", *listUpcomingAct.Activities[0].ActorFullName)
	...
	s.lastActivityMatches(mobius.ActivityTypeInstalledSoftware{}.ActivityName(), fmt.Sprintf(`{
		"host_id": %d,
		"host_display_name": "%s",
		"software_title": "%s",
		"software_package": "%s",
	...
	activityID := s.lastActivityMatches(mobius.ActivityTypeInstalledSoftware{}.ActivityName(), fmt.Sprintf(`{
		"host_id": %d,
		"host_display_name": "%s",
		"software_title": "%s",
		"software_package": "%s",
		...
		"policy_id": %d,
		"policy_name": "%s"
	}`, host2Team1.ID, host2Team1.DisplayName(), "ruby", "ruby.deb", host2LastInstall.ExecutionID, mobius.SoftwareInstallFailed, policy2Team1.ID, policy2Team1.Name), 0)

	// Check that the activity item generated for ruby.deb installation is shown as coming from Mobius
	var actor struct {
		UserID     *uint   `db:"user_id"`
		UserName   *string `db:"user_name"`
		UserEmail  string  `db:"user_email"`
	...
	require.Equal(t, "Mobius", *actor.UserName)
	...
	// host3Team2 posts the installation result for mobius-osquery.msi.
	s.Do("POST", "/api/mobius/orbit/software_install/result", json.RawMessage(fmt.Sprintf(`{
			"orbit_node_key": %q,
			"install_uuid": %q,
			"pre_install_condition_output": "ok",
	...
	activityID = s.lastActivityMatches(mobius.ActivityTypeInstalledSoftware{}.ActivityName(), fmt.Sprintf(`{
		...
		"policy_id": %f,
		"policy_name": "%s"
	}`, host3Team2.ID, host3Team2.DisplayName(), "Mobius osquery", "mobius-osquery.msi", host3LastInstall.ExecutionID,
		mobius.SoftwareInstallFailed, float64(policy4Team2.ID), policy4Team2.Name), 0)

	// Check that the activity item generated for mobius-osquery.msi installation has Mobius set as author.
	mysql.ExecAdhocSQL(t, s.ds, func(q sqlx.ExtContext) error {
		return sqlx.GetContext(ctx, q,
			&actor,
			`SELECT user_id, user_name, user_email, details->>'$.policy_id' policy_id, details->>'$.policy_name' policy_name FROM activities WHERE id = ?`,
	...
	// Mobius should not queue an install for vanilla osquery hosts.
	distributedResp = submitDistributedQueryResultsResponse{}
	s.DoJSONWithoutAuth("POST", "/api/osquery/distributed/write", genDistributedReqWithPolicyResults(
...
func (s *integrationEnterpriseTestSuite) TestPolicyAutomationsSoftwareInstallersLabelScoping() {
	...
	s.DoJSON("POST", "/api/v1/mobius/labels", mobius.LabelPayload{
		Name:  uuid.NewString(),
		Query: "SELECT 1",
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/software/titles/%d", rubyDebTitleID), nil, http.StatusOK, &rubyDetail)
	...
	policy1, err := s.ds.NewTeamPolicy(ctx, 0, nil, mobius.PolicyPayload{
		Name:     "policy1",
		Query:    "SELECT 1;",
		Platform: "linux",
	})
	...
	mtplr := modifyTeamPolicyResponse{}
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/teams/0/policies/%d", policy1.ID), modifyTeamPolicyRequest{
		ModifyPolicyPayload: mobius.ModifyPolicyPayload{
			SoftwareTitleID: optjson.Any[uint]{Set: true, Valid: true, Value: rubyDebTitleID},
		},
	}, http.StatusOK, &mtplr)
	...
	vimPayload := &mobius.UploadSoftwareInstallerPayload{
		InstallScript:    "some deb install script",
		Filename:         "vim.deb",
		TeamID:           nil,
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/software/titles/%d", vimTitleID), nil, http.StatusOK, &vimDetail)
	...
	policy2, err := s.ds.NewTeamPolicy(ctx, 0, nil, mobius.PolicyPayload{
		Name:     "policy2",
		Query:    "SELECT 2;",
		Platform: "linux",
	})
	...
	mtplr = modifyTeamPolicyResponse{}
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/teams/0/policies/%d", policy2.ID), modifyTeamPolicyRequest{
		ModifyPolicyPayload: mobius.ModifyPolicyPayload{
...
func (s *integrationEnterpriseTestSuite) TestPolicyAutomationsScripts() {
	...
	winScript, err := s.ds.NewScript(ctx, &mobius.Script{
	...
	psScript, err := s.ds.NewScript(ctx, &mobius.Script{
		Name:           "windows-script.ps1",
	...
	s.DoJSON("POST", "/api/latest/mobius/scripts/run", mobius.HostScriptRequestPayload{HostID: host1Team1.ID, ScriptID: &script.ID}, http.StatusConflict, &scriptRunResp)
	...
	// Mobius should not queue scripts for vanilla osquery hosts.
	distributedResp = submitDistributedQueryResultsResponse{}
	s.DoJSONWithoutAuth("POST", "/api/osquery/distributed/write", genDistributedReqWithPolicyResults(
		hostVanillaOsquery5Team1,
		map[uint]*bool{
	...
	// activity feed should show script run as pending, with "Mobius" as author, policy ID and name set in body
	var listResp listActivitiesResponse
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/activities/upcoming", host3Team2.ID), nil, http.StatusOK, &listResp)
	require.Len(t, listResp.Activities, 1)
	require.Nil(t, listResp.Activities[0].ActorEmail)
	require.Equal(t, "Mobius", *listResp.Activities[0].ActorFullName)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/activities", host3Team2.ID), nil, http.StatusOK, &listResp)
	require.Len(t, listResp.Activities, 1)
...
func (s *integrationEnterpriseTestSuite) TestSoftwareUploadRPM() {
	...
	s.DoJSON("POST", fmt.Sprintf("/api/v1/mobius/hosts/%d/software/%d/install", host.ID, titleID), nil, http.StatusAccepted, &installSoftwareResp)
	...
	var resp getSoftwareInstallResultsResponse
	s.DoJSON("GET", fmt.Sprintf("/api/v1/mobius/software/install/%s/results", installUUID), nil, http.StatusOK, &resp)
	assert.Equal(t, host.ID, resp.Results.HostID)
	assert.Equal(t, installUUID, resp.Results.InstallUUID)
	assert.Equal(t, mobius.SoftwareInstallFailed, resp.Results.Status)
	assert.NotNil(t, resp.Results.PreInstallQueryOutput)
	assert.Equal(t, mobius.SoftwareInstallerQuerySuccessCopy, *resp.Results.PreInstallQueryOutput)
	assert.NotNil(t, resp.Results.Output)
	assert.Equal(t, fmt.Sprintf(mobius.SoftwareInstallerInstallFailCopy, "failed"), *resp.Results.Output)
...
func (s *integrationEnterpriseTestSuite) TestMaintainedApps() {
	...
	getSoftwareInstallerIDByMAppID := func(mappID uint) uint {
		var id uint
		mysql.ExecAdhocSQL(t, s.ds, func(q sqlx.ExtContext) error {
			return sqlx.GetContext(ctx, q, &id, "SELECT id FROM software_installers WHERE mobius_maintained_app_id = ?", mappID)
		})
	...
	s.Do("POST", "/api/latest/mobius/software/mobius_maintained_apps", &addMobiusMaintainedAppRequest{AppID: 1}, http.StatusNotFound)
	...
	os.Setenv("MOBIUS_DEV_MAINTAINED_APPS_BASE_URL", manifestServer.URL)
	defer os.Unsetenv("MOBIUS_DEV_MAINTAINED_APPS_BASE_URL")
	...
	// Check apps returned
	var listMAResp listMobiusMaintainedAppsResponse
	s.DoJSON(http.MethodGet, "/api/latest/mobius/software/mobius_maintained_apps", listMobiusMaintainedAppsRequest{}, http.StatusOK,
	...
	require.Len(t, listMAResp.MobiusMaintainedApps, len(expectedApps))

	slices.SortFunc(listMAResp.MobiusMaintainedApps, func(a, b mobius.MaintainedApp) int {
		return cmp.Compare(a.Name, b.Name)
	})
	slices.SortFunc(expectedApps, func(a, b mobius.MaintainedApp) int {
		return cmp.Compare(a.Name, b.Name)
	})
	require.Equal(t, expectedApps, listMAResp.MobiusMaintainedApps)

	var listMAResp2 listMobiusMaintainedAppsResponse
	s.DoJSON(
		http.MethodGet,
		"/api/latest/mobius/software/mobius_maintained_apps",
		listMobiusMaintainedAppsRequest{},
		http.StatusOK,
		&listMAResp2,
	...
	require.Len(t, listMAResp2.MobiusMaintainedApps, 2)
	require.Contains(t, listMAResp.MobiusMaintainedApps, listMAResp2.MobiusMaintainedApps[0])

	// Check individual app fetch
	var getMAResp getMobiusMaintainedAppResponse
	s.DoJSON(http.MethodGet, fmt.Sprintf("/api/latest/mobius/software/mobius_maintained_apps/%d", listMAResp.MobiusMaintainedApps[0].ID), getMobiusMaintainedAppRequest{}, http.StatusOK, &getMAResp)
	// TODO this will change when actual install scripts are created.
	dbAppRecord, err := s.ds.GetMaintainedAppByID(ctx, listMAResp.MobiusMaintainedApps[0].ID, nil)
	require.NoError(t, err)
	_, err = maintained_apps.Hydrate(ctx, dbAppRecord)
	require.NoError(t, err)
	dbAppResponse := mobius.MaintainedApp{
		ID:              dbAppRecord.ID,
	...
	require.NotEmpty(t, getMAResp.MobiusMaintainedApp.InstallerURL)
	require.NotEmpty(t, getMAResp.MobiusMaintainedApp.InstallScript)
	require.NotEmpty(t, getMAResp.MobiusMaintainedApp.UninstallScript)
	require.Equal(t, dbAppResponse, *getMAResp.MobiusMaintainedApp)

	// Try adding ingested app with invalid secret
	reqInvalidSecret := &addMobiusMaintainedAppRequest{
		AppID:             1,
		TeamID:            &team.ID,
		SelfService:       true,
		PreInstallQuery:   "SELECT 1",
		InstallScript:     "echo foo $MOBIUS_SECRET_INVALID1",
		PostInstallScript: "echo done $MOBIUS_SECRET_INVALID2",
		UninstallScript:   "echo $MOBIUS_SECRET_INVALID3",
	}
	respBadSecret := s.Do("POST", "/api/latest/mobius/software/mobius_maintained_apps", reqInvalidSecret, http.StatusUnprocessableEntity)
	...
	for _, reason := range errReasons {
		assert.Contains(t, reason, "$MOBIUS_SECRET_INVALID")
	}
	...
	assert.Contains(t, errReasons[0], "$MOBIUS_SECRET_INVALID2")
	...
	require.Len(t, errReasons, 1)
	assert.Contains(t, errReasons[0], "$MOBIUS_SECRET_INVALID3")

	// Add an ingested app to the team
	var addMAResp addMobiusMaintainedAppResponse
	req := &addMobiusMaintainedAppRequest{
		AppID:             1,
		TeamID:            &team.ID,
		SelfService:       true,
		PreInstallQuery:   "SELECT 1",
	...
	s.DoJSON("POST", "/api/latest/mobius/software/mobius_maintained_apps", req, http.StatusOK, &addMAResp)
	...
	require.Equal(t, ptr.Uint(1), i.MobiusMaintainedAppID)
	...
	// Check activity
	s.lastActivityOfTypeMatches(
		mobius.ActivityTypeAddedSoftware{}.ActivityName(),
	...
	mysql.ExecAdhocSQL(t, s.ds, func(q sqlx.ExtContext) error {
		_, err = q.ExecContext(ctx, "UPDATE mobius_maintained_apps SET slug = ? WHERE id = 2", "badinstaller")
		require.NoError(t, err)
		_, err = q.ExecContext(ctx, "UPDATE mobius_maintained_apps SET slug = ? WHERE id = 3", "timeout")
		return err
	})

	// Should return an error; SHAs don't match up
	r := s.Do("POST", "/api/latest/mobius/software/mobius_maintained_apps", &addMobiusMaintainedAppRequest{AppID: 2}, http.StatusInternalServerError)
	require.Contains(t, extractServerErrorText(r.Body), "mismatch in maintained app SHA256 hash")

	// Should timeout
	os.Setenv("MOBIUS_DEV_MAINTAINED_APPS_INSTALLER_TIMEOUT", "1s")
	r = s.Do("POST", "/api/latest/mobius/software/mobius_maintained_apps", &addMobiusMaintainedAppRequest{AppID: 3}, http.StatusGatewayTimeout)
	os.Unsetenv("MOBIUS_DEV_MAINTAINED_APPS_INSTALLER_TIMEOUT")
	...
	addMAResp = addMobiusMaintainedAppResponse{}
	s.DoJSON("POST", "/api/latest/mobius/software/mobius_maintained_apps", req, http.StatusOK, &addMAResp)
	require.Nil(t, addMAResp.Err)
	...
	require.Equal(t, "installer.zip", title.SoftwarePackage.Name)
	titleResponse := getSoftwareTitleResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/v1/mobius/software/titles/%d", title.ID), nil, http.StatusOK, &titleResponse, "team_id", "0")
	...
	require.NoError(t, err)
	require.Equal(t, ptr.Uint(4), i.MobiusMaintainedAppID)
	require.Equal(t, mapp.SHA256, i.StorageID)
	require.NotEmpty(t, i.InstallScriptContentID)
	require.Equal(t, req.PreInstallQuery, i.PreInstallQuery)
	install, err = s.ds.GetAnyScriptContents(ctx, i.InstallScriptContentID)
	...
	updatePayload := &mobius.UpdateSoftwareInstallerPayload{
		TitleID:       title.ID,
	...
	tpResp := teamPolicyResponse{}
	s.DoJSON("POST", "/api/latest/mobius/teams/0/policies", tpParams, http.StatusOK, &tpResp)
	...
	wantAct := mobius.ActivityTypeCreatedPolicy{
		ID:   gotPolicy.ID,
		Name: gotPolicy.Name,
	}
	s.lastActivityMatches(wantAct.ActivityName(), string(jsonMustMarshal(t, wantAct)), 0)
	...
	s.DoJSON(
		"GET", fmt.Sprintf("/api/latest/mobius/software/titles/%d", addMAResp.SoftwareTitleID),
	...
	s.DoJSON(
		"GET", "/api/latest/mobius/teams/0/policies",
	...
	addMAResp = addMobiusMaintainedAppResponse{}
	r = s.Do("POST", "/api/latest/mobius/software/mobius_maintained_apps", req, http.StatusBadRequest)
...
func (s *integrationEnterpriseTestSuite) TestDeleteLabels() {
	...
	installer := &mobius.UploadSoftwareInstallerPayload{
		InstallScript:    "install",
		Filename:         "ruby.deb",
		SelfService:      false,
		TeamID:           nil,
	...
	res := s.Do("DELETE", "/api/v1/mobius/labels/id/"+fmt.Sprint(lbl1), nil, http.StatusUnprocessableEntity)
	errMsg := extractServerErrorText(res.Body)
	...
	s.DoJSON("DELETE", "/api/v1/mobius/labels/id/"+fmt.Sprint(lbl1+1000), nil, http.StatusNotFound, &delLabelResp)
	s.DoJSON("DELETE", "/api/v1/mobius/labels/no-such-label", nil, http.StatusNotFound, &delLabelResp)

	// delete the unused label2
	s.DoJSON("DELETE", "/api/v1/mobius/labels/id/"+fmt.Sprint(lbl2), nil, http.StatusOK, &delLabelResp)
}
...
func (s *integrationEnterpriseTestSuite) TestListHostSoftwareWithLabelScoping() {
	...
	resp := installSoftwareResponse{}
	s.DoJSON("POST", fmt.Sprintf("/api/v1/mobius/hosts/%d/software/%d/install", host.ID, titleID), nil, http.StatusAccepted, &resp)
	installUUID := getLatestSoftwareInstallExecID(t, s.ds, host.ID)
	...
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobius/hosts/%d/software/%d/uninstall", host.ID, titleID), nil, http.StatusAccepted, &resp)
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/software", host.ID), nil, http.StatusOK, &getHostSw)
	require.Len(t, getHostSw.Software, 1)
	...
	assert.Equal(t, mobius.SoftwareUninstallPending, *getHostSw.Software[0].Status)
	require.Nil(t, getHostSw.Software[0].SoftwarePackage)
	...
	getHostSw = getHostSoftwareResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/software?available_for_install=true", host.ID), nil, http.StatusOK, &getHostSw)
	require.Empty(t, getHostSw.Software)
}
...
func (s *integrationEnterpriseTestSuite) TestAutomaticPolicies() {
	...
	pkgTitleID = getSoftwareTitleID(t, s.ds, "DummyApp", "apps")
	respTitle := getSoftwareTitleResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/software/titles/%d?team_id=%d", pkgTitleID, team1.ID), listSoftwareTitlesRequest{}, http.StatusOK, &respTitle)
	require.NotNil(t, respTitle.SoftwareTitle)
	require.NotNil(t, respTitle.SoftwareTitle.SoftwarePackage)
	...
	ts = listTeamPoliciesResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/teams/%d/policies", team2.ID), nil, http.StatusOK, &ts)
	require.Len(t, ts.Policies, 1)
	require.Len(t, ts.InheritedPolicies, 0)
	require.Equal(t, "[Install software] DummyApp (pkg)", ts.Policies[0].Name)

	// Upload ruby.deb to team1 with automatic policy.
	payloadRubyDEB := &mobius.UploadSoftwareInstallerPayload{
		Filename:         "ruby.deb",
		TeamID:           &team1.ID,
		AutomaticInstall: true,
	}
	...
	payloadRubyRPM := &mobius.UploadSoftwareInstallerPayload{
		Filename:         "ruby.rpm",
		TeamID:           &team1.ID,
		AutomaticInstall: true,
	}
	...
	// Upload mobius-osquery.msi to team1 with automatic policy.
	mobiusOsqueryPayload := &mobius.UploadSoftwareInstallerPayload{
		Filename:         "mobius-osquery.msi",
		TeamID:           &team1.ID,
		AutomaticInstall: true,
	...
	require.Equal(t, "[Install software] Mobius osquery (msi)", ts.Policies[3].Name)
}
...
func (s *integrationEnterpriseTestSuite) TestSoftwareInstallerOrbitDownloadFailure() {
	...
	s.Do("POST", fmt.Sprintf("/api/latest/mobius/hosts/%d/software/%d/install", host.ID, titleID), installSoftwareRequest{}, http.StatusAccepted)

	// should be listed in upcoming activities
	var listUpcomingAct listHostUpcomingActivitiesResponse
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/activities/upcoming", host.ID), nil, http.StatusOK, &listUpcomingAct)
	require.Len(t, listUpcomingAct.Activities, 1)
	...
	var runResp runScriptResponse
	s.DoJSON("POST", "/api/latest/mobius/scripts/run",
		mobius.HostScriptRequestPayload{HostID: host.ID, ScriptContents: "echo 'hello'"}, http.StatusAccepted, &runResp)
	require.Equal(t, host.ID, runResp.HostID)
	require.NotEmpty(t, runResp.ExecutionID)

	// software install exec ID is returned in orbit notifications, but not script exec
	var orbitResp orbitGetConfigResponse
	s.DoJSON("POST", "/api/mobius/orbit/config", json.RawMessage(fmt.Sprintf(`{"orbit_node_key": %q}`, *host.OrbitNodeKey)), http.StatusOK, &orbitResp)
...
func (s *integrationEnterpriseTestSuite) TestBatchSoftwareUploadWithSHAs() {
	...
	resp := s.Do("POST", "/api/latest/mobius/software/batch", batchSetSoftwareInstallersRequest{Software: softwareToInstall}, http.StatusUnprocessableEntity, "team_name", team1.Name)
	...
	s.DoJSON("POST", "/api/latest/mobius/software/batch", batchSetSoftwareInstallersRequest{Software: softwareToInstall}, http.StatusAccepted, &batchResponse, "team_name", team1.Name, "dry_run", "true")
	packages = waitBatchSetSoftwareInstallersCompleted(t, s, team1.Name, batchResponse.RequestUUID)
	require.Len(t, packages, 1)
	require.NotNil(t, packages[0].TitleID)
	require.Equal(t, rubyURL, packages[0].URL)
	...
	stResp := getSoftwareTitleResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/software/titles/%d", *packages[0].TitleID), getSoftwareTitleRequest{}, http.StatusOK, &stResp, "team_id", fmt.Sprint(team1.ID))
	...
	softwareToInstall[0].URL = rubyURL
	s.DoJSON("POST", "/api/latest/mobius/software/batch", batchSetSoftwareInstallersRequest{Software: softwareToInstall}, http.StatusAccepted, &batchResponse, "team_name", team2.Name, "dry_run", "true")
	packages = waitBatchSetSoftwareInstallersCompleted(t, s, team2.Name, batchResponse.RequestUUID)
	require.Empty(t, packages)
	require.True(t, hitDebURL)
	hitDebURL = false
	...
	softwareToInstall = append(softwareToInstall, &mobius.SoftwareInstallerPayload{
		URL: exeURL,
	})
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/software/titles/%d", *pkgTitleID), getSoftwareTitleRequest{}, http.StatusOK, &stResp, "team_id", fmt.Sprint(team2.ID))
	require.NotNil(t, stResp.SoftwareTitle.SoftwarePackage)
	...
	softwareToInstall[1].URL = ""
	softwareToInstall[1].SHA256 = exeHash
	s.DoJSON("POST", "/api/latest/mobius/software/batch", batchSetSoftwareInstallersRequest{Software: softwareToInstall[:2]}, http.StatusAccepted, &batchResponse, "team_name", team1.Name)
	errMsg = waitBatchSetSoftwareInstallersFailed(t, s, team2.Name, batchResponse.RequestUUID)
	require.Contains(t, errMsg, "Couldn't edit. Install script is required for .exe packages.")
	require.False(t, hitExeURL)
...
# Mobius extracts and saves package IDs.
pkg_ids=(
  "com.example.dummy"
)
...
do
  ...
  if [[ ! -z "$volume" ]]; then
    ...
    # Only process directories that end with ".app" to prevent Mobius from removing top level directories.
...
func (s *integrationEnterpriseTestSuite) TestBatchSoftwareInstallerAndFMACategories() {
	...
	s.DoJSON("POST", "/api/latest/mobius/hosts/transfer", addHostsToTeamRequest{TeamID: &team1.ID, HostIDs: []uint{host.ID}}, http.StatusOK, &addHostsToTeamResponse{})
	...
	for _, tc := range testCases {
		t.Run(tc.desc, func(t *testing.T) {
			...
			for _, p := range packages {
				...
				stResp := getSoftwareTitleResponse{}
				s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/software/titles/%d", *p.TitleID), getSoftwareTitleRequest{}, http.StatusOK, &stResp, "team_id", fmt.Sprint(team1.ID))
				require.NotNil(t, stResp.SoftwareTitle.SoftwarePackage)
				if stResp.SoftwareTitle.SoftwarePackage.MobiusMaintainedAppID != nil && len(tc.categories) == 0 {
...
func (s *integrationEnterpriseTestSuite) TestConditionalAccessBasicSetup() {
	...
	s.DoJSON("POST", "/api/latest/mobius/conditional-access/microsoft", conditionalAccessMicrosoftCreateRequest{}, http.StatusForbidden, &r)
	var c conditionalAccessMicrosoftConfirmResponse
	s.DoJSON("POST", "/api/latest/mobius/conditional-access/microsoft/confirm", conditionalAccessMicrosoftConfirmRequest{}, http.StatusForbidden, &c)
	var d conditionalAccessMicrosoftDeleteResponse
	s.DoJSON("DELETE", "/api/latest/mobius/conditional-access/microsoft", conditionalAccessMicrosoftDeleteRequest{}, http.StatusForbidden, &d)
	...
	s.DoJSON("POST", "/api/latest/mobius/conditional-access/microsoft", conditionalAccessMicrosoftCreateRequest{
	...
	s.DoJSON("POST", "/api/latest/mobius/conditional-access/microsoft/confirm", conditionalAccessMicrosoftConfirmRequest{}, http.StatusOK, &c)
	...
	s.DoJSON("DELETE", "/api/latest/mobius/conditional-access/microsoft", conditionalAccessMicrosoftDeleteRequest{}, http.StatusOK, &d)
	// Deleting again should fail with bad request.
	d = conditionalAccessMicrosoftDeleteResponse{}
	s.DoJSON("DELETE", "/api/latest/mobius/conditional-access/microsoft", conditionalAccessMicrosoftDeleteRequest{}, http.StatusBadRequest, &d)
...
func (s *integrationEnterpriseTestSuite) TestConditionalAccessPolicies() {
	...
	t1, err := s.ds.NewTeam(context.Background(), &mobius.Team{
		Name:        "team1",
		Description: "desc team1",
	})
	require.NoError(t, err)
	...
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobius/teams/%d/policies", t1.ID), teamPolicyRequest{
		Query:                    "SELECT 1;",
	...
	var tmResp teamResponse
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/teams/%d", t1.ID), map[string]any{
		"integrations": map[string]any{
	...
	err = s.ds.SetOrUpdateMDMData(ctx,
		...
		"Mobius" /* MDM name */, "", /* mobiusEnrollmentRef */
	)
	require.NoError(t, err)
	...
	s.DoRaw("PATCH", "/api/v1/mobius/config", []byte(`{
		"integrations": {
			"conditional_access_enabled": true
		}
	}`), http.StatusOK)
	...
	pr = teamPolicyResponse{}
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobius/teams/%d/policies", mobius.PolicyNoTeamID), teamPolicyRequest{
		Query:                    "SELECT 1;",
		Name:                     "Compliance check 1",
		ConditionalAccessEnabled: true,
	}, http.StatusOK, &pr)
	...
	_, err = s.ds.DeleteTeamPolicies(ctx, mobius.PolicyNoTeamID, []uint{cp1.ID})
	require.NoError(t, err)
