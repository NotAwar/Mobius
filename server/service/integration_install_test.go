...
import (
	...
	software_mock "github.com/notawar/mobius/v4/server/mock/software"
...
func (s *integrationInstallTestSuite) SetupSuite() {
	...
	mobiusConfig.S3.SoftwareInstallersCloudFrontSigner = signer
...
// We test using both mock and real mobius.SoftwareInstallerStore.Sign functions.
func (s *integrationInstallTestSuite) TestSoftwareInstallerSignedURL() {
	...
	s.Do("POST", fmt.Sprintf("/api/latest/mobius/hosts/%d/software/%d/install", hostInTeam.ID, titleID), installSoftwareRequest{},
		http.StatusAccepted)
	...
	s.DoJSON("POST", "/api/mobius/orbit/software_install/details", orbitGetSoftwareInstallRequest{
