...
func (s *liveQueriesTestSuite) SetupSuite() {
	s.T().Setenv("MOBIUS_LIVE_QUERY_REST_PERIOD", "5s")
...
func (s *liveQueriesTestSuite) TestLiveQueriesRestOneHostOneQuery() {
	test := func(endpoint liveQueryEndpoint, savedQuery bool, hasStats bool) {
		...
		if endpoint == oneQueryEndpoint { //nolint:gocritic // ignore ifElseChain
			...
			go func() {
				defer wg.Done()
				s.DoJSON("POST", fmt.Sprintf("/api/latest/mobius/queries/%d/run", q1.ID), liveQueryRequest, http.StatusOK, &oneLiveQueryResp)
			}()
			...
			go func() {
				defer wg.Done()
				s.DoJSON("GET", "/api/latest/mobius/queries/run", liveQueryRequest, http.StatusOK, &liveQueryResp)
			}()
		} else { // customQueryOneHostId(.*)Endpoint
			liveQueryRequest := runLiveQueryOnHostRequest{
				Query: query,
			}
			url := fmt.Sprintf("/api/latest/mobius/hosts/%d/query", host.ID)
			if endpoint == customQueryOneHostIdentifierEndpoint {
				url = fmt.Sprintf("/api/latest/mobius/hosts/identifier/%s/query", host.UUID)
			}
			go func() {
				defer wg.Done()
				s.DoJSON(
		...
		go func() {
			...
			for range ticker.C {
				...
				if len(campaigns) == 1 && campaigns[0].Status == mobius.QueryRunning {
		...
		if endpoint == oneQueryEndpoint { //nolint:gocritic // ignore ifElseChain
			...
			assert.Equal(t, mobius.StatusOnline, liveQueryOnHostResp.Status)
			assert.Equal(t, query, liveQueryOnHostResp.Query)
			result = mobius.QueryResult{
		...
		var activity *mobius.ActivityTypeLiveQuery
		activityUpdated := make(chan *mobius.ActivityTypeLiveQuery)
		go func() {
			ticker := time.NewTicker(100 * time.Millisecond)
			defer ticker.Stop()
			for range ticker.C {
				...
				if err == nil {
					act := mobius.ActivityTypeLiveQuery{}
		...
		aggStats, err := mysql.GetAggregatedStats(context.Background(), s.ds, mobius.AggregatedStatsTypeScheduledQuery, q1.ID)
...
func (s *liveQueriesTestSuite) TestLiveQueriesRestOneHostMultipleQuery() {
	...
	q1, err := s.ds.NewQuery(context.Background(), &mobius.Query{
	...
	q2, err := s.ds.NewQuery(context.Background(), &mobius.Query{
...
func getCIDForQ(s *liveQueriesTestSuite, q1 *mobius.Query) string {
	t := s.T()
	campaigns, err := s.ds.DistributedQueryCampaignsForQuery(context.Background(), q1.ID)
	require.NoError(t, err)
...
func (s *liveQueriesTestSuite) TestLiveQueriesRestMultipleHostMultipleQuery() {
	...
	cid1 := getCIDForQ(s, q1)
	cid2 := getCIDForQ(s, q2)
	for i, h := range []*mobius.Host{h1, h2} {
		distributedReq := SubmitDistributedQueryResultsRequest{
			NodeKey: *h.NodeKey,
			Results: map[string][]map[string]string{
...
func (s *liveQueriesTestSuite) TestLiveQueriesInvalidInputs() {
	...
	liveQueryResp := runLiveQueryResponse{}
	s.DoJSON("GET", "/api/latest/mobius/queries/run", liveQueryRequest, http.StatusBadRequest, &liveQueryResp)
	...
	oneLiveQueryResp := runOneLiveQueryResponse{}
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobius/queries/%d/run", q1.ID), oneLiveQueryRequest, http.StatusBadRequest, &oneLiveQueryResp)
	...
	liveQueryOnHostResp := runLiveQueryOnHostResponse{}
	s.DoJSON(
		"POST", fmt.Sprintf("/api/latest/mobius/hosts/%d/query", host.ID), liveQueryOnHostRequest, http.StatusBadRequest,
		&liveQueryOnHostResp,
	)
	s.DoJSON(
		"POST", fmt.Sprintf("/api/latest/mobius/hosts/identifier/%s/query", host.UUID), liveQueryOnHostRequest, http.StatusBadRequest,
		&liveQueryOnHostResp,
	)
}
...
func (s *liveQueriesTestSuite) TestLiveQueriesFailsToAuthorize() {
	...
	s.DoJSON("GET", "/api/latest/mobius/queries/run", liveQueryRequest, http.StatusForbidden, &liveQueryResp)
	oneLiveQueryRequest := runOneLiveQueryRequest{
		HostIDs: []uint{host.ID},
	}
	oneLiveQueryResp := runOneLiveQueryResponse{}
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobius/queries/%d/run", q1.ID), oneLiveQueryRequest, http.StatusForbidden, &oneLiveQueryResp)
}
...
func (s *liveQueriesTestSuite) TestLiveQueriesRestFailsToCreateCampaign() {
	...
	oneLiveQueryResp := runOneLiveQueryResponse{}
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobius/queries/%d/run", 999), oneLiveQueryRequest, http.StatusNotFound, &oneLiveQueryResp)
...
func (s *liveQueriesTestSuite) TestLiveQueriesRestFailsOnSomeHost() {
	test := func(newEndpoint bool) {
		...
		var qResults []mobius.QueryResult
		if newEndpoint {
			assert.Equal(t, q1.ID, oneLiveQueryResp.QueryID)
...
func (s *liveQueriesTestSuite) TestCreateDistributedQueryCampaign() {
	...
	s.DoJSON("POST", "/api/latest/mobius/queries/run", nil, http.StatusUnprocessableEntity, &createResp)
	...
	s.DoJSON("POST", "/api/latest/mobius/queries/run", req, http.StatusNotFound, &createResp)

	// create with no hosts
	req = createDistributedQueryCampaignRequest{
		QuerySQL: "SELECT 1",
	}
	s.DoJSON("POST", "/api/latest/mobius/queries/run", req, http.StatusBadRequest, &createResp)
	...
	s.DoJSON("POST", "/api/latest/mobius/queries/run", req, http.StatusOK, &createResp)
	...
	s.DoJSON("POST", "/api/latest/mobius/queries/run_by_identifiers", req2, http.StatusOK, &createResp)
	assert.NotEqual(t, camp1.ID, createResp.Campaign.ID)
	...
	s.DoJSON("POST", "/api/latest/mobius/queries/run_by_identifiers", req3, http.StatusBadRequest, &createResp)
}
...
func (s *liveQueriesTestSuite) TestCreateDistributedQueryCampaignBadRequest() {
	...
	req = createDistributedQueryCampaignRequest{
		Selected: mobius.HostTargets{HostIDs: []uint{host.ID}},
	}
	s.DoJSON("POST", "/api/latest/mobius/queries/run", req, http.StatusUnprocessableEntity, &createResp)
	...
	appCfg := mobius.AppConfig{ServerSettings: mobius.ServerSettings{LiveQueryDisabled: true, ServerURL: acResp.ServerSettings.ServerURL}, OrgInfo: mobius.OrgInfo{OrgName: acResp.OrgInfo.OrgName}}
	s.DoRaw("PATCH", "/api/latest/mobius/config", jsonMustMarshal(t, appCfg), http.StatusOK)

	req = createDistributedQueryCampaignRequest{
		QuerySQL: "select \"With automounting enabled anyone with physical access could attach a USB drive or disc and have its contents available in system even if they lacked permissions to mount it themselves.\" as Rationale;",
		Selected: mobius.HostTargets{HostIDs: []uint{host.ID}},
	}
	s.DoJSON("POST", "/api/latest/mobius/queries/run", req, http.StatusForbidden, &createResp)

	s.DoJSON("GET", "/api/latest/mobius/config", nil, http.StatusOK, &acResp)
	appCfg = mobius.AppConfig{ServerSettings: mobius.ServerSettings{LiveQueryDisabled: false, ServerURL: acResp.ServerSettings.ServerURL}, OrgInfo: mobius.OrgInfo{OrgName: acResp.OrgInfo.OrgName}}
	s.DoRaw("PATCH", "/api/latest/mobius/config", jsonMustMarshal(t, appCfg), http.StatusOK)
}
