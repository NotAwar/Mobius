...
func (s *integrationLoggerTestSuite) TestLogger() {
	...
	s.DoJSON("POST", "/api/latest/mobius/queries", params, http.StatusOK, &createResp)
	...
	for i, part := range parts {
		...
		case 0:
			...
			assert.Equal(t, "/api/latest/mobius/login", kv["uri"])
		case 1:
			assert.Equal(t, "debug", kv["level"])
			assert.Equal(t, "GET", kv["method"])
			assert.Equal(t, "/api/latest/mobius/config", kv["uri"])
			assert.Equal(t, "admin1@example.com", kv["user"])
		case 2:
			assert.Equal(t, "debug", kv["level"])
			assert.Equal(t, "POST", kv["method"])
			assert.Equal(t, "/api/latest/mobius/queries", kv["uri"])
			assert.Equal(t, "admin1@example.com", kv["user"])
...
func (s *integrationLoggerTestSuite) TestLoggerLogin() {
	...
	for _, tt := range testCases {
		s.DoJSON("POST", "/api/latest/mobius/login", tt.loginRequest, tt.expectedStatus, &resp)
