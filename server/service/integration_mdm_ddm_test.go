	...
	tmpl := `
{
	...
	"Payload": {
		...
		"DataAssetReference": "com.mobius.asset.bash" %s
	}
}`
	...
	res := s.Do("POST", "/api/latest/mobius/mdm/profiles/batch", batchSetMDMProfilesRequest{Profiles: []mobius.MDMProfileBatchPayload{
	...
	for ft := range mobius.ForbiddenDeclTypes {
		res = s.Do("POST", "/api/latest/mobius/mdm/profiles/batch", batchSetMDMProfilesRequest{Profiles: []mobius.MDMProfileBatchPayload{
	...
	s.DoJSON("GET", "/api/latest/mobius/mdm/profiles", &listMDMConfigProfilesRequest{}, http.StatusOK, &resp)
	...
	s.DoJSON("POST", "/api/latest/mobius/labels", &mobius.LabelPayload{Name: "label_1", Query: "select 1"}, http.StatusOK, &createResp)
	require.NotZero(t, createResp.Label.ID)
	require.Equal(t, "label_1", createResp.Label.Name)
	lbl1 := createResp.Label.Label

	s.DoJSON("POST", "/api/latest/mobius/labels", &mobius.LabelPayload{Name: "label_2", Query: "select 1"}, http.StatusOK, &createResp)
	require.NotZero(t, createResp.Label.ID)
...
func (s *integrationMDMTestSuite) TestMDMAppleDeviceManagementRequests() {
	...
	insertDeclaration := func(t *testing.T, decl mobius.MDMAppleDeclaration) {
		stmt := `
INSERT INTO mdm_apple_declarations (
	...
	insertHostDeclaration := func(t *testing.T, hostUUID string, decl mobius.MDMAppleDeclaration) {
		stmt := `
INSERT INTO host_mdm_apple_declarations (
	...
	noTeamDeclsByUUID := map[string]mobius.MDMAppleDeclaration{
		"123": {
			DeclarationUUID: "123",
			TeamID:          ptr.Uint(0),
	...
	mapDeclsByChecksum := func(byUUID map[string]mobius.MDMAppleDeclaration) map[string]mobius.MDMAppleDeclaration {
		byChecksum := make(map[string]mobius.MDMAppleDeclaration)
	...
	assertDeclarationResponse := func(r *http.Response, expected mobius.MDMAppleDeclaration) {
		require.NotNil(t, r)

		// unmarsal the response and assert it's valid
		var wantParsed mobius.MDMAppleDDMDeclarationResponse
		require.NoError(t, json.Unmarshal(expected.RawJSON, &wantParsed))
		var gotParsed mobius.MDMAppleDDMDeclarationResponse
	...
	checkTokensResp := func(t *testing.T, r mobius.MDMAppleDDMTokensResponse, expectedTimestamp time.Time, prevToken string) {
	...
	checkDeclarationItemsResp := func(t *testing.T, r mobius.MDMAppleDDMDeclarationItemsResponse, expectedDeclTok string, expectedDeclsByChecksum map[string]mobius.MDMAppleDeclaration) {
	...
	t.Run("Tokens", func(t *testing.T) {
		...
		noTeamDeclsByUUID["456"] = mobius.MDMAppleDeclaration{
			DeclarationUUID: "456",
			TeamID:          ptr.Uint(0),
	...
	t.Run("DeclarationItems", func(t *testing.T) {
		...
		// insert a new declaration
		noTeamDeclsByUUID["789"] = mobius.MDMAppleDeclaration{
			DeclarationUUID: "789",
			TeamID:          ptr.Uint(0),
	...
	t.Run("Status", func(t *testing.T) {
		_, err := mdmDevice.DeclarativeManagement("status", mobius.MDMAppleDDMStatusReport{})
		require.NoError(t, err)
	})
	...
	t.Run("Declaration", func(t *testing.T) {
		...
		// insert a new declaration
		noTeamDeclsByUUID["abc"] = mobius.MDMAppleDeclaration{
			DeclarationUUID: "abc",
			TeamID:          ptr.Uint(0),
...
func parseTokensResp(t *testing.T, r *http.Response) mobius.MDMAppleDDMTokensResponse {
	require.NotNil(t, r)
	...
	var tok mobius.MDMAppleDDMTokensResponse
...
func parseDeclarationItemsResp(t *testing.T, r *http.Response) mobius.MDMAppleDDMDeclarationItemsResponse {
	require.NotNil(t, r)
	b, err := io.ReadAll(r.Body)
	require.NoError(t, err)
	defer r.Body.Close()
	...
	// unmarshal the response to make sure it's valid
	var di mobius.MDMAppleDDMDeclarationItemsResponse
...
func (s *integrationMDMTestSuite) TestAppleDDMSecretVariables() {
	...
	checkDeclarationItemsResp := func(t *testing.T, r mobius.MDMAppleDDMDeclarationItemsResponse, expectedDeclTok string,
		expectedDeclsByToken map[string]mobius.MDMAppleDeclaration) {
		require.Equal(t, expectedDeclTok, r.DeclarationsToken)
		require.NotEmpty(t, r.Declarations.Activations)
		require.Empty(t, r.Declarations.Assets)
		require.Empty(t, r.Declarations.Management)
	...
	tmpl := `
{
	...
	decls[1] = []byte(strings.ReplaceAll(string(decls[1]), myBash, "$"+mobius.ServerSecretPrefix+"BASH"))
	secretProfile := decls[2]
	decls[2] = []byte("${" + mobius.ServerSecretPrefix + "PROFILE}")

	// Create declarations
	profilesReq := batchSetMDMProfilesRequest{Profiles: []mobius.MDMProfileBatchPayload{
		{Name: "N0", Contents: decls[0]},
		{Name: "N1", Contents: decls[1]},
		{Name: "N2", Contents: decls[2]},
	}}
	// First dry run
	s.Do("POST", "/api/latest/mobius/mdm/profiles/batch", profilesReq, http.StatusNoContent, "dry_run", "true")
	...
	// Now real run
	s.Do("POST", "/api/latest/mobius/mdm/profiles/batch", profilesReq, http.StatusNoContent)
	s.DoJSON("GET", "/api/latest/mobius/mdm/profiles", &listMDMConfigProfilesRequest{}, http.StatusOK, &resp)
	...
	getDeclaration := func(t *testing.T, name string) mobius.MDMAppleDeclaration {
		stmt := `
SELECT
	declaration_uuid,
	team_id,
...
WHERE name = ?`
	...
	var gotParsed mobius.MDMAppleDDMDeclarationResponse
	require.NoError(t, json.NewDecoder(r.Body).Decode(&gotParsed))
	assert.EqualValues(t, `{"DataAssetReference":"com.mobius.asset.bash","ServiceType":"com.apple.bash0"}`, gotParsed.Payload)

	declarationPath = fmt.Sprintf("declaration/configuration/%s", nameToIdentifier["N1"])
	r, err = mdmDevice.DeclarativeManagement(declarationPath)
	require.NoError(t, err)
	require.NoError(t, json.NewDecoder(r.Body).Decode(&gotParsed))
	assert.EqualValues(t, `{"DataAssetReference":"com.mobius.asset.bash","ServiceType":"com.apple.bash1"}`, gotParsed.Payload)

	declarationPath = fmt.Sprintf("declaration/configuration/%s", nameToIdentifier["N2"])
	r, err = mdmDevice.DeclarativeManagement(declarationPath)
	require.NoError(t, err)
	require.NoError(t, json.NewDecoder(r.Body).Decode(&gotParsed))
	assert.EqualValues(t, `{"DataAssetReference":"com.mobius.asset.bash","ServiceType":"com.apple.bash2"}`, gotParsed.Payload)
	...
	require.NoError(t, err)
	require.NoError(t, json.NewDecoder(r.Body).Decode(&gotParsed))
	assert.EqualValues(t, `{"DataAssetReference":"com.mobius.asset.bash","ServiceType":"my.new.bash"}`, gotParsed.Payload)
	...
	// Delete the profiles
	s.Do("DELETE", "/api/latest/mobius/configuration_profiles/"+nameToUUID["N0"], nil, http.StatusOK)
	s.Do("DELETE", "/api/latest/mobius/configuration_profiles/"+nameToUUID["N1"], nil, http.StatusOK)
	s.Do("DELETE", "/api/latest/mobius/configuration_profiles/"+nameToUUID["N2"], nil, http.StatusOK)
	s.DoJSON("GET", "/api/latest/mobius/mdm/profiles", &listMDMConfigProfilesRequest{}, http.StatusOK, &resp)
	require.Empty(t, resp.Profiles)
}
...
func (s *integrationMDMTestSuite) TestAppleDDMReconciliation() {
	...
	deleteDeclaration := func(declUUID string) {
		...
		s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobius/configuration_profiles/%s", declUUID), nil, http.StatusOK, &deleteResp)
	}
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d", mdmHost.ID), nil, http.StatusOK, &hostResp)
	...
	label, err := s.ds.NewLabel(ctx, &mobius.Label{Name: t.Name(), Query: "select 1;"})
	require.NoError(t, err)
...
func (s *integrationMDMTestSuite) TestAppleDDMStatusReport() {
	...
	assertHostDeclarations := func(hostUUID string, wantDecls []*mobius.MDMAppleHostDeclaration) {
		var gotDecls []*mobius.MDMAppleHostDeclaration
		mysql.ExecAdhocSQL(t, s.ds, func(q sqlx.ExtContext) error {
	...
	declarations := []mobius.MDMProfileBatchPayload{
		{Name: "N1.json", Contents: declarationForTest("I1")},
		{Name: "N2.json", Contents: declarationForTest("I2")},
	}
	// add global declarations
	s.Do("POST", "/api/v1/mobius/mdm/profiles/batch", batchSetMDMProfilesRequest{Profiles: declarations}, http.StatusNoContent)
	...
	assertHostDeclarations(mdmHost.UUID, []*mobius.MDMAppleHostDeclaration{
		{Identifier: "I1", Status: &mobius.MDMDeliveryPending, OperationType: mobius.MDMOperationTypeInstall},
		{Identifier: "I2", Status: &mobius.MDMDeliveryPending, OperationType: mobius.MDMOperationTypeInstall},
	})
	...
	assertHostDeclarations(mdmHost.UUID, []*mobius.MDMAppleHostDeclaration{
		{Identifier: "I1", Status: &mobius.MDMDeliveryVerifying, OperationType: mobius.MDMOperationTypeInstall},
		{Identifier: "I2", Status: &mobius.MDMDeliveryVerifying, OperationType: mobius.MDMOperationTypeInstall},
	})

	// host sends a partial DDM report
	report := mobius.MDMAppleDDMStatusReport{}
	report.StatusItems.Management.Declarations.Configurations = []mobius.MDMAppleDDMStatusDeclaration{
		{Active: true, Valid: mobius.MDMAppleDeclarationValid, Identifier: "I1", ServerToken: i1ServerToken},
	}
	_, err = device.DeclarativeManagement("status", report)
	require.NoError(t, err)
	assertHostDeclarations(mdmHost.UUID, []*mobius.MDMAppleHostDeclaration{
		{Identifier: "I1", Status: &mobius.MDMDeliveryVerified, OperationType: mobius.MDMOperationTypeInstall},
		{Identifier: "I2", Status: &mobius.MDMDeliveryVerifying, OperationType: mobius.MDMOperationTypeInstall},
	})
	...
	report = mobius.MDMAppleDDMStatusReport{}
	report.StatusItems.Management.Declarations.Configurations = []mobius.MDMAppleDDMStatusDeclaration{
		{Active: true, Valid: mobius.MDMAppleDeclarationValid, Identifier: "I2", ServerToken: "foo"},
	}
	_, err = device.DeclarativeManagement("status", report)
	require.NoError(t, err)
	assertHostDeclarations(mdmHost.UUID, []*mobius.MDMAppleHostDeclaration{
	...
	report.StatusItems.Management.Declarations.Configurations = []mobius.MDMAppleDDMStatusDeclaration{
		{Active: true, Valid: mobius.MDMAppleDeclarationValid, Identifier: "I1", ServerToken: i1ServerToken},
		{Active: false, Valid: mobius.MDMAppleDeclarationInvalid, Identifier: "I2", ServerToken: i2ServerToken},
	}
	_, err = device.DeclarativeManagement("status", report)
	require.NoError(t, err)
	assertHostDeclarations(mdmHost.UUID, []*mobius.MDMAppleHostDeclaration{
		{Identifier: "I1", Status: &mobius.MDMDeliveryVerified, OperationType: mobius.MDMOperationTypeInstall},
		{Identifier: "I2", Status: &mobius.MDMDeliveryFailed, OperationType: mobius.MDMOperationTypeInstall},
	})
	...
	assertHostDeclarations(mdmHost.UUID, []*mobius.MDMAppleHostDeclaration{
		{Identifier: "I1", Status: &mobius.MDMDeliveryVerified, OperationType: mobius.MDMOperationTypeInstall},
		{Identifier: "I2", Status: &mobius.MDMDeliveryPending, OperationType: mobius.MDMOperationTypeRemove},
	})
	...
	report = mobius.MDMAppleDDMStatusReport{}
	report.StatusItems.Management.Declarations.Configurations = []mobius.MDMAppleDDMStatusDeclaration{
		{Active: false, Valid: mobius.MDMAppleDeclarationInvalid, Identifier: "I1", ServerToken: i1ServerToken},
	}
	_, err = device.DeclarativeManagement("status", report)
	require.NoError(t, err)
	assertHostDeclarations(mdmHost.UUID, []*mobius.MDMAppleHostDeclaration{
		{Identifier: "I1", Status: &mobius.MDMDeliveryFailed, OperationType: mobius.MDMOperationTypeInstall},
	})
}
...
func (s *integrationMDMTestSuite) TestDDMUnsupportedDevice() {
	...
	ctx := context.Background()
	mobiusHost, mdmDevice := createHostThenEnrollMDM(s.ds, s.server.URL, t)

	getProfiles := func(h *mobius.Host) map[string]*mobius.HostMDMAppleProfile {
		profs, err := s.ds.GetHostMDMAppleProfiles(ctx, h.UUID)
		require.NoError(t, err)
		out := make(map[string]*mobius.HostMDMAppleProfile, len(profs))
		for _, p := range profs {
			p := p
			out[p.Identifier] = &p
		}
	...
	profs := getProfiles(mobiusHost)
	require.Equal(t, &mobius.MDMDeliveryPending, profs["I1"].Status)
	require.Equal(t, &mobius.MDMDeliveryPending, profs["I2"].Status)
	...
	// profiles are failed
	profs = getProfiles(mobiusHost)
	require.Equal(t, &mobius.MDMDeliveryFailed, profs["I1"].Status)
	require.Contains(t, profs["I1"].Detail, "Feature Disabled")
	require.Equal(t, &mobius.MDMDeliveryFailed, profs["I2"].Status)
