...
import (
	...
	"github.com/notawar/mobius/v4/pkg/mobiusdbase"
...
func (s *integrationMDMTestSuite) TestDEPEnrollReleaseDeviceGlobal() {
	...
	// set an enroll secret, the Mobiusd configuration profile will be installed
	// on the host
	...
	require.NoError(t, err)
	signedPkg := b
	s.uploadBootstrapPackage(&mobius.MDMAppleBootstrapPackage{Bytes: signedPkg, Name: "pkg.pkg", TeamID: 0}, http.StatusOK, "")

	// add a custom setup assistant and ensure enable_release_device_manually is
	// false (the default)
	noTeamProf := `{"x": 1}`
	s.Do("POST", "/api/latest/mobius/enrollment_profiles/automatic", createMDMAppleSetupAssistantRequest{
		TeamID:            nil,
		Name:              "no-team",
	...
	s.Do("PATCH", "/api/latest/mobius/setup_experience", json.RawMessage(jsonMustMarshal(t, payload)), http.StatusNoContent)
	...
	s.Do("POST", "/api/v1/mobius/mdm/apple/profiles/batch", batchSetMDMAppleProfilesRequest{Profiles: [][]byte{globalProfile}}, http.StatusNoContent)

	// enable FileVault
	s.Do("PATCH", "/api/latest/mobius/config", json.RawMessage([]byte(`{"mdm":{"macos_settings":{"enable_disk_encryption":true}}}`)), http.StatusOK)
	...
	s.DoRawWithHeaders("POST", "/api/latest/mobius/setup_experience/script", body.Bytes(), http.StatusOK, headers)
	...
	s.Do("DELETE", "/api/latest/mobius/setup_experience/script", nil, http.StatusOK)
	for _, enableReleaseManually := range []bool{false, true} {
	...
	t.Run("manual agent install (no mobiusdaemon)", func(t *testing.T) {
		s.Do("DELETE", "/api/latest/mobius/setup_experience/script", nil, http.StatusOK)
		s.runDEPEnrollReleaseDeviceTest(t, globalDevice, DEPEnrollTestOpts{
...
func (s *integrationMDMTestSuite) TestDEPEnrollReleaseDeviceTeam() {
	...
	tm, err := s.ds.NewTeam(ctx, &mobius.Team{Name: "test-team-device-release"})
	require.NoError(t, err)

	// set an enroll secret, the Mobiusd configuration profile will be installed
	// on the host
	enrollSecret := "test-release-dep-device-team"
	err = s.ds.ApplyEnrollSecrets(ctx, &tm.ID, []*mobius.EnrollSecret{{Secret: enrollSecret}})
	require.NoError(t, err)
	...
	require.NoError(t, err)
	signedPkg := b
	s.uploadBootstrapPackage(&mobius.MDMAppleBootstrapPackage{Bytes: signedPkg, Name: "pkg.pkg", TeamID: tm.ID}, http.StatusOK, "")
	...
	var acResp appConfigResponse
	s.enableABM("mobius_ade_test")
	s.DoJSON("PATCH", "/api/latest/mobius/config", json.RawMessage(fmt.Sprintf(`{
			"mdm": {
		...
		}`, "mobius_ade_test", tm.Name, tm.Name, tm.Name)), http.StatusOK, &acResp)
	require.NotEmpty(t, acResp.MDM.EndUserAuthentication)
	...
	s.Do("POST", "/api/v1/mobius/mdm/apple/profiles/batch", batchSetMDMAppleProfilesRequest{Profiles: [][]byte{teamProfile}}, http.StatusNoContent, "team_id", fmt.Sprint(tm.ID))

	// enable FileVault
	s.Do("PATCH", "/api/latest/mobius/mdm/apple/settings", json.RawMessage([]byte(fmt.Sprintf(`{"enable_disk_encryption":true,"team_id":%d}`, tm.ID))), http.StatusNoContent)
	...
	// remove the setup experience script, run the new setup experience flow when
	// there is no setup experience item to process (so it is bypassed)
	s.Do("DELETE", "/api/latest/mobius/setup_experience/script", nil, http.StatusOK, "team_id", fmt.Sprint(tm.ID))
	for _, enableReleaseManually := range []bool{false, true} {
		t.Run(fmt.Sprintf("enableReleaseManually=%t;bypass_flow", enableReleaseManually), func(t *testing.T) {
			s.runDEPEnrollReleaseDeviceTest(t, teamDevice, DEPEnrollTestOpts{
				EnableReleaseManually:             enableReleaseManually,
...
func (s *integrationMDMTestSuite) runDEPEnrollReleaseDeviceTest(t *testing.T, device godep.Device, opts DEPEnrollTestOpts) {
	...
	s.DoJSON("GET", "/api/latest/mobius/hosts", nil, http.StatusOK, &listHostsRes)
	...
	s.mockDEPResponse("mobius_ade_test", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/profile/devices":
			...
			for _, device := range prof.Devices {
				resp.Devices[device] = string(mobius.DEPAssignProfileResponseSuccess)
			}
	...
	if isIphone {
		...
		// expected commands: install mobiusdaemon, install bootstrap, install CA, install profiles
		// (custom one, mobiusdaemon configuration, FileVault) (not expected: account
	...
	var profileCustomSeen, profileMobiusdSeen, profileMobiusCASeen, profileFileVaultSeen bool
	...
	for _, cmd := range cmds {
		...
		case "InstallProfile":
			...
			} else if strings.Contains(string(cmd.Command.InstallProfile.Payload), fmt.Sprintf("<string>%s</string>", mobileconfig.MobiusdConfigPayloadIdentifier)) {
				profileMobiusdSeen = true
			} else if strings.Contains(string(cmd.Command.InstallProfile.Payload), fmt.Sprintf("<string>%s</string>", mobileconfig.MobiusCARootConfigPayloadIdentifier)) {
				profileMobiusCASeen = true
			} else if strings.Contains(string(cmd.Command.InstallProfile.Payload), fmt.Sprintf("<string>%s</string", mobileconfig.MobiusFileVaultPayloadIdentifier)) &&
	...
	if isIphone {
		...
		require.False(t, profileMobiusdSeen)
		require.True(t, profileMobiusCASeen)
		require.False(t, profileFileVaultSeen)

		// for iDevices, mobiusdaemon is not installed so the rest of this test does not apply.
	...
	if opts.ManualAgentInstall {
		...
		assert.Contains(t, lastInstallEnterpriseApplication.Manifest.ManifestItems[0].Assets[0].URL, "mobius/mdm/bootstrap")
	}
	require.Equal(t, expectedInstallEnterpriseCount, installEnterpriseCount)
	require.Equal(t, 0, otherCount)
	require.True(t, profileCustomSeen)
	require.True(t, profileMobiusdSeen)
	require.True(t, profileMobiusCASeen)
	require.True(t, profileFileVaultSeen)

	// simulate mobiusdaemon being installed and the host being orbit-enrolled now
	...
	// call the /config endpoint as mobiusdaemon would
	var orbitConfigResp orbitGetConfigResponse
	var caps mobius.CapabilityMap
	if opts.UseOldMobiusdFlow {
		// important thing is that it doesn't have the CapabilitySetupExperience
		caps.PopulateFromString(string(mobius.CapabilityEscrowBuddy))
	} else {
		caps = mobius.GetOrbitClientCapabilities()
	}

	res := s.DoRawWithHeaders("POST", "/api/mobius/orbit/config", json.RawMessage(fmt.Sprintf(`{"orbit_node_key": %q}`, *enrolledHost.OrbitNodeKey)),
		http.StatusOK, map[string]string{mobius.CapabilitiesHeader: caps.String()})
	...
	if opts.UseOldMobiusdFlow || !hasSetupExpItems {
		...
		s.DoJSON("POST", "/api/mobius/orbit/setup_experience/status", json.RawMessage(fmt.Sprintf(`{"orbit_node_key": %q}`, *enrolledHost.OrbitNodeKey)), http.StatusOK, &statusResp)
	}
...
func (s *integrationMDMTestSuite) TestDEPProfileAssignment() {
	...
	s.Do("PATCH", "/api/latest/mobius/setup_experience", json.RawMessage(jsonMustMarshal(t, map[string]any{
	...
	checkPostEnrollmentCommands := func(mdmDevice *mdmtest.TestAppleMDMClient, shouldReceive bool) {
		...
		var mobiusdCmd, installProfileCmd *micromdm.CommandPayload
		cmd, err := mdmDevice.Idle()
		require.NoError(t, err)
		for cmd != nil {
			var fullCmd micromdm.CommandPayload
			...
			if fullCmd.Command.RequestType == "InstallEnterpriseApplication" &&
				...
				strings.Contains(*fullCmd.Command.InstallEnterpriseApplication.ManifestURL, mobiusdbase.GetPKGManifestURL()) {
				mobiusdCmd = &fullCmd
		...
		if shouldReceive {
			// received request to install mobiusdaemon
			require.NotNil(t, mobiusdCmd, "host didn't get a command to install mobiusdaemon")
			require.NotNil(t, mobiusdCmd.Command, "host didn't get a command to install mobiusdaemon")
			...
			require.Nil(t, mobiusdCmd, "host got a command to install mobiusdaemon")
	...
	checkHostDEPAssignProfileResponses := func(deviceSerials []string, expectedProfileUUID string, expectedStatus mobius.DEPAssignProfileResponseStatus) map[string]hostDEPRow {
	...
	checkHostCooldown := func(serial, profUUID string, status mobius.DEPAssignProfileResponseStatus, expectUpdatedAt *time.Time, expectRetryJobID *uint) hostDEPRow {
	...
	checkListHostDEPError := func(serial string, expectStatus string, expectError bool) *mobius.HostResponse {
		listHostsRes := listHostsResponse{}
		s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts?query=%s", serial), nil, http.StatusOK, &listHostsRes)
		require.Len(t, listHostsRes.Hosts, 1)
		...
		case "/profile/devices":
			...
			for _, device := range prof.Devices {
				...
				case expectAssignProfileResponseNotAccessible:
					resp.Devices[device] = string(mobius.DEPAssignProfileResponseNotAccessible)
				case expectAssignProfileResponseFailed:
					resp.Devices[device] = string(mobius.DEPAssignProfileResponseFailed)
				default:
					resp.Devices[device] = string(mobius.DEPAssignProfileResponseSuccess)
				}
			}
	...
	require.Len(t, profileAssignmentReqs[0].Devices, 1)
	checkHostDEPAssignProfileResponses(profileAssignmentReqs[0].Devices, profileAssignmentReqs[0].ProfileUUID, mobius.DEPAssignProfileResponseSuccess)
	require.Len(t, profileAssignmentReqs[1].Devices, len(devices))
	checkHostDEPAssignProfileResponses(profileAssignmentReqs[1].Devices, profileAssignmentReqs[1].ProfileUUID, mobius.DEPAssignProfileResponseSuccess)
	...
	listHostsRes = listHostsResponse{}
	s.DoJSON("GET", "/api/latest/mobius/hosts?mdm_enrollment_status=pending", nil, http.StatusOK, &listHostsRes)
	require.Len(t, listHostsRes.Hosts, len(devices))

	// searching by display name works
	listHostsRes = listHostsResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts?query=%s", url.QueryEscape("MacBook Mini")), nil, http.StatusOK, &listHostsRes)
	...
	s.DoJSON("GET", "/api/latest/mobius/activities", nil, http.StatusOK, &activities, "order_key", "created_at")
	found := false
	...
	// Enroll the host to be deleted. It will stay in Mobius after deletion from DEP.
	...
	checkHostDEPAssignProfileResponses(profileAssignmentReqs[ix2Devices].Devices, profileAssignmentReqs[ix2Devices].ProfileUUID, mobius.DEPAssignProfileResponseSuccess)
	...
	checkHostDEPAssignProfileResponses(profileAssignmentReqs[ix1Device].Devices, profileAssignmentReqs[ix1Device].ProfileUUID, mobius.DEPAssignProfileResponseSuccess)
	...
	// delete the device from Mobius
	var delResp deleteHostResponse
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobius/hosts/%d", mdmDeviceID), nil, http.StatusOK, &delResp)

	// the device comes back as pending
	listHostsRes = listHostsResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts?query=%s", mdmDevice.UUID), nil, http.StatusOK, &listHostsRes)
	require.Len(t, listHostsRes.Hosts, 1)
	...
	// The user unenrolls from Mobius (e.g. was DEP enrolled but with `is_mdm_removable: true`
	...
	// Simulate mobiusdaemon re-enrolling automatically.
	err = mdmDevice.Enroll()
	require.NoError(t, err)
	...
	// enroll a host into Mobius
	eHost, err := s.ds.NewHost(context.Background(), &mobius.Host{
		ID:             1,
	...
	require.NoError(t, s.ds.SetOrUpdateMDMData(ctx, eHost.ID, false, true, s.server.URL, true, mobius.WellKnownMDMMobius, ""))
	...
	d := checkHostCooldown(eHost.HardwareSerial, profUUID, mobius.DEPAssignProfileResponseFailed, nil, expectNoJobID)
	...
	// s.Do("DELETE", fmt.Sprintf("/api/v1/mobius/teams/%d", dummyTeam.ID), nil, http.StatusOK)
	// checkPendingMacOSSetupAssistantJob("team_deleted", nil, []string{eHost.HardwareSerial}, 0)
	// s.runIntegrationsSchedule()
	// require.Empty(t, profileAssignmentReqs) // screened for cooldown
	// bySerial = checkHostDEPAssignProfileResponses([]string{eHost.HardwareSerial}, profUUID, mobius.DEPAssignProfileResponseFailed)
	...
	d = checkHostCooldown(eHost.HardwareSerial, profUUID, mobius.DEPAssignProfileResponseFailed, &failedAt, nil)
	...
	d = checkHostCooldown(eHost.HardwareSerial, profUUID, mobius.DEPAssignProfileResponseSuccess, nil, expectNoJobID) // retry job cleared
	...
	s.runIntegrationsSchedule()
	require.Empty(t, profileAssignmentReqs)
	checkHostCooldown(eHost.HardwareSerial, profUUID, mobius.DEPAssignProfileResponseSuccess, &succeededAt, expectNoJobID) // no change
	checkNoJobsPending()
	...
	profUUID = profileAssignmentReqs[0].ProfileUUID
	d = checkHostCooldown(serial, profUUID, mobius.DEPAssignProfileResponseFailed, nil, expectNoJobID)
	require.NotZero(t, d.ResponseUpdatedAt)
	failedAt = d.ResponseUpdatedAt
	checkNoJobsPending()
	...
	s.runIntegrationsSchedule()
	require.Empty(t, profileAssignmentReqs) // assign profile request will be made when the retry job is processed on the next worker run
	d = checkHostCooldown(serial, profUUID, mobius.DEPAssignProfileResponseFailed, &failedAt, nil)
	require.NotZero(t, d.RetryJobID) // retry job created
	jobID = d.RetryJobID
	...
	profUUID = profileAssignmentReqs[0].ProfileUUID
	d = checkHostCooldown(serial, profUUID, mobius.DEPAssignProfileResponseSuccess, nil, expectNoJobID) // retry job cleared
	require.True(t, d.ResponseUpdatedAt.After(failedAt))
	checkNoJobsPending()
	...
	for _, req := range profileAssignmentReqs {
		...
		d := checkHostCooldown(serial, req.ProfileUUID, mobius.DEPAssignProfileResponseNotAccessible, nil, expectNoJobID) // not accessible responses aren't retried
		require.NotZero(t, d.ResponseUpdatedAt)
		failedAt = d.ResponseUpdatedAt
	}
	...
	s.runIntegrationsSchedule()
	require.Empty(t, profileAssignmentReqs)
	checkHostCooldown(serial, expectProfileUUID, mobius.DEPAssignProfileResponseNotAccessible, &failedAt, expectNoJobID) // no change
	checkNoJobsPending()
...
func (s *integrationMDMTestSuite) TestDEPProfileAssignmentWithMultipleABMs() {
	...
	checkHostDEPAssignProfileResponses := func(deviceSerials []string, expectedProfileUUID string,
		expectedStatus mobius.DEPAssignProfileResponseStatus,
	...
	checkHostDEPAssignProfileResponses(defaultSerials, defaultProfileUUIDs[len(defaultProfileUUIDs)-1],
		mobius.DEPAssignProfileResponseSuccess)
	checkHostDEPAssignProfileResponses(teamSerials, teamProfileUUIDs[len(teamProfileUUIDs)-1], mobius.DEPAssignProfileResponseSuccess)
...
func (s *integrationMDMTestSuite) TestSetupExperienceScript() {
	...
	res := s.DoRawWithHeaders("POST", "/api/latest/mobius/setup_experience/script", body.Bytes(), http.StatusOK, headers)
	err = json.NewDecoder(res.Body).Decode(&newScriptResp)
	require.NoError(t, err)

	// test script secret validation
	body, headers = generateNewScriptMultipartRequest(t,
		"script.sh", []byte(`echo "$MOBIUS_SECRET_INVALID"`), s.token, map[string][]string{})
	s.DoRawWithHeaders("POST", "/api/latest/mobius/setup_experience/script", body.Bytes(), http.StatusUnprocessableEntity, headers)

	// get team script metadata
	var getScriptResp getSetupExperienceScriptResponse
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/setup_experience/script?team_id=%d", tm.ID), nil, http.StatusOK, &getScriptResp)
	...
	res = s.Do("GET", fmt.Sprintf("/api/latest/mobius/setup_experience/script?team_id=%d&alt=media", tm.ID), nil, http.StatusOK)
	b, err := io.ReadAll(res.Body)
	require.NoError(t, err)
	...
	body, headers = generateNewScriptMultipartRequest(t,
		"script42.sh", []byte(`echo "hello"`), s.token, map[string][]string{"team_id": {fmt.Sprintf("%d", tm.ID)}})
	res = s.DoRawWithHeaders("POST", "/api/latest/mobius/setup_experience/script", body.Bytes(), http.StatusConflict, headers)
	...
	s.DoJSON("GET", "/api/latest/mobius/setup_experience/script", nil, http.StatusOK, &getScriptResp)
	require.Equal(t, "script42.sh", getScriptResp.Name)
	...
	require.NotZero(t, getScriptResp.UpdatedAt)
	// // TODO: confirm if we will allow team_id=0 requests
	// s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/setup_experience/script?team_id=%d", noTeamID), nil, http.StatusOK, &getScriptResp)

	// get no-team script contents
	res = s.Do("GET", "/api/latest/mobius/setup_experience/script?alt=media", nil, http.StatusOK)
	b, err = io.ReadAll(res.Body)
	require.NoError(t, err)
	require.Equal(t, `echo "hello"`, string(b))
	require.Equal(t, int64(len(`echo "hello"`)), res.ContentLength)
	require.Equal(t, fmt.Sprintf("attachment;filename=\"%s %s\"", time.Now().Format(time.DateOnly), "script42.sh"), res.Header.Get("Content-Disposition"))
	// // TODO: confirm if we will allow team_id=0 requests
	// res = s.Do("GET", fmt.Sprintf("/api/latest/mobius/setup_experience/script?team_id=%d&alt=media", noTeamID), nil, http.StatusOK)

	// delete the no-team script
	s.Do("DELETE", "/api/latest/mobius/setup_experience/script", nil, http.StatusOK)

	// try get the no-team script
	s.Do("GET", "/api/latest/mobius/setup_experience/script", nil, http.StatusNotFound)

	// try deleting the no-team script again
	s.Do("DELETE", "/api/latest/mobius/setup_experience/script", nil, http.StatusOK) // TODO: confirm if we want to return not found

	// // TODO: confirm if we will allow team_id=0 requests
	// s.Do("DELETE", fmt.Sprintf("/api/latest/mobius/setup_experience/script/?team_id=%d", noTeamID), nil, http.StatusOK)

	// delete the team script
	s.Do("DELETE", fmt.Sprintf("/api/latest/mobius/setup_experience/script?team_id=%d", tm.ID), nil, http.StatusOK)

	// try get the team script
	s.Do("GET", fmt.Sprintf("/api/latest/mobius/setup_experience/script?team_id=%d", tm.ID), nil, http.StatusNotFound)

	// try deleting the team script again
	s.Do("DELETE", fmt.Sprintf("/api/latest/mobius/setup_experience/script?team_id=%d", tm.ID), nil, http.StatusOK) // TODO: confirm if we want to return not found
}

func (s *integrationMDMTestSuite) createTeamDeviceForSetupExperienceWithProfileSoftwareAndScript() (device godep.Device, host *mobius.Host, tm *mobius.Team) {
	t := s.T()
	ctx := context.Background()

	// enroll a device in a team with software to install and a script to execute
	s.enableABM("mobius-setup-experience")
	...
	s.DoJSON("PUT", "/api/v1/mobius/setup_experience/software", putSetupExperienceSoftwareRequest{TeamID: tm.ID, TitleIDs: []uint{titleID}}, http.StatusOK, &swInstallResp)
	...
	s.mockDEPResponse("mobius-setup-experience", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		encoder := json.NewEncoder(w)
		switch r.URL.Path {
		case "/session":
			err := encoder.Encode(map[string]string{"auth_session_token": "xyz"})
...
func (s *integrationMDMTestSuite) TestSetupExperienceFlowWithSoftwareAndScriptAutoRelease() {
	...
	// expected commands: install mobiusdaemon (install enterprise), install profiles
	// (custom one, mobiusdaemon configuration, mobius CA root)
	...
	for _, cmd := range cmds {
		...
		case "InstallProfile":
			...
			case strings.Contains(string(cmd.Command.InstallProfile.Payload), "<string>I1</string>"):
				profileCustomSeen = true
			case strings.Contains(string(cmd.Command.InstallProfile.Payload), fmt.Sprintf("<string>%s</string>", mobileconfig.MobiusdConfigPayloadIdentifier)):
				profileMobiusdSeen = true
			case strings.Contains(string(cmd.Command.InstallProfile.Payload), fmt.Sprintf("<string>%s</string>", mobileconfig.MobiusCARootConfigPayloadIdentifier)):
				profileMobiusCASeen = true
			case strings.Contains(string(cmd.Command.InstallProfile.Payload), fmt.Sprintf("<string>%s</string", mobileconfig.MobiusFileVaultPayloadIdentifier)) &&
				strings.Contains(string(cmd.Command.InstallProfile.Payload), "ForceEnableInSetupAssistant"):
				profileFileVaultSeen = true
			}
	...
	require.Len(t, statusResp.Results.ConfigurationProfiles, 3) // mobiusdaemon config, root CA, custom profile
	var profNames []string
	var profStatuses []mobius.MDMDeliveryStatus
	...
	require.ElementsMatch(t, []string{"N1", "Mobiusd configuration", "Mobius root certificate authority (CA)"}, profNames)
	require.ElementsMatch(t, []mobius.MDMDeliveryStatus{mobius.MDMDeliveryVerifying, mobius.MDMDeliveryVerifying, mobius.MDMDeliveryVerifying}, profStatuses)
	...
	require.Equal(t, mobius.SetupExperienceStatusPending, statusResp.Results.Script.Status)
	require.Len(t, statusResp.Results.Software, 1)
	require.Equal(t, "DummyApp", statusResp.Results.Software[0].Name)
	require.Equal(t, mobius.SetupExperienceStatusPending, statusResp.Results.Software[0].Status)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/software/titles/%d", *statusResp.Results.Software[0].SoftwareTitleID), nil, http.StatusOK, &getSoftwareTitleResp, "team_id", fmt.Sprintf("%d", *enrolledHost.TeamID))
	require.NotNil(t, getSoftwareTitleResp.SoftwareTitle)
	require.NotNil(t, getSoftwareTitleResp.SoftwareTitle.SoftwarePackage)

	debugPrintActivities := func(activities []*mobius.UpcomingActivity) []string {
		var res []string
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/activities/upcoming", enrolledHost.ID),
	...
	require.Equal(t, "DummyApp", statusResp.Results.Software[0].Name)
	require.Equal(t, mobius.SetupExperienceStatusRunning, statusResp.Results.Software[0].Status)
	require.NotNil(t, statusResp.Results.Software[0].SoftwareTitleID)
	require.NotZero(t, *statusResp.Results.Software[0].SoftwareTitleID)
	...
	require.Len(t, statusResp.Results.Software, 1)
	require.Equal(t, "DummyApp", statusResp.Results.Software[0].Name)
	require.Equal(t, mobius.SetupExperienceStatusSuccess, statusResp.Results.Software[0].Status)
	...
	require.NotNil(t, statusResp.Results.Script)
	require.Equal(t, "script.sh", statusResp.Results.Script.Name)
	require.Equal(t, mobius.SetupExperienceStatusRunning, statusResp.Results.Script.Status)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d", enrolledHost.ID), nil, http.StatusOK, &getHostResp)
	...
	expectedActivityDetail = fmt.Sprintf(`
{
  ...
  "policy_name": null
	...
	s.lastActivityMatches(mobius.ActivityTypeInstalledSoftware{}.ActivityName(), expectedActivityDetail, 0)
	...
	expectedActivityDetail = fmt.Sprintf(`
{
	...
	require.NotNil(t, statusResp.Results.Script)
	require.Equal(t, "script.sh", statusResp.Results.Script.Name)
	require.Equal(t, mobius.SetupExperienceStatusSuccess, statusResp.Results.Script.Status)
	...
	expectedActivityDetail = fmt.Sprintf(`
{
	...
	s.lastActivityMatches(mobius.ActivityTypeRanScript{}.ActivityName(), expectedActivityDetail, 0)
}
...
func (s *integrationMDMTestSuite) TestSetupExperienceFlowWithSoftwareAndScriptForceRelease() {
	...
	s.DoJSON("POST", "/api/mobius/orbit/setup_experience/status", json.RawMessage(fmt.Sprintf(`{"orbit_node_key": %q, "force_release": true}`, *enrolledHost.OrbitNodeKey)), http.StatusOK, &statusResp)
...
func (s *integrationMDMTestSuite) TestReenrollingADEDeviceAfterRemovingItFromABM() {
	...
	s.DoJSON("GET", fmt.Sprintf("/api/v1/mobius/hosts/%d", h.ID), getHostRequest{}, http.StatusOK, &hostResp)
	// 1 profile with mobiusdaemon configuration + 1 root CA config
	require.Len(t, *hostResp.Host.MDM.Profiles, 2)

	// Turn MDM off in the host
	s.Do("DELETE", fmt.Sprintf("/api/latest/mobius/hosts/%d/mdm", h.ID), nil, http.StatusNoContent)
	...
	// Simulate the device getting unassigned from Mobius in ABM
	devices = []godep.Device{
	...
	a := checkHostDEPAssignProfileResponses([]string{mdmDevice.SerialNumber}, profileAssignmentReqs[0].ProfileUUID, mobius.DEPAssignProfileResponseSuccess)
	...
	devices = []godep.Device{
		// In https://github.com/notawar/mobius/issues/23200, we saw a profileUUID being sent back on
...
func (s *integrationMDMTestSuite) TestEnforceMiniumOSVersion() {
	...
	bySerial := make(map[string]*mobius.Host, len(devices))
	for _, h := range listHostsRes.Hosts {
	...
	checkMDMEnrollEndpoint := func(t *testing.T, machineInfo *mobius.MDMAppleMachineInfo, expectEnrollInfo *mdmtest.AppleEnrollInfo, expectSoftwareUpdate *mobius.MDMAppleSoftwareUpdateRequiredDetails, expectErr string, ssoDisabled bool) error {
		var di string
		if machineInfo != nil {
		...
		cc := mobiushttp.NewClient(mobiushttp.WithTLSClientConfig(&tls.Config{
		...
		case expectSoftwareUpdate != nil:
			...
			var sur mobius.MDMAppleSoftwareUpdateRequired
			require.NoError(t, json.Unmarshal(body, &sur))
			require.NotNil(t, sur)
			require.Equal(t, mobius.MDMAppleSoftwareUpdateRequiredCode, sur.Code)
	...
	// case) and checks for the expected response
	checkMDMSSOEndpoint := func(t *testing.T, machineInfo *mobius.MDMAppleMachineInfo, expectSoftwareUpdate *mobius.MDMAppleSoftwareUpdateRequiredDetails, expectErr string) error {
		...
		case response.StatusCode == http.StatusOK:
			...
			// https://github.com/notawar/mobius/blob/e62956924bbe041a1e75be2b0b7c6d1dd235a09d/server/service/testing_utils.go#L420-L421
			return nil
	...
	setMinOSVersion := func(minVersion string, deadline string, teamID *uint) {
		...
		if teamID == nil {
			acResp := appConfigResponse{}
			s.DoJSON("PATCH", "/api/latest/mobius/config", raw, http.StatusOK, &acResp)
			assert.NotNil(t, acResp.MDM.MacOSUpdates)
		} else {
			tcResp := teamResponse{}
			s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/teams/%d", *teamID), raw, http.StatusOK, &tcResp)
		}
	}
		...
		t.Run("sso disabled", func(t *testing.T) {
			for _, tc := range testCases {
				t.Run(tc.name, func(t *testing.T) {
					var mi *mobius.MDMAppleMachineInfo
...
func (s *integrationMDMTestSuite) TestSetupExperienceVPPInstallError() {
	...
	expDate := expTime.Format(mobius.VPPTimeFormat)
	...
	s.uploadDataViaForm("/api/latest/mobius/vpp_tokens", "token", "token.vpptoken", []byte(base64.StdEncoding.EncodeToString([]byte(tokenJSON))), http.StatusAccepted, "", &validToken)

	var getVPPTokenResp getVPPTokensResponse
	s.DoJSON("GET", "/api/latest/mobius/vpp_tokens", &getVPPTokensRequest{}, http.StatusOK, &getVPPTokenResp)
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobius/vpp_tokens/%d/teams", getVPPTokenResp.Tokens[0].ID), patchVPPTokensTeamsRequest{TeamIDs: []uint{team.ID}}, http.StatusOK, &resPatchVPP)

	// Add the app with 0 licenses available
	s.Do("POST", "/api/latest/mobius/software/app_store_apps", &addAppStoreAppRequest{TeamID: &team.ID, AppStoreID: "5", SelfService: true}, http.StatusOK)
	...
	var swInstallResp putSetupExperienceSoftwareResponse
	s.DoJSON("PUT", "/api/v1/mobius/setup_experience/software", putSetupExperienceSoftwareRequest{TeamID: team.ID, TitleIDs: []uint{vppTitleID, installerTitleID}}, http.StatusOK, &swInstallResp)
	...
	require.Equal(t, mobius.SetupExperienceStatusPending, statusResp.Results.Software[1].Status)
	...
	// should continue
	require.Equal(t, "App 5", statusResp.Results.Software[1].Name)
	require.Equal(t, mobius.SetupExperienceStatusFailure, statusResp.Results.Software[1].Status)
...
func (s *integrationMDMTestSuite) TestSetupExperienceFlowCancelScript() {
	...
	// call the /status endpoint, the software and script should be pending
	var statusResp getOrbitSetupExperienceStatusResponse
	s.DoJSON("POST", "/api/mobius/orbit/setup_experience/status", json.RawMessage(fmt.Sprintf(`{"orbit_node_key": %q}`, *host.OrbitNodeKey)), http.StatusOK, &statusResp)
	require.Nil(t, statusResp.Results.BootstrapPackage)         // no bootstrap package involved
	require.Nil(t, statusResp.Results.AccountConfiguration)     // no SSO involved
	require.Len(t, statusResp.Results.ConfigurationProfiles, 3) // mobiusdaemon config, root CA, custom profile
	...
	// script until after the software is done
	var hostActivitiesResp listHostUpcomingActivitiesResponse
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d/activities/upcoming", host.ID),
		nil, http.StatusOK, &hostActivitiesResp)
	...
	s.Do("DELETE", fmt.Sprintf("/api/latest/mobius/hosts/%d/activities/upcoming/%s", host.ID, swExecID),
	...
	require.Len(t, statusResp.Results.Software, 1)
	require.Equal(t, "DummyApp", statusResp.Results.Software[0].Name)
	require.Equal(t, mobius.SetupExperienceStatusFailure, statusResp.Results.Software[0].Status)
	require.NotNil(t, statusResp.Results.Software[0].SoftwareTitleID)
	require.NotZero(t, *statusResp.Results.Software[0].SoftwareTitleID)
	...
	// cancel the script
	s.Do("DELETE", fmt.Sprintf("/api/latest/mobius/hosts/%d/activities/upcoming/%s", host.ID, scrExecID),
		nil, http.StatusNoContent)
	...
	require.NotNil(t, statusResp.Results.Script)
	require.Equal(t, "script.sh", statusResp.Results.Script.Name)
	require.Equal(t, mobius.SetupExperienceStatusFailure, statusResp.Results.Script.Status)
	require.Len(t, statusResp.Results.Software, 1)
	require.Equal(t, "DummyApp", statusResp.Results.Software[0].Name)
	require.Equal(t, mobius.SetupExperienceStatusFailure, statusResp.Results.Software[0].Status)
	require.NotNil(t, statusResp.Results.Software[0].SoftwareTitleID)
...
func (s *integrationMDMTestSuite) TestDeleteMultipleHostsPendingDEP() {
	...
	bySerial := make(map[string]*mobius.HostResponse, len(devices))
	for _, host := range listHostsRes.Hosts {
	...
	hostResp := getHostResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/%d", target.ID), nil, http.StatusOK, &hostResp)
	...
	s.Do("POST", "/api/latest/mobius/hosts/delete", deleteHostsRequest{IDs: deleteIds}, http.StatusOK)

	// all devices should be restored as pending DEP hosts
	gotHosts := listHostsResponse{}
	s.DoJSON("GET", "/api/latest/mobius/hosts", nil, http.StatusOK, &gotHosts)
