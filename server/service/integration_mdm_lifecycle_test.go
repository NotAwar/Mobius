...
//   - Noah: When MDM is turned on, install mobiusdaemon, bootstrap package (if DEP),
...
type mdmLifecycleAssertion[T any] func(t *testing.T, host *mobius.Host, device T)
...
func (s *integrationMDMTestSuite) TestTurnOnLifecycleEventsApple() {
	...
	testCases := []struct {
	...
	}{
		{
			...
			func(t *testing.T, host *mobius.Host, device *mdmtest.TestAppleMDMClient) {
				s.Do(
					"POST",
					fmt.Sprintf("/api/latest/mobius/hosts/%d/wipe", host.ID),
					nil,
					http.StatusOK,
				)
		...
		},
		{
			...
			func(t *testing.T, host *mobius.Host, device *mdmtest.TestAppleMDMClient) {
				s.Do(
					"POST",
					fmt.Sprintf("/api/latest/mobius/hosts/%d/lock", host.ID),
					nil,
					http.StatusOK,
				)
		...
		},
		{
			...
			func(t *testing.T, host *mobius.Host, device *mdmtest.TestAppleMDMClient) {
				...
				s.Do(
					"DELETE",
					fmt.Sprintf("/api/latest/mobius/hosts/%d/mdm", host.ID),
					nil,
		...
		},
		{
			...
			func(t *testing.T, host *mobius.Host, device *mdmtest.TestAppleMDMClient) {
				...
				s.DoJSON(
					"DELETE",
					fmt.Sprintf("/api/latest/mobius/hosts/%d", host.ID),
					nil,
					http.StatusOK,
					&delResp,
				)
	...
	assertAction := func(t *testing.T, host *mobius.Host, device *mdmtest.TestAppleMDMClient, action mdmLifecycleAssertion[*mdmtest.TestAppleMDMClient]) {
...
func (s *integrationMDMTestSuite) TestTurnOnLifecycleEventsWindows() {
	...
	testCases := []struct {
		...
		{
			"wiped host turns on MDM",
			func(t *testing.T, host *mobius.Host, device *mdmtest.TestWindowsMDMClient) {
				s.Do(
					"POST",
					fmt.Sprintf("/api/latest/mobius/hosts/%d/wipe", host.ID),
				...
				require.Equal(t, wipeCmd.Verb, mobius.CmdExec)
	...
	assertAction := func(t *testing.T, host *mobius.Host, device *mdmtest.TestWindowsMDMClient, action mdmLifecycleAssertion[*mdmtest.TestWindowsMDMClient]) {
	...
	for _, tt := range testCases {
		t.Run(tt.Name, func(t *testing.T) {
			t.Run("programmatic enrollment", func(t *testing.T) {
				...
				err := s.ds.SetOrUpdateMDMData(context.Background(), host.ID, false, true, s.server.URL, false, mobius.WellKnownMDMMobius, "")
				require.NoError(t, err)
				assertAction(t, host, device, tt.Action)
			})
			...
			t.Run("automatic enrollment", func(t *testing.T) {
				...
				err := s.ds.ApplyEnrollSecrets(context.Background(), nil, []*mobius.EnrollSecret{{Secret: t.Name()}})
				require.NoError(t, err)
...
) ([]mobius.ProtoCmdOperation, getHostMDMSummaryResponse, getHostMDMResponseTest) {
	t := s.T()

	var recordedCmds []mobius.ProtoCmdOperation
	cmds, err := device.StartManagementSession()
	require.NoError(t, err)
...
func (s *integrationMDMTestSuite) setupLifecycleSettings() {
	...
	s.uploadBootstrapPackage(
		&mobius.MDMAppleBootstrapPackage{Bytes: bp, Name: "pkg.pkg", TeamID: 0},
		http.StatusOK,
		"",
	)
	...
	s.Do(
		"POST",
		"/api/v1/mobius/mdm/profiles/batch",
		batchSetMDMProfilesRequest{Profiles: []mobius.MDMProfileBatchPayload{
...
func (s *integrationMDMTestSuite) TestLifecycleSCEPCertExpiration() {
	...
	hostsBySerial := make(map[string]mobius.Host)
	...
	require.NoError(t, err)
	signedPkg := b
	s.uploadBootstrapPackage(&mobius.MDMAppleBootstrapPackage{Bytes: signedPkg, Name: "bs.pkg", TeamID: 0}, http.StatusOK, "")
	...
	s.Do(
		"POST",
		"/api/v1/mobius/mdm/apple/profiles/batch",
		batchSetMDMAppleProfilesRequest{Profiles: globalProfiles},
		http.StatusNoContent,
	)
	expectedProfiles := 4 // Mobiusd configuration, Mobius root cert, N1, N2

	s.runWorker()
	s.awaitTriggerProfileSchedule(t)

	ackAllCommands := func(mdmDevice *mdmtest.TestAppleMDMClient, wantMobiusdInstall, wantBootstrapInstall bool) int {
		var count int
		var foundMobiusdInstall, foundBootstrapInstall bool
		...
		for cmd != nil {
			...
			case "InstallEnterpriseApplication":
				if murl := fullCmd.Command.InstallEnterpriseApplication.ManifestURL; murl != nil {
					require.Contains(t, *murl, mobiusdbase.GetPKGManifestURL())
					foundMobiusdInstall = true
				} else {
					...
					require.Contains(t, manifest.ManifestItems[0].Assets[0].URL, "mobius/mdm/bootstrap")
		...
		require.Equal(t, wantMobiusdInstall, foundMobiusdInstall)
	...
	// `MOBIUS_SILENT_MIGRATION_ENROLLMENT_PROFILE` is not set
	cmd, err = migratedDevice.Idle()
	require.NoError(t, err)
	require.Nil(t, cmd)

	// set the env var, and run the cron
	t.Setenv("MOBIUS_SILENT_MIGRATION_ENROLLMENT_PROFILE", base64.StdEncoding.EncodeToString([]byte("<foo></foo>")))
	err = RenewSCEPCertificates(ctx, logger, s.ds, &mobiusCfg, s.mdmCommander)
	require.NoError(t, err)
	...
	// handle the case of a host being deleted, see https://github.com/notawar/mobius/issues/19149
	expireCerts()
	req := deleteHostsRequest{
...
func (s *integrationMDMTestSuite) TestRefetchAfterReenrollIOSNoDelete() {
	...
	triggerRefetchCron := func(hostID uint, expectCmds int) {
		...
		trigger := triggerRequest{
			Name: string(mobius.CronAppleMDMIPhoneIPadRefetcher),
		}
		s.Do("POST", "/api/latest/mobius/trigger", trigger, http.StatusOK)
	...
	s.DoJSON("POST", "/api/latest/mobius/spec/enroll_secret", applyEnrollSecretSpecRequest{
		Spec: &mobius.EnrollSecretSpec{
			Secrets: []*mobius.EnrollSecret{{Secret: globalSecret}},
		},
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobius/hosts/identifier/%s", mdmDevice.UUID), nil, http.StatusOK, &hostByIdentifierResp)
	...
	for cmd != nil {
		...
		case "InstalledApplicationList":
			cmd, err = mdmDevice.AcknowledgeInstalledApplicationList(mdmDevice.UUID, cmd.CommandUUID, []mobius.Software{})
			require.NoError(t, err)
	...
	require.ElementsMatch(t, []string{mobius.RefetchDeviceCommandUUIDPrefix, mobius.RefetchAppsCommandUUIDPrefix, mobius.RefetchCertsCommandUUIDPrefix}, cmdTypes)
...
// See https://github.com/notawar/mobius/issues/30192
func (s *integrationMDMTestSuite) TestMDMLockHostUnenrolled() {
	...
	res := s.Do(
		"POST",
		fmt.Sprintf("/api/latest/mobius/hosts/%d/lock", hostID),
		nil,
