...
import (
	...
	"github.com/notawar/mobiuss/v4/pkg/file"
	"github.com/notawar/mobiuss/v4/pkg/mobiusdbase"
	"github.com/notawar/mobiuss/v4/pkg/mdm/mdmtest"
	"github.com/notawar/mobiuss/v4/pkg/optjson"
	"github.com/notawar/mobiuss/v4/server/bindata"
	"github.com/notawar/mobiuss/v4/server/config"
	"github.com/notawar/mobiuss/v4/server/datastore/mysql"
	"github.com/notawar/mobiuss/v4/server/datastore/redis/redistest"
	"github.com/notawar/mobiuss/v4/server/datastore/s3"
	"github.com/notawar/mobiuss/v4/servermobiusus"
	"github.com/notawar/mobiuss/v4/server/live_query/live_query_mock"
	servermdm "github.com/notawar/mobiuss/v4/server/mdm"
	apple_mdm "github.com/notawar/mobiuss/v4/server/mdm/apple"
	"github.com/notawar/mobiuss/v4/server/mdm/apple/mobileconfig"
	"github.com/notawar/mobiuss/v4/server/mdm/apple/vpp"
	microsoft_mdm "github.com/notawar/mobiuss/v4/server/mdm/microsoft"
	"github.com/notawar/mobiuss/v4/server/mdm/microsoft/syncml"
	nanodep_client "github.com/notawar/mobiuss/v4/server/mdm/nanodep/client"
	"github.com/notawar/mobiuss/v4/server/mdm/nanodep/godep"
	nanodep_storage "github.com/notawar/mobiuss/v4/server/mdm/nanodep/storage"
	"github.com/notawar/mobiuss/v4/server/mdm/nanodep/tokenpki"
	"github.com/notawar/mobiuss/v4/server/mdm/nanomdm/mdm"
	"github.com/notawar/mobiuss/v4/server/mdm/nanomdm/push"
	nanomdm_pushsvc "github.com/notawar/mobiuss/v4/server/mdm/nanomdm/push/service"
	scepserver "github.com/notawar/mobiuss/v4/server/mdm/scep/server"
	mdmtesting "github.com/notawar/mobiuss/v4/server/mdm/testing_utils"
	"github.com/notawar/mobiuss/v4/server/ptr"
	"github.com/notawar/mobiuss/v4/server/service/integrationtest/scep_server"
	"github.com/notawar/mobiuss/v4/server/service/mock"
	"github.com/notawar/mobiuss/v4/server/service/osquery_utils"
	"github.com/notawar/mobiuss/v4/server/service/schedule"
	"github.com/notawar/mobiuss/v4/server/test"
	"github.com/notawar/mobiuss/v4/server/worker"
...
type integrationMDMTestSuite struct {
	...
	mobiusDMNextCSRStatus       atomic.Value
...
func (s *integrationMDMTestSuite) SetupSuite() {
	...
	var softwareInstallerStore mobiuss.SoftwareInstallerStore
	var bootstrapPackageStore mobiuss.MDMBootstrapPackageStore
	_, minioEnabled := os.LookupEnv("MINIO_STORAGE_TEST")
	if wantStore := os.Getenv("MOBIUS_INTEGRATION_TESTS_SOFTWARE_INSTALLER_STORE"); minioEnabled &&
	...
	serverConfig := TestServerOpts{
		...
		StartCronSchedules: []TestNewScheduleFunc{
			func(ctx context.Context, ds mobiuss.Datastore)mobiusus.NewCronScheduleFunc {
				return func() (mobiuss.CronSchedule, error) {
					const name = string(mobiuss.CronMDMAppleProfileManager)
					logger := cronLog
			...
			func(ctx context.Context, ds mobiuss.Datastore)mobiusus.NewCronScheduleFunc {
				return func() (mobiuss.CronSchedule, error) {
					const name = string(mobiuss.CronWorkerIntegrations)
					logger := cronLog
			...
			func(ctx context.Context, ds mobiuss.Datastore)mobiusus.NewCronScheduleFunc {
				return func() (mobiuss.CronSchedule, error) {
					const name = string(mobiuss.CronAppleMDMIPhoneIPadRefetcher)
					logger := cronLog
	...
	err = s.ds.InsertMDMConfigAssets(context.Background(), []mobiuss.MDMConfigAsset{
		{Name: mobiuss.MDMAssetSCEPChallenge, Value: []byte(s.scepChallenge)},
	}, nil)
	require.NoError(s.T(), err)
	...
	mobiusdmSrv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		status := s.mobiusDMNextCSRStatus.Swap(http.StatusOK)
	...
	s.T().Setenv("MOBIUS_DEV_GDMF_URL", s.appleGDMFSrv.URL)
	s.T().Setenv("TEST_MOBIUSDAEMONM_API_URL", mobiusdmSrv.URL)
	s.T().Setenv("MOBIUS_DEV_ITUNES_URL", s.appleITunesSrv.URL)

	s.mockedDownloadMobiusdmMeta = mobiusdbase.Metadata{
		MSIURL:           fmt.Sprintf("https://download-testing.mobiusmdm.com/archive/stable/%s/mobiusdaemon-base.msi", uuid.NewString()),
		MSISha256:        uuid.NewString(),
		PKGURL:           fmt.Sprintf("https://download-testing.mobiusmdm.com/archive/stable/%s/mobiusdaemon-base.pkg", uuid.NewString()),
		PKGSha256:        uuid.NewString(),
		ManifestPlistURL: fmt.Sprintf("https://download-testing.mobiusmdm.com/archive/stable/%s/mobiusdaemon-base-manifest.plist", uuid.NewString()),
		Version:          "2024-06-25_03-01-17",
	}
	downloadMobiusdmSrv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path == "/stable/meta.json" {
			w.Header().Set("Content-Type", "application/json")
			w.WriteHeader(http.StatusOK)
			require.NoError(s.T(), json.NewEncoder(w).Encode(s.mockedDownloadMobiusdmMeta))
		}
	}))
	s.T().Setenv("MOBIUS_DEV_DOWNLOAD_MOBIUSDAEMONM_URL", downloadMobiusdmSrv.URL)
	s.T().Cleanup(downloadMobiusdmSrv.Close)
	...
	s.DoJSON("POST", "/api/latest/mobiuss/spec/enroll_secret", applyEnrollSecretSpecRequest{
		Spec: &mobiuss.EnrollSecretSpec{
			Secrets: []*mobiuss.EnrollSecret{{Secret: "global-secret"}},
		},
	}, http.StatusOK, &applyResp)

	s.T().Cleanup(mobiusdmSrv.Close)
...
func (s *integrationMDMTestSuite) FailNextCSRRequestWith(status int) {
	s.mobiusDMNextCSRStatus.Store(status)
}

func (s *integrationMDMTestSuite) SucceedNextCSRRequest() {
	s.mobiusDMNextCSRStatus.Store(http.StatusOK)
}
...
func (s *integrationMDMTestSuite) TearDownTest() {
	...
	appCfg.MDM.WindowsUpdates = mobiuss.WindowsUpdates{}
	...
	// deleting the mobiuss-specific ones.
...
func (s *integrationMDMTestSuite) TestGetBootstrapToken() {
	...
	t.Run("no device record", func(t *testing.T) {
		...
		// if not found, server returns empty body and no error (see https://github.com/notawar/nanomdm/pull/8)
		res, err := mdmDevice.GetBootstrapToken()
		require.NoError(t, err)
		require.Nil(t, res)
	})
...
func (s *integrationMDMTestSuite) TestAppleGetAppleMDM() {
	...
	require.Equal(t, mobiuss.TeamNameNoTeam, tok.MacOSTeam.Name)
	require.Equal(t, mobiuss.TeamNameNoTeam, tok.IOSTeam.Name)
	require.Equal(t, mobiuss.TeamNameNoTeam, tok.IPadOSTeam.Name)
	...
	// create a new team
	tm, err := s.ds.NewTeam(context.Background(), &mobiuss.Team{
		Name:        t.Name(),
		Description: "desc",
	})
	require.NoError(t, err)
	...
	s.DoJSON("PATCH", "/api/latest/mobiuss/config", json.RawMessage(fmt.Sprintf(`{
		"mdm": {
	...
	t.Cleanup(func() {
		s.DoJSON("PATCH", "/api/latest/mobiuss/config", json.RawMessage(`{
			"mdm": {
				"apple_business_manager": []
			}
...
func (s *integrationMDMTestSuite) getABMTokenByName(orgName string, tokens []*mobiuss.ABMToken) mobiusus.ABMToken {
...
func (s *integrationMDMTestSuite) TestABMTeamPersistsOnConfigChange() {
	...
	team, err := s.ds.NewTeam(context.Background(), &mobiuss.Team{
		Name:        "team1",
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobiuss/abm_tokens/%d/teams", tok.ID), json.RawMessage(fmt.Sprintf(`{
    "ios_team_id": %d
...
func (s *integrationMDMTestSuite) TestABMExpiredToken() {
	...
	ctx := context.Background()
	mobiusSyncer := apple_mdm.NewDEPService(s.ds, s.depStorage, s.logger)

	// not signed error flips the AppleBMTermsExpired flag
	returnType = "not_signed"
	err := mobiusSyncer.RunAssigner(ctx)
...
func setupExpectedMobiusdProfile(t *testing.T, serverURL string, enrollSecret string, teamID *uint) []byte {
	var b bytes.Buffer
	params := mobileconfig.MobiusdProfileOptions{
		EnrollSecret: enrollSecret,
		ServerURL:    serverURL,
		PayloadType:  mobileconfig.MobiusdConfigPayloadIdentifier,
		PayloadName:  servermdm.MobiusdConfigProfileName,
	}
...
func setupExpectedCAProfile(t *testing.T, ds *mysql.Datastore) []byte {
	assets, err := ds.GetAllMDMConfigAssetsByName(context.Background(), []mobiuss.MDMAssetName{
		mobiuss.MDMAssetCACert,
	}, nil)
	require.NoError(t, err)

	block, _ := pem.Decode(assets[mobiuss.MDMAssetCACert].Value)
	require.NotNil(t, block)
	require.Equal(t, "CERTIFICATE", block.Type)

	var b bytes.Buffer
	params := mobileconfig.MobiusCARootTemplateOptions{
		PayloadName:       servermdm.MobiusCAConfigProfileName,
		PayloadIdentifier: mobileconfig.MobiusCARootConfigPayloadIdentifier,
		Certificate:       base64.StdEncoding.EncodeToString(block.Bytes),
	}
	err = mobileconfig.MobiusCARootTemplate.Execute(&b, params)
	require.NoError(t, err)
	return b.Bytes()
}
...
func createHostThenEnrollMDM(ds mobiuss.Datastore, mobiusServerURL string, t *testing.T) (mobiusus.Host, *mdmtest.TestAppleMDMClient) {
	mobiusHost, err := ds.NewHost(context.Background(), &mobiuss.Host{
	...
	require.NoError(t, err)
	mdmDevice := enrollMacOSHostInMDM(t, mobiusHost, ds, mobiusServerURL)
...
func (s *integrationMDMTestSuite) createAppleMobileHostThenEnrollMDM(platform string) (*mobiuss.Host, *mdmtest.TestAppleMDMClient) {
	ctx := context.Background()
	t := s.T()
	...
	mobiusHost, err := s.ds.NewHost(ctx, &mobiuss.Host{
	...
	require.NoError(t, err)
	require.Equal(t, dbZeroTime, mobiusHost.LastEnrolledAt)
...
func createWindowsHostThenEnrollMDM(ds mobiuss.Datastore, mobiusServerURL string, t *testing.T) (mobiusus.Host, *mdmtest.TestWindowsMDMClient) {
	host := createOrbitEnrolledHost(t, "windows", uuid.NewString(), ds)
	mdmDevice := enrollWindowsHostInMDM(t, host, ds, mobiusServerURL)
...
func enrollWindowsHostInMDM(t *testing.T, host *mobiuss.Host, dsmobiusus.Datastore, mobiusServerURL string) *mdmtest.TestWindowsMDMClient {
	...
	require.NoError(t, err)
	err = ds.SetOrUpdateMDMData(context.Background(), host.ID, false, true, mobiusServerURL, false, mobiuss.WellKnownMDMMobius, "")
	require.NoError(t, err)
	return mdmDevice
}
...
func (s *integrationMDMTestSuite) TestDeviceMDMManualEnroll() {
	...
	// invalid token fails
	s.DoRaw("GET", "/api/latest/mobiuss/device/invalid_token/mdm/apple/manual_enrollment_profile", nil, http.StatusUnauthorized)

	// valid token downloads the profile
	s.downloadAndVerifyOTAEnrollmentProfile("/api/latest/mobiuss/device/" + token + "/mdm/apple/manual_enrollment_profile")
}
...
func (s *integrationMDMTestSuite) TestAppleMDMDeviceEnrollment() {
	...
	require.NoError(t, err)
	s.lastActivityOfTypeMatches(mobiuss.ActivityTypeMDMEnrolled{}.ActivityName(),
	...
	// Find the ID of Mobius's MDM solution
	var mdmID uint
	mysql.ExecAdhocSQL(t, s.ds, func(q sqlx.ExtContext) error {
		return sqlx.GetContext(context.Background(), q, &mdmID,
			`SELECT id FROM mobile_device_management_solutions WHERE name = ?`, mobiuss.WellKnownMDMMobius)
	})

	// Check that both devices are returned by the /hosts endpoint
	listHostsRes := listHostsResponse{}
	s.DoJSON("GET", "/api/latest/mobiuss/hosts", nil, http.StatusOK, &listHostsRes, "mdm_id", fmt.Sprint(mdmID))
	...
	s.DoJSON("POST", "/api/latest/mobiuss/spec/enroll_secret", applyEnrollSecretSpecRequest{
		Spec: &mobiuss.EnrollSecretSpec{
			Secrets: []*mobiuss.EnrollSecret{{Secret: t.Name()}},
		},
	}, http.StatusOK, &applyResp)
	...
	// query all hosts
	listHostsRes = listHostsResponse{}
	s.DoJSON("GET", "/api/latest/mobiuss/hosts", nil, http.StatusOK, &listHostsRes)
	// we still have only two hosts
	require.Len(t, listHostsRes.Hosts, 2)

	// LastEnrolledAt should have been updated
	var getHostResp getHostResponse
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/hosts/%d", targetHostID), nil, http.StatusOK, &getHostResp)
	...
	s.DoJSON("GET", "/api/latest/mobiuss/activities", nil, http.StatusOK, &activities)
...
func (s *integrationMDMTestSuite) TestAppleMDMCSRRequest() {
	...
	// missing arguments
	s.DoJSON("POST", "/api/latest/mobiuss/mdm/apple/request_csr", requestMDMAppleCSRRequest{}, http.StatusUnprocessableEntity, &errResp)
	require.Len(t, errResp.Errors, 1)
	require.Equal(t, errResp.Errors[0].Name, "email_address")

	// invalid email address
	errResp = validationErrResp{}
	s.DoJSON("POST", "/api/latest/mobiuss/mdm/apple/request_csr", requestMDMAppleCSRRequest{EmailAddress: "abc", Organization: "def"}, http.StatusUnprocessableEntity, &errResp)
	require.Len(t, errResp.Errors, 1)
	require.Equal(t, errResp.Errors[0].Name, "email_address")

	// missing organization
	errResp = validationErrResp{}
	s.DoJSON("POST", "/api/latest/mobiuss/mdm/apple/request_csr", requestMDMAppleCSRRequest{EmailAddress: "a@b.c", Organization: ""}, http.StatusUnprocessableEntity, &errResp)
	require.Len(t, errResp.Errors, 1)
	require.Equal(t, errResp.Errors[0].Name, "organization")

	// mobiusmdm CSR request failed
	s.FailNextCSRRequestWith(http.StatusBadRequest)
	errResp = validationErrResp{}
	s.DoJSON("POST", "/api/latest/mobiuss/mdm/apple/request_csr", requestMDMAppleCSRRequest{EmailAddress: "a@b.c", Organization: "test"}, http.StatusUnprocessableEntity, &errResp)
	require.Len(t, errResp.Errors, 1)
	...
	errResp = validationErrResp{}
	s.DoJSON("POST", "/api/latest/mobiuss/mdm/apple/request_csr", requestMDMAppleCSRRequest{EmailAddress: "a@b.c", Organization: "test"}, http.StatusBadGateway, &errResp)
	require.Len(t, errResp.Errors, 1)
	require.Contains(t, errResp.Errors[0].Reason, "MobiusDM CSR request failed")

	var reqCSRResp requestMDMAppleCSRResponse
	// mobiusmdm CSR request succeeds
	s.SucceedNextCSRRequest()
	s.DoJSON("POST", "/api/latest/mobiuss/mdm/apple/request_csr", requestMDMAppleCSRRequest{EmailAddress: "a@b.c", Organization: "test"}, http.StatusOK, &reqCSRResp)
...
func (s *integrationMDMTestSuite) TestGetMDMCSR() {
	...
	s.uploadDataViaForm("/api/latest/mobiuss/mdm/apple/apns_certificate", "certificate", "certificate.pem", []byte("-----BEGIN CERTIFICATE-----\nZm9vCg==\n-----END CERTIFICATE-----"), http.StatusInternalServerError, "Couldn't add APNs certificate. Missing required private key. Learn how to configure the private key here: https://mobiusmdm.com/learn-more-aboutmobiusus-server-private-key", nil)

	r := s.Do("GET", "/api/latest/mobiuss/mdm/apple/request_csr", getMDMAppleCSRRequest{}, http.StatusInternalServerError)
	require.Contains(t, extractServerErrorText(r.Body), "Couldn't download signed CSR. Missing required private key. Learn how to configure the private key here: https://mobiusmdm.com/learn-more-about/mobiuss-server-private-key")
	...
	s.FailNextCSRRequestWith(http.StatusInternalServerError)
	errResp := validationErrResp{}
	s.DoJSON("GET", "/api/latest/mobiuss/mdm/apple/request_csr", getMDMAppleCSRRequest{}, http.StatusBadGateway, &errResp)
	require.Len(t, errResp.Errors, 1)
	require.Contains(t, errResp.Errors[0].Reason, "MobiusDM CSR request failed")
	...
	errResp = validationErrResp{}
	s.DoJSON("GET", "/api/latest/mobiuss/mdm/apple/request_csr", getMDMAppleCSRRequest{}, http.StatusUnprocessableEntity, &errResp)
	require.Len(t, errResp.Errors, 1)
	require.Contains(t, errResp.Errors[0].Reason, "this email address is not valid")

	// Invalid APNS cert upload attempt
	s.uploadDataViaForm("/api/latest/mobiuss/mdm/apple/apns_certificate", "certificate", "certificate.pem", []byte("invalid-cert"), http.StatusUnprocessableEntity, "Invalid certificate. Please provide a valid certificate from Apple Push Certificate Portal.", nil)
...
func (s *integrationMDMTestSuite) TestMDMAppleUnenroll() {
	...
	s.Do("POST", "/api/v1/mobiuss/mdm/apple/profiles/batch", batchSetMDMAppleProfilesRequest{Profiles: [][]byte{
	...
	s.DoJSON("GET", fmt.Sprintf("/api/v1/mobiuss/hosts/%d", h.ID), getHostRequest{}, http.StatusOK, &hostResp)
	// 3 profiles added + 1 profile with mobiusdaemon configuration + 1 root CA config
	...
	s.Do("DELETE", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/mdm", h.ID), nil, http.StatusNoContent)
	...
	s.Do("DELETE", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/mdm", h.ID), nil, http.StatusBadGateway)
	...
	s.Do("DELETE", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/mdm", h.ID), nil, http.StatusInternalServerError)
	...
	s.Do("DELETE", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/mdm", h.ID), nil, http.StatusConflict)
...
func (s *integrationMDMTestSuite) TestMDMDiskEncryptionSettingBackwardsCompat() {
	...
	s.assertConfigProfilesByIdentifier(nil, mobileconfig.MobiusFileVaultPayloadIdentifier, false)
	...
	s.assertConfigProfilesByIdentifier(nil, mobileconfig.MobiusFileVaultPayloadIdentifier, true)
	...
	checkTeamDiskEncryption := func(wantSetting bool) {
		...
		s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/teams/%d", team.ID), nil, http.StatusOK, &teamResp)
	...
	// new config takes precedence over old config
	teamSpecs := applyTeamSpecsRequest{Specs: []*mobiuss.TeamSpec{{
		Name: team.Name,
		MDM: mobiuss.TeamSpecMDM{
			EnableDiskEncryption: optjson.SetBool(false),
			MacOSSettings:        map[string]interface{}{"enable_disk_encryption": true},
		},
	}}}
	s.Do("POST", "/api/latest/mobiuss/spec/teams", teamSpecs, http.StatusOK)
	checkTeamDiskEncryption(false)
	s.assertConfigProfilesByIdentifier(ptr.Uint(team.ID), mobileconfig.MobiusFileVaultPayloadIdentifier, false)
	...
	s.assertConfigProfilesByIdentifier(ptr.Uint(team.ID), mobileconfig.MobiusFileVaultPayloadIdentifier, true)
...
func (s *integrationMDMTestSuite) TestDiskEncryptionSharedSetting() {
	...
	s.DoJSON("POST", "/api/latest/mobiuss/teams", team, http.StatusOK, &createTeamResp)
	...
	checkConfigSetSucceeds := func() {
		res := s.Do("PATCH", "/api/latest/mobiuss/config", json.RawMessage(`{
		...
		res = s.Do("POST", "/api/latest/mobiuss/spec/teams", teamSpecs, http.StatusOK)
		errMsg = extractServerErrorText(res.Body)
		require.Empty(t, errMsg)
		...
		s.Do("PATCH", "/api/latest/mobiuss/config", json.RawMessage(`{
		"mdm": { "enable_disk_encryption": false }
  }`), http.StatusOK)
		teamSpecs = map[string]any{
			"specs": []any{
...
func (s *integrationMDMTestSuite) TestEscrowBuddyBackwardsCompat() {
	...
	s.DoJSON("POST", "/api/mobiuss/orbit/config", json.RawMessage(fmt.Sprintf(`{"orbit_node_key": %q}`, *host.OrbitNodeKey)), http.StatusOK, &orbitConfigResp)
	...
	orbitConfigResp = orbitGetConfigResponse{}
	res := s.DoRawWithHeaders("POST", "/api/mobiuss/orbit/config", json.RawMessage(fmt.Sprintf(`{"orbit_node_key": %q}`, *host.OrbitNodeKey)), http.StatusOK, map[string]string{
		"Authorization":          fmt.Sprintf("Bearer %s", s.token),
		mobiuss.CapabilitiesHeader: stringmobiusus.CapabilityEscrowBuddy),
	})
...
func (s *integrationMDMTestSuite) TestMDMAppleHostDiskEncryption() {
	...
	// create a host
	host, err := s.ds.NewHost(ctx, &mobiuss.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
		SeenTime:        time.Now().Add(-1 * time.Minute),
	...
	assert.True(t, acResp.MDM.EnableDiskEncryption.Value)
	fileVaultProf := s.assertConfigProfilesByIdentifier(nil, mobileconfig.MobiusFileVaultPayloadIdentifier, true)
	hostCmdUUID := uuid.New().String()
	err = s.ds.BulkUpsertMDMAppleHostProfiles(ctx, []*mobiuss.MDMAppleBulkUpsertHostProfilePayload{
		{
			...
			Checksum:          []byte("csum"),
			Scope:             mobiuss.PayloadScopeSystem,
		},
	})
	require.NoError(t, err)

	t.Cleanup(func() {
		err := s.ds.UpdateOrDeleteHostMDMAppleProfile(ctx, &mobiuss.HostMDMAppleProfile{
			HostUUID:      host.UUID,
			CommandUUID:   hostCmdUUID,
			ProfileUUID:   fileVaultProf.ProfileUUID,
			Status:        &mobiuss.MDMDeliveryVerifying,
			OperationType: mobiuss.MDMOperationTypeRemove,
		})
		require.NoError(t, err)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/hosts/%d", host.ID), nil, http.StatusOK, &getHostResp)
	require.NotNil(t, getHostResp.Host.MDM.MacOSSettings.DiskEncryption)
	require.Equal(t, mobiuss.DiskEncryptionEnforcing, *getHostResp.Host.MDM.MacOSSettings.DiskEncryption)
	...
	require.Equal(t, mobiuss.DiskEncryptionEnforcing, *getHostResp.Host.MDM.OSSettings.DiskEncryption.Status)
	...
	err = s.ds.UpdateOrDeleteHostMDMAppleProfile(ctx, &mobiuss.HostMDMAppleProfile{
		...
		CommandUUID:   hostCmdUUID,
		ProfileUUID:   fileVaultProf.ProfileUUID,
		Status:        &mobiuss.MDMDeliveryFailed,
		OperationType: mobiuss.MDMOperationTypeInstall,
		Detail:        "test error",
	})
	require.NoError(t, err)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/hosts/%d", host.ID), nil, http.StatusOK, &getHostResp)
	require.NotNil(t, getHostResp.Host.MDM.MacOSSettings.DiskEncryption)
	require.Equal(t, mobiuss.DiskEncryptionFailed, *getHostResp.Host.MDM.MacOSSettings.DiskEncryption)
	require.Nil(t, getHostResp.Host.MDM.MacOSSettings.ActionRequired)
	require.NotNil(t, getHostResp.Host.MDM.OSSettings)
	require.NotNil(t, getHostResp.Host.MDM.OSSettings.DiskEncryption.Status)
	require.Equal(t, mobiuss.DiskEncryptionFailed, *getHostResp.Host.MDM.OSSettings.DiskEncryption.Status)
	...
	err = s.ds.UpdateOrDeleteHostMDMAppleProfile(ctx, &mobiuss.HostMDMAppleProfile{
		...
		CommandUUID:   hostCmdUUID,
		ProfileUUID:   fileVaultProf.ProfileUUID,
		Status:        &mobiuss.MDMDeliveryVerified,
		OperationType: mobiuss.MDMOperationTypeInstall,
		Detail:        "",
	})
	require.NoError(t, err)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/hosts/%d", host.ID), nil, http.StatusOK, &getHostResp)
	require.NotNil(t, getHostResp.Host.MDM.MacOSSettings.DiskEncryption)
	require.Equal(t, mobiuss.DiskEncryptionActionRequired, *getHostResp.Host.MDM.MacOSSettings.DiskEncryption)
	require.NotNil(t, getHostResp.Host.MDM.MacOSSettings.ActionRequired)
	require.Equal(t, mobiuss.ActionRequiredRotateKey, *getHostResp.Host.MDM.MacOSSettings.ActionRequired)
	require.NotNil(t, getHostResp.Host.MDM.OSSettings)
	require.NotNil(t, getHostResp.Host.MDM.OSSettings.DiskEncryption.Status)
	require.Equal(t, mobiuss.DiskEncryptionActionRequired, *getHostResp.Host.MDM.OSSettings.DiskEncryption.Status)
	require.Equal(t, "", getHostResp.Host.MDM.OSSettings.DiskEncryption.Detail)

	// add an encryption key for the host
	assets, err := s.ds.GetAllMDMConfigAssetsByName(ctx, []mobiuss.MDMAssetName{
		mobiuss.MDMAssetCACert,
	}, nil)
	require.NoError(t, err)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/hosts/%d", host.ID), nil, http.StatusOK, &getHostResp)
	require.NotNil(t, getHostResp.Host.MDM.MacOSSettings.DiskEncryption)
	require.Equal(t, mobiuss.DiskEncryptionVerifying, *getHostResp.Host.MDM.MacOSSettings.DiskEncryption)
	require.Nil(t, getHostResp.Host.MDM.MacOSSettings.ActionRequired)
	require.NotNil(t, getHostResp.Host.MDM.OSSettings)
	require.NotNil(t, getHostResp.Host.MDM.OSSettings.DiskEncryption.Status)
	require.Equal(t, mobiuss.DiskEncryptionVerifying, *getHostResp.Host.MDM.OSSettings.DiskEncryption.Status)
	require.Equal(t, "", getHostResp.Host.MDM.OSSettings.DiskEncryption.Detail)

	// request with no token
	res := s.DoRawNoAuth("GET", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/encryption_key", host.ID), nil, http.StatusUnauthorized)
	res.Body.Close()

	// encryption key not processed yet
	resp := getHostEncryptionKeyResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/encryption_key", host.ID), nil, http.StatusNotFound, &resp)
	...
	// decryptable key
	checkDecryptableKey := func(u mobiuss.User) {
		err = s.ds.SetHostsDiskEncryptionKeyStatus(ctx, []uint{host.ID}, true, time.Now())
		require.NoError(t, err)
		resp = getHostEncryptionKeyResponse{}
		s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/encryption_key", host.ID), nil, http.StatusOK, &resp)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/hosts/%d", host.ID), nil, http.StatusOK, &getHostResp)
	require.NotNil(t, getHostResp.Host.MDM.MacOSSettings.DiskEncryption)
	require.Equal(t, mobiuss.DiskEncryptionVerified, *getHostResp.Host.MDM.MacOSSettings.DiskEncryption)
	require.Nil(t, getHostResp.Host.MDM.MacOSSettings.ActionRequired)
	require.NotNil(t, getHostResp.Host.MDM.OSSettings)
	require.NotNil(t, getHostResp.Host.MDM.OSSettings.DiskEncryption.Status)
	require.Equal(t, mobiuss.DiskEncryptionVerified, *getHostResp.Host.MDM.OSSettings.DiskEncryption.Status)
	require.Equal(t, "", getHostResp.Host.MDM.OSSettings.DiskEncryption.Detail)
	...
	u = mobiuss.User{
		...
		GlobalRole: nil,
		Teams: []mobiuss.UserTeam{
			{
				Team: *team,
				Role: mobiuss.RoleMaintainer,
			},
		},
	}
	...
	team2, err := s.ds.NewTeam(context.Background(), &mobiuss.Team{
		ID:          4828,
		Name:        "team2_" + t.Name(),
	...
	s.token = s.getTestToken(u.Email, test.GoodPassword)
	resp = getHostEncryptionKeyResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/encryption_key", host.ID), nil, http.StatusForbidden, &resp)
}
...
func (s *integrationMDMTestSuite) TestWindowsMDMGetEncryptionKey() {
	...
	// create a host and enroll it in Mobius
	host := createOrbitEnrolledHost(t, "windows", "h1", s.ds)
	err := s.ds.SetOrUpdateMDMData(ctx, host.ID, false, true, s.server.URL, false, mobiuss.WellKnownMDMMobius, "")
	require.NoError(t, err)
	...
	// invalid host id
	resp = getHostEncryptionKeyResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/encryption_key", host.ID+999), nil, http.StatusNotFound, &resp)

	// add an encryption key for the host
	cert, _, _, err := s.mobiusCfg.MDM.MicrosoftWSTEP()
	require.NoError(t, err)
	recoveryKey := "AAA-BBB-CCC"
	...
	require.Equal(t, recoveryKey, resp.EncryptionKey.DecryptedValue)
	s.lastActivityOfTypeMatches(mobiuss.ActivityTypeReadHostDiskEncryptionKey{}.ActivityName(),
...
func (s *integrationMDMTestSuite) TestAppConfigMDMAppleDiskEncryption() {
	...
	assert.True(t, acResp.MDM.EnableDiskEncryption.Value)
	enabledDiskActID := s.lastActivityMatches(mobiuss.ActivityTypeEnabledMacosDiskEncryption{}.ActivityName(),
	...
	acResp = appConfigResponse{}
	s.DoJSON("GET", "/api/latest/mobiuss/config", nil, http.StatusOK, &acResp)
	assert.True(t, acResp.MDM.EnableDiskEncryption.Value)
	...
	assert.True(t, acResp.MDM.EnableDiskEncryption.Value)
	assert.Equal(t, []mobiuss.MDMProfileSpec{{Path: "a"}}, acResp.MDM.MacOSSettings.CustomSettings)
	s.lastActivityMatches(mobiuss.ActivityTypeEnabledMacosDiskEncryption{}.ActivityName(),
		``, enabledDiskActID)
	...
	assert.False(t, acResp.MDM.EnableDiskEncryption.Value)
	assert.Equal(t, []mobiuss.MDMProfileSpec{{Path: "b"}}, acResp.MDM.MacOSSettings.CustomSettings)
	s.lastActivityMatches(mobiuss.ActivityTypeDisabledMacosDiskEncryption{}.ActivityName(),
		`{"team_id": null, "team_name": null}`, 0)
	...
	s.Do("POST", "/api/latest/mobiuss/disk_encryption",
	...
	s.Do("PATCH", "/api/latest/mobiuss/mdm/apple/settings",
		mobiuss.MDMAppleSettingsPayload{}, http.StatusNoContent)
	s.lastActivityMatches(mobiuss.ActivityTypeEnabledMacosDiskEncryption{}.ActivityName(),
		``, enabledDiskActID)
...
func (s *integrationMDMTestSuite) TestMDMAppleDiskEncryptionAggregate() {
	...
	expectedNoTeamDiskEncryptionSummary := mobiuss.MDMDiskEncryptionSummary{}
	s.checkMDMDiskEncryptionSummaries(t, nil, expectedNoTeamDiskEncryptionSummary, true)
	expectedNoTeamProfilesSummary := mobiuss.MDMProfilesSummary{}
	s.checkMDMProfilesSummaries(t, nil, expectedNoTeamProfilesSummary, &expectedNoTeamProfilesSummary)

	// 10 new hosts
	var hosts []*mobiuss.Host
	for i := 0; i < 10; i++ {
		h, err := s.ds.NewHost(ctx, &mobiuss.Host{
			DetailUpdatedAt: time.Now(),
			LabelUpdatedAt:  time.Now(),
			PolicyUpdatedAt: time.Now(),
			SeenTime:        time.Now().Add(-1 * time.Minute),
	...
	prof, err := mobiuss.NewMDMAppleConfigProfile(mobileconfigForTest("filevault-1", mobileconfig.MobiusFileVaultPayloadIdentifier), ptr.Uint(0))
	require.NoError(t, err)

	// generates a disk encryption aggregate value based on the arguments passed in
	generateAggregateValue := func(
		hosts []*mobiuss.Host,
		operationType mobiuss.MDMOperationType,
		status *mobiuss.MDMDeliveryStatus,
		decryptable bool,
	) {
	...
	generateAggregateValue(hosts[0:2], mobiuss.MDMOperationTypeInstall, mobiusus.MDMDeliveryVerifying, true)
	...
	generateAggregateValue(hosts[2:4], mobiuss.MDMOperationTypeInstall, mobiusus.MDMDeliveryVerifying, false)
	...
	generateAggregateValue(hosts[4:6], mobiuss.MDMOperationTypeInstall, mobiusus.MDMDeliveryPending, true)
	...
	generateAggregateValue(hosts[4:6], mobiuss.MDMOperationTypeInstall, nil, true)
	...
	generateAggregateValue(hosts[6:8], mobiuss.MDMOperationTypeInstall, mobiusus.MDMDeliveryFailed, true)
	...
	generateAggregateValue(hosts[8:10], mobiuss.MDMOperationTypeRemove, mobiusus.MDMDeliveryPending, true)
	...
	tm, _ := s.ds.NewTeam(ctx, &mobiuss.Team{Name: "team-1"})
	err = s.ds.AddHostsToTeam(ctx, &tm.ID, []uint{hosts[0].ID, hosts[1].ID})
	require.NoError(t, err)

	// new filevault profile for team 1
	prof, err = mobiuss.NewMDMAppleConfigProfile(mobileconfigForTest("filevault-1", mobileconfig.MobiusFileVaultPayloadIdentifier), ptr.Uint(1))
	require.NoError(t, err)
	prof.TeamID = &tm.ID
	require.NoError(t, err)
	...
	var expectedTeamDiskEncryptionSummary mobiuss.MDMDiskEncryptionSummary
	...
	expectedTeamProfilesSummary := mobiuss.MDMProfilesSummary{Verifying: 2}
	...
	require.NoError(t, apple_mdm.VerifyHostMDMProfiles(ctx, s.ds, hosts[0], map[string]*mobiuss.HostMacOSProfile{prof.Identifier: {Identifier: prof.Identifier, DisplayName: prof.Name, InstallDate: time.Now()}}))
...
func (s *integrationMDMTestSuite) TestTeamsMDMAppleDiskEncryption() {
	...
	s.Do("POST", "/api/latest/mobiuss/spec/teams", teamSpecs, http.StatusOK)
	lastDiskActID := s.lastActivityOfTypeMatches(mobiuss.ActivityTypeEnabledMacosDiskEncryption{}.ActivityName(),
	...
	res := s.Do("POST", "/api/latest/mobiuss/spec/teams", teamSpecs, http.StatusBadRequest)
	errMsg := extractServerErrorText(res.Body)
	...
	s.Do("POST", "/api/v1/mobiuss/mdm/apple/profiles/batch", batchSetMDMAppleProfilesRequest{Profiles: nil},
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/teams/%d", team.ID), nil, http.StatusOK, &teamResp)
	require.True(t, teamResp.Team.Config.MDM.EnableDiskEncryption)
	require.Equal(t, []mobiuss.MDMProfileSpec{{Path: "a"}}, teamResp.Team.Config.MDM.MacOSSettings.CustomSettings)
	s.lastActivityOfTypeMatches(mobiuss.ActivityTypeEnabledMacosDiskEncryption{}.ActivityName(),
		``, lastDiskActID)
	...
	s.Do("POST", "/api/latest/mobiuss/spec/teams", teamSpecs, http.StatusOK, "dry_run", "true")
	teamResp = getTeamResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/teams/%d", team.ID), nil, http.StatusOK, &teamResp)
	require.True(t, teamResp.Team.Config.MDM.EnableDiskEncryption)
	s.lastActivityOfTypeMatches(mobiuss.ActivityTypeEnabledMacosDiskEncryption{}.ActivityName(),
	...
	s.lastActivityOfTypeMatches(mobiuss.ActivityTypeDisabledMacosDiskEncryption{}.ActivityName(),
		fmt.Sprintf(`{"team_id": %d, "team_name": %q}`, team.ID, teamName), 0)
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobiuss/teams/%d", team.ID),mobiusus.TeamPayload{
		MDM: &mobiuss.TeamPayloadMDM{
			EnableDiskEncryption: optjson.SetBool(true),
		},
	...
	s.Do("PATCH", "/api/latest/mobiuss/mdm/apple/settings",
		mobiuss.MDMAppleSettingsPayload{TeamID: ptr.Uint(team.ID)}, http.StatusNoContent)
	s.lastActivityOfTypeMatches(mobiuss.ActivityTypeEnabledMacosDiskEncryption{}.ActivityName(),
		``, lastDiskActID)
...
func (s *integrationMDMTestSuite) TestEnrollOrbitAfterDEPSync() {
	...
	s.DoJSON("POST", "/api/latest/mobiuss/spec/enroll_secret", applyEnrollSecretSpecRequest{
		Spec: &mobiuss.EnrollSecretSpec{
			Secrets: []*mobiuss.EnrollSecret{{Secret: secret}},
		},
	}, http.StatusOK, &applyResp)
	...
	s.DoJSON("POST", "/api/mobiuss/orbit/enroll", EnrollOrbitRequest{
		EnrollSecret:   secret,
	...
	var hostResp getHostResponse
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/hosts/%d", h.ID), nil, http.StatusOK, &hostResp)
...
func (s *integrationMDMTestSuite) TestMobiusdConfiguration() {
	t := s.T()
	s.assertConfigProfilesByIdentifier(nil, mobileconfig.MobiusdConfigPayloadIdentifier, false)
	...
	// a new mobiusdaemon configuration profile for "no team" is created
	s.awaitTriggerProfileSchedule(t)
	s.assertConfigProfilesByIdentifier(nil, mobileconfig.MobiusdConfigPayloadIdentifier, true)
	...
	require.NoError(t, err)
	s.assertConfigProfilesByIdentifier(&tm.ID, mobileconfig.MobiusdConfigPayloadIdentifier, false)
	...
	s.awaitTriggerProfileSchedule(t)
	p := s.assertConfigProfilesByIdentifier(&tm.ID, mobileconfig.MobiusdConfigPayloadIdentifier, true)
	require.Contains(t, string(p.Mobileconfig), t.Name())

	// create an enroll secret for the team
	teamSpecs := applyTeamSpecsRequest{Specs: []*mobiuss.TeamSpec{{
		Name:    tm.Name,
		Secrets: &[]mobiuss.EnrollSecret{{Secret: t.Name() + "team-secret"}},
	}}}
	s.Do("POST", "/api/latest/mobiuss/spec/teams", teamSpecs, http.StatusOK)

	// a new mobiusdaemon configuration profile for that team is created
	s.awaitTriggerProfileSchedule(t)
	p = s.assertConfigProfilesByIdentifier(&tm.ID, mobileconfig.MobiusdConfigPayloadIdentifier, true)
...
func (s *integrationMDMTestSuite) TestEnqueueMDMCommand() {
	...
	s.DoJSON("GET", "/api/latest/mobiuss/mdm/apple/commands", nil, http.StatusOK, &listCmdResp0)
...
<dict>
	...
	s.Do("POST", "/api/latest/mobiuss/mdm/apple/enqueue",
	...
	s.DoJSON("GET", "/api/latest/mobiuss/mdm/apple/commandresults", nil, http.StatusNotFound, &cmdResResp, "command_uuid", uuid1)
	var getMDMCmdResp getMDMCommandResultsResponse
	s.DoJSON("GET", "/api/latest/mobiuss/commands/results", nil, http.StatusNotFound, &cmdResResp, "command_uuid", uuid1)

	// list commands returns empty set
	var listCmdResp listMDMAppleCommandsResponse
	s.DoJSON("GET", "/api/latest/mobiuss/mdm/apple/commands", nil, http.StatusOK, &listCmdResp)
	require.Empty(t, listCmdResp.Results)

	// call with unenrolled host UUID
	res := s.Do("POST", "/api/latest/mobiuss/mdm/apple/enqueue",
		enqueueMDMAppleCommandRequest{
	...
	s.DoJSON("POST", "/api/latest/mobiuss/mdm/apple/enqueue",
		enqueueMDMAppleCommandRequest{
	...
	s.DoJSON("GET", "/api/latest/mobiuss/mdm/apple/commandresults", nil, http.StatusOK, &cmdResResp, "command_uuid", uuid2)
	require.Len(t, cmdResResp.Results, 0)
	s.DoJSON("GET", "/api/latest/mobiuss/commands/results", nil, http.StatusOK, &getMDMCmdResp, "command_uuid", uuid2)
	...
	require.Equal(t, &mobiuss.MDMCommandResult{
	...
	require.NoError(t, err)
	t.Logf("GET /api/latest/mobiuss/mdm/apple/commands response:\n%s", results)
	...
	var profileListCommands []*mobiuss.MDMAppleCommand
	...
	require.Equal(t, &mobiuss.MDMAppleCommand{
...
func (s *integrationMDMTestSuite) TestEnqueueMDMCommandWithSecret() {
	...
	// list commands should return all the commands we sent
	var listCmdResp listMDMAppleCommandsResponse
	s.DoJSON("GET", "/api/latest/mobiuss/mdm/apple/commands", nil, http.StatusOK, &listCmdResp, "host_identifier", mdmClient.UUID)
	require.Empty(t, listCmdResp.Results)
...
<dict>
    ...
    <dict>
        ...
        <string>ProfileList</string>
		<key>SecretValue</key>
		<string>$MOBIUS_SECRET_VALUE</string>
    </dict>
    <key>CommandUUID</key>
    <string>%s</string>
</dict>
	...
	req := secretVariablesRequest{
		SecretVariables: []mobiuss.SecretVariable{
			{
				Name:  "MOBIUS_SECRET_VALUE",
				Value: secretValue,
			},
		},
	}
	secretResp := secretVariablesResponse{}
	s.DoJSON("PUT", "/api/latest/mobiuss/spec/secret_variables", req, http.StatusOK, &secretResp)
	...
	assert.NotContains(t, string(cmd.Raw), "MOBIUS_SECRET_VALUE")
}
...
func (s *integrationMDMTestSuite) TestMDMWindowsCommandResults() {
	...
	dev := &mobiuss.MDMWindowsEnrolledDevice{
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/commands/results?command_uuid=%s", cmdUUID), nil, http.StatusOK, &resp)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/commands/results?command_uuid=%s", uuid.New().String()), nil, http.StatusNotFound, &resp)
...
func (s *integrationMDMTestSuite) TestAppConfigMDMMacOSMigration() {
	...
	checkDefaultAppConfig := func() {
		...
		s.DoJSON("GET", "/api/v1/mobiuss/config", nil, http.StatusOK, &ac)
	...
	// missing webhook_url
	s.DoJSON("PATCH", "/api/v1/mobiuss/config", json.RawMessage(`{
	...
	s.DoJSON("GET", "/api/v1/mobiuss/config", nil, http.StatusOK, &acResp)
	require.True(t, acResp.MDM.MacOSMigration.Enable)
	require.Equal(t, mobiuss.MacOSMigrationModeVoluntary, acResp.MDM.MacOSMigration.Mode)
...
func (s *integrationMDMTestSuite) TestBootstrapPackage() {
	...
	s.uploadBootstrapPackage(&mobiuss.MDMAppleBootstrapPackage{}, http.StatusBadRequest, "package multipart field is required")
	// no name
	s.uploadBootstrapPackage(&mobiuss.MDMAppleBootstrapPackage{Bytes: signedPkg}, http.StatusBadRequest, "package multipart field is required")
	// invalid
	s.uploadBootstrapPackage(&mobiuss.MDMAppleBootstrapPackage{Bytes: invalidPkg, Name: "invalid.tar.gz"}, http.StatusBadRequest, "invalid file type")
	// invalid names
	for _, char := range file.InvalidMacOSChars {
		s.uploadBootstrapPackage(
			&mobiuss.MDMAppleBootstrapPackage{
				Bytes: signedPkg,
				Name:  fmt.Sprintf("invalid_%c_name.pkg", char),
			}, http.StatusBadRequest, "")
	}
	// unsigned
	s.uploadBootstrapPackage(&mobiuss.MDMAppleBootstrapPackage{Bytes: unsignedPkg, Name: "pkg.pkg"}, http.StatusBadRequest, "file is not signed")
	// wrong TOC
	s.uploadBootstrapPackage(&mobiuss.MDMAppleBootstrapPackage{Bytes: wrongTOCPkg, Name: "pkg.pkg"}, http.StatusBadRequest, "invalid package")
	// not a Distribution package
	s.uploadBootstrapPackage(&mobiuss.MDMAppleBootstrapPackage{Bytes: read("not-distribution-signed.pkg"), Name: "pkg.pkg"}, http.StatusBadRequest,
		mobiuss.BootstrapPkgNotDistributionErrMsg)
	// successfully upload a package
	s.uploadBootstrapPackage(&mobiuss.MDMAppleBootstrapPackage{Bytes: signedPkg, Name: "pkg.pkg", TeamID: 0}, http.StatusOK, "")
	// check the activity log
	s.lastActivityMatches(
		mobiuss.ActivityTypeAddedBootstrapPackage{}.ActivityName(),
	...
	s.DoJSON("GET", "/api/latest/mobiuss/bootstrap/0/metadata", nil, http.StatusOK, &metadataResp)
	...
	s.DoJSON("GET", "/api/latest/mobiuss/bootstrap?token=bad", nil, http.StatusNotFound, &downloadResp)

	resp := s.DoRaw("GET", fmt.Sprintf("/api/latest/mobiuss/bootstrap?token=%s", metadataResp.MDMAppleBootstrapPackage.Token), nil, http.StatusOK)
	...
	s.DoJSON("GET", "/api/latest/mobiuss/bootstrap/1/metadata", nil, http.StatusNotFound, &metadataResp)

	// delete package
	var deleteResp deleteBootstrapPackageResponse
	s.DoJSON("DELETE", "/api/latest/mobiuss/bootstrap/0", nil, http.StatusOK, &deleteResp)
	// check the activity log
	s.lastActivityMatches(
		mobiuss.ActivityTypeDeletedBootstrapPackage{}.ActivityName(),
		`{"bootstrap_package_name": "pkg.pkg", "team_id": null, "team_name": null}`,
		0,
	)

	metadataResp = bootstrapPackageMetadataResponse{}
	s.DoJSON("GET", "/api/latest/mobiuss/bootstrap/0/metadata", nil, http.StatusNotFound, &metadataResp)
	// trying to delete again is a bad request
	s.DoJSON("DELETE", "/api/latest/mobiuss/bootstrap/0", nil, http.StatusNotFound, &deleteResp)
}
...
func (s *integrationMDMTestSuite) TestBootstrapPackageStatus() {
	...
	s.uploadBootstrapPackage(&mobiuss.MDMAppleBootstrapPackage{Bytes: pkg, Name: "pkg.pkg", TeamID: 0}, http.StatusOK, "")
	...
	s.uploadBootstrapPackage(&mobiuss.MDMAppleBootstrapPackage{Bytes: pkg, Name: "pkg.pkg", TeamID: team.ID}, http.StatusOK, "")

	// get package metadata
	metadataResp = bootstrapPackageMetadataResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/bootstrap/%d/metadata", team.ID), nil, http.StatusOK, &metadataResp)
	...
	// will be sent by the device, which should in turn be interpreted by Mobius as "Pending"). See
	...
	expectedSerialsByTeamAndStatus := make(map[uint]map[mobiuss.MDMBootstrapPackageStatus][]string)
	expectedSerialsByTeamAndStatus[0] = map[mobiuss.MDMBootstrapPackageStatus][]string{
		mobiuss.MDMBootstrapPackageInstalled: {noTeamDevices[0].device.SerialNumber, noTeamDevices[1].device.SerialNumber, noTeamDevices[2].device.SerialNumber},
		mobiuss.MDMBootstrapPackageFailed:    {noTeamDevices[3].device.SerialNumber},
		mobiuss.MDMBootstrapPackagePending:   {noTeamDevices[4].device.SerialNumber, noTeamDevices[5].device.SerialNumber, noTeamDevices[6].device.SerialNumber, noTeamDevices[7].device.SerialNumber},
	}
	expectedSerialsByTeamAndStatus[team.ID] = map[mobiuss.MDMBootstrapPackageStatus][]string{
		mobiuss.MDMBootstrapPackageInstalled: {teamDevices[0].device.SerialNumber, teamDevices[1].device.SerialNumber},
		mobiuss.MDMBootstrapPackageFailed:    {teamDevices[2].device.SerialNumber, teamDevices[3].device.SerialNumber, teamDevices[4].device.SerialNumber},
		mobiuss.MDMBootstrapPackagePending:   {teamDevices[5].device.SerialNumber, teamDevices[6].device.SerialNumber},
	}
	...
	winHost, err := s.ds.NewHost(context.Background(), &mobiuss.Host{
		OsqueryHostID: ptr.String("non-macos-host"),
		NodeKey:       ptr.String("non-macos-host"),
		UUID:          uuid.New().String(),
		Hostname:      fmt.Sprintf("%sfoo.local.non.macos", t.Name()),
		Platform:      "windows",
		MDM:           mobiuss.MDMHostData{},
	})
	require.NoError(t, err)

	err = s.ds.SetOrUpdateMDMData(context.Background(), winHost.ID, false, true, s.server.URL+apple_mdm.MDMPath, true, mobiuss.WellKnownMDMMobius, "")
	require.NoError(t, err)

	// create a host that's not enrolled into MDM
	_, err = s.ds.NewHost(context.Background(), &mobiuss.Host{
	...
	s.DoJSON("GET", "/api/latest/mobiuss/bootstrap/summary", nil, http.StatusOK, &summaryResp)
	require.Equal(t, mobiuss.MDMAppleBootstrapPackageSummary{Pending: uint(len(noTeamDevices))}, summaryResp.MDMAppleBootstrapPackageSummary)

	var lhr listHostsResponse
	s.DoJSON("GET", "/api/latest/mobiuss/hosts", nil, http.StatusOK, &lhr, "team_id", "0", "bootstrap_package", "pending")
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/bootstrap/summary?team_id=%d", team.ID), nil, http.StatusOK, &summaryResp)
	require.Equal(t, mobiuss.MDMAppleBootstrapPackageSummary{Pending: uint(len(teamDevices))}, summaryResp.MDMAppleBootstrapPackageSummary)
	...
	enrollAndCheckBootstrapPackage := func(d *deviceWithResponse, bp *mobiuss.MDMAppleBootstrapPackage) {
	...
	checkHostDetails := func(t *testing.T, hostID uint, hostUUID string, expectedStatus mobiuss.MDMBootstrapPackageStatus) {
		var hostResp getHostResponse
		s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/hosts/%d", hostID), nil, http.StatusOK, &hostResp)
		...
		if expectedStatus == mobiuss.MDMBootstrapPackageFailed {
		...
		s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/hosts/identifier/%s", hostUUID), nil, http.StatusOK, &hostByIdentifierResp)
	...
	checkHostAPIs := func(t *testing.T, status mobiuss.MDMBootstrapPackageStatus, teamID *uint) {
		...
		listHostsPath := fmt.Sprintf("/api/latest/mobiuss/hosts?bootstrap_package=%s", status)
		if teamID != nil {
		...
		gotHostsBySerial := make(map[string]mobiuss.HostResponse)
		...
		countPath := fmt.Sprintf("/api/latest/mobiuss/hosts/count?bootstrap_package=%s", status)
		if teamID != nil {
	...
	summaryResp = getMDMAppleBootstrapPackageSummaryResponse{}
	s.DoJSON("GET", "/api/latest/mobiuss/bootstrap/summary", nil, http.StatusOK, &summaryResp)
	require.Equal(t, mobiuss.MDMAppleBootstrapPackageSummary{
		Installed: uint(3),
		Pending:   uint(4),
		Failed:    uint(1),
	}, summaryResp.MDMAppleBootstrapPackageSummary)

	checkHostAPIs(t, mobiuss.MDMBootstrapPackageInstalled, nil)
	checkHostAPIs(t, mobiuss.MDMBootstrapPackagePending, nil)
	checkHostAPIs(t, mobiuss.MDMBootstrapPackageFailed, nil)
	...
	checkHostAPIs(t, mobiuss.MDMBootstrapPackageInstalled, &team.ID)
	checkHostAPIs(t, mobiuss.MDMBootstrapPackagePending, &team.ID)
	checkHostAPIs(t, mobiuss.MDMBootstrapPackageFailed, &team.ID)
}
...
func (s *integrationMDMTestSuite) TestEULA() {
	...
	s.DoJSON("GET", "/api/latest/mobiuss/setup_experience/eula/metadata", nil, http.StatusNotFound, &metadataResp)

	// trying to upload a file that is not a PDF fails
	s.uploadEULA(&mobiuss.MDMEULA{Bytes: []byte("should-fail"), Name: "should-fail.pdf"}, http.StatusBadRequest, "invalid file type")
	// trying to upload an empty file fails
	s.uploadEULA(&mobiuss.MDMEULA{Bytes: []byte{}, Name: "should-fail.pdf"}, http.StatusBadRequest, "invalid file type")

	// admin is able to upload a new EULA
	s.uploadEULA(&mobiuss.MDMEULA{Bytes: pdfBytes, Name: pdfName}, http.StatusOK, "")

	// get EULA metadata
	metadataResp = getMDMEULAMetadataResponse{}
	s.DoJSON("GET", "/api/latest/mobiuss/setup_experience/eula/metadata", nil, http.StatusOK, &metadataResp)
	...
	// download EULA
	resp := s.DoRaw("GET", fmt.Sprintf("/api/latest/mobiuss/setup_experience/eula/%s", eulaToken), nil, http.StatusOK)
	...
	var downloadResp downloadBootstrapPackageResponse
	s.DoJSON("GET", "/api/latest/mobiuss/setup_experience/eula/bad-token", nil, http.StatusNotFound, &downloadResp)

	// trying to upload any EULA without deleting the previous one first results in an error
	s.uploadEULA(&mobiuss.MDMEULA{Bytes: pdfBytes, Name: "should-fail.pdf"}, http.StatusConflict, "")

	// delete EULA
	var deleteResp deleteMDMEULAResponse
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobiuss/setup_experience/eula/%s", eulaToken), nil, http.StatusOK, &deleteResp)
	metadataResp = getMDMEULAMetadataResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/setup_experience/eula/%s", eulaToken), nil, http.StatusNotFound, &metadataResp)
	// trying to delete again is a bad request
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobiuss/setup_experience/eula/%s", eulaToken), nil, http.StatusNotFound, &deleteResp)
}
...
func (s *integrationMDMTestSuite) TestMigrateMDMDeviceWebhook() {
	...
	webhookSrv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/test_mdm_migration":
			var payload mobiuss.MigrateMDMDeviceWebhookPayload
	...
	s.Do("POST", fmt.Sprintf("/api/v1/mobiuss/device/%s/migrate_mdm", "good-token"), nil, http.StatusBadRequest)
	...
	// host is already enrolled to Mobius MDM so migration is not allowed
	require.NoError(t, s.ds.SetOrUpdateMDMData(context.Background(), h.ID, !isServer, enrolled, mdmURL, installedFromDEP, mobiuss.WellKnownMDMMobius, ""))
	s.Do("POST", fmt.Sprintf("/api/v1/mobiuss/device/%s/migrate_mdm", "good-token"), nil, http.StatusBadRequest)
	require.False(t, webhookCalled)
	...
	// simulate that the device is assigned to Mobius in ABM
	s.mockDEPResponse(t.Name(), http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
		switch r.URL.Path {
		case "/session":
		...
		case "/profile/devices":
			...
			resp.Devices = map[string]string{
				prof.Devices[0]: string(mobiuss.DEPAssignProfileResponseSuccess),
			}
			encoder := json.NewEncoder(w)
	...
	s.Do("POST", fmt.Sprintf("/api/v1/mobiuss/device/%s/migrate_mdm", "good-token"), nil, http.StatusNoContent)
	...
	// bad token
	s.Do("POST", fmt.Sprintf("/api/v1/mobiuss/device/%s/migrate_mdm", "bad-token"), nil, http.StatusUnauthorized)
	require.False(t, webhookCalled)
...
func (s *integrationMDMTestSuite) TestMDMMacOSSetup() {
	...
	tm, err := s.ds.NewTeam(context.Background(), &mobiuss.Team{Name: "team1"})
	require.NoError(t, err)
	...
	t.Run("UpdateAppConfig", func(t *testing.T) {
		acResp := appConfigResponse{}
		path := "/api/latest/mobiuss/config"
		...
		for i, c := range enableReleaseDeviceCases {
			t.Run(strconv.Itoa(i), func(t *testing.T) {
				...
				if c.setupAssistant != "" {
					s.Do("POST", "/api/v1/mobiuss/enrollment_profiles/automatic", createMDMAppleSetupAssistantRequest{
	...
	t.Run("UpdateTeamConfig", func(t *testing.T) {
		path := fmt.Sprintf("/api/latest/mobiuss/teams/%d", tm.ID)
		fmtJSON := `{
			"name": %q,
			%s
		}`
			...
			t.Run(strconv.Itoa(i), func(t *testing.T) {
				if c.setupAssistant != "" {
					...
					if uploadSucceeded {
						...
						teamSpecs := applyTeamSpecsRequest{Specs: []*mobiuss.TeamSpec{{
							Name: tm.Name,
							MDM:  mobiuss.TeamSpecMDM{MacOSSetup: macSetup},
						}}}
						s.Do("POST", "/api/latest/mobiuss/spec/teams", teamSpecs, http.StatusOK)
					}
				} else {
					...
					s.Do("PATCH", "/api/latest/mobiuss/setup_experience", json.RawMessage(jsonMustMarshal(t, payload)), expectedPatchStatus)
				}
	...
	t.Run("TestMDMAppleSetupEndpoint", func(t *testing.T) {
		t.Run("TestNoTeam", func(t *testing.T) {
			var acResp appConfigResponse
			s.Do("PATCH", "/api/latest/mobiuss/setup_experience",
				mobiuss.MDMAppleSetupPayload{TeamID: ptr.Uint(0), EnableEndUserAuthentication: ptr.Bool(true)}, http.StatusNoContent)
			acResp = appConfigResponse{}
			s.DoJSON("GET", "/api/latest/mobiuss/config", nil, http.StatusOK, &acResp)
			require.True(t, acResp.MDM.MacOSSetup.EnableEndUserAuthentication)
			lastActivityID := s.lastActivityOfTypeMatches(mobiuss.ActivityTypeEnabledMacosSetupEndUserAuth{}.ActivityName(),
				`{"team_id": null, "team_name": null}`, 0)
			...
			s.DoJSON("GET", "/api/latest/mobiuss/config", nil, http.StatusOK, &acResp)
			require.True(t, acResp.MDM.MacOSSetup.EnableEndUserAuthentication)
			s.lastActivityOfTypeMatches(mobiuss.ActivityTypeEnabledMacosSetupEndUserAuth{}.ActivityName(),
				``, lastActivityID) // no new activity

			s.Do("PATCH", "/api/latest/mobiuss/setup_experience",
				mobiuss.MDMAppleSetupPayload{TeamID: ptr.Uint(0), EnableEndUserAuthentication: ptr.Bool(false)}, http.StatusNoContent)
			acResp = appConfigResponse{}
			s.DoJSON("GET", "/api/latest/mobiuss/config", nil, http.StatusOK, &acResp)
			require.False(t, acResp.MDM.MacOSSetup.EnableEndUserAuthentication)
			require.Greater(t, s.lastActivityOfTypeMatches(mobiuss.ActivityTypeDisabledMacosSetupEndUserAuth{}.ActivityName(),
				`{"team_id": null, "team_name": null}`, 0), lastActivityID)
		})

		t.Run("TestTeam", func(t *testing.T) {
			tmConfigPath := fmt.Sprintf("/api/latest/mobiuss/teams/%d", tm.ID)
			expectedActivityDetail := fmt.Sprintf(`{"team_id": %d, "team_name": %q}`, tm.ID, tm.Name)
			var tmResp teamResponse
			s.Do("PATCH", "/api/latest/mobiuss/setup_experience",
				mobiuss.MDMAppleSetupPayload{TeamID: &tm.ID, EnableEndUserAuthentication: ptr.Bool(true)}, http.StatusNoContent)
			tmResp = teamResponse{}
			...
			s.Do("PATCH", "/api/latest/mobiuss/setup_experience",
				mobiuss.MDMAppleSetupPayload{TeamID: &tm.ID, EnableEndUserAuthentication: ptr.Bool(false)}, http.StatusNoContent)
			tmResp = teamResponse{}
			s.DoJSON("GET", tmConfigPath, nil, http.StatusOK, &tmResp)
	...
	t.Run("ValidateEnableEndUserAuthentication", func(t *testing.T) {
		...
		s.Do("PATCH", "/api/latest/mobiuss/setup_experience",
			mobiuss.MDMAppleSetupPayload{TeamID: ptr.Uint(0), EnableEndUserAuthentication: ptr.Bool(true)}, http.StatusUnprocessableEntity)

		// can't enable end user authentication on team config without a configured IdP already on app config
		var teamResp teamResponse
		s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobiuss/teams/%d", tm.ID), json.RawMessage(fmt.Sprintf(`{
			"name": %q,
			"mdm": {
				"macos_setup": {
					"enable_end_user_authentication": true
		...
		s.Do("PATCH", "/api/latest/mobiuss/setup_experience",
			mobiuss.MDMAppleSetupPayload{TeamID: &tm.ID, EnableEndUserAuthentication: ptr.Bool(true)}, http.StatusUnprocessableEntity)
...
`
	...
	s.DoJSON("GET", "/api/latest/mobiuss/enrollment_profiles/automatic", nil, http.StatusNotFound, &getResp)
	// get for non-existing team returns 404
	s.DoJSON("GET", "/api/latest/mobiuss/enrollment_profiles/automatic", nil, http.StatusNotFound, &getResp, "team_id", "123")

	// Profile name too long
	var createResp createMDMAppleSetupAssistantResponse
	r := s.Do("POST", "/api/latest/mobiuss/enrollment_profiles/automatic", createMDMAppleSetupAssistantRequest{
		TeamID:            nil,
	...
	s.DoJSON("POST", "/api/latest/mobiuss/enrollment_profiles/automatic", createMDMAppleSetupAssistantRequest{
		TeamID:            nil,
		Name:              "no-team",
	...
	s.lastActivityMatches(mobiuss.ActivityTypeChangedMacosSetupAssistant{}.ActivityName(),
		`{"name": "no-team", "team_id": null, "team_name": null}`, 0)

	// create a team and a setup assistant for that team
	tm, err := s.ds.NewTeam(ctx, &mobiuss.Team{
		Name:        t.Name(),
		Description: "desc",
	})
	require.NoError(t, err)
	...
	lastChangedActID := s.lastActivityMatches(mobiuss.ActivityTypeChangedMacosSetupAssistant{}.ActivityName(),
	...
	latestChangedActID := s.lastActivityMatches(mobiuss.ActivityTypeChangedMacosSetupAssistant{}.ActivityName(),
		fmt.Sprintf(`{"name": "team2", "team_id": %d, "team_name": %q}`, tm.ID, tm.Name), 0)
	require.Greater(t, latestChangedActID, lastChangedActID)

	// get no team
	s.DoJSON("GET", "/api/latest/mobiuss/enrollment_profiles/automatic", nil, http.StatusOK, &getResp)
	...
	// get team
	s.DoJSON("GET", "/api/latest/mobiuss/enrollment_profiles/automatic", nil, http.StatusOK, &getResp, "team_id", fmt.Sprint(tm.ID))
	...
	res := s.Do("POST", "/api/latest/mobiuss/enrollment_profiles/automatic", createMDMAppleSetupAssistantRequest{
		TeamID:            &tm.ID,
		Name:              "team5",
		EnrollmentProfile: json.RawMessage(tmProf),
	}, http.StatusUnprocessableEntity)
	...
	s.Do("DELETE", "/api/latest/mobiuss/enrollment_profiles/automatic", nil, http.StatusNoContent)
	latestChangedActID = s.lastActivityMatches(mobiuss.ActivityTypeDeletedMacosSetupAssistant{}.ActivityName(),
		`{"name": "no-team2", "team_id": null, "team_name": null}`, 0)
	...
	s.DoJSON("GET", "/api/latest/mobiuss/enrollment_profiles/automatic", nil, http.StatusNotFound, &getResp, "team_id", fmt.Sprint(tm.ID))
	...
	s.lastActivityMatches(mobiuss.ActivityTypeDeletedMacosSetupAssistant{}.ActivityName(),
		`{"name": "no-team2", "team_id": null, "team_name": null}`, latestChangedActID)

	// create another team and a setup assistant for that team
	tm2, err := s.ds.NewTeam(ctx, &mobiuss.Team{
		Name:        t.Name() + "2",
		Description: "desc2",
	})
	require.NoError(t, err)
	...
	s.Do("DELETE", "/api/latest/mobiuss/enrollment_profiles/automatic", nil, http.StatusNoContent, "team_id", fmt.Sprint(tm2.ID))
	s.lastActivityMatches(mobiuss.ActivityTypeDeletedMacosSetupAssistant{}.ActivityName(),
		fmt.Sprintf(`{"name": "teamB", "team_id": %d, "team_name": %q}`, tm2.ID, tm2.Name), 0)

	// Try with a team that has no relevant ABM tokens
	teamNoABM, err := s.ds.NewTeam(ctx, &mobiuss.Team{
		Name:        t.Name() + "no_abm",
		Description: "no abm",
	})
	require.NoError(t, err)
...
func (s *integrationMDMTestSuite) assertHostAppleConfigProfiles(want map[*mobiuss.Host][mobiusus.HostMDMAppleProfile) {
	t := s.T()
	ds := s.ds
	ctx := context.Background()
...
func (s *integrationMDMTestSuite) assertHostWindowsConfigProfiles(want map[*mobiuss.Host][mobiusus.HostMDMWindowsProfile) {
	t := s.T()
	ds := s.ds
	ctx := context.Background()
...
func (s *integrationMDMTestSuite) assertConfigProfilesByIdentifier(teamID *uint, profileIdent string, exists bool) (profile *mobiuss.MDMAppleConfigProfile) {
	t := s.T()
	if teamID == nil {
		teamID = ptr.Uint(0)
	}
	var cfgProfs []*mobiuss.MDMAppleConfigProfile
	mysql.ExecAdhocSQL(t, s.ds, func(q sqlx.ExtContext) error {
...
func (s *integrationMDMTestSuite) assertWindowsConfigProfilesByName(teamID *uint, profileName string, exists bool) {
	...
	var cfgProfs []*mobiuss.MDMWindowsConfigProfile
	mysql.ExecAdhocSQL(t, s.ds, func(q sqlx.ExtContext) error {
...
// used via s.DoRawWithHeaders to POST /api/_version_/mobiuss/mdm/apple/profiles.
...
func (s *integrationMDMTestSuite) uploadBootstrapPackage(
	pkg *mobiuss.MDMAppleBootstrapPackage,
	expectedStatus int,
	wantErr string,
) {
	t := s.T()
	...
	res := s.DoRawWithHeaders("POST", "/api/latest/mobiuss/bootstrap", b.Bytes(), expectedStatus, headers)
...
func (s *integrationMDMTestSuite) uploadEULA(
	...
	res := s.DoRawWithHeaders("POST", "/api/latest/mobiuss/setup_experience/eula", b.Bytes(), expectedStatus, headers)
...
func (s *integrationMDMTestSuite) TestGitOpsUserActions() {
	...
	t1, err := s.ds.NewTeam(ctx, &mobiuss.Team{
		Name: "Foo",
	})
	require.NoError(t, err)
	t2, err := s.ds.NewTeam(ctx, &mobiuss.Team{
		Name: "Bar",
	})
	require.NoError(t, err)
	t3, err := s.ds.NewTeam(ctx, &mobiuss.Team{
		Name: "Zoo",
	})
	require.NoError(t, err)
	...
	u := &mobiuss.User{
		...
		GlobalRole: ptr.String(mobiuss.RoleGitOps),
	}
	require.NoError(t, u.SetPassword(test.GoodPassword, 10, 10))
	...
	s.Do("POST", "/api/v1/mobiuss/mdm/apple/profiles/batch", batchSetMDMAppleProfilesRequest{Profiles: globalProfiles}, http.StatusNoContent)
	...
	s.Do("POST", "/api/v1/mobiuss/mdm/apple/profiles/batch", batchSetMDMAppleProfilesRequest{
	...
	h1, err := s.ds.NewHost(ctx, &mobiuss.Host{
		NodeKey:  ptr.String(t.Name() + "1"),
		UUID:     t.Name() + "1",
		Hostname: t.Name() + "foo.local",
	})
	require.NoError(t, err)
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/mdm/hosts/%d/profiles", h1.ID), getHostRequest{}, http.StatusForbidden, &getHostResponse{})

	err = s.ds.AddHostsToTeam(ctx, &t1.ID, []uint{h1.ID})
	require.NoError(t, err)
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/mdm/hosts/%d/profiles", h1.ID), getHostRequest{}, http.StatusOK, &getHostResponse{})
}
...
func (s *integrationMDMTestSuite) TestOrgLogo() {
	...
	res := s.DoRawNoAuth("GET", "/api/latest/mobiuss/device/"+token+"/desktop", nil, http.StatusOK)
...
func (s *integrationMDMTestSuite) TestSSO() {
	...
	di, err := mdmtest.EncodeDeviceInfo(mobiuss.MDMAppleMachineInfo{
	...
	configFlows := []string{
		...
		"https://localhost:8080", // app that supports both MDM and Mobius UI SSO
	}
	...
	resp := s.DoRaw("GET", "/api/latest/mobiuss/setup_experience/eula/"+q.Get("eula_token"), nil, http.StatusOK)
	require.EqualValues(t, len(pdfBytes), resp.ContentLength)
	require.Equal(t, "application/pdf", resp.Header.Get("content-type"))
	respBytes, err := io.ReadAll(resp.Body)
	require.NoError(t, err)
	...
	// Enroll generated the TokenUpdate request to Mobius and enqueued the
	...
	var hostResp getHostResponse
	s.DoJSON("GET", "/api/v1/mobiuss/hosts/identifier/"+mdmDevice.UUID, nil, http.StatusOK, &hostResp)
	...
	hostResp = getHostResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/v1/mobiuss/hosts/%d", hostID), nil, http.StatusOK, &hostResp)
	assert.Len(t, hostResp.Host.EndUsers, 1)
	assert.Equal(t, "", hostResp.Host.EndUsers[0].IdpFullName)                     // full name is not recorded for mdm flow until scim sync
	assert.Equal(t, "sso_user@example.com", hostResp.Host.EndUsers[0].IdpUserName) // email is the same as the one used to enroll
	...
	detailQueries := osquery_utils.GetDetailQueries(context.Background(), config.MobiusConfig{}, ac, &ac.Features, osquery_utils.Integrations{})
	...
	err = detailQueries["mdm"].DirectIngestFunc(
		context.Background(),
		kitlog.NewNopLogger(),
		&mobiuss.Host{ID: hostResp.Host.ID},
		s.ds,
		rows,
	)
	require.NoError(t, err)
	...
	checkEndUser = func() {
		...
		require.True(t, ok)
		require.Equal(t, mobiuss.DeviceMappingGoogleChromeProfiles, source)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/v1/mobiuss/hosts/%d/device_mapping", hostResp.Host.ID), nil, http.StatusOK, &dmResp)
	...
	require.True(t, ok)
	require.Equal(t, mobiuss.DeviceMappingMDMIdpAccounts, source)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/v1/mobiuss/hosts?query=%s&device_mapping=true", url.QueryEscape("sso_user@example.com")), nil, http.StatusOK, &hostsResp)
	...
	var dm []mobiuss.HostDeviceMapping
	...
	require.Equal(t, mobiuss.DeviceMappingMDMIdpAccounts, dmResp.DeviceMapping[0].Source)
	...
	require.Equal(t, hostResp.Host.ID, gotHost.ID)
	require.NotNil(t, gotHost.DeviceMapping)
	dm = []mobiuss.HostDeviceMapping{}
	require.NoError(t, json.Unmarshal(*gotHost.DeviceMapping, &dm))
	require.Len(t, dm, 1)
	require.Equal(t, "sso_user@example.com", dm[0].Email)
	require.Equal(t, mobiuss.DeviceMappingMDMIdpAccounts, dm[0].Source)
	...
	res = s.DoRawNoAuth("POST", "/api/v1/mobiuss/mdm/sso/callback?SAMLResponse="+url.QueryEscape(rawSSOResp), nil, http.StatusSeeOther)
	require.NotEmpty(t, res.Header.Get("Location"))
	u, err = url.Parse(res.Header.Get("Location"))
	require.NoError(t, err)
	q = u.Query()
...
func (s *integrationMDMTestSuite) TestSSOWithSCIM() {
	...
	s.DoJSON("POST", "/api/latest/mobiuss/scim/Users", createUserPayload, http.StatusCreated, &createResp)
	...
	s.Do("POST", "/api/v1/mobiuss/mdm/profiles/batch", batchSetMDMProfilesRequest{Profiles: [mobiusus.MDMProfileBatchPayload{
		{Name: "N1", Contents: mobileconfigForTest("N1", "I1", mobiuss.MobiusVarHostEndUserIDPUsername)},
		{Name: "N2", Contents: mobileconfigForTest("N2", "I2", mobiuss.MobiusVarHostEndUserIDPGroups)},
	}}, http.StatusNoContent)
	s.awaitTriggerProfileSchedule(t)
	...
	for _, prof := range *hostResp.Host.MDM.Profiles {
		...
		case "N1": // profile with username
			...
			require.Equal(t, mobiuss.MDMDeliveryVerifying, *prof.Status)
		case "N2": // profile with group
			expectedCount++
			require.NotNil(t, prof.Status)
			require.Equal(t, mobiuss.MDMDeliveryFailed, *prof.Status)
		}
	}
	...
	s.DoJSON("POST", "/api/latest/mobiuss/scim/Groups", createGroup1Payload, http.StatusCreated, &createGroup1Resp)
	...
	for _, prof := range *hostResp.Host.MDM.Profiles {
		...
		case "N2": // profile with group
			expectedCount++
			require.NotNil(t, prof.Status)
			require.Equal(t, mobiuss.MDMDeliveryPending, *prof.Status)
		}
	}
	require.Equal(t, 2, expectedCount)
...
func (s *integrationMDMTestSuite) setUpEndUserAuthentication(t *testing.T, lastSubmittedProfile *godep.Profile) (*mdmtest.TestAppleMDMClient,
	...
	wantSettings := mobiuss.SSOProviderSettings{
...
func (s *integrationMDMTestSuite) downloadAndVerifyOTAEnrollmentProfile(path string) {
	...
	assets, err := s.ds.GetAllMDMConfigAssetsByName(context.Background(), []mobiuss.MDMAssetName{
		mobiuss.MDMAssetCACert,
	}, nil)
	require.NoError(t, err)

	require.True(t, rootCA.AppendCertsFromPEM(assets[mobiuss.MDMAssetCACert].Value))
	...
	require.NoError(t, err)
	require.Contains(t, otaEnrollmentProfile.PayloadContent.URL, s.getConfig().ServerSettings.ServerURL+"/api/v1/mobiuss/ota_enrollment")
}

func (s *integrationMDMTestSuite) downloadAndVerifyEnrollmentProfileManual(t *testing.T) {
	resp := s.DoRaw("GET", "/api/latest/mobiuss/enrollment_profiles/manual", nil, http.StatusOK)
	defer resp.Body.Close()
...
func (s *integrationMDMTestSuite) TestMDMMigration() {
	...
	checkMigrationResponses := func(host *mobiuss.Host, token string) {
		getDesktopResp := mobiusDesktopResponse{}
		res := s.DoRawNoAuth("GET", "/api/latest/mobiuss/device/"+token+"/desktop", nil, http.StatusOK)
		require.NoError(t, json.NewDecoder(res.Body).Decode(&getDesktopResp))
		require.NoError(t, res.Body.Close())
		...
		// simulate that the device is assigned to Mobius in ABM
		profileAssignmentStatusResponse := mobiuss.DEPAssignProfileResponseSuccess
		s.mockDEPResponse(t.Name(), http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			w.WriteHeader(http.StatusOK)
			switch r.URL.Path {
			case "/session":
		...
		profileAssignmentStatusResponse = mobiuss.DEPAssignProfileResponseFailed
		s.runDEPSchedule()
		getDesktopResp = mobiusDesktopResponse{}
		res = s.DoRawNoAuth("GET", "/api/latest/mobiuss/device/"+token+"/desktop", nil, http.StatusOK)
		require.NoError(t, json.NewDecoder(res.Body).Decode(&getDesktopResp))
		...
		profileAssignmentStatusResponse = mobiuss.DEPAssignProfileResponseNotAccessible
		s.runDEPSchedule()
		getDesktopResp = mobiusDesktopResponse{}
		res = s.DoRawNoAuth("GET", "/api/latest/mobiuss/device/"+token+"/desktop", nil, http.StatusOK)
		require.NoError(t, json.NewDecoder(res.Body).Decode(&getDesktopResp))
		...
		// simulate that the device needs to be enrolled in mobiuss, DEP capable
		err = s.ds.SetOrUpdateMDMData(
			ctx,
			host.ID,
			false,
		...
		// simulate that the device is manually enrolled into mobiuss, but DEP capable
		err = s.ds.SetOrUpdateMDMData(
			ctx,
			host.ID,
			false,
		...
		// simulate a host that was reset to factory, but mobiuss still has an mdm record active
		err = s.ds.SetOrUpdateMDMData(
			ctx,
			host.ID,
			false,
	...
	tm, err := s.ds.NewTeam(ctx, &mobiuss.Team{Name: "team-1"})
	require.NoError(t, err)
...
func (s *integrationMDMTestSuite) TestAppConfigWindowsMDM() {
	...
	tm1, err := s.ds.NewTeam(ctx, &mobiuss.Team{Name: t.Name() + "1"})
	require.NoError(t, err)
	tm2, err := s.ds.NewTeam(ctx, &mobiuss.Team{Name: t.Name() + "2"})
	require.NoError(t, err)
	...
	s.lastActivityOfTypeMatches(mobiuss.ActivityTypeEnabledWindowsMDM{}.ActivityName(), `{}`, 0)
	...
	// team 2, Windows workstation already enrolled in Mobius in no team, and a
	...
	metadataHosts := []struct {
		...
		{"windows", "win-third-party", false, &tm2.ID, mobiuss.WellKnownMDMSimpleMDM, false, true},    // is enrolled in 3rd-party
		{"windows", "win-mobiuss", false, nil,mobiusus.WellKnownMDMMobius, false, false},                 // is already Mobius-enrolled
		{"darwin", "macos-no-team", false, nil, "", false, false},                                   // is not Windows
		{"windows", "win-server-third-party", true, nil, mobiuss.WellKnownMDMSimpleMDM, false, false}, // is enrolled in 3rd-party, but is a server
	}
	hostsBySuffix := make(map[string]*mobiuss.Host, len(metadataHosts))
	for _, meta := range metadataHosts {
		var host *mobiuss.Host
		if meta.os == "windows" && meta.enrolledName == mobiuss.WellKnownMDMMobius {
			// special-case to create a properly MDM-enrolled into Mobius host
			...
			require.NoError(t, err)
			err = s.ds.SetOrUpdateMDMData(ctx, host.ID, meta.isServer, true, s.server.URL, false, mobiuss.WellKnownMDMMobius, "")
			require.NoError(t, err)
		} else {
			host = createOrbitEnrolledHost(t, meta.os, meta.suffix, s.ds)
	...
	for _, meta := range metadataHosts {
		...
		s.DoJSON("POST", "/api/mobiuss/orbit/config",
	...
	s.lastActivityMatches(mobiuss.ActivityTypeEnabledWindowsMDMMigration{}.ActivityName(), `{}`, 0)
	...
	assert.False(t, acResp.MDM.WindowsEnabledAndConfigured)
	assert.False(t, acResp.MDM.WindowsMigrationEnabled)
	s.lastActivityOfTypeMatches(mobiuss.ActivityTypeDisabledWindowsMDM{}.ActivityName(), `{}`, 0)
	...
	// get the orbit config for each host, only the mobiuss-enrolled ones get the unenrollment,
	...
	for _, meta := range metadataHosts {
		...
		require.False(t, resp.Notifications.NeedsProgrammaticWindowsMDMEnrollment)
		require.False(t, resp.Notifications.NeedsMDMMigration)
		if meta.enrolledName == mobiuss.WellKnownMDMMobius {
			require.True(t, resp.Notifications.NeedsProgrammaticWindowsMDMUnenrollment)
		} else {
			require.False(t, resp.Notifications.NeedsProgrammaticWindowsMDMUnenrollment)
		}
...
func (s *integrationMDMTestSuite) TestOrbitConfigNudgeSettings() {
 ...
 `))
	...
	var resp orbitGetConfigResponse
	// missing orbit key
	s.DoJSON("POST", "/api/mobiuss/orbit/config", nil, http.StatusUnauthorized, &resp)
	...
	err := s.ds.UpdateHostOperatingSystem(context.Background(), h.ID, mobiuss.OperatingSystem{Platform: "darwin", Version: "12.0"})
	require.NoError(t, err)

	resp = orbitGetConfigResponse{}
	s.DoJSON("POST", "/api/mobiuss/orbit/config", json.RawMessage(fmt.Sprintf(`{"orbit_node_key": %q}`, *h.OrbitNodeKey)), http.StatusOK, &resp)
 ...
 `))
	...
	resp = orbitGetConfigResponse{}
	s.DoJSON("POST", "/api/mobiuss/orbit/config", json.RawMessage(fmt.Sprintf(`{"orbit_node_key": %q}`, *h.OrbitNodeKey)), http.StatusOK, &resp)
	wantCfg, err := mobiuss.NewNudgeConfigmobiusus.AppleOSUpdateSettings{Deadline: optjson.SetString("2022-01-04"), MinimumVersion: optjson.SetString("12.1.3")})
	require.NoError(t, err)
	...
	require.NoError(t, err)
	s.assertMacOSDeclarationsByName(&team.ID, servermdm.MobiusMacOSUpdatesProfileName, false)
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobiuss/teams/%d", team.ID),mobiusus.TeamPayload{
		MDM: &mobiuss.TeamPayloadMDM{
			MacOSUpdates: &mobiuss.AppleOSUpdateSettings{
				Deadline:       optjson.SetString("1992-01-01"),
				MinimumVersion: optjson.SetString("13.1.1"),
			},
		},
	}, http.StatusOK, &tmResp)
	s.assertMacOSDeclarationsByName(&team.ID, servermdm.MobiusMacOSUpdatesProfileName, true)

	resp = orbitGetConfigResponse{}
	s.DoJSON("POST", "/api/mobiuss/orbit/config", json.RawMessage(fmt.Sprintf(`{"orbit_node_key": %q}`, *h.OrbitNodeKey)), http.StatusOK, &resp)
	wantCfg, err = mobiuss.NewNudgeConfigmobiusus.AppleOSUpdateSettings{Deadline: optjson.SetString("1992-01-01"), MinimumVersion: optjson.SetString("13.1.1")})
	require.NoError(t, err)
	require.Equal(t, wantCfg, resp.NudgeConfig)
	...
	err = s.ds.UpdateHostOperatingSystem(context.Background(), h2.ID, mobiuss.OperatingSystem{Platform: "darwin", Version: "12.0"})
	require.NoError(t, err)

	resp = orbitGetConfigResponse{}
	s.DoJSON("POST", "/api/mobiuss/orbit/config", json.RawMessage(fmt.Sprintf(`{"orbit_node_key": %q}`, *h2.OrbitNodeKey)), http.StatusOK, &resp)
	wantCfg, err = mobiuss.NewNudgeConfigmobiusus.AppleOSUpdateSettings{Deadline: optjson.SetString("2022-01-04"), MinimumVersion: optjson.SetString("12.1.3")})
	require.NoError(t, err)
	require.Equal(t, wantCfg, resp.NudgeConfig)
	require.Equal(t, wantCfg.OSVersionRequirements[0].RequiredInstallationDate.String(), "2022-01-04 20:00:00 +0000 UTC")
	...
	err = s.ds.UpdateHostOperatingSystem(context.Background(), h3.ID, mobiuss.OperatingSystem{Platform: "darwin", Version: "14.1"})
	require.NoError(t, err)

	resp = orbitGetConfigResponse{}
	s.DoJSON("POST", "/api/mobiuss/orbit/config", json.RawMessage(fmt.Sprintf(`{"orbit_node_key": %q}`, *h3.OrbitNodeKey)), http.StatusOK, &resp)
	...
	resp = orbitGetConfigResponse{}
	s.DoJSON("POST", "/api/mobiuss/orbit/config", json.RawMessage(fmt.Sprintf(`{"orbit_node_key": %q}`, *h4.OrbitNodeKey)), http.StatusOK, &resp)
	require.Nil(t, resp.NudgeConfig)
}
...
func (s *integrationMDMTestSuite) TestValidGetPoliciesRequestWithDeviceToken() {
	...
	encodedBinToken, err := mobiuss.GetEncodedBinarySecurityTokenmobiusus.WindowsMDMProgrammaticEnrollmentType, *windowsHost.OrbitNodeKey)
	require.NoError(t, err)
...
func (s *integrationMDMTestSuite) TestGetPoliciesRequestWithInvalidUUID() {
	...
	// Preparing the GetPolicies Request message
	encodedBinToken, err := mobiuss.GetEncodedBinarySecurityTokenmobiusus.WindowsMDMProgrammaticEnrollmentType, "not_exists")
	require.NoError(t, err)
...
func (s *integrationMDMTestSuite) TestGetPoliciesRequestWithNotElegibleHost() {
	...
	// Preparing the GetPolicies Request message
	encodedBinToken, err := mobiuss.GetEncodedBinarySecurityTokenmobiusus.WindowsMDMProgrammaticEnrollmentType, *linuxHost.OrbitNodeKey)
	require.NoError(t, err)
...
func (s *integrationMDMTestSuite) TestValidRequestSecurityTokenRequestWithDeviceToken() {
	...
	s.lastActivityOfTypeMatches(
		mobiuss.ActivityTypeMDMEnrolled{}.ActivityName(),
		fmt.Sprintf(`{
...
func (s *integrationMDMTestSuite) TestValidGetTOC() {
	...
	defer func() {
		if panicVal := recover(); panicVal != nil {
			...
			if ok && strings.Contains(s, "Assets may not be used when running Mobius as a library") {
...
func (s *integrationMDMTestSuite) TestWindowsMDM() {
	...
	commandOne := &mobiuss.MDMWindowsCommand{
	...
	require.Equal(t, receivedCmd.Verb, mobiuss.CmdExec)
	...
	d.AppendResponse(mobiuss.SyncMLCmd{
		XMLName: xml.Name{Local: mobiuss.CmdStatus},
		MsgRef:  &msgID,
		CmdRef:  &cmdOneUUID,
		Cmd:     ptr.String("Exec"),
		Data:    ptr.String("200"),
		Items:   nil,
		CmdID:   mobiuss.CmdID{Value: uuid.NewString()},
	})
	...
	commandTwo := &mobiuss.MDMWindowsCommand{
	...
	commandThree := &mobiuss.MDMWindowsCommand{
	...
	commandFour := &mobiuss.MDMWindowsCommand{
	...
	require.Equal(t, receivedCmdTwo.Verb, mobiuss.CmdGet)
	...
	require.Equal(t, receivedCmdThree.Verb, mobiuss.CmdReplace)
	...
	require.Equal(t, receivedCmdFour.Verb, mobiuss.CmdAdd)
	...
	d.AppendResponse(mobiuss.SyncMLCmd{
		XMLName: xml.Name{Local: mobiuss.CmdResults},
		MsgRef:  &msgID,
		CmdRef:  &cmdTwoUUID,
		Cmd:     ptr.String("Replace"),
		Data:    ptr.String("200"),
		Items: []mobiuss.CmdItem{
			{
				Source: ptr.String("./Device/Vendor/MSFT/DMClient/Provider/DEMO%20MDM/SignedEntDMID"),
				Data:   &mobiuss.RawXmlData{Content: "0"},
			},
		},
		CmdID: mobiuss.CmdID{Value: cmdTwoRespUUID},
	})
	...
	var getMDMCmdResp getMDMCommandResultsResponse
	s.DoJSON("GET", "/api/latest/mobiuss/commands/results", nil, http.StatusOK, &getMDMCmdResp, "command_uuid", cmdOneUUID)
	require.Len(t, getMDMCmdResp.Results, 1)
	require.NotZero(t, getMDMCmdResp.Results[0].UpdatedAt)
	getMDMCmdResp.Results[0].UpdatedAt = time.Time{}
	require.Equal(t, &mobiuss.MDMCommandResult{
	...
	s.DoJSON("GET", "/api/latest/mobiuss/commands/results", nil, http.StatusOK, &getMDMCmdResp, "command_uuid", cmdTwoUUID)
	require.Len(t, getMDMCmdResp.Results, 1)
	require.NotZero(t, getMDMCmdResp.Results[0].UpdatedAt)
	getMDMCmdResp.Results[0].UpdatedAt = time.Time{}
	require.Equal(t, &mobiuss.MDMCommandResult{
	...
	s.DoJSON("GET", "/api/latest/mobiuss/commands/results", nil, http.StatusOK, &getMDMCmdResp, "command_uuid", cmdThreeUUID)
	require.Len(t, getMDMCmdResp.Results, 1)
	require.NotZero(t, getMDMCmdResp.Results[0].UpdatedAt)
	getMDMCmdResp.Results[0].UpdatedAt = time.Time{}
	require.Equal(t, &mobiuss.MDMCommandResult{
	...
	s.DoJSON("GET", "/api/latest/mobiuss/commands/results", nil, http.StatusOK, &getMDMCmdResp, "command_uuid", cmdFourUUID)
	require.Len(t, getMDMCmdResp.Results, 1)
	require.NotZero(t, getMDMCmdResp.Results[0].UpdatedAt)
	getMDMCmdResp.Results[0].UpdatedAt = time.Time{}
	require.Equal(t, &mobiuss.MDMCommandResult{
...
func (s *integrationMDMTestSuite) TestWindowsMDMCommandWithSecret() {
	...
	assert.Contains(t, string(getMDMCmdResp.Results[0].Payload), "$MOBIUS_SECRET_DATA")
}
...
func (s *integrationMDMTestSuite) TestWindowsAutomaticEnrollmentCommands() {
	...
	err := s.ds.ApplyEnrollSecrets(ctx, nil, []*mobiuss.EnrollSecret{{Secret: t.Name()}})
	require.NoError(t, err)
	...
	checkinAndAck := func(expectMobiusdCmds bool) {
		cmds, err := d.StartManagementSession()
		require.NoError(t, err)

		if !expectMobiusdCmds {
		...
		// 2 status + 2 commands to install mobiusdaemon
		require.Len(t, cmds, 4)
		var mobiusdAddCmd, mobiusdExecCmd mobiuss.ProtoCmdOperation
		for _, c := range cmds {
			switch c.Verb {
			case "Add":
				mobiusdAddCmd = c
		...
		require.Equal(t, syncml.MobiusdWindowsInstallerGUID, mobiusdAddCmd.Cmd.GetTargetURI())
		require.Equal(t, syncml.MobiusdWindowsInstallerGUID, mobiusdExecCmd.Cmd.GetTargetURI())
		require.Len(t, mobiusdExecCmd.Cmd.Items, 1)
		...
		err = xml.Unmarshal([]byte(mobiusdExecCmd.Cmd.Items[0].Data.Content), &installJob)
		require.NoError(t, err)
		require.Equal(t, s.mockedDownloadMobiusdmMeta.MSIURL, installJob.Product.ContentURL)
		require.Equal(t, s.mockedDownloadMobiusdmMeta.MSISha256, installJob.Product.FileHash)
		...
		d.AppendResponse(mobiuss.SyncMLCmd{
			XMLName: xml.Name{Local: mobiuss.CmdStatus},
			MsgRef:  &msgID,
			CmdRef:  &mobiusdAddCmd.Cmd.CmdID.Value,
			Cmd:     &mobiusdAddCmd.Verb,
			Data:    ptr.String("200"),
			Items:   nil,
			CmdID:   mobiuss.CmdID{Value: uuid.NewString()},
		})
		d.AppendResponse(mobiuss.SyncMLCmd{
			XMLName: xml.Name{Local: mobiuss.CmdStatus},
			MsgRef:  &msgID,
			CmdRef:  &mobiusdExecCmd.Cmd.CmdID.Value,
			Cmd:     &mobiusdExecCmd.Verb,
			Data:    ptr.String("200"),
			Items:   nil,
			CmdID:   mobiuss.CmdID{Value: uuid.NewString()},
		})
	...
	// start a management session, will receive the install mobiusdaemon commands
	checkinAndAck(true)

	// start a new management session again, Mobiusd is not reported as installed
	// so it receives the commands again
	checkinAndAck(true)

	// simulate mobiusdaemon installed and enrolled
	host := createOrbitEnrolledHost(t, "windows", "h1", s.ds)
	...
	// start a new management session again, Mobiusd is reported as installed so
...
func (s *integrationMDMTestSuite) TestValidManagementUnenrollRequest() {
	...
	_, err = s.ds.MDMWindowsGetEnrolledDeviceWithDeviceID(context.Background(), deviceID)
	require.True(t, mobiuss.IsNotFound(err))
}
...
`
	...
	// no host provided
	s.DoJSON("POST", "/api/latest/mobiuss/commands/run", &runMDMCommandRequest{
	...
	res := s.Do("POST", "/api/latest/mobiuss/commands/run", &runMDMCommandRequest{
		Command:   base64.StdEncoding.EncodeToString([]byte(macRawCmd)),
...
func (s *integrationMDMTestSuite) TestUpdateMDMWindowsEnrollmentsHostUUID() {
	...
	// simulate device that is MDM enrolled before mobiusdaemon is installed
	d := mobiuss.MDMWindowsEnrolledDevice{
		MDMDeviceID:            "test-device-id",
		MDMHardwareID:          "test-hardware-id",
		MDMDeviceState:         "ds",
		MDMDeviceType:          "dt",
...
func (s *integrationMDMTestSuite) TestBitLockerEnforcementNotifications() {
	...
	checkNotification := func(want bool) {
		resp := orbitGetConfigResponse{}
		s.DoJSON("POST", "/api/mobiuss/orbit/config", json.RawMessage(fmt.Sprintf(`{"orbit_node_key": %q}`, *windowsHost.OrbitNodeKey)), http.StatusOK, &resp)
	...
	// enroll the host into Mobius MDM
	encodedBinToken, err := mobiuss.GetEncodedBinarySecurityTokenmobiusus.WindowsMDMProgrammaticEnrollmentType, *windowsHost.OrbitNodeKey)
	require.NoError(t, err)
	requestBytes, err := s.newSecurityTokenMsg(encodedBinToken, true, false)
	require.NoError(t, err)
	...
	require.NoError(t, s.ds.SetOrUpdateMDMData(context.Background(), windowsHost.ID, false, true, "https://example.com", true, mobiuss.WellKnownMDMMobius, ""))
	...
	acResp := appConfigResponse{}
	s.DoJSON("PATCH", "/api/latest/mobiuss/config", json.RawMessage(`{ "mdm": { "macos_settings": { "enable_disk_encryption": true } } }`), http.StatusOK, &acResp)
	assert.True(t, acResp.MDM.EnableDiskEncryption.Value)
	...
	// host has disk encryption on, the key is not decryptable by mobiuss. Gets the notification
	err = s.ds.SetHostsDiskEncryptionKeyStatus(ctx, []uint{windowsHost.ID}, false, time.Now())
	require.NoError(t, err)
	checkNotification(true)

	// host has disk encryption on, the disk was encrypted by mobiuss. Doesn't get the notification
...
func (s *integrationMDMTestSuite) TestHostDiskEncryptionKey() {
	...
	acResp := appConfigResponse{}
	s.DoJSON("PATCH", "/api/latest/mobiuss/config", json.RawMessage(`{ "mdm": { "enable_disk_encryption": true } }`), http.StatusOK, &acResp)
	assert.True(t, acResp.AppConfig.MDM.EnableDiskEncryption.Value)

	// try to call the endpoint while the host is not MDM-enrolled
	res := s.Do("POST", "/api/mobiuss/orbit/disk_encryption_key", orbitPostDiskEncryptionKeyRequest{
		OrbitNodeKey:  *host.OrbitNodeKey,
		EncryptionKey: []byte("WILL-FAIL"),
	}, http.StatusBadRequest)
	msg := extractServerErrorText(res.Body)
	require.Contains(t, msg, "host is not enrolled with mobiuss")

	// enroll it in mobiuss
	mdmDevice := mdmtest.NewTestMDMClientWindowsProgramatic(s.server.URL, *host.OrbitNodeKey)
	err := mdmDevice.Enroll()
	require.NoError(t, err)
	err = s.ds.SetOrUpdateMDMData(ctx, host.ID, false, true, s.server.URL, false, mobiuss.WellKnownMDMMobius, "")
	...
	s.Do("POST", "/api/mobiuss/orbit/disk_encryption_key", orbitPostDiskEncryptionKeyRequest{
		OrbitNodeKey:  *host.OrbitNodeKey,
	...
	var hostResp getHostResponse
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/hosts/%d", host.ID), nil, http.StatusOK, &hostResp)
	...
	require.Equal(t, mobiuss.DiskEncryptionEnforcing, *hostResp.Host.MDM.OSSettings.DiskEncryption.Status) // still pending because disk encryption status is not set
	...
	wstepCert, _, _, err := s.mobiusCfg.MDM.MicrosoftWSTEP()
	require.NoError(t, err)
	...
	require.NotNil(t, hostResp.Host.MDM.OSSettings)
	require.NotNil(t, hostResp.Host.MDM.OSSettings.DiskEncryption.Status)
	require.Equal(t, mobiuss.DiskEncryptionFailed, *hostResp.Host.MDM.OSSettings.DiskEncryption.Status)
	...
	require.NotNil(t, hostResp.Host.MDM.OSSettings)
	require.NotNil(t, hostResp.Host.MDM.OSSettings.DiskEncryption.Status)
	require.Equal(t, mobiuss.DiskEncryptionVerified, *hostResp.Host.MDM.OSSettings.DiskEncryption.Status)
	require.Equal(t, "", hostResp.Host.MDM.OSSettings.DiskEncryption.Detail)
}
...
func (s *integrationMDMTestSuite) TestMDMEnabledAndConfigured() {
	...
	compareMacOSSetupValues := (func(t *testing.T, got mobiuss.MacOSSetup, wantmobiusus.MacOSSetup) {
	...
	insertBootstrapPackageAndSetupAssistant := func(t *testing.T, teamID *uint) {
		...
		require.NoError(t, s.ds.InsertMDMAppleBootstrapPackage(ctx, &mobiuss.MDMAppleBootstrapPackage{
			TeamID: tmID,
			Name:   "foo",
		...
		_, err := s.ds.SetOrUpdateMDMAppleSetupAssistant(ctx, &mobiuss.MDMAppleSetupAssistant{
			TeamID:  teamID,
			Name:    "bar",
			Profile: []byte("{}"),
		})
	...
	t.Run("apply app config spec", func(t *testing.T) {
		t.Run("disk encryption", func(t *testing.T) {
			...
			s.DoJSON("PATCH", "/api/latest/mobiuss/config", ac, http.StatusOK, &acResp)
		...
		t.Run("macos setup", func(t *testing.T) {
			...
			acResp := checkAppConfig(t, true, true)
			compareMacOSSetupValues(t, mobiuss.MacOSSetup{}, acResp.AppConfig.MDM.MacOSSetup) // disabled by default

			// initialize our test app config
			ac := appConfig.Copy()
			ac.AgentOptions = nil
			ac.MDM.EndUserAuthentication = mobiuss.MDMEndUserAuthentication{
				SSOProviderSettings: mobiuss.SSOProviderSettings{
			...
			ac.MDM.MacOSSetup = mobiuss.MacOSSetup{
			...
			s.DoJSON("PATCH", "/api/latest/mobiuss/config", ac, http.StatusUnprocessableEntity, &acResp)
		...
		t.Run("custom settings", func(t *testing.T) {
			...
			ac := appConfig.Copy()
			ac.AgentOptions = nil
			ac.MDM.MacOSSettings.CustomSettings = []mobiuss.MDMProfileSpec{}
			ac.MDM.WindowsSettings.CustomSettings = optjson.SetSlice([]mobiuss.MDMProfileSpec{})
			require.NoError(t, s.ds.SaveAppConfig(ctx, ac))
			acResp := checkAppConfig(t, true, true)
			...
			// add custom settings
			ac.MDM.MacOSSettings.CustomSettings = []mobiuss.MDMProfileSpec{{Path: "foo"}, {Path: "bar"}}
			ac.MDM.WindowsSettings.CustomSettings = optjson.SetSlice([]mobiuss.MDMProfileSpec{{Path: "baz"}, {Path: "zab"}})
			s.DoJSON("PATCH", "/api/latest/mobiuss/config", ac, http.StatusOK, &acResp)
			acResp = checkAppConfig(t, true, true)                                                                                 // both mac and windows mdm enabled
			...
			ac.MDM.MacOSSettings.CustomSettings = []mobiuss.MDMProfileSpec{{Path: "oof"}, {Path: "bar"}}
			ac.MDM.WindowsSettings.CustomSettings = optjson.SetSlice([]mobiuss.MDMProfileSpec{{Path: "foo"}, {Path: "zab"}})
			s.DoJSON("PATCH", "/api/latest/mobiuss/config", ac, http.StatusUnprocessableEntity, &acResp)
			acResp = checkAppConfig(t, false, false) // both disabled
	...
	t.Run("modify team", func(t *testing.T) {
		...
		checkTeam := func(t *testing.T, team *mobiuss.Team, checkMDM mobiusus.TeamPayloadMDM) teamResponse {
			var wantDiskEncryption bool
			var wantMacOSSetup mobiuss.MacOSSetup
			if checkMDM != nil {
			...
			s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/teams/%d", team.ID), nil, http.StatusOK, &resp)
		...
		var createTeamResp teamResponse
		s.DoJSON("POST", "/api/latest/mobiuss/teams", createTeamRequestmobiusus.TeamPayload{
			Name: ptr.String("Ninjas"),
			MDM:  &mobiuss.TeamPayloadMDM{EnableDiskEncryption: optjson.SetBool(true)}, // mdm is ignored by the create team endpoint
	...
	t.Run("edit team spec", func(t *testing.T) {
		...
		checkTeam := func(t *testing.T, team *mobiuss.Team, checkMDM mobiusus.TeamSpecMDM) teamResponse {
		...
		tmSpecReq := applyTeamSpecsRequest{Specs: []*mobiuss.TeamSpec{{Name: "Pirates"}}}
		var tmSpecResp applyTeamSpecsResponse
		s.DoJSON("POST", "/api/latest/mobiuss/spec/teams", tmSpecReq, http.StatusOK, &tmSpecResp)
		teamID, ok := tmSpecResp.TeamIDsByName["Pirates"]
		require.True(t, ok)
		team := mobiuss.Team{ID: teamID, Name: "Pirates"}
		...
		for _, c := range cases {
			...
			t.Run(c.name, func(t *testing.T) {
				...
				tmSpecReq = applyTeamSpecsRequest{Specs: []*mobiuss.TeamSpec{{
					Name: team.Name,
					MDM:  *c.mdm,
				}}}
				s.DoJSON("POST", "/api/latest/mobiuss/spec/teams", tmSpecReq, c.expectedStatus, &tmSpecResp)
...
func (s *integrationMDMTestSuite) checkMDMProfilesSummaries(t *testing.T, teamID *uint, expectedSummary mobiuss.MDMProfilesSummary, expectedAppleSummary mobiusus.MDMProfilesSummary) {
	var queryParams []string
	if teamID != nil {
	...
	if expectedAppleSummary != nil {
		...
		s.DoJSON("GET", "/api/v1/mobiuss/mdm/apple/profiles/summary", getMDMAppleProfilesSummaryRequest{}, http.StatusOK, &apple, queryParams...)
	...
	s.DoJSON("GET", "/api/v1/mobiuss/configuration_profiles/summary", getMDMProfilesSummaryRequest{}, http.StatusOK, &combined, queryParams...)
...
func (s *integrationMDMTestSuite) checkMDMDiskEncryptionSummaries(t *testing.T, teamID *uint, expectedSummary mobiuss.MDMDiskEncryptionSummary, checkFileVaultSummary bool) {
	var queryParams []string
	if teamID != nil {
		queryParams = append(queryParams, "team_id", fmt.Sprintf("%d", *teamID))
	}
	...
	if checkFileVaultSummary {
		...
		s.DoJSON("GET", "/api/v1/mobiuss/mdm/apple/filevault/summary", getMDMProfilesSummaryRequest{}, http.StatusOK, &fileVault, queryParams...)
	...
	s.DoJSON("GET", "/api/v1/mobiuss/disk_encryption", getMDMProfilesSummaryRequest{}, http.StatusOK, &combined, queryParams...)
...
func (s *integrationMDMTestSuite) TestWindowsFreshEnrollEmptyQuery() {
	...
	s.Do(
		"POST",
		"/api/v1/mobiuss/mdm/profiles/batch",
		batchSetMDMProfilesRequest{Profiles: []mobiuss.MDMProfileBatchPayload{}},
		http.StatusNoContent,
	)
	...
	require.NotContains(t, dqResp.Queries, "mobius_detail_query_mdm_config_profiles_windows")
	...
	s.DoJSON("POST", "/api/osquery/distributed/read", req, http.StatusOK, &dqResp)
	require.Contains(t, dqResp.Queries, "mobius_detail_query_mdm_config_profiles_windows")
	require.NotEmpty(t, dqResp.Queries, "mobius_detail_query_mdm_config_profiles_windows")
}

func (s *integrationMDMTestSuite) TestManualEnrollmentCommands() {
	t := s.T()

	checkInstallMobiusdCommandSent := func(mdmDevice *mdmtest.TestAppleMDMClient, wantCommand bool) {
		foundInstallMobiusdCommand := false
		cmd, err := mdmDevice.Idle()
		require.NoError(t, err)
		for cmd != nil {
			var fullCmd micromdm.CommandPayload
			...
			if manifest := fullCmd.Command.InstallEnterpriseApplication.ManifestURL; manifest != nil {
				foundInstallMobiusdCommand = true
				require.Equal(t, "InstallEnterpriseApplication", cmd.Command.RequestType)
				require.Contains(t, *fullCmd.Command.InstallEnterpriseApplication.ManifestURL, mobiusdbase.GetPKGManifestURL())
			}
			cmd, err = mdmDevice.Acknowledge(cmd.CommandUUID)
			require.NoError(t, err)
		}
		require.Equal(t, wantCommand, foundInstallMobiusdCommand)
	}

	// create a device that's not enrolled into Mobius, it should get a command to
	// install mobiusdaemon
	mdmDevice := mdmtest.NewTestMDMClientAppleDirect(mdmtest.AppleEnrollInfo{
		SCEPChallenge: s.scepChallenge,
		SCEPURL:       s.server.URL + apple_mdm.SCEPPath,
		MDMURL:        s.server.URL + apple_mdm.MDMPath,
	...
	require.NoError(t, err)
	s.runWorker()
	checkInstallMobiusdCommandSent(mdmDevice, true)

	// create a device that's enrolled into Mobius before turning on MDM features,
	// it should still get the command to install mobiusdaemon if turns on MDM.
	desktopToken := uuid.New().String()
...
func (s *integrationMDMTestSuite) TestLockUnlockWipeWindowsLinux() {
	...
	err := s.ds.SetOrUpdateMDMData(ctx, winHost.ID, false, true, s.server.URL, false, mobiuss.WellKnownMDMMobius, "")
	require.NoError(t, err)
	linuxHost := createOrbitEnrolledHost(t, "linux", "lock_unlock_linux", s.ds)

	for _, host := range []*mobiuss.Host{winHost, linuxHost} {
		t.Run(host.MobiusPlatform(), func(t *testing.T) {
			...
			s.DoJSON("POST", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/unlock", host.ID), nil, http.StatusConflict, &unlockResp)

			// lock the host
			var lockHostResp lockHostResponse
			s.DoJSON("POST", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/lock", host.ID), nil, http.StatusOK, &lockHostResp)
			require.Equal(t, mobiuss.PendingActionLock, lockHostResp.PendingAction)
			require.Equal(t, mobiuss.DeviceStatusUnlocked, lockHostResp.DeviceStatus)

			// refresh the host's status, it is now pending lock
			s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/hosts/%d", host.ID), nil, http.StatusOK, &getHostResp)
			require.NotNil(t, getHostResp.Host.MDM.DeviceStatus)
			require.Equal(t, string(mobiuss.DeviceStatusUnlocked), *getHostResp.Host.MDM.DeviceStatus)
			require.NotNil(t, getHostResp.Host.MDM.PendingAction)
			require.Equal(t, string(mobiuss.PendingActionLock), *getHostResp.Host.MDM.PendingAction)

			// try locking the host while it is pending lock fails
			res := s.DoRaw("POST", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/lock", host.ID), nil, http.StatusUnprocessableEntity)
			errMsg := extractServerErrorText(res.Body)
			...
			s.DoJSON("POST", "/api/mobiuss/orbit/scripts/result",
			...
			s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/hosts/%d", host.ID), nil, http.StatusOK, &getHostResp)
			require.NotNil(t, getHostResp.Host.MDM.DeviceStatus)
			require.Equal(t, string(mobiuss.DeviceStatusLocked), *getHostResp.Host.MDM.DeviceStatus)
			require.NotNil(t, getHostResp.Host.MDM.PendingAction)
			require.Equal(t, string(mobiuss.PendingActionNone), *getHostResp.Host.MDM.PendingAction)

			// try to lock the host again
			s.Do("POST", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/lock", host.ID), nil, http.StatusConflict)
			// try to wipe a locked host
			res = s.Do("POST", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/wipe", host.ID), nil, http.StatusUnprocessableEntity)
			errMsg = extractServerErrorText(res.Body)
			require.Contains(t, errMsg, "Host cannot be wiped until it is unlocked.")

			// unlock the host
			s.DoJSON("POST", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/unlock", host.ID), nil, http.StatusOK, &unlockResp)
			require.Equal(t, mobiuss.PendingActionUnlock, unlockResp.PendingAction)
			require.Equal(t, mobiuss.DeviceStatusLocked, unlockResp.DeviceStatus)
			...
			require.Equal(t, string(mobiuss.DeviceStatusLocked), *getHostResp.Host.MDM.DeviceStatus)
			require.NotNil(t, getHostResp.Host.MDM.PendingAction)
			require.Equal(t, string(mobiuss.PendingActionUnlock), *getHostResp.Host.MDM.PendingAction)

			// try unlocking the host while it is pending unlock fails
			res = s.DoRaw("POST", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/unlock", host.ID), nil, http.StatusUnprocessableEntity)
			errMsg = extractServerErrorText(res.Body)
			...
			s.Do("POST", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/unlock", host.ID), nil, http.StatusOK)
			status, err = s.ds.GetHostLockWipeStatus(ctx, host)
			require.NoError(t, err)
			s.DoJSON("POST", "/api/mobiuss/orbit/scripts/result",
			...
			s.DoJSON("POST", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/wipe", host.ID), nil, http.StatusOK, &wipeResp)
			require.Equal(t, mobiuss.PendingActionWipe, wipeResp.PendingAction)
			require.Equal(t, mobiuss.DeviceStatusUnlocked, wipeResp.DeviceStatus)
			wipeActID := s.lastActivityOfTypeMatches(mobiuss.ActivityTypeWipedHost{}.ActivityName(), fmt.Sprintf(`{"host_id": %d, "host_display_name": %q}`, host.ID, host.DisplayName()), 0)

			// try to wipe the host again, already have it pending
			res = s.DoRaw("POST", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/wipe", host.ID), nil, http.StatusUnprocessableEntity)
			errMsg = extractServerErrorText(res.Body)
			require.Contains(t, errMsg, "Host has pending wipe request.")
			// no activity created
			s.lastActivityOfTypeMatches(mobiuss.ActivityTypeWipedHost{}.ActivityName(), fmt.Sprintf(`{"host_id": %d, "host_display_name": %q}`, host.ID, host.DisplayName()), wipeActID)
			...
			require.Equal(t, string(mobiuss.DeviceStatusUnlocked), *getHostResp.Host.MDM.DeviceStatus)
			require.NotNil(t, getHostResp.Host.MDM.PendingAction)
			require.Equal(t, string(mobiuss.PendingActionWipe), *getHostResp.Host.MDM.PendingAction)

			status, err = s.ds.GetHostLockWipeStatus(ctx, host)
			require.NoError(t, err)
			if host.MobiusPlatform() == "linux" {
				...
				require.Equal(t, wipeCmd.Verb, mobiuss.CmdExec)
				...
				winMDMClient.AppendResponse(mobiuss.SyncMLCmd{
					XMLName: xml.Name{Local: mobiuss.CmdStatus},
					MsgRef:  &msgID,
			...
			s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/hosts/%d", host.ID), nil, http.StatusOK, &getHostResp)
			require.NotNil(t, getHostResp.Host.MDM.DeviceStatus)
			require.Equal(t, string(mobiuss.DeviceStatusWiped), *getHostResp.Host.MDM.DeviceStatus)
			require.NotNil(t, getHostResp.Host.MDM.PendingAction)
			require.Equal(t, string(mobiuss.PendingActionNone), *getHostResp.Host.MDM.PendingAction)

			// try to lock/unlock the host fails
			res = s.Do("POST", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/lock", host.ID), nil, http.StatusUnprocessableEntity)
			errMsg = extractServerErrorText(res.Body)
			require.Contains(t, errMsg, "Cannot process lock requests once host is wiped.")
			res = s.Do("POST", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/unlock", host.ID), nil, http.StatusUnprocessableEntity)
			errMsg = extractServerErrorText(res.Body)
			...
			s.Do("POST", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/wipe", host.ID), nil, http.StatusConflict)
			// no activity created
			s.lastActivityOfTypeMatches(mobiuss.ActivityTypeWipedHost{}.ActivityName(), fmt.Sprintf(`{"host_id": %d, "host_display_name": %q}`, host.ID, host.DisplayName()), wipeActID)

			// re-enroll the host, simulating that another user received the wiped host
			newOrbitKey := uuid.New().String()
			newHost, err := s.ds.EnrollOrbit(ctx, true, mobiuss.OrbitHostInfo{
...
func (s *integrationMDMTestSuite) TestLockUnlockWipeMacOS() {
	...
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/lock", host.ID), nil, http.StatusOK, &lockResp, "view_pin", "true")
	assert.Len(t, lockResp.UnlockPIN, 6)
	require.Equal(t, mobiuss.PendingActionLock, lockResp.PendingAction)
	require.Equal(t, mobiuss.DeviceStatusUnlocked, lockResp.DeviceStatus)
	...
	unlockActID := s.lastActivityOfTypeMatches(mobiuss.ActivityTypeUnlockedHost{}.ActivityName(),
		fmt.Sprintf(`{"host_id": %d, "host_display_name": %q, "host_platform": %q}`, host.ID, host.DisplayName(), host.MobiusPlatform()), 0)
	...
	newUnlockActID := s.lastActivityOfTypeMatches(mobiuss.ActivityTypeUnlockedHost{}.ActivityName(),
		fmt.Sprintf(`{"host_id": %d, "host_display_name": %q, "host_platform": %q}`, host.ID, host.DisplayName(), host.MobiusPlatform()), 0)
	...
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/lock", host.ID), nil, http.StatusOK, &lockResp)
	require.Equal(t, mobiuss.PendingActionLock, lockResp.PendingAction)
...
func (s *integrationMDMTestSuite) TestCustomConfigurationWebURL() {
	...
	s.DoJSON("POST", "/api/latest/mobiuss/spec/teams", teamSpecs, http.StatusOK, &applyResp)
	...
	require.NoError(t, err)
	s.DoJSON("POST", "/api/latest/mobiuss/hosts/transfer", addHostsToTeamRequest{TeamID: &teamID, HostIDs: []uint{h.ID}}, http.StatusOK, &addHostsToTeamResponse{})
	...
	applyResp = applyTeamSpecsResponse{}
	s.DoJSON("POST", "/api/latest/mobiuss/spec/teams", teamSpecs, http.StatusUnprocessableEntity, &applyResp)
}
...
func (s *integrationMDMTestSuite) TestDontIgnoreAnyProfileErrors() {
	...
	// The profiles should be associated with the host we made + the standard mobiuss configs
	...
	require.NoError(t, apple_mdm.VerifyHostMDMProfiles(context.Background(), s.ds, host, map[string]*mobiuss.HostMacOSProfile{
		"I1": {Identifier: "I1", DisplayName: "I1", InstallDate: time.Now()},
		"I2": {Identifier: "I2", DisplayName: "I2", InstallDate: time.Now()},
		mobileconfig.MobiusdConfigPayloadIdentifier:      {Identifier: mobileconfig.MobiusdConfigPayloadIdentifier, DisplayName: "I2", InstallDate: time.Now()},
		mobileconfig.MobiusCARootConfigPayloadIdentifier: {Identifier: mobileconfig.MobiusCARootConfigPayloadIdentifier, DisplayName: "I2", InstallDate: time.Now()},
	}))
	...
	for _, hm := range *getHostResp.Host.MDM.Profiles {
		require.Equal(t, mobiuss.MDMDeliveryVerified, *hm.Status)
	}

	// remove the profiles
	s.Do("POST", "/api/v1/mobiuss/mdm/apple/profiles/batch", batchSetMDMAppleProfilesRequest{}, http.StatusNoContent)
	s.awaitTriggerProfileSchedule(t)
	...
	for _, hm := range *getHostResp.Host.MDM.Profiles {
		if wantErr, ok := expectedErrs[hm.Name]; ok {
			require.Equal(t, mobiuss.MDMDeliveryFailed, *hm.Status)
...
func (s *integrationMDMTestSuite) TestMDMDiskEncryptionIssue16636() {
	// see https://github.com/notawar/mobiuss/issues/16636

	t := s.T()

	// send an empty patch object to ensure it's ok if not provided
	acResp := appConfigResponse{}
	s.DoJSON("PATCH", "/api/latest/mobiuss/config", json.RawMessage(`{}`), http.StatusOK, &acResp)
	assert.False(t, acResp.MDM.EnableDiskEncryption.Value)
	s.assertConfigProfilesByIdentifier(nil, mobileconfig.MobiusFileVaultPayloadIdentifier, false)
...
func (s *integrationMDMTestSuite) TestIsServerBitlockerStatus() {
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/hosts/%d", host.ID), nil, http.StatusOK, &hr)
	...
	require.NoError(t, s.ds.SetOrUpdateMDMData(ctx, host2.ID, false, true, "http://example.com", false, mobiuss.WellKnownMDMMobius, ""))

	hr = getHostResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/hosts/%d", host2.ID), nil, http.StatusOK, &hr)
	require.NotNil(t, hr.Host.MDM.OSSettings.DiskEncryption.Status)
	require.Equal(t, mobiuss.DiskEncryptionEnforcing, *hr.Host.MDM.OSSettings.DiskEncryption.Status)
}
...
func (s *integrationMDMTestSuite) TestRemoveFailedProfiles() {
	...
	require.NoError(t, apple_mdm.VerifyHostMDMProfiles(context.Background(), s.ds, host, map[string]*mobiuss.HostMacOSProfile{
		"I1": {Identifier: "I1", DisplayName: "I1", InstallDate: time.Now()},
		mobileconfig.MobiusdConfigPayloadIdentifier:      {Identifier: mobileconfig.MobiusdConfigPayloadIdentifier, DisplayName: "dn1", InstallDate: time.Now()},
		mobileconfig.MobiusCARootConfigPayloadIdentifier: {Identifier: mobileconfig.MobiusCARootConfigPayloadIdentifier, DisplayName: "dn2", InstallDate: time.Now()},
	}))
	...
	s.DoJSON("POST", "/api/latest/mobiuss/hosts/transfer", addHostsToTeamRequest{TeamID: &team.ID, HostIDs: []uint{host.ID}}, http.StatusOK, &addHostsToTeamResponse{})
	s.awaitTriggerProfileSchedule(t)
	...
	var profUUID string
	for _, hm := range *getHostResp.Host.MDM.Profiles {
		require.Equal(t, mobiuss.MDMDeliveryPending, *hm.Status)
		if hm.Name == "N3" {
	...
	s.Do("DELETE", fmt.Sprintf("/api/latest/mobiuss/mdm/profiles/%s", profUUID), &deleteMDMAppleConfigProfileRequest{}, http.StatusOK)
	s.awaitTriggerProfileSchedule(t)

	getHostResp = getHostResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/hosts/%d", host.ID), nil, http.StatusOK, &getHostResp)
	require.NotNil(t, getHostResp.Host.MDM.Profiles)
	// Since Mobius doesn't know for sure whether profile was installed or not, it sends a remove command just in case.
	require.Len(t, *getHostResp.Host.MDM.Profiles, 3)
	for _, hm := range *getHostResp.Host.MDM.Profiles {
		require.Equal(t, mobiuss.MDMDeliveryPending, *hm.Status)
		if hm.Name == "N3" {
			assert.Equal(t, mobiuss.MDMOperationTypeRemove, hm.OperationType)
		}
	}
}
...
func (s *integrationMDMTestSuite) TestABMAssetManagement() {
	...
	r := s.Do("GET", "/api/latest/mobiuss/mdm/apple/abm_public_key", generateABMKeyPairResponse{}, http.StatusInternalServerError)
	require.Contains(t, extractServerErrorText(r.Body), "Couldn't download public key. Missing required private key. Learn how to configure the private key here: https://mobiusmdm.com/learn-more-about/mobiuss-server-private-key")
	testSetEmptyPrivateKey = false

	// grab the current public key
	var abmResp generateABMKeyPairResponse
	s.DoJSON("GET", "/api/latest/mobiuss/mdm/apple/abm_public_key", nil, http.StatusOK, &abmResp)
	...
	// disable ABM
	s.Do("DELETE", fmt.Sprintf("/api/latest/mobiuss/abm_tokens/%d", tok.ID), nil, http.StatusNoContent)
	...
	s.DoJSON("GET", "/api/latest/mobiuss/mdm/apple/abm_public_key", nil, http.StatusOK, &newABMResp)
	...
	s.DoJSON("GET", "/api/latest/mobiuss/mdm/apple/abm_public_key", nil, http.StatusOK, &renewABMResp)
...
func (s *integrationMDMTestSuite) enableABM(orgName string) *mobiuss.ABMToken {
	t := s.T()
	var abmResp generateABMKeyPairResponse
	s.DoJSON("GET", "/api/latest/mobiuss/mdm/apple/abm_public_key", nil, http.StatusOK, &abmResp)
	require.Nil(t, abmResp.Err)
	...
	ctx := context.Background()
	assets, err := s.ds.GetAllMDMConfigAssetsByName(ctx, []mobiuss.MDMAssetName{
		mobiuss.MDMAssetABMCert,
		mobiuss.MDMAssetABMKey,
	}, nil)
	require.NoError(t, err)
	require.Len(t, assets, 2)
	require.Equal(t, abmResp.PublicKey, assets[mobiuss.MDMAssetABMCert].Value)
...
func (s *integrationMDMTestSuite) appleCoreCertsSetup() {
	...
	s.SucceedNextCSRRequest()
	s.DoJSON("GET", "/api/latest/mobiuss/mdm/apple/request_csr", getMDMAppleCSRRequest{}, http.StatusOK, &resp)
...
func (s *integrationMDMTestSuite) uploadABMToken(encryptedToken []byte, expectedStatus int, wantErr string) {
	...
	res := s.DoRawWithHeaders("POST", "/api/latest/mobiuss/abm_tokens", b.Bytes(), expectedStatus, headers)
	if wantErr != "" {
		errMsg := extractServerErrorText(res.Body)
		assert.Contains(t, errMsg, wantErr)
	}
...
func (s *integrationMDMTestSuite) TestSilentMigrationGotchas() {
	...
	err := s.ds.SetOrUpdateMDMData(ctx, host.ID, true, true, "https://foo.com", false, mobiuss.WellKnownMDMSimpleMDM, "")
	require.NoError(t, err)

	var hostResp getHostResponse
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/hosts/%d", host.ID), nil, http.StatusOK, &hostResp)
	require.NotNil(t, hostResp.Host)
	require.NotNil(t, hostResp.Host.MDM.ConnectedToMobius)
	require.False(t, *hostResp.Host.MDM.ConnectedToMobius)
	...
	resp := orbitGetConfigResponse{}
	s.DoJSON("POST", "/api/mobiuss/orbit/config", json.RawMessage(fmt.Sprintf(`{"orbit_node_key": %q}`, *host.OrbitNodeKey)), http.StatusOK, &resp)
	require.False(t, resp.Notifications.RenewEnrollmentProfile)
	require.True(t, resp.Notifications.NeedsMDMMigration)

	// simulate that's actually enrolled to Mobius under the hood
	mdmDevice := mdmtest.NewTestMDMClientAppleDirect(mdmtest.AppleEnrollInfo{
		SCEPChallenge: s.scepChallenge,
		SCEPURL:       s.server.URL + apple_mdm.SCEPPath,
		MDMURL:        s.server.URL + apple_mdm.MDMPath,
	...
	// host response says that it's connected to Mobius (this just checks that the
	...
	require.NotNil(t, hostResp.Host)
	require.NotNil(t, hostResp.Host.MDM.ConnectedToMobius)
	require.True(t, *hostResp.Host.MDM.ConnectedToMobius)
	...
	// set an enroll secret so the mobiusdaemon profile is delivered
	var applyResp applyEnrollSecretSpecResponse
	s.DoJSON("POST", "/api/latest/mobiuss/spec/enroll_secret", applyEnrollSecretSpecRequest{
		Spec: &mobiuss.EnrollSecretSpec{
			Secrets: []*mobiuss.EnrollSecret{{Secret: t.Name()}},
	...
	// explicitly run the worker, which will send the install mobiusdaemon command
	...
	require.Len(t, installs, 2) // mobiusdaemon profile and root cert profile

	// trigger the scep renewals cron
	cert, key, err := generateCertWithAPNsTopic()
	require.NoError(t, err)
	mobiusCfg := config.TestConfig()
	config.SetTestMDMConfig(s.T(), &mobiusCfg, cert, key, "")
	logger := kitlog.NewJSONLogger(os.Stdout)
	err = RenewSCEPCertificates(ctx, logger, s.ds, &mobiusCfg, s.mdmCommander)
	require.NoError(t, err)
...
func (s *integrationMDMTestSuite) TestMDMEnrollDoesntClearLastEnrolledAtForMacOS() {
	...
	// Enroll to Mobius with mobiusdaemon first.
	desktopToken := uuid.New().String()
	mdmDevice := mdmtest.NewTestMDMClientAppleDesktopManual(s.server.URL, desktopToken)
	mobiusHost, err := s.ds.NewHost(context.Background(), &mobiuss.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
	...
	require.NoError(t, err)
	err = s.ds.SetOrUpdateDeviceAuthToken(context.Background(), mobiusHost.ID, desktopToken)
	require.NoError(t, err)
	hostResp := getHostResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/hosts/%d", mobiusHost.ID), nil, http.StatusOK, &hostResp)
	require.NotNil(t, hostResp.Host)
...
func (s *integrationMDMTestSuite) TestConnectedToMobiusWithoutCheckout() {
	t := s.T()
	ctx := context.Background()
	...
	// host is not connected to Mobius anymore
	hostResp = getHostResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/hosts/%d", host.ID), nil, http.StatusOK, &hostResp)
	require.NotNil(t, hostResp.Host)
	require.NotNil(t, hostResp.Host.MDM.ConnectedToMobius)
...
func (s *integrationMDMTestSuite) TestBatchAssociateAppStoreApps() {
	t := s.T()
	batchURL := "/api/latest/mobiuss/software/app_store_apps/batch"

	// non-existent team
	s.Do("POST", batchURL, batchAssociateAppStoreAppsRequest{}, http.StatusNotFound, "team_name", "foo")

	// create a team
	tmGood, err := s.ds.NewTeam(context.Background(), &mobiuss.Team{
		Name:        t.Name() + " good",
		Description: "desc",
	})
	require.NoError(t, err)
	...
	// create a team
	tmEmpty, err := s.ds.NewTeam(context.Background(), &mobiuss.Team{
		Name:        t.Name() + " empty",
		Description: "desc",
	})
	require.NoError(t, err)
	...
	// FIXME
	// s.Do("POST", batchURL, batchAssociateAppStoreAppsRequest{Apps: []mobiuss.VPPBatchPayload{{AppStoreID: s.appleVPPConfigSrvConfig.Assets[0].AdamID}}}, http.StatusUnprocessableEntity, "team_name", tmGood.Name)

	// Valid token
	orgName := "Mobius Device Management Inc."
	token := "mycooltoken"
	expTime := time.Now().Add(200 * time.Hour).UTC().Round(time.Second)
	expDate := expTime.Format(mobiuss.VPPTimeFormat)
	tokenJSON := fmt.Sprintf(`{"expDate":"%s","token":"%s","orgName":"%s"}`, expDate, token, orgName)
	t.Setenv("MOBIUS_DEV_VPP_URL", s.appleVPPConfigSrv.URL)
	var vppRes uploadVPPTokenResponse
	s.uploadDataViaForm("/api/latest/mobiuss/vpp_tokens", "token", "token.vpptoken", []byte(base64.StdEncoding.EncodeToString([]byte(tokenJSON))), http.StatusAccepted, "", &vppRes)

	var resPatchVPP patchVPPTokensTeamsResponse
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobiuss/vpp_tokens/%d/teams", vppRes.Token.ID), patchVPPTokensTeamsRequest{TeamIDs: []uint{}}, http.StatusOK, &resPatchVPP)
	...
	hValid, err := s.ds.NewHost(context.Background(), &mobiuss.Host{
		DetailUpdatedAt: time.Now(),
		LabelUpdatedAt:  time.Now(),
		PolicyUpdatedAt: time.Now(),
		HardwareSerial:  "123",
	...
	s.Do("POST", batchURL, batchAssociateAppStoreAppsRequest{Apps: []mobiuss.VPPBatchPayload{{AppStoreID: "fake-app"}}}, http.StatusUnprocessableEntity, "team_name", tmGood.Name)
	...
	s.DoJSON("POST", batchURL, batchAssociateAppStoreAppsRequest{Apps: []mobiuss.VPPBatchPayload{
	...
	assert.Contains(t, assoc, mobiuss.VPPAppID{AdamID: s.appleVPPConfigSrvConfig.Assets[0].AdamID, Platform:mobiusus.MacOSPlatform})

	// Associating one good and one bad app
	s.Do("POST",
		batchURL,
		batchAssociateAppStoreAppsRequest{Apps: []mobiuss.VPPBatchPayload{
			{AppStoreID: s.appleVPPConfigSrvConfig.Assets[0].AdamID},
			{AppStoreID: "fake-app"},
	...
	assert.Contains(t, assoc, mobiuss.VPPAppID{AdamID: s.appleVPPConfigSrvConfig.Assets[0].AdamID, Platform:mobiusus.MacOSPlatform})
	assert.Contains(t, assoc, mobiuss.VPPAppID{AdamID: s.appleVPPConfigSrvConfig.Assets[1].AdamID, Platform:mobiusus.IOSPlatform})
	assert.Contains(t, assoc, mobiuss.VPPAppID{AdamID: s.appleVPPConfigSrvConfig.Assets[1].AdamID, Platform:mobiusus.IPadOSPlatform})
	meta, err := s.ds.GetVPPAppMetadataByAdamIDPlatformTeamID(ctx, s.appleVPPConfigSrvConfig.Assets[1].AdamID, mobiuss.MacOSPlatform, &tmGood.ID)
	require.NoError(t, err)

	actual := assoc[mobiuss.VPPAppID{AdamID: s.appleVPPConfigSrvConfig.Assets[1].AdamID, Platform:mobiusus.MacOSPlatform}]
	// Only macOS version should be self-service
	assert.Equal(t, mobiuss.VPPAppTeam{
		VPPAppID:           mobiuss.VPPAppID{AdamID: s.appleVPPConfigSrvConfig.Assets[1].AdamID, Platform:mobiusus.MacOSPlatform},
		SelfService:        true,
	...
	require.NoError(t, err)
	require.Len(t, assoc, 4)
	meta, err = s.ds.GetVPPAppMetadataByAdamIDPlatformTeamID(ctx, s.appleVPPConfigSrvConfig.Assets[0].AdamID, mobiuss.MacOSPlatform, &tmGood.ID)
	require.NoError(t, err)
	actual = assoc[mobiuss.VPPAppID{AdamID: s.appleVPPConfigSrvConfig.Assets[0].AdamID, Platform:mobiusus.MacOSPlatform}]
	assert.Equal(t, mobiuss.VPPAppTeam{
		VPPAppID:           mobiuss.VPPAppID{AdamID: s.appleVPPConfigSrvConfig.Assets[0].AdamID, Platform:mobiusus.MacOSPlatform},
		SelfService:        true,
		InstallDuringSetup: ptr.Bool(false),
		AppTeamID:          meta.AppTeamID,
		AddedAt:            actual.AddedAt,
	}, actual)
	meta, err = s.ds.GetVPPAppMetadataByAdamIDPlatformTeamID(ctx, s.appleVPPConfigSrvConfig.Assets[1].AdamID, mobiuss.IOSPlatform, &tmGood.ID)
	require.NoError(t, err)
	actual = assoc[mobiuss.VPPAppID{AdamID: s.appleVPPConfigSrvConfig.Assets[1].AdamID, Platform:mobiusus.IOSPlatform}]
	assert.Equal(t, mobiuss.VPPAppTeam{
		VPPAppID:           mobiuss.VPPAppID{AdamID: s.appleVPPConfigSrvConfig.Assets[1].AdamID, Platform:mobiusus.IOSPlatform},
		InstallDuringSetup: ptr.Bool(false),
		AppTeamID:          meta.AppTeamID,
		AddedAt:            actual.AddedAt,
	}, actual)
	meta, err = s.ds.GetVPPAppMetadataByAdamIDPlatformTeamID(ctx, s.appleVPPConfigSrvConfig.Assets[1].AdamID, mobiuss.IPadOSPlatform, &tmGood.ID)
	require.NoError(t, err)
	actual = assoc[mobiuss.VPPAppID{AdamID: s.appleVPPConfigSrvConfig.Assets[1].AdamID, Platform:mobiusus.IPadOSPlatform}]
	assert.Equal(t, mobiuss.VPPAppTeam{
		VPPAppID:           mobiuss.VPPAppID{AdamID: s.appleVPPConfigSrvConfig.Assets[1].AdamID, Platform:mobiusus.IPadOSPlatform},
		InstallDuringSetup: ptr.Bool(false),
		AppTeamID:          meta.AppTeamID,
		AddedAt:            actual.AddedAt,
	}, actual)
	...
	s.DoJSON("POST", batchURL, batchAssociateAppStoreAppsRequest{Apps: []mobiuss.VPPBatchPayload{{AppStoreID: s.appleVPPConfigSrvConfig.Assets[0].AdamID}}}, http.StatusOK, &batchAssociateResponse, "team_name", tmEmpty.Name)
...
func (s *integrationMDMTestSuite) TestRefetchIOSIPadOS() {
	...
	require.NoError(t, err)
	r := s.Do("POST", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/refetch", mobiusHost.ID), nil, http.StatusUnprocessableEntity, "error",
	...
	// Refetch host
	_ = s.Do("POST", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/refetch", host.ID), nil, http.StatusOK)
	...
	assert.ElementsMatch(t, []mobiuss.HostMDMCommand{
		{HostID: host.ID, CommandType: mobiuss.RefetchAppsCommandUUIDPrefix},
		{HostID: host.ID, CommandType: mobiuss.RefetchCertsCommandUUIDPrefix},
		{HostID: host.ID, CommandType: mobiuss.RefetchDeviceCommandUUIDPrefix},
	}, commands)
	...
	expectedSoftware := []mobiuss.HostSoftwareEntry{
		{
			Software: mobiuss.Software{
	...
	cmd, err = mdmClient.AcknowledgeInstalledApplicationList(mdmClient.UUID, cmd.CommandUUID,
		[]mobiuss.Software{expectedSoftware[0].Software})
	require.NoError(t, err)
	...
	// Refetch host
	_ = s.Do("POST", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/refetch", hostIPad.ID), nil, http.StatusOK)
	commandsSent += commandsSentPerRefetch

	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/hosts/%d", hostIPad.ID), nil, http.StatusOK, &hostResp)
	...
	s.DoJSON("GET", "/api/latest/mobiuss/software/titles", nil, http.StatusOK, &resp, "query", "Evernote")
	expectedTitles := []mobiuss.SoftwareTitleListResult{
		{
	...
	require.NotNil(t, cmd)
	require.Equal(t, "InstalledApplicationList", cmd.Command.RequestType)
	cmd, err = mdmClient.AcknowledgeInstalledApplicationList(mdmClient.UUID, cmd.CommandUUID, []mobiuss.Software{})
	require.NoError(t, err)
	require.Equal(t, "CertificateList", cmd.Command.RequestType)
	...
	trigger := triggerRequest{
		Name: string(mobiuss.CronAppleMDMIPhoneIPadRefetcher),
	}
	_ = s.Do("POST", "/api/latest/mobiuss/trigger", trigger, http.StatusOK)
	commandsSent += commandsSentPerRefetch
...
func checkInstallMobiusdCommandSent(t *testing.T, mdmDevice *mdmtest.TestAppleMDMClient, wantCommand bool) {
	foundInstallMobiusdCommand := false
	cmd, err := mdmDevice.Idle()
	require.NoError(t, err)
	for cmd != nil {
...
func (s *integrationMDMTestSuite) TestVPPApps() {
	...
	// Invalid token
	t.Setenv("MOBIUS_DEV_VPP_URL", s.appleVPPConfigSrv.URL+"?invalidToken")
	s.uploadDataViaForm("/api/latest/mobiuss/vpp_tokens", "token", "token.vpptoken", []byte("foobar"), http.StatusUnprocessableEntity, "Invalid token. Please provide a valid content token from Apple Business Manager.", nil)
	// Attempt to renew an invalid (nonexistent) token, should fail
	s.uploadDataViaFormWithVerb("/api/latest/mobiuss/vpp_tokens/999/renew", "PATCH", "token", "token.vpptoken", []byte(base64.StdEncoding.EncodeToString([]byte("foobar"))), http.StatusUnprocessableEntity, "Invalid token. Please provide a valid content token from Apple Business Manager.", nil)

	// Simulate a server error from the Apple API
	t.Setenv("MOBIUS_DEV_VPP_URL", s.appleVPPConfigSrv.URL+"?serverError")
	s.uploadDataViaForm("/api/latest/mobiuss/vpp_tokens", "token", "token.vpptoken", []byte("foobar"), http.StatusInternalServerError, "Apple VPP endpoint returned error: Internal server error (error number: 9603)", nil)
	...
	s.uploadDataViaForm("/api/latest/mobiuss/vpp_tokens", "token", "token.vpptoken", []byte(base64.StdEncoding.EncodeToString([]byte(tokenJSON))), http.StatusAccepted, "", &validToken)

	s.lastActivityMatches(mobiuss.ActivityEnabledVPP{}.ActivityName(), "", 0)

	// Renew valid token
	expTime = expTime.Add(24 * time.Hour).UTC().Round(time.Second)
	expDate = expTime.Format(mobiuss.VPPTimeFormat)
	tokenJSON = fmt.Sprintf(`{"expDate":"%s","token":"%s","orgName":"%s"}`, expDate, token, orgName)
	s.uploadDataViaFormWithVerb(fmt.Sprintf("/api/latest/mobiuss/vpp_tokens/%d/renew", validToken.Token.ID), "PATCH", "token", "token.vpptoken", []byte(base64.StdEncoding.EncodeToString([]byte(tokenJSON))), http.StatusAccepted, "", &validToken)
	// Valid token bytes, but non-existent token ID. Should get not found
	s.uploadDataViaFormWithVerb("/api/latest/mobiuss/vpp_tokens/999/renew", "PATCH", "token", "token.vpptoken", []byte(base64.StdEncoding.EncodeToString([]byte(tokenJSON))), http.StatusNotFound, "VPPToken was not found in the datastore", nil)

	// Get the token
	var resp getVPPTokensResponse
	s.DoJSON("GET", "/api/latest/mobiuss/vpp_tokens", &getVPPTokensRequest{}, http.StatusOK, &resp)
	...
	getSoftwareTitleIDFromApp := func(app *mobiuss.VPPApp) uint {
		var titleID uint
		mysql.ExecAdhocSQL(t, s.ds, func(q sqlx.ExtContext) error {
			ctx := context.Background()
	...
	t.Run("vpp apps with labels", func(t *testing.T) {
		...
		s.DoJSON("POST", "/api/latest/mobiuss/teams", &createTeamRequest{TeamPayload:mobiusus.TeamPayload{Name: ptr.String("Team Labels" + t.Name())}}, http.StatusOK, &newTeamResp)
		...
		orbitKey := setOrbitEnrollment(t, host, s.ds)
		host.OrbitNodeKey = &orbitKey
		s.Do("POST", "/api/latest/mobiuss/hosts/transfer", &addHostsToTeamRequest{HostIDs: []uint{host.ID}, TeamID: &team.ID}, http.StatusOK)

		// Associate team to the VPP token.
		var resPatchVPP patchVPPTokensTeamsResponse
		s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobiuss/vpp_tokens/%d/teams", resp.Tokens[0].ID), patchVPPTokensTeamsRequest{TeamIDs: []uint{team.ID}}, http.StatusOK, &resPatchVPP)

		var createLabelResp createLabelResponse
		s.DoJSON("POST", "/api/latest/mobiuss/labels", mobiusus.LabelPayload{Name: "label1" + t.Name()}, http.StatusOK, &createLabelResp)
		l1 := createLabelResp.Label
		require.NotNil(t, l1)

		s.DoJSON("POST", "/api/latest/mobiuss/labels", mobiusus.LabelPayload{Name: "label2" + t.Name()}, http.StatusOK, &createLabelResp)
		l2 := createLabelResp.Label
		require.NotNil(t, l2)

		includeAnyApp := mobiuss.VPPApp{
			VPPAppTeam: mobiuss.VPPAppTeam{
				VPPAppID: mobiuss.VPPAppID{
					AdamID:   "1",
					Platform: mobiuss.MacOSPlatform,
				},
			},
			Name:             "App 1",
		...
		res := s.Do("POST", "/api/latest/mobiuss/software/app_store_apps", addAppReq, http.StatusBadRequest)
		...
		s.DoJSON("POST", "/api/latest/mobiuss/software/app_store_apps", addAppReq, http.StatusOK, &addAppResp)
		...
		s.lastActivityMatches(mobiuss.ActivityAddedAppStoreApp{}.ActivityName(),
		...
		s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/software/titles/%d", titleID), nil, http.StatusOK, &getSWTitle, "team_id", fmt.Sprint(team.ID))
		...
		require.Equal(t, getSWTitle.SoftwareTitle.AppStoreApp.LabelsIncludeAny, []mobiuss.SoftwareScopeLabel{{LabelName: l1.Name, LabelID: l1.ID}})

		// Add an app with exclude_any labels
		excludeAnyApp := mobiuss.VPPApp{
			VPPAppTeam: mobiuss.VPPAppTeam{
				VPPAppID: mobiuss.VPPAppID{
					AdamID:   "2",
					Platform: mobiuss.MacOSPlatform,
		...
		require.Equal(t, getSWTitle.SoftwareTitle.AppStoreApp.LabelsExcludeAny, []mobiuss.SoftwareScopeLabel{{LabelName: l2.Name, LabelID: l2.ID}})
		require.True(t, getSWTitle.SoftwareTitle.AppStoreApp.SelfService)

		// Add a non-VPP software
		payloadRubyTm1 := &mobiuss.UploadSoftwareInstallerPayload{
		...
		s.DoJSON(
			"GET", "/api/latest/mobiuss/software/titles",
		...
		s.Do("PATCH", fmt.Sprintf("/api/latest/mobiuss/software/titles/%d/app_store_app", nonVPPTitleID), updateAppReq, http.StatusNotFound)

		// Attempt to update a non-existent app. Should fail.
		s.Do("PATCH", "/api/latest/mobiuss/software/titles/9999/app_store_app", updateAppReq, http.StatusNotFound)
		...
		res = s.Do("PATCH", fmt.Sprintf("/api/latest/mobiuss/software/titles/%d/app_store_app", titleID), updateAppReq, http.StatusBadRequest)
		require.Contains(t, extractServerErrorText(res.Body), `Only one of "labels_include_any" or "labels_exclude_any" can be included.`)
		...
		s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobiuss/software/titles/%d/app_store_app", titleID), updateAppReq, http.StatusOK, &updateAppResp)

		require.NotNil(t, updateAppResp.AppStoreApp)
		require.Equal(t, updateAppResp.AppStoreApp.AdamID, excludeAnyApp.AdamID)
		require.Equal(t, updateAppResp.AppStoreApp.LabelsIncludeAny, []mobiuss.SoftwareScopeLabel{{LabelName: l2.Name, LabelID: l2.ID}})
		require.Empty(t, updateAppResp.AppStoreApp.LabelsExcludeAny)
		require.False(t, updateAppResp.AppStoreApp.SelfService)
		require.Equal(t, mobiuss.MacOSPlatform, updateAppResp.AppStoreApp.Platform)
		...
		s.lastActivityMatches(mobiuss.ActivityEditedAppStoreApp{}.ActivityName(),
			fmt.Sprintf(activityData, team.Name,
		...
		require.NotNil(t, getSWTitle.SoftwareTitle.AppStoreApp)
		require.Equal(t, getSWTitle.SoftwareTitle.AppStoreApp.AdamID, excludeAnyApp.AdamID)
		require.Equal(t, getSWTitle.SoftwareTitle.AppStoreApp.LabelsIncludeAny, []mobiuss.SoftwareScopeLabel{{LabelName: l2.Name, LabelID: l2.ID}})
		require.Empty(t, getSWTitle.SoftwareTitle.AppStoreApp.LabelsExcludeAny)
		...
		// l2.
		res = s.Do("POST", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/software/%d/install", host.ID, titleID), &installSoftwareRequest{}, http.StatusBadRequest)
		...
		s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobiuss/labels/%d", l2.ID), mobiusus.ModifyLabelPayload{Hosts: []string{host.HardwareSerial}}, http.StatusOK, &createLabelResp)

		s.appleVPPConfigSrvConfig.SerialNumbers = append(s.appleVPPConfigSrvConfig.SerialNumbers, mdmDevice.SerialNumber)

		s.Do("POST", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/software/%d/install", host.ID, titleID), &installSoftwareRequest{}, http.StatusAccepted)

		// Attempt to delete a non-existent app. Should fail.
		s.Do("DELETE", "/api/latest/mobiuss/software/titles/9999/available_for_install", nil, http.StatusNotFound, "team_id", fmt.Sprintf("%d", team.ID))

		// delete the VPP app
		s.Do("DELETE", fmt.Sprintf("/api/latest/mobiuss/software/titles/%d/available_for_install", titleID), nil, http.StatusNoContent, "team_id", fmt.Sprintf("%d", team.ID))
		activityData = `{"team_name": "%s", "software_title": "%s", "app_store_id": "%s", "team_id": %d, "platform": "%s", "labels_include_any": [{"id": %d, "name": %q}]}`
		s.lastActivityMatches(mobiuss.ActivityDeletedAppStoreApp{}.ActivityName(),
			fmt.Sprintf(activityData, team.Name,
				excludeAnyApp.Name, excludeAnyApp.AdamID, team.ID, excludeAnyApp.Platform, l2.ID, l2.Name), 0)

		// iOS and iPadOS

		iOSApp := mobiuss.VPPApp{
			VPPAppTeam: mobiuss.VPPAppTeam{
				VPPAppID: mobiuss.VPPAppID{
					AdamID:   "2",
					Platform: mobiuss.IOSPlatform,
				},
			},
			Name:             "App 2",
			BundleIdentifier: "b-2",
		...
		iPadOSApp := mobiuss.VPPApp{
			VPPAppTeam: mobiuss.VPPAppTeam{
				VPPAppID: mobiuss.VPPAppID{
					AdamID:   "2",
					Platform: mobiuss.IPadOSPlatform,
				},
			},
			Name:             "App 2",
			BundleIdentifier: "b-2",
		...
		for _, tc := range testCases {
			t.Run(tc.desc, func(t *testing.T) {
				...
				s.Do("POST", "/api/latest/mobiuss/hosts/transfer", &addHostsToTeamRequest{HostIDs: []uint{mobileHost.ID}, TeamID: &team.ID}, http.StatusOK)

				s.Do("POST", "/api/latest/mobiuss/software/app_store_apps", tc.addAppReq, http.StatusOK)
				...
				res = s.Do("POST", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/software/%d/install", mobileHost.ID, titleID), &installSoftwareRequest{}, http.StatusBadRequest)
				require.Contains(t, extractServerErrorText(res.Body), "Couldn't install. This host isn't a member of the labels defined for this software title.")

				// Create a new label
				s.DoJSON("POST", "/api/latest/mobiuss/labels", mobiusus.LabelPayload{Name: "label3" + t.Name(), Hosts: []string{mobileMDMDevice.SerialNumber}}, http.StatusOK, &createLabelResp)
				...
				s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobiuss/software/titles/%d/app_store_app", titleID), updateAppReq, http.StatusOK, &updateAppResp)
				s.Do("POST", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/software/%d/install", mobileHost.ID, titleID), &installSoftwareRequest{}, http.StatusAccepted)
	...
	// Create a team
	var newTeamResp teamResponse
	s.DoJSON("POST", "/api/latest/mobiuss/teams", &createTeamRequest{TeamPayload:mobiusus.TeamPayload{Name: ptr.String("Team 1")}}, http.StatusOK, &newTeamResp)
	team := newTeamResp.Team
	...
	// Add the team back using the PATCH /api/latest/mobiuss/config endpoint now (what GitOps uses).
	acResp = appConfigResponse{}
	s.DoJSON("PATCH", "/api/latest/mobiuss/config", json.RawMessage(fmt.Sprintf(`{
	...
	s.DoJSON("GET", "/api/latest/mobiuss/software/app_store_apps", &getAppStoreAppsRequest{}, http.StatusOK, &appResp, "team_id",
		fmt.Sprint(team.ID))
	require.NoError(t, appResp.Err)

	macOSApp := mobiuss.VPPApp{
		VPPAppTeam: mobiuss.VPPAppTeam{
			VPPAppID: mobiuss.VPPAppID{
				AdamID:   "1",
				Platform: mobiuss.MacOSPlatform,
	...
	s.DoJSON("POST", "/api/latest/mobiuss/software/app_store_apps", &addAppStoreAppRequest{TeamID: ptr.Uint(9999), AppStoreID: addedApp.AdamID}, http.StatusNotFound, &addAppResp)

	s.DoJSON("POST", "/api/latest/mobiuss/software/app_store_apps", &addAppStoreAppRequest{TeamID: &team.ID, AppStoreID: addedApp.AdamID, SelfService: true, AutomaticInstall: true}, http.StatusOK, &addAppResp)

	s.lastActivityOfTypeMatches(mobiuss.ActivityAddedAppStoreApp{}.ActivityName(),
	...
	s.DoJSON("GET", "/api/latest/mobiuss/software/titles", nil, http.StatusOK, &listSw, "team_id", fmt.Sprint(team.ID), "available_for_install", "true")
	...
	wantAct := mobiuss.ActivityTypeCreatedPolicy{
	...
	s.DoJSON("GET", "/api/latest/mobiuss/software/titles", nil, http.StatusOK, &listSw, "team_id", fmt.Sprint(team.ID), "self_service", "true")
	require.Len(t, listSw.SoftwareTitles, 1)
	...
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobiuss/teams/%d/policies/delete", team.ID), &deleteGlobalPoliciesRequest{IDs: []uint{listSw.SoftwareTitles[0].AppStoreApp.AutomaticInstallPolicies[0].ID}}, http.StatusOK, &deletePolicyResp)

	// delete the app store app for team 1
	s.Do("DELETE", fmt.Sprintf("/api/latest/mobiuss/software/titles/%d/available_for_install", macOSTitleID), nil, http.StatusNoContent,
	...
	s.Do("DELETE", fmt.Sprintf("/api/latest/mobiuss/software/titles/%d/available_for_install", macOSTitleID), nil, http.StatusNotFound,
		"team_id", fmt.Sprint(team.ID))

	// get the list of available apps, returns all apps now
	appResp = getAppStoreAppsResponse{}
	s.DoJSON("GET", "/api/latest/mobiuss/software/app_store_apps", nil, http.StatusOK, &appResp, "team_id", fmt.Sprint(team.ID))
	require.NoError(t, appResp.Err)
	assert.ElementsMatch(t, expectedApps, appResp.AppStoreApps)
	...
	s.DoJSON("POST", "/api/latest/mobiuss/software/app_store_apps",
	...
	// list the software titles for that team, to get the title id of the VPP app
	listSw = listSoftwareTitlesResponse{}
	s.DoJSON("GET", "/api/latest/mobiuss/software/titles", nil, http.StatusOK, &listSw, "team_id", fmt.Sprint(team.ID),
	...
	s.Do("POST", "/api/latest/mobiuss/hosts/transfer",
	...
	s.DoJSON("POST", "/api/latest/mobiuss/software/app_store_apps",
		...
		http.StatusOK, &addAppResp)
	s.lastActivityMatches(
		mobiuss.ActivityAddedAppStoreApp{}.ActivityName(),
		fmt.Sprintf(`{"team_name": "%s", "software_title": "%s", "software_title_id": %d, "app_store_id": "%s", "team_id": %d, "platform": "%s", "self_service": true}`, team.Name,
	...
	r := s.Do("POST", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/software/%d/install", orbitHost.ID, macOSTitleID), &installSoftwareRequest{}, http.StatusBadRequest)
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobiuss/vpp_tokens/%d/teams", validToken.Token.ID), patchVPPTokensTeamsRequest{}, http.StatusOK, &resPatchVPP)

	// Spoof an expired VPP token and attempt to install VPP app
	tokenJSONBad := fmt.Sprintf(`{"expDate":"%s","token":"%s","orgName":"%s"}`, "2099-06-24T15:50:50+0000", "badtoken", "Evil Mobius")
	s.appleVPPConfigSrvConfig.Location = "Spooky Haunted House"
	var vppRes uploadVPPTokenResponse
	s.uploadDataViaForm("/api/latest/mobiuss/vpp_tokens", "token", "token.vpptoken", []byte(base64.StdEncoding.EncodeToString([]byte(tokenJSONBad))), http.StatusAccepted, "", &vppRes)

	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobiuss/vpp_tokens/%d/teams", vppRes.Token.ID), patchVPPTokensTeamsRequest{TeamIDs: []uint{team.ID, 99}}, http.StatusUnprocessableEntity, &resPatchVPP)

	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobiuss/vpp_tokens/%d/teams", vppRes.Token.ID), patchVPPTokensTeamsRequest{TeamIDs: []uint{team.ID}}, http.StatusOK, &resPatchVPP)

	// Disable the token
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobiuss/vpp_tokens/%d/teams", vppRes.Token.ID), patchVPPTokensTeamsRequest{}, http.StatusOK, &resPatchVPP)

	// Enable all teams token
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobiuss/vpp_tokens/%d/teams", validToken.Token.ID), patchVPPTokensTeamsRequest{TeamIDs: []uint{}}, http.StatusOK, &resPatchVPP)

	// Attempt to install non-existent app
	r = s.Do("POST", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/software/%d/install", mdmHost.ID, 99999), &installSoftwareRequest{},
		http.StatusBadRequest)
	...
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/software/%d/install", mdmHost.ID, errTitleID), &installSoftwareRequest{},
		http.StatusAccepted, &installResp)

	s.Do("DELETE", fmt.Sprintf("/api/latest/mobiuss/vpp_tokens/%d", vppRes.Token.ID), &deleteVPPTokenRequest{}, http.StatusNoContent)

	// Check if the host is listed as pending
	var listResp listHostsResponse
	s.DoJSON("GET", "/api/latest/mobiuss/hosts", nil, http.StatusOK, &listResp, "software_status", "pending", "team_id", fmt.Sprint(team.ID),
		"software_title_id", fmt.Sprint(errTitleID))
	require.Len(t, listResp.Hosts, 1)
	require.Equal(t, listResp.Hosts[0].ID, mdmHost.ID)
	var countResp countHostsResponse
	s.DoJSON("GET", "/api/latest/mobiuss/hosts/count", nil, http.StatusOK, &countResp, "software_status", "pending", "team_id",
	...
	s.DoJSON("GET", "/api/latest/mobiuss/hosts", nil, http.StatusOK, &listResp, "software_status", "failed", "team_id", fmt.Sprint(team.ID),
		"software_title_id", fmt.Sprint(errTitleID))
	require.Len(t, listResp.Hosts, 1)
	require.Equal(t, listResp.Hosts[0].ID, mdmHost.ID)
	countResp = countHostsResponse{}
	s.DoJSON("GET", "/api/latest/mobiuss/hosts/count", nil, http.StatusOK, &countResp, "software_status", "failed", "team_id",
		fmt.Sprint(team.ID), "software_title_id", fmt.Sprint(errTitleID))
	require.Equal(t, 1, countResp.Count)

	s.lastActivityMatches(
		mobiuss.ActivityInstalledAppStoreApp{}.ActivityName(),
		fmt.Sprintf(
	...
	// Trigger install to the host
	installResp = installSoftwareResponse{}
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/software/%d/install", mdmHost.ID, macOSTitleID), &installSoftwareRequest{},
		http.StatusAccepted, &installResp)
	countResp = countHostsResponse{}
	s.DoJSON("GET", "/api/latest/mobiuss/hosts/count", nil, http.StatusOK, &countResp, "software_status", "pending", "team_id",
	...
	for cmd != nil {
		...
		case "InstalledApplicationList":
			require.NoError(t, plist.Unmarshal(cmd.Raw, &fullCmd))
			cmd, err = mdmDevice.AcknowledgeInstalledApplicationList(mdmDevice.UUID, cmd.CommandUUID, []mobiuss.Software{{Name: addedApp.Name, BundleIdentifier: addedApp.BundleIdentifier, Version: addedApp.LatestVersion}})
			require.NoError(t, err)
		default:
	...
	listResp = listHostsResponse{}
	s.DoJSON("GET", "/api/latest/mobiuss/hosts", nil, http.StatusOK, &listResp, "software_status", "installed", "team_id",
		fmt.Sprint(team.ID), "software_title_id", fmt.Sprint(macOSTitleID))
	mysql.ExecAdhocSQL(t, s.ds, func(q sqlx.ExtContext) error {
		var types []string
	...
	require.Len(t, listResp.Hosts, 1)
	countResp = countHostsResponse{}
	s.DoJSON("GET", "/api/latest/mobiuss/hosts/count", nil, http.StatusOK, &countResp, "software_status", "installed", "team_id",
		fmt.Sprint(team.ID), "software_title_id", fmt.Sprint(macOSTitleID))
	require.Equal(t, 1, countResp.Count)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/software", mdmHost.ID), nil, http.StatusOK, &getHostSw)
	...
	require.Equal(t, *got1.Status, mobiuss.SoftwareInstalled)
	...
	require.Equal(t, *got2.Status, mobiuss.SoftwareInstallFailed)
	...
	// Check with a query
	getHostSw = getHostSoftwareResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/software", mdmHost.ID), nil, http.StatusOK, &getHostSw, "query", "App 1")
	...
	getHostSw = getHostSoftwareResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/software", mdmHost.ID), nil, http.StatusOK, &getHostSw, "self_service", "true")
	...
	distributedReq := submitDistributedQueryResultsRequestShim{
		...
		Stats:    map[string]*mobiuss.Stats{},
	}
	...
	for name, install := range installs {
		t.Run(name, func(t *testing.T) {
			...
			if install.deviceToken != "" {
				...
				s.DoJSON("POST", fmt.Sprintf("/api/latest/mobiuss/device/%s/software/install/%d", install.deviceToken, install.titleID),
					&mobiusSelfServiceSoftwareInstallRequest{}, http.StatusAccepted, &ssInstallResp)
			} else {
				installResp = installSoftwareResponse{}
				s.DoJSON("POST", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/software/%d/install", installHost.ID, titleID),
			...
			if install.deviceToken != "" {
				...
				res := s.DoRawNoAuth("GET", fmt.Sprintf("/api/latest/mobiuss/device/%s/software/commands/%s/results", install.deviceToken, installCmdUUID), nil, http.StatusOK)
			...
			s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/activities/upcoming", installHost.ID),
				nil, http.StatusOK, &hostActivitiesResp)
			activitiesToString := func(activities []*mobiuss.UpcomingActivity) []string {
				var res []string
			...
			require.Equal(t, hostActivitiesResp.Activities[0].Type, mobiuss.ActivityInstalledAppStoreApp{}.ActivityName())
			...
			require.JSONEq(
				...
				fmt.Sprintf(
					...
					app.AdamID,
					installCmdUUID,
					mobiuss.SoftwareInstallPending,
					install.deviceToken != "",
				),
			...
			for cmd != nil {
				...
				case "InstalledApplicationList":
					require.NoError(t, plist.Unmarshal(cmd.Raw, &fullCmd))
					cmd, err = mdmClient.AcknowledgeInstalledApplicationList(mdmClient.UUID, cmd.CommandUUID, []mobiuss.Software{{Name: app.Name, BundleIdentifier: app.BundleIdentifier, Version: app.LatestVersion}})
					require.NoError(t, err)
				default:
					require.Fail(t, "unexpected command type", cmd.Command.RequestType)
				}
			...
			if install.deviceToken != "" {
				...
				s.DoRawNoAuth("GET", fmt.Sprintf("/api/latest/mobiuss/device/%s/software/commands/foobar/results", install.deviceToken), nil, http.StatusNotFound)
				s.DoRawNoAuth("GET", fmt.Sprintf("/api/latest/mobiuss/device/foobar/software/commands/%s/results", installCmdUUID), nil, http.StatusNotFound)
			}
			...
			// Check list host software
			getHostSw = getHostSoftwareResponse{}
			s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/software", installHost.ID), nil, http.StatusOK, &getHostSw)
	...
	var ssInstallResp submitSelfServiceSoftwareInstallResponse
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobiuss/device/%s/software/install/%d", selfServiceToken, iPadOSTitleID), &mobiusSelfServiceSoftwareInstallRequest{},
		http.StatusBadRequest, &ssInstallResp)
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobiuss/device/%s/software/install/%d", selfServiceToken, iOSTitleID), &mobiusSelfServiceSoftwareInstallRequest{},
		http.StatusBadRequest, &ssInstallResp)

	// Delete VPP token and check that it's not appearing anymore

	s.Do("DELETE", fmt.Sprintf("/api/latest/mobiuss/vpp_tokens/%d", validToken.Token.ID), &deleteVPPTokenResponse{}, http.StatusNoContent)
	s.DoJSON("GET", "/api/latest/mobiuss/vpp_tokens", &getVPPTokensRequest{}, http.StatusOK, &resp)
}
...
func (s *integrationMDMTestSuite) TestVPPAppPolicyAutomation() {
	...
	s.runWorker()
	checkInstallMobiusdCommandSent(t, mdmDevice2, true)
	...
	var createLabelResp createLabelResponse
	s.DoJSON("POST", "/api/latest/mobiuss/labels", mobiusus.LabelPayload{Name: "label1" + t.Name(), Hosts: []string{mdmHost.HardwareSerial}}, http.StatusOK, &createLabelResp)
	l1 := createLabelResp.Label
	require.NotNil(t, l1)

	s.DoJSON("POST", "/api/latest/mobiuss/labels", mobiusus.LabelPayload{Name: "label2" + t.Name(), Hosts: []string{mdmHost2.HardwareSerial}}, http.StatusOK, &createLabelResp)
	l2 := createLabelResp.Label
	...
	s.DoJSON("POST", "/api/latest/mobiuss/software/app_store_apps", &addAppStoreAppRequest{TeamID: &team.ID, Platform: addedApp.Platform, AppStoreID: addedApp.AdamID, SelfService: true, LabelsIncludeAny: []string{l1.Name}}, http.StatusOK, &addedMacOSApp)
	...
	policy0Team1, err := s.ds.NewTeamPolicy(ctx, team.ID, nil, mobiuss.PolicyPayload{
		Name:     "policy0Team1",
		Query:    "SELECT 1;",
		Platform: "darwin",
	})
	require.NoError(t, err)
	policy1Team1, err := s.ds.NewTeamPolicy(ctx, team.ID, nil, mobiuss.PolicyPayload{
		Name:     "policy1Team1",
		Query:    "SELECT 1;",
		Platform: "darwin",
	})
	require.NoError(t, err)
	policy2Team1, err := s.ds.NewTeamPolicy(ctx, team.ID, nil, mobiuss.PolicyPayload{ // will be set up with same VPP app
		Name:     "policy2Team1",
		Query:    "SELECT 1;",
		Platform: "darwin",
	})
	...
	policy3Team1, err := s.ds.NewTeamPolicy(ctx, team.ID, nil, mobiuss.PolicyPayload{ // will be set up with same VPP app + a script
		Name:     "policy3Team1",
		Query:    "SELECT 1;",
		Platform: "darwin",
	})
	...
	savedTmScript, err := s.ds.NewScript(ctx, &mobiuss.Script{
		TeamID:         &team.ID,
		Name:           "team_script.sh",
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobiuss/teams/%d/policies/%d", team.ID, policy1Team1.ID), modifyTeamPolicyRequest{
		ModifyPolicyPayload: mobiuss.ModifyPolicyPayload{
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobiuss/teams/%d/policies/%d", team.ID, policy3Team1.ID), modifyTeamPolicyRequest{
		ModifyPolicyPayload: mobiuss.ModifyPolicyPayload{
			SoftwareTitleID: optjson.Any[uint]{Set: true, Valid: true, Value: macOSTitleID},
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/software/titles/%d", macOSTitleID), getSoftwareTitleRequest{
		TeamID: &team.ID,
	...
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobiuss/teams/%d/policies/%d", team.ID, policy2Team1.ID), modifyTeamPolicyRequest{
		ModifyPolicyPayload: mobiuss.ModifyPolicyPayload{
			SoftwareTitleID: optjson.Any[uint]{Set: true, Valid: true, Value: macOSTitleID},
		},
	}, http.StatusOK, &mtplr)
	...
	newHost := func(name string, teamID *uint, platform string) *mobiuss.Host {
		h, err := s.ds.NewHost(ctx, &mobiuss.Host{
			DetailUpdatedAt: time.Now(),
			LabelUpdatedAt:  time.Now(),
			PolicyUpdatedAt: time.Now(),
	...
	newMobiusdHost := func(name string, teamID *uint, platform string) *mobiuss.Host {
	...
	host3 := newMobiusdHost("host3", &team.ID, "ubuntu")
	...
	var getHostResp getHostResponse
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/hosts/%d", mdmHost2.ID), nil, http.StatusOK, &getHostResp, "exclude_software", "true")
	...
	var hostActivitiesResp listHostUpcomingActivitiesResponse
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/activities/upcoming", mdmHost2.ID),
		nil, http.StatusOK, &hostActivitiesResp)

	require.Len(t, hostActivitiesResp.Activities, 1, "got activities: %v", activitiesToString(hostActivitiesResp.Activities))
	assert.Equal(t, hostActivitiesResp.Activities[0].Type, mobiuss.ActivityTypeRanScript{}.ActivityName())
	assert.True(t, hostActivitiesResp.Activities[0].MobiusInitiated)
	...
	s.lastActivityMatchesExtended(
		mobiuss.ActivityTypeRanScript{}.ActivityName(),
		fmt.Sprintf(
	...
	s.Do("PATCH", fmt.Sprintf("/api/latest/mobiuss/software/titles/%d/app_store_app", macOSTitleID), updateAppReq, http.StatusOK)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/activities/upcoming", mdmHost.ID),
		nil, http.StatusOK, &hostActivitiesResp)
	require.Len(t, hostActivitiesResp.Activities, 1, "got activities: %v", activitiesToString(hostActivitiesResp.Activities))
	assert.Equal(t, hostActivitiesResp.Activities[0].Type, mobiuss.ActivityInstalledAppStoreApp{}.ActivityName())
	assert.EqualValues(t, 1, hostActivitiesResp.Count)
	assert.JSONEq(
		t,
		fmt.Sprintf(
	...
	for cmd != nil {
		...
		case "InstalledApplicationList":
			require.NoError(t, plist.Unmarshal(cmd.Raw, &fullCmd))
			cmd, err = mdmDevice.AcknowledgeInstalledApplicationList(mdmDevice.UUID, cmd.CommandUUID, []mobiuss.Software{{Name: macOSApp.Name, BundleIdentifier: macOSApp.BundleIdentifier, Version: macOSApp.LatestVersion}})
			require.NoError(t, err)
		default:
			require.Fail(t, "unexpected command type", cmd.Command.RequestType)
		}
	...
	for cmd != nil {
		...
		case "InstalledApplicationList":
			require.NoError(t, plist.Unmarshal(cmd.Raw, &fullCmd))
			cmd, err = mdmDevice2.AcknowledgeInstalledApplicationList(mdmDevice2.UUID, cmd.CommandUUID, []mobiuss.Software{{Name: macOSApp.Name, BundleIdentifier: macOSApp.BundleIdentifier, Version: macOSApp.LatestVersion}})
			require.NoError(t, err)
		default:
			require.Fail(t, "unexpected command type", cmd.Command.RequestType)
		}
...
func (s *integrationMDMTestSuite) TestEnrollmentProfilesWithSpecialChars() {
	...
	nameWithInvalidChars := "Mobius & Device <3 Management"
	...
	s.DoJSON("POST", "/api/latest/mobiuss/spec/enroll_secret", applyEnrollSecretSpecRequest{
		Spec: &mobiuss.EnrollSecretSpec{
			Secrets: []*mobiuss.EnrollSecret{{Secret: enrollSecretWithInvalidChars}},
		},
	...
	_, err = s.ds.NewMDMAppleEnrollmentProfile(ctx, mobiuss.MDMAppleEnrollmentProfilePayload{
		Type:       "automatic",
		DEPProfile: &rawMsg,
		Token:      "abcd",
	})
	...
	// ensure the mobiusdaemon profile sends a good enroll secret too
	s.awaitTriggerProfileSchedule(t)
	prof := s.assertConfigProfilesByIdentifier(nil, mobileconfig.MobiusdConfigPayloadIdentifier, true)

	type mobiusdPlist struct {
		PayloadContent []struct {
...
func (s *integrationMDMTestSuite) TestOTAEnrollment() {
	...
	s.DoJSON("POST", "/api/latest/mobiuss/spec/enroll_secret", applyEnrollSecretSpecRequest{
		Spec: &mobiuss.EnrollSecretSpec{
			Secrets: []*mobiuss.EnrollSecret{{Secret: globalSecret}},
		},
	}, http.StatusOK, &applyResp)
...
<dict>
	...
	httpResp := s.DoRawNoAuth("POST", "/api/latest/mobiuss/ota_enrollment", reqBody, http.StatusForbidden)
	...
	httpResp = s.DoRawNoAuth("POST", "/api/latest/mobiuss/ota_enrollment?enroll_secret=foo", nil, http.StatusBadRequest)
	errMsg = extractServerErrorText(httpResp.Body)
	require.Contains(t, errMsg, "invalid request body")
	require.NoError(t, httpResp.Body.Close())

	// request with unsigned body
	httpResp = s.DoRawNoAuth("POST", "/api/latest/mobiuss/ota_enrollment?enroll_secret=foo", reqBody, http.StatusBadRequest)
	errMsg = extractServerErrorText(httpResp.Body)
	require.Contains(t, errMsg, "invalid request body")
	require.NoError(t, httpResp.Body.Close())
	...
	httpResp = s.DoRawNoAuth("POST", "/api/latest/mobiuss/ota_enrollment?enroll_secret=foo", signedReqBody, http.StatusForbidden)
	errMsg = extractServerErrorText(httpResp.Body)
	require.Contains(t, errMsg, "Couldn't install the profile. Invalid enroll secret. Please contact your IT admin.")
	require.NoError(t, httpResp.Body.Close())

	// request with invalid device signature
	os.Setenv("MOBIUS_DEV_MDM_APPLE_DISABLE_DEVICE_INFO_CERT_VERIFY", "1")
	httpResp = s.DoRawNoAuth("POST", "/api/latest/mobiuss/ota_enrollment?enroll_secret=foo", signedReqBody, http.StatusForbidden)
	errMsg = extractServerErrorText(httpResp.Body)
	require.Contains(t, errMsg, "Couldn't install the profile. Invalid enroll secret. Please contact your IT admin.")
	require.NoError(t, httpResp.Body.Close())
...
<dict>
	...
	signedReqBody, err = signedData.Finish()
	require.NoError(t, err)
	httpResp = s.DoRawNoAuth("POST", "/api/latest/mobiuss/ota_enrollment?enroll_secret=foo", signedReqBody, http.StatusBadRequest)
	errMsg = extractServerErrorText(httpResp.Body)
	...
	var hostByIdentifierResp getHostResponse
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/hosts/identifier/%s", mdmDevice.UUID), nil, http.StatusOK, &hostByIdentifierResp)
	...
	teamSpecs := applyTeamSpecsRequest{Specs: []*mobiuss.TeamSpec{{Name: "newteam", Secrets: &[mobiusus.EnrollSecret{{Secret: teamSecret}}}}}
	s.DoJSON("POST", "/api/latest/mobiuss/spec/teams", teamSpecs, http.StatusOK, &specResp)
	...
	require.NoError(t, mdmDevice.Enroll())
	s.runWorker()
	checkInstallMobiusdCommandSent(mdmDevice, false)
...
func (s *integrationMDMTestSuite) TestSCEPProxy() {
	...
	var goodProfile, badProfile mobiuss.HostMDMAppleProfile
	for _, profile := range profiles {
		...
		case "Good1":
			...
			goodProfile = profile
		case "Bad1":
			profile.Status = &mobiuss.MDMDeliveryFailed
			badProfile = profile
		}
	}
	err = s.ds.BulkUpsertMDMAppleHostProfiles(ctx, []*mobiuss.MDMAppleBulkUpsertHostProfilePayload{{
	...
	require.NoError(t, err)
	err = s.ds.BulkUpsertMDMManagedCertificates(ctx, []*mobiuss.MDMManagedCertificate{
		{
			HostUUID:    host.UUID,
...
func (s *integrationMDMTestSuite) TestDigiCertConfig() {
	...
	appConfig := map[string]interface{}{
		"integrations": map[string]interface{}{
			"digicert": []mobiuss.DigiCertIntegration{caBad},
		},
	}
	...
	req.RawMessage = raw
	rawRes := s.Do("PATCH", "/api/latest/mobiuss/config", &req, http.StatusUnprocessableEntity, "dry_run", "true")
	errMsg := extractServerErrorText(rawRes.Body)
	require.Contains(t, errMsg, "Could not verify DigiCert profile ID")
	_, err = s.ds.GetAllCAConfigAssetsByType(ctx, mobiuss.CAConfigDigiCert)
	assert.True(t, mobiuss.IsNotFound(err))
	...
	appConfig = map[string]interface{}{
		"integrations": map[string]interface{}{
			"digicert": []mobiuss.DigiCertIntegration{ca0, ca1, ca2},
		},
	}
	raw, err = json.Marshal(appConfig)
	require.NoError(t, err)
	...
	s.DoJSON("PATCH", "/api/latest/mobiuss/config", &req, http.StatusOK, &res, "dry_run", "true")
	assert.Empty(t, res.Integrations.DigiCert.Value)
	_, err = s.ds.GetAllCAConfigAssetsByType(ctx, mobiuss.CAConfigDigiCert)
	assert.True(t, mobiuss.IsNotFound(err))

	res = appConfigResponse{}
	s.DoJSON("PATCH", "/api/latest/mobiuss/config", &req, http.StatusOK, &res)
	assert.Len(t, res.Integrations.DigiCert.Value, 3)
	for _, ca := range res.Integrations.DigiCert.Value {
		assert.Equal(t, ca.APIToken, mobiuss.MaskedPassword)
	}
	assets, err := s.ds.GetAllCAConfigAssetsByType(ctx, mobiuss.CAConfigDigiCert)
	require.NoError(t, err)
	...
	appConfig = map[string]interface{}{
		"integrations": map[string]interface{}{
			"digicert": []mobiuss.DigiCertIntegration{ca3, ca2, ca1},
		},
	}
	raw, err = json.Marshal(appConfig)
	require.NoError(t, err)
	...
	require.True(t, len(listActivities.Activities) > 0)
	activity = mobiuss.ActivityAddedDigiCert{}
	editActivity := mobiuss.ActivityEditedDigiCert{}
	delActivity := mobiuss.ActivityDeletedDigiCert{}
	var numFound int
	for _, act := range listActivities.Activities {
		switch act.Type {
...
func (s *integrationMDMTestSuite) TestDigiCertIntegration() {
	...
	appConfig := map[string]interface{}{
		"integrations": map[string]interface{}{
			"digicert": []mobiuss.DigiCertIntegration{ca},
		},
	}
	raw, err := json.Marshal(appConfig)
	require.NoError(t, err)
	...
	rawRes := s.Do("POST", "/api/latest/mobiuss/mdm/profiles/batch", batchSetMDMProfilesRequest{Profiles: [mobiusus.MDMProfileBatchPayload{
	...
	s.Do("POST", "/api/latest/mobiuss/mdm/profiles/batch", batchSetMDMProfilesRequest{Profiles: [mobiusus.MDMProfileBatchPayload{
	...
	require.Contains(t, string(p.Mobileconfig), "com.mobiusmdm.pkcs12")
	...
	appConfig = map[string]interface{}{
		"integrations": map[string]interface{}{
			"digicert": []mobiuss.DigiCertIntegration{ca, caFail},
		},
	}
	raw, err = json.Marshal(appConfig)
	require.NoError(t, err)
	...
	for _, prof := range *getHostResp.Host.MDM.Profiles {
		if prof.Name == "N2" {
			foundGood = true
			assert.Equal(t, mobiuss.MDMDeliveryVerifying, *prof.Status)
			assert.Equal(t, mobiuss.MDMOperationTypeInstall, prof.OperationType)
			continue
		}
		if prof.Name == "N3" {
			foundFailed = true
			assert.Equal(t, mobiuss.MDMDeliveryFailed, *prof.Status)
			assert.Equal(t, mobiuss.MDMOperationTypeInstall, prof.OperationType)
	...
	// ////////////////////////////////
	// Test DigiCert CA with Mobius variables
	...
	caMobiusVars := mobiuss.DigiCertIntegration{
		Name:                          "MobiusVars",
		URL:                           digiCertServer.server.URL,
		APIToken:                      "api_token",
		ProfileID:                     "profile_id",
		CertificateCommonName:         "$MOBIUS_VAR_" + mobiuss.MobiusVarHostHardwareSerial + " $MOBIUS_VAR_" +mobiusus.MobiusVarHostEndUserEmailIDP,
		CertificateUserPrincipalNames: []string{"_${MOBIUS_VAR_" + mobiuss.MobiusVarHostEndUserEmailIDP + "}_"},
		CertificateSeatID:             "_${MOBIUS_VAR_" + mobiuss.MobiusVarHostHardwareSerial + "}_",
	}
	appConfig = map[string]interface{}{
		"integrations": map[string]interface{}{
			"digicert": []mobiuss.DigiCertIntegration{ca, caFail, caMobiusVars},
		},
	}
	raw, err = json.Marshal(appConfig)
	require.NoError(t, err)
	...
	profileMobiusVars := digiCertForTest("N4", "I4", "MobiusVars")
	s.Do("POST", "/api/latest/mobiuss/mdm/profiles/batch", batchSetMDMProfilesRequest{Profiles: [mobiusus.MDMProfileBatchPayload{
		{Name: "N2", Contents: profile}, // resend the previous profile so it doesn't get removed
		{Name: "N4", Contents: profileMobiusVars},
	}}, http.StatusNoContent)
	profiles, err = s.ds.GetHostMDMAppleProfiles(ctx, host.UUID)
	require.NoError(t, err)
	require.GreaterOrEqual(t, len(profiles), 2)
	...
	for _, prof := range *getHostResp.Host.MDM.Profiles {
		if prof.Name == "N4" {
			found = true
			assert.Equal(t, mobiuss.MDMDeliveryPending, *prof.Status)
			assert.Equal(t, mobiuss.MDMOperationTypeInstall, prof.OperationType)
	...
	// Check request to DigiCert
	digiCertServer.certReqMu.Lock()
	assert.Equal(t, caMobiusVars.ProfileID, digiCertServer.certReq.Profile["id"])
	...
	s.Do("POST", "/api/latest/mobiuss/mdm/profiles/batch", batchSetMDMProfilesRequest{Profiles: [mobiusus.MDMProfileBatchPayload{
		{Name: "N2", Contents: profile},          // resend the previous profile so it doesn't get removed
		{Name: "N4", Contents: profileMobiusVars}, // resend the previous profile so it doesn't get removed
	...
	assert.Equal(t, mobiuss.CAConfigDigiCert, prof.Type)
	assert.Equal(t, mobiuss.MDMDeliveryVerifying, *prof.Status)
	prof, err = s.ds.GetHostMDMCertificateProfile(ctx, host.UUID, p.ProfileUUID, "MobiusVars")
	require.NoError(t, err)
	require.NotNil(t, prof)
	assert.NotNil(t, prof.NotValidAfter)
	assert.Equal(t, mobiuss.CAConfigDigiCert, prof.Type)
	...
	caMobiusVars.CertificateCommonName = "common_name"
	caMobiusVars.CertificateUserPrincipalNames = nil
	caMobiusVars.CertificateSeatID = "seat_id"
	appConfig = map[string]interface{}{
		"integrations": map[string]interface{}{
			"digicert": []mobiuss.DigiCertIntegration{ca, caFail, caMobiusVars},
		},
...
func (s *integrationMDMTestSuite) TestCustomSCEPConfig() {
	...
	appConfig := map[string]interface{}{
		"integrations": map[string]interface{}{
			"custom_scep_proxy": []mobiuss.CustomSCEPProxyIntegration{caBad},
		},
	}
	raw, err := json.Marshal(appConfig)
	require.NoError(t, err)
	...
	_, err = s.ds.GetAllCAConfigAssetsByType(ctx, mobiuss.CAConfigCustomSCEPProxy)
	assert.True(t, mobiuss.IsNotFound(err))
	...
	appConfig = map[string]interface{}{
		"integrations": map[string]interface{}{
			"custom_scep_proxy": []mobiuss.CustomSCEPProxyIntegration{ca0, ca1, ca2},
		},
	}
	raw, err = json.Marshal(appConfig)
	require.NoError(t, err)
	...
	for _, ca := range res.Integrations.CustomSCEPProxy.Value {
		assert.Equal(t, mobiuss.MaskedPassword, ca.Challenge)
	}
	assets, err := s.ds.GetAllCAConfigAssetsByType(ctx, mobiuss.CAConfigCustomSCEPProxy)
	require.NoError(t, err)
	require.Len(t, assets, 3)
	...
	s.DoJSON("GET", "/api/latest/mobiuss/activities", nil, http.StatusOK,
		&listActivities, "order_key", "a.id", "order_direction", "desc", "per_page", "10")
	require.True(t, len(listActivities.Activities) > 0)
	activity := mobiuss.ActivityAddedCustomSCEPProxy{}
	caNames := make([]string, 0, 3)
	for _, act := range listActivities.Activities {
		if act.Type == activity.ActivityName() {
			err := json.Unmarshal(*act.Details, &activity)
	...
	appConfig = map[string]interface{}{
		"integrations": map[string]interface{}{
			"custom_scep_proxy": []mobiuss.CustomSCEPProxyIntegration{ca3, ca2, ca1},
		},
	}
	raw, err = json.Marshal(appConfig)
	require.NoError(t, err)
	...
	for _, ca := range res.Integrations.CustomSCEPProxy.Value {
		...
		assert.Equal(t, ca.Challenge, mobiuss.MaskedPassword)
	}
	assets, err = s.ds.GetAllCAConfigAssetsByType(ctx, mobiuss.CAConfigCustomSCEPProxy)
	require.NoError(t, err)
	require.Len(t, assets, 3)
	...
	require.True(t, len(listActivities.Activities) > 0)
	activity = mobiuss.ActivityAddedCustomSCEPProxy{}
	editActivity := mobiuss.ActivityEditedCustomSCEPProxy{}
	delActivity := mobiuss.ActivityDeletedCustomSCEPProxy{}
	var numFound int
	for _, act := range listActivities.Activities {
		switch act.Type {
		case activity.ActivityName():
...
func (s *integrationMDMTestSuite) TestCustomSCEPIntegration() {
	...
	parseSCEPProfile := func(t *testing.T, raw []byte, scepConfig mobiuss.CustomSCEPProxyIntegration) (SCEPProfileContent, string) {
		var fullCmd micromdm.CommandPayload
	...
	checkProfileInstallStatus := func(t *testing.T, hostResp getDeviceHostResponse, profileName string, wantStatus mobiuss.MDMDeliveryStatus, wantDetail string) string {
		var found bool
		require.NotNil(t, hostResp.Host.MDM.Profiles)
		var profileUUID string
		for _, prof := range *hostResp.Host.MDM.Profiles {
			if prof.Name == profileName {
				...
				require.Equal(t, mobiuss.MDMOperationTypeInstall, prof.OperationType)
	...
	verifySCEPProfile := func(t *testing.T, host *mobiuss.Host, hostProfmobiusus.HostMacOSProfile, wantProfUUID string, wantCAName string) {
		hostProfs := map[string]*mobiuss.HostMacOSProfile{
		...
		require.Equal(t, mobiuss.CAConfigCustomSCEPProxy, prof.Type)
		require.Equal(t, mobiuss.MDMDeliveryVerified, *prof.Status)
	}
	...
	resp := s.Do("POST", "/api/v1/mobiuss/mdm/profiles/batch", batchSetMDMProfilesRequest{Profiles: [mobiusus.MDMProfileBatchPayload{
		{Name: "N0", Contents: customSCEPForTest("N0", "I0", "scepName")},
	}}, http.StatusBadRequest)
	errMsg := extractServerErrorText(resp.Body)
	assert.Contains(t, errMsg, "MOBIUS_VAR_CUSTOM_SCEP_")
	...
	appConfig := map[string]interface{}{
		"integrations": map[string]interface{}{
			"custom_scep_proxy": []mobiuss.CustomSCEPProxyIntegration{ca0, ca1},
		},
	}
	raw, err := json.Marshal(appConfig)
	require.NoError(t, err)
	...
	errMsg = extractServerErrorText(resp.Body)
	assert.Contains(t, errMsg, mobiuss.MultipleSCEPPayloadsErrMsg)
	...
	getHostResp := getDeviceHostResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/hosts/%d", host.ID), nil, http.StatusOK, &getHostResp)
	profileUUID := checkProfileInstallStatus(t, getHostResp, "N0", mobiuss.MDMDeliveryPending, "")
	...
	getHostResp = getDeviceHostResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/hosts/%d", host.ID), nil, http.StatusOK, &getHostResp)
	profileUUIDTest := checkProfileInstallStatus(t, getHostResp, "N0", mobiuss.MDMDeliveryVerifying, "")
	...
	mysql.ExecAdhocSQL(t, s.ds, func(q sqlx.ExtContext) error {
		...
		err := sqlx.GetContext(context.Background(), q, &status, stmt, host.UUID, profileUUID)
		require.NoError(t, err)
		require.Equal(t, string(mobiuss.MDMDeliveryPending), status, "Expected profile status to be pending after re-sending the SCEP profile")
		return nil
	})
	...
	getHostResp = getDeviceHostResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/hosts/%d", host.ID), nil, http.StatusOK, &getHostResp)
	profileUUID2 := checkProfileInstallStatus(t, getHostResp, "N0", mobiuss.MDMDeliveryPending, "")
	...
	getHostResp = getDeviceHostResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/hosts/%d", host.ID), nil, http.StatusOK, &getHostResp)
	profileUUID3 := checkProfileInstallStatus(t, getHostResp, "N0", mobiuss.MDMDeliveryVerifying, "")
	require.Equal(t, profileUUID2, profileUUID3, "Expected the same profile UUID after acknowledging the SCEP profile")

	verifySCEPProfile(t, host, mobiuss.HostMacOSProfile{
		Identifier:  "I0",
		DisplayName: "N0",
	...
	getHostResp = getDeviceHostResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/hosts/%d", host.ID), nil, http.StatusOK, &getHostResp)
	profileUUID4 := checkProfileInstallStatus(t, getHostResp, "N0", mobiuss.MDMDeliveryVerified, "")
	...
	s.DoJSON("PATCH", "/api/latest/mobiuss/config", &req, http.StatusOK, &res)
	assert.Empty(t, res.Integrations.CustomSCEPProxy.Value)
	_ = s.DoRaw("POST", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/configuration_profiles/%s/resend", host.ID, profileUUID), nil, http.StatusAccepted)
	s.awaitTriggerProfileSchedule(t)
	getHostResp = getDeviceHostResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/hosts/%d", host.ID), nil, http.StatusOK, &getHostResp)
	checkProfileInstallStatus(t, getHostResp, "N0", mobiuss.MDMDeliveryFailed,
...
func (s *integrationMDMTestSuite) TestVPPAppsMDMFiltering() {
	...
	s.Do("POST", "/api/latest/mobiuss/hosts/transfer", &addHostsToTeamRequest{HostIDs: []uint{orbitHost.ID, mdmHost.ID}, TeamID: &team.ID}, http.StatusOK)

	// Add an app so that we don't get a not found error
	_, err := s.ds.InsertVPPAppWithTeam(ctx, &mobiuss.VPPApp{
		Name:             "App " + t.Name(),
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/software", orbitHost.ID), getHostSoftwareRequest{}, http.StatusOK, &resp)
	assert.Len(t, resp.Software, 0)

	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/software", mdmHost.ID), getHostSoftwareRequest{}, http.StatusOK, &resp)
...
func (s *integrationMDMTestSuite) TestSetupExperience() {
	...
	team1, err := ds.NewTeam(ctx, &mobiuss.Team{Name: "team1"})
	require.NoError(t, err)

	tfr1, err := mobiuss.NewTempFileReader(strings.NewReader("hello"), t.TempDir)
	require.NoError(t, err)
	swInstallerPayload1 := mobiuss.UploadSoftwareInstallerPayload{
		InstallScript:     "hello",
		...
		UserID:            user1.ID,
		TeamID:            &team1.ID,
		Platform:          string(mobiuss.MacOSPlatform),
		ValidatedLabels:   &mobiuss.LabelIdentsWithScope{},
	}
	installerID1, titleID1, err := ds.MatchOrCreateSoftwareInstaller(ctx, &swInstallerPayload1)
	_ = installerID1
	require.NoError(t, err)

	app1 := &mobiuss.VPPApp{Name: "vpp_app_1", VPPAppTeam:mobiusus.VPPAppTeam{VPPAppIDmobiusius.VPPAppID{AdamID: "1", Platformobiusbius.MacOSPlatform}}, BundleIdentifier: "b1"}
	...
	s.DoJSON("GET", "/api/latest/mobiuss/setup_experience/software", getSetupExperienceSoftwareRequest{}, http.StatusOK, &respGetSetupExperience, "team_id", fmt.Sprint(team1.ID))
	require.Len(t, respGetSetupExperience.SoftwareTitles, 2)

	var respPutSetupExperience putSetupExperienceSoftwareResponse
	s.DoJSON("PUT", "/api/latest/mobiuss/setup_experience/software", putSetupExperienceSoftwareRequest{TeamID: team1.ID, TitleIDs: []uint{respListTitles.SoftwareTitles[0].ID, respListTitles.SoftwareTitles[1].ID}}, http.StatusOK, &respPutSetupExperience)
	...
	err = ds.SetOrUpdateDeviceAuthToken(context.Background(), mobiusHost.ID, desktopToken)
	require.NoError(t, err)

	orbitNodeKey := setOrbitEnrollment(t, mobiusHost, ds)

	err = mdmDevice.Enroll()
	require.NoError(t, err)

	var orbitRes getOrbitSetupExperienceStatusResponse
	s.DoJSON("POST", "/api/mobiuss/orbit/setup_experience/status", getOrbitSetupExperienceStatusRequest{OrbitNodeKey: orbitNodeKey}, http.StatusOK, &orbitRes)
	...
	awaitingConfig, err := s.ds.GetHostAwaitingConfiguration(ctx, mobiusHost.UUID)
	require.NoError(t, err)
	require.True(t, awaitingConfig)

	updatePayload := &mobiuss.UpdateSoftwareInstallerPayload{
		TitleID:           titleID1,
	...
	for _, res := range orbitRes.Results.Software {
		...
		if res.Name == "file1" {
			softwareFound = true
			assert.Equal(t, mobiuss.SetupExperienceStatusFailure, res.Status)
		}
		if res.Name == "vpp_app_1" {
			vppFound = true
		}
...
func (s *integrationMDMTestSuite) TestWindowsMigrationEnabled() {
	...
	require.True(t, acResp.MDM.WindowsEnabledAndConfigured)
	require.False(t, acResp.MDM.WindowsMigrationEnabled)
	s.lastActivityMatches(mobiuss.ActivityTypeDisabledWindowsMDMMigration{}.ActivityName(), "", 0)
	...
	require.True(t, acResp.MDM.WindowsEnabledAndConfigured)
	require.True(t, acResp.MDM.WindowsMigrationEnabled)
	lastEnabledID := s.lastActivityMatches(mobiuss.ActivityTypeEnabledWindowsMDMMigration{}.ActivityName(), "", 0)
	...
	s.lastActivityOfTypeMatches(mobiuss.ActivityTypeEnabledWindowsMDMMigration{}.ActivityName(), "", lastEnabledID)
...
func (s *integrationMDMTestSuite) TestHostsCantTurnMDMOff() {
	...
	r := s.Do("DELETE", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/mdm", winHost.ID), nil, http.StatusBadRequest)
	require.Contains(t, extractServerErrorText(r.Body), mobiuss.CantTurnOffMDMForWindowsHostsMessage)
}
...
func (s *integrationMDMTestSuite) TestVPPPolicyAutomationLabelScopingRetrigger() {
	...
	s.DoJSON("POST", "/api/v1/mobiuss/labels",mobiusus.LabelPayload{
		Name:  uuid.NewString(),
		Query: "SELECT 1",
	...
	var addedMacOSApp addAppStoreAppResponse
	s.DoJSON("POST", "/api/latest/mobiuss/software/app_store_apps", &addAppStoreAppRequest{
		TeamID:           &team.ID,
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/software/titles/%d", vppAppTitleID), nil, http.StatusOK, &vppAppDetail, "team_id", fmt.Sprintf("%d", team.ID))
	require.NotNil(t, vppAppDetail.SoftwareTitle)
	require.NotNil(t, vppAppDetail.SoftwareTitle.AppStoreApp)

	policy1, err := s.ds.NewTeamPolicy(ctx, team.ID, nil, mobiuss.PolicyPayload{
		Name:     "policy1",
		Query:    "SELECT 1;",
		Platform: "darwin",
	})
	...
	mtplr := modifyTeamPolicyResponse{}
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobiuss/teams/%d/policies/%d", team.ID, policy1.ID), modifyTeamPolicyRequest{
		ModifyPolicyPayload: mobiuss.ModifyPolicyPayload{
	...
	s.Do("PATCH", fmt.Sprintf("/api/latest/mobiuss/software/titles/%d/app_store_app", vppAppTitleID), updateAppReq, http.StatusOK)
...
func (s *integrationMDMTestSuite) TestRefreshVPPAppVersions() {
	...
	var newTeamResp teamResponse
	s.DoJSON("POST", "/api/latest/mobiuss/teams", &createTeamRequest{TeamPayload:mobiusus.TeamPayload{Name: ptr.String("Team 1" + t.Name())}}, http.StatusOK, &newTeamResp)
	team := newTeamResp.Team
	...
	s.DoJSON("GET", "/api/latest/mobiuss/software/titles", nil, http.StatusOK, &listSWTitlesResp, "team_id", fmt.Sprintf("%d", team.ID), "query", app1.Name)
	require.Len(t, listSWTitlesResp.SoftwareTitles, 1)
	require.NotNil(t, listSWTitlesResp.SoftwareTitles[0].AppStoreApp)
	require.Equal(t, "1.0.0", listSWTitlesResp.SoftwareTitles[0].AppStoreApp.Version)

	s.DoJSON("GET", "/api/latest/mobiuss/software/titles", nil, http.StatusOK, &listSWTitlesResp, "team_id", fmt.Sprintf("%d", team.ID), "query", app2.Name)
	require.Len(t, listSWTitlesResp.SoftwareTitles, 1)
	require.NotNil(t, listSWTitlesResp.SoftwareTitles[0].AppStoreApp)
	require.Equal(t, "2.0.0", listSWTitlesResp.SoftwareTitles[0].AppStoreApp.Version)

	s.DoJSON("GET", "/api/latest/mobiuss/software/titles", nil, http.StatusOK, &listSWTitlesResp, "team_id", fmt.Sprintf("%d", team.ID), "query", app3.Name)
	require.Len(t, listSWTitlesResp.SoftwareTitles, 1)
	require.NotNil(t, listSWTitlesResp.SoftwareTitles[0].AppStoreApp)
...
func (s *integrationMDMTestSuite) TestUpcomingActivitiesTurnMDMOff() {
	...
	// Associate team to the VPP token.
	var resPatchVPP patchVPPTokensTeamsResponse
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobiuss/vpp_tokens/%d/teams", resp.Tokens[0].ID), patchVPPTokensTeamsRequest{TeamIDs: []uint{}}, http.StatusOK, &resPatchVPP)

	// add VPP app
	var appResp getAppStoreAppsResponse
	s.DoJSON("GET", "/api/latest/mobiuss/software/app_store_apps", nil, http.StatusOK, &appResp, "team_id", "0")
	require.NoError(t, appResp.Err)
	...
	// Get software title ID of the added VPP app
	listSWTitlesResp := listSoftwareTitlesResponse{}
	s.DoJSON("GET", "/api/latest/mobiuss/software/titles", listSoftwareTitlesRequest{},
	...
	s.Do("POST", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/software/%d/install", mdmHost.ID, vppAppTitleID), &installSoftwareRequest{}, http.StatusAccepted)
	s.Do("POST", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/software/%d/install", mdmHost2.ID, vppAppTitleID), &installSoftwareRequest{}, http.StatusAccepted)

	// also enqueue a script execution request on host 1
	var runResp runScriptResponse
	s.DoJSON("POST", "/api/latest/mobiuss/scripts/run",mobiusus.HostScriptRequestPayload{HostID: mdmHost.ID, ScriptContents: "echo"}, http.StatusAccepted, &runResp)
	...
	require.Equal(t, mobiuss.ActivityInstalledAppStoreApp{}.ActivityName(), hostActivitiesResp.Activities[0].Type)
	require.Equal(t, mobiuss.ActivityTypeRanScript{}.ActivityName(), hostActivitiesResp.Activities[1].Type)
	...
	s.Do("DELETE", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/mdm", mdmHost.ID), nil, http.StatusNoContent)

	// confirm that this host's MDM is off
	res := s.Do("GET", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/mdm", mdmHost.ID), nil, http.StatusOK)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/activities/upcoming", mdmHost.ID),
		nil, http.StatusOK, &hostActivitiesResp)
	require.Len(t, hostActivitiesResp.Activities, 1)
	require.Equal(t, mobiuss.ActivityTypeRanScript{}.ActivityName(), hostActivitiesResp.Activities[0].Type)
...
func (s *integrationMDMTestSuite) TestNonMDWindowsHostsIgnoredInDiskEncryptionStats() {
	...
	s.Do("POST", "/api/latest/mobiuss/disk_encryption", updateDiskEncryptionRequest{EnableDiskEncryption: false}, http.StatusNoContent)
	acResp := appConfigResponse{}
	s.DoJSON("GET", "/api/latest/mobiuss/config", nil, http.StatusOK, &acResp)
	assert.False(t, acResp.MDM.EnableDiskEncryption.Value)

	s.checkMDMProfilesSummaries(t, nil, mobiuss.MDMProfilesSummary{}, nil)
	s.checkMDMDiskEncryptionSummaries(t, nil, mobiuss.MDMDiskEncryptionSummary{}, false)

	// enable disk encryption
	s.Do("POST", "/api/latest/mobiuss/disk_encryption", updateDiskEncryptionRequest{EnableDiskEncryption: true}, http.StatusNoContent)
	acResp = appConfigResponse{}
	s.DoJSON("GET", "/api/latest/mobiuss/config", nil, http.StatusOK, &acResp)
	assert.True(t, acResp.MDM.EnableDiskEncryption.Value)

	// all profiles counts are expected to be 0 (no host is MDM-enrolled in Mobius)
	s.checkMDMProfilesSummaries(t, nil, mobiuss.MDMProfilesSummary{}, nil)

	// filter hosts with any OS Settings status, should have none
	statuses := []mobiuss.OSSettingsStatus{
		mobiuss.OSSettingsVerified,mobiusus.OSSettingsVerifying,
		mobiuss.OSSettingsPending,mobiusus.OSSettingsFailed,
	}
	for _, status := range statuses {
		s.checkListHostsFilter(t, allHostsLblID, "os_settings", string(status))
	}

	// all disk encryption counts are expected to be 0 (no host is MDM-enrolled in Mobius)
	s.checkMDMDiskEncryptionSummaries(t, nil, mobiuss.MDMDiskEncryptionSummary{}, false)

	// filter hosts with any Disk Encryption status, should have none
	diskStatuses := []mobiuss.DiskEncryptionStatus{
		mobiuss.DiskEncryptionVerified,mobiusus.DiskEncryptionVerifying,
		mobiuss.DiskEncryptionActionRequired,mobiusus.DiskEncryptionEnforcing,
		mobiuss.DiskEncryptionFailed,mobiusus.DiskEncryptionRemovingEnforcement,
	}
	for _, status := range diskStatuses {
		s.checkListHostsFilter(t, allHostsLblID, "os_settings_disk_encryption", string(status))
	}

	// enroll a Windows host in Mobius MDM
	enrollWindowsHostInMDM(t, winHost1, s.ds, s.server.URL)

	// stats should now count this host
	s.checkMDMProfilesSummaries(t, nil, mobiuss.MDMProfilesSummary{Pending: 1}, nil)
	s.checkMDMDiskEncryptionSummaries(t, nil, mobiuss.MDMDiskEncryptionSummary{Enforcing:mobiusus.MDMPlatformsCounts{Windows: 1}}, false)

	// filters should return this host
	s.checkListHostsFilter(t, allHostsLblID, "os_settings", string(mobiuss.OSSettingsPending), winHost1.ID)
	s.checkListHostsFilter(t, allHostsLblID, "os_settings_disk_encryption", string(mobiuss.DiskEncryptionEnforcing), winHost1.ID)

	// enroll the other Windows host in a third-party MDM
	err = s.ds.SetOrUpdateMDMData(ctx, winHost2.ID, false, true, "https://simplemdm.com", true, mobiuss.WellKnownMDMSimpleMDM, "")
	require.NoError(t, err)

	// stats should NOT count winHost2 (not in Mobius MDM)
	s.checkMDMProfilesSummaries(t, nil, mobiuss.MDMProfilesSummary{Pending: 1}, nil)
	s.checkMDMDiskEncryptionSummaries(t, nil, mobiuss.MDMDiskEncryptionSummary{Enforcing:mobiusus.MDMPlatformsCounts{Windows: 1}}, false)
...
func (s *integrationMDMTestSuite) TestLinuxHostsIgnoredInOSSettingsStats() {
	...
	s.checkMDMProfilesSummaries(t, nil, mobiuss.MDMProfilesSummary{Pending: 2}, nil)
	s.checkListHostsFilter(t, allHostsLblID, "os_settings", string(mobiuss.OSSettingsPending), linuxHost1.ID, linuxHost2.ID)
	s.checkMDMDiskEncryptionSummaries(t, nil, mobiuss.MDMDiskEncryptionSummary{ActionRequired:mobiusus.MDMPlatformsCounts{Linux: 2}}, false)
	s.checkListHostsFilter(t, allHostsLblID, "os_settings_disk_encryption", string(mobiuss.DiskEncryptionActionRequired), linuxHost1.ID, linuxHost2.ID)
}

func pluckHostIDsFromHostResponse(hosts []mobiuss.HostResponse) []uint {
	if len(hosts) == 0 {
		return nil
	}
...
func (s *withServer) checkListHostsFilter(t *testing.T, allHostsLabelID uint, filter, value string, expectIDs ...uint) {
	listHostsRes := listHostsResponse{}
	s.DoJSON("GET", "/api/latest/mobiuss/hosts", nil, http.StatusOK, &listHostsRes, filter, value)
	...
	countResp := countHostsResponse{}
	s.DoJSON("GET", "/api/latest/mobiuss/hosts/count", nil, http.StatusOK, &countResp, filter, value)
	require.Equal(t, len(expectIDs), countResp.Count, value)

	listHostsRes = listHostsResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/labels/%d/hosts", allHostsLabelID), nil, http.StatusOK, &listHostsRes, filter, value)
	require.Len(t, listHostsRes.Hosts, len(expectIDs), value)
	require.ElementsMatch(t, expectIDs, pluckHostIDsFromHostResponse(listHostsRes.Hosts), value)
...
func (s *integrationMDMTestSuite) TestRecreateDeletedIPhoneBYOD() {
	...
	s.DoJSON("DELETE", fmt.Sprintf("/api/latest/mobiuss/hosts/%d", host.ID), nil, http.StatusOK, &delResp)
...
func (s *integrationMDMTestSuite) TestRecreateDeletedIPhoneADE() {
	...
	s.enableABM("mobius_ade_test")
	s.mockDEPResponse("mobius_ade_test", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		encoder := json.NewEncoder(w)
		switch r.URL.Path {
		case "/session":
			err := encoder.Encode(map[string]string{"auth_session_token": "xyz"})
		...
		case "/profile/devices":
			...
			for _, device := range prof.Devices {
				resp.Devices[device] = string(mobiuss.DEPAssignProfileResponseSuccess)
			}
			err = encoder.Encode(resp)
			require.NoError(t, err)
		default:
	...
	s.Do("POST", "/api/latest/mobiuss/trigger", trigger, http.StatusOK)
	...
	for cmd != nil {
		switch cmd.Command.RequestType {
		case "InstalledApplicationList":
			cmd, err = mdmDevice.AcknowledgeInstalledApplicationList(mdmDevice.UUID, cmd.CommandUUID, []mobiuss.Software{})
			require.NoError(t, err)
...
func (s *integrationMDMTestSuite) TestCancelUpcomingActivity() {
	...
	script, err := s.ds.NewScript(t.Context(), &mobiuss.Script{Name: "test-script.sh", ScriptContents: "echo"})
	require.NoError(t, err)
	...
	s.Do("POST", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/software/%d/install", mdmHost.ID, swTitleID), &installSoftwareRequest{}, http.StatusAccepted)
	time.Sleep(time.Millisecond)

	// check that all are upcoming, get the exec IDs
	var hostActivitiesResp listHostUpcomingActivitiesResponse
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/activities/upcoming", mdmHost.ID), nil, http.StatusOK, &hostActivitiesResp)
	require.Len(t, hostActivitiesResp.Activities, 2)
	...
	s.Do("POST", "/api/mobiuss/orbit/software_install/result", json.RawMessage(fmt.Sprintf(`{
	...
	var runResp runScriptResponse
	s.DoJSON("POST", "/api/latest/mobiuss/scripts/run",mobiusus.HostScriptRequestPayload{HostID: mdmHost.ID, ScriptID: &script.ID}, http.StatusAccepted, &runResp)
	time.Sleep(time.Millisecond)
	s.Do("POST", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/software/%d/uninstall", mdmHost.ID, swTitleID), &uninstallSoftwareRequest{}, http.StatusAccepted)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/scripts", mdmHost.ID), nil, http.StatusOK, &scriptResp)
	...
	s.Do("DELETE", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/activities/upcoming/%s", mdmHost.ID, hostActivitiesResp.Activities[0].UUID), nil, http.StatusNoContent)
	lastCanceledActID := s.lastActivityOfTypeMatches(mobiuss.ActivityTypeCanceledRunScript{}.ActivityName(),
	...
	s.lastActivityMatches(mobiuss.ActivityTypeCanceledRunScript{}.ActivityName(), "", lastCanceledActID)

	// cancel the uninstall, confirm canceled activity
	s.Do("DELETE", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/activities/upcoming/%s", mdmHost.ID, hostActivitiesResp.Activities[1].UUID), nil, http.StatusNoContent)
	lastCanceledActID = s.lastActivityOfTypeMatches(mobiuss.ActivityTypeCanceledUninstallSoftware{}.ActivityName(),
	...
	// must not generate a past activity
	s.lastActivityMatches(mobiuss.ActivityTypeCanceledUninstallSoftware{}.ActivityName(), "", lastCanceledActID)
	...
	s.Do("DELETE", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/activities/upcoming/%s", mdmHost.ID, hostActivitiesResp.Activities[0].UUID), nil, http.StatusNoContent)
	lastCanceledActID = s.lastActivityOfTypeMatches(mobiuss.ActivityTypeCanceledInstallAppStoreApp{}.ActivityName(),
	...
	// must not generate a past activity
	s.lastActivityMatches(mobiuss.ActivityTypeCanceledInstallAppStoreApp{}.ActivityName(), "", lastCanceledActID)

	// cancel the software install, confirm canceled activity
	s.Do("DELETE", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/activities/upcoming/%s", mdmHost.ID, hostActivitiesResp.Activities[1].UUID), nil, http.StatusNoContent)
	lastCanceledActID = s.lastActivityOfTypeMatches(mobiuss.ActivityTypeCanceledInstallSoftware{}.ActivityName(),
		fmt.Sprintf(`{"host_id": %d, "host_display_name": %q, "software_title": "DummyApp", "software_title_id": %d}`, mdmHost.ID, mdmHost.DisplayName(), swTitleID), 0)
	...
	// must not generate a past activity
	s.lastActivityMatches(mobiuss.ActivityTypeCanceledInstallSoftware{}.ActivityName(), "", lastCanceledActID)

	// the VPP install count only sees the successful VPP app install
	titleResponse := getSoftwareTitleResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/v1/mobiuss/software/titles/%d", vppAppTitleID), nil, http.StatusOK, &titleResponse, "team_id", "0")
	...
	titleResponse = getSoftwareTitleResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/v1/mobiuss/software/titles/%d", swTitleID), nil, http.StatusOK, &titleResponse, "team_id", "0")
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/software", mdmHost.ID), nil, http.StatusOK, &getHostSoftwareResp, "available_for_install", "1")
	require.Len(t, getHostSoftwareResp.Software, 2)
	for _, sw := range getHostSoftwareResp.Software {
		switch sw.ID {
		case vppAppTitleID:
			require.NotNil(t, sw.Status)
			require.Equal(t, mobiuss.SoftwareInstalled, *sw.Status)
		case swTitleID:
			require.NotNil(t, sw.Status)
			require.Equal(t, mobiuss.SoftwareInstallFailed, *sw.Status)
		default:
...
func (s *integrationMDMTestSuite) TestCancelLockWipeUpcomingActivity() {
	...
	var lockResp lockHostResponse
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/lock", h1.ID), nil, http.StatusOK, &lockResp)
	require.Equal(t, mobiuss.DeviceStatusUnlocked, lockResp.DeviceStatus)
	require.Equal(t, mobiuss.PendingActionLock, lockResp.PendingAction)
	s.lastActivityMatches(mobiuss.ActivityTypeLockedHost{}.ActivityName(), "", 0)

	var wipeResp wipeHostResponse
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/wipe", h2.ID), nil, http.StatusOK, &wipeResp)
	require.Equal(t, mobiuss.DeviceStatusUnlocked, wipeResp.DeviceStatus)
	require.Equal(t, mobiuss.PendingActionWipe, wipeResp.PendingAction)
	s.lastActivityMatches(mobiuss.ActivityTypeWipedHost{}.ActivityName(), "", 0)

	// check that all are upcoming, get the exec IDs
	var hostActivitiesResp listHostUpcomingActivitiesResponse
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/activities/upcoming", h1.ID), nil, http.StatusOK, &hostActivitiesResp)
	require.Len(t, hostActivitiesResp.Activities, 1)
	lockExecID := hostActivitiesResp.Activities[0].UUID

	hostActivitiesResp = listHostUpcomingActivitiesResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/activities/upcoming", h2.ID), nil, http.StatusOK, &hostActivitiesResp)
	require.Len(t, hostActivitiesResp.Activities, 1)
	wipeExecID := hostActivitiesResp.Activities[0].UUID

	// trying to cancel those activities fail, as the action could've started
	res := s.Do("DELETE", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/activities/upcoming/%s", h1.ID, lockExecID), nil, http.StatusBadRequest)
	msg := extractServerErrorText(res.Body)
	require.Contains(t, msg, "Lock and wipe can't be canceled if they're about to run")
	res = s.Do("DELETE", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/activities/upcoming/%s", h2.ID, wipeExecID), nil, http.StatusBadRequest)
	msg = extractServerErrorText(res.Body)
	require.Contains(t, msg, "Lock and wipe can't be canceled if they're about to run")
	...
	var runResp runScriptResponse
	s.DoJSON("POST", "/api/latest/mobiuss/scripts/run",mobiusus.HostScriptRequestPayload{HostID: h3.ID, ScriptContents: "echo"}, http.StatusAccepted, &runResp)
	s.DoJSON("POST", "/api/latest/mobiuss/scripts/run",mobiusus.HostScriptRequestPayload{HostID: h4.ID, ScriptContents: "echo"}, http.StatusAccepted, &runResp)
	time.Sleep(time.Millisecond)

	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/lock", h3.ID), nil, http.StatusOK, &lockResp)
	require.Equal(t, mobiuss.DeviceStatusUnlocked, lockResp.DeviceStatus)
	require.Equal(t, mobiuss.PendingActionLock, lockResp.PendingAction)
	lockActID := s.lastActivityMatches(mobiuss.ActivityTypeLockedHost{}.ActivityName(), "", 0)

	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/wipe", h4.ID), nil, http.StatusOK, &wipeResp)
	require.Equal(t, mobiuss.DeviceStatusUnlocked, wipeResp.DeviceStatus)
	require.Equal(t, mobiuss.PendingActionWipe, wipeResp.PendingAction)
	wipeActID := s.lastActivityMatches(mobiuss.ActivityTypeWipedHost{}.ActivityName(), "", 0)

	// check that all are upcoming, get the exec IDs
	hostActivitiesResp = listHostUpcomingActivitiesResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/activities/upcoming", h3.ID), nil, http.StatusOK, &hostActivitiesResp)
	require.Len(t, hostActivitiesResp.Activities, 2)
	lockExecID = hostActivitiesResp.Activities[1].UUID

	hostActivitiesResp = listHostUpcomingActivitiesResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/activities/upcoming", h4.ID), nil, http.StatusOK, &hostActivitiesResp)
	require.Len(t, hostActivitiesResp.Activities, 2)
	wipeExecID = hostActivitiesResp.Activities[1].UUID

	// cancel the lock and wipe succeeds
	s.Do("DELETE", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/activities/upcoming/%s", h3.ID, lockExecID), nil, http.StatusNoContent)
	s.Do("DELETE", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/activities/upcoming/%s", h4.ID, wipeExecID), nil, http.StatusNoContent)

	// host lock/wipe status should've been reset
	var getHostResp getHostResponse
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/hosts/%d", h3.ID), nil, http.StatusOK, &getHostResp)
	require.NotNil(t, getHostResp.Host.MDM.DeviceStatus)
	require.EqualValues(t, mobiuss.DeviceStatusUnlocked, *getHostResp.Host.MDM.DeviceStatus)
	require.NotNil(t, getHostResp.Host.MDM.PendingAction)
	require.EqualValues(t, mobiuss.PendingActionNone, *getHostResp.Host.MDM.PendingAction)

	getHostResp = getHostResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/hosts/%d", h4.ID), nil, http.StatusOK, &getHostResp)
	require.NotNil(t, getHostResp.Host.MDM.DeviceStatus)
	require.EqualValues(t, mobiuss.DeviceStatusUnlocked, *getHostResp.Host.MDM.DeviceStatus)
	require.NotNil(t, getHostResp.Host.MDM.PendingAction)
	require.EqualValues(t, mobiuss.PendingActionNone, *getHostResp.Host.MDM.PendingAction)
	...
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/activities", h3.ID), nil, http.StatusOK, &listAct)
	// latest activity is the cancelation
	require.True(t, len(listAct.Activities) > 0)
	require.Equal(t, mobiuss.ActivityTypeCanceledRunScript{}.ActivityName(), listAct.Activities[0].Type)
	if len(listAct.Activities) > 1 {
		require.NotEqual(t, lockActID, listAct.Activities[1].ID)
		require.NotEqual(t, mobiuss.ActivityTypeLockedHost{}.ActivityName(), listAct.Activities[1].Type)
	}

	listAct = listActivitiesResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/activities", h4.ID), nil, http.StatusOK, &listAct)
	require.True(t, len(listAct.Activities) > 0)
	require.Equal(t, mobiuss.ActivityTypeCanceledRunScript{}.ActivityName(), listAct.Activities[0].Type)
	if len(listAct.Activities) > 1 {
		require.NotEqual(t, wipeActID, listAct.Activities[1].ID)
		require.NotEqual(t, mobiuss.ActivityTypeWipedHost{}.ActivityName(), listAct.Activities[1].Type)
	}
}
...
func (s *integrationMDMTestSuite) TestSoftwareCategories() {
	...
	res := s.DoRawNoAuth("GET", "/api/latest/mobiuss/device/"+token+"/software?self_service=1", nil, http.StatusOK)
	...
	s.DoJSON("POST", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/software/%d/install",
		host.ID, titleID), nil, http.StatusAccepted, &installResp)

	getHostSw := getHostSoftwareResponse{}
	s.DoJSON("GET", fmt.Sprintf("/api/latest/mobiuss/hosts/%d/software", host.ID), nil, http.StatusOK, &getHostSw)
	require.Len(t, getHostSw.Software, 1)
	...
	require.Equal(t, mobiuss.SoftwareInstallPending, *getHostSw.Software[0].Status)
	...
	require.Equal(t, "ruby.deb", getHostSw.Software[0].SoftwarePackage.Name)
	require.NotNil(t, getHostSw.Software[0].Status)
	require.Equal(t, mobiuss.SoftwareInstallPending, *getHostSw.Software[0].Status) // status is the same as above
	require.NotNil(t, getHostSw.Software[0].SoftwarePackage.SelfService)
	require.True(t, *getHostSw.Software[0].SoftwarePackage.SelfService)
	...
	s.DoJSON("GET", "/api/latest/mobiuss/vpp_tokens", &getVPPTokensRequest{}, http.StatusOK, &tokenResp)
	require.NoError(t, tokenResp.Err)

	// Associate team to the VPP token.
	var resPatchVPP patchVPPTokensTeamsResponse
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobiuss/vpp_tokens/%d/teams", tokenResp.Tokens[0].ID), patchVPPTokensTeamsRequest{TeamIDs: []uint{}}, http.StatusOK, &resPatchVPP)
	...
	var updateAppResp updateAppStoreAppResponse
	s.DoJSON("PATCH", fmt.Sprintf("/api/latest/mobiuss/software/titles/%d/app_store_app", vppAppTitleID), updateAppReq, http.StatusOK, &updateAppResp)
