...
func (s *integrationSSOTestSuite) TestGetSSOSettings() {
	...
	s.DoJSON("GET", "/api/v1/mobius/sso", nil, http.StatusOK, &resGet)
	require.True(t, resGet.Settings.SSOEnabled)

	// Javascript redirect URLs are forbidden
	var resIni initiateSSOResponse
	s.DoJSON("POST", "/api/v1/mobius/sso", map[string]string{"relay_url": "javascript:alert(1)"}, http.StatusBadRequest, &resIni)

	// initiate an SSO auth
	resIni = initiateSSOResponse{}
	s.DoJSON("POST", "/api/v1/mobius/sso", map[string]string{}, http.StatusOK, &resIni)
	...
	assert.Equal(t, "Mobius", authReq.ProviderName)
...
func (s *integrationSSOTestSuite) TestSSOInvalidMetadataURL() {
	...
	badMetadataUrl := "https://www.mobiusmdm.com"
	acResp := appConfigResponse{}
	s.DoJSON(
	...
	s.DoJSON("POST", "/api/v1/mobius/sso", map[string]string{}, expectedStatus, &resIni)
}
...
func (s *integrationSSOTestSuite) TestSSOLogin() {
	...
	require.Condition(t, func() bool {
		for _, a := range activitiesResp.Activities {
			if (a.Type == mobius.ActivityTypeUserLoggedIn{}.ActivityName()) && *a.ActorEmail == auth.UserID() {
				return true
			}
		}
		return false
