...
type runLiveQueryResponse struct {
	...
	Results []mobius.QueryCampaignResult `json:"live_query_results"`
}
...
func runOneLiveQueryEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
	...
	queryResults := []mobius.QueryResult{}
...
func runLiveQueryEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func runLiveQueryOnHostEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func runLiveQueryOnHostByIDEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func runLiveQueryOnHost(svc mobius.Service, ctx context.Context, host *mobius.HostLite, query string) (mobius.Errorer, error) {
...
func runLiveQuery(ctx context.Context, svc mobius.Service, queryIDs []uint, query string, hostIDs []uint) (
	[]mobius.QueryCampaignResult, int, error,
) {
	// The period used here should always be less than the request timeout for any load
	// balancer/proxy between Mobius and the API client.
	period := os.Getenv("MOBIUS_LIVE_QUERY_REST_PERIOD")
	if period == "" {
		period = "25s"
	}
...
func (svc *Service) RunLiveQueryDeadline(
	...
	resultsCh := make(chan mobius.QueryCampaignResult)
	...
	for _, queryID := range queryIDs {
		...
		go func() {
			...
			campaign, err := svc.NewDistributedQueryCampaign(ctx, queryString, queryIDPtr, mobius.HostTargets{HostIDs: hostIDs})
			if err != nil {
				...
				resultsCh <- mobius.QueryCampaignResult{QueryID: queryID, Error: ptr.String(err.Error()), Err: err}
				return
			}
			...
			defer func() {
				...
				if err != nil {
					...
					resultsCh <- mobius.QueryCampaignResult{
						QueryID: queryID,
			...
			var results []mobius.QueryResult
		...
		loop:
			for {
				...
				case res := <-readChan:
					switch res := res.(type) {
					case mobius.DistributedQueryResult:
						results = append(results, mobius.QueryResult{HostID: res.Host.ID, Rows: res.Rows, Error: res.Error})
						counterMutex.Lock()
					...
					case error:
						resultsCh <- mobius.QueryCampaignResult{QueryID: queryID, Error: ptr.String(res.Error()), Err: res}
						return
					}
				case <-timeout:
					break loop
			...
			resultsCh <- mobius.QueryCampaignResult{QueryID: queryID, Results: results}
		}()
	}
	...
	var results []mobius.QueryCampaignResult
...
func (svc *Service) GetCampaignReader(ctx context.Context, campaign *mobius.DistributedQueryCampaign) (<-chan interface{}, context.CancelFunc, error) {
	...
	campaign.Status = mobius.QueryRunning
...
func (svc *Service) CompleteCampaign(ctx context.Context, campaign *mobius.DistributedQueryCampaign) error {
