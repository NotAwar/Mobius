...
import (
	...
	"github.com/notawar/mobiuss/v4/server"
	"github.com/notawar/mobiuss/v4/server/contexts/ctxerr"
	"github.com/notawar/mobiuss/v4/server/contexts/logging"
	"github.com/notawar/mobiuss/v4/servermobiusus"
	mdmlifecycle "github.com/notawar/mobiuss/v4/server/mdm/lifecycle"
	...
	mdm_types "github.com/notawar/mobiuss/v4/servermobiusus"
...
// isMobiusdPresentOnDevice checks if the device requires Mobiusd to be deployed
func (svc *Service) isMobiusdPresentOnDevice(ctx context.Context, deviceID string) (bool, error) {
	...
	// This means that mobiusdaemon might not be installed
...
func (svc *Service) enqueueInstallMobiusdCommand(ctx context.Context, deviceID string) error {
	...
	if len(secrets) == 0 {
		level.Warn(svc.logger).Log("msg", "unable to find a global enroll secret to install mobiusdaemon")
		return nil
	}
	...
	mobiusdMetadata, err := mobiusdbase.GetMetadata()
	if err != nil {
		level.Warn(svc.logger).Log("msg", "unable to get mobiusdaemon-base metadata")
		return nil
	}
	...
	mobiusURL := appCfg.ServerSettings.ServerURL
...
<Add>
	...
	<Item>
		<Target>
			<LocURI>` + syncml.MobiusdWindowsInstallerGUID + `</LocURI>
		</Target>
	</Item>
</Add>`)
	...
	// MobiusURL and MobiusSecret properties are passed to the Mobius MSI
	rawExecCmd := []byte(`
<Exec>
	...
	<Item>
		...
		<Data>
			...
			<Product Version="1.0.0.0">
				<Download>
					<ContentURLList>
						<ContentURL>` + mobiusdMetadata.MSIURL + `</ContentURL>
					</ContentURLList>
				</Download>
				<Validation>
					<FileHash>` + mobiusdMetadata.MSISha256 + `</FileHash>
				</Validation>
				<Enforcement>
					<CommandLine>/quiet MOBIUS_URL="` + mobiusURL + `" MOBIUS_SECRET="` + globalEnrollSecret + `" ENABLE_SCRIPTS="True"</CommandLine>
					<TimeOut>10</TimeOut>
...
`)
	...
	if err := svc.ds.MDMWindowsInsertCommandForHosts(ctx, []string{deviceID}, addMobiusdCmd); err != nil {
	...
	if err := svc.ds.MDMWindowsInsertCommandForHosts(ctx, []string{deviceID}, execMobiusCmd); err != nil {
		return ctxerr.Wrap(ctx, err, "insert exec command to install mobiusdaemon")
	}
...
func (svc *Service) processNewSessionAlert(ctx context.Context, messageID string, deviceID string, cmd mdm_types.ProtoCmdOperation) error {
	...
	if !mobiusdPresent {
		return svc.enqueueInstallMobiusdCommand(ctx, deviceID)
	}
...
func (svc *Service) getPendingMDMCmds(ctx context.Context, deviceID string) ([]*mdm_types.SyncMLCmd, error) {
	...
	for _, pendingCmd := range pendingCmds {
		// The raw MDM command may contain a $MOBIUS_SECRET_XXX, the value of which should never be exposed or stored unencrypted.
