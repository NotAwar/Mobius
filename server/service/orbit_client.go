...
// OrbitClient exposes the Orbit API to communicate with the Mobius server.
type OrbitClient struct {
	*baseClient
	nodeKeyFilePath string
	enrollSecret    string
	hostInfo        mobius.OrbitHostInfo
	...
	ConfigReceivers []mobius.OrbitConfigReceiver
...
// requestWithExternal is used to make requests to Mobius or external URLs. If external is true, the pathOrURL
...
func (oc *OrbitClient) requestWithExternal(verb string, pathOrURL string, params interface{}, resp interface{}, external bool) error {
	...
	if os.Getenv("MOBIUSDAEMON_TEST_HTTPTRACE") == "1" {
...
//   - addr is the address of the Mobius server.
//   - orbitHostInfo is the host system information used for enrolling to Mobius.
...
func NewOrbitClient(
	...
	enrollSecret string,
	mobiusClientCert *tls.Certificate,
	orbitHostInfo mobius.OrbitHostInfo,
	onGetConfigErrFns *OnGetConfigErrFuncs,
) (*OrbitClient, error) {
	orbitCapabilities := mobius.GetOrbitClientCapabilities()
	bc, err := newBaseClient(addr, insecureSkipVerify, rootCA, "", mobiusClientCert, orbitCapabilities)
	if err != nil {
		return nil, err
	}
...
// connection causing errors in the client (e.g. https://github.com/notawar/mobius/issues/18783).
...
func (oc *OrbitClient) RegisterConfigReceiver(cr mobius.OrbitConfigReceiver) {
...
// GetConfig returns the Orbit config fetched from Mobius server for this instance of OrbitClient.
// Since this method is called in multiple places, we use a cache with configCacheTTL time-to-live
// to reduce traffic to the Mobius server.
// Upon network errors, this method will retry the get config request (every 30 seconds).
func (oc *OrbitClient) GetConfig() (*mobius.OrbitConfig, error) {
	...
	if now.After(oc.configCache.lastUpdated.Add(configCacheTTL)) {
		verb, path := "POST", "/api/mobius/orbit/config"
		var (
			resp mobius.OrbitConfig
			err  error
		)
...
func (oc *OrbitClient) SetOrUpdateDeviceToken(deviceAuthToken string) error {
	verb, path := "POST", "/api/mobius/orbit/device_token"
...
func (oc *OrbitClient) SetOrUpdateDeviceMappingEmail(email string) error {
	verb, path := "PUT", "/api/mobius/orbit/device_mapping"
...
// GetHostScript returns the script fetched from Mobius server to run on this
// host.
func (oc *OrbitClient) GetHostScript(execID string) (*mobius.HostScriptResult, error) {
	verb, path := "POST", "/api/mobius/orbit/scripts/request"
...
func (oc *OrbitClient) SaveHostScriptResult(result *mobius.HostScriptResultPayload) error {
	verb, path := "POST", "/api/mobius/orbit/scripts/result"
...
func (oc *OrbitClient) GetInstallerDetails(installId string) (*mobius.SoftwareInstallDetails, error) {
	verb, path := "POST", "/api/mobius/orbit/software_install/details"
...
func (oc *OrbitClient) SaveInstallerResult(payload *mobius.HostSoftwareInstallResultPayload) error {
	verb, path := "POST", "/api/mobius/orbit/software_install/result"
...
func (oc *OrbitClient) DownloadSoftwareInstaller(installerID uint, downloadDirectory string, progressFunc func(n int)) (string, error) {
	verb, path := "POST", "/api/mobius/orbit/software_install/package?alt=media"
	resp := FileResponse{
...
func (oc *OrbitClient) Ping() error {
	verb, path := "HEAD", "/api/mobius/orbit/ping"
...
func (oc *OrbitClient) enroll() (string, error) {
	verb, path := "POST", "/api/mobius/orbit/enroll"
...
// otherwise it enrolls the host with Mobius and saves the node key to disk
...
func orbitEnrollRetryInterval() time.Duration {
	interval := os.Getenv("MOBIUSDAEMON_ENROLL_RETRY_INTERVAL")
	if interval != "" {
...
func (oc *OrbitClient) SetOrUpdateDiskEncryptionKey(diskEncryptionStatus mobius.OrbitHostDiskEncryptionKeyPayload) error {
	verb, path := "POST", "/api/mobius/orbit/disk_encryption_key"
...
func (oc *OrbitClient) GetSetupExperienceStatus() (*mobius.SetupExperienceStatusPayload, error) {
	verb, path := "POST", "/api/mobius/orbit/setup_experience/status"
...
func (oc *OrbitClient) SendLinuxKeyEscrowResponse(lr luks.LuksResponse) error {
	verb, path := "POST", "/api/mobius/orbit/luks_data"
