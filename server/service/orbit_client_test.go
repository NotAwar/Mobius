...
func TestGetConfig(t *testing.T) {
	t.Run(
		"config cache", func(t *testing.T) {
			oc := OrbitClient{}
			oc.configCache.config = &mobius.OrbitConfig{}
...
func clientWithConfig(cfg *mobius.OrbitConfig) *OrbitClient {
...
func TestConfigReceiverCalls(t *testing.T) {
	...
	rfunc1 := mobius.OrbitConfigReceiverFunc(func(cfg *mobius.OrbitConfig) error {
	...
	rfunc2 := mobius.OrbitConfigReceiverFunc(func(cfg *mobius.OrbitConfig) error {
		if !reflect.DeepEqual(cfg.Flags, testmsg) {
			return errors.New("not equal testmsg")
		}
		called2 = true
	...
	client := clientWithConfig(&mobius.OrbitConfig{Flags: testmsg})
...
func TestConfigReceiverErrors(t *testing.T) {
	...
	efunc1 := mobius.OrbitConfigReceiverFunc(func(cfg *mobius.OrbitConfig) error {
		return err1
	})
	efunc2 := mobius.OrbitConfigReceiverFunc(func(cfg *mobius.OrbitConfig) error {
		return err2
	})
	// Make sure we don't get stuck or crash on receiver panic
	pfunc := mobius.OrbitConfigReceiverFunc(func(cfg *mobius.OrbitConfig) error {
		panic("woah")
	})

	client := clientWithConfig(&mobius.OrbitConfig{})
...
func TestExecuteConfigReceiversCancel(t *testing.T) {
	...
	cfunc := mobius.OrbitConfigReceiverFunc(func(cfg *mobius.OrbitConfig) error {
		calls1++
	...
	rfunc := mobius.OrbitConfigReceiverFunc(func(cfg *mobius.OrbitConfig) error {
		calls2++
		return nil
	})
