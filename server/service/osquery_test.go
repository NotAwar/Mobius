...
import (
	...
	mobiusLogging "github.com/notawar/mobius/v4/server/contexts/logging"
	"github.com/notawar/mobius/v4/server/contexts/viewer"
	"github.com/notawar/mobius/v4/server/datastore/mysqlredis"
	...
	mockresult "github.com/notawar/mobius/v4/server/mock/mockresult"
	...
	"github.com/notawar/mobius/v4/server/service/redis_policy_set"
...
func TestGetClientConfig(t *testing.T) {
	...
	ds.ListPacksForHostFunc = func(ctx context.Context, hid uint) ([]*mobius.Pack, error) {
		return []*mobius.Pack{}, nil
	}
	ds.ListScheduledQueriesInPackFunc = func(ctx context.Context, pid uint) (mobius.ScheduledQueryList, error) {
		tru := true
		fals := false
		fortytwo := uint(42)
		switch pid {
	...
	ds.ListScheduledQueriesForAgentsFunc = func(ctx context.Context, teamID *uint, hostID *uint, queryReportsDisabled bool) ([]*mobius.Query, error) {
		if teamID == nil {
			return nil, nil
		}
		return []*mobius.Query{
	...
	ds.AppConfigFunc = func(ctx context.Context) (*mobius.AppConfig, error) {
		return &mobius.AppConfig{AgentOptions: ptr.RawMessage(json.RawMessage(`{"config":{"options":{"baz":"bar"}}}`))}, nil
	}
	...
	ds.HostLiteFunc = func(ctx context.Context, id uint) (*mobius.Host, error) {
		...
		return &mobius.Host{ID: id}, nil
	}

	svc, ctx := newTestService(t, ds, nil, nil)

	ctx1 := hostctx.NewContext(ctx, &mobius.Host{ID: 1})
	ctx2 := hostctx.NewContext(ctx, &mobius.Host{ID: 2})
	ctx3 := hostctx.NewContext(ctx, &mobius.Host{ID: 1, TeamID: ptr.Uint(1)})
...
var allDetailQueries = osquery_utils.GetDetailQueries(
	context.Background(),
	config.MobiusConfig{Vulnerabilities: config.VulnerabilitiesConfig{DisableWinOSVulnerabilities: true}},
	nil,
	&mobius.Features{
...
func TestEnrollAgent(t *testing.T) {
	ds := new(mock.Store)
	ds.VerifyEnrollSecretFunc = func(ctx context.Context, secret string) (*mobius.EnrollSecret, error) {
		switch secret {
		case "valid_secret":
			return &mobius.EnrollSecret{Secret: "valid_secret", TeamID: ptr.Uint(3)}, nil
		default:
			return nil, errors.New("not found")
		}
	}
	ds.EnrollHostFunc = func(ctx context.Context, isMDMEnabled bool, osqueryHostId, hUUID, hSerial, nodeKey string, teamID *uint, cooldown time.Duration) (*mobius.Host, error) {
...
func TestEnrollAgentEnforceLimit(t *testing.T) {
	runTest := func(t *testing.T, pool mobius.RedisPool) {
		...
		ds.VerifyEnrollSecretFunc = func(ctx context.Context, secret string) (*mobius.EnrollSecret, error) {
			switch secret {
			case "valid_secret":
				return &mobius.EnrollSecret{Secret: "valid_secret"}, nil
			default:
				return nil, errors.New("not found")
			}
		}
		...
		svc, ctx := newTestService(t, redisWrapDS, nil, nil, &TestServerOpts{
			...
			License:           &mobius.LicenseInfo{DeviceCount: maxHosts},
		})
...
func TestAuthenticateHost(t *testing.T) {
	...
	var gotKey string
	host := mobius.Host{ID: 1, Hostname: "foobar"}
	ds.LoadHostByNodeKeyFunc = func(ctx context.Context, nodeKey string) (*mobius.Host, error) {
		gotKey = nodeKey
		return &host, nil
	}
	var gotHostIDs []uint
	...
	host = mobius.Host{ID: 7, Hostname: "foobar"}
...
func TestSubmitResultLogsToLogDestination(t *testing.T) {
	...
	ds.OverwriteQueryResultRowsFunc = func(ctx context.Context, rows []*mobius.ScheduledQueryResultRow, maxQueryReportRows int) error {
		if len(rows) == 0 {
			return nil
		}
	...
	validLogResults := []string{
		...
		// Mobius doesn't know of this query, so this result should be streamed as is (This is to support streaming results for osquery nodes that are configured outside of Mobius, e.g. `--config_plugin=filesystem`).
		...
		// The "name" field has invalid format, so this result will be streamed as is (This is to support streaming results for osquery nodes that are configured outside of Mobius, e.g. `--config_plugin=filesystem`).
...
func TestSaveResultLogsToQueryReports(t *testing.T) {
	...
	results := []*mobius.ScheduledQueryResult{
		{
	...
	discardDataFalse := map[string]*mobius.Query{
		"pack/Global/Uptime": {
			ID:          1,
			DiscardData: true,
			Logging:     mobius.LoggingSnapshot,
	...
	serv.saveResultLogsToQueryReports(ctx, results, discardDataFalse, mobius.DefaultMaxQueryReportRows)
	assert.False(t, ds.OverwriteQueryResultRowsFuncInvoked)

	// Happy Path: Results saved
	discardDataTrue := map[string]*mobius.Query{
		"pack/Global/Uptime": {
			ID:          1,
			DiscardData: false,
			Logging:     mobius.LoggingSnapshot,
	...
	serv.saveResultLogsToQueryReports(ctx, results, discardDataTrue, mobius.DefaultMaxQueryReportRows)
...
func TestGetMostRecentResults(t *testing.T) {
	tests := []struct {
		name     string
		input    []*mobius.ScheduledQueryResult
		expected []*mobius.ScheduledQueryResult
	}{
		{
			name: "basic test",
			input: []*mobius.ScheduledQueryResult{
...
func TestHostDetailQueries(t *testing.T) {
	...
	ds.AppConfigFunc = func(ctx context.Context) (*mobius.AppConfig, error) {
		return &mobius.AppConfig{Features: mobius.Features{
...
func TestQueriesAndHostFeatures(t *testing.T) {
	...
	ds.TeamFeaturesFunc = func(ctx context.Context, id uint) (*mobius.Features, error) {
		switch id {
		case uint(1):
	...
	ds.PolicyQueriesForHostFunc = func(ctx context.Context, host *mobius.Host) (map[string]string, error) {
		return map[string]string{}, nil
	}
	...
	t.Run("free license", func(t *testing.T) {
		...
		require.NoError(t, err)
		require.NotContains(t, queries, "mobius_detail_query_users")
		require.NotContains(t, queries, "mobius_detail_query_software_macos")
		require.NotContains(t, queries, "mobius_detail_query_software_linux")
		require.NotContains(t, queries, "mobius_detail_query_software_windows")
	...
	t.Run("premium license", func(t *testing.T) {
		...
		queries, _, _, err = svc.GetDistributedQueries(ctx)
		require.NoError(t, err)
		require.Contains(t, queries, "mobius_detail_query_users")
		require.NotContains(t, queries, "mobius_detail_query_software_macos")
		require.NotContains(t, queries, "mobius_detail_query_software_linux")
		require.NotContains(t, queries, "mobius_detail_query_software_windows")
		...
		require.NoError(t, err)
		require.NotContains(t, queries, "mobius_detail_query_users")
		require.Contains(t, queries, "mobius_detail_query_software_macos")
	})
}
...
func TestGetDistributedQueriesEmptyQuery(t *testing.T) {
	...
	ds.UpdateHostFunc = func(ctx context.Context, gotHost *mobius.Host) error {
		return nil
	}
	ds.AppConfigFunc = func(ctx context.Context) (*mobius.AppConfig, error) {
		return &mobius.AppConfig{Features: mobius.Features{EnableHostUsers: true}}, nil
	}
	ds.PolicyQueriesForHostFunc = func(ctx context.Context, host *mobius.Host) (map[string]string, error) {
...
func TestLabelQueries(t *testing.T) {
	...
	require.NoError(t, err)
	// +1 for the mobius_no_policies_wildcard query.
	...
	queries, discovery, acc, err = svc.GetDistributedQueries(ctx)
	require.NoError(t, err)
	require.Len(t, queries, 1) // mobius_no_policies_wildcard query
	verifyDiscovery(t, queries, discovery)
	assert.Zero(t, acc)
	...
	ds.RecordLabelQueryExecutionsFunc = func(ctx context.Context, host *mobius.Host, results map[uint]*bool, t time.Time, deferred bool) error {
		gotHost = host
		gotResults = results
		gotTime = t
		return nil
	...
	err = svc.SubmitDistributedQueryResults(
		...
		map[string]mobius.OsqueryStatus{},
		map[string]string{},
		map[string]*mobius.Stats{},
	)
	require.NoError(t, err)
	...
	queries, discovery, acc, err = svc.GetDistributedQueries(ctx)
	require.NoError(t, err)
	// +3 for label queries, +1 for the mobius_no_policies_wildcard query.
...
func TestDetailQueriesWithEmptyStrings(t *testing.T) {
	...
	ds.LabelQueriesForHostFunc = func(context.Context, *mobius.Host) (map[string]string, error) {
		return map[string]string{}, nil
	}
	ds.PolicyQueriesForHostFunc = func(ctx context.Context, host *mobius.Host) (map[string]string, error) {
		return map[string]string{}, nil
	...
	queries, discovery, acc, err := svc.GetDistributedQueries(ctx)
	require.NoError(t, err)
	// +1 due to 'windows_update_history', +1 due to mobius_no_policies_wildcard query.
	...
	resultJSON := `
{
  "mobius_detail_query_network_interface_windows": [
    {
      "address": "192.168.0.1",
      "mac": "5f:3d:4b:10:25:82"
    }
  ],
  "mobius_detail_query_os_version": [
    {
      "platform": "darwin",
      "build": "15G1004",
      "major": "10",
  ...
  "mobius_detail_query_osquery_info": [
    {
      "build_distro": "10.10",
  ...
  "mobius_detail_query_system_info": [
    {
  ...
  "mobius_detail_query_osquery_flags": [
    {
  ...
  "mobius_detail_query_orbit_info": [
    {
      "name": "version",
      "value": "42"
    },
...
`
	...
	var results mobius.OsqueryDistributedQueryResults
	...
	require.NoError(
		t, svc.SubmitDistributedQueryResults(ctx, results, map[string]mobius.OsqueryStatus{}, map[string]string{}, map[string]*mobius.Stats{}),
	)
	...
	// +1 due to mobius_no_policies_wildcard query.
...
func TestDetailQueries(t *testing.T) {
	...
	queries, discovery, acc, err := svc.GetDistributedQueries(ctx)
	require.NoError(t, err)
	// +1 for mobius_no_policies_wildcard
	...
	resultJSON := `
{
"mobius_detail_query_network_interface": [
    {
        "address": "192.168.0.1",
...
"mobius_detail_query_software_macos": [
    {
      "name": "app1",
      "version": "1.0.0",
      "source": "source1"
...
"mobius_detail_query_disk_space_unix": [
	{
...
"mobius_detail_query_munki_info": [
	{
		"version": "3.4.5"
	}
],
...
`
	...
	var gotSoftware []mobius.Software
	...
	ds.UpdateHostSoftwareInstalledPathsFunc = func(ctx context.Context, hostID uint, paths map[string]struct{}, result *mobius.UpdateHostSoftwareDBResult) error {
		return nil
	}
	...
	assert.Equal(t, "computer.local", gotHost.Hostname)
	assert.Equal(t, "uuid", gotHost.UUID)
	// The hardware serial should not have updated because return value was -1. See: https://github.com/notawar/mobius/issues/19789
	...
	assert.Equal(t, mobius.HostUser{
		Uid:       1234,
		Username:  "user1",
		Type:      "sometype",
		GroupName: "somegroup",
	...
	assert.Equal(t, []mobius.Software{
		{
			Name:    "app1",
			Version: "1.0.0",
			Source:  "source1",
	...
	queries, discovery, acc, err = svc.GetDistributedQueries(ctx)
	require.NoError(t, err)
	// +1 mobius_no_policies_wildcard query
	require.Equal(t, len(expectedDetailQueriesForPlatform(host.Platform))+1, len(queries), distQueriesMapKeys(queries))
	verifyDiscovery(t, queries, discovery)
	assert.Zero(t, acc)
}
...
func TestMDMQueries(t *testing.T) {
	...
	expectedMDMQueries := []struct {
		...
		{"mobius_detail_query_mdm_config_profiles_darwin", "macos_profiles"},
		{"mobius_detail_query_mdm_disk_encryption_key_file_darwin", "filevault_prk"},
		{"mobius_detail_query_mdm_disk_encryption_key_file_lines_darwin", "file_lines"},
	}

	mdmEnabled := true
	ds.AppConfigFunc = func(ctx context.Context) (*mobius.AppConfig, error) {
		return &mobius.AppConfig{MDM: mobius.MDM{EnabledAndConfigured: mdmEnabled}}, nil
	}
...
func TestNewDistributedQueryCampaign(t *testing.T) {
	...
	var gotTargets []*mobius.DistributedQueryCampaignTarget
	...
	campaign, err := svc.NewDistributedQueryCampaign(viewerCtx, q, nil, mobius.HostTargets{HostIDs: []uint{2}, LabelIDs: []uint{1}})
	require.NoError(t, err)
	assert.Equal(t, gotQuery.ID, gotCampaign.QueryID)
	assert.Equal(t, []*mobius.DistributedQueryCampaignTarget{
		{
...
func TestDistributedQueryResults(t *testing.T) {
	...
	require.NoError(t, err)
	// +1 for the distributed query for campaign ID 42, +1 for windows update history, +1 for the mobius_no_policies_wildcard query.
	...
	go func() {
		...
		select {
		case val := <-readChan:
			if res, ok := val.(mobius.DistributedQueryResult); ok {
	...
	err = svc.SubmitDistributedQueryResults(
		hostCtx, results, map[string]mobius.OsqueryStatus{}, map[string]string{}, stats,
	)
	require.NoError(t, err)
...
func TestIngestDistributedQueryOrphanedCampaignLoadError(t *testing.T) {
	...
	err := svc.ingestDistributedQuery(context.Background(), host, "mobius_distributed_query_42", []map[string]string{}, "", nil)
	require.Error(t, err)
...
func TestIngestDistributedQueryOrphanedCloseError(t *testing.T) {
	...
	ds.SaveDistributedQueryCampaignFunc = func(ctx context.Context, campaign *mobius.DistributedQueryCampaign) error {
...
func TestUpdateHostIntervals(t *testing.T) {
	...
	testCases := []struct {
		name                  string
		initIntervals         mobius.HostOsqueryIntervals
		finalIntervals        mobius.HostOsqueryIntervals
		configOptions         json.RawMessage
		updateIntervalsCalled bool
	}{
		{
			"Both updated",
			mobius.HostOsqueryIntervals{
				ConfigTLSRefresh: 60,
			},
			mobius.HostOsqueryIntervals{
		...
		{
			"Mobius not managing distributed_interval",
			mobius.HostOsqueryIntervals{
				ConfigTLSRefresh:    60,
				DistributedInterval: 11,
			},
	...
	for _, tt := range testCases {
		t.Run(tt.name, func(t *testing.T) {
			ctx := hostctx.NewContext(ctx, &mobius.Host{
				ID:                  1,
			...
			ds.AppConfigFunc = func(ctx context.Context) (*mobius.AppConfig, error) {
				return &mobius.AppConfig{AgentOptions: ptr.RawMessage(json.RawMessage(`{"config":` + string(tt.configOptions) + `}`))}, nil
			}

			updateIntervalsCalled := false
			ds.UpdateHostOsqueryIntervalsFunc = func(ctx context.Context, hostID uint, intervals mobius.HostOsqueryIntervals) error {
				if hostID != 1 {
					return errors.New("not found")
				}
...
func TestAuthenticationErrors(t *testing.T) {
	ms := new(mock.Store)
	ms.LoadHostByNodeKeyFunc = func(ctx context.Context, nodeKey string) (*mobius.Host, error) {
		return nil, nil
	}
...
func TestDistributedQueriesLogsManyErrors(t *testing.T) {
	...
	lCtx := &mobiusLogging.LoggingContext{}
	ctx = mobiusLogging.NewContext(ctx, lCtx)
	ctx = hostctx.NewContext(ctx, host)
...
func TestObserversCanOnlyRunDistributedCampaigns(t *testing.T) {
	...
	viewerCtx := viewer.NewContext(ctx, viewer.Viewer{
		User: &mobius.User{ID: 0, GlobalRole: ptr.String(mobius.RoleObserver)},
	})
	...
	_, err = svc.NewDistributedQueryCampaign(viewerCtx, "", ptr.Uint(42), mobius.HostTargets{HostIDs: []uint{2}, LabelIDs: []uint{1}})
	require.Error(t, err)
...
func TestTeamMaintainerCanRunNewDistributedCampaigns(t *testing.T) {
	...
	viewerCtx := viewer.NewContext(ctx, viewer.Viewer{
		User: &mobius.User{ID: 99, Teams: []mobius.UserTeam{{Team: mobius.Team{ID: 123}, Role: mobius.RoleMaintainer}}},
	})
	...
	_, err := svc.NewDistributedQueryCampaign(viewerCtx, q, nil, mobius.HostTargets{HostIDs: []uint{2}, LabelIDs: []uint{1}, TeamIDs: []uint{123}})
	require.NoError(t, err)
}
...
func TestPolicyQueries(t *testing.T) {
	...
	ds.RecordPolicyQueryExecutionsFunc = func(ctx context.Context, gotHost *mobius.Host, results map[uint]*bool, updated time.Time, deferred bool) error {
...
func TestPolicyWebhooks(t *testing.T) {
	...
	cmpSets := func(expSets map[uint][]mobius.PolicySetHost) error {
	...
	assert.Eventually(t, func() bool {
		err = cmpSets(map[uint][]mobius.PolicySetHost{
			3: {{
				ID:       host.ID,
	...
	err = failingPolicySet.RemoveHosts(1, []mobius.PolicySetHost{{
		ID:       host.ID,
		Hostname: host.Hostname,
	}})
	require.NoError(t, err)
...
func TestLiveQueriesFailing(t *testing.T) {
	...
	queries, discovery, _, err := svc.GetDistributedQueries(ctx)
	require.NoError(t, err)
	// +1 to account for the mobius_no_policies_wildcard query.
	require.Equal(t, len(expectedDetailQueriesForPlatform(host.Platform))+1, len(queries), distQueriesMapKeys(queries))
	verifyDiscovery(t, queries, discovery)
...
func distQueriesMapKeys(m map[string]string) []string {
	...
	for k := range m {
		keys = append(keys, strings.TrimPrefix(k, "mobius_detail_query_"))
	}
	sort.Strings(keys)
	return keys
}
...
func TestPreProcessSoftwareResults(t *testing.T) {
	...
	pythonPackageTwo := map[string]string{
		...
		"source":         "python_packages",
		"vendor":         "",
		"installed_path": "/Users/mobiusmdm/.pyenv/versions/3.13.1/lib/python3.13/site-packages",
	}

	for _, tc := range []struct {
		name       string
		host       *mobius.Host
		resultsIn  mobius.OsqueryDistributedQueryResults
		statusesIn map[string]mobius.OsqueryStatus
		messagesIn map[string]string
		overrides  map[string]osquery_utils.DetailQuery

		resultsOut mobius.OsqueryDistributedQueryResults
	}{
		{
			name: "python packages using original query in extras adds results",

			statusesIn: map[string]mobius.OsqueryStatus{
				hostDetailQueryPrefix + "software_macos":           mobius.StatusOK,
				hostDetailQueryPrefix + "software_python_packages": mobius.StatusOK,
			},
			resultsIn: mobius.OsqueryDistributedQueryResults{
		...
		},
		{
			...
			statusesIn: map[string]mobius.OsqueryStatus{
				hostDetailQueryPrefix + "software_macos":                          mobius.StatusOK,
				hostDetailQueryPrefix + "software_python_packages_with_users_dir": mobius.StatusOK,
			},
			resultsIn: mobius.OsqueryDistributedQueryResults{
				hostDetailQueryPrefix + "software_macos": []map[string]string{
					foobarApp,
		...
		},
		{
			...
			statusesIn: map[string]mobius.OsqueryStatus{
				hostDetailQueryPrefix + "other_detail_query":         mobius.StatusOK,
				hostDetailQueryPrefix + "software_macos":             mobius.StatusOK,
				hostDetailQueryPrefix + "software_vscode_extensions": mobius.StatusOK,
			},
			resultsIn: mobius.OsqueryDistributedQueryResults{
		...
		},
		{
			...
			statusesIn: map[string]mobius.OsqueryStatus{
				hostDetailQueryPrefix + "software_macos":             mobius.StatusOK,
				hostDetailQueryPrefix + "software_vscode_extensions": mobius.OsqueryStatus(1),
			},
			resultsIn: mobius.OsqueryDistributedQueryResults{
				hostDetailQueryPrefix + "software_macos": []map[string]string{
					foobarApp,
		...
		},
		{
			...
			statusesIn: map[string]mobius.OsqueryStatus{
				hostDetailQueryPrefix + "software_macos":             mobius.OsqueryStatus(1),
				hostDetailQueryPrefix + "software_vscode_extensions": mobius.StatusOK,
			},
			resultsIn: mobius.OsqueryDistributedQueryResults{
				hostDetailQueryPrefix + "software_macos": []map[string]string{},
		...
		},
		{
			...
			statusesIn: map[string]mobius.OsqueryStatus{
				hostDetailQueryPrefix + "software_macos":      mobius.StatusOK,
				hostDetailQueryPrefix + "software_overrideMe": mobius.StatusOK,
			},
			resultsIn: mobius.OsqueryDistributedQueryResults{
				hostDetailQueryPrefix + "software_macos": []map[string]string{
					appToOverride,
		...
		},
		{
			...
			statusesIn: map[string]mobius.OsqueryStatus{
				hostDetailQueryPrefix + "software_linux": mobius.StatusOK,
			},
			resultsIn: mobius.OsqueryDistributedQueryResults{
		...
		},
		{
			...
			host: &mobius.Host{ID: 1, Platform: "debian"},
			statusesIn: map[string]mobius.OsqueryStatus{
				hostDetailQueryPrefix + "software_linux": mobius.StatusOK,
			},
			resultsIn: mobius.OsqueryDistributedQueryResults{
		...
		},
		{
			...
			host: &mobius.Host{ID: 1, Platform: "rhel"},
			statusesIn: map[string]mobius.OsqueryStatus{
				hostDetailQueryPrefix + "software_linux": mobius.StatusOK,
			},
			resultsIn: mobius.OsqueryDistributedQueryResults{
		...
		},
		{
			...
			host: &mobius.Host{ID: 1, Platform: "darwin"},
			statusesIn: map[string]mobius.OsqueryStatus{
				hostDetailQueryPrefix + "software_macos_codesign": mobius.StatusOK,
			},
			resultsIn: mobius.OsqueryDistributedQueryResults{
...
func TestDetailQueriesLinuxDistros(t *testing.T) {
	for _, linuxPlatform := range mobius.HostLinuxOSs {
...
func mockUbuntuResults() *mobius.OsqueryDistributedQueryResults {
	results := &mobius.OsqueryDistributedQueryResults{
...
func BenchmarkPreprocessUbuntuPythonPackageFilter(b *testing.B) {
	...
	statuses := &map[string]mobius.OsqueryStatus{
		hostDetailQueryPrefix + "software_linux": mobius.StatusOK,
	}

	for i := 0; i < b.N; i++ {
		preProcessSoftwareResults(&mobius.Host{ID: 1, Platform: platform}, results, statuses, nil, nil, log.NewNopLogger())
	}
}
