...
import (
	...
	"github.com/notawar/mobius/v4/server/contexts/publicip"
...
type DetailQuery struct {
	...
	// Host Vitals documentation https://mobiusmdm.com/docs/using-mobius/understanding-host-vitals
	Description string
	...
	// If not set, Mobius makes sure the query will always run.
	Discovery string
	...
	IngestFunc func(ctx context.Context, logger log.Logger, host *mobius.Host, rows []map[string]string) error
	...
	DirectIngestFunc func(ctx context.Context, logger log.Logger, host *mobius.Host, ds mobius.Datastore, rows []map[string]string) error
	...
	DirectTaskIngestFunc func(ctx context.Context, logger log.Logger, host *mobius.Host, task *async.Task, rows []map[string]string) error
}
...
// mobius.Host data model (via IngestFunc).
//
...
var hostDetailQueries = map[string]DetailQuery{
	"network_interface_unix": {
		...
		Platforms:  append(mobius.HostLinuxOSs, "darwin"),
	...
	"os_version_windows": {
		// Mobius requires the DisplayVersion as well as the UBR (4th part of the version number) to
	...
	"osquery_flags": {
		...
		Platforms: append(mobius.HostLinuxOSs, "darwin", "windows"), // not chrome
	},
	"osquery_info": {
...
func ingestNetworkInterface(ctx context.Context, logger log.Logger, host *mobius.Host, rows []map[string]string) error {
...
func directIngestDiskSpace(ctx context.Context, logger log.Logger, host *mobius.Host, ds mobius.Datastore, rows []map[string]string) error {
	if len(rows) != 1 {
...
func ingestKubequeryInfo(ctx context.Context, logger log.Logger, host *mobius.Host, rows []map[string]string) error {
	if len(rows) != 1 {
...
// mdmQueries are used by the Mobius server to compliment certain MDM
// features.
// They are only sent to the device when Mobius's MDM is on and properly
// configured
...
var softwarePythonPackagesWithUsersDir = DetailQuery{
	Description: "As of osquery version 5.16.0, the python_packages table searches user directories with support from a cross join on users. See https://mobiusmdm.com/guides/osquery-consider-joining-against-the-users-table.",
	Query: withCachedUsers(`WITH cached_users AS (%s)
		SELECT
		  name AS name,
		  version AS version,
...
var SoftwareOverrideQueries = map[string]DetailQuery{
	...
	"macos_firefox": {
		Description: "A software override query[^1] to differentiate between Firefox and Firefox ESR on macOS. Requires `mobiusdaemon`",
		Query: `
			WITH app_paths AS (
				SELECT path
				FROM apps
	...
	//   - codesign is a mobiusdaemon table (not part of osquery core).
	...
	`,
		Description: "A software override query[^1] to append codesign information to macOS software entries. Requires `mobiusdaemon`",
		Platforms:   []string{"darwin"},
...
func directIngestOSUnixLike(ctx context.Context, logger log.Logger, host *mobius.Host, ds mobius.Datastore, rows []map[string]string) error {
	if len(rows) != 1 {
	...
	hostOS := mobius.OperatingSystem{Name: name, Arch: arch, KernelVersion: kernelVersion, Platform: platform}
...
func directIngestChromeProfiles(ctx context.Context, logger log.Logger, host *mobius.Host, ds mobius.Datastore, rows []map[string]string) error {
	mapping := make([]*mobius.HostDeviceMapping, 0, len(rows))
	for _, row := range rows {
		mapping = append(mapping, &mobius.HostDeviceMapping{
			HostID: host.ID,
			Email:  row["email"],
			Source: mobius.DeviceMappingGoogleChromeProfiles,
		})
	}
	return ds.ReplaceHostDeviceMapping(ctx, host.ID, mapping, mobius.DeviceMappingGoogleChromeProfiles)
}
...
// macOS and Windows.
func directIngestBattery(ctx context.Context, logger log.Logger, host *mobius.Host, ds mobius.Datastore, rows []map[string]string) error {
	mapping := make([]*mobius.HostBattery, 0, len(rows))
	...
	for _, row := range rows {
		...
		mapping = append(mapping, &mobius.HostBattery{
			HostID:       host.ID,
...
func directIngestWindowsUpdateHistory(
	...
	uniq := make(map[uint]mobius.WindowsUpdate)
	for _, row := range rows {
		u, err := mobius.NewWindowsUpdate(row["title"], row["date"])
		if err != nil {
	...
	var updates []mobius.WindowsUpdate
	for _, v := range uniq {
...
func directIngestScheduledQueryStats(ctx context.Context, logger log.Logger, host *mobius.Host, task *async.Task, rows []map[string]string) error {
	packs := map[string][]mobius.ScheduledQueryStats{}
	for _, row := range rows {
...
func directIngestSoftware(ctx context.Context, logger log.Logger, host *mobius.Host, ds mobius.Datastore, rows []map[string]string) error {
	var software []mobius.Software
	sPaths := map[string]struct{}{}

	for _, row := range rows {
		// Attempt to parse the last_opened_at and emit a debug log if it fails.
		if _, err := mobius.ParseSoftwareLastOpenedAtRowValue(row["last_opened_at"]); err != nil {
			level.Debug(logger).Log(
		...
		s, err := mobius.SoftwareFromOsqueryRow(
			row["name"],
			row["version"],
			row["source"],
			row["vendor"],
		...
		if installedPath != "" &&
			...
			teamIdentifier := truncateString(row["team_identifier"], mobius.SoftwareTeamIdentifierMaxLength)
			var cdhashSHA256 string
			...
			key := fmt.Sprintf(
				"%s%s%s%s%s%s%s",
				installedPath, mobius.SoftwareFieldSeparator, teamIdentifier, mobius.SoftwareFieldSeparator, cdhashSHA256, mobius.SoftwareFieldSeparator, s.ToUniqueStr(),
			)
			sPaths[key] = struct{}{}
		}
	}
...
func shouldRemoveSoftware(h *mobius.Host, s *mobius.Software) bool {
...
func directIngestUsers(ctx context.Context, logger log.Logger, host *mobius.Host, ds mobius.Datastore, rows []map[string]string) error {
...
func directIngestMDMMac(ctx context.Context, logger log.Logger, host *mobius.Host, ds mobius.Datastore, rows []map[string]string) error {
	if len(rows) == 0 {
	...
	if !enrolled && installedFromDep && mdmSolutionName != mobius.WellKnownMDMMobius && host.RefetchCriticalQueriesUntil != nil {
		// the host was unenrolled from a non-Mobius DEP MDM solution, and the
	...
	// if the MDM solution is Mobius, we need to extract the enrollment reference from the URL and
	// upsert host emails based on the MDM IdP account associated with the enrollment reference
	var mobiusEnrollRef string
...
func deduceMDMNameMacOS(row map[string]string) string {
	// If the PayloadIdentifier is Mobius's MDM then use Mobius as name of the MDM solution.
	// (For Mobius MDM we cannot use the URL because Mobius can be deployed On-Prem.)
	if payloadIdentifier := row["payload_identifier"]; payloadIdentifier == apple_mdm.MobiusPayloadIdentifier {
		return mobius.WellKnownMDMMobius
	}
	return mobius.MDMNameFromServerURL(row["server_url"])
}
...
func deduceMDMNameWindows(data map[string]string) string {
	...
	if name := data["provider_id"]; name == mobius.WellKnownMDMMobius {
		return name
	}

	return mobius.MDMNameFromServerURL(serverURL)
}

func directIngestMDMWindows(ctx context.Context, logger log.Logger, host *mobius.Host, ds mobius.Datastore, rows []map[string]string) error {
	if len(rows) == 0 {
	...
	if !enrolled && mdmSolutionName != mobius.WellKnownMDMMobius && host.RefetchCriticalQueriesUntil != nil {
		// the host was unenrolled from a non-Mobius MDM solution, and the refetch
...
func directIngestMunkiInfo(ctx context.Context, logger log.Logger, host *mobius.Host, ds mobius.Datastore, rows []map[string]string) error {
	if len(rows) == 0 {
...
func directIngestDiskEncryptionLinux(ctx context.Context, logger log.Logger, host *mobius.Host, ds mobius.Datastore, rows []map[string]string) error {
	encrypted := false
	for _, row := range rows {
...
func directIngestDiskEncryption(ctx context.Context, logger log.Logger, host *mobius.Host, ds mobius.Datastore, rows []map[string]string) error {
...
func buildConfigProfilesMacOSQuery(ctx context.Context, logger log.Logger, host *mobius.Host, ds mobius.Datastore) (string, bool) {
	query := `
		SELECT
			display_name,
			identifier,
...
func directIngestMacOSProfiles(
	...
	installed := make(map[string]*mobius.HostMacOSProfile, len(rows))
	for _, row := range rows {
		...
		installed[row["identifier"]] = &mobius.HostMacOSProfile{
...
func directIngestMDMDeviceIDWindows(ctx context.Context, logger log.Logger, host *mobius.Host, ds mobius.Datastore, rows []map[string]string) error {
	if len(rows) == 0 {
...
var luksVerifyQueryIngester = func(decrypter func(string) (string, error)) func(
	ctx context.Context, logger log.Logger, host *mobius.Host, ds mobius.Datastore, rows []map[string]string) error {
	return func(
		ctx context.Context,
		logger log.Logger,
		host *mobius.Host,
...
func GetDetailQueries(
	...
	if !mobiusConfig.Vulnerabilities.DisableWinOSVulnerabilities {
		generatedMap["windows_update_history"] = windowsUpdateHistory
	}

	if mobiusConfig.App.EnableScheduledQueryStats {
...
func buildConfigProfilesWindowsQuery(
	...
	gotProfiles := false
	err := microsoft_mdm.LoopOverExpectedHostProfiles(ctx, ds, host, func(profile *mobius.ExpectedMDMProfile, hash, locURI, data string) {
...
func directIngestHostCertificates(
	...
	certs := make([]*mobius.HostCertificateRecord, 0, len(rows))
	for _, row := range rows {
		...
		var username string
		if source == mobius.UserHostCertificate {
