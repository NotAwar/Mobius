...
func TestRedisLock(t *testing.T) {
	for _, f := range []func(*testing.T, mobius.Lock){
...
func setupRedis(t testing.TB, cluster, redir bool) mobius.Lock {
...
// only in tests.
func NewLockTest(t TestName, pool mobius.RedisPool) mobius.Lock {
	lock := &redisLock{
		pool:       pool,
		testPrefix: t.Name() + ":",
	}
	return mobius.Lock(lock)
}

func testRedisAcquireLock(t *testing.T, lock mobius.Lock) {
...
func testRedisSet(t *testing.T, lock mobius.Lock) {
	ctx := context.Background()
