...
var _ mobius.FailingPolicySet = (*redisFailingPolicySet)(nil)

// NewFailing creates a redis policy set for failing policies.
func NewFailing(pool mobius.RedisPool) *redisFailingPolicySet {
...
// only in tests.
func NewFailingTest(t TestNamer, pool mobius.RedisPool) *redisFailingPolicySet {
	return &redisFailingPolicySet{
		pool:       pool,
		testPrefix: t.Name() + ":",
	}
...
func (r *redisFailingPolicySet) AddHost(policyID uint, host mobius.PolicySetHost) error {
...
func (r *redisFailingPolicySet) ListHosts(policyID uint) ([]mobius.PolicySetHost, error) {
	hostEntries, err := r.scanPolicySet(policyID)
	if err != nil {
		return nil, err
	}
	hosts := make([]mobius.PolicySetHost, len(hostEntries))
...
func (r *redisFailingPolicySet) RemoveHosts(policyID uint, hosts []mobius.PolicySetHost) error {
	if len(hosts) == 0 {
		return nil
	}
...
func (r *redisFailingPolicySet) addHostToPolicySet(policyID uint, host mobius.PolicySetHost) error {
	conn := redis.ConfigureDoer(r.pool, r.pool.Get())
	defer conn.Close()
...
func hostEntry(host mobius.PolicySetHost) string {
	return fmt.Sprint(host.ID) + "," + host.Hostname
}

func parseHostEntry(v string) (*mobius.PolicySetHost, error) {
