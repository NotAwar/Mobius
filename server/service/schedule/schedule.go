...
type Schedule struct {
	...
	jobs   []Job
	errors mobius.CronScheduleErrors
...
type CronStatsStore interface {
	...
	GetLatestCronStats(ctx context.Context, name string) ([]mobius.CronStats, error)
	// InsertCronStats inserts cron stats for the named cron schedule
	InsertCronStats(ctx context.Context, statsType mobius.CronStatsType, name string, instance string, status mobius.CronStatsStatus) (int, error)
	// UpdateCronStats updates the status of the identified cron stats record
	UpdateCronStats(ctx context.Context, id int, status mobius.CronStatsStatus, cronErrors *mobius.CronScheduleErrors) error
}
...
// is used. This affects when the schedule starts running after Mobius is
// started, e.g. if the schedule has an interval of 1h and has no previous run
// recorded, by default its first run after Mobius starts will be in 1h.
...
func New(
	...
	sch.errors = make(mobius.CronScheduleErrors)
	return sch
}
...
func (s *Schedule) Start() {
	...
	} else if s.runOnce && prevScheduledRun.Status == mobius.CronStatsStatusCompleted {
	...
	go func() {
		...
		for {
			...
			case <-s.trigger:
				...
				s.runWithStats(mobius.CronStatsTypeTriggered)
			...
			case <-schedTicker.C:
				...
				if prevScheduledRun.Status == mobius.CronStatsStatusPending || prevTriggeredRun.Status == mobius.CronStatsStatusPending {
					// skip ahead to the next interval
				...
				s.runWithStats(mobius.CronStatsTypeScheduled)
...
func (s *Schedule) Trigger() (*mobius.CronStats, error) {
	sched, trig, err := s.GetLatestStats()
	switch {
	case err != nil:
		return nil, err
	case sched.Status == mobius.CronStatsStatusPending:
		return &sched, nil
	case trig.Status == mobius.CronStatsStatusPending:
		return &trig, nil
	default:
		// ok
	}
...
func (s *Schedule) runWithStats(statsType mobius.CronStatsType) {
	statsID, err := s.insertStats(statsType, mobius.CronStatsStatusPending)
	if err != nil {
	...
	if err := s.updateStats(statsID, mobius.CronStatsStatusCompleted); err != nil {
...
func (s *Schedule) runAllJobs() {
	...
	s.errors = make(mobius.CronScheduleErrors)
...
func (s *Schedule) GetLatestStats() (mobius.CronStats, mobius.CronStats, error) {
	var scheduled, triggered mobius.CronStats

	cs, err := s.statsStore.GetLatestCronStats(s.ctx, s.name)
	if err != nil {
		return mobius.CronStats{}, mobius.CronStats{}, err
	}
	if len(cs) > 2 {
		return mobius.CronStats{}, mobius.CronStats{}, fmt.Errorf("get latest stats expected length to be no more than two but got length: %d", len(cs))
	}

	for _, stats := range cs {
		switch stats.StatsType {
		case mobius.CronStatsTypeScheduled:
			scheduled = stats
		case mobius.CronStatsTypeTriggered:
			triggered = stats
		default:
...
func (s *Schedule) insertStats(statsType mobius.CronStatsType, status mobius.CronStatsStatus) (int, error) {
	return s.statsStore.InsertCronStats(s.ctx, statsType, s.name, s.instanceID, status)
}

func (s *Schedule) updateStats(id int, status mobius.CronStatsStatus) error {
