...
func TestNewSchedule(t *testing.T) {
	...
	jobRan := false
	s := New(ctx, "test_new_schedule", "test_instance", 1*time.Second, NopLocker{}, SetUpMockStatsStore("test_new_schedule", mobius.CronStats{
		ID:        1,
...
func TestScheduleLocker(t *testing.T) {
	...
	statsStore := SetUpMockStatsStore(name, mobius.CronStats{
		ID:        1,
		StatsType: mobius.CronStatsTypeScheduled,
		Name:      name,
		Instance:  instance,
...
func TestMultipleSchedules(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		s := New(ctx, tc.name, tc.instanceID, tc.interval, NopLocker{}, SetUpMockStatsStore(tc.name, mobius.CronStats{
			ID:        1,
			StatsType: mobius.CronStatsTypeScheduled,
			Name:      tc.name,
...
func TestMultipleJobsInOrder(t *testing.T) {
	...
	s := New(ctx, "test_schedule", "test_instance", 1000*time.Millisecond, NopLocker{}, SetUpMockStatsStore("test_schedule", mobius.CronStats{
		ID:        1,
		StatsType: mobius.CronStatsTypeScheduled,
		Name:      "test_schedule",
		Instance:  "test_instance",
...
func TestConfigReloadCheck(t *testing.T) {
	...
	jobsRun := 0
	s := New(ctx, "test_schedule", "test_instance", initialSchedInterval, NopLocker{}, SetUpMockStatsStore("test_schedule", mobius.CronStats{
		ID:        1,
		StatsType: mobius.CronStatsTypeScheduled,
		Name:      "test_schedule",
		Instance:  "test_instance",
...
func TestJobPanicRecover(t *testing.T) {
	...
	s := New(ctx, "test_schedule", "test_instance", 1*time.Second, NopLocker{}, SetUpMockStatsStore("test_schedule", mobius.CronStats{
		ID:        1,
		StatsType: mobius.CronStatsTypeScheduled,
		Name:      "test_schedule",
		Instance:  "test_instance",
...
func TestScheduleReleaseLock(t *testing.T) {
	...
	ms := SetUpMockStatsStore(name, mobius.CronStats{
		ID:        1,
		StatsType: mobius.CronStatsTypeScheduled,
		Name:      name,
		Instance:  instance,
...
func TestTriggerReleaseLock(t *testing.T) {
	...
	case <-locker.Unlocked:
		...
		statsByType := make(map[mobius.CronStatsType]mobius.CronStats)
		for _, s := range stats {
			statsByType[s.StatsType] = s
		}
		require.Len(t, statsByType, 2)
		require.Contains(t, statsByType, mobius.CronStatsTypeTriggered)
		require.Contains(t, statsByType, mobius.CronStatsTypeScheduled)

		require.Equal(t, mobius.CronStatsStatusCompleted, statsByType[mobius.CronStatsTypeTriggered].Status)
		require.Equal(t, seedStats, statsByType[mobius.CronStatsTypeScheduled])
	}
...
func TestMultipleScheduleInstancesConfigChangesDS(t *testing.T) {
	...
	_, err := ds.InsertCronStats(ctx, mobius.CronStatsTypeScheduled, name, "a", mobius.CronStatsStatusCompleted)
	require.NoError(t, err)
...
func TestTriggerMultipleInstances(t *testing.T) {
	...
	for _, c := range cases {
		...
		statsStore := SetUpMockStatsStore(c.name, mobius.CronStats{
			ID:        1,
			StatsType: mobius.CronStatsTypeScheduled,
			Name:      c.name,
