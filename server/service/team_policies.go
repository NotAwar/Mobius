...
func teamPolicyEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
	req := request.(*teamPolicyRequest)
	resp, err := svc.NewTeamPolicy(ctx, req.TeamID, mobius.NewTeamPolicyPayload{
		QueryID:                  req.QueryID,
		Name:                     req.Name,
		Query:                    req.Query,
...
func (svc Service) NewTeamPolicy(ctx context.Context, teamID uint, tp mobius.NewTeamPolicyPayload) (*mobius.Policy, error) {
...
func (svc *Service) populatePolicyInstallSoftware(ctx context.Context, p *mobius.Policy) error {
	if p.SoftwareInstallerID != nil {
		...
		p.InstallSoftware = &mobius.PolicySoftwareTitle{
...
func (svc *Service) populatePolicyRunScript(ctx context.Context, p *mobius.Policy) error {
	if p.ScriptID == nil {
		return nil
	}
	...
	p.RunScript = &mobius.PolicyScript{ID: *p.ScriptID, Name: scriptMetadata.Name}
	return nil
}

func (svc *Service) newTeamPolicyPayloadToPolicyPayload(ctx context.Context, teamID uint, p mobius.NewTeamPolicyPayload) (mobius.PolicyPayload, error) {
	softwareInstallerID, vppAppsTeamsID, err := svc.getInstallerOrVPPAppForTitle(ctx, &teamID, p.SoftwareTitleID)
	if err != nil {
		return mobius.PolicyPayload{}, err
	}
	return mobius.PolicyPayload{
		QueryID:                  p.QueryID,
		Name:                     p.Name,
		Query:                    p.Query,
		Critical:                 p.Critical,
...
func listTeamPoliciesEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
	req := request.(*listTeamPoliciesRequest)

	inheritedListOptions := mobius.ListOptions{
...
func (svc *Service) ListTeamPolicies(ctx context.Context, teamID uint, opts mobius.ListOptions, iopts mobius.ListOptions, mergeInherited bool) (teamPolicies, inheritedPolicies []*mobius.Policy, err error) {
	if err := svc.authz.Authorize(ctx, &mobius.Policy{
		PolicyData: mobius.PolicyData{
			TeamID: ptr.Uint(teamID),
		},
...
func countTeamPoliciesEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func getTeamPolicyByIDEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc Service) GetTeamPolicyByIDQueries(ctx context.Context, teamID uint, policyID uint) (*mobius.Policy, error) {
	if err := svc.authz.Authorize(ctx, &mobius.Policy{
		PolicyData: mobius.PolicyData{
			TeamID: ptr.Uint(teamID),
		},
...
func deleteTeamPoliciesEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func modifyTeamPolicyEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) ModifyTeamPolicy(ctx context.Context, teamID uint, id uint, p mobius.ModifyPolicyPayload) (*mobius.Policy, error) {
	return svc.modifyPolicy(ctx, &teamID, id, p)
}

func checkTeamID(teamID *uint, policy *mobius.Policy) bool {
	return policy != nil && reflect.DeepEqual(teamID, policy.TeamID)
}

func (svc *Service) modifyPolicy(ctx context.Context, teamID *uint, id uint, p mobius.ModifyPolicyPayload) (*mobius.Policy, error) {
	if err := svc.authz.Authorize(ctx, &mobius.Policy{
		PolicyData: mobius.PolicyData{
			TeamID: teamID,
		},
	...
	if p.LabelsIncludeAny != nil {
		policy.LabelsIncludeAny = make([]mobius.LabelIdent, 0, len(p.LabelsIncludeAny))
		for _, label := range p.LabelsIncludeAny {
			policy.LabelsIncludeAny = append(policy.LabelsIncludeAny, mobius.LabelIdent{LabelName: label})
		}
	}
	if p.LabelsExcludeAny != nil {
		policy.LabelsExcludeAny = make([]mobius.LabelIdent, 0, len(p.LabelsExcludeAny))
		for _, label := range p.LabelsExcludeAny {
			policy.LabelsExcludeAny = append(policy.LabelsExcludeAny, mobius.LabelIdent{LabelName: label})
		}
	}
	...
	if err := svc.NewActivity(
		ctx,
		authz.UserFromContext(ctx),
		mobius.ActivityTypeEditedPolicy{
			ID:   policy.ID,
			Name: policy.Name,
		},
	); err != nil {
...
func (svc *Service) getInstallerOrVPPAppForTitle(ctx context.Context, teamID *uint, softwareTitleID *uint) (installerID *uint, vppAppsTeamsID *uint, err error) {
	...
	if teamID == nil {
		return nil, nil, ctxerr.Wrap(ctx, &mobius.BadRequestError{
	...
	if softwareTitle.AppStoreApp != nil {
		if softwareTitle.AppStoreApp.Platform != mobius.MacOSPlatform {
			return nil, nil, ctxerr.Wrap(ctx, &mobius.BadRequestError{
				Message: fmt.Sprintf(
