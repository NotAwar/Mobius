...
import (
	...
	"github.com/notawar/mobiuss/v4/server/pubsub"
	"github.com/notawar/mobiuss/v4/server/service/contract"
	"github.com/notawar/mobiuss/v4/server/sso"
	"github.com/notawar/mobiuss/v4/server/test"
	mobius_httptest "github.com/notawar/mobiuss/v4/server/test/httptest"
...
func (ts *withServer) commonTearDownTest(t *testing.T) {
	...
	mysql.ExecAdhocSQL(t, ts.ds, func(q sqlx.ExtContext) error {
		_, err := q.ExecContext(ctx, "DELETE FROM mobius_maintained_apps; ")
		return err
	})
	...
	mysql.ExecAdhocSQL(t, ts.ds, func(q sqlx.ExtContext) error {
		_, err := q.ExecContext(ctx, "ALTER TABLE mobius_maintained_apps AUTO_INCREMENT = 1;")
		return err
	})
	mysql.ExecAdhocSQL(t, ts.ds, func(q sqlx.ExtContext) error {
...
func (ts *withServer) DoRawWithHeaders(
	...
	return mobius_httptest.DoHTTPReq(ts.s.T(), decodeJSON, verb, rawBytes, ts.server.URL+path, headers, expectedStatusCode, queryParams...)
}
...
func GetToken(t *testing.T, email string, password string, serverURL string) string {
	...
	resp, err := http.Post(serverURL+"/api/latest/mobiuss/login", "application/json", requestBody)
	require.NoError(t, err)
...
func (ts *withServer) applyConfig(spec []byte) {
	...
	ts.Do("PATCH", "/api/latest/mobiuss/config", appConfigSpec, http.StatusOK)
}

func (ts *withServer) getConfig() *appConfigResponse {
	var responseBody *appConfigResponse
	ts.DoJSON("GET", "/api/latest/mobiuss/config", nil, http.StatusOK, &responseBody)
	return responseBody
}
...
func (ts *withServer) LoginMDMSSOUser(username, password string) *http.Response {
	_, res := ts.loginSSOUser(username, password, "/api/v1/mobiuss/mdm/sso", http.StatusSeeOther)
	return res
}
...
func (ts *withServer) loginSSOUser(username, password string, basePath string, callbackStatus int) (mobiuss.Auth, *http.Response) {
	...
	client := mobiushttp.NewClient(
		mobiushttp.WithFollowRedir(false),
		mobiushttp.WithCookieJar(jar),
	)
	...
	// to submit to the Mobius server from here
	defer resp.Body.Close()
...
// latest activity.
func (ts *withServer) lastActivityMatchesExtended(name, details string, id uint, mobiusInitiated *bool) uint {
	t := ts.s.T()
	var listActivities listActivitiesResponse
	ts.DoJSON("GET", "/api/latest/mobiuss/activities", nil, http.StatusOK, &listActivities, "order_key", "a.id", "order_direction", "desc", "per_page", "1")
	...
	if mobiusInitiated != nil {
		assert.Equal(t, *mobiusInitiated, act.MobiusInitiated)
	}
	return act.ID
}
...
func (ts *withServer) lastActivityOfTypeMatches(name, details string, id uint) uint {
	...
	var listActivities listActivitiesResponse
	ts.DoJSON("GET", "/api/latest/mobiuss/activities", nil, http.StatusOK,
...
func (ts *withServer) uploadSoftwareInstallerWithErrorNameReason(
	...
	r := ts.DoRawWithHeaders("POST", "/api/latest/mobiuss/software/package", b.Bytes(), expectedStatus, headers)
	defer r.Body.Close()
...
func (ts *withServer) updateSoftwareInstaller(
	...
	r := ts.DoRawWithHeaders("PATCH", fmt.Sprintf("/api/latest/mobiuss/software/titles/%d/package", payload.TitleID), b.Bytes(), expectedStatus, headers)
	defer r.Body.Close()
