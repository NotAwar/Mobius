...
func listOptionsFromRequest(r *http.Request) (mobius.ListOptions, error) {
	var err error
	...
	if pageString != "" {
		...
		if err != nil {
			return mobius.ListOptions{}, ctxerr.Wrap(r.Context(), badRequest("non-int page value"))
		}
		if page < 0 {
			return mobius.ListOptions{}, ctxerr.Wrap(r.Context(), badRequest("negative page value"))
		}
	}
	...
	if perPageString != "" {
		...
		if err != nil {
			return mobius.ListOptions{}, ctxerr.Wrap(r.Context(), badRequest("non-int per_page value"))
		}
		if perPage <= 0 {
			return mobius.ListOptions{}, ctxerr.Wrap(r.Context(), badRequest("invalid per_page value"))
		}
	}
	...
	if orderKey == "" && orderDirectionString != "" {
		return mobius.ListOptions{}, ctxerr.Wrap(r.Context(), badRequest("order_key must be specified with order_direction"))
	}

	if orderKey == "" && afterString != "" {
		return mobius.ListOptions{}, ctxerr.Wrap(r.Context(), badRequest("order_key must be specified with after"))
	}

	var orderDirection mobius.OrderDirection
...
func hostListOptionsFromRequest(r *http.Request) (mobius.HostListOptions, error) {
	opt, err := listOptionsFromRequest(r)
	if err != nil {
		return mobius.HostListOptions{}, err
	}

	hopt := mobius.HostListOptions{ListOptions: opt}

	status := r.URL.Query().Get("status")
	switch mobius.HostStatus(status) {
	case mobius.StatusNew, mobius.StatusOnline, mobius.StatusOffline, mobius.StatusMIA, mobius.StatusMissing:
		hopt.StatusFilter = mobius.HostStatus(status)
	case "":
		// No error when unset
	default:
	...
	softwareStatus := mobius.SoftwareInstallerStatus(strings.ToLower(r.URL.Query().Get("software_status")))
	if softwareStatus != "" {
	...
	switch mobius.MDMEnrollStatus(enrollmentStatus) {
	case mobius.MDMEnrollStatusManual, mobius.MDMEnrollStatusAutomatic,
		mobius.MDMEnrollStatusPending, mobius.MDMEnrollStatusUnenrolled, mobius.MDMEnrollStatusEnrolled:
		hopt.MDMEnrollmentStatusFilter = mobius.MDMEnrollStatus(enrollmentStatus)
	case "":
		// No error when unset
	default:
		return hopt, ctxerr.Wrap(
	...
	connectedToMobius := r.URL.Query().Has("connected_to_mobius")
	if connectedToMobius {
		hopt.ConnectedToMobiusFilter = ptr.Bool(true)
	}

	macOSSettingsStatus := r.URL.Query().Get("macos_settings")
	switch mobius.OSSettingsStatus(macOSSettingsStatus) {
	case mobius.OSSettingsFailed, mobius.OSSettingsPending, mobius.OSSettingsVerifying, mobius.OSSettingsVerified:
		hopt.MacOSSettingsFilter = mobius.OSSettingsStatus(macOSSettingsStatus)
	case "":
		// No error when unset
	default:
		return hopt, ctxerr.Wrap(
	...
	switch mobius.DiskEncryptionStatus(macOSSettingsDiskEncryptionStatus) {
	case
		...
		hopt.MacOSSettingsDiskEncryptionFilter = mobius.DiskEncryptionStatus(macOSSettingsDiskEncryptionStatus)
	case "":
		// No error when unset
	default:
		return hopt, ctxerr.Wrap(
	...
	switch mobius.OSSettingsStatus(osSettingsStatus) {
	case mobius.OSSettingsFailed, mobius.OSSettingsPending, mobius.OSSettingsVerifying, mobius.OSSettingsVerified:
		hopt.OSSettingsFilter = mobius.OSSettingsStatus(osSettingsStatus)
	case "":
		// No error when unset
	default:
		return hopt, ctxerr.Wrap(
	...
	switch mobius.DiskEncryptionStatus(osSettingsDiskEncryptionStatus) {
	case
		mobius.DiskEncryptionVerifying,
		mobius.DiskEncryptionVerified,
		mobius.DiskEncryptionActionRequired,
		...
		mobius.DiskEncryptionFailed,
		mobius.DiskEncryptionRemovingEnforcement:
		hopt.OSSettingsDiskEncryptionFilter = mobius.DiskEncryptionStatus(osSettingsDiskEncryptionStatus)
	case "":
		// No error when unset
	default:
		return hopt, ctxerr.Wrap(
	...
	switch mobius.MDMBootstrapPackageStatus(mdmBootstrapPackageStatus) {
	case mobius.MDMBootstrapPackageFailed, mobius.MDMBootstrapPackagePending, mobius.MDMBootstrapPackageInstalled:
		bpf := mobius.MDMBootstrapPackageStatus(mdmBootstrapPackageStatus)
	...
	case profileUUID != "" && profileStatus != "":
		...
		if mobius.OSSettingsStatus(profileStatus).IsValid() {
			psf := mobius.OSSettingsStatus(profileStatus)
	...
	if batchScriptExecutionID != "" {
		...
		if batchScriptExecutionStatus != "" {
			if mobius.BatchScriptExecutionStatus(batchScriptExecutionStatus).IsValid() {
				bsef := mobius.BatchScriptExecutionStatus(batchScriptExecutionStatus)
...
func carveListOptionsFromRequest(r *http.Request) (mobius.CarveListOptions, error) {
	opt, err := listOptionsFromRequest(r)
	if err != nil {
		return mobius.CarveListOptions{}, err
	}

	carveOpts := mobius.CarveListOptions{ListOptions: opt}
...
func userListOptionsFromRequest(r *http.Request) (mobius.UserListOptions, error) {
	opt, err := listOptionsFromRequest(r)
	if err != nil {
		return mobius.UserListOptions{}, err
	}

	userOpts := mobius.UserListOptions{ListOptions: opt}
