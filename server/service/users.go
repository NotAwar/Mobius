...
type createUserResponse struct {
	User *mobius.User `json:"user,omitempty"`
...
func createUserEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
var errMailerRequiredForMFA = badRequest("Email must be set up to enable Mobius MFA")

func (svc *Service) CreateUser(ctx context.Context, p mobius.UserPayload) (*mobius.User, *string, error) {
	var teams []mobius.UserTeam
	if p.Teams != nil {
		teams = *p.Teams
	}
	if err := svc.authz.Authorize(ctx, &mobius.User{Teams: teams}, mobius.ActionWrite); err != nil {
		return nil, nil, err
	}
		...
		for _, userTeam := range teams {
			...
			if !ok {
				return nil, nil, ctxerr.Wrap(
					ctx, mobius.NewInvalidArgumentError("teams.id", fmt.Sprintf("team with id %d does not exist", userTeam.Team.ID)),
				)
			}
		}
	}
...
func createUserFromInviteEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
	req := request.(*createUserRequest)
...
func (svc *Service) CreateUserFromInvite(ctx context.Context, p mobius.UserPayload) (*mobius.User, error) {
...
type listUsersRequest struct {
	ListOptions mobius.UserListOptions `url:"user_options"`
}

type listUsersResponse struct {
	Users []mobius.User `json:"users"`
	Err   error         `json:"error,omitempty"`
}

func (r listUsersResponse) Error() error { return r.Err }

func listUsersEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) ListUsers(ctx context.Context, opt mobius.UserListOptions) ([]*mobius.User, error) {
	user := &mobius.User{}
	if opt.TeamID != 0 {
		user.Teams = []mobius.UserTeam{{Team: mobius.Team{ID: opt.TeamID}}}
	}
	if err := svc.authz.Authorize(ctx, user, mobius.ActionRead); err != nil {
		return nil, err
	}
...
func meEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
	...
	if err != nil {
		if errors.Is(err, mobius.ErrMissingLicense) {
			availableTeams = []*mobius.TeamSummary{}
		} else {
			return getUserResponse{Err: err}, nil
		}
	}
	req := request.(*getMeRequest)
	var userSettings *mobius.UserSettings
...
func (svc *Service) AuthenticatedUser(ctx context.Context) (*mobius.User, error) {
	vc, ok := viewer.FromContext(ctx)
	if !ok {
		return nil, mobius.ErrNoContext
	}

	if err := svc.authz.Authorize(ctx, &mobius.User{ID: vc.UserID()}, mobius.ActionRead); err != nil {
		return nil, err
	}

	if !vc.IsLoggedIn() {
		return nil, mobius.NewPermissionError("not logged in")
	}
	return vc.User, nil
}
...
type getUserResponse struct {
	User           *mobius.User          `json:"user,omitempty"`
	AvailableTeams []*mobius.TeamSummary `json:"available_teams"`
	Settings       *mobius.UserSettings  `json:"settings,omitempty"`
	Err            error                 `json:"error,omitempty"`
}

func (r getUserResponse) Error() error { return r.Err }

func getUserEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) GetUserSettings(ctx context.Context, userID uint) (*mobius.UserSettings, error) {
	if err := svc.authz.Authorize(ctx, &mobius.User{ID: userID}, mobius.ActionRead); err != nil {
		return nil, err
	}
...
func (svc *Service) User(ctx context.Context, id uint) (*mobius.User, error) {
...
func modifyUserEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) ModifyUser(ctx context.Context, userID uint, p mobius.UserPayload) (*mobius.User, error) {
	...
	if err := svc.authz.Authorize(ctx, user, mobius.ActionWrite); err != nil {
		return nil, err
	}
	...
	if p.GlobalRole != nil || p.Teams != nil {
		if err := svc.authz.Authorize(ctx, user, mobius.ActionWriteRole); err != nil {
			return nil, err
		}
		...
		if err := mobius.ValidateUserRoles(false, p, *license); err != nil {
			return nil, ctxerr.Wrap(ctx, err, "validate role")
		}
	}

	if p.NewPassword != nil {
		if err := svc.authz.Authorize(ctx, user, mobius.ActionChangePassword); err != nil {
			return nil, err
		}
		if err := mobius.ValidatePasswordRequirements(*p.NewPassword); err != nil {
			return nil, ctxerr.Wrap(ctx, mobius.NewInvalidArgumentError("new_password", err.Error()))
		}
		if ownUser {
			...
			if err := vc.User.ValidatePassword(*p.NewPassword); err == nil {
				return nil, ctxerr.Wrap(ctx, mobius.NewInvalidArgumentError("new_password", "Cannot reuse old password"))
			}
			if err := vc.User.ValidatePassword(*p.Password); err != nil {
				return nil, ctxerr.Wrap(ctx, mobius.NewPermissionError("incorrect password"))
			}
		}
	}
	...
	if p.SSOEnabled != nil {
		if !*p.SSOEnabled && user.SSOEnabled && p.NewPassword == nil {
			return nil, mobius.NewInvalidArgumentError("missing password", "a new password must be provided when disabling SSO")
		}
	...
	if p.GlobalRole != nil && *p.GlobalRole != "" {
		if currentUser.GlobalRole == nil {
			return nil, authz.ForbiddenWithInternal(
				...
				currentUser, user, mobius.ActionWriteRole,
			)
		}

		if p.Teams != nil && len(*p.Teams) > 0 {
			return nil, mobius.NewInvalidArgumentError("teams", "may not be specified with global_role")
		}
		user.GlobalRole = p.GlobalRole
		user.Teams = []mobius.UserTeam{}
	} else if p.Teams != nil {
	...
	if err := mobius.LogRoleChangeActivities(ctx, svc, adminUser, oldGlobalRole, oldTeams, user); err != nil {
		return nil, err
	}
...
func deleteUserEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) DeleteUser(ctx context.Context, id uint) error {
	...
	if err := svc.NewActivity(
		ctx,
		adminUser,
		mobius.ActivityTypeDeletedUser{
			UserID:    user.ID,
			UserName:  user.Name,
...
func requirePasswordResetEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) RequirePasswordReset(ctx context.Context, uid uint, require bool) (*mobius.User, error) {
	if err := svc.authz.Authorize(ctx, &mobius.User{ID: uid}, mobius.ActionWrite); err != nil {
		return nil, err
	}
...
func changePasswordEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) ChangePassword(ctx context.Context, oldPass, newPass string) error {
	...
	if err := svc.authz.Authorize(ctx, vc.User, mobius.ActionChangePassword); err != nil {
		return err
	}

	if oldPass == "" {
		return ctxerr.Wrap(ctx, mobius.NewInvalidArgumentError("old_password", "Old password cannot be empty"))
	}
	if newPass == "" {
		return ctxerr.Wrap(ctx, mobius.NewInvalidArgumentError("new_password", "New password cannot be empty"))
	}
	if err := mobius.ValidatePasswordRequirements(newPass); err != nil {
		return ctxerr.Wrap(ctx, mobius.NewInvalidArgumentError("new_password", err.Error()))
	}
	...
	if err := vc.User.ValidatePassword(newPass); err == nil {
		return ctxerr.Wrap(ctx, mobius.NewInvalidArgumentError("new_password", "Cannot reuse old password"))
	}
	if err := vc.User.ValidatePassword(oldPass); err != nil {
		return ctxerr.Wrap(ctx, mobius.NewInvalidArgumentError("old_password", "old password does not match"))
	}
...
func getInfoAboutSessionsForUserEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) GetInfoAboutSessionsForUser(ctx context.Context, id uint) ([]*mobius.Session, error) {
	if err := svc.authz.Authorize(ctx, &mobius.Session{UserID: id}, mobius.ActionRead); err != nil {
		return nil, err
	}

	var validatedSessions []*mobius.Session
...
func deleteSessionsForUserEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) DeleteSessionsForUser(ctx context.Context, id uint) error {
	if err := svc.authz.Authorize(ctx, &mobius.Session{UserID: id}, mobius.ActionWrite); err != nil {
		return err
	}
...
func changeEmailEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) ChangeUserEmail(ctx context.Context, token string) (string, error) {
	...
	if err := svc.authz.Authorize(ctx, &mobius.User{ID: vc.UserID()}, mobius.ActionWrite); err != nil {
		return "", err
	}
...
func (svc *Service) modifyEmailAddress(ctx context.Context, user *mobius.User, email string, password *string) error {
	// password requirement handled in validation middleware
	if password != nil {
		err := user.ValidatePassword(*password)
		if err != nil {
			return mobius.NewPermissionError("incorrect password")
		}
	}
	...
	changeEmail := mobius.Email{
		Subject:      "Confirm Mobius Email Change",
		To:           []string{email},
...
func (svc *Service) saveUser(ctx context.Context, user *mobius.User) error {
...
func performRequiredPasswordResetEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) PerformRequiredPasswordReset(ctx context.Context, password string) (*mobius.User, error) {
	vc, ok := viewer.FromContext(ctx)
	if !ok {
	...
	if !vc.CanPerformPasswordReset() {
		svc.authz.SkipAuthorization(ctx)
		return nil, mobius.NewPermissionError("cannot reset password")
	}
	user := vc.User
	...
	if user.SSOEnabled {
		...
		err := mobius.NewPermissionError("password reset for single sign on user not allowed")
		return nil, ctxerr.Wrap(ctx, err)
	}
	if !user.IsAdminForcedPasswordReset() {
		// should never happen because this would get caught by the
		// CanPerformPasswordReset check above
		err := mobius.NewPermissionError("cannot reset password")
		return nil, ctxerr.Wrap(ctx, err)
	}

	// prevent setting the same password
	if err := user.ValidatePassword(password); err == nil {
		return nil, mobius.NewInvalidArgumentError("new_password", "Cannot reuse old password")
	}

	if err := mobius.ValidatePasswordRequirements(password); err != nil {
		return nil, mobius.NewInvalidArgumentError("new_password", "Password does not meet required criteria: Must include 12 characters, at least 1 number (e.g. 0 - 9), and at least 1 symbol (e.g. &*#).")
	}
...
// performed.
func (svc *Service) setNewPassword(ctx context.Context, user *mobius.User, password string) error {
...
func resetPasswordEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
...
func (svc *Service) ResetPassword(ctx context.Context, token, password string) error {
	...
	if token == "" {
		return ctxerr.Wrap(ctx, mobius.NewInvalidArgumentError("token", "Token cannot be empty field"))
	}
	if password == "" {
		return ctxerr.Wrap(ctx, mobius.NewInvalidArgumentError("new_password", "New password cannot be empty field"))
	}
	if err := mobius.ValidatePasswordRequirements(password); err != nil {
		return ctxerr.Wrap(ctx, mobius.NewInvalidArgumentError("new_password", err.Error()))
	}
	...
	if err != nil {
		return ctxerr.Wrap(ctx, mobius.NewAuthFailedError(err.Error()), "find password reset request by token")
	}
	user, err := svc.ds.UserByID(ctx, reset.UserID)
	if err != nil {
		return ctxerr.Wrap(ctx, mobius.NewAuthFailedError(err.Error()), "find user by id")
	}
	...
	// prevent setting the same password
	if err := user.ValidatePassword(password); err == nil {
		return mobius.NewInvalidArgumentError("new_password", "Cannot reuse old password")
	}

	// password requirements are validated as part of `setNewPassword``
	err = svc.setNewPassword(ctx, user, password)
	if err != nil {
		return mobius.NewInvalidArgumentError("new_password", err.Error())
	}
...
func forgotPasswordEndpoint(ctx context.Context, request interface{}, svc mobius.Service) (mobius.Errorer, error) {
	...
	// server logs).
	if err := svc.RequestPasswordReset(ctx, req.Email); errors.Is(err, mobius.ErrPasswordResetNotConfigured) {
...
func (svc *Service) RequestPasswordReset(ctx context.Context, email string) error {
	...
	if !svc.mailService.CanSendEmail(*config.SMTPSettings) {
		return mobius.ErrPasswordResetNotConfigured
	}
	...
	resetEmail := mobius.Email{
		Subject:      "Reset Your Mobius Password",
...
func (svc *Service) ListAvailableTeamsForUser(ctx context.Context, user *mobius.User) ([]*mobius.TeamSummary, error) {
