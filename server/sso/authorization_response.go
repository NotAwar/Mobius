...
// UserID partially implements the mobius.Auth interface.
...
// UserDisplayName partially implements the mobius.Auth interface.
...
// RequestID partially implements the mobius.Auth interface.
...
// AssertionAttributes partially implements the mobius.Auth interface.
func (r resp) AssertionAttributes() []mobius.SAMLAttribute {
	if r.response == nil {
		return nil
	}
	var attrs []mobius.SAMLAttribute
	for _, attr := range r.response.Assertion.AttributeStatement.Attributes {
		var values []mobius.SAMLAttributeValue
		for _, value := range attr.AttributeValues {
			values = append(values, mobius.SAMLAttributeValue{
				Type:  value.Type,
				Value: value.Value,
			})
		}
		attrs = append(attrs, mobius.SAMLAttribute{
			Name:   attr.Name,
			Values: values,
		})
	}
...
func DecodeAuthResponse(samlResponse string) (mobius.Auth, error) {
...
func decodeSAMLResponse(rawXML []byte) (mobius.Auth, error) {
	var saml Response
