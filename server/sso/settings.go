...
type Settings struct {
	...
	// SessionTTL determines how long (in seconds) a user has to complete SSO before the session is purged from Mobius
	SessionTTL  uint
	OriginalURL string
}

func GetMetadata(config *mobius.SSOProviderSettings) (*Metadata, error) {
...
func getMetadata(metadataURL string) (*Metadata, error) {
	client := mobiushttp.NewClient(mobiushttp.WithTimeout(5 * time.Second))
