...
func DoHTTPReq(t *testing.T, jsonDecoder func(r io.Reader, v interface{}) error, verb string, rawBytes []byte, urlPath string,
	headers map[string]string, expectedStatusCode int, queryParams ...string,
) *http.Response {
	...
	opts := []mobiushttp.ClientOpt{}
	if expectedStatusCode >= 300 && expectedStatusCode <= 399 {
		opts = append(opts, mobiushttp.WithFollowRedir(false))
	}
	...
	if resp.StatusCode != expectedStatusCode {
		...
		if err != nil {
			t.Logf("Error trying to decode response body as Mobius jsonError: %s", err)
			require.Equal(t, expectedStatusCode, resp.StatusCode, fmt.Sprintf("response: %+v", resp))
		}
		require.Equal(t, expectedStatusCode, resp.StatusCode, fmt.Sprintf("Mobius jsonError: %+v", je))
	}
	return resp
}
