...
func TestIntegrationsAnalyzer(t *testing.T) {
	...
	t.Run("vulnerable versions", func(t *testing.T) {
		...
		var powerpoint mobius.HostSoftwareEntry
		var word mobius.HostSoftwareEntry
		...
		ok, err := ds.InsertSoftwareVulnerability(ctx, mobius.SoftwareVulnerability{
			SoftwareID: word.ID, CVE: "3000-3000",
		}, mobius.MacOfficeReleaseNotesSource)
		require.True(t, ok)
		require.NoError(t, err)
		...
		stored, err := ds.ListSoftwareVulnerabilitiesByHostIDsSource(ctx, []uint{host.ID}, mobius.MacOfficeReleaseNotesSource)
		require.NoError(t, err)
