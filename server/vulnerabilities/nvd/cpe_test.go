...
func TestCPEFromSoftware(t *testing.T) {
	...
	cpe, err := CPEFromSoftware(log.NewNopLogger(), db, &mobius.Software{Name: "Vendor Product-1.app", Version: "1.2.3", BundleIdentifier: "vendor", Source: "apps"}, nil, reCache)
	require.NoError(t, err)
	require.Equal(t, "cpe:2.3:a:vendor:product-1:1.2.3:*:*:*:*:macos:*:*", cpe)

	// follows many deprecations
	cpe, err = CPEFromSoftware(log.NewNopLogger(), db, &mobius.Software{Name: "Vendor2 Product2.app", Version: "0.3", BundleIdentifier: "vendor2", Source: "apps"}, nil, reCache)
	require.NoError(t, err)
	require.Equal(t, "cpe:2.3:a:vendor2:product4:0.3:*:*:*:*:macos:*:*", cpe)

	// Does not error on Unicode Names
	_, err = CPEFromSoftware(log.NewNopLogger(), db, &mobius.Software{Name: "Девушка Фонарём", Version: "1.2.3", BundleIdentifier: "vendor", Source: "apps"}, nil, reCache)
	require.NoError(t, err)
}
...
func TestSyncCPEDatabase(t *testing.T) {
	...
	npmCPE, err := CPEFromSoftware(log.NewNopLogger(), db, &mobius.Software{Name: "Adaltas Mixme 0.4.0 for Node.js", Version: "0.4.0", Source: "npm_packages"}, nil, reCache)
	require.NoError(t, err)
	assert.Equal(t, "cpe:2.3:a:adaltas:mixme:0.4.0:*:*:*:*:node.js:*:*", npmCPE)

	windowsCPE, err := CPEFromSoftware(log.NewNopLogger(), db, &mobius.Software{Name: "HP Storage Data Protector 8.0 for Windows 8", Version: "8.0", Source: "programs"}, nil, reCache)
	require.NoError(t, err)
...
func (f *fakeSoftwareIterator) Value() (*mobius.Software, error) {
...
func TestConsumeCPEBuffer(t *testing.T) {
	...
	t.Run("empty buffer", func(t *testing.T) {
		var upserted []mobius.SoftwareCPE
		var deleted []mobius.SoftwareCPE
...
func TestTranslateSoftwareToCPE(t *testing.T) {
	...
	ds.UpsertSoftwareCPEsFunc = func(ctx context.Context, vals []mobius.SoftwareCPE) (int64, error) {
		for _, v := range vals {
	...
	ds.AllSoftwareIteratorFunc = func(ctx context.Context, q mobius.SoftwareIterQueryOptions) (mobius.SoftwareIterator, error) {
		return iterator, nil
	}
...
// version of Mobius storing an incorrect CPE for the software, to test that an upgrade
			...
			softwares: []*mobius.Software{
				{
					...
					// for this software on a previous version of Mobius.
...
func TestLegacyCPEDB(t *testing.T) {
	// Older versions of mobius used "select * ..." when querying from the cpe and cpe_search tables
...
func TestCPEFromSoftwareIntegration(t *testing.T) {
	...
	testCases := []struct {
		...
		*/{
		...
		// FIXME: https://github.com/notawar/mobius/issues/28490
		// {
		// 	software: mobius.Software{
		// 		Name:             "urllib3",
		// 		Source:           "python_packages",
