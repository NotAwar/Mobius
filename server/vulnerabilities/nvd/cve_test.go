...
func (d *threadSafeDSMock) ListSoftwareCPEs(ctx context.Context) ([]mobius.SoftwareCPE, error) {
	d.mu.Lock()
	defer d.mu.Unlock()
	return d.Store.ListSoftwareCPEs(ctx)
}

func (d *threadSafeDSMock) InsertSoftwareVulnerability(ctx context.Context, vuln mobius.SoftwareVulnerability, src mobius.VulnerabilitySource) (bool, error) {
	d.mu.Lock()
	defer d.mu.Unlock()
...
func TestTranslateCPEToCVE(t *testing.T) {
	...
	cveTests := map[string]struct {
		...
		// Skipping test while troubleshooting https://github.com/notawar/mobius/issues/24286
		//
	...
	cveOSTests := []struct {
		...
		},
		{
			...
			version:  "15.3",
			osID:     3,
			// This was resolved in 15.3, so it should be excluded. See https://github.com/notawar/mobius/issues/26561.
	...
	t.Run("find_vulns_on_cpes", func(t *testing.T) {
		...
		ds.ListSoftwareCPEsFunc = func(ctx context.Context) ([]mobius.SoftwareCPE, error) {
			...
			for cpe := range cveTests {
				softwareCPEs = append(softwareCPEs, mobius.SoftwareCPE{CPE: cpe, SoftwareID: i})
		...
		var osIDs []uint
		ds.ListOperatingSystemsForPlatformFunc = func(ctx context.Context, p string) ([]mobius.OperatingSystem, error) {
			var oss []mobius.OperatingSystem
			for _, os := range cveOSTests {
				oss = append(oss, mobius.OperatingSystem{
					ID:       os.osID,
		...
		ds.InsertOSVulnerabilityFunc = func(ctx context.Context, vuln mobius.OSVulnerability, src mobius.VulnerabilitySource) (bool, error) {
			osCVELock.Lock()
	...
	t.Run("recent_vulns", func(t *testing.T) {
		...
		softwareCPEs := []mobius.SoftwareCPE{
...
func TestGetMacOSCPEs(t *testing.T) {
	...
	ds.ListOperatingSystemsForPlatformFunc = func(ctx context.Context, p string) ([]mobius.OperatingSystem, error) {
		return []mobius.OperatingSystem{os}, nil
	}
