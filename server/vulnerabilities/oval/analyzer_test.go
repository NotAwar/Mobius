...
func loadSoftware(
	...
	for _, s := range h.Software {
		cpes = append(cpes, mobius.SoftwareCPE{SoftwareID: s.ID, CPE: fmt.Sprintf("%s-%s", s.Name, s.Version)})
	}
	...
	b.Run("Ubuntu", func(b *testing.B) {
		...
		for _, v := range systems {
			b.Run(fmt.Sprintf("for %s %s", v.Platform, v.Name), func(b *testing.B) {
				withTestFixture(v, ovalFixtureDir, softwareFixtureDir, vulnPath, ds, func(h *mobius.Host) {
					b.ResetTimer()
					for i := 0; i < b.N; i++ {
	...
	b.Run("RHEL", func(b *testing.B) {
		...
		systems := []struct {
		...
		}{
			{
				...
				version:            mobius.OSVersion{Platform: "rhel", Name: "Red Hat Enterprise Linux Server 7.9.0"},
			},
			{
				ovalFixtureDir:     "rhel",
				softwareFixtureDir: filepath.Join("rhel", "software", "0802"),
				version:            mobius.OSVersion{Platform: "rhel", Name: "Red Hat Enterprise Linux Server 8.2.0"},
			},
			{
				ovalFixtureDir:     "rhel",
				softwareFixtureDir: filepath.Join("rhel", "software", "0804"),
				version:            mobius.OSVersion{Platform: "rhel", Name: "Red Hat Enterprise Linux 8.4.0"},
			},
			{
				ovalFixtureDir:     "rhel",
				softwareFixtureDir: filepath.Join("rhel", "software", "0806"),
				version:            mobius.OSVersion{Platform: "rhel", Name: "Red Hat Enterprise Linux 8.6.0"},
			},
			{
				ovalFixtureDir:     "rhel",
		...
		for _, v := range systems {
			b.Run(fmt.Sprintf("for %s %s", v.version.Platform, v.version.Name), func(b *testing.B) {
				withTestFixture(v.version, v.ovalFixtureDir, v.softwareFixtureDir, vulnPath, ds, func(h *mobius.Host) {
					b.ResetTimer()
					for i := 0; i < b.N; i++ {
...
func TestOvalAnalyzer(t *testing.T) {
	t.Run("analyzing RHEL software", func(t *testing.T) {
		...
		for _, s := range systems {
			withTestFixture(s.version, s.ovalFixtureDir, s.softwareFixtureDir, vulnPath, ds, func(h *mobius.Host) {
				_, err := Analyze(ctx, ds, s.version, vulnPath, true)
				require.NoError(t, err)
				p := NewPlatform(s.version.Platform, s.version.Name)
				assertVulns(t, ds, vulnPath, h, p, mobius.RHELOVALSource)
			}, t)
		}
	})
	...
	t.Run("analyzing Ubuntu software", func(t *testing.T) {
		...
		for _, v := range systems {
			withTestFixture(v, ovalFixtureDir, softwareFixtureDir, vulnPath, ds, func(h *mobius.Host) {
				...
				assertVulns(t, ds, vulnPath, h, p, mobius.UbuntuOVALSource)
			}, t)
		}
	})
