...
func TestOvalPlatform(t *testing.T) {
	...
	t.Run("ToFilename", func(t *testing.T) {
		cases := []struct {
			...
			{time.Date(2020, 1, 1, 0, 0, 0, 0, time.UTC), "mobius_oval_ubuntu_2004-2020_01_01.json"},
			{time.Date(2020, 10, 10, 0, 0, 0, 0, time.UTC), "mobius_oval_ubuntu_2004-2020_10_10.json"},
		}
		for _, c := range cases {
	...
	t.Run("ToGovalDictionaryFilename", func(t *testing.T) {
		cases := []struct {
			...
			{"Amazon Linux 2.0.0", "mobius_goval_dictionary_amzn_02.sqlite3"},
			{"Amazon Linux 2023.0.0", "mobius_goval_dictionary_amzn_2023.sqlite3"},
		}
		for _, c := range cases {
