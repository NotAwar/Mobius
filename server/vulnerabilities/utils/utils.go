...
func RecentVulns[T mobius.Vulnerability](
	vulns []T,
	meta []mobius.CVEMeta,
) ([]T, map[string]mobius.CVEMeta) {
	if len(vulns) == 0 {
		return nil, nil
	}

	recent := make(map[string]mobius.CVEMeta)
	for _, r := range meta {
		recent[r.CVE] = r
	}
...
func BatchProcess[T mobius.Vulnerability](
	values map[string]T,
...
func VulnsDelta[T mobius.Vulnerability](
	found []T,
	existing []T,
