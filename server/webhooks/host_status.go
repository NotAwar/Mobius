...
func triggerGlobalHostStatusWebhook(ctx context.Context, ds mobius.Datastore, logger kitlog.Logger) error {
...
func processWebhook(ctx context.Context, ds mobius.Datastore, teamID *uint, settings mobius.HostStatusWebhookSettings) error {
	...
	if percentUnseen >= settings.HostPercentage {
		...
		message := fmt.Sprintf(
			"More than %.2f%% of your hosts have not checked into Mobius for more than %d days. "+
				"You've been sent this message because the Host status webhook is enabled in your Mobius instance.",
...
func triggerTeamHostStatusWebhook(ctx context.Context, ds mobius.Datastore, logger kitlog.Logger) error {
