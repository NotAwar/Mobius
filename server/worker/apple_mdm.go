...
const (
	...
	// backward compatibility (processing existing jobs after a mobius upgrade)
...
func (a *AppleMDM) runPostManualEnrollment(ctx context.Context, args appleMDMArgs) error {
	if isMacOS(args.Platform) {
		if _, err := a.installMobiusd(ctx, args.HostUUID); err != nil {
...
func (a *AppleMDM) runPostDEPEnrollment(ctx context.Context, args appleMDMArgs) error {
	...
	if isMacOS(args.Platform) {
		...
		if !manualAgentInstall {
			mobiusdCmdUUID, err := a.installMobiusd(ctx, args.HostUUID)
			if err != nil {
				return ctxerr.Wrap(ctx, err, "installing post-enrollment packages")
			}
			awaitCmdUUIDs = append(awaitCmdUUIDs, mobiusdCmdUUID)
		}
...
func (a *AppleMDM) getTeamConfig(ctx context.Context, team *mobius.Team, teamID uint) (*mobius.Team, error) {
	if team == nil {
		var err error
...
func (a *AppleMDM) getAppConfig(ctx context.Context, appConfig *mobius.AppConfig) (*mobius.AppConfig, error) {
	if appConfig == nil {
		var err error
...
func (a *AppleMDM) getIdPDisplayName(ctx context.Context, acct *mobius.MDMIdPAccount, args appleMDMArgs) (string, error) {
...
func (a *AppleMDM) runPostDEPReleaseDevice(ctx context.Context, args appleMDMArgs) error {
	...
	for _, cmdUUID := range args.EnrollmentCommands {
		...
		for _, r := range res {
			...
			if r.Status == mobius.MDMAppleStatusAcknowledged || r.Status == mobius.MDMAppleStatusError ||
				r.Status == mobius.MDMAppleStatusCommandFormatError {
				completed = true
				break
			}
		}
	...
	for _, prof := range profs {
		...
		// NOTE: user-scoped profiles are ignored because they are not sent by Mobius
...
func (a *AppleMDM) installMobiusd(ctx context.Context, hostUUID string) (string, error) {
	manifestURL := mobiusdbase.GetPKGManifestURL()
	cmdUUID := uuid.New().String()
	if err := a.Commander.InstallEnterpriseApplication(ctx, []string{hostUUID}, cmdUUID, manifestURL); err != nil {
		return "", err
	}
	a.Log.Log("info", "sent command to install mobiusdaemon", "host_uuid", hostUUID)
	return cmdUUID, nil
}
...
func (a *AppleMDM) getSignedURL(ctx context.Context, meta *mobius.MDMAppleBootstrapPackage) string {
	var url string
	if a.BootstrapPackageStore != nil {
		pkgID := hex.EncodeToString(meta.Sha256)
		signedURL, err := a.BootstrapPackageStore.Sign(ctx, pkgID)
		switch {
		case errors.Is(err, mobius.ErrNotConfigured):
