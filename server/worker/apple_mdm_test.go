...
func TestAppleMDM(t *testing.T) {
	...
	testOrgName := "mobius-test"

	createEnrolledHost := func(t *testing.T, i int, teamID *uint, depAssignedToMobius bool) *mobius.Host {
		// create the host
		...
		if depAssignedToMobius {
			err := ds.UpsertMDMAppleHostDEPAssignments(ctx, []mobius.Host{*h}, abmTokenID)
			require.NoError(t, err)
		}
		err = ds.SetOrUpdateMDMData(ctx, h.ID, false, true, "http://example.com", depAssignedToMobius, mobius.WellKnownMDMMobius, "")
		require.NoError(t, err)
		return h
	}
	...
	t.Run("fails with unknown task", func(t *testing.T) {
		...
		require.Equal(t, mobius.JobStateQueued, jobs[0].State)
	...
	t.Run("enroll reference but SSO disabled", func(t *testing.T) {
		...
		err := ds.InsertMDMIdPAccount(ctx, &mobius.MDMIdPAccount{
			Username: "test",
			Fullname: "test",
	...
	t.Run("installs mobiusdaemon for manual enrollments", func(t *testing.T) {
		mysql.SetTestABMAssets(t, ds, testOrgName)
		defer mysql.TruncateTables(t, ds)
	...
	t.Run("automatic release retries and give up", func(t *testing.T) {
		...
		for i := 0; i <= 10; i++ {
			...
			releaseJob := jobs[0]
			require.Equal(t, mobius.JobStateQueued, releaseJob.State)
...
func TestGetSignedURL(t *testing.T) {
	...
	ctx := context.Background()
	meta := &mobius.MDMAppleBootstrapPackage{
		Sha256: []byte{1, 2, 3},
	}
	...
	mockStore.SignFunc = func(ctx context.Context, fileID string) (string, error) {
		return "bozo", mobius.ErrNotConfigured
	}
	assert.Empty(t, a.getSignedURL(ctx, meta))
	assert.Empty(t, buf.String())
