...
type macosSetupAssistantArgs struct {
	...
	// Note that only DEP-enrolled hosts in Mobius MDM should be provided.
...
func (m *MacosSetupAssistant) runUpdateAllProfiles(ctx context.Context, args macosSetupAssistantArgs) error {
	...
	processTeam := func(team *mobius.TeamSummary) error {
		var teamID *uint
		if team != nil {
			teamID = &team.ID
		}
...
func (m *MacosSetupAssistant) getTeamNoTeam(ctx context.Context, tmID *uint) (*mobius.Team, error) {
...
func ProcessDEPCooldowns(ctx context.Context, ds mobius.Datastore, logger kitlog.Logger) error {
