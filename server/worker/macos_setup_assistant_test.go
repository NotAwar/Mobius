...
func TestMacosSetupAssistant(t *testing.T) {
	...
	mysql.SetTestABMAssets(t, ds, "mobius")
	...
	for i := 0; i < len(hosts); i++ {
		...
		err = ds.UpsertMDMAppleHostDEPAssignments(ctx, []mobius.Host{*h}, tokID)
		require.NoError(t, err)
		hosts[i] = h
	...
	const defaultProfileName = "Mobius default enrollment profile"
	...
	defer srv.Close()
	err = depStorage.StoreConfig(ctx, "mobius", &nanodep_client.Config{BaseURL: srv.URL})
	require.NoError(t, err)
	...
	autoProf, err := ds.GetMDMAppleEnrollmentProfileByType(ctx, mobius.MDMAppleEnrollmentTypeAutomatic)
	require.NoError(t, err)
	...
	tm1Asst, err := ds.SetOrUpdateMDMAppleSetupAssistant(ctx, &mobius.MDMAppleSetupAssistant{
		TeamID:  &tm1.ID,
		Name:    "team1",
	...
	tm2Asst, err := ds.SetOrUpdateMDMAppleSetupAssistant(ctx, &mobius.MDMAppleSetupAssistant{
		TeamID:  &tm2.ID,
		Name:    "team2",
	...
	tm3Asst, err := ds.SetOrUpdateMDMAppleSetupAssistant(ctx, &mobius.MDMAppleSetupAssistant{
		TeamID:  &tm3.ID,
		Name:    "team3",
	...
	noTmAsst, err := ds.SetOrUpdateMDMAppleSetupAssistant(ctx, &mobius.MDMAppleSetupAssistant{
		TeamID:  nil,
		Name:    "no-team",
