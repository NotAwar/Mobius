...
func (v *VPPVerification) verifyVPPInstalls(ctx context.Context, hostUUID, verificationCommandUUID string) error {
	...
	if len(pendingCmds) == 0 {
		newListCmdUUID := mobius.RefetchVPPAppInstallsCommandUUIDPrefix + uuid.NewString()
...
func QueueVPPInstallVerificationJob(ctx context.Context, ds mobius.Datastore, logger kitlog.Logger, task VPPVerificationTask, requestDelay time.Duration, hostUUID, verificationCommandUUID string) error {
