...
func NewWorker(ds mobius.Datastore, log kitlog.Logger) *Worker {
	return &Worker{
		ds:       ds,
		log:      log,
...
func QueueJob(ctx context.Context, ds mobius.Datastore, name string, args interface{}) (*mobius.Job, error) {
...
func QueueJobWithDelay(ctx context.Context, ds mobius.Datastore, name string, args interface{}, delay time.Duration) (*mobius.Job, error) {
		...
		for _, job := range jobs {
			...
			if err := w.processJob(ctx, job); err != nil {
				...
				if job.Retries < maxRetries {
					...
					job.State = mobius.JobStateFailure
				}
			} else {
				job.State = mobius.JobStateSuccess
				job.Error = ""
			}
...
func (w *Worker) processJob(ctx context.Context, job *mobius.Job) error {
