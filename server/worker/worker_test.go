...
func TestWorker(t *testing.T) {
	...
	ds.GetQueuedJobsFunc = func(ctx context.Context, maxNumJobs int, now time.Time) ([]*mobius.Job, error) {
	...
	ds.UpdateJobFunc = func(ctx context.Context, id uint, job *mobius.Job) (*mobius.Job, error) {
		assert.Equal(t, mobius.JobStateSuccess, job.State)
		return job, nil
	}
...
func TestWorkerRetries(t *testing.T) {
	...
	theJob := &mobius.Job{
		...
		Name:    "test",
		Args:    &argsJSON,
		State:   mobius.JobStateQueued,
		Retries: 0,
	}
	ds.GetQueuedJobsFunc = func(ctx context.Context, maxNumJobs int, now time.Time) ([]*mobius.Job, error) {
		if theJob.State == mobius.JobStateQueued {
			return []*mobius.Job{theJob}, nil
		}
		return nil, nil
	}
	...
	ds.UpdateJobFunc = func(ctx context.Context, id uint, job *mobius.Job) (*mobius.Job, error) {
		...
		if job.State == mobius.JobStateFailure {
			jobFailed = true
...
func TestWorkerMiddleJobFails(t *testing.T) {
	...
	ds.GetQueuedJobsFunc = func(ctx context.Context, maxNumJobs int, now time.Time) ([]*mobius.Job, error) {
		var queued []*mobius.Job
		for _, j := range jobs {
			if j.State == mobius.JobStateQueued {
	...
	require.Equal(t, mobius.JobStateSuccess, jobs[0].State)
	require.Equal(t, mobius.JobStateQueued, jobs[1].State)
	require.Equal(t, 1, jobs[1].Retries)
	require.Equal(t, mobius.JobStateSuccess, jobs[2].State)
