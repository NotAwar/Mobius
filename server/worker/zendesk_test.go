...
func TestZendeskRun(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.desc, func(t *testing.T) {
			...
			err = zendesk.Run(license.NewContext(context.Background(), &mobius.LicenseInfo{Tier: c.licenseTier}), json.RawMessage(c.payload))
			require.NoError(t, err)
		})
	}
}
...
func TestZendeskQueueVulnJobs(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		...
		err := QueueZendeskVulnJobs(ctx, ds, logger, []mobius.SoftwareVulnerability{{CVE: theCVE}}, meta)
		require.NoError(t, err)
		require.True(t, ds.NewJobFuncInvoked)
	})
...
func TestZendeskRunClientUpdate(t *testing.T) {
	...
	ds.TeamFunc = func(ctx context.Context, tid uint) (*mobius.Team, error) {
		...
		if zendesk0.GroupID == 1 {
			...
			zendesk0.GroupID = 2
			teamCfg.Config.Integrations.Zendesk = []*mobius.TeamZendeskIntegration{&zendesk0}
