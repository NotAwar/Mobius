...
# list queries
./tools/api/mobius/queries/list
{
  "queries": []
}

# use jq to filter a specific query and get the id
./tools/api/mobius/queries/list | jq '.queries[]|select(.name == "osquery_info")|.id'
2

# create a query
./tools/api/mobius/queries/create 'system_info' 'SELECT * FROM system_info;'
{
  "query": {
...
./tools/api/mobius/schedule/add_query_to_pack 2 4

# get scheduled queries in a pack
./tools/api/mobius/packs/scheduled 2 | jq '.scheduled[]|{"name": .name, "schedule_id": .id, "query_id": .query_id}'

# run a live queries on hosts (queries with id=1 and id=2 on hosts with id=3 and id=4)
./tools/api/mobius/queries/run "[1,2]" "[3,4]"
```
