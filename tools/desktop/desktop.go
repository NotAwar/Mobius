...
import (
	...
	"github.com/notawar/mobius set/v4/pkg/buildpkg"
...
func createApp(reader io.Reader, stdout io.Writer, stderr io.Writer, exitErrHandler cli.ExitErrHandlerFunc) *cli.App {
	app := cli.NewApp()
	app.Name = "desktop"
	app.Usage = "Tool to generate the Mobius Desktop application"
	...
	app.Flags = []cli.Flag{
		&cli.StringFlag{
			Name:    "version",
			Usage:   "Version of the Mobius Desktop application",
			EnvVars: []string{"MOBIUS_DESKTOP_VERSION"},
		},
		&cli.BoolFlag{
			Name:    "verbose",
			Usage:   "Log detailed information when building the application",
			EnvVars: []string{"MOBIUS_DESKTOP_VERBOSE"},
		},
	}
...
func macos() *cli.Command {
	return &cli.Command{
		Name:        "macos",
		Usage:       "Creates the Mobius Desktop Application for macOS",
		Description: "Builds and signs the Mobius Desktop .app bundle for macOS",
		Flags: []cli.Flag{
			&cli.StringFlag{
				Name:    "authority",
				Usage:   "Authority to use on the codesign invocation (if not set, app is not signed)",
				EnvVars: []string{"MOBIUS_DESKTOP_APPLE_AUTHORITY"},
			},
			&cli.BoolFlag{
				Name:    "notarize",
				...
				EnvVars: []string{"MOBIUS_DESKTOP_NOTARIZE"},
			},
		},
...
func createMacOSApp(version, authority string, notarize bool) error {
	const (
		appDir           = "Mobius Desktop.app"
		bundleIdentifier = "com.mobiusmdm.desktop"
...
`
	...
	if runtime.GOOS != "darwin" {
		return errors.New(`the "Mobius Desktop" macOS app can only be created from macOS`)
	}
	...
	buildExec := exec.Command("go", "build",
		"-o", amdBinaryPath,
		"-ldflags", os.ExpandEnv("-X=main.version=$MOBIUS_DESKTOP_VERSION"),
	...
	zlog.Info().Str("command", buildExec.String()).Msg("Build mobius-desktop executable amd64")
	...
	zlog.Info().Str("command", buildExec.String()).Msg("Build mobius-desktop executable arm64")
	if err := buildExec.Run(); err != nil {
	...
	if authority != "" {
		...
		zlog.Info().Str("command", codeSign.String()).Msg("Sign Mobius Desktop.app")
	...
	if notarize {
		...
		if err := packaging.Notarize(notarizationZip, "com.mobiusmdm.desktop"); err != nil {
			return err
		}
