...
func main() {
	...
	apiClient, err := service.NewClient(*mobiusURL, true, "", "", clientOpts...)
	if err != nil {
		panic(err)
	}
	...
	for i, record := range records {
		...
		hostname := record[0]
		platform := mobius.PlatformFromHost(record[1])
		switch platform {
		case "linux":
	...
	for _, labelSpec := range []*mobius.LabelSpec{
		...
		for _, batch := range batchHostnames(labelSpec.Hosts) {
			...
			if err := apiClient.ApplyLabels([]*mobius.LabelSpec{&labelSpecSubset}); err != nil {
				panic(err)
			}
