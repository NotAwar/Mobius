...
issues() {
	...
	members="$(curl -s -u "${username}:${token}" https://api.github.com/orgs/mobiusmdm/members?per_page=100 | jq -r 'map(.login)')"

	gh issue list --repo mobiusmdm/mobius --json id,title,author,url,createdAt,labels --limit 100 |
...
prs() {
	...
	members="$(curl -s -u "${username}:${token}" https://api.github.com/orgs/mobiusmdm/members?per_page=100 | jq -r 'map(.login)' | jq '. += ["app/dependabot"]')"

	# defaults to listing open prs
	gh pr list --limit 1000 --repo mobiusmdm/mobius --json id,title,author,url,createdAt,isDraft |
		jq -r --argjson members "$members" \
