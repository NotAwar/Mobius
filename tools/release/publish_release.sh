...
print_announce_info() {
    if [ "$dry_run" = "false" ]; then
        ...
        docker_deploy=$(gh run list --workflow goreleaser-snapshot-mobius.yaml --json event,url,headBranch --limit 100 | jq -r "[.[]|select(.headBranch==\"$target_branch\")][0].url")
        echo
...
general_announce_info() {
    if [[ "$minor" == "true" ]]; then
        article_url="https://mobiusmdm.com/releases/mobius-${target_milestone//./-}"
    ...
    echo "========================================================================="
    echo "Update osquery Slack Mobius channel topic to say the correct version $next_ver"
    echo "========================================================================="
    # Slack
    slack_hook_url=https://hooks.slack.com/services
    app_id=T019PP37ALW
...
tag() {
    if [ "$dry_run" = "false" ]; then
        ...
        # off the `tag` branch ie: mobius-v4.47.2 to watch until it completes
...
publish() {
    if [ "$dry_run" = "false" ]; then
        if [ "$announce_only" = "false" ]; then
            ...
            latest_npm=$(npm view mobiuscli --json | jq -r '.version' | sed -e 's/^v//')
            latest_local=$(jq -r '.version' tools/mobiuscli-npm/package.json | sed -e 's/^v//')
            ...
            if [ "$(node -e "console.log(require('compare-versions').compareVersions('${latest_local}', '${latest_npm}'))")" = "-1" ]; then
                ...
                cd tools/mobiuscli-npm && npm publish "--tag=last-patched-version"
            else
                # We're publishing the latest version
                cd tools/mobiuscli-npm && npm publish
            fi
...
# patch-mobius-v4.48.0
target_branch="rc-patch-mobius-$next_ver"
if [[ "$minor" == "true" ]]; then
    target_branch="rc-minor-mobius-$next_ver"
fi
        ...
        do
            ...
            if [[ "$state" != "MERGED" || "$target_pr_branch" != "main" ]]; then
                ...
                if [[ "$commit" != "" && "$commit" != "null" ]]; then
                    ...
                    echo " WARNING - invalid commit for pr https://github.com/notawar/mobius/pull/$pr - $commit"
                fi
            fi
            #echo "======================================="
        done
