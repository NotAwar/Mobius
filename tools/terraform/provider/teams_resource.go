...
// This file implements the "mobiusdm_teams" Terraform resource.
...
type teamsResource struct {
	client *mobiusdm_client.MobiusDMClient
}
...
func (r *teamsResource) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	client, ok := req.ProviderData.(*mobiusdm_client.MobiusDMClient)

	if !ok {
		resp.Diagnostics.AddError(
			"Unexpected Data Source Configure Type",
			fmt.Sprintf("Expected *MobiusDMClient, got: %T. Please report this issue to the provider developers.", req.ProviderData),
		)
...
// to the MobiusDM API and convert it to the Terraform state.
func teamModelToTF(ctx context.Context, tm *mobiusdm_client.TeamGetResponse, tf *TeamModel) error {
...
func (r *teamsResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	if err != nil {
		resp.Diagnostics.Append(diag.NewErrorDiagnostic(
			"failed to convert mobius api return to TF structs",
			fmt.Sprintf("failed to convert mobius api return to TF structs: %s", err)))
...
// Read queries the MobiusDM API and sets the TF state to what it finds.
...
// differ. It will then update the team in MobiusDM to match the plan,
...
func (r *teamsResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	var upTeam *mobiusdm_client.TeamGetResponse
	var err error
...
// as input, query the MobiusDM API to get the ID, and then set the ID.
