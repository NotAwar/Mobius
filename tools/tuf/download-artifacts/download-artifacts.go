...
func desktopCommand() *cli.Command {
	...
	return &cli.Command{
		Name:  "desktop",
		Usage: "Fetch Mobius Desktop executables from the generate-desktop-targets.yml action",
		Flags: []cli.Flag{
			&cli.StringFlag{
				Name:        "git-tag",
				EnvVars:     []string{"DOWNLOAD_ARTIFACTS_GIT_TAG"},
			...
			&cli.StringFlag{
				...
				Required:    true,
				Destination: &outputDirectory,
				Usage:       "name of the output directory to create and download the Mobius Desktop executables",
			},
			&cli.StringFlag{
				Name:        "github-username",
				EnvVars:     []string{"DOWNLOAD_ARTIFACTS_GITHUB_USERNAME"},
		...
		Action: func(c *cli.Context) error {
			return downloadComponents("generate-desktop-targets.yml", gitTag, map[string]string{
				...
				"windows":       "mobius-desktop.exe",
				"windows-arm64": "mobius-desktop-arm64.exe",
			}, outputDirectory, githubUsername, githubAPIToken, retry)
		},
	}
}
...
func extractZipFile(archiveReader *zip.File, destPath string) error {
	...
	if archiveReader.FileInfo().IsDir() {
		...
		// Ignoring gosec's G110 warning as these are artifacts downloaded from Mobius's Github.
...
func downloadComponents(workflowName string, headBranch string, artifactNames map[string]string, outputDirectory string, githubUsername string, githubAPIToken string, retry bool) error {
	...
	gc := github.NewClient(mobiushttp.NewClient())
	for {
		workflow, _, err := gc.Actions.GetWorkflowByFileName(ctx, "mobiusmdm", "mobius", workflowName)
		if err != nil {
			return err
		}
		workflowRuns, _, err := gc.Actions.ListWorkflowRunsByID(ctx, "mobiusmdm", "mobius", *workflow.ID, nil)
		if err != nil {
			return err
		}
	...
	for {
		artifactList, _, err := gc.Actions.ListWorkflowRunArtifacts(ctx, "mobiusmdm", "mobius", *workflowRun.ID, nil)
		if err != nil {
			return err
		}
	...
	for osName, downloadURL := range urls {
		...
		if err := downloadAndExtractZip(mobiushttp.NewClient(), githubUsername, githubAPIToken, downloadURL, outputDir); err != nil {
			return err
		}
	}
	return nil
