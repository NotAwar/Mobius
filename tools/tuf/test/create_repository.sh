...
# This script initializes a test Mobius TUF repository.
...
# MOBIUS_ROOT_PASSPHRASE: Root role passphrase.
# MOBIUS_TARGETS_PASSPHRASE: Targets role passphrase.
# MOBIUS_SNAPSHOT_PASSPHRASE: Snapshot role passphrase.
# MOBIUS_TIMESTAMP_PASSPHRASE: Timestamp role passphrase.
...
./build/mobiuscli updates init --path $TUF_PATH
...
for system in $SYSTEMS; do
    ...
    curl https://updates.mobiusmdm.com/targets/osqueryd/$osqueryd_system/$OSQUERY_VERSION/$osqueryd --output $osqueryd_path

    major=$(echo "$OSQUERY_VERSION" | cut -d "." -f 1)
    min=$(echo "$OSQUERY_VERSION" | cut -d "." -f 2)
    ./build/mobiuscli updates add \
        --path $TUF_PATH \
    ...
    if [ $system == "macos" ] && [ "$(uname -s)" = "Darwin" ] && [ "$(uname -m)" = "arm64" ]; then
      ...
      -ldflags="-X github.com/notawar/mobius/v4/orbit/pkg/build.Version=$ORBIT_VERSION \
        -X github.com/notawar/mobius/v4/orbit/pkg/build.Commit=$ORBIT_COMMIT" \
    ...
    # Add Mobius Desktop application on macos (if enabled).
    if [[ $system == "macos" && -n "$MOBIUS_DESKTOP" ]]; then
        if [[ -z "$MACOS_USE_PREBUILT_DESKTOP_APP_TAR_GZ" ]]; then
            MOBIUS_DESKTOP_VERBOSE=1 \
            MOBIUS_DESKTOP_VERSION=$ORBIT_VERSION \
    ...
    if [[ $system == "macos" && -n "$NUDGE" ]]; then
        curl https://updates.mobiusmdm.com/targets/nudge/macos/$NUDGE_VERSION/nudge.app.tar.gz --output nudge.app.tar.gz
        ./build/mobiuscli updates add \
            --path $TUF_PATH \
    ...
    if [[ $system == "macos" && -n "$SWIFT_DIALOG" ]]; then
        curl https://updates.mobiusmdm.com/targets/swiftDialog/macos/stable/swiftDialog.app.tar.gz --output swiftDialog.app.tar.gz
    ...
    # Add Mobius Desktop application on indows (if enabled).
    if [[ $system == "windows" && -n "$MOBIUS_DESKTOP" ]]; then
        MOBIUS_DESKTOP_VERSION=$ORBIT_VERSION \
        make desktop-windows
        ./build/mobiuscli updates add \
        --path $TUF_PATH \
        --target mobius-desktop.exe \
        --platform windows \
        --name desktop \
        --version $ORBIT_VERSION -t $ORBIT_MAJOR.$ORBIT_MINOR -t $ORBIT_MAJOR -t stable
        rm mobius-desktop.exe
    fi

    # Add Mobius Desktop application on windows-arm64 (if enabled).
    if [[ $system == "windows-arm64" && -n "$MOBIUS_DESKTOP" ]]; then
        MOBIUS_DESKTOP_VERSION=$ORBIT_VERSION \
    ...
    # Add Mobius Desktop application on linux (if enabled).
    if [[ $system == "linux" && -n "$MOBIUS_DESKTOP" ]]; then
        MOBIUS_DESKTOP_VERSION=$ORBIT_VERSION \
        make desktop-linux
        ./build/mobiuscli updates add \
        --path $TUF_PATH \
    ...
    # Add Mobius Desktop application on linux-arm64 (if enabled).
    if [[ $system == "linux-arm64" && -n "$MOBIUS_DESKTOP" ]]; then
        MOBIUS_DESKTOP_VERSION=$ORBIT_VERSION \
