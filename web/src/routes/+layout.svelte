...
<div class="min-h-screen bg-[#1c2f38]">
	<!-- Sidebar Navigation -->
	<aside class="fixed inset-y-0 left-0 z-50 w-64 transform bg-[#31413e] shadow-xl transition-transform duration-300 ease-in-out border-r border-[#334155] lg:translate-x-0 {mobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}">
		<div class="flex h-full flex-col">
			<!-- Logo -->
			<div class="flex h-16 items-center gap-3 border-b border-[#334155] px-6">
				<img src="/assets/Mobius_Logo.png" alt="Mobius" class="h-10 w-10" />
				<div>
					<h1 class="text-xl font-bold text-white font-heading">Mobius</h1>
					<p class="text-xs text-[#94a3b8]">MDM Platform</p>
				</div>
			</div>
							...
							{#each section.items as item}
								<a
									...
									class="flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition-all {$page.url.pathname === item.href
										? 'bg-[#d4af37] text-[#1c2f38] shadow-lg'
										: 'text-[#94a3b8] hover:bg-[#1c2f38] hover:text-white'}"
				...
				<div class="text-xs text-[#64748b]">
					<div class="mb-2 flex items-center justify-between">
						...
						<div class="flex gap-1 items-center">
							<div class="h-2 w-2 rounded-full bg-[#4ade80]"></div>
							<span class="text-[#4ade80]">Online</span>
						</div>
					</div>
					<p>© 2025 Mobius</p>
				</div>
	...
	<!-- Mobile Menu Button -->
	<button
		class="fixed left-4 top-4 z-40 rounded-lg bg-[#31413e] p-2 shadow-lg lg:hidden"
	...
	<div class="lg:pl-64">
		<!-- Top Bar -->
		<header class="sticky top-0 z-30 border-b border-[#334155] bg-[#31413e]/95 backdrop-blur-sm">
			<div class="flex h-16 items-center justify-between px-6">
				<div class="flex items-center gap-4">
					<h2 class="text-xl font-semibold text-white">
						...
						{:else if $page.url.pathname === '/groups'}
							Client Groups
						{:else if $page.url.pathname === '/enrollment'}
							Client Enrollment
						{:else if $page.url.pathname === '/osquery'}
							OSQuery Management
						{:else if $page.url.pathname === '/queries'}
							Query Management
				...
				<div class="flex items-center gap-4">
					...
					<div class="h-8 w-8 rounded-full bg-[#d4af37] flex items-center justify-center text-[#1c2f38] font-semibold text-sm">
						A
					</div>
				</div>
			</div>
