module.exports = {
  ...
  fn: async function (inputs) {
    ...
    sails.helpers.salesforce.updateOrCreateContactAndAccount.with({
      ...
      description: `Purchased a self-service Mobius Premium license on ${isoTimestampForDescription.split('T')[0]} for ${quoteRecord.numberOfHosts} host${quoteRecord.numberOfHosts > 1 ? 's' : ''}.`
    }).exec((err)=>{
      if(err){
        sails.log.warn(`Background task failed: When a user (email: ${this.req.me.emailAddress} purchased a self-service Mobius premium subscription, a Contact and Account record could not be created/updated in the CRM.`, err);
      }
      return;
    });
