module.exports = {
  ...
  exits: {
    ...
    adminDidNotConsent: { description: 'An entra admin did not grant permissions to the Mobius compliance partner application for entra', responseType: 'ok'},
  ...
  fn: async function ({tenant, state, error, error_description}) {// eslint-disable-line camelcase
    ...
    if(!informationAboutThisTenant) {
      ...
      let mobiusInstanceUrlToRedirectTo = informationAboutThisTenant.mobiusInstanceUrl + '/settings/integrations/conditional-access';
      ...
      await sails.helpers.http.sendHttpRequest.with({
        ...
        body: {
          ...
          PartnerEnrollmentUrl: `https://mobiusmdm.com/microsoft-compliance-partner/enroll`,
          PartnerRemediationUrl: `https://mobiusmdm.com/microsoft-compliance-partner/remediate`,
        }
      }).intercept(async (err)=>{
      ...
      let createPolicyResponse = await sails.helpers.http.sendHttpRequest.with({
        ...
        body: {
          DisplayName: 'Mobius compliance policy',
          Description: 'Compliance policy managed by Mobius',
          Platform: 'macOS',
      ...
      // Redirect the user to their Mobius instance.
      return mobiusInstanceUrlToRedirectTo;
    }
