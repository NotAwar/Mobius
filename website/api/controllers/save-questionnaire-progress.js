module.exports = {
  ...
  fn: async function ({currentStep, formData}) {
    ...
    // When the 'what-are-you-using-mobius-for' is completed, update this user's DB record and session to include their answer.
    if(currentStep === 'what-are-you-using-mobius-for') {
    ...
    //  - yes-deployed-local: » Stage 3 (Tried Mobius but might not have a use case)
    //  - yes-deployed-long-time: Stage 2 (Tried Mobius long ago but might not fully grasp)
    //  - no: Stage 2 (Never tried Mobius and might not fully grasp)
    ...
    //  - All other options » Stage 4
    // 'is-it-any-good': Stage 3/4 (depends on answer from 'have-you-ever-used-mobius' & the buying situation specific step)
    // 'what-did-you-think'
    //  - host-mobius-for-me » Stage 5
    //  - deploy-mobius-in-environment » Stage 5
    //  - let-me-think-about-it »  Stage 2
    // FUTURE: Should the step about deploying mobius in your env be here?  (For same reason is-it-any-good is here: when navigating back then forwards?)
    // 'how-was-your-deployment'
    //  - up-and-running »  Stage 5
    //  - kinda-stuck »  Stage 5
    //  - havent-gotten-to-it » Stage 5
    //  - changed-mind-want-managed-deployment » Stage 5
    //  - decided-to-not-use-mobius » Stage 2
    // 'whats-left-to-get-you-set-up'
    //  - need-premium-license-key » No change (Stage ??)
    //  - help-show-mobius-to-my-team » No change (Stage ??)
    ...
    if(currentStep === 'start') {
    ...
    } else if(currentStep === 'what-are-you-using-mobius-for') {
      psychologicalStage = '2 - Aware';
    } else if(currentStep === 'have-you-ever-used-mobius') {
      if(valueFromFormData === 'yes-deployed') {
        // If the user has Mobius deployed, set their stage to 6.
      ...
      let currentSelectedBuyingSituation = questionnaireProgress['what-are-you-using-mobius-for'].primaryBuyingSituation;
      // Get the user's answer to the "Have you ever used Mobius?" question.
      let hasUsedMobiusAnswer = questionnaireProgress['have-you-ever-used-mobius'].mobiusUseStatus;
      if(['what-are-you-working-on-eo-security','what-does-your-team-manage-eo-it','what-does-your-team-manage-vm','what-do-you-manage-mdm'].includes(currentStep)){
        ...
        // When the user submits the step before the "Is it any good?" step, we will generate them a 30 day Trial key for Mobius Premium that they can use with mobiuscli preview
        if(!userRecord.mobiusPremiumTrialLicenseKey) {
          ...
          let trialLicenseKeyForThisUser = await sails.helpers.createLicenseKey.with({
            numberOfHosts: 10,
            organization: this.req.me.organization ? this.req.me.organization : 'Mobius Premium trial',
        ...
        if(currentSelectedBuyingSituation === 'mdm') {
          ...
          if(typeof questionnaireProgress['what-do-you-manage-mdm'] !== 'undefined' && questionnaireProgress['what-do-you-manage-mdm'].mdmUseCase === 'no-use-case-yet'){
            // Check the user's answer to the have-you-ever-used-mobius question.
            psychologicalStage = '3 - Intrigued';
          } else {
            psychologicalStage = '4 - Has use case';
          }
        ...
        if(valueFromFormData === 'let-me-think-about-it') {
          psychologicalStage = '2 - Aware';
        } else if (['host-mobius-for-me', 'deploy-mobius-in-environment'].includes(valueFromFormData)) {
          psychologicalStage = '5 - Personally confident';
        } else { require('assert')(false,'This should never happen.'); }
      } else if(currentStep === 'how-was-your-deployment') {
        if(valueFromFormData === 'decided-to-not-use-mobius') {
          psychologicalStage = '2 - Aware';
        ...
        if(['yes-deployed'].includes(hasUsedMobiusAnswer)) {
          psychologicalStage = '6 - Has team buy-in';
        } else {
          psychologicalStage = '5 - Personally confident';
        }
        ...
        if(['yes-deployed'].includes(hasUsedMobiusAnswer)) {
          psychologicalStage = '6 - Has team buy-in';
        } else if(['yes-recently-deployed'].includes(hasUsedMobiusAnswer)){
          psychologicalStage = '5 - Personally confident';
        } else { require('assert')(false, 'This should never happen.'); }
      } else if(currentStep === 'thanks-for-checking-out-mobius') {
        psychologicalStage = '2 - Aware';
      }//ﬁ
    }//ﬁ
    ...
    try {
      ...
      .replace(/:\w+:/g, ':\t')// Replace the key from the formData with a colon and tab, (e.g., what-are-you-using-mobius-for:primaryBuyingSituation:eo-security, » what-are-you-using-mobius-for:   eo-security)
    ...
    questionnaireProgressAsAFormattedString = `organization-according-to-mobiusmdm.com: ${this.req.me.organization}\n` + questionnaireProgressAsAFormattedString;
