module.exports = {
  ...
  inputs: {
    ...
    repository: { required: true, type: {}, example: {name: 'mobius', owner: {login: 'mobiusmdm'}} },
  ...
  fn: async function ({botSignature, action, sender, repository, changes, issue, comment, pull_request: pr, label, release}) {
    ...
    // > https://github.com/notawar/mobius/pull/5628#issuecomment-1196175485
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    ...
    if (
      ...
      let openAiReport = await sails.helpers.http.post('https://api.openai.com/v1/chat/completions', {
        ...
        messages: [// https://platform.openai.com/docs/guides/chat/introduction
          {
            role: 'user',
            content: `You are an empathetic product designer.  I will give you a Github issue with information about a particular improvement to Mobius, an open-source device management and security platform.  You will write a haiku about how this improvement could benefit users or contributors.  Be detailed and specific in the haiku.  Do not use hyperbole.  Be matter-of-fact.  Be positive.  Do not make Mobius (or anyone) sound bad.  But be honest.  If appropriate, mention imagery from nature, or from a glass city in the clouds.  Do not give orders.\n\nThe first GitHub issue is:\n${issueSummary}`,
          }
        ],
        temperature: 0.7,
      ...
      if (!openAiReport) {// If OpenAI could not be reached…
        newBotComment = 'I couldn\'t think of a haiku this time.  (See mobiusmdm.com logs for more information.)';
      ...
      // https://github.com/notawar/mobius/blob/0a59adc2dd65bce5c1201a752e9c218faea7be35/website/api/controllers/webhooks/receive-from-github.js#L145-L216
      //
      ...
      if (action === 'edited' && pr.state !== 'open') {// PR edited ‡
        ...
        // let mergeFreezeMainBranchStatusReport = await sails.helpers.http.get('https://www.mergefreeze.com/api/branches/mobiusmdm/mobius/main', { access_token: sails.config.custom.mergeFreezeAccessToken }) //eslint-disable-line camelcase
        ...
        //   // Note: We'll only do this if the PR is from the mobiusmdm/mobius repo.
        //   if (isMainBranchFrozen && repo === 'mobius') {
        ...
        //     await sails.helpers.http.post(`https://www.mergefreeze.com/api/branches/mobiusmdm/mobius/main?access_token=${encodeURIComponent(sails.config.custom.mergeFreezeAccessToken)}`, {
        //       user_name: 'mobius-release',//eslint-disable-line camelcase
      ...
      // posting to the Mobius Slack.
      // > FUTURE: also post to Slack about deleted issues, new repos, and deleted repos
      await sails.helpers.http.post(
        sails.config.custom.slackWebhookUrlForGithubBot,//« #g-marketing channel (Mobius Slack workspace)
        {
          text:
          (
      ...
      //  ╚═╝  ╚═╝╚══════╝╚══════╝╚══════╝╚═╝  ╚═╝╚══════╝╚══════╝╚══════╝
      //
      // Handle new Mobius releases by sending a POST request to Zapier to
      // trigger an automation that updates Slack channel topics with the latest version of Mobius.
      let owner = repository.owner.login;
      let repo = repository.name;

      // Only continue if this release came from the mobiusmdm/mobius repo,
      if(owner === 'mobiusmdm' && repo === 'mobius') {
        if(release
          && _.startsWith(release.tag_name, 'mobius-v')// Only send requests for releases with tag names that start with 'mobius'
          && _.endsWith(release.tag_name, '.0')// Only send requests if the release is a major or minor version. This works because all Mobius semvers have 2 periods.
        ) {
