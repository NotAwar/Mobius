  ...
  fn: async function ({id, type, data, webhookSecret}) {
    ...
    if(type === 'invoice.upcoming' && stripeEventData.billing_reason === 'upcoming') {
      ...
      await sails.helpers.sendTemplateEmail.with({
        ...
        subject: 'Your Mobius Premium subscription',
    ...
    // If the event was triggered by a user's card successfully being charged by Stripe, we'll generate a new license key, update the subscription's database record, and send the user a renewal confirmation email.
      ...
      await Subscription.updateOne({id: subscriptionForThisEvent.id}).set({
        mobiusLicenseKey: newLicenseKeyForThisSubscription,
    ...
    //  ┴┘└┘ └┘ └─┘┴└─┘└─┘  └  └─┘┴└─  └─┘┴  ─┴┘┴ ┴ ┴ └─┘─┴┘  └─┘└─┘└─┘└─┘└─┘┴└─┴ ┴ └─┘┘└┘  ┴  ┴ ┴┴─┴┘
      ...
      let updatedSubscriptionInfo = _.find(itemsOnThisInvoice, (item)=>{
        ...
        // e.g., 'Remaining time on 9 × Mobius premium hosts after 17 Feb 2024'
      ...
      await Subscription.create({
        ...
        mobiusLicenseKey: newLicenseKey,
      ...
      await sails.helpers.sendTemplateEmail.with({
        ...
        fromName: sails.config.custom.fromName,
        subject: 'Your Mobius Premium order',
      ...
      sails.helpers.salesforce.updateOrCreateContactAndAccount.with({
        ...
        description: `Purchased a self-service Mobius Premium license on ${isoTimestampForDescription.split('T')[0]} for ${numberOfHosts} host${numberOfHosts > 1 ? 's' : ''}.`
      }).exec((err)=>{
        if(err){
          sails.log.warn(`Background task failed: When a user (email: ${userForThisSubscription.emailAddress} purchased a self-service Mobius premium subscription, a Contact and Account record could not be created/updated in the CRM.`, err);
        }
        return;
      });
