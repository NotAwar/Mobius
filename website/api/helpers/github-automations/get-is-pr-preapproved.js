module.exports = {
  ...
  inputs: {
    repo: { type: 'string', example: 'mobius', required: true, isIn: ['mobius', 'confidential']},
  ...
  fn: async function ({repo, prNumber, githubUserToCheck, isGithubUserMaintainerOrDoesntMatter}) {
    ...
    require('assert')(sails.config.custom.mobiusMdmGitopsGithubRepoMaintainersByPath);
    ...
    if (repo === 'mobius-gitops') {
      MAINTAINERS_BY_PATH = sails.config.custom.mobiusMdmGitopsGithubRepoMaintainersByPath;
    }

    let owner = 'mobiusmdm';
    let baseHeaders = {
      'User-Agent': 'Mobius auto-approve',
