module.exports = {
  ...
  inputs: {
    ...
    intentSignal: {
      type: 'string',
      isIn: [
        'Followed the Mobius LinkedIn company page',
        'LinkedIn comment',
        'LinkedIn share',
        'LinkedIn reaction',
        'Mobius channel member in MacAdmins Slack',
        'Mobius channel member in osquery Slack',
        'Implemented a trial key',
        'Engaged with mobiusie at community event',
        'Attended a Mobius happy hour',
        'Stared the mobiusmdm/mobius repo on GitHub',
        'Forked the mobiusmdm/mobius repo on GitHub',
        'Subscribed to the Mobius newsletter',
        'Attended a Mobius training course'
      ]
    },
    eventContent: {
      type: 'string',
  ...
  fn: async function ({ salesforceAccountId, salesforceContactId, eventType, linkedinUrl, intentSignal, eventContent, eventContentUrl, mobiusWebsitePageUrl, websiteVisitReason}) {
    ...
    if(eventType === 'Intent signal') {
      ...
      if(!mobiusWebsitePageUrl){
        throw new Error(`A mobiusWebsitePageUrl value is required when creating "Website page view" type historical events`);
      }
    }
    ...
    let newHistoricalRecord = await sails.helpers.flow.build(async ()=>{
      ...
      return await salesforceConnection.sobject('mobius_website_page_views__c')
      .create({
        ...
        Page_URL__c: mobiusWebsitePageUrl,// eslint-disable-line camelcase
        Website_visit_reason__c: websiteVisitReason// eslint-disable-line camelcase
      });
    }).intercept((err)=>{
      return new Error(`An error occurred when creating a new Historical event record in Salesforce. full error ${require('util').inpsect(err, {depth: null})}`);
    });
