module.exports = {
  ...
  inputs: {
    ...
    intentSignal: {
      type: 'string',
      isIn: [
        'Subscribed to the Mobius newsletter',
        // 'Signed up for a mobiusmdm.com account',//
  ...
  fn: async function ({emailAddress, linkedinUrl, firstName, lastName, organization, jobTitle, primaryBuyingSituation, psychologicalStage, psychologicalStageChangeReason, contactSource, description, getStartedResponses, intentSignal}) {
    ...
    let salesforceConnection = new jsforce.Connection({
      loginUrl : 'https://mobiusmdm.my.salesforce.com'
    });
    ...
    if(emailAddress) {
      ...
      if(!existingContactRecord){
        // If no contacts are found when searching by the provided email address, look for existing contact records that have a "last email associated by mobiusmdm.com" set to the providded email address.
        existingContactRecord = await salesforceConnection.sobject('Contact')
        .findOne({
          Last_email_associated_by_mobiusdm_com__c:  emailAddress, // eslint-disable-line camelcase
        });
        // If we matched a contact record by searchign last email associated by mobiusmdm.com, remove the email address from the update criteria.
        if(existingContactRecord){
    ...
    if(!existingContactRecord) {
      ...
      if(!enrichmentData.employer || !enrichmentData.employer.emailDomain || !enrichmentData.employer.organization) {
        ...
        // https://mobiusmdm.lightning.force.com/lightning/r/Account/0014x000025JC8DAAW/view
      ...
      if(!duplicateContactWasFound) {
        ...
        // If an email address was provided, and the existing contact has an email address set, remove it from the ValuesToSet dictionairy and set it as the "last email associated by mobiusmdm.com"
        if(emailAddress && existingContactRecord.Email){
          delete valuesToSet.Email;
          valuesToSet.Last_email_associated_by_mobiusdm_com__c =  emailAddress;// eslint-disable-line camelcase
        }
    ...
    if(existingContactRecord) {
      ...
      if(psychologicalStage && existingContactRecord.Stage__c !== null) {
        ...
        if(psyStageStageNumberToChangeTo < recordsCurrentPsyStageNumber) {
          ...
          // This is done to prevent automated psyStage regressions caused by users taking other action on the website. (e.g, Booking a meeting or requesting Mobius swag.)
