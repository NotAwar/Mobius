      ...
      sails.after('hook:organics:loaded', ()=>{
        ...
        if(sails.config.environment === 'production' && process.env.DYNO === 'web.1'){
          sails.helpers.http.post.with({
            ...
            if(err){
              sails.log.warn('When trying to send a request to Algolia to refresh the Mobius website search index, an error occurred: '+err);
            }
          });//_∏_
        }//ﬁ
      ...
      before: {
        '/*': {
          ...
          fn: async function(req, res, next){
            ...
            // e.g.
            //   https://mobiusmdm.com/device-management?utm_content=mdm
            ...
            if (req.method === 'GET') {
              ...
              // Create a timestamp of thirty seconds ago. We'll use this to check the age of the user account before creating a mobiuswebsite page view record in Salesforce.
              ...
              res.once('finish', function onceFinish() {
                ...
                if(res.statusCode === 200 && sanitizedUser.createdAt < thirtySecondsAgoAt){
                  sails.helpers.flow.build(async ()=>{
                    ...
                    // Create the new Mobius website page view record.
                    await sails.helpers.salesforce.createHistoricalEvent.with({
                      salesforceContactId: recordIds.salesforceContactId,
                      salesforceAccountId: recordIds.salesforceAccountId,
                      mobiusWebsitePageUrl: `https://mobiusmdm.com${req.url}`,
                      eventType: 'Website page view',
                      websiteVisitReason: websiteVisitReason
                    }).intercept((err)=>{
                      return new Error(`Could not create new Mobius website page view record. Error: ${err}`);
                    });
                  })
                  .exec((err)=>{
