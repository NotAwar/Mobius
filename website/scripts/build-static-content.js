module.exports = {
  ...
  description: 'Generate HTML partials from source files in mobiusmdm/mobius repo (e.g. docs in markdown, or queries in YAML), and configure metadata about the generated files so it is available in `sails.config.builtStaticContent`.',
  ...
  fn: async function ({ dry, githubAccessToken }) {
    ...
    await sails.helpers.flow.simultaneously([
      async()=>{// Parse queries.yml library (Informational queries and host vital queries) from YAML and prepare to bake them into the Sails app's configuration.
        let RELATIVE_PATH_TO_QUERY_LIBRARY_YML_IN_MOBIUS_REPO = 'docs/queries.yml';
        let yaml = await sails.helpers.fs.read(path.join(topLvlRepoPath, RELATIVE_PATH_TO_QUERY_LIBRARY_YML_IN_MOBIUS_REPO)).intercept('doesNotExist', (err)=>new Error(`Could not find standard query library YAML file at "${RELATIVE_PATH_TO_QUERY_LIBRARY_YML_IN_MOBIUS_REPO}".  Was it accidentally moved?  Raw error: `+err.message));
        ...
        builtStaticContent.queryLibraryYmlRepoPath = RELATIVE_PATH_TO_QUERY_LIBRARY_YML_IN_MOBIUS_REPO;
      },
      async()=>{
        ...
        let RELATIVE_PATH_TO_POLICY_LIBRARY_YML_IN_MOBIUS_REPO = 'docs/01-Using-Mobius/standard-query-library/standard-query-library.yml';
        // let RELATIVE_PATH_TO_QUERY_LIBRARY_YML_IN_MOBIUS_REPO = 'docs/queries.yml';
        let yaml = await sails.helpers.fs.read(path.join(topLvlRepoPath, RELATIVE_PATH_TO_POLICY_LIBRARY_YML_IN_MOBIUS_REPO)).intercept('doesNotExist', (err)=>new Error(`Could not find standard query library YAML file at "${RELATIVE_PATH_TO_POLICY_LIBRARY_YML_IN_MOBIUS_REPO}".  Was it accidentally moved?  Raw error: `+err.message));
        let linesInYamlFile = yaml.split('\n');
        ...
        builtStaticContent.policyLibraryYmlRepoPath = RELATIVE_PATH_TO_POLICY_LIBRARY_YML_IN_MOBIUS_REPO;
      },
      async()=>{// Parse markdown pages, compile & generate HTML files, and prepare to bake directory trees into the Sails app's configuration.
        ...
        for (let sectionRepoPath of Object.keys(SECTION_INFOS_BY_SECTION_REPO_PATHS)) {// FUTURE: run this in parallel
          ...
          for (let pageSourcePath of thinTree) {// FUTURE: run this in parallel
            ...
            // also include the article's category: https://github.com/notawar/mobius/blob/9d2acb5751f4cebfb211ae1f15c9289143b5b79c/website/scripts/build-static-content.js#L441-L447
            ...
            // > all in one place.  (For more context, see https://github.com/notawar/confidential/issues/1537 )
            // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
            let rootRelativeUrlPath = (
              (
                ...
                '/' + (
                  ...
                  .split(/\//).map((fileOrFolderName) => encodeURIComponent(fileOrFolderName.replace(/^[0-9]+[\-]+/,''))).join('/')// « Get URL-friendly by encoding characters and stripping off ordering prefixes (like the "1-" in "1-Using-Mobius") for all folder and file names in the path.
                )
            ...
            if (path.extname(pageSourcePath) !== '.md') {// If this file doesn't end in `.md`: skip it (we won't create a page for it)
              ...
              // >  • syntax highlighting
              // >  • data type bubbles
              // >  • transforming relative markdown links to their mobiusmdm.com equivalents
              // >
              // > For more info about how these additional features work, see: https://github.com/notawar/mobius/issues/706#issuecomment-884622252
              // >
              // > • What about images referenced in markdown files? :: For documentation and handbook files, they need to be referenced using an absolute URL of the src-- e.g. ![](https://raw.githubusercontent.com/mobiusmdm/mobius/main/docs/images/foo.png). For articles, you can use the absolute URL of the src - e.g. ![](https://mobiusmdm.com/images/articles/foo.png) OR the relative repo path e.g. ![](../website/assets/images/articles/foo.png). See also https://github.com/notawar/mobius/issues/706#issuecomment-884641081 for reasoning.
              // > • What about GitHub-style emojis like `:white_check_mark:`?  :: Use actual unicode emojis instead.  Need to revisit this?  Visit https://github.com/notawar/mobius/pull/1380/commits/19a6e5ffc70bf41569293db44100e976f3e2bda7 for more info.
              let mdString = await sails.helpers.fs.read(pageSourcePath);

              // Look for non example @mobiusmdm.com email addresses in the Markdown string, if any are found, throw an error.
              if(mdString.match(/[A-Z0-9._%+-]+@mobiusmdm\.com/gi)) {
                throw new Error(`A Markdown file (${pageSourcePath}) contains a @mobiusmdm.com email address. To resolve this error, remove the email address in that file or change it to be an @example.com email address and try running this script again.`);
              }
              ...
              // [?] Looking for code that used to be here related to syntax highlighting?  Please see https://github.com/notawar/mobius/pull/14124/files  -mikermcneil, 2023-09-25
              ...
              htmlString = htmlString.replace(/\(\(([^())]*)\)\)/g, '<bubble type="$1" class="colors"><span is="bubble-heart"></span></bubble>');// « Replace ((bubble))s with HTML. For more background, see https://github.com/notawar/mobius/issues/706#issuecomment-884622252
              htmlString = htmlString.replace(/(href="(\.\/[^"]+|\.\.\/[^"]+)")/g, (hrefString)=>{// « Modify path-relative links like `./…` and `../…` to make them absolute.  (See https://github.com/notawar/mobius/issues/706#issuecomment-884641081 for more background)
                ...
                let referencedPageNewUrl = 'https://mobiusmdm.com/' + (
                  (path.relative(topLvlRepoPath, referencedPageSourcePath).replace(/(^|\/)([^/]+)\.[^/]*$/, '$1$2').split(/\//)
                  .map((fileOrFolderNameWithPossibleUrlEncodedSpaces) => fileOrFolderNameWithPossibleUrlEncodedSpaces.toLowerCase().replace(/%20/g, '-'))// « Replaces url-encoded spaces with dashes to support relative links to folders with spaces on Github.com and the Mobius website.
                  .map((fileOrFolderNameWithPossibleSpaces) => fileOrFolderNameWithPossibleSpaces.toLowerCase().replace(/\s/g, '-')).join('/'))// « Replaces spaces in folder names with dashes to support relative links to folders with spaces on Mobius website.
              ...
              htmlString = htmlString.replace(/(href="https?:\/\/([^"]+)")/g, (hrefString)=>{// « Modify links that are potentially external
                ...
                // to some page on the destination site where this will be hosted, like `(*.)?mobiusmdm.com`.
                // If external, add target="_blank" so the link will open in a new tab.
                // Note: links to trust.mobiusmdm.com and blog.mobiusmdm.com will be treated as an external link.
                let isExternal = ! hrefString.match(/^href=\"https?:\/\/([^\.|trust|blog]+\.)*mobiusmdm\.com/g);// « FUTURE: make this smarter with sails.config.baseUrl + _.escapeRegExp()
                // Check if this link is to mobiusmdm.com or www.mobiusmdm.com.
                let isBaseUrl = hrefString.match(/^(href="https?:\/\/)([^\.]+\.)*mobiusmdm\.com"$/g);
                if (isExternal) {
                  ...
                  if (isBaseUrl) {
                    return hrefString.replace(/href="https?:\/\//, '').replace(/([^\.]+\.)*mobiusmdm\.com/, 'href="/');
                  } else {
                    return hrefString.replace(/href="https?:\/\//, '').replace(/^([^\.]+\.)*mobiusmdm\.com/, 'href="');
                  }
                }
              ...
              if (sectionRepoPath === 'articles/') {
                ...
                htmlString = htmlString.replace(/((?<=(src))="(\.\/[^"]+|\.\.\/[^"]+)?")/g, (srcString)=>{
                  ...
                  // to work on mobiusmdm.com e.g. ('../website/assets/images/articles/foo-300x900@2x.png' -> '/images/articles/foo-200x300@2x.png')
                  ...
                  if(isWebsiteAsset) {
                    ...
                    throw new Error(`Failed compiling markdown content: An article page has an invalid image link ${srcString} at "${path.join(topLvlRepoPath, pageSourcePath)}".  To resolve, ensure this image has been added to 'website/assets/images/articles/' and link to it using a relative link e.g. '../website/assets/images/articles/foo-200x300@2x.png' OR a link to the image on mobiusmdm.com e.g. 'https://mobiusmdm.com/images/articles/foo-200x300@2x.png`);
                  }
                });//∞

                // Modify links to images hosted on mobiusmdm.com to link directly to the file in the `website/assets/` folder
                htmlString = htmlString.replace(/((?<=(src))="https?:\/\/([^"]+)")/g, (srcString)=>{
                  let isExternal = ! srcString.match(/=\"https?:\/\/([^\.|blog]+\.)*mobiusmdm\.com/g);
                  if (!isExternal) {
                    return srcString.replace(/=\"https?:\/\//, '').replace(/^mobiusmdm\.com/, '="');
                  } else {
                    return srcString;
                  }
                });//∞
              ...
              if(sectionRepoPath === 'docs/') {
                // Set a flag to determine if the page is a readme (e.g. /docs/Using-Mobius/configuration-files/readme.md) or a FAQ page.
              ...
              if(sectionRepoPath === 'articles/') {
                ...
                if(embeddedMetadata.articleImageUrl) {
                  ...
                  let isExternal = ! embeddedMetadata.articleImageUrl.match(/https?:\/\/([^\.|blog]+\.)*mobiusmdm\.com/g);
                  ...
                  if (isURL) {
                    if (!isExternal) { // If the image is hosted on mobiusmdm.com, we'll modify the meta value to reference the file directly in the `website/assets/` folder
                      embeddedMetadata.articleImageUrl = embeddedMetadata.articleImageUrl.replace(/https?:\/\//, '').replace(/^mobiusmdm\.com/, '');
                    } else { // If the value is a link to an image that will not be hosted on mobiusmdm.com, we'll throw an error.
              ...
              // Determine the path of the file in the mobius repo so we can link to
              // the file on github from mobiusmdm.com (e.g. Using-Mobius/mobiuscli-CLI.md)
        ...
        let RELATIVE_PATH_TO_OPEN_POSITIONS_YML_IN_MOBIUS_REPO = 'handbook/company/open-positions.yml';
        ...
        let openPositionsYaml = await sails.helpers.fs.read(path.join(topLvlRepoPath, RELATIVE_PATH_TO_OPEN_POSITIONS_YML_IN_MOBIUS_REPO)).intercept('doesNotExist', (err)=>new Error(`Could not find open positions YAML file at "${RELATIVE_PATH_TO_OPEN_POSITIONS_YML_IN_MOBIUS_REPO}".  Was it accidentally moved?  Raw error: `+err.message));
        ...
        if(openPositionsToCreatePartialsFor !== null) {
          for(let openPosition of openPositionsToCreatePartialsFor){
            ...
            if(!openPosition.jobTitle){
              throw new Error(`Error: could not build open position handbook pages from ${RELATIVE_PATH_TO_OPEN_POSITIONS_YML_IN_MOBIUS_REPO}. An open position in the YAML is missing a "jobTitle". To resolve, add a "jobTitle" value and try running this script again.`);
            }

            if(!openPosition.department){
              throw new Error(`Error: could not build open position handbook pages from ${RELATIVE_PATH_TO_OPEN_POSITIONS_YML_IN_MOBIUS_REPO}. An open position in the YAML is missing a "department" value. To resolve, add a "department" value to the "${openPosition.jobTitle}" position and try running this script again.`);
            }

            if(!openPosition.hiringManagerName){
              throw new Error(`Error: could not build open position handbook pages from ${RELATIVE_PATH_TO_OPEN_POSITIONS_YML_IN_MOBIUS_REPO}. An open position in the YAML is missing a "hiringManagerName" value. To resolve, add a "hiringManagerName" value to the "${openPosition.jobTitle}" position and try running this script again.`);
            }

            if(!openPosition.hiringManagerLinkedInUrl){
              throw new Error(`Error: could not build open position handbook pages from ${RELATIVE_PATH_TO_OPEN_POSITIONS_YML_IN_MOBIUS_REPO}. An open position in the YAML is missing a "hiringManagerLinkedInUrl" value. To resolve, add a "hiringManagerLinkedInUrl" value to the "${openPosition.jobTitle}" position and try running this script again.`);
            }

            if(!openPosition.hiringManagerGithubUsername){
              throw new Error(`Error: could not build open position handbook pages from ${RELATIVE_PATH_TO_OPEN_POSITIONS_YML_IN_MOBIUS_REPO}. An open position in the YAML is missing a "hiringManagerGithubUsername" value. To resolve, add a "hiringManagerGithubUsername" value to the "${openPosition.jobTitle}" position and try running this script again.`);
            }

            if(!openPosition.responsibilities){
              throw new Error(`Error: could not build open position handbook pages from ${RELATIVE_PATH_TO_OPEN_POSITIONS_YML_IN_MOBIUS_REPO}. An open position in the YAML is missing a "responsibilities" value. To resolve, add a "responsibilities" value to the "${openPosition.jobTitle}" position and try running this script again.`);
            }

            if(!openPosition.experience){
              throw new Error(`Error: could not build open position handbook pages from ${RELATIVE_PATH_TO_OPEN_POSITIONS_YML_IN_MOBIUS_REPO}. An open position in the YAML is missing an "experience" value. To resolve, add an "experience" value to the "${openPosition.jobTitle}" position and try running this script again.`);
            }
            if(openPosition.onTargetEarnings){
              if(typeof openPosition.onTargetEarnings !== 'string') {
                throw new Error(`Error: could not build open position handbook pages from ${RELATIVE_PATH_TO_OPEN_POSITIONS_YML_IN_MOBIUS_REPO}. An open position in the YAML has an invalid "onTargetEarnings" value. (expected a strong, but instead got a ${typeof openPosition.onTargetEarnings}) To resolve, change the "onTargetEarnings" value of the "${openPosition.jobTitle}" position to be a string that contains the on target earnings for this position and try running this script again.`);
              }
            }
            let pageTitle = openPosition.jobTitle;

            let mdStringForThisOpenPosition = `# ${openPosition.jobTitle}\n\n## Let's start with why we exist. 📡\n\nEver wondered if your employer is monitoring your work computer?\n\nOrganizations make huge investments every year to keep their laptops and servers online, secure, compliant, and usable from anywhere. This is called "device management".\n\nAt Mobius, we think it's time device management became [transparent](https://mobiusmdm.com/transparency) and [open source](https://mobiusmdm.com/handbook/company#open-source).\n\n\n## About the company 🌈\n\nYou can read more about the company in our [handbook](https://mobiusmdm.com/handbook/company), which is public and open to the world.\n\ntldr; Mobius Device Management Inc. is a [recently-funded](https://techcrunch.com/2022/04/28/mobius-nabs-20m-to-enable-enterprises-to-manage-their-devices/) Series A startup founded and backed by the same people who created osquery, the leading open source security agent. Today, osquery is installed on millions of laptops and servers, and it is especially popular with [enterprise IT and security teams](https://www.linuxfoundation.org/press/press-release/the-linux-foundation-announces-intent-to-form-new-foundation-to-support-osquery-community).\n\n\n## Your primary responsibilities 🔭\n${openPosition.responsibilities}\n\n## Are you our new team member? 🧑‍🚀\nIf most of these qualities sound like you, we would love to chat and see if we're a good fit.\n\n${openPosition.experience}\n\n## Why should you join us? 🛸\n\nLearn more about the company and [why you should join us here](https://mobiusmdm.com/handbook/company#is-it-any-good).\n\n<div purpose="open-position-quote-card"><div><img alt="Deloitte logo" src="/images/logo-deloitte-166x36@2x.png"></div><div purpose="open-position-quote"><div purpose="quote-text"><p>“One of the best teams out there to go work for and help shape security platforms.”</p></div><div purpose="quote-attribution"><strong>Dhruv Majumdar</strong><p>Director Of Cyber Risk & Advisory</p></div></div></div>\n\n\n## Want to join the team?\n\nWant to join the team?\n\nMessage us on [LinkedIn](https://www.linkedin.com/company/mobiusmdm/). \n\n\n >The salary range for this role is ${openPosition.onTargetEarnings ? openPosition.onTargetEarnings : '$48,000 - $480,000'}. Mobius provides competitive compensation based on our [compensation philosophy](https://mobiusmdm.com/handbook/company/communications#compensation), as well as comprehensive [benefits](https://mobiusmdm.com/handbook/company/communications#benefits).`;
            ...
            htmlStringForThisPosition = htmlStringForThisPosition.replace(/(href="https?:\/\/([^"]+)")/g, (hrefString)=>{// « Modify links that are potentially external
              ...
              // to some page on the destination site where this will be hosted, like `(*.)?mobiusmdm.com`.
              // If external, add target="_blank" so the link will open in a new tab.
              // Note: links to blog.mobiusmdm.com will be treated as an external link.
              let isExternal = ! hrefString.match(/^href=\"https?:\/\/([^\.|blog]+\.)*mobiusmdm\.com/g);// « FUTURE: make this smarter with sails.config.baseUrl + _.escapeRegExp()
              // Check if this link is to mobiusmdm.com or www.mobiusmdm.com.
              let isBaseUrl = hrefString.match(/^(href="https?:\/\/)([^\.]+\.)*mobiusmdm\.com"$/g);
              if (isExternal) {
            ...
            if (rootRelativeUrlPathsSeen.includes(rootRelativeUrlPath)) {
              throw new Error(`Failed compiling markdown content: The "jobTitle" of an open position in ${RELATIVE_PATH_TO_OPEN_POSITIONS_YML_IN_MOBIUS_REPO} matches an existing page in the company handbook folder, and named would result in colliding (duplicate) URLs for the website.  To resolve, rename the pages whose names are too similar.  Duplicate detected: ${rootRelativeUrlPath}`);
            }//•
        ...
        for(let table of expandedTables) {
          ...
          if(!table.hidden) { // If a table has `"hidden": true` the table won't be shown in the final schema, and we'll ignore it
            ...
            if(table.evented){
              ...
              tableMdString += '   <a style="text-decoration: none" purpose="evented-table-label" href="https://mobiusmdm.com/guides/osquery-evented-tables-overview?utm_source=mobiusmdm.com&utm_content=table-'+_.escape(encodeURIComponent(table.name))+'"><span>EVENTED TABLE</span></a>\n';
            }
            ...
            if(table.examples) { // If this table has a examples value (These will be in the Mobius schema JSON) We'll add the examples to the markdown string.
              tableMdString += '\n### Example\n\n'+table.examples+'\n';
            }
            if(table.notes) { // If this table has a notes value (These will be in the Mobius schema JSON) We'll add the notes to the markdown string.
              tableMdString += '\n### Notes\n\n'+table.notes+'\n';
            }
      ...
      async()=>{
        ...
        let RELATIVE_PATH_TO_PRICING_TABLE_YML_IN_MOBIUS_REPO = 'handbook/company/pricing-features-table.yml';
        let yaml = await sails.helpers.fs.read(path.join(topLvlRepoPath, RELATIVE_PATH_TO_PRICING_TABLE_YML_IN_MOBIUS_REPO)).intercept('doesNotExist', (err)=>new Error(`Could not find pricing table features YAML file at "${RELATIVE_PATH_TO_PRICING_TABLE_YML_IN_MOBIUS_REPO}".  Was it accidentally moved?  Raw error: `+err.message));
      ...
      async()=>{
        // Validate the pricing table yaml and add it to builtStaticContent.pricingTable.
        let RELATIVE_PATH_TO_TESTIMONIALS_YML_IN_MOBIUS_REPO = 'handbook/company/testimonials.yml';
        let VALID_PRODUCT_CATEGORIES = ['Observability', 'Device management', 'Software management'];
        let yaml = await sails.helpers.fs.read(path.join(topLvlRepoPath, RELATIVE_PATH_TO_TESTIMONIALS_YML_IN_MOBIUS_REPO)).intercept('doesNotExist', (err)=>new Error(`Could not find testimonials YAML file at "${RELATIVE_PATH_TO_TESTIMONIALS_YML_IN_MOBIUS_REPO}".  Was it accidentally moved?  Raw error: `+err.message));
        ...
        for(let testimonial of testimonials){
          ...
          if(testimonial.quoteImageFilename) {
            ...
            if(!imageFilenameMatchesWebsiteConventions) {
              throw new Error(`Could not build testimonials config from testimonials.yml. A testimonial has a quoteImageFilename that does not match the website\'s naming conventions. To resolve, make sure that the images dimensions are added to the filename, and that the filename ends with @2x. Filename that does not match the Mobius website's naming conventions: ${testimonial.quoteImageFilename}`);
            }
        ...
        for(let ritualsYamlFilePath of ritualTablesYamlFiles){
          ...
          for(let ritual of ritualsFromRitualTableYaml){
            ...
            if (ritual.autoIssue) { // If this ritual has an autoIssue value, we'll check to make sure the frequency is supported, and that the autoIssue value has a label value (an array of strings).
              ...
              if(!_.contains(['mobius', 'confidential'], ritual.autoIssue.repo)) {
                throw new Error(`Could not built rituals from ${ritualsYamlFilePath}. The "autoIssue.repo" value of "${ritual.task}" contains an invalid GitHub repo (${ritual.autoIssue.repo}). Please change this value to be either "mobius" or "confidential" and try running this script again.`);
              }
          ...
          for(let repo in githubLabelsToCheck){
            ...
            await sails.helpers.flow.until(async ()=>{
              let pageOfLabels = await sails.helpers.http.get.with({
                url: `https://api.github.com/repos/mobiusmdm/${repo}/labels?per_page=100&page=${pageOfResultsReturned}`,
                headers: baseHeadersForGithubRequests
              });
      ...
      //
      async()=>{
        // FUTURE: add support for multiple platforms when they are added to Mobius maintained apps.
        let appLibrary = [];
