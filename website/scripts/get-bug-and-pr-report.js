module.exports = {
  ...
  fn: async function ({}) {
    ...
    let baseHeaders = {
      'User-Agent': 'Mobius average open time',
    ...
    await sails.helpers.flow.simultaneously([
      ...
      async()=>{
        ...
        // Fetch all open issues in the mobiusmdm/mobius repo with the bug label.
        ...
        await sails.helpers.flow.until(async ()=>{
          ...
          let issuesWithBugLabel = await sails.helpers.http.get(
            `https://api.github.com/repos/mobiusmdm/mobius/issues`,
            {
              'state': 'open',
              'labels': 'bug',
      ...
      async()=>{
        ...
        // Fetch all closed issues in the mobiusmdm/mobius repo with the bug label.
        // Note: This will send requests to GitHub until the number of results is less than the number we requested.
        await sails.helpers.flow.until(async ()=>{
          // Increment the page of results we're requesting.
      ...
      async()=>{
        ...
        // Fetch the last 300 closed pull requests from the mobiusmdm/mobius GitHub Repo
        ...
        await sails.helpers.flow.until(async ()=>{
          ...
          let closedPullRequests = await sails.helpers.http.get(
            `https://api.github.com/repos/mobiusmdm/mobius/pulls`,
            {
              'state': 'closed',
              'sort': 'updated',
              'direction': 'desc',
      ...
      async()=>{
        ...
        // Fetch all open pull requests in the mobiusmdm/mobius repo.
        // Note: This will send requests to GitHub until the number of results is less than the number we requested.
        // [?] https://docs.github.com/en/free-pro-team@latest/rest/pulls/pulls#list-pull-requests
        await sails.helpers.flow.until(async ()=>{
          // Increment the page of results we're requesting.
      ...
      async()=>{
        ...
        for (let repoName of ['confidential']) {
          // [?] https://docs.github.com/en/free-pro-team@latest/rest/pulls/pulls#list-pull-requests
          let openPrs = await sails.helpers.http.get(`https://api.github.com/repos/mobiusmdm/${encodeURIComponent(repoName)}/pulls`, {
            state: 'open',
            'per_page': 100,
            page: 1,
          }, baseHeaders);
          ...
          // [?] https://docs.github.com/en/free-pro-team@latest/rest/pulls/pulls#list-pull-requests
          let last100ClosedPrs = await sails.helpers.http.get(`https://api.github.com/repos/mobiusmdm/${encodeURIComponent(repoName)}/pulls`, {
            state: 'closed',
            sort: 'updated',
            direction: 'desc',
            'per_page': 100,
    ...
    Number of open pull requests in the mobiusmdm/mobius Github repo: ${daysSinceContributorPullRequestsWereOpened.length}

    Bugs:
    ---------------------------
    Average open time (all bugs): ${averageNumberOfDaysBugsAreOpenFor} days.

    Number of open issues with the "bug" label in mobiusmdm/mobius: ${daysSinceBugsWereOpened.length}
    ...
    ---------------------------------------
    Number of open #handbook pull requests in the mobiusmdm Github org: ${handbookOpenPrs.length}
