const userAgent = 'Mobius/Vanta Updater';
...
module.exports = {
  ...
  description: 'An hourly script that gathers and formats host and user data from Mobius instances sends it to Vanta on behalf of a Vanta user.',
  ...
  fn: async function () {
    ...
    sails.log('Syncing Mobius instance data with Vanta for '+allActiveVantaConnections.length+(allActiveVantaConnections.length > 1 ? ' connections.' : ' connection.'));
    ...
    await sails.helpers.flow.simultaneouslyForEach(allActiveVantaConnections, async (vantaConnection)=>{
      ...
      //  ╠╣ │  ├┤ ├┤  │   │ │└─┐├┤ ├┬┘└─┐
      //  ╚  ┴─┘└─┘└─┘ ┴   └─┘└─┘└─┘┴└─└─┘
      // Request user data from the Mobius instance to send to Vanta.
      ...
      try {
        responseFromUserEndpoint = await sails.helpers.http.get(
          updatedRecord.mobiusInstanceUrl + '/api/v1/mobius/users',
          {},
          {'Authorization': 'Bearer '+updatedRecord.mobiusApiKey, 'User-Agent': userAgent }
        )
        .retry()
        .tolerate((err)=>{// If an error occurs while sending a request to the Mobius instance, we'll add the error to the errorReportById object, with this connections ID set as the key.
          errorReportById[connectionIdAsString] = new Error(`When sending a request to the /users endpoint of a Mobius instance for a VantaConnection (id: ${connectionIdAsString}), the Mobius instance returned an Error: ${util.inspect(err.raw)}`);
        });
      } catch(error) {
        errorReportById[connectionIdAsString] = new Error(`When sending a request to the /users endpoint of a Mobius instance for a VantaConnection (id: ${connectionIdAsString}), the Mobius instance returned an Error: ${util.inspect(error.raw)}`);
      }
      ...
      for(let user of responseFromUserEndpoint.users) {
        ...
        // Create a user object for this Mobius user.
        let userToSyncWithVanta = {
          ...
          status: 'ACTIVE',// Always set to 'ACTIVE', if a user is removed from the Mobius instance, it will not be included in the response from the Mobius instance's /users endpoint.
          mfaEnabled,
          mfaMethods,
          externalUrl: vantaConnection.mobiusInstanceUrl,// Setting externalUrl (Required by Vanta) for all users to be the Mobius instance url.
          authMethod,
          permissionLevel,
        };
      ...
      //  │ ┬├┤  │   ├─┤│ │└─┐ │ └─┐
      //  └─┘└─┘ ┴   ┴ ┴└─┘└─┘ ┴ └─┘
      // Get all hosts on the Mobius instance.
      let pageNumberForPossiblePaginatedResults = 0;
      let numberOfHostsPerRequest = 100;
      let allHostsOnThisMobiusInstance = [];

      // Start sending requests to the Mobius instance's /hosts endpoint until we have all hosts.
      await sails.helpers.flow.until(async ()=>{
        let getHostsResponse = await sails.helpers.http.get(
          `${updatedRecord.mobiusInstanceUrl}/api/v1/mobius/hosts?per_page=${numberOfHostsPerRequest}&page=${pageNumberForPossiblePaginatedResults}`,
          {},
          {'Authorization': 'bearer '+updatedRecord.mobiusApiKey, 'User-Agent': userAgent},
        )
        .retry();
        // Add the results to the allHostsOnThisMobiusInstance array.
        allHostsOnThisMobiusInstance = allHostsOnThisMobiusInstance.concat(getHostsResponse.hosts);
      ...
      .tolerate(()=>{// If an error occurs while sending a request to the Mobius instance, we'll add the error to the errorReportById object, with this connections ID set as the key.
        errorReportById[connectionIdAsString] = new Error(`When requesting all hosts from a Mobius instance for a VantaConnection (id: ${connectionIdAsString}), the Mobius instance did not respond with all of it's hosts in the set amount of time.`);
      });
      ...
      // If this is Mobius's Vanta connection, exclude hosts on the "Compliance exclusions" team.
      // See https://github.com/notawar/mobius/issues/19312 for more information.
      if(vantaConnection.id === 64){
        allHostsOnThisMobiusInstance = allHostsOnThisMobiusInstance.filter((host)=>{
          return [275, 274].includes(host.team_id);
        });
      }

      let macOsHosts = allHostsOnThisMobiusInstance.filter((host)=>{
        return host.platform === 'darwin';
      });

      let windowsHosts = allHostsOnThisMobiusInstance.filter((host)=>{
      ...
      await sails.helpers.flow.simultaneouslyForEach(macOsHosts, async (host) => {
        ...
        // Start building the host resource to send to Vanta, using information we get from the Mobius instance's get Hosts endpoint
        let macOsHostToSyncWithVanta = {
          displayName: host.display_name,
          uniqueId: hostIdAsString,
          externalUrl: updatedRecord.mobiusInstanceUrl + '/hosts/'+encodeURIComponent(hostIdAsString),
        ...
        if(host.mdm !== undefined && host.mdm.enrollment_status !== null) {
          ...
          // More info about pending hosts: https://github.com/notawar/mobius/blob/3cc7c971c2c24e28d06323c329475ae32e9a8198/docs/Using-Mobius/MDM-setup.md#pending-hosts
        ...
        try {
          detailedInformationAboutThisHost = await sails.helpers.http.get(
            updatedRecord.mobiusInstanceUrl + '/api/v1/mobius/hosts/'+encodeURIComponent(hostIdAsString),
            {},
            {'Authorization': 'bearer '+updatedRecord.mobiusApiKey, 'User-Agent': userAgent}
          )
          .retry();
        } catch(error) {
          hostApiErrorReportById[connectionIdAsString].push(new Error(`When sending a request to the Mobius instance's /hosts/${host.id} endpoint for a Vanta connection (id: ${connectionIdAsString}), an error occurred: ${util.inspect(error.raw)}`));
        }
        ...
        if (softwareList) {
          for (let software of softwareList) {
            ...
            if(software.source === 'firefox_addons' || software.source === 'chrome_extensions') {
              ...
              if(software.extension_id !== undefined && software.extension_id !== null) {// If the Mobius instance reported an extension_id for the extension, we'll use that value.
        ...
        if (softwareList) {
          for (let software of softwareList) {
            ...
            if (software.source === 'firefox_addons' || software.source === 'chrome_extensions') {
              softwareToAdd.browser = software.source.toUpperCase().split('_')[0];// Get the uppercased first word of the software source, this will either be CHROME or FIREFOX.
              softwareToAdd.extensionId = software.name + ' ' + software.version;// Set the extensionId to be the software's name and the software version.
              if(software.extension_id !== undefined && software.extension_id !== null) {// If the Mobius instance reported an extension_id for this extension, we'll use that value.
                softwareToAdd.extensionId = software.extension_id;
              }
